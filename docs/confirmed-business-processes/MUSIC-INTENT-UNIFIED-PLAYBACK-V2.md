...
`MusicIntentDecisionV2`保存动作、策略、搜索主题、歌手、情绪、曲风、来源模式、置信度和签名语义回执。来源模式取值为`auto | local | netease | bilibili | douyin`，由调用方授权确定，模型不能把本地请求扩大到网易、B站或抖音。抖音候选以`awemeId`作为稳定来源身份，助手回复、下载任务和真实播放引用同一个播放决定checksum。
