...
项目 Agent 行内的实时过程按以下来源优先级合并，而不是把 CLI 控制台原样搬到页面：

```text
Agent 主动 report_progress
→ 已验证的运行时结构化事件
→ 60 秒系统观察兜底
```

运行中可展开查看 Agent 的安全业务说明、工具批次、文件变化和验证状态；同一工具与同一 Agent 始终原位更新。完成后这些内容随对应项目 Agent 折叠到“项目 Agent”阶段，TestAgent 与主 Agent 总结保持独立阶段。普通页面不展示来源类型、运行时版本或事件 checksum；这些信息只在“运行时技术详情”中用于排障。

每个新事件直接绑定目标消息的 `anchorMessageId`，跨会话来源另带 `originMessageId`，所以同一会话并发任务不会再依赖时间窗口猜测对应消息。第三方 Agent 不主动汇报且运行时没有结构化协议时，页面只显示可靠的启动、活性、文件计数和最终结果，不编造实现步骤。
