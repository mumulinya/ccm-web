...
- 需求池、工作台和全局 Agent只选择目标群聊或项目；正式任务到达时按`requirement_pool/workbench/global_agent`来源解析自动化会话绑定，没有绑定则自动创建。任务保存binding revision/checksum和精确会话快照，重试、返工与验收不跟随后续绑定调整。
- 群聊和项目的自动化会话列表可以多选绑定需求池、工作台和全局 Agent来源；同一目标的一个来源只能有一个活动接收会话。普通会话和飞书会话不能成为这些入口的隐式任务目标。
