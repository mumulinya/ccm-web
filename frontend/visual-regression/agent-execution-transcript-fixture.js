...
const base = { schema:'ccm-user-visible-agent-event-v1', scope:'project', scopeId:'demo', exactSessionId:'session-demo', anchorMessageId:'fixture-message', generation:2, attempt:2, contentStored:false, visibility:'default' }
const rows = [
  { ...base, attempt:1, generation:1, eventId:'historical-progress', sequence:.1, eventType:'assistant_progress', taskId:'fixture-task', display:{ title:'项目主 Agent', summary:'首次执行正在核对构建结果。', status:'running' }, createdAt:at(-2), detail:{ progress:{ kind:'verification', text:'首次执行正在核对构建结果。', modelCallIndex:1, relatedToolCallIds:['historical-build'] } } },
  { ...base, attempt:1, generation:1, eventId:'historical-build', sequence:.2, eventType:'tool_failed', taskId:'fixture-task', toolCallId:'historical-build', toolName:'run_command', display:{ title:'运行命令', target:'npm run build', summary:'首次构建未通过', status:'failed', durationMs:1800 }, createdAt:at(-1), detail:{ toolDisplay:{ tool:{ name:'run_command', label:'运行命令', family:'terminal' }, arguments:[{ label:'命令', value:'npm run build' }], result:{ summary:'首次构建未通过' } } } },
  { ...base, attempt:1, generation:1, eventId:'historical-result', sequence:.3, eventType:'result', taskId:'fixture-task', display:{ title:'任务中断', summary:'首次构建未通过，等待恢复', status:'waiting', durationMs:2200 }, createdAt:at(-.5), detail:{ completionSummary:{ schema:'ccm-completion-summary-v1', status:'interrupted', headline:'首次执行已中断', detail:'首次构建未通过，已保留现场', filesChanged:0, verificationPassed:0, verificationFailed:1, blockers:[], source:'query_projection', contentStored:false } } },
  ...
  { ...base, eventId:'parallel-directory-result', sequence:6.1, eventType:'tool_completed', toolCallId:'parallel-directory', parallelGroupId:'parallel-unclaimed', display:{ title:'List directory', target:'src/modules/erp', summary:'发现 6 项', status:'success', durationMs:240 }, createdAt:at(4.05), detail:{ toolDisplay:{ tool:{ name:'list_directory', label:'查找目录', family:'search' }, arguments:{ path:'src/modules/erp' }, result:{ summary:'发现 6 项' } } } },
  { ...base, eventId:'parallel-read-result', sequence:6.2, eventType:'tool_completed', toolCallId:'parallel-read', parallelGroupId:'parallel-unclaimed', display:{ title:'Read files', target:'src/modules/erp', summary:'已读取 5 个文件', status:'success', durationMs:310 }, createdAt:at(4.1), detail:{ toolDisplay:{ tool:{ name:'read_files', label:'批量读取项目文件', family:'read' }, arguments:{ path:'src/modules/erp' }, result:{ summary:'已读取 5 个文件' } } } },
  ...
  { ...base, eventId:'worker-terminal', agentRunId:'agent-run-worker', parallelGroupId:'agent-parallel-1', sequence:9, eventType:'agent_completed', taskId:'fixture-task', workItemId:'worker', display:{ title:'smart-live-ui · Codex', target:'实现后台前端', summary:'CCM 已完成终态验收', status:'success', durationMs:9300 }, createdAt:at(7), detail:{ agentDisplay:{ projectId:'smart-live-ui', projectName:'smart-live-ui', runtimeLabel:'Codex', workItemTitle:'实现后台前端', phase:'completed', attempt:2, isParallel:true }, agentAttemptHistory:[{ attempt:1, status:'failed', summary:'首次构建未通过，已返工修正' }], fileChanges:[{ path:'src/feature.ts' }], evidenceIds:['evidence-worker'] } },
