...
export const getTechnicalDetailSections = (source, fallbackTechnical = null) => {
  ...
  const failureShotRows = Array.isArray(technical.failure_step_screenshot_rows)
    ? technical.failure_step_screenshot_rows
    : Array.isArray(technical.failure_step_screenshots)
      ? technical.failure_step_screenshots.map((item) => {
          const step = item?.stepName || item?.step_name || 'browser-failure'
          const check = item?.checkName || item?.check_name || ''
          const file = String(item?.path || '').replace(/\\/g, '/').split('/').pop() || item?.path || ''
          return `失败步骤 → 截图：${check ? `${check} / ` : ''}${step} → ${file}`
        })
      : []
  if (failureShotRows.length) {
    troubleshooting.push({
      label: '失败步骤 → 截图',
      value: failureShotRows.slice(0, 6).join('；'),
    })
  }
  const envPrep = technical.test_agent_environment_prep || technical.testAgentEnvironmentPrep || null
  if (envPrep?.userSummary || envPrep?.missingEnvNames?.length) {
    troubleshooting.push({
      label: '环境准备清单',
      value: [
        envPrep.userSummary || '',
        Array.isArray(envPrep.missingEnvNames) && envPrep.missingEnvNames.length
          ? `缺环境变量名：${envPrep.missingEnvNames.join('、')}`
          : '',
        envPrep.storageStateHint || '',
      ].filter(Boolean).join('；'),
    })
  }
