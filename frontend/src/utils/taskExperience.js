...
export const taskPhasePresentation = (status, fallback = 'planning') => {
  ...
  const phase = ({
    ...
    reviewing: 'reviewing', testing: 'reviewing', self_verifying: 'reviewing',
  ...
  return {
    phase,
    label: key === 'self_verifying' ? '主 Agent 正在自验' : ({ planning: '正在分析', queued: '准备开始', executing: '正在修改', reviewing: '正在运行测试', reworking: '正在修复问题', needs_user: '需要你确认', blocked: '正在恢复', completed: '已完成', failed: '失败', cancelled: '已取消', reverted: '已安全撤销' })[phase] || '正在处理',
...
export const globalMissionTaskCard = (message = {}) => {
  ...
  const acceptanceDecision = mission.acceptance_decision || mission.acceptanceDecision || null
  const acceptanceReview = mission.acceptance_review || mission.acceptanceReview || (acceptanceDecision ? {
    title: acceptanceDecision.mode === 'risk_based_auto_acceptance' ? '主 Agent 风险验收' : '最终验收',
    status: acceptanceDecision.status === 'approved' ? 'complete' : 'pending',
    headline: acceptanceDecision.status === 'approved'
      ? (acceptanceDecision.mode === 'risk_based_auto_acceptance' ? '低风险任务的源码证据和全部验收门禁已通过，主 Agent 已自动完成最终验收。' : '用户已批准本次 Epic 交付。')
      : `仍需人工批准：${asArray(acceptanceDecision.blockers).join('；') || '当前风险策略不允许自动验收'}`,
    checks: [
      { id: 'delivery_gate', label: '子任务交付门禁', ok: acceptanceDecision.gate_passed === true, detail: acceptanceDecision.gate_passed === true ? '全部工作项已通过' : '仍有工作项未通过' },
      { id: 'source_evidence', label: '源码证据', ok: acceptanceDecision.source_evidence_ready === true, detail: acceptanceDecision.source_evidence_ready === true ? '源码checksum已核验' : '源码证据仍不完整' },
      { id: 'permissions', label: '权限申请', ok: Number(acceptanceDecision.pending_permission_count || 0) === 0, detail: Number(acceptanceDecision.pending_permission_count || 0) === 0 ? '没有待处理权限' : `${acceptanceDecision.pending_permission_count} 项待处理` },
    ],
    next_action: acceptanceDecision.status === 'approved' ? '可以查看交付总结和任务回放。' : '请审阅整批变更后批准交付，或退回指定子任务返工。',
  } : null)
  return {
    version: 1,
    task_id: mission.id,
    ...
    acceptance_state: mission.acceptance_state || mission.acceptanceState || '',
    acceptance_decision: acceptanceDecision,
    acceptanceDecision,
    acceptance_review: acceptanceReview,
    acceptanceReview,
    terminal_state_receipt: mission.terminal_state_receipt || mission.terminalStateReceipt || null,
    terminal_decision: mission.terminal_decision || mission.terminalDecision || null,
    terminal_gate: mission.terminal_gate || mission.terminalGate || null,
    scheduler_state: mission.scheduler_state || mission.schedulerState || null,
...
export const globalAgentRunTaskCard = (message = {}) => {
  ...
  return {
    ...
    title: compact(run.goal || run.original_user_message || run.originalUserMessage || run.user_message || message.content || (resultPresentation === PRESENTATION_PLAN ? '执行前计划' : '全局任务'), 90),
    goal: compact(run.goal || run.original_user_message || run.originalUserMessage || run.user_message || '', 240),
    phase: presentation.phase,
    phase_label: resultPresentation === PRESENTATION_PLAN && presentation.phase === 'completed' ? '计划已就绪' : presentation.label,
    status: run.status,
    usage_summary: {
      model_calls: Number.isFinite(Number(run.model_calls)) ? Number(run.model_calls) : undefined,
      input_tokens: Number.isFinite(Number(run.input_tokens ?? run.usage?.inputTokens)) ? Number(run.input_tokens ?? run.usage?.inputTokens) : undefined,
      output_tokens: Number.isFinite(Number(run.output_tokens ?? run.usage?.outputTokens)) ? Number(run.output_tokens ?? run.usage?.outputTokens) : undefined,
      retry_count: Number.isFinite(Number(run.provider_retry_count ?? run.providerRetryCount)) ? Number(run.provider_retry_count ?? run.providerRetryCount) : undefined,
      test_agent_rounds: Number.isFinite(Number(run.test_agent_rounds ?? run.testAgentRounds)) ? Number(run.test_agent_rounds ?? run.testAgentRounds) : undefined,
    },
...
export const projectExecutionTaskCard = (message = {}, project = '') => {
  ...
  const acceptanceMode = task.acceptance_mode || task.acceptanceMode || (task.test_agent_enabled === false ? 'main_agent_self_verification' : 'test_agent')
  const mainAgentSelfVerification = task.main_agent_self_verification || task.mainAgentSelfVerification || null
  const selfVerificationMode = acceptanceMode === 'main_agent_self_verification'
  const selfVerificationActive = acceptanceState === 'main_agent_self_verifying'
  const projectMainActive = ['planning', 'main_agent_accepting', 'main_agent_self_verifying'].includes(acceptanceState) || ['planning', 'reviewing'].includes(presentation.phase)
  const testAgentActive = ['test_agent_running', 'test_agent_recheck'].includes(acceptanceState)
  const workerActive = ['executing', 'reworking'].includes(acceptanceState) || presentation.phase === 'executing'
  const testAgentSummary = testAgent?.canAccept === true
    ? '独立验收通过'
    : acceptanceState === 'test_agent_recheck'
      ? '正在按失败范围增量复验'
      : acceptanceState === 'environment_blocked'
        ? '验收环境或登录条件需要处理'
        : acceptanceState === 'needs_user'
          ? '验收需要用户确认'
          : acceptanceState === 'reworking'
            ? '真实失败证据已交给开发 Agent返工'
            : testAgentActive
              ? '正在读取源码与真实证据'
              : testAgent
                ? '验收尚未通过'
                : '等待开发结果'
  const selfVerificationSummary = mainAgentSelfVerification?.canAccept === true
    ? '主 Agent 自验通过，未经过独立 TestAgent'
    : selfVerificationActive
      ? '正在核对开发结果和真实验证证据'
      : mainAgentSelfVerification
        ? '主 Agent 自验未通过'
        : '等待开发结果'
  ...
  return {
    ...
    message_id: task.message_id || task.messageId || (taskId ? `project-main-task:${taskId}` : ''),
    ...
    queue_position: Number(task.queue_position || task.queuePosition || task.scheduler_state?.position || 0),
    queue_state: task.queue_state || task.queueState || task.scheduler_state?.state || '',
    scheduler_state: task.scheduler_state || task.schedulerState || null,
    workspace_lane: task.workspace_lane || task.workspaceLane || task.scheduler_state?.workspace_lane || '',
    terminal_decision: task.terminal_decision || task.terminalDecision || null,
    terminal_gate: task.terminal_gate || task.terminalGate || null,
    acceptance_mode: acceptanceMode,
    acceptanceMode,
    test_agent_enabled: !selfVerificationMode,
    testAgentEnabled: !selfVerificationMode,
    main_agent_self_verification: mainAgentSelfVerification,
    mainAgentSelfVerification,
    plan_revision_count: Number(task.plan_revision_count || task.planRevisionCount || asArray(task.plan_revisions || task.planRevisions).length || 0),
    plan_revisions: asArray(task.plan_revisions || task.planRevisions),
    ...
    active_agents: [
      ...
      ...(selfVerificationActive ? ['项目主 Agent 正在自验'] : []),
    ],
    agents: [
      ...
      selfVerificationMode
        ? { name: '主 Agent 自验', status: selfVerificationActive ? 'running' : mainAgentSelfVerification?.canAccept === true ? 'done' : mainAgentSelfVerification ? 'failed' : 'waiting', summary: selfVerificationSummary }
        : { name: 'TestAgent', status: testAgentActive ? 'running' : testAgent?.canAccept === true ? 'done' : testAgent ? 'failed' : 'waiting', summary: testAgentSummary },
    ],
    change_summary: projectChangeSummary,
    changeSummary: projectChangeSummary,
    plan_mode: projectPlanMode,
    planMode: projectPlanMode,
    source_evidence: task.source_evidence || task.sourceEvidence || null,
    sourceEvidence: task.source_evidence || task.sourceEvidence || null,
    ...
    technical: taskId ? { trace_id: task.trace_id || message.projectRun?.trace_id || '', execution_ids: task.execution_ids || [], session_ids: task.session_ids || [], run_id: message.projectRun?.id || taskId, parent_run_id: message.projectRun?.parent_run_id || task.parent_run_id || '', source_ingestion: task.source_ingestion || task.sourceIngestion || null, requirement_extraction: task.requirement_extraction || task.requirementExtraction || null, plan_alignment: projectPlanAlignment, user_handoff: projectUserHandoff, scheduler_state: task.scheduler_state || task.schedulerState || null, terminal_decision: task.terminal_decision || task.terminalDecision || null } : null,
  }
}
