...
const sourceIngestion = computed(() => preview.value?.source_ingestion || preview.value?.intake?.source_ingestion || null)
const sourceCoverage = computed(() => sourceIngestion.value?.coverage_receipt || null)
const sourceGatePass = computed(() => !sourceCoverage.value || sourceCoverage.value.complete === true)
...
const confirmAndQueue = async () => {
  ...
  if (!sourceGatePass.value) return toast.warning('必需资料尚未完整读取，不能开始自动执行')
  ...
  <div class="modal-overlay automated-intake-overlay" @click.self="emit('close')" @paste.capture="onPaste">
    <section class="automated-intake-modal" role="dialog" aria-modal="true" aria-labelledby="automated-intake-title">
      ...
      <footer>
        ...
        <button class="primary" :disabled="busy || (preview && (!planValidation.pass || !sourceGatePass))" @click="preview ? confirmAndQueue() : createPreview()"><Sparkles :size="16" />{{ busy ? '处理中' : preview ? '确认并自动执行' : '让模型整理任务' }}</button>
      </footer>
    </section>
  </div>
</template>
...
.target-grid select,.field input,.field textarea{box-sizing:border-box;border-radius:var(--radius-md,6px);background:var(--control-bg)}.target-grid select,.field input{height:var(--control-height-lg,36px);padding:0 var(--control-padding-x,10px)}.field textarea{min-height:84px;padding:9px var(--control-padding-x,10px);line-height:1.5}.target-grid select:focus,.field input:focus,.field textarea:focus{box-shadow:var(--focus-ring)}
