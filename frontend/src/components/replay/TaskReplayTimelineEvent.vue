...
import { ChevronDown, ChevronRight, FileDiff } from '@lucide/vue'
...
import InlineAgentDiff from '../common/InlineAgentDiff.vue'
import ReplayToolDetail from './ReplayToolDetail.vue'

const props = defineProps({ item: { type: Object, required: true }, focused: Boolean, showRawGroups: Boolean, variant: { type: String, default: 'virtual' } })
...
const hasDetails = computed(() => summary.value.length > 220 || props.item.tool_display || props.item.file_changes?.length || props.item.evidence_ids?.length || props.item.group_count > 1 || props.item.replay_link?.anchorMessageId || (props.item.technical && Object.keys(props.item.technical).length))
...
const sourceEvent = computed(() => {
  const link = props.item.replay_link || {}
  return {
    eventId: props.item.id,
    scope: link.scope,
    scopeId: link.scopeId,
    exactSessionId: link.exactSessionId,
    taskId: link.taskId || props.item.task_id,
    generation: link.generation,
    workItemId: link.workItemId,
    detail: { fileChanges: props.item.file_changes || [], agentDisplay: { projectId: props.item.project, projectName: props.item.project, attempt: link.attempt || 1 } },
  }
})
const toggleDiff = file => {
...
const fileStats = file => Number(file?.additions || 0) || Number(file?.deletions || 0) ? `+${Number(file?.additions || 0)} -${Number(file?.deletions || 0)}` : ''
</script>

<template>
  <article :id="`replay-event-${item.id}`" :class="['virtual-event-card', `variant-${variant}`, item.status, { focused }]">
    ...
    <div v-if="open" class="virtual-details">
      <ReplayToolDetail v-if="item.tool_display" :item="item" />
      <section v-if="item.file_changes?.length" class="replay-file-changes" aria-label="项目子 Agent文件变化">
        <article v-for="file in item.file_changes" :key="file.path">
          <button type="button" :aria-expanded="expandedDiffs.has(file.path)" @click="toggleDiff(file)">
            <FileDiff :size="13" /><strong>{{ file.path }}</strong><small>{{ fileStats(file) || file.status || '文件已变化' }}</small>
            <ChevronDown v-if="expandedDiffs.has(file.path)" :size="13" /><ChevronRight v-else :size="13" />
          </button>
          <InlineAgentDiff v-if="expandedDiffs.has(file.path)" :event="sourceEvent" :file="file" :show-full-action="false" />
        </article>
      </section>
...
.virtual-event-card.variant-static{margin:4px 0 10px}
.replay-file-changes{display:grid;gap:2px;margin:0 0 8px}.replay-file-changes>article{min-width:0}.replay-file-changes>article>button{width:100%;display:grid;grid-template-columns:18px minmax(0,1fr) auto 16px;align-items:center;gap:6px;padding:6px 2px;border:0;background:transparent;color:var(--text-secondary);text-align:left;cursor:pointer}.replay-file-changes>article>button strong{overflow:hidden;font:600 10px/1.4 ui-monospace,monospace;text-overflow:ellipsis;white-space:nowrap}.replay-file-changes>article>button small{color:var(--text-muted);font-size:9px}.replay-file-changes :deep(.inline-agent-diff){margin-left:18px}
</style>
