...
const props = defineProps({
  ...
  interruptedAttemptCount: { type: Number, default: 0 },
})
...
<template>
  <section class="replay-executive" :class="outcome.status || 'running'">
    ...
    <dl class="outcome-facts">
      ...
      <div><dt>正式交付文件</dt><dd>{{ fileCount }}</dd></div>
      <div><dt>执行次数</dt><dd>{{ attemptCount || 1 }}<small v-if="interruptedAttemptCount"> · 中断/失败 {{ interruptedAttemptCount }}</small></dd></div>
