...
<template>
  <section class="task-experience-card" :class="[`context-${context}`, `phase-${card.phase}`]">
    ...
    <div v-if="agentCoordination" class="task-card-section agent-coordination" :class="agentCoordination.status">
      <div class="section-head">
        <label>{{ agentCoordination.title || '主 Agent ↔ 子 Agent 协作' }}</label>
      ...
      <div class="coord-grid">
        ...
        <div v-if="agentCoordination.contract_transfer?.rows?.length">
          ...
          <small v-for="row in (agentCoordination.contract_injection_gate?.rows?.length ? agentCoordination.contract_injection_gate.rows : agentCoordination.contract_transfer.rows).slice(0, 4)" :key="row.id">{{ row.target }}：{{ row.endpoint || row.type }} · {{ contractInjectionLabel(row.injected ? 'injected' : row.status) }}</small>
        </div>
      </div>
    ...
    <details v-if="card.technical" class="task-card-technical">
      ...
      <div v-if="card.technical.parent_run_id"><span>上轮</span><code>{{ card.technical.parent_run_id }}</code></div>
