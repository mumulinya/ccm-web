      ...
      <article v-for="doc in documents" :key="doc.name" class="document-row" :data-status="doc.parseStatus">
        <button class="document-main" type="button" @click="emit('preview', doc.name)">
          ...
          <span class="document-copy">
            <span class="name-line"><strong :title="doc.name">{{ doc.name }}</strong><span v-if="doc.parseStatus === 'failed'" class="status failed">解析失败</span><span v-else-if="doc.parseStatus === 'partial'" class="status partial">部分解析</span><span v-if="doc.duplicateOf" class="status duplicate" :title="`内容与 ${doc.duplicateOf} 相同`">重复</span></span>
...
.status.duplicate { background:var(--panel-muted); color:var(--text-secondary, #64748b); }
</style>
