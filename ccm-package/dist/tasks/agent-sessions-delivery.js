...
function recordTaskAgentMemoryContextDelivery(sessionId, input = {}) {
    ...
    return (0, agent_sessions_shared_1.withTaskAgentSessionStoreLock)(() => {
        ...
        const snapshotPromptChecksum = snapshotPrompt ? (0, agent_sessions_shared_1.hashValue)(snapshotPrompt) : "";
        ...
        const receiptId = `tamdr_${(0, agent_sessions_shared_1.hashValue)([id, snapshotId, input.executionId || "", input.attempt || 0, deliveredAt].join("\0"), 20)}`;
        const receiptFile = path.join((0, agent_sessions_shared_1.getMemoryContextSnapshotDir)(id), `${snapshotId}.${receiptId}.delivery.json`);
        const groupSessionMemoryBinding = snapshot.context?.group_session_memory_binding || (0, agent_sessions_shared_1.extractGroupSessionMemoryBinding)(snapshot.context?.memory_context || {});
        ...
        const continuationBaseline = (0, agent_sessions_shared_1.verifyTaskAgentMemoryContinuationBaselineDelivery)(snapshot, input);
        ...
        const fileChangeRows = (Array.isArray(input.fileChanges?.files)
            ...
            diffChecksum: item?.diff ? (0, agent_sessions_shared_1.hashValue)(item.diff, 32) : "",
        }))
            .filter((item) => item.path)
            .slice(0, 80);
        const fileChangeChecksum = fileChangeRows.length ? (0, agent_sessions_shared_1.hashValue)(fileChangeRows, 64) : "";
        const outputChecksum = input.output ? (0, agent_sessions_shared_1.hashValue)(String(input.output)) : "";
        ...
        const providerContextUsageBaselineAdmissionIssues = [];
        if (providerContextUsageReported) {
            if (!providerModel.trim())
                providerContextUsageBaselineAdmissionIssues.push("provider_model_missing");
            if (!delivered)
                providerContextUsageBaselineAdmissionIssues.push("delivery_prompt_not_bound");
            if (!memoryDeliveryEvidenceReady)
                providerContextUsageBaselineAdmissionIssues.push("memory_delivery_evidence_not_ready");
            if (input.executionSucceeded === false)
                providerContextUsageBaselineAdmissionIssues.push("provider_execution_failed");
            if (!runnerStarted)
                providerContextUsageBaselineAdmissionIssues.push("runner_not_started");
            if (!finalDispatchPayloadGateVerification.valid || finalDispatchPayloadGate?.provider_call_allowed !== true)
                providerContextUsageBaselineAdmissionIssues.push("final_dispatch_gate_not_admitted");
            if (snapshotId !== String(current.memoryContextSnapshotId || ""))
                providerContextUsageBaselineAdmissionIssues.push("delivery_snapshot_not_current");
            if (!compactHeadFenceValid)
                providerContextUsageBaselineAdmissionIssues.push("compact_head_stale");
            if (!sessionLifecycleFenceValid)
                providerContextUsageBaselineAdmissionIssues.push("session_lifecycle_stale");
        }
        const providerContextUsageBaselineCandidate = providerContextUsageReported
            ...
            ? (0, final_dispatch_payload_gate_1.buildFinalDispatchProviderUsageBaseline)({
        ...
        const providerContextUsageBaselineVerification = providerContextUsageBaselineCandidate
            ? (0, final_dispatch_payload_gate_1.verifyFinalDispatchProviderUsageBaseline)(providerContextUsageBaselineCandidate, {
                groupId: String(snapshot.session?.group_id || current.groupId || ""),
                groupSessionId: String(groupSessionMemoryBinding?.groupSessionId || ""),
                taskId: String(snapshot.session?.task_id || current.taskId || ""),
                taskAgentSessionId: id,
        ...
        const previousProviderContextUsageBaselineVerification = current.providerContextUsageBaseline
            ? (0, final_dispatch_payload_gate_1.verifyFinalDispatchProviderUsageBaseline)(current.providerContextUsageBaseline, {
                groupId: String(snapshot.session?.group_id || current.groupId || ""),
                groupSessionId: String(groupSessionMemoryBinding?.groupSessionId || ""),
                taskId: String(snapshot.session?.task_id || current.taskId || ""),
                taskAgentSessionId: id,
        ...
        const payload = {
            ...
            actualRenderedPromptChecksum: (0, agent_sessions_shared_1.hashValue)(actualPrompt),
            promptBindingMode,
            executionSucceeded: input.executionSucceeded !== false,
            outputChecksum,
            runnerStarted,
        ...
        const receipt = { ...payload, checksum: (0, agent_sessions_shared_1.hashValue)(payload, 64), receiptFile };
        (0, agent_sessions_shared_1.writeJsonAtomic)(receiptFile, receipt);
        const memorySnapshotSync = snapshot.context?.memory_snapshot_sync || null;
        const memorySnapshotSyncVerification = (0, agent_sessions_shared_1.verifyTaskAgentMemorySnapshotSyncDecision)(memorySnapshotSync, {
            groupId: String(snapshot.session?.group_id || current.groupId || ""),
            groupSessionId: String(groupSessionMemoryBinding?.groupSessionId || ""),
            taskId: String(snapshot.session?.task_id || current.taskId || ""),
            taskAgentSessionId: id,
        ...
        const memoryPromptInjectionVerification = (0, agent_sessions_shared_1.verifyTaskAgentMemoryPromptInjectionProof)(memoryPromptInjectionProof, {
            groupId: String(snapshot.session?.group_id || current.groupId || ""),
            groupSessionId: String(groupSessionMemoryBinding?.groupSessionId || ""),
            taskId: String(snapshot.session?.task_id || current.taskId || ""),
            taskAgentSessionId: id,
        ...
        const syncCommitFile = (0, agent_sessions_shared_1.getMemorySnapshotSyncCommitFile)(id, snapshotId);
        const existingCanonicalReceipt = ref?.deliveryReceiptPath
            ? (0, agent_sessions_shared_1.safeReadJson)(ref.deliveryReceiptPath, null)
            : null;
        const existingCanonicalReceiptValid = !!existingCanonicalReceipt
            && (0, agent_sessions_shared_1.verifyMemoryContextDeliveryReceiptChecksum)(existingCanonicalReceipt)
        ...
        const existingSyncCommit = fs.existsSync(syncCommitFile) ? (0, agent_sessions_shared_1.safeReadJson)(syncCommitFile, null) : null;
        const existingSyncCommitVerification = existingCanonicalReceiptValid
            ? (0, agent_sessions_shared_1.verifyTaskAgentMemorySnapshotSyncCommit)(existingSyncCommit, {
                groupId: String(snapshot.session?.group_id || current.groupId || ""),
                groupSessionId: String(groupSessionMemoryBinding?.groupSessionId || ""),
                taskId: String(snapshot.session?.task_id || current.taskId || ""),
                taskAgentSessionId: id,
        ...
        const syncCommitPayload = {
            schema: agent_sessions_shared_1.TASK_AGENT_MEMORY_SNAPSHOT_SYNC_COMMIT_SCHEMA,
            version: 1,
        ...
        const syncCommitCandidate = {
            ...syncCommitPayload,
            commit_checksum: (0, agent_sessions_shared_1.memorySnapshotSyncCommitChecksum)(syncCommitPayload),
            commit_file: syncCommitFile,
        };
        const syncCommitVerification = (0, agent_sessions_shared_1.verifyTaskAgentMemorySnapshotSyncCommit)(syncCommitCandidate, {
        ...
        if (!preserveExistingCommittedBaseline)
            (0, agent_sessions_shared_1.writeJsonAtomic)(syncCommitFile, syncCommitCandidate);
        ...
        const previousCompaction = (0, session_compaction_core_1.normalizeSessionCompactionState)(current.compaction || {}, { scope: "task_agent", sessionId: id });
        const normalizedUsage = (0, session_compaction_core_1.normalizeSessionProviderUsage)(providerContextUsageBaseline ? {
            ...providerContextUsageBaseline,
            scope: "task_agent",
            sessionId: id,
            provider: input.runtime || current.agentType || "",
        ...
        const taskCompaction = {
            ...
            preservedRecentMessageIds: Array.isArray(parentContinuity?.recent_messages) ? parentContinuity.recent_messages.map((message) => String(message?.id || "")) : previousCompaction.preservedRecentMessageIds,
...
function readTaskAgentMemoryContextDeliveryReceipt(file) {
    const receipt = (0, agent_sessions_shared_1.safeReadJson)(String(file || ""), null);
    if (!receipt)
        return null;
    return { ...receipt, checksumValid: (0, agent_sessions_shared_1.verifyMemoryContextDeliveryReceiptChecksum)(receipt) };
}
//# sourceMappingURL=agent-sessions-delivery.js.map