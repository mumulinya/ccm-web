...
exports.getTaskAgentInvocationRecoveryLeaseFile = getTaskAgentInvocationRecoveryLeaseFile;
exports.readTaskAgentInvocationRecoveryLease = readTaskAgentInvocationRecoveryLease;
exports.acquireTaskAgentInvocationRecoveryLease = acquireTaskAgentInvocationRecoveryLease;
exports.finalizeTaskAgentInvocationRecoveryLease = finalizeTaskAgentInvocationRecoveryLease;
exports.buildTaskAgentInvocationRecoveryStatus = buildTaskAgentInvocationRecoveryStatus;
exports.reconcileTaskAgentInvocationRecovery = reconcileTaskAgentInvocationRecovery;
exports.verifyTaskAgentInvocationEdge = verifyTaskAgentInvocationEdge;
exports.buildTaskAgentInvocationLineageReport = buildTaskAgentInvocationLineageReport;
exports.deleteTaskAgentInvocationLineageArtifacts = deleteTaskAgentInvocationLineageArtifacts;
// Behavior-freeze split from task-agent-invocation-lineage.ts (part 2/2).
const crypto = __importStar(require("crypto"));
const fs = __importStar(require("fs"));
const os = __importStar(require("os"));
const path = __importStar(require("path"));
const typed_memory_dispatch_wal_1 = require("../modules/collaboration/typed-memory-dispatch-wal");
...
__exportStar(require("./task-agent-invocation-lineage-part-01"), exports);
const task_agent_invocation_lineage_part_01_1 = require("./task-agent-invocation-lineage-part-01");
function getTaskAgentInvocationRecoveryLeaseFile(groupId, groupSessionId) {
    (0, task_agent_invocation_lineage_part_01_1.assertGroupSessionIdentity)(groupId, groupSessionId);
    return path.join(task_agent_invocation_lineage_part_01_1.TASK_AGENT_INVOCATION_RECOVERY_DIR, `${(0, task_agent_invocation_lineage_part_01_1.clean)(groupId)}--${(0, task_agent_invocation_lineage_part_01_1.clean)(groupSessionId)}.lease.json`);
}
function recoveryLeaseChecksum(lease) {
    const payload = { ...(lease || {}) };
    delete payload.checksum;
    delete payload.checksum_valid;
    delete payload.file;
    return (0, task_agent_invocation_lineage_part_01_1.sha256)(payload);
}
function readTaskAgentInvocationRecoveryLease(groupId, groupSessionId) {
    ...
    try {
        ...
        const checksumValid = lease.schema === task_agent_invocation_lineage_part_01_1.TASK_AGENT_INVOCATION_RECOVERY_LEASE_SCHEMA
            && String(lease.checksum || "") === recoveryLeaseChecksum(lease);
        return { ...lease, checksum_valid: checksumValid, file };
    }
    catch {
        return { schema: task_agent_invocation_lineage_part_01_1.TASK_AGENT_INVOCATION_RECOVERY_LEASE_SCHEMA, checksum_valid: false, file, status: "invalid" };
    }
}
function acquireTaskAgentInvocationRecoveryLease(groupId, groupSessionId, options = {}) {
    (0, task_agent_invocation_lineage_part_01_1.assertGroupSessionIdentity)(groupId, groupSessionId);
    const file = getTaskAgentInvocationRecoveryLeaseFile(groupId, groupSessionId);
    const release = (0, task_agent_invocation_lineage_part_01_1.acquireLock)(file);
    try {
        ...
        const activeOwner = current?.checksum_valid === true
            ...
            && expiresMs > nowMs
            && (0, task_agent_invocation_lineage_part_01_1.processAlive)(Number(current.owner_pid || 0));
        if (activeOwner) {
        ...
        const leaseDurationMs = Math.max(5_000, Number(options.leaseMs || options.lease_ms || task_agent_invocation_lineage_part_01_1.RECOVERY_LEASE_MS));
        const takeover = !!current && current.checksum_valid === true && current.status === "active";
        const payload = {
            schema: task_agent_invocation_lineage_part_01_1.TASK_AGENT_INVOCATION_RECOVERY_LEASE_SCHEMA,
            version: 1,
            lease_id: `tirl_${(0, task_agent_invocation_lineage_part_01_1.sha256)(`${groupId}\0${groupSessionId}\0${ownerPid}\0${acquiredAt}\0${crypto.randomBytes(8).toString("hex")}`, 24)}`,
            group_id: groupId,
        ...
        (0, task_agent_invocation_lineage_part_01_1.writeJsonAtomic)(file, payload);
...
function finalizeTaskAgentInvocationRecoveryLease(leaseInput, report = {}) {
    ...
    try {
        ...
        const payload = {
            ...
            report_checksum: (0, task_agent_invocation_lineage_part_01_1.sha256)(report),
        };
        payload.checksum = recoveryLeaseChecksum(payload);
        (0, task_agent_invocation_lineage_part_01_1.writeJsonAtomic)(file, payload);
...
function readInvocationRecoveryHistory(file) {
    if (!fs.existsSync(file))
        return { valid: true, events: [], issues: [], headChecksum: "", lastSequence: 0 };
    const events = [];
    const issues = [];
    let previous = "";
    ...
    for (const line of fs.readFileSync(file, "utf-8").split(/\r?\n/).filter(Boolean)) {
        try {
            ...
            const checksumValid = String(event.event_checksum || "") === (0, task_agent_invocation_lineage_part_01_1.recoveryEventChecksum)(event);
            if (event.schema !== task_agent_invocation_lineage_part_01_1.TASK_AGENT_INVOCATION_RECOVERY_EVENT_SCHEMA)
                issues.push("recovery_event_schema_invalid");
            if (Number(event.sequence || 0) !== expectedSequence)
                issues.push("recovery_event_sequence_invalid");
            if (String(event.previous_event_checksum || "") !== previous)
                issues.push("recovery_event_chain_broken");
            if (!checksumValid)
                issues.push("recovery_event_checksum_invalid");
...
function appendInvocationRecoveryAudit(groupId, groupSessionId, report) {
    const historyFile = (0, task_agent_invocation_lineage_part_01_1.getInvocationRecoveryHistoryFile)(groupId, groupSessionId);
    const release = (0, task_agent_invocation_lineage_part_01_1.acquireLock)(historyFile);
    try {
        const history = readInvocationRecoveryHistory(historyFile);
        if (!history.valid)
            throw new Error(`task-agent invocation recovery history invalid: ${history.issues.join(",")}`);
        const event = {
            schema: task_agent_invocation_lineage_part_01_1.TASK_AGENT_INVOCATION_RECOVERY_EVENT_SCHEMA,
            version: 1,
        ...
        event.event_checksum = (0, task_agent_invocation_lineage_part_01_1.recoveryEventChecksum)(event);
        ...
        status.checksum = (0, task_agent_invocation_lineage_part_01_1.recoveryStatusChecksum)(status);
        (0, task_agent_invocation_lineage_part_01_1.writeJsonAtomic)((0, task_agent_invocation_lineage_part_01_1.getInvocationRecoveryStatusFile)(groupId, groupSessionId), status);
        return { event, status };
    }
    finally {
        release();
...
function relinkInvocationEdgeToAncestor(edge, edgesById, recoveryLease = null) {
    const ancestry = Array.isArray(edge.parent_ancestry) ? edge.parent_ancestry : [];
    const replacement = ancestry.slice(1).map((row) => ({ row, edge: edgesById.get(String(row.invocation_edge_id || "")) }))
        .find(({ row, edge: candidate }) => candidate
    ...
    if (!replacement?.edge)
        return { repaired: false, reason: "no_checksum_verified_surviving_ancestor" };
    const current = (0, task_agent_invocation_lineage_part_01_1.findTaskAgentInvocationEdge)(String(edge.invocation_edge_id || ""));
    if (!current)
        return { repaired: false, reason: "edge_missing_during_repair" };
    ...
    next.edge_checksum = (0, task_agent_invocation_lineage_part_01_1.edgeChecksum)(next);
    (0, task_agent_invocation_lineage_part_01_1.appendEvent)(current.ledger_file, next, "parent_relinked");
    return { repaired: true, edge: { ...next, ledger_file: current.ledger_file }, ancestor };
}
function rebindInvocationEdgeToRecoveredParent(edge, parent, recoveryLease = null) {
    ...
    const current = (0, task_agent_invocation_lineage_part_01_1.findTaskAgentInvocationEdge)(String(edge.invocation_edge_id || ""));
    if (!current)
        return { repaired: false, reason: "edge_missing_during_head_rebind" };
    ...
    delete next.ledger_valid;
    next.edge_checksum = (0, task_agent_invocation_lineage_part_01_1.edgeChecksum)(next);
    (0, task_agent_invocation_lineage_part_01_1.appendEvent)(current.ledger_file, next, "parent_head_rebound");
    return { repaired: true, edge: { ...next, ledger_file: current.ledger_file }, parent };
}
function directResultEvidence(edge) {
    const runnerRequestId = String(edge.runner_request_id || "");
    if (!runnerRequestId.startsWith("adr_"))
        return { present: false, valid: false, request: null, result: null, issues: [] };
    const request = (0, direct_dispatch_spool_1.readDirectAgentDispatchRequest)(runnerRequestId);
    const result = (0, direct_dispatch_spool_1.readDirectAgentDispatchResult)(runnerRequestId);
    if (!request || !result)
        return { present: !!request || !!result, valid: false, request, result, issues: [request ? "result_missing" : "request_missing"] };
    const pair = (0, direct_dispatch_spool_1.validateDirectAgentDispatchPair)(request, result);
    const issues = [...pair.issues];
    if (String(request.groupId || "") !== String(edge.group_id || ""))
        issues.push("edge_runner_group_mismatch");
    if (String(request.taskId || "") !== String(edge.task_id || ""))
        issues.push("edge_runner_task_mismatch");
    if (String(request.taskAgentSessionId || "") !== String(edge.task_agent_session_id || ""))
        issues.push("edge_runner_task_session_mismatch");
    if (edge.execution_id && String(request.executionId || "") !== String(edge.execution_id || ""))
        issues.push("edge_runner_execution_mismatch");
    if (edge.prompt_checksum && String(request.prompt_checksum || "") !== String(edge.prompt_checksum || ""))
        issues.push("edge_runner_prompt_mismatch");
    return { present: true, valid: issues.length === 0, request, result, issues: Array.from(new Set(issues)) };
}
function buildTaskAgentInvocationRecoveryStatus(options = {}) {
    const rows = [];
    const issues = [];
    if (fs.existsSync(task_agent_invocation_lineage_part_01_1.TASK_AGENT_INVOCATION_RECOVERY_DIR)) {
        for (const entry of fs.readdirSync(task_agent_invocation_lineage_part_01_1.TASK_AGENT_INVOCATION_RECOVERY_DIR, { withFileTypes: true })) {
            if (!entry.isFile() || !entry.name.endsWith(".latest.json"))
                continue;
            const file = path.join(task_agent_invocation_lineage_part_01_1.TASK_AGENT_INVOCATION_RECOVERY_DIR, entry.name);
            try {
                const status = JSON.parse(fs.readFileSync(file, "utf-8"));
                const checksumValid = String(status.checksum || "") === (0, task_agent_invocation_lineage_part_01_1.recoveryStatusChecksum)(status);
                if (!checksumValid)
                    issues.push({ file, issues: ["recovery_status_checksum_invalid"] });
                if (options.groupId && status.group_id !== String(options.groupId))
                    continue;
                if (options.groupSessionId && status.group_session_id !== String(options.groupSessionId))
                    continue;
...
function reconcileTaskAgentInvocationRecovery(options = {}) {
    const startedAt = new Date().toISOString();
    const nowMs = Date.parse(String(options.now || "")) || Date.now();
    const minimumAgeMs = Math.max(0, Number(options.minimumAgeMs ?? options.minimum_age_ms ?? 30_000));
    const listed = (0, task_agent_invocation_lineage_part_01_1.listTaskAgentInvocationEdges)(options);
    const recoveryRunId = `tirr_${(0, task_agent_invocation_lineage_part_01_1.sha256)(`${process.pid}\0${startedAt}\0${crypto.randomBytes(8).toString("hex")}`, 24)}`;
    const leaseAttempts = new Map();
    for (const edge of listed.edges) {
        const scopeKey = `${edge.group_id}\0${edge.group_session_id}`;
        if (leaseAttempts.has(scopeKey))
            continue;
    ...
    const edgesById = new Map(listed.edges.map((edge) => [String(edge.invocation_edge_id || ""), edge]));
    const walByTicket = new Map((0, typed_memory_dispatch_wal_1.listTypedMemoryDispatchWal)().map((record) => [String(record.ticket_id || ""), record]));
    const rows = [];
    for (const source of listed.edges) {
        ...
        if (task_agent_invocation_lineage_part_01_1.TERMINAL.has(String(edge.status || "")))
            continue;
        ...
        if (wal) {
            const walVerification = (0, typed_memory_dispatch_wal_1.verifyTypedMemoryDispatchWal)(wal);
            const walIssues = [...walVerification.issues];
            if (String(wal.group_id || "") !== String(edge.group_id || ""))
                walIssues.push("wal_group_mismatch");
            if (String(wal.group_session_id || "") !== String(edge.group_session_id || ""))
                walIssues.push("wal_group_session_mismatch");
            if (String(wal.task_id || "") !== String(edge.task_id || ""))
                walIssues.push("wal_task_mismatch");
            if (String(wal.task_agent_session_id || "") !== String(edge.task_agent_session_id || ""))
                walIssues.push("wal_task_session_mismatch");
            if (edge.dispatch_ticket_checksum && String(wal.ticket_checksum || "") !== String(edge.dispatch_ticket_checksum || ""))
                walIssues.push("wal_ticket_checksum_mismatch");
        ...
        if (direct.valid && direct.result) {
            let recovered = (0, task_agent_invocation_lineage_part_01_1.completeTaskAgentInvocationEdge)(edge, {
            ...
            if (recovered.status === "completed") {
                recovered = (0, task_agent_invocation_lineage_part_01_1.bindTaskAgentInvocationMemoryDelivery)(recovered, {
        ...
        if (wal && ["committed", "cancelled", "expired", "uncertain_after_crash"].includes(String(wal.state || ""))) {
            ...
            if (recovered.status === "completed" && wal.delivery_receipt) {
                recovered = (0, task_agent_invocation_lineage_part_01_1.bindTaskAgentInvocationMemoryDelivery)(recovered, { deliveryReceipt: wal.delivery_receipt });
            }
        ...
        const request = direct.request || (edge.runner_request_id ? (0, direct_dispatch_spool_1.readDirectAgentDispatchRequest)(String(edge.runner_request_id)) : null);
        if (request?.status === "running" && (0, task_agent_invocation_lineage_part_01_1.processAlive)(Number(request.runner_pid || request.runnerPid || 0))) {
        ...
        const recovered = (0, task_agent_invocation_lineage_part_01_1.completeTaskAgentInvocationEdge)(edge, {
            success: false,
            runnerRequestId: edge.runner_request_id || wal?.runner_request_id || "",
    ...
    const acquiredLeases = Array.from(leaseAttempts.values()).filter((attempt) => attempt?.acquired);
    const contendedLeases = Array.from(leaseAttempts.values()).filter((attempt) => !attempt?.acquired);
    return {
        schema: "ccm-task-agent-invocation-recovery-run-v1",
        recovery_run_id: recoveryRunId,
        started_at: startedAt,
        ...
        lease_takeover: acquiredLeases.filter((attempt) => attempt.lease?.takeover === true).length,
        max_fencing_token: acquiredLeases.reduce((max, attempt) => Math.max(max, Number(attempt.lease?.fencing_token || 0)), 0),
        rows,
    };
}
function verifyTaskAgentInvocationEdge(edge, options = {}) {
    if (!edge?.invocation_edge_id)
        return { valid: false, issues: ["edge_missing"] };
    const issues = [];
    if (edge.schema !== task_agent_invocation_lineage_part_01_1.TASK_AGENT_INVOCATION_EDGE_SCHEMA || Number(edge.version || 0) !== 1)
        issues.push("edge_schema_invalid");
    if (!String(edge.group_session_id || "").startsWith("gcs_"))
        issues.push("group_session_identity_invalid");
    if (!String(edge.task_agent_session_id || "").startsWith("tas_"))
        issues.push("task_agent_session_identity_invalid");
    if (!edge.branch_id || !edge.root_invocation_edge_id)
        issues.push("branch_identity_missing");
    if (edge.parent_invocation_edge_id === edge.invocation_edge_id)
        issues.push("self_parent_cycle");
    const checksumValid = String(edge.edge_checksum || "") === (0, task_agent_invocation_lineage_part_01_1.edgeChecksum)(edge);
    if (!checksumValid)
        issues.push("edge_checksum_invalid");
    const parent = edge.parent_invocation_edge_id
        ? options.edgesById?.get?.(edge.parent_invocation_edge_id) || (0, task_agent_invocation_lineage_part_01_1.findTaskAgentInvocationEdge)(edge.parent_invocation_edge_id)
        : null;
    if (edge.parent_invocation_edge_id && !parent)
        issues.push("parent_edge_missing");
    if (parent && (parent.group_id !== edge.group_id || parent.group_session_id !== edge.group_session_id || parent.task_id !== edge.task_id || parent.target_project !== edge.target_project))
...
function buildTaskAgentInvocationLineageReport(filter = {}) {
    const listed = (0, task_agent_invocation_lineage_part_01_1.listTaskAgentInvocationEdges)(filter);
    const edgesById = new Map(listed.edges.map((edge) => [String(edge.invocation_edge_id || ""), edge]));
    const recoveryStatus = buildTaskAgentInvocationRecoveryStatus(filter);
    const rows = listed.edges.map((edge) => {
        const verification = verifyTaskAgentInvocationEdge(edge, { edgesById });
        const gaps = [...verification.issues];
        const adoptionVerification = edge.adoption_receipt ? (0, task_agent_invocation_lineage_part_01_1.verifyTaskAgentInvocationAdoptionReceipt)(edge.adoption_receipt, edge) : { valid: false, issues: [] };
        const reinjectionVerification = edge.reinjection_proof ? (0, task_agent_invocation_lineage_part_01_1.verifyTaskAgentInvocationReinjectionProof)(edge.reinjection_proof, edge) : { valid: false, issues: [] };
        const nativeContinuationVerification = edge.native_continuation_receipt ? (0, task_agent_invocation_lineage_part_01_1.verifyTaskAgentNativeContinuationReceipt)(edge.native_continuation_receipt, edge) : { valid: false, issues: [] };
        const contextRebudgetVerification = edge.context_rebudget_proof ? (0, task_agent_invocation_lineage_part_01_1.verifyTaskAgentContextRebudgetProof)(edge.context_rebudget_proof, edge) : { valid: false, issues: [] };
        if (edge.adoption_receipt && !adoptionVerification.valid)
            gaps.push(...adoptionVerification.issues);
        if (edge.reinjection_proof && !reinjectionVerification.valid)
            gaps.push(...reinjectionVerification.issues);
        if (edge.native_continuation_receipt && !nativeContinuationVerification.valid)
            gaps.push(...nativeContinuationVerification.issues);
        if (edge.context_rebudget_proof && !contextRebudgetVerification.valid)
            gaps.push(...contextRebudgetVerification.issues);
        if (["dispatched", "completed", "failed"].includes(edge.status) && !edge.worker_context_packet_id)
            gaps.push("worker_context_packet_missing");
        if (["dispatched", "completed", "failed"].includes(edge.status) && !edge.memory_context_snapshot_id)
            gaps.push("memory_context_snapshot_missing");
        ...
        if (["completed", "failed"].includes(edge.status) && !edge.runner_request_id && !recoveryWithoutRunnerAllowed)
            gaps.push("runner_request_missing");
        if (edge.compact_head_dispatch_fence_valid === false)
            gaps.push("compact_head_dispatch_stale");
        if (["dispatched", "completed", "failed"].includes(edge.status)
            ...
            gaps.push("compact_head_dispatch_fence_missing");
        if (edge.session_lifecycle_dispatch_fence_valid === false)
            gaps.push("session_lifecycle_dispatch_stale");
        if (["dispatched", "completed", "failed"].includes(edge.status)
            ...
            gaps.push("session_lifecycle_dispatch_fence_missing");
        if (edge.dispatch_ticket_id && !edge.typed_memory_dispatch_wal_file)
            gaps.push("typed_memory_dispatch_wal_file_missing");
        if (edge.dispatch_ticket_id && !edge.typed_memory_dispatch_wal_record_checksum)
            gaps.push("typed_memory_dispatch_wal_checksum_missing");
        const continuityProofRequired = edge.continuity_contract_required === true && edge.status === "completed";
        if (continuityProofRequired && edge.adoption_status !== "adopted")
            gaps.push("adoption_receipt_unverified");
        if (continuityProofRequired && edge.reinjection_status !== "proven")
            gaps.push("recovery_reinjection_unproven");
        if (continuityProofRequired && edge.native_continuation_status === "unverified")
    ...
    return {
        ...
        overall: {
            ...
            nonTerminalCount: rows.filter(row => !task_agent_invocation_lineage_part_01_1.TERMINAL.has(String(row.status || ""))).length,
...
function deleteTaskAgentInvocationLineageArtifacts(groupId, groupSessionId, taskAgentSessionId) {
    let deleted = 0;
    for (const file of (0, task_agent_invocation_lineage_part_01_1.listFiles)()) {
        const name = path.basename(file);
        const prefix = `${(0, task_agent_invocation_lineage_part_01_1.clean)(groupId)}--${(0, task_agent_invocation_lineage_part_01_1.clean)(groupSessionId)}--`;
        if (!name.startsWith(prefix))
            continue;
        if (taskAgentSessionId && name !== `${prefix}${(0, task_agent_invocation_lineage_part_01_1.clean)(taskAgentSessionId)}.jsonl`)
            continue;
        try {
            fs.unlinkSync(file);
            deleted += 1;
    ...
    if (!taskAgentSessionId) {
        for (const file of [(0, task_agent_invocation_lineage_part_01_1.getInvocationRecoveryHistoryFile)(groupId, groupSessionId), (0, task_agent_invocation_lineage_part_01_1.getInvocationRecoveryStatusFile)(groupId, groupSessionId)]) {
            try {
                if (fs.existsSync(file)) {
                    fs.unlinkSync(file);
                    recoveryDeleted += 1;
        ...
        try {
            if (fs.existsSync(leaseFile)) {
                ...
                recoveryLeaseDeleted += 1;
            }
        }
        catch { }
        try {
            fs.unlinkSync(`${leaseFile}.lock`);
        }
        catch { }
    }
    return { deleted, recoveryDeleted, recoveryLeaseDeleted };
}
//# sourceMappingURL=task-agent-invocation-lineage-part-02.js.map