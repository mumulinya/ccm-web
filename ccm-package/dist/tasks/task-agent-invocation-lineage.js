"use strict";
// task-agent-invocation-lineage.ts — merged from 2 part files (behavior-freeze merge).
...
// ===== merged from task-agent-invocation-lineage-part-01.ts =====
...
// ===== merged from task-agent-invocation-lineage-part-02.ts =====
function getTaskAgentInvocationRecoveryLeaseFile(groupId, groupSessionId) {
    assertGroupSessionIdentity(groupId, groupSessionId);
    return path.join(exports.TASK_AGENT_INVOCATION_RECOVERY_DIR, `${clean(groupId)}--${clean(groupSessionId)}.lease.json`);
}
...
function readTaskAgentInvocationRecoveryLease(groupId, groupSessionId) {
    ...
    try {
        ...
        const checksumValid = lease.schema === exports.TASK_AGENT_INVOCATION_RECOVERY_LEASE_SCHEMA
            && String(lease.checksum || "") === recoveryLeaseChecksum(lease);
        return { ...lease, checksum_valid: checksumValid, file };
    }
    catch {
        return { schema: exports.TASK_AGENT_INVOCATION_RECOVERY_LEASE_SCHEMA, checksum_valid: false, file, status: "invalid" };
    }
}
function acquireTaskAgentInvocationRecoveryLease(groupId, groupSessionId, options = {}) {
    ...
    try {
        ...
        const leaseDurationMs = Math.max(5_000, Number(options.leaseMs || options.lease_ms || exports.RECOVERY_LEASE_MS));
        const takeover = !!current && current.checksum_valid === true && current.status === "active";
        const payload = {
            schema: exports.TASK_AGENT_INVOCATION_RECOVERY_LEASE_SCHEMA,
            version: 1,
...
function readInvocationRecoveryHistory(file) {
    ...
    for (const line of fs.readFileSync(file, "utf-8").split(/\r?\n/).filter(Boolean)) {
        try {
            ...
            if (event.schema !== exports.TASK_AGENT_INVOCATION_RECOVERY_EVENT_SCHEMA)
...
function appendInvocationRecoveryAudit(groupId, groupSessionId, report) {
    ...
    try {
        ...
        const event = {
            schema: exports.TASK_AGENT_INVOCATION_RECOVERY_EVENT_SCHEMA,
            version: 1,
...
function buildTaskAgentInvocationRecoveryStatus(options = {}) {
    const rows = [];
    const issues = [];
    if (fs.existsSync(exports.TASK_AGENT_INVOCATION_RECOVERY_DIR)) {
        for (const entry of fs.readdirSync(exports.TASK_AGENT_INVOCATION_RECOVERY_DIR, { withFileTypes: true })) {
            if (!entry.isFile() || !entry.name.endsWith(".latest.json"))
                continue;
            const file = path.join(exports.TASK_AGENT_INVOCATION_RECOVERY_DIR, entry.name);
            try {
...
function reconcileTaskAgentInvocationRecovery(options = {}) {
    ...
    for (const source of listed.edges) {
        ...
        if (exports.TERMINAL.has(String(edge.status || "")))
            continue;
...
function verifyTaskAgentInvocationEdge(edge, options = {}) {
    ...
    const issues = [];
    if (edge.schema !== exports.TASK_AGENT_INVOCATION_EDGE_SCHEMA || Number(edge.version || 0) !== 1)
...
function buildTaskAgentInvocationLineageReport(filter = {}) {
    ...
    return {
        ...
        overall: {
            ...
            nonTerminalCount: rows.filter(row => !exports.TERMINAL.has(String(row.status || ""))).length,
...
//# sourceMappingURL=task-agent-invocation-lineage.js.map