...
function recoveryConversationProjection(task, reason) {
    const receipt = (0, task_conversation_projection_1.syncTaskConversationProjection)(task, reason);
...
function mergeRecoveryConversationProjections(first, second) {
...
function taskAttempt(task) {
    ...
    const workItemAttempts = (Array.isArray(task?.task_context?.workItems) ? task.task_context.workItems : [])
        .map((item) => Number(item?.latestAttempt || item?.latest_attempt || 0));
    return Math.max(0, Number(task?.execution_attempt || 0), Number(task?.project_main_execution?.attempt || 0), Number(task?.attempt || 0), Number(task?.task_context?.latestAttempt || 0), ...timelineAttempts, ...workItemAttempts);
}
...
function runTaskRecoveryOrchestrator(taskInput, options) {
    ...
    if (!operation.acquired) {
        const duplicateTask = (0, db_1.getTaskById)(taskId);
    ...
    try {
        ...
        const staged = (0, db_1.updateTaskByIdCas)(taskId, current => String(current?.interruption_receipt?.checksum || "") === receiptChecksum
            ...
            && !(String(current?.recovery_transaction?.status || "") === "committed" && current?.recovery_pending !== true), current => {
            const next = {
                ...current,
                status: "blocked",
        ...
        const commitIdentity = (0, task_context_1.taskTimelineIdentity)(staged.task || latest);
        if (!commitIdentity.exactSessionId)
        ...
        const committedTimeline = (0, session_task_timeline_1.persistTaskMutationWithTimelineAtomically)({
            ...
            buildContext: (taskForContext, previousContext) => {
                const context = (0, task_context_1.buildTaskContextCapsule)(taskForContext, previousContext, "recovery_committed");
                const sessionBindings = userSession.binding
                    ? [...(Array.isArray(context.sessionBindings) ? context.sessionBindings : []).filter((item) => item.bindingChecksum !== userSession.binding.bindingChecksum), userSession.binding]
        ...
        const latestTask = (0, db_1.getTaskById)(taskId) || committed.task;
        ...
        if (resolvedUserSession?.created === true)
            (0, task_recovery_sessions_1.rollbackResolvedTaskUserSession)(resolvedUserSession, options.scope, options.scopeId);
        if (transaction) {
            ...
            if (currentTask && recoveryPreflight && Number(currentTask.execution_attempt || 0) === recoveryPreflight.nextAttempt) {
                try {
                    const identity = (0, task_context_1.taskTimelineIdentity)(currentTask);
                    ...
                    (0, session_task_timeline_1.persistTaskMutationWithTimelineAtomically)({
                        task: rollbackTask,
                        ...
                        forceSnapshot: true,
                        buildContext: (taskForContext, previousContext) => {
                            const context = (0, task_context_1.buildTaskContextCapsule)(taskForContext, previousContext, "recovery_rolled_back");
                            return { ...context, sessionBindings: (context.sessionBindings || []).map((binding) => binding.exactSessionId === resolvedUserSession?.activeSessionId ? { ...binding, status: "unavailable", releasedAt: new Date().toISOString() } : binding) };
                        },
                    });
            ...
            if (!atomicallyClosedAttempt)
                (0, db_1.updateTaskByIdCas)(taskId, current => String(current?.recovery_transaction?.transactionId || "") === transaction?.transactionId, current => ({ ...current, status: "blocked", acceptance_state: "recovery_required", auto_execute: false, paused: true, is_paused: true, recovery_pending: true, recovery_transaction: rolledBack, status_detail: `恢复未提交：${String(error?.message || error).slice(0, 300)}`, updated_at: new Date().toISOString() }));
            ...
            const rolledBackTask = (0, db_1.getTaskById)(taskId);
            if (rolledBackTask)
                recoveryConversationProjection(rolledBackTask, "recovery_transaction_rolled_back");
        }
        if (leaseAcquired)
