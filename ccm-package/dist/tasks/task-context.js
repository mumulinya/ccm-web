...
function updateTaskContext(taskId, delta = {}, expectedTaskRevision) {
    const id = text(taskId);
    if (!id)
        throw new Error("任务上下文缺少 taskId");
    const result = (0, db_1.updateTaskByIdCas)(id, current => expectedTaskRevision === undefined || Number(current?.revision || 0) === Number(expectedTaskRevision), current => {
    ...
    if (!result.updated)
        throw new Error("任务上下文版本冲突，请刷新后重试");
    return result.task;
}
function addTaskFileEvidence(taskId, evidence, expectedRevision) {
    ...
    const result = (0, db_1.updateTaskByIdCas)(text(taskId), current => expectedRevision === undefined || Number(current?.revision || 0) === Number(expectedRevision), current => { const context = buildTaskContextCapsule(current, current?.task_context || prior, "file_evidence"); const fileEvidence = [...(context.fileEvidence || []).filter((x) => x.evidenceId !== item.evidenceId), item].slice(-500); const next = { ...context, fileEvidence, revision: context.revision + 1, updatedAt: new Date().toISOString() }; return { ...current, task_context: { ...next, checksum: checksumWithoutChecksum(next) }, task_context_revision_receipt: { revision: next.revision, checksum: checksumWithoutChecksum(next), reason: "file_evidence", at: next.updatedAt, contentStored: false } }; });
    if (!result.updated)
        throw new Error("任务上下文版本冲突，请刷新后重试");
    return result.task;
}
