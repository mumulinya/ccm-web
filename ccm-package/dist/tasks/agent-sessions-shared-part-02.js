...
const agent_sessions_shared_part_01_1 = require("./agent-sessions-shared-part-01");
function memorySnapshotSyncCommitChecksum(value) {
    ...
    return (0, agent_sessions_shared_part_01_1.hashValue)(payload, 64);
}
function verifyTaskAgentMemorySnapshotSyncCommit(commit, expected = {}) {
    const issues = [];
    if (commit?.schema !== agent_sessions_shared_part_01_1.TASK_AGENT_MEMORY_SNAPSHOT_SYNC_COMMIT_SCHEMA)
...
function createTaskAgentMemorySnapshotSyncDecision(input) {
    ...
    const previousSnapshot = previousRef?.snapshotPath ? (0, agent_sessions_shared_part_01_1.safeReadJson)(previousRef.snapshotPath, null) : null;
    const previousSnapshotOuterTrusted = !!previousSnapshot
        && previousSnapshot.schema === agent_sessions_shared_part_01_1.TASK_AGENT_MEMORY_CONTEXT_SNAPSHOT_SCHEMA
        && (0, agent_sessions_shared_part_01_1.verifyMemoryContextSnapshotChecksum)(previousSnapshot)
    ...
    const previousSnapshotSyncVerification = previousSnapshotOuterTrusted
        ? (0, agent_sessions_shared_part_01_1.verifyTaskAgentMemorySnapshotSyncDecision)(previousSnapshotSync, {
    ...
    const previousMemoryPromptInjectionVerification = previousSnapshotOuterTrusted && previousSnapshotSyncVerification.valid === true
        ? (0, agent_sessions_shared_part_01_1.verifyTaskAgentMemoryPromptInjectionProof)(previousMemoryPromptInjectionProof, {
            groupId: input.session.groupId,
            groupSessionId: String(previousSnapshot?.context?.group_session_memory_binding?.groupSessionId || ""),
            taskId: input.session.taskId,
            taskAgentSessionId: input.session.id,
    ...
    const previousDeliveryReceipt = previousRef?.deliveryReceiptPath
        ? (0, agent_sessions_shared_part_01_1.safeReadJson)(previousRef.deliveryReceiptPath, null)
        : null;
    ...
    const previousSyncCommitFile = String(previousRef?.memorySnapshotSyncCommitPath
        || (previousRef?.snapshotId ? (0, agent_sessions_shared_part_01_1.getMemorySnapshotSyncCommitFile)(input.session.id, previousRef.snapshotId) : "")).trim();
    const previousSyncCommit = previousSyncCommitFile ? (0, agent_sessions_shared_part_01_1.safeReadJson)(previousSyncCommitFile, null) : null;
    ...
    const payload = {
        schema: agent_sessions_shared_part_01_1.TASK_AGENT_MEMORY_SNAPSHOT_SYNC_SCHEMA,
        version: 1,
        action,
        reason,
    ...
    const decision = { ...payload, sync_checksum: (0, agent_sessions_shared_part_01_1.memorySnapshotSyncChecksum)(payload) };
    const verification = (0, agent_sessions_shared_part_01_1.verifyTaskAgentMemorySnapshotSyncDecision)(decision, {
        groupId: input.session.groupId,
        groupSessionId,
        taskId: input.session.taskId,
        taskAgentSessionId: input.session.id,
...
function extractGroupSessionMemoryBinding(memoryContext = {}) {
    ...
    const replayEvidenceChecksumValid = !!replayEvidence?.checksum
        ...
        && (0, agent_sessions_shared_part_01_1.hashValue)(replayEvidencePayload || {}, 64) === String(replayEvidence.checksum || "");
    ...
    return { ...binding, checksum: (0, agent_sessions_shared_part_01_1.hashValue)(binding) };
}
function verifyMemoryContextDeliveryReceiptChecksum(receipt) {
    ...
    return (0, agent_sessions_shared_part_01_1.hashValue)(payload, 64) === expected;
}
...
function emptyFinalDispatchReactiveCompactCircuit(session, groupSessionId = "") {
    const payload = {
        ...
        max_consecutive_failures: agent_sessions_shared_part_01_1.FINAL_DISPATCH_REACTIVE_COMPACT_MAX_CONSECUTIVE_FAILURES,
        revision: 0,
        opened_at: "",
        last_failure_at: "",
        last_success_at: "",
...
function recordTaskAgentMemoryEntryRenderContention(sessionId, input) {
    return (0, agent_sessions_shared_part_01_1.withTaskAgentSessionStoreLock)(() => {
        const store = (0, agent_sessions_shared_part_01_1.loadStore)();
        const session = store.sessions.find((item) => item.id === sessionId);
        if (!session)
            return null;
        const observedAt = new Date().toISOString();
        const payload = {
            schema: agent_sessions_shared_part_01_1.MEMORY_ENTRY_RENDER_CONTENTION_SCHEMA,
            version: 1,
            status: input.status,
        ...
        (0, agent_sessions_shared_part_01_1.saveStore)(store);
        return receipt;
    });
}
...
function sessionSnapshotContextWindow(session) {
    const snapshot = (0, agent_sessions_shared_part_01_1.safeReadJson)(String(session.memoryContextSnapshotPath || ""), null);
...
//# sourceMappingURL=agent-sessions-shared-part-02.js.map