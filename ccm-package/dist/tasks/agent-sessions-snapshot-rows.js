...
exports.buildTaskAgentMemorySnapshotRow = buildTaskAgentMemorySnapshotRow;
...
function buildTaskAgentMemorySnapshotRow(input) {
    ...
    const groupSessionMemoryBinding = context.group_session_memory_binding || (0, agent_sessions_shared_1.extractGroupSessionMemoryBinding)(memoryContext || {});
    const memorySnapshotSync = context.memory_snapshot_sync || null;
    const memorySnapshotSyncPresent = memorySnapshotSync?.schema === agent_sessions_shared_1.TASK_AGENT_MEMORY_SNAPSHOT_SYNC_SCHEMA;
    const memorySnapshotSyncVerification = memorySnapshotSyncPresent
        ? (0, agent_sessions_shared_1.verifyTaskAgentMemorySnapshotSyncDecision)(memorySnapshotSync, {
    ...
    const memoryPromptInjectionProofPresent = memoryPromptInjectionProof?.schema === agent_sessions_shared_1.TASK_AGENT_MEMORY_PROMPT_INJECTION_PROOF_SCHEMA;
    const memoryPromptInjectionVerification = memoryPromptInjectionProofPresent
        ? (0, agent_sessions_shared_1.verifyTaskAgentMemoryPromptInjectionProof)(memoryPromptInjectionProof, {
            groupId: String(loadedSession.group_id || session?.groupId || ""),
            groupSessionId: String(groupSessionMemoryBinding?.groupSessionId || ""),
            taskId: String(loadedSession.task_id || session?.taskId || ""),
            taskAgentSessionId: String(loadedSession.id || session?.id || ""),
    ...
    const finalDispatchPayloadGateVerification = finalDispatchPayloadGatePresent
        ? (0, final_dispatch_payload_gate_1.verifyFinalWorkerDispatchPayloadGate)(finalDispatchPayloadGate, {
            ...
            groupSessionId: String(groupSessionMemoryBinding?.groupSessionId || (0, agent_sessions_shared_1.capacityRevalidationGroupSessionId)(context.worker_context_packet || {})),
    ...
    const providerContextUsageBaselineIntegrityVerification = providerContextUsageBaselinePresent
        ? (0, final_dispatch_payload_gate_1.verifyFinalDispatchProviderUsageBaseline)(providerContextUsageBaseline, {
    ...
    const providerContextUsageBaselineStaleAfterCompact = providerContextUsageBaselineStale
        && providerContextUsageBaselineVerification.issues.some((issue) => issue.includes("source_compact_"));
    ...
    const finalDispatchReactiveCompactCircuitBreakerVerification = finalDispatchReactiveCompactCircuitBreakerPresent
        ? require("./agent-sessions-resume").verifyTaskAgentFinalDispatchReactiveCompactCircuitBreaker(finalDispatchReactiveCompactCircuitBreaker, {
            groupId: String(loadedSession.group_id || session?.groupId || ""),
            groupSessionId: String(finalDispatchPayloadGate?.group_session_id || (0, agent_sessions_shared_1.capacityRevalidationGroupSessionId)(context.worker_context_packet || {})),
            taskId: String(loadedSession.task_id || session?.taskId || ""),
            taskAgentSessionId: String(loadedSession.id || session?.id || ""),
        })
        : { valid: true, issues: [] };
    ...
    const sessionBound = input.source === "session_ref"
        ...
        : false;
    const schemaOk = loaded?.schema === agent_sessions_shared_1.TASK_AGENT_MEMORY_CONTEXT_SNAPSHOT_SCHEMA;
    const checksumMatches = !!loaded && (0, agent_sessions_shared_1.verifyMemoryContextSnapshotChecksum)(loaded);
    const memoryContextPresent = (0, agent_sessions_shared_1.hasMeaningfulMemoryContext)(memoryContext);
    const deliveryReceiptFile = String(input.ref?.deliveryReceiptPath
        ...
        || "").trim();
    const deliveryReceipt = deliveryReceiptFile ? (0, agent_sessions_shared_1.safeReadJson)(deliveryReceiptFile, null) : null;
    const deliveryReceiptChecksumValid = !!deliveryReceipt && (0, agent_sessions_shared_1.verifyMemoryContextDeliveryReceiptChecksum)(deliveryReceipt);
    ...
    const memoryContinuationBaselineVerification = deliveryReceipt
        ? (0, agent_sessions_shared_1.verifyTaskAgentMemoryContinuationBaselineDelivery)(loaded, {
    ...
    const latestDeliveryAttemptReceipt = latestDeliveryAttemptReceiptFile
        ? (0, agent_sessions_shared_1.safeReadJson)(latestDeliveryAttemptReceiptFile, null)
        : null;
    ...
    const latestDeliveryAttemptValid = !!latestDeliveryAttemptReceipt
        && (0, agent_sessions_shared_1.verifyMemoryContextDeliveryReceiptChecksum)(latestDeliveryAttemptReceipt)
    ...
    const memorySnapshotSyncCommitFile = String(input.ref?.memorySnapshotSyncCommitPath
        ...
        || (sessionId && snapshotId ? (0, agent_sessions_shared_1.getMemorySnapshotSyncCommitFile)(sessionId, snapshotId) : "")).trim();
    ...
    const memorySnapshotSyncCommit = memorySnapshotSyncCommitPresent
        ? (0, agent_sessions_shared_1.safeReadJson)(memorySnapshotSyncCommitFile, null)
        : null;
    const memorySnapshotSyncCommitVerification = memorySnapshotSyncCommitPresent
        ? (0, agent_sessions_shared_1.verifyTaskAgentMemorySnapshotSyncCommit)(memorySnapshotSyncCommit, {
            groupId: String(loadedSession.group_id || session?.groupId || ""),
            groupSessionId: String(groupSessionMemoryBinding?.groupSessionId || ""),
            taskId: String(loadedSession.task_id || session?.taskId || ""),
            taskAgentSessionId: sessionId,
    ...
    const prunable = fileExists
        && (0, agent_sessions_shared_1.pathIsInsideMemorySnapshotDir)(actualFile)
        && !latestForSession
    ...
    if (loaded && providerContextUsageBaselinePresent && !providerContextUsageBaselineIntegrityVerification.valid)
        hardGaps.push({ reason: `Provider usage 下一轮基线无效：${providerContextUsageBaselineIntegrityVerification.issues.join(",") || "unknown"}` });
    if (loaded && providerContextUsageBaselineStale)
        warningGaps.push({ reason: `Provider usage 下一轮基线已过期：${providerContextUsageBaselineVerification.issues.join(",") || "identity_or_lineage_changed"}` });
    ...
    return {
        ...
        groupSessionId: String(groupSessionMemoryBinding?.groupSessionId || finalDispatchPayloadGate?.group_session_id || (0, agent_sessions_shared_1.capacityRevalidationGroupSessionId)(context.worker_context_packet || {}) || ""),
...
//# sourceMappingURL=agent-sessions-snapshot-rows.js.map