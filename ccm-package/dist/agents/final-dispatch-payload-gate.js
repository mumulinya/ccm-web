...
exports.FINAL_WORKER_DISPATCH_PAYLOAD_GATE_SCHEMA = void 0;
...
function verifyFinalWorkerDispatchPayloadGate(gate, expected = {}) {
    ...
    if (!["ready", "recompact_required"].includes(String(gate?.status || "")))
        issues.push("final_dispatch_payload_gate_status_invalid");
    if (gate?.status === "ready" && Number(gate?.estimated_total_input_tokens || 0) >= Number(gate?.auto_compact_threshold || 0))
        issues.push("final_dispatch_ready_above_threshold");
    if (gate?.status === "recompact_required" && Number(gate?.estimated_total_input_tokens || 0) < Number(gate?.auto_compact_threshold || 0))
