...
exports.summarizeTestAgentBrowserActionEffects = summarizeTestAgentBrowserActionEffects;
exports.summarizeTestAgentBrowserRecovery = summarizeTestAgentBrowserRecovery;
exports.summarizeTestAgentAdversarialEvidence = summarizeTestAgentAdversarialEvidence;
exports.summarizeTestAgentBrowserAuthentication = summarizeTestAgentBrowserAuthentication;
exports.compactTestAgentBrowserAuthenticationSummary = compactTestAgentBrowserAuthenticationSummary;
exports.summarizeTestAgentMultiSessionBrowser = summarizeTestAgentMultiSessionBrowser;
exports.summarizeTestAgentBrowserFlows = summarizeTestAgentBrowserFlows;
exports.runTestAgentReviewBridgeSelfTest = runTestAgentReviewBridgeSelfTest;
...
function compactVisibleText(value, fallback = "", max = 160) {
...
function isBrowserFlowSummary(value) {
    return !!value
        && typeof value === "object"
        && !Array.isArray(value)
        && value.statusCounts
...
function isBrowserMultiSessionSummary(value) {
    return !!value
        && typeof value === "object"
        && !Array.isArray(value)
        && value.statusCounts
        ...
        && (value.sessionCount !== undefined
            || value.uniqueSessionCount !== undefined
            || Array.isArray(value.sessionNames)
            || value.items.some((item) => Array.isArray(item?.sessionNames) || Array.isArray(item?.sessions)));
}
function isBrowserAuthenticationSummary(value) {
    return !!value
        && typeof value === "object"
        && !Array.isArray(value)
...
function isBrowserActionEffectSummary(value) {
    return !!value
        && typeof value === "object"
        && !Array.isArray(value)
...
function isBrowserRecoverySummary(value) {
    return !!value
        && typeof value === "object"
        && !Array.isArray(value)
        && (value.attempted !== undefined
...
function isAdversarialEvidenceSummary(value) {
...
function findBrowserFlowSummary(source, depth = 0, seen = new Set()) {
    if (!source || typeof source !== "object" || depth > 4 || seen.has(source))
        return null;
    seen.add(source);
    if (isBrowserFlowSummary(source))
        return source;
    ...
    if (isBrowserFlowSummary(direct))
        return direct;
    const nestedKeys = [
        "verdict",
        "testAgentVerdict",
...
function findBrowserMultiSessionSummary(source, depth = 0, seen = new Set()) {
    if (!source || typeof source !== "object" || depth > 4 || seen.has(source))
        return null;
    seen.add(source);
    if (isBrowserMultiSessionSummary(source))
        return source;
    if (Array.isArray(source)) {
        for (const item of source) {
    ...
    if (isBrowserMultiSessionSummary(direct))
        return direct;
    const nestedKeys = [
        "verdict",
        "testAgentVerdict",
...
function findBrowserAuthenticationSummary(source, depth = 0, seen = new Set()) {
    if (!source || typeof source !== "object" || depth > 5 || seen.has(source))
        return null;
    seen.add(source);
    if (isBrowserAuthenticationSummary(source))
        return source;
    if (Array.isArray(source)) {
        for (const item of source) {
    ...
    if (isBrowserAuthenticationSummary(direct))
        return direct;
    const nestedKeys = [
        "metadata",
        "verdict",
...
function findBrowserActionEffectSummary(source, depth = 0, seen = new Set()) {
    if (!source || typeof source !== "object" || depth > 5 || seen.has(source))
        return null;
    seen.add(source);
    if (isBrowserActionEffectSummary(source))
        return source;
    if (Array.isArray(source)) {
        for (const item of source) {
    ...
    if (isBrowserActionEffectSummary(direct))
        return direct;
    const nestedKeys = [
        "verdict",
        "testAgentVerdict",
...
function findBrowserRecoverySummary(source, depth = 0, seen = new Set()) {
    if (!source || typeof source !== "object" || depth > 5 || seen.has(source))
        return null;
    seen.add(source);
    if (isBrowserRecoverySummary(source))
        return source;
    if (Array.isArray(source)) {
        for (const item of source) {
    ...
    if (isBrowserRecoverySummary(direct))
        return direct;
    const nestedKeys = [
        "verdict",
        "testAgentVerdict",
...
function findAdversarialEvidenceSummary(source, depth = 0, seen = new Set()) {
    if (!source || typeof source !== "object" || depth > 5 || seen.has(source))
        return null;
    seen.add(source);
    if (isAdversarialEvidenceSummary(source))
        return source;
    if (Array.isArray(source)) {
        for (const item of source) {
    ...
    if (isAdversarialEvidenceSummary(direct))
        return direct;
    const nestedKeys = [
        "verdict",
        "testAgentVerdict",
...
function summarizeTestAgentBrowserActionEffects(...sources) {
    ...
    if (!checks && !actions && !asArray(raw.items).length)
        return null;
    const statusParts = [
...
function summarizeTestAgentBrowserRecovery(...sources) {
    let raw = null;
    for (const source of sources) {
    ...
    if (!checks && !attempted && !asArray(raw.items).length)
        return null;
    const statusParts = [
...
function summarizeTestAgentAdversarialEvidence(...sources) {
    let raw = null;
    for (const source of sources) {
...
function summarizeTestAgentBrowserAuthentication(...sources) {
    let raw = null;
    for (const source of sources) {
    ...
    if (!configuredChecks && !passedChecks && !failedChecks && !blockedChecks)
        return null;
...
function compactTestAgentBrowserAuthenticationSummary(summary) {
    if (!summary)
        return null;
    return {
...
function summarizeTestAgentMultiSessionBrowser(...sources) {
    let raw = null;
    for (const source of sources) {
    ...
    if (!total && !asArray(raw.items).length)
        return null;
    const statusParts = [
...
function summarizeTestAgentBrowserFlows(...sources) {
    let raw = null;
    for (const source of sources) {
...
function runTestAgentReviewBridgeSelfTest() {
...
//# sourceMappingURL=test-agent-review-bridge.js.map