...
const workchain_part_01_1 = require("./workchain-part-01");
function buildWorkchainQualityFollowup(quality) {
    if (!quality?.required || quality?.passed)
        return null;
    const missing = (0, workchain_part_01_1.narrativeList)(quality.missing, 5, "交付总结缺少必要内容。");
    if (!missing.length)
        return null;
    ...
    const postReviewSpotCheckGap = Array.isArray(quality.checks)
        ...
        : null;
    const independentReviewDetail = (0, workchain_part_01_1.sanitizeWorkchainUserText)(independentReviewGap?.detail || "", "", 220);
    ...
    const postReviewSpotCheckDetail = (0, workchain_part_01_1.sanitizeWorkchainUserText)(postReviewSpotCheckGap?.detail || "", "", 220);
...
function buildWorkchainQualityFollowupUserVisibleText(baseText, qualityFollowup) {
    if (!qualityFollowup)
        return baseText;
    const missing = (0, workchain_part_01_1.narrativeList)(qualityFollowup.missing, 3, "交付总结缺少必要内容。");
    const missingText = missing.length ? `，还缺少${missing.join("、")}` : "";
    const nextAction = (0, workchain_part_01_1.sanitizeWorkchainUserText)(qualityFollowup.next_action || qualityFollowup.nextAction, "", 220);
    const base = workchain_part_01_1.GENERIC_COMPLETION_REPLY_PATTERN.test(String(baseText || "").trim()) || /已完成|已处理|任务已处理|处理完成/.test(String(baseText || ""))
        ? "任务已有处理结果"
        : (0, workchain_part_01_1.sanitizeWorkchainUserText)(baseText, "任务已有处理结果", 180).replace(/[。.!！\s]+$/, "");
    return (0, workchain_part_01_1.compactText)(`${base}，但最终交付总结还在补齐${missingText}。${nextAction ? `下一步：${nextAction}` : ""}`, 360);
}
function buildUserVisibleText(input, evidence) {
    ...
    if (status === "supervising") {
        const explicitText = (0, workchain_part_01_1.sanitizeWorkchainUserText)(explicit, "", 420);
        const nextText = (0, workchain_part_01_1.sanitizeWorkchainUserText)(completion.next_action || completion.nextAction || "", "", 260);
    ...
    if (evidence.postReviewSpotCheck?.failed) {
        const failure = (0, workchain_part_01_1.sanitizeWorkchainUserText)(evidence.postReviewSpotCheck.failedText || "", "", 260);
        return failure
            ? `这轮还不能算完成：${failure}`
            : "这轮还不能算完成：TestAgent 已通过，但我的完成前抽查尚未一致。";
    }
    if (evidence.postReviewSpotCheck?.needsUser) {
        const detail = (0, workchain_part_01_1.sanitizeWorkchainUserText)(evidence.postReviewSpotCheck.failedText || "", "", 260);
        return detail
            ? `这轮还需要确认：${detail}`
            : "这轮还需要确认：完成前抽查缺少可复跑的验证条件。";
    }
    if (evidence.independentReviewGate?.failed) {
        const failure = (0, workchain_part_01_1.sanitizeWorkchainUserText)(evidence.independentReviewGate.failedText || "", "", 260);
        return failure
            ? `这轮还不能算完成：${failure}。`
            : "这轮还不能算完成：独立复核未通过，需要原实现成员返工后重新复核。";
    }
    if (evidence.independentReviewGate?.needsRecheck) {
        const detail = (0, workchain_part_01_1.sanitizeWorkchainUserText)(evidence.independentReviewGate.riskText || "", "", 260);
        return detail
            ? `这轮还不能算完成：${detail}。`
    ...
    if (["failed"].includes(status) || phase === "failed")
        return (0, workchain_part_01_1.sanitizeWorkchainUserText)(explicit, "这次处理没有完成；原因和排障信息已放在技术详情里。");
    ...
    if (["completed", "done", "succeeded"].includes(status) || phase === "completed") {
        if (explicit && !workchain_part_01_1.GENERIC_COMPLETION_REPLY_PATTERN.test(String(explicit).trim()))
            return (0, workchain_part_01_1.sanitizeWorkchainUserText)(explicit, "本轮处理已完成。");
        const parts = [];
    ...
    if (["waiting_confirmation", "waiting_clarification"].includes(status) || phase === "needs_user")
        return (0, workchain_part_01_1.sanitizeWorkchainUserText)(explicit, "我需要你确认目标、范围或授权后再继续。");
    return (0, workchain_part_01_1.sanitizeWorkchainUserText)(explicit, input.surface === "global" ? "我正在处理你的需求。" : "协作群正在协调处理你的需求。");
}
function buildTechnicalSections(input) {
    ...
    if (technicalContent)
        records.push({ label: "原始回复", value: (0, workchain_part_01_1.compactText)(technicalContent, 1600) });
    const blockers = (0, workchain_part_01_1.stringList)([...(technical.blockers || []), ...((input.summary || {}).blockers || [])], 6);
    if (blockers.length)
...
function buildMainAgentWorkchain(input) {
    const evidence = (0, workchain_part_01_1.collectCompletionEvidence)(input);
    const userVisibleText = buildUserVisibleText(input, evidence);
    const stages = [
        { id: "intake", label: "理解需求", status: (0, workchain_part_01_1.stageStatus)(input, "intake"), summary: "确认用户真正想完成什么" },
        { id: "plan", label: "形成计划", status: (0, workchain_part_01_1.stageStatus)(input, "plan"), summary: "拆成可执行步骤和验收标准" },
        { id: "execute", label: input.surface === "global" ? "调度执行" : "协作执行", status: (0, workchain_part_01_1.stageStatus)(input, "execute"), summary: "调用工具或安排执行成员落地" },
        { id: "verify", label: "检查验收", status: (0, workchain_part_01_1.stageStatus)(input, "verify"), summary: "核对文件、结果说明、验证和风险" },
        { id: "summarize", label: "总结交付", status: (0, workchain_part_01_1.stageStatus)(input, "summarize"), summary: "用用户能看懂的话说明结果" },
    ];
    const terminal = (0, workchain_part_01_1.terminalWorkchain)(input);
    const baseProgressCheckpoints = (0, workchain_part_01_1.buildMainAgentProgressCheckpoints)(input, stages, evidence);
    const baseTodoPlan = (0, workchain_part_01_1.buildWorkchainTodoPlan)(input, stages, evidence, terminal);
    const explicitNextAction = (0, workchain_part_01_1.sanitizeWorkchainUserText)(input.completion?.next_action, "", 260);
    ...
    const provisionalQuality = (0, workchain_part_01_1.buildFinalSummaryQuality)(input, evidence, terminal, userVisibleText, provisionalNextAction, { todoPlan: baseTodoPlan, progressCheckpoints: baseProgressCheckpoints });
    const provisionalQualityFollowup = buildWorkchainQualityFollowup(provisionalQuality);
    const todoPlan = provisionalQualityFollowup ? (0, workchain_part_01_1.buildWorkchainTodoPlan)(input, stages, evidence, terminal, { qualityFollowup: provisionalQualityFollowup }) : baseTodoPlan;
    const progressCheckpoints = provisionalQualityFollowup ? (0, workchain_part_01_1.buildMainAgentProgressCheckpoints)(input, stages, evidence, { qualityFollowup: provisionalQualityFollowup }) : baseProgressCheckpoints;
    ...
    const finalSummaryQuality = provisionalQualityFollowup
        ? (0, workchain_part_01_1.buildFinalSummaryQuality)(input, evidence, terminal, provisionalUserVisibleText, nextAction, { todoPlan, progressCheckpoints })
        : provisionalQuality;
...
function formatMainAgentCompletionReply(options) {
    const original = (0, workchain_part_01_1.sanitizeWorkchainUserText)(options.reply, "", 1200);
    if (options.workchain?.mode === "conversation" && options.includeDetails !== true)
        return original;
    const generic = !original || workchain_part_01_1.GENERIC_COMPLETION_REPLY_PATTERN.test(original);
    ...
    if (!generic && shouldShape && replyAlreadyHasFinalSummaryShape(original))
        return original;
    const headline = (0, workchain_part_01_1.sanitizeWorkchainUserText)(summary.headline, "处理结果已整理，是否已经交付以验收和最终总结为准。", 360);
    const evidenceLines = (0, workchain_part_01_1.narrativeList)(summary.evidence, 6, "处理证据已整理，技术细节已放入技术详情。");
    const verificationLines = (0, workchain_part_01_1.narrativeList)(summary.verification, 6, "验证记录已整理，技术细节已放入技术详情。");
    const verificationStatus = (0, workchain_part_01_1.sanitizeWorkchainUserText)(summary.verification_status, "", 260);
    const reviewLines = [
        ...(0, workchain_part_01_1.narrativeList)(summary.acceptance, 4, "验收结论已整理，技术细节已放入技术详情。").map(item => `验收：${item}`),
        ...(0, workchain_part_01_1.narrativeList)(summary.independent_review || summary.independentReview, 4, "复核结论已整理，技术细节已放入技术详情。").map(item => `复核：${item}`),
        ...(0, workchain_part_01_1.narrativeList)(summary.post_review_spot_check_summary?.rows || summary.postReviewSpotCheckSummary?.rows, 3, "完成前抽查结论已整理。").map(item => `抽查：${item}`),
    ];
    const riskLines = (0, workchain_part_01_1.narrativeList)(summary.risks, 5, "风险信息已整理，技术细节已放入技术详情。");
    const riskStatus = (0, workchain_part_01_1.sanitizeWorkchainUserText)(summary.risk_status, "", 260);
    const qualityFollowup = summary.quality_followup || summary.qualityFollowup || null;
    const qualityMissing = qualityFollowup ? (0, workchain_part_01_1.narrativeList)(qualityFollowup.missing, 5, "交付总结缺少必要内容。") : [];
    const qualityHeadline = qualityFollowup ? (0, workchain_part_01_1.sanitizeWorkchainUserText)(qualityFollowup.headline, "", 320) : "";
    const nextAction = (0, workchain_part_01_1.sanitizeWorkchainUserText)(summary.next_action, "", 260);
...
function runMainAgentWorkchainSelfTest() {
    ...
    const rawLeakQuality = (0, workchain_part_01_1.buildFinalSummaryQuality)({
        surface: "group",
        status: "completed",
        mode: "delegation",
    }, {
    ...
    const protectedFailureCopy = (0, workchain_part_01_1.sanitizeWorkchainUserText)("CCM_AGENT_RECEIPT failed raw payload trace_id=hidden denied");
    const testAgentFailureFallbackCopy = (0, workchain_part_01_1.sanitizeTestAgentFailureText)(null);
    const checks = {
        protectedFailureCopyUsesInvestigationLanguage: protectedFailureCopy.includes("我会继续排查")
            ...
            && !workchain_part_01_1.INTERNAL_TEXT_PATTERN.test(protectedFailureCopy),
        ...
        workchainVisibleProtocolLeakSanitized: !workchain_part_01_1.WORKCHAIN_USER_VISIBLE_PROTOCOL_PATTERN.test(protocolLeak.user_visible_text)
            && !workchain_part_01_1.WORKCHAIN_USER_VISIBLE_PROTOCOL_PATTERN.test(protocolLeakReply)
            && !workchain_part_01_1.WORKCHAIN_USER_VISIBLE_PROTOCOL_PATTERN.test((0, workchain_part_01_1.collectWorkchainVisibleQualityText)([
        ...
        legacyCompletionReplySanitizesVisibleSummary: !workchain_part_01_1.WORKCHAIN_USER_VISIBLE_PROTOCOL_PATTERN.test(legacySummaryReply)
        ...
        noInternalLeakInUserText: !workchain_part_01_1.INTERNAL_TEXT_PATTERN.test(group.user_visible_text),
        ...
        shapedReplyHidesTechnicalBlockers: !workchain_part_01_1.INTERNAL_TEXT_PATTERN.test(shapedReply) && shapedReply.includes("排障信息已放入技术详情"),
        ...
        progressCheckpointsHideRawProtocol: !workchain_part_01_1.INTERNAL_TEXT_PATTERN.test(JSON.stringify(group.progress_checkpoints.items)),
    };
...
//# sourceMappingURL=workchain-part-02.js.map