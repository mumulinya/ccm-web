...
exports.buildMainAgentWorkItemUnlockSummary = buildMainAgentWorkItemUnlockSummary;
exports.buildMainAgentWorkItemClaimSummary = buildMainAgentWorkItemClaimSummary;
...
function workItemVisibleText(value, fallback = "", max = 240) {
    const text = (0, user_facing_text_1.sanitizeMainAgentUserFacingText)(value || fallback);
...
function buildWorkItemDependencySummary(items, nextClaimable) {
    ...
    if (!rows.length && !nextClaimable.length)
        return null;
...
function claimMainAgentWorkItem(items, itemRef, owner, options = {}) {
    ...
    if (options.checkOwnerBusy) {
        ...
        if (busy)
            return { ok: false, reason: "agent_busy", item: target, busy, items };
    }
...
function buildMainAgentWorkItemUnlockSummary(previousItems, nextItems, options = {}) {
    const unlocked = nextItems.filter(item => {
        if (!isWorkItemClaimableByDependency(nextItems, item))
            return false;
...
function buildMainAgentWorkItemClaimSummary(result, owner = "", itemRef = "") {
...
function runMainAgentWorkItemSelfTest() {
    ...
    const checks = {
        ...
        dependencySummaryExplainsUnlockedWork: summary.dependency_summary?.schema === "ccm-main-agent-work-item-dependency-summary-v1"
            ...
            && summary.dependency_summary?.rows?.some((row) => row.label.includes("前置依赖已完成")),
