"use strict";
// delivery-report.ts — merged from 3 part files (behavior-freeze merge).
...
// ===== merged from delivery-report-part-01-part-01.ts =====
...
function deliveryIndependentReviewLabel(value) {
    ...
    if (!status)
        return "";
    if (["failed", "fail", "rejected", "reject", "blocked", "block", "needs_rework", "changes_requested"].includes(status))
        return "未通过";
    if (["passed", "pass", "approved", "approve", "success", "ok", "done", "complete", "completed"].includes(status))
        return "已通过";
    if (["partial", "incomplete", "inconclusive", "unable_to_verify", "skipped", "missing", "pending", "waiting", "required"].includes(status))
        return "待补齐";
    if (["not_required", "none"].includes(status))
        return "未触发";
    return "验收状态无法证明";
}
...
function deliveryIndependentReviewIncompleteText(value) {
    if (deliveryIndependentReviewFailureText(value))
        return false;
    const status = (typeof value === "object" && value ? String(value.status || value.verdict || value.result || "") : String(value || "")).trim().toLowerCase().replace(/[\s-]+/g, "_");
...
// ===== merged from delivery-report-part-01-part-02.ts =====
function collectDeliveryIndependentReview(input, status) {
    ...
    return uniqueDeliveryLines(headline, required === true && reason ? `触发原因：${reason}` : "", evidence.slice(0, 4)).slice(0, 8);
}
function collectDeliveryPostReviewSpotCheck(input) {
    const { report, summary, completion, workchainSummary } = getNestedReport(input);
    const task = input.task || {};
    const gate = firstObject(report.post_review_spot_check_gate, report.postReviewSpotCheckGate, summary.post_review_spot_check_gate, summary.postReviewSpotCheckGate, completion.post_review_spot_check_gate, completion.postReviewSpotCheckGate, workchainSummary.post_review_spot_check_gate, workchainSummary.postReviewSpotCheckGate, task.delivery_summary?.post_review_spot_check_gate, task.delivery_summary?.postReviewSpotCheckGate);
    const spotCheck = firstObject(report.post_review_spot_check, report.postReviewSpotCheck, summary.post_review_spot_check, summary.postReviewSpotCheck, completion.post_review_spot_check, completion.postReviewSpotCheck, workchainSummary.post_review_spot_check, workchainSummary.postReviewSpotCheck, task.delivery_summary?.post_review_spot_check, task.delivery_summary?.postReviewSpotCheck, gate?.latest);
    const visibleSummary = firstObject(report.post_review_spot_check_summary, report.postReviewSpotCheckSummary, summary.post_review_spot_check_summary, summary.postReviewSpotCheckSummary, completion.post_review_spot_check_summary, completion.postReviewSpotCheckSummary, workchainSummary.post_review_spot_check_summary, workchainSummary.postReviewSpotCheckSummary, task.delivery_summary?.post_review_spot_check_summary, task.delivery_summary?.postReviewSpotCheckSummary, gate?.summary, (0, post_review_spot_check_1.buildPostReviewSpotCheckSummary)(spotCheck));
    const required = firstBoolean(report.post_review_spot_check_required, report.postReviewSpotCheckRequired, summary.post_review_spot_check_required, summary.postReviewSpotCheckRequired, completion.post_review_spot_check_required, completion.postReviewSpotCheckRequired, workchainSummary.post_review_spot_check_required, workchainSummary.postReviewSpotCheckRequired, task.delivery_summary?.post_review_spot_check_required, task.delivery_summary?.postReviewSpotCheckRequired, gate?.required, spotCheck?.required);
    if (required !== true && !visibleSummary)
        return [];
    return uniqueDeliveryLines(sanitizeMainAgentDeliveryText(visibleSummary?.headline || gate?.reason || "", required === true ? "TestAgent 通过后的完成前抽查仍在进行。" : "完成前抽查已记录。", 260), ...(Array.isArray(visibleSummary?.rows)
        ? visibleSummary.rows.map((item) => sanitizeMainAgentDeliveryText(item, "抽查状态已记录。", 220))
        : [])).slice(0, 6);
}
function collectDeliveryRisks(input) {
    ...
    const postReviewSpotCheckGate = firstObject(report.post_review_spot_check_gate, report.postReviewSpotCheckGate, summary.post_review_spot_check_gate, summary.postReviewSpotCheckGate, completion.post_review_spot_check_gate, completion.postReviewSpotCheckGate, workchainSummary.post_review_spot_check_gate, workchainSummary.postReviewSpotCheckGate, task.delivery_summary?.post_review_spot_check_gate, task.delivery_summary?.postReviewSpotCheckGate);
    const postReviewSpotCheckRequired = firstBoolean(report.post_review_spot_check_required, summary.post_review_spot_check_required, completion.post_review_spot_check_required, workchainSummary.post_review_spot_check_required, task.delivery_summary?.post_review_spot_check_required, postReviewSpotCheckGate?.required);
    const postReviewSpotCheckPassed = firstBoolean(report.post_review_spot_check_gate_passed, summary.post_review_spot_check_gate_passed, completion.post_review_spot_check_gate_passed, workchainSummary.post_review_spot_check_gate_passed, task.delivery_summary?.post_review_spot_check_gate_passed, postReviewSpotCheckGate?.pass);
...
function formatDeliveryPlanGapItem(item) {
    ...
    const label = sanitizeMainAgentDeliveryText(item.label || item.title || item.name || item.subject || item.step || item.content || item.summary || "", "", 140);
    ...
    const reason = uniqueDeliveryLines(item.reason, item.detail, item.message, item.status_label, item.statusLabel, missing.length ? `缺少：${missing.join("、")}` : "").slice(0, 2).join("；");
    if (label && reason && !label.includes(reason))
        return sanitizeMainAgentDeliveryText(`${label}（${reason}）`, "", 320);
...
function collectDeliveryPlanAlignmentGaps(planAlignment) {
    ...
    return uniqueDeliveryLines(asArray(planAlignment.deviations).map(formatDeliveryPlanGapItem), asArray(planAlignment.gaps).map(formatDeliveryPlanGapItem), asArray(planAlignment.remaining_gaps).map(formatDeliveryPlanGapItem), asArray(planAlignment.remainingGaps).map(formatDeliveryPlanGapItem), asArray(planAlignment.uncovered_steps).map(formatDeliveryPlanGapItem), asArray(planAlignment.uncoveredSteps).map(formatDeliveryPlanGapItem), asArray(planAlignment.failed_checks).map(formatDeliveryPlanGapItem), asArray(planAlignment.failedChecks).map(formatDeliveryPlanGapItem), failedChecks).slice(0, 3);
}
// ===== merged from delivery-report-part-02.ts =====
function collectDeliveryPlanReview(input, status) {
    ...
    const items = uniqueDeliveryLines(modeTitle ? `执行前计划：${modeTitle}` : "", alignmentGaps.length ? `计划缺口：${alignmentGaps.join("；")}` : "", scope.length ? `计划范围：${scope.join("；")}` : "", steps.length ? `计划步骤：${steps.join("；")}` : "", acceptedFeedback.length ? `确认补充要求：${acceptedFeedback.join("；")}` : "", acceptance.length ? `验收标准：${acceptance.join("；")}` : "", alignmentText).slice(0, 7);
...
function collectDeliveryCompleted(input, files, verification, status, risks = []) {
    ...
    if (status !== "done") {
        ...
        return uniqueDeliveryStrings(statusLine, risks.slice(0, 3).map(item => status === "cancelled" ? `停止原因：${item}` : status === "failed" ? `待排查：${item}` : `待确认：${item}`), status === "failed" && files.length ? `已整理 ${files.length} 个文件变更，需继续修复或复核。` : "", status === "failed" && verification.length ? `已整理 ${verification.length} 项验证记录，需继续核对。` : "").slice(0, 6);
    }
...
function buildDeliveryFinalSummaryQuality(status, sections, nextAction, options = {}) {
    ...
    checks.push({
        ...
        label: "普通文本不含内部协议",
        passed: !exports.FINAL_SUMMARY_PROTOCOL_LEAK_PATTERN.test(visibleText),
    });
    ...
    checks.push({
        ...
        label: "交付卡普通文本不含内部协议",
        passed: !exports.FINAL_SUMMARY_PROTOCOL_LEAK_PATTERN.test(visibleCardText),
    });
    if (status !== "done") {
    ...
    if (planGaps.length) {
        const nextActionText = uniqueDeliveryLines(nextAction, sections.find(item => item?.id === "next_action")?.items || []).join("；");
        checks.push({
...
function collectDeliveryPlanReviewGaps(planReview) {
    return uniqueDeliveryLines(planReview
...
function buildDeliveryUserHandoff(input, status, completed, planReview, files, verification, acceptance, independentReview, risks, nextAction) {
    ...
    const actions = [];
    const planGaps = collectDeliveryPlanReviewGaps(planReview);
    const unresolvedItems = uniqueDeliveryLines(planGaps.map(item => `计划缺口：${item}`), risks).slice(0, 8);
...
function compactDeliveryVisibleScope(files) {
    if (!files.length)
        return ["本轮没有检测到代码文件变更。"];
...
function buildCompactDeliveryMarkdown(input) {
    const verificationItems = uniqueDeliveryStrings(input.verification.slice(0, 3), input.independentReview.slice(0, 2).map(item => `TestAgent（独立验收）：${item}`), input.acceptance.slice(0, 1)).slice(0, 5);
...
function formatMainAgentDeliveryReply(report) {
    ...
    if (report.schema === "ccm-main-agent-delivery-report-v1")
        return report.user_text || report.markdown || report.headline || "";
...
function runMainAgentDeliveryReportSelfTest() {
    ...
    const checks = {
        protectedFailureCopyUsesInvestigationLanguage: protectedFailureCopy.includes("我会继续定位")
            ...
            && !exports.INTERNAL_DELIVERY_TEXT_PATTERN.test(protectedFailureCopy),
        ...
        groupHasVerificationEvidenceQuality: group.verification_evidence?.schema === "ccm-main-agent-verification-evidence-v1"
            ...
            && group.verification_evidence?.items?.some((item) => item.includes("独立验证环境") && item.includes("1 项"))
        ...
        legacyProtocolTextSanitized: !exports.INTERNAL_DELIVERY_TEXT_PATTERN.test(legacy.markdown) && legacy.markdown.includes("结果说明") && !legacy.markdown.includes("raw payload"),
        noInternalLeak: !exports.INTERNAL_DELIVERY_TEXT_PATTERN.test(group.markdown) && !exports.INTERNAL_DELIVERY_TEXT_PATTERN.test(global.markdown),
    };
