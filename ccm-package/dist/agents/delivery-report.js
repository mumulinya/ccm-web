...
function collectDeliveryCompleted(input, files, verification, status, risks = []) {
    ...
    if (status !== "done") {
        ...
        return uniqueDeliveryStrings(statusLine, risks.slice(0, 3).map(item => status === "cancelled" ? `停止原因：${item}` : `待处理：${item}`), status === "failed" && files.length ? `已整理 ${files.length} 个文件变更，需继续修复或复核。` : "", status === "failed" && verification.length ? `已整理 ${verification.length} 项验证记录，需继续核对。` : "").slice(0, 6);
    }
