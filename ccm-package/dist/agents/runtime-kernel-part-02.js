...
exports.buildWorkerContextPacket = buildWorkerContextPacket;
exports.renderWorkerContextPacket = renderWorkerContextPacket;
exports.buildContractInjectionEvent = buildContractInjectionEvent;
...
const runtime_kernel_part_01_1 = require("./runtime-kernel-part-01");
function providerOverrideFollowupStringList(value, limit = 8) {
...
function extractPressureProvenanceProviderDispatchOverrideFollowupReceiptContract(memory = {}, fallback = null, advisoryInput = null) {
    const direct = fallback
        ...
        || null;
    const advisory = (0, runtime_kernel_part_01_1.extractPressureProvenanceProviderDispatchAdvisory)(memory, advisoryInput);
    const selected = advisory?.selected_candidate || advisory?.selectedCandidate || advisory || {};
    const dispatchPolicy = String(selected.dispatch_policy
    ...
    if (!active && !direct?.schema)
        return null;
    const relPaths = providerOverrideFollowupStringList(direct?.rel_paths || direct?.relPaths || selected.provider_override_followup_rel_paths || selected.providerOverrideFollowupRelPaths, 12);
    const workItemIds = providerOverrideFollowupStringList(direct?.followup_work_item_ids || direct?.followupWorkItemIds || selected.provider_override_followup_work_item_ids || selected.providerOverrideFollowupWorkItemIds, 12);
    const overrideIds = providerOverrideFollowupStringList(direct?.override_ids || direct?.overrideIds || selected.provider_override_followup_override_ids || selected.providerOverrideFollowupOverrideIds, 12);
...
function renderPressureProvenanceProviderDispatchOverrideFollowupReceiptContract(contract = {}) {
...
function buildWorkerContextMemoryReinjectionProof(packet = {}) {
    ...
    const memoryText = (0, runtime_kernel_part_01_1.renderWorkerPacketMemory)(memory);
    const expectedDeliveryBinding = (0, runtime_kernel_part_01_1.buildWorkerTypedMemoryDeliveryExpectedBinding)(packet, memory);
    const typedMemoryDeliveryCapsule = (0, runtime_kernel_part_01_1.validateWorkerTypedMemoryDeliveryCapsule)(packet?.typed_memory_delivery_capsule
        || packet?.typedMemoryDeliveryCapsule
        || (0, runtime_kernel_part_01_1.extractWorkerTypedMemoryDeliveryCapsule)(memory), { expectedBinding: expectedDeliveryBinding });
    const memoryRawText = memory == null ? "" : (typeof memory === "string" ? memory : JSON.stringify(memory || {}));
    const packetMemoryHash = memoryRawText ? (0, runtime_kernel_part_01_1.hash)(memoryRawText, 24) : "";
    ...
    return {
        ...
        rendered_memory_hash: memoryText ? (0, runtime_kernel_part_01_1.hash)(memoryText, 24) : "",
...
function buildWorkerContextUsage(packet = {}, options = {}) {
    ...
    const memory = packet?.memory || null;
    const memoryPolicy = packet?.memory_policy || packet?.memoryPolicy || normalizeWorkerMemoryPolicy({}, memory);
    const memoryRendered = (0, runtime_kernel_part_01_1.renderWorkerPacketMemory)(memory);
    const expectedDeliveryBinding = (0, runtime_kernel_part_01_1.buildWorkerTypedMemoryDeliveryExpectedBinding)(packet, memory);
    const typedMemoryDeliveryCapsule = (0, runtime_kernel_part_01_1.validateWorkerTypedMemoryDeliveryCapsule)(packet?.typed_memory_delivery_capsule
        || packet?.typedMemoryDeliveryCapsule
        || (0, runtime_kernel_part_01_1.extractWorkerTypedMemoryDeliveryCapsule)(memory), { expectedBinding: expectedDeliveryBinding });
    const pressureMemoryProvenanceReceiptDiscipline = (0, runtime_kernel_part_01_1.extractPressureMemoryProvenanceReceiptDiscipline)(memory, packet?.pressure_memory_provenance_receipt_discipline || packet?.pressureMemoryProvenanceReceiptDiscipline || null);
    const pressureProvenanceDispatchFeedbackPolicy = (0, runtime_kernel_part_01_1.extractPressureProvenanceDispatchFeedbackPolicy)(memory, packet?.pressure_provenance_dispatch_feedback_policy || packet?.pressureProvenanceDispatchFeedbackPolicy || null);
    const pressureProvenanceProviderDispatchAdvisory = (0, runtime_kernel_part_01_1.extractPressureProvenanceProviderDispatchAdvisory)(memory, packet?.pressure_provenance_provider_dispatch_advisory || packet?.pressureProvenanceProviderDispatchAdvisory || null);
    const pressureProvenanceProviderDispatchOverrideFollowupReceiptContract = extractPressureProvenanceProviderDispatchOverrideFollowupReceiptContract(memory, packet?.pressure_provenance_provider_dispatch_override_followup_receipt_contract || packet?.pressureProvenanceProviderDispatchOverrideFollowupReceiptContract || null, pressureProvenanceProviderDispatchAdvisory);
    const providerRankingCompactRepairReceiptMemoryContract = (0, runtime_kernel_part_01_1.extractProviderRankingCompactRepairReceiptMemoryContract)(memory, packet?.provider_ranking_compact_repair_receipt_memory_contract || packet?.providerRankingCompactRepairReceiptMemoryContract || null);
    const postCompactReinjectionRepairReceiptMemoryContract = (0, runtime_kernel_part_01_1.extractPostCompactReinjectionRepairReceiptMemoryContract)(memory, packet?.post_compact_reinjection_repair_receipt_memory_contract || packet?.postCompactReinjectionRepairReceiptMemoryContract || null);
    ...
    const memoryRecallTrustContract = packet?.memory_recall_trust_contract
        ...
        || (0, runtime_kernel_part_01_1.buildWorkerMemoryRecallTrustContract)(memory, memoryPolicy);
    const categories = [
...
function evaluateAgentRuntimePermission(input, rules = runtime_kernel_part_01_1.DEFAULT_PERMISSION_RULES) {
    const matched = [...rules].reverse().find(rule => matchesRule(rule, input));
    const risk = input.risk || "read";
    const fallback = risk === "read"
        ? runtime_kernel_part_01_1.DEFAULT_PERMISSION_RULES[0]
        : risk === "high"
            ? runtime_kernel_part_01_1.DEFAULT_PERMISSION_RULES[2]
...
function buildArtifactBudget(value, maxChars = 12_000) {
    ...
    return {
        ...
        max_chars: maxChars,
        truncated,
        artifact_hash: truncated ? (0, runtime_kernel_part_01_1.hash)(text, 16) : "",
        preview: truncated ? (0, runtime_kernel_part_01_1.compact)(text, maxChars) : text,
    };
}
function recordAgentRuntimeLifecycle(input) {
    ...
    const record = {
        id: `arl_${Date.now().toString(36)}_${(0, runtime_kernel_part_01_1.hash)(input, 8)}`,
...
function buildWorkerContextPacket(input) {
    ...
    const groupMemoryContext = (0, runtime_kernel_part_01_1.workerGroupMemoryContext)(input.memory || null);
    ...
    const taskAgentInvocationLineage = groupMemoryContext.task_agent_invocation_lineage
        ...
        || null;
    const typedMemoryDeliveryExpectedBinding = (0, runtime_kernel_part_01_1.buildWorkerTypedMemoryDeliveryExpectedBinding)(input, input.memory || null);
    const typedMemoryDeliveryCapsule = (0, runtime_kernel_part_01_1.validateWorkerTypedMemoryDeliveryCapsule)((0, runtime_kernel_part_01_1.extractWorkerTypedMemoryDeliveryCapsule)(input.memory || null), { expectedBinding: typedMemoryDeliveryExpectedBinding });
    const memoryRecallTrustContract = (0, runtime_kernel_part_01_1.buildWorkerMemoryRecallTrustContract)(input.memory || null, memoryPolicy, typedMemoryDeliveryCapsule, typedMemoryDeliveryExpectedBinding);
    const pressureMemoryProvenanceReceiptDiscipline = (0, runtime_kernel_part_01_1.extractPressureMemoryProvenanceReceiptDiscipline)(input.memory || null, input.pressureMemoryProvenanceReceiptDiscipline || input.pressure_memory_provenance_receipt_discipline || null);
    const pressureProvenanceDispatchFeedbackPolicy = (0, runtime_kernel_part_01_1.extractPressureProvenanceDispatchFeedbackPolicy)(input.memory || null, input.pressureProvenanceDispatchFeedbackPolicy || input.pressure_provenance_dispatch_feedback_policy || null);
    const pressureProvenanceProviderDispatchAdvisory = (0, runtime_kernel_part_01_1.extractPressureProvenanceProviderDispatchAdvisory)(input.memory || null, input.pressureProvenanceProviderDispatchAdvisory || input.pressure_provenance_provider_dispatch_advisory || null);
    const pressureProvenanceProviderSelectedCandidate = pressureProvenanceProviderDispatchAdvisory?.selected_candidate
        ...
        || {};
    const pressureProvenanceProviderDispatchOverrideFollowupReceiptContract = extractPressureProvenanceProviderDispatchOverrideFollowupReceiptContract(input.memory || null, input.pressureProvenanceProviderDispatchOverrideFollowupReceiptContract
        || input.pressure_provenance_provider_dispatch_override_followup_receipt_contract
        || null, pressureProvenanceProviderDispatchAdvisory);
    const providerRankingCompactRepairReceiptMemoryContract = (0, runtime_kernel_part_01_1.extractProviderRankingCompactRepairReceiptMemoryContract)(input.memory || null, input.providerRankingCompactRepairReceiptMemoryContract
        || input.provider_ranking_compact_repair_receipt_memory_contract
        || null);
    const postCompactReinjectionRepairReceiptMemoryContract = (0, runtime_kernel_part_01_1.extractPostCompactReinjectionRepairReceiptMemoryContract)(input.memory || null, input.postCompactReinjectionRepairReceiptMemoryContract
    ...
    const packet = {
        ...
        contract_injections: contractInjections.map((item) => ({
            injection_id: item.injection_id || item.injectionId || `ci_${(0, runtime_kernel_part_01_1.hash)(item, 12)}`,
...
function renderWorkerContextPacket(packet) {
    const contractLines = Array.isArray(packet?.contract_injections) && packet.contract_injections.length
        ? [
            "contract injection：",
            ...packet.contract_injections.map((item) => `- injection_id=${item.injection_id}；endpoint/type=${item.endpoint || "contract"}；source=${item.source_agent || "unknown"}；${item.summary || ""}`),
    ...
    const cleanupCommitRepairLines = cleanupCommitRepairContext?.brief_count > 0
        ...
        : [];
    const memoryText = (0, runtime_kernel_part_01_1.renderWorkerPacketMemory)(packet?.memory || null);
    const memoryPolicyText = renderWorkerMemoryPolicy(packet?.memory_policy || packet?.memoryPolicy || null);
    const typedMemoryDeliveryExpectedBinding = (0, runtime_kernel_part_01_1.buildWorkerTypedMemoryDeliveryExpectedBinding)(packet, packet?.memory || null);
    ...
    const typedMemoryDeliveryCapsuleText = ["delta", "continuation"].includes(memoryEntryTransport.mode)
        ? ""
        : (0, runtime_kernel_part_01_1.renderWorkerTypedMemoryDeliveryCapsule)(packet?.typed_memory_delivery_capsule
            || packet?.typedMemoryDeliveryCapsule
            || (0, runtime_kernel_part_01_1.extractWorkerTypedMemoryDeliveryCapsule)(packet?.memory || null), typedMemoryDeliveryExpectedBinding);
    ...
    const postTurnSummaryDeliveryCapsuleText = postTurnSummaryDeliveryCapsule?.capsule_checksum
        ...
        : "";
    const memoryRecallTrustContractText = (0, runtime_kernel_part_01_1.renderWorkerMemoryRecallTrustContract)(packet?.memory_recall_trust_contract || packet?.memoryRecallTrustContract || null);
    const pressureMemoryProvenanceReceiptDisciplineText = (0, runtime_kernel_part_01_1.renderPressureMemoryProvenanceReceiptDiscipline)((0, runtime_kernel_part_01_1.extractPressureMemoryProvenanceReceiptDiscipline)(packet?.memory || null, packet?.pressure_memory_provenance_receipt_discipline || packet?.pressureMemoryProvenanceReceiptDiscipline || null));
    const pressureProvenanceDispatchFeedbackPolicyText = (0, runtime_kernel_part_01_1.renderPressureProvenanceDispatchFeedbackPolicy)((0, runtime_kernel_part_01_1.extractPressureProvenanceDispatchFeedbackPolicy)(packet?.memory || null, packet?.pressure_provenance_dispatch_feedback_policy || packet?.pressureProvenanceDispatchFeedbackPolicy || null));
    const pressureProvenanceProviderDispatchAdvisoryText = (0, runtime_kernel_part_01_1.renderPressureProvenanceProviderDispatchAdvisory)((0, runtime_kernel_part_01_1.extractPressureProvenanceProviderDispatchAdvisory)(packet?.memory || null, packet?.pressure_provenance_provider_dispatch_advisory || packet?.pressureProvenanceProviderDispatchAdvisory || null));
    const pressureProvenanceProviderDispatchOverrideFollowupReceiptContractText = renderPressureProvenanceProviderDispatchOverrideFollowupReceiptContract(extractPressureProvenanceProviderDispatchOverrideFollowupReceiptContract(packet?.memory || null, packet?.pressure_provenance_provider_dispatch_override_followup_receipt_contract || packet?.pressureProvenanceProviderDispatchOverrideFollowupReceiptContract || null, packet?.pressure_provenance_provider_dispatch_advisory || packet?.pressureProvenanceProviderDispatchAdvisory || null));
    const providerRankingCompactRepairReceiptMemoryContractText = (0, runtime_kernel_part_01_1.renderProviderRankingCompactRepairReceiptMemoryContract)((0, runtime_kernel_part_01_1.extractProviderRankingCompactRepairReceiptMemoryContract)(packet?.memory || null, packet?.provider_ranking_compact_repair_receipt_memory_contract || packet?.providerRankingCompactRepairReceiptMemoryContract || null));
    const postCompactReinjectionRepairReceiptMemoryContractText = (0, runtime_kernel_part_01_1.renderPostCompactReinjectionRepairReceiptMemoryContract)((0, runtime_kernel_part_01_1.extractPostCompactReinjectionRepairReceiptMemoryContract)(packet?.memory || null, packet?.post_compact_reinjection_repair_receipt_memory_contract || packet?.postCompactReinjectionRepairReceiptMemoryContract || null));
    const providerSwitchDecisionReceiptText = (0, runtime_kernel_part_01_1.renderProviderSwitchDecisionReceipt)(packet?.provider_switch_decision_receipt || packet?.providerSwitchDecisionReceipt || null);
    const retry = packet?.context_compaction_retry || packet?.contextCompactionRetry || null;
...
function buildContractInjectionEvent(input) {
    const injectionId = input.contract?.injection_id || input.contract?.injectionId || `ci_${(0, runtime_kernel_part_01_1.hash)([input.taskId, input.sourceAgent, input.targetAgent, input.contract], 16)}`;
    const event = {
...
function replayAgentTrace(traceId) {
    const trace = (0, reliability_ledger_1.getTrace)(traceId);
    const events = Array.isArray(trace?.events) ? trace.events : [];
    const lifecycle = events.filter((event) => event.type === "agent_runtime.lifecycle");
    const blocked = events.filter((event) => /blocked|confirmation|required|failed|error/i.test(`${event.type} ${event.status}`));
    const tools = events.filter((event) => /tool|dispatch|agent_runtime\.lifecycle/.test(String(event.type || "")));
    const contractInjections = events.filter((event) => event.type === "agent_runtime.contract_injection");
    const ackSignals = events.filter((event) => /ack/i.test(`${event.type} ${event.message} ${JSON.stringify(event.data || {})}`));
    return {
        success: !!trace,
        trace_id: traceId,
...
function buildTraceReplaySuite(limit = 20) {
    const traces = (0, reliability_ledger_1.listTraces)(limit);
    const replays = traces.map((trace) => replayAgentTrace(trace.trace_id));
    const pass = replays.every((item) => item.verdict === "pass");
    return {
        pass,
        total: replays.length,
        needs_review: replays.filter((item) => item.verdict === "needs_review").length,
        replays,
    };
}
//# sourceMappingURL=runtime-kernel-part-02.js.map