"use strict";
// workchain.ts — merged from 3 part files (behavior-freeze merge).
Object.defineProperty(exports, "__esModule", { value: true });
exports.GENERIC_COMPLETION_REPLY_PATTERN = exports.WORKCHAIN_USER_VISIBLE_PROTOCOL_PATTERN = exports.INTERNAL_TEXT_PATTERN = void 0;
exports.compactText = compactText;
exports.compactMultilineText = compactMultilineText;
exports.sanitizeWorkchainTerminology = sanitizeWorkchainTerminology;
exports.stringList = stringList;
exports.narrativeList = narrativeList;
exports.sanitizeTestAgentFailureText = sanitizeTestAgentFailureText;
exports.getIndependentReviewGateState = getIndependentReviewGateState;
exports.getPostReviewSpotCheckState = getPostReviewSpotCheckState;
exports.collectAcceptanceEvidence = collectAcceptanceEvidence;
exports.collectIndependentReviewEvidence = collectIndependentReviewEvidence;
exports.checkpointStatus = checkpointStatus;
exports.normalizeWorkchainTodoSteps = normalizeWorkchainTodoSteps;
exports.buildWorkchainTodoVerificationReminder = buildWorkchainTodoVerificationReminder;
exports.applyQualityFollowupTodoStep = applyQualityFollowupTodoStep;
exports.sanitizeWorkchainUserText = sanitizeWorkchainUserText;
exports.collectCompletionEvidence = collectCompletionEvidence;
exports.hasExecutableWorkEvidence = hasExecutableWorkEvidence;
exports.hasStrongWorkchainVerificationEvidence = hasStrongWorkchainVerificationEvidence;
exports.workchainVerificationFailureText = workchainVerificationFailureText;
exports.isBareWorkchainAcceptanceLine = isBareWorkchainAcceptanceLine;
exports.workchainAcceptanceFailureText = workchainAcceptanceFailureText;
...
function compactMultilineText(value, max = 240) {
...
function collectTestAgentBrowserProviderGapsSummary(input) {
...
function getIndependentReviewGateState(input) {
    ...
    const flakyStabilityGroups = Number(summary?.test_agent_report?.browserStabilitySummary?.statusCounts?.flaky
...
function sanitizeWorkchainUserText(value, fallback = "我正在处理当前请求。", max = 260) {
    ...
    if (!text)
        text = fallback;
    if (exports.INTERNAL_TEXT_PATTERN.test(text)) {
...
function buildMainAgentProgressCheckpoints(input, stages, evidence, options = {}) {
    ...
    for (const event of (input.rawEvents || []).filter(Boolean)) {
        ...
        if (exports.INTERNAL_TEXT_PATTERN.test(raw))
            continue;
        push({
...
function buildFinalSummaryQuality(input, evidence, terminal, headline, nextAction, options = {}) {
    ...
    checks.push({
        ...
        label: "普通文本不含内部协议",
        passed: !exports.WORKCHAIN_USER_VISIBLE_PROTOCOL_PATTERN.test(visibleText),
...
// ===== merged from workchain-part-02.ts =====
...
function buildWorkchainQualityFollowupUserVisibleText(baseText, qualityFollowup) {
    ...
    const base = exports.GENERIC_COMPLETION_REPLY_PATTERN.test(String(baseText || "").trim()) || /已完成|已处理|任务已处理|处理完成/.test(String(baseText || ""))
        ? "任务已有处理结果"
...
function buildUserVisibleText(input, evidence) {
    ...
    if (["completed", "done", "succeeded"].includes(status) || phase === "completed") {
        if (explicit && !exports.GENERIC_COMPLETION_REPLY_PATTERN.test(String(explicit).trim()))
...
function formatMainAgentCompletionReply(options) {
    ...
    if (options.workchain?.mode === "conversation" && options.includeDetails !== true)
        return original;
    const generic = !original || exports.GENERIC_COMPLETION_REPLY_PATTERN.test(original);
...
function runMainAgentWorkchainSelfTest() {
    ...
    const checks = {
        protectedFailureCopyUsesInvestigationLanguage: protectedFailureCopy.includes("我会继续排查")
            ...
            && !exports.INTERNAL_TEXT_PATTERN.test(protectedFailureCopy),
        ...
        workchainVisibleProtocolLeakSanitized: !exports.WORKCHAIN_USER_VISIBLE_PROTOCOL_PATTERN.test(protocolLeak.user_visible_text)
            && !exports.WORKCHAIN_USER_VISIBLE_PROTOCOL_PATTERN.test(protocolLeakReply)
            && !exports.WORKCHAIN_USER_VISIBLE_PROTOCOL_PATTERN.test(collectWorkchainVisibleQualityText([
        ...
        legacyCompletionReplySanitizesVisibleSummary: !exports.WORKCHAIN_USER_VISIBLE_PROTOCOL_PATTERN.test(legacySummaryReply)
        ...
        noInternalLeakInUserText: !exports.INTERNAL_TEXT_PATTERN.test(group.user_visible_text),
        ...
        shapedReplyHidesTechnicalBlockers: !exports.INTERNAL_TEXT_PATTERN.test(shapedReply) && shapedReply.includes("排障信息已放入技术详情"),
        ...
        progressCheckpointsHideRawProtocol: !exports.INTERNAL_TEXT_PATTERN.test(JSON.stringify(group.progress_checkpoints.items)),
    };
