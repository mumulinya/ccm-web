...
const delivery_report_part_01_1 = require("./delivery-report-part-01");
function collectDeliveryPlanReview(input, status) {
    const { report, summary, completion, workchainSummary } = (0, delivery_report_part_01_1.getNestedReport)(input);
    const task = input.task || {};
    const run = input.run || {};
    const plan = (0, delivery_report_part_01_1.firstObject)(report.plan_mode, report.planMode, summary.plan_mode, summary.planMode, completion.plan_mode, completion.planMode, workchainSummary.plan_mode, workchainSummary.planMode, task.plan_mode, task.planMode, run.plan_mode, run.planMode);
    const planAlignment = (0, delivery_report_part_01_1.firstObject)(report.plan_alignment, report.planAlignment, summary.plan_alignment, summary.planAlignment, completion.plan_alignment, completion.planAlignment, workchainSummary.plan_alignment, workchainSummary.planAlignment, task.delivery_summary?.plan_alignment, task.delivery_summary?.planAlignment);
    const todo = (0, delivery_report_part_01_1.firstObject)(report.todo_plan, report.todoPlan, summary.todo_plan, summary.todoPlan, task.todo_plan, task.todoPlan, run.todo_plan, run.todoPlan);
    const modeTitle = (0, delivery_report_part_01_1.sanitizeMainAgentDeliveryText)(plan?.title || plan?.mode_label || plan?.modeLabel || (plan ? "执行前计划" : ""), "", 120);
    const scope = (0, delivery_report_part_01_1.uniqueDeliveryStrings)(plan?.impact_scope?.areas, plan?.impactScope?.areas, plan?.impact_scope?.projects, plan?.impactScope?.projects).slice(0, 3);
    const steps = (0, delivery_report_part_01_1.uniqueDeliveryStrings)(report.plan_steps, report.planSteps, summary.plan_steps, summary.planSteps, (0, delivery_report_part_01_1.asArray)(plan?.steps).map(delivery_report_part_01_1.deliveryPlanStepText), (0, delivery_report_part_01_1.asArray)(todo?.steps).map(delivery_report_part_01_1.deliveryPlanStepText)).slice(0, 4);
    const acceptedFeedback = (0, delivery_report_part_01_1.collectDeliveryPlanAcceptedFeedback)(input, plan);
    const acceptance = (0, delivery_report_part_01_1.uniqueDeliveryStrings)(plan?.acceptance, plan?.acceptance_criteria, plan?.acceptanceCriteria, report.acceptance_criteria, report.acceptanceCriteria, summary.acceptance_criteria, summary.acceptanceCriteria, task.acceptance_criteria, task.acceptanceCriteria, run.acceptance_criteria, run.acceptanceCriteria).slice(0, 3);
    const alignmentGaps = (0, delivery_report_part_01_1.collectDeliveryPlanAlignmentGaps)(planAlignment);
    ...
    const items = (0, delivery_report_part_01_1.uniqueDeliveryLines)(modeTitle ? `执行前计划：${modeTitle}` : "", alignmentGaps.length ? `计划缺口：${alignmentGaps.join("；")}` : "", scope.length ? `计划范围：${scope.join("；")}` : "", steps.length ? `计划步骤：${steps.join("；")}` : "", acceptedFeedback.length ? `确认补充要求：${acceptedFeedback.join("；")}` : "", acceptance.length ? `验收标准：${acceptance.join("；")}` : "", alignmentText).slice(0, 7);
    return items.length ? items : [fallback];
}
function formatDeliveryAcceptanceCheck(item) {
    if (!item || typeof item !== "object")
        return (0, delivery_report_part_01_1.sanitizeMainAgentDeliveryText)(item, "", 260);
    const label = item.label || item.title || item.name || item.id || "验收项";
    const ok = (0, delivery_report_part_01_1.firstBoolean)(item.ok, item.pass, item.passed, item.status);
    const detail = (0, delivery_report_part_01_1.sanitizeMainAgentDeliveryText)(item.detail || item.reason || item.summary || item.message || "", "", 160);
    const prefix = ok === true ? "通过" : ok === false ? "未通过" : "核对";
    return (0, delivery_report_part_01_1.sanitizeMainAgentDeliveryText)(`${prefix}：${label}${detail && !String(label).includes(detail) ? `（${detail}）` : ""}`, "", 260);
}
function collectDeliveryAcceptance(input, status, verification, risks) {
    const { report, summary, completion, workchainSummary } = (0, delivery_report_part_01_1.getNestedReport)(input);
    const task = input.task || {};
    const run = input.run || {};
    const gate = (0, delivery_report_part_01_1.firstObject)(report.acceptance_gate, summary.acceptance_gate, completion.acceptance_gate, workchainSummary.acceptance_gate, task.delivery_summary?.acceptance_gate);
    const planAlignment = (0, delivery_report_part_01_1.firstObject)(report.plan_alignment, report.planAlignment, summary.plan_alignment, summary.planAlignment, completion.plan_alignment, completion.planAlignment, workchainSummary.plan_alignment, workchainSummary.planAlignment, task.delivery_summary?.plan_alignment, task.delivery_summary?.planAlignment);
    const passed = (0, delivery_report_part_01_1.firstBoolean)(report.acceptance_gate_passed, report.acceptance_passed, summary.acceptance_gate_passed, summary.acceptance_passed, completion.acceptance_gate_passed, completion.acceptance_passed, workchainSummary.acceptance_gate_passed, task.delivery_summary?.acceptance_gate_passed, gate?.pass, gate?.passed);
    ...
    const items = [];
    if (passed === true && !(status === "failed" && blockingRisk))
        items.push("最终验收：已通过");
    else if (passed === true && status === "failed" && blockingRisk)
        items.push("最终验收：未通过，仍有待补齐项");
    else if (passed === false)
        items.push("最终验收：未通过，仍有待补齐项");
    else if (status === "done")
        items.push(verification.length && !risks.length ? "最终验收：已完成交付复核" : "最终验收：待核对验证明细");
    else if (status === "failed")
        items.push("最终验收：未通过，原因已整理在未完成原因里");
    else if (status === "cancelled")
        items.push("最终验收：任务已停止，未继续验收");
    else
        items.push("最终验收：仍在等待最终复核");
    const failedChecks = (0, delivery_report_part_01_1.asArray)(gate?.failed_checks || gate?.failedChecks)
        .map((item) => formatDeliveryAcceptanceCheck({ ...(typeof item === "object" ? item : { label: item }), ok: false }));
    const checks = (0, delivery_report_part_01_1.asArray)(gate?.checks).map(formatDeliveryAcceptanceCheck);
    const criteria = (0, delivery_report_part_01_1.uniqueDeliveryStrings)(report.acceptance, report.acceptance_criteria, report.acceptanceCriteria, summary.acceptance, summary.acceptance_criteria, summary.acceptanceCriteria, task.acceptance_criteria, task.acceptanceCriteria, run.acceptance_criteria, run.acceptanceCriteria).slice(0, 3).map(item => `验收标准：${item}`);
    const planStatus = String(planAlignment?.status || "").toLowerCase();
    ...
    return (0, delivery_report_part_01_1.uniqueDeliveryStrings)(items, failedChecks, checks, criteria, planLabel).slice(0, 8);
}
function collectDeliveryCompleted(input, files, verification, status, risks = []) {
    const { report, summary, completion, workchainSummary } = (0, delivery_report_part_01_1.getNestedReport)(input);
    const task = input.task || {};
    if (status !== "done") {
        ...
        return (0, delivery_report_part_01_1.uniqueDeliveryStrings)(statusLine, risks.slice(0, 3).map(item => status === "cancelled" ? `停止原因：${item}` : status === "failed" ? `待排查：${item}` : `待确认：${item}`), status === "failed" && files.length ? `已整理 ${files.length} 个文件变更，需继续修复或复核。` : "", status === "failed" && verification.length ? `已整理 ${verification.length} 项验证记录，需继续核对。` : "").slice(0, 6);
    }
    const headline = (0, delivery_report_part_01_1.sanitizeMainAgentDeliveryText)(report.headline || report.summary || report.user_text || summary.headline || summary.summary || completion.summary || input.detail || task.status_detail || workchainSummary.headline, "", 500);
    const evidence = (0, delivery_report_part_01_1.uniqueDeliveryStrings)(headline, report.completed, report.evidence, summary.completed, summary.evidence, completion.evidence, workchainSummary.evidence).filter(item => item !== headline).slice(0, 6);
...
function collectDeliveryNextAction(input, status, risks, planReview = []) {
    const { report, summary, completion, workchainSummary } = (0, delivery_report_part_01_1.getNestedReport)(input);
    const planGaps = collectDeliveryPlanReviewGaps(planReview);
    if (planGaps.length)
        return [`先补齐计划缺口：${planGaps[0]}`];
    ...
    if (blockingRisk) {
        if (/完成前抽查/i.test(blockingRisk))
            return ["先沿用原复核工作单重新运行 TestAgent，并再次抽查关键验证；结论一致后再给出最终总结。"];
        if (/独立复核.*证据不足/i.test(blockingRisk))
            return ["先补齐独立复核的命令、截图或文件复核证据；证据充分后再给出最终总结。"];
        if (/独立复核.*(未完全|无法确认|部分|待补齐)/i.test(blockingRisk))
            return ["先补齐独立复核无法确认的内容；必要时让原执行成员补充后重新运行 TestAgent/独立复核，再给出最终总结。"];
        if (/独立复核|复核未通过/i.test(blockingRisk))
            return ["先让原执行成员按复核意见返工；修复后重新运行 TestAgent/独立复核，再给出最终总结。"];
        if (/验证失败|失败验证|验证未完成|未完成验证|验证证据不足|无法确认.*验证|必需验证|缺少.*验证/i.test(blockingRisk))
            return ["先补齐失败、未完成、缺失或不足的验证证据；验证通过后再进行最终验收。"];
        return ["先处理验收缺口或阻塞项；处理完成后我会重新验收并总结。"];
    }
    const explicit = (0, delivery_report_part_01_1.sanitizeMainAgentDeliveryText)(report.next_action || summary.next_action || completion.next_action || workchainSummary.next_action, "", 260);
    if (explicit)
        return [explicit];
    if (risks.length && status !== "failed" && status !== "cancelled")
        return ["先处理风险或缺口，我会继续跟进验收。"];
    if (status === "done")
        return ["可以查看改动详情，或继续补充新的要求。"];
    if (status === "failed")
        return ["可以根据上面的风险重新执行，或补充范围后让我继续处理。"];
    if (status === "cancelled")
...
function buildDeliverySection(id, title, items, empty) {
    const normalized = (0, delivery_report_part_01_1.uniqueDeliveryStrings)(items).slice(0, 12);
...
function collectDeliveryVisibleCardText(value, include = false, depth = 0) {
...
function hasFalseDoneVisibleTextForStatus(status, text) {
    if (status === "done")
        return false;
    return /状态[:：]\s*已完成|任务已完成|任务交付完成|本轮处理已经完成|最终验收[:：]\s*已通过|下一步[:：]?\s*可以查看改动详情|可以查看改动详情，或继续补充新的要求/i.test(text);
}
function buildDeliveryFinalSummaryQuality(status, sections, nextAction, options = {}) {
    ...
    checks.push({
        ...
        label: "普通文本不含内部协议",
        passed: !delivery_report_part_01_1.FINAL_SUMMARY_PROTOCOL_LEAK_PATTERN.test(visibleText),
    });
    ...
    checks.push({
        ...
        label: "交付卡普通文本不含内部协议",
        passed: !delivery_report_part_01_1.FINAL_SUMMARY_PROTOCOL_LEAK_PATTERN.test(visibleCardText),
    });
    if (status !== "done") {
    ...
    const planReviewText = (0, delivery_report_part_01_1.uniqueDeliveryLines)(planReviewItems).join("；");
    const acceptedFeedback = (0, delivery_report_part_01_1.uniqueDeliveryLines)(options.acceptedFeedback || []).slice(0, 2);
    ...
    if (planGaps.length) {
        const nextActionText = (0, delivery_report_part_01_1.uniqueDeliveryLines)(nextAction, sections.find(item => item?.id === "next_action")?.items || []).join("；");
        checks.push({
...
function hasConcreteDoneDeliveryEvidence(completed, files, verification, verificationEvidence, independentReview) {
    const meaningfulCompleted = (0, delivery_report_part_01_1.uniqueDeliveryStrings)(completed).some(item => !/交付结果已整理|是否可验收|任务没有完成|任务仍在处理中|任务已停止|当前状态已经整理/i.test(item));
    const verificationReady = verificationEvidence?.status === "ready"
        ...
        || verification.some(item => (0, delivery_report_part_01_1.deliveryVerificationSuccessText)(item) && !(0, delivery_report_part_01_1.deliveryVerificationFailureText)(item));
...
function buildDeliveryPickupSummary(input, status, completed, planReview, files, verification, acceptance, independentReview, risks, nextAction) {
    ...
    const reviewItems = (0, delivery_report_part_01_1.uniqueDeliveryStrings)(risks.slice(0, 4).map(item => `${cancelled ? "停止原因" : "注意"}：${item}`), independentReview.slice(0, 2).map(item => `复核：${item}`), acceptance.slice(0, 2).map(item => `验收：${item}`), planReview.slice(0, 2).map(item => `计划：${item}`), files.slice(0, 2).map(item => `改动：${item}`), verification.slice(0, 2).map(item => `验证：${item}`)).slice(0, 8);
    return {
...
function buildDeliveryCompletionCard(input, status, completed, files, verification, verificationEvidence, acceptance, independentReview, risks, nextAction) {
    ...
    const highlights = (0, delivery_report_part_01_1.uniqueDeliveryStrings)(completed).slice(0, 4);
    ...
    return {
        ...
        status,
        status_label: (0, delivery_report_part_01_1.deliveryStatusLabel)(status),
        headline: highlights[0] || (done ? "交付结果已整理，建议核对验证和验收详情。" : failed ? "任务没有完成，未完成原因已经整理。" : cancelled ? "任务已停止，当前状态已经整理。" : "任务仍在处理中。"),
        metrics: [
            { id: "status", label: "状态", value: (0, delivery_report_part_01_1.deliveryStatusLabel)(status), tone: done ? "success" : failed ? "danger" : cancelled ? "muted" : "warning" },
...
function collectDeliveryPlanReviewGaps(planReview) {
    return (0, delivery_report_part_01_1.uniqueDeliveryLines)(planReview
        .filter(item => /^计划缺口[:：]/.test(String(item || "")))
        .map(item => String(item || "").replace(/^计划缺口[:：]\s*/, "").trim())).slice(0, 5);
}
function buildDeliveryUserHandoff(input, status, completed, planReview, files, verification, acceptance, independentReview, risks, nextAction) {
    ...
    const actions = [];
    const planGaps = collectDeliveryPlanReviewGaps(planReview);
    const unresolvedItems = (0, delivery_report_part_01_1.uniqueDeliveryLines)(planGaps.map(item => `计划缺口：${item}`), risks).slice(0, 8);
    ...
    const addAction = (id, label, detail = "", kind = "", tone = "outline") => {
        ...
        actions.push({
            id,
            label: (0, delivery_report_part_01_1.sanitizeMainAgentDeliveryText)(label, "继续跟进", 80),
            detail: (0, delivery_report_part_01_1.sanitizeMainAgentDeliveryText)(detail || label, "", 220),
            kind: kind || id,
            tone,
        });
    };
    if (failed)
        addAction("retry_or_continue", planGaps.length ? "补齐计划缺口后继续" : "重新执行或继续修复", primaryGap || risks[0] || nextAction[0] || "我会复用已有证据继续处理。", "retry", "primary");
    else if (cancelled)
        addAction("restart_request", "重新发起需求", nextAction[0] || "任务已经停止；需要继续时可以重新发起。", "continue", "primary");
    else if (planGaps.length)
        addAction("review_plan_gaps", "先处理计划缺口", primaryGap, "gap_continue", "warning");
    else if (risks.length)
        addAction("review_risks", "先处理风险与待确认", risks[0], "review_risks", "warning");
    if (files.length)
        addAction("view_changes", "查看改动", `已整理 ${files.length} 个文件变更。`, "view_changes", done && !risks.length && !planGaps.length ? "primary" : "outline");
    ...
    if (status === "waiting")
        addAction("wait_for_summary", "等待最终总结", nextAction[0] || "我会继续协调执行并在完成后总结。", "continue", "outline");
    if (!actions.length)
        addAction("next_action", "继续跟进", nextAction[0] || "我会继续处理并更新结果。", "continue", "primary");
    const handoffStatus = failed ? "failed" : cancelled ? "cancelled" : (planGaps.length || risks.length) ? "needs_attention" : done ? "ready" : "tracking";
    const evidence = (0, delivery_report_part_01_1.uniqueDeliveryStrings)(planReview[0] ? `计划：${planReview[0]}` : "", files.length ? `改动：${files.length} 个文件` : "", verification.length ? `验证：${verification.length} 项已执行` : "", acceptance[0] ? `验收：${acceptance[0]}` : "", independentReview[0] ? `复核：${independentReview[0]}` : "", completed[0] ? `结果：${completed[0]}` : "", planGaps.length ? `计划缺口：${planGaps.length} 项` : "", risks.length ? `待确认：${risks.length} 项` : "").slice(0, 6);
    return {
...
function deliveryUserHandoffSectionItems(handoff) {
    if (!handoff)
        return [];
    return (0, delivery_report_part_01_1.uniqueDeliveryStrings)(handoff.primary_action?.label ? `${handoff.primary_action.label}${handoff.primary_action.detail ? `：${handoff.primary_action.detail}` : ""}` : "", ...(Array.isArray(handoff.secondary_actions) ? handoff.secondary_actions.map((item) => item?.label ? `${item.label}${item.detail ? `：${item.detail}` : ""}` : "") : [])).slice(0, 4);
}
function shouldShowMainAgentDeliveryReport(input) {
    ...
    return actionEvidence.some(value => (0, delivery_report_part_01_1.asArray)(value).length > 0)
        || (0, delivery_report_part_01_1.hasBlockingDeliveryCompletionGap)(input)
        || (0, delivery_report_part_01_1.normalizeDeliveryStatus)(input.status) !== "done";
}
function buildMainAgentDeliveryReport(input) {
    const rawStatus = (0, delivery_report_part_01_1.normalizeDeliveryStatus)(input.status);
    const status = rawStatus === "done" && (0, delivery_report_part_01_1.hasBlockingDeliveryCompletionGap)(input) ? "failed" : rawStatus;
    const title = (0, delivery_report_part_01_1.sanitizeMainAgentDeliveryText)(input.title || input.task?.title || input.run?.original_user_message || input.run?.user_message || input.goal || "本轮任务", "本轮任务", 180);
    const files = (0, delivery_report_part_01_1.collectDeliveryFiles)(input);
    const verification = (0, delivery_report_part_01_1.collectDeliveryVerification)(input);
    const verificationEvidence = (0, delivery_report_part_01_1.collectDeliveryVerificationEvidence)(input, status);
    const risks = (0, delivery_report_part_01_1.collectDeliveryRisks)(input);
    ...
    const independentReview = (0, delivery_report_part_01_1.collectDeliveryIndependentReview)(input, status);
    const postReviewSpotCheck = (0, delivery_report_part_01_1.collectDeliveryPostReviewSpotCheck)(input);
    ...
    const acceptedFeedbackForQuality = (0, delivery_report_part_01_1.collectDeliveryAcceptedFeedbackForQuality)(input);
    ...
    const header = `【${(0, delivery_report_part_01_1.deliveryTitle)(status)}】`;
    const intro = [
        header,
        `任务：${title}`,
        `状态：${(0, delivery_report_part_01_1.deliveryStatusLabel)(status)}`,
    ].join("\n");
...
function runMainAgentDeliveryReportSelfTest() {
    ...
    const protectedFailureCopy = (0, delivery_report_part_01_1.sanitizeMainAgentDeliveryText)("CCM_AGENT_RECEIPT failed raw payload trace_id=hidden denied");
    const checks = {
        protectedFailureCopyUsesInvestigationLanguage: protectedFailureCopy.includes("我会继续定位")
            ...
            && !delivery_report_part_01_1.INTERNAL_DELIVERY_TEXT_PATTERN.test(protectedFailureCopy),
        ...
        groupPlanReviewIncludesAcceptedFeedback: group.plan_review?.some((item) => item.includes("确认补充要求") && item.includes("同步更新列表筛选说明"))
            && group.markdown.includes("确认补充要求：同步更新列表筛选说明"),
        globalPlanReviewIncludesAcceptedFeedback: global.plan_review?.some((item) => item.includes("确认补充要求") && item.includes("上线前保留旧支付入口回滚说明"))
        ...
        groupFinalSummaryQualityRequiresAcceptedFeedback: group.final_summary_quality?.passed === true
            && group.final_summary_quality?.checks?.some((item) => item.id === "plan_accept_feedback_visible" && item.passed === true && item.label === "确认补充要求可见"),
        globalFinalSummaryQualityRequiresAcceptedFeedback: global.final_summary_quality?.passed === true
            && global.final_summary_quality?.checks?.some((item) => item.id === "plan_accept_feedback_visible" && item.passed === true && item.label === "确认补充要求可见"),
        ...
        finalSummaryQualityRequiresVisibleProtocolSanitizer: group.final_summary_quality?.checks?.some((item) => item.id === "user_visible_protocol_sanitized" && item.passed === true)
            && failed.final_summary_quality?.checks?.some((item) => item.id === "user_visible_protocol_sanitized" && item.passed === true)
            && legacy.final_summary_quality?.checks?.some((item) => item.id === "user_visible_protocol_sanitized" && item.passed === true),
        finalSummaryQualityRequiresVisibleCardSanitizer: group.final_summary_quality?.checks?.some((item) => item.id === "user_visible_cards_sanitized" && item.passed === true)
            && failed.final_summary_quality?.checks?.some((item) => item.id === "user_visible_cards_sanitized" && item.passed === true)
            && legacy.final_summary_quality?.checks?.some((item) => item.id === "user_visible_cards_sanitized" && item.passed === true),
        visibleCardQualityGateCatchesProtocolLeaks: structuredLeakQuality.passed === false
            && structuredLeakQuality.checks?.some((item) => item.id === "user_visible_cards_sanitized" && item.passed === false && item.label === "交付卡普通文本不含内部协议"),
        finalSummaryQualityCatchesFalseDoneForFailedStatus: falseDoneFailedQuality.passed === false
            && falseDoneFailedQuality.checks?.some((item) => item.id === "failed_status_false_done_visible" && item.passed === false && item.label === "未完成状态不含已完成口径"),
        ...
        failedHandoffPrioritizesPlanGap: failed.user_handoff?.primary_action?.label === "补齐计划缺口后继续"
            ...
            && failed.user_handoff?.unresolved?.some((item) => item.includes("计划缺口：登录恢复验证通过"))
            && failed.user_handoff?.evidence?.some((item) => item.includes("计划缺口")),
        ...
        doneWithFailedIndependentReviewPrioritizesRework: failedIndependentReviewDone.status === "failed"
            ...
            && failedIndependentReviewDone.independent_review?.some((item) => item.includes("未通过"))
            && failedIndependentReviewDone.risks?.some((item) => item.includes("独立复核"))
            ...
            && failedIndependentReviewDone.completion_card?.metrics?.some((item) => item.id === "status" && item.value === "未完成")
        ...
        failedIndependentReviewEvidenceOnlyDonePrioritizesRework: failedIndependentReviewEvidenceOnlyDone.status === "failed"
            ...
            && failedIndependentReviewEvidenceOnlyDone.independent_review?.some((item) => item.includes("未通过"))
            && failedIndependentReviewEvidenceOnlyDone.risks?.some((item) => item.includes("独立复核未通过"))
            ...
            && failedIndependentReviewEvidenceOnlyDone.completion_card?.metrics?.some((item) => item.id === "status" && item.value === "未完成")
        ...
        failedVerificationResultDoneBlocksCompletion: failedVerificationResultDone.status === "failed"
            ...
            && failedVerificationResultDone.verification_evidence?.items?.some((item) => item.includes("失败验证"))
            && failedVerificationResultDone.risks?.some((item) => item.includes("验证失败"))
            && failedVerificationResultDone.acceptance?.some((item) => item.includes("未通过"))
            && failedVerificationResultDone.next_action?.includes("补齐失败、未完成、缺失或不足的验证证据")
            && failedVerificationResultDone.completion_card?.metrics?.some((item) => item.id === "status" && item.value === "未完成")
        ...
        failedDeliveryPrimarySummaryAvoidsOptimisticHeadline: failedVerificationResultDone.headline?.includes("任务没有完成")
            && failedVerificationResultDone.sections?.find((item) => item.id === "completed")?.items?.[0]?.includes("任务没有完成")
            && !failedVerificationResultDone.sections?.find((item) => item.id === "completed")?.items?.[0]?.includes("已调整"),
        partialIndependentReviewDoneBlocksCompletion: partialIndependentReviewDone.status === "failed"
            ...
            && partialIndependentReviewDone.independent_review?.some((item) => item.includes("部分完成"))
            && partialIndependentReviewDone.risks?.some((item) => item.includes("独立复核未完全确认"))
        ...
        weakPassedIndependentReviewDoneBlocksCompletion: weakPassedIndependentReviewDone.status === "failed"
            ...
            && weakPassedIndependentReviewDone.independent_review?.some((item) => item.includes("证据仍需补齐"))
            && weakPassedIndependentReviewDone.risks?.some((item) => item.includes("独立复核证据不足"))
            && weakPassedIndependentReviewDone.next_action?.includes("补齐独立复核的命令")
            && weakPassedIndependentReviewDone.completion_card?.metrics?.some((item) => item.id === "status" && item.value === "未完成")
        ...
        weakPassedReviewPrimarySummaryAvoidsOptimisticHeadline: weakPassedIndependentReviewDone.headline?.includes("任务没有完成")
            && weakPassedIndependentReviewDone.sections?.find((item) => item.id === "completed")?.items?.[0]?.includes("任务没有完成")
            && !weakPassedIndependentReviewDone.sections?.find((item) => item.id === "completed")?.items?.[0]?.includes("已调整"),
        incompleteVerificationResultDoneBlocksCompletion: incompleteVerificationResultDone.status === "failed"
            ...
            && incompleteVerificationResultDone.verification_evidence?.items?.some((item) => item.includes("未完成验证"))
            && incompleteVerificationResultDone.risks?.some((item) => item.includes("验证未完成"))
        ...
        noVerificationEvidenceDoneBlocksCompletion: noVerificationEvidenceDone.status === "failed"
            ...
            && noVerificationEvidenceDone.verification_evidence?.items?.some((item) => item.includes("验证证据不足"))
            && noVerificationEvidenceDone.risks?.some((item) => item.includes("验证证据不足"))
            && noVerificationEvidenceDone.acceptance?.some((item) => item.includes("未通过"))
        ...
        passedPostReviewSpotCheckAllowsCompletion: passedPostReviewSpotCheckDone.status === "done"
            && passedPostReviewSpotCheckDone.post_review_spot_check?.some((item) => item.includes("2 项结果一致"))
        ...
        failedPostReviewSpotCheckDoneBlocksCompletion: failedPostReviewSpotCheckDone.status === "failed"
            ...
            && failedPostReviewSpotCheckDone.post_review_spot_check?.some((item) => item.includes("1 项不一致"))
            && failedPostReviewSpotCheckDone.risks?.some((item) => item.includes("完成前抽查尚未通过"))
        ...
        bareDoneQualityRequiresEvidence: bareDone.status === "done"
            ...
            && bareDone.final_summary_quality?.checks?.some((item) => item.id === "done_evidence_present" && item.passed === false)
            && bareDone.headline?.includes("是否可验收")
            && bareDone.sections?.find((item) => item.id === "completed")?.items?.[0]?.includes("是否可验收")
        ...
        failedFinalSummaryQualityRequiresPlanGapNextAction: failed.final_summary_quality?.passed === true
            && failed.final_summary_quality?.checks?.some((item) => item.id === "plan_gap_next_action" && item.passed === true && item.label === "计划缺口下一步"),
        ...
        legacyProtocolTextSanitized: !delivery_report_part_01_1.INTERNAL_DELIVERY_TEXT_PATTERN.test(legacy.markdown) && legacy.markdown.includes("结果说明") && !legacy.markdown.includes("raw payload"),
        noInternalLeak: !delivery_report_part_01_1.INTERNAL_DELIVERY_TEXT_PATTERN.test(group.markdown) && !delivery_report_part_01_1.INTERNAL_DELIVERY_TEXT_PATTERN.test(global.markdown),
    };
...
//# sourceMappingURL=delivery-report-part-02.js.map