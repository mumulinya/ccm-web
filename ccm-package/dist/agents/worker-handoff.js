...
function renderMemoryContextForWorker(memory) {
    ...
    if (!text && !invokedSkillAttachmentText && !planAttachmentText && !dynamicContextDeltaText)
        return "";
...
function renderSelfContainedWorkerHandoff(handoff) {
    ...
    return [
        ...
        ...(handoff?.references?.constraints || []).map((item) => `- 用户约束：${compact(item, 260)}`),
        contractInjections,
        ...
        ...(handoff?.verification?.hints || []).map((item) => `- 推荐验证：${item}`),
        ...(handoff?.verification?.acceptance || []).map((item) => `- 验收标准：${item}`),
        "",
        "ACK gate：",
