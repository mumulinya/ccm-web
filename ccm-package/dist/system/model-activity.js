...
function modelActivityDefaultLabel(phase) {
    if (phase === "tool_result_review")
        return "已取得检查结果，正在归纳关键结论";
    if (phase === "verification")
        return "验证结果已返回，正在核对交付条件";
    if (phase === "final_synthesis")
        return "执行结果已收口，正在整理最终结论";
    if (phase === "tool_decision")
        return "正在确定下一步需要核对的项目信息";
...
function createModelActivityController(input) {
    ...
    return {
        ...
        updateLabel(label) {
            if (!stopped && ["waiting", "retrying"].includes(state))
                publish(state, { label });
        },
