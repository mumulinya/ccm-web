...
exports.reviewSessionSummaryIfSelected = reviewSessionSummaryIfSelected;
exports.runSessionSummarySecondaryReviewSelfTest = runSessionSummarySecondaryReviewSelfTest;
...
function selected(config, identity) {
    if (config?.summaryReviewerEnabled !== true)
        return false;
...
async function reviewSessionSummaryIfSelected(input) {
    const identity = {
        scope: input.scope,
    ...
    if (!selected(input.config, identity))
        return {
    ...
    if (!reviewer.apiUrl || !reviewer.apiKey || !reviewer.model) {
        const error = new Error("摘要双模型抽检已命中，但复核模型配置不完整");
    ...
    const invoke = input.modelCall || (async (config, options) => {
        if (config.format === "anthropic-compatible")
            return (0, group_orchestrator_llm_client_1.callAnthropicCompatibleJson)(config, options);
        if (config.format === "gemini-compatible")
            return (0, group_orchestrator_llm_client_1.callGeminiCompatibleJson)(config, options);
        return (0, group_orchestrator_llm_client_1.callOpenAiCompatibleJson)(config, options);
    });
    ...
    if (!passed) {
        const error = new Error(`摘要独立抽检未通过：${receipt.reason || "发现遗漏或幻觉"}`);
...
async function runSessionSummarySecondaryReviewSelfTest() {
    let calls = 0;
...
//# sourceMappingURL=session-summary-secondary-review.js.map