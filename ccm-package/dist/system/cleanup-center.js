...
function cleanupExpiredPreviews() {
    ...
    for (const [token, snapshot] of previewSnapshots.entries()) {
        if (snapshot.expiresAt <= now || snapshot.consumed)
            previewSnapshots.delete(token);
    }
}
...
function getStorageStats() {
    const executionRoot = path.join(utils_1.CCM_DIR, "execution-kernel");
    ...
    const projectSessions = countFilesAndBytes(path.join(utils_1.CCM_DIR, "web-sessions"));
    const groupMessages = countFilesAndBytes(utils_1.GROUP_MESSAGES_DIR);
...
function runCleanupAction(action, options = {}) {
    ...
    try {
        return (0, atomic_json_file_1.withFileLock)(CLEANUP_LOCK_FILE, () => {
            cleanupExpiredPreviews();
            ...
            if (requestedIds.some(id => !availableIds.has(id)))
                return { success: false, error: "选择内容不属于本次预览，请重新预览" };
            ...
            if (action === "purge_archived_cron") {
                ...
                (0, db_1.saveCronJobs)(jobs.filter((job) => !selected.has(String(job.id || ""))));
                for (const id of requestedIds)
