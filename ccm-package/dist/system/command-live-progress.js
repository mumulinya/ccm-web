...
exports.sanitizeCommandLiveOutputForSelfTest = sanitizeCommandLiveOutputForSelfTest;
exports.createCommandLiveProgress = createCommandLiveProgress;
exports.getCommandLiveTail = getCommandLiveTail;
...
function cleanLine(value) {
    ...
    if (!line || SOURCE_LIKE.test(line) || /-----BEGIN|bearer\s+[A-Za-z0-9._-]+/i.test(line))
        return "";
    return line.slice(0, 320);
}
function progressFrom(lines, description) {
    ...
    if (test) {
        ...
        return { phase: "testing", safeSummary: `已执行 ${count} 项测试${failed ? "，当前存在未通过项" : "，正在继续验证"}`, completed: count };
    }
    if (module) {
        const count = Number(module[1] || 0);
        return { phase: "building", safeSummary: `已编译 ${count} 个模块，正在生成构建结果`, completed: count };
    }
    if (percent) {
        const completed = Math.min(100, Number(percent[1] || 0));
        return { phase: /test|测试/i.test(description) ? "testing" : "building", safeSummary: `${description}正在进行 · ${completed}%`, completed, total: 100 };
    }
    return { phase: /test|测试|验收/i.test(description) ? "testing" : /build|compile|构建|编译/i.test(description) ? "building" : "running", safeSummary: `${description}仍在运行` };
}
function sanitizeCommandLiveOutputForSelfTest(input, description = "运行项目构建") {
    const lines = String(input || "").split(/\r?\n/).map(cleanLine).filter(Boolean).slice(-20);
    return { lines, progress: progressFrom(lines, description), contentStored: false };
}
function createCommandLiveProgress(identity) {
    const entry = { identity, lines: [], lastSummary: "", lastPublishedAt: 0, startedAt: Date.now(), finished: false };
    ...
    entry.feishuTimer = setTimeout(() => {
        ...
        void Promise.resolve().then(() => __importStar(require("../modules/collaboration/feishu-channel"))).then(({ notifyFeishuTaskStage }) => notifyFeishuTaskStage({
            stage: "long_running",
            title: "任务仍在执行",
            markdown: summary,
    ...
    const observe = (chunk) => {
        ...
        const now = Date.now();
        if (progress.safeSummary === entry.lastSummary || now - entry.lastPublishedAt < 1000)
            return;
    ...
    const finish = (status) => {
        entry.finished = true;
        if (entry.feishuTimer)
            clearTimeout(entry.feishuTimer);
...
function getCommandLiveTail(commandRunId) {
...
//# sourceMappingURL=command-live-progress.js.map