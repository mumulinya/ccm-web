...
function handleCodeIntelligenceApi(pathname, req, res) {
    ...
    if (projectAction && req.method === "POST") {
        ...
        body(req).then(input => {
            try {
                const result = (0, code_intelligence_1.startCodeIntelligenceIndexRun)(decodeURIComponent(projectAction[1]), projectAction[2], String(input?.reason || ""));
    ...
    if (repairMatch && req.method === "POST") {
        ...
        body(req).then(input => {
            try {
                (0, utils_1.sendJson)(res, { success: true, accepted: true, run: (0, code_intelligence_1.startCodeIntelligenceIndexRun)(decodeURIComponent(repairMatch[1]), "repair", String(input?.reason || "管理员修复代码智能索引")), contentStored: false }, 202);
            }
            catch (error) {
                (0, utils_1.sendJson)(res, { success: false, error: String(error?.message || error) }, 400);
            }
