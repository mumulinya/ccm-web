...
exports.CCM_COMPLETION_SUMMARY_SCHEMA = void 0;
exports.buildCcmCompletionSummary = buildCcmCompletionSummary;
exports.CCM_COMPLETION_SUMMARY_SCHEMA = "ccm-completion-summary-v1";
const MAX_TEXT = 240;
...
function safeText(value, max = MAX_TEXT) {
    ...
    if (!text || SECRET_PATTERN.test(text) || INTERNAL_PATTERN.test(text))
        return "";
    return text.slice(0, max);
}
function list(value, max = 12) {
    const source = Array.isArray(value) ? value : value == null ? [] : [value];
    return [...new Set(source.map(item => safeText(typeof item === "object" ? item?.label || item?.title || item?.message : item, 180)).filter(Boolean))].slice(0, max);
}
function numeric(value) {
    const result = Number(value);
    return Number.isFinite(result) && result >= 0 ? Math.floor(result) : 0;
}
function verificationCounts(value) {
    ...
    for (const row of rows) {
        const ok = typeof row === "boolean"
            ...
            : row?.ok === true
                || row?.passed === true
                || ["passed", "pass", "success", "completed", "done"].includes(String(row?.status || "").toLowerCase());
        const bad = row?.ok === false
            || row?.passed === false
            || row?.failed === true
            || ["failed", "fail", "blocked", "error", "cancelled"].includes(String(row?.status || "").toLowerCase());
        if (ok)
            passed += 1;
        else if (bad)
            failed += 1;
...
function normalizeStatus(input, gatePassed) {
    ...
    if (raw === "interrupted")
        return "interrupted";
    if (raw === "waiting" || raw === "clarify")
        return "waiting";
    return "failed";
}
function buildCcmCompletionSummary(input = {}) {
    const source = input?.source === "terminal_gate" ? "terminal_gate" : "query_projection";
    ...
    const files = Array.isArray(fileInput)
        ? fileInput
        : [];
    const filePaths = new Set(files.map((file) => safeText(typeof file === "string" ? file : file?.path || file?.file, 300)).filter(Boolean));
    const counts = input?.schema === exports.CCM_COMPLETION_SUMMARY_SCHEMA
    ...
    const additions = files.reduce((sum, file) => sum + numeric(file?.additions), 0);
    const deletions = files.reduce((sum, file) => sum + numeric(file?.deletions), 0);
    return {
        schema: exports.CCM_COMPLETION_SUMMARY_SCHEMA,
        status,
        headline,
        ...(detail && detail !== headline ? { detail } : {}),
        filesChanged: input?.schema === exports.CCM_COMPLETION_SUMMARY_SCHEMA ? numeric(input?.filesChanged) : filePaths.size || files.length,
        ...((input?.schema === exports.CCM_COMPLETION_SUMMARY_SCHEMA ? numeric(input?.additions) : additions) ? { additions: input?.schema === exports.CCM_COMPLETION_SUMMARY_SCHEMA ? numeric(input?.additions) : additions } : {}),
        ...((input?.schema === exports.CCM_COMPLETION_SUMMARY_SCHEMA ? numeric(input?.deletions) : deletions) ? { deletions: input?.schema === exports.CCM_COMPLETION_SUMMARY_SCHEMA ? numeric(input?.deletions) : deletions } : {}),
