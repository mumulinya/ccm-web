...
exports.LIVE_PROVIDER_MEMORY_WAVE_APPROVAL_SCHEMA = void 0;
exports.buildLiveProviderMemoryWaveApprovalPreview = buildLiveProviderMemoryWaveApprovalPreview;
exports.buildLiveProviderMemoryVersionTransitionCanaryPreview = buildLiveProviderMemoryVersionTransitionCanaryPreview;
exports.buildLiveProviderInitialMemoryBaselineCanaryPreview = buildLiveProviderInitialMemoryBaselineCanaryPreview;
exports.verifyLiveProviderMemoryWaveApprovalReceipt = verifyLiveProviderMemoryWaveApprovalReceipt;
exports.createLiveProviderMemoryWaveApproval = createLiveProviderMemoryWaveApproval;
exports.createLiveProviderMemoryVersionTransitionCanaryApproval = createLiveProviderMemoryVersionTransitionCanaryApproval;
exports.createLiveProviderInitialMemoryBaselineCanaryApproval = createLiveProviderInitialMemoryBaselineCanaryApproval;
exports.readLiveProviderMemoryWaveApproval = readLiveProviderMemoryWaveApproval;
exports.revokeLiveProviderMemoryWaveApproval = revokeLiveProviderMemoryWaveApproval;
exports.claimLiveProviderMemoryWaveApproval = claimLiveProviderMemoryWaveApproval;
exports.executeApprovedLiveProviderMemoryWave = executeApprovedLiveProviderMemoryWave;
exports.reconcileLiveProviderMemoryWaveApprovals = reconcileLiveProviderMemoryWaveApprovals;
...
const live_provider_memory_endurance_1 = require("./live-provider-memory-endurance");
const live_provider_memory_soak_report_store_1 = require("./live-provider-memory-soak-report-store");
exports.LIVE_PROVIDER_MEMORY_WAVE_APPROVAL_SCHEMA = "ccm-live-provider-memory-wave-approval-v1";
const APPROVAL_DIR = path.join(utils_1.CCM_DIR, "reliability", "live-provider-memory-wave-approvals");
const APPROVAL_SECRET_FILE = path.join(utils_1.CCM_DIR, "reliability", ".live-provider-memory-wave-approval-secret");
const APPROVAL_RETENTION_AUDIT_FILE = path.join(utils_1.CCM_DIR, "reliability", "live-provider-memory-wave-approval-retention-audit.jsonl");
...
function ensureSecret() {
    ...
    if (!/^[a-f0-9]{64}$/.test(secret))
        throw new Error("live memory wave approval secret is invalid");
...
function receiptSignature(checksum) {
    return crypto.createHmac("sha256", ensureSecret()).update(checksum).digest("base64url");
}
function signatureMatches(checksum, supplied) {
...
function approvalFile(receiptId) {
    if (!/^lpmwa_[a-f0-9]{28}$/.test(receiptId))
        throw new Error("live memory wave approval id is invalid");
...
function normalizePlan(report) {
    ...
    const providerVersions = Array.from(new Set((Array.isArray(advised.providerVersions) ? advised.providerVersions : latestEpoch.providerVersions || []).map((value) => safeCode(value, "")).filter(Boolean)));
...
function defaultProviderModel(provider, requested = "") {
...
function buildLiveProviderMemoryWaveApprovalPreview(options = {}) {
    const operation = () => {
        const requestedProvider = safeCode(options.provider, "");
        const report = (0, live_provider_memory_endurance_1.readLatestLiveProviderMemoryEnduranceReport)({ provider: options.provider });
        const reportValid = !!report && (0, live_provider_memory_soak_report_store_1.verifyLiveProviderMemorySoakReport)(report, "endurance").valid;
        const plan = normalizePlan(report);
        const runtime = plan.provider || requestedProvider ? (0, runtime_1.captureAgentRuntimeVersionSnapshot)(plan.provider || requestedProvider) : null;
    ...
    return options.reportSetLockHeld === true ? operation() : (0, live_provider_memory_soak_report_store_1.withLiveProviderMemorySoakReportSetLock)(operation);
}
function buildLiveProviderMemoryVersionTransitionCanaryPreview(options = {}) {
    const operation = () => {
        const requestedProvider = safeCode(options.provider, "");
        const report = (0, live_provider_memory_endurance_1.readLatestLiveProviderMemoryEnduranceReport)({ provider: options.provider });
        const reportValid = !!report && (0, live_provider_memory_soak_report_store_1.verifyLiveProviderMemorySoakReport)(report, "endurance").valid;
        ...
        const runtime = selectedProvider ? (0, runtime_1.captureAgentRuntimeVersionSnapshot)(selectedProvider) : null;
...
function buildLiveProviderInitialMemoryBaselineCanaryPreview(options = {}) {
    const operation = () => {
        const provider = safeCode(options.provider, "");
        const report = provider ? (0, live_provider_memory_endurance_1.readLatestLiveProviderMemoryEnduranceReport)({ provider }) : null;
        const reportValid = !!report && (0, live_provider_memory_soak_report_store_1.verifyLiveProviderMemorySoakReport)(report, "endurance").valid;
        const runtime = provider ? (0, runtime_1.captureAgentRuntimeVersionSnapshot)(provider) : null;
        const runtimeReady = runtime?.status === "ok"
            && !!safeCode(runtime.semanticVersion, "")
            && /^[a-f0-9]{64}$/.test(String(runtime.executableIdentityChecksum || ""));
...
function approvalMode(receipt) {
    return String(receipt?.approvalMode || receipt?.approval_mode || "endurance_wave");
}
function currentPreviewForReceipt(receipt, options = {}) {
    const scopedOptions = { ...options, provider: String(receipt?.plan?.provider || "") };
    if (approvalMode(receipt) === "version_transition_canary")
        return buildLiveProviderMemoryVersionTransitionCanaryPreview(scopedOptions);
    if (approvalMode(receipt) === "initial_provider_baseline_canary")
        return buildLiveProviderInitialMemoryBaselineCanaryPreview({ ...scopedOptions, model: receipt?.plan?.model });
    return buildLiveProviderMemoryWaveApprovalPreview(scopedOptions);
}
function verifyLiveProviderMemoryWaveApprovalReceipt(receipt, options = {}) {
    const issues = [];
    if (receipt?.schema !== exports.LIVE_PROVIDER_MEMORY_WAVE_APPROVAL_SCHEMA || Number(receipt?.version || 0) !== 1)
        issues.push("approval_schema_invalid");
    const expectedChecksum = digest(receiptCore(receipt));
    if (String(receipt?.receiptChecksum || "") !== expectedChecksum)
        issues.push("approval_checksum_invalid");
    if (!signatureMatches(expectedChecksum, receipt?.receiptSignature))
        issues.push("approval_signature_invalid");
    if (String(receipt?.planChecksum || "") !== digest(receipt?.plan || {}))
        issues.push("approval_plan_checksum_invalid");
    if (receipt?.approved !== true || receipt?.explicitApproval !== true || receipt?.riskAccepted !== true || receipt?.singleUse !== true || receipt?.allowLiveProviderExecution !== true)
        issues.push("approval_policy_invalid");
    if (!new Set(["endurance_wave", "version_transition_canary", "initial_provider_baseline_canary"]).has(approvalMode(receipt)))
        issues.push("approval_mode_invalid");
    if (approvalMode(receipt) === "version_transition_canary"
        && (receipt?.transitionAcknowledged !== true || receipt?.canaryOnly !== true || receipt?.evidencePromotionRequired !== true
            ...
            || receipt?.plan?.approvalMode !== "version_transition_canary" || receipt?.plan?.canaryOnly !== true))
        issues.push("transition_canary_policy_invalid");
    if (approvalMode(receipt) === "initial_provider_baseline_canary"
        && (receipt?.initialBaselineAcknowledged !== true || receipt?.canaryOnly !== true || receipt?.evidencePromotionRequired !== true
            ...
            || String(receipt?.baselineAbsenceChecksum || "") !== String(receipt?.plan?.baselineAbsenceChecksum || "")))
        issues.push("initial_baseline_canary_policy_invalid");
    if (!safeCode(receipt?.approvedBy, ""))
        issues.push("approval_actor_invalid");
    const atMs = Number(options.atMs || Date.now());
    const expiresMs = Date.parse(String(receipt?.expiresAt || ""));
    if (!Number.isFinite(expiresMs) || atMs > expiresMs)
        issues.push("approval_expired");
    if (receipt?.revoked === true)
        issues.push("approval_revoked");
    if (receipt?.consumed === true || !["approved"].includes(String(receipt?.status || "")))
        issues.push("approval_already_consumed");
    if (options.requireCurrent === true) {
        ...
        if (!preview.approvable)
            issues.push("approval_current_plan_not_approvable");
        if (String(receipt?.enduranceReportChecksum || "") !== preview.enduranceReportChecksum)
            issues.push("approval_endurance_report_stale");
        if (String(receipt?.sourceSetChecksum || "") !== preview.sourceSetChecksum)
            issues.push("approval_source_set_stale");
        if (String(receipt?.providerVersionKey || "") !== preview.providerVersionKey)
            issues.push("approval_provider_version_stale");
        if (String(receipt?.planChecksum || "") !== preview.planChecksum)
            issues.push("approval_plan_stale");
    }
    return { valid: issues.length === 0, issues, expectedChecksum };
}
function assertNoActiveApprovalForPlan(planChecksum, createdAtMs) {
    try {
        if (!fs.existsSync(APPROVAL_DIR))
            return;
        for (const entry of fs.readdirSync(APPROVAL_DIR, { withFileTypes: true })) {
            if (!entry.isFile() || !/^lpmwa_[a-f0-9]{28}\.json$/.test(entry.name))
                continue;
            const existing = (0, atomic_json_file_1.readJsonWithBackup)(path.join(APPROVAL_DIR, entry.name), null);
            if (!existing || existing.planChecksum !== planChecksum || existing.status !== "approved" || existing.consumed === true || existing.revoked === true)
                continue;
            const verification = verifyLiveProviderMemoryWaveApprovalReceipt(existing, { atMs: createdAtMs });
            if (verification.valid)
                throw new Error("live memory wave plan already has an active approval");
        }
    }
    catch (error) {
        if (String(error?.message || error).includes("already has an active approval"))
            throw error;
    }
}
function createLiveProviderMemoryWaveApproval(input = {}) {
    if (input.explicitApproval !== true && input.explicit_approval !== true)
        throw new Error("live memory wave approval requires explicitApproval=true");
    if (input.riskAccepted !== true && input.risk_accepted !== true)
        throw new Error("live memory wave approval requires riskAccepted=true");
    const approvedBy = safeCode(input.approvedBy || input.approved_by, "");
    if (!approvedBy)
        throw new Error("live memory wave approval requires approvedBy");
    return (0, live_provider_memory_soak_report_store_1.withLiveProviderMemorySoakReportSetLock)(() => {
        const preview = buildLiveProviderMemoryWaveApprovalPreview({ reportSetLockHeld: true, provider: input.provider });
        if (!preview.approvable)
            throw new Error("live memory wave plan is not currently approvable");
        if (String(input.enduranceReportChecksum || input.endurance_report_checksum || "") !== preview.enduranceReportChecksum)
            throw new Error("live memory wave endurance report confirmation mismatch");
        if (String(input.planChecksum || input.plan_checksum || "") !== preview.planChecksum)
            throw new Error("live memory wave plan confirmation mismatch");
        ...
        const receipt = sealReceipt({
            schema: exports.LIVE_PROVIDER_MEMORY_WAVE_APPROVAL_SCHEMA,
            version: 1,
            receiptId,
        ...
        (0, atomic_json_file_1.writeJsonAtomic)(approvalFile(receiptId), receipt);
        return receipt;
    });
}
function createLiveProviderMemoryVersionTransitionCanaryApproval(input = {}) {
    if (input.explicitApproval !== true && input.explicit_approval !== true)
        throw new Error("transition canary approval requires explicitApproval=true");
    if (input.riskAccepted !== true && input.risk_accepted !== true)
        throw new Error("transition canary approval requires riskAccepted=true");
    if (input.transitionAcknowledged !== true && input.transition_acknowledged !== true)
        throw new Error("transition canary approval requires transitionAcknowledged=true");
    const approvedBy = safeCode(input.approvedBy || input.approved_by, "");
    if (!approvedBy)
        throw new Error("transition canary approval requires approvedBy");
    return (0, live_provider_memory_soak_report_store_1.withLiveProviderMemorySoakReportSetLock)(() => {
        const preview = buildLiveProviderMemoryVersionTransitionCanaryPreview({ reportSetLockHeld: true, provider: input.provider });
        if (!preview.approvable)
            throw new Error("version transition canary is not currently approvable");
        if (String(input.enduranceReportChecksum || input.endurance_report_checksum || "") !== preview.enduranceReportChecksum)
            throw new Error("transition canary baseline report confirmation mismatch");
        if (String(input.planChecksum || input.plan_checksum || "") !== preview.planChecksum)
            throw new Error("transition canary plan confirmation mismatch");
        const createdAtMs = Number(input.atMs || Date.now());
        assertNoActiveApprovalForPlan(preview.planChecksum, createdAtMs);
...
function createLiveProviderInitialMemoryBaselineCanaryApproval(input = {}) {
    if (input.explicitApproval !== true && input.explicit_approval !== true)
        throw new Error("initial memory baseline canary approval requires explicitApproval=true");
    if (input.riskAccepted !== true && input.risk_accepted !== true)
        throw new Error("initial memory baseline canary approval requires riskAccepted=true");
    if (input.initialBaselineAcknowledged !== true && input.initial_baseline_acknowledged !== true)
        throw new Error("initial memory baseline canary approval requires initialBaselineAcknowledged=true");
    const approvedBy = safeCode(input.approvedBy || input.approved_by, "");
    if (!approvedBy)
        throw new Error("initial memory baseline canary approval requires approvedBy");
    return (0, live_provider_memory_soak_report_store_1.withLiveProviderMemorySoakReportSetLock)(() => {
        ...
        if (!preview.approvable)
            throw new Error("initial memory baseline canary is not currently approvable");
        if (String(input.baselineAbsenceChecksum || input.baseline_absence_checksum || "") !== preview.baselineAbsenceChecksum)
            throw new Error("initial memory baseline absence confirmation mismatch");
        if (String(input.planChecksum || input.plan_checksum || "") !== preview.planChecksum)
            throw new Error("initial memory baseline canary plan confirmation mismatch");
        const createdAtMs = Number(input.atMs || Date.now());
        assertNoActiveApprovalForPlan(preview.planChecksum, createdAtMs);
        const ttlMs = Math.max(60_000, Math.min(30 * 60_000, Number(input.ttlMs || input.ttl_ms || 15 * 60_000)));
        const receiptId = `lpmwa_${crypto.randomBytes(14).toString("hex")}`;
...
function readLiveProviderMemoryWaveApproval(receiptId) {
    return (0, atomic_json_file_1.readJsonWithBackup)(approvalFile(receiptId), null);
}
function revokeLiveProviderMemoryWaveApproval(input = {}) {
    if (input.explicitRevocation !== true && input.explicit_revocation !== true)
        throw new Error("live memory wave approval revocation requires explicitRevocation=true");
    ...
    return (0, atomic_json_file_1.withFileLock)(file, () => {
        const receipt = (0, atomic_json_file_1.readJsonWithBackup)(file, null);
        if (!receipt)
            throw new Error("live memory wave approval receipt not found");
        if (String(input.receiptChecksum || input.receipt_checksum || "") !== String(receipt.receiptChecksum || ""))
            throw new Error("live memory wave revocation confirmation mismatch");
        const verification = verifyLiveProviderMemoryWaveApprovalReceipt(receipt, { atMs: input.atMs });
        if (!verification.valid)
            throw new Error(`live memory wave approval cannot be revoked:${verification.issues.join(",")}`);
        ...
        (0, atomic_json_file_1.writeJsonAtomic)(file, revoked);
        return revoked;
    });
}
function claimLiveProviderMemoryWaveApproval(input = {}) {
    if (input.explicitExecution !== true && input.explicit_execution !== true)
        throw new Error("live memory wave execution requires explicitExecution=true");
    const receiptId = String(input.receiptId || input.receipt_id || "");
    const file = approvalFile(receiptId);
    return (0, live_provider_memory_soak_report_store_1.withLiveProviderMemorySoakReportSetLock)(() => (0, atomic_json_file_1.withFileLock)(file, () => {
        const receipt = (0, atomic_json_file_1.readJsonWithBackup)(file, null);
        if (!receipt)
            throw new Error("live memory wave approval receipt not found");
        if (String(input.receiptChecksum || input.receipt_checksum || "") !== String(receipt.receiptChecksum || ""))
            throw new Error("live memory wave receipt confirmation mismatch");
        ...
        if (!verification.valid)
            throw new Error(`live memory wave approval rejected:${verification.issues.join(",")}`);
        ...
        (0, atomic_json_file_1.writeJsonAtomic)(file, claimed);
        return claimed;
    }));
}
function findSingleReportByChecksum(checksum) {
    try {
        if (!/^[a-f0-9]{64}$/.test(checksum) || !fs.existsSync(live_provider_memory_soak_report_store_1.LIVE_PROVIDER_MEMORY_SOAK_SINGLE_REPORT_DIR))
            return null;
        for (const entry of fs.readdirSync(live_provider_memory_soak_report_store_1.LIVE_PROVIDER_MEMORY_SOAK_SINGLE_REPORT_DIR, { withFileTypes: true })) {
            if (!entry.isFile() || !entry.name.endsWith(".json"))
                continue;
            try {
                const report = JSON.parse(fs.readFileSync(path.join(live_provider_memory_soak_report_store_1.LIVE_PROVIDER_MEMORY_SOAK_SINGLE_REPORT_DIR, entry.name), "utf8"));
                const verification = (0, live_provider_memory_soak_report_store_1.verifyLiveProviderMemorySoakReport)(report, "single");
                if (verification.valid && verification.reportChecksum === checksum)
                    return report;
            }
            catch { }
        }
...
function verifyTransitionCanaryOutcome(plan, report) {
    const issues = [];
    if (report?.accountBacked !== true)
        issues.push("transition_canary_not_account_backed");
    if (report?.isolationValid !== true)
        issues.push("transition_canary_isolation_invalid");
    if (Number(report?.passedCount || 0) !== 2 || Number(report?.failedCount || 0) !== 0)
        issues.push("transition_canary_group_failure");
    const groups = Array.isArray(report?.groups) ? report.groups : [];
    if (groups.length !== 2 || new Set(groups.map((row) => row.groupId)).size !== 2 || new Set(groups.map((row) => row.groupSessionId)).size !== 2)
        issues.push("transition_canary_group_identity_invalid");
    for (const group of groups) {
        ...
        if (group?.status !== "passed" || group?.receiptValid !== true || group?.memoryContinuityVerified !== true
            || String(group?.memoryContinuityMode || "") !== expectedMode || String(group?.recoveryStatus || "") !== expectedRecoveryStatus)
            issues.push("transition_canary_recovery_unproven");
        ...
        if (child.accountBacked !== true)
            issues.push("transition_canary_child_not_account_backed");
        ...
        if (provider?.status !== "passed"
            ...
            || !plan.providerRuntimeIdentityChecksums.includes(String(provider?.providerRuntimeIdentityChecksum || "")))
            issues.push("transition_canary_runtime_evidence_mismatch");
        if (provider?.workspaceUnchanged !== true)
            issues.push("transition_canary_workspace_changed");
        if (codexMode) {
            if (provider?.receiptRecovery?.recovered !== true || provider?.receiptRecovery?.receiptValid !== true || provider?.receiptRecovery?.suppressTaskReplay === true)
                issues.push("transition_canary_memory_recovery_unproven");
        }
        else if (provider?.initial?.status !== "passed" || provider?.resume?.status !== "passed"
            || !/^[a-f0-9]{64}$/.test(String(provider?.sessionChecksum || ""))
            || !/^[a-f0-9]{64}$/.test(String(provider?.sentinelChecksum || "")))
            issues.push("transition_canary_native_session_memory_unproven");
    }
    return [...new Set(issues)];
}
function verifyWaveOutcome(plan, outcome) {
    const issues = [];
    const file = path.resolve(String(outcome?.reportFile || ""));
    if (!pathInside(file, live_provider_memory_soak_report_store_1.LIVE_PROVIDER_MEMORY_SOAK_MULTI_REPORT_DIR))
        issues.push("execution_report_path_invalid");
    let report = null;
    try {
        report = JSON.parse(fs.readFileSync(file, "utf8"));
    }
    catch {
        issues.push("execution_report_missing");
    }
    const verification = report ? (0, live_provider_memory_soak_report_store_1.verifyLiveProviderMemorySoakReport)(report, "multi") : null;
    if (!verification?.valid)
        issues.push("execution_report_invalid");
    if (String(outcome?.reportChecksum || "") !== String(verification?.reportChecksum || ""))
        issues.push("execution_report_checksum_mismatch");
    if (String(report?.provider || "") !== String(plan?.provider || ""))
        issues.push("execution_provider_mismatch");
    if (String(report?.model || "") !== String(plan?.model || ""))
        issues.push("execution_model_mismatch");
    if (Number(report?.requestedGroupCount || 0) !== Number(plan?.groups || 0))
        issues.push("execution_group_count_mismatch");
    if (Number(report?.concurrency || 0) !== Number(plan?.concurrency || 0))
        issues.push("execution_concurrency_mismatch");
    if (!String(report?.groups?.[0]?.groupId || "").startsWith(String(plan?.groupPrefix || "")))
        issues.push("execution_group_prefix_mismatch");
    if (["version_transition_canary", "initial_provider_baseline_canary"].includes(String(plan?.approvalMode || "")))
        issues.push(...verifyTransitionCanaryOutcome(plan, report));
    return { valid: issues.length === 0, issues, report };
}
function finalizeLiveProviderMemoryWaveExecution(claimed, outcome, error = null) {
    const file = approvalFile(String(claimed.receiptId || ""));
    return (0, atomic_json_file_1.withFileLock)(file, () => {
        const current = (0, atomic_json_file_1.readJsonWithBackup)(file, null);
        if (!current || current.status !== "claimed" || current.executionId !== claimed.executionId || current.receiptChecksum !== claimed.receiptChecksum)
            throw new Error("live memory wave execution claim changed before finalization");
        ...
        if (canary && executionCompleted) {
            try {
                promotionReport = (0, live_provider_memory_endurance_1.auditLiveProviderMemoryEndurance)({
                ...
                if (!promoted)
                    evidencePromotionIssue = "transition_canary_endurance_promotion_unverified";
            }
        ...
        (0, atomic_json_file_1.writeJsonAtomic)(file, finalized);
        return finalized;
    });
}
function executeApprovedLiveProviderMemoryWave(input = {}, runner) {
    if (typeof runner !== "function")
        throw new Error("live memory wave execution runner is required");
...
function reconcileLiveProviderMemoryWaveApprovals(options = {}) {
    ...
    try {
        if (fs.existsSync(APPROVAL_DIR)) {
            for (const entry of fs.readdirSync(APPROVAL_DIR, { withFileTypes: true })) {
                ...
                try {
                    let receipt = (0, atomic_json_file_1.readJsonWithBackup)(file, null);
                    ...
                    if (receipt.status === "claimed" && Number.isFinite(claimedMs) && nowMs - claimedMs >= staleMs) {
                        ...
                        (0, atomic_json_file_1.writeJsonAtomic)(file, receipt);
                    }
                    const expiresMs = Date.parse(String(receipt.expiresAt || ""));
    ...
    for (let index = 0; index < terminalRows.length; index += 1) {
        ...
        if (row.ageHours >= graceHours && index >= minimumRetained) {
            if (row.ageHours >= retentionDays * 24)
                reasons.push("terminal_retention_expired");
            if (index >= maximumReceipts)
                reasons.push("terminal_count_overflow");
        }
    ...
    if (prune) {
        for (const candidate of candidates) {
            try {
                if (!pathInside(candidate.file, APPROVAL_DIR))
                    throw new Error("approval_path_outside_managed_root");
                ...
                if (!checksumValid || !TERMINAL_APPROVAL_STATUSES.has(String(current.status || "")))
                    throw new Error("approval_changed_or_invalid_before_prune");
                fs.unlinkSync(candidate.file);
                try {
                    fs.unlinkSync(`${candidate.file}.bak`);
                }
                catch { }
...
//# sourceMappingURL=live-provider-memory-wave-approval.js.map