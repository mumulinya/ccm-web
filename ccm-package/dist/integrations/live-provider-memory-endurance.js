...
exports.LIVE_PROVIDER_MEMORY_ENDURANCE_SCHEDULER_SCHEMA = exports.LIVE_PROVIDER_MEMORY_ENDURANCE_SCHEMA = void 0;
exports.auditLiveProviderMemoryEndurance = auditLiveProviderMemoryEndurance;
exports.readLatestLiveProviderMemoryEnduranceSummary = readLatestLiveProviderMemoryEnduranceSummary;
exports.readLatestLiveProviderMemoryEnduranceReport = readLatestLiveProviderMemoryEnduranceReport;
exports.runLiveProviderMemoryEnduranceSchedulerTick = runLiveProviderMemoryEnduranceSchedulerTick;
exports.getLiveProviderMemoryEnduranceSchedulerStatus = getLiveProviderMemoryEnduranceSchedulerStatus;
...
exports.LIVE_PROVIDER_MEMORY_ENDURANCE_SCHEMA = "ccm-live-provider-memory-endurance-report-v1";
exports.LIVE_PROVIDER_MEMORY_ENDURANCE_SCHEDULER_SCHEMA = "ccm-live-provider-memory-endurance-scheduler-v1";
const ENDURANCE_SCHEDULER_STATE_FILE = path.join(utils_1.CCM_DIR, "reliability", "live-provider-memory-endurance-scheduler.json");
...
function classifyObservation(source, child, childValid) {
...
function buildEnduranceReportLocked(options = {}) {
    ...
    const singleScan = readReports(live_provider_memory_soak_report_store_1.LIVE_PROVIDER_MEMORY_SOAK_SINGLE_REPORT_DIR);
    const multiScan = readReports(live_provider_memory_soak_report_store_1.LIVE_PROVIDER_MEMORY_SOAK_MULTI_REPORT_DIR);
    const validSingles = new Map();
    let invalidSingleReportCount = singleScan.invalidJsonCount;
    for (const row of singleScan.rows) {
        const verification = (0, live_provider_memory_soak_report_store_1.verifyLiveProviderMemorySoakReport)(row.report, "single");
        if (verification.valid)
            validSingles.set(verification.reportChecksum, row.report);
        else
            invalidSingleReportCount += 1;
    }
    const sourceReports = [];
    let invalidSourceReportCount = multiScan.invalidJsonCount;
    for (const row of multiScan.rows) {
        const kind = (0, live_provider_memory_soak_report_store_1.verifyLiveProviderMemorySoakReport)(row.report).kind;
        if (kind !== "multi")
            continue;
        const verification = (0, live_provider_memory_soak_report_store_1.verifyLiveProviderMemorySoakReport)(row.report, "multi");
        if (!verification.valid) {
            invalidSourceReportCount += 1;
            continue;
        }
        if (!includeFixtures && row.report?.accountBacked !== true)
            continue;
        if (providerFilter && String(row.report?.provider || "") !== providerFilter)
            continue;
        const groups = (Array.isArray(row.report?.groups) ? row.report.groups : []).filter((group) => !groupPrefix || String(group?.groupId || "").startsWith(groupPrefix));
        if (groups.length)
            sourceReports.push({ ...row.report, groups });
    }
    const waves = sourceReports.map(source => {
        const observations = source.groups.map((group) => {
            const childChecksum = String(group?.childReportChecksum || "");
            const child = validSingles.get(childChecksum) || null;
            const childVerification = child ? (0, live_provider_memory_soak_report_store_1.verifyLiveProviderMemorySoakReport)(child, "single") : null;
        ...
        const count = (classification) => observations.filter((row) => row.classification === classification).length;
        const durations = observations.map((row) => row.durationMs).filter((value) => value > 0);
        const providerVersions = [...new Set(observations.map((row) => row.providerVersion).filter(Boolean))];
        const providerRuntimeIdentityChecksums = [...new Set(observations.map((row) => row.providerRuntimeIdentityChecksum).filter(Boolean))];
        return {
            ...
            replaySuppressedCount: observations.filter((row) => row.replaySuppressed).length,
            workspaceChangedCount: observations.filter((row) => row.workspaceUnchanged === false).length,
    ...
    for (const wave of waves) {
        ...
        wave.versionEpoch = epoch.epoch;
    }
    const roundedRate = (count, total) => total ? Math.round(count / total * 10_000) / 10_000 : 0;
    for (const epoch of versionEpochs) {
        const durations = waves.filter((wave) => wave.versionEpoch === epoch.epoch).flatMap(wave => wave.observations.map((row) => Number(row.durationMs || 0))).filter((value) => value > 0);
    ...
    const versionComparisons = versionEpochs.slice(1).map((epoch, index) => {
        ...
        const reasons = [];
        if (epoch.memoryReceiptFailureCount > 0)
            reasons.push("new_epoch_memory_receipt_failure");
        if (epoch.ccmEvidenceFailureCount > 0)
            reasons.push("new_epoch_ccm_evidence_failure");
        if (epoch.isolationFailureCount > 0)
            reasons.push("new_epoch_group_isolation_failure");
        if (epoch.workspaceChangedCount > 0)
            reasons.push("new_epoch_workspace_changed");
        if (epoch.passRate < from.passRate - 0.1)
            reasons.push("new_epoch_pass_rate_regression");
        if (epoch.providerLatencyTimeoutRate > from.providerLatencyTimeoutRate + 0.2)
            reasons.push("new_epoch_timeout_rate_regression");
        if (epoch.providerUnavailableRate > from.providerUnavailableRate + 0.2)
            reasons.push("new_epoch_unavailable_rate_regression");
    ...
    for (const wave of waves)
        bucketMap.set(wave.concurrency, [...(bucketMap.get(wave.concurrency) || []), wave]);
    ...
    const allDurations = waves.flatMap(wave => wave.observations.map((row) => Number(row.durationMs || 0))).filter((value) => value > 0);
    ...
    const unsigned = {
        schema: exports.LIVE_PROVIDER_MEMORY_ENDURANCE_SCHEMA,
        version: 1,
        classifierVersion: 3,
...
function auditLiveProviderMemoryEndurance(options = {}) {
    return (0, live_provider_memory_soak_report_store_1.withLiveProviderMemorySoakReportSetLock)(() => {
        ...
        const reportFile = options.persist === false
            ? ""
            : (0, live_provider_memory_soak_report_store_1.commitLiveProviderMemorySoakReport)(report, { kind: "endurance", lockHeld: true });
        if (options.persist !== false)
            (0, live_provider_memory_version_transition_ledger_1.recordLiveProviderMemoryVersionTransitionEvidence)(report, { nowMs: options.nowMs });
        return { ...report, reportFile };
    }, { timeoutMs: Math.max(1_000, Number(options.lockTimeoutMs || options.lock_timeout_ms || 60_000)) });
}
function readLatestLiveProviderMemoryEnduranceSummary(options = {}) {
    const provider = String(typeof options === "string" ? options : options.provider || "").trim();
    const rows = [];
    try {
        if (fs.existsSync(live_provider_memory_soak_report_store_1.LIVE_PROVIDER_MEMORY_SOAK_MULTI_REPORT_DIR)) {
            for (const entry of fs.readdirSync(live_provider_memory_soak_report_store_1.LIVE_PROVIDER_MEMORY_SOAK_MULTI_REPORT_DIR, { withFileTypes: true })) {
                if (!entry.isFile() || !entry.name.startsWith("endurance-") || !entry.name.endsWith(".json"))
                    continue;
                try {
                    const report = JSON.parse(fs.readFileSync(path.join(live_provider_memory_soak_report_store_1.LIVE_PROVIDER_MEMORY_SOAK_MULTI_REPORT_DIR, entry.name), "utf8"));
                    if ((0, live_provider_memory_soak_report_store_1.verifyLiveProviderMemorySoakReport)(report, "endurance").valid
                        && (!provider || String(report?.advisory?.nextWave?.provider || report?.providerFilter || "") === provider))
                        rows.push(report);
                }
                catch { }
            }
...
function readLatestLiveProviderMemoryEnduranceReport(options = {}) {
    const provider = String(typeof options === "string" ? options : options.provider || "").trim();
    const rows = [];
    try {
        if (fs.existsSync(live_provider_memory_soak_report_store_1.LIVE_PROVIDER_MEMORY_SOAK_MULTI_REPORT_DIR)) {
...
function runLiveProviderMemoryEnduranceSchedulerTick(options = {}) {
    const stateFile = String(options.stateFile || options.state_file || ENDURANCE_SCHEDULER_STATE_FILE);
    if (options.stateLockHeld !== true) {
        return (0, atomic_json_file_1.withFileLock)(stateFile, () => runLiveProviderMemoryEnduranceSchedulerTick({ ...options, stateFile, stateLockHeld: true }), {
    ...
    const previous = (0, atomic_json_file_1.readJsonWithBackup)(stateFile, null);
    ...
    if (!due) {
        return {
            ...previous,
            schema: exports.LIVE_PROVIDER_MEMORY_ENDURANCE_SCHEDULER_SCHEMA,
            status: "not_due",
            due: false,
    ...
    (0, atomic_json_file_1.writeJsonAtomic)(stateFile, state);
    return state;
}
function getLiveProviderMemoryEnduranceSchedulerStatus(options = {}) {
    const stateFile = String(options.stateFile || options.state_file || ENDURANCE_SCHEDULER_STATE_FILE);
    const state = (0, atomic_json_file_1.readJsonWithBackup)(stateFile, null);
    return {
        schema: exports.LIVE_PROVIDER_MEMORY_ENDURANCE_SCHEDULER_SCHEMA,
        present: !!state,
...
//# sourceMappingURL=live-provider-memory-endurance.js.map