...
function handleGroupLiveRoutes(req, res, parsed, ctx, deps) {
    ...
    if (pathname === "/api/groups/send" && req.method === "POST") {
        ...
        const handleGroupSend = async (payload, uploadedFiles = []) => {
            ...
            let configs;
            try {
                const preface = await (0, group_live_routes_part_02_part_01_1.handleGroupLiveRoutesSendPreface)(payload, uploadedFiles, ctx, deps, res);
                if (preface.done)
                    return;
                ({
                    payload,
                ...
                if (persistentTaskRequest) {
                    ...
                    const intakeSummary = (0, group_live_routes_part_01_1.buildGroupTaskIntakeSummary)({
                        task: taskAfterQueue,
                    ...
                    if (clarificationContext)
                        (0, group_live_routes_part_01_1.resolveStoredGroupClarification)(group_id, pendingClarification, userMsg.id, groupSessionId);
                ...
                if (isBroadcast && isOrchestrated) {
                    ...
                    try {
                        ...
                        const clarificationSummary = dispatchPolicy?.action === "ask_user"
                            ? (0, group_live_routes_part_01_1.buildGroupClarificationSummary)({
                                group,
                ...
                if (isBroadcast) {
                    ...
                    const getAgentPrompt = async (member) => {
                        ...
                        const memberInfo = memberConfig ? (0, db_1.getConfigInfo)(memberConfig.path)[0] || {} : {};
                        ...
                        const prepared = await (0, group_session_model_context_1.prepareExactGroupSessionRenderedPayload)({
                            groupId: group_id,
                            groupSessionId,
                    ...
                    const preparedAgentPrompts = new Map();
                    for (const member of targetMembers)
                        preparedAgentPrompts.set(member.project, await getAgentPrompt(member));
                ...
                const runtimeConfig = configs.find((item) => item.name === target_project_actual);
                const runtimeInfo = runtimeConfig ? (0, db_1.getConfigInfo)(runtimeConfig.path)[0] || {} : {};
                const capacity = (0, model_capability_cache_1.resolveTrustedModelContextCapacity)({
                    provider: agentType,
                ...
                const preparedDirectPayload = await (0, group_session_model_context_1.prepareExactGroupSessionRenderedPayload)({
                    groupId: group_id,
                    groupSessionId,
                    modelCapacity: capacity,
        ...
        req.on("end", async () => {
            try {
                ...
                for (const member of group.members) {
                    ...
                    const preparedBroadcastPayload = await (0, group_session_model_context_1.prepareExactGroupSessionRenderedPayload)({
                        groupId: group_id,
                        groupSessionId,
                        modelCapacity: capacity,
...
//# sourceMappingURL=group-live-routes-part-02-part-02.js.map