...
exports.GROUP_POST_TURN_SUMMARY_LEDGER_DIR = exports.GROUP_POST_TURN_SUMMARY_DELIVERY_CAPSULE_SCHEMA = exports.GROUP_POST_TURN_SUMMARY_SCHEMA = void 0;
exports.getGroupPostTurnSummaryLedgerFile = getGroupPostTurnSummaryLedgerFile;
exports.verifyGroupPostTurnSummaryDeliveryCapsuleChecksum = verifyGroupPostTurnSummaryDeliveryCapsuleChecksum;
exports.extractGroupPostTurnSummaryDeliveryCapsule = extractGroupPostTurnSummaryDeliveryCapsule;
exports.buildGroupPostTurnSummaryDeliveryCapsule = buildGroupPostTurnSummaryDeliveryCapsule;
exports.validateGroupPostTurnSummaryDeliveryCapsule = validateGroupPostTurnSummaryDeliveryCapsule;
exports.buildGroupPostTurnSummary = buildGroupPostTurnSummary;
exports.readGroupPostTurnSummaries = readGroupPostTurnSummaries;
exports.recordGroupPostTurnSummary = recordGroupPostTurnSummary;
exports.backfillGroupPostTurnSummaries = backfillGroupPostTurnSummaries;
exports.deleteGroupPostTurnSummaryArtifacts = deleteGroupPostTurnSummaryArtifacts;
const crypto = __importStar(require("crypto"));
const fs = __importStar(require("fs"));
const os = __importStar(require("os"));
const path = __importStar(require("path"));
const utils_1 = require("../../core/utils");
exports.GROUP_POST_TURN_SUMMARY_SCHEMA = "ccm-group-post-turn-summary-v1";
exports.GROUP_POST_TURN_SUMMARY_DELIVERY_CAPSULE_SCHEMA = "ccm-group-post-turn-summary-delivery-capsule-v1";
exports.GROUP_POST_TURN_SUMMARY_LEDGER_DIR = path.join(utils_1.CCM_DIR, "group-post-turn-summaries");
...
function ledgerFile(groupId, sessionId) {
    if (!groupId || !String(sessionId || "").startsWith("gcs_"))
        throw new Error("post-turn summary requires groupId--gcs_* identity");
    return path.join(exports.GROUP_POST_TURN_SUMMARY_LEDGER_DIR, `${cleanPart(groupId)}--${cleanPart(sessionId)}.jsonl`);
}
function getGroupPostTurnSummaryLedgerFile(groupId, sessionId) {
...
function deliveryCapsuleChecksum(capsule) {
...
function verifyGroupPostTurnSummaryDeliveryCapsuleChecksum(capsule) {
    return !!capsule?.capsule_checksum && String(capsule.capsule_checksum) === deliveryCapsuleChecksum(capsule);
}
function extractGroupPostTurnSummaryDeliveryCapsule(value, seen = new Set()) {
    if (!value || typeof value !== "object" || seen.has(value))
        return null;
    seen.add(value);
    if (value.schema === exports.GROUP_POST_TURN_SUMMARY_DELIVERY_CAPSULE_SCHEMA
        && !!value.capsule_checksum
        && Array.isArray(value.selected_summaries))
        return value;
...
function selectDeliverySummaries(rows = [], limit = 6) {
    const current = [...rows]
    ...
    const priority = current.slice(0, -recentCount)
        ...
        .sort((a, b) => {
        const statusWeight = (row) => ["failed", "blocked", "waiting", "review_ready"].includes(String(row.status_category || "")) ? 2 : row.is_noteworthy === true ? 1 : 0;
    ...
    for (const row of priority) {
        if (selected.length >= max)
            break;
        if (!recentIds.has(String(row.summary_id || "")))
            selected.push(row);
    }
    if (selected.length < max) {
        for (const row of [...current].reverse()) {
            if (selected.length >= max)
                break;
            if (!selected.some(item => String(item.summary_id || "") === String(row.summary_id || "")))
                selected.push(row);
        }
    }
    return selected.sort((a, b) => Number(a.sequence || 0) - Number(b.sequence || 0));
}
function buildGroupPostTurnSummaryDeliveryCapsule(input = {}) {
    ...
    if (!groupId || !groupSessionId.startsWith("gcs_") || !taskAgentSessionId.startsWith("tas_") || ledger?.valid !== true)
        return null;
    ...
    const capsule = {
        schema: exports.GROUP_POST_TURN_SUMMARY_DELIVERY_CAPSULE_SCHEMA,
        version: 1,
        group_id: groupId,
...
function validateGroupPostTurnSummaryDeliveryCapsule(input = null, options = {}) {
    if (!input || typeof input !== "object")
        return null;
    const issues = [];
    if (input.schema !== exports.GROUP_POST_TURN_SUMMARY_DELIVERY_CAPSULE_SCHEMA || Number(input.version || 0) !== 1)
        issues.push("schema_invalid");
    const checksumValid = verifyGroupPostTurnSummaryDeliveryCapsuleChecksum(input);
    if (!checksumValid)
        issues.push("capsule_checksum_invalid");
    if (!String(input.group_session_id || "").startsWith("gcs_"))
        issues.push("group_session_identity_invalid");
    if (!String(input.task_agent_session_id || "").startsWith("tas_"))
        issues.push("task_agent_session_identity_invalid");
    if (!input.ledger_head_checksum)
        issues.push("ledger_head_checksum_missing");
    if (!Number.isInteger(Number(input.attempt_sequence || 0)) || Number(input.attempt_sequence || 0) < 1)
        issues.push("attempt_sequence_invalid");
    if (!["spawn", "resume"].includes(String(input.invocation_kind || "")))
        issues.push("invocation_kind_invalid");
    if (Number(input.attempt_sequence || 0) > 1 && input.invocation_kind !== "resume")
        issues.push("invocation_kind_attempt_mismatch");
    if (input.invocation_edge_id && !String(input.invocation_edge_id).startsWith("tie_"))
        issues.push("invocation_edge_identity_invalid");
    if (input.invocation_edge_id && (!String(input.root_invocation_edge_id || "").startsWith("tie_") || !String(input.branch_id || "").startsWith("tbr_")))
        issues.push("invocation_lineage_identity_invalid");
    if (input.invocation_edge_id && input.parent_invocation_edge_id === input.invocation_edge_id)
        issues.push("invocation_self_parent");
    const selected = Array.isArray(input.selected_summaries) ? input.selected_summaries : [];
    if (!selected.length || Number(input.selected_count || 0) !== selected.length || selected.length > 12)
        issues.push("selected_summaries_invalid");
    const selectedIds = new Set();
    for (const row of selected) {
        const id = String(row?.summary_id || "");
        if (!id || selectedIds.has(id) || !row?.summarizes_message_id || !row?.message_checksum || !row?.event_checksum)
            issues.push("selected_summary_binding_invalid");
        selectedIds.add(id);
    }
    if (selected.length && String(input.latest_summarized_message_id || "") !== String(selected[selected.length - 1]?.summarizes_message_id || ""))
        issues.push("latest_summary_binding_invalid");
    const expected = options.expectedBinding || options.expected_binding || {};
    const compare = (field, aliases) => {
        const expectedValue = aliases.map(alias => expected?.[alias]).find(value => value !== undefined && value !== null && String(value) !== "");
        if (expectedValue !== undefined && String(input[field] || "") !== String(expectedValue || ""))
    ...
    const ledger = options.ledger || (options.checkLedger === true || options.check_ledger === true
        ...
        : null);
    if (ledger) {
        if (ledger.valid !== true) {
            ledgerFresh = false;
            issues.push("ledger_invalid");
        }
        ...
        const currentById = new Map((Array.isArray(ledger.latest) ? ledger.latest : []).map((row) => [String(row.summary_id || ""), row]));
        for (const row of selected) {
            const current = currentById.get(String(row.summary_id || ""));
    ...
    if (renderedPrompt && !promptBound)
        issues.push("prompt_missing_capsule_checksum");
...
function acquireLock(file) {
    ...
    for (let attempt = 0; attempt < 8; attempt += 1) {
        try {
            ...
            if (!stale)
                throw new Error("post-turn summary ledger is locked by another process");
            try {
...
function appendDurableLine(file, event) {
...
function parseLedgerFile(file) {
    ...
    if (!fs.existsSync(file))
        return { rows, issues, valid: true, bytes: 0, headChecksum: "", chainOriginChecksum: "", firstSequence: 0, lastSequence: 0 };
    let previous = "";
    ...
    try {
        ...
        for (let index = 0; index < lines.length; index += 1) {
            ...
            try {
                ...
                issues.push({ line: index + 1, code: "json_invalid" });
                break;
            }
            if (index === 0) {
            ...
            if (row.schema !== exports.GROUP_POST_TURN_SUMMARY_SCHEMA)
                issues.push({ line: index + 1, code: "schema_invalid" });
            if (Number(row.sequence || 0) !== expectedSequence)
                issues.push({ line: index + 1, code: "sequence_invalid" });
            if (String(row.previous_checksum || "") !== previous)
                issues.push({ line: index + 1, code: "chain_invalid" });
            if (String(row.event_checksum || "") !== eventChecksum(row))
                issues.push({ line: index + 1, code: "checksum_invalid" });
            rows.push(row);
...
function archiveHotLedger(file, parsed) {
    if (!fs.existsSync(file) || parsed.rows.length < HOT_EVENT_LIMIT)
        return { archived: false, previousChecksum: parsed.headChecksum, previousSequence: parsed.lastSequence };
...
function statusCategory(message) {
    ...
    if (/block/.test(value))
        return "blocked";
    if (/wait|pending|clarif/.test(value))
        return "waiting";
    if (/review|verify|accept/.test(value))
        return "review_ready";
    return "completed";
}
function artifactUrls(message, content) {
...
function buildGroupPostTurnSummary(groupId, sessionId, message, options = {}) {
    if (!groupId || !String(sessionId || "").startsWith("gcs_"))
        return null;
...
function readGroupPostTurnSummaries(groupId, sessionId, options = {}) {
    ...
    for (let index = 0; index < parsedFiles.length; index += 1) {
        ...
        issues.push(...parsed.issues.map((issue) => ({ ...issue, file: parsed.file })));
    ...
    for (const row of rows)
        latestByMessage.set(String(row.summarizes_message_id || ""), row);
...
function recordGroupPostTurnSummary(groupId, sessionId, message, options = {}) {
    const summary = buildGroupPostTurnSummary(groupId, sessionId, message, options);
    if (!summary)
        return { recorded: false, reason: "assistant_gcs_message_required" };
    const file = ledgerFile(groupId, sessionId);
    const lock = acquireLock(file);
    try {
        let parsed = parseLedgerFile(file);
        if (!parsed.valid)
            throw new Error("post-turn summary ledger checksum mismatch");
        const completeLedger = readGroupPostTurnSummaries(groupId, sessionId, { limit: 10_000 });
        if (!completeLedger.valid)
            throw new Error("post-turn summary archive chain checksum mismatch");
        const latest = [...completeLedger.latest].reverse().find(row => String(row.summarizes_message_id || "") === summary.summarizes_message_id);
        if (latest && String(latest.message_checksum || "") === summary.message_checksum)
            return { recorded: false, idempotent: true, reason: "summary_already_recorded", summary: latest, ledger: readGroupPostTurnSummaries(groupId, sessionId) };
        const rotation = archiveHotLedger(file, parsed);
        if (rotation.archived)
            parsed = parseLedgerFile(file);
        ...
        const event = {
            schema: exports.GROUP_POST_TURN_SUMMARY_SCHEMA,
            version: 1,
...
function backfillGroupPostTurnSummaries(groupId, sessionId, messages = [], options = {}) {
    if (!groupId || !String(sessionId || "").startsWith("gcs_"))
        return { recorded: 0, skipped: messages.length, reason: "gcs_session_required", ledger: null };
    let recorded = 0;
    let skipped = 0;
...
function deleteGroupPostTurnSummaryArtifacts(groupId, sessionId) {
    if (!groupId || !String(sessionId || "").startsWith("gcs_"))
    ...
    try {
        for (const name of fs.readdirSync(path.dirname(file))) {
            if (name.startsWith(`${path.basename(file)}.archive-`))
                candidates.push(path.join(path.dirname(file), name));
        }
    }
    catch { }
...
//# sourceMappingURL=group-post-turn-summary.js.map