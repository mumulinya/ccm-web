...
exports.COORDINATOR_USER_INTERNAL_TEXT_PATTERN = void 0;
...
exports.sanitizeCoordinatorUserList = sanitizeCoordinatorUserList;
exports.buildCoordinatorFollowUpSummary = buildCoordinatorFollowUpSummary;
exports.collectCoordinatorFollowUpSpecificHints = collectCoordinatorFollowUpSpecificHints;
exports.buildCoordinatorFollowUpQuality = buildCoordinatorFollowUpQuality;
exports.normalizeCoordinatorFollowUpTask = normalizeCoordinatorFollowUpTask;
exports.coordinatorNotificationStatusLabel = coordinatorNotificationStatusLabel;
exports.coordinatorNotificationGaps = coordinatorNotificationGaps;
exports.buildCodedCoordinatorNotificationRows = buildCodedCoordinatorNotificationRows;
exports.buildCoordinatorReplayRepairDispatchContext = buildCoordinatorReplayRepairDispatchContext;
...
function buildCoordinatorPrompt(input) {
    ...
    const memberList = (0, group_orchestrator_routing_1.getRoutableMembers)(group).map((m) => `${m.project}(${m.agent || "agent"})`).join(", ");
...
function buildCoordinatorMaintenanceNotificationInstructions(groupInput, options = {}) {
    const group = (0, group_orchestrator_routing_1.normalizeGroupOrchestrator)(groupInput);
    const groupId = String(group?.id || "").trim();
    if (!groupId)
        return { text: "", context: null, health: null };
    const groupSessionId = (0, group_orchestrator_coded_1.normalizeWorkerContextCompactGroupSessionIdForCoordinator)(options.groupSessionId || options.group_session_id || "");
...
function buildMemberPrompt(input) {
    const memberList = (0, group_orchestrator_routing_1.getMemberNames)(input.group, input.projectName);
...
function buildCoordinatorReplayRepairDispatchContext(group) {
    ...
    if (!groupId)
        return "";
    const summary = (0, group_orchestrator_coded_1.readReplayRepairDispatchCandidatesForCoordinator)(groupId, 8);
    if (!summary?.schema || Number(summary.candidateCount || 0) <= 0)
        return "";
    const dispatchPlanLedger = (0, group_orchestrator_coded_1.syncReplayRepairDispatchPlansForCoordinator)(groupId, summary, { limit: 8 });
    ...
    for (const candidate of Array.isArray(summary.candidates) ? summary.candidates.slice(0, 8) : []) {
        const nativeBinding = (0, group_orchestrator_coded_1.candidateNativeBindingForCoordinator)(candidate).join("；");
        lines.push([
    ...
    if (readyBriefs.length) {
        ...
        for (const brief of readyBriefs.slice(0, 5)) {
            const nativeBinding = (0, group_orchestrator_coded_1.candidateNativeBindingForCoordinator)(brief).join("；");
            lines.push([
...
//# sourceMappingURL=group-orchestrator-prompts.js.map