...
function summarizeGroupTestAgentFailureItem(item) {
    ...
    const project = (0, group_routes_part_02_1.cleanGroupStatusFollowupText)(scrubGroupTestAgentEvidencePathText(item?.project || ""), "", 70);
    const title = (0, group_routes_part_02_1.cleanGroupStatusFollowupText)(scrubGroupTestAgentEvidencePathText(item?.title || item?.reason || ""), "复核发现问题", 100);
    const reason = (0, group_routes_part_02_1.cleanGroupStatusFollowupText)(scrubGroupTestAgentEvidencePathText(item?.reason || item?.status || ""), "需要补齐或修复后再验收。", 160);
...
function summarizeGroupTestAgentDiagnosticItem(item) {
    ...
    const first = [...diagnostics, ...nextActions]
        .map(value => (0, group_routes_part_02_1.cleanGroupStatusFollowupText)(scrubGroupTestAgentEvidencePathText(value), "", 180))
        .find(Boolean);
    if (!first)
        return "";
    const title = (0, group_routes_part_02_1.cleanGroupStatusFollowupText)(scrubGroupTestAgentEvidencePathText(item?.title || item?.type), "该问题", 70);
    return `${title}：${first}`;
}
function buildGroupStatusIndependentReviewSummaryFromTestAgentFailure(...sources) {
    ...
    ].map(item => (0, group_routes_part_02_1.cleanGroupStatusFollowupText)(item, "", 180)).filter(Boolean);
    return {
...
function summarizeGroupStatusIndependentReview(source = {}) {
    ...
    const statusLabel = (0, group_routes_part_02_1.cleanGroupStatusFollowupText)(summary.status_label || summary.statusLabel || fallbackLabel, fallbackLabel, 60);
    const rows = (Array.isArray(summary.rows) ? summary.rows : Array.isArray(summary.items) ? summary.items : [])
        .map((item) => (0, group_routes_part_02_1.cleanGroupStatusFollowupText)(groupReviewRowText(item), "", 140))
        .filter(Boolean)
        .slice(0, 3);
    const headline = (0, group_routes_part_02_1.cleanGroupStatusFollowupText)(summary.headline || summary.summary || rows[0] || "", status === "needs_rework"
        ? "独立复核发现未通过项。"
        : status === "needs_user"
            ? "独立复核需要你确认。"
            : status === "passed"
                ? "独立复核已通过。"
                : "独立复核结论已记录。", 200);
    const nextAction = (0, group_routes_part_02_1.cleanGroupStatusFollowupText)(summary.next_action || summary.nextAction || "", status === "needs_rework"
...
function getGroupStatusTestAgentExecutionPlanSummary(source = {}) {
    ...
    const issues = groupStatusSafeArray(plan?.issues)
        .map((item) => (0, group_routes_part_02_1.cleanGroupStatusFollowupText)(scrubGroupTestAgentEvidencePathText(groupStatusTestAgentPlanRowText(item)), "", 160))
        .filter(Boolean)
        .slice(0, 5);
    ...
    const fallbackHeadline = (0, group_routes_part_02_1.cleanGroupStatusFollowupText)(rawTextSummary || detail, "TestAgent 复核计划已生成，我会先确认计划可执行，再启动真实复核。", 260);
    return {
        summary: {
...
function summarizeGroupStatusTestAgentExecutionPlan(source = {}) {
    ...
    const seen = new Set();
    const rows = [
        ...groupStatusSafeArray(summary.rows).map((item) => (0, group_routes_part_02_1.cleanGroupStatusFollowupText)(scrubGroupTestAgentEvidencePathText(groupStatusTestAgentPlanRowText(item)), "", 140)),
        ...groupStatusSafeArray(summary.issues).map((item) => `预检问题：${(0, group_routes_part_02_1.cleanGroupStatusFollowupText)(scrubGroupTestAgentEvidencePathText(groupStatusTestAgentPlanRowText(item)), "", 120)}`),
    ].filter((item) => {
    ...
    const headline = (0, group_routes_part_02_1.cleanGroupStatusFollowupText)(scrubGroupTestAgentEvidencePathText(summary.headline || summary.summary || rows[0] || ""), status === "ready"
        ? "TestAgent 复核计划已生成，我会按计划启动真实验证。"
        : status === "blocked"
            ? "TestAgent 复核计划预检未通过，我会先修复交接信息再执行。"
            : "TestAgent 复核计划已记录，正在等待下一步复核。", 220);
    const nextAction = (0, group_routes_part_02_1.cleanGroupStatusFollowupText)(scrubGroupTestAgentEvidencePathText(summary.next_action || summary.nextAction || ""), status === "ready"
        ? "启动 TestAgent 真实复核，并把结论纳入最终验收。"
        : status === "blocked"
            ? "修复 TestAgent 工作单或项目路径后重新生成复核计划。"
...
//# sourceMappingURL=group-routes-part-01.js.map