...
function buildGroupCompletionSummary(task, summary) {
...
function buildGroupPickupSummary(task, summary, completionSummary) {
    const report = summary?.delivery_report || summary?.deliveryReport || null;
...
function buildGroupCurrentTodoSummary(latestCard, latestTask, latestStatusMeta) {
    ...
    const needsAction = friendlyGroupCompletionText(activeStep.needs_action || activeStep.needsAction || todo?.needs_action || todo?.needsAction || todoNextAction || latestCard.next_action || latestTask?.status_detail || detail, "", 220);
    return {
