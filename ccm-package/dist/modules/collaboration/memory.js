...
function buildProviderRankingProvenanceCompactRepairReceiptWorkerContextRecall(groupId, task = "", memory = {}, options = {}) {
    ...
    const receiptIds = uniqueProviderRankingCompactRepairRecallStrings(recentRows.map((row) => row.provider_switch_decision_receipt_id), 12);
    const receiptChecksums = uniqueProviderRankingCompactRepairRecallStrings(recentRows.map((row) => row.provider_switch_decision_receipt_checksum), 12);
    const rowReasons = uniqueProviderRankingCompactRepairRecallStrings(recentRows.map((row) => row.reason), 8)
        .map((item) => compactMemoryText(item, 260));
    const queryAppend = [
...
function buildPostCompactReinjectionRepairReceiptWorkerContextRecall(groupId, task = "", memory = {}, options = {}) {
    ...
    const gateIds = uniqueProviderRankingCompactRepairRecallStrings(recentRows.map((row) => row.reinjection_gate_id), 16);
    const candidateIds = uniqueProviderRankingCompactRepairRecallStrings(recentRows.map((row) => row.post_compact_candidate_id), 16);
    const candidateValues = uniqueProviderRankingCompactRepairRecallStrings(recentRows.map((row) => row.post_compact_candidate_value), 16);
    const sourceMessageIds = uniqueProviderRankingCompactRepairRecallStrings(recentRows.map((row) => row.post_compact_candidate_source_message_id), 16);
    const taskAgentSessionIds = uniqueProviderRankingCompactRepairRecallStrings(recentRows.map((row) => row.task_agent_session_id), 16);
    const nativeSessionIds = uniqueProviderRankingCompactRepairRecallStrings(recentRows.map((row) => row.native_session_id), 16);
    const completionWorkItemIds = uniqueProviderRankingCompactRepairRecallStrings(recentCompletionRows.map((row) => row.work_item_id), 16);
    const completionTimelineBindingIds = uniqueProviderRankingCompactRepairRecallStrings(recentCompletionRows.map((row) => row.timeline_binding_id), 16);
    const completionOriginalWorkerContextPacketIds = uniqueProviderRankingCompactRepairRecallStrings(recentCompletionRows.map((row) => row.original_worker_context_packet_id), 16);
    const preservationRepairWorkItemIds = uniqueProviderRankingCompactRepairRecallStrings(recalledPreservationClosureRows.map((row) => row.work_item_id), 16);
    const preservationFailedRetryIds = uniqueProviderRankingCompactRepairRecallStrings(recalledPreservationClosureRows.map((row) => row.failed_retry_id), 16);
    const preservationFailedOutcomeIds = uniqueProviderRankingCompactRepairRecallStrings(recalledPreservationClosureRows.map((row) => row.failed_outcome_id), 16);
    const preservationCorrectedRetryIds = uniqueProviderRankingCompactRepairRecallStrings(recalledPreservationClosureRows.map((row) => row.corrected_retry_id), 16);
    const preservationCorrectedOutcomeIds = uniqueProviderRankingCompactRepairRecallStrings(recalledPreservationClosureRows.map((row) => row.corrected_outcome_id), 16);
    const preservationCompletionDocRelPaths = uniqueProviderRankingCompactRepairRecallStrings(recalledPreservationClosureRows.flatMap((row) => row.completion_doc_rel_paths || []), 16);
    const preservationCompletionWorkItemIds = uniqueProviderRankingCompactRepairRecallStrings(recalledPreservationClosureRows.flatMap((row) => row.completion_work_item_ids || []), 24);
    const preservationCompletionTimelineBindingIds = uniqueProviderRankingCompactRepairRecallStrings(recalledPreservationClosureRows.flatMap((row) => row.completion_timeline_binding_ids || []), 24);
    const originalTaskAgentSessionIds = uniqueProviderRankingCompactRepairRecallStrings(recentCompletionRows.map((row) => row.original_task_agent_session_id), 16);
    const originalNativeSessionIds = uniqueProviderRankingCompactRepairRecallStrings(recentCompletionRows.map((row) => row.original_native_session_id), 16);
    const repairTaskAgentSessionIds = uniqueProviderRankingCompactRepairRecallStrings(recentCompletionRows.map((row) => row.repair_task_agent_session_id), 16);
    const repairNativeSessionIds = uniqueProviderRankingCompactRepairRecallStrings(recentCompletionRows.map((row) => row.repair_native_session_id), 16);
    const preservationHistoricalTaskAgentSessionIds = uniqueProviderRankingCompactRepairRecallStrings(recalledPreservationClosureRows.flatMap((row) => [
    ...
    const preservationHistoricalNativeSessionIds = uniqueProviderRankingCompactRepairRecallStrings(recalledPreservationClosureRows.flatMap((row) => [
        ...(Array.isArray(row.historical_native_session_ids) ? row.historical_native_session_ids : []),
        row.current_native_session_id,
    ]), 24);
    const preservationConflictResolutionEntryIds = uniqueProviderRankingCompactRepairRecallStrings(recalledPreservationClosureConflictResolutionRows.map((row) => row.resolution_entry_id), 16);
    const preservationConflictResolutionTaskAgentSessionIds = uniqueProviderRankingCompactRepairRecallStrings(recalledPreservationClosureConflictResolutionRows.map((row) => row.task_agent_session_id), 16);
    const preservationConflictResolutionNativeSessionIds = uniqueProviderRankingCompactRepairRecallStrings(recalledPreservationClosureConflictResolutionRows.map((row) => row.native_session_id), 16);
...
function renderGroupMemoryContextBundle(bundle) {
    ...
    if (providerRankingCompactRepairReceiptRecall.active === true) {
        ...
        ], 12).filter((item) => item === PROVIDER_RANKING_MEMORY_USAGE_RECEIPT_DISCIPLINE_REL_PATH);
