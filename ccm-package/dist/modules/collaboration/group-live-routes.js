...
async function handleGroupLiveRoutesSendPreface(payload, uploadedFiles, ctx, deps, res, readOnly = false) {
    ...
    if (explicitRouteChoice === "continue_original") {
        if (!explicitRouteCandidate) {
            (0, utils_1.sendJson)(res, { error: "原任务已不可恢复，请重新选择处理方式", code: "CONVERSATION_ROUTE_CANDIDATE_STALE" }, 409);
            return { done: true };
        }
        ...
        if (["resume_task", "revise_task"].includes(routeDecision.decision) && routeDecision.candidate) {
            ...
            if (!routeTurn || routeTurn.status !== "sending") {
                (0, utils_1.sendJson)(res, { error: "消息处理方式已经变化，请刷新后重试", code: "QUEUE_REVISION_CONFLICT" }, 409);
                return { done: true };
            }
            const routed = conversation_turn_control_1.conversationTurnControl.requireRoute({
                id: routeTurn.id,
            ...
            try {
                ...
                if (isBroadcast && isOrchestrated) {
                    ...
                    const coordinatorResult = await (0, group_orchestrator_1.runGroupOrchestrator)({
                        ...
                        onModelActivity: (activity) => writeSse(res, { type: "model_activity", activity, agent: coordinator.project }),
                    });
                    writeSse(res, {
                        type: "turn_decision",
