...
function runGroupMemoryCompactWarningSelfTest() {
    ...
    const ok = (0, group_memory_compaction_1.calculateGroupCompactWarningState)({ activeTokens: 10_000, config, now: "2026-07-07T00:00:00.000Z" });
    const warning = (0, group_memory_compaction_1.calculateGroupCompactWarningState)({ activeTokens: 30_000, config, now: "2026-07-07T00:00:00.000Z" });
    const error = (0, group_memory_compaction_1.calculateGroupCompactWarningState)({ activeTokens: 40_000, config, now: "2026-07-07T00:00:00.000Z" });
    const autoCompact = (0, group_memory_compaction_1.calculateGroupCompactWarningState)({ activeTokens: 48_000, config, activeMessageCount: 120, now: "2026-07-07T00:00:00.000Z" });
    const blocking = (0, group_memory_compaction_1.calculateGroupCompactWarningState)({ activeTokens: 58_000, config, now: "2026-07-07T00:00:00.000Z" });
    const suppressed = (0, group_memory_compaction_1.calculateGroupCompactWarningState)({
        activeTokens: 20_000,
        config,
        suppressed: true,
    ...
    const checks = {
        effectiveWindowMatchesCcStyleBudget: (0, group_memory_compaction_1.getGroupEffectiveContextWindow)(config) === 60_000,
        autoThresholdMatchesBuffer: (0, group_memory_compaction_1.getGroupAutoCompactThreshold)(config) === 47_000,
...
function runGroupMemoryCompactionSelfTest() {
    ...
    const keepIndex = (0, group_memory_compaction_1.calculateGroupMessagesToKeepIndex)(messages, { minMessages: 8, minTokens: 500, maxTokens: 1800 });
    const boundaryKeepIndex = (0, group_memory_compaction_1.calculateGroupMessagesToKeepIndex)(messages, { floorIndex: 60, minMessages: 8, minTokens: 500, maxTokens: 1800 });
    const compacted = messages.slice(0, keepIndex);
    const kept = messages.slice(keepIndex);
    const summary = (0, group_memory_compaction_1.buildDeterministicConversationSummary)(compacted, { goal: "实现订单审核", decisions: [], completed: [], blocked: [], nextActions: [{ action: "继续测试" }] });
    const bounded = (0, group_memory_compaction_1.buildBoundedRecentGroupContext)([{ id: "large", role: "assistant", agent: "worker", content: "x".repeat(20_000) }], 1);
    const retrieval = (0, group_memory_compaction_1.buildRelevantHistoricalGroupContext)(messages, Math.max(0, keepIndex - 1), "订单审核 权限校验");
    const unsafeModel = { ...(0, group_memory_compaction_1.createEmptyConversationSummary)(), filesAndCode: ["src/fake-hallucination.ts"], completedWork: ["已经上线生产"] };
    const safeMerged = (0, group_memory_compaction_1.mergeSafeConversationSummary)((0, group_memory_compaction_1.createEmptyConversationSummary)(), summary, unsafeModel, compacted);
    const anchors = (0, group_memory_compaction_1.extractFactAnchors)(compacted);
    const checks = {
        ...
        adaptiveThresholdMatchesDefaultBudget: (0, group_memory_compaction_1.getGroupAutoCompactThreshold)({}) === group_memory_compaction_1.GROUP_COMPACT_TRIGGER_TOKENS,
    };
...
function runGroupMemoryModelCapacitySelfTest() {
    const defaultCapacity = (0, group_memory_compaction_1.resolveGroupModelContextCapacity)({});
    const preset516 = {
    ...
    const fallback = (0, group_memory_compaction_1.buildDeterministicConversationSummary)(messages, { goal: sentinel });
    const request = (0, group_memory_compaction_1.buildGroupCompactionModelRequest)(messages, {}, fallback, {
    ...
    const checks = {
        ...
        ccDefaultAutoCompactThresholdIs167k: (0, group_memory_compaction_1.getGroupAutoCompactThreshold)({}) === 167_000,
        preset516IsApplied: (0, group_memory_compaction_1.getGroupEffectiveContextWindow)(preset516) === 496_000
            && (0, group_memory_compaction_1.getGroupAutoCompactThreshold)(preset516) === 460_000,
        preset1mIsApplied: (0, group_memory_compaction_1.getGroupEffectiveContextWindow)(preset1m) === 980_000
            && (0, group_memory_compaction_1.getGroupAutoCompactThreshold)(preset1m) === 900_000,
    ...
    return {
        ...
        checks,
        defaultCapacity,
        preset516Threshold: (0, group_memory_compaction_1.getGroupAutoCompactThreshold)(preset516),
        preset1mThreshold: (0, group_memory_compaction_1.getGroupAutoCompactThreshold)(preset1m),
...
async function runGroupCompactStrategyDecisionSelfTest() {
    ...
    const directKeepIndex = (0, group_memory_compaction_1.calculateGroupMessagesToKeepIndex)(messages, { minMessages: 2, minTokens: 1, maxTokens: 1400 });
    const directMicro = (0, group_memory_compaction_1.buildGroupMicroCompactPlan)(messages.slice(0, directKeepIndex), { maxChars: 900 });
    const directPreserved = (0, group_memory_compaction_1.buildGroupPreservedSegment)(messages, directKeepIndex, {
        minMessages: 2,
        minTokens: 1,
        maxTokens: 1400,
    ...
    const directDecision = (0, group_memory_compaction_1.buildGroupCompactStrategyDecision)({
    ...
    const compacted = await (0, group_memory_compaction_1.compactGroupConversationMemory)({
...
async function runGroupPostCompactCleanupAuditSelfTest() {
    ...
    const auditVerification = (0, group_memory_compaction_1.verifyGroupPostCompactCleanupAudit)(audit, { groupId, groupSessionId, boundaryId: result.boundary?.id });
    ...
    tamperedAudit.audit_checksum = (0, group_memory_compaction_1.groupPostCompactCleanupAuditChecksum)(tamperedAudit);
    const crossSessionVerification = (0, group_memory_compaction_1.verifyGroupPostCompactCleanupAudit)(tamperedAudit, { groupId, groupSessionId, boundaryId: result.boundary?.id });
    const reboundReceiptVerification = (0, group_memory_compaction_1.verifyGroupCompactTransactionReceipt)(receipt, {
        groupId,
        groupSessionId,
        boundaryId: result.boundary?.id,
...
async function runGroupApiMicroCompactEditPlanSelfTest() {
    ...
    const direct = (0, group_memory_compaction_1.buildGroupApiMicroCompactEditPlan)(messages, {
...
function runGroupApiMicrocompactNativeApplyPlanSelfTest() {
    const editPlan = (0, group_memory_compaction_1.buildGroupApiMicroCompactEditPlan)([
        {
    ...
    const cli = (0, group_memory_compaction_1.buildGroupApiMicrocompactNativeApplyPlan)(editPlan, {
        agentType: "claudecode",
        transport: "cli",
        now: "2026-07-08T07:01:00.000Z",
    });
    const native = (0, group_memory_compaction_1.buildGroupApiMicrocompactNativeApplyPlan)(editPlan, {
    ...
    const missingBeta = (0, group_memory_compaction_1.buildGroupApiMicrocompactNativeApplyPlan)(editPlan, {
        agentType: "claude-api",
        transport: "anthropic_api",
        provider: "anthropic",
        supportsApiContextManagement: true,
    ...
    const checks = {
        ...
        nativeApiBuildsRealRequestPatch: native.mode === "native_api_context_management"
            ...
            && native.requestPatch?.beta_headers?.includes(group_memory_compaction_1.GROUP_API_MICROCOMPACT_CONTEXT_MANAGEMENT_BETA),
...
function runGroupMemoryQualityGateSelfTest() {
    ...
    const persistentRequirements = (0, group_memory_compaction_1.mergePersistentRequirements)([], (0, group_memory_compaction_1.extractPersistentRequirements)(messages));
    const factAnchors = (0, group_memory_compaction_1.mergeFactAnchors)([], (0, group_memory_compaction_1.extractFactAnchors)(messages));
    const good = (0, group_memory_compaction_1.evaluateGroupMemorySummaryQuality)(fallback, fallback, messages, {}, { persistentRequirements, factAnchors });
    const bad = {
        ...fallback,
        userMessages: [],
        errorsAndFixes: [],
        ...
        nextStep: "",
        completedWork: (0, group_memory_compaction_1.mergeUnique)(fallback.completedWork, ["released to production PROD_RELEASE_999"], 30, 700),
    };
    const badQuality = (0, group_memory_compaction_1.evaluateGroupMemorySummaryQuality)(bad, fallback, messages, {}, { persistentRequirements, factAnchors });
    const checks = {
...
function runGroupMemoryMicroCompactSelfTest() {
    ...
    const micro = (0, group_memory_compaction_1.buildGroupMicroCompactPlan)(messages, { maxChars: 1400 });
    const reinject = (0, group_memory_compaction_1.buildPostCompactReinjectionPlan)(messages, micro);
    const checks = {
...
function runGroupMemoryTimeBasedMicroCompactSelfTest() {
    ...
    const plan = (0, group_memory_compaction_1.buildGroupMicroCompactPlan)(messages, {
        timeBased: {
            enabled: true,
    ...
    const notTriggered = (0, group_memory_compaction_1.buildGroupMicroCompactPlan)(messages, {
        timeBased: {
            enabled: true,
    ...
    const checks = {
        ...
        recordsClearedPlaceholder: cleared.every((record) => String(record.text || "").includes(group_memory_compaction_1.GROUP_TIME_BASED_MC_CLEARED_MESSAGE)),
...
async function runGroupMemoryCompactionHookSelfTest() {
    ...
    const ledgerFile = (0, group_memory_compaction_1.getGroupMemoryCompactionHookLedgerFile)(groupId, groupSessionId);
    ...
    const unregisterPre = (0, group_memory_compaction_1.registerGroupMemoryCompactionHook)("pre", input => ({
    ...
    const unregisterPost = (0, group_memory_compaction_1.registerGroupMemoryCompactionHook)("post", input => ({
    ...
    try {
        ...
        const checks = {
            ...
            hookLedgerReadable: (0, group_memory_compaction_1.readGroupMemoryCompactionHookLedger)(groupId, groupSessionId).entries?.length >= 2
                && (0, group_memory_compaction_1.readGroupMemoryCompactionHookLedger)(groupId, groupSessionId).stats?.pre?.ok >= 1
                && (0, group_memory_compaction_1.readGroupMemoryCompactionHookLedger)(groupId, groupSessionId).stats?.post?.ok >= 1,
        };
...
async function runGroupMemoryPartialCompactSidecarSelfTest() {
    ...
    const cleanupVerification = (0, group_memory_compaction_1.verifyGroupPostCompactCleanupAudit)(cleanupAudit, {
        groupId: "partial-sidecar-self-test",
        groupSessionId: "gcs_partial_sidecar_selftest",
...
async function runGroupMemoryPtlRecoverySelfTest() {
    ...
    const previousEmergency = {
        ...
        version: group_memory_compaction_1.GROUP_PTL_EMERGENCY_VERSION,
        engaged: true,
        emergencyLevel: "critical",
        reason: "previous_context_pressure_exhausted",
        triggerTokens: group_memory_compaction_1.GROUP_COMPACT_TRIGGER_TOKENS,
...
async function runGroupMemoryCompactionIntegrationSelfTest() {
    ...
    const originalMessages = JSON.stringify(messages);
    const first = await (0, group_memory_compaction_1.compactGroupConversationMemory)({
    ...
    const second = first.compacted
        ...
        : { compacted: false };
    const migrated = await (0, group_memory_compaction_1.compactGroupConversationMemory)({
    ...
    const checks = {
        ...
        legacyVersionRebuildsFromRawTranscript: migrated.memory?.compaction?.version === group_memory_compaction_1.GROUP_MEMORY_COMPACTION_VERSION
...
async function runGroupMemoryCompactionStressSelfTest() {
    ...
    const retrieval = (0, group_memory_compaction_1.buildRelevantHistoricalGroupContext)(messages, lastBoundaryIndex, "审计日志 AUDIT_SENTINEL_73921");
