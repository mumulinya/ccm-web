...
exports.parseGroupSessionMemoryTemplate = parseGroupSessionMemoryTemplate;
exports.validateGroupSessionMemoryCustomPrompt = validateGroupSessionMemoryCustomPrompt;
exports.readGroupSessionMemoryCustomPromptProfile = readGroupSessionMemoryCustomPromptProfile;
exports.saveGroupSessionMemoryCustomPrompt = saveGroupSessionMemoryCustomPrompt;
exports.readGroupSessionMemoryCustomTemplateProfile = readGroupSessionMemoryCustomTemplateProfile;
exports.saveGroupSessionMemoryCustomTemplate = saveGroupSessionMemoryCustomTemplate;
exports.inspectGroupSessionMemoryTemplateState = inspectGroupSessionMemoryTemplateState;
...
const SESSION_MEMORY_CUSTOM_PROMPT_GLOBAL_FILE = path.join(utils_1.CCM_DIR, "session-memory", "config", "prompt.md");
const SESSION_MEMORY_CUSTOM_TEMPLATE_MAX_CHARS = 48_000;
const SESSION_MEMORY_CUSTOM_TEMPLATE_GLOBAL_FILE = path.join(utils_1.CCM_DIR, "session-memory", "config", "template.md");
function hashText(value, length = 32) {
    return crypto.createHash("sha256").update(String(value || "")).digest("hex").slice(0, length);
}
function safeSessionMemoryCustomizationScopeId(scopeId) {
    const value = String(scopeId || "").trim();
    if (!value || !value.includes("--gcs_"))
        throw new Error("session_memory_custom_prompt_exact_session_required");
    return value.replace(/[^a-zA-Z0-9._-]+/g, "-").slice(0, 240);
}
function sessionMemoryCustomPromptFile(scopeId = "") {
    if (!String(scopeId || "").trim())
        return SESSION_MEMORY_CUSTOM_PROMPT_GLOBAL_FILE;
    return path.join(utils_1.CCM_DIR, "group-session-memory", safeSessionMemoryCustomizationScopeId(scopeId), "config", "prompt.md");
}
function sessionMemoryCustomTemplateFile(scopeId = "") {
    if (!String(scopeId || "").trim())
        return SESSION_MEMORY_CUSTOM_TEMPLATE_GLOBAL_FILE;
    return path.join(utils_1.CCM_DIR, "group-session-memory", safeSessionMemoryCustomizationScopeId(scopeId), "config", "template.md");
}
function parseGroupSessionMemoryTemplate(content) {
    const template = String(content ?? "").replace(/\r\n/g, "\n").trim();
    if (!template)
        throw new Error("session_memory_custom_template_empty");
    if (template.length > SESSION_MEMORY_CUSTOM_TEMPLATE_MAX_CHARS)
        throw new Error("session_memory_custom_template_too_large");
    if (/\u0000/.test(template))
        throw new Error("session_memory_custom_template_invalid_nul");
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        if (/^#{2,6}\s+/.test(line))
            throw new Error("session_memory_custom_template_nested_headers_unsupported");
        if (!/^#\s+\S/.test(line)) {
            if (!sawSection && line.trim())
                throw new Error("session_memory_custom_template_preamble_invalid");
            continue;
        }
        sawSection = true;
        const description = String(lines[index + 1] || "").trimEnd();
        if (!/^_[^\r\n]+_$/.test(description))
            throw new Error("session_memory_custom_template_description_invalid");
        sections.push([line, description]);
    }
    if (!sections.length || sections.length > 20)
        throw new Error("session_memory_custom_template_section_count_invalid");
...
function validateGroupSessionMemoryCustomPrompt(content) {
    const value = String(content ?? "").replace(/\r\n/g, "\n").trim();
    if (value.length > SESSION_MEMORY_CUSTOM_PROMPT_MAX_CHARS)
        throw new Error("session_memory_custom_prompt_too_large");
    if (/\u0000/.test(value))
        throw new Error("session_memory_custom_prompt_invalid_nul");
    return value;
}
function readSessionMemoryCustomPromptFile(file) {
    try {
        if (!fs.existsSync(file))
            return { present: false, content: "", file, checksum: "" };
...
function readGroupSessionMemoryCustomPromptProfile(scopeId = "") {
...
function saveGroupSessionMemoryCustomPrompt(scopeId, content, options = {}) {
    ...
    if (!reset && !normalized)
        throw new Error("session_memory_custom_prompt_empty_use_reset");
...
function readSessionMemoryCustomTemplateFile(file) {
    try {
        if (!fs.existsSync(file))
            return { present: false, content: "", file, checksum: "", sectionCount: 0 };
...
function readGroupSessionMemoryCustomTemplateProfile(scopeId = "") {
    ...
    const fallback = parseGroupSessionMemoryTemplate(exports.GROUP_SESSION_MEMORY_MODEL_TEMPLATE);
    return {
...
function saveGroupSessionMemoryCustomTemplate(scopeId, content, options = {}) {
    const targetScopeId = String(scopeId || "").trim();
...
function inspectGroupSessionMemoryTemplateState(scopeId, content) {
...
//# sourceMappingURL=group-session-memory-customization.js.map