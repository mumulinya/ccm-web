...
exports.GROUP_SESSION_MEMORY_COMPACT_SELECTION_VERSION = 1;
...
function verifyGroupSessionMemoryCompactSelectionReceipt(receipt, expected = {}) {
    ...
    if (receipt?.schema !== "ccm-group-session-memory-compact-selection-v1"
        || Number(receipt?.version || 0) !== exports.GROUP_SESSION_MEMORY_COMPACT_SELECTION_VERSION)
    ...
    if (receipt?.selected === true && (!receipt?.markdown_checksum_matches || receipt?.cursor_status !== "resolved"))
...
async function selectGroupSessionMemoryForCompact(input = {}) {
    ...
    if (!cursor)
        return fallback("last_summarized_cursor_missing", { ...sourceFields, cursorStatus: "missing" });
    ...
    if (candidateKeepIndex < 0)
        return fallback("last_summarized_cursor_not_found", { ...sourceFields, cursorStatus: "not_found" });
