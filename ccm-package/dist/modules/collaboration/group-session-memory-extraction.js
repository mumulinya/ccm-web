...
exports.GROUP_SESSION_MEMORY_EXTRACTION_RETRY_MAX_MS = exports.GROUP_SESSION_MEMORY_EXTRACTION_RETRY_BASE_MS = exports.GROUP_SESSION_MEMORY_EXTRACTION_WAIT_TIMEOUT_MS = exports.GROUP_SESSION_MEMORY_EXTRACTION_LEASE_TTL_MS = void 0;
exports.getGroupSessionMemoryExtractionLeaseFile = getGroupSessionMemoryExtractionLeaseFile;
exports.getGroupSessionMemoryExtractionStateFile = getGroupSessionMemoryExtractionStateFile;
exports.readGroupSessionMemoryExtractionState = readGroupSessionMemoryExtractionState;
exports.inspectGroupSessionMemoryExtractionLease = inspectGroupSessionMemoryExtractionLease;
exports.acquireGroupSessionMemoryExtractionLease = acquireGroupSessionMemoryExtractionLease;
exports.releaseGroupSessionMemoryExtractionLease = releaseGroupSessionMemoryExtractionLease;
exports.verifyGroupSessionMemoryExtractionLease = verifyGroupSessionMemoryExtractionLease;
exports.renewGroupSessionMemoryExtractionLease = renewGroupSessionMemoryExtractionLease;
exports.waitForGroupSessionMemoryExtraction = waitForGroupSessionMemoryExtraction;
exports.runGroupSessionMemoryExtractionTransaction = runGroupSessionMemoryExtractionTransaction;
exports.runGroupSessionMemoryExtractionTransactionAsync = runGroupSessionMemoryExtractionTransactionAsync;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const os = __importStar(require("os"));
const crypto = __importStar(require("crypto"));
const utils_1 = require("../../core/utils");
exports.GROUP_SESSION_MEMORY_EXTRACTION_LEASE_TTL_MS = 60_000;
exports.GROUP_SESSION_MEMORY_EXTRACTION_WAIT_TIMEOUT_MS = 15_000;
exports.GROUP_SESSION_MEMORY_EXTRACTION_RETRY_BASE_MS = 30_000;
exports.GROUP_SESSION_MEMORY_EXTRACTION_RETRY_MAX_MS = 30 * 60_000;
function cleanScope(value) {
    return String(value || "").replace(/[^a-zA-Z0-9._-]+/g, "-").slice(0, 240) || "unknown";
}
function extractionDir(scopeId) {
    return path.join(utils_1.CCM_DIR, "group-session-memory", cleanScope(scopeId));
}
function getGroupSessionMemoryExtractionLeaseFile(scopeId) {
    return path.join(extractionDir(scopeId), ".extraction-lease.json");
}
function getGroupSessionMemoryExtractionStateFile(scopeId) {
...
function readGroupSessionMemoryExtractionState(scopeId) {
...
function inspectGroupSessionMemoryExtractionLease(scopeId, options = {}) {
    ...
    if (!lease)
        return { file, present: false, valid: true, active: false, stale: false, lease: null };
...
function writeLeaseHandle(handle, value) {
    const lease = { ...value };
...
function acquireGroupSessionMemoryExtractionLease(scopeId, options = {}) {
    const file = String(options.file || getGroupSessionMemoryExtractionLeaseFile(scopeId));
    const at = String(options.at || new Date().toISOString());
    const atMs = Date.parse(at) || Date.now();
    const ttlMs = Math.max(5_000, Math.min(5 * 60_000, Number(options.ttlMs || options.ttl_ms || exports.GROUP_SESSION_MEMORY_EXTRACTION_LEASE_TTL_MS)));
...
function releaseLease(handle, finalStatus) {
    ...
    try {
        ...
        if (current.lease?.leaseId === handle.lease.leaseId && current.lease?.fencingToken === handle.lease.fencingToken)
...
function releaseGroupSessionMemoryExtractionLease(handle, finalStatus = "completed") {
    return releaseLease(handle, finalStatus);
}
function verifyGroupSessionMemoryExtractionLease(handle, options = {}) {
...
function renewGroupSessionMemoryExtractionLease(handle, options = {}) {
    ...
    const at = String(options.at || new Date().toISOString());
    const atMs = Date.parse(at) || Date.now();
    const ttlMs = Math.max(5_000, Math.min(5 * 60_000, Number(options.ttlMs || options.ttl_ms || handle.ttlMs || exports.GROUP_SESSION_MEMORY_EXTRACTION_LEASE_TTL_MS)));
    try {
        ...
        if (!after.owned)
            return { renewed: false, reason: "lease_lost_during_renewal", verification: after, lease };
...
async function waitForGroupSessionMemoryExtraction(scopeId, options = {}) {
    const startedAt = Date.now();
    const timeoutMs = Math.max(0, Number(options.timeoutMs || options.timeout_ms || exports.GROUP_SESSION_MEMORY_EXTRACTION_WAIT_TIMEOUT_MS));
    ...
    while (true) {
        ...
        if (!status.active)
            return { completed: !status.present || !status.stale, timedOut: false, stale: status.stale, status };
        if (Date.now() - startedAt >= timeoutMs)
            return { completed: false, timedOut: true, stale: false, status };
        await new Promise(resolve => setTimeout(resolve, pollMs));
    }
}
function runGroupSessionMemoryExtractionTransaction(scopeId, operation, options = {}) {
    const acquired = acquireGroupSessionMemoryExtractionLease(scopeId, options);
    if (!acquired.acquired)
        return { committed: false, status: acquired.reason || "lease_unavailable", acquired };
    ...
    try {
        ...
        const operationResult = operation({
            ...
            state: base,
            verifyLease: (verifyOptions = {}) => verifyGroupSessionMemoryExtractionLease(acquired.handle, verifyOptions),
            renewLease: (renewOptions = {}) => renewGroupSessionMemoryExtractionLease(acquired.handle, renewOptions),
        });
        ...
        if (!renewal.renewed)
            throw new Error(`session_memory_extraction_lease_lost_before_commit:${renewal.reason}`);
        const value = staged
        ...
        if (!ownership.owned)
            throw new Error(`session_memory_extraction_lease_lost_after_commit:${ownership.reason}`);
        ...
        if (!superseded) {
            try {
                writeJsonAtomic(stateFile, failed);
            }
            catch { }
        }
...
function extractionRetryBackoffMs(consecutiveFailures, options = {}) {
    const baseMs = Math.max(1_000, Number(options.retryBaseMs || options.retry_base_ms || exports.GROUP_SESSION_MEMORY_EXTRACTION_RETRY_BASE_MS));
    const maxMs = Math.max(baseMs, Number(options.retryMaxMs || options.retry_max_ms || exports.GROUP_SESSION_MEMORY_EXTRACTION_RETRY_MAX_MS));
    return Math.min(maxMs, baseMs * Math.pow(2, Math.max(0, Math.min(10, consecutiveFailures - 1))));
}
async function runGroupSessionMemoryExtractionTransactionAsync(scopeId, operation, options = {}) {
    ...
    writeJsonAtomic(stateFile, base);
    const heartbeatMs = Math.max(1_000, Math.min(20_000, Number(options.heartbeatMs || options.heartbeat_ms || Math.floor(Number(acquired.handle.ttlMs || exports.GROUP_SESSION_MEMORY_EXTRACTION_LEASE_TTL_MS) / 3))));
    let heartbeat = null;
    ...
    if (options.autoRenew !== false && options.auto_renew !== false) {
        heartbeat = setInterval(() => {
            ...
            if (!renewed.renewed)
                heartbeatFailure = renewed.reason || "lease_heartbeat_failed";
        }, heartbeatMs);
        heartbeat.unref?.();
    }
    try {
        ...
        heartbeat = null;
        if (heartbeatFailure)
            throw new Error(`session_memory_extraction_lease_heartbeat_lost:${heartbeatFailure}`);
        const staged = operationResult?.schema === "ccm-group-session-memory-extraction-staged-commit-v1"
            && typeof operationResult.commit === "function";
        const renewal = renewGroupSessionMemoryExtractionLease(acquired.handle, {
            at: options.commitAt || options.commit_at,
        ...
        const failureClass = String(error?.code || options.failureClass || options.failure_class || (/timeout|abort/i.test(errorText) ? "timeout"
...
//# sourceMappingURL=group-session-memory-extraction.js.map