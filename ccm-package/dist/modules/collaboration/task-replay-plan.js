...
function planStepRows(steps, source = "") {
    return steps.map((step, index) => ({
        id: (0, task_replay_shared_1.safeText)(step?.id, 80) || `step_${index + 1}`,
        title: (0, task_replay_shared_1.safeText)(step?.label || step?.content || step?.title, 200) || `步骤 ${index + 1}`,
        detail: (0, task_replay_shared_1.safeText)(step?.detail || (source === "live" ? step?.activeForm : ""), 300),
        status: planStepStatus(step?.status),
        source: source || planStepSource(step),
        added_at: (0, task_replay_shared_1.iso)(step?.added_at),
    }));
}
...
function coordinationPlanView(task, plan, history) {
    ...
    const steps = phases.map((phase, index) => {
        const text = (0, task_replay_shared_1.safeText)(phase, 300);
        const [head, ...rest] = text.split(/[：:]/);
        return {
            id: `coordination_phase_${index + 1}`,
            title: (0, task_replay_shared_1.safeText)(rest.length ? head : text, 200) || `阶段 ${index + 1}`,
            detail: rest.length ? (0, task_replay_shared_1.safeText)(rest.join("："), 300) : "",
            status: stepStatus,
            source: "skeleton",
            added_at: "",
        };
