...
function normalizeReplayChange(item, fallbackProject = "") {
    ...
    const unavailableReason = safeText(sourceDiff.reason || (rawDiff ? "" : "该任务当时只保存了文件与行数统计，无法还原逐行代码内容"), 180);
    return {
        path: filePath,
        project,
...
function taskEvidence(tasks, artifactRuns) {
    ...
    for (const task of tasks) {
        ...
        const checks = [...new Set([...(summary.verification_executed || []), ...(summary.verification || []), ...(task.receipt?.verification || [])].map((item) => safeText(item?.command || item?.name || item, 240)).filter(Boolean))];
        if (checks.length)
            evidence.push({ id: stableId("evidence", `${task.id}:checks`), type: "verification", title: "项目子 Agent 验证记录", task_id: task.id, project: task.target_project || "", status: "available", preview_kind: "list", items: checks, retained_until: "任务删除前", url: "" });
    }
    for (const run of artifactRuns)
...
function paginateReplayEventsForView(allEvents, options = {}) {
    ...
    const filteredEvents = hasEventViewOptions ? allEvents.filter(item => {
        ...
        if (!options.includeSystemEvents && lowLevelSource && !["failed", "blocked", "warning"].includes(item.status))
            return false;
...
function buildTaskReplayIndexFromRecords(tasks, groups, input = 40) {
    ...
    const rows = roots.map((task) => {
        ...
        const projects = [...new Set(members.map((item) => safeText(item.target_project, 100)).filter(Boolean))].sort();
