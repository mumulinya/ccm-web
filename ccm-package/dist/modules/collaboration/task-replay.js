...
function buildMessageEvents(tasks) {
    ...
    for (const task of tasks) {
        if (!task.group_id)
            continue;
        const sessionId = String(task.group_session_id || task.groupSessionId || "default");
        const messages = (0, storage_1.getGroupMessages)(task.group_id, sessionId).filter((message) => String(message?.task_id || message?.task?.id || "") === String(task.id));
...
function buildCompleteTaskReplay(taskId) {
...
function buildTaskReplayIndex(limit = 40) {
    const tasks = (0, db_1.loadTasks)();
    const byId = new Map(tasks.map((task) => [String(task.id), task]));
    const roots = tasks.filter((task) => !task.parent_task_id || !byId.has(String(task.parent_task_id)))
    ...
    return {
        ...
        generated_at: new Date().toISOString(),
        total: roots.length,
        tasks: roots.map((task) => {
            const children = tasks.filter((item) => String(item.parent_task_id || "") === String(task.id));
