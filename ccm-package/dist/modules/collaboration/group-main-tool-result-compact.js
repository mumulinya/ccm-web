...
function compactGroupMainToolResultsForPayload(rows = [], budgetTokens = 40_000) {
    ...
    if (before <= budget)
        return { rows: next, changed: false, tokens: before };
    let changed = false;
...
function compactGroupNativeTranscript(messages, rows = [], budgetTokens = 40_000) {
