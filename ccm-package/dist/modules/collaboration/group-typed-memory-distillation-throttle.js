...
exports.TYPED_MEMORY_DISTILLATION_MAX_IDLE_MS = exports.TYPED_MEMORY_DISTILLATION_MIN_PENDING_MESSAGES = void 0;
exports.decideTypedMemoryDistillationRun = decideTypedMemoryDistillationRun;
exports.runTypedMemoryDistillationIfDue = runTypedMemoryDistillationIfDue;
exports.TYPED_MEMORY_DISTILLATION_MIN_PENDING_MESSAGES = Math.max(1, Number(process.env.CCM_TYPED_MEMORY_DISTILLATION_MIN_PENDING_MESSAGES || 12));
exports.TYPED_MEMORY_DISTILLATION_MAX_IDLE_MS = Math.max(10_000, Number(process.env.CCM_TYPED_MEMORY_DISTILLATION_MAX_IDLE_MS || 5 * 60_000));
function timeMs(value) {
    const parsed = Date.parse(String(value || ""));
    return Number.isFinite(parsed) ? parsed : 0;
}
/** 纯决策函数：给定 preflight 信号，判断本轮是否值得进蒸馏事务。 */
function decideTypedMemoryDistillationRun(input = {}) {
    const nowMs = Number(input.nowMs || Date.now());
    const minPending = Math.max(1, Number(input.minPendingMessages || exports.TYPED_MEMORY_DISTILLATION_MIN_PENDING_MESSAGES));
    const maxIdleMs = Math.max(1_000, Number(input.maxIdleMs || exports.TYPED_MEMORY_DISTILLATION_MAX_IDLE_MS));
    ...
    if (input.compacted === true)
        return { ...base, run: true, reason: "compact_boundary_committed" };
    if (input.force === true)
        return { ...base, run: true, reason: "forced" };
    if (input.disabled === true)
        return { ...base, run: false, reason: "disabled" };
    // 台账自身需要维护（驱逐、归档变更、回收）时不受批量阈值限制。
    if (input.maintenanceRequired === true)
        return { ...base, run: true, reason: "ledger_maintenance_required" };
    if (input.recoveryRequired === true)
        return { ...base, run: true, reason: "transaction_recovery_required" };
    if (pendingMessageCount <= 0)
        return { ...base, run: false, reason: "no_pending_messages" };
    if (pendingMessageCount >= minPending)
        return { ...base, run: true, reason: "pending_batch_reached" };
    if (idleMs >= maxIdleMs)
        return { ...base, run: true, reason: "idle_interval_reached" };
    return { ...base, run: false, reason: "throttled_below_batch_and_interval" };
}
/** 节流后的跳过结果，形状与蒸馏的 skipped 返回保持兼容。 */
function throttledSkipResult(groupId, decision, preflight, ledger) {
    return {
 ...
 */
function runTypedMemoryDistillationIfDue(groupId, messages = [], memory = {}, options = {}) {
    ...
    if (!decision.run)
        return throttledSkipResult(groupId, decision, preflight, ledger);
...
//# sourceMappingURL=group-typed-memory-distillation-throttle.js.map