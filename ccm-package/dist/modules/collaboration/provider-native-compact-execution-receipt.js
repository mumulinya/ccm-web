...
exports.PROVIDER_NATIVE_COMPACT_EXECUTION_LEDGER_SCHEMA = exports.PROVIDER_NATIVE_COMPACT_EXECUTION_RECEIPT_SCHEMA = exports.PROVIDER_NATIVE_COMPACT_EXECUTION_RECEIPT_VERSION = void 0;
exports.getProviderNativeCompactExecutionReceiptLedgerFile = getProviderNativeCompactExecutionReceiptLedgerFile;
exports.verifyProviderNativeCompactExecutionReceipt = verifyProviderNativeCompactExecutionReceipt;
exports.buildProviderNativeCompactExecutionReceipt = buildProviderNativeCompactExecutionReceipt;
exports.readProviderNativeCompactExecutionReceiptLedger = readProviderNativeCompactExecutionReceiptLedger;
exports.recordProviderNativeCompactExecutionReceipt = recordProviderNativeCompactExecutionReceipt;
exports.buildProviderNativeCompactExecutionReceiptSummary = buildProviderNativeCompactExecutionReceiptSummary;
...
exports.PROVIDER_NATIVE_COMPACT_EXECUTION_RECEIPT_VERSION = 2;
exports.PROVIDER_NATIVE_COMPACT_EXECUTION_RECEIPT_SCHEMA = "ccm-provider-native-compact-execution-receipt-v2";
exports.PROVIDER_NATIVE_COMPACT_EXECUTION_LEDGER_SCHEMA = "ccm-provider-native-compact-execution-ledger-v2";
const LEGACY_PROVIDER_NATIVE_COMPACT_EXECUTION_RECEIPT_SCHEMA = "ccm-provider-native-compact-execution-receipt-v1";
const ROOT = path.join(utils_1.CCM_DIR, "provider-native-compact-execution-receipts");
...
function planChecksum(value, length = 24) {
...
function betaHeaders(input, requestPatch) {
    return stringList(input.betaHeaders || input.beta_headers || [], requestPatch?.beta_headers || requestPatch?.betaHeaders || [], headerValue(input.headers || input.requestHeaders || input.request_headers, "anthropic-beta").split(","), headerValue(input.headers || input.requestHeaders || input.request_headers, "x-anthropic-beta").split(","));
}
...
function summarizeAppliedEdits(value) {
    return (Array.isArray(value) ? value : []).map((edit) => ({
    ...
    })).filter((edit) => edit.type).slice(0, 24);
}
function responseUsageTokens(providerResponse) {
...
function getProviderNativeCompactExecutionReceiptLedgerFile(groupId, groupSessionId = "") {
    const sessionId = String(groupSessionId || "default").trim() || "default";
    if (sessionId === "default")
        return path.join(ROOT, `${cleanScopePart(groupId)}.json`);
    return path.join(ROOT, cleanScopePart(groupId), `${cleanScopePart(sessionId)}.json`);
}
function verifyProviderNativeCompactExecutionReceipt(receipt, expected = {}) {
    const issues = [];
    const receiptVersion = Number(receipt?.version || 0);
    const supportedSchema = receipt?.schema === exports.PROVIDER_NATIVE_COMPACT_EXECUTION_RECEIPT_SCHEMA && receiptVersion === exports.PROVIDER_NATIVE_COMPACT_EXECUTION_RECEIPT_VERSION
    ...
    for (const [field, wanted] of bindings) {
        if (String(wanted || "").trim() && String(receipt?.[field] || "") !== String(wanted))
            issues.push(field);
    }
    if (receipt?.status === "native_applied" && receiptVersion >= 2) {
        if (receipt?.strong_proof !== true)
            issues.push("native_applied_without_strong_proof");
        if (receipt?.provider_outcome_verified !== true)
            issues.push("native_applied_without_provider_outcome");
        if (Number(receipt?.applied_edit_count || 0) < 1)
            issues.push("native_applied_without_applied_edits");
    }
    return { valid: issues.length === 0, issues, expected_checksum: expectedChecksum };
}
function buildProviderNativeCompactExecutionReceipt(input = {}) {
    ...
    const capacityGeneration = Math.max(1, Number(input.capacityGeneration
    ...
    const base = {
        schema: exports.PROVIDER_NATIVE_COMPACT_EXECUTION_RECEIPT_SCHEMA,
        version: exports.PROVIDER_NATIVE_COMPACT_EXECUTION_RECEIPT_VERSION,
        status,
        ...
        cleared_input_tokens: appliedEdits.reduce((sum, edit) => sum + Number(edit.cleared_input_tokens || 0), 0),
        cleared_thinking_turns: appliedEdits.reduce((sum, edit) => sum + Number(edit.cleared_thinking_turns || 0), 0),
        cleared_tool_uses: appliedEdits.reduce((sum, edit) => sum + Number(edit.cleared_tool_uses || 0), 0),
        sent_at: sentAt,
...
function readProviderNativeCompactExecutionReceiptLedger(groupId, groupSessionId = "") {
    ...
    try {
        ...
        return {
            schema: exports.PROVIDER_NATIVE_COMPACT_EXECUTION_LEDGER_SCHEMA,
            version: exports.PROVIDER_NATIVE_COMPACT_EXECUTION_RECEIPT_VERSION,
            groupId,
...
function recordProviderNativeCompactExecutionReceipt(input = {}) {
    const receipt = [exports.PROVIDER_NATIVE_COMPACT_EXECUTION_RECEIPT_SCHEMA, LEGACY_PROVIDER_NATIVE_COMPACT_EXECUTION_RECEIPT_SCHEMA].includes(String(input?.schema || ""))
        ? input
    ...
    const recorded = (0, atomic_json_file_1.withFileLock)(file, () => {
        ...
        const entryMap = new Map((ledger.entries || []).map((entry) => [entry.receipt_id, entry]));
        const existed = entryMap.has(receipt.receipt_id);
        entryMap.set(receipt.receipt_id, receipt);
        const entries = Array.from(entryMap.values()).sort((a, b) => String(a.sent_at || "").localeCompare(String(b.sent_at || ""))).slice(-320);
    ...
    let sessionCapacityOutcome = null;
    try {
        sessionCapacityOutcome = (0, provider_native_compact_session_capacity_1.recordProviderNativeCompactSessionOutcome)(receipt);
    }
    catch { }
    return { ...recorded, sessionCapacityOutcome };
}
function buildProviderNativeCompactExecutionReceiptSummary(groupId, options = {}) {
    ...
    const entries = (ledger.entries || [])
        .filter((entry) => !targetProject || String(entry.target_project || "").trim().toLowerCase() === targetProject)
        .filter((entry) => !planChecksums.size || planChecksums.has(String(entry.plan_checksum || "")))
        .map((entry) => {
    ...
    return {
        ...
        totals,
        entries,
        native_applied_entries: entries.filter((entry) => entry.status === "native_applied" && entry.checksum_valid).slice(-12).reverse(),
        failed_entries: entries.filter((entry) => ["request_failed", "unverified"].includes(entry.status) || !entry.checksum_valid).slice(-12).reverse(),
        accepted_unverified_entries: entries.filter((entry) => entry.status === "request_accepted").slice(-12).reverse(),
        advisory_entries: entries.filter((entry) => ["no_edits_applied", "advisory_only", "not_supported"].includes(entry.status)).slice(-12).reverse(),
        recent_entries: entries.slice(-20).reverse(),
        updatedAt: ledger.updatedAt || "",
    };
}
//# sourceMappingURL=provider-native-compact-execution-receipt.js.map