...
function runWorkerContextPreDispatchGateSelfTest() {
    ...
    try {
        ...
        const assignment = buildAssignment({ project: "api", agent: "claude-code" }, largeTask, "selftest context budget gate", "", {
            group,
...
function runWorkerContextMemoryFirstCompactionRetrySelfTest() {
    ...
    try {
        ...
        const assignment = buildAssignment({ project: "api", agent: "claude-code" }, task, "selftest memory-first compact retry", "", {
            group,
            memory,
            workerContextUsageOptions: {
                maxTokens: 3800,
...
function runWorkerContextCompletionMemoryCompactionPreservationSelfTest() {
    ...
    const memoryFor = (groupId, padding = "") => {
    ...
    const baseAssignmentFor = (groupId, kind, task) => ({
        project: "api",
        task,
    ...
    const runScenario = (kind, groupId) => {
        const group = normalizeGroupOrchestrator({
            id: groupId,
        ...
        const baseAssignment = baseAssignmentFor(groupId, kind, task);
        const options = {
            group,
        ...
        return {
            ...
            preservation,
            outcome,
            rendered: (0, runtime_kernel_1.renderWorkerContextPacket)(result.packet),
        };
    };
    try {
        const scenarios = groups.map((groupId, index) => runScenario(["memory", "replay", "metadata", "ptl"][index], groupId));
        const memoryScenario = scenarios.find(row => row.kind === "memory") || {};
        const replayScenario = scenarios.find(row => row.kind === "replay") || {};
        const metadataScenario = scenarios.find(row => row.kind === "metadata") || {};
        const ptlScenario = scenarios.find(row => row.kind === "ptl") || {};
        ...
        const tamperedContract = {
            ...
            memory_receipt_required_doc_rel_paths: (validContract.memory_receipt_required_doc_rel_paths || []).filter((relPath) => relPath !== conflictResolutionDoc),
...
function runWorkerContextIgnoreMemoryPolicySelfTest() {
    ...
    try {
        ...
        const binding = recordWorkerContextPacketAssignmentBindingForCoordinator(groupId, assignment) || {};
        const rendered = (0, runtime_kernel_1.renderWorkerContextPacket)(packet);
        const ledger = readReplayRepairDispatchBindingLedgerForCoordinator(groupId);
        const persisted = (ledger.entries || []).find((item) => item.assignment_id === baseAssignment.assignmentId) || {};
        ...
        const checks = {
            ...
            usageCategorizesPolicy: Number(categories.get("memory_policy")?.tokens || 0) > 0,
...
function runWorkerContextPressureProvenanceProviderDispatchGateSelfTest() {
    ...
    try {
        ...
        const checks = {
            ...
            usageCategorizesProviderAdvisory: Number(categories.get("pressure_provenance_provider_dispatch_advisory")?.tokens || 0) > 0,
...
function runWorkerContextProviderDispatchOverrideFollowupReceiptValidationPolicySelfTest() {
    ...
    try {
        ...
        const escalatedAdvisory = buildPressureProvenanceProviderDispatchAdvisoryForCoordinator(groupId, targetProject, agentType, escalatedPolicy) || {};
        ...
        const repairedAdvisory = buildPressureProvenanceProviderDispatchAdvisoryForCoordinator(groupId, targetProject, agentType, repairedPolicy) || {};
...
function runWorkerContextCrossGroupProviderReliabilityGuidanceSelfTest() {
    ...
    try {
        ...
        const crossAdvisory = buildPressureProvenanceProviderDispatchAdvisoryForCoordinator(targetGroup, targetProject, agentType, crossPolicy) || {};
        ...
        const localAdvisory = buildPressureProvenanceProviderDispatchAdvisoryForCoordinator(targetGroup, targetProject, agentType, localPolicy) || {};
...
function runWorkerContextProviderReliabilitySnapshotRankingSelfTest() {
    ...
    try {
        ...
        const advisory = buildPressureProvenanceProviderDispatchAdvisoryForCoordinator(targetGroup, targetProject, "codex", policy, {
            group: {
                id: targetGroup,
                members: [{
                        project: targetProject,
        ...
        const localAdvisory = buildPressureProvenanceProviderDispatchAdvisoryForCoordinator(targetGroup, targetProject, "codex", localPolicy, {
...
function runWorkerContextProviderSwitchExecutionRankingSelfTest() {
    ...
    try {
        ...
        const advisory = buildPressureProvenanceProviderDispatchAdvisoryForCoordinator(groupId, project, currentProvider, policy, {
            group: {
                id: groupId,
                members: [{
                        project,
        ...
        const compactRetry = maybeRetryWorkerContextPacketCompactionForCoordinator(compactAssignment, "", [], compactInitialPacket, compactInitialGate, compactOptions);
...
function runWorkerContextProviderSwitchDecisionReceiptSelfTest() {
    ...
    try {
        ...
        switchedAssignment.worker_context_provider_dispatch_decision = buildWorkerContextProviderDispatchDecisionForCoordinator(switchedAssignment, switchedPacket, switchedAssignment.worker_context_pre_dispatch_gate, { at: nowAt });
        ...
        mismatchAssignment.worker_context_provider_dispatch_decision = buildWorkerContextProviderDispatchDecisionForCoordinator(mismatchAssignment, mismatchPacket, mismatchAssignment.worker_context_pre_dispatch_gate, { at: nowAt });
        ...
        advisedOnlyAssignment.worker_context_provider_dispatch_decision = buildWorkerContextProviderDispatchDecisionForCoordinator(advisedOnlyAssignment, packet, advisedOnlyAssignment.worker_context_pre_dispatch_gate, { at: nowAt });
        ...
        const compactRetry = maybeRetryWorkerContextPacketCompactionForCoordinator(largeAssignment, "", [], largePacket, largeGate, {
            group: { id: targetGroup, members: [{ project, agent: newProvider }] },
            providerSwitchDecisionReceipt: receipt,
            workerContextUsageOptions: { maxTokens: 2200, autoCompactBufferTokens: 120 },
            workerContextRetryOptions: { maxTaskChars: 1400 },
        });
        const compactRetryDecision = buildWorkerContextProviderDispatchDecisionForCoordinator(largeAssignment, compactRetry.packet, compactRetry.gate, { at: nowAt });
...
function runWorkerContextPressureProvenanceProviderDispatchDecisionLedgerSelfTest() {
    ...
    try {
        ...
        const activeAssignment = buildAssignment({ project: targetProject, agent: agentType }, "验证 pressure provenance provider dispatch decision ledger 会记录 hold 决策。", "selftest pressure provenance provider dispatch decision", "", { group, workerContextUsageOptions: { maxTokens: 5000, autoCompactBufferTokens: 120 } });
        (0, group_memory_index_1.distillPressureProvenancePreDispatchComplianceRecoveryToTypedMemory)(groupId, {
            packets: [
                {
                    groupId,
        ...
        const recoveredAssignment = buildAssignment({ project: targetProject, agent: agentType }, "验证 pressure provenance provider dispatch decision ledger 会记录恢复后的 sampling 放行决策。", "selftest pressure provenance provider dispatch decision recovered", "", { group, workerContextUsageOptions: { maxTokens: 5000, autoCompactBufferTokens: 120 } });
        const ledger = readReplayRepairDispatchBindingLedgerForCoordinator(groupId);
...
function runWorkerContextPressureProvenanceProviderDispatchOverrideReceiptSelfTest() {
    ...
    try {
        ...
        const invalidAssignment = buildAssignment({ project: targetProject, agent: agentType }, "验证无效 provider dispatch override 不会绕过 hold。", "selftest invalid provider override", "", {
            group,
        ...
        const validAssignment = buildAssignment({ project: targetProject, agent: agentType }, "验证有效 provider dispatch override receipt 可以一次性放行。", "selftest valid provider override", "", {
            group,
...
function runWorkerContextPressureProvenanceProviderDispatchOverrideCompletionSelfTest() {
    ...
    try {
        ...
        const assignment = buildAssignment({ project: targetProject, agent: agentType }, "验证 provider dispatch override completion 会关闭 follow-up repair work item。", "selftest provider override completion", "", {
            group,
            providerDispatchOverride: validOverride,
            workerContextUsageOptions: { maxTokens: 5000, autoCompactBufferTokens: 120 },
        });
...
function maybeRetryWorkerContextPacketCompactionForCoordinator(baseAssignment, dependsOn, replayRepairDispatchBriefs, initialPacket, initialGate, options = {}) {
    ...
    const replayBriefPartialCompact = compactReplayRepairDispatchBriefsForWorkerContextRetry(activeReplayRepairDispatchBriefs, retryOptions.replayRepairDispatchBriefs || retryOptions.replay_repair_dispatch_briefs || retryOptions.partialCompact || retryOptions.partial_compact || {});
    ...
    const metadataPartialCompact = compactWorkerContextMetadataCategoriesForRetry(initialPacket, options, {
...
function buildWorkerContextPreDispatchGateForCoordinator(assignment = {}, packet = {}) {
    ...
    const completionMemoryPreservation = normalizePostCompactReceiptMemoryUsageRepairCompletionPreservationForCoordinator(retry?.post_compact_receipt_memory_usage_repair_completion_preservation
        || retry?.postCompactReceiptMemoryUsageRepairCompletionPreservation
        || null);
    ...
    const providerOverrideReceipt = normalizeProviderDispatchOverrideReceiptForCoordinator(rawProviderDispatchOverrideForCoordinator(assignment, packet), {
...
function recordWorkerContextPacketAssignmentBindingForCoordinator(groupId, assignment = {}, options = {}) {
    ...
    const next = {
        ...
        nativeBindingCount: entries.filter((item) => isApiMicrocompactNativeProofRepairSourceForCoordinator(item.source)).length,
...
function recordReplayRepairDispatchBriefTimelineBinding(groupId, input = {}, options = {}) {
    ...
    const entry = {
        ...
        provider_ranking_provenance_rel_paths: providerRankingProof.typedMemoryRelPaths?.length
            ...
            : providerRankingProvenanceProofStringListForCoordinator(brief.provider_ranking_provenance_rel_paths, brief.providerRankingProvenanceRelPaths, input.provider_ranking_provenance_rel_paths, input.providerRankingProvenanceRelPaths).slice(0, 24),
        provider_ranking_provenance_row_ids: providerRankingProof.typedMemoryRowIds?.length
            ? providerRankingProof.typedMemoryRowIds
            : providerRankingProvenanceProofStringListForCoordinator(brief.provider_ranking_provenance_row_ids, brief.providerRankingProvenanceRowIds, input.provider_ranking_provenance_row_ids, input.providerRankingProvenanceRowIds).slice(0, 32),
    ...
    if (existingIndex >= 0)
        entries[existingIndex] = mergeReplayRepairTimelineBinding(entries[existingIndex], entry);
    else
        entries.push({ ...entry, first_seen_at: at });
    const next = {
        schema: "ccm-replay-repair-main-agent-dispatch-brief-timeline-ledger-v1",
    ...
    if (eventType === "child_agent_receipt") {
        attachReplayRepairAssignmentReceiptForCoordinator(groupId, finalEntry, input.receipt || input.ccm_receipt || input.delivery_summary || null, at);
    }
