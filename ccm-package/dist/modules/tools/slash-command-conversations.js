...
function rewindApply(input) {
    ...
    if (boundary.id.scope === "global") {
        ...
        const result = (0, sessions_1.replaceProjectSessionConversation)(boundary.id.scopeId, boundary.id.sessionId, kept, "用户通过 /rewind 回退会话");
        generation = Number(result.generation || generation);
    }
    else {
        (0, execution_kernel_1.requestGroupSessionAgentCancellation)({ groupId: boundary.id.scopeId, groupSessionId: boundary.id.sessionId, reason: "用户通过 /rewind 回退会话", actor: "slash-command" });
        (0, storage_1.saveGroupMessages)(boundary.id.scopeId, kept, boundary.id.sessionId);
        generation = Number((0, group_session_lifecycle_head_1.rotateGroupSessionLifecycleHead)({ groupId: boundary.id.scopeId, groupSessionId: boundary.id.sessionId, reason: "用户通过 /rewind 回退会话" }).head?.generation || generation);
    }
...
function handleSlashCommandConversationApi(pathname, req, res, parsed) {
    ...
    if (!handler || !["POST", "PATCH"].includes(String(req.method)))
        return false;
    void body(req).then(value => (0, utils_1.sendJson)(res, { success: true, result: handler(value) })).catch((error) => (0, utils_1.sendJson)(res, { success: false, error: error.message || String(error) }, 400));
    return true;
}
