...
exports.runConversationAside = runConversationAside;
...
const activeAsideRequests = new Map();
...
function asideModelMessages(input) {
    const boundary = currentBoundary(input);
    const question = String(input.question || input.message || "").trim();
    if (!question)
        throw new Error("请输入临时问题");
    if (question.length > 4_000)
        throw new Error("临时问题最多4000个字符");
...
async function runAside(input, options = {}) {
    const prepared = asideModelMessages(input);
    if (activeAsideRequests.has(prepared.boundary.id.key))
        throw new Error("当前会话已有一个临时提问正在回答");
    ...
    let answer = "";
    try {
        answer = await (0, global_agent_model_1.callGlobalModelWithRetry)((0, group_orchestrator_config_1.loadOrchestratorConfig)(), prepared.messages, {
            retryProfile: "interactive_first_turn",
            signal: controller.signal,
            onDelta: options.onDelta,
            onUsage: value => { usage = value; },
        });
        (0, db_1.recordMetric)(prepared.boundary.id.scope === "global" ? "global-agent" : `${prepared.boundary.id.scope}-main-agent`, {
            success: true,
        ...
        if (activeAsideRequests.get(prepared.boundary.id.key) === controller)
            activeAsideRequests.delete(prepared.boundary.id.key);
    }
}
async function runConversationAside(input, options = {}) {
...
function handleSlashCommandConversationApi(pathname, req, res, parsed) {
    ...
    if (clarificationAction && req.method === "POST") {
        ...
        }).then(result => (0, utils_1.sendJson)(res, { success: true, result })).catch((error) => (0, utils_1.sendJson)(res, { success: false, error: error.message || String(error), code: error.code }, error.statusCode || 409));
        return true;
    }
    ...
    if (pathname === "/api/conversations/aside" && req.method === "POST") {
        void body(req).then(async (value) => {
            ...
            if (!stream)
                return (0, utils_1.sendJson)(res, { success: true, result: await runAside(value) });
            res.writeHead(200, {
