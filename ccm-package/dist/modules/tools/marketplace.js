...
async function loadSmitheryItems() {
    ...
    const items = servers.map((server) => normalizeMarketplaceItem({
...
async function loadMarketplaceItemsForSource(source) {
    ...
    if (baseSourceId === "smithery" || source.kind === "smithery") {
        ...
        if (result.needKey)
            throw new Error("Smithery API Key 未配置，无法复验安装来源");
        return result.items;
    }
...
function handleMarketplaceApi(pathname, req, res, parsed) {
    ...
    if (pathname === "/api/marketplace/list" && req.method === "GET") {
        ...
        (async () => {
            if (source === "local")
                return { items: localMarketplaceItems(), needKey: false };
            if (source === "smithery")
                return loadSmitheryItems();
            if (source === "github") {
                return {
            ...
            .then(result => (0, utils_1.sendJson)(res, { success: true, needKey: result.needKey, items: decorateInstallState(result.items) }))
