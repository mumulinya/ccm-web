...
exports.handleAnimeCoverApi = handleAnimeCoverApi;
...
function contentTypeForPath(filePath, buffer) {
    const lower = (filePath || "").toLowerCase();
    if (lower.endsWith(".png") || (buffer && buffer[0] === 0x89))
        return "image/png";
    if (lower.endsWith(".gif") || (buffer && buffer.toString("ascii", 0, 3) === "GIF"))
        return "image/gif";
    if (lower.endsWith(".webp") || (buffer && buffer.toString("ascii", 8, 12) === "WEBP"))
        return "image/webp";
    return "image/jpeg";
}
...
function sendLocalAnimeCover(res, preferredHash = "", cacheControl = "no-cache", cachePath = "") {
    const anime = pickLocalAnimeCover(preferredHash || String(Date.now()));
    if (anime) {
        if (cachePath) {
            try {
                fs.writeFileSync(cachePath, anime.buffer);
            }
            catch { }
        }
...
function sendDefaultCover(res, cachePath = "", preferredHash = "", cacheControl = "public, max-age=31536000") {
 ...
 *   ?seed=xxx         — 本地按种子稳定选
 */
function handleAnimeCoverApi(res, parsed) {
    const query = parsed?.query || {};
    const nRaw = String(query.n || "").trim();
    const seed = String(query.seed || "").trim();
    const forceLocal = query.local === "1" ||
        query.local === "true" ||
        /^\d+$/.test(nRaw) ||
        !!seed;
    if (forceLocal) {
...
function handleMusicCoverApi(res, parsed) {
    ...
    try {
        const embedded = (0, library_1.getMp3Cover)(filePath);
        if (embedded?.data?.length && looksLikeImage(embedded.data)) {
            ...
            try {
                fs.writeFileSync(embeddedPath, embedded.data);
            }
            catch { }
            try {
                fs.writeFileSync(cachePath, embedded.data);
            }
            catch { }
