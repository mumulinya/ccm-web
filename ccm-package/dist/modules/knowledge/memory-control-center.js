...
exports.buildGroupPostCompactUsageDiagnostics = buildGroupPostCompactUsageDiagnostics;
...
exports.buildPostCompactCandidateDisciplineTrend = buildPostCompactCandidateDisciplineTrend;
exports.buildPostCompactDispatchMarkerTrend = buildPostCompactDispatchMarkerTrend;
exports.buildChildAgentMemoryReliabilityReport = buildChildAgentMemoryReliabilityReport;
exports.getGroupCompactBoundaryReplayRepairLedgerFile = getGroupCompactBoundaryReplayRepairLedgerFile;
exports.readGroupCompactBoundaryReplayRepairLedger = readGroupCompactBoundaryReplayRepairLedger;
exports.getGroupCompactBoundaryReplayRepairWorkItemsFile = getGroupCompactBoundaryReplayRepairWorkItemsFile;
exports.readGroupCompactBoundaryReplayRepairWorkItems = readGroupCompactBoundaryReplayRepairWorkItems;
exports.buildReplayRepairMainAgentDispatchCandidates = buildReplayRepairMainAgentDispatchCandidates;
exports.updateCompactBoundaryReplayRepairWorkItem = updateCompactBoundaryReplayRepairWorkItem;
exports.buildMemoryQualityReport = buildMemoryQualityReport;
...
function compactMemoryCenterText(value, max = 360) {
    const text = String(value || "").replace(/\r\n/g, "\n").replace(/[ \t]+$/gm, "").trim();
    return text.length > max ? `${text.slice(0, Math.max(1, max - 1))}…` : text;
}
function summarizePostCompactUsageRow(row = {}) {
    return {
...
function summarizePostCompactUsageEntry(entry = {}) {
    return {
...
function buildGroupPostCompactUsageOverview(groupId) {
...
function buildGroupPostCompactRecallProbeQuery(memory = {}, usageSummary = {}, archive = {}) {
    const rows = [
    ...
    const memoryHints = [
        ...
        ...(Array.isArray(memory?.persistentRequirements) ? memory.persistentRequirements.map((item) => item?.text || item) : []),
        ...(Array.isArray(memory?.factAnchors) ? memory.factAnchors.map((item) => item?.text || item) : []),
        ...(Array.isArray(memory?.decisions) ? memory.decisions.map((item) => item?.decision || item?.text || item) : []),
    ];
    const usageHints = rows.map((row) => row?.value || row?.candidate_id || "").filter(Boolean);
...
function buildGroupPostCompactUsageDiagnostics(groupId, memory = {}) {
    const id = String(groupId || "").trim();
    if (!id)
        return null;
    try {
        const { readGroupPostCompactCandidateUsageLedger, buildGroupPostCompactCandidateUsageSummary, buildGroupCompactFileReferences, buildGroupCompactFileReferenceReadPlan, buildGroupMemorySourceManifest, summarizeGroupCompactFileReferenceAccess, summarizeGroupCompactFileReferenceReadPlanAccess, summarizeGroupCompactFileReferenceReadPlanFreshness, buildGroupCompactFileReferenceReadPlanRevalidationGate, latestGroupCompactFileReferenceReadPlanRevalidationGate, } = require("../collaboration/memory");
        ...
        const docsByType = (Array.isArray(docs) ? docs : []).reduce((acc, doc) => {
        ...
        const compactFileReferences = buildGroupCompactFileReferences(id, {
            ...
            typedMemory: {
                sync: {
                    index_file: docs.find((doc) => String(doc.relPath || "").toUpperCase() === "MEMORY.md")?.file || "",
                    memory_dir: path.dirname(docs[0]?.file || path.join(utils_1.CCM_DIR, "group-memory-md", sidecarFileId(id), "MEMORY.md")),
                },
            },
            rawSources: {
                group_memory_file: path.join(GROUP_MEMORY_DIR, `${id}.json`),
                group_messages_file: path.join(utils_1.GROUP_MESSAGES_DIR, `${id}.json`),
        ...
        const compactFileReferenceReadPlanForFreshness = {
            ...
            plannedCount: surfacedReadPlanRows.rows.filter((entry) => entry.action !== "skip_missing").length,
        ...
        const compactFileReferenceReadPlanRevalidationSessionBinding = compactFileReferenceReadPlanRevalidationDiscipline ? {
            ...
            sessionMatchRate: qualityRate(Number(compactFileReferenceReadPlanRevalidationDiscipline.sessionMatched || 0), Number(compactFileReferenceReadPlanRevalidationDiscipline.sessionRequired || 0)),
            rows: (Array.isArray(compactFileReferenceReadPlanRevalidationDiscipline.rows) ? compactFileReferenceReadPlanRevalidationDiscipline.rows : [])
                .filter((row) => row.session_required)
                .slice(0, 20),
            gaps: (Array.isArray(compactFileReferenceReadPlanRevalidationDiscipline.gaps) ? compactFileReferenceReadPlanRevalidationDiscipline.gaps : [])
                .filter((gap) => gap.session_mismatch)
                .slice(0, 12),
        } : null;
        ...
        return {
            ...
            typedMemory: {
                ...
                boostedDocs: recallDiagnostics
                    .filter((item) => Number(item.postCompactUsage?.adjustment || 0) > 0)
                    .slice(0, 8)
                    .map((item) => ({ relPath: item.relPath, score: Number(item.score || 0), adjustment: Number(item.postCompactUsage?.adjustment || 0), matched: item.postCompactUsage?.matched || [] })),
                deprioritizedDocs: recallDiagnostics
                    .filter((item) => Number(item.postCompactUsage?.adjustment || 0) < 0)
                    .slice(0, 8)
                    .map((item) => ({ relPath: item.relPath, score: Number(item.score || 0), adjustment: Number(item.postCompactUsage?.adjustment || 0), matched: item.postCompactUsage?.matched || [] })),
                diagnostics: recallDiagnostics.slice(-16).map((item) => ({
...
function tokenizeQualityText(value) {
...
function qualityRate(ok, total) {
    return total > 0 ? Math.round((ok / total) * 1000) / 10 : null;
}
function makeQualityCheck(id, label, checked, passed, evidence = [], gaps = [], note = "") {
...
function normalizeQualityStringList(value) {
...
function normalizeQualityPostCompactUsageState(value) {
...
function collectPostCompactQualityRows(summary = {}) {
...
function summarizePostCompactQualityRow(row = {}) {
    return {
...
function evaluatePostCompactCandidateDiscipline(options = {}) {
    const taskLimit = Math.max(5, Number(options.taskLimit || 40));
    const tasks = Array.isArray(options.tasks) ? options.tasks : (0, db_1.loadTasks)().slice(-taskLimit);
    ...
    const getUsageSummary = (groupId) => {
        if (!groupId)
            return null;
        if (usageSummaries.has(groupId))
            return usageSummaries.get(groupId);
        try {
            ...
            const historicallyIgnoredRows = Object.values(usageLedger?.stats || {}).filter((row) => Number(row?.ignored_count || 0) > 0);
    ...
    for (const task of tasks.filter((item) => item?.delivery_summary)) {
        ...
        for (const rawRow of rows) {
            ...
            checked++;
            const stale = staleRows.find((item) => postCompactQualityCandidateMatches(row, item));
    ...
    const check = makeQualityCheck("post_compact_candidate_discipline", "压缩候选纪律", checked, passed, evidence, gaps, "检查压缩后重注入候选是否逐条 classified，并防止历史 ignored/归档候选未经 verified 又被 promoted 为 used。");
...
function qualityTaskTime(task = {}) {
    return String(task.completed_at
        || task.completedAt
        || task.updated_at
        || task.updatedAt
...
function qualityTaskTimeMs(task = {}) {
    const parsed = Date.parse(qualityTaskTime(task));
    return Number.isFinite(parsed) ? parsed : 0;
}
function postCompactDisciplineBucketKey(value) {
    const parsed = Date.parse(String(value || ""));
    if (!Number.isFinite(parsed))
        return "unknown";
    return new Date(parsed).toISOString().slice(0, 10);
}
function makePostCompactDisciplineStats(groupId) {
    return {
        groupId,
        taskCount: 0,
        checked: 0,
...
function addPostCompactDisciplineBucket(stats, bucketKey, sample = {}) {
    ...
    if (sample.taskId && !bucket.taskIds?.has?.(sample.taskId)) {
        bucket.taskIds = bucket.taskIds || new Set();
        bucket.taskIds.add(sample.taskId);
        bucket.taskCount = bucket.taskIds.size;
    }
    stats.buckets.set(key, bucket);
}
function finalizePostCompactDisciplineStats(stats, threshold, minSample) {
    ...
    return {
        ...
        buckets,
        ledger: stats.ledger,
        recentRows: (stats.recentRows || []).sort((a, b) => Number(b.atMs || 0) - Number(a.atMs || 0)).slice(0, 12).map((row) => {
            const { atMs, ...rest } = row;
            return rest;
        }),
        stalePromotions: (stats.stalePromotions || []).sort((a, b) => Number(b.atMs || 0) - Number(a.atMs || 0)).slice(0, 8).map((row) => {
            const { atMs, ...rest } = row;
            return rest;
        }),
    };
}
function buildPostCompactCandidateDisciplineTrend(options = {}) {
    ...
    for (const task of tasks) {
        const groupId = String(task?.group_id || task?.groupId || "").trim();
        if (groupId && (!explicitGroupIds || groupIds.has(groupId)))
            groupIds.add(groupId);
    }
    const usageSummaries = new Map();
    const staleByGroup = new Map();
    ...
    const ensureStats = (groupId) => {
        const id = groupId || "unknown";
        if (!groups.has(id))
            groups.set(id, makePostCompactDisciplineStats(id));
    ...
    for (const task of tasks.filter((item) => item?.delivery_summary)) {
        const taskGroupId = String(task.group_id || task.groupId || "").trim();
        if (explicitGroupIds && !groupIds.has(taskGroupId))
            continue;
        const summary = task.delivery_summary || {};
        const gateSummary = summary.post_compact_reinjection_gate_summary || summary.postCompactReinjectionGateSummary || {};
        const required = gateSummary.required === true
        ...
        for (const rawRow of rows) {
            ...
            stats.checked++;
            if (strictPass)
                stats.strictClassified++;
            if (!classified) {
                stats.unclassified++;
                if (state === "mentioned" || state === "unreferenced" || state === "unclassified")
                    stats.missing++;
            }
            if (stalePromoted)
                stats.stalePromoted++;
            if (stats.stateCounts[state] === undefined)
                stats.stateCounts[state] = 0;
            ...
            if (stalePromoted)
                stats.stalePromotions.push(recent);
        }
    }
    for (const groupId of groupIds) {
    ...
    const groupRows = Array.from(groups.values())
        .map((stats) => finalizePostCompactDisciplineStats(stats, threshold, minSample))
        .sort((a, b) => {
    ...
    const overall = groupRows.reduce((acc, group) => {
    ...
    return {
        ...
        overall,
        groups: groupRows,
        alertGroups: groupRows.filter((group) => group.alert).slice(0, 12),
    };
}
const POST_COMPACT_DISPATCH_WAIT_ALERT_MS = 30 * 60 * 1000;
function compactMemoryHasPostCompactBoundary(memory = {}) {
    const compaction = memory?.compaction || {};
    const boundary = memory?.compactBoundary || {};
    const restore = boundary.post_compact_restore || {};
    const compactedMessageCount = Number(compaction.compactedMessageCount
        ...
        || 0);
    const summaryChecksum = String(compaction.summaryChecksum
    ...
    return {
        ...
        preCompactTokenCount: Number(compaction.preCompactTokenCount
            ...
            || 0),
        postCompactTokenCount: Number(compaction.postCompactTokenCount
...
function summarizePostCompactDispatchEntry(entry = {}) {
    return {
...
function postCompactDispatchScopeKey(entry = {}) {
    return String(entry.scope || entry.target_project || entry.targetProject || "child").trim() || "child";
}
function makePostCompactDispatchStats(groupId, memory = {}) {
    ...
    return {
        ...
        boundaries: new Map(),
        latestBoundaryId: "",
        latestBoundaryEntries: [],
        ledger: { file: "", updatedAt: "" },
    };
}
function finalizePostCompactDispatchStats(stats, options = {}) {
    ...
    const firstDispatches = entries.filter((entry) => entry.first_dispatch_after_compact === true);
    const followups = entries.filter((entry) => entry.first_dispatch_after_compact !== true);
    ...
    for (const entry of entries) {
        ...
        if (entry.first_dispatch_after_compact)
            target.firstDispatchCount++;
        else
            target.followupDispatchCount++;
        ...
        const boundary = boundaries.get(boundaryId) || { boundary_id: boundaryId, summary_checksum: entry.summary_checksum, dispatchCount: 0, firstDispatchCount: 0, followupDispatchCount: 0, scopes: new Set(), firstScopes: new Set(), latestDispatchAt: "" };
    ...
    const currentBoundaryByChecksum = currentChecksum
        ? entries.filter((entry) => entry.summary_checksum === currentChecksum)
        : [];
    const latestBoundaryId = currentBoundaryByChecksum[0]?.boundary_id
        || entries[entries.length - 1]?.boundary_id
        || "";
    const latestBoundaryEntries = latestBoundaryId ? entries.filter((entry) => entry.boundary_id === latestBoundaryId) : [];
    const latestScopes = new Set(latestBoundaryEntries.map(postCompactDispatchScopeKey));
    const latestFirstScopes = new Set(latestBoundaryEntries.filter((entry) => entry.first_dispatch_after_compact).map(postCompactDispatchScopeKey));
    ...
    const firstAfterCompact = currentBoundaryByChecksum.find((entry) => entry.first_dispatch_after_compact)
        || firstDispatches[0]
        || null;
    ...
    const invariantGapCount = gaps.filter((gap) => !String(gap.reason || "").includes("尚未观测")).length;
        ...
        : staleWaiting
            ...
            : stats.boundary?.hasBoundary && entries.length === 0
                ...
                : entries.length > 0
                    ? "ok"
                    : "empty";
    const boundaryRows = Array.from(boundaries.values()).map((boundary) => ({
    ...
    })).sort((a, b) => String(b.latestDispatchAt || "").localeCompare(String(a.latestDispatchAt || "")));
    return {
        ...
        waitingAgeMs,
        targets: Array.from(targets.values()).sort((a, b) => String(a.scope || "").localeCompare(String(b.scope || ""))).slice(0, 20),
...
function buildPostCompactDispatchMarkerTrend(options = {}) {
    const explicitGroupIds = Array.isArray(options.groupIds || options.group_ids)
        ? (options.groupIds || options.group_ids).map((item) => String(item || "").trim()).filter(Boolean)
        : null;
    const groupIds = new Set(explicitGroupIds || []);
    const memoryByGroup = new Map();
    if (!explicitGroupIds) {
        for (const file of listJsonFiles(GROUP_MEMORY_DIR)) {
            const memory = readMemoryFile(file);
            const id = String(memory?.groupId || path.basename(file, ".json")).trim();
    ...
    rows.sort((a, b) => {
        const rank = { fail: 0, warn: 1, waiting: 2, ok: 3, empty: 4 };
        return Number(rank[a.status] ?? 9) - Number(rank[b.status] ?? 9)
            || Number(b.entryCount || 0) - Number(a.entryCount || 0);
    });
    const overall = rows.reduce((acc, group) => {
        acc.groups += 1;
        if (group.compacted)
            acc.compactedGroups += 1;
        if (group.entryCount > 0)
            acc.groupsWithDispatch += 1;
    ...
    return {
        ...
        overall,
        groups: rows,
        alertGroups: rows.filter((group) => group.alert).slice(0, 12),
    };
}
function evaluatePostCompactDispatchContinuity(options = {}) {
    const trend = buildPostCompactDispatchMarkerTrend(options);
    const checked = Number(trend.overall.compactedGroups || 0)
        + (trend.groups || []).filter((group) => group.compacted !== true && Number(group.entryCount || 0) > 0).length;
    const failedGroups = (trend.groups || []).filter((group) => group.status === "fail" || group.status === "warn");
    const waitingGroups = (trend.groups || []).filter((group) => group.status === "waiting");
    ...
    const gaps = [
        ...
        ...waitingGroups.slice(0, 6).map((group) => ({
            groupId: group.groupId,
    ...
    const check = makeQualityCheck("post_compact_dispatch_marker", "压缩后首派发", checked, passed, evidence, gaps, "检查每个群聊 compact boundary 后的子 Agent 首次派发是否被一次性 marker 记录，并暴露等待或异常序列。");
    check.trend = trend;
    return check;
}
function normalizeReliabilityAgent(value) {
    const text = String(value || "").trim();
    if (!text)
        return "";
    return text.replace(/^child:/, "").trim() || text;
}
function makeChildAgentReliabilityStats(groupId, agent) {
    return {
        groupId,
        agent,
        receiptTasks: new Set(),
        memoryDeclaredTasks: new Set(),
        memoryUsedTasks: new Set(),
        memoryIgnoredTasks: new Set(),
        candidateChecked: 0,
        candidateStrict: 0,
        candidateMissing: 0,
        stalePromoted: 0,
...
function addReliabilityEvidence(stats, item) {
    stats.evidence.push(item);
    if (stats.evidence.length > 16)
        stats.evidence = stats.evidence.slice(-16);
}
function addReliabilityGap(stats, item) {
    stats.gaps.push(item);
    if (stats.gaps.length > 16)
        stats.gaps = stats.gaps.slice(-16);
}
function scoreChildAgentReliability(stats, groupCompacted = false) {
...
function buildChildAgentMemoryReliabilityReport(options = {}) {
    const taskLimit = Math.max(5, Number(options.taskLimit || 120));
    const tasks = Array.isArray(options.tasks) ? options.tasks : (0, db_1.loadTasks)().slice(-taskLimit);
    const explicitGroupIds = Array.isArray(options.groupIds || options.group_ids)
        ? (options.groupIds || options.group_ids).map((item) => String(item || "").trim()).filter(Boolean)
    ...
    const staleByGroup = new Map();
    const getStaleRows = (groupId) => {
        if (staleByGroup.has(groupId))
            return staleByGroup.get(groupId) || [];
        try {
            ...
            const archiveRows = Array.isArray(distillationLedger?.postCompactUsageArchive?.rows) ? distillationLedger.postCompactUsageArchive.rows : [];
            const rows = [
                ...Object.values(usageLedger?.stats || {}).filter((row) => Number(row?.ignored_count || 0) > 0),
                ...archiveRows,
            ];
    ...
    const ensureAgent = (groupId, agentValue) => {
        const group = ensureGroup(groupId);
        const agent = normalizeReliabilityAgent(agentValue) || "unknown";
        if (!group.agents.has(agent))
            group.agents.set(agent, makeChildAgentReliabilityStats(group.groupId, agent));
    ...
    for (const dispatchGroup of dispatchTrend.groups || []) {
        for (const target of dispatchGroup.targets || []) {
            ...
            if (Number(target.firstDispatchCount || 0) <= 0)
                addReliabilityGap(stats, { type: "post_compact_dispatch", reason: "压缩后 dispatch 有记录但缺少 first_dispatch_after_compact", dispatchCount: target.dispatchCount });
        }
    }
    for (const task of tasks.filter((item) => item?.delivery_summary)) {
        const groupId = String(task.group_id || task.groupId || "").trim();
        if (explicitGroupIds && !groupIds.has(groupId))
            continue;
        const taskId = String(task.id || task.task_id || task.taskId || "");
        const summary = task.delivery_summary || {};
        ...
        for (const receipt of receiptByAgent.values()) {
            ...
            if (taskId)
                stats.receiptTasks.add(taskId);
            if (receipt.declared)
                stats.memoryDeclaredTasks.add(taskId || `${receipt.agent}:${stats.receiptTasks.size}`);
            if (receipt.used.length)
                stats.memoryUsedTasks.add(taskId || `${receipt.agent}:used:${stats.memoryUsedTasks.size}`);
            if (receipt.ignored.length)
                stats.memoryIgnoredTasks.add(taskId || `${receipt.agent}:ignored:${stats.memoryIgnoredTasks.size}`);
            if (receipt.used.length)
                addReliabilityEvidence(stats, { type: "memory_used", taskId, values: receipt.used.slice(0, 4) });
            else if (receipt.ignored.length)
                addReliabilityGap(stats, { type: "memory_ignored", taskId, reason: receipt.ignored.slice(0, 2).join("；") || "子 Agent 声明未使用记忆" });
            else
                addReliabilityGap(stats, { type: "memory_receipt", taskId, reason: "子 Agent 回执未声明 memoryUsed/memoryIgnored" });
        }
    ...
    const groups = Array.from(groupStats.values()).map((group) => {
        ...
        const agents = Array.from(group.agents.values())
            .map((stats) => scoreChildAgentReliability(stats, group.compacted))
            .sort((a, b) => {
        ...
        const scored = agents.filter((agent) => agent.score !== null && agent.score !== undefined);
        const score = scored.length ? Math.round(scored.reduce((sum, agent) => sum + Number(agent.score || 0), 0) / scored.length * 10) / 10 : null;
        return {
            ...
            agents,
            gaps: agents.flatMap((agent) => (agent.gaps || []).map((gap) => ({ agent: agent.agent, ...gap }))).slice(0, 12),
        };
    }).sort((a, b) => {
        const rank = { fail: 0, warn: 1, ok: 2, empty: 3 };
        return Number(rank[a.status] ?? 9) - Number(rank[b.status] ?? 9)
            || Number(a.score ?? 101) - Number(b.score ?? 101);
    });
    const allAgents = groups.flatMap(group => group.agents.map((agent) => ({ ...agent, groupId: group.groupId })));
    const scoredAgents = allAgents.filter((agent) => agent.score !== null && agent.score !== undefined);
    const overallScore = scoredAgents.length ? Math.round(scoredAgents.reduce((sum, agent) => sum + Number(agent.score || 0), 0) / scoredAgents.length * 10) / 10 : null;
    return {
        ...
        overall: {
            ...
            failAgents: scoredAgents.filter((agent) => agent.status === "fail").length,
            warnAgents: scoredAgents.filter((agent) => agent.status === "warn").length,
            okAgents: scoredAgents.filter((agent) => agent.status === "ok").length,
        },
        groups,
        weakAgents: allAgents.filter((agent) => agent.status === "fail" || agent.status === "warn").slice(0, 20),
    };
}
function evaluateChildAgentMemoryReliability(options = {}) {
    const report = buildChildAgentMemoryReliabilityReport(options);
    const agents = report.groups.flatMap((group) => group.agents.map((agent) => ({ ...agent, groupId: group.groupId })));
    const checked = agents.filter((agent) => agent.score !== null && agent.score !== undefined).length;
    const passed = agents.filter((agent) => agent.status === "ok").length;
    const evidence = agents.filter((agent) => agent.status === "ok").slice(0, 12).map((agent) => ({
    ...
    const gaps = report.weakAgents.slice(0, 12).map((agent) => ({
        groupId: agent.groupId,
        agent: agent.agent,
        score: agent.score,
        reason: agent.gaps?.[0]?.reason || "子 Agent 记忆可靠性低于阈值",
    }));
    const check = makeQualityCheck("child_agent_memory_reliability", "子 Agent 记忆可靠性", checked, passed, evidence, gaps, "按子 Agent 汇总 memoryUsed/memoryIgnored、压缩候选分类纪律和 post-compact 首派发 marker，定位不可靠的第三方会话。");
    check.report = report;
    return check;
}
function timelineStatusFromScore(score) {
...
function compactBoundaryRecoveryAudit(memory = {}) {
...
function compactBoundaryQuality(memory = {}) {
    return memory?.compaction?.quality
        || memory?.messageCompression?.quality
        || memory?.compactBoundary?.quality
        || null;
}
function buildGroupCompactBoundaryTimeline(groupId, memory = {}, diagnostics = {}) {
    const boundary = compactMemoryHasPostCompactBoundary(memory);
    const compaction = memory?.compaction || {};
    ...
    const tokenPressure = postTokens > 0 ? Math.round((postTokens / context_budget_1.DEFAULT_CONTEXT_WINDOW_TOKENS) * 1000) / 10 : null;
    ...
    const components = [
        ...
        },
        {
            ...
            metrics: {
                ...
                weakAgents: Array.isArray(reliability.agents) ? reliability.agents.filter((agent) => agent.status === "fail" || agent.status === "warn").length : 0,
            },
        },
        {
            key: "typed_recall",
    ...
    if (boundary.hasBoundary && !recoveryAudit)
        gaps.push({ reason: "compact boundary 缺少 post-compact recovery audit", component: "recovery" });
    if (boundary.hasBoundary && dispatch.status === "waiting")
        gaps.push({ reason: "compact boundary 后尚未观测首派发 marker", component: "dispatch" });
    if (dispatch.status === "fail" || dispatch.status === "warn")
        gaps.push(...(Array.isArray(dispatch.gaps) ? dispatch.gaps.slice(0, 4).map((gap) => ({ ...gap, component: "dispatch" })) : []));
    if (Number(discipline.stalePromoted || 0) > 0)
        gaps.push({ reason: `存在 ${discipline.stalePromoted} 个 stale promoted 候选`, component: "candidate_usage" });
    if (Number(discipline.missing || 0) > 0)
        gaps.push({ reason: `存在 ${discipline.missing} 个候选缺少严格分类`, component: "candidate_usage" });
    if (Array.isArray(reliability.agents)) {
        for (const agent of reliability.agents.filter((item) => item.status === "fail" || item.status === "warn").slice(0, 4)) {
    ...
    const firstDispatch = Array.isArray(dispatch.recentEntries)
        ? [...dispatch.recentEntries].filter((entry) => entry.first_dispatch_after_compact === true).sort((a, b) => String(a.generated_at || "").localeCompare(String(b.generated_at || "")))[0]
        : null;
...
function buildCompactBoundaryTimelineReport(options = {}) {
    const explicitGroupIds = Array.isArray(options.groupIds || options.group_ids)
        ? (options.groupIds || options.group_ids).map((item) => String(item || "").trim()).filter(Boolean)
        : null;
    const rows = [];
    const files = explicitGroupIds
        ? explicitGroupIds.map((id) => path.join(GROUP_MEMORY_DIR, `${id}.json`))
...
function evaluateCompactBoundaryTimeline(options = {}) {
    const report = buildCompactBoundaryTimelineReport(options);
    const checked = Number(report.overall.compactedGroupCount || 0);
    const passed = (report.groups || []).filter((row) => row.boundary?.compacted === true && row.status === "ok").length;
    const evidence = (report.groups || []).filter((row) => row.boundary?.compacted === true && row.status === "ok").slice(0, 12).map((row) => ({
        groupId: row.groupId,
        score: row.score,
        eventCount: row.events?.length || 0,
        reductionRate: row.boundary?.reductionRate,
    }));
    const gaps = (report.weakGroups || []).slice(0, 12).map((row) => ({
        groupId: row.groupId,
        score: row.score,
        reason: row.gaps?.[0]?.reason || "compact boundary timeline 存在缺口",
    }));
    const check = makeQualityCheck("compact_boundary_timeline", "压缩边界时间线", checked, passed, evidence, gaps, "把 token 压力、恢复审计、首派发、候选使用、子 Agent 可靠性和类型化召回串成同一条 compact boundary 诊断时间线。");
    check.report = report;
    return check;
}
function summarizeCompactionHookLedger(groupId, memory = {}, ledger = {}) {
    const compaction = memory?.compaction || {};
    ...
    const preEntries = merged.filter((entry) => entry.phase === "pre");
    const postEntries = merged.filter((entry) => entry.phase === "post");
    const failedEntries = merged.filter((entry) => entry.ok === false || entry.status === "fail");
    const latestAt = merged.reduce((latest, entry) => String(entry.at || "") > latest ? String(entry.at || "") : latest, "");
    const totalDuration = merged.reduce((sum, entry) => sum + Number(entry.duration_ms || 0), 0);
    const boundary = compactMemoryHasPostCompactBoundary(memory);
    const gaps = [];
    if (boundary.hasBoundary && preEntries.length === 0)
        gaps.push({ component: "pre", reason: "compact boundary 缺少 pre-compact hook ledger" });
    if (boundary.hasBoundary && postEntries.length === 0)
        gaps.push({ component: "post", reason: "compact boundary 缺少 post-compact hook ledger" });
    ...
    return {
        ...
        latestAt,
        hookRunIds: Array.from(new Set(merged.map((entry) => String(entry.hook_run_id || "")).filter(Boolean))).slice(-8),
        recentEntries: merged.slice(-12).reverse().map((entry) => ({
...
function buildCompactionHookLedgerReport(options = {}) {
    const explicitGroupIds = Array.isArray(options.groupIds || options.group_ids)
        ? (options.groupIds || options.group_ids).map((item) => String(item || "").trim()).filter(Boolean)
        : null;
    const files = explicitGroupIds
    ...
    return {
        ...
        overall: {
            ...
            missingPhaseGroupCount: rows.filter(row => row.gaps?.some((gap) => String(gap.reason || "").includes("缺少"))).length,
        },
        groups: rows.sort((a, b) => Number(a.score ?? 101) - Number(b.score ?? 101)).slice(0, 50),
        weakGroups: rows.filter(row => row.status === "fail" || row.status === "warn").slice(0, 20),
    };
}
function evaluateCompactionHookLedger(options = {}) {
    const report = buildCompactionHookLedgerReport(options);
    const checked = Number(report.overall.checkedGroupCount || 0);
    const passed = (report.groups || []).filter((row) => row.entryCount > 0 && row.status === "ok").length;
    const evidence = (report.groups || []).filter((row) => row.entryCount > 0 && row.status === "ok").slice(0, 12).map((row) => ({
        groupId: row.groupId,
    ...
    const check = makeQualityCheck("compaction_hook_ledger", "压缩 Hook Ledger", checked, passed, evidence, gaps, "检查 pre/post compact hook 是否有独立 ledger、耗时和失败证据，避免压缩前后上下文增强过程不可追溯。");
    check.report = report;
    return check;
}
function replayCandidateValue(row = {}) {
    return compactMemoryCenterText(row.value || row.text || row.path || row.command || row.name || row.summary || "", 420);
}
function collectPostCompactReplayCandidates(memory = {}) {
...
function replayNeedleMatches(rendered, value) {
...
function addReplayNeedle(needles, type, label, value, required = true) {
    const text = compactMemoryCenterText(value, 520);
    if (!text)
        return;
    needles.push({ type, label, value: text, required });
}
function getGroupCompactBoundaryReplayRepairLedgerFile(groupId) {
    return path.join(GROUP_MEMORY_REPLAY_REPAIR_DIR, `${sidecarFileId(groupId)}.json`);
}
function readGroupCompactBoundaryReplayRepairLedger(groupId) {
...
function writeGroupCompactBoundaryReplayRepairLedger(groupId, ledger) {
    const file = getGroupCompactBoundaryReplayRepairLedgerFile(groupId);
    fs.mkdirSync(path.dirname(file), { recursive: true });
    const value = {
        schema: "ccm-compact-boundary-replay-repair-ledger-v1",
...
function summarizeReplayRepairActions(actions = []) {
...
function summarizeReplayRepairLedger(groupId, ledgerInput = null) {
    const ledger = ledgerInput || readGroupCompactBoundaryReplayRepairLedger(groupId);
    const entries = Array.isArray(ledger.entries) ? ledger.entries : [];
    const latest = entries[entries.length - 1] || null;
    const historicalOpenEntries = entries.filter((entry) => Number(entry.required_action_count || 0) > 0);
    ...
    return {
        ...
        warnCount: entries.filter((entry) => entry.status === "warn").length,
        ...
        recentAttempts: entries.slice(-8).reverse().map((entry) => ({
...
function recordCompactBoundaryReplayRepairAttempt(groupId, replay = {}, options = {}) {
    if (!groupId || !replay?.schema)
        return summarizeReplayRepairLedger(groupId);
    ...
    const entries = Array.isArray(ledger.entries) ? [...ledger.entries] : [];
    const existingIndex = entries.findIndex((entry) => entry.attempt_id === attemptId);
    const entry = {
    ...
    const stats = {
        attemptCount: entries.length,
        okCount: entries.filter((item) => item.status === "ok").length,
        warnCount: entries.filter((item) => item.status === "warn").length,
        failCount: entries.filter((item) => item.status === "fail").length,
        emptyCount: entries.filter((item) => item.status === "empty").length,
        historicalReworkRequiredCount: entries.filter((item) => Number(item.required_action_count || 0) > 0).length,
...
function getGroupCompactBoundaryReplayRepairWorkItemsFile(groupId) {
    return path.join(GROUP_MEMORY_REPLAY_REPAIR_WORK_ITEMS_DIR, `${sidecarFileId(groupId)}.json`);
}
function readGroupCompactBoundaryReplayRepairWorkItems(groupId) {
...
function replayRepairWorkItemStatus(value) {
...
function replayRepairWorkItemOpen(status) {
    return ["pending", "in_progress", "blocked"].includes(replayRepairWorkItemStatus(status));
}
function replayRepairWorkItemSignature(item = {}) {
...
function buildReplayRepairPendingWorkItem(groupId, replay = {}, action = {}, index = 0, existing = {}, at = now()) {
    const repairPlan = replay.repairPlan || replay.repair_plan || {};
    const boundary = replay.boundary || repairPlan.boundary || {};
...
function buildReadPlanRevalidationRepairWorkItem(groupId, discipline = {}, gap = {}, index = 0, existing = {}, at = now()) {
...
function replayRepairWorkItemStats(items = []) {
...
function writeGroupCompactBoundaryReplayRepairWorkItems(groupId, ledger) {
    const file = getGroupCompactBoundaryReplayRepairWorkItemsFile(groupId);
...
function summarizeReplayRepairPendingWorkItems(groupId, ledgerInput = null) {
    ...
    const openItems = items.filter((item) => replayRepairWorkItemOpen(item.status))
        .sort((a, b) => replayRepairPriorityRank(a.priority) - replayRepairPriorityRank(b.priority));
    return {
        ...
        items: [...openItems, ...items.filter((item) => !replayRepairWorkItemOpen(item.status)).slice(-8)]
            .slice(0, 12)
            .map((item) => ({
        ...
        openItems: openItems.slice(0, 8).map((item) => ({
            id: item.id || item.work_item_id || "",
            status: replayRepairWorkItemStatus(item.status),
            owner: item.owner || "",
            priority: item.priority || "",
...
function replayRepairWorkItemDispatchCandidatePriority(item = {}) {
    ...
    if (dispatchTarget)
        return 0;
    if (status === "in_progress" && owner === "group-main-agent")
        return 1;
    if (["critical", "high"].includes(String(item.priority || "").toLowerCase()) && replayRepairWorkItemOpen(status))
        return 2;
    return 9;
}
function shouldSurfaceReplayRepairDispatchCandidate(item = {}) {
    const status = replayRepairWorkItemStatus(item.status);
    if (!replayRepairWorkItemOpen(status))
        return false;
    const dispatchTarget = String(item.dispatch_target || item.dispatchTarget || "").trim();
    const owner = String(item.owner || "").trim();
...
function buildReplayRepairDispatchCandidate(groupId, item = {}, index = 0) {
    const status = replayRepairWorkItemStatus(item.status);
...
function buildReplayRepairMainAgentDispatchCandidates(groupId, options = {}) {
    ...
    const candidates = items
        ...
        .map((item, index) => buildReplayRepairDispatchCandidate(groupId, item, index));
    const openItems = items.filter((item) => replayRepairWorkItemOpen(item.status));
    const claimedItems = openItems.filter((item) => replayRepairWorkItemStatus(item.status) === "in_progress" && String(item.owner || "") === "group-main-agent");
    const dispatchMarkedItems = openItems.filter((item) => String(item.dispatch_target || item.dispatchTarget || "").trim());
    const criticalItems = openItems.filter((item) => ["critical", "high"].includes(String(item.priority || "").toLowerCase()));
    return {
        ...
        readyCount: candidates.filter((candidate) => candidate.dispatchMarked || candidate.claimedByMainAgent).length,
        shouldCreateRealTask: false,
        candidates,
    };
}
function findReplayRepairWorkItemIndex(items = [], itemRef) {
...
function updateCompactBoundaryReplayRepairWorkItem(input = {}) {
    ...
    if (!itemRef)
        throw new Error("缺少 replay repair work item id");
    const action = String(input.action || input.operation || "").trim().toLowerCase();
    if (!["claim", "dispatch", "complete", "resolve", "block", "cancel", "reopen"].includes(action))
        throw new Error("不支持的 replay repair work item 操作");
    ...
    if (index < 0)
        throw new Error("replay repair work item 不存在");
    ...
    if (action === "claim" || action === "dispatch") {
        if (terminal)
            throw new Error("已关闭的 replay repair work item 不能认领或派发");
        if (status === "in_progress" && item.owner && item.owner !== owner)
            throw new Error(`该 work item 已由 ${item.owner} 认领`);
        ...
        if (terminal)
            throw new Error("已关闭的 replay repair work item 不能标记阻塞");
        nextStatus = "blocked";
        item.owner = owner || item.owner;
...
function syncCompactBoundaryReplayRepairPendingWorkItems(groupId, replay = {}, options = {}) {
    if (!groupId || !replay?.schema)
        return summarizeReplayRepairPendingWorkItems(groupId);
    const at = String(options.at || now());
    const repairPlan = replay.repairPlan || replay.repair_plan || {};
    ...
    const previousById = new Map(previousItems.map((item) => [String(item.id || item.work_item_id || ""), item]));
...
function syncCompactFileReferenceReadPlanRevalidationRepairWorkItems(groupId, discipline = {}, options = {}) {
    if (!groupId || !discipline?.schema)
        return summarizeReplayRepairPendingWorkItems(groupId);
    const at = String(options.at || now());
    const ledger = readGroupCompactBoundaryReplayRepairWorkItems(groupId);
    const previousItems = Array.isArray(ledger.items) ? ledger.items : [];
    ...
    const activeGaps = gaps.filter((gap) => gap?.read_plan_id || gap?.reference_id || gap?.session_mismatch);
    const currentIds = new Set();
    const nextRepairItems = [];
    let changed = false;
    ...
    const currentComparable = JSON.stringify({
        ...
        items: previousItems.filter((item) => String(item.source || "") === "compact_read_plan_revalidation_repair"),
    });
    const nextComparable = JSON.stringify({
        latestReadPlanRevalidation: nextLedger.latestReadPlanRevalidation || null,
        items: nextItems.filter((item) => String(item.source || "") === "compact_read_plan_revalidation_repair"),
    });
    if (changed || currentComparable !== nextComparable || !fs.existsSync(ledger.file || getGroupCompactBoundaryReplayRepairWorkItemsFile(groupId))) {
        return summarizeReplayRepairPendingWorkItems(groupId, writeGroupCompactBoundaryReplayRepairWorkItems(groupId, nextLedger));
    }
...
function replayRepairPriorityRank(priority) {
...
function compactReplayRepairAction(groupId, gap = {}, index = 0) {
...
function buildCompactBoundaryReplayRepairPlan(groupId, memory = {}, options = {}) {
    ...
    const actions = gaps
        .map((gap, index) => compactReplayRepairAction(groupId, gap, index))
        .sort((a, b) => replayRepairPriorityRank(a.priority) - replayRepairPriorityRank(b.priority));
    ...
    const promptPatch = actions.length ? [
        ...
        ...actions.slice(0, 6).map((action) => `- ${action.priority}:${action.component}：${action.instruction}${action.expected ? `；expected=${action.expected}` : ""}`),
    ].join("\n") : "";
    return {
        ...
        rawRecovery: {
            ...
            groupMessagesFile: path.join(utils_1.GROUP_MESSAGES_DIR, `${groupId}.json`),
...
function buildGroupCompactBoundaryReplayGate(groupId, memory = {}, options = {}) {
    const boundary = compactMemoryHasPostCompactBoundary(memory);
    const compaction = memory?.compaction || {};
    ...
    const replayTask = [
        ...
        ...(Array.isArray(memory?.persistentRequirements) ? memory.persistentRequirements.slice(-4).map((item) => item.text || item.value || item) : []),
    ].filter(Boolean).join("\n");
    let rendered = "";
    try {
        ...
        rendered = renderGroupMemoryContextBundle({
            ...
            compaction: {
                ...
                hookLedger: compaction.hookLedger || (hookLedger?.hasLedger ? {
                    ...
                    recentEntries: (hookLedger.recentEntries || []).map((entry) => ({
                        phase: entry.phase,
                        ok: entry.ok,
                        status: entry.status,
            ...
            raw_sources: {
                ...
                group_messages_file: path.join(utils_1.GROUP_MESSAGES_DIR, `${groupId}.json`),
            },
        });
    }
    catch (error) {
    ...
    if (boundary.summaryChecksum)
        addReplayNeedle(needles, "boundary", "摘要校验", boundary.summaryChecksum, true);
    else if (boundary.summarizedThroughMessageId)
        addReplayNeedle(needles, "boundary", "压缩边界", boundary.summarizedThroughMessageId, true);
...
function normalizeHistoricalCompactBoundary(boundary = {}, index = 0) {
    const restore = boundary.post_compact_restore || boundary.postCompactRestore || {};
    const summaryChecksum = String(boundary.summaryChecksum
        || boundary.summary_checksum
        || restore.summaryChecksum
        || restore.summary_checksum
        || "").trim();
    const summarizedThroughMessageId = String(boundary.summarizedThroughMessageId
...
function collectHistoricalCompactBoundaries(memory = {}, options = {}) {
...
function buildHistoricalBoundaryMemorySnapshot(memory = {}, boundary = {}) {
    const restore = boundary.post_compact_restore || {};
    const compaction = memory?.compaction || {};
    return {
        ...memory,
...
function buildGroupHistoricalCompactBoundaryReplay(groupId, memory = {}, options = {}) {
    const hookLedger = options.hookLedger || summarizeCompactionHookLedger(groupId, memory, {});
    const targetProject = String(options.targetProject || options.target_project || Object.keys(memory?.agentMemories || {})[0] || "api");
    ...
    const gaps = rows.flatMap(row => (row.gaps || []).map((gap) => ({
...
function buildHistoricalCompactBoundaryReplayReport(options = {}) {
    const explicitGroupIds = Array.isArray(options.groupIds || options.group_ids)
        ? (options.groupIds || options.group_ids).map((item) => String(item || "").trim()).filter(Boolean)
        : null;
    const files = explicitGroupIds
...
function evaluateHistoricalCompactBoundaryReplay(options = {}) {
    const report = buildHistoricalCompactBoundaryReplayReport(options);
    const checked = Number(report.overall.checkedGroupCount || 0);
    const passed = (report.groups || []).filter((row) => row.status === "ok").length;
    ...
    const check = makeQualityCheck("historical_compact_boundary_replay", "历史压缩边界 Replay", checked, passed, evidence, gaps, "抽样 compaction.boundaries 中的历史压缩边界，验证旧边界的摘要校验、重注入候选和回执契约仍能进入第三方子 Agent 新会话上下文。");
    check.report = report;
    return check;
}
function normalizeChildAgentReplayType(value) {
...
function collectChildAgentTypeReplayTargets(groupId, memory = {}, options = {}) {
    const rows = [];
    ...
    const group = groupList.find((item) => String(item.id || item.groupId || "") === String(groupId));
    ...
    if (!rows.length)
        add(Object.keys(memory?.agentMemories || {})[0] || "api", "unknown", "fallback");
    const seen = new Set();
...
function buildChildAgentTypeReplayProfile(agentType, replay = {}, memory = {}, target = {}) {
    const checks = [
        {
            key: "base_replay",
            label: "基础 replay",
...
function buildGroupChildAgentTypeReplayMatrix(groupId, memory = {}, options = {}) {
    const hookLedger = options.hookLedger || summarizeCompactionHookLedger(groupId, memory, {});
    ...
    for (const row of targetRows) {
        ...
        current.gaps.push(...(row.gaps || []).map((gap) => ({ ...gap, targetProject: row.targetProject })));
    ...
    return {
        ...
        agentTypes,
        targets: targetRows,
        gaps: agentTypes.flatMap(row => (row.gaps || []).map((gap) => ({ ...gap, agentType: row.agentType }))).slice(0, 16),
    };
}
function buildChildAgentTypeReplayMatrixReport(options = {}) {
    const explicitGroupIds = Array.isArray(options.groupIds || options.group_ids)
        ? (options.groupIds || options.group_ids).map((item) => String(item || "").trim()).filter(Boolean)
        : null;
    const files = explicitGroupIds
...
function evaluateChildAgentTypeReplayMatrix(options = {}) {
    ...
    const check = makeQualityCheck("child_agent_type_replay_matrix", "子 Agent 类型 Replay", checked, passed, evidence, gaps, "按 Claude Code / Cursor / Codex 等子 Agent 类型分维度重放群聊记忆包，验证每种第三方新会话都能拿到压缩边界、候选使用和回执契约。");
    check.report = report;
    return check;
}
function buildCompactBoundaryReplayReport(options = {}) {
    const explicitGroupIds = Array.isArray(options.groupIds || options.group_ids)
        ? (options.groupIds || options.group_ids).map((item) => String(item || "").trim()).filter(Boolean)
        : null;
    const files = explicitGroupIds
    ...
    return {
        ...
        topRepairActions: checkedRows
            .flatMap(row => (row.repairPlan?.actions || []).slice(0, 2).map((action) => ({ groupId: row.groupId, score: row.score, ...action })))
            .slice(0, 20),
    };
}
function evaluateCompactBoundaryReplayGate(options = {}) {
    ...
    const check = makeQualityCheck("compact_boundary_replay_gate", "压缩边界 Replay Gate", checked, passed, evidence, gaps, "只读重放子 Agent 记忆包，验证压缩边界、重注入候选、hook 注入要求和回执契约是否能进入第三方子 Agent 新会话上下文。");
    check.report = report;
    return check;
}
function replayOverrideForGroup(options = {}, groupId) {
    const overrides = options.replays || options.replayRows || options.replay_rows;
    if (Array.isArray(overrides))
        return overrides.find((row) => String(row.groupId || row.group_id || "") === String(groupId)) || null;
    if (overrides && typeof overrides === "object")
        return overrides[groupId] || overrides[String(groupId)] || null;
    return null;
}
function buildReplayRepairPendingWorkItemReport(options = {}) {
    const explicitGroupIds = Array.isArray(options.groupIds || options.group_ids)
        ? (options.groupIds || options.group_ids).map((item) => String(item || "").trim()).filter(Boolean)
        : null;
    const overrideGroupIds = Array.isArray(options.replays || options.replayRows || options.replay_rows)
        ? (options.replays || options.replayRows || options.replay_rows).map((row) => String(row.groupId || row.group_id || "")).filter(Boolean)
        : [];
    const groupIds = explicitGroupIds || overrideGroupIds;
    const files = groupIds?.length
        ? groupIds.map((id) => path.join(GROUP_MEMORY_DIR, `${id}.json`))
        : listJsonFiles(GROUP_MEMORY_DIR);
    const rows = [];
    for (const file of files) {
...
function evaluateReplayRepairPendingWorkItems(options = {}) {
    const report = buildReplayRepairPendingWorkItemReport(options);
    const checked = Number(report.overall.checkedGroupCount || 0);
    const passed = checked === 0
        ? 0
        : (report.groups || []).filter((row) => (Number(row.requiredActionCount || 0) > 0 || Number(row.openItemCount || 0) > 0) && row.status === "ok").length;
    const evidence = (report.groups || []).filter((row) => row.status === "ok" && Number(row.openItemCount || 0) > 0).slice(0, 12).map((row) => ({
        groupId: row.groupId,
    ...
    const check = makeQualityCheck("replay_repair_pending_work_items", "Replay 修复待办", checked, passed, evidence, gaps, "把 compact boundary replay 的修复动作物化为群聊主 Agent 可读的 sidecar work items；不自动创建真实任务，避免打开诊断页污染任务队列。");
    check.report = report;
    return check;
}
function replayRepairDispatchCandidateGroupIds(options = {}) {
...
function buildReplayRepairDispatchCandidateReport(options = {}) {
    ...
    const rows = groupIds.map((groupId) => {
        ...
        const ledger = readGroupCompactBoundaryReplayRepairWorkItems(groupId);
        const openItems = (Array.isArray(ledger.items) ? ledger.items : []).filter((item) => replayRepairWorkItemOpen(item.status));
        const claimedItems = openItems.filter((item) => replayRepairWorkItemStatus(item.status) === "in_progress" && String(item.owner || "") === "group-main-agent");
        const dispatchMarkedItems = openItems.filter((item) => String(item.dispatch_target || item.dispatchTarget || "").trim());
        const highPriorityPendingItems = openItems.filter((item) => replayRepairWorkItemStatus(item.status) === "pending" && ["critical", "high"].includes(String(item.priority || "").toLowerCase()));
        const expectedCandidateCount = new Set([
            ...claimedItems,
            ...dispatchMarkedItems,
            ...highPriorityPendingItems,
        ].map((item) => String(item.id || item.work_item_id || ""))).size;
        const candidateWorkIds = new Set((summary.candidates || []).map((candidate) => String(candidate.work_item_id || "")));
        const missing = [...claimedItems, ...dispatchMarkedItems, ...highPriorityPendingItems]
            .filter((item) => !candidateWorkIds.has(String(item.id || item.work_item_id || "")));
        ...
        return {
            ...
            gaps: missing.slice(0, 8).map((item) => ({
...
function evaluateReplayRepairDispatchCandidates(options = {}) {
    ...
    const gaps = (report.weakGroups || []).flatMap((row) => (row.gaps || []).slice(0, 3).map((gap) => ({
        groupId: row.groupId,
    ...
    const check = makeQualityCheck("replay_repair_dispatch_candidates", "Replay 修复派发候选", checked, passed, evidence, gaps, "把已认领/已派发/高优先级 replay repair work items 转成群聊主 Agent 可读候选；只作为规划上下文，不自动创建真实任务。");
    check.report = report;
    return check;
}
function buildCompactFileReferenceReadPlanRevalidationRepairWorkItemReport(options = {}) {
    const revalidationReport = buildCompactFileReferenceReadPlanRevalidationGateReport(options);
    const rows = (revalidationReport.groups || []).map((group) => {
        ...
        const revalidationItems = (Array.isArray(ledger.items) ? ledger.items : [])
            .filter((item) => String(item.source || "") === "compact_read_plan_revalidation_repair");
        const openItems = revalidationItems.filter((item) => replayRepairWorkItemOpen(item.status));
        const completedItems = revalidationItems.filter((item) => replayRepairWorkItemStatus(item.status) === "completed");
        ...
        return {
            ...
            pendingCount: revalidationItems.filter((item) => replayRepairWorkItemStatus(item.status) === "pending").length,
            inProgressCount: revalidationItems.filter((item) => replayRepairWorkItemStatus(item.status) === "in_progress").length,
            completedCount: completedItems.length,
            sessionMismatchCount: revalidationItems.filter((item) => item.session_mismatch === true).length,
            file: workItems.file || getGroupCompactBoundaryReplayRepairWorkItemsFile(groupId),
            items: revalidationItems.slice(0, 12).map((item) => ({
                id: item.id || item.work_item_id || "",
                status: replayRepairWorkItemStatus(item.status),
                priority: item.priority || "",
                target_project: item.target_project || "",
...
function evaluateCompactFileReferenceReadPlanRevalidationRepairWorkItems(options = {}) {
    ...
    const check = makeQualityCheck("compact_file_reference_read_plan_revalidation_repair_work_items", "读取计划重读修复待办", checked, passed, evidence, gaps, "当 compact read plan revalidation gate 失败或回执会话不匹配时，把缺口物化为群聊主 Agent 可读 sidecar work items，并进入派发候选上下文。");
    check.report = report;
    return check;
}
        ...
        const status = !requiresSnapshot
            ...
            : snapshotExists && markdownExists && checksumMatches && hasSummary ? "ok"
                : snapshotExists && markdownExists && hasSummary ? "warn"
                    : "fail";
        const gaps = [];
        if (requiresSnapshot && !snapshotExists)
            gaps.push({ reason: "缺少 group session memory snapshot.json" });
        if (requiresSnapshot && !markdownExists)
            gaps.push({ reason: "缺少 group session memory summary.md" });
        if (requiresSnapshot && markdownExists && !checksumMatches)
            gaps.push({ reason: "summary.md checksum 与 snapshot.json 不一致" });
        if (requiresSnapshot && !hasSummary)
            gaps.push({ reason: "group session memory 缺少可注入摘要内容" });
        return {
...
function evaluateGroupSessionMemorySnapshots(options = {}) {
    ...
    const check = makeQualityCheck("group_session_memory_snapshot", "Group Session Memory 快照", checked, passed, evidence, gaps, "对齐 Claude Code Session Memory：压缩后的群聊会话摘要必须落到稳定 summary.md/snapshot.json，并可被主 Agent、全局 Agent 和子 Agent 上下文重注入。");
    check.report = report;
    return check;
}
function memoryCenterToolSet(...sets) {
...
function memoryHasRuntimeToolContinuityEvidence(memory = {}) {
    return (Array.isArray(memory.workerLedger) ? memory.workerLedger : []).some((item) => {
...
function buildGroupToolContinuitySnapshotReport(options = {}) {
    const explicitGroupIds = Array.isArray(options.groupIds || options.group_ids)
        ? (options.groupIds || options.group_ids).map((item) => String(item || "").trim()).filter(Boolean)
        : null;
    const files = explicitGroupIds?.length
    ...
    const rows = files.map(file => {
        ...
        const gaps = [];
        if (requiresSnapshot && !snapshotExists)
            gaps.push({ reason: "缺少 group tool continuity snapshot.json" });
        if (requiresSnapshot && !markdownExists)
            gaps.push({ reason: "缺少 group tool continuity summary.md" });
        if (requiresSnapshot && markdownExists && !checksumMatches)
            gaps.push({ reason: "summary.md checksum 与 snapshot.json 不一致" });
        if (requiresSnapshot && !contextOnly)
            gaps.push({ reason: "工具/技能连续性快照必须只作为上下文，不能声明绕过授权" });
        if (requiresSnapshot && !hasContinuity)
            gaps.push({ reason: "工具/技能连续性快照缺少 allowed/requested/synced/missing/invoked 证据" });
        return {
...
function evaluateGroupToolContinuitySnapshots(options = {}) {
    ...
    const check = makeQualityCheck("group_tool_continuity_snapshot", "工具/技能连续性快照", checked, passed, evidence, gaps, "对齐 Claude Code 的 discovered tools / invoked skills 保留：压缩后必须把工具、技能、缺失项作为上下文恢复，但不能绕过当前授权与 runtime tool gate。");
    check.report = report;
    return check;
}
function buildCompactFileReferenceReport(options = {}) {
    const explicitGroupIds = Array.isArray(options.groupIds || options.group_ids)
        ? (options.groupIds || options.group_ids).map((item) => String(item || "").trim()).filter(Boolean)
        : null;
    const files = explicitGroupIds?.length
    ...
    const rows = files.map(file => {
        ...
        const referenceTypes = new Set((refs.references || []).map((item) => String(item.type || "")));
        ...
        const status = !requiresReferences
            ...
            : Number(refs.referenceCount || 0) >= 2 && hasRawSources && hasCompactSource ? "ok"
                : Number(refs.referenceCount || 0) >= 2 && hasRawSources ? "warn"
                    : "fail";
        const gaps = [];
        if (requiresReferences && Number(refs.referenceCount || 0) < 2)
            gaps.push({ reason: "compact file references 数量不足" });
        if (requiresReferences && !hasRawSources)
            gaps.push({ reason: "缺少 group memory/raw messages 权威来源引用" });
        if (requiresReferences && !hasCompactSource)
            gaps.push({ reason: "缺少 session memory、typed MEMORY 或 tool continuity 压缩恢复来源引用" });
        if (Number(refs.missingCount || 0) > 0)
            gaps.push({ reason: `存在 ${refs.missingCount} 个缺失文件引用` });
        return {
            ...
            references: (refs.references || []).slice(0, 12).map((item) => ({
...
function evaluateCompactFileReferences(options = {}) {
    ...
    const check = makeQualityCheck("compact_file_references", "压缩文件引用", checked, passed, evidence, gaps, "对齐 Claude Code compact_file_reference / sessionFileAccessHooks：压缩后把关键记忆来源作为可读文件引用暴露，并追踪子 Agent 是否声明使用。");
    check.report = report;
    return check;
}
function buildCompactFileReferenceReadPlanReport(options = {}) {
    const explicitGroupIds = Array.isArray(options.groupIds || options.group_ids)
        ? (options.groupIds || options.group_ids).map((item) => String(item || "").trim()).filter(Boolean)
        : null;
    const files = explicitGroupIds?.length
    ...
    const rows = files.map(file => {
        ...
        const hasSourceOfTruth = readPlan.hasSourceOfTruth === true || (readPlan.entries || []).some((entry) => ["raw_group_messages_json", "group_memory_json"].includes(String(entry.type || "")));
        const hasCompactSummary = readPlan.hasCompactSummary === true || (readPlan.entries || []).some((entry) => ["group_session_memory", "typed_memory_index", "tool_continuity_summary"].includes(String(entry.type || "")));
        const hasReceiptContract = (readPlan.entries || []).some((entry) => String(entry.receipt || "").includes("memoryUsed"));
        const plannedCount = Number(readPlan.plannedCount || (readPlan.entries || []).filter((entry) => entry.action !== "skip_missing").length || 0);
        const status = !requiresPlan
            ...
            : plannedCount >= 2 && hasSourceOfTruth && hasCompactSummary && hasReceiptContract ? "ok"
                : plannedCount >= 2 && hasSourceOfTruth ? "warn"
                    : "fail";
        const gaps = [];
        if (requiresPlan && plannedCount < 2)
            gaps.push({ reason: "compact file reference read plan 可执行条目不足" });
        if (requiresPlan && !hasSourceOfTruth)
            gaps.push({ reason: "read plan 缺少 raw messages 或 group memory source-of-truth 入口" });
        if (requiresPlan && !hasCompactSummary)
            gaps.push({ reason: "read plan 缺少 Session Memory、typed MEMORY 或 tool continuity 摘要入口" });
        if (requiresPlan && !hasReceiptContract)
            gaps.push({ reason: "read plan 条目缺少 memoryUsed/memoryIgnored 回执要求" });
        return {
            ...
            hasCompactSummary,
            hasReceiptContract,
            entries: (readPlan.entries || []).slice(0, 12).map((entry) => ({
...
function evaluateCompactFileReferenceReadPlan(options = {}) {
    ...
    const check = makeQualityCheck("compact_file_reference_read_plan", "压缩文件引用读取计划", checked, passed, evidence, gaps, "对齐 Claude Code post-compact file attachment：压缩后给子 Agent 明确的按需读取顺序、source-of-truth 入口和回执要求。");
    check.report = report;
    return check;
}
function buildCompactFileReferenceReadPlanFreshnessReport(options = {}) {
    const explicitGroupIds = Array.isArray(options.groupIds || options.group_ids)
        ? (options.groupIds || options.group_ids).map((item) => String(item || "").trim()).filter(Boolean)
        : null;
    const files = explicitGroupIds?.length
    ...
    const rows = files.map(file => {
        ...
        const freshnessReadPlan = {
            ...
            plannedCount: surfaced.rows.filter((entry) => entry.action !== "skip_missing").length,
...
function evaluateCompactFileReferenceReadPlanFreshness(options = {}) {
    ...
    const check = makeQualityCheck("compact_file_reference_read_plan_freshness", "压缩读取计划源新鲜度", checked, passed, evidence, gaps, "compact read plan 下发时记录源文件指纹；如果源文件变化，子 Agent 使用前必须重新读取当前源并在回执声明。");
    check.report = report;
    return check;
}
function compactReadPlanRevalidationEvidenceSources(groupId, memory = {}) {
    const sources = [];
    const push = (source) => {
        const text = [
            source.text,
...
function compactReadPlanRevalidationMatch(source = {}, row = {}, gate = {}) {
...
function buildCompactFileReferenceReadPlanRevalidationGateReport(options = {}) {
    const explicitGroupIds = Array.isArray(options.groupIds || options.group_ids)
        ? (options.groupIds || options.group_ids).map((item) => String(item || "").trim()).filter(Boolean)
        : null;
    const files = explicitGroupIds?.length
...
function evaluateCompactFileReferenceReadPlanRevalidationGate(options = {}) {
    ...
    const check = makeQualityCheck("compact_file_reference_read_plan_revalidation_gate", "压缩读取计划重读门禁", checked, passed, evidence, gaps, "当历史 compact read plan 源文件变化时，子 Agent 回执必须按 read_plan_id 声明已 re-read/current source verified，或在 memoryIgnored 说明不使用。");
    check.report = report;
    return check;
}
function evaluateCompactFileReferenceReadPlanRevalidationSessionBinding(options = {}) {
    const report = buildCompactFileReferenceReadPlanRevalidationGateReport(options);
    const checked = Number(report.overall.sessionRequired || 0);
    const passed = Number(report.overall.sessionMatched || 0);
    const evidence = (report.groups || []).filter((row) => row.sessionRequired > 0 && row.sessionMismatch === 0).slice(0, 12).map((row) => ({
        groupId: row.groupId,
        gateId: row.gateId,
    ...
    const gaps = (report.groups || []).flatMap((row) => (row.gaps || [])
        .filter((gap) => gap.session_mismatch === true)
        .slice(0, 3)
        .map((gap) => ({
        groupId: row.groupId,
        gateId: row.gateId,
    ...
    const check = makeQualityCheck("compact_file_reference_read_plan_revalidation_session_binding", "读取计划重读会话绑定", checked, passed, evidence, gaps, "compact read plan stale source 重读回执必须来自收到该 gate 的具体子 Agent 会话；错会话或缺 session 的回执不能证明当前第三方 Agent 会话已重读。");
    check.report = report;
    return check;
}
function latestCompactFileReferenceReadPlanRowsForDiscipline(groupId, fallbackReadPlan = {}) {
    const ledger = readJson(getGroupCompactFileReferenceLedgerFile(groupId), null);
    const entries = Array.isArray(ledger?.entries) ? ledger.entries : [];
    const fromLedger = entries.slice(-8).flatMap((entry) => (Array.isArray(entry.read_plan_entries) ? entry.read_plan_entries : []).map((row) => ({
        ...row,
...
function buildCompactFileReferenceReadPlanUsageDisciplineReport(options = {}) {
    const explicitGroupIds = Array.isArray(options.groupIds || options.group_ids)
        ? (options.groupIds || options.group_ids).map((item) => String(item || "").trim()).filter(Boolean)
        : null;
    const files = explicitGroupIds?.length
    ...
    const rows = files.map(file => {
        ...
        const passed = checked ? accessRows.filter((row) => row.read_plan_id_mentioned === true).length : 0;
        const mentioned = checked ? accessRows.filter((row) => row.mentioned === true).length : 0;
        const score = qualityRate(passed, checked);
        const mentionRate = qualityRate(mentioned, checked);
        const status = checked === 0 ? "empty" : Number(score) >= 90 ? "ok" : Number(score) >= 70 ? "warn" : "fail";
        const gaps = checked
            ? accessRows.filter((row) => row.read_plan_id_mentioned !== true).slice(0, 12).map((row) => ({
                read_plan_id: row.read_plan_id || "",
                reference_id: row.reference_id || "",
                type: row.type || "",
                action: row.action || "",
...
function evaluateCompactFileReferenceReadPlanUsageDiscipline(options = {}) {
    ...
    const check = makeQualityCheck("compact_file_reference_read_plan_usage_discipline", "压缩读取计划回执纪律", checked, passed, evidence, gaps, "compact file reference read plan 一旦下发给子 Agent，后续回执或群聊消息应优先用 memoryUsed/memoryIgnored 声明 read_plan_id，形成按需读取计划的使用闭环。");
    check.report = report;
    return check;
}
function latestCompactFileReferenceRowsForDiscipline(groupId, fallbackRefs = {}) {
    const ledger = readJson(getGroupCompactFileReferenceLedgerFile(groupId), null);
    const entries = Array.isArray(ledger?.entries) ? ledger.entries : [];
    const fromLedger = entries.slice(-8).flatMap((entry) => (Array.isArray(entry.references) ? entry.references : []).map((ref) => ({
        ...ref,
        surfaced_at: entry.generated_at || "",
        surfacing_scope: entry.scope || "",
        target_project: entry.target_project || "",
...
function buildCompactFileReferenceUsageDisciplineReport(options = {}) {
    const explicitGroupIds = Array.isArray(options.groupIds || options.group_ids)
        ? (options.groupIds || options.group_ids).map((item) => String(item || "").trim()).filter(Boolean)
        : null;
    const files = explicitGroupIds?.length
    ...
    const rows = files.map(file => {
        ...
        const accessRows = Array.isArray(access.rows) ? access.rows : [];
        const checked = surfaced.ledgerEntryCount > 0 ? surfaced.rows.length : 0;
        const passed = checked ? accessRows.filter((row) => row.mentioned === true).length : 0;
        const score = qualityRate(passed, checked);
        const status = checked === 0 ? "empty" : Number(score) >= 90 ? "ok" : Number(score) >= 70 ? "warn" : "fail";
        const gaps = checked
            ? accessRows.filter((row) => row.mentioned !== true).slice(0, 12).map((row) => ({
                reference_id: row.reference_id || "",
                type: row.type || "",
                path: row.path || "",
...
function evaluateCompactFileReferenceUsageDiscipline(options = {}) {
    ...
    const check = makeQualityCheck("compact_file_reference_usage_discipline", "压缩文件引用使用纪律", checked, passed, evidence, gaps, "compact file references 一旦下发给子 Agent，后续回执或群聊消息必须用 memoryUsed/memoryIgnored 声明 reference id 或路径，形成压缩上下文使用闭环。");
    check.report = report;
    return check;
}
function extractTaskMemoryUsageEvidence(task) {
    const summary = task?.delivery_summary || {};
    const usage = [
        ...(Array.isArray(summary.memory_usage) ? summary.memory_usage : []),
        ...(Array.isArray(summary.receipts) ? summary.receipts : []),
    ...
    for (const item of usage) {
        ...
        if (itemUsed.length)
            used.push({ agent, values: itemUsed });
        if (itemIgnored.length)
            ignored.push({ agent, values: itemIgnored });
    }
    ...
    if (groupId) {
        ...
        for (const item of ledger.filter((entry) => !task?.id || entry.taskId === task.id).slice(-20)) {
            ...
            if (itemUsed.length)
                used.push({ agent: item.project || item.agent || "", values: itemUsed });
            if (itemIgnored.length)
                ignored.push({ agent: item.project || item.agent || "", values: itemIgnored });
        }
    }
    return { used, ignored };
}
function sourceMessageExists(groupId, messageId, expectedText = "") {
    const messages = readJson(path.join(utils_1.GROUP_MESSAGES_DIR, `${groupId}.json`), []);
    const found = (Array.isArray(messages) ? messages : []).find((message) => String(message.id || message.uuid || "") === String(messageId));
    if (!found)
        return { exists: false, matched: false };
    return { exists: true, matched: !expectedText || evidenceTextMatches(found.content, expectedText) };
}
function evaluateConstraintRetention(options = {}) {
...
function evaluateRagRecall(options = {}) {
...
function evaluateLongTaskGoalConsistency(options = {}) {
    const tasks = (0, db_1.loadTasks)().slice(-Math.max(5, Number(options.taskLimit || 30)));
    let checked = 0;
    let passed = 0;
    const evidence = [];
    const gaps = [];
    for (const task of tasks) {
        ...
        const recoveredOk = recoveries.every((item) => item.goal_revalidated !== false && item.acceptance_revalidated !== false);
...
function evaluateSourceTraceability(options = {}) {
    const perScopeLimit = Math.max(20, Number(options.perScopeLimit || 200));
    const traceSources = options.traceSources !== false;
    let checked = 0;
    let passed = 0;
    ...
    for (const file of listJsonFiles(PROJECT_MEMORY_DIR)) {
        ...
        for (const archive of archives.slice(-perScopeLimit)) {
            ...
            if (ok) {
                ...
                gaps.push({ scope: "project", scopeId: memory.project, reason: "项目归档缺少 archive id/checksum/records" });
        }
    }
    try {
        const globalMemory = require("../../agents/global/memory").loadGlobalAgentMemory({ recover: false });
        for (const archive of globalMemory.archives || []) {
            ...
            if (ids.length > 0 && ids.length === Number(archive.count || ids.length)) {
                ...
                gaps.push({ scope: "global", scopeId: "global-agent", reason: "全局归档 sourceMessageIds 不完整" });
        }
    }
    catch { }
...
function childGlobalAgentMemoryBridgeTarget(memory = {}) {
    const agentMemoryKeys = Object.keys(memory.agentMemories || {}).filter(Boolean);
    if (agentMemoryKeys.length)
        return agentMemoryKeys[0];
    const completedProject = (Array.isArray(memory.completed) ? memory.completed : []).map((item) => item.project).find(Boolean);
    if (completedProject)
        return completedProject;
    const ledgerProject = (Array.isArray(memory.workerLedger) ? memory.workerLedger : []).map((item) => item.project || item.agent).find(Boolean);
    return ledgerProject || "coordinator";
}
function buildChildGlobalAgentMemoryBridgeReport(options = {}) {
    const explicitGroupIds = Array.isArray(options.groupIds || options.group_ids)
        ? (options.groupIds || options.group_ids).map((item) => String(item || "").trim()).filter(Boolean)
        : null;
    const files = explicitGroupIds?.length
    ...
    const rows = files.map(file => {
        ...
        const renderedHasCrossGroupSuppression = crossGroupSuppressedCount > 0
            ...
            : true;
        const sourceManifestHasGlobalMemory = (bundle.source_manifest?.entries || []).some((entry) => entry.id === "global_agent_memory" || entry.type === "global_agent_memory_json");
        const sourceManifestHasArbitrationLedger = (bundle.source_manifest?.entries || []).some((entry) => entry.id === "global_memory_arbitration_ledger" || entry.type === "global_memory_arbitration_ledger");
        const sourceManifestHasCrossGroupArbitration = (bundle.source_manifest?.entries || []).some((entry) => entry.id === "global_memory_cross_group_arbitration" || entry.type === "global_memory_cross_group_arbitration_ledgers");
        const compactReferencesHasGlobalMemory = (bundle.compact_file_references?.references || []).some((entry) => entry.type === "global_agent_memory_json");
        const compactReferencesHasArbitrationLedger = (bundle.compact_file_references?.references || []).some((entry) => entry.type === "global_memory_arbitration_ledger");
        const compactReferencesHasCrossGroupArbitration = (bundle.compact_file_references?.references || []).some((entry) => entry.type === "global_memory_cross_group_arbitration");
        ...
        return {
            ...
            recalledIds: (Array.isArray(recall.items) ? recall.items : []).map((item) => item.id).filter(Boolean).slice(0, 8),
...
function evaluateChildGlobalAgentMemoryBridge(options = {}) {
    ...
    const check = makeQualityCheck("child_global_agent_memory_bridge", "子 Agent 全局记忆桥接", checked, passed, evidence, gaps, "当全局 Agent 长期记忆与群聊/项目子 Agent 任务相关时，必须作为独立、有来源边界的上下文注入子 Agent 记忆包，并纳入 source manifest 与 compact file references。");
    check.report = report;
    return check;
}
function evaluateGlobalMemoryArbitrationLedger(options = {}) {
    const report = buildChildGlobalAgentMemoryBridgeReport(options);
    const rows = (report.groups || []).filter((row) => row.arbitrationLedgerRequired === true);
    const passedRows = rows.filter((row) => row.arbitrationLedgerRecorded === true
        && row.sourceManifestHasArbitrationLedger === true
        && row.compactReferencesHasArbitrationLedger === true);
    const evidence = passedRows.slice(0, 12).map((row) => ({
        groupId: row.groupId,
        targetProject: row.targetProject,
        demotedCount: row.demotedCount,
        conflictCount: row.conflictCount,
    ...
    const gaps = rows.filter((row) => !passedRows.includes(row)).slice(0, 12).map((row) => ({
        groupId: row.groupId,
        targetProject: row.targetProject,
    ...
    const check = makeQualityCheck("global_memory_arbitration_ledger", "全局记忆仲裁账本", rows.length, passedRows.length, evidence, gaps, "当全局 Agent 记忆被群聊新证据降权或冲突时，必须写入群聊仲裁账本，并纳入 source manifest 与 compact file references，支持后续 typed memory 蒸馏。");
    check.report = {
...
function evaluateGlobalMemoryArbitrationDistillation(options = {}) {
    const report = buildChildGlobalAgentMemoryBridgeReport(options);
    const rows = (report.groups || []).filter((row) => Number(row.arbitrationLedgerRepeatedConflictCount || 0) > 0);
    const passedRows = rows.filter((row) => Number(row.arbitrationPendingDistillationCount || 0) === 0
    ...
    const check = makeQualityCheck("global_memory_arbitration_distillation", "全局记忆仲裁蒸馏", rows.length, passedRows.length, evidence, gaps, "同一全局记忆多次被群聊新证据降权/冲突时，必须自动沉淀为 typed MEMORY.md，供后续子 Agent 稳定召回。");
    check.report = {
...
function evaluateGlobalMemoryCrossGroupSuppression(options = {}) {
    const report = buildChildGlobalAgentMemoryBridgeReport(options);
    const rows = (report.groups || []).filter((row) => Number(row.crossGroupSuppressedCount || 0) > 0);
    const passedRows = rows.filter((row) => row.renderedHasCrossGroupSuppression === true
    ...
    const check = makeQualityCheck("global_memory_cross_group_suppression", "跨群聊全局记忆抑制", rows.length, passedRows.length, evidence, gaps, "当同一全局 Agent 记忆已在其他群聊被降权或冲突时，后续子 Agent 包必须显式降为 background-only，并保留跨群聊仲裁 ledger 来源。");
    check.report = {
...
function normalizeQualityCheckIds(options = {}) {
...
function memoryQualityCheckDescriptors(lightweight, options = {}) {
    return [
...
function buildMemoryQualityReport(options = {}) {
    ...
    const report = {
        ...
        status: overallScore === null ? "empty" : overallScore >= 90 ? "ok" : overallScore >= 70 ? "warn" : "fail",
        checks,
        nextActions: checks.flatMap(check => (check.gaps || []).slice(0, 2).map((gap) => `${check.label}：${gap.reason || "存在缺口"}`)).slice(0, 8),
        lightweight,
        targeted,
        requestedCheckIds,
        unknownCheckIds,
    ...
    if (!targeted || options.writeTargeted === true || options.write_targeted === true)
...
function buildMemoryCenterOverview() {
    ...
    if (postCompactDisciplineTrend.overall?.alert) {
        ...
        addSystemAlert(postCompactDisciplineTrend.overall.status === "fail" ? "critical" : "warning", "post_compact_candidate_discipline", `压缩重注入候选严格分类率 ${rate}%，低于 ${postCompactDisciplineTrend.threshold}% 阈值；stale promoted ${postCompactDisciplineTrend.overall.stalePromoted || 0} 个`);
    }
    ...
    if (postCompactDispatchTrend.overall?.status === "fail" || postCompactDispatchTrend.overall?.status === "warn") {
        addSystemAlert(postCompactDispatchTrend.overall.status === "fail" ? "critical" : "warning", "post_compact_dispatch_marker", `压缩后首派发 marker 异常群聊 ${postCompactDispatchTrend.overall.failedGroups || 0} 个，等待超时 ${postCompactDispatchTrend.overall.warnGroups || 0} 个`);
    }
    ...
    if (childAgentReliabilityReport.overall?.status === "fail" || childAgentReliabilityReport.overall?.status === "warn") {
        addSystemAlert(childAgentReliabilityReport.overall.status === "fail" ? "critical" : "warning", "child_agent_memory_reliability", `子 Agent 记忆可靠性 ${childAgentReliabilityReport.overall.score ?? "待采样"}%，弱 Agent ${childAgentReliabilityReport.weakAgents?.length || 0} 个`);
    }
    ...
    if (compactBoundaryTimelineReport.overall?.status === "fail" || compactBoundaryTimelineReport.overall?.status === "warn") {
        addSystemAlert(compactBoundaryTimelineReport.overall.status === "fail" ? "critical" : "warning", "compact_boundary_timeline", `压缩边界时间线 ${compactBoundaryTimelineReport.overall.score ?? "待采样"}%，缺口 ${compactBoundaryTimelineReport.overall.gapCount || 0} 个`);
    }
    ...
    if (compactionHookLedgerReport.overall?.status === "fail" || compactionHookLedgerReport.overall?.status === "warn") {
        addSystemAlert(compactionHookLedgerReport.overall.status === "fail" ? "critical" : "warning", "compaction_hook_ledger", `压缩 Hook Ledger ${compactionHookLedgerReport.overall.score ?? "待采样"}%，失败 hook ${compactionHookLedgerReport.overall.failedHookCount || 0} 个，缺阶段 ${compactionHookLedgerReport.overall.missingPhaseGroupCount || 0} 个群聊`);
    }
    ...
    if (compactBoundaryReplayReport.overall?.status === "fail" || compactBoundaryReplayReport.overall?.status === "warn") {
        addSystemAlert(compactBoundaryReplayReport.overall.status === "fail" ? "critical" : "warning", "compact_boundary_replay_gate", `压缩边界 Replay Gate ${compactBoundaryReplayReport.overall.score ?? "待采样"}%，缺口 ${compactBoundaryReplayReport.overall.gapCount || 0} 个，待修复动作 ${compactBoundaryReplayReport.overall.repairActionCount || 0} 个`);
    }
    ...
    if (replayRepairPendingWorkItemReport.overall?.status === "fail" || replayRepairPendingWorkItemReport.overall?.status === "warn") {
        addSystemAlert(replayRepairPendingWorkItemReport.overall.status === "fail" ? "critical" : "warning", "replay_repair_pending_work_items", `Replay 修复待办覆盖率 ${replayRepairPendingWorkItemReport.overall.coverageRate ?? "待采样"}%，open ${replayRepairPendingWorkItemReport.overall.openItemCount || 0} 个，required ${replayRepairPendingWorkItemReport.overall.requiredActionCount || 0} 个`);
    }
    ...
    if (historicalCompactBoundaryReplayReport.overall?.status === "fail" || historicalCompactBoundaryReplayReport.overall?.status === "warn") {
        addSystemAlert(historicalCompactBoundaryReplayReport.overall.status === "fail" ? "critical" : "warning", "historical_compact_boundary_replay", `历史压缩边界 Replay ${historicalCompactBoundaryReplayReport.overall.score ?? "待采样"}%，历史边界 ${historicalCompactBoundaryReplayReport.overall.boundaryCount || 0} 个，缺口 ${historicalCompactBoundaryReplayReport.overall.gapCount || 0} 个`);
    }
    ...
    if (childAgentTypeReplayMatrixReport.overall?.status === "fail" || childAgentTypeReplayMatrixReport.overall?.status === "warn") {
        addSystemAlert(childAgentTypeReplayMatrixReport.overall.status === "fail" ? "critical" : "warning", "child_agent_type_replay_matrix", `子 Agent 类型 Replay ${childAgentTypeReplayMatrixReport.overall.score ?? "待采样"}%，弱类型 ${childAgentTypeReplayMatrixReport.overall.weakTypeCount || 0} 个，缺口 ${childAgentTypeReplayMatrixReport.overall.gapCount || 0} 个`);
    }
    ...
    if (compactFileReferenceReadPlanReport.overall?.status === "fail" || compactFileReferenceReadPlanReport.overall?.status === "warn") {
        addSystemAlert(compactFileReferenceReadPlanReport.overall.status === "fail" ? "critical" : "warning", "compact_file_reference_read_plan", `compact file reference read plan 覆盖率 ${compactFileReferenceReadPlanReport.overall.coverageRate ?? "待采样"}%，弱群聊 ${compactFileReferenceReadPlanReport.weakGroups?.length || 0} 个`);
    }
    ...
    if (compactFileReferenceReadPlanFreshnessReport.overall?.status === "fail" || compactFileReferenceReadPlanFreshnessReport.overall?.status === "warn") {
        addSystemAlert(compactFileReferenceReadPlanFreshnessReport.overall.status === "fail" ? "critical" : "warning", "compact_file_reference_read_plan_freshness", `compact read plan 源新鲜度 ${compactFileReferenceReadPlanFreshnessReport.overall.freshnessRate ?? "待采样"}%，changed ${compactFileReferenceReadPlanFreshnessReport.overall.changed || 0}，unverifiable ${compactFileReferenceReadPlanFreshnessReport.overall.unverifiable || 0}`);
    }
    ...
    if (compactFileReferenceReadPlanRevalidationGateReport.overall?.status === "fail" || compactFileReferenceReadPlanRevalidationGateReport.overall?.status === "warn") {
        addSystemAlert(compactFileReferenceReadPlanRevalidationGateReport.overall.status === "fail" ? "critical" : "warning", "compact_file_reference_read_plan_revalidation_gate", `compact read plan 重读门禁完成率 ${compactFileReferenceReadPlanRevalidationGateReport.overall.score ?? "待采样"}%，缺少 ${compactFileReferenceReadPlanRevalidationGateReport.overall.missing || 0} 个 current source verified 回执`);
    }
    ...
    if (Number(compactFileReferenceReadPlanRevalidationSessionBindingReport.overall?.sessionMismatch || 0) > 0) {
        addSystemAlert("critical", "compact_file_reference_read_plan_revalidation_session_binding", `compact read plan 重读回执存在 ${compactFileReferenceReadPlanRevalidationSessionBindingReport.overall.sessionMismatch || 0} 个子 Agent 会话不匹配`);
    }
    for (const row of compactFileReferenceReadPlanRevalidationSessionBindingReport.groups || []) {
        if (Number(row.sessionMismatch || 0) <= 0)
            continue;
        allAlerts.push({
            ...
            message: row.gaps?.find((gap) => gap.session_mismatch)?.reason || "compact read plan 重读回执来自错误子 Agent 会话",
        });
        const summary = groups.find(item => item.id === row.groupId);
        if (summary) {
            summary.alerts = Number(summary.alerts || 0) + 1;
    ...
    if (compactFileReferenceReadPlanRevalidationRepairWorkItemReport.overall?.status === "fail" || compactFileReferenceReadPlanRevalidationRepairWorkItemReport.overall?.status === "warn") {
        addSystemAlert(compactFileReferenceReadPlanRevalidationRepairWorkItemReport.overall.status === "fail" ? "critical" : "warning", "compact_file_reference_read_plan_revalidation_repair_work_items", `读取计划重读修复待办覆盖率 ${compactFileReferenceReadPlanRevalidationRepairWorkItemReport.overall.coverageRate ?? "待采样"}%，open ${compactFileReferenceReadPlanRevalidationRepairWorkItemReport.overall.openItemCount || 0} 个，required ${compactFileReferenceReadPlanRevalidationRepairWorkItemReport.overall.requiredActionCount || 0} 个`);
    }
    ...
    if (compactFileReferenceUsageDisciplineReport.overall?.status === "fail" || compactFileReferenceUsageDisciplineReport.overall?.status === "warn") {
        addSystemAlert(compactFileReferenceUsageDisciplineReport.overall.status === "fail" ? "critical" : "warning", "compact_file_reference_usage_discipline", `compact file reference 使用声明率 ${compactFileReferenceUsageDisciplineReport.overall.score ?? "待采样"}%，缺少 ${compactFileReferenceUsageDisciplineReport.overall.missing || 0} 个 memoryUsed/memoryIgnored 证据`);
    }
...
function runMemoryCenterPostCompactUsageDiagnosticsSelfTest() {
    ...
    try {
        ...
        const detail = getMemoryCenterScope("group", groupId);
...
function runMemoryCenterPostCompactDispatchMarkerTrendSelfTest() {
    ...
    const reloadFile = path.join(utils_1.CCM_DIR, "group-memory-reload", `${cleanId(groupId)}.json`);
    let dispatchFile = "";
    let typedDir = "";
    try {
...
function runMemoryCenterChildGlobalAgentMemoryBridgeSelfTest() {
    ...
    const messageFile = path.join(utils_1.GROUP_MESSAGES_DIR, `${groupId}.json`);
    const reloadFile = path.join(utils_1.CCM_DIR, "group-memory-reload", `${cleanId(groupId)}.json`);
    const typedDir = path.join(utils_1.CCM_DIR, "group-memory-md", sidecarFileId(groupId));
    ...
    try {
        ...
        try {
            ...
            if (previousGlobalMemory === null)
                fs.rmSync(GLOBAL_MEMORY_FILE, { force: true });
            else
                fs.writeFileSync(GLOBAL_MEMORY_FILE, previousGlobalMemory, "utf-8");
            if (previousGlobalMemoryBak === null)
                fs.rmSync(`${GLOBAL_MEMORY_FILE}.bak`, { force: true });
            else
                fs.writeFileSync(`${GLOBAL_MEMORY_FILE}.bak`, previousGlobalMemoryBak, "utf-8");
        }
        catch { }
    }
}
...
function runMemoryCenterChildGlobalAgentMemoryCrossGroupSuppressionSelfTest() {
    ...
    const groupFiles = [sourceGroupId, targetGroupId].flatMap(groupId => [
        ...
        path.join(utils_1.CCM_DIR, "group-memory-reload", `${cleanId(groupId)}.json`),
        path.join(utils_1.CCM_DIR, "group-memory-reload", `${cleanId(groupId)}.json.bak`),
    ...
    const cleanupDirs = [sourceGroupId, targetGroupId].flatMap(groupId => [
        path.join(utils_1.CCM_DIR, "group-memory-md", sidecarFileId(groupId)),
    ...
    try {
        ...
        try {
            if (previousQuality === null)
                fs.rmSync(QUALITY_FILE, { force: true });
            else
                fs.writeFileSync(QUALITY_FILE, previousQuality, "utf-8");
        }
        catch { }
    }
}
...
function runMemoryCenterGroupSessionMemorySnapshotSelfTest() {
    ...
    try {
        const { saveGroupMessages, } = require("../collaboration/storage");
        const { buildGroupContextPacket, buildAgentMemoryContextBundle, renderGroupMemoryContextBundle, buildGlobalGroupMemoryContext, renderGlobalGroupMemoryContextBundle, loadGroupMemory, readGroupSessionMemorySnapshotSummary, } = require("../collaboration/memory");
...
function runMemoryCenterCompactFileReferenceReadPlanRevalidationRepairWorkItemsSelfTest() {
    ...
    try {
        ...
        const failingDetail = getMemoryCenterScope("group", groupId);
        ...
        const resolvedDetail = getMemoryCenterScope("group", groupId);
        ...
        const checks = {
            ...
            correctSessionClosesRepairWorkItem: resolvedReadPlanRepairItems.length >= 1
                && !resolvedReadPlanRepairItems.some((item) => replayRepairWorkItemOpen(item.status))
                && resolvedReadPlanRepairItems.some((item) => item.component === "compact_read_plan_revalidation" && replayRepairWorkItemStatus(item.status) === "completed"),
        };
        return {
            pass: Object.values(checks).every(Boolean),
            checks,
