...
function globalRunVisibleReply(run, fallback = GLOBAL_AGENT_VISIBLE_RESULT_FALLBACK, max = 8000) {
...
function buildGlobalAgentEventUi(event = {}) {
    ...
    if (type === "completed")
        return withCheckpoint({ phase: "completed", tone: "ok", title: "处理结果", text: text(event.reply || GLOBAL_AGENT_VISIBLE_COMPLETED_EVENT_FALLBACK) });
    if (type === "failed")
...
function normalizeGlobalAgentMessage(item) {
    ...
    const visible = role === "assistant"
        ? (0, loop_1.buildGlobalVisibleReplyContent)({ value: rawContent, rawSource: item.technical_content || item.technicalContent || rawContent, fallback: "回复已整理，技术细节已放入技术详情。", max: 8000 })
...
function runGlobalAgentIntentSelfTest() {
    ...
    const modelUnavailableGreeting = localActionToAgenticDecision(localGreeting, { steps: [], user_message: "你好", explicit_write_authorization: false });
    ...
    const feishuTaskVisible = formatGlobalTaskDispatchVisibleResult({ task: { id: "task-secret-3", title: "修复登录问题" }, id: "task-secret-3", queue: { queued: true, position: 1 } }, { title: "修复登录问题" });
    ...
    const testAgentRelayChecks = {
        ...
        globalTestAgentUnknownCoverageRelayNeedsUser: unknownCoverageTestAgentRelay?.independentReviewSummary?.status === "needs_user"
            ...
            && unknownCoverageTestAgentRelay?.independentReviewSummary?.rows.some((item) => item.includes("验收条件待确认"))
        ...
        globalTestAgentNotVerifiedCoverageRelayNeedsRework: notVerifiedCoverageTestAgentRelay?.independentReviewSummary?.status === "needs_rework"
            ...
            && notVerifiedCoverageTestAgentRelay?.independentReviewSummary?.rows.some((item) => item.includes("必检项：浏览器流程未覆盖"))
            && notVerifiedCoverageTestAgentRelay?.independentReviewSummary?.rows.some((item) => item.includes("验收条件未通过"))
        ...
        globalTestAgentSummaryOnlyGapRelayNeedsRework: summaryOnlyGapTestAgentRelay?.independentReviewSummary?.status === "needs_rework"
            && summaryOnlyGapTestAgentRelay?.independentReviewSummary?.rows.some((item) => item.includes("必检项：浏览器流程未覆盖"))
        ...
        globalTestAgentWeakSummaryRelayNeedsUser: weakSummaryTestAgentRelay?.independentReviewSummary?.status === "needs_user"
            ...
            && weakSummaryTestAgentRelay?.independentReviewSummary?.rows.some((item) => item.includes("验收证据待确认") && item.includes("整体复核结果推断"))
        ...
        globalTestAgentFailureSummaryRelayNeedsRework: failureSummaryTestAgentRelay?.independentReviewSummary?.status === "needs_rework"
            ...
            && failureSummaryTestAgentRelay?.independentReviewSummary?.rows.some((item) => item.includes("排查建议") && item.includes("打开失败截图核对页面"))
...
function compactGlobalTestAgentExecutionPlanRelayEvent(event = {}, options = {}) {
    ...
    const blocked = plan?.valid === false || String(event.status || "").toLowerCase() === "warn";
    const detail = globalVisibleText(blocked
        ? "TestAgent 复核计划预检未通过，我会先修复交接信息再执行。"
        : "TestAgent 已生成复核计划，我会按计划启动独立复核。", "TestAgent 复核计划已整理。", 260);
    return {
...
function compactGlobalTestAgentReviewRelayEvent(event = {}, options = {}) {
    if (String(event?.type || "") !== "test_agent_review_ready" && String(event?.type || "") !== "agent_done")
        return null;
    ...
    if (!receipt && !report && !/TestAgent\s*独立复核完成|TestAgent.+复核/i.test(String(event.text || event.detail || "")))
        return null;
    ...
    const evidence = [
        ...
        ...blockers.slice(0, 3).map((item) => `待处理：${globalVisibleText(item, "复核发现待处理缺口。", 180)}`),
        ...coverageGaps.weakLines.slice(0, 2).map((item) => `待确认：${globalVisibleText(item, "复核证据强度仍需确认。", 180)}`),
    ].filter(Boolean);
    const summary = {
        schema: "ccm-main-agent-independent-review-summary-v1",
        title: "独立复核",
...
function sanitizeGlobalDirectAgentOutput(value, fallback = "执行目标已返回结果，详细排障信息已放入技术详情。", max = 700) {
    ...
    if (!text)
        return (0, display_1.sanitizeMainAgentUserText)(fallback, fallback, max);
    if (GLOBAL_DIRECT_DISPATCH_INTERNAL_PATTERN.test(text)) {
        if (/error|失败|denied|invalid|权限|门禁/i.test(text))
            return (0, display_1.sanitizeMainAgentUserText)("执行时遇到需要排查的问题，详细原因已放入技术详情。", fallback, max);
        if (/done|完成|receipt|回执/i.test(text))
            return (0, display_1.sanitizeMainAgentUserText)("执行成员已提交结果说明，我会继续汇总验收。", fallback, max);
        return (0, display_1.sanitizeMainAgentUserText)(fallback, fallback, max);
    }
    text = text.replace(/\n{3,}/g, "\n\n").trim();
    return (0, display_1.sanitizeMainAgentUserText)(text.length > max ? `${text.slice(0, max)}...` : text, fallback, max);
}
...
function getGlobalStatusRunFromMission(mission, runs) {
    ...
    if (runId) {
        try {
            return (0, loop_1.getGlobalAgentRun)(runId);
        }
        catch { }
    }
    return null;
...
function collectStandaloneGlobalStatusRuns(inputRuns, missions, tasks) {
    const runs = Array.isArray(inputRuns) ? inputRuns : (0, loop_1.listGlobalAgentRuns)({ limit: 12 });
...
async function executeFeishuManagementAction(baseUrl, action, originalText = "") {
    ...
    if (action.type === "manage_cron") {
        ...
        if (operation === "list")
            ...
            result = await postLocalApi(baseUrl, "/api/projects/archive", { name: project });
    }
...
async function executeFeishuAction(baseUrl, action, originalText = "", traceId = "", options = {}) {
    ...
    if (action.type === "send_group_cmd") {
        ...
        const result = await postLocalSseOrJsonApi(baseUrl, "/api/groups/send", {
            ...
            onEvent: (event) => relayGlobalTestAgentEventFromGroup(event, {
    ...
    return {
        ...
        onCompleted: async (record, report) => {
            ...
            if (/feishu/i.test(record.source)) {
                (0, feishu_channel_1.bindFeishuIdentifiersFromValue)(record.session_id, report);
                (0, feishu_channel_1.bindFeishuTaskContext)({ sessionId: record.session_id, runIds: [record.global_run_id], missionIds: [record.mission_id], source: record.source });
                const delivered = await (0, feishu_channel_1.notifyFeishuTaskStage)({ stage: "completion", title: "任务已经完成", markdown: formatted, sessionId: record.session_id, runId: record.global_run_id, missionId: record.mission_id, dedupeKey: `mission:${record.mission_id}:completed` });
        ...
        onProgress: async (record, event) => {
            ...
            (0, feishu_channel_1.bindFeishuTaskContext)({ sessionId: record.session_id, runIds: [record.global_run_id], missionIds: [record.mission_id], taskIds, source: record.source });
            if (event?.type === "waiting_user") {
                const lines = (event.items || []).map((item) => `- ${item.reason || "需要你补充信息"}`);
                const markdown = `任务暂时需要你的帮助：\n${lines.join("\n")}`;
                const delivered = await (0, feishu_channel_1.notifyFeishuTaskStage)({ stage: "waiting_user", title: "任务需要你补充信息", markdown, sessionId: record.session_id, missionId: record.mission_id, dedupeKey: `mission:${record.mission_id}:waiting-user:${record.cycle_count}` });
                if (delivered.reason === "no_binding")
                    await (0, collaboration_1.sendFeishuReportMessage)({ title: "全局 Agent 等待人工处理", markdown });
                return;
            }
        ...
        onTerminal: async (record, outcome, report) => {
            ...
            if (/feishu/i.test(record.source)) {
                ...
                const delivered = await (0, feishu_channel_1.notifyFeishuTaskStage)({ stage: outcome, title, markdown, sessionId: record.session_id, missionId: record.mission_id, dedupeKey: `mission:${record.mission_id}:terminal:${outcome}` });
                if (delivered.reason === "no_binding")
                    await (0, collaboration_1.sendFeishuReportMessage)({ title, markdown });
            }
        },
    };
}
...
function createAgenticRuntime(baseUrl, ctx, input = {}) {
    ...
    return {
        ...
        fallbackDecision: (run, error) => {
            ...
            (0, runtime_1.recordGlobalAgentRuntimeOutput)(run, { type: "model_fallback", status: "warning", error: detail });
...
async function runAgenticGlobalRequest(baseUrl, ctx, input) {
    ...
    if (!startsNewTopic && requestedClarificationRunId) {
        const requestedRun = (0, loop_1.getGlobalAgentRun)(requestedClarificationRunId);
        ...
        waitingClarification = (0, loop_1.findClarifyingGlobalAgentRun)(sessionId);
    }
...
async function processFeishuGlobalAgentMessage(baseUrl, ctx, text, payload, options = {}) {
    ...
    const destination = (0, feishu_channel_1.recordFeishuInbound)({ payload, sessionId: conversationId, messageId: getFeishuMessageId(payload) });
    (0, feishu_channel_1.bindFeishuTaskContext)({ sessionId: conversationId, destination, source: "feishu-control-bot" });
    ...
    try {
        ...
        if (confirmationMatch) {
            ...
            const onFeishuRuntimeEvent = (event) => {
                (0, feishu_channel_1.bindFeishuIdentifiersFromValue)(conversationId, event, destination);
                const presentation = (0, feishu_channel_1.feishuRuntimeEventPresentation)(event);
                if (!presentation)
                    return;
        ...
        const missionSnapshot = run.mission_id ? (0, collaboration_1.getGlobalDevelopmentMission)(run.mission_id) : null;
...
async function drainFeishuConversationTurns(baseUrl, ctx, conversationId, payload) {
    ...
    try {
        while (true) {
            const turn = conversation_turn_control_1.conversationTurnControl.claim({ scope: "feishu", conversation_id: conversationId });
            if (!turn)
                break;
            try {
                ...
                conversation_turn_control_1.conversationTurnControl.settle({ id: turn.id, status: "completed", result: { reply } });
            }
            catch (error) {
...
function startFeishuConversationTurnRecoveryForServer(baseUrl, ctx) {
    ...
    const tick = () => {
        const queued = conversation_turn_control_1.conversationTurnControl.list({ scope: "feishu", statuses: "queued", limit: 500 }).turns;
        const conversationIds = [...new Set(queued.map((turn) => String(turn.conversation_id || "")).filter(Boolean))];
        for (const conversationId of conversationIds) {
            const active = (0, loop_1.listGlobalAgentRuns)({ sessionId: conversationId, limit: 20 })
...
async function processFeishuControlledMessage(baseUrl, ctx, text, payload, options = {}) {
    ...
    const activeRun = (0, loop_1.listGlobalAgentRuns)({ sessionId: conversationId, limit: 20 })
        .find((run) => ["running", "supervising", "paused"].includes(String(run?.status || ""))) || null;
    if (command.kind === "stop") {
        if (activeRun?.id)
            (0, loop_1.cancelGlobalAgentRun)(activeRun.id);
    ...
    if (activeRun && command.kind === "steer") {
        ...
        try {
            (0, loop_1.steerGlobalAgentRun)(activeRun.id, command.message, {
                kind: "auto",
                source: "feishu_mid_turn",
                requestId: queued.turn.request_id,
            });
            conversation_turn_control_1.conversationTurnControl.settle({ id: queued.turn.id, status: "applied", active_run_id: activeRun.id });
            return { reply: "已把这条要求纳入当前工作，我会在安全节点重新核对计划并继续。", turn: queued.turn, run_id: activeRun.id };
        }
        catch (error) {
            conversation_turn_control_1.conversationTurnControl.settle({ id: queued.turn.id, status: "failed", error: error?.message || String(error) });
            throw error;
        }
    }
    if (activeRun && (command.kind === "queue" || command.kind === "normal")) {
        ...
        const position = conversation_turn_control_1.conversationTurnControl.list({ scope: "feishu", conversation_id: conversationId, statuses: "queued,sending" })
    ...
    if (pathname === "/api/feishu/control-bot/message" && req.method === "POST") {
        ...
        req.on("end", async () => {
            try {
                ...
                if (operationKey)
                    (0, reliability_ledger_1.completeIdempotency)("feishu-control-message", operationKey, controlled);
                (0, utils_1.sendJson)(res, { success: true, message: controlled.queued ? "控制机器人消息已排队" : "控制机器人消息已处理", ...controlled, trace_id: operation?.traceId || "" });
            }
            catch (error) {
                if (!res.headersSent)
        ...
        req.on("end", () => {
            try {
                ...
                void processFeishuControlledMessage(getRequestBaseUrl(req), ctx, text, payload, { traceId: operation?.traceId })
                    .then(result => {
                    if (operationKey)
                        (0, reliability_ledger_1.completeIdempotency)("feishu-event", operationKey, result);
                    if (result.queued || result.stopped_run_id || result.turn?.mode === "steer") {
                        return (0, collaboration_1.sendFeishuReportMessage)({ title: "全局 Agent", markdown: result.reply });
                    }
                })
        ...
        const handleRun = async (payload, files = []) => {
            ...
            try {
                ...
                if (reliabilityOperationKey && reliabilityOperationAcquired) {
                    try {
                        (0, reliability_ledger_1.failIdempotency)("global-agent-request", reliabilityOperationKey, error);
                    }
                    catch { }
                }
        ...
        const handleAgenticChatProxy = async (payload, files = []) => {
            ...
            try {
                ...
                if (isStream) {
                    ...
                    (0, utils_1.sendJson)(res, { success: true, reply: globalRunVisibleReply(run, ""), run: result, files: responseFiles, agentic: true });
                }
            }
            catch (error) {
                if (isStream) {
