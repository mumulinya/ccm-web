...
function buildAgenticContext(query = "", sessionId = "") {
    ...
    return {
        ...
        global_memory: query ? (0, memory_2.buildGlobalAgentMemoryPacket)(query, { sessionId, limit: 7 }) : "",
    };
}
...
async function runAgenticGlobalRequest(baseUrl, ctx, input) {
    ...
    if (!/feishu/i.test(input.source || "")) {
        try {
            (0, memory_2.ingestGlobalAgentConversation)({ sessionId, source: input.source || "web", messages: [{ role: "assistant", content: run.final_reply || "", timestamp: new Date().toISOString(), trace_id: run.trace_id, mission_id: run.mission_id }] });
        }
        catch (error) {
