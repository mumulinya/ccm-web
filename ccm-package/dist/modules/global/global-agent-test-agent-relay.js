...
exports.createGlobalAgentTestAgentRelay = createGlobalAgentTestAgentRelay;
// TestAgent plan and review events projected from a group task into the global run.
function createGlobalAgentTestAgentRelay(deps) {
    ...
    function compactGlobalTestAgentReviewRelayEvent(event = {}, options = {}) {
        ...
        const { summarizeTestAgentBrowserProviderGaps, deriveIndependentReviewDecision, } = require("../collaboration/test-agent-independent-review-decision");
        ...
        const forceRework = hasFailedCoverage
            ...
            || rawStatus === "failed";
        const forceRecheck = !forceRework && (spotCheckNeedsRecheck
        ...
        const { buildTestAgentEnvironmentPrepChecklist, collectTestAgentFailureScreenshotRefs, formatFailureScreenshotTechnicalRows, formatTestAgentEnvironmentPrepUserLines, } = require("../collaboration/test-agent-environment-prep");
        ...
        const evidence = [
            ...
            ...providerGaps.lines.slice(0, 3).map((item) => `Provider缺口：${globalVisibleText(item, "浏览器 Provider 能力不足。", 180)}`),
...
//# sourceMappingURL=global-agent-test-agent-relay.js.map