...
function createGlobalAgentAgenticRuntime(deps) {
    ...
    function buildGlobalProviderPayloadSnapshot(messages, sessionId, run) {
        ...
        const modelVisibleToolSpecs = GLOBAL_AGENT_TOOL_SPECS.filter(spec => !(0, global_tool_load_policy_1.isGlobalDeferredTool)(spec.name, loadedToolNames));
            ...
            callModel: async (messages, run, signal) => {
                ...
                const invoke = (providerMessages) => {
                    ...
                    return (0, global_native_query_adapter_1.runGlobalNativeQueryCall)({
                        ...
                        if (String(result?.message || "").trim() || !["answer", "clarify", "needs_clarification", "needs_confirmation"].includes(resultState))
                            return result;
