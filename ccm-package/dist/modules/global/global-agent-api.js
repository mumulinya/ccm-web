            ...
            req.on("end", () => {
                try {
                    ...
                    void processFeishuControlledMessage(getRequestBaseUrl(req), ctx, text, payload, { traceId: operation?.traceId })
                        .then(result => {
                        if (operationKey)
                            completeIdempotency("feishu-event", operationKey, result);
                        ...
                        failIdempotency("feishu-event", operationKey, error); });
                }
                catch (error) {
                    if (!res.headersSent)
            ...
            req.on("end", async () => {
                try {
                    ...
                    if (storedRun?.supervisor_id && ["supervising", "paused"].includes(storedRun.status)) {
                        ...
                        const kind = classifyGlobalAgentUserSteer(message, payload.kind || payload.steering_kind || payload.steeringKind || "auto");
            ...
            const handleRun = async (payload, files = []) => {
                ...
                try {
                    ...
                    if (isGlobalProgressStatusRequest(message)) {
                        ...
                        if (operationKey)
                            completeIdempotency("global-agent-request", operationKey, { run: result, status: result.status });
                        if (isStream) {
