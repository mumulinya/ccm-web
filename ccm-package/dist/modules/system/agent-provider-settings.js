...
function loginCommand(provider) {
    if (provider === "codex")
        return { command: "codex", args: ["login"], title: "CCM - Codex 登录" };
    if (provider === "cursor") {
        const command = resolveCursorAgentCommand();
        return { command, args: ["login"], title: "CCM - Cursor 登录" };
    }
    if (provider === "gemini")
        return { command: "gemini", args: [], title: "CCM - Gemini CLI 登录" };
    if (provider === "opencode")
        return { command: "opencode", args: ["auth", "login"], title: "CCM - OpenCode Provider 登录" };
...
function getAgentProviderModels(providerValue) {
    ...
    if (provider === "opencode") {
        if (!commandExists("opencode"))
            return { provider, selected: settings.opencode.model, models: [], allowsCustom: true, error: "OpenCode 尚未安装" };
...
function startAgentProviderLogin(providerValue) {
    ...
    if (!['codex', 'cursor', 'gemini', 'opencode'].includes(provider))
        throw new Error("该 Agent 不支持 CLI 登录");
