...
exports.isBrowserRequest = isBrowserRequest;
exports.isTrustedLocalAgentRequest = isTrustedLocalAgentRequest;
...
function resolveInitialAdminPassword() {
    ...
    if (fromEnv.length >= 8 && fromEnv.length <= 128)
        return { password: fromEnv, generated: false };
    return { password: crypto.randomBytes(18).toString("base64url"), generated: true };
}
function persistInitialAdminPassword(password) {
    try {
        ensureAuthDir();
...
function discardInitialAdminPassword() {
    try {
        fs.unlinkSync(INITIAL_PASSWORD_FILE);
    }
    catch { }
}
function normalizeLoginTheme(value) {
    return ["command", "minimal", "light"].includes(String(value || "")) ? value : "command";
}
function validateLoginTheme(value) {
    const theme = String(value || "");
    if (!["command", "minimal", "light"].includes(theme))
        throw new Error("登录主题无效");
    return theme;
}
...
function saveUsers(store) {
    ensureAuthDir();
    (0, atomic_json_file_1.writeJsonAtomic)(USERS_FILE, { ...store, updatedAt: now() });
    try {
...
function saveSessions(store) {
    ensureAuthDir();
    (0, atomic_json_file_1.writeJsonAtomic)(SESSIONS_FILE, { ...store, updatedAt: now() });
    try {
...
function readUsersUnlocked() {
    ...
    const loaded = (0, atomic_json_file_1.readJsonWithBackup)(USERS_FILE, emptyUserStore());
...
function loadUsers() {
    return (0, atomic_json_file_1.withFileLock)(USERS_FILE, readUsersUnlocked);
}
function readSessionsUnlocked() {
    ensureAuthDir();
    const loaded = (0, atomic_json_file_1.readJsonWithBackup)(SESSIONS_FILE, emptySessionStore());
    const current = Date.now();
    const sessions = (Array.isArray(loaded?.sessions) ? loaded.sessions : []).filter(session => Date.parse(session.expiresAt) > current);
    if (sessions.length !== (loaded?.sessions || []).length)
        saveSessions({ schema: "ccm-local-auth-sessions-v1", sessions, updatedAt: now() });
    return { schema: "ccm-local-auth-sessions-v1", sessions, updatedAt: String(loaded?.updatedAt || now()) };
}
function loadSessions() {
    return (0, atomic_json_file_1.withFileLock)(SESSIONS_FILE, readSessionsUnlocked);
}
...
function parseCookies(req) {
    ...
    for (const entry of String(req.headers.cookie || "").split(";")) {
        ...
        try {
            values[key] = decodeURIComponent(entry.slice(index + 1).trim());
        }
        catch { }
    }
    return values;
...
function createSession(req, res, user) {
    ...
    (0, atomic_json_file_1.withFileLock)(SESSIONS_FILE, () => {
        const store = readSessionsUnlocked();
        const otherSessions = store.sessions.filter(session => session.userId !== user.id).concat(store.sessions.filter(session => session.userId === user.id).slice(-19));
        otherSessions.push({
...
function deleteRequestSession(req, res) {
    const token = parseCookies(req)[SESSION_COOKIE];
    if (token) {
        const hash = tokenHash(token);
        (0, atomic_json_file_1.withFileLock)(SESSIONS_FILE, () => {
...
function isTrustedLocalAgentRequest(req) {
    if (!isLoopbackAddress(req.socket?.remoteAddress))
        return false;
    const forwarded = String(req.headers["x-forwarded-for"] || "").split(",")[0].trim();
    return !forwarded || isLoopbackAddress(forwarded);
}
function browserApiAccessAllowed(req) {
    if (!sameOrigin(req))
        return false;
    if (resolveLocalAuthSession(req))
        return true;
...
function completeOnboarding() {
    (0, atomic_json_file_1.withFileLock)(USERS_FILE, () => {
...
function handleLocalAuthApi(pathname, req, res) {
    ...
    if (!sameOrigin(req)) {
        (0, utils_1.sendJson)(res, { success: false, error: "请求来源无效" }, 403);
        return true;
    }
    if (pathname === "/api/auth/login" && req.method === "POST") {
        void readJsonBody(req).then(payload => {
            ...
            (0, utils_1.sendJson)(res, { success: true, user: publicUser(user), session, registration_enabled: authState.registrationEnabled, first_install: false, login_theme: authState.loginTheme });
        }).catch((error) => (0, utils_1.sendJson)(res, { success: false, error: error?.message || "登录失败" }, 429));
        return true;
    }
    if (pathname === "/api/auth/register" && req.method === "POST") {
        void readJsonBody(req).then(payload => {
            ...
            const user = (0, atomic_json_file_1.withFileLock)(USERS_FILE, () => {
                const store = readUsersUnlocked();
                const firstInstall = !store.onboardingCompleted;
                if (!store.registrationEnabled && !firstInstall)
                    throw new Error("当前未开放注册");
                const normalized = normalizedUsername(username);
                if (store.users.some(item => item.normalizedUsername === normalized))
            ...
            const session = createSession(req, res, user);
            const authState = loadUsers();
            (0, utils_1.sendJson)(res, { success: true, user: publicUser(user), session, registration_enabled: authState.registrationEnabled, first_install: false, login_theme: authState.loginTheme }, 201);
        }).catch((error) => {
            const message = error?.message || "注册失败";
            (0, utils_1.sendJson)(res, { success: false, error: message }, /未开放/.test(message) ? 403 : 400);
        });
        return true;
    }
    ...
    if (pathname === "/api/auth/settings" && (req.method === "PUT" || req.method === "POST")) {
        ...
        if (!auth)
            return true;
        void readJsonBody(req).then(payload => {
            const store = (0, atomic_json_file_1.withFileLock)(USERS_FILE, () => {
                const current = readUsersUnlocked();
                if (typeof payload.registration_enabled === "boolean")
                    current.registrationEnabled = payload.registration_enabled;
                if (payload.login_theme !== undefined)
                    current.loginTheme = validateLoginTheme(payload.login_theme);
                current.onboardingCompleted = true;
                saveUsers(current);
                return current;
            });
            (0, utils_1.sendJson)(res, { success: true, registration_enabled: store.registrationEnabled, login_theme: store.loginTheme, user_count: store.users.length });
        }).catch((error) => (0, utils_1.sendJson)(res, { success: false, error: error?.message || "保存失败" }, 400));
        return true;
    }
    if (pathname === "/api/auth/password" && req.method === "POST") {
        ...
        void readJsonBody(req).then(payload => {
            ...
            if (!passwordMatches(currentPassword, auth.user.password))
            ...
            if (auth.user.normalizedUsername === normalizedUsername(INITIAL_USERNAME))
                discardInitialAdminPassword();
            (0, atomic_json_file_1.withFileLock)(SESSIONS_FILE, () => {
            ...
            setSessionCookie(req, res, "", 0);
            (0, utils_1.sendJson)(res, { success: true, relogin_required: true });
        }).catch((error) => (0, utils_1.sendJson)(res, { success: false, error: error?.message || "修改密码失败" }, 400));
        return true;
    }
    (0, utils_1.sendJson)(res, { success: false, error: "Not Found" }, 404);
    return true;
}
function localAuthStorageFiles() {
