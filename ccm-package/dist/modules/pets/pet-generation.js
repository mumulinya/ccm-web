...
exports.setPetGenerationConfigChangedNotifier = setPetGenerationConfigChangedNotifier;
exports.setPetGenerationLifecycleNotifier = setPetGenerationLifecycleNotifier;
exports.createPetGenerationJob = createPetGenerationJob;
exports.listPetGenerationJobs = listPetGenerationJobs;
exports.toPublicPetGenerationJob = toPublicPetGenerationJob;
exports.getPetGenerationJob = getPetGenerationJob;
exports.cancelPetGenerationJob = cancelPetGenerationJob;
exports.retryPetGenerationJob = retryPetGenerationJob;
exports.recoverPetGenerationJobs = recoverPetGenerationJobs;
exports.runPetGenerationContractSelfTest = runPetGenerationContractSelfTest;
...
const JOBS_FILE = path.join(utils_1.CCM_DIR, "pet-generation-jobs.json");
const RUNS_DIR = path.join(utils_1.CCM_DIR, "pet-generation");
const HATCH_SKILL_PATH = path.join(utils_1.CCM_DIR, "..", ".codex", "skills", "hatch-pet", "SKILL.md");
const activeProcesses = new Map();
const progressWatchers = new Map();
const pendingQueue = [];
const MAX_CONCURRENT_GENERATIONS = 1;
let configChangedNotifier = null;
let lifecycleNotifier = null;
let cachedPetCodexRuntime;
...
function discoverPatchedCodexExecutables(root, maxDepth = 9) {
    const results = [];
...
function probeCodexRuntime(command) {
    try {
...
function resolvePetCodexRuntime() {
    if (cachedPetCodexRuntime !== undefined)
    ...
    const available = candidates.map(probeCodexRuntime).filter(Boolean);
...
function buildPetCodexCommand(promptPath, runtime) {
...
function petGenerationExitError(job, code, signal) {
    let log = "";
    try {
        log = fs.readFileSync(job.logPath, "utf-8").slice(-12_000);
    }
    catch { }
...
function setPetGenerationConfigChangedNotifier(notifier) {
    configChangedNotifier = notifier;
}
function setPetGenerationLifecycleNotifier(notifier) {
...
function validateReferencePath(referencePath) {
    ...
    if (!resolved || !isPathInside(utils_1.CCM_DIR, resolved))
        throw new Error("参考图片必须来自本次上传的 CCM 附件");
    if (!fs.existsSync(resolved) || !fs.statSync(resolved).isFile())
        throw new Error("参考图片不存在");
    const ext = path.extname(resolved).toLowerCase();
    if (![".png", ".jpg", ".jpeg", ".webp"].includes(ext))
        throw new Error("参考图片仅支持 PNG、JPG 或 WebP");
    const size = fs.statSync(resolved).size;
    if (size <= 0 || size > 12 * 1024 * 1024)
        throw new Error("参考图片必须小于 12 MB");
    return resolved;
}
...
function verifyGeneratedPackage(job) {
    const finalDir = path.join(job.runDir, "final");
    const manifestPath = path.join(finalDir, "pet.json");
    if (!fs.existsSync(manifestPath))
        throw new Error("生成任务没有产出 final/pet.json");
    const manifest = JSON.parse(fs.readFileSync(manifestPath, "utf-8"));
    if (Number(manifest.spriteVersionNumber) !== 2)
        throw new Error("生成宠物不是 Codex v2 图集");
    ...
    if (!fs.existsSync(spritePath))
        throw new Error("生成任务没有产出 spritesheet.webp");
    ...
    const requiredJson = (relative) => {
        const file = path.join(job.runDir, relative);
        if (!fs.existsSync(file))
            throw new Error(`缺少动作验收证据：${relative}`);
        return JSON.parse(fs.readFileSync(file, "utf-8"));
    };
    const validation = requiredJson(path.join("final", "validation-extended.json"));
    if (validation.ok !== true)
        throw new Error("扩展动作图集校验未通过");
    const review = requiredJson(path.join("qa", "review.json"));
    if (review.ok === false || (Array.isArray(review.errors) && review.errors.length))
        throw new Error("动作逐帧 QA 未通过");
    ...
    const previewCount = fs.existsSync(previewsDir)
        ...
        : 0;
    if (previewCount < 9)
        throw new Error(`动作预览不完整：仅找到 ${previewCount}/9 组`);
...
function installGeneratedPackage(job, verified) {
    const relativeDir = path.join("generated", job.petId);
    const targets = [
        path.join(utils_1.PUBLIC_DIR, "pets", relativeDir),
        path.resolve(__dirname, "..", "..", "..", "pet", "assets", relativeDir),
    ];
    const devWebRoot = path.resolve(utils_1.PUBLIC_DIR, "..", "..", "frontend", "public", "pets", relativeDir);
    if (fs.existsSync(path.resolve(utils_1.PUBLIC_DIR, "..", "..", "frontend")))
        targets.push(devWebRoot);
    ...
    const customTypes = Array.isArray(config.customTypes) ? config.customTypes.filter((item) => item?.id !== job.petId) : [];
    const skin = {
        id: job.petId,
        name: job.name,
    ...
    writeJsonAtomic(utils_1.PETS_FILE, config);
    return skin;
}
function stopProgressWatcher(jobId) {
...
function enqueueGenerationJob(job) {
    if (!pendingQueue.includes(job.id) && !activeProcesses.has(job.id))
        pendingQueue.push(job.id);
    pumpGenerationQueue();
}
function createPetGenerationJob(input) {
...
function getPetGenerationJob(id) {
    return readJobs().find(item => item.id === id) || null;
}
function cancelPetGenerationJob(id) {
    const job = getPetGenerationJob(id);
    if (!job)
        throw new Error("宠物生成任务不存在");
    ...
    if (child) {
        try {
            if (process.platform === "win32" && child.pid) {
                (0, child_process_1.spawn)("taskkill", ["/PID", String(child.pid), "/T", "/F"], { windowsHide: true, stdio: "ignore" });
            }
            else if (child.pid) {
...
function retryPetGenerationJob(id) {
    const job = getPetGenerationJob(id);
    if (!job)
        throw new Error("宠物生成任务不存在");
    ...
    for (const relative of ["final", "qa", "progress.json"]) {
        try {
            fs.rmSync(path.join(job.runDir, relative), { recursive: true, force: true });
        }
        catch { }
    }
    const next = patchJob(id, { status: "queued", stageLabel: "已重新进入生成队列", progress: 0, error: "", pid: undefined });
    if (next)
        setTimeout(() => enqueueGenerationJob(next), 10);
    return next;
}
function recoverPetGenerationJobs() {
    const rows = readJobs();
    let recovered = 0;
...
function runPetGenerationContractSelfTest() {
    const dimensions = (width, height) => width === 1536 && height === 2288;
    ...
    const writePngHeader = (file, width, height) => {
    ...
    const writeQaFixtures = (job) => {
        ...
        for (let index = 0; index < 9; index++)
            fs.writeFileSync(path.join(previewsDir, `${index}.gif`), "fixture");
    };
    let rejectsV1 = false;
    ...
    try {
        ...
        try {
            verifyGeneratedPackage(wrong);
        }
        catch {
            rejectsWrongDimensions = true;
        }
    ...
    const checks = {
        ...
        reference_path_is_workspace_scoped: !isPathInside(utils_1.CCM_DIR, path.resolve(utils_1.CCM_DIR, "..", "outside.png")),
...
//# sourceMappingURL=pet-generation.js.map