...
exports.GlobalPetActivityCoordinator = void 0;
exports.normalizeCoordinatedPetState = normalizeCoordinatedPetState;
exports.inferPetActorKind = inferPetActorKind;
exports.runPetActivityCoordinatorSelfTest = runPetActivityCoordinatorSelfTest;
...
function normalizeCoordinatedPetState(state) {
    const value = String(state || "idle");
    return ALLOWED_STATES.has(value) ? value : "idle";
}
function inferPetActorKind(update) {
    ...
    if (actor === "global-agent")
        return "global";
    if (/test[-_ ]?agent/.test(actor) || /test[-_ ]?agent/.test(source))
        return "test-agent";
...
function friendlyActorName(update, kind) {
...
function coordinatedDetail(update, displayName, kind) {
    const detail = String(update.detail || "").trim();
    if (kind === "global")
        return detail || "全局 Agent 正在处理";
    return detail ? `${displayName}：${detail}` : `${displayName} 正在工作`;
}
class GlobalPetActivityCoordinator {
    activities = new Map();
    update(update) {
        const actor = String(update.actor || "").trim();
        if (!actor)
            return this.resolve(update.timestamp);
    ...
    resolve(at = Date.now()) {
        ...
        return [...this.activities.values()].sort((left, right) => right.priority - left.priority || right.timestamp - left.timestamp)[0] || null;
    }
...
exports.GlobalPetActivityCoordinator = GlobalPetActivityCoordinator;
function runPetActivityCoordinatorSelfTest() {
...
//# sourceMappingURL=pet-activity-coordinator.js.map