...
function buildUserConflictWarnings(summary) {
    ...
    return plans.flatMap((item) => {
        ...
        if (!conflicts.length && item.detail)
            return [{ id: item.id || "conflict", title: "已启用冲突保护", detail: compactMemoryText(item.detail, 160), agents: [] }];
...
function splitUserAcceptanceText(value) {
    if (Array.isArray(value))
        return value.map((item) => compactMemoryText(item, 160)).filter(Boolean);
    return String(value || "")
        .split(/(?:\n|；|;|。|\d+[、.]\s*)+/)
...
function buildUserAgentCoordinationProtocol(task, summary = {}, executions = [], workOrderPreview = null, acceptanceReview = null) {
    ...
    ]).slice(0, 10).map((item, index) => ({ id: `heartbeat_${index + 1}`, text: compactMemoryText(item, 180) }));
...
function collectProjectAnalysisFiles(root, maxEntries = 1200) {
    const files = [];
    let visited = 0;
    const walk = (dir, depth = 0) => {
        if (visited >= maxEntries || depth > 5)
            return;
        let entries = [];
        try {
...
function buildGroupProjectAnalysisContext(group, message, ctx, configs = (0, db_1.getConfigs)()) {
    ...
    for (const member of members.slice(0, 8)) {
        ...
        lines.push(compactMemoryText((0, project_memory_1.buildProjectMemoryPacket)(member.project, { workDir, resources, query: message }), 2200));
        if (workDir)
            lines.push(compactPreserveLines(buildProjectCodeReadOnlySnapshot(member.project, workDir, message), 18_000));
    }
    try {
...
function buildDeliverySummary(task, execution, finalStatus) {
    ...
    const ackReviewForGate = (0, collaboration_protocol_gates_1.buildAckPreflightReview)(task, receiptEvidence, assignmentEvidence.map((item) => ({ project: item.project, objective: item.task })));
    ...
    const contractSyncForGate = (0, collaboration_protocol_gates_1.extractContractSyncHints)(task, {
...
async function processCrossAgents(groupId, group, sourceProject, output, atMentions, configs, ctx, streamRes = null, depth = 0, seenMentions = new Set(), executionOrder = "parallel", planMessageId = "", taskId = "") {
    ...
    const executeMentionJob = async (mention) => {
        ...
        const projectExecutionBrief = (0, project_memory_1.buildProjectExecutionBrief)(targetName, childTaskText, {
            workDir: tWorkDir,
...
function buildTaskGapContinuationDraft(task) {
    ...
    const ackRewriteRows = (0, collaboration_protocol_gates_1.getTaskAckRewriteRows)(task);
...
function getTaskGapItems(task) {
    ...
    for (const row of (0, collaboration_protocol_gates_1.getTaskAckRewriteRows)(task)) {
...
function handleCollaborationApi(pathname, req, res, parsed, ctx) {
    ...
    if (pathname === "/api/tasks/runtime-debt/cleanup" && req.method === "POST") {
        ...
        req.on("end", () => {
            try {
                const payload = body ? JSON.parse(body) : {};
                (0, utils_1.sendJson)(res, cleanupRuntimeDebt(payload));
            }
            catch (e) {
                (0, utils_1.sendJson)(res, { error: e.message }, 400);
            }
    ...
    if (pathname === "/api/tasks/queue/resume" && req.method === "POST") {
        try {
            (0, utils_1.sendJson)(res, { success: true, ...resumeTaskQueues(ctx, { force: true }) });
        }
        catch (e) {
            (0, utils_1.sendJson)(res, { error: e.message }, 500);
        }
        ...
        req.on("end", () => {
            try {
                ...
                if (payload.clear_memory === true || payload.clearMemory === true) {
                    try {
                        fs.unlinkSync(getGroupMemoryFile(groupId));
                    }
                    catch { }
                }
