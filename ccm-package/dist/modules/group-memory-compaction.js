...
function calculateGroupMessagesToKeepIndex(messages, options = {}) {
    ...
    const minMessages = Math.max(4, Number(options.minMessages || exports.GROUP_COMPACT_MIN_KEEP_MESSAGES));
    const minTokens = Math.max(500, Number(options.minTokens || exports.GROUP_COMPACT_MIN_KEEP_TOKENS));
...
async function compactGroupConversationMemory(input) {
    ...
    if (keepIndex <= summarizedThroughIndex + 1)
        return { compacted: false, memory, keepIndex };
    ...
    const shouldCompact = !!input.force || activeTokens >= triggerTokens || activeMessageCount >= exports.GROUP_COMPACT_MAX_ACTIVE_MESSAGES;
    if (!shouldCompact || !messagesToCompact.length)
        return { compacted: false, memory, keepIndex };
