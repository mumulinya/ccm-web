...
function evaluateSample(sample, baseline) {
    ...
    if (sample.ledger.leases.stale > Number(baseline?.ledger?.leases?.stale || 0))
        alerts.push(makeAlert("new_stale_task_lease", "warning", `新增失效任务租约：${sample.ledger.leases.stale - Number(baseline?.ledger?.leases?.stale || 0)}`));
    if (sample.ledger.operations.stale_in_progress > Number(baseline?.ledger?.operations?.stale_in_progress || 0))
        alerts.push(makeAlert("new_stale_idempotency", "warning", `新增失效幂等操作：${sample.ledger.operations.stale_in_progress - Number(baseline?.ledger?.operations?.stale_in_progress || 0)}`));
    ...
    if (sample.tasks.stuck_without_lease > Number(baseline?.tasks?.stuck_without_lease || 0))
        alerts.push(makeAlert("new_stuck_tasks", "critical", `新增无有效租约的长时间执行任务：${sample.tasks.stuck_without_lease - Number(baseline?.tasks?.stuck_without_lease || 0)}`));
    if (sample.event_loop_lag_ms > 500)
        alerts.push(makeAlert("event_loop_lag", "warning", `事件循环延迟 ${sample.event_loop_lag_ms}ms`));
...
async function collectSoakSample() {
    const tasks = (0, db_1.loadTasks)();
    const leases = (0, reliability_ledger_1.listTaskLeases)();
    ...
    const stuckWithoutLease = tasks.filter((task) => task.status === "in_progress" && Date.now() - Date.parse(task.updated_at || task.started_at || task.created_at || 0) > 30 * 60 * 1000 && !activeLeaseTasks.has(task.id)).length;
    ...
    return {
        ...
        ledger: (0, reliability_ledger_1.getReliabilityLedgerStats)(),
        ...
        drill: (0, reliability_drills_1.getReliabilityDrillStatus)(),
    };
}
...
function reportMarkdown(state, summary) {
    ...
    return [
        ...
        "## 告警",
        "",
        ...(summary.alerts.items.length ? summary.alerts.items.map((item) => `- [${item.severity}] ${item.code}：${item.message}（${item.first_at}，出现 ${item.count} 次）`) : ["- 无"]),
        "",
    ].join("\n");
}
...
async function takeSampleAndPersist() {
    ...
    try {
        ...
        for (const alert of alerts) {
            const existing = existingAlerts.find((item) => item.code === alert.code && item.severity === alert.severity);
            if (existing) {
