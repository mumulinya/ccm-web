...
function buildRuntimeRecoveryCandidates(primary, configured = [], availability = isRuntimeCommandAvailable) {
    const normalizedPrimary = (0, agent_runtime_1.normalizeAgentRuntimeId)(primary);
    const explicit = (Array.isArray(configured) ? configured : String(configured || "").split(/[;,\s]+/))
        .map(item => (0, agent_runtime_1.normalizeAgentRuntimeId)(String(item || "")))
...
function getRepoKey(workDir) {
    ...
    const result = (0, child_process_1.spawnSync)("git", ["rev-parse", "--show-toplevel"], { cwd: resolved, encoding: "utf-8", windowsHide: true });
...
function runCollaborationResilienceIntegrationSelfTest() {
    ...
    try {
        ...
        const first = (0, agent_worktree_1.prepareChildAgentWorkDir)(repo, { mode: "worktree", failClosed: true, taskId: "resilience-test", agentName: key, sourceProject: "coordinator", reuseKey: key });
        ...
        const second = (0, agent_worktree_1.prepareChildAgentWorkDir)(repo, { mode: "worktree", failClosed: true, taskId: "resilience-test", agentName: key, sourceProject: "coordinator", reuseKey: key });
        const checks = {
