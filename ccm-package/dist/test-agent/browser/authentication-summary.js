...
exports.buildBrowserAuthenticationSummary = buildBrowserAuthenticationSummary;
exports.formatBrowserAuthenticationEvidence = formatBrowserAuthenticationEvidence;
exports.formatBrowserAuthenticationSummaryLine = formatBrowserAuthenticationSummaryLine;
function resultAuthenticationEvidence(result) {
    return [
...
function buildBrowserAuthenticationSummary(results) {
    ...
    const credentialEnvNames = Array.from(new Set(evidence.flatMap(item => item.credentialEnvNames || []))).sort();
    const storageStates = new Set(evidence
        .map(item => item.storageState?.sha256 || "")
        .filter(Boolean));
    const managedChecks = configured.filter(result => resultAuthenticationEvidence(result).some(item => item.mode !== "existing_session"));
    const existingSessionChecks = configured.filter(result => resultAuthenticationEvidence(result).some(item => item.mode === "existing_session"));
    ...
    return {
        ...
        authenticatedSessions: configured.reduce((sum, result) => sum + (result.browserSessions || []).filter(session => session.authentication).length, 0),
        credentialEnvNames,
        storageStateCount: storageStates.size,
        sensitiveArtifactSuppressionCount: configured.filter(result => result.authentication?.sensitiveArtifactsSuppressed
            || (result.browserSessions || []).some(session => session.authentication?.sensitiveArtifactsSuppressed)).length,
        existingSessionProviders: Array.from(new Set(existingSessionEvidence.map(item => item.provider))).sort(),
        minimalEvidenceChecks: existingSessionChecks.filter(result => resultAuthenticationEvidence(result).some(item => item.existingSession?.evidencePolicy === "minimal")).length,
        fullEvidenceChecks: existingSessionChecks.filter(result => resultAuthenticationEvidence(result).some(item => item.existingSession?.evidencePolicy === "full")).length,
        tabContextCheckedChecks: existingSessionChecks.filter(result => resultAuthenticationEvidence(result).some(item => item.existingSession?.tabContextChecked)).length,
        newTabChecks: existingSessionChecks.filter(result => resultAuthenticationEvidence(result).some(item => item.existingSession?.createdNewTab)).length,
    };
}
function formatBrowserAuthenticationEvidence(evidence) {
    if (!evidence)
        return "none";
...
function formatBrowserAuthenticationSummaryLine(summary) {
    return [
...
//# sourceMappingURL=authentication-summary.js.map