...
exports.writePlaywrightAccessibilitySnapshotArtifact = writePlaywrightAccessibilitySnapshotArtifact;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const utils_1 = require("../utils");
async function readNativeAriaSnapshot(page) {
...
async function readFallbackAccessibilityOutline(page) {
    if (!page?.evaluate)
        return null;
...
async function writePlaywrightAccessibilitySnapshotArtifact(page, artifactDir, projectName, checkName, index) {
    ...
    let snapshot = null;
    try {
        snapshot = await readNativeAriaSnapshot(page);
    }
    catch { }
    if (!snapshot) {
        try {
            snapshot = await readFallbackAccessibilityOutline(page);
        }
        catch { }
    }
    if (!snapshot?.text)
        return [];
    const snapshotDir = (0, utils_1.ensureDir)(path.join(artifactDir, "accessibility-snapshots"));
...
//# sourceMappingURL=accessibility-snapshot-artifacts.js.map