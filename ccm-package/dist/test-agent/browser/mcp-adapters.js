"use strict";
// mcp-adapters.ts — merged from 3 part files (behavior-freeze merge).
...
// ===== merged from mcp-adapters-part-02.ts =====
class PlaywrightMcpAdapter {
    ...
    async readConsoleMessages() {
        ...
        return text && !emptyLike(text) ? (0, console_assertions_1.normalizeBrowserConsoleLines)((0, utils_1.compactText)(text, 4000).split(/\r?\n/)) : [];
    }
    ...
    async readNetworkRequests() {
        ...
        if (!tool)
            return [];
        const text = extractToolText(await this.call(tool, {}));
        return text && !emptyLike(text) ? (0, utils_1.compactText)(text, 4000).split(/\r?\n/).filter(Boolean) : [];
    }
...
// ===== merged from mcp-adapters-part-03.ts =====
class ComputerUseAdapter {
    ...
    async runAction(project, action, defaultTimeout) {
        try {
            ...
            if (action.type === "goto") {
                ...
                return step("action", "computer-use:goto", "blocked", url, "Computer Use cannot prove navigation without a live URL observation. Use Playwright for URL-gated flows.");
            }
            if (action.type === "reload") {
                const keyTool = pick(this.tools, [/__key$/]);
                if (!keyTool)
    ...
    async runAssertion(assertion) {
        if (assertion.type === "urlEquals" || assertion.type === "urlIncludes" || assertion.type === "urlNotIncludes") {
            return assertWithLiveUrl("computer-use", assertion);
        }
        if (assertion.type === "consoleNoErrors"
...
function createMcpBrowserAdapter(tools, call, options = {}) {
    ...
    if (has(browserTools, /mcp__playwright__browser_/))
        return new PlaywrightMcpAdapter(browserTools, call);
    if (has(browserTools, /mcp__claude-in-chrome__/))
        return new ClaudeChromeAdapter(browserTools, call);
    if (has(browserTools, /mcp__(chrome-devtools|chromedevtools|chrome)__(new_page|navigate_page|take_snapshot|click|fill|evaluate_script)/))
        return new ChromeDevtoolsAdapter(browserTools, call);
    if (has(browserTools, /mcp__computer-use__/))
        return new ComputerUseAdapter(browserTools, call);
    return null;
}
//# sourceMappingURL=mcp-adapters.js.map