...
exports.BROWSER_ACTION_EFFECT_SIGNALS = void 0;
exports.browserActionEffectRequired = browserActionEffectRequired;
exports.browserActionEffectSession = browserActionEffectSession;
exports.browserActionEffectTimeout = browserActionEffectTimeout;
exports.buildBrowserActionEffectSnapshot = buildBrowserActionEffectSnapshot;
exports.browserActionEffectEvidenceErrors = browserActionEffectEvidenceErrors;
exports.browserActionEffectResultErrors = browserActionEffectResultErrors;
exports.verifyBrowserActionEffect = verifyBrowserActionEffect;
exports.suppressBrowserActionEffectDetails = suppressBrowserActionEffectDetails;
const crypto = __importStar(require("crypto"));
exports.BROWSER_ACTION_EFFECT_SIGNALS = [
    "url",
    "title",
    "page_text",
    "dom",
...
function normalizedSignals(action) {
    const configured = action.effectSignals || action.effect_signals || [];
    const source = configured.length ? configured : exports.BROWSER_ACTION_EFFECT_SIGNALS;
    return exports.BROWSER_ACTION_EFFECT_SIGNALS.filter(signal => source.includes(signal));
}
function browserActionEffectRequired(action) {
    return action.verifyEffect === true
        || action.verify_effect === true
        || action.expectEffect === true
        || action.expect_effect === true;
}
function browserActionEffectSession(action) {
    return String(action.effectSession || action.effect_session || "").trim();
}
function browserActionEffectTimeout(action, fallback) {
    const configured = Number(action.effectTimeoutMs
...
function buildBrowserActionEffectSnapshot(observation, requestedSignals) {
    const snapshot = {};
    if (requestedSignals.includes("url") && observation.url !== undefined)
        snapshot.urlSha256 = sha256(String(observation.url));
    if (requestedSignals.includes("title") && observation.title !== undefined)
        snapshot.titleSha256 = sha256(String(observation.title));
    if (requestedSignals.includes("page_text") && observation.pageText !== undefined)
        snapshot.pageTextSha256 = sha256(String(observation.pageText));
    if (requestedSignals.includes("dom") && observation.dom !== undefined)
        snapshot.domSha256 = sha256(String(observation.dom));
    if (requestedSignals.includes("network") && observation.networkCount !== undefined)
        snapshot.networkCount = observation.networkCount;
    if (requestedSignals.includes("dialog") && observation.dialogCount !== undefined)
        snapshot.dialogCount = observation.dialogCount;
    if (requestedSignals.includes("popup") && observation.popupCount !== undefined)
        snapshot.popupCount = observation.popupCount;
    if (requestedSignals.includes("download") && observation.downloadCount !== undefined)
        snapshot.downloadCount = observation.downloadCount;
    return snapshot;
}
function observedSignals(snapshot) {
    const signals = [];
    if (snapshot.urlSha256)
        signals.push("url");
    if (snapshot.titleSha256)
        signals.push("title");
    if (snapshot.pageTextSha256)
        signals.push("page_text");
    if (snapshot.domSha256)
        signals.push("dom");
    if (snapshot.networkCount !== undefined)
        signals.push("network");
    if (snapshot.dialogCount !== undefined)
        signals.push("dialog");
    if (snapshot.popupCount !== undefined)
        signals.push("popup");
    if (snapshot.downloadCount !== undefined)
        signals.push("download");
    return signals;
}
function changedSignals(before, after) {
    const changed = [];
    if (before.urlSha256 && after.urlSha256 && before.urlSha256 !== after.urlSha256)
        changed.push("url");
    if (before.titleSha256 && after.titleSha256 && before.titleSha256 !== after.titleSha256)
        changed.push("title");
    if (before.pageTextSha256 && after.pageTextSha256 && before.pageTextSha256 !== after.pageTextSha256)
        changed.push("page_text");
    if (before.domSha256 && after.domSha256 && before.domSha256 !== after.domSha256)
        changed.push("dom");
    if (before.networkCount !== undefined && after.networkCount !== undefined && before.networkCount !== after.networkCount)
        changed.push("network");
    if (before.dialogCount !== undefined && after.dialogCount !== undefined && before.dialogCount !== after.dialogCount)
        changed.push("dialog");
    if (before.popupCount !== undefined && after.popupCount !== undefined && before.popupCount !== after.popupCount)
        changed.push("popup");
    if (before.downloadCount !== undefined && after.downloadCount !== undefined && before.downloadCount !== after.downloadCount)
        changed.push("download");
    return changed;
}
function signalList(value) {
    return Array.isArray(value)
        ? value.filter(signal => exports.BROWSER_ACTION_EFFECT_SIGNALS.includes(signal))
        : [];
}
function sameSignalList(left, right) {
    return left.length === right.length && left.every((value, index) => value === right[index]);
}
function snapshotErrors(snapshot, label) {
    const errors = [];
    ...
    for (const key of Object.keys(snapshot || {})) {
        if (!allowed.has(key))
            errors.push(`${label}.${key} is not permitted; action-effect snapshots may contain only digests and counts.`);
    }
    for (const key of ["urlSha256", "titleSha256", "pageTextSha256", "domSha256"]) {
        const value = snapshot?.[key];
        if (value !== undefined && !/^[a-f0-9]{64}$/i.test(String(value))) {
            errors.push(`${label}.${key} must be a SHA-256 digest.`);
        }
    }
    for (const key of ["networkCount", "dialogCount", "popupCount", "downloadCount"]) {
        const value = snapshot?.[key];
...
function browserActionEffectEvidenceErrors(evidence, label = "browser action effect") {
    const errors = [];
    ...
    if (!String(evidence.actionType || "").trim())
        errors.push(`${label}.actionType is required.`);
    ...
    for (const [key, raw, normalized] of [
        ...
        const canonical = exports.BROWSER_ACTION_EFFECT_SIGNALS.filter(signal => normalized.includes(signal));
        if (!sameSignalList(normalized, canonical))
            errors.push(`${label}.${key} must use canonical signal order.`);
    }
    if (!requested.length)
        errors.push(`${label}.requestedSignals must not be empty.`);
    ...
    if (evidence.detailSuppressed) {
        ...
        const expectedObserved = exports.BROWSER_ACTION_EFFECT_SIGNALS.filter(signal => observedSignals(before).includes(signal) || observedSignals(after).includes(signal));
...
function browserActionEffectResultErrors(result, label = "browser result") {
    const effects = Array.isArray(result?.actionEffects) ? result.actionEffects : [];
    const errors = effects.flatMap((effect, index) => browserActionEffectEvidenceErrors(effect, `${label}.actionEffects[${index}]`));
    ...
    const effectSteps = (Array.isArray(result?.steps) ? result.steps : [])
        .filter((step) => step.name === "assert:actionEffect" || step.name.endsWith(":assert:actionEffect"));
    const actionSteps = (Array.isArray(result?.steps) ? result.steps : [])
        .filter((step) => step.kind === "action");
    const sessionNames = new Set((Array.isArray(result?.browserSessions) ? result.browserSessions : [])
...
async function verifyBrowserActionEffect(input) {
...
function suppressBrowserActionEffectDetails(evidence) {
