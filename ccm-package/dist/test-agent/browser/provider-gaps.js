...
const MCP_REQUIRES_PLAYWRIGHT_ACTIONS = new Set([
    "uploadFile",
    "dragTo",
    "doubleClick",
    "rightClick",
...
const MCP_REQUIRES_PLAYWRIGHT_ASSERTIONS = new Set([
    "focused",
    "notFocused",
    "enabled",
    "disabled",
...
function clean(value, max = 320) {
    return (0, utils_1.compactText)(String(value || "").replace(/\s+/g, " ").trim(), max);
}
function categoryFor(kind, text) {
    if (MISSING_TOOL_PATTERN.test(text))
        return "missing_tool";
    if (/availability|provider unavailable|No browser provider was available/i.test(text))
...
function isProviderGap(text) {
    return !!text && (MISSING_TOOL_PATTERN.test(text) || GAP_PATTERNS.some(pattern => pattern.test(text)));
}
function recommendationFor(text, provider) {
    if (/playwright/i.test(text))
        return "Rerun this browser check with the Playwright provider for deterministic DOM/browser-state control.";
    ...
    if (/missing/i.test(text))
        return `Enable the missing browser tool for ${provider || "the selected provider"} or switch to Playwright.`;
...
function gapFromResult(result) {
    const text = clean(result.error || "");
    if (!isProviderGap(text) && !/availability|provider unavailable|No browser provider was available/i.test(text))
        return null;
    const provider = result.provider || "none";
    return {
        provider,
...
function buildBrowserProviderGaps(results) {
    const gaps = [];
...
function formatBrowserProviderGapLine(item) {
...
function hasBrowserContextOptions(check) {
    return Boolean(check.viewport
        || check.viewportWidth
...
function actionPlanWarning(provider, project, check, action) {
...
function assertionPlanWarning(provider, project, check, assertion) {
    if (!MCP_REQUIRES_PLAYWRIGHT_ASSERTIONS.has(assertion.type))
        return null;
    return planWarning({
        provider,
        project,
...
function buildBrowserProviderPlanWarnings(workOrder) {
    ...
    const warnings = [];
    const browserChecks = workOrder.projects.flatMap(project => (0, shared_1.checksForProject)(project, workOrder.acceptanceCriteria).map(check => ({ project, check })));
    if (provider === "none") {
...
function formatBrowserProviderPlanWarningLine(item) {
    const location = [item.project, item.check, item.item].filter(Boolean).join(" / ");
    return `${item.provider} ${item.category}${location ? ` ${location}` : ""}: ${item.reason} Recommendation: ${item.recommendation}`;
}
//# sourceMappingURL=provider-gaps.js.map