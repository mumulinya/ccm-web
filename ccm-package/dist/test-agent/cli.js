...
function parseWorkOrderJson(file, readFile, label = "work order") {
    ...
    try {
        return { input: JSON.parse(text), error: "" };
    }
    catch (error) {
        return {
            input: null,
...
async function runTestAgentCli(args = process.argv.slice(2), io = {}) {
    ...
    if (!workOrderJson.input) {
        stderr.write(`${workOrderJson.error}\n`);
        return { exitCode: 2 };
    }
    const overrides = (0, cli_options_1.cliOverrides)(options);
    const workOrderInput = options.handoffPath
        ? (0, work_order_builder_1.buildTestAgentWorkOrderFromHandoff)(workOrderJson.input).workOrder
        : workOrderJson.input;
    const validation = (0, contract_1.validateTestAgentWorkOrderContract)(workOrderInput, overrides);
