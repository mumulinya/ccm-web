...
exports.adversarialContextCriteria = adversarialContextCriteria;
exports.buildAdversarialEvidenceRelevance = buildAdversarialEvidenceRelevance;
...
function relevanceScore(subject, descriptor) {
    const normalizedSubject = normalize(subject);
    const normalizedDescriptor = normalize(descriptor);
    if (!normalizedSubject || !normalizedDescriptor)
        return 0;
    if (normalizedSubject.length >= 6 && normalizedDescriptor.includes(normalizedSubject))
        return 1;
    ...
    if (subjectTokens.length <= 2 && hits.length === subjectTokens.length)
        return 1;
    if (hits.length < 2)
        return 0;
...
function adversarialContextCriteria(context) {
    if (!context)
        return [];
...
function buildAdversarialEvidenceRelevance(input) {
...
//# sourceMappingURL=adversarial-relevance.js.map