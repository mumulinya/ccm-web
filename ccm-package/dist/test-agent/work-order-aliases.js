...
exports.HTTP_CONCURRENCY_ASSERTION_ALIASES = exports.HTTP_CONCURRENCY_ASSERTION_TYPES = exports.HTTP_ASSERTION_ALIASES = exports.HTTP_ASSERTION_TYPES = exports.BROWSER_ASSERTION_ALIASES = exports.BROWSER_ASSERTION_TYPES = exports.BROWSER_ACTION_ALIASES = exports.BROWSER_ACTION_TYPES = exports.DEFAULT_OPTIONS = void 0;
exports.text = text;
exports.normalizedType = normalizedType;
exports.optionalNumber = optionalNumber;
exports.optionalBoolean = optionalBoolean;
exports.optionalNumberList = optionalNumberList;
exports.coordinate = coordinate;
exports.normalizeBrowserApps = normalizeBrowserApps;
exports.normalizeBrowserUploadFiles = normalizeBrowserUploadFiles;
exports.normalizeStringList = normalizeStringList;
exports.browserScrollDirection = browserScrollDirection;
exports.normalizeBrowserStorageArea = normalizeBrowserStorageArea;
exports.normalizeBrowserGeolocation = normalizeBrowserGeolocation;
exports.normalizeBrowserActionKey = normalizeBrowserActionKey;
...
exports.BROWSER_ACTION_TYPES = new Set([
    "goto",
    "click",
    "doubleClick",
    "rightClick",
...
exports.BROWSER_ACTION_ALIASES = {
    navigate: "goto",
    navigation: "goto",
    open: "goto",
...
exports.BROWSER_ASSERTION_TYPES = new Set([
    "visible",
    "notVisible",
    "present",
    "notPresent",
...
exports.BROWSER_ASSERTION_ALIASES = {
    hidden: "notVisible",
...
exports.HTTP_ASSERTION_TYPES = new Set([
    "status",
    "contentTypeIncludes",
    "textIncludes",
    "textNotIncludes",
...
exports.HTTP_ASSERTION_ALIASES = {
    status_code: "status",
...
exports.HTTP_CONCURRENCY_ASSERTION_TYPES = new Set([
    "responseCount",
    "statusCount",
    "jsonPathUniqueCount",
    "jsonPathAllEqual",
]);
exports.HTTP_CONCURRENCY_ASSERTION_ALIASES = {
...
function normalizeBrowserUploadFiles(raw) {
    const fileItems = (0, utils_1.asArray)(raw.files || raw.fileList || raw.file_list || raw.uploads).map(file => {
...
function normalizeStringList(value) {
    return (0, utils_1.asArray)(value).map(item => text(item)).filter(Boolean);
}
function browserScrollDirection(raw) {
    ...
    if (actionName.includes("up"))
        return "up";
    if (actionName.includes("left"))
        return "left";
    if (actionName.includes("right"))
        return "right";
    if (actionName.includes("down"))
        return "down";
    return raw?.direction;
}
function normalizeBrowserStorageArea(raw, type) {
    if (type === "setLocalStorage")
        return "localStorage";
    if (type === "setSessionStorage")
        return "sessionStorage";
    const explicit = text(raw.storage || raw.storageArea || raw.storage_area || raw.scope || raw.area).toLowerCase();
    if (explicit === "local" || explicit === "localstorage" || explicit === "local_storage")
        return "localStorage";
    if (explicit === "session" || explicit === "sessionstorage" || explicit === "session_storage")
        return "sessionStorage";
    if (explicit === "both" || explicit === "all")
        return "both";
    const actionName = text(raw?.type || raw?.action || raw?.kind).toLowerCase().replace(/[\s-]+/g, "_");
    if (actionName.includes("session_storage"))
        return "sessionStorage";
    if (actionName.includes("local_storage"))
        return "localStorage";
    return explicit || undefined;
}
function normalizeBrowserGeolocation(value) {
    if (!value || typeof value !== "object")
        return undefined;
...
function normalizeBrowserActionKey(raw, type) {
...
function normalizeSameSite(value) {
    ...
    if (raw === "lax")
        return "Lax";
    if (raw === "none" || raw === "no_restriction" || raw === "no-restriction")
        return "None";
    return text(value) || undefined;
}
//# sourceMappingURL=work-order-aliases.js.map