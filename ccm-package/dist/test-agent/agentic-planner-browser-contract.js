...
exports.browserCheckContractPrompt = browserCheckContractPrompt;
exports.validatePlannedBrowserChecks = validatePlannedBrowserChecks;
...
const ACTION_TYPE_LIST = Array.from(work_order_aliases_1.BROWSER_ACTION_TYPES).sort();
const ASSERTION_TYPE_LIST = Array.from(work_order_aliases_1.BROWSER_ASSERTION_TYPES).sort();
function browserCheckContractPrompt() {
    return [
...
function canonicalActionType(value) {
    ...
    if (!raw)
        return "";
    if (work_order_aliases_1.BROWSER_ACTION_TYPES.has(raw))
        return raw;
    const alias = work_order_aliases_1.BROWSER_ACTION_ALIASES[raw.toLowerCase()];
    return alias && work_order_aliases_1.BROWSER_ACTION_TYPES.has(alias) ? alias : "";
}
function canonicalAssertionType(value) {
    const raw = String(value || "").trim();
    if (!raw)
        return "";
    if (work_order_aliases_1.BROWSER_ASSERTION_TYPES.has(raw))
        return raw;
    const alias = work_order_aliases_1.BROWSER_ASSERTION_ALIASES[raw.toLowerCase()];
    return alias && work_order_aliases_1.BROWSER_ASSERTION_TYPES.has(alias) ? alias : "";
}
 ...
 */
function validatePlannedBrowserChecks(raw, projectName, acceptanceCriteria = []) {
    const issues = [];
    const checks = [];
    let droppedChecks = 0;
    ...
    asArray(raw).forEach((candidate, index) => {
        ...
        for (const assertion of asArray(candidate.assertions).concat(asArray(candidate.expectations))) {
            ...
            assertions.push({ ...assertion, type });
        }
...
//# sourceMappingURL=agentic-planner-browser-contract.js.map