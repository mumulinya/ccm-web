...
exports.discoverTestAgentSelfTests = discoverTestAgentSelfTests;
exports.runTestAgentSelfTestMatrix = runTestAgentSelfTestMatrix;
exports.formatTestAgentSelfTestMatrixSummary = formatTestAgentSelfTestMatrixSummary;
...
function discoverTestAgentSelfTests(selfTestModulePath = defaultSelfTestModulePath(), pattern) {
...
async function runOneSelfTest(selfTestModulePath, name, options) {
    const started = Date.now();
    const child = (0, child_process_1.spawn)(process.execPath, ["-e", childScript(), path.resolve(selfTestModulePath), name, String(options.timeoutMs)], {
        cwd: options.cwd,
...
async function runTestAgentSelfTestMatrix(options = {}) {
    const started = Date.now();
    ...
    for (const name of names) {
        ...
        results.push(result);
        if (!result.pass && options.stopOnFailure)
            break;
    }
...
function formatTestAgentSelfTestMatrixSummary(report) {
    const lines = [
...
//# sourceMappingURL=self-test-matrix.js.map