...
export function registerTools(server, client) {
    // Tool 1: 列出群聊
    server.tool("list_chats", "列出机器人所在的飞书群聊。返回群聊名称、chat_id、成员数量等信息。", {
        page_size: z
            .number()
            .optional()
    ...
    // Tool 2: 获取群聊历史消息
    server.tool("get_chat_history", "获取指定飞书群聊的历史消息。需要提供chat_id（通过list_chats获取）。返回消息列表，包含发送者、时间、内容。注意：飞书的start_time和end_time是Unix时间戳（秒级字符串），日期转换示例：2025-05-28 00:00:00 UTC+8 → '1748361600'", {
        chat_id: z.string().describe("群聊ID，通过 list_chats 获取"),
        start_time: z
            .string()
            .optional()
            .describe("起始时间，Unix时间戳（秒级字符串），如 '1748361600' 表示 2025-05-28"),
        end_time: z
            .string()
            .optional()
    ...
    // Tool 3: 搜索消息（本地过滤）
    server.tool("search_messages", "在飞书群聊中搜索包含关键词的消息。此工具通过拉取最近消息并在本地过滤实现，搜索范围限制在最近50条消息内。建议先用list_chats获取chat_id，再指定chat_id搜索以提高效率。", {
    ...
    // Tool 4: 获取消息详情
    server.tool("get_message_detail", "获取飞书消息的完整详情，包括完整的富文本内容、附件信息等。message_id格式如 om_xxxxx。", {
