import { z } from "zod";
import { parseMessageContent, formatTimestamp, } from "./feishu-client.js";
export function registerTools(server, client) {
    // Tool 1: list chats
    server.tool("list_chats", "List Feishu chats where the bot is a member. Return chat names, chat IDs, member counts, and related metadata.", {
        page_size: z
            .number()
            .optional()
        ...
        const data = await client.listChats(params.page_size || 20, params.page_token);
        const items = data.items || [];
        const lines = items.map((chat, i) => `${i + 1}. ${chat.name || "未命名群聊"} (chat_id: ${chat.chat_id}, 成员数: ${chat.user_count || "?"}, 描述: ${chat.description || "无"})`);
        const result = [
    ...
    server.tool("get_chat_history", "Read historical messages from a specified Feishu chat. The chat_id comes from list_chats. Results include sender, time, and content. Feishu start_time and end_time use Unix timestamps in seconds as strings; for example, 2025-05-28 00:00:00 UTC+8 is '1748361600'.", {
        chat_id: z.string().describe("Feishu chat ID returned by list_chats."),
        start_time: z
            .string()
            .optional()
            .describe("Start time as a Unix timestamp in seconds, represented as a string; for example '1748361600'."),
        end_time: z
            .string()
            .optional()
        ...
        const [history, chatInfo] = await Promise.all([
            client.getChatHistory(params.chat_id, params.start_time, params.end_time, params.page_size || 20, params.page_token),
    ...
    server.tool("search_messages", "Search Feishu chat messages containing a query. The tool fetches recent messages and filters locally; the search is limited to the latest 50 messages. Prefer list_chats first and pass chat_id to keep the search bounded.", {
        ...
        for (const chatId of chatIds) {
            try {
                const history = await client.getChatHistory(chatId, params.start_time, params.end_time, 50);
    ...
    server.tool("get_message_detail", "Read complete Feishu message details, including rich text content and attachments. message_id has a format such as om_xxxxx.", {
