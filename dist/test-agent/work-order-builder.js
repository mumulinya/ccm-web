...
function buildProject(project, index, warnings, globalCompletedTasks, completedTasksContextOnly = false) {
    ...
    return {
        ...
        httpChecks: completedTasksContextOnly
            ? (0, utils_1.asArray)(project.httpChecks || project.http_checks || project.apiChecks || project.api_checks)
            : normalizeCompletedTaskCoverage((0, utils_1.asArray)(project.httpChecks || project.http_checks || project.apiChecks || project.api_checks), coverageTaskAliases),
        adversarialHttpChecks: completedTasksContextOnly
            ? (0, utils_1.asArray)(project.adversarialHttpChecks || project.adversarial_http_checks || project.adversarialApiChecks || project.adversarial_api_checks)
            : normalizeCompletedTaskCoverage((0, utils_1.asArray)(project.adversarialHttpChecks || project.adversarial_http_checks || project.adversarialApiChecks || project.adversarial_api_checks), coverageTaskAliases),
        browserChecks: completedTasksContextOnly
            ? (0, utils_1.asArray)(project.browserChecks || project.browser_checks)
            : normalizeCompletedTaskCoverage((0, utils_1.asArray)(project.browserChecks || project.browser_checks), coverageTaskAliases),
        adversarialBrowserChecks: completedTasksContextOnly
            ? (0, utils_1.asArray)(project.adversarialBrowserChecks || project.adversarial_browser_checks)
            : normalizeCompletedTaskCoverage((0, utils_1.asArray)(project.adversarialBrowserChecks || project.adversarial_browser_checks), coverageTaskAliases),
...
function buildTestAgentWorkOrderFromHandoff(input) {
    const warnings = [];
    // Official CCM orchestration already has signed lifecycle/Terminal Gate
    // evidence for work-item completion. `completedTasks` is therefore a claim
    // to inform TestAgent, not a product criterion that a command or browser
    // probe could independently prove. Legacy/external handoffs keep the old
    // conversion unless they opt into this explicit policy.
    const completedTasksContextOnly = input.metadata?.completedTasksContextOnly === true;
    ...
    const projects = rawProjects.map((project, index) => buildProject(project, index, warnings, globalCompletedTasks, completedTasksContextOnly));
    ...
    const acceptanceCriteria = uniqueStrings([
        ...
        ...(completedTasksContextOnly ? [] : completedTaskCriteria([...globalCompletedTasks, ...projectCompletedTasks])),
    ]);
