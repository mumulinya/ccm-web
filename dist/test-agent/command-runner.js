...
function runSingleCommand(project, command, timeoutMs, maxOutputChars, policyContext = null, liveIdentity = null) {
    ...
    return new Promise(resolve => {
        ...
        const liveProgress = liveIdentity?.exactSessionId ? (0, command_live_progress_1.createCommandLiveProgress)({
            commandRunId: liveIdentity.commandRunId || `test-${crypto.randomUUID()}`,
...
async function runVerificationCommands(workOrder) {
    ...
    const liveIdentity = (0, user_visible_progress_1.testAgentVisibleProgressContext)(workOrder);
    for (const project of workOrder.projects) {
        for (const [index, command] of project.verificationCommands.entries()) {
            const trace = (0, user_visible_progress_1.beginTestAgentVisibleTool)(liveIdentity, {
                kind: "command",
                key: `${project.name}:command:${index}:${command}`,
                project: project.name,
                label: command,
                command,
            });
