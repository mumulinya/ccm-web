...
function toCriterionCheckStatus(status) {
    if (status === "passed" || status === "started" || status === "already_running")
        return "passed";
    if (status === "failed" || status === "timed_out")
        return "failed";
...
function buildCriterionLinkedEvidence(workOrder, commandResults, httpResults, browserResults) {
    const rows = Array.isArray(workOrder.metadata?.semanticPlan?.criterionCoverage)
        ? workOrder.metadata.semanticPlan.criterionCoverage
        : Array.isArray(workOrder.metadata?.criterionCoverage)
            ? workOrder.metadata.criterionCoverage
            : [];
    const surfaceAudit = workOrder.metadata?.surfaceAudit || workOrder.metadata?.surface_audit;
    const statusByName = new Map();
    const add = (name, status) => {
        const key = String(name || "").trim().toLowerCase();
        if (!key)
            return;
        statusByName.set(key, [...(statusByName.get(key) || []), toCriterionCheckStatus(status)]);
    };
    for (const result of commandResults)
        add(result.command, result.status);
    for (const result of httpResults)
        add(result.name, result.status);
    for (const result of browserResults)
        add(result.name, result.status);
    ...
    for (const row of rows) {
        ...
        const checkNames = Array.isArray(row?.checkNames) ? row.checkNames.map((item) => String(item || "").trim()).filter(Boolean) : [];
        if (!criterion || row?.status !== "planned" || !checkNames.length)
            continue;
        const resolved = checkNames.map((name) => ({ name, statuses: statusByName.get(name.toLowerCase()) || [] }));
        if (resolved.some(item => !item.statuses.length))
            continue;
        const failed = resolved.some(item => item.statuses.some(status => status === "failed" || status === "blocked"));
        const passed = !failed && resolved.every(item => item.statuses.some(status => status === "passed"));
        evidence.push({
            type: "work_order",
            title: `Acceptance criterion: ${criterion}`,
            status: failed ? "failed" : passed ? "passed" : "skipped",
            detail: `criterion-linked checks: ${checkNames.join(", ")}`,
        });
    }
    return evidence;
}
...
function buildTestAgentReport(input) {
    ...
    const evidence = [
        ...buildEvidence(commandResults, devServerResults, httpResults, browserResults, browserToolCalls),
        ...buildCriterionLinkedEvidence(workOrder, commandResults, httpResults, browserResults),
    ];
