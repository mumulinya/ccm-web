...
exports.summarizeBrowserSessionComparisonValue = summarizeBrowserSessionComparisonValue;
exports.runBrowserSessionComparison = runBrowserSessionComparison;
...
function summarizeBrowserSessionComparisonValue(value) {
    ...
    return {
        ...
        sha256: (0, crypto_1.createHash)("sha256").update(serialized).digest("hex"),
    };
}
function comparisonPassed(operator, left, right) {
    ...
    if (Array.isArray(left))
        return left.some(item => stableSerialize(item) === rightSerialized);
    return leftSerialized.includes(rightSerialized);
}
function valueSummaryDetail(label, summary) {
    if (!summary)
...
function summarizeEvaluationError(reason) {
    ...
    const sha256 = (0, crypto_1.createHash)("sha256").update(message).digest("hex");
    return `${name}(messageBytes=${serializedBytes},messageSha256=${sha256})`;
}
function evaluateBeforeDeadline(runtime, expression, timeoutMs) {
    return new Promise((resolve, reject) => {
        ...
        const timer = setTimeout(() => finish(reject, new Error("Browser session comparison evaluation timed out.")), Math.max(1, timeoutMs));
        Promise.resolve().then(() => runtime.page.evaluate(expression)).then(value => finish(resolve, value), error => finish(reject, error));
    });
}
async function runBrowserSessionComparison(input) {
    ...
    while (true) {
        ...
        if (!Object.keys(evaluationErrors).length)
