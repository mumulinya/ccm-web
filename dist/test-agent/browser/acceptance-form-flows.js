...
function fieldForRadioSegment(segment) {
    const values = quotedText(segment);
    if (!values.length || (!/\bradio(?:\s+button)?\b/i.test(segment) && !/单选/.test(segment)))
        return null;
    return { actionType: "check", fieldLabel: values[0], inputValue: "true" };
}
...
function hasInvalidFormIntent(criterion, fields, expectedText) {
...
function flowDerivedAssertions(assertions, fields, buttonName, expectedText) {
    ...
    return assertions
        ...
        if (normalized === normalizedExpectedText)
            return false;
        if (fieldInputValues.has(normalized))
            return false;
        if (formControlText.has(normalized))
            return false;
        return true;
    })
...
function buildAcceptanceFormFlowBrowserChecks(project, acceptanceCriteria = []) {
    return buildAcceptanceFormFlows(project, acceptanceCriteria).map(flow => ({
        ...
        context: {
            ...
            generatedBy: exports.ACCEPTANCE_FORM_FLOW_PROBE_TYPE,
