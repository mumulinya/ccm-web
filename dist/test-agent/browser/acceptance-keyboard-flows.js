...
exports.ACCEPTANCE_KEYBOARD_FLOW_PROBE_TYPE = void 0;
exports.acceptanceKeyboardIntent = acceptanceKeyboardIntent;
exports.buildAcceptanceKeyboardFlows = buildAcceptanceKeyboardFlows;
exports.buildAcceptanceKeyboardFlowBrowserChecks = buildAcceptanceKeyboardFlowBrowserChecks;
const utils_1 = require("../utils");
exports.ACCEPTANCE_KEYBOARD_FLOW_PROBE_TYPE = "acceptance_keyboard_flow";
const NAMED_KEYS = {
    enter: "Enter",
    return: "Enter",
    esc: "Escape",
...
function acceptanceKeyboardIntent(criterion) {
...
function looksLikeKeyboardCriterion(criterion) {
    if (!acceptanceKeyboardIntent(criterion))
        return false;
...
function canonicalKeyPart(part) {
    const normalized = clean(part).replace(/\s+/g, " ");
    const lower = normalized.toLowerCase();
    if (lower === "ctrl" || lower === "control")
        return "Control";
    if (lower === "cmd" || lower === "command" || lower === "meta" || lower === "win" || lower === "windows")
        return "Meta";
...
function canonicalKeyboardKey(value) {
    ...
    if (NAMED_KEYS[single.toLowerCase()] || /^F(?:[1-9]|1[0-2])$/.test(single))
        return single;
    return "";
}
...
function keyboardKeyFromCriterion(criterion) {
...
function expectedTextFromCriterion(criterion, key) {
    ...
    if (expected && expected.toLowerCase() !== key.toLowerCase())
        return expected;
...
function buildAcceptanceKeyboardFlows(project, acceptanceCriteria = []) {
    ...
    for (const raw of acceptanceCriteria) {
        ...
        if (!criterion || !looksLikeKeyboardCriterion(criterion))
            continue;
...
function buildAcceptanceKeyboardFlowBrowserChecks(project, acceptanceCriteria = []) {
    return buildAcceptanceKeyboardFlows(project, acceptanceCriteria).map(flow => ({
        name: flowName(project, flow),
        url: flow.url,
        probeType: exports.ACCEPTANCE_KEYBOARD_FLOW_PROBE_TYPE,
        context: {
            source: "acceptance_criteria",
            generatedBy: exports.ACCEPTANCE_KEYBOARD_FLOW_PROBE_TYPE,
...
//# sourceMappingURL=acceptance-keyboard-flows.js.map