...
exports.ACCEPTANCE_HOVER_FLOW_PROBE_TYPE = void 0;
exports.buildAcceptanceHoverFlows = buildAcceptanceHoverFlows;
exports.buildAcceptanceHoverFlowBrowserChecks = buildAcceptanceHoverFlowBrowserChecks;
const utils_1 = require("../utils");
exports.ACCEPTANCE_HOVER_FLOW_PROBE_TYPE = "acceptance_hover_flow";
...
function hoverMatch(criterion) {
    return hoverPattern("i").exec(criterion);
}
function looksLikeHoverCriterion(criterion) {
    if (!hoverMatch(criterion))
        return false;
...
function targetRoleFromCriterion(criterion, hoverIndex) {
...
function unquotedTargetNameFromCriterion(criterion, match) {
...
function buildAcceptanceHoverFlows(project, acceptanceCriteria = []) {
    ...
    for (const raw of acceptanceCriteria) {
        ...
        if (!criterion || !looksLikeHoverCriterion(criterion))
            continue;
        ...
        if (!url || !targetName || !expectedText)
            continue;
...
function buildAcceptanceHoverFlowBrowserChecks(project, acceptanceCriteria = []) {
    return buildAcceptanceHoverFlows(project, acceptanceCriteria).map(flow => ({
        name: flowName(project, flow),
        url: flow.url,
        probeType: exports.ACCEPTANCE_HOVER_FLOW_PROBE_TYPE,
        context: {
            source: "acceptance_criteria",
            generatedBy: exports.ACCEPTANCE_HOVER_FLOW_PROBE_TYPE,
...
//# sourceMappingURL=acceptance-hover-flows.js.map