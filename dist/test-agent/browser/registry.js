...
async function runBrowserVerificationWithProviders(workOrder, runtime = {}) {
    ...
    const progress = workOrder.projects.flatMap(project => (0, shared_1.checksForProject)(project, workOrder.acceptanceCriteria).map((check, index) => ({
        project: project.name,
        name: String(check.name || `浏览器检查 ${index + 1}`),
        trace: (0, user_visible_progress_1.beginTestAgentVisibleTool)(progressContext, {
            kind: "browser",
            key: `${project.name}:browser:${index}:${check.name || "check"}`,
            project: project.name,
            label: String(check.name || `浏览器检查 ${index + 1}`),
        }),
    })));
    const preferred = preferredProvider(workOrder, runtime);
    ...
    for (const item of progress) {
        const result = reconciled.results.find(row => String(row.project || "") === item.project && String(row.name || "") === item.name)
            || reconciled.results.find(row => String(row.name || "") === item.name)
            || { status: "blocked", error: "浏览器检查没有返回绑定结果" };
        item.trace.finish(result);
    }
