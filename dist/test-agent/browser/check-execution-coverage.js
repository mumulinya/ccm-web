...
exports.buildBrowserCheckExecutionPlan = buildBrowserCheckExecutionPlan;
exports.browserCheckExecutionPlanErrors = browserCheckExecutionPlanErrors;
exports.browserCheckExecutionIdentity = browserCheckExecutionIdentity;
exports.withBrowserCheckExecutionIdentity = withBrowserCheckExecutionIdentity;
exports.buildBrowserCheckExecutionCoverage = buildBrowserCheckExecutionCoverage;
exports.browserCheckExecutionEvidenceErrors = browserCheckExecutionEvidenceErrors;
exports.reconcileBrowserCheckExecution = reconcileBrowserCheckExecution;
exports.formatBrowserCheckExecutionCoverageLine = formatBrowserCheckExecutionCoverageLine;
exports.formatBrowserCheckExecutionCoverageAttentionLines = formatBrowserCheckExecutionCoverageAttentionLines;
const utils_1 = require("../utils");
const provider_routing_1 = require("./provider-routing");
const shared_1 = require("./shared");
const stability_summary_1 = require("./stability-summary");
function executionCheckId(projectIndex, checkIndex) {
    return `browser-check:${projectIndex + 1}:${checkIndex + 1}`;
}
function buildBrowserCheckExecutionPlan(workOrder, preferredProvider = workOrder.options.browserProvider || "auto") {
    const items = [];
    for (const [projectIndex, project] of workOrder.projects.entries()) {
        ...
        for (const [checkIndex, check] of checks.entries()) {
            const route = (0, provider_routing_1.browserProviderRouteForCheck)(workOrder, check, preferredProvider);
            items.push({
                ...
                url: (0, utils_1.resolveUrl)(project.targetUrl, check.url || project.targetUrl),
                expectedRuns: (0, stability_summary_1.browserCheckStabilityRuns)(check),
    ...
    return {
        ...
        planId: (0, utils_1.makeRunId)("browser-execution-plan"),
...
function browserCheckExecutionPlanErrors(plan) {
    const errors = [];
    if (plan?.schema !== "ccm-test-agent-browser-execution-plan-v1")
        errors.push("browser execution plan schema is invalid.");
    if (!String(plan?.planId || "").trim())
        errors.push("browser execution plan planId is missing.");
    if (!String(plan?.createdAt || "").trim())
        errors.push("browser execution plan createdAt is missing.");
    const items = Array.isArray(plan?.items) ? plan.items : [];
    if (plan?.plannedCheckCount !== items.length)
        errors.push("browser execution plan plannedCheckCount does not match items.length.");
    const expectedRunCount = items.reduce((sum, item) => sum + Number(item.expectedRuns || 0), 0);
    if (plan?.expectedRunCount !== expectedRunCount)
        errors.push("browser execution plan expectedRunCount does not match item runs.");
    const ids = items.map(item => item.checkId);
    if (new Set(ids).size !== ids.length)
        errors.push("browser execution plan contains duplicate checkId values.");
    for (const [index, item] of items.entries()) {
        const label = `browser execution plan item ${index}`;
        if (item.checkId !== executionCheckId(item.projectIndex, item.checkIndex))
            errors.push(`${label} checkId does not match its indexes.`);
        if (!Number.isInteger(item.projectIndex) || item.projectIndex < 0)
            errors.push(`${label} projectIndex is invalid.`);
        if (!Number.isInteger(item.checkIndex) || item.checkIndex < 0)
            errors.push(`${label} checkIndex is invalid.`);
        if (!Number.isInteger(item.expectedRuns) || item.expectedRuns < 1 || item.expectedRuns > stability_summary_1.MAX_BROWSER_STABILITY_RUNS) {
            errors.push(`${label} expectedRuns is invalid.`);
        }
        if (!item.project || !item.name)
            errors.push(`${label} is missing project or name.`);
    }
    return errors;
}
function browserCheckExecutionIdentity(input) {
    const projectIndex = input.workOrder.projects.indexOf(input.project);
    let plan = input.workOrder.metadata?.browserCheckExecutionPlan;
    if (!plan?.planId) {
...
function withBrowserCheckExecutionIdentity(input) {
    return {
        ...input.result,
...
function identityMatchesPlan(identity, plan, item) {
...
function buildBrowserCheckExecutionCoverage(plan, results) {
    ...
    for (const result of results) {
        ...
        if (identity.evidence === "synthetic_missing") {
            ...
            const runs = syntheticRuns.get(identity.checkId) || new Set();
        ...
        const runs = providerRuns.get(identity.checkId) || new Map();
        runs.set(identity.run, (runs.get(identity.run) || 0) + 1);
        providerRuns.set(identity.checkId, runs);
    }
    const items = plan.items.map(item => {
        const runs = providerRuns.get(item.checkId) || new Map();
        ...
        const syntheticBlockedRuns = [...(syntheticRuns.get(item.checkId) || new Set())]
            .filter(run => missingRuns.includes(run))
            .sort((a, b) => a - b);
        const status = duplicateRuns.length
            ? "invalid"
            : missingRuns.length
                ? "incomplete"
                : "complete";
    ...
    const status = invalidResultCount || duplicateResultCount
        ? "invalid"
...
function browserCheckExecutionEvidenceErrors(input) {
    ...
    if (!input.summary)
        errors.push("browserCheckExecutionCoverage is missing.");
    ...
    for (const [index, result] of (input.results || []).entries()) {
        ...
        if (identity.evidence === "synthetic_missing") {
            ...
            if (result.status !== "blocked")
                errors.push(`browserResults[${index}] synthetic missing evidence must be blocked.`);
        }
    }
    for (const item of expected.items) {
        for (const run of item.missingRuns) {
            ...
            if (count !== 1)
                errors.push(`browser check ${item.checkId} missing run ${run} requires exactly one synthetic blocked result, found ${count}.`);
        }
        for (const run of item.observedRuns) {
            if (syntheticCounts.has(`${item.checkId}::${run}`))
                errors.push(`browser check ${item.checkId} observed run ${run} must not also have synthetic missing evidence.`);
        }
    }
...
function missingResult(plan, item, run, diagnostic) {
...
function coverageDiagnosticResult(summary) {
    const at = (0, utils_1.nowIso)();
    return {
        provider: "none",
        project: "",
...
function reconcileBrowserCheckExecution(plan, providerResults) {
...
function formatBrowserCheckExecutionCoverageLine(summary) {
    if (!summary)
        return "status=incomplete; checks=0; runs=0/0; missing=0; duplicate=0; invalid=0";
    return [
...
function formatBrowserCheckExecutionCoverageAttentionLines(summary, limit = 5) {
    const attention = (summary?.items || []).filter(item => item.status !== "complete");
    if (!attention.length)
        return ["Browser execution coverage attention: none"];
    return [
        "Browser execution coverage attention:",
        ...attention.slice(0, Math.max(0, limit)).map(item => `- ${item.project} / ${item.name}: ${item.status}; observed=${item.observedRuns.join(",") || "none"}; missing=${item.missingRuns.join(",") || "none"}; duplicate=${item.duplicateRuns.join(",") || "none"}`),
    ];
}
//# sourceMappingURL=check-execution-coverage.js.map