...
function uploadFilePayloadForItem(project, item, allowedWorkDirs) {
    const filePath = String(item.filePath || item.file_path || item.path || "").trim();
    if (filePath) {
        const workSafety = (0, utils_1.validateTestAgentWorkDir)(project.workDir || process.cwd(), allowedWorkDirs);
        if (!workSafety.valid)
...
function uploadFilePayload(project, action, allowedWorkDirs) {
    const payloads = uploadFileItems(action).map(item => uploadFilePayloadForItem(project, item, allowedWorkDirs));
...
async function runAction(page, project, action, defaultTimeout, allowedWorkDirs) {
    ...
    try {
        ...
        if (action.type === "goto") {
            ...
            const payload = uploadFilePayload(project, action, allowedWorkDirs);
...
async function runAssertion(page, assertion, signals, defaultTimeout) {
    ...
    try {
        if (assertion.type === "visible") {
            ...
            if (!passed)
                throw new Error(`Expected browser cookie value to ${assertion.type === "cookieValueEquals" ? "equal requested value" : "include requested substring"}: ${cookieAssertionDetail(assertion)}.`);
        }
    ...
    try {
        ...
        for (let actionIndex = 0; actionIndex < actions.length; actionIndex += 1) {
            ...
            const beforeObservation = verifyEffect
                ...
                : {};
            const step = redactBrowserStepResult(await runAction(page, project, action, timeout, context.runtime.allowedWorkDirs), secretBindings);
            steps.push(step);
...
async function runPlaywrightMultiSessionAction(input) {
    ...
    const beforeObservation = verifyEffect
        ...
        : {};
    const actionStep = (0, multi_session_1.prefixBrowserSessionStep)(runtime.name, redactBrowserStepResult(await runAction(runtime.page, project, action, timeout, input.allowedWorkDirs), runtime.secretBindings));
            ...
            for (const session of check.sessions || []) {
                ...
                for (const action of setupActions) {
                    ...
                    const executed = await runPlaywrightMultiSessionAction({
                        ...
                        timeout,
                        allowedWorkDirs: context.runtime.allowedWorkDirs,
                    });
                    nextActionIndex += 1;
