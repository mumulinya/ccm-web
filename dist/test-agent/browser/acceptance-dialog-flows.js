...
exports.ACCEPTANCE_DIALOG_FLOW_PROBE_TYPE = void 0;
exports.acceptanceDialogIntent = acceptanceDialogIntent;
exports.buildAcceptanceDialogFlows = buildAcceptanceDialogFlows;
exports.buildAcceptanceDialogFlowBrowserChecks = buildAcceptanceDialogFlowBrowserChecks;
const utils_1 = require("../utils");
exports.ACCEPTANCE_DIALOG_FLOW_PROBE_TYPE = "acceptance_dialog_flow";
...
function acceptanceDialogIntent(criterion) {
...
function looksLikeDialogCriterion(criterion) {
...
function dialogTypeFromCriterion(criterion) {
    if (/\bprompt\b/i.test(criterion) || /(?:输入框|输入对话框|提示输入)/.test(criterion))
        return "prompt";
    if (/\bconfirm\b/i.test(criterion) || /(?:确认框|确认对话框)/.test(criterion))
        return "confirm";
    if (/\balert\b/i.test(criterion) || /(?:提示框|警告框|alert)/i.test(criterion))
        return "alert";
    return "";
}
function messageIncludesFromCriterion(criterion, targetName) {
...
function flowAssertions(flow) {
    return [
        ...
        ...(flow.dialogType ? [{ type: "dialogTypeEquals", value: flow.dialogType }] : []),
        { type: "urlIncludes", text: flow.path || "/" },
        { type: "consoleNoErrors" },
        { type: "networkNoErrors" },
    ];
}
function buildAcceptanceDialogFlows(project, acceptanceCriteria = []) {
    ...
    for (const raw of acceptanceCriteria) {
        ...
        if (!criterion || !looksLikeDialogCriterion(criterion))
            continue;
        ...
        if (!url || !targetName || !messageIncludes)
            continue;
...
function buildAcceptanceDialogFlowBrowserChecks(project, acceptanceCriteria = []) {
    return buildAcceptanceDialogFlows(project, acceptanceCriteria).map(flow => ({
        name: flowName(project, flow),
        url: flow.url,
        probeType: exports.ACCEPTANCE_DIALOG_FLOW_PROBE_TYPE,
        context: {
            source: "acceptance_criteria",
            generatedBy: exports.ACCEPTANCE_DIALOG_FLOW_PROBE_TYPE,
...
//# sourceMappingURL=acceptance-dialog-flows.js.map