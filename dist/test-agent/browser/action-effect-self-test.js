...
exports.runTestAgentPlaywrightActionEffectSelfTest = runTestAgentPlaywrightActionEffectSelfTest;
exports.runTestAgentMultiSessionActionEffectSelfTest = runTestAgentMultiSessionActionEffectSelfTest;
exports.runTestAgentCrossSessionActionEffectSelfTest = runTestAgentCrossSessionActionEffectSelfTest;
exports.runTestAgentMcpActionEffectSelfTest = runTestAgentMcpActionEffectSelfTest;
...
async function listen(server) {
    ...
    if (!address || typeof address === "string")
        throw new Error("Unable to allocate action-effect self-test port.");
...
function failedActionEffectArtifact(verification) {
...
async function runTestAgentPlaywrightActionEffectSelfTest() {
    ...
    try {
        ...
        const generatedChecks = (0, acceptance_click_flows_1.buildAcceptanceClickFlowBrowserChecks)({
            name: "generated-click-flow",
            workDir: dir,
            targetUrl,
        }, ['Click "Apply change", then "Changed" should be visible.']);
...
async function runTestAgentMultiSessionActionEffectSelfTest() {
    ...
    try {
        ...
        const verifyTamper = (mutate) => {
            const tampered = JSON.parse(originalReportText);
            mutate(tampered);
            const tamperedContract = (0, contract_1.validateTestAgentReportContract)(tampered);
            fs.writeFileSync(reportJsonPath, `${JSON.stringify(tampered, null, 2)}\n`, "utf-8");
            refreshManifestItemIntegrity(manifestPath, "report_json");
            return {
                contract: tamperedContract,
                artifact: (0, artifact_verifier_1.verifyTestAgentArtifactManifestFile)(manifestPath),
            };
        };
        const missingSessionTamper = verifyTamper(tampered => {
            delete tampered.browserResults
                .find((result) => result.name === "Session-scoped setup and parallel effects")
        ...
        const pass = report.status === "failed"
            ...
            && passing?.steps.filter(step => step.name.endsWith(":assert:actionEffect")
                && String(step.detail || "").includes("parallelGroup=1")).length === 2
...
async function runTestAgentCrossSessionActionEffectSelfTest() {
    ...
    try {
        const plan = (0, execution_plan_1.buildTestAgentExecutionPlan)(passInput);
        const planText = (0, cli_1.formatTestAgentCliExecutionPlanSummary)(plan);
        const passReport = await (0, self_test_policy_1.runTestAgentForSelfTest)(passInput, { browserProvider: "playwright" });
        const failReport = await (0, self_test_policy_1.runTestAgentForSelfTest)(failInput, { browserProvider: "playwright" });
        ...
        const verdict = (0, verdict_1.buildTestAgentVerdict)(passReport);
        const passContract = (0, contract_1.validateTestAgentReportContract)(passReport);
        const failContract = (0, contract_1.validateTestAgentReportContract)(failReport);
        ...
        const reportJsonPath = String(passReport.metadata.artifactFiles?.reportJsonPath || "");
        const markdownPath = String(passReport.metadata.artifactFiles?.reportMarkdownPath || "");
        const originalReportText = fs.readFileSync(reportJsonPath, "utf-8");
        const markdown = fs.readFileSync(markdownPath, "utf-8");
        const artifactVerification = (0, artifact_verifier_1.verifyTestAgentArtifactManifestFile)(manifestPath);
        const verifyTamper = (mutate) => {
            const tampered = JSON.parse(originalReportText);
            mutate(tampered);
            const contract = (0, contract_1.validateTestAgentReportContract)(tampered);
            fs.writeFileSync(reportJsonPath, `${JSON.stringify(tampered, null, 2)}\n`, "utf-8");
            refreshManifestItemIntegrity(manifestPath, "report_json");
            return {
                contract,
        ...
        fs.writeFileSync(reportJsonPath, originalReportText, "utf-8");
        refreshManifestItemIntegrity(manifestPath, "report_json");
        const invalidSingleSession = (0, self_test_policy_1.normalizeTestAgentWorkOrderForSelfTest)({
            ...passInput,
        ...
        const invalidUnknownTarget = (0, self_test_policy_1.normalizeTestAgentWorkOrderForSelfTest)({
            ...passInput,
        ...
        const invalidMissingVerify = (0, self_test_policy_1.normalizeTestAgentWorkOrderForSelfTest)({
            ...passInput,
        ...
        const invalidSameTarget = (0, self_test_policy_1.normalizeTestAgentWorkOrderForSelfTest)({
            ...passInput,
        ...
        const pass = passReport.status === "passed"
            ...
            && passBrowser.steps.some(step => step.name === "session:sender:assert:actionEffect"
                && step.status === "passed"
            ...
            && failBrowser.steps.some(step => step.name === "session:sender:assert:actionEffect"
                && step.status === "failed"
                && String(step.detail || "").includes("effectSession=receiver"))
...
async function runTestAgentMcpActionEffectSelfTest() {
...
//# sourceMappingURL=action-effect-self-test.js.map