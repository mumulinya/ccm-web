...
exports.ACCEPTANCE_POPUP_FLOW_PROBE_TYPE = void 0;
exports.acceptancePopupIntent = acceptancePopupIntent;
exports.buildAcceptancePopupFlows = buildAcceptancePopupFlows;
exports.buildAcceptancePopupFlowBrowserChecks = buildAcceptancePopupFlowBrowserChecks;
const utils_1 = require("../utils");
exports.ACCEPTANCE_POPUP_FLOW_PROBE_TYPE = "acceptance_popup_flow";
...
function explicitUrlPathSpans(criterion) {
    ...
    while ((match = pattern.exec(criterion))) {
        ...
        if (value.length >= 2)
            out.push({ value, index: match.index + match[1].length });
    }
    return out;
}
...
function acceptancePopupIntent(criterion) {
    return popupPattern("i").test(criterion);
}
function looksLikePopupCriterion(criterion) {
...
function popupTitleFromCriterion(criterion, targetName) {
...
function popupTextFromCriterion(criterion, targetName, title) {
...
function flowAssertions(flow) {
    return [
        { type: "pageNotBlank" },
        { type: "popupOpened" },
        ...(flow.popupUrlPath ? [{ type: "popupUrlIncludes", url: flow.popupUrlPath }] : []),
        ...(flow.popupTextIncludes ? [{ type: "popupTextIncludes", text: flow.popupTextIncludes }] : []),
        ...(flow.popupTitleIncludes ? [{ type: "popupTitleIncludes", title: flow.popupTitleIncludes }] : []),
        { type: "urlIncludes", text: flow.path || "/" },
        { type: "consoleNoErrors" },
        { type: "networkNoErrors" },
    ];
}
function buildAcceptancePopupFlows(project, acceptanceCriteria = []) {
    ...
    for (const raw of acceptanceCriteria) {
        ...
        if (!criterion || !looksLikePopupCriterion(criterion))
            continue;
        ...
        const popupUrlPath = paths.find(item => item.index > popup.index)?.value
            ...
            || "";
        const url = sourcePath ? (0, utils_1.resolveUrl)(project.targetUrl, sourcePath) : project.targetUrl;
        ...
        if (!url || !targetName || (!popupUrlPath && !popupTextIncludes && !popupTitleIncludes))
            continue;
...
function buildAcceptancePopupFlowBrowserChecks(project, acceptanceCriteria = []) {
    return buildAcceptancePopupFlows(project, acceptanceCriteria).map(flow => ({
        name: flowName(project, flow),
        url: flow.url,
        probeType: exports.ACCEPTANCE_POPUP_FLOW_PROBE_TYPE,
        context: {
            source: "acceptance_criteria",
            generatedBy: exports.ACCEPTANCE_POPUP_FLOW_PROBE_TYPE,
...
//# sourceMappingURL=acceptance-popup-flows.js.map