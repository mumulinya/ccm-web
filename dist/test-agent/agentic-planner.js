...
function hasProjectGitMetadata(workDir) {
    try {
        return fs.existsSync(path.join(path.resolve(workDir), ".git"));
    }
    catch {
        return false;
    }
...
function isGitCommand(command) {
    ...
    if (invocation.error)
        return false;
    const executable = path.basename(String(invocation.executable || "")).toLowerCase();
    return executable === "git" || executable === "git.exe";
}
function modelCommandCapabilityIssue(command, workDir) {
    if (isGitCommand(command) && !hasProjectGitMetadata(workDir))
        return "git_metadata_unavailable";
    return "";
}
function safeAuthoritativeEvidence(workOrder) {
    const raw = workOrder.metadata?.surfaceAudit || workOrder.metadata?.surface_audit;
    if (!raw || raw.schema !== "ccm-test-agent-surface-audit-v1")
        return { surfaceAudit: null };
    return {
        surfaceAudit: {
            status: cleanText(raw.status, 40),
            declaredFiles: (Array.isArray(raw.declaredFiles) ? raw.declaredFiles : []).map((item) => cleanText(item, 400)).filter(Boolean).slice(0, 200),
            actualFiles: (Array.isArray(raw.actualFiles) ? raw.actualFiles : []).map((item) => ({
                path: cleanText(item?.path, 400),
                state: cleanText(item?.state, 40),
            })).filter((item) => item.path).slice(0, 200),
            undeclaredChanges: (Array.isArray(raw.undeclaredChanges) ? raw.undeclaredChanges : []).map((item) => cleanText(item?.path || item, 400)).filter(Boolean).slice(0, 200),
            missingDeclaredChanges: (Array.isArray(raw.missingDeclaredChanges) ? raw.missingDeclaredChanges : []).map((item) => cleanText(item?.path || item, 400)).filter(Boolean).slice(0, 200),
            criterionStatuses: (Array.isArray(raw.criteria) ? raw.criteria : []).map((item) => ({
                criterionId: cleanText(item?.criterionId, 160),
                status: cleanText(item?.status, 40),
            })).filter((item) => item.criterionId).slice(0, 100),
            canAccept: raw.canAccept === true,
            checksum: cleanText(raw.checksum, 128),
            contentStored: false,
        },
    };
}
function allowedCriterionCheckNames(workOrder, evidence) {
    const names = [];
    for (const project of workOrder.projects) {
        names.push(...project.verificationCommands);
        names.push(...project.httpChecks.map(check => cleanText(check?.name, 300)));
        names.push(...project.browserChecks.map(check => cleanText(check?.name, 300)));
    }
...
function buildSourceContext(workOrder) {
    ...
    return workOrder.projects.map(project => {
        ...
        return {
            ...
            gitMetadataAvailable: hasProjectGitMetadata(project.workDir),
            excerpts,
        };
    });
}
function plannerSystemPrompt() {
    return [
        ...
        "Every planned checkNames entry must exactly equal an existing or proposed command string, an HTTP/browser check name, or the reserved name 'CCM authoritative change-surface audit'. Never invent a descriptive check label that is not present in the returned projects plan.",
        "When allowedCriterionCheckNames is non-empty, use those exact names whenever they cover the criterion. Inline interpreter commands such as node -e, python -c, shell -c, or generated scripts are forbidden even when they only appear to read files.",
...
function testAgentCacheContext(workOrder, stage) {
    const groupId = cleanText(workOrder.groupId || workOrder.metadata?.groupId || workOrder.metadata?.group_id, 240);
    const projectId = cleanText(workOrder.projects[0]?.name || workOrder.metadata?.projectId || workOrder.metadata?.project_id || "test-agent", 240);
    const scope = groupId ? "group" : "project";
    const scopeId = groupId || projectId;
    const exactSessionId = cleanText(workOrder.metadata?.projectSessionId
        || workOrder.metadata?.groupSessionId
        || workOrder.id, 240);
    const affinity = (0, agent_cache_affinity_1.testAgentCacheAffinity)({
        scope,
        scopeId,
        ...(projectId ? { projectId } : {}),
        exactSessionId,
        taskId: workOrder.taskId || workOrder.id,
        generation: Number(workOrder.metadata?.generation || 0),
        attempt: Number(workOrder.metadata?.attempt || 1),
        stage,
    });
    return {
        affinity,
        providerContextCache: {
            ...
            source: stage === "test_followup" ? "test_agent_followup" : "semantic_test_agent_plan",
...
async function callDefaultFollowupPlanner(input) {
    ...
    const cache = testAgentCacheContext(input.workOrder, "test_followup");
    const options = {
        ...
        messages: [{ role: "user", content: JSON.stringify({
                    ...
                    projectCapabilities: buildSourceContext(input.workOrder).map(item => ({
                        project: item.project,
                        scripts: item.packageScripts,
                        gitMetadataAvailable: item.gitMetadataAvailable,
                    })),
                    evidence,
                }) }],
        temperature: 0,
        ...
        providerContextCache: cache.providerContextCache,
        onUsage: (usage) => { providerUsage = usage; },
        onRetry: (notice) => (0, user_visible_progress_1.publishTestAgentVisibleModelRetry)(input.workOrder, "test_followup", notice),
    };
...
async function callDefaultPlanner(input) {
    const semanticInput = {
        ...
        authoritativeEvidence: input.authoritativeEvidence,
        allowedCriterionCheckNames: input.allowedCriterionCheckNames,
        readonlyCapabilities: input.readonlyCapabilityPrompt || "",
        ...(input.repairReason ? {
            repair: {
                attempt: 1,
                reason: input.repairReason,
                instruction: "Return a corrected complete plan. Use only safe project checks and exact allowed criterion check names.",
            },
        } : {}),
    };
    const stage = input.repairReason ? "test_plan_repair" : "test_plan";
    const cache = testAgentCacheContext(input.workOrder, stage);
    const result = await (0, semantic_decision_runtime_1.runSemanticDecision)({
        ...
        retryProfile: "agent_orchestration",
        onRetry: (notice) => (0, user_visible_progress_1.publishTestAgentVisibleModelRetry)(input.workOrder, stage, notice),
    });
    return { ...result.value, semanticDecisionReceipt: result.receipt, semanticRepairApplied: Boolean(input.repairReason) };
}
function normalizeSemanticTestPlan(value, workOrder) {
    ...
    const availableCheckNames = new Set();
    const plannedByProject = new Map(projects.map((item) => [String(item?.name || "").trim(), item]));
    for (const project of workOrder.projects) {
        for (const command of project.verificationCommands)
            availableCheckNames.add(command.trim().toLowerCase());
        const proposed = plannedByProject.get(project.name);
        for (const command of Array.isArray(proposed?.commands) ? proposed.commands : []) {
            const normalized = cleanText(command, 300);
            if (normalized && !(0, utils_1.isUnsafeVerificationCommand)(normalized) && !modelCommandCapabilityIssue(normalized, project.workDir)) {
                availableCheckNames.add(normalized.toLowerCase());
            }
        }
        for (const check of [...project.httpChecks, ...project.browserChecks]) {
            const name = cleanText(check?.name, 300);
            if (name)
                availableCheckNames.add(name.toLowerCase());
        }
        for (const check of [...(Array.isArray(proposed?.httpChecks) ? proposed.httpChecks : []), ...(Array.isArray(proposed?.browserChecks) ? proposed.browserChecks : [])]) {
            const name = cleanText(check?.name, 300);
            if (name)
                availableCheckNames.add(name.toLowerCase());
        }
    ...
    for (const criterion of criteria) {
        ...
        if (matches[0].status === "planned") {
            const unknown = matches[0].checkNames.filter((name) => !availableCheckNames.has(name.trim().toLowerCase()));
            if (unknown.length)
                throw new Error(`TestAgent 语义计划绑定了不存在的检查：${unknown.join(", ")}`);
        }
    }
...
function mergePlan(workOrder, plan) {
    ...
    const projects = workOrder.projects.map(project => {
        ...
        const candidateCommands = (Array.isArray(candidate.commands) ? candidate.commands : [])
            .map(command => cleanText(command, 300))
            .filter(command => !(0, utils_1.isUnsafeVerificationCommand)(command))
            .filter(command => {
            const reason = modelCommandCapabilityIssue(command, project.workDir);
            if (!reason)
                return true;
            rejectedCommands.push({ command, reason });
            issues.push({
                severity: "warning",
                code: "agentic_command_capability_unavailable",
                message: `${project.name}: ignored model-planned Git diagnostic because the isolated verification workspace has no Git metadata.`,
                project: project.name,
            });
            return false;
        })
...
async function applyAgenticTestPlanning(workOrder, runtime, preexistingIssues = []) {
    ...
    const authoritativeEvidence = safeAuthoritativeEvidence(workOrder);
    const exactCriterionCheckNames = allowedCriterionCheckNames(workOrder, authoritativeEvidence);
    try {
        ...
        const planningInput = {
            ...
            allowedCriterionCheckNames: exactCriterionCheckNames,
            readonlyCapabilityPrompt: runtime.readonlyCapabilityPrompt || "",
        };
        let plan;
        try {
            plan = await planner(planningInput);
        }
        catch (error) {
            const repairable = !runtime.agenticPlanner
                && /语义计划绑定了不存在的检查|semantic plan.*unknown|unknown.*check/i.test(String(error?.message || error));
            if (!repairable)
                throw error;
            plan = await callDefaultPlanner({
                ...planningInput,
                repairReason: cleanText(error?.message || error, 700),
            });
        }
        ...
        const renormalized = (0, work_order_1.normalizeTestAgentWorkOrder)({
            ...
            metadata: {
                ...
                agenticPlanning: {
                    ...
                    semanticRepairApplied: plan?.semanticRepairApplied === true,
                    readOnly: true,
