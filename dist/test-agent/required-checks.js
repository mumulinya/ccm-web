...
function consoleSignal(browserResults) {
    ...
    const bad = browserResults.filter(item => {
        const consoleErrors = [...(item.consoleErrors || []), ...(0, console_assertions_1.filterBrowserConsoleErrorLines)(item.consoleMessages || [])];
    ...
    if (bad.length) {
        return {
            ...
            evidence: bad.map(item => evidence(item.name, item.status, [
                ...(item.consoleErrors || []),
                ...(0, console_assertions_1.filterBrowserConsoleErrorLines)(item.consoleMessages || []),
                ...(item.pageErrors || []),
                ...item.steps
    ...
    if (!matches.length)
        return { status: "unknown", evidence: [], missingReason: "No console error assertion or telemetry evidence was recorded." };
...
function consoleWarningSignal(browserResults) {
    if (!browserResults.length)
        return { status: "unknown", evidence: [], missingReason: "No browser results were recorded for console warning checks." };
    const warningPattern = /consoleNoWarnings|no console warning|console warning/i;
    const matches = browserResults.filter(item => hasConsoleTelemetry(item)
        || item.steps.some(step => warningPattern.test(`${step.name} ${step.detail || ""}`)));
    if (!matches.length)
        return { status: "unknown", evidence: [], missingReason: "No console warning assertion or telemetry evidence was recorded." };
    const failed = matches.filter(item => (item.consoleMessages || []).some(console_assertions_1.isBrowserConsoleWarningLine)
    ...
    if (failed.length) {
        return {
            ...
            evidence: failed.map(item => evidence(item.name, item.status, [
                ...(item.consoleMessages || []).filter(console_assertions_1.isBrowserConsoleWarningLine).slice(0, 3),
                ...item.steps
...
function browserNetworkSignal(browserResults) {
    if (!browserResults.length)
        return { status: "unknown", evidence: [], missingReason: "No browser results were recorded for browser network checks." };
    const networkPattern = /networkNoErrors|networkRequest|networkResponse|browser network|network telemetry/i;
    const matches = browserResults.filter(item => (item.networkRequests || []).length > 0
        || Boolean(item.networkLogPath)
        || (item.networkErrors || []).length > 0
        || item.steps.some(step => networkPattern.test(`${step.name} ${step.detail || ""}`)));
    if (!matches.length)
        return { status: "unknown", evidence: [], missingReason: "No browser network assertion or telemetry evidence was recorded." };
    const failed = matches.filter(item => (item.networkErrors || []).length > 0
...
function browserAccessibilitySignal(browserResults) {
    if (!browserResults.length)
        return { status: "unknown", evidence: [], missingReason: "No browser results were recorded for accessibility checks." };
    ...
    const matches = browserResults.filter(item => item.steps.some(step => accessibilityPattern.test(`${step.name} ${step.detail || ""}`))
        || (item.browserArtifacts || []).some(artifact => artifact.type === "accessibility_snapshot"));
    if (!matches.length)
        return { status: "unknown", evidence: [], missingReason: "No accessibility/ARIA browser assertion or accessibility snapshot evidence was recorded." };
...
function browserInteractionLogSignal(browserResults, kind) {
    if (!browserResults.length)
        return { status: "unknown", evidence: [], missingReason: `No browser results were recorded for ${kind} log checks.` };
    const logs = browserResults.flatMap(item => {
...
function browserDialogSignal(browserResults) {
    if (!browserResults.length)
        return { status: "unknown", evidence: [], missingReason: "No browser results were recorded for dialog checks." };
    const dialogPattern = /dialogAppeared|dialogMessageIncludes|dialogTypeEquals|browser dialog|native dialog/i;
    const matches = browserResults.filter(item => (item.dialogMessages || []).length > 0
        || item.steps.some(step => dialogPattern.test(stepText(step))));
    if (!matches.length)
        return { status: "unknown", evidence: [], missingReason: "No browser dialog assertion or dialog telemetry evidence was recorded." };
    const failed = matches.filter(item => item.status === "blocked"
    ...
    const passed = matches.filter(item => item.status !== "blocked"
...
function browserPopupSignal(browserResults) {
    if (!browserResults.length)
        return { status: "unknown", evidence: [], missingReason: "No browser results were recorded for popup checks." };
    ...
    const matches = browserResults.filter(item => (item.popupMessages || []).length > 0
        || item.steps.some(step => popupPattern.test(stepText(step))));
    if (!matches.length)
        return { status: "unknown", evidence: [], missingReason: "No browser popup assertion or popup telemetry evidence was recorded." };
    const failed = matches.filter(item => item.status === "blocked"
...
function browserDownloadSignal(browserResults) {
    if (!browserResults.length)
        return { status: "unknown", evidence: [], missingReason: "No browser results were recorded for download checks." };
    const downloadPattern = /downloadedFile|downloaded file|browser download|download artifact/i;
    const matches = browserResults.filter(item => (item.browserArtifacts || []).some(artifact => artifact.type === "download")
        || item.steps.some(step => downloadPattern.test(stepText(step))));
    if (!matches.length)
        return { status: "unknown", evidence: [], missingReason: "No downloaded-file assertion or download artifact evidence was recorded." };
    const failed = matches.filter(item => item.status === "blocked"
...
function browserUploadSignal(browserResults) {
    if (!browserResults.length)
        return { status: "unknown", evidence: [], missingReason: "No browser results were recorded for upload checks." };
    const uploadPattern = /uploadFile|file upload|browser upload|acceptance_upload_flow/i;
    const matches = browserResults.filter(item => item.probeType === "acceptance_upload_flow"
        || item.steps.some(step => uploadPattern.test(stepText(step))));
    if (!matches.length)
        return { status: "unknown", evidence: [], missingReason: "No browser upload action evidence was recorded." };
    const failed = matches.filter(item => item.status === "blocked"
...
function responsiveSignal(browserResults) {
    if (!browserResults.length)
        return { status: "unknown", evidence: [], missingReason: "No browser results were recorded for responsive/mobile checks." };
    const matches = browserResults.filter(item => item.probeType === "acceptance_responsive_viewport"
        || item.viewport?.isMobile === true
        || item.steps.some(step => /noHorizontalOverflow|inViewport/i.test(step.name)));
    if (!matches.length)
        return { status: "unknown", evidence: [], missingReason: "No responsive/mobile viewport browser evidence was recorded." };
    ...
    if (failed.length)
        return { status: "not_verified", evidence: failed.map(item => evidence(item.name, item.status, item.error || item.finalUrl || item.url)) };
    const passed = matches.filter(item => item.status === "passed");
    if (passed.length) {
        return {
            status: "verified",
...
function browserStorageSignal(browserResults) {
    if (!browserResults.length)
        return { status: "unknown", evidence: [], missingReason: "No browser results were recorded for Web Storage checks." };
    ...
    if (!matches.length)
        return { status: "unknown", evidence: [], missingReason: "No localStorage/sessionStorage browser evidence was recorded." };
    const failed = matches.filter(item => item.status === "failed" || item.status === "blocked" || item.steps.some(step => /localStorage|sessionStorage|clearStorage|setLocalStorage|setSessionStorage/i.test(`${step.name} ${step.detail || ""}`) && step.status === "failed"));
    if (failed.length)
        return { status: "not_verified", evidence: failed.map(item => evidence(item.name, item.status, item.error || item.finalUrl || item.url)) };
    const passed = matches.filter(item => item.status === "passed");
    if (passed.length) {
...
function browserCookieSignal(browserResults) {
    if (!browserResults.length)
        return { status: "unknown", evidence: [], missingReason: "No browser results were recorded for cookie checks." };
    ...
    if (!matches.length)
        return { status: "unknown", evidence: [], missingReason: "No browser cookie evidence was recorded." };
    const failed = matches.filter(item => item.status === "failed" || item.status === "blocked" || item.steps.some(step => cookiePattern.test(`${step.name} ${step.detail || ""}`) && step.status === "failed"));
    if (failed.length)
        return { status: "not_verified", evidence: failed.map(item => evidence(item.name, item.status, item.error || item.finalUrl || item.url)) };
    const passed = matches.filter(item => item.status === "passed");
    if (passed.length) {
...
function browserClipboardSignal(browserResults) {
    if (!browserResults.length)
        return { status: "unknown", evidence: [], missingReason: "No browser results were recorded for clipboard checks." };
    ...
    if (!matches.length)
        return { status: "unknown", evidence: [], missingReason: "No browser clipboard action or assertion evidence was recorded." };
    const failed = matches.filter(item => item.status === "blocked"
...
function browserFocusSignal(browserResults) {
    if (!browserResults.length)
        return { status: "unknown", evidence: [], missingReason: "No browser results were recorded for focus checks." };
    ...
    if (!matches.length)
        return { status: "unknown", evidence: [], missingReason: "No browser focus action or assertion evidence was recorded." };
    const failed = matches.filter(item => item.status === "blocked"
...
function browserKeyboardSignal(browserResults) {
    if (!browserResults.length)
        return { status: "unknown", evidence: [], missingReason: "No browser results were recorded for keyboard checks." };
    ...
    if (!matches.length)
        return { status: "unknown", evidence: [], missingReason: "No browser keyboard action evidence was recorded." };
    const failed = matches.filter(item => item.status === "blocked"
...
function browserVisualSignal(browserResults) {
    if (!browserResults.length)
        return { status: "unknown", evidence: [], missingReason: "No browser results were recorded for visual checks." };
    ...
    if (!matches.length)
        return { status: "unknown", evidence: [], missingReason: "No browser visual assertion evidence was recorded." };
    const failed = matches.filter(item => item.status === "blocked"
...
function browserLayoutSignal(browserResults) {
    if (!browserResults.length)
        return { status: "unknown", evidence: [], missingReason: "No browser results were recorded for layout checks." };
    ...
    if (!matches.length)
        return { status: "unknown", evidence: [], missingReason: "No browser layout assertion evidence was recorded." };
    const failed = matches.filter(item => item.status === "blocked"
...
function isBrowserStepAssertion(step) {
    return step.kind === "assertion" || /^assert:/i.test(step.name);
}
function isMeaningfulFormCompletionAssertion(step) {
    return isBrowserStepAssertion(step)
        && !/consoleNoErrors|networkNoErrors|pageNotBlank/i.test(stepText(step));
}
function browserFormStateSignal(browserResults) {
    if (!browserResults.length)
        return { status: "unknown", evidence: [], missingReason: "No browser results were recorded for form/control state checks." };
    ...
    if (!matches.length)
        return { status: "unknown", evidence: [], missingReason: "No browser form/control state assertion evidence was recorded." };
    const failed = matches.filter(item => item.status === "blocked"
...
function browserFormSignal(browserResults) {
    if (!browserResults.length)
        return { status: "unknown", evidence: [], missingReason: "No browser results were recorded for form checks." };
    const actionPattern = /acceptance_form_flow|(^|:)fill\b|selectOption|(^|:)check\b|uncheck|form flow|form submit|form validation/i;
    const statePattern = /inputValueEquals|inputValueIncludes|selectedValue|selectedTextIncludes|(^|:)checked\b|notChecked|(^|:)enabled\b|(^|:)disabled\b|form state|control state/i;
    const matches = browserResults.filter(item => item.probeType === "acceptance_form_flow"
        || item.steps.some(step => actionPattern.test(stepText(step)) || statePattern.test(stepText(step))));
    if (!matches.length)
        return { status: "unknown", evidence: [], missingReason: "No browser form flow or form/control state evidence was recorded." };
    const failed = matches.filter(item => item.status === "blocked"
...
function browserTableSignal(browserResults) {
    if (!browserResults.length)
        return { status: "unknown", evidence: [], missingReason: "No browser results were recorded for table checks." };
    ...
    if (!matches.length)
        return { status: "unknown", evidence: [], missingReason: "No browser table row/cell assertion evidence was recorded." };
    const failed = matches.filter(item => item.status === "blocked"
...
function browserListSignal(browserResults) {
    if (!browserResults.length)
        return { status: "unknown", evidence: [], missingReason: "No browser results were recorded for list/count checks." };
    ...
    if (!matches.length)
        return { status: "unknown", evidence: [], missingReason: "No browser list/count assertion evidence was recorded." };
    const failed = matches.filter(item => item.status === "blocked"
...
function browserTextOrderSignal(browserResults) {
    if (!browserResults.length)
        return { status: "unknown", evidence: [], missingReason: "No browser results were recorded for text/order checks." };
    ...
    if (!matches.length)
        return { status: "unknown", evidence: [], missingReason: "No browser text-order assertion evidence was recorded." };
    const failed = matches.filter(item => item.status === "blocked"
...
function browserStepAssertionSignal(browserResults, pattern, missingReason, noPassReason) {
    if (!browserResults.length)
        return { status: "unknown", evidence: [], missingReason };
...
function browserUrlSignal(browserResults) {
    return browserStepAssertionSignal(browserResults, /urlEquals|urlIncludes|urlNotIncludes|browser url|url state|route url/i, "No browser URL assertion evidence was recorded.", "URL checks ran but did not produce passing evidence.");
}
function browserTitleSignal(browserResults) {
    return browserStepAssertionSignal(browserResults, /titleEquals|titleIncludes|titleNotIncludes|browser title|document title/i, "No browser title assertion evidence was recorded.", "Title checks ran but did not produce passing evidence.");
}
function browserNavigationSignal(browserResults) {
    return browserStepAssertionSignal(browserResults, /urlEquals|urlIncludes|urlNotIncludes|waitForUrl|goBack|goForward|reload|browser navigation|route navigation/i, "No browser navigation or URL-transition evidence was recorded.", "Navigation checks ran but did not produce passing evidence.");
}
function browserAttributeSignal(browserResults) {
    return browserStepAssertionSignal(browserResults, /attributeEquals|attributeIncludes|browser attribute|dom attribute|aria attribute|data attribute/i, "No browser DOM/attribute assertion evidence was recorded.", "Attribute checks ran but did not produce passing evidence.");
}
function browserNetworkStateSignal(browserResults) {
    return browserStepAssertionSignal(browserResults, /onlineState|browserOnline|browserOffline|setOffline|setOnline|browser network state|network state|offline state|online state/i, "No browser online/offline state evidence was recorded.", "Browser online/offline checks ran but did not produce passing evidence.");
}
function browserPresenceSignal(browserResults) {
    return browserStepAssertionSignal(browserResults, /assert:(?:visible|notVisible|present|notPresent)\b|browser presence|browser visibility|dom presence|element visibility/i, "No browser presence/visibility assertion evidence was recorded.", "Presence/visibility checks ran but did not produce passing evidence.");
}
function browserHoverSignal(browserResults) {
    return browserStepAssertionSignal(browserResults, /action:hover\b|computer-use:hover|browser hover|hover action|hover interaction/i, "No browser hover action evidence was recorded.", "Hover checks ran but did not produce passing evidence.");
}
function browserDragSignal(browserResults) {
    return browserStepAssertionSignal(browserResults, /action:dragTo\b|(?:^|:)dragTo\b|drag\/drop|drag and drop|browser drag|drag action/i, "No browser drag/drop action evidence was recorded.", "Drag/drop checks ran but did not produce passing evidence.");
}
function browserScrollSignal(browserResults) {
    return browserStepAssertionSignal(browserResults, /action:scroll\b|computer-use:scroll|browser scroll|scroll action|scroll interaction/i, "No browser scroll action evidence was recorded.", "Scroll checks ran but did not produce passing evidence.");
}
...
function browserWaitSignal(browserResults) {
    return browserStepAssertionSignal(browserResults, /action:waitFor(?:Selector|Text|Url)\b|(?:^|:)waitFor(?:Selector|Text|Url)\b|browser wait|wait condition|async ui/i, "No browser conditional wait evidence was recorded.", "Browser conditional wait checks ran but did not produce passing evidence.");
}
...
function coverageFor(check, input) {
    ...
    if (/^commands?$/.test(normalized))
        ...
        signal = consoleWarningSignal(input.browserResults);
    else if (/network_log|browser_network_log/.test(normalized))
        signal = browserLogSignal(input.browserResults, "network");
    else if (/browser_dialog_log|dialog_log/.test(normalized))
        signal = browserInteractionLogSignal(input.browserResults, "dialog");
    else if (/browser_popup_log|popup_log/.test(normalized))
        signal = browserInteractionLogSignal(input.browserResults, "popup");
    else if (/browser_network_state|network_state|browser_online|browser_offline|online_state|offline_state|(^|_)online(_|$)|(^|_)offline(_|$)/.test(normalized))
        signal = browserNetworkStateSignal(input.browserResults);
    else if (/browser_network|browser_requests?|browser_responses?|network_assertions?/.test(normalized))
        signal = browserNetworkSignal(input.browserResults);
    ...
    else if (/browser_download|file_download|download_assertions?|downloads?/.test(normalized) || normalized === "download")
        signal = browserDownloadSignal(input.browserResults);
    else if (/browser_accessibility_snapshot|accessibility_snapshot|a11y_snapshot|aria_snapshot_artifact/.test(normalized))
        signal = browserArtifactSignal(input.browserResults, "accessibility_snapshot");
    else if (/browser_accessibility|accessibility|a11y|screen_reader|aria_assertions?|aria_state|aria\b/.test(normalized))
        signal = browserAccessibilitySignal(input.browserResults);
    ...
    else if (/browser_visual|visual_assertions?|visuals?|element_visual|element_screenshot|chart_render|canvas_render/.test(normalized) || normalized === "visual")
        signal = browserVisualSignal(input.browserResults);
    else if (/browser_layout|layout_assertions?|layout|in_viewport|horizontal_overflow/.test(normalized))
        signal = browserLayoutSignal(input.browserResults);
    else if (/(^|_)(form_state|control_state|input_values?|selected_value|selected_text|checked_state|enabled_state|disabled_state)(_|$)/.test(normalized) || /(^|_)(selected|checked|not_checked|enabled|disabled)(_|$)/.test(normalized))
        signal = browserFormStateSignal(input.browserResults);
    else if (/browser_forms?|form_flows?|form_submits?|form_submission|form_validation|(^|_)forms?(_|$)/.test(normalized))
        signal = browserFormSignal(input.browserResults);
    else if (/browser_tables?|table_assertions?|table_rows?|table_cells?|data_tables?|(^|_)tables?(_|$)/.test(normalized))
        signal = browserTableSignal(input.browserResults);
    else if (/browser_lists?|list_assertions?|element_counts?|item_counts?|collection_counts?|card_counts?|(^|_)lists?(_|$)/.test(normalized))
        signal = browserListSignal(input.browserResults);
    else if (/browser_text_order|text_order|content_order|sort_order|sorted_order|ordering|ordered_text/.test(normalized))
        signal = browserTextOrderSignal(input.browserResults);
    else if (/browser_url|url_assertions?|route_assertions?|route_url|current_url|url_state/.test(normalized) || normalized === "url")
        signal = browserUrlSignal(input.browserResults);
    else if (/browser_title|title_assertions?|document_title|page_title/.test(normalized) || normalized === "title")
        signal = browserTitleSignal(input.browserResults);
    else if (/browser_navigation|navigation_assertions?|url_transition|route_transition|route_change|page_navigation/.test(normalized) || normalized === "navigation")
        signal = browserNavigationSignal(input.browserResults);
    else if (/browser_attribute|attribute_assertions?|dom_attributes?|aria_attribute|data_attribute/.test(normalized) || normalized === "attribute")
        signal = browserAttributeSignal(input.browserResults);
    else if (/browser_presence|browser_visibility|presence_assertions?|visibility_assertions?|dom_presence|element_presence|element_visibility/.test(normalized) || normalized === "presence" || normalized === "visibility")
        signal = browserPresenceSignal(input.browserResults);
    else if (/browser_hover|hover_actions?|hover_interactions?|hover_state/.test(normalized) || normalized === "hover")
        signal = browserHoverSignal(input.browserResults);
    else if (/browser_drag|drag_drop|drag_and_drop|drag_actions?|drop_actions?/.test(normalized) || normalized === "drag")
        signal = browserDragSignal(input.browserResults);
    else if (/browser_scroll|scroll_actions?|scroll_interactions?|scroll_position/.test(normalized) || normalized === "scroll")
        signal = browserScrollSignal(input.browserResults);
    else if (/browser_history|history_navigation|browser_reload|reload_actions?|browser_back|browser_forward|back_forward/.test(normalized) || normalized === "history" || normalized === "reload")
        signal = browserHistorySignal(input.browserResults);
    else if (/browser_script|browser_javascript|browser_js|js_assertions?|javascript_assertions?|script_assertions?|browser_evaluate|page_expression/.test(normalized) || normalized === "javascript" || normalized === "js")
        signal = browserScriptSignal(input.browserResults);
    else if (/browser_wait|wait_assertions?|wait_conditions?|wait_for_selector|wait_for_text|wait_for_url|async_ui/.test(normalized) || normalized === "wait")
        signal = browserWaitSignal(input.browserResults);
    else if (/responsive|mobile|viewport|small_screen|phone|horizontal_overflow|overflow/.test(normalized))
        signal = responsiveSignal(input.browserResults);
    else if (/web_storage|browser_storage|local_storage|session_storage|localstorage|sessionstorage/.test(normalized) || normalized === "storage")
        signal = browserStorageSignal(input.browserResults);
    else if (/browser_cookie|cookie_assertions?|cookie_actions?|cookies?/.test(normalized) || normalized === "cookie")
        signal = browserCookieSignal(input.browserResults);
    else if (/browser_clipboard|clipboard_assertions?|clipboard_actions?|clipboards?/.test(normalized) || normalized === "clipboard")
        signal = browserClipboardSignal(input.browserResults);
    else if (/browser_focus|focus_assertions?|focus_state|focused|keyboard_focus/.test(normalized) || normalized === "focus")
        signal = browserFocusSignal(input.browserResults);
    else if (/browser_keyboard|keyboard_actions?|keyboard_input|keyboard_shortcuts?|hotkeys?|shortcuts?/.test(normalized) || normalized === "keyboard")
        signal = browserKeyboardSignal(input.browserResults);
    else if (/browser_upload|file_upload|upload_assertions?|uploads?/.test(normalized) || normalized === "upload")
        signal = browserUploadSignal(input.browserResults);
    else if (/browser_dialog|dialog_assertions?|dialogs?|alerts?|confirms?|prompts?/.test(normalized) || normalized === "dialog")
        signal = browserDialogSignal(input.browserResults);
    else if (/browser_popup|popup_assertions?|popups?|new_tab|new_window/.test(normalized) || normalized === "popup")
        signal = browserPopupSignal(input.browserResults);
