...
async function fetchWithTimeout(url, timeoutMs, init = {}) {
    const safety = (0, utils_1.validateTestAgentUrl)(url);
    if (!safety.valid)
        return { response: null, text: "", durationMs: 0, error: safety.error, finalUrl: url, redirectCount: 0 };
    ...
    try {
        while (true) {
            ...
            if (redirectMode === "error") {
                ...
                return { response: null, text: "", durationMs: Date.now() - started, error: "HTTP redirect is not allowed for this request.", finalUrl: currentUrl, redirectCount };
            }
            if (redirectCount >= 10) {
                try {
                    await response.body?.cancel();
                ...
                return { response: null, text: "", durationMs: Date.now() - started, error: "HTTP redirect limit exceeded.", finalUrl: currentUrl, redirectCount };
            }
            const nextSafety = (0, utils_1.validateTestAgentUrl)(location, currentUrl);
            if (!nextSafety.valid) {
                try {
                    await response.body?.cancel();
                }
        ...
        return { response: null, text: "", durationMs: Date.now() - started, error: error.message || String(error), finalUrl: currentUrl, redirectCount };
    }
    finally {
        clearTimeout(timer);
    }
...
async function verifyProjectPageHttp(workOrder, project) {
    ...
    const resourceQueue = (0, http_page_resources_1.extractHtmlPageResources)(verifiedPageUrl, main.text, Math.max(maxResourceChecks * 3, maxResourceChecks));
    ...
    while (resourceQueue.length && resourceChecks.length < maxResourceChecks) {
        ...
        if (candidate.kind === "stylesheet" && checked.evidence.status === "passed" && checked.text) {
            const nested = (0, http_page_resources_1.extractCssPageResources)(verifiedPageUrl, candidate.url, checked.text, maxResourceChecks * 2)
...
async function runHttpVerification(workOrder) {
    ...
    for (const project of workOrder.projects) {
        if (projectNeedsAutomaticPageProbe(workOrder, project)) {
            const trace = (0, user_visible_progress_1.beginTestAgentVisibleTool)(progressContext, { kind: "http", key: `${project.name}:http:auto`, project: project.name, label: "页面可用性" });
            try {
                const result = await verifyProjectPageHttp(workOrder, project);
                trace.finish(result);
                results.push(result);
            }
            catch (error) {
        ...
        for (let i = 0; i < project.httpChecks.length; i += 1) {
            const check = project.httpChecks[i];
            const trace = (0, user_visible_progress_1.beginTestAgentVisibleTool)(progressContext, { kind: "http", key: `${project.name}:http:${i}:${check.name || "check"}`, project: project.name, label: check.name || `HTTP 检查 ${i + 1}` });
            try {
                const result = await verifyExplicitHttpCheck(workOrder, project, check, i);
                trace.finish(result);
                results.push(result);
            }
            catch (error) {
        ...
        for (let i = 0; i < project.adversarialHttpChecks.length; i += 1) {
            const check = project.adversarialHttpChecks[i];
            const trace = (0, user_visible_progress_1.beginTestAgentVisibleTool)(progressContext, { kind: "http", key: `${project.name}:http:adversarial:${i}:${check.name || "check"}`, project: project.name, label: check.name || `对抗 HTTP 检查 ${i + 1}` });
            try {
                const result = await verifyExplicitHttpCheck(workOrder, project, check, i);
                trace.finish(result);
                results.push(result);
