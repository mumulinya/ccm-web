...
exports.configuredTestAgentAllowedWorkDirs = configuredTestAgentAllowedWorkDirs;
exports.normalizeTestAgentAllowedWorkDirs = normalizeTestAgentAllowedWorkDirs;
...
function configuredTestAgentAllowedWorkDirs() {
...
function normalizeTestAgentAllowedWorkDirs(values = []) {
    return Array.from(new Set(values.map(String).filter(Boolean).map(realPathOrResolved)));
}
function validateTestAgentWorkDir(workDir, allowedWorkDirs) {
    const resolved = path.resolve(String(workDir || ""));
    const allowedRoots = normalizeTestAgentAllowedWorkDirs(allowedWorkDirs === undefined ? configuredTestAgentAllowedWorkDirs() : allowedWorkDirs);
