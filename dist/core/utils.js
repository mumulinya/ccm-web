...
exports.readGitRevisionFileText = readGitRevisionFileText;
...
var runtime_paths_1 = require("./runtime-paths");
Object.defineProperty(exports, "CCM_DIR", { enumerable: true, get: function () { return runtime_paths_1.CCM_DIR; } });
Object.defineProperty(exports, "DEFAULT_CCM_DIR", { enumerable: true, get: function () { return runtime_paths_1.DEFAULT_CCM_DIR; } });
Object.defineProperty(exports, "LEGACY_CCM_DIR", { enumerable: true, get: function () { return runtime_paths_1.LEGACY_CCM_DIR; } });
Object.defineProperty(exports, "ensureCcmRuntimeHomeMigrationSync", { enumerable: true, get: function () { return runtime_paths_1.ensureCcmRuntimeHomeMigrationSync; } });
const runtime_paths_2 = require("./runtime-paths");
...
exports.CONFIGS_DIR = path.join(runtime_paths_2.CCM_DIR, "configs");
exports.PID_DIR = path.join(runtime_paths_2.CCM_DIR, "pids");
exports.LOG_DIR = path.join(runtime_paths_2.CCM_DIR, "logs");
...
exports.GROUP_LOGS_FILE_SHARED = path.join(runtime_paths_2.CCM_DIR, "group-logs.json");
...
// 业务级别配置文件路径
exports.METRICS_FILE = path.join(runtime_paths_2.CCM_DIR, "metrics.json");
exports.FEISHU_CONFIG_FILE = path.join(runtime_paths_2.CCM_DIR, "feishu-config.json");
exports.PROJECT_CONFIGS_FILE = path.join(runtime_paths_2.CCM_DIR, "project-configs.json");
exports.GROUP_LOGS_FILE = path.join(runtime_paths_2.CCM_DIR, "group-logs.json");
exports.MUSIC_CONFIG_FILE = path.join(runtime_paths_2.CCM_DIR, "music-config.json");
exports.PETS_FILE = path.join(runtime_paths_2.CCM_DIR, "pets.json");
...
function readHeadFileText(workDir, filePath) {
    return readGitRevisionFileText(workDir, "HEAD", filePath);
}
function readGitRevisionFileText(workDir, revision, filePath) {
    try {
        const safeRevision = String(revision || "HEAD").trim() || "HEAD";
        let buffer = (0, child_process_1.execFileSync)("git", ["show", `${safeRevision}:${filePath}`], {
            cwd: workDir,
            timeout: 5000,
...
function readGitHead(workDir) {
    try {
        return String((0, child_process_1.execFileSync)("git", ["rev-parse", "HEAD"], {
            cwd: workDir,
            encoding: "utf-8",
            timeout: 5000,
            stdio: ["pipe", "pipe", "pipe"],
...
function parseCommittedFileChanges(workDir, baselineHead) {
    const currentHead = readGitHead(workDir);
    if (!baselineHead || !currentHead || baselineHead === currentHead)
        return [];
    try {
        const output = (0, child_process_1.execFileSync)("git", ["-c", "core.quotepath=false", "diff", "--name-status", "-z", `${baselineHead}..${currentHead}`], {
            cwd: workDir,
            encoding: "utf-8",
            timeout: 5000,
            maxBuffer: 8 * 1024 * 1024,
        ...
        for (let index = 0; index < fields.length;) {
            const rawStatus = String(fields[index++] || "").trim();
            if (!rawStatus)
                continue;
            const kind = rawStatus[0] || "M";
            const previousPath = String(fields[index++] || "").trim();
            const renamed = kind === "R" || kind === "C";
            const filePath = renamed ? String(fields[index++] || "").trim() : previousPath;
            if (!filePath)
                continue;
            let stat = null;
            try {
                ...
                if (fs.existsSync(absolute)) {
                    const value = fs.statSync(absolute);
                    stat = { mtimeMs: value.mtimeMs, size: value.size };
                }
            }
            catch { }
            rows.push({
                ...
                statusCode: kind === "D" ? "D " : kind === "A" ? "A " : kind === "R" ? "R " : kind === "C" ? "C " : "M ",
                stat,
            });
        }
        return rows;
...
function createFileChangeSnapshot(workDir) {
    try {
        ...
        return { workDir, head: readGitHead(workDir), files };
    }
    catch {
        return { workDir, head: "", files: {} };
    }
}
function getFileChanges(projectName, beforeSnapshot = null) {
    try {
        ...
        const workingEntries = parseGitStatus(workDir);
        const committedEntries = beforeSnapshot?.head
            ? parseCommittedFileChanges(workDir, String(beforeSnapshot.head || ""))
            : [];
        const entriesByPath = new Map();
        for (const entry of committedEntries)
            entriesByPath.set(entry.path, { ...entry, committed: true });
        for (const entry of workingEntries) {
            const committed = entriesByPath.get(entry.path);
            entriesByPath.set(entry.path, { ...committed, ...entry, committed: committed?.committed === true });
        }
        const files = [...entriesByPath.values()]
            .filter(entry => {
            if (!beforeFiles)
                return true;
            ...
            if (entry.committed === true) {
                const after = readWorkingFileText(workDir, entry.path);
                const baseline = before.contentSnapshot;
                if (baseline?.binary || after?.binary) {
                    return baseline?.exists !== after?.exists || Number(baseline?.size || 0) !== Number(after?.size || 0);
                }
                return baseline?.exists !== after?.exists || String(baseline?.text || "") !== String(after?.text || "");
            }
            ...
            return {
                ...
                ...describeFileStatus(entry.statusCode, before),
            };
        });
        for (const file of files) {
            const snapshotted = beforeFiles ? beforeFiles[file.path] : null;
            const committed = committedEntries.find(entry => entry.path === file.path);
            const baselinePath = committed?.previousPath || file.path;
            const before = snapshotted || (beforeSnapshot?.head ? {
                contentSnapshot: readGitRevisionFileText(workDir, String(beforeSnapshot.head), baselinePath),
            } : null);
