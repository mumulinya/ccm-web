...
function getRuntimeVersionCommand(agentType) {
    ...
    if (provider === "claudecode")
        return (0, agent_provider_settings_1.resolveClaudeCodeCommand)();
    if (provider === "codex")
        return "codex";
    if (provider === "cursor") {
...
function formatBuiltinToolsArg(options = {}) {
    // Claude Code documents `--tools ""` as the fail-closed way to disable the
    // complete built-in tool set. MCP tools remain governed independently by
    // the signed MCP config and --allowed-tools policy.
    return options.disableBuiltinTools === true ? ` --tools ${quoteCmdArg("")}` : "";
}
function pipeFileToCommand(msgFile, command, options = {}) {
    return `type "${msgFile}" | ${command}${formatBuiltinToolsArg(options)}${formatAllowedToolsArg(options)}`;
}
...
function getClaudePermissionMode(options = {}) {
    if (["full_access", "main_agent_only", "ask_before_edit"].includes(String(options.conversationPermissionMode || "")))
        return "acceptEdits";
...
exports.AGENT_RUNTIMES = [
    {
        ...
        buildCommand: (msgFile, options = {}) => {
            ...
            const claudeCommand = (0, agent_provider_settings_1.resolveClaudeCodeCommand)();
            const executable = /[\\/\s]/.test(claudeCommand) ? quoteCmdArg(claudeCommand) : claudeCommand;
            return `${pipeFileToCommand(msgFile, `${executable} --permission-mode ${getClaudePermissionMode(options)}${modelArg}`, options)}${formatStrictMcpConfigArg(options)}${formatPluginDirArg(metadata)}${formatAppendSystemPromptFileArg(options)}${sessionArg} --output-format stream-json --verbose -p`;
        },
    },
    {
        id: "cursor",
        ...
        for (const line of raw.split(/\r?\n/)) {
            ...
            try {
                ...
                if (event.type === "result" && typeof event.result === "string" && event.result.trim())
                    messages.push(event.result.trim());
            }
            catch { }
        }
...
function runAgentRuntimeSessionSelfTest() {
    ...
    const claudeNoBuiltinTools = buildAgentCommand("claudecode", "prompt.txt", { persistSession: true, sessionId, disableBuiltinTools: true, cliAllowedTools: [] });
    ...
    const checks = {
        ...
        claudeCanFailClosedWithoutBuiltinTools: claudeNoBuiltinTools.includes('--tools ""') && !claudeNoBuiltinTools.includes("--allowed-tools"),
