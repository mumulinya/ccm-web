...
exports.applyEvidencePolicyToPlan = applyEvidencePolicyToPlan;
exports.buildPlanReviewReceipt = buildPlanReviewReceipt;
exports.planningReviewSystemPrompt = planningReviewSystemPrompt;
exports.planningReviewPrompt = planningReviewPrompt;
exports.planningReviewInputPrompt = planningReviewInputPrompt;
...
const STORE_FILE = path.join(os.homedir(), ".ccm", "planning-sessions.json");
...
function planningEvidenceManifestFromToolResults(rows) {
    ...
    const visit = (value, depth = 0) => {
        ...
        if (Array.isArray(value.planningEvidenceEntries))
            receipts.push(...value.planningEvidenceEntries);
        if (Array.isArray(value.planning_evidence_entries))
            receipts.push(...value.planning_evidence_entries);
    ...
    // A source inquiry can truthfully be insufficient for the *whole task*
    // because write-time verification has not happened yet while still carrying
    // valid, checksummed read evidence. Extract only explicit evidence receipts
    // regardless of the wrapper outcome; never discard those facts merely
    // because the broader inquiry is incomplete.
...
function confirmPlanningSession(input) {
    ...
    if (!(0, evidence_policy_1.isPlanReviewPassed)(session.reviewReceipt?.verdict) || session.reviewReceiptChecksum !== session.reviewReceipt?.checksum) {
...
function normalizedEvidencePath(project, value) {
    const normalized = clean(value, 500).replace(/\\/g, "/").replace(/^\.\//, "");
    const prefix = `${clean(project, 180)}/`;
    return prefix !== "/" && normalized.startsWith(prefix) ? normalized.slice(prefix.length) : normalized;
}
function matchingEvidenceEntries(file, evidenceManifest) {
    const project = clean(file?.project, 180);
    const filePath = normalizedEvidencePath(project, file?.path);
    return evidenceManifest.entries.filter(item => (!!project
        && item.project === project
        && normalizedEvidencePath(project, item.path) === filePath));
}
function applyEvidencePolicyToPlan(plan, evidenceManifest, evidencePolicy) {
    if (!plan || evidencePolicy.level !== "lightweight")
        return plan;
    const next = JSON.parse(JSON.stringify(plan));
    next.evidencePolicy = evidencePolicy;
    next.files = (Array.isArray(next.files) ? next.files : []).map((file) => {
        const matches = matchingEvidenceEntries(file, evidenceManifest);
        const matchingIds = new Set(matches.map(item => item.evidenceId));
        const current = (Array.isArray(file?.sourceEvidenceIds) ? file.sourceEvidenceIds : []).map(String).filter((id) => matchingIds.has(id));
        const inferred = matches.map(item => item.evidenceId);
        return { ...file, sourceEvidenceIds: Array.from(new Set([...current, ...inferred])) };
    });
    // Lightweight plans must not fail merely because the model split a safe
    // preparatory action (for example, recording the baseline) into its own
    // step without repeating the requirement binding. The authoritative
    // requirement contract already owns the acceptance semantics, so the
    // server can deterministically inherit those exact IDs/descriptions. This
    // does not invent a criterion or a result, and strict plans still require
    // the model/reviewer to provide every per-step binding explicitly.
    const criteria = Array.isArray(next?.businessRequirement?.acceptanceCriteria)
        ? next.businessRequirement.acceptanceCriteria
            .map((item) => ({ id: clean(item?.id, 120), description: clean(item?.description, 800) }))
            .filter((item) => item.id && item.description)
        : [];
    next.steps = (Array.isArray(next.steps) ? next.steps : []).map((step) => {
        const ids = Array.isArray(step?.acceptanceCriterionIds)
            ? step.acceptanceCriterionIds.map((id) => clean(id, 120)).filter(Boolean)
            : [];
        const acceptance = Array.isArray(step?.acceptance)
            ? step.acceptance.map((value) => clean(value, 800)).filter(Boolean)
            : [];
        if (ids.length && acceptance.length)
            return step;
        return {
            ...step,
            acceptanceCriterionIds: criteria.map((item) => item.id),
            acceptance: criteria.map((item) => item.description),
        };
    });
    next.checksum = (0, implementation_plan_1.implementationPlanChecksum)(next);
    return next;
}
function buildPlanReviewReceipt(input) {
    const evidencePolicy = input.evidencePolicy || input.plan?.evidencePolicy || (0, evidence_policy_1.resolveEvidencePolicy)({
        requiresCodeChanges: true,
        targetProjects: input.plan?.businessRequirement?.targetProjects || [],
        riskLevel: input.plan?.riskLevel,
    });
    const deterministic = (0, implementation_plan_1.validateImplementationPlanV2)(input.plan);
    const blockers = deterministic.issues.map((message, index) => ({ code: `structure_${index + 1}`, message }));
    const warnings = [];
    const qualityReport = input.qualityReport || (0, presented_plan_quality_1.assessImplementationPlanQuality)(input.plan, input.evidenceManifest, {
        allowedProjects: input.plan?.businessRequirement?.targetProjects || [],
    });
    for (const issue of qualityReport.issues) {
        const item = { code: issue.code, message: issue.message, ...(issue.stepId ? { stepId: issue.stepId } : {}) };
    ...
    for (const file of files) {
        ...
        const filePath = normalizedEvidencePath(clean(file?.project, 180), file?.path);
        const project = clean(file?.project, 180);
        const exact = refs.length && refs.every((id) => {
            ...
            return !!evidence
                && normalizedEvidencePath(project, evidence.path) === filePath
                && (!!project && evidence.project === project);
        });
        if (exact)
            evidencedFiles += 1;
        else {
            const issue = { code: "file_evidence_missing", message: `计划文件缺少同项目、同路径的真实读取证据：${filePath || "unknown"}` };
            if (evidencePolicy.perFileEvidenceRequired)
                blockers.push(issue);
            else
                warnings.push(issue);
        }
    }
    if (evidencePolicy.sourceGroundingRequired && files.length && input.evidenceManifest.entries.length === 0) {
        blockers.push({ code: "source_evidence_absent", message: "开发任务尚未取得任何真实源码读取证据" });
    }
    ...
    if (acceptance.length && coveredAcceptance < acceptance.length)
        blockers.push({ code: "acceptance_verification_gap", message: "部分验收标准没有映射到验证方式" });
    for (const issue of Array.isArray(input.reviewer?.issues) ? input.reviewer.issues : []) {
        const message = clean(issue?.message || issue, 800);
        if (message)
            blockers.push({ code: clean(issue?.code || "independent_review", 100), message, ...(clean(issue?.stepId, 100) ? { stepId: clean(issue.stepId, 100) } : {}) });
    }
    ...
    if (["repair_required", "blocked"].includes(reviewerVerdict) && !Array.isArray(input.reviewer?.issues)) {
        blockers.push({ code: "independent_review_rejected", message: "独立复核未通过，但未返回可修复的问题清单" });
    }
    const uniqueBlockers = [...new Map(blockers.map(issue => [`${issue.code}:${issue.message}:${issue.stepId || ""}`, issue])).values()];
    const uniqueWarnings = [...new Map(warnings.map(issue => [`${issue.code}:${issue.message}:${issue.stepId || ""}`, issue])).values()];
    const issues = [...uniqueBlockers, ...uniqueWarnings];
    const core = {
        schema: exports.CCM_PLAN_REVIEW_RECEIPT_SCHEMA,
        verdict: (input.blocked || reviewerVerdict === "blocked" ? "blocked" : uniqueBlockers.length || reviewerVerdict === "repair_required" ? "repair_required" : uniqueWarnings.length ? "passed_with_warnings" : "passed"),
        issues,
        ...
        qualityReport,
        reviewerBindingChecksum: hash({ planChecksum: input.plan?.checksum || (0, implementation_plan_1.implementationPlanChecksum)(input.plan), evidenceManifestChecksum: input.evidenceManifest.checksum, evidencePolicy, reviewer: input.reviewer?.reviewer || "deterministic" }),
        contentStored: false,
    };
    return { ...core, checksum: hash(core) };
}
function planningReviewSystemPrompt() {
    return [
        implementation_plan_1.IMPLEMENTATION_PLAN_PROMPTS.planning_review,
        "Return JSON only: {\"verdict\":\"passed|repair_required|blocked\",\"issues\":[{\"code\":\"\",\"message\":\"\",\"stepId\":\"\"}]}. The server separately classifies evidence-format warnings. Do not include hidden reasoning.",
    ].join("\n\n");
}
function planningReviewPrompt(plan, evidenceManifest) {
    return [planningReviewSystemPrompt(), JSON.stringify({ plan, evidenceManifest })].join("\n\n");
}
function planningReviewInputPrompt(plan, evidenceManifest) {
    return JSON.stringify({ instruction: "Review this plan independently using the fixed review contract.", plan, evidenceManifest });
}
...
function runPlanningOrchestratorSelfTest() {
    ...
    const plan = (0, implementation_plan_1.normalizeImplementationPlanV2)({
        ...
        approach: "方案",
        scope: ["web"],
        files: [{ project: "web", path: "src/a.ts", reason: "入口", sourceEvidenceIds: [evidence.entries[0].evidenceId] }],
        steps: [{ id: "a", title: "调整结果展示入口", objective: "在现有入口统一状态更新，使完成结果能够及时显示给用户。", changeSummary: "完成状态写入后刷新结果区域，避免旧状态继续展示。", projects: ["web"], files: ["src/a.ts"], dependsOn: [], acceptanceCriterionIds: ["acceptance-1"], acceptance: ["结果可见"] }],
        verification: [{ expected: "结果可见", acceptanceCriteria: ["结果可见"] }],
        risks: [],
        exclusions: [],
        openQuestions: [],
        revision: 1,
        promptVersion: implementation_plan_1.CCM_IMPLEMENTATION_PLAN_PROMPT_VERSION,
        outputLanguage: "zh-CN",
        contentStored: false,
    }, {
        requirementContract: {
            requirementId: "requirement-selftest",
            revision: 1,
            title: "计划",
            businessGoal: "结果可见",
            background: "原因",
            acceptanceCriteria: [{ id: "acceptance-1", description: "结果可见" }],
            constraints: [],
            exclusions: [],
            targetProjects: ["web"],
            sourceMessageIds: ["message-selftest"],
        },
    });
