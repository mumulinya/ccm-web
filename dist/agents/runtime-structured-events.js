...
function safeToolArguments(event, item) {
    ...
    for (const key of allowed) {
        ...
        result[key] = Array.isArray(value) ? value.slice(0, 40).map(entry => {
            if (key !== "paths" || !entry || typeof entry !== "object")
                return compact(entry, 500);
...
function safeToolResult(event, item, status) {
...
function normalizedEvidencePath(value) {
    ...
    if (!result || /^\//.test(result) || /^[a-z]:\//i.test(result) || result.split("/").some(part => part === ".."))
        return "";
    return result;
}
function rangeFromArguments(args) {
    const offset = Math.max(1, Number(args.offset || 1));
    const limit = Math.max(0, Number(args.limit || 0));
    return [{ start: offset, end: limit ? offset + limit - 1 : offset + 1_999 }];
}
function inferReadCommand(commandValue) {
    ...
    for (const pattern of wrappers) {
        ...
        if (/\s-[A-Za-z]/.test(candidate))
            return null;
        if (/\s/.test(candidate) && !/^(['"]).*\1$/.test(candidate))
            return null;
        const filePath = normalizedEvidencePath(candidate);
        if (filePath)
            return { path: filePath, ranges: [{ start: 1, end: 2_000 }] };
    }
    ...
    if (sed) {
        ...
        if (filePath)
            return { path: filePath, ranges: [{ start, end }] };
    }
    return null;
}
function fileReadEvidence(event, item, toolName, args) {
    ...
    if (["read", "readfile", "fileread"].includes(normalizedOperation)) {
        ...
        if (!filePath)
            return null;
        return { project, path: filePath, ranges: rangeFromArguments(args), checksum: compact(args.checksum || args.expected_checksum, 160) || undefined, source: "structured_tool", contentStored: false };
    }
    const inferred = inferReadCommand(item?.command ?? args.command);
    if (!inferred)
        return null;
    return { project, ...inferred, source: "safe_command_inference", contentStored: false };
}
...
function toolDescriptor(event) {
    ...
    const target = compact(args?.path || args?.file || args?.file_path || args?.query || args?.pattern || args?.command || "", 240);
    return {
...
function createAgentRuntimeStructuredEventParser(options) {
    ...
    const flushPendingAssistant = (relatedToolCallIds = [], role = "progress") => {
        for (const pending of pendingAssistant)
            emit(pending.rawLine, pending.event, relatedToolCallIds, role);
        pendingAssistant = [];
    };
    const consumeLine = (line) => {
        ...
        try {
            ...
            if (relatedToolCallIds.length)
                flushPendingAssistant(relatedToolCallIds, "progress");
            else if (!assistantEvents.length || role === "final")
                flushPendingAssistant([], role);
