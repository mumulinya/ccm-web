...
const live_workspace_changes_1 = require("../system/live-workspace-changes");
...
function normalizeRuntimeToolSnapshot(request) {
    ...
    return {
        ...
        executionRoleSkills: Array.from(new Set((raw.executionRoleSkills || raw.execution_role_skills || []).map((item) => String(item || "").trim()).filter(Boolean))).sort(),
...
function expectedTaskExecutionRoleSkills(task) {
    if (!task)
        return [];
    const selected = [
    ...
    ].map((item) => String(item || "").trim()).filter(internal_skill_catalog_1.isCcmInternalSkillName);
    const taskText = [task?.title, task?.description, task?.business_goal, task?.acceptance_criteria].filter(Boolean).join("\n");
    return (0, role_skills_1.selectRoleSkills)("project-child-agent", taskText, {
        forceWork: true,
        phase: "execution",
...
function validateRuntimeToolSnapshotV2(request, snapshot, currentScope, options = {}) {
    if (snapshot.schema !== "ccm-runtime-tool-authorization-snapshot-v2")
    ...
    if (!snapshot.authorizationSignature || !(0, internal_mcp_runtime_1.verifyInternalMcpEvidenceSignature)(core, snapshot.authorizationSignature)) {
    ...
    if (String(identity.projectName || "") !== requestProject
        ...
        || (0, runtime_1.normalizeAgentRuntimeId)(identity.runtime || "claudecode") !== requestRuntime
    ...
    const task = requestTaskId
        ? typeof options.loadTask === "function"
            ...
            : (0, db_1.loadTasks)().find((item) => String(item?.id || "") === requestTaskId)
        : null;
    if (requestTaskId && !task)
        return { ok: false, reason: "运行时授权绑定的任务不存在" };
    if (task) {
        ...
        if (taskProject && taskProject !== requestProject)
            return { ok: false, reason: "任务与运行时授权项目不匹配" };
        if (taskGroup !== requestGroup)
            return { ok: false, reason: "任务与运行时授权群聊不匹配" };
        if (taskSession && String(identity.exactSessionId || "") !== taskSession)
            return { ok: false, reason: "任务与运行时授权精确会话不匹配" };
    }
...
function validateExternalRunnerRuntimeToolGate(request, options = {}) {
    ...
    const scopeCheck = v2Check.ok && v2Check.legacy
...
async function runRequest(file) {
    ...
    try {
        ...
        if (runtimeProgressIdentity?.anchorMessageId && runtimeProgressIdentity?.agentRunId && changeSnapshot && !isAgentProbeRequest(request)) {
            liveWorkspaceTracker = (0, live_workspace_changes_1.createLiveWorkspaceChangesTracker)({
                identity: {
                    ...runtimeProgressIdentity,
                    turnId: runtimeProgressIdentity.turnId || runtimeProgressIdentity.turn_id || runtimeProgressIdentity.anchorMessageId,
                },
                projectId: String(request.projectName || runtimeProgressIdentity.project || ""),
                baseline: changeSnapshot,
                intervalMs: 2_000,
                publish: receipt => (0, agent_runtime_progress_1.publishLiveWorkspaceChanges)(runtimeProgressIdentity, receipt),
            });
        }
        runtimeEventParser = runtimeProgressIdentity?.anchorMessageId && runtimeProgressIdentity?.agentRunId
            ? (0, runtime_structured_events_1.createAgentRuntimeStructuredEventParser)({
                ...
                onEvent: event => {
                    (0, agent_runtime_progress_1.projectAgentRuntimeStructuredEvent)(event);
                    if (event.eventType === "file_changed" || event.eventType === "tool_completed" || event.eventType === "tool_failed") {
                        liveWorkspaceTracker?.schedule();
                    }
                },
            }) : null;
        ...
        let nativeSessionRecoveryTriggered = false;
        try {
        ...
        if (effectiveAgentSession?.resumeSession === true && (resumeLaunchError || (commandFailure.failed && isMissingNativeSessionFailure(commandFailure.message)))) {
            ...
            nativeSessionRecoveryTriggered = true;
            effectiveAgentSession = {
                ...effectiveAgentSession,
                persistSession: true,
                resumeSession: false,
        ...
        const projectWorkerCacheAffinity = (0, agent_cache_affinity_1.externalProjectWorkerCacheAffinity)({
            projectId: String(request.projectName || "unassigned-project"),
            exactSessionId: String(request.taskAgentSessionId || request.task_agent_session_id || ""),
            taskId: String(request.taskId || taskId || ""),
            generation: Number(request.generation || 0),
            attempt: Number(request.attempt || 1),
        });
        const projectWorkerCacheStageMetrics = (0, agent_cache_affinity_1.agentCacheStageMetricsFromUsage)(projectWorkerCacheAffinity, normalizedOutput.usage || { reported: false }, {
            capabilityStatus: normalizedOutput.usage?.reported === true
                ? (Number(normalizedOutput.usage?.cacheReadInputTokens || 0) > 0 ? "confirmed" : "unproven")
                : "unproven",
            missReason: nativeSessionRecoveryTriggered
                ? "cold_start_after_native_session_loss"
                : normalizedOutput.usage?.reported === true
                    ? "dynamic_evidence_first_seen"
                    : "external_runtime_unreported",
        });
        const projectWorkerUsage = {
            ...(normalizedOutput.usage || { reported: false, provider: agentType }),
            cacheAffinity: projectWorkerCacheAffinity,
            agentCacheStageMetrics: projectWorkerCacheStageMetrics,
        };
        (0, provider_cache_scope_metrics_1.recordExternalAgentCacheStageMetric)(projectWorkerCacheAffinity, projectWorkerCacheStageMetrics);
