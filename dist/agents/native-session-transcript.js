...
function toolResultOutput(event, options) {
    ...
    // The execution ledger may contain a compact audit projection.  Prefer the
...
function turnFromPair(pair, options) {
    ...
    const call = {
        ...
        arguments: args,
        argumentsChecksum: projectionChecksum(args),
    };
    return {
        turn: {
            text: "",
...
function splitNativeSystemSegments(input) {
    ...
    if (guidance)
        messages.push({ role: "system", contextBlockType: "dynamic_context", content: guidance });
    const mcp = String(input.mcpPolicy || "").trim();
    if (mcp)
        messages.push({ role: "system", contextBlockType: "dynamic_context", content: mcp });
    return messages;
}
...
function materializeNativeSessionTranscript(input) {
    ...
    const summaryText = input.canonicalSummary != null && String(input.canonicalSummary || "").trim()
        ? `【当前精确会话压缩摘要】\n${typeof input.canonicalSummary === "string" ? input.canonicalSummary : JSON.stringify(input.canonicalSummary)}`
        : "";
    const summaryMeta = summaryText ? buildNativeMetaUserMessage([], [summaryText]) : null;
    const summaryPlacement = input.canonicalSummaryPlacement || "after_conversation";
    let summaryInserted = false;
    let messages = summaryMeta && (summaryPlacement === "before_conversation" || (summaryPlacement === "after_message" && !input.canonicalSummaryAfterMessageId))
        ? [summaryMeta]
        : [];
    summaryInserted = messages.length > 0;
    const appendSummaryAfter = (id) => {
        if (!summaryInserted && summaryMeta && summaryPlacement === "after_message" && id === String(input.canonicalSummaryAfterMessageId || "")) {
            messages.push(summaryMeta);
    ...
    const extras = [];
    if (summaryText && !summaryInserted && summaryPlacement === "after_conversation") {
        extras.push(summaryText);
        summaryInserted = true;
    }
    else if (summaryText && !summaryInserted) {
        // A stale partial pivot cannot silently drop the authoritative summary.
        extras.push(summaryText);
        summaryInserted = true;
    }
