...
exports.requestGroupSessionAgentCancellation = requestGroupSessionAgentCancellation;
exports.runManagedCommand = runManagedCommand;
exports.disposeManagedCommandRawOutput = disposeManagedCommandRawOutput;
exports.cleanupOrphanedManagedCommandOutputs = cleanupOrphanedManagedCommandOutputs;
...
const completedAgentRunResources = new Map();
...
function sampleAgentRunResources(run) {
    const pid = Number(run?.pid || 0);
    if (!Number.isInteger(pid) || pid <= 0 || run.resourceSamplePending)
        return;
    run.resourceSamplePending = true;
    const finish = (cpuSeconds, rssBytes, childProcessCount, directCpuPercent = 0) => {
    ...
    if (process.platform === "win32") {
        ...
        (0, child_process_1.execFile)("powershell.exe", ["-NoProfile", "-NonInteractive", "-Command", script], { windowsHide: true, timeout: 4_000 }, (error, stdout) => {
            if (error) {
                run.resourceSamplePending = false;
                return;
            }
            try {
                const row = JSON.parse(String(stdout || "{}"));
                finish(Number(row.cpu || 0), Number(row.rss || 0), Number(row.count || 1));
            }
            catch {
                run.resourceSamplePending = false;
            }
    ...
    (0, child_process_1.execFile)("ps", ["-p", String(pid), "-o", "%cpu=,rss="], { timeout: 3_000 }, (error, stdout) => {
        if (error) {
            run.resourceSamplePending = false;
            return;
        }
...
function finishActiveAgentRun(runId, status = "finished") {
    ...
    run.status = status;
    run.updatedAt = now();
    if (run.resourceTimer)
        clearInterval(run.resourceTimer);
    const summary = resourceSummary(run);
    for (const key of [run.executionId, run.taskId].filter(Boolean))
        completedAgentRunResources.set(String(key), summary);
    while (completedAgentRunResources.size > 300)
        completedAgentRunResources.delete(completedAgentRunResources.keys().next().value);
...
function metricAgentResourceSummary(taskId = "", executionId = "") {
    const active = Array.from(activeAgentRuns.values()).find(run => ((executionId && run.executionId === executionId) || (taskId && run.taskId === taskId)));
    if (active)
        return resourceSummary(active);
...
function requestGroupSessionAgentCancellation(input = {}) {
    ...
    if (!groupId || !groupSessionId.startsWith("gcs_"))
        throw new Error("group session cancellation requires groupId + gcs_* identity");
        ...
        if (fs.existsSync(AGENT_RUNNER_REQUESTS_DIR)) {
            for (const name of fs.readdirSync(AGENT_RUNNER_REQUESTS_DIR).filter(name => name.endsWith(".json"))) {
                ...
                if (!request
                    ...
                    || ["done", "failed", "cancelled", "expired"].includes(String(request.status || "")))
                    continue;
    ...
    const cancellations = Array.from(taskIds).map(taskId => {
        try {
            return requestTaskCancellation(taskId, reason, actor);
        }
        catch (error) {
            return { success: false, taskId, error: error?.message || String(error) };
        }
    });
    return {
...
function disposeManagedCommandRawOutput(result) {
    const outputFile = String(result?.outputFile || "").trim();
    if (!outputFile)
        return { removed: false, bytes: Math.max(0, Number(result?.totalOutputBytes || 0)), checksum: "" };
    let checksum = "";
...
function cleanupOrphanedManagedCommandOutputs(maxAgeMs = 60 * 60_000) {
    ...
    for (const entry of fs.readdirSync(OUTPUTS_DIR, { withFileTypes: true })) {
        ...
        try {
            if (Date.now() - fs.statSync(file).mtimeMs < Math.max(60_000, maxAgeMs))
                continue;
...
function createExecutionCheckpoint(input) {
    ...
    const statusForCheckpoint = runGit(repoRoot, ["status", "--porcelain"]);
    const reservedPaths = (0, workspace_path_safety_1.reservedPathsFromPorcelain)(statusForCheckpoint);
    const addArgs = reservedPaths.length
        ? ["add", "-A", "--", ".", ...reservedPaths.map(filePath => `:(exclude)${filePath}`)]
        : ["add", "-A"];
    runGit(repoRoot, addArgs, env);
...
function inspectBranchFreshness(workDir, baseRef = "") {
    ...
    const baseFull = base.startsWith("origin/") ? base : ((0, child_process_1.spawnSync)("git", ["show-ref", "--verify", "--quiet", `refs/remotes/origin/${base}`], {
        cwd: repoRoot,
        windowsHide: true,
        stdio: "ignore",
...
function runExecutionKernelSelfTest() {
    ...
    try {
        ...
        const checks = {
            ...
            acceptsPartialProviderReceiptOnlyAfterCcmRevalidation: revalidatedPartial.pass && revalidatedPartial.checks.find((item) => item.id === "receipt")?.pass === true,
