...
function commandBlockComplete(item) {
    return !!String(item?.command || "").trim()
        ...
        && !!String(item?.startedAt || "").trim()
        && !!String(item?.finishedAt || "").trim()
...
function signedIsolationBindingFor(item, report, projectRoot) {
    const receipt = report?.metadata?.verificationHardening?.isolationReceipt
        || report?.metadata?.verification_hardening?.isolation_receipt;
    if (receipt?.schema !== "ccm-test-agent-isolation-receipt-v1" || receipt?.mode !== "disposable_copy" || receipt?.cleanup?.status !== "passed")
        return null;
    const root = existingDirectory(projectRoot);
    if (!root)
        return null;
    const reportedCwd = path.resolve(String(item?.cwd || ""));
    return asArray(receipt.projectBindings).find(binding => {
        if (binding?.copied !== true || String(binding?.project || "") !== String(item?.project || ""))
            return false;
        const source = path.resolve(String(binding?.sourceWorkDir || ""));
        const execution = path.resolve(String(binding?.executionWorkDir || ""));
        return source === path.resolve(root) && execution === reportedCwd;
    }) || null;
}
function resolveCommandCwd(item, projectRoot, report) {
    const root = existingDirectory(projectRoot);
    const candidate = existingDirectory(item?.cwd);
    if (!candidate)
        return signedIsolationBindingFor(item, report, projectRoot) ? root : "";
...
function failedCheckFromCandidate(item, projectRoot, report, issue) {
    const reviewOutput = outputText(item);
    const cwd = resolveCommandCwd(item, projectRoot, report);
    return {
...
async function runMainAgentPostReviewSpotCheck(input) {
    ...
    for (const item of candidates) {
        ...
        const cwd = resolveCommandCwd(item, String(input.projectRoot || ""), report);
        ...
        if (!complete || !safe || !cwd) {
            const issue = !complete
                ...
                : !safe
                    ? "命令不符合完成前安全抽查规则。"
                    : "命令工作目录不可用或超出被复核项目。";
            result.checks.push(failedCheckFromCandidate(item, String(input.projectRoot || ""), report, issue));
