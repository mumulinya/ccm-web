...
exports.NATIVE_CONTINUATION_CAPABILITY_PROFILE_SCHEMA = exports.NATIVE_SESSION_CONTINUATION_EVIDENCE_SCHEMA = void 0;
exports.getNativeContinuationCapabilityProfile = getNativeContinuationCapabilityProfile;
exports.buildNativeSessionContinuationEvidence = buildNativeSessionContinuationEvidence;
exports.verifyNativeSessionContinuationEvidence = verifyNativeSessionContinuationEvidence;
const crypto = __importStar(require("crypto"));
exports.NATIVE_SESSION_CONTINUATION_EVIDENCE_SCHEMA = "ccm-native-session-continuation-evidence-v1";
exports.NATIVE_CONTINUATION_CAPABILITY_PROFILE_SCHEMA = "ccm-native-continuation-capability-profile-v1";
const NATIVE_CONTINUATION_PROFILES = {
    claudecode: {
        provider: "claudecode",
...
function getNativeContinuationCapabilityProfile(provider) {
    ...
    return {
        schema: exports.NATIVE_CONTINUATION_CAPABILITY_PROFILE_SCHEMA,
        version: 1,
...
function evidenceChecksum(evidence) {
    const payload = { ...(evidence || {}) };
    delete payload.evidenceChecksum;
    delete payload.checksumValid;
    return checksum(payload);
}
function buildNativeSessionContinuationEvidence(input = {}) {
    ...
    const freshProviderSession = input.freshProviderSession === true || input.fresh_provider_session === true;
    ...
    const evidence = {
        schema: exports.NATIVE_SESSION_CONTINUATION_EVIDENCE_SCHEMA,
        version: 4,
        provider: profile.provider,
...
function verifyNativeSessionContinuationEvidence(evidence, expected = {}) {
    const issues = [];
    if (evidence?.schema !== exports.NATIVE_SESSION_CONTINUATION_EVIDENCE_SCHEMA || Number(evidence?.version || 0) !== 4)
        issues.push("schema_invalid");
    if (String(evidence?.evidenceChecksum || "") !== evidenceChecksum(evidence))
    ...
    if (!allowedSources.has(String(evidence?.evidenceSource || "")))
    ...
    for (const [key, aliases] of Object.entries({
        ...
        const normalizedExpected = key === "provider" && expectedValue
            ...
            : expectedValue;
        if (normalizedExpected && String(evidence?.[key] || "") !== String(normalizedExpected))
...
//# sourceMappingURL=native-continuation.js.map