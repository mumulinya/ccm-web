...
function tryBuildGlobalNativeModelMessages(input) {
    ...
    const history = (0, native_session_transcript_1.materializeNativeSessionTranscript)({
        ...
        canonicalSummaryPlacement: input.continuation?.partialCompaction?.summaryPlacement === "after_preserved" ? "after_message" : input.continuation?.partialCompaction ? "before_conversation" : "after_conversation",
        canonicalSummaryAfterMessageId: input.continuation?.partialCompaction?.summaryPlacement === "after_preserved" ? String(input.continuation.partialCompaction.preservedMessageIds?.at(-1) || "") : "",
