...
exports.normalizeGlobalAgentUserSteer = normalizeGlobalAgentUserSteer;
exports.normalizeGlobalAgentUserSteers = normalizeGlobalAgentUserSteers;
...
exports.findGlobalAgentRunsForTaskIds = findGlobalAgentRunsForTaskIds;
...
exports.hasUserVisibleGlobalPlan = hasUserVisibleGlobalPlan;
exports.classifyGlobalAgentRunPresentation = classifyGlobalAgentRunPresentation;
exports.isReadOnlyGlobalConsultation = isReadOnlyGlobalConsultation;
exports.stable = stable;
exports.toolSignature = toolSignature;
...
exports.activeRuns = new Set();
exports.pauseRequests = new Set();
exports.cancelRequests = new Set();
exports.volatileRuns = new Map();
exports.activeRunObjects = new Map();
let runStoreCache = null;
...
function truncateStepObservation(step, run) {
    ...
    const persisted = (0, tool_result_storage_1.persistToolResultIfNeeded)({
        ...
        context: sessionId ? { scope: "global", scopeId: "global", sessionId } : null,
    });
...
function destructiveOperation(args) {
    return ["delete", "remove", "remove_member", "purge", "drop", "stop", "restart", "build", "connect_agent", "disconnect_agent"].includes(String(args?.operation || "").toLowerCase());
}
...
function normalizeRun(run) {
    ...
    return {
        ...
        status: (["running", "supervising", "paused", "waiting_confirmation", "waiting_clarification", "blocked", "completed", "failed", "cancelled"].includes(run?.status) ? run.status : "failed"),
        ...
        turn_id: String(run?.turn_id || run?.turnId || "") || undefined,
        queue_scope: String(run?.queue_scope || run?.queueScope || "") || undefined,
        write_authorization_receipt: run?.write_authorization_receipt || run?.writeAuthorizationReceipt || null,
        writeAuthorizationReceipt: run?.writeAuthorizationReceipt || run?.write_authorization_receipt || null,
        retryable: run?.retryable === true,
        degraded: run?.degraded === true,
        failure_category: String(run?.failure_category || run?.failureCategory || ""),
        terminal_receipt: run?.terminal_receipt || run?.terminalReceipt || null,
        authorization_message: String(run?.authorization_message || run?.authorizationMessage || run?.original_user_message || run?.user_message || ""),
        ...
        pending_plan: run?.pending_plan || run?.pendingPlan || null,
        presented_plan: run?.presented_plan || run?.presentedPlan || null,
        ...
        loaded_tool_names: Array.from(new Set((Array.isArray(run?.loaded_tool_names) ? run.loaded_tool_names : Array.isArray(run?.loadedToolNames) ? run.loadedToolNames : []).map((value) => String(value || "").trim()).filter(Boolean))).slice(0, 256),
        loadedToolNames: Array.from(new Set((Array.isArray(run?.loadedToolNames) ? run.loadedToolNames : Array.isArray(run?.loaded_tool_names) ? run.loaded_tool_names : []).map((value) => String(value || "").trim()).filter(Boolean))).slice(0, 256),
...
function loadStore() {
    try {
        if (fs.existsSync(exports.STORE_FILE)) {
            const mtimeMs = fs.statSync(exports.STORE_FILE).mtimeMs;
            if (runStoreCache && runStoreCache.mtimeMs === mtimeMs) {
                return { version: 1, runs: runStoreCache.runs.slice() };
            }
            const data = JSON.parse(fs.readFileSync(exports.STORE_FILE, "utf-8"));
    ...
    for (const file of [exports.STORE_BACKUP]) {
        try {
            if (!fs.existsSync(file))
                continue;
...
function saveRun(run, persist = true) {
    if (!persist) {
        exports.volatileRuns.set(run.id, normalizeRun(run));
        if (exports.volatileRuns.size > 50)
            exports.volatileRuns.delete(exports.volatileRuns.keys().next().value);
        return;
    }
...
function findGlobalAgentRunsForTaskIds(taskIds) {
...
function hasUserVisibleGlobalPlan(run) {
    ...
    if (plan) {
        ...
        if (steps.length)
            return true;
        if (String(plan.content || plan.summary || plan.markdown || plan.title || plan.next_step || "").trim())
            return true;
        if (Array.isArray(plan.clarification_questions) && plan.clarification_questions.length)
            return true;
    }
    ...
    if (Array.isArray(decision?.user_plan_steps) && decision.user_plan_steps.length)
        return true;
    return false;
}
function classifyGlobalAgentRunPresentation(run, status = run?.status) {
    const explicit = String(run?.presentation || "").trim().toLowerCase();
    if (explicit === "reply" || explicit === "plan" || explicit === "delivery")
        return explicit;
    const normalizedStatus = String(status || run?.status || "").toLowerCase();
    if (run?.mission_id || run?.supervisor_id)
        return "delivery";
    if (normalizedStatus === "supervising")
        return "delivery";
    const tools = (Array.isArray(run?.steps) ? run.steps : [])
        .map((step) => step?.tool)
        .filter(Boolean)
        .concat(run?.pending_tool ? [run.pending_tool] : []);
    const hasWriteOrDispatch = tools.some((tool) => {
        const risk = String(tool?.risk || "").toLowerCase();
        const name = String(tool?.name || "");
        return risk === "write" || risk === "high" || exports.GLOBAL_DISPATCH_TOOL_NAMES.includes(name);
    });
    if (hasWriteOrDispatch)
        return "delivery";
    ...
    if (toolCalls === 0 && !tools.length)
        return "reply";
    const allLight = tools.length > 0 && tools.every((tool) => {
        const risk = String(tool?.risk || "").toLowerCase();
        const name = String(tool?.name || "");
        return risk === "read" || exports.LIGHT_UI_TOOL_NAMES.includes(name);
    });
    // 轻量 UI/只读工具优先 reply；勿因误挂的 plan_mode 升成 plan 卡
    if (allLight)
        return "reply";
    if (Array.isArray(run?.client_effects) && run.client_effects.length && !hasWriteOrDispatch)
        return "reply";
    if (hasUserVisibleGlobalPlan(run))
        return "plan";
    return "reply";
}
function isReadOnlyGlobalConsultation(run, status) {
    if (status !== "completed" || run.mission_id)
        return false;
...
exports.GLOBAL_AGENT_TOOL_SPECS = [
    { name: "tool_search", description: "按需发现本轮尚未加载的低频只读或管理工具。", required: ["query"], risk: "read" },
    ...
    { name: "read_scope_instruction", description: "按 documentId 读取当前全局目录中已授权的项目或群聊认知文档；正文仅进入本轮上下文。", required: ["documentId"], risk: "read" },
    { name: "request_project_source_inquiry", description: "委托指定项目主 Agent只读核对当前源码，返回安全结论、相对路径和证据引用；全局 Agent本身不读取项目文件。", required: ["project", "question", "read_depth"], risk: "read" },
    { name: "request_group_source_inquiry", description: "委托指定群聊主 Agent只读核对其授权成员项目源码，适用于群聊归属或多项目问题；全局 Agent本身不读取项目文件。", required: ["group", "question", "read_depth"], risk: "read" },
    ...
    { name: "read_global_shared_files", description: "按需读取全局 Agent 已授权共享文件的完整受控上下文与分片清单。普通问候和自包含问答不要调用。", risk: "read" },
    ...
    { name: "orchestrate_development", description: "创建跨项目开发任务并持久派发给真实群聊或项目 Agent。群聊目标可附带 group_session_id（gcs_ 开头）绑定该群的精确会话；缺省时系统绑定该群当前可写会话。", required: ["business_goal", "targets"], risk: "write" },
    ...
    { name: "create_task", description: "创建并派发单群聊开发任务。可选 group_session_id（gcs_ 开头）绑定该群的精确会话；缺省时系统绑定该群当前可写会话。", required: ["title", "business_goal", "group_id"], risk: "write" },
