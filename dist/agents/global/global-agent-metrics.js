...
function alreadyRecorded(executionId) {
    if (!executionId)
        return false;
    try {
        return (0, metrics_v3_1.hasMetricExecutionV3)(executionId, "global", GLOBAL_AGENT_NAME);
    }
    catch {
        return false;
    }
...
function recordGlobalAgentRunMetric(run, status, options = {}) {
    ...
    const firstRoundReads = (Array.isArray(run?.steps) ? run.steps : []).filter((step) => /^(?:read_file|read_files|glob_files|grep_text)$/i.test(String(step?.tool?.name || "")) && Number(step?.index || 0) <= 1);
    streamingMetric.initialReadFileCount = firstRoundReads.reduce((count, step) => count + Math.max(1, Number(step?.observation?.safeReceipt?.itemCount || step?.observation?.itemCount || 0)), 0);
    streamingMetric.initialReadTokens = firstRoundReads.reduce((count, step) => count + Math.max(0, Number(step?.observation?.outputTokens || step?.output_tokens || 0)), 0);
