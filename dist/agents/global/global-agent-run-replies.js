...
exports.stripNonExecutionReportSections = stripNonExecutionReportSections;
exports.hasGlobalUserSummaryTechnicalDetails = hasGlobalUserSummaryTechnicalDetails;
exports.compactGlobalUserSummaryText = compactGlobalUserSummaryText;
exports.uniqueGlobalStrings = uniqueGlobalStrings;
exports.sanitizeGlobalVisibleReplyTerminology = sanitizeGlobalVisibleReplyTerminology;
exports.globalVisibleReplyFallback = globalVisibleReplyFallback;
exports.buildGlobalVisibleReplyContent = buildGlobalVisibleReplyContent;
exports.attachGlobalReplyTechnicalContent = attachGlobalReplyTechnicalContent;
exports.getGlobalToolUserLabel = getGlobalToolUserLabel;
exports.summarizeGlobalToolTarget = summarizeGlobalToolTarget;
...
exports.buildGlobalPlanSteps = buildGlobalPlanSteps;
exports.buildGlobalStructuredPlanModeSummary = buildGlobalStructuredPlanModeSummary;
exports.buildGlobalPlanExecutionFollowup = buildGlobalPlanExecutionFollowup;
exports.buildGlobalPlanModeSummary = buildGlobalPlanModeSummary;
exports.updateGlobalPlanModeStatus = updateGlobalPlanModeStatus;
exports.sanitizeGlobalDispatchVisibleText = sanitizeGlobalDispatchVisibleText;
exports.normalizeDispatchDependency = normalizeDispatchDependency;
exports.buildGlobalDispatchRow = buildGlobalDispatchRow;
...
exports.GLOBAL_USER_SUMMARY_INTERNAL_PATTERN = /CCM_AGENT_RECEIPT|CCM_AGENT_REQUESTS|<\s*\/?\s*task-notification|task-notification|receipt[-_\s]*status|trace_id|session_id|run_id|WorkerContextPacket|raw[_\s-]*payload|raw[_\s-]*receipt|raw_report|Runtime Kernel|workflow_timeline|native_session|task_agent_session|scratchpad|execution_lease|workchain|shouldDelegate|Trace Replay|回执要求/i;
exports.GLOBAL_USER_SUMMARY_TECHNICAL_EVIDENCE_PATTERN = /(?:[A-Za-z]:[\\/][^\r\n]*(?:test-agent-artifacts|artifact-manifest\.json|report\.md|report\.json|verdict\.json))|(?:test-agent-artifacts|artifact-manifest\.json|verdict\.json|raw[_\s-]*stack|stack\s+trace)/i;
function hasGlobalUserSummaryTechnicalDetails(value) {
    const text = String(value || "");
    return exports.GLOBAL_USER_SUMMARY_INTERNAL_PATTERN.test(text) || exports.GLOBAL_USER_SUMMARY_TECHNICAL_EVIDENCE_PATTERN.test(text);
}
...
function getGlobalToolUserLabel(toolName) {
    const labels = {
        ...
        request_project_source_inquiry: "请求项目主 Agent 核对源码",
        request_group_source_inquiry: "请求群聊主 Agent 核对源码",
        list_groups: "读取群聊列表",
        list_tasks: "读取任务列表",
        list_cron: "读取定时任务",
...
function buildGlobalPlanSteps(decision, toolName = "") {
    const structuredSteps = !Array.isArray(decision.plan) && Array.isArray(decision.plan?.steps)
        ? decision.plan.steps
        : [];
    const rawSteps = Array.isArray(decision.plan)
        ? decision.plan
        : structuredSteps.map((item) => item?.title || item?.label || item?.objective || item?.id || "").filter(Boolean);
    const fallbackSteps = [
...
function buildGlobalStructuredPlanModeSummary(input) {
    const plan = input.decision.plan && !Array.isArray(input.decision.plan) ? input.decision.plan : {};
    const requirement = plan.businessRequirement || {};
    const steps = (Array.isArray(plan.steps) ? plan.steps : []).slice(0, 24).map((item, index) => ({
        id: String(item?.id || `global-plan-step-${index + 1}`),
        label: compactGlobalUserSummaryText(item?.title || item?.label || item?.objective, `计划步骤 ${index + 1}`, 180),
        detail: compactGlobalUserSummaryText(item?.objective || item?.detail || "", "", 420),
        status: "pending",
        projects: uniqueGlobalStrings(Array.isArray(item?.projects) ? item.projects : [item?.project]).slice(0, 12),
        acceptance_criterion_ids: uniqueGlobalStrings(Array.isArray(item?.acceptanceCriterionIds) ? item.acceptanceCriterionIds : []).slice(0, 24),
    }));
    const projects = uniqueGlobalStrings([
        ...(Array.isArray(requirement?.targetProjects) ? requirement.targetProjects : []),
        ...(Array.isArray(plan.files) ? plan.files.map((item) => item?.project) : []),
        ...steps.flatMap((item) => item.projects || []),
    ]).slice(0, 20);
    const acceptance = (Array.isArray(requirement?.acceptanceCriteria) ? requirement.acceptanceCriteria : [])
        .map((item) => compactGlobalUserSummaryText(item?.description || item?.title || item, "", 360))
        .filter(Boolean)
        .slice(0, 24);
    return {
        ...
        confirmation_kind: "session_plan",
        source: "ccm-implementation-plan-v2",
        requirement: compactGlobalUserSummaryText(requirement?.businessGoal || plan.goal || input.run.original_user_message || input.run.user_message, "本轮全局任务", 720),
        action: compactGlobalUserSummaryText(plan.title || input.decision.message, "按已复核计划执行", 180),
        steps: steps.length ? steps : buildGlobalPlanSteps(input.decision),
        acceptance,
        risk: {
            level: "medium",
            summary: "计划已通过复核，确认后才会编译并派发执行。",
            reasons: uniqueGlobalStrings(Array.isArray(plan.risks) ? plan.risks : []).slice(0, 12),
        },
        impact_scope: { projects, areas: uniqueGlobalStrings(Array.isArray(plan.scope) ? plan.scope : []).slice(0, 16), multi_agent: projects.length > 1 },
        exclusions: uniqueGlobalStrings(Array.isArray(plan.exclusions) ? plan.exclusions : []).slice(0, 20),
        requires_confirmation: true,
        auto_continue: false,
        confirmation_status: "awaiting_confirmation",
        next_step: "确认后将按当前 revision 和 checksum 编译工作单，并派发到目标自动化任务会话。",
        display_policy: {
            user_text_first: true,
            technical_default_collapsed: true,
            hide_internal_protocols: true,
        ...
        technical: {
            ...
            plan_revision: Math.max(1, Number(plan.revision || 1)),
            plan_checksum: String(plan.checksum || ""),
            requirement_id: String(plan.requirementBinding?.requirementId || requirement?.requirementId || ""),
            requirement_revision: Math.max(1, Number(plan.requirementBinding?.revision || requirement?.revision || 1)),
            requirement_checksum: String(plan.requirementBinding?.checksum || requirement?.checksum || ""),
...
exports.GLOBAL_DISPATCH_VISIBLE_TEXT_PATTERN = /CCM_AGENT_RECEIPT|CCM_AGENT_REQUESTS|WorkerContextPacket|task-notification|receipt[-_\s]*status|trace_id|session_id|run_id|native_session|task_agent_session|raw[_\s-]*payload|raw[_\s-]*receipt|scratchpad|Runtime Kernel|Trace Replay|回执要求/i;
function sanitizeGlobalDispatchVisibleText(value, fallback = "派发信息已整理，技术细节已放入技术详情。", max = 260) {
    ...
    if (exports.GLOBAL_DISPATCH_VISIBLE_TEXT_PATTERN.test(text))
        return (0, user_facing_text_1.sanitizeMainAgentRoleLanguage)(fallback);
...
function isGlobalDispatchTool(name) {
    return global_agent_run_store_1.GLOBAL_DISPATCH_TOOL_NAMES.includes(String(name || ""));
}
...
//# sourceMappingURL=global-agent-run-replies.js.map