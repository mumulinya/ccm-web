...
exports.runGlobalAgentLoopSelfTest = exports.updateGlobalAgentSupervisionState = exports.globalSupervisionStateVisibleSummary = exports.completeGlobalAgentSupervision = exports.attachGlobalAgentRunSupervision = void 0;
...
function emit(runtime, event, run) {
    ...
    try {
        ...
        const globalAcceptancePassed = !!run.mission_id && (run.final_report?.acceptance_gate_passed === true
        ...
        const formalIntentWithoutMission = !run.mission_id && (run.workflow_decision?.requiresCodeChanges === true
            || (run.workflow_decision?.actionRequired === true && run.workflow_decision?.readAction !== "inspect_source"));
        const result = ["completed", "failed", "cancelled", "blocked", "paused", "interrupted"].includes(sourceType)
            ? (0, user_visible_agent_events_1.buildUserVisibleAgentResult)({
                // Finishing the model loop is not the same thing as completing a
                // requested development task. Without a real mission/task ledger and
                // Terminal Gate, a write-intent turn must remain un-delivered.
                status: sourceType === "completed" ? (formalIntentWithoutMission ? "blocked" : "success") : sourceType,
                ...
                blockers: run.final_report?.blockers || run.final_delivery_report?.blockers
                    || (formalIntentWithoutMission ? ["正式开发任务尚未创建或派发，未通过 Terminal Gate。"] : []),
            })
            : undefined;
        ...
        const eventInput = {
            ...
            turnId: String(run.id || ""),
            generation: Math.max(0, Number(run.resume_count || 0)),
            attempt: Math.max(1, Number(run.resume_count || 0) + 1),
...
function appendGlobalRequirementPlan(run, decision, terminalStatus) {
    ...
    const structuredSteps = decision?.plan && !Array.isArray(decision.plan) && Array.isArray(decision.plan.steps)
        ? decision.plan.steps
        : [];
    const planSteps = Array.isArray(decision?.plan) && decision.plan.length
        ...
        : structuredSteps.length
            ? structuredSteps.map((step) => String(step?.title || step?.label || step?.objective || step?.id || "").trim()).filter(Boolean)
    ...
    if (!planSteps.length || (!actionRequired && !terminalStatus && !current))
        return null;
    const runtimeDebug = (0, runtime_1.buildGlobalAgentSessionDebug)(run);
    const runtimeTodos = Array.isArray(runtimeDebug?.todos) ? runtimeDebug.todos : [];
    const todoByTitle = new Map(runtimeTodos.map((todo) => [String(todo?.text || "").trim(), String(todo?.status || "")]));
    const currentByTitle = new Map((Array.isArray(current?.steps) ? current.steps : []).map((step) => [String(step?.title || "").trim(), String(step?.status || "pending")]));
    const blockedStepIndex = Math.max(0, planSteps.findIndex((title) => {
    ...
    const stepStatuses = planSteps.map((title, index) => {
        if (terminalStatus === "completed")
            return "completed";
        if (terminalStatus === "blocked") {
            if (index === blockedStepIndex)
                return "blocked";
...
function globalDecisionPlanSteps(decision) {
    if (Array.isArray(decision?.plan))
        return decision.plan.map(String).filter(Boolean);
    const rows = Array.isArray(decision?.plan?.steps) ? (decision?.plan).steps : [];
    return rows.map((item) => String(item?.title || item?.label || item?.objective || item?.id || "").trim()).filter(Boolean);
}
function markGlobalAgentToolTodo(run, tool, status, text = "") {
    const todos = (0, runtime_1.markGlobalAgentToolTodo)(run, tool, status, text);
...
function completeRun(run, runtime, status, reply, error = "") {
    ...
    run.pending_tool = null;
    run.pending_plan = null;
    run.terminal_receipt = (0, global_terminal_delivery_1.createGlobalRunTerminalReceipt)({
        id: "",
        mission_id: run.mission_id || "",
...
async function continueLoop(run, runtime) {
    ...
    const pauseAtSafeBoundary = (reply = "已在最近安全检查点暂停，运行上下文已经保留。") => {
        ...
        run.pause_control = {
            ...
            state: "paused",
            sequence: Math.max(0, Number(run.pause_control?.sequence || 0)),
            requested_at: String(run.pause_control?.requested_at || nowIso(runtime)),
        ...
        run.updated_at = nowIso(runtime);
        saveRun(run, runtime.persist !== false);
        (0, reliability_ledger_1.appendTraceEvent)(run.trace_id, { id: `${run.id}:paused:${run.updated_at}`, type: "global_agent.paused", status: "warning", message: reply, data: { phase: run.phase, pause_sequence: run.pause_control.sequence } });
    ...
    try {
        ...
        while (run.status === "running") {
            ...
            if (pausedAtLoopBoundary)
                return pausedAtLoopBoundary;
            ...
            try {
                ...
                if (pausedAfterModelFailure)
                    return pausedAfterModelFailure;
                if (applyPendingGlobalAgentUserSteers(run, runtime).length)
                    continue;
                const providerFailure = (0, model_retry_presentation_1.modelProviderFailurePresentation)(error);
                if (providerFailure.presentable) {
                    run.provider_retry_count = providerFailure.retryCount;
                    run.providerRetryCount = providerFailure.retryCount;
                    return completeRun(run, runtime, "failed", providerFailure.text, error?.message || String(error));
                }
            ...
            const decisionPlanSteps = globalDecisionPlanSteps(decision);
            (0, reasoning_loop_1.updateReasoningPlan)(run.reasoning_loop, decisionPlanSteps, normalizedIntent.reason || `decision:${decision.state}`);
            (0, runtime_1.updateGlobalAgentTodoLedger)(run, decisionPlanSteps, decision.tool?.name || "");
            ...
            if (!decision.tool) {
                ...
                if (decision.state === "plan") {
                    const plan = decision.plan && !Array.isArray(decision.plan) ? decision.plan : null;
                    const planRevision = Math.max(1, Number(plan?.revision || 1));
                    const planChecksum = String(plan?.checksum || "").trim();
                    if (!plan || plan?.schema !== "ccm-implementation-plan-v2" || !planChecksum) {
                        markGlobalAgentToolTodo(run, "", "blocked", "计划缺少有效的 revision 或 checksum");
                        return completeRun(run, runtime, "failed", "计划结构或版本绑定不完整，无法提交确认。", "invalid_presented_plan_binding");
                    }
                    const confirmedAt = nowIso(runtime);
                    run.status = "waiting_confirmation";
                    run.phase = "plan";
                    run.pending_tool = null;
                    run.presented_plan = plan;
                    run.pending_plan = {
                        schema: "ccm-global-pending-plan-v1",
                        plan,
                        plan_id: String(plan.planId || ""),
                        plan_revision: planRevision,
                        plan_checksum: planChecksum,
                        requirement_id: String(plan.requirementBinding?.requirementId || plan.businessRequirement?.requirementId || ""),
                        requirement_revision: Math.max(1, Number(plan.requirementBinding?.revision || plan.businessRequirement?.revision || 1)),
                        requirement_checksum: String(plan.requirementBinding?.checksum || plan.businessRequirement?.checksum || ""),
                        source_manifest_checksum: String(plan.sourceManifestChecksum || ""),
                        contentStored: false,
                    };
                    run.plan_mode = buildGlobalStructuredPlanModeSummary({ run, decision });
                    run.final_reply = decision.message || "计划已经完成复核，等待你确认后执行。";
                    run.confirmation_summary = {
                        ...
                        status: "waiting_user",
                        status_label: "计划已复核",
                        headline: "计划已通过复核，确认前不会派发写入任务。",
                        action: "确认并执行当前计划",
                        risk: "write",
                        risk_label: "写入操作",
                        target: uniqueGlobalStrings(Array.isArray(plan.businessRequirement?.targetProjects) ? plan.businessRequirement.targetProjects : []).join("、"),
                        question: "是否确认按当前计划执行？",
                        reason: compactGlobalUserSummaryText(plan.planningReason || plan.context || decision.message, "计划已绑定当前源码证据和验收标准。", 360),
                        answer_suggestions: ["确认并执行：锁定当前 revision/checksum 后派发。", "取消：停止本次计划，不创建写入任务。"],
                        next_action: "确认后继续同一个全局运行，并按已确认计划派发到目标自动化任务会话。",
                        display_policy: { user_text_first: true, technical_default_collapsed: true, hide_internal_protocols: true, show_todo: false, show_for_ordinary_conversation: false },
                        technical: { run_id: run.id, trace_id: run.trace_id, plan_revision: planRevision, plan_checksum: planChecksum, source: "global-agent-loop", contentStored: false },
                    };
                    run.clarification_summary = null;
                    run.presentation = "plan";
                    run.updated_at = confirmedAt;
                    saveRun(run, runtime.persist !== false);
                    (0, reliability_ledger_1.appendTraceEvent)(run.trace_id, { id: `${run.id}:plan_confirmation:${planRevision}:${planChecksum.slice(0, 12)}`, type: "global_agent.plan_confirmation_required", status: "warning", message: run.final_reply, data: { plan_revision: planRevision, plan_checksum: planChecksum } });
                    emit(runtime, { type: "plan_mode_ready", message: run.final_reply, plan_mode: run.plan_mode, planMode: run.plan_mode }, run);
                    emit(runtime, { type: "confirmation_required", reply: run.final_reply, pending_plan: run.pending_plan, confirmation_summary: run.confirmation_summary, confirmationSummary: run.confirmation_summary, plan_mode: run.plan_mode, planMode: run.plan_mode }, run);
                    (0, runtime_1.recordGlobalAgentRuntimeOutput)(run, { type: "plan_mode_ready", plan_revision: planRevision, plan_checksum: planChecksum, requires_confirmation: true });
                    return run;
                }
            ...
            if (quality.requiresClarification) {
                const semanticMismatchOnly = quality.groundedTarget === true
                    && quality.clarificationReasons.length > 0
                    && quality.clarificationReasons.every(reason => reason === "当前语义属于咨询或分析，却拟调用写工具"
                        || reason === "决策没有确认用户要求实际执行");
                const semanticRepairCount = Math.max(0, Number(run.decision_semantic_repair_count || 0));
                if (semanticMismatchOnly && semanticRepairCount < 1) {
                    run.decision_semantic_repair_count = semanticRepairCount + 1;
                    step.error = "模型提出了写工具，但结构化意图错误地标记为咨询；要求模型在本轮内修正，不打扰用户。";
                    step.observation = {
                        success: false,
                        code: "CCM_DECISION_SEMANTICS_REPAIR_REQUIRED",
                        required: {
                            category: "execution",
                            action_required: true,
                            authorization_basis: "current_message",
                            grounded_targets: quality.targets,
                        },
                        contentStored: false,
                    };
                    run.steps.push(step);
                    run.updated_at = nowIso(runtime);
                    saveRun(run, runtime.persist !== false);
                    (0, reasoning_loop_1.recordReasoningDeviation)(run.reasoning_loop, "decision_semantics_repair", quality.clarificationReasons.join("；"), "warning");
                    continue;
                }
            ...
            const receiptAuthorization = (0, global_agent_authorization_1.globalWriteAuthorizationAllowsTool)({ run, tool: decision.tool.name, args, risk });
            const approved = run.approved_tool_signatures.includes(signature) || permission.allowed;
            const requiresUserConfirmation = (run.pre_plan_clarification_resolved === true && risk !== "read")
            ...
            const confirmedStructuredPlanBound = String(run?.plan_mode?.confirmation_status || "") === "confirmed"
            ...
            // Do not replace the user-confirmed structured plan with a generic
            // per-tool auto-continue summary. The confirmed revision/checksum must
            // remain authoritative for every later dispatch and repair round.
            if (shouldExposePlanMode && !confirmedStructuredPlanBound) {
            ...
            if ((0, assistant_progress_1.assistantProgressNarrationEnabled)(progressConfig)) {
                ...
                const decisionProgress = (0, assistant_progress_1.sanitizeAssistantProgressText)((0, user_facing_text_1.sanitizeMainAgentUserFacingText)(String(decision.message || "")));
            ...
            try {
                ...
                if (toolSucceeded && (0, assistant_progress_1.assistantProgressNarrationEnabled)(progressConfig)) {
                    const outcomeProgress = (0, assistant_progress_1.buildToolBatchOutcomeProgress)([{ name: decision.tool.name, ok: true, output: result }], {
...
async function resumeGlobalAgentRun(id, runtime, options = {}) {
    ...
    if (run.status === "waiting_confirmation") {
        if (options.approved !== true)
            return run;
        if (run.pending_plan && !run.pending_tool?.signature) {
            const pendingPlan = run.pending_plan;
            const plan = pendingPlan.plan || run.presented_plan || null;
            const confirmed = (0, planning_orchestrator_1.confirmPlanningSession)({
                scope: "global",
                scopeId: "global",
                exactSessionId: run.session_id,
                planRevision: Math.max(1, Number(pendingPlan.plan_revision || plan?.revision || 1)),
                planChecksum: String(pendingPlan.plan_checksum || plan?.checksum || ""),
            });
            if (!confirmed.ok) {
                const error = new Error(`计划确认失败：${confirmed.code}`);
            ...
            const confirmedPlanContext = {
                schema: "ccm-confirmed-global-plan-binding-v1",
                planId: String(pendingPlan.plan_id || plan?.planId || ""),
                planRevision: Math.max(1, Number(pendingPlan.plan_revision || plan?.revision || 1)),
                planChecksum: String(pendingPlan.plan_checksum || plan?.checksum || ""),
                requirementId: String(pendingPlan.requirement_id || plan?.requirementBinding?.requirementId || ""),
                requirementRevision: Math.max(1, Number(pendingPlan.requirement_revision || plan?.requirementBinding?.revision || 1)),
                requirementChecksum: String(pendingPlan.requirement_checksum || plan?.requirementBinding?.checksum || ""),
                sourceManifestChecksum: String(pendingPlan.source_manifest_checksum || plan?.sourceManifestChecksum || ""),
                businessGoal: String(plan?.businessRequirement?.businessGoal || plan?.goal || ""),
                targetProjects: Array.isArray(plan?.businessRequirement?.targetProjects) ? plan.businessRequirement.targetProjects : [],
                steps: Array.isArray(plan?.steps) ? plan.steps.map((item) => ({ id: item?.id, title: item?.title, objective: item?.objective, projects: item?.projects, acceptanceCriterionIds: item?.acceptanceCriterionIds, acceptance: item?.acceptance })) : [],
                exclusions: Array.isArray(plan?.exclusions) ? plan.exclusions : [],
                userFeedback: acceptFeedback,
                contentStored: false,
            };
            run.history.push({ role: "assistant", content: `已提交计划供确认：${JSON.stringify(confirmedPlanContext)}` }, { role: "user", content: acceptFeedback ? `已确认执行当前计划。补充要求：${acceptFeedback}` : "已确认执行当前计划。请严格按已确认的 revision、checksum、项目范围和验收标准继续派发。" });
            run.history = run.history.slice(-12);
            run.explicit_write_authorization = true;
            run.status = "running";
            run.phase = "execute";
            run.confirmation_summary = null;
            run.clarification_summary = null;
            ...
            run.resume_count += 1;
            run.updated_at = confirmedAt;
            (0, reasoning_loop_1.captureReasoningFacts)(run.reasoning_loop, "confirmed_plan_binding", confirmedPlanContext);
            (0, reasoning_loop_1.setReasoningAssertion)(run.reasoning_loop, {
                id: "confirmed_plan_binding",
                label: "用户确认的计划版本已绑定到后续派发",
                kind: "intent",
                status: "passed",
                evidence: [confirmedPlanContext.planChecksum],
                reason: "服务端已校验 planning session、revision、checksum 和复核回执",
            });
            saveRun(run, runtime.persist !== false);
            (0, reliability_ledger_1.appendTraceEvent)(run.trace_id, { id: `${run.id}:plan_confirmed:${confirmedPlanContext.planRevision}:${confirmedPlanContext.planChecksum.slice(0, 12)}`, type: "global_agent.plan_confirmed", status: "ok", message: acceptFeedback ? "用户已确认计划并补充执行要求" : "用户已确认计划", data: confirmedPlanContext });
            (0, runtime_1.recordGlobalAgentRuntimeOutput)(run, { type: "plan_confirmed", plan_revision: confirmedPlanContext.planRevision, plan_checksum: confirmedPlanContext.planChecksum, requirement_checksum: confirmedPlanContext.requirementChecksum, has_accept_feedback: !!acceptFeedback });
            emit(runtime, { type: "plan_confirmed", plan_mode: run.plan_mode, planMode: run.plan_mode }, run);
            return continueLoop(run, runtime);
        }
        if (!run.pending_tool?.signature)
            throw new Error("等待确认的工具或计划信息不完整");
        ...
        if (resumedFromPause) {
            ...
            run.last_pause_control = run.pause_control || null;
            run.pause_control = {
                ...(run.pause_control || {}),
                state: "resuming",
            ...
            (0, reliability_ledger_1.appendTraceEvent)(run.trace_id, { id: `${run.id}:resumed:${resumedAt}`, type: "global_agent.resumed_from_pause", status: "ok", message: "已从暂停处原位继续", data: { phase: run.phase, pause_sequence: run.pause_control?.sequence || 0 } });
        }
...
async function continueGlobalAgentRunWithClarification(id, answer, runtime, options = {}) {
    ...
    run.pre_plan_clarification_resolved = true;
    run.prePlanClarificationResolved = true;
    if (options.writeAuthorizationReceipt) {
        run.write_authorization_receipt = options.writeAuthorizationReceipt;
        run.writeAuthorizationReceipt = options.writeAuthorizationReceipt;
        run.authorization_message = clarification;
        run.turn_id = String(options.turnId || run.turn_id || "");
    }
    if (revokesAuthorization)
        (0, global_agent_authorization_1.revokeGlobalWriteAuthorization)(run);
    run.status = "running";
    run.phase = "plan";
...
function pauseGlobalAgentRun(id) {
    ...
    run.pause_control = {
        schema: "ccm-global-run-pause-control-v1",
        state: "requested",
        sequence: Math.max(0, Number(run.pause_control?.sequence || 0)) + 1,
    ...
    saveRun(run, !volatileRuns.has(id));
    (0, reliability_ledger_1.appendTraceEvent)(run.trace_id, { id: `${run.id}:pause-requested:${run.updated_at}`, type: "global_agent.pause_requested", status: "warning", message: "正在暂停，等待当前操作安全收口", data: { phase: run.phase, pause_sequence: run.pause_control.sequence } });
    return run;
}
...
async function recoverInterruptedGlobalAgentRuns(runtime) {
    ...
    for (const stored of candidates) {
        const run = normalizeRun(stored);
        if (["requested", "quiescing", "paused"].includes(String(run.pause_control?.state || ""))) {
            run.status = "paused";
            run.pause_control = {
                ...run.pause_control,
                state: "paused",
                paused_at: run.pause_control?.paused_at || nowIso(runtime),
                pending_writer_count: 0,
            };
            run.updated_at = nowIso(runtime);
            saveRun(run, runtime.persist !== false);
            (0, reliability_ledger_1.appendTraceEvent)(run.trace_id, { id: `${run.id}:pause-recovered:${run.updated_at}`, type: "global_agent.pause_recovered", status: "warning", message: "服务重启后保持安全暂停，不自动续跑" });
            results.push(run);
            continue;
        }
        ...
        try {
            ...
            if (boundary === false || (typeof boundary === "object" && boundary?.valid !== true))
                throw new Error("恢复时上下文边界无法证明");
            ...
            run.resume_count += 1;
            run.explicit_write_authorization = false;
            run.write_authorization_receipt = null;
            run.writeAuthorizationReceipt = null;
            (0, runtime_1.recordGlobalAgentRuntimeOutput)(run, { type: "recovered", status: "running", authorization_revalidation_required: true });
            ...
            run.updated_at = nowIso(runtime);
            saveRun(run, runtime.persist !== false);
            (0, reliability_ledger_1.appendTraceEvent)(run.trace_id, { id: `${run.id}:recovery-blocked:${run.updated_at}`, type: "global_agent.recovery_blocked", status: "warning", message: run.final_reply });
            results.push(run);
        }
    }
...
exports.runGlobalAgentLoopSelfTest = (0, global_agent_loop_self_tests_1.createGlobalAgentLoopSelfTest)({ GLOBAL_USER_SUMMARY_INTERNAL_PATTERN, applyGlobalAgentSupervisionSteer, attachGlobalAgentRunSupervision: exports.attachGlobalAgentRunSupervision, buildGlobalDispatchLaunchSummary, completeGlobalAgentSupervision: exports.completeGlobalAgentSupervision, continueGlobalAgentRunWithClarification, parseGlobalAgentDecision, pauseGlobalAgentRun, resumeGlobalAgentRun, runMainAgentDeliveryReportSelfTest: delivery_report_1.runMainAgentDeliveryReportSelfTest, runMainAgentWorkchainSelfTest: workchain_1.runMainAgentWorkchainSelfTest, startGlobalAgentRun, steerGlobalAgentRun, updateGlobalAgentSupervisionState: exports.updateGlobalAgentSupervisionState });
