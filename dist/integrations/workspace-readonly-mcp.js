...
function capabilityFromContext(context) {
    ...
    if (scope === "global")
        throw new Error("全局主 Agent禁止使用项目工作区 MCP");
    const scopeId = scope === "group" ? String(context.groupId || "") : String(context.project || "");
    const exactSessionId = scope === "group"
        ? String(context.groupSessionId || context.taskId || "")
        : String(context.projectSessionId || context.taskId || "");
    const allowedProjects = scope === "group" ? (context.projects || []).map(project => project.name) : [context.project];
