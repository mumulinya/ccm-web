...
exports.MEMORY_CONTEXT_CONSUMPTION_RECEIPT_SCHEMA = exports.MEMORY_CONTEXT_CONSUMPTION_CHALLENGE_SCHEMA = void 0;
exports.createMemoryContextConsumptionChallenge = createMemoryContextConsumptionChallenge;
exports.verifyMemoryContextConsumptionChallenge = verifyMemoryContextConsumptionChallenge;
exports.memoryContextConsumptionReceiptFile = memoryContextConsumptionReceiptFile;
exports.memoryContextConsumptionReceiptDirectory = memoryContextConsumptionReceiptDirectory;
exports.inspectMemoryContextConsumptionReceiptFile = inspectMemoryContextConsumptionReceiptFile;
exports.memoryContextConsumptionChallengeReferenceState = memoryContextConsumptionChallengeReferenceState;
exports.removeMemoryContextConsumptionReceiptIfUnreferenced = removeMemoryContextConsumptionReceiptIfUnreferenced;
exports.reconcileMemoryContextConsumptionReceipts = reconcileMemoryContextConsumptionReceipts;
exports.attachMemoryContextConsumptionChallenge = attachMemoryContextConsumptionChallenge;
exports.recordMemoryContextConsumptionReceipt = recordMemoryContextConsumptionReceipt;
exports.readMemoryContextConsumptionReceipt = readMemoryContextConsumptionReceipt;
const crypto = __importStar(require("crypto"));
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const internal_mcp_runtime_1 = require("./internal-mcp-runtime");
const utils_1 = require("../core/utils");
exports.MEMORY_CONTEXT_CONSUMPTION_CHALLENGE_SCHEMA = "ccm-memory-context-consumption-challenge-v1";
exports.MEMORY_CONTEXT_CONSUMPTION_RECEIPT_SCHEMA = "ccm-memory-context-consumption-receipt-v1";
// Keep receipts and snapshot lifecycle data inside the invocation's CCM data
// directory.  MCP workers receive CCM_TASK_STORE_DIR for isolated real-chain
// runs; using os.homedir() here silently wrote to the user's global store and
// made the server report receipt_missing even after a successful MCP ACK.
const RECEIPT_DIR = path.join(utils_1.CCM_DIR, "memory-context-consumption-receipts");
const SNAPSHOT_DIR = path.join(utils_1.CCM_DIR, "task-agent-memory-context-snapshots");
...
function challengeCore(input = {}) {
    return {
        schema: exports.MEMORY_CONTEXT_CONSUMPTION_CHALLENGE_SCHEMA,
        version: 1,
...
function createMemoryContextConsumptionChallenge(input = {}) {
    ...
    return { ...core, challenge_signature: (0, internal_mcp_runtime_1.signInternalMcpEvidence)(core) };
}
function verifyMemoryContextConsumptionChallenge(challenge, expected = {}) {
    const issues = [];
    const core = challengeCore(challenge);
    if (core.schema !== exports.MEMORY_CONTEXT_CONSUMPTION_CHALLENGE_SCHEMA || core.version !== 1)
        issues.push("challenge_schema_invalid");
    if (!/^mcrc_[a-f0-9]{28}$/.test(core.challenge_id))
        issues.push("challenge_id_invalid");
    if (!(0, internal_mcp_runtime_1.verifyInternalMcpEvidenceSignature)(core, challenge?.challenge_signature))
        issues.push("challenge_signature_invalid");
    for (const [field, aliases] of Object.entries({
        ...
        if (expectedValue && String(core[field] || "") !== String(expectedValue))
...
function memoryContextConsumptionReceiptFile(challengeId) {
    const id = String(challengeId || "");
    if (!/^mcrc_[a-f0-9]{28}$/.test(id))
        return "";
    return path.join(RECEIPT_DIR, `${id}.json`);
}
function memoryContextConsumptionReceiptDirectory() {
    return RECEIPT_DIR;
}
function pathIsInside(base, file) {
    try {
...
function collectReceiptReferences(snapshotDir = SNAPSHOT_DIR) {
    ...
    for (const snapshotFile of listSnapshotFiles(snapshotDir)) {
        ...
        try {
            ...
            for (const challengeId of recoveredIds)
                references.push({ challengeId, snapshotFile, unreadable: true });
            continue;
        }
        if (snapshot?.context?.memory_context_consumption_receipt_required !== true)
            continue;
...
function verifyReceiptFileWithoutChallenge(file, expectedId) {
    const issues = [];
    let receipt = null;
    try {
        receipt = JSON.parse(fs.readFileSync(file, "utf-8"));
    }
    catch {
        issues.push("receipt_unreadable");
    }
    if (receipt) {
        const { receipt_signature: supplied, ...core } = receipt;
        if (receipt.schema !== exports.MEMORY_CONTEXT_CONSUMPTION_RECEIPT_SCHEMA || Number(receipt.version || 0) !== 1)
            issues.push("receipt_schema_invalid");
        if (!(0, internal_mcp_runtime_1.verifyInternalMcpEvidenceSignature)(core, supplied))
            issues.push("receipt_signature_invalid");
        if (String(receipt.challenge_id || "") !== expectedId)
            issues.push("receipt_challenge_mismatch");
        if (receipt.state !== "loaded" || receipt.source !== "provider_model_mcp_call")
            issues.push("receipt_state_invalid");
    }
    return { valid: issues.length === 0, issues, receipt };
}
function inspectMemoryContextConsumptionReceiptFile(challengeId) {
    const id = String(challengeId || "");
    const file = memoryContextConsumptionReceiptFile(id);
    if (!file || !fs.existsSync(file))
        return { present: false, valid: false, issues: ["receipt_missing"], receipt: null, file, receiptSignature: "" };
...
function memoryContextConsumptionChallengeReferenceState(challengeId, options = {}) {
    ...
    const unrecoverableSnapshots = collected.unreadableSnapshots.filter((row) => !row.recoveredChallengeIds.length);
    if (unrecoverableSnapshots.length)
        return { referenced: true, uncertain: true, referenceCount: 0, unreadableSnapshotCount: collected.unreadableSnapshots.length };
    const matches = collected.references.filter((row) => row.challengeId === challengeId);
    return {
...
function removeMemoryContextConsumptionReceiptIfUnreferenced(challengeId, options = {}) {
    const id = String(challengeId || "");
    const file = memoryContextConsumptionReceiptFile(id);
    const snapshotDir = String(options.snapshotDir || options.snapshot_dir || SNAPSHOT_DIR);
    if (!file || !pathIsInside(RECEIPT_DIR, file))
    ...
    if (reference.referenced)
        return { removed: false, reason: reference.uncertain ? "snapshot_reference_uncertain" : "snapshot_still_references_receipt", challengeId: id, file };
    try {
...
function reconcileMemoryContextConsumptionReceipts(options = {}) {
    ...
    try {
        if (fs.existsSync(RECEIPT_DIR)) {
            for (const entry of fs.readdirSync(RECEIPT_DIR, { withFileTypes: true })) {
                ...
                if (!match) {
                    unexpectedFiles.push(path.join(RECEIPT_DIR, entry.name));
                    continue;
                }
    ...
    for (const [challengeId, references] of referencesById) {
        ...
        let issues = [];
        if (!challengeId || !receiptFile)
            issues.push("challenge_id_invalid");
        else if (!fs.existsSync(receiptFile))
            issues.push("receipt_missing");
        else if (!readableReferences.length)
            issues.push("snapshot_reference_unreadable");
        else {
        ...
        if (issues.length && status === "referenced_missing" && !issues.includes("receipt_missing"))
            status = "referenced_invalid";
    ...
    const referencedRows = allReferencedRows.filter(row => {
        ...
        return references.some(reference => (!scopeFilter.groupId || reference.groupId === scopeFilter.groupId)
    ...
    const pruningBlocked = collected.unreadableSnapshots.some((row) => !row.recoveredChallengeIds.length);
    const pruned = [];
    const skipped = [];
    if (prune) {
        for (const row of allOrphanRows.filter(row => row.prunable)) {
            if (pruningBlocked) {
                skipped.push({ challengeId: row.challengeId, reason: "unreadable_snapshot_blocks_orphan_pruning" });
                continue;
            }
            const result = removeMemoryContextConsumptionReceiptIfUnreferenced(row.challengeId, { snapshotDir });
            if (result.removed)
                pruned.push({ challengeId: row.challengeId, reason: row.status, file: row.file });
            else
                skipped.push({ challengeId: row.challengeId, reason: result.reason });
        }
    }
    return {
...
function attachMemoryContextConsumptionChallenge(memoryContext, challenge) {
    if (!memoryContext || !challenge)
...
function recordMemoryContextConsumptionReceipt(context, args = {}) {
    ...
    if (!verification.valid)
        throw new Error(`记忆接收 challenge 无效：${verification.issues.join(",")}`);
    if (String(args.challenge_id || args.challengeId || "") !== String(challenge.challenge_id || ""))
        throw new Error("记忆接收 challenge id 不匹配");
    const receiptFile = String(context.memoryReceiptFile || "");
    if (!receiptFile || path.basename(receiptFile) !== `${challenge.challenge_id}.json`)
        throw new Error("记忆接收回执路径未绑定");
    const core = {
        schema: exports.MEMORY_CONTEXT_CONSUMPTION_RECEIPT_SCHEMA,
        version: 1,
    ...
    const receipt = { ...core, receipt_signature: (0, internal_mcp_runtime_1.signInternalMcpEvidence)(core) };
    atomicWrite(receiptFile, receipt);
    return receipt;
}
function readMemoryContextConsumptionReceipt(challenge, expected = {}) {
    ...
    let receipt = null;
    try {
        receipt = file ? JSON.parse(fs.readFileSync(file, "utf-8")) : null;
    }
    catch {
        issues.push("receipt_missing");
    }
    if (receipt) {
        ...
        if (!(0, internal_mcp_runtime_1.verifyInternalMcpEvidenceSignature)(core, supplied))
            issues.push("receipt_signature_invalid");
        if (String(receipt.challenge_id || "") !== String(challenge?.challenge_id || ""))
            issues.push("receipt_challenge_mismatch");
        if (String(receipt.challenge_signature || "") !== String(challenge?.challenge_signature || ""))
            issues.push("receipt_challenge_signature_mismatch");
        if (receipt.state !== "loaded" || receipt.source !== "provider_model_mcp_call")
            issues.push("receipt_state_invalid");
        for (const [field, aliases] of Object.entries({
            group_id: ["groupId", "group_id"],
            ...
            const expectedValue = aliases.map(alias => expected?.[alias]).find(value => String(value || "").trim());
            if (expectedValue && String(receipt?.[field] || "") !== String(expectedValue))
                issues.push(`receipt_${field}_mismatch`);
        }
    }
    return {
...
//# sourceMappingURL=memory-context-consumption-receipt.js.map