...
const tools = [
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                requirement_checksum: { type: "string", description: "Exact business requirement checksum from the signed work order." },
                plan_checksum: { type: "string", description: "Exact confirmed plan checksum from the signed work order." },
                step_id: { type: "string", description: "Exact assigned plan step ID." },
                step_ids: { type: "array", items: { type: "string" }, description: "Every assigned plan step ID when one project owns multiple confirmed steps." },
                acceptance_criterion_ids: { type: "array", items: { type: "string" }, description: "Every acceptance criterion ID assigned to this work item." },
...
function normalizeAcknowledgementArgs(args) {
    ...
    return {
        ...
        requirement_checksum: String(args?.requirement_checksum || args?.requirementChecksum || "").trim(),
        plan_checksum: String(args?.plan_checksum || args?.planChecksum || "").trim(),
        step_id: String(args?.step_id || args?.stepId || "").trim(),
        step_ids: normalizedStringList(args?.step_ids ?? args?.stepIds ?? [args?.step_id || args?.stepId]),
        acceptance_criterion_ids: normalizedStringList(args?.acceptance_criterion_ids ?? args?.acceptanceCriterionIds),
