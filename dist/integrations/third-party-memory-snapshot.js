...
function assertSnapshotContext(snapshot, context) {
    ...
    const projectSessionSnapshot = String(identity.bindingKind || "") === "project_session";
    for (const [field, actual] of [
        ...
        // Project-session memory is materialized before the worker session lease
        // is allocated, so its durable snapshot intentionally carries an empty
        // taskAgentSessionId.  Bind the snapshot to the project/session first;
        // the challenge and receipt still bind the concrete worker session.  Do
        // not apply this wildcard to group or task-scoped snapshots.
        if (field === "taskAgentSessionId" && projectSessionSnapshot && !String(identity[field] || "") && String(actual || ""))
            continue;
