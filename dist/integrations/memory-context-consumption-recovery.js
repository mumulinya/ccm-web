...
const RECOVERY_DIR = path.join(os.homedir(), ".ccm", "memory-context-consumption-recoveries");
...
function verifyMemoryContextConsumptionRecovery(record, expected = {}) {
    ...
    if (record?.status === "recovered") {
        ...
        // Claude Code recovery deliberately starts a fresh provider session.  The
        // CCM challenge, receipt and runner contract still bind the recovery, but
        // the provider-native session id is not expected to equal the parent id.
        const freshClaudeRecovery = (0, runtime_1.normalizeAgentRuntimeId)(record?.provider) === "claudecode"
            && record?.recovery_native_continuation_evidence?.freshProviderSession === true;
        if (!continuation.valid || (!freshClaudeRecovery && record?.recovery_native_continuation_evidence?.nativeContinuationAcknowledged !== true))
...
async function recoverMemoryContextConsumptionReceipt(input, executeContinuation) {
    ...
    try {
        ...
        const recoveryContinuation = (0, native_continuation_1.buildNativeSessionContinuationEvidence)({
            ...
            freshProviderSession: normalizedProvider === "claudecode" && input.allowFreshProviderSession === true,
        });
        const freshClaudeRecovery = normalizedProvider === "claudecode" && input.allowFreshProviderSession === true;
        ...
        const issues = [
            ...
            ...(freshClaudeRecovery || recoveryContinuation.nativeContinuationAcknowledged === true ? [] : ["recovery_native_continuation_unacknowledged"]),
