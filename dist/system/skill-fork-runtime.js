...
const RECEIPT_FILE = path.join(process.env.CCM_SKILL_FORK_DIR || path.join(os.homedir(), ".ccm"), "skill-fork-receipts.json");
...
async function executeSkillFork(input) {
    ...
    const cacheScope = input.parent.scope === "global" || input.parent.scope === "group" ? input.parent.scope : "project";
    const cacheAffinity = (0, agent_cache_affinity_1.semanticCacheAffinity)({
        scope: cacheScope,
        scopeId: input.parent.scopeId,
        exactSessionId: input.parent.exactSessionId,
        taskId: communicationIdentity.taskId,
        generation: input.parent.generation,
        decisionKind: "skill_fork",
    });
    const messages = [{
            role: "system",
            content: [
                "You are an isolated CCM Skill child Agent. Complete only the supplied Skill objective. Use only authorized read-only tools. Do not edit code, dispatch development work, or declare the parent task complete. If a write would be needed, return only a structured WorkItem proposal.",
                "Return a JSON object containing reply/result and optional toolRequests. Dynamic Skill instructions, parent context, and current input are supplied in the user message.",
            ].join("\n\n"),
        }, { role: "user", content: JSON.stringify({
                skillName: String(skill.name || ""),
                skillHash: String(skill.contentHash || ""),
                skillInstructions: String(skill.renderedPrompt || skill.prompt || ""),
                visibleParentContext: String(input.modelVisibleContext || "").slice(0, 100_000),
                skillInput: String(skill.input || ""),
            }) }];
    const evidenceRefs = [];
