...
const conversation_execution_reconciliation_1 = require("./conversation-execution-reconciliation");
...
function eventReadProject(event) {
    ...
    if (explicit)
        return explicit;
    const task = event.taskId ? (0, db_1.loadTasks)().find((item) => String(item?.id || "") === String(event.taskId)) : null;
...
async function rehydrateReadonlyToolDetail(event, options = {}) {
    ...
    if (event.scope === "global")
        throw Object.assign(new Error("全局主 Agent没有项目源码读取权限"), { statusCode: 403 });
...
function handleUserVisibleAgentEventsApi(pathname, req, res, parsed) {
    ...
    if (pathname === "/api/agent-execution/events" && req.method === "GET") {
        try {
            ...
            const reconciliation = enabled
                ? (0, conversation_execution_reconciliation_1.reconcileConversationExecutionsForIdentity)(identity(parsed?.query))
                : null;
            const page = enabled
                ? taskId && attempt
            ...
            (0, utils_1.sendJson)(res, enabled
                ? { success: true, enabled, ...page, reconciliation, turnStates: (0, unified_agent_turn_state_1.projectUnifiedAgentTurnStates)(page?.events || []) }
    ...
    if (pathname === "/api/agent-execution/attempts" && req.method === "GET") {
        try {
            const taskId = String(parsed?.query?.task_id || parsed?.query?.taskId || "").trim();
            const task = taskId ? (0, db_1.loadTasks)().find((item) => String(item?.id || "") === taskId) : null;
            if (!task || !(0, access_policy_1.hasTaskResourceAccess)(task, req.ccmAuth, "use")) {
