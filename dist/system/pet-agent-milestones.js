...
exports.PET_AGENT_MILESTONE_SCHEMA = void 0;
exports.createPetAgentMilestoneProjector = createPetAgentMilestoneProjector;
exports.runPetAgentMilestoneSelfTest = runPetAgentMilestoneSelfTest;
const crypto = __importStar(require("crypto"));
const user_visible_agent_events_1 = require("./user-visible-agent-events");
const user_notifications_1 = require("./user-notifications");
exports.PET_AGENT_MILESTONE_SCHEMA = "ccm-pet-agent-milestone-v1";
function hash(value) {
    return crypto.createHash("sha256").update(JSON.stringify(value ?? null)).digest("hex");
}
function compact(value, max = 240) {
    return (0, user_notifications_1.sanitizePetNotificationText)(value, max);
}
...
function stateFor(kind) {
    ...
    if (kind === "implementation_started")
        return "building";
    if (kind === "key_finding" || kind === "direction_change")
        return "building";
    if (kind === "blocked" || kind === "rework")
        return "debugging";
    if (["verification_started", "verification_passed", "summary_started", "result_submitted"].includes(kind))
        return "reviewing";
    if (kind === "needs_user")
        return "waiting";
    if (kind === "completed")
...
function sourceFor(event) {
    if (isTestAgent(event) || event.detail?.executionStage?.kind === "independent_verification")
        return "test_agent";
    if (event.agentRunId || event.detail?.runtimeObservation)
        return "runtime";
    return "main_agent";
}
function project(event) {
    if (["assistant_progress", "agent_progress"].includes(event.eventType)
        && event.detail?.progress?.source === "system_observed")
        return null;
    let kind = null;
    ...
    if (event.eventType === "requirement_plan") {
        ...
        if (event.detail?.progress?.text)
            kind = progressKind(event) || "key_finding";
        ...
        if (/cancel/.test(statusText))
            kind = "cancelled";
        else if (event.display.status === "failed")
            kind = "failed";
        else if (event.display.status === "waiting")
            kind = "needs_user";
        else
            kind = "completed";
    ...
    return {
        schema: exports.PET_AGENT_MILESTONE_SCHEMA,
...
function createPetAgentMilestoneProjector(options) {
    ...
    const acceptIdentity = (event, milestone) => {
        ...
        if (previous && (generation < previous.generation || (generation === previous.generation && attempt < previous.attempt)))
            return false;
        if (previous?.terminal && generation === previous.generation && attempt === previous.attempt && !milestone.terminal)
            return false;
        watermarks.set(key, { generation, attempt, terminal: milestone.terminal || previous?.terminal === true });
        return true;
    };
    const scheduleFallback = (event, milestone) => {
        if (!event.agentRunId || milestone.terminal || milestone.kind === "result_submitted")
            return;
        const key = runtimeIdentity(event);
        clearFallback(key);
    ...
    const onEvent = (event) => {
        ...
        if (!milestone || !acceptIdentity(event, milestone))
            return;
        const key = runtimeIdentity(event);
        if (milestone.terminal || milestone.kind === "result_submitted" || milestone.kind === "verification_passed")
            clearFallback(key);
        else
            scheduleFallback(event, milestone);
        ...
        if (options.getMode?.() === "terminal_only" || delivered.has(milestone.dedupeKey))
            return;
        delivered.add(milestone.dedupeKey);
        options.emit(milestone);
    };
    const unsubscribe = (0, user_visible_agent_events_1.subscribeUserVisibleAgentEvents)(onEvent);
    return {
        stop() {
            unsubscribe();
...
function runPetAgentMilestoneSelfTest() {
    const base = {
...
//# sourceMappingURL=pet-agent-milestones.js.map