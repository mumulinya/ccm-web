...
exports.hasMetricExecutionV3 = hasMetricExecutionV3;
...
const agent_trajectory_evaluation_1 = require("./agent-trajectory-evaluation");
const main_agent_harness_rollout_1 = require("../agents/main-agent-harness-rollout");
...
function updateAggregate(db, event, bucketDate) {
  ...
  ON CONFLICT(bucket_date, scope_type, scope_id, role, agent) DO UPDATE SET
    ...
    duration_samples_json=excluded.duration_samples_json, last_call=excluded.last_call`).run(bucketDate, event.scopeType, event.scopeId, event.role, event.agent, event.status === "completed" ? 1 : 0, event.status === "failed" ? 1 : 0, event.status === "blocked" ? 1 : 0, event.status === "cancelled" ? 1 : 0, event.status === "unknown" ? 1 : 0, event.durationMs, event.fileChangeCount, event.fileChangeCount, event.inputTokens, event.outputTokens, event.directInputTokens, event.cacheCreationInputTokens, event.cacheReadInputTokens, event.providerTotalTokens, event.totalCostUsd, event.usageReported ? 1 : 0, event.usageSource === "local_no_model" ? 1 : 0, event.usageSource === "unreported" ? 1 : 0, event.modelMs, event.toolWallMs, event.queueWaitMs, event.dependencyWaitMs, event.verificationMs, event.summaryMs, event.peakCpuPercent, event.peakRssBytes, event.peakChildProcessCount, JSON.stringify(samples), event.at);
}
...
function hasMetricExecutionV3(executionId, scopeType, agent) {
    const id = String(executionId || "").trim();
    if (!id)
        return false;
    const row = (0, observability_database_1.getObservabilityDatabase)().prepare(`
    SELECT 1 FROM metric_events_v3
    WHERE execution_id=? AND scope_type=? AND agent=?
    LIMIT 1
  `).get(id, String(scopeType || ""), String(agent || ""));
    return !!row;
}
...
function loadMetricsDashboardV3() {
    ...
    const coverageRows = (0, observability_database_1.getObservabilityDatabase)().prepare(`SELECT scope_type, scope_id, role, agent, runtime, usage_source, usage_missing_reason, COUNT(*) calls
    FROM metric_events_v3 GROUP BY scope_type, scope_id, role, agent, runtime, usage_source, usage_missing_reason`).all();
    ...
    metrics.agentTrajectory = (0, agent_trajectory_evaluation_1.loadAgentTrajectoryMetrics)();
    metrics.mainAgentHarness = (0, main_agent_harness_rollout_1.loadMainAgentHarnessRolloutStatus)();
    return metrics;
}
