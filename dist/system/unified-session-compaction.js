...
function resolveUnifiedCompactionPolicy(config = {}, overrides = {}) {
    ...
    const capacity = (0, ccm_context_accounting_v2_1.normalizeCcmContextCapacity)({
        rawWindowTokens: contextWindow,
        windowSemantics: "total_context",
        reservedOutputTokens: reserved,
        autoCompactBufferTokens: 13_000,
    });
    // Only a canonical caller-provided capacity may override the derived line.
    // Retired user fields such as modelAutoCompactTokenLimit are read by config
    // migration code but no longer influence runtime compaction decisions.
    const threshold = positive(overrides.autoCompactThreshold, capacity.autoCompactThresholdTokens, overrides.autoCompactThreshold !== undefined ? 1 : 18_000);
    return {
        strategy: "cc_two_stage",
        microCompactEnabled: false,
        pressureFirst: overrides.pressureFirst ?? true,
        idleAssistEnabled: false,
        idleGapMinutes: 0,
...
function buildUnifiedCompactionReceipt(input) {
    const core = {
        ...
        strategyApplied: input.strategyApplied || (input.summarySource === "session_memory" ? "session_memory" : input.summarySource === "model" ? "model_summary" : input.microCompactApplied ? "microcompact" : "none"),
        sessionMemoryChecksum: String(input.sessionMemoryChecksum || ""),
        sessionMemoryCursor: String(input.sessionMemoryCursor || ""),
        ptlRecoveryAttempts: Math.max(0, Number(input.ptlRecoveryAttempts || 0)),
        ptlDroppedRoundCount: Math.max(0, Number(input.ptlDroppedRoundCount || 0)),
        ptlDroppedMessageIds: Array.isArray(input.ptlDroppedMessageIds) ? input.ptlDroppedMessageIds.map(String).slice(-64) : [],
        ptlDroppedMessagesChecksum: String(input.ptlDroppedMessagesChecksum || ""),
        executionPath: input.executionPath || "none",
        compactionRunId: String(input.compactionRunId || ""),
        contentStored: false,
...
function projectUnifiedCompactionReceipt(receipt, summaryQuality) {
    ...
    return {
        ...
        strategyApplied: String(receipt.strategyApplied || "none"),
        ptlRecoveryAttempts: Math.max(0, Number(receipt.ptlRecoveryAttempts || 0)),
        ptlDroppedRoundCount: Math.max(0, Number(receipt.ptlDroppedRoundCount || 0)),
        executionPath: receipt.executionPath || "none",
        compactionRunId: String(receipt.compactionRunId || ""),
        contentStored: false,
    };
}
