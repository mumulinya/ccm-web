...
exports.DEFAULT_AGENT_COMMUNICATION_POLICY = exports.AGENT_TERMINAL_RECEIPT_SCHEMA = exports.AGENT_RESULT_RECEIPT_SCHEMA = exports.AGENT_PROGRESS_RECEIPT_SCHEMA = exports.AGENT_DISPATCH_ACK_SCHEMA = exports.AGENT_COMMUNICATION_ENVELOPE_SCHEMA = void 0;
exports.readAgentCommunicationPolicy = readAgentCommunicationPolicy;
exports.createAgentCommunicationEnvelope = createAgentCommunicationEnvelope;
exports.getAgentCommunication = getAgentCommunication;
exports.bindAgentCommunicationSemanticContract = bindAgentCommunicationSemanticContract;
exports.listAgentCommunications = listAgentCommunications;
exports.transitionAgentCommunication = transitionAgentCommunication;
exports.recordAgentCommunicationAuditEvent = recordAgentCommunicationAuditEvent;
exports.acquireAgentCommunicationLease = acquireAgentCommunicationLease;
exports.heartbeatAgentCommunication = heartbeatAgentCommunication;
exports.recordAgentCommunicationReceipt = recordAgentCommunicationReceipt;
exports.releaseAgentCommunicationLease = releaseAgentCommunicationLease;
exports.reconcileAgentCommunications = reconcileAgentCommunications;
exports.performAgentCommunicationAction = performAgentCommunicationAction;
exports.getAgentCommunicationDiagnostics = getAgentCommunicationDiagnostics;
exports.buildAgentCommunicationTaskSummary = buildAgentCommunicationTaskSummary;
exports.startAgentCommunicationWatchdog = startAgentCommunicationWatchdog;
exports.stopAgentCommunicationWatchdog = stopAgentCommunicationWatchdog;
exports.startAgentCommunicationDispatch = startAgentCommunicationDispatch;
exports.waitForAgentCommunicationDispatch = waitForAgentCommunicationDispatch;
exports.markAgentCommunicationRunnerStarted = markAgentCommunicationRunnerStarted;
exports.ensureAgentCommunicationAcknowledged = ensureAgentCommunicationAcknowledged;
exports.submitAgentCommunicationResult = submitAgentCommunicationResult;
exports.finalizeAgentCommunication = finalizeAgentCommunication;
...
const agent_runtime_progress_1 = require("./agent-runtime-progress");
exports.AGENT_COMMUNICATION_ENVELOPE_SCHEMA = "ccm-agent-communication-envelope-v2";
exports.AGENT_DISPATCH_ACK_SCHEMA = "ccm-agent-dispatch-ack-v2";
exports.AGENT_PROGRESS_RECEIPT_SCHEMA = "ccm-agent-progress-receipt-v2";
exports.AGENT_RESULT_RECEIPT_SCHEMA = "ccm-agent-result-receipt-v2";
exports.AGENT_TERMINAL_RECEIPT_SCHEMA = "ccm-agent-terminal-receipt-v2";
function runtimeDisplayLabel(value) {
    ...
    if (normalized === "qoder")
        return "Qoder";
    if (normalized.includes("testagent"))
        return "TestAgent";
    return runtime;
}
function projectCommunicationEvent(envelope, eventType, input = {}) {
    if (!envelope?.scope || !envelope?.scopeId || !envelope?.exactSessionId)
        return null;
    const phase = String(input?.phase || envelope.state || "");
    const waiting = /queued|waiting|ack|verifying|submitted/.test(phase);
    let payload = {};
    try {
        payload = getAgentCommunication(envelope.messageId, { includeEvents: false, includeReceipts: false })?.payload || {};
    }
    catch { }
...
function readAgentCommunicationPolicy(overrides = {}) {
    let stored = {};
    try {
        stored = require("../modules/collaboration/group-orchestrator-config").loadOrchestratorConfig() || {};
    }
    catch { }
    const source = { ...exports.DEFAULT_AGENT_COMMUNICATION_POLICY, ...stored, ...overrides };
    const globalPerProject = Math.max(1, integer(stored.agentMaxParallelPerProject, exports.DEFAULT_AGENT_COMMUNICATION_POLICY.agentMaxParallelPerProject));
    const globalTotal = Math.max(1, integer(stored.agentMaxParallelGlobal, exports.DEFAULT_AGENT_COMMUNICATION_POLICY.agentMaxParallelGlobal));
    return {
...
function checksum(value) { return crypto.createHash("sha256").update(JSON.stringify(value ?? null)).digest("hex"); }
function id(prefix) { return `${prefix}_${Date.now().toString(36)}_${crypto.randomBytes(6).toString("hex")}`; }
function text(value, max = 500) { return String(value || "").trim().slice(0, max); }
function integer(value, fallback = 0) { const parsed = Math.floor(Number(value)); return Number.isFinite(parsed) ? parsed : fallback; }
function json(value, fallback = null) { try {
    return JSON.parse(String(value || ""));
}
catch {
    return fallback;
} }
function safeStringList(value, maxItems = 40, maxLength = 500) {
...
function safeVerificationResults(value) {
...
function rowToEnvelope(row) {
    return {
        schema: exports.AGENT_COMMUNICATION_ENVELOPE_SCHEMA,
...
function appendEvent(db, row) {
  ...
  ) VALUES(?,?,?,?,?,?,?,?,?,?)`).run(row.messageId, row.eventType, row.fromState || "", row.toState || "", integer(row.generation), Math.max(1, integer(row.attempt, 1)), text(row.leaseId, 240), text(row.receiptChecksum, 128), JSON.stringify(sanitizeMetadata(row.detail || {})), nowIso());
}
function assertIdentity(row, input) {
    ...
    if (input.senderAgentId !== undefined || input.sender_agent_id !== undefined)
        compare("sender", row.receiver_agent_id, input.senderAgentId ?? input.sender_agent_id);
    if (input.receiverAgentId !== undefined || input.receiver_agent_id !== undefined)
        compare("receiver", row.sender_agent_id, input.receiverAgentId ?? input.receiver_agent_id);
    return issues;
}
function createAgentCommunicationEnvelope(input) {
    ...
    return (0, task_store_1.withSqliteTaskStore)(db => {
        ...
        const transaction = db.transaction(() => {
            ...
            if (existing)
                return { envelope: rowToEnvelope(existing), deduplicated: true };
      ...
      ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(messageId, input.messageType || "task_dispatch", text(input.correlationId, 240) || messageId, text(input.parentMessageId, 240), input.taskId, input.workItemId || input.taskId, input.scope, input.scopeId, input.exactSessionId, Math.max(0, integer(input.generation)), Math.max(1, integer(input.attempt, 1)), "", input.senderAgentId, input.receiverAgentId, state, input.deadlineAt || new Date(Date.now() + 24 * 60 * 60_000).toISOString(), idempotencyKey, checksum(payload), JSON.stringify(payload), createdAt, createdAt);
...
function getAgentCommunication(messageId, options = {}) {
    return (0, task_store_1.withSqliteTaskStore)(db => {
        configure(db);
        const row = db.prepare("SELECT * FROM agent_communication_messages WHERE message_id=?").get(text(messageId, 240));
        if (!row)
            return null;
        const value = { ...rowToEnvelope(row), payload: sanitizeMetadata(json(row.payload_json, {})) };
        if (options.includeEvents !== false)
            value.events = db.prepare("SELECT * FROM agent_communication_events WHERE message_id=? ORDER BY event_id ASC").all(row.message_id).map((event) => ({
        ...
        if (options.includeReceipts !== false)
            value.receipts = db.prepare("SELECT * FROM agent_communication_receipts WHERE message_id=? ORDER BY created_at ASC").all(row.message_id).map((receipt) => ({
...
function bindAgentCommunicationSemanticContract(messageId, input = {}) {
    const binding = sanitizeMetadata({
        requirementId: text(input.requirementId || input.requirement_id, 240),
        requirementRevision: Math.max(1, integer(input.requirementRevision || input.requirement_revision, 1)),
        requirementChecksum: text(input.requirementChecksum || input.requirement_checksum, 128),
        planRevision: Math.max(1, integer(input.planRevision || input.plan_revision, 1)),
        planChecksum: text(input.planChecksum || input.plan_checksum, 128),
        stepId: text(input.stepId || input.step_id, 160),
        stepIds: safeStringList(input.stepIds || input.step_ids || [input.stepId || input.step_id], 60, 160),
        acceptanceCriterionIds: safeStringList(input.acceptanceCriterionIds || input.acceptance_criterion_ids, 60, 160),
        title: text(input.title, 240),
        objective: text(input.objective, 1800),
        businessGoal: text(input.businessGoal || input.business_goal, 2400),
        contentStored: false,
    });
    if (!binding.requirementChecksum || !binding.planChecksum || !binding.stepId || !binding.stepIds.length || !binding.acceptanceCriterionIds.length) {
        return { bound: false, reason: "semantic_binding_incomplete", binding };
    }
    return (0, task_store_1.withSqliteTaskStore)(db => {
        configure(db);
        const transaction = db.transaction(() => {
            const row = db.prepare("SELECT * FROM agent_communication_messages WHERE message_id=?").get(text(messageId, 240));
            if (!row)
                return { bound: false, reason: "message_not_found", binding };
            if (["acknowledged", "executing", "result_submitted", "completed", "failed", "cancelled"].includes(String(row.state || ""))) {
                return { bound: false, reason: "semantic_binding_too_late", binding };
            }
            const payload = sanitizeMetadata(json(row.payload_json, {}));
            const nextPayload = { ...payload, semanticBinding: binding };
            db.prepare("UPDATE agent_communication_messages SET payload_json=?,payload_checksum=?,updated_at=? WHERE message_id=?")
                .run(JSON.stringify(nextPayload), checksum(nextPayload), nowIso(), row.message_id);
            appendEvent(db, { messageId: row.message_id, eventType: "semantic_contract_bound", fromState: row.state, toState: row.state, generation: row.generation, attempt: row.attempt, leaseId: row.lease_id, detail: { requirementChecksum: binding.requirementChecksum, planChecksum: binding.planChecksum, stepId: binding.stepId, stepIds: binding.stepIds, acceptanceCriterionIds: binding.acceptanceCriterionIds, contentStored: false } });
            return { bound: true, binding, contentStored: false };
        });
        return transaction();
    });
}
function listAgentCommunications(query = {}) {
    return (0, task_store_1.withSqliteTaskStore)(db => {
        configure(db);
        const clauses = [];
        const args = [];
        const add = (column, value) => { if (String(value || "").trim()) {
            clauses.push(`${column}=?`);
...
function transitionAgentCommunication(messageId, toState, input = {}) {
    return (0, task_store_1.withSqliteTaskStore)(db => {
        configure(db);
        const transaction = db.transaction(() => {
            const row = db.prepare("SELECT * FROM agent_communication_messages WHERE message_id=?").get(text(messageId, 240));
            ...
            if (fromState === toState)
                return { envelope: rowToEnvelope(row), unchanged: true };
            if (!allowedTransitions[fromState]?.has(toState))
                throw new Error(`Agent Communication状态迁移不合法：${fromState} -> ${toState}`);
            if (input.expectedState && input.expectedState !== fromState)
                throw new Error(`Agent Communication状态已变化：expected=${input.expectedState}, actual=${fromState}`);
...
function recordAgentCommunicationAuditEvent(messageId, eventType, detail = {}) {
    return (0, task_store_1.withSqliteTaskStore)(db => {
        configure(db);
        const row = db.prepare("SELECT * FROM agent_communication_messages WHERE message_id=?").get(text(messageId, 240));
        if (!row)
            return { recorded: false, reason: "message_not_found" };
        appendEvent(db, {
...
function acquireAgentCommunicationLease(messageId, ownerId, options = {}) {
    ...
    return (0, task_store_1.withSqliteTaskStore)(db => {
        ...
        const transaction = db.transaction(() => {
            ...
            if (!row)
                throw new Error("Agent Communication消息不存在");
            const existing = db.prepare("SELECT * FROM agent_communication_leases WHERE message_id=?").get(row.message_id);
            const now = Date.now();
            if (existing?.status === "active" && Date.parse(existing.expires_at) > now)
                return { acquired: false, lease: sanitizeMetadata(existing), envelope: rowToEnvelope(row) };
            ...
            const activeGlobal = Number(db.prepare("SELECT COUNT(*) AS count FROM agent_communication_leases WHERE status='active' AND expires_at>?").get(nowIso())?.count || 0);
        ...
        WHERE l.status='active' AND l.expires_at>? AND m.receiver_agent_id=?`).get(nowIso(), row.receiver_agent_id)?.count || 0);
            if (activeGlobal >= policy.agentMaxParallelGlobal || activeProject >= policy.agentMaxParallelPerProject) {
              ...
              WHERE message_type='task_dispatch' AND state='queued' AND receiver_agent_id=?
                AND (created_at<? OR (created_at=? AND message_id<=?))`).get(row.receiver_agent_id, row.created_at, row.created_at, row.message_id))?.count || 1);
        ...
        VALUES(?,?,?,?,?,'active','none',?,?,?,?)
        ON CONFLICT(message_id) DO UPDATE SET lease_id=excluded.lease_id,owner_id=excluded.owner_id,generation=excluded.generation,attempt=excluded.attempt,status='active',side_effect_state='none',heartbeat_at=excluded.heartbeat_at,expires_at=excluded.expires_at,created_at=excluded.created_at,updated_at=excluded.updated_at`).run(leaseId, row.message_id, text(ownerId, 240), row.generation, attempt, at, expiresAt, at, at);
...
function heartbeatAgentCommunication(messageId, identity, input = {}) {
    ...
    return (0, task_store_1.withSqliteTaskStore)(db => {
        ...
        const transaction = db.transaction(() => {
            ...
            const lease = db.prepare("SELECT * FROM agent_communication_leases WHERE message_id=?").get(row.message_id);
            if (!lease || lease.status !== "active")
                return { accepted: false, stale: true, issues: ["lease_inactive"], envelope: rowToEnvelope(row) };
            const at = nowIso();
...
function receiptSchema(type) {
    if (type === "dispatch_ack")
        return exports.AGENT_DISPATCH_ACK_SCHEMA;
    if (type === "progress")
        return exports.AGENT_PROGRESS_RECEIPT_SCHEMA;
    if (type === "result")
        return exports.AGENT_RESULT_RECEIPT_SCHEMA;
    if (type === "terminal")
        return exports.AGENT_TERMINAL_RECEIPT_SCHEMA;
    throw new Error(`不支持的Agent回执类型：${type}`);
}
function recordAgentCommunicationReceipt(messageId, receiptType, identity, rawReceipt, options = {}) {
    const result = (0, task_store_1.withSqliteTaskStore)(db => {
        configure(db);
        const transaction = db.transaction(() => {
            const row = db.prepare("SELECT * FROM agent_communication_messages WHERE message_id=?").get(text(messageId, 240));
            if (!row)
            ...
            if (receiptType === "dispatch_ack" && semanticBinding) {
                if (text(rawReceipt?.requirementChecksum ?? rawReceipt?.requirement_checksum, 128) !== text(semanticBinding.requirementChecksum, 128))
                    issues.push("requirement_checksum_mismatch");
                if (text(rawReceipt?.planChecksum ?? rawReceipt?.plan_checksum, 128) !== text(semanticBinding.planChecksum, 128))
                    issues.push("plan_checksum_mismatch");
                if (text(rawReceipt?.stepId ?? rawReceipt?.step_id, 160) !== text(semanticBinding.stepId, 160))
                    issues.push("step_id_mismatch");
                const expectedSteps = safeStringList(semanticBinding.stepIds || [semanticBinding.stepId], 60, 160).sort();
                const receivedSteps = safeStringList(rawReceipt?.stepIds ?? rawReceipt?.step_ids ?? [rawReceipt?.stepId ?? rawReceipt?.step_id], 60, 160).sort();
                if (JSON.stringify(expectedSteps) !== JSON.stringify(receivedSteps))
                    issues.push("step_ids_mismatch");
                const expectedCriteria = safeStringList(semanticBinding.acceptanceCriterionIds, 60, 160).sort();
                const receivedCriteria = safeStringList(rawReceipt?.acceptanceCriterionIds ?? rawReceipt?.acceptance_criterion_ids, 60, 160).sort();
                if (JSON.stringify(expectedCriteria) !== JSON.stringify(receivedCriteria))
                    issues.push("acceptance_criterion_ids_mismatch");
            }
            if (receiptType === "terminal" && options.ccmTrusted !== true)
                issues.push("terminal_requires_ccm");
            ...
            const projected = {
                ...
                requirementChecksum: text(rawReceipt?.requirementChecksum ?? rawReceipt?.requirement_checksum, 128),
                planChecksum: text(rawReceipt?.planChecksum ?? rawReceipt?.plan_checksum, 128),
                stepId: text(rawReceipt?.stepId ?? rawReceipt?.step_id, 160),
                stepIds: safeStringList(rawReceipt?.stepIds ?? rawReceipt?.step_ids ?? [rawReceipt?.stepId ?? rawReceipt?.step_id], 60, 160),
                acceptanceCriterionIds: safeStringList(rawReceipt?.acceptanceCriterionIds ?? rawReceipt?.acceptance_criterion_ids, 60, 160),
            ...
            if (existing)
                return { accepted: issues.length === 0, deduplicated: true, stale: issues.length > 0, issues, receiptChecksum, envelope: rowToEnvelope(row) };
            if (issues.length) {
            ...
            const nextState = receiptType === "dispatch_ack" ? "acknowledged" : receiptType === "progress" ? (projected.phase === "waiting_dependency" ? "waiting_dependency" : "executing") : receiptType === "result" ? "result_submitted" : (projected.status === "accepted" ? "completed" : projected.status === "cancelled" ? "cancelled" : "failed");
            if (row.state !== nextState && !allowedTransitions[row.state]?.has(nextState))
                throw new Error(`当前状态不能接收${receiptType}回执：${row.state}`);
            db.prepare("INSERT INTO agent_communication_receipts(receipt_checksum,message_id,receipt_type,generation,attempt,lease_id,status,receipt_json,created_at) VALUES(?,?,?,?,?,?,?,?,?)").run(receiptChecksum, row.message_id, receiptType, row.generation, row.attempt, row.lease_id, projected.status, JSON.stringify(projected), projected.recordedAt);
    ...
    if (result?.accepted && receiptType === "result" && result.receipt) {
        ...
        try {
            if (workDir)
                repoStateIdentity = (0, unified_evidence_registry_1.captureRepoStateIdentity)(workDir, receipt.filesChanged || []);
        }
        catch { }
        const evidenceBase = {
        ...
        for (const item of Array.isArray(receipt.verificationResults) ? receipt.verificationResults.slice(0, 80) : []) {
            ...
            (0, unified_evidence_registry_1.recordEvidence)({ ...evidenceBase, evidenceType: "test", subject: command, status: /pass|success|ok|recorded|completed|0/.test(status.toLowerCase()) ? "valid" : "invalid", summary: status });
        }
        if (Array.isArray(receipt.filesChanged) && receipt.filesChanged.length) {
            (0, unified_evidence_registry_1.recordEvidence)({ ...evidenceBase, evidenceType: "diff", subject: "worker diff", status: "valid", references: receipt.filesChanged, summary: `${receipt.filesChanged.length} files changed` });
        }
    }
    if (result?.accepted && !result?.deduplicated && result?.envelope) {
        ...
        if (receiptType === "progress")
            (0, agent_runtime_progress_1.markAgentReportedSemanticProgress)(result.envelope.messageId, result.envelope.generation, result.envelope.attempt);
    }
    return result;
}
function releaseAgentCommunicationLease(messageId, status = "released") {
    return (0, task_store_1.withSqliteTaskStore)(db => {
        configure(db);
        const at = nowIso();
...
function reconcileAgentCommunications(options = {}) {
    const now = Number(options.now || Date.now());
    const policy = readAgentCommunicationPolicy(options.policy);
    return (0, task_store_1.withSqliteTaskStore)(db => {
        configure(db);
        const transaction = db.transaction(() => {
            const rows = db.prepare("SELECT * FROM agent_communication_messages WHERE state NOT IN ('completed','cancelled','failed')").all();
            const outcomes = [];
            for (const row of rows) {
                const lease = db.prepare("SELECT * FROM agent_communication_leases WHERE message_id=?").get(row.message_id);
                let next = "";
                if (lease?.status === "active" && Date.parse(lease.expires_at) <= now)
                    next = lease.side_effect_state === "none" ? "lease_expired" : "recovery_required";
                else if (row.state === "runner_starting" && now - Date.parse(row.updated_at) > policy.agentRunnerStartTimeoutMs)
                    next = "startup_timeout";
                else if (row.state === "runner_started" && now - Date.parse(row.updated_at) > policy.agentAckTimeoutMs)
                    next = "ack_timeout";
                else if (["acknowledged", "executing", "waiting_dependency"].includes(row.state) && lease && now - Date.parse(lease.heartbeat_at) > policy.agentHeartbeatLostTimeoutMs)
                    next = lease.side_effect_state === "none" ? "heartbeat_lost" : "recovery_required";
                if (!next || !allowedTransitions[row.state]?.has(next))
                    continue;
                const at = nowIso();
                db.prepare("UPDATE agent_communication_messages SET state=?,updated_at=? WHERE message_id=?").run(next, at, row.message_id);
                if (lease?.status === "active")
                    db.prepare("UPDATE agent_communication_leases SET status='expired',updated_at=? WHERE lease_id=?").run(at, lease.lease_id);
...
function performAgentCommunicationAction(messageId, action, input = {}) {
    if (action === "reconcile")
        return { action, outcomes: reconcileAgentCommunications(input) };
    const current = getAgentCommunication(messageId, { includeEvents: false, includeReceipts: false });
    if (!current)
        throw new Error("Agent Communication消息不存在");
    if (action === "cancel")
        return transitionAgentCommunication(messageId, current.state === "created" ? "cancelled" : "cancel_requested", { detail: { reason: text(input.reason, 500) } });
    if (action === "retry") {
        const policy = readAgentCommunicationPolicy(input.policy);
        if (!["rejected", "startup_timeout", "ack_timeout", "heartbeat_lost", "lease_expired", "recovery_required", "failed"].includes(current.state))
            throw new Error(`当前状态不能重试：${current.state}`);
        if (current.attempt >= policy.agentMaxAttempts)
            throw new Error("Agent Communication执行次数已达到上限");
        releaseAgentCommunicationLease(messageId, "retrying");
        (0, task_store_1.withSqliteTaskStore)(db => db.prepare("UPDATE agent_communication_messages SET attempt=?,updated_at=? WHERE message_id=?").run(current.attempt + 1, nowIso(), current.messageId));
    ...
    if (action === "takeover") {
        if (terminalStates.has(current.state))
            throw new Error(`终态消息不能接管：${current.state}`);
...
function getAgentCommunicationDiagnostics() {
    ...
    return (0, task_store_1.withSqliteTaskStore)(db => {
        ...
        const activeLeases = db.prepare("SELECT * FROM agent_communication_leases WHERE status='active'").all();
        const messages = db.prepare("SELECT * FROM agent_communication_messages ORDER BY updated_at DESC LIMIT 500").all();
      ...
      WHERE e1.event_type='created' AND e2.to_state='runner_started' ORDER BY e2.event_id DESC LIMIT 100`).all();
      ...
      WHERE e.to_state='runner_started' AND r.receipt_type='dispatch_ack' ORDER BY r.created_at DESC LIMIT 100`).all();
      ...
      ORDER BY r.created_at DESC LIMIT 100`).all();
        ...
        return {
            ...
            states: Object.fromEntries(states.map((row) => [row.state, Number(row.count || 0)])),
            concurrency: {
            ...
            metrics: {
                ...
                heartbeat_lost_total: Number(db.prepare("SELECT COUNT(*) AS count FROM agent_communication_events WHERE to_state='heartbeat_lost'").get()?.count || 0),
                lease_recovery_total: Number(db.prepare("SELECT COUNT(*) AS count FROM agent_communication_events WHERE event_type='watchdog_reconciled'").get()?.count || 0),
                stale_receipt_total: Number(db.prepare("SELECT COUNT(*) AS count FROM agent_communication_events WHERE event_type='stale_receipt'").get()?.count || 0),
                ...
                worktree_merge_conflict_total: Number(db.prepare("SELECT COUNT(*) AS count FROM agent_communication_events WHERE event_type='worktree_merge_conflict'").get()?.count || 0),
            },
...
function buildAgentCommunicationTaskSummary(taskId) {
    ...
    const lease = current?.leaseId ? (0, task_store_1.withSqliteTaskStore)(db => db.prepare("SELECT * FROM agent_communication_leases WHERE lease_id=?").get(current.leaseId)) : null;
    return {
...
function startAgentCommunicationWatchdog(options = {}) {
    if (watchdogTimer)
        return { started: false, running: true };
    watchdogTimer = setInterval(() => {
        try {
            const outcomes = reconcileAgentCommunications();
            for (const outcome of outcomes) {
                if (!outcome.safeRetry || typeof options.onSafeRetry !== "function")
                    continue;
                try {
                    options.onSafeRetry(outcome);
                }
                catch { }
            }
        }
...
function stopAgentCommunicationWatchdog() {
    if (!watchdogTimer)
...
function reverseReceiptIdentity(envelope) {
    return {
        taskId: envelope.taskId,
        workItemId: envelope.workItemId,
        exactSessionId: envelope.exactSessionId,
...
function startAgentCommunicationDispatch(input) {
    const policy = readAgentCommunicationPolicy(input.policy);
    if (!policy.agentCommunicationV2Enabled)
        return { enabled: false, acquired: true, envelope: null, lease: null };
    ...
    const requestedAttempt = Math.max(1, integer(input.attempt, envelope.attempt || 1), resumesAfterAttempt ? Number(envelope.attempt || 1) + 1 : Number(envelope.attempt || 1));
...
async function waitForAgentCommunicationDispatch(input, options = {}) {
    let dispatch = options.initialDispatch || startAgentCommunicationDispatch(input);
    ...
    while (dispatch?.enabled !== false && dispatch?.acquired !== true && dispatch?.capacity === true) {
        ...
        if (options.shouldCancel?.()) {
            if (messageId) {
                try {
                    finalizeAgentCommunication(messageId, "cancelled", { sideEffectState: "none", reason: "任务等待执行容量期间被取消" });
                }
                catch { }
            }
...
function markAgentCommunicationRunnerStarted(messageId, detail = {}) {
    const current = getAgentCommunication(messageId, { includeEvents: false, includeReceipts: false });
    if (!current)
        throw new Error("Agent Communication消息不存在");
    if (["runner_started", "acknowledged", "executing", "waiting_dependency", "result_submitted", "verifying", "accepted", "completed"].includes(current.state))
        return { envelope: current, unchanged: true };
...
function ensureAgentCommunicationAcknowledged(messageId, ack = {}) {
    let current = getAgentCommunication(messageId, { includeEvents: false, includeReceipts: false });
    if (!current)
        throw new Error("Agent Communication消息不存在");
    if (current.state === "runner_starting")
        current = markAgentCommunicationRunnerStarted(messageId, { legacyBridge: true }).envelope;
    if (["acknowledged", "executing", "waiting_dependency", "result_submitted", "verifying", "accepted", "completed"].includes(current.state))
        return { accepted: true, deduplicated: true, envelope: current };
    if (current.state !== "runner_started")
        throw new Error(`当前状态不能补齐ACK：${current.state}`);
...
function submitAgentCommunicationResult(messageId, result = {}) {
    let current = getAgentCommunication(messageId, { includeEvents: false, includeReceipts: false });
    if (!current)
        throw new Error("Agent Communication消息不存在");
    ...
    if (current.state === "acknowledged")
        current = transitionAgentCommunication(messageId, "executing", { eventType: "execution_observed" }).envelope;
    if (["result_submitted", "verifying", "accepted", "completed", "rejected", "failed"].includes(current.state))
        return { accepted: true, deduplicated: true, envelope: current };
    const recorded = recordAgentCommunicationReceipt(messageId, "result", reverseReceiptIdentity(current), {
    ...
    if (recorded?.accepted === true)
        releaseAgentCommunicationLease(messageId, "result_submitted");
    return recorded;
}
/** CCM-only acceptance gate. A worker result can never create its own terminal state. */
function finalizeAgentCommunication(messageId, outcome, evidence = {}) {
    let current = getAgentCommunication(messageId, { includeEvents: false, includeReceipts: false });
    if (!current)
        throw new Error("Agent Communication消息不存在");
    ...
    if (outcome === "cancelled") {
        if (current.state !== "cancel_requested")
            transitionAgentCommunication(messageId, "cancel_requested", { detail: evidence });
    ...
    if (current.state === "result_submitted")
        current = transitionAgentCommunication(messageId, "verifying", { eventType: "ccm_verification_started", detail: evidence }).envelope;
    const accepted = outcome === "accepted";
    if (current.state === "verifying")
        current = transitionAgentCommunication(messageId, accepted ? "accepted" : "rejected", { eventType: accepted ? "ccm_verification_passed" : "ccm_verification_failed", detail: evidence }).envelope;
...
//# sourceMappingURL=agent-communication-v2.js.map