...
function normalizeStore(value, expected) {
    ...
    return {
        ...
        invokedSkills,
        loadedMcpSchemas,
        latestManifest: source.latestManifest?.schema === "ccm-main-agent-post-compact-restore-manifest-v3" ? source.latestManifest : null,
...
function validateMainAgentPostCompactRestoreManifest(value, expected) {
    const issues = [];
    const schemaVersionValid = value?.schema === "ccm-main-agent-post-compact-restore-manifest-v3"
        && Number(value?.version || 0) === 3
        && value?.contentStored === false
        && value?.contextSourceManifest?.contentStored === false;
