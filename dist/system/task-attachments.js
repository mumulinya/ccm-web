...
async function ingestSources(files, userText, urls = [], onlineDocumentFetcher, extractUrlsFromText = true) {
    const onlineUrls = [...new Set([...(urls || []), ...(extractUrlsFromText ? (0, source_ingestion_1.extractOnlineDocumentUrls)(userText) : [])])];
...
async function buildTaskAttachmentMutation(input) {
    ...
    let reparsedWarnings = [];
    let reparsedTechnical = null;
