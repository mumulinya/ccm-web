...
exports.getProcessBootId = getProcessBootId;
exports.registerRestartIntent = registerRestartIntent;
exports.initializeProcessLifecycle = initializeProcessLifecycle;
exports.touchProcessLifecycle = touchProcessLifecycle;
exports.markProcessShutdown = markProcessShutdown;
exports.recordProcessFault = recordProcessFault;
exports.installProcessLifecycleFaultHandlers = installProcessLifecycleFaultHandlers;
exports.getProcessLifecycleSnapshot = getProcessLifecycleSnapshot;
exports.runProcessLifecycleSelfTest = runProcessLifecycleSelfTest;
...
const ROOT = path.join(utils_1.CCM_DIR, "reliability", "process-lifecycle");
...
function activeExecutionContext() {
    try {
        const now = Date.now();
        const leases = (0, reliability_ledger_1.listTaskLeases)().filter((lease) => lease.status === "active"
        ...
        return {
            ...
            task_ids: leases.map((lease) => String(lease.task_id || "")).filter(Boolean),
            trace_ids: Array.from(new Set(leases.map((lease) => String(lease.trace_id || "")).filter(Boolean))),
...
function normalizeCategory(value) {
    ...
    if (["manual", "manual_restart", "user_restart"].includes(raw))
        return "manual_restart";
    if (["reload", "code_reload", "deploy", "deployment"].includes(raw))
        return "code_reload";
    if (["system", "system_shutdown", "shutdown", "service_stop"].includes(raw))
        return "system_shutdown";
    if (["crash", "unexpected_crash", "fatal"].includes(raw))
...
function consumeRestartIntent() {
    ...
    if (envReason)
        return { category: normalizeCategory(envReason), reason: envDetail || envReason, source: "environment" };
    if (validPersisted)
        return { ...persisted, category: normalizeCategory(persisted.category), source: "intent_file" };
    return null;
}
function getProcessBootId() {
    return BOOT_ID;
}
function registerRestartIntent(input = {}) {
    const now = Date.now();
    const intent = {
        version: 1,
...
function initializeProcessLifecycle() {
    if (initialized)
    ...
    let restart = { category: "initial_start", expected: true, reason: "首次记录的 CCM 进程启动" };
    if (previous) {
        if (intent)
            restart = { category: intent.category, expected: true, reason: intent.reason, source: intent.source, task_id: intent.task_id || "", trace_id: intent.trace_id || "" };
        ...
        else if (processAlive(Number(previous.pid || 0)))
            restart = { category: "parallel_instance", expected: false, reason: "检测到上一 CCM 实例仍存活" };
        else
            restart = { category: "unexpected_crash", expected: false, reason: "上一进程没有留下正常退出记录", task_id: previousExecution.task_id || "", trace_id: previousExecution.trace_id || "" };
        appendEvent({
...
function touchProcessLifecycle() {
    const current = readJson(CURRENT_FILE, null);
    if (!current || current.boot_id !== BOOT_ID)
        return false;
...
function markProcessShutdown(input = {}) {
    if (shutdownRecorded)
        return readJson(CURRENT_FILE, null);
...
function recordProcessFault(error, type = "uncaught_exception") {
...
function installProcessLifecycleFaultHandlers(onFatal) {
    ...
    process.on("unhandledRejection", reason => {
        ...
        if (onFatal)
            onFatal(reason, "unhandled_rejection");
        else {
            process.exitCode = 1;
...
function getProcessLifecycleSnapshot(options = {}) {
    const since = Date.parse(String(options.since || options.since_at || 0));
    const all = readEvents(Number(options.limit || 5000));
    const events = Number.isFinite(since) && since > 0 ? all.filter((event) => Date.parse(event.at || 0) >= since) : all;
    const starts = events.filter((event) => event.type === "process.started");
    const unexpected = starts.filter((event) => ["unexpected_crash", "parallel_instance"].includes(event.category));
    const planned = starts.filter((event) => ["manual_restart", "code_reload", "system_shutdown", "planned_restart"].includes(event.category));
    return {
...
function runProcessLifecycleSelfTest() {
