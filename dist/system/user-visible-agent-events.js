...
const live_workspace_changes_1 = require("./live-workspace-changes");
...
const STORE_ROOT = path.resolve(process.env.CCM_USER_VISIBLE_AGENT_EVENT_DIR || path.join(os.homedir(), ".ccm", "agent-execution-events"));
...
function sanitizeLiveWorkspaceChanges(value) {
    if (!value || typeof value !== "object" || value.schema !== live_workspace_changes_1.LIVE_WORKSPACE_CHANGES_SCHEMA || value.contentStored !== false)
        return null;
    const scope = ["global", "group", "project"].includes(String(value.scope)) ? value.scope : "";
    const scopeId = compactText(value.scopeId, 240);
    const exactSessionId = compactText(value.exactSessionId, 240);
    const turnId = compactText(value.turnId, 240);
    const agentRunId = compactText(value.agentRunId, 240);
    const projectId = compactText(value.projectId, 240);
    const checksum = compactText(value.checksum, 128);
    if (!scope || !scopeId || !exactSessionId || !turnId || !agentRunId || !projectId || !checksum)
        return null;
    const files = (Array.isArray(value.files) ? value.files : []).slice(0, 500).map((file) => ({
        path: compactText(file?.path || file?.file, 1_000).replace(/\\/g, "/"),
        status: ["added", "modified", "deleted", "renamed"].includes(String(file?.status)) ? file.status : "modified",
        ...(Number.isFinite(Number(file?.additions)) ? { additions: Math.max(0, Math.floor(Number(file.additions))) } : {}),
        ...(Number.isFinite(Number(file?.deletions)) ? { deletions: Math.max(0, Math.floor(Number(file.deletions))) } : {}),
        diffAvailable: file?.diffAvailable === true,
    })).filter((file) => file.path && !file.path.startsWith("/") && !/^[A-Za-z]:\//.test(file.path) && !file.path.split("/").includes(".."));
    return {
        schema: live_workspace_changes_1.LIVE_WORKSPACE_CHANGES_SCHEMA,
        scope,
        scopeId,
        exactSessionId,
        turnId,
        ...
        ...(compactText(value.taskId, 240) ? { taskId: compactText(value.taskId, 240) } : {}),
        agentRunId,
        projectId,
        revision: Math.max(1, Number(value.revision || 1)),
        files,
        totalFiles: files.length,
        ...(Number.isFinite(Number(value.totalAdditions)) ? { totalAdditions: Math.max(0, Math.floor(Number(value.totalAdditions))) } : {}),
        ...(Number.isFinite(Number(value.totalDeletions)) ? { totalDeletions: Math.max(0, Math.floor(Number(value.totalDeletions))) } : {}),
        checksum,
        contentStored: false,
    };
}
...
function normalizeEventType(input) {
    ...
    if ([
        ...
        "permission_required", "clarification_required", "context_compaction", "context_compacted", "result",
    ].includes(source))
        return source;
    ...
    if (["compaction", "context_compaction"].includes(source))
...
function normalizeUserVisibleAgentEvent(input, sequence = 0) {
    ...
    const resultStatus = eventType === "result"
        ? compactText(input?.result?.completionSummary?.status
            || input?.result?.completion_summary?.status
            || input?.result?.status
            || input?.detail?.completionSummary?.status
            || input?.detail?.completion_summary?.status
            || input?.detail?.safeResult?.completionSummary?.status
            || input?.detail?.safeResult?.completion_summary?.status
            || input?.detail?.safeResult?.status, 40)
        : "";
    // A terminal result's safe completion projection is authoritative.  The
    // display envelope must not default to success when the model loop ended
    // without creating or accepting a formal task.
    const status = resultStatus || input?.display?.status || (eventType.endsWith("_failed") ? "failed"
    ...
    const detail = {
        ...
        ...(sanitizeLiveWorkspaceChanges(detailSource.workspaceChanges || detailSource.workspace_changes || input?.workspaceChanges || input?.workspace_changes)
            ? { workspaceChanges: sanitizeLiveWorkspaceChanges(detailSource.workspaceChanges || detailSource.workspace_changes || input?.workspaceChanges || input?.workspace_changes) } : {}),
        ...
        ...(detailSource.modelActivity && typeof detailSource.modelActivity === "object"
            ...
            modelActivity: {
                ...
                ...(Number(detailSource.modelActivity.revision) > 0 ? { revision: Math.max(1, Number(detailSource.modelActivity.revision)) } : {}),
                ...(Number(detailSource.modelActivity.retryAttempt) > 0 ? { retryAttempt: Math.max(1, Number(detailSource.modelActivity.retryAttempt)) } : {}),
                ...(Number(detailSource.modelActivity.maxRetries) > 0 ? { maxRetries: Math.max(1, Number(detailSource.modelActivity.maxRetries)) } : {}),
                ...(Number.isFinite(Number(detailSource.modelActivity.retryDelayMs)) && Number(detailSource.modelActivity.retryDelayMs) >= 0
                    ? { retryDelayMs: Math.max(0, Number(detailSource.modelActivity.retryDelayMs)) } : {}),
                ...
                requestDispatched: detailSource.modelActivity.requestDispatched === true,
                responseStarted: detailSource.modelActivity.responseStarted === true,
                providerRequestIdPresent: detailSource.modelActivity.providerRequestIdPresent === true,
                requestDispatchCount: Math.max(0, Number(detailSource.modelActivity.requestDispatchCount || 0)),
                responseStartedCount: Math.max(0, Number(detailSource.modelActivity.responseStartedCount || 0)),
                ...(["preparation_failed", "connection_timeout", "provider_error", "user_cancelled"].includes(String(detailSource.modelActivity.failureKind))
                    ? { failureKind: String(detailSource.modelActivity.failureKind) } : {}),
        ...
        ...(detailSource.keyProgress && typeof detailSource.keyProgress === "object"
            ...
            keyProgress: {
                ...
                status: ["running", "success", "failed", "waiting"].includes(String(detailSource.keyProgress.status))
                    ...
                    : "running",
                attempt: Math.max(1, Number(detailSource.keyProgress.attempt || input?.attempt || 1)),
        ...
        ...(detailSource.providerRetry && typeof detailSource.providerRetry === "object" ? {
            providerRetry: {
                ...
                maxRetries: Math.max(0, Math.min(5, Number(detailSource.providerRetry.maxRetries || 5))),
                exhausted: detailSource.providerRetry.exhausted === true,
                contentStored: false,
            },
        } : {}),
...
function sameVisibleEventLane(item, next) {
    const itemTurnId = compactText(item.turnId, 240);
    const nextTurnId = compactText(next.turnId, 240);
    if (itemTurnId && nextTurnId && itemTurnId !== nextTurnId)
        return false;
    ...
    if (itemAttempt && nextAttempt && itemAttempt !== nextAttempt)
        return false;
    // Legacy rows may not carry an attempt.  They are compatible only when the
    // stable turn identity proves they belong to the same request; otherwise a
    // missing attempt must not block or absorb another recovery attempt.
    if ((itemAttempt || nextAttempt) && (!itemTurnId || !nextTurnId || itemTurnId !== nextTurnId))
        return false;
...
function shouldDropLateAssistantProgress(store, next) {
    ...
    const nextTurnId = compactText(next.turnId, 240);
    if (nextTurnId) {
        const exactTerminal = store.events.some(item => item.eventType === "result"
            && compactText(item.turnId, 240) === nextTurnId
            && sameVisibleEventLane(item, next));
        if (exactTerminal)
            return true;
        // Older result rows did not persist turnId/attempt.  Only associate such a
        // terminal with this turn when the same turn already emitted an event
        // before that terminal.  A new turn that starts after the result remains
        // visible and cannot be blocked by the legacy row.
        const legacyResult = [...store.events].reverse().find(item => {
            if (item.eventType !== "result" || compactText(item.turnId, 240))
                return false;
            if (Number(item.generation || 0) !== Number(next.generation || 0))
                return false;
            const itemTask = compactText(item.taskId, 240);
            const nextTask = compactText(next.taskId, 240);
            if (itemTask !== nextTask)
                return false;
            const itemAttempt = Number(item.attempt || 0);
            const nextAttempt = Number(next.attempt || 0);
            return !(itemAttempt && nextAttempt && itemAttempt !== nextAttempt);
        });
        if (!legacyResult)
            return false;
        return store.events.some(item => compactText(item.turnId, 240) === nextTurnId
            && Number(item.sequence || 0) <= Number(legacyResult.sequence || 0));
    }
...
function appendUserVisibleAgentEvent(input) {
    ...
    (0, atomic_json_file_1.withFileLock)(file, () => {
        ...
        if (existing) {
            if (initial.eventType === "context_compaction" && existing.eventType === "context_compaction") {
                const currentRevision = Number(existing.detail?.contextCompaction?.revision || 0);
                const incomingRevision = Number(initial.detail?.contextCompaction?.revision || 0);
                if (incomingRevision > currentRevision || (initial.display?.status !== existing.display?.status)) {
                    event = { ...existing, ...initial, sequence: existing.sequence, createdAt: existing.createdAt || initial.createdAt,
                        detail: { ...(existing.detail || {}), ...(initial.detail || {}) } };
...
function appendAssistantProgress(input) {
    ...
    const attempt = Math.max(1, Number(input?.attempt || input?.detail?.technical?.attempt || 1));
    ...
    const keyProgress = existingKeyProgress && existingKeyProgress.schema === "ccm-agent-key-progress-v1"
        ? {
            ...existingKeyProgress,
            eventId: compactText(existingKeyProgress.eventId || eventId, 240),
            attempt: Math.max(1, Number(existingKeyProgress.attempt || attempt)),
        }
        : {
            schema: "ccm-agent-key-progress-v1",
            eventId,
...
function runUserVisibleAgentEventSelfTest() {
    ...
    const checks = {
        ...
        nextTurnProgressAfterPreviousResult: (() => {
            ...
            appendUserVisibleAgentEvent({
                ...identity, eventId: "next-turn-started", turnId: "next-turn", attempt: 1, eventType: "turn_started",
                display: { title: "项目主 Agent", summary: "本轮开始", status: "running" },
            });
            const nextProgress = appendAssistantProgress({
                ...identity, turnId: "next-turn", attempt: 1, text: "我先定位相关代码和配置，再根据结果继续判断。",
            ...
            return nextProgress?.eventType === "assistant_progress" && nextProgress?.attempt === 1
                && listed.events.some(item => item.eventId === nextProgress.eventId);
        })(),
        sameTurnProgressAfterResultDropped: (() => {
            const identity = { scope: "project", scopeId: "demo", exactSessionId: "session-late-progress" };
            appendUserVisibleAgentEvent({
                ...identity, eventId: "same-turn-started", turnId: "same-turn", attempt: 2, eventType: "turn_started",
                display: { title: "项目主 Agent", summary: "开始", status: "running" },
            });
            appendUserVisibleAgentEvent({
                ...identity, eventId: "same-turn-result", turnId: "same-turn", attempt: 2, eventType: "result",
                display: { title: "回复完成", summary: "完成", status: "success" },
            });
            const late = appendAssistantProgress({
                ...identity, turnId: "same-turn", attempt: 2, text: "这条迟到正文不能覆盖终态。",
                kind: "before_tools", modelCallIndex: 1, relatedToolCallIds: ["late-tool"],
            });
            return !listUserVisibleAgentEvents({ ...identity, cursor: 0, limit: 20 }).events.some(item => item.eventId === late?.eventId);
        })(),
    };
