...
exports.applyInteractiveConversationModePolicy = applyInteractiveConversationModePolicy;
...
function applyInteractiveConversationModePolicy(scope, planModeEnabled, parsed) {
    if (!["project", "group"].includes(scope) || planModeEnabled)
        return parsed;
...
function applyConversationPlanModeToRound(input) {
    ...
    const blockedResults = blocked.map(request => ({
        // Preserve the Provider-issued call identity. The next model turn must
        // receive one tool result for every function call, including calls that
        // CCM rejected at the Plan Mode boundary.
        callId: String(request?.id || request?.callId || request?.toolCallId || request?.tool_call_id || ""),
...
function runConversationPlanModeGateSelfTest() {
    const readOnly = (request) => [
        "read_file", "read_files", "glob_files", "grep_text", "query_knowledge",
        "request_project_source_inquiry", "request_group_source_inquiry",
    ...
    ].includes(String(request?.name || "").replace(/^mcp__ccm__ccm_workspace_readonly__/, ""));
    ...
    const mixedRound = applyConversationPlanModeToRound({
        enabled: true,
        parsed: dispatchParsed,
        requests: [{ id: "call_read", name: "read_file" }, { id: "call_blocked", name: "unknown_write_tool" }],
        isReadOnly: readOnly,
    });
    ...
    const checks = {
        ...
        unknownToolsHeld: mixedRound.requests.map((item) => item.name).join(",") === "read_file"
            ...
            && mixedRound.blockedResults.some((item) => item.callId === "call_blocked")
