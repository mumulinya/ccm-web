...
exports.normalizedWorkspacePersistenceToolName = normalizedWorkspacePersistenceToolName;
exports.isWorkspaceToolResultReference = isWorkspaceToolResultReference;
...
const WORKSPACE_RECEIPT_KIND = {
    read_file: "text", read_files: "text", grep_text: "grep", glob_files: "glob",
    list_directory: "glob", inspect_notebook: "notebook",
};
const WORKSPACE_ANALYSIS_PERSISTENCE_TOOLS = new Set([
    "analyze_change_impact", "find_related_tests", "inspect_dependency_graph", "inspect_public_contracts",
    "compare_project_contracts", "read_git_blame", "discover_verification_commands", "run_inspection_command",
]);
function normalizedWorkspacePersistenceToolName(toolName) {
    return clean(toolName, 240)
        .replace(/^mcp__ccm__ccm_workspace_readonly__/, "")
        .replace(/^mcp__ccm__ccm_workspace_edit__/, "");
}
function safeWorkspaceEvidencePath(value) {
    const candidate = clean(value, 1200).replace(/\\/g, "/");
    if (!candidate || /^[a-zA-Z]:\//.test(candidate) || candidate.startsWith("/") || candidate.startsWith("../"))
        return "";
    return candidate.replace(/^\.\//, "");
}
function isWorkspaceToolResultReference(value) {
...
function projectContextSourceToolResultForPersistence(toolName, value, query = "") {
    if (isWorkspaceToolResultReference(value))
        return value;
    const normalizedTool = normalizedWorkspacePersistenceToolName(toolName);
    if (normalizedTool === "read_scope_instruction") {
        const candidates = nestedCandidates(value);
        const source = candidates.find(candidate => candidate?.schema === "ccm-scope-instruction-read-result-v1")
            || candidates.find(candidate => candidate?.result?.schema === "ccm-scope-instruction-read-result-v1")?.result
            || {};
        return {
            schema: "ccm-scope-instruction-tool-result-reference-v1",
            toolName: normalizedTool,
            documentId: clean(source?.documentId, 240),
            kind: clean(source?.kind, 40),
            projectId: clean(source?.projectId, 180),
            groupId: clean(source?.groupId, 180),
            revision: Math.max(0, Number(source?.revision || 0)),
            checksum: clean(source?.checksum || checksum(value), 300),
            rehydratable: true,
            contentStored: false,
        };
    }
    if (WORKSPACE_PERSISTENCE_TOOLS.has(normalizedTool)) {
        ...
        const source = candidates.find(candidate => candidate && typeof candidate === "object" && (candidate.safeReceipt || candidate.safe_receipt || candidate.toolContractVersion || /^ccm-workspace-/.test(String(candidate.schema || "")))) || candidates.find(candidate => candidate && typeof candidate === "object") || {};
        ...
        const kind = clean(receipt.kind || (WORKSPACE_ANALYSIS_PERSISTENCE_TOOLS.has(normalizedTool) ? "analysis" : WORKSPACE_RECEIPT_KIND[normalizedTool]) || "text", 40);
        ...
        const evidenceRefs = [
            ...
            ...files.map((file) => {
                ...
                const lineNumbers = lines.map((line) => Number(line?.line || line?.lineNumber || line?.line_number || 0)).filter((line) => line > 0);
                return {
                    path: safeWorkspaceEvidencePath(file?.path),
                    checksum: clean(file?.checksum || receipt.checksum || receipt.resultChecksum || checksum(file), 300),
                    ...(lineNumbers.length ? { ranges: [{ startLine: Math.min(...lineNumbers), endLine: Math.max(...lineNumbers) }] } : {}),
                };
            }),
        ].filter((row) => row && (row.path || row.checksum)).slice(0, 64);
        return {
            schema: "ccm-workspace-tool-result-reference-v1",
            toolName: normalizedTool,
            toolContractVersion: Number(source?.toolContractVersion || 2),
            kind,
            projectId: clean(receipt.projectId || receipt.project_id || source?.projectId || source?.project_id || body?.projectId || body?.project_id, 180),
            path: safeWorkspaceEvidencePath(receipt.path || body?.path || files[0]?.path),
            checksum: clean(receipt.checksum || receipt.resultChecksum || source?.checksum || source?.result_checksum || body?.checksum || checksum(value), 300),
            resultChecksum: clean(receipt.resultChecksum || receipt.result_checksum || source?.resultChecksum || source?.result_checksum || body?.resultChecksum || body?.result_checksum || receipt.checksum || checksum(value), 300),
            repoStateChecksum: clean(receipt.repoStateChecksum || receipt.repo_state_checksum || source?.repoStateChecksum || source?.repo_state_checksum || body?.repoStateChecksum || body?.repo_state_checksum, 300),
            evidenceRefs,
            itemCount: Math.max(0, WORKSPACE_ANALYSIS_PERSISTENCE_TOOLS.has(normalizedTool) ? Number(receipt.evidenceRefs?.length || itemCount) : itemCount),
            ...
            originalTokens: (0, context_budget_1.estimateTextTokens)(JSON.stringify(value ?? null)),
            truncated,
            rehydratable: !WORKSPACE_ANALYSIS_PERSISTENCE_TOOLS.has(normalizedTool),
            contentStored: false,
        };
    }
