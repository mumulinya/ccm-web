...
function parseToolName(input) {
    ...
    const labels = {
        list_directory: "List directory", glob_files: "Find files", grep_text: "Search code (rg)",
        ...
        read_code_diagnostics: "Diagnostics", query_knowledge: "Search knowledge", request_project_source_inquiry: "Ask project Agent", request_group_source_inquiry: "Ask group Agent", tool_search: "Tool search", invoke_skill: "Skill", read_scope_instruction: "Read scope instruction",
        inspect_notebook: "Inspect notebook", web_search: "Web search", web_fetch: "Web fetch",
        analyze_change_impact: "Analyze change impact", find_related_tests: "Find related tests",
        inspect_dependency_graph: "Inspect dependency graph", inspect_public_contracts: "Inspect public contracts",
        compare_project_contracts: "Compare project contracts", read_git_blame: "Git blame",
        discover_verification_commands: "Discover verification commands", run_inspection_command: "Run read-only inspection",
        run_command: "Run command", get_command_output: "Command output", stop_command: "Stop command",
        dispatch_project_worker: "Dispatch project Agent", run_test_agent_review: "Start TestAgent review",
    };
    const label = labels[normalized] || operation.replace(/[_-]+/g, " ").replace(/^./, character => character.toUpperCase());
    const userLabels = {
        list_directory: "查找目录", glob_files: "查找文件", grep_text: "搜索代码（rg）", read_file: "读取文件", read_files: "批量读取文件",
        ...
        query_knowledge: "搜索知识库", request_project_source_inquiry: "请求项目主 Agent 核对源码", request_group_source_inquiry: "请求群聊主 Agent 核对源码", tool_search: "搜索工具", invoke_skill: "运行技能", read_scope_instruction: "读取认知文档", inspect_notebook: "检查 Notebook",
        web_search: "搜索网页", web_fetch: "读取网页",
        analyze_change_impact: "分析影响范围", find_related_tests: "查找相关测试",
        inspect_dependency_graph: "检查依赖关系", inspect_public_contracts: "检查公共契约",
        compare_project_contracts: "比较项目契约", read_git_blame: "查看代码来源",
        discover_verification_commands: "查找验证命令", run_inspection_command: "运行只读诊断",
        run_command: "运行项目命令", get_command_output: "读取命令状态", stop_command: "停止项目命令",
        dispatch_project_worker: "派发项目 Agent", run_test_agent_review: "启动 TestAgent 验收",
    };
    let family = "other";
    if (/dispatch|test_agent|agent_|request_(?:project|group)_source_inquiry/i.test(canonicalName))
        family = "agent";
        ...
        family = "git";
    else if (/find_definition|find_references|find_implementations|find_type_definition|find_incoming_calls|find_outgoing_calls|workspace_symbols|document_symbols|diagnostic|analyze_change_impact|inspect_dependency_graph|inspect_public_contracts|compare_project_contracts/.test(normalized))
        family = "symbol";
...
function targetFromArgs(args) {
    if (args?.project && (args?.question || args?.read_depth || args?.readDepth))
        return cleanText(args.project, 300);
...
function scopeInstructionPresentation(rawInput) {
    ...
    const labels = {
        project: "项目认知",
        group: "群聊认知",
        group_project: "群聊项目认知",
    };
    const label = labels[kind] || "认知文档";
    const revision = Math.max(0, Number(raw.revision || 0));
    const target = kind === "group_project"
        ? [cleanText(raw.groupId, 180), cleanText(raw.projectId, 180)].filter(Boolean).join(" · ")
        : cleanText(kind === "group" ? raw.groupId : raw.projectId, 180);
    return {
        userLabel: `读取${label}`,
        summary: revision ? `已加载第 ${revision} 版` : `已加载${label}`,
        target,
    };
}
function resultProjection(operation, rawInput, error, transientBody) {
    ...
    if (operation === "dispatch_project_worker") {
        const failed = raw?.success === false || ["failed", "blocked", "error"].includes(String(raw?.status || "").toLowerCase());
        return {
            kind: failed ? "error" : "summary",
            summary: failed ? cleanText(raw?.error || raw?.reason, 500) || "项目 Agent 派发未完成" : "项目 Agent 已提交",
            truncated: false,
        };
    }
    if (operation === "run_test_agent_review") {
        const accepted = raw?.canAccept === true || raw?.can_accept === true
            || raw?.recommendation === "accept" || raw?.verdict?.canAccept === true || raw?.verdict?.can_accept === true;
        const rejected = raw?.canAccept === false || raw?.can_accept === false
            || ["failed", "blocked", "runtime_error", "rejected"].includes(String(raw?.status || "").toLowerCase())
            || ["rework", "need_human"].includes(String(raw?.recommendation || "").toLowerCase());
        return {
            kind: rejected ? "error" : "summary",
            summary: accepted ? "TestAgent 验收通过" : rejected ? "TestAgent 验收未通过" : "TestAgent 验收已返回",
            truncated: false,
        };
    }
    ...
    if (operation === "read_scope_instruction") {
        return {
            kind: "summary",
            summary: scopeInstructionPresentation(rawInput)?.summary || "已加载认知文档",
            truncated: false,
        };
    }
    if (operation === "list_directory") {
    ...
    if (operation === "request_project_source_inquiry") {
        const receipt = raw?.receipt || raw?.result?.receipt || raw;
        const projectReceipts = Array.isArray(receipt?.projectReceipts) ? receipt.projectReceipts : [];
        const paths = projectReceipts.flatMap((item) => Array.isArray(item?.paths) ? item.paths : []).slice(0, 40);
        const sufficient = receipt?.sufficient === true || projectReceipts.some((item) => item?.sufficient === true);
        return {
            kind: paths.length ? "list" : sufficient ? "summary" : "empty",
            summary: sufficient ? `项目主 Agent 已核对 ${paths.length} 个相关文件` : cleanText(receipt?.reason || raw?.error, 500) || "项目源码证据不足",
            ...(paths.length ? { rows: paths.map((filePath) => ({ path: cleanText(filePath, 800) })) } : {}),
            total: paths.length,
            truncated: false,
        };
    }
    if (operation === "request_group_source_inquiry") {
        const receipt = raw?.receipt || raw?.result?.receipt || raw;
        const evidence = Array.isArray(receipt?.evidence) ? receipt.evidence : [];
        const projects = Array.isArray(receipt?.targetProjects) ? receipt.targetProjects : [];
        const sufficient = receipt?.sufficient === true;
        return {
            kind: evidence.length ? "list" : sufficient ? "summary" : "empty",
            summary: sufficient ? `群聊主 Agent 已核对 ${projects.length} 个相关项目、${evidence.length} 份源码证据` : cleanText(receipt?.reason || raw?.error, 500) || "群聊源码证据不足",
            ...(evidence.length ? { rows: evidence.slice(0, 40).map((item) => ({ project: cleanText(item?.project, 200), path: cleanText(item?.path, 800) })) } : {}),
    ...
    if (["analyze_change_impact", "find_related_tests", "inspect_dependency_graph", "inspect_public_contracts", "compare_project_contracts", "read_git_blame", "discover_verification_commands", "run_inspection_command"].includes(operation)) {
        const item = (label, options = {}) => ({
            label: cleanText(label, 500),
            ...(options.secondary ? { secondary: cleanText(options.secondary, 500) } : {}),
            ...(options.path ? { path: cleanText(options.path, 800) } : {}),
        ...
        if (operation === "analyze_change_impact") {
            items = [
                ...(Array.isArray(raw?.directDependents) ? raw.directDependents : []).map((file) => item("直接影响", { path: file })),
                ...(Array.isArray(raw?.relatedTests) ? raw.relatedTests : []).map((row) => item("相关测试", { path: row?.path, secondary: row?.score ? `相关度 ${row.score}` : "" })),
                ...(Array.isArray(raw?.publicContracts) ? raw.publicContracts : []).map((row) => item(row?.name || "公共契约", { path: row?.path, line: row?.line, secondary: row?.kind })),
            ].slice(0, 80);
            summary = `影响分析完成 · ${items.length} 项证据 · 置信度 ${cleanText(raw?.confidence || "unknown", 40)}`;
        }
        else if (operation === "find_related_tests") {
            items = (Array.isArray(raw?.tests) ? raw.tests : []).slice(0, 80).map((row) => item("相关测试", { path: row?.path, secondary: row?.score ? `相关度 ${row.score}` : "" }));
            summary = `找到 ${items.length} 个相关测试候选`;
        }
        else if (operation === "inspect_dependency_graph") {
            items = [
                ...(Array.isArray(raw?.entrypoints) ? raw.entrypoints : []).map((file) => item("模块入口", { path: file })),
                ...(Array.isArray(raw?.manifests) ? raw.manifests : []).map((file) => item("依赖配置", { path: file })),
                ...(Array.isArray(raw?.cycles) ? raw.cycles : []).map((cycle) => item("循环依赖", { secondary: Array.isArray(cycle) ? `${cycle.length} 个节点` : "" })),
            ].slice(0, 80);
            summary = `${Number(raw?.nodes || 0)} 个节点 · ${Number(raw?.edges?.length || 0)} 条依赖 · ${Number(raw?.cycles?.length || 0)} 个循环`;
        }
        else if (operation === "inspect_public_contracts") {
            items = (Array.isArray(raw?.contracts) ? raw.contracts : []).slice(0, 100).map((row) => item(row?.name || "公共契约", { path: row?.path, line: row?.line, secondary: row?.kind }));
            summary = `识别 ${items.length} 项公共契约`;
        }
        else if (operation === "compare_project_contracts") {
            items = [
                ...(Array.isArray(raw?.shared) ? raw.shared : []).map((row) => item(row?.contract || "共享契约", { status: "shared" })),
                ...(Array.isArray(raw?.onlyLeft) ? raw.onlyLeft : []).map((row) => item(row?.name || "仅生产方", { path: row?.path, line: row?.line, status: "left_only" })),
                ...(Array.isArray(raw?.onlyRight) ? raw.onlyRight : []).map((row) => item(row?.name || "仅消费方", { path: row?.path, line: row?.line, status: "right_only" })),
            ].slice(0, 100);
            summary = `${cleanText(raw?.leftProject, 100)} 与 ${cleanText(raw?.rightProject, 100)} 的契约比较已完成`;
        }
        else if (operation === "read_git_blame") {
            items = (Array.isArray(raw?.rows) ? raw.rows : []).slice(0, 100).map((row) => item(row?.commit || "提交", { line: row?.line, secondary: [row?.author, row?.at].filter(Boolean).join(" · ") }));
            summary = `读取 ${items.length} 行提交来源`;
        }
        else if (operation === "discover_verification_commands") {
            items = (Array.isArray(raw?.commands) ? raw.commands : []).slice(0, 80).map((row) => item(row?.kind || "验证方式", { path: row?.source, secondary: row?.command }));
            summary = `从项目配置发现 ${items.length} 个验证命令（未执行）`;
        }
        else {
            const status = Number(raw?.exitCode) === 0 ? "完成" : "返回非零状态";
            summary = `只读诊断${status} · ${Math.max(0, Number(raw?.durationMs || 0))}ms${raw?.truncated ? " · 结果已截断" : ""}`;
        }
        return {
            kind: Number(raw?.exitCode) > 0 ? "error" : items.length ? "list" : "summary",
            summary,
            ...(items.length ? { rows: items } : {}),
            presentation: { layout: operation === "discover_verification_commands" ? "verification" : "analysis", groups: items.length ? [{ id: "analysis", label: "分析结果", count: items.length, items }] : [] },
            total: items.length,
            truncated: raw?.truncated === true,
            rehydratable: false,
            authoritativeRevision: cleanText(raw?.safeReceipt?.repoStateChecksum || raw?.safe_receipt?.repoStateChecksum || "", 160),
            ...(operation === "run_inspection_command" ? { commandExecution: { status: Number(raw?.exitCode) === 0 ? "completed" : "failed", exitCode: Number(raw?.exitCode || 0), durationMs: Math.max(0, Number(raw?.durationMs || 0)) } } : {}),
        };
    }
...
function buildToolDisplayDetail(input) {
    ...
    const scopeInstruction = parsed.operation === "read_scope_instruction" ? scopeInstructionPresentation(input.result) : null;
    const target = scopeInstruction?.target || targetFromArgs(args);
    return {
        schema: TOOL_DISPLAY_SCHEMA,
        tool: {
            name: parsed.canonicalName,
            label: parsed.label,
            userLabel: scopeInstruction?.userLabel || parsed.userLabel,
        ...
        arguments: (parsed.operation === "read_scope_instruction" ? [] : Object.entries(args).slice(0, 40)).map(([key, value]) => ({
...
const WORKSPACE_READONLY_SHORT_NAMES = new Set([
    ...
    "analyze_change_impact", "find_related_tests", "inspect_dependency_graph", "inspect_public_contracts",
    "compare_project_contracts", "read_git_blame", "discover_verification_commands", "run_inspection_command",
]);
