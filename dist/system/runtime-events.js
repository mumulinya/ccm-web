...
function publicData(value) {
    ...
    for (const key of [
        ...
        "inquiryId", "inquiry_id", "targetScope", "target_scope", "targetId", "target_id",
        // Safe context-window telemetry. Values are numeric or a bounded schema
        // object; prompt contents and tool payloads are never exposed here.
        "schema", "scope", "scopeId", "exactSessionId", "requestId", "sequence", "currentTokens",
        "predictedNextRequestTokens", "contextWindow", "autoCompactThreshold",
        "tokenSource", "tokenBreakdown", "contentStored",
    ]) {
        if (!Object.prototype.hasOwnProperty.call(source, key))
            continue;
        const item = source[key];
        if (key === "tokenBreakdown" && item && typeof item === "object" && !Array.isArray(item)) {
            const breakdown = {};
            for (const [name, raw] of Object.entries(item).slice(0, 24)) {
                const number = Number(raw);
                if (Number.isFinite(number) && number >= 0)
                    breakdown[cleanText(name, 64)] = Math.floor(number);
            }
            allowed[key] = breakdown;
        }
        else {
