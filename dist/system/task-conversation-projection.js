...
function scopeOf(task) {
    if (text(task?.group_id || task?.groupId, 200))
        return "group";
    if (text(task?.project_session_id || task?.projectSessionId, 300) || text(task?.orchestration_scope, 80) === "project_session")
        return "project";
    if (text(task?.source_channel || task?.request_origin, 60).toLowerCase().includes("feishu"))
        return "feishu";
    // Parent global missions use target_project=global-agent as a routing marker,
    // not as a real project workspace. Classify them before the generic
    // target_project fallback so terminal results return to the source global
    // conversation instead of attempting to open a non-existent project.
    if (text(task?.workflow_type, 80) === "global_mission"
        || text(task?.target_project, 240) === "global-agent"
        || text(task?.source_conversation_ref?.scope, 40) === "global" && !text(task?.global_mission_id || task?.globalMissionId, 240))
        return "global";
    if (text(task?.global_mission_id || task?.globalMissionId || task?.workflow_meta?.global_direct_dispatch?.session_id, 240))
        return "global";
    if (text(task?.target_project || task?.project_id || task?.project, 240))
        return "project";
    return "global";
}
function sourceSessionOf(task) {
...
function activeSessionOf(task) {
    const bindings = Array.isArray(task?.task_context?.sessionBindings) ? task.task_context.sessionBindings : [];
    const activeBinding = [...bindings].reverse().find((item) => item?.status === "active" && item?.role !== "source");
...
function changedFileCount(task) {
...
function verificationCount(task) {
...
function taskConversationProjectionContent(task, options = {}) {
    const status = text(task?.status, 40).toLowerCase();
    ...
    if (status === "failed")
        return `任务未能完成：${detail || "执行失败，证据和现场已经保留"}。`;
    if (status === "blocked")
        return `任务需要处理：${detail || "当前存在阻塞，任务现场已经保留"}。`;
    if (["cancelled", "canceled"].includes(status))
        return `任务已取消：${detail || "任务现场和历史记录已经保留"}。`;
    if (status === "interrupted")
        return `任务已中断：${detail || "可以从最近安全检查点继续"}。`;
...
function projectionRevision(task) {
...
function shouldReplaceMessage(existing, task, content) {
    if (!existing)
        return true;
    ...
    if (existingRevision > nextRevision)
        return false;
    const expectedRuntime = (0, task_user_runtime_1.buildTaskUserRuntimeStatus)(task);
    ...
    if (existingRevision === nextRevision && existingStatus === nextStatus && text(existing?.content, 2_000) === text(content, 2_000))
        return false;
    return true;
}
function publicTaskProjection(task, activeSessionId) {
    const terminal = terminalProjection(task);
    const derivedRuntimeStatus = (0, task_user_runtime_1.buildTaskUserRuntimeStatus)(task);
    ...
    const projected = {
        ...
        completion_summary: task?.completion_summary || task?.delivery_summary?.completionSummary || (terminal ? {
            ...
            verificationFailed: terminal.accepted ? 0 : verification.filter((item) => text(item?.status, 40).toLowerCase() === "failed").length,
...
function appendTerminalExecutionProjection(task, scope, scopeId, sessionId) {
    const terminal = terminalProjection(task);
    if (!terminal || !scopeId || !sessionId)
        return null;
...
function syncProjectSession(task, sessionId, content, sourceLink) {
    const project = text(task?.target_project || task?.project_id || task?.project, 240);
    if (!project || !sessionId)
        return { status: "skipped", updated: false, issue: "project_session_identity_missing" };
    const sessions = require("../modules/projects/sessions");
    const detail = sessions.getSessionDetail(project, sessionId);
    if (!detail)
        return { status: "skipped", updated: false, issue: "project_session_unavailable" };
    const history = Array.isArray(detail.history) ? detail.history : [];
    const taskId = text(task?.id, 160);
    const preferredAnchorIds = new Set([
    ...
    // A global delegation stores both the original user intake and one Assistant
    // task card under the same taskId. Projection updates must target the card;
    // otherwise the intake is converted into a second card and the UI renders a
    // duplicate terminal result.
    const existing = history.find((message) => message?.role === "assistant"
        && messageTaskId(message) === taskId
        && preferredAnchorIds.has(text(message?.id, 180)))
        || history.find((message) => message?.role === "assistant" && messageTaskId(message) === taskId)
        || history.find((message) => messageTaskId(message) === taskId);
    const existingRevision = Math.max(-1, Number(existing?.taskExperience?.revision ?? existing?.task?.revision ?? -1));
    if (existingRevision > projectionRevision(task))
        return { status: "skipped", updated: false, issue: "stale_task_revision" };
    if (!shouldReplaceMessage(existing, task, content)) {
        appendTerminalExecutionProjection(task, "project", project, sessionId);
        return { status: "unchanged", updated: false, issue: "" };
    }
    ...
    appendTerminalExecutionProjection(task, "project", project, sessionId);
    return { status: "synced", updated: true, issue: "" };
}
function syncGroupSession(task, sessionId, content, sourceLink = false) {
    const groupId = text(task?.group_id || task?.groupId, 240);
    if (!groupId || !sessionId)
        return { status: "skipped", updated: false, issue: "group_session_identity_missing" };
    ...
    return result.projectionUpdated
        ? { status: "synced", updated: true, issue: "" }
        : { status: "unchanged", updated: false, issue: "" };
}
function syncGlobalSession(task, sessionId, content, sourceLink = false) {
    if (!sessionId)
        return { status: "skipped", updated: false, issue: "global_session_identity_missing" };
    ...
    return result?.updated ? { status: "synced", updated: true, issue: "" } : result?.reason === "session_unavailable" ? { status: "skipped", updated: false, issue: "global_session_unavailable" } : { status: "unchanged", updated: false, issue: "" };
}
function syncTaskConversationProjection(task, reason = "task_updated") {
    const taskId = text(task?.id, 160);
    const scope = scopeOf(task);
    ...
    try {
        if (scope === "project") {
            ...
            if (activeResult.updated)
                receipt.updatedSessionIds.push(targetSessionId);
            if (activeResult.issue)
                receipt.issues.push(activeResult.issue);
            if (sourceSessionId && sourceSessionId !== targetSessionId) {
                ...
                if (sourceResult.updated)
                    receipt.updatedSessionIds.push(sourceSessionId);
                if (sourceResult.issue && sourceResult.issue !== "project_session_unavailable")
                    receipt.issues.push(sourceResult.issue);
            }
            ...
            if (result.updated)
                receipt.updatedSessionIds.push(targetSessionId);
            if (result.issue)
                receipt.issues.push(result.issue);
            if (sourceSessionId && sourceSessionId !== targetSessionId) {
                ...
                if (sourceResult.updated)
                    receipt.updatedSessionIds.push(sourceSessionId);
                if (sourceResult.issue && sourceResult.issue !== "group_task_message_unavailable")
                    receipt.issues.push(sourceResult.issue);
            }
            if (receipt.updatedSessionIds.length)
            ...
            if (sourceSessionId && sourceSessionId !== targetSessionId) {
                ...
                if (sourceResult.updated)
                    receipt.updatedSessionIds.push(sourceSessionId);
                if (sourceResult.issue && sourceResult.issue !== "global_session_unavailable")
                    receipt.issues.push(sourceResult.issue);
            }
            if (receipt.updatedSessionIds.length)
                receipt.status = "synced";
    ...
    // The terminal conversation row is only one projection of a formal task.
    // Reconcile the source-backed execution tree as part of the same sync so a
    // restart or recovery cannot leave the conversation with only Terminal Gate.
    try {
        const startupTerminal = reason === "startup_reconciliation" && terminalStatuses.has(text(task?.status, 40).toLowerCase());
        if (scope !== "feishu" && !startupTerminal)
            require("./conversation-execution-reconciliation")
                .reconcileConversationExecutionForTask(task, { reason });
    }
    catch (error) {
        receipt.issues.push(`execution_reconciliation:${text(error?.message || error, 300)}`);
    }
    return receipt;
}
...
function shouldSyncTaskConversationProjection(updates) {
...
//# sourceMappingURL=task-conversation-projection.js.map