...
async function analyzeImageWithConfiguredModel(filePath, name, configOverride) {
    ...
    const receiptBase = {
        ...
        provider: (0, provider_cache_protocol_1.resolveProviderTransport)(config).protocol,
...
async function requestPinnedPublicDocument(checked) {
    ...
    if (!selected?.address || !net.isIP(selected.address))
        throw new Error("在线文档 DNS 解析结果无效");
