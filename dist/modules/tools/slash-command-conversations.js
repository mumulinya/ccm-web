...
const group_session_lifecycle_head_1 = require("../collaboration/group-session-lifecycle-head");
const group_session_lifecycle_head_2 = require("../collaboration/group-session-lifecycle-head");
const project_session_agent_binding_1 = require("../projects/project-session-agent-binding");
...
const global_agent_model_1 = require("../global/global-agent-model");
...
const global_agent_loop_engine_1 = require("../../agents/global/global-agent-loop-engine");
...
const clarification_answer_validation_1 = require("../../agents/clarification-answer-validation");
...
function normalizeScope(value) { return value === "project" || value === "group" ? value : "global"; }
function safeId(value, label) {
    const id = String(value || "").trim();
    if (!id || !/^[\p{L}\p{N}._:-]{1,180}$/u.test(id))
...
function assertApiResourceAccess(req, input, required = "use") {
    const id = identity(input);
    const principal = req?.ccmAuth;
    if (id.scope === "global" || !principal || principal.kind === "internal" || principal.role === "admin")
        return id;
    if (!(0, access_policy_1.hasResourceAccess)(String(principal.userId || ""), String(principal.role || "viewer"), id.scope, id.scopeId, required)) {
        const error = new Error(id.scope === "project" ? "当前账户没有该项目的访问权限" : "当前账户没有该群聊的访问权限");
...
function readState() {
    return (0, atomic_json_file_1.readJsonWithBackup)(STATE_FILE, { schema: "ccm-slash-command-conversation-state-v1", revision: 0, sessions: {}, records: [] });
}
function mutateState(operation) {
    return (0, atomic_json_file_1.withFileLock)(STATE_FILE, () => {
        ...
        (0, atomic_json_file_1.writeJsonAtomic)(STATE_FILE, state);
        return result;
    });
}
...
function cloneVisibleMessage(message) {
    if (!message || !["user", "assistant"].includes(String(message.role || "")))
        return null;
    if (message.hidden_execution === true || message.modelVisible === false || message.model_visible === false)
        return null;
    if (["tool_use", "tool_result", "local_command", "command_result"].includes(String(message.type || "")))
        return null;
...
function cloneVisibleMessages(messages) { return (messages || []).map(cloneVisibleMessage).filter(Boolean); }
function messageId(message, index) { return String(message?.id || message?.message_id || message?.uuid || `index:${index}`); }
function anchorIndex(messages, requested) {
    if (!messages.length)
        return -1;
    ...
    if (found < 0)
        throw new Error("锚点消息不存在或已漂移");
    return found;
}
function loadGlobalStore() { return (0, atomic_json_file_1.readJsonWithBackup)(GLOBAL_HISTORY_FILE, { current_session_id: "", sessions: [] }); }
function writeGlobalStore(store) { (0, atomic_json_file_1.writeJsonAtomic)(GLOBAL_HISTORY_FILE, store); }
function readConversation(input) {
    const id = identity(input);
    if (id.scope === "global") {
        const store = loadGlobalStore();
        const session = (store.sessions || []).find((item) => String(item.id) === id.sessionId);
        if (!session)
    ...
    if (id.scope === "project") {
        const detail = (0, sessions_1.getSessionDetail)(id.scopeId, id.sessionId);
        if (!detail)
    ...
    const messages = (0, storage_1.getGroupMessages)(id.scopeId, id.sessionId);
    const session = (0, storage_1.listGroupChatSessions)(id.scopeId).sessions.find((item) => String(item?.id || "") === id.sessionId) || null;
    return { id, title: String(session?.title || "群聊会话"), messages, raw: session };
}
function isAutomationConversation(boundary) {
    return String(boundary?.raw?.session_kind || boundary?.raw?.sessionKind || "").toLowerCase() === "automation";
}
function currentBoundary(input) {
    ...
    return {
        ...
        generation: conversation.id.scope === "project"
            ? Number((0, project_session_agent_binding_1.getProjectSessionAgentBinding)(conversation.id.scopeId, conversation.id.sessionId)?.generation || 0)
            : conversation.id.scope === "group"
                ? Number((0, group_session_lifecycle_head_2.readGroupSessionLifecycleHead)(conversation.id.scopeId, conversation.id.sessionId)?.generation || 0)
                : Number(sessionState.generation || 0),
        conversationChecksum: checksum(conversation.messages),
    };
}
function assertExpected(boundary, input) {
    ...
    if (expectedChecksum && expectedChecksum !== boundary.conversationChecksum)
        throw new Error("会话内容已漂移，请重新预览");
    if (expectedRevision !== undefined && Number(expectedRevision) !== boundary.revision)
        throw new Error("会话 revision 已漂移，请重新预览");
    if (expectedGeneration !== undefined && Number(expectedGeneration) !== boundary.generation)
        throw new Error("会话 generation 已漂移，请重新预览");
}
function prePlanProjection(message) {
...
function activePrePlanClarification(input) {
    const boundary = currentBoundary(input);
    const row = [...boundary.messages].reverse().map((message) => ({ message, projection: prePlanProjection(message) }))
        .find((item) => item.projection?.schema === "ccm-pre-plan-clarification-v1" && item.projection.status === "pending");
    return {
...
function validatePrePlanClarificationAction(input, expectedStatus = "pending") {
    const boundary = currentBoundary(input);
    ...
    if (!projection || projection.status !== expectedStatus) {
        const error = new Error("当前会话没有等待处理的计划前业务澄清");
        error.statusCode = 404;
        throw error;
    }
    if (String(input.clarificationId || input.clarification_id || input.id || "") !== String(projection.id || "")) {
        const error = new Error("澄清请求已经变化，请刷新后重试");
        error.statusCode = 409;
        error.code = "CLARIFICATION_REVISION_CONFLICT";
        throw error;
    }
    const projectionScope = normalizeScope(projection.scope);
    const projectionScopeId = projectionScope === "global" ? "global" : String(projection.scopeId || projection.scope_id || "");
    const projectionSessionId = String(projection.exactSessionId || projection.exact_session_id || "");
    if (projectionScope !== boundary.id.scope
        || projectionScopeId !== boundary.id.scopeId
        || projectionSessionId !== boundary.id.sessionId) {
        const error = new Error("澄清请求不属于当前会话，请刷新后重试");
        error.statusCode = 409;
        error.code = "CLARIFICATION_SCOPE_MISMATCH";
        throw error;
    }
    if (Number(input.revision) !== Number(projection.revision) || Number(input.generation) !== Number(projection.generation)) {
        const error = new Error("澄清版本已经变化，请刷新后重试");
        error.statusCode = 409;
        error.code = "CLARIFICATION_REVISION_CONFLICT";
        throw error;
    }
    if (Number(projection.generation) !== Number(boundary.generation)) {
        const error = new Error("会话代际已经变化，请刷新后重试");
        error.statusCode = 409;
        error.code = "CLARIFICATION_REVISION_CONFLICT";
        throw error;
    }
    const expectedChecksum = String(input.conversationChecksum || input.conversation_checksum || "");
    if (expectedChecksum && expectedChecksum !== boundary.conversationChecksum) {
        const error = new Error("会话内容已漂移，请刷新后重试");
        error.statusCode = 409;
        error.code = "CLARIFICATION_REVISION_CONFLICT";
        throw error;
    }
...
function recordClarificationSubmission(input) {
    const submissionId = safeId(input.submissionId, "提交 ID");
    const identityKey = String(input.boundary?.id?.key || "");
    const clarificationId = String(input.projection?.id || "");
    const revision = Number(input.projection?.revision || 0);
    const generation = Number(input.projection?.generation || 0);
    return mutateState(state => {
        const records = Array.isArray(state.records) ? state.records : [];
        const sameClarification = records.filter((record) => (record?.schema === "ccm-clarification-submission-receipt-v1"
        ...
        const replay = sameClarification.find((record) => record.submissionId === submissionId);
        if (replay) {
            if (replay.answerChecksum !== input.answerChecksum) {
                const error = new Error("同一提交 ID 的回答内容已经变化，请重新读取询问状态");
                error.statusCode = 409;
                error.code = "CLARIFICATION_SUBMISSION_CONFLICT";
                throw error;
            }
            return { ...replay, duplicate: true };
        }
        const equivalent = sameClarification.find((record) => record.answerChecksum === input.answerChecksum);
        if (equivalent)
            return { ...equivalent, duplicate: true };
        if (sameClarification.length) {
            const error = new Error("该询问已经提交，请等待任务继续");
            error.statusCode = 409;
            error.code = "CLARIFICATION_ALREADY_SUBMITTED";
            throw error;
        }
        const record = {
            schema: "ccm-clarification-submission-receipt-v1",
            identityKey,
            clarificationId,
            submissionId,
            revision,
            generation,
            answerChecksum: input.answerChecksum,
...
function updatePrePlanClarificationStatus(boundary, clarificationId, status) {
    const updatedAt = now();
    const messages = (boundary.messages || []).map((message) => {
        const projection = prePlanProjection(message);
        if (String(projection?.id || "") !== clarificationId)
            return message;
    ...
    if (boundary.id.scope === "global") {
        (0, atomic_json_file_1.withFileLock)(GLOBAL_HISTORY_FILE, () => {
            const store = loadGlobalStore();
            const session = (store.sessions || []).find((item) => String(item.id) === boundary.id.sessionId);
            if (!session)
                throw new Error("全局会话不存在");
        ...
        const changed = messages.find((message) => String(prePlanProjection(message)?.id || "") === clarificationId);
        if (changed)
            (0, sessions_1.upsertProjectSessionTaskMessage)(boundary.id.scopeId, boundary.id.sessionId, changed);
    }
    else {
        (0, storage_1.saveGroupMessages)(boundary.id.scopeId, messages, boundary.id.sessionId);
    }
    return nextProjectionReceipt(status, clarificationId, updatedAt);
}
function nextProjectionReceipt(status, clarificationId, updatedAt) {
    return { status, clarificationId, updatedAt, contentStored: false };
}
function persistSessionState(id, patch, expected = {}) {
    return mutateState(state => {
        const previous = state.sessions[id.key] || { revision: 0, generation: 0 };
        if (expected.revision !== undefined && Number(expected.revision) !== Number(previous.revision || 0))
            throw new Error("会话命令状态 revision 已漂移，请重新读取");
        if (expected.generation !== undefined && Number(expected.generation) !== Number(previous.generation || 0))
            throw new Error("会话命令状态 generation 已漂移，请重新读取");
        const next = { ...previous, ...patch, revision: Number(previous.revision || 0) + 1, updatedAt: now() };
        state.sessions[id.key] = next;
        return next;
    });
}
function branchConversation(input) {
    const boundary = currentBoundary(input);
    if (!input.anchorMessageId && !input.anchor_message_id)
        throw new Error("/branch 必须绑定锚点消息");
    if (!input.conversationChecksum && !input.conversation_checksum)
        throw new Error("/branch 必须提交会话 checksum");
    if (input.revision === undefined || input.generation === undefined)
        throw new Error("/branch 必须提交 revision 与 generation");
    ...
    if (boundary.id.scope === "global") {
        ...
        created = (0, sessions_1.writeProjectSessionConversationBranch)(boundary.id.scopeId, title, messages);
    }
    else {
        const session = (0, storage_1.createGroupChatSession)(boundary.id.scopeId, title);
        (0, storage_1.saveGroupMessages)(boundary.id.scopeId, messages.map(message => ({ ...message, group_session_id: session.id })), session.id);
        (0, storage_1.selectGroupChatSession)(boundary.id.scopeId, session.id);
...
function rewindMode(value) {
    const mode = String(value || "both").toLowerCase();
    if (!["both", "code", "conversation"].includes(mode))
        throw new Error("回退模式只支持 both、code、conversation");
    return mode;
}
function turnStart(messages, anchor) {
    let userIndex = anchor;
    while (userIndex >= 0 && String(messages[userIndex]?.role || "") !== "user")
        userIndex--;
    if (userIndex < 0)
        throw new Error("没有找到这轮回答对应的用户消息");
...
function collectExecutionIdentities(messages) {
    const taskIds = new Set();
    const executionIds = new Set();
    const globalRunIds = new Set();
    const visit = (value, depth = 0) => {
        ...
        for (const [key, item] of Object.entries(value)) {
            if (["task_id", "taskId", "source_task_id", "root_task_id", "retry_of_task_id"].includes(key) && typeof item === "string" && item.trim())
                taskIds.add(item.trim());
            if (["execution_id", "executionId"].includes(key) && typeof item === "string" && item.trim())
                executionIds.add(item.trim());
            if (["global_run_id", "globalRunId", "run_id", "runId"].includes(key) && typeof item === "string" && item.trim())
                globalRunIds.add(item.trim());
...
function latestRecoveryManifests(identities) {
    const seen = new Set();
    return (0, execution_recovery_1.listExecutionRecoveryManifests)(identities).filter((item) => {
...
function rewindPreview(input) {
    ...
    const codePreviews = mode === "conversation" ? [] : manifests.map((manifest) => (0, execution_recovery_1.previewExecutionRecovery)(manifest.checkpointId));
    const codeConflicts = codePreviews.flatMap((item) => item.conflicts || []);
    const unavailableCode = codePreviews.filter((item) => item.available === false);
    const plan = {
        scope: boundary.id.scope,
        scopeId: boundary.id.scopeId,
        ...
        code: codePreviews.map((item) => ({ checkpointId: item.checkpointId, previewToken: item.previewToken || "", currentHead: item.currentHead || "", available: item.available !== false })),
    };
    return {
        ...
        codeRestoreAvailable: codePreviews.some((item) => item.available !== false && (item.files || []).length > 0),
        codePreviews,
...
async function waitForTaskStop(taskIds, globalRunIds = [], boundary) {
    const activeTaskIds = taskIds.filter(taskId => (0, execution_kernel_2.listActiveAgentRuns)({ taskId }).length > 0);
    for (const taskId of activeTaskIds)
        (0, execution_kernel_2.requestTaskCancellation)(taskId, "用户请求回退本轮，安全停止当前执行", "conversation-rewind");
    const activeGlobalRunIds = globalRunIds.filter(runId => {
        try {
            return !["completed", "failed", "cancelled"].includes(String((0, global_agent_run_store_1.getGlobalAgentRun)(runId)?.status || ""));
        }
        catch {
            return false;
        }
    });
    for (const runId of activeGlobalRunIds)
        try {
            (0, global_agent_loop_engine_1.cancelGlobalAgentRun)(runId);
        }
        catch { }
    if (!activeTaskIds.length && !activeGlobalRunIds.length && boundary?.id?.scope !== "project")
        return;
    const deadline = Date.now() + 8_000;
    while (Date.now() < deadline) {
        const taskRunning = activeTaskIds.some(taskId => (0, execution_kernel_2.listActiveAgentRuns)({ taskId }).length > 0);
        ...
        const projectDispatchRunning = boundary?.id?.scope === "project"
            ? (0, project_session_agent_binding_1.isProjectSessionAgentDispatchActive)(boundary.id.scopeId, boundary.id.sessionId)
            : false;
        if (!taskRunning && !globalRunning && !projectDispatchRunning)
            return;
        await new Promise(resolve => setTimeout(resolve, 150));
    }
    throw new Error("仍有Agent进程未安全停止，暂不能回退");
}
function replaceConversation(boundary, messages, reason) {
    let generation = boundary.generation + 1;
    if (boundary.id.scope === "global") {
        (0, atomic_json_file_1.withFileLock)(GLOBAL_HISTORY_FILE, () => {
            const store = loadGlobalStore();
            const session = (store.sessions || []).find((item) => String(item.id) === boundary.id.sessionId);
            if (!session || checksum(session.messages || []) !== boundary.conversationChecksum)
                throw new Error("全局会话已漂移，请重新预览");
            session.messages = messages;
            session.updatedAt = now();
            writeGlobalStore(store);
        });
        try {
            (0, provider_neutral_context_cache_1.invalidateProviderNeutralContextCacheState)({ scope: "global", scopeId: "global", sessionId: boundary.id.sessionId }, reason);
        }
        catch { }
    }
    else if (boundary.id.scope === "project") {
        const result = (0, sessions_1.replaceProjectSessionConversation)(boundary.id.scopeId, boundary.id.sessionId, messages, reason);
        generation = Number(result.generation || generation);
    }
    else {
        (0, execution_kernel_1.requestGroupSessionAgentCancellation)({ groupId: boundary.id.scopeId, groupSessionId: boundary.id.sessionId, reason, actor: "conversation-rewind" });
        (0, storage_1.saveGroupMessages)(boundary.id.scopeId, messages, boundary.id.sessionId);
        generation = Number((0, group_session_lifecycle_head_1.rotateGroupSessionLifecycleHead)({ groupId: boundary.id.scopeId, groupSessionId: boundary.id.sessionId, reason }).head?.generation || generation);
    }
    return generation;
}
async function rewindApply(input) {
    const preview = rewindPreview(input);
    if (!input.planChecksum || String(input.planChecksum) !== preview.planChecksum)
        throw new Error("回退计划 checksum 不匹配，请重新预览");
    if (!preview.canExecute)
        throw new Error("存在文件冲突或不可用检查点，请处理后重新预览");
    const boundary = currentBoundary(input);
    assertExpected(boundary, preview);
    if (preview.taskIds.length || preview.globalRunIds.length || boundary.id.scope === "project")
        await waitForTaskStop(preview.taskIds, preview.globalRunIds, boundary);
    const codeReceipts = [];
    if (preview.mode !== "conversation") {
        try {
            for (const item of preview.codePreviews || []) {
                if (item.available === false || !(item.files || []).length)
                    continue;
                codeReceipts.push((0, execution_recovery_1.applyExecutionRecovery)(item.checkpointId, {
            ...
            for (const receipt of [...codeReceipts].reverse()) {
                if (!receipt?.recoveryCommit)
                    continue;
                try {
                    (0, execution_recovery_1.compensateExecutionRecovery)(receipt.checkpointId, receipt.recoveryCommit);
                }
                catch (compensationError) {
                    compensationFailures.push(`${receipt.checkpointId}: ${compensationError?.message || compensationError}`);
                }
            }
            if (compensationFailures.length)
                throw new Error(`多项目恢复未全部成功，且部分补偿需要人工处理：${compensationFailures.join("；")}`);
    ...
    if (preview.mode !== "code") {
        ...
        (0, atomic_json_file_1.writeJsonAtomic)(path.join(REWIND_DIR, `${snapshotId}.json`), {
        ...
        if (boundary.id.scope === "project")
            generation = Number((0, project_session_agent_binding_1.rotateProjectSessionAgentBinding)(boundary.id.scopeId, boundary.id.sessionId, "用户仅回退本轮代码").nextGeneration || generation);
        else if (boundary.id.scope === "group")
            generation = Number((0, group_session_lifecycle_head_1.rotateGroupSessionLifecycleHead)({ groupId: boundary.id.scopeId, groupSessionId: boundary.id.sessionId, reason: "用户仅回退本轮代码" }).head?.generation || generation);
        else
...
function listHistoryBranches(input) {
    const boundary = currentBoundary(input);
    const id = boundary.id;
    fs.mkdirSync(REWIND_DIR, { recursive: true });
    const branches = fs.readdirSync(REWIND_DIR).filter(name => name.endsWith(".json"))
        .map(name => (0, atomic_json_file_1.readJsonWithBackup)(path.join(REWIND_DIR, name), null)).filter(Boolean)
        .filter((item) => item.scope === id.scope && item.scopeId === id.scopeId && item.exactSessionId === id.sessionId)
        .sort((a, b) => String(b.createdAt || "").localeCompare(String(a.createdAt || "")))
        .map((item) => ({ snapshotId: item.snapshotId, kind: item.kind || "rewind", action: item.action || "", createdAt: item.createdAt, anchorMessageId: item.anchorMessageId || "", removedMessages: Array.isArray(item.removedMessages) ? item.removedMessages.length : 0, originalMessages: Array.isArray(item.originalMessages) ? item.originalMessages.length : 0, status: item.status || "available", contentStored: false }));
...
function summarizeAction(value) {
    ...
    if (!new Set(["summarize_from", "summarize_up_to"]).has(action))
        throw new Error("未知的会话总结范围");
    return action;
}
function assertConversationIdleForSummary(boundary) {
    const activeRuns = (0, execution_kernel_2.listActiveAgentRuns)();
    const projectBusy = boundary.id.scope === "project" && (0, project_session_agent_binding_1.isProjectSessionAgentDispatchActive)(boundary.id.scopeId, boundary.id.sessionId);
    const globalBusy = boundary.id.scope === "global" && (0, global_agent_run_store_1.listGlobalAgentRuns)({ sessionId: boundary.id.sessionId, limit: 20 })
        .some((run) => ["running", "supervising", "paused", "waiting_confirmation", "waiting_clarification"].includes(String(run?.status || "")));
    const affected = collectExecutionIdentities(boundary.messages);
    const taskBusy = activeRuns.some((run) => affected.taskIds.includes(String(run.taskId || "")));
    if (projectBusy || globalBusy || taskBusy) {
        const error = new Error("当前任务仍在运行，请先等待完成、停止任务或使用“调整方向”后再总结会话");
        error.statusCode = 409;
...
function summarizePreview(input) {
    ...
    if (count < 2)
        throw new Error("所选范围不足以形成可靠的会话总结");
    const plan = {
        action,
        scope: boundary.id.scope,
        scopeId: boundary.id.scopeId,
...
function summarySourceText(messages) {
...
function assertSafeConversationSummary(value) {
...
function restoreHistoryBranch(input, snapshotId) {
    const boundary = currentBoundary(input);
    assertExpected(boundary, input);
    const file = path.join(REWIND_DIR, `${safeId(snapshotId, "历史分支 ID")}.json`);
    const branch = (0, atomic_json_file_1.readJsonWithBackup)(file, null);
    if (!branch || branch.scope !== boundary.id.scope || branch.scopeId !== boundary.id.scopeId || branch.exactSessionId !== boundary.id.sessionId)
        throw new Error("历史分支不存在或无权访问");
    if (!Array.isArray(branch.originalMessages))
        throw new Error("该历史快照不支持恢复");
    ...
    (0, atomic_json_file_1.writeJsonAtomic)(file, branch);
...
function safeProjection(value, depth = 0) {
    ...
    if (typeof value === "string")
        return value.replace(/\b(?:Bearer\s+)?(?:sk-)?[A-Za-z0-9_-]{24,}\b/g, "[redacted]").slice(0, 1200);
    if (Array.isArray(value))
        return value.slice(0, 80).map(item => safeProjection(item, depth + 1));
    if (typeof value === "object")
        return Object.fromEntries(Object.entries(value).filter(([key]) => !BLOCKED_DETAIL_KEY.test(key)).slice(0, 100).map(([key, item]) => [key, safeProjection(item, depth + 1)]));
    return String(value).slice(0, 200);
}
function persistLocalCommandRecord(input) {
    const id = identity(input);
    ...
    if (id.scope === "global") {
        ...
        (0, sessions_1.appendProjectSessionLocalCommandRecord)(id.scopeId, id.sessionId, message);
    }
    else {
        (0, storage_1.saveGroupMessages)(id.scopeId, [...(0, storage_1.getGroupMessages)(id.scopeId, id.sessionId), { ...message, group_session_id: id.sessionId }], id.sessionId);
    }
    mutateState(state => { state.records.push(record); return record; });
    return record;
}
function readSessionState(input) {
    const id = identity(input);
    const state = readState();
    const stored = state.sessions?.[id.key] || { revision: 0, generation: 0 };
    const generation = id.scope === "project"
        ? Number((0, project_session_agent_binding_1.getProjectSessionAgentBinding)(id.scopeId, id.sessionId)?.generation || 0)
        : id.scope === "group"
            ? Number((0, group_session_lifecycle_head_2.readGroupSessionLifecycleHead)(id.scopeId, id.sessionId)?.generation || 0)
...
function updatePlanMode(input) {
    ...
    if (input.revision === undefined || input.generation === undefined)
        throw new Error("Plan Mode 更新必须绑定 revision 与 generation");
    if (Number(input.generation) !== Number(state.generation || 0))
...
function updatePreferences(input) {
    const { id, state } = readSessionState(input);
    if (input.revision === undefined || input.generation === undefined)
        throw new Error("会话偏好更新必须绑定 revision 与 generation");
    if (Number(input.generation) !== Number(state.generation || 0))
        throw new Error("会话 generation 已漂移，请重新读取");
    const previous = state.preferences || {};
    const patch = {};
    if (input.model !== undefined)
        patch.model = String(input.model || "").slice(0, 200);
    if (input.effort !== undefined) {
        const effort = String(input.effort || "").toLowerCase();
        if (!["low", "medium", "high"].includes(effort))
            throw new Error("effort 只支持 low、medium、high");
        patch.effort = effort;
    }
    if (input.fast !== undefined) {
        if (input.fast === true)
            throw new Error("当前 Provider 适配器尚未声明可核验的 Fast Mode 能力，本会话不会伪装为已启用");
        patch.fast = false;
    }
    if (input.outputStyle !== undefined || input.output_style !== undefined) {
        ...
        if (!["concise", "balanced", "detailed"].includes(outputStyle))
            throw new Error("output-style 只支持 concise、balanced、detailed");
...
async function body(req) { return JSON.parse((await (0, utils_1.collectRequestBuffer)(req)).toString("utf8") || "{}"); }
function handleSlashCommandConversationApi(pathname, req, res, parsed) {
    if (pathname === "/api/conversations/clarifications/active" && req.method === "GET") {
        try {
            assertApiResourceAccess(req, parsed.query || {}, "use");
            (0, utils_1.sendJson)(res, { success: true, result: activePrePlanClarification(parsed.query || {}) });
        }
        catch (error) {
            (0, utils_1.sendJson)(res, { success: false, error: error.message || String(error) }, error.statusCode || 400);
        }
        return true;
    }
    ...
    if (clarificationAction && req.method === "POST") {
        ...
        void body(req).then(value => {
            ...
            if (action === "cancel")
                updatePrePlanClarificationStatus(checked.boundary, checked.projection.id, "cancelled");
            const validated = action === "cancel"
                ? null
                : (0, clarification_answer_validation_1.validateClarificationAnswerSubmission)(checked.projection, value, { useDefaults: action === "defaults" });
            const submission = validated
                ? recordClarificationSubmission({
                    boundary: checked.boundary,
                    projection: checked.projection,
                    submissionId: value.submissionId || value.submission_id,
                    answerChecksum: validated.answerChecksum,
            ...
            return {
                ...
                ...(validated ? {
                    ...
                    answerChecksum: validated.answerChecksum,
                    answeredQuestionCount: validated.completedQuestionIds.length,
            ...
            .catch((error) => (0, utils_1.sendJson)(res, { success: false, error: error.message || String(error), code: error.code }, error.statusCode || 409));
        return true;
    }
    if (pathname === "/api/conversations/permission-mode" && req.method === "GET") {
        try {
            ...
            let policy = (0, conversation_permission_policy_1.readConversationPermissionPolicy)(parsed.query || {});
            if (isAutomationConversation(boundary) && policy.revision === 0 && policy.source === "manual_default") {
                policy = (0, conversation_permission_policy_1.updateConversationPermissionPolicy)({ ...parsed.query, mode: "full_access", source: "automation_default", revision: 0, generation: boundary.generation });
            }
            (0, utils_1.sendJson)(res, { success: true, result: { ...policy, generation: boundary.generation } });
        }
        catch (error) {
            (0, utils_1.sendJson)(res, { success: false, error: error.message || String(error) }, error.statusCode || 400);
        }
    ...
    if (pathname === "/api/conversations/permission-mode" && req.method === "PATCH") {
        void body(req).then(async (value) => {
            assertApiResourceAccess(req, value, "use");
            const current = (0, conversation_permission_policy_1.readConversationPermissionPolicy)(value);
            ...
            if (value.generation === undefined || Number(value.generation) !== Number(currentConversation.generation)) {
                const error = new Error("会话 generation 已更新，请重新读取权限模式");
                error.statusCode = 409;
                throw error;
            }
            const rank = { ask_before_edit: 0, main_agent_only: 1, full_access: 2 };
            if (rank[String(value.mode)] < rank[current.mode]) {
                ...
                const activeTaskIds = execution.taskIds.filter(taskId => (0, execution_kernel_2.listActiveAgentRuns)({ taskId }).length > 0);
                if (activeTaskIds.length || (boundary.id.scope === "project" && (0, project_session_agent_binding_1.isProjectSessionAgentDispatchActive)(boundary.id.scopeId, boundary.id.sessionId))) {
                    await waitForTaskStop(activeTaskIds, [], boundary);
                }
            }
            return (0, conversation_permission_policy_1.updateConversationPermissionPolicy)(value);
        })
            .then(result => (0, utils_1.sendJson)(res, { success: true, result })).catch((error) => (0, utils_1.sendJson)(res, { success: false, error: error.message || String(error), code: error.code }, error.statusCode || 400));
        return true;
    }
    ...
    if (pathname === "/api/conversations/history-branches" && req.method === "GET") {
        try {
            assertApiResourceAccess(req, parsed.query || {}, "use");
            (0, utils_1.sendJson)(res, { success: true, result: listHistoryBranches(parsed.query || {}) });
        }
        catch (error) {
    ...
    if (restoreMatch && req.method === "POST") {
        void body(req).then(value => { assertApiResourceAccess(req, value, "use"); return (0, utils_1.sendJson)(res, { success: true, result: restoreHistoryBranch(value, decodeURIComponent(restoreMatch[1])) }); }).catch((error) => (0, utils_1.sendJson)(res, { success: false, error: error.message || String(error) }, error.statusCode || 409));
        return true;
    }
    ...
    if (pathname === "/api/conversations/preferences" && req.method === "GET") {
        try {
            const { id, state } = readSessionState(parsed.query || {});
            (0, utils_1.sendJson)(res, { success: true, scope: id.scope, scopeId: id.scopeId, exactSessionId: id.sessionId, generation: Number(state.generation || 0), revision: Number(state.revision || 0), preferences: state.preferences || {} });
        }
        catch (error) {
            (0, utils_1.sendJson)(res, { success: false, error: error.message }, 400);
        }
    ...
    if (!handler || !["POST", "PATCH"].includes(String(req.method)))
        return false;
    void body(req).then(value => { assertApiResourceAccess(req, value, "use"); return Promise.resolve(handler(value)); }).then(result => (0, utils_1.sendJson)(res, { success: true, result })).catch((error) => (0, utils_1.sendJson)(res, { success: false, error: error.message || String(error) }, error.statusCode || 409));
    return true;
}
function runSlashCommandConversationSelfTest() {
...
//# sourceMappingURL=slash-command-conversations.js.map