...
async function performRebuild(reason) {
    ...
    // MCP servers communicate over stdout using JSON-RPC; keep diagnostics on
    // stderr so Claude Code does not interpret them as malformed protocol
    // frames.  The regular web process keeps the existing console output.
    const log = process.env.CCM_INTERNAL_MCP_CONTEXT ? console.error : console.log;
    log(`[RAG] 索引构建完成：${files.length} 份文档，${nextChunks.length} 个语义分片，缓存命中 ${indexStatus.cacheHits}`);
