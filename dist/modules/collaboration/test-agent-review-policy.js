...
exports.normalizeTestAgentAcceptanceEvidencePlan = normalizeTestAgentAcceptanceEvidencePlan;
exports.normalizeTestAgentVerificationProfile = normalizeTestAgentVerificationProfile;
exports.deriveTestAgentReviewPolicy = deriveTestAgentReviewPolicy;
exports.buildTestAgentIncrementalScope = buildTestAgentIncrementalScope;
exports.classifyTestAgentReview = classifyTestAgentReview;
exports.runTestAgentReviewPolicySelfTest = runTestAgentReviewPolicySelfTest;
const EVIDENCE_TYPES = new Set(["code_diff", "command", "http", "browser", "artifact"]);
const REVIEW_TIERS = new Set(["lightweight", "standard", "interactive", "critical"]);
const CHANGE_CLASSES = new Set(["documentation", "configuration", "code", "interactive", "critical"]);
const TIER_RANK = { lightweight: 0, standard: 1, interactive: 2, critical: 3 };
function text(value, max = 800) {
    return String(value || "").trim().slice(0, max);
}
function strings(value, max = 20, itemMax = 800) {
...
function normalizeTestAgentAcceptanceEvidencePlan(value) {
    ...
    const normalized = rows.map((row, index) => {
        ...
        const evidenceTypes = strings(row.evidenceTypes || row.evidence_types, 5, 40)
            .filter(item => EVIDENCE_TYPES.has(item));
    ...
    if (!normalized.length)
        throw new Error("主 Agent 未生成可执行的验收证据计划");
    return normalized;
}
function normalizeTestAgentVerificationProfile(value) {
...
function deriveTestAgentReviewPolicy(input) {
    ...
    const requestedTier = REVIEW_TIERS.has(input.profile?.tier)
        ? input.profile.tier
        : "lightweight";
    ...
    if (decision.riskLevel === "high" || modes.has("release") || changeClass === "critical")
        tier = "critical";
    else if (browserEnabled || changeClass === "interactive")
        tier = atLeast(tier, "interactive");
    else if (decision.requiresCodeChanges === true || changeClass === "code")
...
function coverageCriteria(review, expectedStatus) {
    ...
    return strings(rows
        .filter((item) => expectedStatus.includes(status(item?.status)))
        .map((item) => item?.criterion), 20, 800);
}
function resultCommands(review, expectedStatus) {
    const report = review?.report || review?.invocation?.report || {};
    return strings((Array.isArray(report.commandResults) ? report.commandResults : [])
        .filter((item) => expectedStatus.includes(status(item?.status)))
        .map((item) => item?.command), 20, 300);
}
function buildTestAgentIncrementalScope(input) {
    ...
    const focusedAcceptanceCriteria = strings([
        ...
        ...(previous?.verdict?.failedAcceptanceCriteria || []).map((item) => item?.criterion),
        ...(previous?.verdict?.unknownAcceptanceCriteria || []).map((item) => item?.criterion),
    ], 20, 800);
    ...
    const browserCheckNames = strings((Array.isArray(report.browserResults) ? report.browserResults : [])
        .filter((item) => ["failed", "blocked", "partial", "timed_out"].includes(status(item?.status)))
        .map((item) => item?.name), 12, 300);
...
function resultsByStatus(report, expected) {
    return ["commandResults", "devServerResults", "httpResults", "browserResults"]
        .flatMap(key => Array.isArray(report?.[key]) ? report[key] : [])
        .filter((item) => expected.includes(status(item?.status)));
}
function classifyTestAgentReview(review) {
    if (review?.canAccept === true)
        return { route: "accept", reason: "TestAgent 证据门禁已通过" };
    const report = review?.report || review?.invocation?.report || {};
    const verdict = review?.verdict || review?.invocation?.verdict || {};
    ...
    const unknownCoverage = [
        ...
        ...(Array.isArray(report.acceptanceCoverage) ? report.acceptanceCoverage : []),
    ].filter((item) => status(item?.status) === "unknown");
...
function runTestAgentReviewPolicySelfTest() {
...
//# sourceMappingURL=test-agent-review-policy.js.map