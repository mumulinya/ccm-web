"use strict";
// collaboration-agent-probes.ts — merged from 2 part files (behavior-freeze merge).
...
// ===== merged from collaboration-agent-probes-part-01.ts =====
...
// ===== merged from collaboration-agent-probes-part-02.ts =====
// Extracted functional module. The original entry remains a compatibility facade.
...
function buildDailyDevAgentDiagnostics() {
    ...
    // 自测抛错时降级为 fail 检查而不是让整个诊断端点崩掉，后续检查（含计划核心自检）仍能注册。
    const runSelfTestSafely = (runner, label) => {
        try {
            return runner();
        }
        catch (error) {
            return { pass: false, error: String(error?.message || error || `${label} 自检执行时抛出异常`) };
        }
    };
    const collaborationUx = runSelfTestSafely(() => (0, collaboration_1.runCollaborationUxSelfTest)(), "群聊协作体验");
    ...
    const planCore = runSelfTestSafely(() => (0, main_agent_plan_core_1.runMainAgentPlanCoreSelfTest)(), "主 Agent 计划核心");
    checks.push((0, collaboration_1.createDiagnosticCheck)("main-agent-plan-core", "主 Agent 计划核心", planCore.pass ? "ok" : "fail", planCore.pass
        ? "计划书修订与执行期贯穿自检通过：追加要求会并入计划步骤并记录修订历史，等待确认的计划要求重新确认，模型计划步骤随任务阶段推进"
        : "主 Agent 计划核心自检失败，需要检查追加要求并入、修订历史或模型步骤贯穿逻辑", planCore));
    ...
    const mainAgentToolLoop = runSelfTestSafely(() => (0, collaboration_1.runGroupMainAgentToolLoopSelfTest)(), "群聊主 Agent 多步工作循环");
