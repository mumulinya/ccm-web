...
function evaluateGroupSessionMemoryUpdateCadence(messages, previousSnapshot = {}, options = {}) {
    ...
    const shared = (0, session_compaction_core_1.evaluateSessionMemoryCadence)(rows, {
        summary: previousSnapshot?.hasSummary === true || previousSnapshot?.modelExtracted === true || previous.initialized === true ? { present: true } : null,
        lastExtractedMessageId: previous.lastExtractionMessageId || previous.last_extraction_message_id,
        tokensAtLastExtraction: previous.tokensAtLastExtraction,
    }, {
        tokenBasis: options.tokenBasis || options.token_basis,
        requireCanonicalTokenBasis: options.requireCanonicalTokenBasis === true || options.require_canonical_token_basis === true,
    });
    const currentContextTokens = shared.totalTokens;
    ...
    const tokensSinceLastExtraction = shared.growthTokens;
    ...
    const toolCallsSinceLastExtraction = shared.toolCallsSinceLastExtraction;
    const lastAssistantTurnHasToolCalls = !shared.naturalBreak;
    const tokenThresholdMet = initialized && shared.growthReady;
    const toolCallThresholdMet = toolCallsSinceLastExtraction >= toolCallsBetweenUpdates;
    const naturalBreak = shared.naturalBreak;
    const shouldExtract = shared.shouldExtract;
    const lastMessageId = rows.length ? (0, group_memory_shared_1.getMemoryMessageIdentity)(rows[rows.length - 1], rows.length - 1) : "";
    const status = shared.reason === "canonical_payload_unavailable" ? "canonical_payload_unavailable"
        : shouldExtract
            ? "extraction_due"
