"use strict";
// group-session-memory-model-extraction.ts — merged from 3 part files (behavior-freeze merge).
...
exports.appendHookRegistered = exports.pending = exports.running = exports.typedMemoryRetryRunning = exports.typedMemoryRetryTimers = exports.timers = exports.configuredExecutor = exports.TYPED_MEMORY_RETRY_MAX_ATTEMPTS = exports.TYPED_MEMORY_RETRY_MAX_DELAY_MS = exports.TYPED_MEMORY_RETRY_BASE_DELAY_MS = exports.MODEL_EXECUTOR_UNAVAILABLE_RETRY_MS = exports.MODEL_EXTRACTION_EXECUTION_TIMEOUT_MS = exports.MODEL_EXTRACTION_DEBOUNCE_MS = exports.validateGroupSessionMemoryCustomPrompt = exports.saveGroupSessionMemoryCustomTemplate = exports.saveGroupSessionMemoryCustomPrompt = exports.readGroupSessionMemoryCustomTemplateProfile = exports.readGroupSessionMemoryCustomPromptProfile = exports.parseGroupSessionMemoryTemplate = exports.inspectGroupSessionMemoryTemplateState = exports.GROUP_SESSION_MEMORY_MODEL_TEMPLATE = void 0;
exports.substituteGroupSessionMemoryCustomPrompt = substituteGroupSessionMemoryCustomPrompt;
exports.setConfiguredExecutor = setConfiguredExecutor;
exports.setAppendHookRegistered = setAppendHookRegistered;
exports.hashText = hashText;
exports.readBoundedGroupTypedMemoryManifest = readBoundedGroupTypedMemoryManifest;
exports.readCommittedDirectMemoryWriteProofs = readCommittedDirectMemoryWriteProofs;
exports.writeJsonAtomic = writeJsonAtomic;
exports.writeBufferAtomic = writeBufferAtomic;
exports.fileChecksum = fileChecksum;
exports.modelReceiptFile = modelReceiptFile;
exports.modelFailureReceiptFile = modelFailureReceiptFile;
...
exports.withArtifactRetentionLock = withArtifactRetentionLock;
exports.readGroupSessionMemoryModelExtractionHistory = readGroupSessionMemoryModelExtractionHistory;
exports.appendGroupSessionMemoryModelExtractionHistory = appendGroupSessionMemoryModelExtractionHistory;
exports.receiptChecksum = receiptChecksum;
...
exports.persistGroupSessionMemoryModelExtractionReplayEvidence = persistGroupSessionMemoryModelExtractionReplayEvidence;
exports.messageIdentity = messageIdentity;
exports.transcriptRows = transcriptRows;
exports.renderGroupSessionMemoryModelExtractionPrompt = renderGroupSessionMemoryModelExtractionPrompt;
exports.buildGroupSessionMemoryModelExtractionPrompt = buildGroupSessionMemoryModelExtractionPrompt;
exports.safeExtractionExecutionId = safeExtractionExecutionId;
exports.getGroupSessionMemoryModelExtractionArtifactFile = getGroupSessionMemoryModelExtractionArtifactFile;
exports.extractionArtifactRetentionManifestChecksum = extractionArtifactRetentionManifestChecksum;
exports.extractionArtifactArchiveFile = extractionArtifactArchiveFile;
exports.readExtractionArtifactFile = readExtractionArtifactFile;
...
exports.writeGroupSessionMemoryModelExtractionArtifact = writeGroupSessionMemoryModelExtractionArtifact;
exports.readGroupSessionMemoryModelExtractionArtifact = readGroupSessionMemoryModelExtractionArtifact;
exports.resolveExtractionArtifactRetentionPolicy = resolveExtractionArtifactRetentionPolicy;
exports.listHotExtractionArtifacts = listHotExtractionArtifacts;
exports.readCurrentExtractionReceiptExecutionIds = readCurrentExtractionReceiptExecutionIds;
...
// ===== merged from group-session-memory-model-extraction-part-01.ts =====
...
exports.MODEL_EXTRACTION_DEBOUNCE_MS = Math.max(250, Number(process.env.CCM_GROUP_SESSION_MEMORY_MODEL_DEBOUNCE_MS || 2500));
...
exports.MODEL_EXTRACTION_EXECUTION_TIMEOUT_MS = Math.max(10_000, Number(process.env.CCM_GROUP_SESSION_MEMORY_MODEL_TIMEOUT_MS || 130_000));
exports.MODEL_EXECUTOR_UNAVAILABLE_RETRY_MS = Math.max(5_000, Number(process.env.CCM_GROUP_SESSION_MEMORY_EXECUTOR_RETRY_MS || 30_000));
...
exports.TYPED_MEMORY_RETRY_BASE_DELAY_MS = Math.max(1_000, Number(process.env.CCM_MODEL_EXTRACTION_TYPED_MEMORY_RETRY_MS || 30_000));
exports.TYPED_MEMORY_RETRY_MAX_DELAY_MS = Math.max(exports.TYPED_MEMORY_RETRY_BASE_DELAY_MS, 30 * 60_000);
exports.TYPED_MEMORY_RETRY_MAX_ATTEMPTS = Math.max(1, Number(process.env.CCM_MODEL_EXTRACTION_TYPED_MEMORY_RETRY_ATTEMPTS || 12));
...
exports.configuredExecutor = null;
function setConfiguredExecutor(value) {
    exports.configuredExecutor = value;
    return exports.configuredExecutor;
}
exports.timers = new Map();
exports.typedMemoryRetryTimers = new Map();
exports.typedMemoryRetryRunning = new Set();
exports.running = new Set();
exports.pending = new Set();
exports.appendHookRegistered = false;
function setAppendHookRegistered(value) {
    exports.appendHookRegistered = value;
    return exports.appendHookRegistered;
}
...
// ===== merged from group-session-memory-model-extraction-part-02.ts =====
...
function upsertGroupSessionMemoryTypedMemoryRetry(scopeId, input) {
    ...
    const next = {
        ...
        maxAttempts: exports.TYPED_MEMORY_RETRY_MAX_ATTEMPTS,
...
function typedMemoryRetryDelay(attempts) {
    return Math.min(exports.TYPED_MEMORY_RETRY_MAX_DELAY_MS, exports.TYPED_MEMORY_RETRY_BASE_DELAY_MS * Math.pow(2, Math.max(0, attempts - 1)));
}
function scheduleGroupSessionMemoryTypedMemoryRetry(scopeId, executionId, options = {}) {
    const key = `${scopeId}::${executionId}`;
    const existing = exports.typedMemoryRetryTimers.get(key);
    if (existing)
        clearTimeout(existing);
    const delayMs = Math.max(0, Number(options.delayMs ?? options.delay_ms ?? exports.TYPED_MEMORY_RETRY_BASE_DELAY_MS));
    const timer = setTimeout(() => {
        exports.typedMemoryRetryTimers.delete(key);
        const state = readGroupSessionMemoryTypedMemoryRetryState(scopeId);
    ...
    timer.unref?.();
    exports.typedMemoryRetryTimers.set(key, timer);
...
function retryGroupSessionModelExtractionTypedMemory(scopeId, executionId, options = {}) {
    const key = `${scopeId}::${executionId}`;
    if (exports.typedMemoryRetryRunning.has(key))
    ...
    exports.typedMemoryRetryRunning.add(key);
    ...
    try {
        ...
        const exhausted = attempts >= exports.TYPED_MEMORY_RETRY_MAX_ATTEMPTS;
        ...
        exports.typedMemoryRetryRunning.delete(key);
    }
}
...
function configureGroupSessionMemoryModelExecutor(executor) {
    const wasConfigured = typeof exports.configuredExecutor === "function";
    setConfiguredExecutor(typeof executor === "function" ? executor : null);
    const shouldRecover = !wasConfigured && !!exports.configuredExecutor;
    if (shouldRecover) {
        const timer = setTimeout(() => {
            try {
    ...
    return { configured: !!exports.configuredExecutor, recoveryScheduled: shouldRecover };
}
// ===== merged from group-session-memory-model-extraction-part-03.ts =====
...
async function runGroupSessionMemoryModelExtractionNow(groupId, options = {}) {
    ...
    const currentGroupMemory = (0, memory_1.loadGroupMemory)(id, groupSessionId);
    const boundaryGeneration = Math.max(0, Number(currentGroupMemory?.unifiedSessionCompaction?.boundaryGeneration || currentGroupMemory?.compaction?.boundaryGeneration || 0));
    const tokenBasisSelection = (0, session_memory_token_basis_1.selectCanonicalSessionMemoryTokenBasis)((0, canonical_context_accounting_1.readCanonicalContextAccounting)("group", String(id || ""), groupSessionId), { scope: "group", scopeId: String(id || ""), exactSessionId: groupSessionId, boundaryGeneration });
    const suppliedCadence = options.cadenceDecision || options.cadence_decision || null;
    const cadence = suppliedCadence?.schema
        ? suppliedCadence
        : (0, memory_1.evaluateGroupSessionMemoryUpdateCadence)(messages, previousSnapshot, {
            ...(options.cadence || options),
            tokenBasis: tokenBasisSelection.basis,
            requireCanonicalTokenBasis: true,
        });
    ...
    const executor = options.executor || exports.configuredExecutor;
    if (typeof executor !== "function") {
        ...
        return { committed: false, status: "model_executor_unavailable", retryInMs: exports.MODEL_EXECUTOR_UNAVAILABLE_RETRY_MS, cadence, state, modelInvoked: false };
    }
    ...
    const transaction = await (0, group_session_memory_extraction_1.runGroupSessionMemoryExtractionTransactionAsync)(scopeId, async (extraction) => {
        ...
        const executionTimeoutMs = Math.max(1_000, Number(options.modelTimeoutMs || options.model_timeout_ms || exports.MODEL_EXTRACTION_EXECUTION_TIMEOUT_MS));
        let timeout = null;
        modelInvoked = true;
        ...
        const safeCursorIndex = cursorAdvance.cursorAfter
            ? messages.findIndex((message, index) => messageIdentity(message, index) === cursorAdvance.cursorAfter)
            : -1;
        if (memory?.conversationSummary && safeCursorIndex >= 0) {
            preparedSnapshot.rollingSessionMemory = (0, rolling_session_memory_1.finalizeSharedRollingSessionMemory)({
                scope: "group",
                exactSessionId: `${id}:${groupSessionId}`,
                generation: boundaryGeneration,
                summary: memory.conversationSummary,
                messages,
                summarizedThroughMessageId: cursorAdvance.cursorAfter,
                cadence: committedCadence,
                provider: executorMetadata.provider,
                model: executorMetadata.model,
            });
        }
...
function scheduleGroupSessionMemoryModelExtraction(groupId, options = {}) {
    ...
    if (exports.timers.has(scopeKey))
        clearTimeout(exports.timers.get(scopeKey));
    const delayMs = Math.max(0, Number(options.delayMs ?? options.delay_ms ?? exports.MODEL_EXTRACTION_DEBOUNCE_MS));
    const timer = setTimeout(async () => {
        exports.timers.delete(scopeKey);
        if (exports.running.has(scopeKey)) {
            exports.pending.add(scopeKey);
            return;
        }
        exports.running.add(scopeKey);
        let result = null;
        try {
            result = await runGroupSessionMemoryModelExtractionNow(id, { ...options, groupSessionId });
        }
        finally {
            exports.running.delete(scopeKey);
            if (exports.pending.has(scopeKey)) {
                exports.pending.delete(scopeKey);
                scheduleGroupSessionMemoryModelExtraction(id, { ...options, groupSessionId, delayMs: exports.MODEL_EXTRACTION_DEBOUNCE_MS });
            }
