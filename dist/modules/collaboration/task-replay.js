...
exports.buildTaskReplayFreshness = buildTaskReplayFreshness;
exports.buildTaskReplayUserReport = buildTaskReplayUserReport;
exports.buildTaskReplayAuditExport = buildTaskReplayAuditExport;
exports.projectTaskReplayForAccess = projectTaskReplayForAccess;
exports.buildTaskReplayIndex = buildTaskReplayIndex;
...
const task_replay_delivery_1 = require("./task-replay-delivery");
const task_replay_plan_1 = require("./task-replay-plan");
const task_replay_presentation_1 = require("./task-replay-presentation");
...
function safeTechnical(value, depth = 0) {
    ...
    if (Array.isArray(value))
        return value.slice(0, 60).map(item => safeTechnical(item, depth + 1));
    if (typeof value === "string")
        return (0, task_replay_shared_1.safeText)(value, 800);
    if (typeof value !== "object")
        return value;
    const result = {};
    for (const [key, item] of Object.entries(value)) {
        if (/nativeSession|native_session|previousNative|mcpConfigPath|mcp_config_path|snapshotPath|snapshot_path|workDir|realWorkDir|stdoutPath|stderrPath|handoffPath|file_path/i.test(key))
            continue;
        if (/(api.?key|token|password|authorization|secret)/i.test(key)) {
            result[key] = "[已隐藏]";
            continue;
        }
...
function safeDiffText(value, max = 220_000) {
...
function replayChangeRows(value) {
    if (Array.isArray(value))
        return value;
...
function normalizeReplayChange(item, fallbackProject = "") {
    const filePath = (0, task_replay_shared_1.publicFile)(item);
    if (!filePath)
        return null;
    ...
    const project = (0, task_replay_shared_1.safeText)(source.project || source.target_project || source.projectName || source.agent || fallbackProject, 100);
    const statusText = (0, task_replay_shared_1.safeText)(source.statusText || source.status_label || source.status || "变更", 40);
    const unavailableReason = (0, task_replay_shared_1.safeText)(sourceDiff.reason || (rawDiff ? "" : "该任务当时只保存了文件与行数统计，无法还原逐行代码内容"), 180);
    return {
        path: filePath,
        project,
        agent: (0, task_replay_shared_1.safeText)(source.agent || project, 100),
        statusText,
        statusKind: (0, task_replay_shared_1.safeText)(source.statusKind || source.status_kind || "changed", 40),
        statusColor: (0, task_replay_shared_1.safeText)(source.statusColor || source.status_color || "#64748b", 20),
        additions,
        deletions,
        diff: {
            available: !!rawDiff,
...
function mergeReplayChange(current, incoming) {
...
function replayTaskChanges(task) {
    const fallbackProject = String(task?.target_project || "");
    const summary = task?.delivery_summary || {};
    const rows = [];
    const append = (value, project = fallbackProject) => {
        for (const item of replayChangeRows(value))
    ...
    for (const execution of (0, execution_kernel_1.listExecutions)({ taskId: task?.id })) {
        append(execution?.fileChanges, String(execution?.project || execution?.agent || fallbackProject));
    }
    if (task?.group_id) {
        const sessionId = String(task.group_session_id || task.groupSessionId || "default");
        for (const message of (0, storage_1.getGroupMessages)(task.group_id, sessionId)) {
            if (String(message?.task_id || message?.task?.id || "") !== String(task.id || ""))
                continue;
...
function eventAudience(input) {
    if (["failed", "blocked", "warning"].includes(input.status))
        return "user";
    if (!LOW_LEVEL_SOURCES.has(input.source))
        return "user";
...
function actor(type, label) {
    ...
    return { type, label: (0, task_replay_shared_1.safeText)(label || defaults[type], 80) || defaults[type] };
}
function event(input) {
    const source = String(input.source || "task");
    const at = (0, task_replay_shared_1.iso)(input.at, new Date(0).toISOString());
    const status = input.status || "info";
    const title = (0, task_replay_shared_1.safeText)(input.title, 180) || "任务记录";
    const rawSummary = (0, task_replay_shared_1.safeText)(input.summary, 1200);
...
function addReplayEventLinks(events, tasks) {
    ...
    return events.map(row => {
        ...
        const workItemId = (0, task_replay_shared_1.safeText)(technical.work_item_id || technical.workItemId, 120);
        const batchId = (0, task_replay_shared_1.safeText)(technical.batch_id || technical.batchId, 120);
        const dependencyIds = (0, task_replay_shared_1.stringList)(technical.dependency_ids || technical.dependencyIds || technical.blocked_by, 30);
        const criterionIds = (0, task_replay_shared_1.stringList)(technical.criterion_ids || technical.criterionIds, 30);
...
function rootTaskFor(task, byId) {
    let current = task;
    const seen = new Set();
    ...
    while (changed) {
        ...
        for (const task of tasks) {
            ...
            const related = ids.has(String(task.parent_task_id || ""))
                || (Array.isArray(task.child_task_ids) && task.child_task_ids.some((child) => ids.has(String(child))))
                || (!!task.global_mission_id && missionIds.has(String(task.global_mission_id)));
            if (id && related && !ids.has(id)) {
                ids.add(id);
                changed = true;
            }
        }
    }
    return { root, selected, tasks: tasks.filter((task) => ids.has(String(task.id))), ids };
}
function taskLabel(task) {
    return (0, task_replay_shared_1.safeText)(task?.title || task?.business_goal || task?.description || "未命名任务", 180);
}
function messageActor(message) {
    if (message?.role === "user")
        return actor("user", "用户");
    const name = String(message?.agent || message?.project || message?.target || "");
    if (/test.?agent/i.test(name))
        return actor("test_agent", "TestAgent");
    if (name && !/coordinator|主.?agent|global/i.test(name))
        return actor("project_agent", name);
    return actor("group_agent", "群聊主 Agent");
}
function taskOwnerActor(task) {
    if (task?.orchestration_scope === "project_session")
        return actor("project_agent", "项目主 Agent");
    if (task?.workflow_type === "global_mission" || task?.assign_type === "global")
        return actor("global_agent", "全局主 Agent");
    return actor("group_agent", "群聊主 Agent");
}
// 门禁/检查类时间线事件常只带"N 项未通过"的计数文本，明细在 data.failed_checks / data.checks 里；
// 把未通过项拼进摘要、完整清单放进技术详情，用户才能看懂到底是哪些没过。
function failedCheckLines(data) {
    const rows = Array.isArray(data?.failed_checks) && data.failed_checks.length
        ? data.failed_checks
        : Array.isArray(data?.checks) ? data.checks.filter((row) => row && row.ok === false) : [];
    return rows.map((row) => {
        const label = (0, task_replay_shared_1.safeText)(row?.label || row?.id, 80);
        const detail = (0, task_replay_shared_1.safeText)(row?.detail || row?.reason, 160);
        return label ? (detail && detail !== "通过" && detail !== label ? `${label}（${detail}）` : label) : detail;
    }).filter(Boolean).slice(0, 10);
}
function timelineSummary(item) {
    const base = (0, task_replay_shared_1.safeText)(item?.detail || item?.message, 600);
    const data = item?.data && typeof item.data === "object" ? item.data : null;
    if (!data)
        return base;
    const failed = failedCheckLines(data);
    if (failed.length)
        return `${base || `${failed.length} 项未通过`}：${failed.join("；")}`;
    return base || (0, task_replay_shared_1.safeText)(data.user_detail || data.userDetail || data.headline || data.reason || data.summary, 600);
}
function timelineTechnical(item) {
    const data = item?.data && typeof item.data === "object" ? item.data : null;
    if (!data)
        return undefined;
    const checks = Array.isArray(data.checks)
        ? data.checks.slice(0, 30).map((row) => ({ label: (0, task_replay_shared_1.safeText)(row?.label || row?.id, 80), ok: row?.ok === true, detail: (0, task_replay_shared_1.safeText)(row?.detail, 160) }))
        : [];
    ...
    return {
        phase: item.phase || "",
        files: (0, task_replay_shared_1.stringList)(data.files || data.files_changed, 30),
        ...
        ...(routeDecision ? {
            semantic_reason: (0, task_replay_shared_1.safeText)(routeDecision.reason, 600),
...
function userVisibleEventsForTask(task, max = 5000) {
    const identity = replayTaskScope(task);
    if (!identity.scopeId || !identity.exactSessionId)
        return [];
    const rows = [];
    let cursor = 0;
    try {
        while (rows.length < max) {
            const page = (0, user_visible_agent_events_1.listUserVisibleAgentEvents)({ scope: identity.scope, scopeId: identity.scopeId, exactSessionId: identity.exactSessionId, cursor, limit: Math.min(500, max - rows.length) });
            for (const row of page.events || [])
                if (String(row.taskId || "") === String(task.id || ""))
                    rows.push(row);
...
function replayMutableSourceIdentity(family, extraTraceIds) {
    ...
    const userVisibleEvents = family.tasks.map((task) => {
        const identity = replayTaskScope(task);
        if (!identity.scopeId || !identity.exactSessionId)
            return { task_id: String(task.id || ""), event_count: 0, last_id: "", last_sequence: 0 };
        try {
...
function storeReplaySourceManifest(taskId, sourceChecksum, eventCount, mutableSources) {
    ...
    const manifest = {
        ...
        sources: {
            ...
            user_visible_events: mutableSources.user_visible_events?.reduce((sum, row) => sum + Number(row.event_count || 0), 0) || 0,
        },
    };
    (0, observability_database_1.withImmediateObservabilityTransaction)((db) => {
...
function buildTaskEvents(tasks) {
    ...
    for (const task of tasks) {
        ...
        for (const [index, item] of (0, logs_1.getTaskLogs)(taskId, 100).entries()) {
            events.push(event({ id: `task-log:${taskId}:${index}:${item.timestamp}`, at: item.timestamp, category: "task_log", status: (0, task_replay_shared_1.normalizeStatus)(item.level), title: item.level === "error" ? "任务运行异常" : item.level === "warning" ? "任务运行提示" : "任务运行记录", summary: item.message, actor: actor("system", "任务运行器"), task_id: taskId, parent_task_id: task.parent_task_id, trace_id: task.trace_id, source: "task_log" }));
        }
...
function latestGroupTodoPlan(task) {
    ...
    try {
        ...
        for (let index = messages.length - 1; index >= 0; index--) {
            ...
            if (String(message?.task_id || message?.task?.id || message?.taskExperience?.task_id || "") !== String(task.id || ""))
                continue;
...
function buildPlanAndWorkItemData(tasks) {
    const plans = [];
    const workItems = [];
    const events = [];
    for (const task of tasks) {
        ...
        const plan = (0, task_replay_plan_1.buildTaskReplayPlanView)(task, { fallbackTodo: latestGroupTodoPlan(task) });
        if (plan) {
            plans.push(plan);
...
function executionStateText(state) {
    return EXECUTION_STATE_TEXT[String(state || "").toLowerCase()] || "";
}
function buildExecutionEvents(tasks) {
    const events = [];
    for (const task of tasks) {
        for (const executionRecord of (0, execution_kernel_1.listExecutions)({ taskId: task.id })) {
...
function buildTraceEvents(tasks, extraTraceIds) {
    ...
    for (const traceId of ids) {
        ...
        for (const row of Array.isArray(trace?.events) ? trace.events : []) {
            ...
            events.push(event({ id: `trace:${traceId}:${row.id || (0, task_replay_shared_1.stableId)("row", row)}`, at: row.at, category: row.type || "trace", status: (0, task_replay_shared_1.normalizeStatus)(row.status), title: (0, task_replay_shared_1.safeText)(row.message || row.type || "系统轨迹", 180), summary: row.message, actor: row.agent ? actor(/test.?agent/i.test(row.agent) ? "test_agent" : "project_agent", row.agent) : actor(/global/i.test(row.type) ? "global_agent" : "system", /global/i.test(row.type) ? "全局主 Agent" : "系统"), task_id: String(row.task_id || task?.id || ""), parent_task_id: String(task?.parent_task_id || ""), trace_id: traceId, project: row.agent, source: "trace", technical: { type: row.type || "", runtime: row.runtime || "" } }));
        }
    }
    return events;
}
function buildJournalEvents(tasks) {
    const taskById = new Map(tasks.map(task => [String(task.id), task]));
    return (0, task_replay_journal_1.listTaskReplayJournalEvents)(tasks.map(task => String(task.id))).map(row => {
        const source = row.event || {};
        const task = taskById.get(String(source.task_id || row.task_id || "")) || tasks[0];
        return event({ id: `journal:${row.task_id}:${source.id || (0, task_replay_shared_1.stableId)("row", source)}`, at: source.at || row.recorded_at, category: source.type || "journal", status: (0, task_replay_shared_1.normalizeStatus)(source.status), title: (0, task_replay_shared_1.safeText)(source.message || source.type || "任务轨迹", 180), summary: source.message, actor: source.agent ? actor(/test.?agent/i.test(source.agent) ? "test_agent" : "project_agent", source.agent) : actor(/global/i.test(source.type) ? "global_agent" : "system", /global/i.test(source.type) ? "全局主 Agent" : "系统"), task_id: String(source.task_id || row.task_id || task?.id || ""), parent_task_id: String(task?.parent_task_id || ""), trace_id: source.trace_id || task?.trace_id || "", project: source.agent, source: "journal", technical: { type: source.type || "", runtime: source.runtime || "" } });
    });
}
...
function userVisibleReplayStage(row) {
    const explicit = String(row?.detail?.executionStage?.kind || "");
    if (explicit === "preparation")
        return "execution";
    if (explicit === "project_execution")
        return "execution";
    if (explicit === "independent_verification")
        return "test";
    if (explicit === "main_agent_summary")
        return "review";
    if (row.eventType === "requirement_plan")
        return "planning";
    if (String(row.eventType || "").startsWith("agent_"))
        return /test.?agent/i.test(String(row?.detail?.agentDisplay?.runtimeLabel || row?.display?.title || "")) ? "test" : "execution";
    if (String(row.eventType || "").startsWith("tool_"))
        return "execution";
    if (row.eventType === "result")
        return "completion";
    return "system";
}
function buildUserVisibleExecutionEvents(tasks) {
    const output = [];
    for (const task of tasks) {
        const identity = replayTaskScope(task);
        if (!identity.scopeId || !identity.exactSessionId)
        ...
        for (const row of rows) {
            if (String(row.taskId || "") !== String(task.id || ""))
                continue;
            if (["assistant_text_delta", "thinking_status", "turn_started"].includes(String(row.eventType)))
                continue;
            const project = (0, task_replay_shared_1.safeText)(row?.detail?.agentDisplay?.projectName || row?.detail?.agentDisplay?.projectId || task.target_project, 100);
            ...
            output.push(event({
                id: String(row.eventId || (0, task_replay_shared_1.stableId)("uve", row)), at: row.createdAt, stage: userVisibleReplayStage(row), category: String(row.eventType || "execution_event"),
                ...
                technical: {
                    ...
                    tool_call_id: row.toolCallId || "", tool_name: row.toolName || "", agent_run_id: row.agentRunId || "",
                    event_type: row.eventType || "", execution_stage_kind: row?.detail?.executionStage?.kind || "",
                    related_tool_call_ids: progress.relatedToolCallIds || [],
...
function buildGlobalEvents(records, fallbackTask) {
    ...
    for (const run of records.runs) {
        ...
        for (const step of run.steps || []) {
            ...
            events.push(event({ id: `global:${run.id}:step:${step.index}`, at: step.at, category: `global_${step.state || "step"}`, status: (0, task_replay_shared_1.normalizeStatus)(step.error ? "failed" : step.state), title: tool ? `全局主 Agent 执行 ${tool}` : `全局主 Agent ${step.state || "推进任务"}`, summary: step.error || step.message || (step.plan || []).join("；"), actor: actor("global_agent", "全局主 Agent"), task_id: run.mission_id || fallbackTask.id, trace_id: run.trace_id, source: "global_agent", technical: { run_id: run.id, step: step.index, tool, duration_ms: Number(step.duration_ms || 0) } }));
        }
        if (run.final_reply || run.final_report)
            events.push(event({ id: `global:${run.id}:final`, at: run.completed_at || run.updated_at, stage: "completion", category: "global_summary", status: (0, task_replay_shared_1.normalizeStatus)(run.status), title: "全局主 Agent 汇总任务结果", summary: run.final_reply || run.final_report?.summary || run.final_report, actor: actor("global_agent", "全局主 Agent"), task_id: run.mission_id || fallbackTask.id, trace_id: run.trace_id, source: "global_agent", technical: { run_id: run.id } }));
    }
    for (const record of records.supervisors) {
        for (const [index, actionRow] of (record.actions || []).entries())
            events.push(event({ id: `supervisor:${record.id}:action:${index}`, at: actionRow.at || record.updated_at, category: actionRow.type || actionRow.action || "supervision", status: (0, task_replay_shared_1.normalizeStatus)(actionRow.status || "running"), title: actionRow.title || actionRow.message || "全局主 Agent 跟踪群聊任务", summary: actionRow.detail || actionRow.reason || actionRow.message, actor: actor("global_agent", "全局主 Agent"), task_id: record.mission_id, trace_id: record.trace_id, source: "mission_supervisor", technical: { supervisor_id: record.id, cycle: actionRow.cycle || "" } }));
        for (const [index, incident] of (record.incidents || []).entries())
            events.push(event({ id: `supervisor:${record.id}:incident:${index}`, at: incident.at || record.updated_at, category: incident.type || "incident", status: (0, task_replay_shared_1.normalizeStatus)(incident.status || incident.type || "warning"), title: incident.title || incident.message || "任务出现需要处理的问题", summary: incident.reason || incident.detail || incident.message, actor: actor("global_agent", "全局主 Agent"), task_id: incident.task_id || record.mission_id, trace_id: record.trace_id, source: "mission_supervisor", technical: { supervisor_id: record.id, occurrence_count: Number(incident.occurrence_count || 1), resolved_at: incident.resolved_at || "" } }));
    }
    return events;
}
function buildTestAgentEvents(taskIds, artifactRuns) {
    const events = [];
    const records = (0, test_agent_runner_1.listTestAgentRunnerRecords)({ taskIds, limit: 1000 });
    for (const record of records) {
        ...
        events.push(event({ id: `test-agent-run:${record.id}`, at: record.finishedAt || record.startedAt || record.createdAt, stage: record.mode === "plan" ? "planning" : "test", category: record.mode === "plan" ? "test_plan" : "test_run", status: (0, task_replay_shared_1.normalizeStatus)(record.status === "completed" && invocation.canAccept !== false ? "passed" : record.status), title, summary, actor: actor("test_agent", "TestAgent"), task_id: record.taskId, source: "test_agent_runner", technical: {
                run_id: record.id,
                mode: record.mode,
...
function taskEvidence(tasks, artifactRuns) {
    ...
    for (const task of tasks) {
        ...
        if (files.length) {
            ...
            evidence.push({
                id: (0, task_replay_shared_1.stableId)("evidence", `${task.id}:files`),
                type: "code_changes",
        ...
        const checks = [...new Set([...(summary.verification_executed || []), ...(summary.verification || []), ...(task.receipt?.verification || [])].map((item) => (0, task_replay_shared_1.safeText)(item?.command || item?.name || item, 240)).filter(Boolean))];
        if (checks.length)
            evidence.push({ id: (0, task_replay_shared_1.stableId)("evidence", `${task.id}:checks`), type: "verification", title: "项目子 Agent 验证记录", task_id: task.id, project: task.target_project || "", status: "available", preview_kind: "list", items: checks, retained_until: "任务删除前", url: "" });
    }
    for (const run of artifactRuns)
        for (const item of run.artifacts)
            evidence.push({ ...item, run_id: run.run_id, task_id: run.task_id, retained_until: run.retained_until, retention_status: run.retention_status, status: item.available ? run.retention_status : "expired", url: item.available ? `/api/tasks/replay/artifact?task_id=${encodeURIComponent(run.task_id)}&run_id=${encodeURIComponent(run.run_id)}&artifact_id=${encodeURIComponent(item.id)}` : "" });
    return evidence;
}
...
function paginateReplayEventsForView(allEvents, options = {}) {
    ...
    const filteredEvents = hasEventViewOptions ? allEvents.filter(item => {
        if (!options.includeSystemEvents && item.audience === "technical")
            return false;
        if (options.stage && options.stage !== "all" && item.stage !== options.stage)
            return false;
        if (options.status && options.status !== "all" && item.status !== options.status)
            return false;
        if (options.actor && options.actor !== "all" && item.actor?.type !== options.actor)
            return false;
        if (options.task && options.task !== "all" && item.task_id !== options.task)
            return false;
        const haystack = `${item.title} ${item.summary} ${item.actor?.label} ${item.project} ${item.category}`.toLowerCase();
        if (eventQuery && !haystack.includes(eventQuery))
            return false;
        if (options.preset === "issues" && !["failed", "blocked", "warning"].includes(item.status))
            return false;
        if (options.preset === "test" && !(item.actor?.type === "test_agent" || item.stage === "test"))
            return false;
        if (options.preset === "browser" && !/browser|playwright|screenshot|页面|浏览器/i.test(haystack))
            return false;
        if (options.preset === "changes" && !["change", "execution", "rework"].includes(item.stage))
            return false;
        return true;
    }) : allEvents;
    ...
    const afterAt = (0, task_replay_shared_1.iso)(options.afterEventAt || "");
...
function taskPublicRow(task, rootId) {
    ...
    return {
        ...
        schedule_origin: normalized.cron_job_id ? {
            ...
            scheduledFor: (0, task_replay_shared_1.iso)(normalized.cron_scheduled_for),
...
function buildCompleteTaskReplay(taskId, options = {}) {
    ...
    for (const item of evidence) {
        if (!["code_changes", "verification"].includes(item.type))
            continue;
    ...
    const phases = STAGE_ORDER.map(stage => {
        ...
        const status = rows.some(item => item.status === "failed") ? "failed" : rows.some(item => item.status === "blocked") ? "blocked" : rows.some(item => item.status === "warning") ? "warning" : rows.some(item => item.status === "running") ? "running" : rows.length ? "passed" : "info";
    ...
    const recordedUsage = (source, keys) => {
    ...
    const sumRecordedUsage = (keys) => {
        const rows = usageSources.map((source) => recordedUsage(source, keys)).filter((value) => value !== null);
    ...
    const replayTokenCount = replayInputTokens !== null || replayOutputTokens !== null
        ...
        : null;
    const startedAt = (0, task_replay_shared_1.iso)(family.root.started_at || family.root.created_at);
    const finishedAt = (0, task_replay_shared_1.iso)(family.root.completed_at || (TERMINAL.has(rootStatus.toLowerCase()) ? family.root.updated_at : ""));
    const presentation = (0, task_replay_presentation_1.buildTaskReplayPresentation)({
        root: family.root,
        tasks: family.tasks,
    ...
    const conversationLinks = family.tasks.flatMap((task) => (0, task_conversation_links_1.buildTaskConversationLinks)(task)?.links || []);
    const navigation = [...new Map(conversationLinks.map((item) => [`${item.relation}|${item.scope}|${item.scopeId}|${item.exactSessionId}|${item.messageId || ""}`, item])).values()];
    const result = {
        ...
        schedule_origin: normalizedRoot.cron_job_id ? {
            ...
            scheduledFor: (0, task_replay_shared_1.iso)(normalizedRoot.cron_scheduled_for),
        ...
        actors: [
            ...
            { id: "project_agent", label: "项目子 Agent", present: family.tasks.some(task => (0, execution_kernel_1.listExecutions)({ taskId: task.id }).length > 0) },
            { id: "test_agent", label: "TestAgent", present: artifactRuns.length > 0 || (0, test_agent_runner_1.listTestAgentRunnerRecords)({ taskIds: ids, limit: 1 }).length > 0 },
        ],
...
function projectWorkDir(project) {
    const config = (0, db_1.getConfigs)().find(item => String(item.name || "").toLowerCase() === String(project || "").toLowerCase());
    if (!config)
        return "";
    try {
        return String((0, db_1.getConfigInfo)(config.path)?.[0]?.workDir || "");
    }
    catch {
        return "";
    }
...
function taskDeclaredFiles(task) {
    ...
    return [...new Set([
        ...
        ].map(task_replay_shared_1.publicFile).filter(Boolean))].slice(0, 500);
}
function buildTaskReplayFreshness(taskId) {
    const family = taskFamily(String(taskId || ""));
    if (!family)
        return null;
    const projects = new Map();
    const evidenceRows = family.tasks.flatMap((task) => (0, unified_evidence_registry_1.listEvidence)({ taskId: String(task.id || "") }));
    for (const task of family.tasks) {
        ...
        let current = null;
        try {
            current = (0, unified_evidence_registry_1.captureRepoStateIdentity)(workDir, files);
        }
        catch { }
        const taskEvidenceRows = evidenceRows.filter(row => row.taskId === String(task.id || "") && row.repoStateIdentity);
        const states = taskEvidenceRows.map(row => (0, unified_evidence_registry_1.compareRepoStateIdentity)(row.repoStateIdentity, current));
        ...
        const fileRows = files.map(file => {
            ...
            if (!contained)
                return { path: file, freshness: "permission_revoked" };
            try {
                return { path: file, freshness: fs.statSync(absolute).isFile() ? repoFreshness : "deleted" };
            }
            catch (error) {
                return { path: file, freshness: error?.code === "EACCES" ? "permission_revoked" : "deleted" };
            }
        });
        projects.set(project, {
            ...
            authoritativeRevision: current ? (0, unified_evidence_registry_1.repoStateFingerprint)(current) : "",
            files: fileRows,
        });
    }
    const evidence = evidenceRows.map(row => {
        const project = family.tasks.find((task) => String(task.id || "") === row.taskId)?.target_project || "";
        ...
        if (currentProject?.freshness && currentProject.freshness !== "current")
            freshness = currentProject.freshness;
...
function safeAuditEvidence(item) {
    return {
        id: String(item?.id || ""), type: String(item?.type || ""), title: (0, task_replay_shared_1.safeText)(item?.title, 300), task_id: String(item?.task_id || ""), project: (0, task_replay_shared_1.safeText)(item?.project, 100),
        status: String(item?.status || ""), retained_until: (0, task_replay_shared_1.safeText)(item?.retained_until, 80), item_count: Math.max(0, Number(item?.file_count || item?.items?.length || 0)), contentStored: false,
    };
}
function safeAuditTechnical(value, depth = 0) {
    if (depth > 5 || value == null)
        return value == null ? value : "[详情已收起]";
    if (Array.isArray(value))
        return value.slice(0, 80).map(item => safeAuditTechnical(item, depth + 1));
    if (typeof value === "string")
        return (0, task_replay_shared_1.safeText)(value, 800);
    if (typeof value !== "object")
        return value;
    const output = {};
    for (const [key, item] of Object.entries(value)) {
        if (/(prompt|thinking|reasoning|stdout|stderr|raw.?output|source.?code|diff|patch|body|native.?session|lease.?id|work.?dir|file.?path)/i.test(key))
            continue;
        output[key] = safeAuditTechnical(item, depth + 1);
    }
    return output;
}
function buildTaskReplayUserReport(taskId) {
    const replay = buildCompleteTaskReplay(taskId, { eventTail: false, eventOffset: 0, eventLimit: 1, includeDetails: true });
    if (!replay)
        return null;
    return {
        ...
        requirementsAndDelivery: (replay.deliveries || []).map((row) => ({ businessGoal: row.business_goal, acceptanceCriteria: row.acceptance_criteria, finalReport: row.final_report || row.user_report || row.headline, verification: row.verification, risks: row.blockers, unfinished: row.needs })),
        plan: (replay.plans || []).map((row) => ({ title: row.title, status: row.status, steps: (row.steps || []).map((step) => ({ title: step.title, detail: step.detail, status: step.status })) })),
        attempts: (replay.presentation?.attemptComparisons || []).map((group) => ({ project: group.project, attempts: (group.attempts || []).map((row) => ({ attempt: row.attempt, status: row.status, accepted: row.accepted, superseded: row.superseded, summary: row.summary, failureReason: row.failureReason, filesChanged: row.filesChanged, verificationCount: row.verificationCount })) })),
        acceptance: (replay.presentation?.acceptanceMatrix || []).map((row) => ({ description: row.description, status: row.status, verifier: row.verifier, freshness: row.freshness, reason: row.reason })),
        fileStatistics: (replay.evidence || []).filter((item) => item.type === "code_changes").map((item) => ({ project: item.project, fileCount: item.file_count, additions: (item.files || []).reduce((sum, file) => sum + Number(file.additions || 0), 0), deletions: (item.files || []).reduce((sum, file) => sum + Number(file.deletions || 0), 0) })),
        contentStored: false,
    };
}
function buildTaskReplayAuditExport(taskId) {
    const replay = buildCompleteTaskReplay(taskId, { includeDetails: true, includeSystemEvents: true });
    if (!replay)
        return null;
    return {
        ...
        tasks: replay.tasks,
        events: (replay.events || []).map((row) => ({ ...row, summary: (0, task_replay_shared_1.safeText)(row.summary, 1200), technical: safeAuditTechnical(row.technical) })),
        evidence: (replay.evidence || []).map(safeAuditEvidence),
        retention: replay.retention,
        contentStored: false,
    };
}
function projectTaskReplayForAccess(replay, canManage) {
    if (!replay || canManage)
        return replay;
    const cleanEvent = (row) => {
        const { technical, ...safe } = row || {};
        return safe;
    };
    const { replay_source_checksum: _sourceChecksum, terminal_state_receipt: _terminalReceipt, terminal_decision: _terminalDecision, terminal_gate: _terminalGate, scheduler_state: _schedulerState, ...businessReplay } = replay;
    ...
    return {
        ...
        events: (replay.events || []).filter((row) => row.audience !== "technical").map(cleanEvent),
        evidence: (replay.evidence || []).map(cleanEvidence),
        summary: { ...(replay.summary || {}), technical_event_count: 0 },
        presentation: replay.presentation ? {
            ...replay.presentation,
            actionCenter: (replay.presentation.actionCenter || []).filter((row) => row.kind === "view_error").map(({ bindingChecksum, ...row }) => row),
        } : null,
...
function replayIndexStage(task, members) {
    const explicit = (0, task_replay_shared_1.safeText)(task.current_stage || task.execution_stage || task.workflow_stage || task.phase, 80).toLowerCase();
    ...
    if (explicit && explicitLabels[explicit])
        return { id: explicit, label: explicitLabels[explicit] };
    const statuses = members.map((item) => String(item.status || "").toLowerCase());
    const acceptance = String(task.acceptance_state || "").toLowerCase();
    if (statuses.some(status => ["failed", "blocked", "recovery_required"].includes(status)))
        return { id: "attention", label: "需要处理" };
    if (["done", "completed", "accepted"].includes(String(task.status || "").toLowerCase()) || acceptance === "accepted")
        return { id: "completion", label: "已完成" };
    if (statuses.some(status => ["in_progress", "running", "executing", "verifying"].includes(status)))
        return { id: "execution", label: "执行中" };
    if (statuses.some(status => ["queued", "pending", "created"].includes(status)))
        return { id: "queued", label: "排队等待" };
    return { id: "unknown", label: "等待更新" };
}
function replayIndexUnresolvedCount(task, members) {
    const rows = new Set();
    const append = (value) => {
        if (!Array.isArray(value))
            return;
        value.forEach((item, index) => rows.add((0, task_replay_shared_1.safeText)(item?.id || item?.code || item?.title || item?.summary || item, 200) || `item-${index}`));
    };
    ...
    for (const member of members) {
        ...
        if (["failed", "blocked", "recovery_required"].includes(status))
            rows.add(`task:${String(member?.id || status)}`);
    }
    return rows.size;
}
function buildTaskReplayIndexFromRecords(tasks, groups, input = 40) {
    const byId = new Map(tasks.map((task) => [String(task.id), task]));
    ...
    const groupById = new Map(groups.map((group) => [String(group.id || ""), String(group.name || group.id || "")]));
    const rootIdFor = (task) => {
        let current = task;
        const seen = new Set();
    ...
    for (const task of tasks) {
        ...
        if (!membersByRoot.has(rootId))
            membersByRoot.set(rootId, []);
        membersByRoot.get(rootId).push(task);
    }
    const roots = tasks.filter((task) => rootIdFor(task) === String(task.id || ""))
        .sort((a, b) => String(b.updated_at || b.created_at || "").localeCompare(String(a.updated_at || a.created_at || "")));
    const rows = roots.map((task) => {
        const members = membersByRoot.get(String(task.id)) || [task];
        const projects = [...new Set(members.map((item) => (0, task_replay_shared_1.safeText)(item.target_project, 100)).filter(Boolean))].sort();
        const groupIds = [...new Set(members.map((item) => String(item.group_id || "")).filter(Boolean))];
        const primaryGroupId = String(task.group_id || groupIds[0] || "");
        const groupName = groupById.get(primaryGroupId) || (0, task_replay_shared_1.safeText)(task.workflow_meta?.group_name, 100) || primaryGroupId;
    ...
    const facetCount = (values, matches) => [...new Set(values.filter(Boolean))].map(value => ({
        value,
        label: value,
        count: rows.filter((row) => matches(row, value)).length,
    }));
    const filtered = rows.filter((row) => {
        const updatedMs = Date.parse(row.updated_at || row.created_at || "");
        if (projectFilter && !row.projects.some((value) => value.toLowerCase() === projectFilter))
            return false;
        if (groupFilter && !row._group_ids.includes(groupFilter))
            return false;
        if (statusFilter && String(row.status).toLowerCase() !== statusFilter)
            return false;
        if (Number.isFinite(fromMs) && (!Number.isFinite(updatedMs) || updatedMs < fromMs))
            return false;
        if (Number.isFinite(toMs) && (!Number.isFinite(updatedMs) || updatedMs > toMs))
            return false;
        if (query) {
    ...
    const projectValues = rows.flatMap((row) => row.projects || []);
    const statusValues = rows.map((row) => String(row.status || "")).filter(Boolean);
    const groupFacets = groups.map((group) => ({ value: String(group.id || ""), label: String(group.name || group.id || ""), count: rows.filter((row) => row._group_ids.includes(String(group.id || ""))).length })).filter((item) => item.value && item.count > 0);
    return {
        ...
        tasks: filtered.slice(offset, offset + limit).map(({ _group_ids, ...row }) => row),
    };
}
function resolveTaskReplayArtifact(input) {
    return (0, artifact_retention_1.resolveTestAgentArtifactForTask)(input);
}
function runTaskReplayContractSelfTest() {
    const secret = (0, task_replay_shared_1.safeText)("api_key=secret-value C:\\Users\\someone\\private\\report.json");
    ...
    const journal = (0, task_replay_journal_1.runTaskReplayJournalSelfTest)();
    const planSelfTest = (0, task_replay_plan_1.runTaskReplayPlanSelfTest)();
    const deliverySelfTest = (0, task_replay_delivery_1.runTaskReplayDeliverySelfTest)();
    const presentationSelfTest = (0, task_replay_presentation_1.runTaskReplayPresentationSelfTest)();
