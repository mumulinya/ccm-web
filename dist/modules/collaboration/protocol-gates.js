...
function buildAckPreflightReview(task, receipts = [], orders = []) {
    const rows = receipts.map((receipt) => {
        ...
        const order = orders.find((item) => String(item?.project || item?.agent || "").toLowerCase() === String(receipt?.agent || receipt?.project || "").toLowerCase()) || null;
        const semantic = order?.semantic_contract_binding || order?.semanticContractBinding
            || order?.semanticBinding || order?.semantic_binding
            || order?.work_item_contract || order?.workItemContract || null;
        ...
        const semanticMatches = !semantic || (String(ack?.requirementChecksum || ack?.requirement_checksum || "") === String(semantic?.requirementChecksum || semantic?.requirement_checksum || "")
            && String(ack?.planChecksum || ack?.plan_checksum || "") === String(semantic?.planChecksum || semantic?.plan_checksum || "")
            && String(ack?.stepId || ack?.step_id || "") === String(semantic?.stepId || semantic?.step_id || "")
            && JSON.stringify(normalizeStringArray(ack?.stepIds || ack?.step_ids || [ack?.stepId || ack?.step_id]).sort()) === JSON.stringify(normalizeStringArray(semantic?.stepIds || semantic?.step_ids || [semantic?.stepId || semantic?.step_id]).sort())
            && JSON.stringify(normalizeStringArray(ack?.acceptanceCriterionIds || ack?.acceptance_criterion_ids).sort()) === JSON.stringify(normalizeStringArray(semantic?.acceptanceCriterionIds || semantic?.acceptance_criterion_ids).sort()));
        const status = !ack ? "missing"
            : unclear.length ? "needs_rewrite"
                : !semanticMatches ? "semantic_mismatch"
        ...
        return {
            ...
            requirement_checksum: String(ack?.requirementChecksum || ack?.requirement_checksum || ""),
            plan_checksum: String(ack?.planChecksum || ack?.plan_checksum || ""),
            step_id: String(ack?.stepId || ack?.step_id || ""),
            step_ids: normalizeStringArray(ack?.stepIds || ack?.step_ids || [ack?.stepId || ack?.step_id]).slice(0, 40),
            acceptance_criterion_ids: normalizeStringArray(ack?.acceptanceCriterionIds || ack?.acceptance_criterion_ids).slice(0, 20),
            unclear: unclear.slice(0, 8),
            reason: status === "approved" ? "ACK 目标和范围清晰"
                : status === "needs_rewrite" ? `ACK 仍有疑问：${unclear.join("；")}`
                    : status === "semantic_mismatch" ? "ACK 与已确认需求、计划、步骤或验收标准绑定不一致"
    ...
    const rejected = allRows.filter((row) => ["missing", "needs_rewrite", "semantic_mismatch", "weak"].includes(row.status));
    return {
