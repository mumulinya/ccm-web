...
exports.extractPriorGroupPlanDraft = extractPriorGroupPlanDraft;
exports.formatPriorGroupPlanBlock = formatPriorGroupPlanBlock;
exports.runGroupPriorPlanContextSelfTest = runGroupPriorPlanContextSelfTest;
const SKIP_PRIOR_PLAN = /模型这次没有给出可用回复|只读检查已完成，但没能生成计划/;
function asMessages(value) {
    return Array.isArray(value) ? value : [];
}
function parseVisibleMessages(context) {
...
function extractPriorGroupPlanDraft(context) {
    ...
    for (let index = messages.length - 1; index >= 0; index -= 1) {
        ...
        if (content && !SKIP_PRIOR_PLAN.test(content))
            return content.slice(0, 4000);
    }
    return "";
}
function formatPriorGroupPlanBlock(draft) {
    const text = String(draft || "").trim();
    if (!text)
        return "";
    return `已有计划稿（上一轮主 Agent 输出，请在此基础上展开或修订，不要重新扫仓库）：\n${text}`;
}
function runGroupPriorPlanContextSelfTest() {
    const context = [
        "【当前精确群聊会话连续性】",
        "【压缩前完整会话原文 · 2/2 条】",
        JSON.stringify([
...
//# sourceMappingURL=group-prior-plan-context.js.map