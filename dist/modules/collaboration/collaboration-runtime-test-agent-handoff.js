"use strict";
// collaboration-runtime-test-agent-handoff.ts — merged from 2 part files (behavior-freeze merge).
...
// ===== merged from collaboration-runtime-test-agent-handoff-part-01.ts =====
function getTestAgentHandoffPayload(handoff = null) {
    const direct = handoff?.handoff || handoff;
    return direct?.schema === "ccm-test-agent-handoff-v2" && (Array.isArray(direct.projects) || direct.project) ? direct : null;
}
...
function collectConfiguredTestAgentReviewConfig(projectName) {
    ...
    const browserScenarios = (0, collaboration_runtime_plan_tools_1.normalizeProjectConfigList)(nestedConfig.browserScenarios || nestedConfig.browser_scenarios || baseConfig.test_agent_browser_scenarios || baseConfig.testAgentBrowserScenarios).slice(0, 12);
    ...
    if (browserScenarios.length)
        project.browserScenarios = browserScenarios;
    ...
    const requiredChecks = (0, collaboration_runtime_status_helpers_1.uniqueStrings)((0, collaboration_runtime_plan_tools_1.normalizeProjectConfigList)(nestedConfig.requiredChecks || nestedConfig.required_checks || baseConfig.test_agent_required_checks || baseConfig.testAgentRequiredChecks), targetUrl || startupUrl || httpChecks.length || adversarialHttpChecks.length ? ["http"] : [], targetUrl || browserChecks.length || browserScenarios.length || adversarialBrowserChecks.length || adversarialBrowserProbeTemplates.length ? ["browser_e2e", "screenshots", "console_errors", "browser_snapshots", "browser_console_logs", "browser_network_logs"] : [], targetUrl || browserChecks.length || browserScenarios.length || adversarialBrowserChecks.length || adversarialBrowserProbeTemplates.length ? ["browser_trace", "browser_har"] : [], hasMultiSessionBrowserCheck ? ["browser_multi_session"] : [], adversarialHttpChecks.length || adversarialBrowserChecks.length || adversarialBrowserProbeTemplates.length ? ["adversarial"] : []).slice(0, 16);
...
function resolveTestAgentDecisionVerdict(report, artifactVerdict = readTestAgentVerdictArtifact(report)) {
    ...
    const verdict = artifactVerdict && reportVerdict
        ...
        : artifactVerdict || reportVerdict;
    return verdict ? strengthenTestAgentVerdictWithReportCoverage(report, verdict) : null;
}
...
// ===== merged from collaboration-runtime-test-agent-handoff-part-02.ts =====
...
function summarizeTestAgentBrowserFailedStep(step) {
    ...
    return `${step?.kind === "action" ? "操作" : "断言"} ${(0, memory_1.compactMemoryText)(step?.name || "未命名步骤", 80)} ${testAgentStatusLabel(step?.status)}${step?.error ? `：${(0, memory_1.compactMemoryText)(step.error, 120)}` : ""}`;
}
...
function collectTestAgentVerificationLines(report, verdict = resolveTestAgentDecisionVerdict(report)) {
    return (0, collaboration_runtime_status_helpers_1.uniqueStrings)([
        ...
        ...(Array.isArray(report.requiredCheckCoverage)
            ? report.requiredCheckCoverage.map((item) => `必检项 ${testAgentEvidenceTypeLabel(item.check)}：${testAgentStatusLabel(item.status)}${item.missingReason ? `（${item.missingReason}）` : ""}`)
            : []),
    ]).slice(0, 20);
}
...
exports.COORDINATOR_REVIEW_MAX_ROUNDS = rework_policy_1.COORDINATOR_REVIEW_TURN_BUDGET;
...
//# sourceMappingURL=collaboration-runtime-test-agent-handoff.js.map