...
function createTaskWithScopedIdentity(task) {
    ...
    const presentedPlan = task?.workflow_meta?.presentedPlan || task?.workflowMeta?.presentedPlan
        || task?.presentedPlan || task?.presented_plan || null;
    const authoritativeRequirement = presentedPlan?.schema === "ccm-implementation-plan-v2"
        ? presentedPlan.businessRequirement || null
        : null;
    const authoritativeAcceptance = Array.isArray(authoritativeRequirement?.acceptanceCriteria)
        ? authoritativeRequirement.acceptanceCriteria.map((item) => String(item?.description || "").trim()).filter(Boolean)
        : [];
    ...
    const contentChecksum = suppliedContentChecksum || crypto.createHash("sha256").update(JSON.stringify({
        ...
        business_goal: authoritativeRequirement?.businessGoal || task.business_goal || task.businessGoal || "",
        acceptance_criteria: authoritativeAcceptance.length ? authoritativeAcceptance : task.acceptance_criteria || task.acceptanceCriteria || "",
    })).digest("hex");
    const identityInput = {
        ...task,
    ...
    const newTask = {
        ...
        business_goal: authoritativeRequirement?.businessGoal || task.business_goal || task.businessGoal || "",
        acceptance_criteria: authoritativeAcceptance.length ? authoritativeAcceptance.join("；") : task.acceptance_criteria || task.acceptanceCriteria || "",
        ...
        acceptance_level: task.acceptance_level || task.acceptanceLevel || task.acceptance_policy_snapshot?.level || null,
        acceptance_route: task.acceptance_route || task.acceptanceRoute || task.acceptance_policy_snapshot?.route || null,
        acceptance_reasons: Array.isArray(task.acceptance_reasons || task.acceptanceReasons)
            ? (task.acceptance_reasons || task.acceptanceReasons)
            : [],
        acceptance_actual_level: task.acceptance_actual_level || task.acceptanceActualLevel || null,
        acceptance_actual_route: task.acceptance_actual_route || task.acceptanceActualRoute || null,
        acceptance_escalation_reasons: Array.isArray(task.acceptance_escalation_reasons || task.acceptanceEscalationReasons)
            ? (task.acceptance_escalation_reasons || task.acceptanceEscalationReasons)
            : [],
        acceptance_escalation: task.acceptance_escalation || task.acceptanceEscalation || null,
        acceptance_review_round: Math.max(0, Number(task.review_round_total || task.reviewRoundTotal || task.review_round || task.reviewRound || 0)),
        acceptance_policy_snapshot: task.acceptance_policy_snapshot || task.acceptancePolicySnapshot || null,
    ...
    if (acceptancePolicy) {
        ...
        newTask.acceptance_level = acceptancePolicy.schema === "ccm-task-acceptance-policy-snapshot-v3" ? acceptancePolicy.level : (acceptancePolicy.mode === "test_agent" ? "strict" : "standard");
        newTask.acceptance_route = acceptancePolicy.schema === "ccm-task-acceptance-policy-snapshot-v3" ? acceptancePolicy.route : (acceptancePolicy.mode === "test_agent" ? "independent_test_agent" : "main_agent_self_verification");
        newTask.acceptance_reasons = acceptancePolicy.schema === "ccm-task-acceptance-policy-snapshot-v3" ? acceptancePolicy.reasons : [];
    }
...
function buildRequirementEpicTaskRecord(input, id, traceId, now) {
    ...
    const groupSession = groupId
        ...
        : null;
    const presentedPlan = input?.workflow_meta?.presentedPlan || input?.workflowMeta?.presentedPlan
        || input?.presentedPlan || input?.presented_plan || null;
    const authoritativeRequirement = presentedPlan?.schema === "ccm-implementation-plan-v2"
        ? presentedPlan.businessRequirement || null
        : null;
    const authoritativeAcceptance = Array.isArray(authoritativeRequirement?.acceptanceCriteria)
    ...
    const record = {
        ...
        business_goal: authoritativeRequirement?.businessGoal || input.business_goal || input.businessGoal || "",
        acceptance_criteria: authoritativeAcceptance.length ? authoritativeAcceptance.join("；") : input.acceptance_criteria || input.acceptanceCriteria || "",
...
function hasStructuredTaskAcceptanceEvidence(task, updates = {}) {
    ...
    if (exactAcceptanceWorkflow) {
        ...
        const finalAcceptanceValid = finalAcceptance?.accepted === true
            ...
            && (finalAcceptance?.mode === effectiveMode || finalAcceptance?.mode === "not_required");
        ...
        const hardeningGate = review?.completionGate
            ...
            || null;
        const hardeningRequired = ["ccm-task-acceptance-policy-snapshot-v2", "ccm-task-acceptance-policy-snapshot-v3"].includes(policyResult.snapshot.schema);
...
function updateTask(id, updates) {
    ...
    if (policyValidation.valid && policyValidation.snapshot) {
        ...
        updates = {
            ...
            acceptance_level: policy.schema === "ccm-task-acceptance-policy-snapshot-v3" ? policy.level : tasks[idx].acceptance_level,
            acceptance_route: policy.schema === "ccm-task-acceptance-policy-snapshot-v3" ? policy.route : tasks[idx].acceptance_route,
            acceptance_reasons: policy.schema === "ccm-task-acceptance-policy-snapshot-v3" ? policy.reasons : tasks[idx].acceptance_reasons,
