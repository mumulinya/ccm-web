...
function getAgentProbeBatchTargets(payload = {}) {
    ...
    const directProjects = (0, collaboration_runtime_status_helpers_1.uniqueStrings)((0, db_1.loadTasks)()
        .filter((task) => !String(task?.group_id || task?.groupId || "").trim())
        .filter((task) => ["pending", "queued", "in_progress", "running", "blocked", "recovery_required"].includes(String(task?.status || "").toLowerCase()))
        .map((task) => String(task?.target_project || task?.targetProject || "").trim())
        .filter(Boolean));
    const directTargets = directProjects.map((project) => {
        const config = (0, db_1.getConfigs)().find((item) => String(item?.name || "") === project);
        const infos = config ? (0, db_1.getConfigInfo)(config.path) : [];
        const info = infos.find((item) => String(item?.name || "") === project) || infos[0] || null;
        const workDir = String(info?.workDir || "").trim();
        const agentType = (0, runtime_1.normalizeAgentRuntimeId)(info?.agent || "claudecode");
        const probe = readAgentProbeStatus({ groupId: "", project, agentType });
        const probeHealth = getAgentProbeHealth(probe);
        let workDirExists = false;
        let workDirWritable = false;
        try {
            workDirExists = !!workDir && fs.statSync(workDir).isDirectory();
            if (workDirExists) {
        ...
        return { group_id: "", group_name: `${project} 项目自动化`, project, agent_type: agentType, configured: !!config && !!info, workDirExists, workDirWritable, probeHealth, ready: probeHealth.successFresh === true };
    });
    const targets = [...directTargets, ...(diagnostics.agent_probe_matrix?.targets || [])]
...
async function runAgentCliProbeBatch(payload, ctx) {
    ...
    for (const target of selection.targets) {
        const result = await (0, collaboration_runtime_daily_dev_1.runAgentCliProbe)({
            ...payload,
            ...(target.group_id ? { group_id: target.group_id, target_member: target.project } : { target_project: target.project }),
