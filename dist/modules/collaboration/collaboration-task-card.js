...
exports.hasValidAcceptedTerminalStateReceipt = hasValidAcceptedTerminalStateReceipt;
...
function hasValidAcceptedTerminalStateReceipt(task) {
    const receipt = task?.terminal_state_receipt;
    if (!receipt || receipt.schema !== "ccm-task-terminal-state-receipt-v1")
        return false;
    if (String(task?.status || "").toLowerCase() !== "done")
        return false;
    if (!['accepted', 'terminal_gate_passed'].includes(String(task?.acceptance_state || '').toLowerCase()))
        return false;
    if (String(receipt.task_id || '') !== String(task?.id || ''))
        return false;
    if (String(receipt.status || '').toLowerCase() !== 'done')
        return false;
    if (!['accepted', 'terminal_gate_passed'].includes(String(receipt.acceptance_state || '').toLowerCase()))
        return false;
    const checksum = String(receipt.checksum || '');
    if (!checksum)
        return false;
    const { checksum: _checksum, ...receiptBase } = receipt;
    const expected = crypto.createHash('sha256').update(JSON.stringify(receiptBase)).digest('hex');
    return checksum === expected;
}
function hasStrongTaskAcceptanceEvidence(task, executions = [], explicitSummary = null) {
    // The terminal receipt is issued only after the authoritative acceptance path
    // has completed.  Dashboard/read projections must not demote that signed
    // terminal state merely because their compact summary omits verbose evidence.
    if (hasValidAcceptedTerminalStateReceipt(task))
        return true;
...
function buildTaskCardView(task, executions, sessions) {
    ...
    if (phase === "queued")
        ...
        nextAction = task?.intake_state === "awaiting_confirmation" ? "需要你确认后才执行；确认后才会派发子 Agent" : "请补充卡片中列出的信息";
    ...
    return {
        ...
        plan_mode: planMode ? {
            ...
            display_policy: {
                user_visible: planMode.display_policy?.user_visible !== false,
                show_todo: planMode.display_policy?.show_todo !== false,
                technical_details_default_collapsed: planMode.display_policy?.technical_details_default_collapsed !== false,
                hide_internal_protocols: planMode.display_policy?.hide_internal_protocols !== false,
            },
