...
exports.runGroupOrchestratorFailureSelfTest = runGroupOrchestratorFailureSelfTest;
...
function classifyGroupOrchestratorFailure(error) {
    ...
    if (CONTEXT_FAILURE.test(haystack)) {
        if (hasObservations)
            return afterObservationCopy("context");
        return withLegacyGuidance({
            kind: "context",
    ...
    if (hasObservations)
        return afterObservationCopy("internal");
    return withLegacyGuidance({
        kind: "internal",
...
function runGroupOrchestratorFailureSelfTest() {
    ...
    const emptyReply = classifyGroupOrchestratorFailure(Object.assign(new Error("OpenAI-compatible JSON model call失败：已完成 2 次尝试，总耗时 4630ms；最后错误：模型返回空响应"), { code: "CCM_MODEL_RETRY_EXHAUSTED" }));
...
//# sourceMappingURL=group-orchestrator-failure.js.map