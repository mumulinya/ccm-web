...
async function classifyGroupProjectTaskIntentWithAgent(input) {
    ...
    try {
        const coordinatorResult = await (0, group_orchestrator_1.runGroupOrchestrator)({
            ...
            authorizedProjectIds: input.authorizedProjectIds || input.authorized_project_ids,
...
async function buildGroupPlanModePreflight(input) {
    ...
    // Do not guess by group order. With no explicit model target, the read-only
    // source planner resolves the smallest affected project set from manifests.
    let selectedProjects = relevantProjects;
    ...
    const planningSource = firstTurnSource?.schema === "ccm-group-main-source-planning-v1"
        ...
        : await (0, project_analysis_1.buildModelDrivenGroupPlanningSourceContext)(group, message, configs, {
            ...
            maxRounds: 3,
            exactSessionId: String(input.taskIntent?.groupSessionId || input.taskIntent?.group_session_id || `group-source-${String(group?.id || "unknown")}`),
        });
    selectedProjects = Array.isArray(planningSource.requestedProjects)
        ? planningSource.requestedProjects.map(String).filter(Boolean)
        : selectedProjects;
...
function buildChildAgentWorkerHandoff(targetProject, taskText = "", options = {}) {
    ...
    const derivedPlanDispatchContract = !options.plan_dispatch_contract && !options.planDispatchContract && sourcePlan?.schema === "ccm-implementation-plan-v2"
        ? (0, plan_dispatch_contract_1.buildPlanDispatchContract)({
            ...
            evidencePolicy: sourcePlan.evidencePolicy,
        })
        : null;
    ...
    const communicationMessageId = String(handoff?.communication_envelope?.messageId || "").trim();
    if (communicationMessageId && handoff?.semantic_contract_binding) {
        handoff.semantic_contract_binding = (0, agent_communication_v2_1.bindAgentCommunicationSemanticContract)(communicationMessageId, handoff.semantic_contract_binding);
    }
    return handoff;
}
