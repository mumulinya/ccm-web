"use strict";
// group-routes.ts — merged from 3 part files (behavior-freeze merge).
...
exports.compactGroupMessageTaskRuntime = compactGroupMessageTaskRuntime;
exports.compactGroupStatusText = compactGroupStatusText;
exports.taskUpdatedMs = taskUpdatedMs;
exports.checkpointStatus = checkpointStatus;
exports.groupTaskStatusMeta = groupTaskStatusMeta;
exports.groupDeliveryCheckpointLabel = groupDeliveryCheckpointLabel;
exports.groupTaskDisplayStatus = groupTaskDisplayStatus;
exports.buildGroupCompletionSummary = buildGroupCompletionSummary;
exports.buildGroupPickupSummary = buildGroupPickupSummary;
exports.groupTodoTextNeedsUserAction = groupTodoTextNeedsUserAction;
exports.groupStatusPhaseNeedsUserAction = groupStatusPhaseNeedsUserAction;
exports.buildGroupCurrentTodoSummary = buildGroupCurrentTodoSummary;
exports.buildGroupProgressRefreshSummary = buildGroupProgressRefreshSummary;
exports.getGroupStatusIndependentReviewSummary = getGroupStatusIndependentReviewSummary;
exports.summarizeGroupStatusIndependentReview = summarizeGroupStatusIndependentReview;
exports.getGroupStatusTestAgentExecutionPlanSummary = getGroupStatusTestAgentExecutionPlanSummary;
exports.summarizeGroupStatusTestAgentExecutionPlan = summarizeGroupStatusTestAgentExecutionPlan;
...
function compactGroupMessageTaskRuntime(runtime) {
    ...
    return {
        ...
        taskCard: card ? {
            ...
            // 降级卡片必须保留执行前计划：确认卡的步骤、追加修订和确认按钮语义都依赖它。
            plan_mode: card.plan_mode,
...
function groupProgressTimeMs(...values) {
    const times = values
        .map(value => Date.parse(String(value || "")))
        .filter(value => Number.isFinite(value) && value > 0);
    return times.length ? Math.max(...times) : 0;
}
function groupProgressAgeLabel(ageMs) {
...
function checkpointStatus(status) {
    ...
    if (["done", "completed", "ok", "success", "succeeded"].includes(value))
        return "done";
    if (["active", "running", "in_progress", "reviewing", "reworking"].includes(value))
        return "active";
    if (["warn", "warning", "blocked", "needs_confirmation", "cancelled", "canceled", "stopped"].includes(value))
        return "warning";
    if (["fail", "failed", "error"].includes(value))
        return "failed";
    return "pending";
}
function groupTaskStatusMeta(status) {
    const value = String(status || "").toLowerCase();
    ...
    if (["reworking", "needs_rework", "retrying", "repairing"].includes(value))
        return { phase: "reworking", label: "返工中", terminal: false, deliveryStatus: "active", checkpointStatus: "active" };
    if (["blocked", "needs_attention", "partial", "missing_receipt", "needs_info"].includes(value))
        return { phase: "needs_user", label: "待补齐", terminal: false, deliveryStatus: "active", checkpointStatus: "warning" };
    if (["reviewing", "review", "verifying"].includes(value))
        return { phase: "reviewing", label: "验收中", terminal: false, deliveryStatus: "active", checkpointStatus: "active" };
    if (["waiting_user", "needs_user", "needs_confirmation"].includes(value))
        return { phase: "needs_user", label: "等待你确认", terminal: false, deliveryStatus: "active", checkpointStatus: "warning" };
    if (["pending", "queued"].includes(value))
        return { phase: value || "queued", label: "排队中", terminal: false, deliveryStatus: "active", checkpointStatus: "pending" };
    if (["in_progress", "running", "executing", "active"].includes(value))
        return { phase: "executing", label: "正在处理", terminal: false, deliveryStatus: "active", checkpointStatus: "active" };
    return { phase: value || "idle", label: value ? "正在处理" : "空闲", terminal: false, deliveryStatus: "active", checkpointStatus: value ? "active" : "pending" };
}
function groupDeliveryStatusLabel(status) {
    if (status === "done")
        return "已完成";
    if (status === "failed")
        return "未完成";
...
function groupDeliveryCheckpointLabel(status) {
    if (status === "done")
        return "任务交付完成";
    if (status === "failed")
        return "任务未完成，已整理原因";
...
function countGroupStatusItems(...values) {
    let count = 0;
...
function flattenGroupAcceptanceRows(...values) {
    const rows = [];
    const visit = (value) => {
        if (!value)
            return;
...
function isBareGroupAcceptanceMarker(value) {
    return /^(最终验收|主\s*Agent\s*验收|验收结论)\s*[：:]?\s*(已通过|通过)$/i.test(String(value || "").trim());
}
function isStrongGroupVerificationText(value) {
    const text = String(value || "").trim();
    if (!text)
        return false;
...
function groupHasStrongAcceptanceEvidence(task, summary = {}, latestCard = null) {
    if (latestCard?.delivery?.acceptance_passed === true)
        return true;
    if (latestCard?.acceptance_review?.pass === true || latestCard?.acceptanceReview?.pass === true)
        return true;
    if (latestCard?.mainAgentDecision?.verify?.passed === true || latestCard?.main_agent_decision?.verify?.passed === true)
        return true;
    ...
    if (summary?.independent_review_gate_passed === true && Number(summary?.independent_review_gate?.evidence_count || reviewRows.length || 0) > 0)
        return true;
    if (reviewRows.some((row) => isPositiveGroupAcceptanceText(groupEvidenceText(row)) && !isBareGroupAcceptanceMarker(groupEvidenceText(row))))
        return true;
    const acceptanceRows = flattenGroupAcceptanceRows(summary?.acceptance, summary?.acceptance_evidence, summary?.acceptanceEvidence, report?.acceptance, report?.acceptance_evidence, report?.acceptanceEvidence);
...
function groupTaskDisplayStatus(task, summary = {}, latestCard = null, rawStatus = "") {
...
function friendlyGroupCompletionText(value, fallback, max = 220) {
    ...
    if (/CCM_AGENT_RECEIPT|trace[_-]?id|session[_-]?id|run[_-]?id|workflow_timeline|runtime kernel|raw_report|stack/i.test(text)) {
        return (0, delivery_report_1.sanitizeMainAgentDeliveryText)(text, fallbackText, max);
    }
    return (0, delivery_report_1.sanitizeMainAgentDeliveryText)(text, fallbackText, max);
}
function firstDeliverySectionItem(report, sectionId) {
    const section = Array.isArray(report?.sections)
        ? report.sections.find((item) => item?.id === sectionId || item?.title === sectionId)
        : null;
    return Array.isArray(section?.items) ? section.items.find(Boolean) : "";
}
function buildGroupCompletionSummary(task, summary, latestCard = null) {
    const report = summary?.delivery_report || summary?.deliveryReport || null;
    ...
    const fallbackHeadline = status === "done"
        ...
        : status === "failed"
            ...
            : status === "cancelled"
                ? "任务已取消，当前状态已整理。"
                : "我已整理阶段性处理结果。";
    const headline = friendlyGroupCompletionText(report?.headline || summary?.headline || summary?.summary || task?.status_detail || firstDeliverySectionItem(report, "完成内容"), fallbackHeadline, 260);
    const fileCount = countGroupStatusItems(summary?.actual_file_change_count, summary?.file_change_count, summary?.file_changes?.count, summary?.actual_file_changes, report?.files);
    const verificationCount = countGroupStatusItems(summary?.external_runner_verification_count, summary?.verification_count, summary?.verification_executed, summary?.verification, report?.verification);
    const riskCount = countGroupStatusItems(summary?.blockers, summary?.needs, summary?.failed_gates, summary?.acceptance_gate?.failed_checks, report?.risks);
    const nextAction = friendlyGroupCompletionText(Array.isArray(report?.next_action) ? report.next_action[0] : report?.next_action || firstDeliverySectionItem(report, "下一步"), status === "done" ? "可以查看改动详情，或继续补充新的要求。" : "请先处理上面的风险或补充信息。", 220);
    return {
...
function buildGroupPickupSummary(task, summary, completionSummary, latestCard = null) {
    const report = summary?.delivery_report || summary?.deliveryReport || null;
    ...
    const statusMeta = groupTaskStatusMeta(groupTaskDisplayStatus(task, summary, latestCard, rawStatus));
    if (!task && !existing && !report)
        return null;
    if (!existing && !report && !statusMeta.terminal)
        return null;
    if (!statusMeta.terminal && (String(rawStatus || "").toLowerCase() === "done" || String(report?.status || "").toLowerCase() === "done"))
        return null;
    const status = existing?.status || report?.status || completionSummary?.status || statusMeta.deliveryStatus;
    const headline = friendlyGroupCompletionText(existing?.headline || report?.headline || completionSummary?.headline || task?.status_detail || "", statusMeta.terminal ? "我已整理本轮任务结果。" : "我已整理当前任务状态。", 260);
    const currentState = friendlyGroupCompletionText(existing?.current_state || existing?.currentState || headline, headline, 260);
    const reviewItems = Array.isArray(existing?.review_items || existing?.reviewItems)
        ? (existing.review_items || existing.reviewItems)
            .map((item) => friendlyGroupCompletionText(item, "", 180))
            .filter(Boolean)
            .slice(0, 5)
        : [];
    const resumeAction = friendlyGroupCompletionText(existing?.resume_action
        ...
        || "", statusMeta.terminal ? "可以查看交付总结，或继续补充新的要求。" : "我会继续推进当前任务，并在完成后汇总给你。", 220);
    return {
        ...
        technical_hint: friendlyGroupCompletionText(existing?.technical_hint || existing?.technicalHint || "底层执行记录和排障信息默认收在技术详情里。", "底层执行记录和排障信息默认收在技术详情里。", 180),
        display_policy: {
            user_visible: true,
...
function liveTodoStatusLabel(status) {
    const value = String(status || "").toLowerCase();
    if (["completed", "done", "success", "succeeded"].includes(value))
        return "已完成";
    if (["in_progress", "active", "running"].includes(value))
        return "进行中";
    if (["reviewing"].includes(value))
        return "验收中";
    if (["reworking"].includes(value))
        return "返工中";
    if (["needs_confirmation", "needs_user", "blocked"].includes(value))
        return "待确认";
...
function groupTodoTextNeedsUserAction(value, phase = "") {
    const text = String(value || "").replace(/\s+/g, " ").trim();
    if (!text)
        return false;
    ...
    if (userActionPattern.test(text))
        return true;
    if (["needs_user", "waiting_user", "needs_confirmation"].includes(phaseText) && /确认(?:计划|范围|授权|执行|操作)|补充(?:目标|范围|信息|材料|上下文|账号|环境变量|验收标准|需求)|提供(?:目标|信息|材料|凭证|截图|账号|环境变量)|回复|授权|选择|上传|输入|填写|处理\s*\d+\s*个待确认问答/.test(text))
        return true;
    return false;
}
function groupStatusPhaseNeedsUserAction(status) {
    if (String(status?.phase || "").toLowerCase() !== "needs_user")
        return false;
    return [
        status?.label,
        status?.status_label,
    ...
    ].some((item) => groupTodoTextNeedsUserAction(item, "needs_user"));
}
function buildGroupCurrentTodoSummary(latestCard, latestTask, latestStatusMeta) {
    if (!latestCard || latestStatusMeta?.terminal)
        return null;
    ...
    const steps = Array.isArray(todo?.steps) ? todo.steps.filter((item) => item?.content || item?.label || item?.activeForm || item?.active_form) : [];
    if (!steps.length)
        return null;
    const policy = { ...(todo?.display || {}), ...(todo?.display_policy || todo?.displayPolicy || {}) };
    const allDone = steps.every((item) => ["completed", "done", "success", "succeeded"].includes(String(item?.status || "").toLowerCase()));
    ...
    if (allDone && archiveCompleted && !hasVerificationReminder)
        return null;
    const activeStatuses = new Set(["in_progress", "active", "running", "reviewing", "reworking", "needs_confirmation", "needs_user", "failed"]);
    const activeStep = steps.find((item) => activeStatuses.has(String(item?.status || "").toLowerCase()))
        || steps.find((item) => !["completed", "done", "cancelled", "canceled"].includes(String(item?.status || "").toLowerCase()))
        || steps[steps.length - 1];
    if (!activeStep)
        return null;
    const completedCount = steps.filter((item) => ["completed", "done", "success", "succeeded"].includes(String(item?.status || "").toLowerCase())).length;
    ...
    const recentStep = [...steps]
        .reverse()
        .find((item) => ["completed", "done", "success", "succeeded"].includes(String(item?.status || "").toLowerCase()) && item !== activeStep);
    const recentAction = friendlyGroupCompletionText(recentStep?.activeForm || recentStep?.active_form || recentStep?.content || recentStep?.label || latestCard.latest_progress_checkpoint?.label || "", "", 180);
    const needsAction = friendlyGroupCompletionText(groupTodoTextNeedsUserAction(activeStep.needs_action || activeStep.needsAction || todo?.needs_action || todo?.needsAction || "", latestStatusMeta?.phase || status)
...
function normalizeGroupProgressWorkItem(row, fallbackTask, index) {
    if (!row || typeof row !== "object")
        return null;
    const target = friendlyGroupCompletionText(row.target || row.owner || row.agent || row.project || row.name || fallbackTask?.target_project || "", "", 80);
    const subject = friendlyGroupCompletionText(row.subject || row.title || row.task || row.message || row.description || fallbackTask?.title || "执行成员工作项", "执行成员工作项", 140);
    if (!target && !subject)
        return null;
    return {
...
function groupProgressWorkItems(latestCard, latestTask) {
    const sources = [];
    ...
    if (Array.isArray(summary.next_claimable || summary.nextClaimable)) {
        sources.push(...(summary.next_claimable || summary.nextClaimable).map((item) => ({ ...item, status: item.status || "pending" })));
    }
    if (Array.isArray(summary.blocked)) {
        sources.push(...summary.blocked.map((item) => ({ ...item, status: item.status || "blocked" })));
    }
...
function getGroupWorkItemRequeueMarker(latestCard, latestTask) {
...
function buildGroupProgressRefreshSummary(latestTask, latestCard, latestStatusMeta, childAgentStatusSummary, latestCheckpoint, nowMs = Date.now()) {
    if (!latestTask || latestStatusMeta?.terminal)
        return null;
    ...
    const lastProgressMs = groupProgressTimeMs(latestCheckpoint?.at, latestCard?.updated_at, latestCard?.updatedAt, latestTask?.updated_at, latestTask?.updatedAt, latestTask?.started_at, latestTask?.startedAt, latestTask?.created_at);
    ...
    const stalledItems = items.filter((item) => {
        if (!["in_progress", "running", "active", "reviewing"].includes(item.status))
            return false;
        const itemMs = groupProgressTimeMs(item.updated_at, latestTask?.updated_at);
        return itemMs > 0 && nowMs - itemMs >= staleMs;
    });
    const nextClaimable = items.filter((item) => ["pending", "queued", "waiting"].includes(item.status)).slice(0, 3);
    ...
    if (!stalledItems.length && !requeueMarker && !longWithoutVisibleProgress && !pendingTooLong)
        return null;
...
function firstGroupStatusObject(...values) {
    return values.find(value => value && typeof value === "object" && !Array.isArray(value)) || null;
}
function groupStatusSafeArray(value) {
    return Array.isArray(value) ? value : [];
}
function scrubGroupTestAgentEvidencePathText(value) {
    return String(value || "")
...
function groupTestAgentFailureTypeLabel(type) {
...
function collectGroupStatusTestAgentFailureItems(source, depth = 0, seenObjects = new Set()) {
    ...
    for (const key of nestedKeys) {
        if (source[key])
            rows.push(...collectGroupStatusTestAgentFailureItems(source[key], depth + 1, seenObjects));
    }
    return rows;
}
function summarizeGroupTestAgentFailureItem(item) {
...
function summarizeGroupTestAgentDiagnosticItem(item) {
...
function buildGroupStatusIndependentReviewSummaryFromTestAgentFailure(...sources) {
    const seen = new Set();
    const items = [];
...
function getGroupStatusIndependentReviewSummary(source = {}) {
    ...
    const explicitSummary = firstGroupStatusObject(source?.independent_review_summary, source?.independentReviewSummary, source?.test_agent_review_summary, source?.testAgentReviewSummary, delivery?.independent_review_summary, delivery?.independentReviewSummary, delivery?.test_agent_review_summary, delivery?.testAgentReviewSummary, report?.independent_review_summary, report?.independentReviewSummary, report?.test_agent_review_summary, report?.testAgentReviewSummary, card?.independent_review_summary, card?.independentReviewSummary, card?.test_agent_review_summary, card?.testAgentReviewSummary, technical?.independent_review_summary, technical?.independentReviewSummary, technical?.test_agent_review_summary, technical?.testAgentReviewSummary);
    if (explicitSummary)
        return explicitSummary;
    return buildGroupStatusIndependentReviewSummaryFromTestAgentFailure(source, delivery, report, card, technical);
}
function groupReviewRowText(item) {
...
function groupIndependentReviewStatusKind(summary = {}) {
    const text = [
        summary.status,
        summary.verdict,
...
function summarizeGroupStatusIndependentReview(source = {}) {
    ...
    const rows = (Array.isArray(summary.rows) ? summary.rows : Array.isArray(summary.items) ? summary.items : [])
        .map((item) => cleanGroupStatusFollowupText(groupReviewRowText(item), "", 140))
        .filter(Boolean)
        .slice(0, 3);
    const headline = cleanGroupStatusFollowupText(summary.headline || summary.summary || rows[0] || "", status === "needs_rework"
        ...
        : status === "needs_user"
            ...
            : status === "passed"
                ? "独立复核已通过。"
                : "独立复核结论已记录。", 200);
    const nextAction = cleanGroupStatusFollowupText(summary.next_action || summary.nextAction || "", status === "needs_rework"
...
function groupStatusTestAgentPlanRowText(item) {
    if (!item || typeof item !== "object")
        return item;
    return [item.summary, item.detail, item.message, item.label, item.title, item.status].filter(Boolean).join(" ");
}
function groupStatusTestAgentPlanStatusKind(summary = {}, plan = null) {
    const text = [
        summary.status,
        summary.verdict,
        summary.status_label,
...
function getGroupStatusTestAgentExecutionPlanSummary(source = {}) {
    const card = source?.latest_card || source?.latestCard || source?.task_card || source?.taskCard || {};
    const delivery = source?.latest_delivery_summary
        || source?.latestDeliverySummary
        || source?.delivery_summary
    ...
    const report = source?.final_report
        ...
        || {};
    const technical = card?.technical || delivery?.technical || report?.technical || {};
    const explicitSummary = firstGroupStatusObject(source?.test_agent_execution_plan_summary, source?.testAgentExecutionPlanSummary, delivery?.test_agent_execution_plan_summary, delivery?.testAgentExecutionPlanSummary, report?.test_agent_execution_plan_summary, report?.testAgentExecutionPlanSummary, card?.test_agent_execution_plan_summary, card?.testAgentExecutionPlanSummary, technical?.test_agent_execution_plan_summary, technical?.testAgentExecutionPlanSummary);
    const plan = firstGroupStatusObject(source?.test_agent_execution_plan, source?.testAgentExecutionPlan, delivery?.test_agent_execution_plan, delivery?.testAgentExecutionPlan, report?.test_agent_execution_plan, report?.testAgentExecutionPlan, card?.test_agent_execution_plan, card?.testAgentExecutionPlan, technical?.test_agent_execution_plan, technical?.testAgentExecutionPlan);
    if (explicitSummary)
    ...
    const issues = groupStatusSafeArray(plan?.issues)
        .map((item) => cleanGroupStatusFollowupText(scrubGroupTestAgentEvidencePathText(groupStatusTestAgentPlanRowText(item)), "", 160))
        .filter(Boolean)
        .slice(0, 5);
    ...
    const fallbackHeadline = cleanGroupStatusFollowupText(rawTextSummary || detail, "TestAgent 复核计划已生成，我会先确认计划可执行，再启动真实复核。", 260);
    return {
        summary: {
...
function summarizeGroupStatusTestAgentExecutionPlan(source = {}) {
    ...
    const seen = new Set();
    const rows = [
        ...groupStatusSafeArray(summary.rows).map((item) => cleanGroupStatusFollowupText(scrubGroupTestAgentEvidencePathText(groupStatusTestAgentPlanRowText(item)), "", 140)),
        ...groupStatusSafeArray(summary.issues).map((item) => `预检问题：${cleanGroupStatusFollowupText(scrubGroupTestAgentEvidencePathText(groupStatusTestAgentPlanRowText(item)), "", 120)}`),
    ].filter((item) => {
    ...
    const headline = cleanGroupStatusFollowupText(scrubGroupTestAgentEvidencePathText(summary.headline || summary.summary || rows[0] || ""), status === "ready"
        ? "TestAgent 复核计划已生成，我会按计划启动真实验证。"
        : status === "blocked"
            ? "TestAgent 复核计划预检未通过，我会先修复交接信息再执行。"
            : "TestAgent 复核计划已记录，正在等待下一步复核。", 220);
    const nextAction = cleanGroupStatusFollowupText(scrubGroupTestAgentEvidencePathText(summary.next_action || summary.nextAction || ""), status === "ready"
        ? "启动 TestAgent 真实复核，并把结论纳入最终验收。"
        : status === "blocked"
            ? "修复 TestAgent 工作单或项目路径后重新生成复核计划。"
...
// ===== merged from group-routes-part-02.ts =====
function buildGroupMainAgentStatus(input) {
    ...
    ].map((item) => compactGroupStatusText(item, 120)).filter(Boolean).slice(0, 5);
    const needs = [
...
function buildGroupStatusUserActionSummary(status) {
    ...
    const userNeeds = needs.filter((item) => groupTodoTextNeedsUserAction(item, phase));
    const userBlockers = blockers.filter((item) => groupTodoTextNeedsUserAction(item, phase));
...
// ===== merged from group-routes-part-03.ts =====
function handleBasicGroupRoutes(req, res, parsed, ctx, deps) {
    ...
    if (pathname === "/api/groups/create" && req.method === "POST") {
        ...
        req.on("end", () => {
            try {
                const { name, members, purpose } = JSON.parse(body);
                ...
                const projectNames = new Set((0, db_1.getConfigs)().map(item => String(item.name || "")));
                const invalidMembers = allMembers.map((member) => String(member?.project || "").trim()).filter((project) => project && project !== "coordinator" && !projectNames.has(project));
                if (invalidMembers.length)
                    throw new Error(`群聊成员项目不存在：${invalidMembers.join("、")}`);
                const principal = req.ccmAuth;
                if (principal?.kind === "browser" && principal.role !== "admin") {
                    const unauthorized = allMembers.map((member) => String(member?.project || "").trim()).filter((project) => project && project !== "coordinator" && !(0, access_policy_1.hasResourceAccess)(principal.userId, principal.role, "project", project, "manage"));
                    if (unauthorized.length)
                        throw Object.assign(new Error(`当前账户无权管理成员项目：${unauthorized.join("、")}`), { code: "RESOURCE_ACCESS_DENIED" });
                }
                const group = (0, group_orchestrator_1.normalizeGroupOrchestrator)({
                    id, name, purpose: cleanPurpose.slice(0, 2_000), purpose_revision: 1, purpose_updated_at: new Date().toISOString(), members: allMembers,
                    created_at: new Date().toISOString(),
                });
                groups.push(group);
                (0, storage_1.saveGroups)(groups);
                const instructionGeneration = (0, scope_instructions_1.ensureGroupScopeInstructions)({ groupId: id, name, purpose: group.purpose, projectIds: group.members.map((member) => String(member?.project || "")).filter((project) => project && project !== "coordinator") });
                (0, utils_1.sendJson)(res, { success: true, group: (0, group_test_targets_1.publicGroupWithoutTestTargetSecrets)(group), instruction_generation: instructionGeneration });
            }
            catch (e) {
    ...
    if (pathname === "/api/groups/members" && req.method === "POST") {
        ...
        req.on("end", () => {
            try {
                ...
                if (!(0, access_policy_1.authorizeResource)(req, res, "group", id, "manage"))
                    return;
                const groups = (0, storage_1.loadGroups)();
                ...
                (0, storage_1.saveGroups)(groups);
                const instructionGeneration = membershipChanged ? (0, scope_instructions_1.ensureGroupScopeInstructions)({ groupId: String(group.id || id), name: group.name, purpose: group.purpose, projectIds: group.members.map((member) => String(member?.project || "")).filter((project) => project && project !== "coordinator") }) : [];
                ...
                (0, utils_1.sendJson)(res, { success: true, group: (0, group_test_targets_1.publicGroupWithoutTestTargetSecrets)(group), context_refresh: contextRefresh, instruction_generation: instructionGeneration });
            }
            catch (e) {
                (0, utils_1.sendJson)(res, { error: e.message }, 400);
            }
    ...
    if (pathname === "/api/groups/delete" && req.method === "POST") {
        ...
        req.on("end", () => {
            try {
                ...
                (0, scope_instructions_1.deactivateGroupScopeInstructions)(id);
    ...
    if (pathname === "/api/groups/rename" && req.method === "POST") {
        ...
        req.on("end", () => {
            try {
                ...
                (0, storage_1.saveGroups)(groups);
                const instructionGeneration = (0, scope_instructions_1.ensureGroupScopeInstructions)({ groupId: String(group.id || id), name: group.name, purpose: group.purpose, projectIds: group.members.map((member) => String(member?.project || "")).filter((project) => project && project !== "coordinator") });
                (0, utils_1.sendJson)(res, { success: true, group: (0, group_test_targets_1.publicGroupWithoutTestTargetSecrets)(group), instruction_generation: instructionGeneration });
            }
            catch (e) {
                (0, utils_1.sendJson)(res, { error: e.message }, 400);
    ...
    if (pathname === "/api/groups/memory/capacity" && req.method === "GET") {
        ...
        (0, utils_1.sendJson)(res, {
            ...
            strategy: "request_preflight_pressure",
    ...
    if (pathname === "/api/groups/memory/compact" && req.method === "POST") {
        ...
        req.once("aborted", () => requestAbort.abort(new Error("压缩请求连接已中断")));
        res.once("close", () => { if (!res.writableEnded)
            requestAbort.abort(new Error("压缩响应连接已中断")); });
        let body = "";
        req.on("data", (chunk) => body += chunk);
        req.on("end", async () => {
            try {
                ...
                const result = await (0, group_memory_context_1.runGroupMemoryAutoCompactionNow)(groupId, {
                    ...
                    compactionRequest: {
                        mode: payload.mode,
                        pivotMessageId: payload.pivotMessageId || payload.pivot_message_id,
                        direction: payload.direction,
                        customInstructions,
                        expectedGeneration: payload.expectedGeneration ?? payload.expected_generation,
                        expectedPayloadChecksum: payload.expectedPayloadChecksum || payload.expected_payload_checksum,
                        trigger: "manual",
                    },
                    compactionRunId: payload.compactionRunId || payload.compaction_run_id,
                ...
                (0, utils_1.sendJson)(res, {
                    ...
                    summary: result.compacted === true ? "当前群聊会话已手动压缩。" : "当前群聊会话没有可压缩的对话消息。",
                    metrics: {
                        群聊会话: sessionId,
                        压缩对话: Number(compression.compressedMessages || result.messagesToCompact?.length || 0),
                        保留未压缩原文: Number(result.keptMessages?.length || 0),
                    },
                    boundaryId: result.boundary?.id || "",
                    sessionId,
                    mode: result.compactionMode || "full",
                    partial_compaction: result.partialCompaction || null,
                    compaction_run_id: String(result?.receipt?.compactionRunId || payload.compactionRunId || payload.compaction_run_id || ""),
                    execution_path: String(result?.receipt?.executionPath || "none"),
                });
            }
            catch (error) {
