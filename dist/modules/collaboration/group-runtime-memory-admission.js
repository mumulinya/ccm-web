...
exports.isModelVisibleGroupDecision = isModelVisibleGroupDecision;
exports.isModelVisibleGroupNextAction = isModelVisibleGroupNextAction;
exports.isModelVisibleGroupFactAnchor = isModelVisibleGroupFactAnchor;
exports.modelVisibleGroupRuntimeState = modelVisibleGroupRuntimeState;
exports.isCanonicalGroupSessionMemory = isCanonicalGroupSessionMemory;
...
function isModelVisibleGroupDecision(value) {
    const text = textOf(value, ["decision", "text", "value"]);
    return text.length >= 8 && !TRANSIENT_COORDINATION.test(text);
}
function isModelVisibleGroupNextAction(value) {
    const text = textOf(value, ["action", "text", "value"]);
    return text.length >= 6 && !TRANSIENT_COORDINATION.test(text);
}
function isModelVisibleGroupFactAnchor(value) {
    if (value?.memoryAdmission?.admitted === true || value?.memory_admission?.admitted === true || value?.persistent === true)
        return true;
...
function modelVisibleGroupRuntimeState(memory) {
    return {
