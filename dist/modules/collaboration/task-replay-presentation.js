...
function replayEvidencePolicyLevel(input) {
    const explicit = input.tasks.map(task => task?.evidencePolicy
        || task?.evidence_policy
        || task?.implementation_plan?.evidencePolicy
        || task?.dispatch_contract?.evidencePolicy
        || task?.plan_dispatch_contract?.evidencePolicy)
        .find(policy => policy?.schema === "ccm-evidence-policy-v1");
    if (explicit?.level)
        return explicit.level;
    const projects = unique(input.tasks.flatMap(task => [
        (0, task_replay_shared_1.safeText)(task?.target_project || task?.project, 200),
        ...(Array.isArray(task?.target_projects) ? task.target_projects.map((item) => (0, task_replay_shared_1.safeText)(typeof item === "string" ? item : item?.project || item?.id, 200)) : []),
    ]).filter(project => project && !["global-agent", "coordinator"].includes(project)));
    const requiresCodeChanges = input.tasks.some(task => task?.requires_code_changes === true
        || (Array.isArray(task?.file_changes?.files) && task.file_changes.files.length > 0)
        || (Array.isArray(task?.delivery_summary?.actual_file_changes) && task.delivery_summary.actual_file_changes.length > 0)
        || (Array.isArray(task?.receipt?.filesChanged) && task.receipt.filesChanged.length > 0));
    return (0, evidence_policy_1.resolveEvidencePolicy)({
        requiresCodeChanges,
        targetProjects: projects,
        riskLevel: input.tasks.some(task => String(task?.risk_level || task?.riskLevel || "").toLowerCase() === "high") ? "high" : "write",
        hasArchitectureOrPublicContractChange: input.tasks.some(task => task?.hasArchitectureOrPublicContractChange === true || task?.public_contract_change === true),
        hasPermissionOrSecurityChange: input.tasks.some(task => task?.hasPermissionOrSecurityChange === true || task?.permission_change === true || task?.security_change === true),
        hasMigration: input.tasks.some(task => task?.hasMigration === true || task?.migration === true),
        hasReleaseOrDeployment: input.tasks.some(task => task?.hasReleaseOrDeployment === true || task?.release === true || task?.deployment === true),
        destructive: input.tasks.some(task => task?.destructive === true),
        scopeExpanded: input.tasks.some(task => task?.scopeExpanded === true || task?.scope_expanded === true),
    }).level;
}
function lightweightTerminalCoverage(input) {
    if (replayEvidencePolicyLevel(input) !== "lightweight")
        return null;
    const terminalTask = input.tasks.find(task => COMPLETED.has(String(task?.status || "").toLowerCase())
        && ["accepted", "terminal_gate_passed"].includes(String(task?.acceptance_state || "").toLowerCase())
        && task?.terminal_gate?.passed === true
        && task?.terminal_decision?.gate_passed === true);
    if (!terminalTask)
        return null;
    const fileRows = [
        ...(Array.isArray(terminalTask?.file_changes?.files) ? terminalTask.file_changes.files : []),
        ...(Array.isArray(terminalTask?.delivery_summary?.actual_file_changes) ? terminalTask.delivery_summary.actual_file_changes : []),
        ...(Array.isArray(terminalTask?.receipt?.filesChanged) ? terminalTask.receipt.filesChanged : []),
    ];
    const verificationRows = [
        ...(Array.isArray(terminalTask?.verification) ? terminalTask.verification : []),
        ...(Array.isArray(terminalTask?.delivery_summary?.verification) ? terminalTask.delivery_summary.verification : []),
        ...(Array.isArray(terminalTask?.delivery_summary?.verification_executed) ? terminalTask.delivery_summary.verification_executed : []),
        ...(Array.isArray(terminalTask?.receipt?.verification) ? terminalTask.receipt.verification : []),
    ];
    if (!fileRows.length || !verificationRows.length)
        return null;
    const evidenceIds = unique(input.evidence
        .filter(item => String(item?.task_id || item?.taskId || "") === String(terminalTask.id || "")
        && ["code_changes", "verification", "file_change", "terminal_gate"].includes(String(item?.type || "")))
        .map(item => String(item.id || "")));
    return { terminalTask, evidenceIds };
}
function buildAcceptanceMatrix(input) {
    ...
    const lightweightCoverage = lightweightTerminalCoverage(input);
    return criteria.map((description, index) => {
        ...
        if (status === "satisfied" && freshness === "stale")
            status = "stale";
        if (!related && lightweightCoverage) {
            status = "satisfied";
            freshness = "current";
            evidenceIds.push(...lightweightCoverage.evidenceIds.filter(id => !evidenceIds.includes(id)));
        }
        else if (!related && normalizedOutcome(input.status) === "completed" && input.acceptanceState.toLowerCase() === "accepted")
            status = "not_run";
        return {
            criterionId: (0, task_replay_shared_1.safeText)(related?.criterion_id || related?.criterionId, 100) || `criterion_${index + 1}`,
            description,
            status,
            verifier: (0, task_replay_shared_1.safeText)(related?.verifier || related?.event?.actor?.label, 100) || (related ? "TestAgent" : lightweightCoverage ? "Terminal Gate" : "未记录"),
            evidenceIds,
            freshness,
            reason: (0, task_replay_shared_1.safeText)(related?.reason || related?.detail, 500)
                || (!related && lightweightCoverage ? "轻量核验已自动采集真实文件变化、验证结果并通过 Terminal Gate；格式化映射缺口仅作为技术告警。" : ""),
        };
    });
}
...
function explicitActions(value, fallbackTaskId = "") {
    ...
    return (Array.isArray(value) ? value : []).flatMap((row, index) => {
        ...
        return [{
                ...
                kind,
                label: (0, task_replay_shared_1.safeText)(row?.label, 80) || { retry: "继续任务", resume_interrupted: "继续任务", resume_paused: "继续任务", continue: "继续任务", resolve_permission: "处理授权", view_error: "查看错误", recheck: "重新核验", takeover: "人工接管" }[kind],
...
function runTaskReplayPresentationSelfTest() {
    ...
    const lightweightTerminal = buildTaskReplayPresentation({
        root: { id: "lightweight", business_goal: "更新说明" },
        tasks: [{
                id: "lightweight",
                status: "done",
                acceptance_state: "accepted",
                acceptance_criteria: "README 已更新并通过构建",
                evidencePolicy: (0, evidence_policy_1.resolveEvidencePolicy)({ requiresCodeChanges: true, targetProjects: ["demo"] }),
                file_changes: { files: [{ path: "README.md" }] },
                receipt: { verification: ["npm run build"] },
                terminal_gate: { passed: true },
                terminal_decision: { gate_passed: true },
            }],
        plans: [], workItems: [], deliveries: [],
        evidence: [
            { id: "ev-file", task_id: "lightweight", type: "code_changes", status: "available" },
            { id: "ev-check", task_id: "lightweight", type: "verification", status: "available" },
        ],
        events: [], status: "done", acceptanceState: "accepted",
        startedAt: "2026-08-01T00:00:00.000Z", finishedAt: "2026-08-01T00:02:00.000Z",
    });
    const strictTerminal = buildTaskReplayPresentation({
        root: { id: "strict", business_goal: "跨项目迁移" },
        tasks: [{
                id: "strict", status: "done", acceptance_state: "accepted", acceptance_criteria: "迁移完成",
                evidencePolicy: (0, evidence_policy_1.resolveEvidencePolicy)({ requiresCodeChanges: true, targetProjects: ["web", "api"] }),
                file_changes: { files: [{ path: "migration.sql" }] }, receipt: { verification: ["migration check"] },
                terminal_gate: { passed: true }, terminal_decision: { gate_passed: true },
            }],
        plans: [], workItems: [], deliveries: [], evidence: [], events: [], status: "done", acceptanceState: "accepted",
        startedAt: "2026-08-01T00:00:00.000Z", finishedAt: "2026-08-01T00:02:00.000Z",
    });
    const checks = {
        ...
        lightweight_terminal_coverage: lightweightTerminal.acceptanceMatrix[0]?.status === "satisfied" && lightweightTerminal.acceptanceMatrix[0]?.verifier === "Terminal Gate",
        strict_still_requires_mapping: strictTerminal.acceptanceMatrix[0]?.status === "not_run",
