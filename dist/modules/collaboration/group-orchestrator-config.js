...
exports.migrateLegacyProviderTransportConfig = migrateLegacyProviderTransportConfig;
...
exports.CCM_DIR = path.join(os.homedir(), ".ccm");
...
function defaultOrchestratorConfig() {
    return {
        ...
        agentToolBatchSize: readonly_tool_concurrency_1.CCM_READONLY_TOOL_CONCURRENCY_DEFAULT,
        agentReadOnlyParallelism: readonly_tool_concurrency_1.CCM_READONLY_TOOL_CONCURRENCY_DEFAULT,
        ...
        timeBasedMicrocompactGapMinutes: 0,
        timeBasedMicrocompactKeepRecent: 0,
...
function migrateLegacyProviderTransportConfig(stored = {}) {
    const hasUnifiedProtocolConfig = Object.prototype.hasOwnProperty.call(stored, "protocolMode")
        || Object.prototype.hasOwnProperty.call(stored, "protocol_mode")
        || Object.prototype.hasOwnProperty.call(stored, "protocolOverride")
        || Object.prototype.hasOwnProperty.call(stored, "protocol_override");
    const automaticTransport = (0, provider_cache_protocol_1.resolveProviderTransport)({ ...stored, format: "auto", protocolMode: "auto", protocolOverride: "" });
    const legacyTransport = (0, provider_cache_protocol_1.resolveProviderTransport)({ ...stored, protocolMode: "auto", protocolOverride: "" });
    const migratedProtocol = hasUnifiedProtocolConfig
        ? {
            protocolMode: String(stored.protocolMode || stored.protocol_mode || "auto").trim().toLowerCase() === "manual" ? "manual" : "auto",
            protocolOverride: String(stored.protocolOverride || stored.protocol_override || "").trim().toLowerCase(),
        }
        : legacyTransport.protocol !== automaticTransport.protocol
            ? { protocolMode: "manual", protocolOverride: legacyTransport.protocol }
            : { protocolMode: "auto", protocolOverride: "" };
    return { format: "auto", ...migratedProtocol };
}
function loadOrchestratorConfig() {
    try {
        ...
        const migratedProtocol = migrateLegacyProviderTransportConfig(stored);
        const retiredRuntimeConfig = {
            modelAutoCompactTokenLimit: 0,
            timeBasedMicrocompactEnabled: false,
            timeBasedThinkingClearEnabled: false,
            timeBasedMicrocompactGapMinutes: 0,
        ...
        if (Object.entries(retiredRuntimeConfig).some(([key, value]) => stored[key] !== value)) {
            Object.assign(stored, retiredRuntimeConfig);
            try {
                writeStoredOrchestratorConfig(stored);
            }
            catch { }
        ...
        return {
            ...
            ...stored,
            // The legacy format field is consumed above only as a migration hint.
...
function saveOrchestratorConfig(updates) {
    ...
    const protocolMode = updates.protocolMode ?? updates.protocol_mode;
    if (protocolMode !== undefined) {
        const value = String(protocolMode || "auto").trim().toLowerCase();
        if (!["auto", "manual"].includes(value))
            throw new Error("接口协议模式必须是 auto 或 manual");
        next.protocolMode = value;
    }
    const protocolOverride = updates.protocolOverride ?? updates.protocol_override;
    if (protocolOverride !== undefined) {
        const value = String(protocolOverride || "").trim().toLowerCase();
        if (value && !["chat_completions", "responses", "anthropic_messages", "gemini_generate_content", "custom"].includes(value)) {
            throw new Error("接口协议高级覆盖值无效");
        }
        next.protocolOverride = value;
    }
    if (next.protocolMode === "manual" && !next.protocolOverride)
    ...
    void modelAutoCompactTokenLimit;
    next.modelAutoCompactTokenLimit = 0;
    ...
    for (const [camelKey, snakeKey, min, max] of [
        ...
        ["agentToolBatchSize", "agent_tool_batch_size", 1, readonly_tool_concurrency_1.CCM_READONLY_TOOL_CONCURRENCY_MAX],
        ["agentReadOnlyParallelism", "agent_read_only_parallelism", 1, readonly_tool_concurrency_1.CCM_READONLY_TOOL_CONCURRENCY_MAX],
    ]) {
        const raw = updates[camelKey] ?? updates[snakeKey];
        if (raw === undefined)
            continue;
    ...
    // Legacy user cache controls are read for migration only. Cache strategy,
    // retention, family detection and native fields are now fully automatic.
    next.providerContextCacheMode = "auto";
    next.providerPromptCacheRetention = "automatic";
    next.providerNativeCacheEnabled = false;
    next.providerNativeCacheFamily = "auto";
    next.providerNativeCacheFamilyManual = false;
    next.anthropicCacheReferenceEnabled = false;
    ...
    next.timeBasedMicrocompactEnabled = false;
    next.timeBasedThinkingClearEnabled = false;
    next.timeBasedMicrocompactGapMinutes = 0;
    next.timeBasedMicrocompactKeepRecent = 0;
    ...
    if (next.apiUrl)
        (0, provider_cache_protocol_1.assertProviderTransportResolution)(next);
    persistOrchestratorConfig(next);
    return next;
}
function publicOrchestratorConfig(config = loadOrchestratorConfig()) {
    const { apiKey, summaryReviewerApiKey, searchMcpToken, braveSearchApiKey, bingSearchApiKey, googleCseApiKey, googleCseId, searchMcpUrl, providerContextCacheMode, providerPromptCacheRetention, providerNativeCacheEnabled, providerNativeCacheFamily, providerNativeCacheFamilyManual, anthropicCacheReferenceEnabled, modelAutoCompactTokenLimit, timeBasedMicrocompactEnabled, timeBasedThinkingClearEnabled, timeBasedMicrocompactGapMinutes, timeBasedMicrocompactKeepRecent, ...safe } = config;
    void providerContextCacheMode;
    void providerPromptCacheRetention;
    void providerNativeCacheEnabled;
    void providerNativeCacheFamily;
    void providerNativeCacheFamilyManual;
    void anthropicCacheReferenceEnabled;
    void modelAutoCompactTokenLimit;
    void timeBasedMicrocompactEnabled;
    void timeBasedThinkingClearEnabled;
    void timeBasedMicrocompactGapMinutes;
    void timeBasedMicrocompactKeepRecent;
    return {
        ...safe,
        automaticCacheOptimizationEnabled: true,
        providerTransport: publicProviderTransportResolution(config),
...
function publicProviderTransportResolution(config) {
    const resolution = (0, provider_cache_protocol_1.resolveProviderTransport)(config);
    let normalizedEndpoint = resolution.normalizedEndpoint;
    try {
    ...
    return { ...resolution, normalizedEndpoint };
}
...
async function testUnifiedModelConnection() {
    ...
    const providerTransport = publicProviderTransportResolution(config);
    const provider = {
        anthropic_messages: "anthropic-compatible",
        gemini_generate_content: "gemini-compatible",
        responses: "openai-responses",
        chat_completions: "openai-compatible",
        custom: "openai-compatible",
    }[providerTransport.protocol] || "openai-compatible";
    const consumers = [
    ...
    try {
        ...
        // Project and group conversations use the streaming native-tool request
        // shape. A tiny non-streaming health response alone cannot prove those
        // user-facing main-Agent transports are usable.
        await (0, group_orchestrator_llm_client_1.callNativeAgentTurn)(testConfig, {
            messages: [
                { role: "system", content: "You are running a CCM main-Agent transport check. Return only OK and do not call tools." },
                ...messages,
            ],
            nativeTools: [{
                    name: "ccm_connection_readonly_probe",
                    description: "A connection-shape probe that must not be called.",
        ...
        return {
            ...
            endpoint: providerTransport.normalizedEndpoint,
            providerTransport,
            protocol: provider,
