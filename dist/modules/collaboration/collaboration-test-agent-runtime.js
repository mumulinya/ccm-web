...
function buildCoordinatorTestAgentHandoff(item, input) {
    ...
    const presentedPlan = (0, group_presented_plan_1.presentedPlanFromTask)(task)
    ...
    const businessRequirement = presentedPlan?.businessRequirement || null;
    const acceptanceMatrix = Array.isArray(businessRequirement?.acceptanceCriteria)
        ? businessRequirement.acceptanceCriteria.map((criterion) => ({
            id: String(criterion?.id || ""),
            description: String(criterion?.description || ""),
            stepIds: (Array.isArray(presentedPlan?.steps) ? presentedPlan.steps : [])
                .filter((step) => (step?.acceptanceCriterionIds || []).includes(criterion?.id))
    ...
    return {
        ...
        acceptanceCriteria,
        requirementBinding: presentedPlan?.requirementBinding || null,
        planBinding: presentedPlan ? { planId: presentedPlan.planId || "", revision: presentedPlan.revision || 1, checksum: presentedPlan.checksum || "", contentStored: false } : null,
        acceptanceMatrix,
        completedTasks,
