...
function groupPlanningProjectRefs(task) {
    return Array.from(new Set([
        ...
        ...(Array.isArray(task?.business_requirement_contract?.targetProjects) ? task.business_requirement_contract.targetProjects : []),
        ...(Array.isArray(task?.business_requirement_contract?.target_projects) ? task.business_requirement_contract.target_projects : []),
        ...(Array.isArray(task?.target_projects) ? task.target_projects : []),
    ...
    for (const candidate of candidates) {
        try {
            ...
            return {
                ...
                requirement_checksum: String(parsed?.requirement_checksum || parsed?.requirementChecksum || "").trim(),
                plan_checksum: String(parsed?.plan_checksum || parsed?.planChecksum || "").trim(),
                step_id: String(parsed?.step_id || parsed?.stepId || "").trim(),
                step_ids: list(parsed?.step_ids ?? parsed?.stepIds ?? [parsed?.step_id || parsed?.stepId]),
                acceptance_criterion_ids: list(parsed?.acceptance_criterion_ids ?? parsed?.acceptanceCriterionIds),
...
function summarizeGroupPlanningSource(source) {
    return {
        ...
        projects: (Array.isArray(source?.projects) ? source.projects : []).map((project) => ({
            ...
            manifest_files: Array.isArray(project?.files) ? project.files.length : 0,
            ...
            files: (Array.isArray(project?.files) ? project.files : []).map((file) => ({
                ...
                evidence_id: String(file?.evidenceId || ""),
            })),
...
function attachGroupPlanningSourceToAssignments(assignments, source) {
    ...
    return (assignments || []).map((assignment) => {
        ...
        const evidence = {
            ...
            manifest_checksum: String(project?.manifestChecksum || ""),
            selected_paths: Array.isArray(project?.selectedPaths) ? project.selectedPaths : [],
            source_evidence_ids: (Array.isArray(project?.files) ? project.files : []).map((file) => String(file?.evidenceId || "")).filter(Boolean),
            status: String(project?.status || ""),
        };
        const evidenceText = [
            "[主 Agent 只读源码规划依据]",
...
async function executeTask(task, ctx, deps) {
    ...
    const pauseBoundary = (phase, workItemId = "") => {
        const latest = loadTasks().find((item) => String(item?.id || "") === String(task.id || "")) || task;
    ...
    if (task.assign_type === "group" && task.group_id) {
        ...
        const appendGroupTaskPlanState = (phase) => {
            ...
            const steps = normalized.map((step) => {
                ...
                if (explicitWorkItems.length) {
                    const dependencyWaiting = step.dependsOn.length > 0 && step.dependsOn.some((id) => {
                        const dependency = normalized.find((item) => item.id === id || item.workItemId === id);
                    ...
                    if (phase === "reviewing" && !completedStatus(step.status))
                        return { ...step, status: "reviewing" };
        ...
        let planningSource = await buildGroupMainPlanningSourceContext(group, message, configs, {
            ...
            maxRounds: 3,
            exactSessionId: groupSessionIdForTask(task),
        });
        pauseBoundary("planning");
        let planningSourceSummary = summarizeGroupPlanningSource(planningSource);
        let sourceGrounding = (0, development_source_grounding_1.validateDevelopmentSourceGrounding)({
            requiresCodeChanges: taskRequiresCodeChanges(task),
            targetProjects: planningSource.requestedProjects,
            sourceManifestChecksum: planningSource.checksum,
            projects: planningSource.projects.map((project) => ({
                project: project.project,
                manifestChecksum: project.manifestChecksum,
                manifestFiles: Array.isArray(project.files) ? project.files.length : 0,
                selectedPaths: project.selectedPaths,
                files: (project.files || []).map((file) => ({ path: file.path, checksum: file.checksum, evidenceId: file.evidenceId })),
        ...
        if (taskRequiresCodeChanges(task) && (!planningSource.ready || !sourceGrounding.ready)) {
            const sourceIssues = [...planningSource.issues, ...sourceGrounding.issues];
            const detail = `群聊主 Agent 无法在规划前读取完整的目标项目源码：${sourceIssues.join("；") || "没有可用源码证据"}`;
            updateTask(task.id, {
                status: "blocked",
        ...
        let sharedFilesContext = mergeCoordinatorDocumentContexts(buildCoordinatorSharedFilesContext(ctx, group, { groupSessionId: groupSessionIdForTask(task), message }), buildTaskSourceDocumentsContext(task), planningSource.rendered);
        const runCoordinatorForCurrentSource = () => runGroupOrchestrator({
            group,
            message,
            context,
            source: "task",
        ...
        let coordinatorResult = await runCoordinatorForCurrentSource();
        pauseBoundary("planning");
        ...
        let sourceBoundProjects = new Set(sourceGrounding.groundedProjects);
        let ungroundedAssignments = planAssignments
        ...
        if (taskRequiresCodeChanges(task) && ungroundedAssignments.length) {
            const additionalProjects = Array.from(new Set(ungroundedAssignments));
            appendTaskTimelineEvent(task.id, {
                type: "group_main_source_scope_expanded",
                title: "计划新增项目，正在补充核对源码",
                detail: `仅补充读取实际新增的项目：${additionalProjects.join("、")}`,
                status: "active",
                phase: "planning",
                agent: coordinatorProject,
                data: { additional_projects: additionalProjects },
            });
            planningSource = await buildGroupMainPlanningSourceContext(group, message, configs, {
                targetProjects: Array.from(new Set([
                    ...planningSource.requestedProjects,
                    ...additionalProjects,
                ])),
                maxRounds: 3,
                exactSessionId: groupSessionIdForTask(task),
            ...
            pauseBoundary("planning");
            planningSourceSummary = summarizeGroupPlanningSource(planningSource);
            sourceGrounding = (0, development_source_grounding_1.validateDevelopmentSourceGrounding)({
            ...
            if (!planningSource.ready || !sourceGrounding.ready) {
                const detail = `计划新增项目的源码证据不足：${[...planningSource.issues, ...sourceGrounding.issues].join("；") || additionalProjects.join("、")}`;
                updateTask(task.id, {
                    status: "blocked",
                ...
                appendTaskTimelineEvent(task.id, {
                    ...
                    phase: "planning",
                    agent: coordinatorProject,
                    data: { ungrounded_projects: additionalProjects, source_evidence: planningSourceSummary },
                });
                return { status: "blocked", detail, sourceEvidence: planningSourceSummary };
            }
            sharedFilesContext = mergeCoordinatorDocumentContexts(buildCoordinatorSharedFilesContext(ctx, group, { groupSessionId: groupSessionIdForTask(task), message }), buildTaskSourceDocumentsContext(task), planningSource.rendered);
            coordinatorResult = await runCoordinatorForCurrentSource();
            pauseBoundary("planning");
            coordinatorOutput = coordinatorResult.content;
            coordinatorTranscript.push(coordinatorOutput);
            ...
            sourceBoundProjects = new Set(sourceGrounding.groundedProjects);
            ungroundedAssignments = planAssignments
                .map((assignment) => String(assignment?.project || ""))
                .filter((project) => project && !sourceBoundProjects.has(project));
            if (ungroundedAssignments.length) {
                const detail = `重新规划后仍包含未核对源码的项目：${Array.from(new Set(ungroundedAssignments)).join("、")}；已停止派发。`;
                updateTask(task.id, {
                    status: "blocked",
                    acceptance_state: "source_scope_mismatch",
                    status_detail: detail,
        ...
        if (task.workflow_type === "daily_dev" && validMentions.length === 0 && getRoutableMembers(group).length > 0) {
            ...
            if (repairMentions.length > 0 || repairAssignments.length > 0) {
                ...
                addTaskLog(task.id, "info", `daily_dev 主 Agent 空派发已由模型重新规划: ${validMentions.map(m => m.mention).join(", ") || planAssignments.map((item) => `@${item.project}`).join(", ")}`);
                updateGroupMemory(task.group_id, {
                    groupSessionId: groupSessionIdForTask(task),
                    currentPhase: "dispatching",
        ...
        if (!independentTestAgentEnabled && acceptancePolicy.schema === "ccm-task-acceptance-policy-snapshot-v3" && acceptancePolicy.route === "main_agent_with_escalation" && plannedProjects.length > 1) {
            const escalation = (0, task_acceptance_policy_1.buildTaskAcceptanceEscalationReceipt)({
                ...
                reasons: ["群聊实际派发涉及多个项目"],
                changedFileCount: 0,
                topLevelModuleCount: plannedProjects.length,
            });
            independentTestAgentEnabled = true;
            updateTask(task.id, { acceptance_escalation: escalation, acceptance_actual_level: "strict", acceptance_actual_route: "independent_test_agent", acceptance_escalation_reasons: escalation.reasons });
            appendTaskTimelineEvent(task.id, { type: "acceptance_escalated", title: "跨项目任务已升级严格验收", detail: plannedProjects.join("、"), status: "warn", phase: "planning", agent: coordinatorProject, data: { escalation, project_count: plannedProjects.length, contentStored: false } });
        }
        ...
        if (taskRequiresCodeChanges(task)) {
            pauseBoundary("dispatching");
            const permission = await (0, conversation_permission_policy_1.authorizeProjectChildAgentStart)({ task, project: task.target_project, workDir, agentType });
            pauseBoundary("dispatching");
        ...
        if (communicationDispatch.enabled !== false && communicationDispatch.acquired !== true) {
            ...
            const capacityError = new Error(`Agent Communication并发容量等待：${communicationDispatch.reason || "capacity_limit"}`);
...
请直接完成开发工作。${directMemoryConsumptionChallenge ? "\n开始任何源码修改或命令前，必须先调用内部 MCP `ccm__knowledge_context` 的 `acknowledge_memory_context`，使用当前上下文中提供的 challenge_id、snapshot_id、snapshot_checksum、context_plan_checksum 和 confirmation_cursor；调用成功后再继续执行。" : ""}
完成后必须追加 CCM_AGENT_RECEIPT 结构化回执，格式如下：
\`\`\`json
{
  "ccm_receipt": true,
...
\`\`\``;
        ...
        try {
            if (communicationEnvelope?.messageId && communicationPolicy.strictPreExecutionAckEnabled === true) {
                ...
                const preflightPrompt = [
                    ...
                    directWorkerHandoff?.semantic_contract_binding ? `Semantic binding: ${JSON.stringify({ requirement_checksum: directWorkerHandoff.semantic_contract_binding.requirementChecksum, plan_checksum: directWorkerHandoff.semantic_contract_binding.planChecksum, step_id: directWorkerHandoff.semantic_contract_binding.stepId, step_ids: directWorkerHandoff.semantic_contract_binding.stepIds, acceptance_criterion_ids: directWorkerHandoff.semantic_contract_binding.acceptanceCriterionIds })}` : "",
                    ...
                    "Use exactly these canonical arguments: message_id (string), understood_goal (string), requirement_checksum (string), plan_checksum (string), step_id (string), step_ids (string array), acceptance_criterion_ids (string array), planned_scope (string array), forbidden_scope (string array), verification_plan (string array), unclear (string array). When a semantic binding is present, echo it exactly.",
                    "After the tool reports success, stop immediately.",
                    "If and only if the provider runtime cannot invoke that MCP tool, return exactly one JSON object and no prose: {\"schema\":\"ccm-agent-ack-fallback-v1\",\"message_id\":\"<the signed message id above>\",\"understood_goal\":\"<goal>\",\"requirement_checksum\":\"<exact binding>\",\"plan_checksum\":\"<exact binding>\",\"step_id\":\"<exact primary binding>\",\"step_ids\":[\"<every assigned step ID>\"],\"acceptance_criterion_ids\":[\"<every assigned ID>\"],\"planned_scope\":[\"<allowed path/action>\"],\"forbidden_scope\":[\"<forbidden path/action>\"],\"verification_plan\":[\"<verification>\"],\"unclear\":[]}",
                ].join("\n");
                ...
                if (Number(preflightChanges?.count || 0) > 0) {
                    (0, agent_communication_v2_1.transitionAgentCommunication)(communicationEnvelope.messageId, "recovery_required", {
                ...
                if (!acknowledged || !["acknowledged", "executing"].includes(String(acknowledged.state))) {
                    ...
                    if (providerCanDegrade && preflightTransportHealthy && communicationIdentity) {
                        const recorded = (0, agent_communication_v2_1.recordAgentCommunicationReceipt)(communicationEnvelope.messageId, "dispatch_ack", communicationIdentity, {
                            ...
                            requirement_checksum: directWorkerHandoff?.work_item_contract?.requirementChecksum || "",
                            plan_checksum: directWorkerHandoff?.work_item_contract?.planChecksum || "",
                            step_id: directWorkerHandoff?.work_item_contract?.stepId || "",
                            step_ids: directWorkerHandoff?.semantic_contract_binding?.stepIds || [],
                            acceptance_criterion_ids: directWorkerHandoff?.work_item_contract?.acceptanceCriterionIds || [],
                            planned_scope: [
        ...
        if (requiresProjectReview && acceptancePolicy && !independentTestAgentEnabled) {
            ...
            if (runtimeEscalation.escalate) {
                ...
                appendTaskTimelineEvent(task.id, { type: "acceptance_escalated", title: "验收已升级为严格独立验收", detail: escalation.reasons.join("；"), status: "warn", phase: "reviewing", agent: task.target_project, data: { escalation, contentStored: false } });
            }
        }
        if (requiresProjectReview && !independentTestAgentEnabled) {
            ...
            updateTask(task.id, {
                ...
                status_detail: coordinationDependency ? "群聊主 Agent 正在核对文件变化和项目验证" : acceptancePolicy?.schema === "ccm-task-acceptance-policy-snapshot-v3" && acceptancePolicy.level === "lightweight" ? "项目主 Agent 正在执行轻量自验" : "项目主 Agent 正在执行标准验收",
            });
            appendTaskTimelineEvent(task.id, {
                ...
                detail: coordinationDependency ? "此协作依赖由群聊主 Agent核对" : "主 Agent正在核对范围、文件变化、验收标准和真实验证证据",
                status: "active",
                phase: "reviewing",
            ...
            if (!projectReview.canAccept) {
                ...
                if (runtimeEscalation.escalate && !runtimeEscalation.implementationFailure) {
                    ...
                    independentTestAgentEnabled = true;
                    projectReview = null;
                    updateTask(task.id, { acceptance_escalation: escalation, acceptance_actual_level: "strict", acceptance_actual_route: "independent_test_agent", acceptance_escalation_reasons: escalation.reasons, acceptance_state: "awaiting_test_agent", status_detail: "标准验收证据未闭环，已升级TestAgent独立复核" });
                    appendTaskTimelineEvent(task.id, { type: "acceptance_escalated", title: "标准验收已升级", detail: escalation.reasons.join("；"), status: "warn", phase: "reviewing", agent: task.target_project, data: { escalation, contentStored: false } });
                }
                else {
        ...
        if (requiresProjectReview && independentTestAgentEnabled) {
            ...
            const maxAcceptanceReviewRounds = (0, task_acceptance_policy_1.taskAcceptanceReviewRounds)(loadTasks().find((item) => item.id === task.id) || task, acceptancePolicy);
            while (reviewRound <= maxAcceptanceReviewRounds) {
                updateTask(task.id, {
                    status: "reviewing",
                    ...
                    status_detail: `TestAgent 正在执行第 ${reviewRound}/${maxAcceptanceReviewRounds} 轮独立验收`,
                });
                appendTaskTimelineEvent(task.id, {
                ...
                if (projectReview.canAccept || decision.route !== "test_agent_recheck" || reviewRound >= maxAcceptanceReviewRounds)
                    break;
            ...
            if (!projectReview.canAccept) {
                ...
                if (reviewDecision.route === "implementation_rework" && reviewRound < maxAcceptanceReviewRounds) {
                    updateTask(task.id, {
                        status: "pending",
                ...
                verificationResults: [
                    ...
                    ...(Array.isArray(projectReview?.verification_results)
                        ? projectReview.verification_results.filter((item) => item?.status === "passed").map((item) => ({
                            ...
                            source: independentTestAgentEnabled ? "test_agent" : "main_agent_self_verification",
                            evidence: [item.id],
                        }))
                        : []),
                ],
        ...
        if (ccmRevalidated) {
            if (workDir)
                (0, project_analysis_index_1.scheduleProjectAnalysisIndexRefresh)({ projectId: String(task.target_project || ""), root: workDir, delayMs: 100 });
