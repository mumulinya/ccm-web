...
function sourceEvidenceId(project, relativePath, checksum) {
    return `src_${crypto.createHash("sha256").update(JSON.stringify([project, relativePath, checksum])).digest("hex").slice(0, 24)}`;
}
function buildProjectCodeReadOnlyEvidence(project, workDir, message, options = {}) {
    ...
    try {
        ...
        for (const item of ranked.slice(0, maxFiles)) {
            ...
            const checksum = crypto.createHash("sha256").update(fs.readFileSync(item.file)).digest("hex");
            files.push({
                path: relativePath,
                checksum,
                evidenceId: sourceEvidenceId(project, relativePath, checksum),
...
function buildGroupMainPlanningSourceContext(group, message, configs, options = {}) {
    ...
    // This deterministic builder cannot safely infer business scope. An empty
    // target list must stay unresolved instead of silently hydrating every group
    // member and letting an unrelated project block the task.
    ...
    const issues = [
        ...(!requested.length ? ["尚未解析本次需求实际影响的项目"] : []),
        ...projects
    ...
    const rendered = [
        ...
        `requested_projects=${requested.join(", ") || "<scope unresolved>"}`,
        ...renderedProjects,
    ].join("\n\n");
    return {
...
function groupSourceCacheBinding(group, exactSessionId, decisionKind) {
    const scopeId = String(group?.id || group?.groupId || group?.name || "group").trim();
    const sessionId = String(exactSessionId || `group-source:${scopeId}`).trim();
    return {
        scope: "group",
        scopeId,
        sessionId,
...
async function callSourcePlanningModel(config, payload, providerContextCache) {
    const messages = [
        {
            role: "system",
            content: `You are the read-only source planner for the group main Agent. Based on the user goal, authorized project catalog, project manifests, and source already read, resolve the smallest sufficient set of affected projects and select the next evidence batch. Do not determine scope mechanically by keyword.\nReturn JSON only: {"sufficient":false,"reason":"decision basis","target_projects":["project-id"],"selected_files":[{"project":"project-id","path":"relative/path","reason":"read reason"}],"search_queries":[{"project":"project-id","query":"business-relevant text or symbol to search"}],"plan_steps":[],"impact_scope":[],"clarification_questions":[]}\nRules: target_projects must contain only projects actually affected by the requested change; select only projects and paths present in the manifest; do not include every authorized project as a fallback; search_queries must follow from business semantics; set sufficient=false when project scope or evidence is incomplete; select at most 24 files and 8 queries; never claim to have edited or executed source.`,
        },
        { role: "user", content: JSON.stringify(payload) },
    ];
    return (0, group_orchestrator_llm_client_1.shouldUseAnthropic)(config)
        ? (0, group_orchestrator_llm_client_1.callAnthropicCompatibleJson)(config, { messages, maxTokens: 1400, defaultTimeoutMs: config.timeoutMs || 120_000, httpErrorPrefix: "源码规划模型调用失败", providerContextCache })
        : (0, group_orchestrator_llm_client_1.callOpenAiCompatibleJson)(config, { messages, defaultTimeoutMs: config.timeoutMs || 120_000, httpErrorPrefix: "源码规划模型调用失败", providerContextCache });
}
async function resolveGroupSourceProjects(config, group, message, requested, exactSessionId = "") {
    const allowedMembers = (0, group_orchestrator_1.getRoutableMembers)((0, group_orchestrator_1.normalizeGroupOrchestrator)(group));
    const allowed = new Set(allowedMembers.map((member) => String(member?.project || "").trim()).filter(Boolean));
    if (requested.length)
        return { projects: requested.filter(project => allowed.has(project)), reason: "调用方已绑定目标项目", sufficient: true };
    const messages = [{
            role: "system",
            content: `You resolve the smallest set of authorized CCM projects whose group main Agent must inspect for the user goal. Use only the supplied project catalog. Do not inspect source, invent projects, or select every project as a fallback. If the catalog is insufficient to decide, return an empty target list and sufficient=false.
Return JSON only: {"target_projects":["project-id"],"reason":"routing basis","sufficient":true}`,
        }, {
            role: "user",
            content: JSON.stringify({
                user_goal: message,
                projects: allowedMembers.map((member) => ({
    ...
    const resolved = (0, group_orchestrator_llm_client_1.shouldUseAnthropic)(config)
        ? await (0, group_orchestrator_llm_client_1.callAnthropicCompatibleJson)(config, { messages, maxTokens: 700, defaultTimeoutMs: config.timeoutMs || 120_000, httpErrorPrefix: "项目路由模型调用失败", providerContextCache: groupSourceCacheBinding(group, exactSessionId, "group_source_route") })
        : await (0, group_orchestrator_llm_client_1.callOpenAiCompatibleJson)(config, { messages, maxTokens: 700, defaultTimeoutMs: config.timeoutMs || 120_000, httpErrorPrefix: "项目路由模型调用失败", providerContextCache: groupSourceCacheBinding(group, exactSessionId, "group_source_route") });
    return {
        projects: sourcePlannerList(resolved?.target_projects || resolved?.targetProjects, 8)
            .map((value) => String(value || "").trim())
            .filter((value) => allowed.has(value)),
        reason: String(resolved?.reason || "").slice(0, 1200),
        sufficient: resolved?.sufficient === true,
    };
}
async function buildDirectGroupPlanningSourceContext(group, message, configs, options = {}) {
    ...
    if (!config.enabled || !String(config.apiUrl || "").trim() || !String(config.apiKey || "").trim() || !String(config.model || "").trim()) {
        throw new Error("统一大模型尚未配置，无法形成可靠的群聊源码证据");
    }
    const normalized = (0, group_orchestrator_1.normalizeGroupOrchestrator)(group);
    const requested = Array.from(new Set((options.targetProjects || []).map(value => String(value || "").trim()).filter(Boolean)));
    const route = await resolveGroupSourceProjects(config, normalized, message, requested, options.exactSessionId);
    const targetProjects = Array.from(new Set(route.projects)).slice(0, 8);
    const evidence = buildGroupMainPlanningSourceContext(normalized, message, configs, { targetProjects });
    const sufficient = route.sufficient === true && evidence.ready === true;
    const requestedFiles = evidence.projects.flatMap(project => project.files.map(file => ({
        project: project.project,
        path: file.path,
        reason: "群聊主 Agent直接读取的源码证据",
    })));
    const receiptBase = {
        schema: "ccm-model-driven-source-planning-receipt-v1",
        rounds: targetProjects.length ? 1 : 0,
        sufficient,
        reason: sufficient ? route.reason || "群聊主 Agent已取得最小充分源码证据" : route.reason || "尚未确定可核对源码的目标项目",
        planSteps: [],
        impactScope: targetProjects,
        clarificationQuestions: targetProjects.length ? [] : ["请明确需要核对源码的目标项目"],
        targetProjects,
        requestedFiles,
        searchQueries: [],
        tokenBudget: 0,
    ...
    return {
        ...evidence,
        ready: sufficient,
        issues: [...evidence.issues, ...(!route.sufficient ? [route.reason || "项目范围尚未解析"] : [])],
...
async function buildModelDrivenGroupPlanningSourceContext(group, message, configs, options = {}) {
    // The group main Agent owns this bounded read-only evidence loop for its
    // authorized member projects. Project child Agents remain the only writers.
    const config = (0, group_orchestrator_config_1.loadOrchestratorConfig)();
    if (!config.enabled || !String(config.apiUrl || "").trim() || !String(config.apiKey || "").trim() || !String(config.model || "").trim()) {
    ...
    // When the caller has no explicit scope, expose authorized manifests to the
    // model for semantic scope resolution. File contents are still read only for
    // projects selected by the model below.
    ...
    const searchQueries = [];
    const modelTargetProjects = new Set();
    ...
    for (let round = 1; round <= maxRounds; round += 1) {
        ...
        finalDecision = await callSourcePlanningModel(config, payload, groupSourceCacheBinding(normalized, options.exactSessionId, "group_source_planning"));
        ...
        for (const project of sourcePlannerList(finalDecision?.target_projects || finalDecision?.targetProjects, 8)) {
            const normalizedProject = String(project || "").trim();
            if (allowedNames.has(normalizedProject) && projectNames.includes(normalizedProject))
                modelTargetProjects.add(normalizedProject);
        }
        for (const selection of selections) {
            const selectedProject = String(selection?.project || "").trim();
            if (allowedNames.has(selectedProject) && projectNames.includes(selectedProject))
                modelTargetProjects.add(selectedProject);
            const state = projectState.find(item => item.project === selectedProject);
            ...
            if (!file)
                continue;
            file.evidenceId = sourceEvidenceId(state.project, file.path, file.checksum);
        ...
        for (const queryRow of queries) {
            const selectedProject = String(queryRow?.project || "").trim();
            if (allowedNames.has(selectedProject) && projectNames.includes(selectedProject))
                modelTargetProjects.add(selectedProject);
            const state = projectState.find(item => item.project === selectedProject);
    ...
    const resolvedProjects = requested.length
        ? requested
        : projectNames.filter(project => modelTargetProjects.has(project));
    const resolvedProjectSet = new Set(resolvedProjects);
    const resolvedProjectState = projectState.filter(state => resolvedProjectSet.has(state.project));
    const sufficient = finalDecision?.sufficient === true
        && resolvedProjects.length > 0
        && resolvedProjectState.some(state => state.files.size > 0);
    const projects = resolvedProjectState.map(state => {
    ...
    const issues = [
        ...
        ...(!resolvedProjects.length ? ["主 Agent 尚未确定本次需求实际影响的项目"] : []),
    ...
    const rendered = [
        ...
        `target_projects=${resolvedProjects.join(", ") || "<unresolved>"}`,
