...
const group_typed_memory_distillation_throttle_1 = require("./group-typed-memory-distillation-throttle");
const typed_memory_promotion_1 = require("./typed-memory-promotion");
const typed_memory_model_judgment_1 = require("./typed-memory-model-judgment");
...
function mergeVerifiedGroupPostCompactReinjection(groupId, groupSessionId, stored, rebuilt) {
    ...
    const fileValid = previous.preservedFileDedup
        && (0, group_memory_compaction_1.verifyGroupPostCompactFileRestoreDedupReceipt)(previous.preservedFileDedup, expected).valid === true;
    const skillsValid = previous.invokedSkillAttachmentReceipt
        && (0, group_memory_compaction_1.verifyGroupPostCompactInvokedSkillAttachmentReceipt)(previous.invokedSkillAttachmentReceipt, {
            ...expected,
            attachments: previous.invokedSkillAttachments || [],
        }).valid === true;
    const planValid = previous.planAttachmentReceipt
        && (0, group_memory_compaction_1.verifyGroupPostCompactPlanAttachmentReceipt)(previous.planAttachmentReceipt, {
            ...expected,
            attachment: previous.planAttachment || null,
        }).valid === true;
    const dynamicValid = previous.dynamicContextDeltaReceipt
        && (0, group_memory_compaction_1.verifyGroupPostCompactDynamicContextDeltaReceipt)(previous.dynamicContextDeltaReceipt, {
            ...expected,
    ...
    merged.hasCandidates = !!((merged.files || []).length
...
function prepareGroupMemoryResumeProjection(groupId, groupSessionId, allMessages, storedMemory, options = {}) {
    ...
    if (recoveryRequired) {
        ...
        // Legacy summaries are cleared here and rebuilt only by a later measured
        // request-preflight. Merely opening/resuming a session must not schedule
        // model-context compaction in the background.
    }
    ...
    if (storedPostCompactReinject?.schema === "ccm-post-compact-reinjection-v1") {
        const currentPlan = memory?.compaction?.postCompactReinject
            ...
            || null;
        const mergedPlan = mergeVerifiedGroupPostCompactReinjection(groupId, groupSessionId, storedPostCompactReinject, currentPlan);
        ...
        const storedTaskStatusValid = storedTaskStatusProjection
            && (0, group_memory_compaction_1.verifyGroupPostCompactTaskStatusProjectionReceipt)(storedTaskStatusProjection, {
                groupId,
                groupSessionId,
...
function scheduleGroupMemoryAutoCompaction(groupId, options = {}) {
    ...
    // Read-only compatibility entry point. Message append, idle time and legacy
    // migration must not start model-context compaction in the background. The
    // request-preflight callers invoke runGroupMemoryAutoCompactionNow directly
    // after measuring the final Provider payload.
    return {
        scheduled: false,
        reason: "background_compaction_retired",
        mode: "request_preflight_only",
...
async function runGroupMemoryAutoCompactionNow(groupId, options = {}) {
    ...
    const initialCircuitAdmission = (0, group_memory_auto_compact_circuit_breaker_1.readGroupMemoryAutoCompactCircuitAdmission)(id, sessionId);
    ...
    const requestedCompactionRunId = String(options.compactionRunId || options.compaction_run_id || "").trim();
    ...
    const sharedCompactionRun = (0, session_compaction_runs_1.startSessionCompactionRun)({ scope: "group", exactSessionId: `${id}:${sessionId}`, runId: requestedCompactionRunId, reason: options.reason || "message_append", signal: options.signal }).run;
    const compactionAbortController = sharedCompactionRun.controller;
    const force = options.force === true;
    let lifecycleRunning = false;
    const publishLifecycle = (update) => (0, context_compaction_lifecycle_1.publishContextCompactionLifecycle)({
        scope: "group", scopeId: id, exactSessionId: `${id}:${sessionId}`,
        compactionRunId: sharedCompactionRun.runId,
        trigger: (0, context_compaction_lifecycle_1.resolveContextCompactionTrigger)(options),
        turnId: String(options.turnId || ""), taskId: String(options.taskId || ""),
        generation: Number(options.generation || 0), attempt: Number(options.attempt || 1),
    ...
    try {
        ...
        const config = {
            ...
            onCompactionActivity: ({ stage } = {}) => {
                (0, group_compaction_activity_1.pulseGroupCompactionActivity)({ lifecycleFence: compactionLifecycleFence, operationId: autoCompactAttemptId, stage: stage || "running", leaseMs: activityLeaseMs });
                if (!lifecycleRunning && ["partial_boundary", "session_memory", "pre_compact", "model_summary", "model_summary_request", "ptl_recovery"].includes(String(stage || ""))) {
                    lifecycleRunning = true;
                    publishLifecycle({ state: "running", stage: String(stage || "request_preflight") });
                }
            },
            onCompactionLifecycle: (update = {}) => {
                const state = String(update?.state || "");
                if (state === "running")
                    lifecycleRunning = true;
                publishLifecycle(update);
        ...
        const result = await compactRunner({
            ...
            rebuild,
            partialCompact: options.compactionRequest || options.compaction_request || options.partialCompact || options.partial_compact,
            compactionRequest: options.compactionRequest || options.compaction_request || options.partialCompact || options.partial_compact,
        });
        ...
        const committed = (0, group_session_lifecycle_head_1.withGroupSessionLifecycleCommitFence)(compactionLifecycleFence, ({ validation: commitLifecycleValidation }) => {
            return (0, group_compaction_activity_1.withGroupCompactionActivityCommitFence)({
                ...
                const logDistillation = (0, group_typed_memory_distillation_throttle_1.runTypedMemoryDistillationIfDue)(typedMemoryScopeId, messages, nextMemory, {
                ...
                return { success: true, compacted: !!result.compacted, boundary: boundaryWithPostCompactState, keepIndex: result.keepIndex, background, memory: saved, compactHead, typedMemoryScopeId, logDistillation, providerNativeCompactSessionCapacityReset, postCompactSessionStateReset, promptCacheCompactionNotification, circuitBreaker, lifecycleValidation: commitLifecycleValidation, lifecycleCommitProof, compactionMode: result.compactionMode || "full", partialCompaction: result.partialCompaction || null, hookResults: result.hookResults || [] };
            });
        });
        const committedResult = { ...committed.value, compactionActivity: committed.compactionActivity };
        if (committedResult.compacted === true && lifecycleRunning) {
            publishLifecycle({ state: "completed", stage: "post_compact", afterTokens: Number(committedResult.memory?.compaction?.postCompactTokenCount || 0), mode: committedResult.compactionMode || "full" });
            lifecycleRunning = false;
        }
        ...
        if (lifecycleRunning) {
            const cancelled = error?.code === "GROUP_COMPACTION_CANCELLED" || compactionAbortController.signal.aborted;
            publishLifecycle({ state: cancelled ? "cancelled" : "failed", stage: "error", errorCode: String(error?.code || "GROUP_COMPACTION_FAILED") });
            lifecycleRunning = false;
        }
        if (error?.code === "GROUP_COMPACTION_CANCELLED" || compactionAbortController.signal.aborted) {
        ...
        const failureClassification = (0, group_memory_auto_compact_circuit_policy_1.classifyAutoCompactFailure)(error);
        ...
        (0, session_compaction_runs_1.finishSessionCompactionRun)("group", `${id}:${sessionId}`, sharedCompactionRun.runId);
        ...
        if (compactionWasCancelled)
            ...
            // Do not schedule a detached follow-up compaction. A concurrent Provider
            // request will perform a fresh canonical preflight of its own.
            group_memory_shared_1.groupMemoryAutoCompactPending.delete(scopeKey);
        }
    }
}
function ensureGroupMemoryAutoCompactionHook() {
    ...
    (0, storage_1.registerGroupMessageAppendHook)((groupId, message) => {
        ...
        if (!sessionId.startsWith("gcs_"))
            return;
        // This ledger is execution/handoff evidence only. It does not mutate the
        // Provider projection and no longer schedules context compaction.
    ...
    return { registered: true, already: false, compactionScheduled: false, mode: "request_preflight_only" };
}
...
function buildAgentMemoryContextBundle(groupId, targetProject, task = "", options = {}) {
    ...
    const memoryPromotion = options.enableMemoryPromotion === false || options.enable_memory_promotion === false
        ...
        ? null
        : (0, typed_memory_promotion_1.safeMemoryPromotion)(typedMemoryScopeId, project);
    const promotedMemoryImport = options.includePromotedMemory === false || options.include_promoted_memory === false
        ? null
        : (0, typed_memory_promotion_1.safePromotedMemoryImport)(typedMemoryScopeId, project);
    ...
    const liveDynamicContextCatalog = groupSessionId.startsWith("gcs_")
        ...
        : null;
    if (liveDynamicContextCatalog && project && !liveDynamicContextCatalog.agents.some((item) => String(item?.project || item?.name || "") === project)) {
        const agentType = (0, runtime_1.normalizeAgentRuntimeId)(options.agentType || options.agent_type || "claudecode");
    ...
    const liveDynamicContextDelta = groupSessionId.startsWith("gcs_")
        ? (0, group_memory_compaction_1.buildGroupPostCompactDynamicContextDeltaProjection)(liveDynamicContextCatalog, {
            groupId,
            groupSessionId,
            scanMode: "full",
...
async function buildAgentMemoryContextBundleWithManifestSelection(groupId, targetProject, task = "", options = {}) {
    ...
    const memoryModelConfig = (0, group_memory_shared_1.loadGroupMemoryCompactionConfig)(options.compactionConfig || options.compaction_config || {});
    // 升格判定：本地阈值只做初筛，「是否值得跨会话继承」由模型决定。
    const memoryPromotion = options.enableMemoryPromotion === false || options.enable_memory_promotion === false
        ? null
        : await (0, typed_memory_promotion_1.safeMemoryPromotionWithModel)(scopeId, targetProject, { modelConfig: memoryModelConfig });
    if (memoryPromotion) {
    ...
    const modelRerank = await (0, typed_memory_model_judgment_1.applyModelRecallRerankToBundle)(scopeId, String(typedMemory.recallQuery || task || ""), bundle, memoryModelConfig);
    ...
    if (modelRerank.applied || modelRerank.degraded) {
        bundle.rendered_text = (0, group_memory_shared_1.compactPreserveLines)(renderGroupMemoryContextBundle(bundle), Number(options.maxRenderedChars || 14_000));
    }
    return bundle;
}
...
function renderGroupMemoryContextBundle(bundle) {
    ...
    const appendCriticalMemory = (title, items, mapper, limit) => {
    ...
    appendCriticalMemory("持久用户要求/验收约束", groupState.persistentRequirements, (item) => `#${item.messageId || ""} ${item.text || item}`, 6);
    appendCriticalMemory("关键事实锚点", groupState.factAnchors, (item) => `#${item.messageId || ""} [${item.actor || item.type || ""}] ${item.text || item}`, 5);
