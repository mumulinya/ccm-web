...
exports.isGroupModelRecoveryContinuePhrase = isGroupModelRecoveryContinuePhrase;
exports.findUnrecoveredGroupModelFailure = findUnrecoveredGroupModelFailure;
exports.groupModelRecoveryAnchorId = groupModelRecoveryAnchorId;
exports.resolveGroupModelRecovery = resolveGroupModelRecovery;
exports.runGroupModelRecoverySelfTest = runGroupModelRecoverySelfTest;
...
function isGroupModelRecoveryContinuePhrase(value) {
    return CONTINUE_PHRASE.test(String(value || "").replace(/\s+/g, " ").trim());
}
function findUnrecoveredGroupModelFailure(messages = []) {
    for (let index = messages.length - 1; index >= 0; index -= 1) {
        ...
        if (item?.role === "assistant" && String(item?.content || "").trim() && !RECOVERABLE_RUNTIME.has(runtime))
            break;
    }
    return { failure: null, index: -1 };
}
function groupModelRecoveryAnchorId(failure) {
    return String(failure?.execution_anchor_message_id
...
function resolveGroupModelRecovery(messages = [], input = {}) {
    ...
    if (failure?.role !== "assistant" || !RECOVERABLE_RUNTIME.has(String(failure?.runtime || "").toLowerCase()))
        return null;
    if (CLOSED_RECOVERY_STATES.has(String(failure?.recovery?.state || "").toLowerCase()))
        return null;
...
function runGroupModelRecoverySelfTest() {
    ...
    const recovered = resolveGroupModelRecovery([originalUser, { ...failure, recovery: { ...failure.recovery, state: "recovered" } }], { message: "继续" });
    const checks = {
...
//# sourceMappingURL=group-model-recovery.js.map