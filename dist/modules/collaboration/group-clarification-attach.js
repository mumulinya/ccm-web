...
exports.groupTurnNeedsClarificationCards = groupTurnNeedsClarificationCards;
exports.resolveGroupLiveDispatchPolicy = resolveGroupLiveDispatchPolicy;
exports.runGroupClarificationAttachSelfTest = runGroupClarificationAttachSelfTest;
const clarification_turn_1 = require("../../agents/clarification-turn");
function groupTurnNeedsClarificationCards(input = {}) {
    if (String(input.dispatchPolicy?.action || "").trim() === "ask_user")
        return true;
    const structured = input.workflowDecision?.structuredClarificationQuestions
        || input.workflowDecision?.structured_clarification_questions
        || [];
    if (String(input.mainAgentTurnDecision?.responseKind || "").trim() === "clarify" && Array.isArray(structured) && structured.length)
        return true;
    return (0, clarification_turn_1.parsedRequestsUserClarification)(input.parsed);
}
function resolveGroupLiveDispatchPolicy(input) {
    ...
    if (needsCards) {
        const questions = native?.structuredClarificationQuestions || native?.structured_clarification_questions
            || input.coordinatorResult?.workflowDecision?.structuredClarificationQuestions
            || input.coordinatorResult?.workflowDecision?.structured_clarification_questions
            || input.coordinatorResult?.analysis?.workflowDecision?.structuredClarificationQuestions
            || input.coordinatorResult?.analysis?.workflowDecision?.structured_clarification_questions
            || [];
        return {
...
function runGroupClarificationAttachSelfTest() {
    ...
    const greeting = resolveGroupLiveDispatchPolicy({
        ...
        coordinatorResult: {
            ...
            mainAgentTurnDecision: { responseKind: "clarify" },
        },
    });
...
//# sourceMappingURL=group-clarification-attach.js.map