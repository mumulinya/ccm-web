...
exports.buildFeishuConversationIdentityV2 = buildFeishuConversationIdentityV2;
exports.buildFeishuInboundEnvelopeV2 = buildFeishuInboundEnvelopeV2;
exports.acquireFeishuInboundReceipt = acquireFeishuInboundReceipt;
exports.updateFeishuInboundReceipt = updateFeishuInboundReceipt;
exports.completeFeishuInboundReceipt = completeFeishuInboundReceipt;
exports.failFeishuInboundReceipt = failFeishuInboundReceipt;
exports.getFeishuInboundReceipt = getFeishuInboundReceipt;
exports.buildFeishuQueuedTurnContextV2 = buildFeishuQueuedTurnContextV2;
exports.buildFeishuOriginReceiptV2 = buildFeishuOriginReceiptV2;
const crypto = __importStar(require("crypto"));
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const utils_1 = require("../../core/utils");
const atomic_json_file_1 = require("../../core/atomic-json-file");
const RECEIPT_FILE = process.env.CCM_FEISHU_INBOUND_RECEIPT_FILE || path.join(utils_1.CCM_DIR, "feishu-inbound-receipts-v2.json");
...
function targetType(value) {
    ...
    if (normalized === "global_agent" || normalized === "project_agent")
        return normalized;
...
function parsePlatformKey(value) {
...
function buildFeishuConversationIdentityV2(input) {
    ...
    if (resolvedTarget === "project_agent" && !projectId)
        throw new Error("项目飞书消息缺少精确项目 ID");
...
function buildFeishuInboundEnvelopeV2(input) {
    const payload = input.payload || {};
    ...
    return { schema: "ccm-feishu-inbound-envelope-v2", ...core, checksum: digest(core) };
}
function emptyStore() {
    return { schema: "ccm-feishu-inbound-receipts-v2", generation: 0, updated_at: nowIso(), receipts: [] };
}
function readStore() {
    const raw = (0, atomic_json_file_1.readJsonWithBackup)(RECEIPT_FILE, emptyStore());
    return {
...
function mutateStore(operation) {
    ...
    return (0, atomic_json_file_1.withFileLock)(RECEIPT_FILE, () => {
...
function acquireFeishuInboundReceipt(envelope, leaseMs = DEFAULT_LEASE_MS) {
    return mutateStore(store => {
        ...
        if (existing && (existing.processing_state === "completed" || active)) {
            if (!existing.transports.includes(envelope.transport))
                existing.transports.push(envelope.transport);
        ...
        if (!receipt.transports.includes(envelope.transport))
            receipt.transports.push(envelope.transport);
        if (!existing)
            store.receipts.push(receipt);
        return { acquired: true, duplicate: !!existing, in_progress: false, receipt };
    });
}
function updateFeishuInboundReceipt(receiptId, stage, result = {}) {
    return mutateStore(store => {
        const receipt = store.receipts.find(item => item.id === receiptId);
        if (!receipt)
            throw new Error("飞书入站回执不存在");
...
function completeFeishuInboundReceipt(receiptId, result = {}) {
    return mutateStore(store => {
        const receipt = store.receipts.find(item => item.id === receiptId);
        if (!receipt)
            throw new Error("飞书入站回执不存在");
...
function failFeishuInboundReceipt(receiptId, error, retryable = true) {
    return mutateStore(store => {
        const receipt = store.receipts.find(item => item.id === receiptId);
        if (!receipt)
            throw new Error("飞书入站回执不存在");
...
function getFeishuInboundReceipt(idempotencyKey) {
    return readStore().receipts.find(item => item.idempotency_key === clean(idempotencyKey, 128)) || null;
}
function buildFeishuQueuedTurnContextV2(envelope, payload, destination) {
...
function buildFeishuOriginReceiptV2(input) {
    const core = {
        source: input.envelope.target_type === "project_agent" ? "project_feishu" : "global_feishu",
...
//# sourceMappingURL=feishu-conversation-v2.js.map