"use strict";
// collaboration-acceptance.ts — merged from 3 part files (behavior-freeze merge).
...
function buildIndependentReviewGate(task, actualFileChanges = [], receipts = [], agentQa = []) {
    const { explainIndependentReviewTriggerDecision, formatIndependentReviewGateUserDetail, } = require("./collaboration-runtime-status-helpers");
...
// ===== merged from collaboration-acceptance-part-01-part-02.ts =====
// Extracted functional module. The original entry remains a compatibility facade.
function buildDeliverySummary(task, execution, finalStatus) {
    ...
    const latestTestAgentCompletionGate = [...receiptEvidence]
        .reverse()
        .map((receipt) => receipt.completionGate || receipt.completion_gate)
        .find((gate) => gate?.schema === "ccm-test-agent-completion-gate-v2") || null;
    ...
    const selfVerificationMode = acceptancePolicyResult.valid && acceptancePolicyResult.snapshot
        ? !(0, task_acceptance_policy_1.taskAcceptanceUsesIndependentReview)(task, acceptancePolicyResult.snapshot)
        : String(task?.acceptance_policy_snapshot?.mode || task?.acceptance_mode || "") === "main_agent_self_verification";
