...
async function runCoordinatorReviewLoop(input) {
    ...
    const policyTask = input.taskId ? (0, db_1.loadTasks)().find((item) => item.id === input.taskId) : null;
    const independentTestAgentEnabled = policyTask
        ? (0, task_acceptance_policy_1.taskAcceptanceUsesIndependentReview)(policyTask, input.acceptancePolicy)
        : input.acceptancePolicy?.mode === "test_agent";
    if (!independentTestAgentEnabled) {
        if (input.taskId) {
            (0, collaboration_runtime_runtime_tools_1.updateTask)(input.taskId, {
                ...
                status_detail: input.acceptancePolicy?.level === "lightweight" ? "群聊主 Agent 正在执行轻量自验" : "群聊主 Agent 正在执行标准验收",
            });
            (0, logs_1.appendTaskTimelineEvent)(input.taskId, {
                type: "group_main_self_verification_started",
                title: "群聊主 Agent 开始自验",
                detail: "群聊主 Agent正在核对范围、项目结果和真实验证证据",
                status: "active",
                phase: "reviewing",
        ...
        const escalationDecision = (0, task_acceptance_policy_1.evaluateTaskAcceptanceEscalation)({
            task: currentTask,
            policy: input.acceptancePolicy,
            changedFiles: actualFileChanges,
            selfVerification: selfReceipt,
        });
        if (!selfReceipt.canAccept && escalationDecision.escalate && !escalationDecision.implementationFailure && input.taskId) {
            const escalation = (0, task_acceptance_policy_1.buildTaskAcceptanceEscalationReceipt)({
                task: currentTask,
                policy: input.acceptancePolicy,
                reasons: escalationDecision.reasons,
                changedFileCount: escalationDecision.changedFileCount,
                topLevelModuleCount: escalationDecision.topLevelModuleCount,
            });
            (0, collaboration_runtime_runtime_tools_1.updateTask)(input.taskId, {
            ...
            (0, logs_1.appendTaskTimelineEvent)(input.taskId, { type: "acceptance_escalated", title: "标准验收已升级", detail: escalation.reasons.join("；"), status: "warn", phase: "reviewing", agent: coordinator.project, data: { escalation, contentStored: false } });
            return runCoordinatorReviewLoop(input);
        }
        const review = {
            ...selfReceipt,
...
// 队列处理
function scheduleTaskPauseFinalization(taskId) {
    setTimeout(() => {
        try {
            const tasks = (0, db_1.loadTasks)();
            const task = tasks.find((item) => String(item?.id || "") === String(taskId || ""));
            if (!task || !(0, task_pause_control_1.isTaskPauseRequested)(task))
                return;
            const deps = {
                loadTasks: db_1.loadTasks,
                ...
                runningTaskIds: collaboration_runtime_task_queue_1.runningTaskIds,
                updateTask: collaboration_runtime_runtime_tools_1.updateTask,
                appendTaskTimelineEvent: logs_1.appendTaskTimelineEvent,
            ...
            (0, task_pause_routes_1.finalizeTaskPauseAtSafeBoundary)(task, deps);
            const byId = new Map((0, db_1.loadTasks)().map((item) => [String(item?.id || ""), item]));
            let root = byId.get(String(taskId || "")) || task;
            while (root?.parent_task_id && byId.has(String(root.parent_task_id)))
                root = byId.get(String(root.parent_task_id));
            if (root && String(root.id || "") !== String(taskId || "") && (0, task_pause_control_1.isTaskPauseRequested)(root))
                (0, task_pause_routes_1.reconcileTaskPauseTree)(root, deps);
        }
        catch (error) {
            (0, logs_1.addTaskLog)(taskId, "warning", `暂停现场收口失败，等待重新检查：${String(error?.message || error).slice(0, 300)}`);
        }
    }, 0);
}
    ...
    try {
        while (queue.length > 0) {
            ...
            try {
                ...
                const startedTask = (0, collaboration_runtime_runtime_tools_1.updateTask)(taskId, {
                    status: "in_progress",
                    trace_id: traceId,
                ...
                if (String(error?.code || "") === "CCM_TASK_PAUSE_SAFE_BOUNDARY") {
                    const pausedTask = (0, db_1.loadTasks)().find((item) => item.id === taskId) || task;
                    (0, collaboration_runtime_runtime_tools_1.updateTask)(taskId, {
                        status_detail: "已到达安全暂停点，正在确认所有写入均已停止",
                        acceptance_state: pausedTask.acceptance_state || "executing",
                    });
                    (0, collaboration_runtime_task_queue_1.updateGroupTaskInlineStatus)(pausedTask, "paused", "正在收口并保留代码现场");
                    (0, logs_1.addTaskLog)(taskId, "info", `已在“${String(error?.phase || pausedTask.acceptance_state || "当前")}”阶段到达安全暂停点`);
