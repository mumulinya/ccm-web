...
function confirmBoundPlanningSessionForTask(task, plan) {
    ...
    if (!session || session.phase === "confirmed")
        return;
    // Group authorization is enforced by the coordinator target/member gate. A
    // group task can legitimately contain several projects while target_project
    // only names its current primary worker.
    const allowedProjects = groupId ? [] : [project].filter(Boolean);
    const semanticValidation = (0, group_presented_plan_1.validatePresentedPlanForConfirmation)(plan, allowedProjects);
    if (!semanticValidation.ok) {
        const error = new Error(`计划业务语义合同校验失败：${semanticValidation.issues.join("；")}`);
        error.code = "plan_semantic_contract_invalid";
        throw error;
    }
...
function handleCollaborationApiReplayAndExecutionRoutes(pathname, req, res, parsed, ctx) {
    ...
    if (pathname === "/api/conversations/runtime-status" && req.method === "GET") {
        ...
        const taskConversationIdentities = (task) => {
            const rows = [];
            const add = (scopeName, scopeValue, sessionValue) => {
                const session = String(sessionValue || "").trim();
                const owner = String(scopeValue || "").trim();
                if (!session || (scopeName !== "global" && !owner))
                    return;
                rows.push({ scope: scopeName === "project" || scopeName === "group" ? scopeName : "global", scopeId: scopeName === "global" ? "global" : owner, exactSessionId: session });
            };
            add("project", task?.target_project || task?.project || task?.project_id, task?.project_session_id || task?.projectSessionId);
            add("group", task?.group_id || task?.groupId, task?.group_session_id || task?.groupSessionId);
            add("global", "global", task?.workflow_meta?.global_direct_dispatch?.session_id || task?.origin_session_id || task?.originSessionId);
            const binding = task?.automation_session_binding_snapshot || task?.automationSessionBindingSnapshot || {};
            add(binding.scope || (task?.group_id ? "group" : task?.target_project ? "project" : "global"), binding.scopeId || binding.scope_id || task?.group_id || task?.target_project || "global", binding.exactSessionId || binding.exact_session_id);
            const link = task?.conversation_link || task?.conversationLink || {};
            add(link.scope || "global", link.scopeId || link.scope_id || "global", link.exactSessionId || link.exact_session_id);
            return rows;
        };
        const taskMatchesSession = (task) => taskConversationIdentities(task).some(identity => identity.scope === scope && identity.scopeId === scopeId && identity.exactSessionId === exactSessionId);
        ...
        const projects = [...projectNames].filter(project => (0, access_policy_1.hasResourceAccess)(String(principal?.userId || ""), principal?.role, "project", project, "use")).slice(0, 20).map(project => {
            ...
            try {
                const git = (0, project_git_1.inspectProjectGitQuick)(workDir);
                const risk = !git.git_available || !git.is_repository || git.inspection_complete !== true ? "unavailable" : git.dirty ? "changed" : "normal";
        ...
        const providerCache = {
            ...
            prefixExtensionStatus: String(providerCacheCapability?.prefixExtensionStatus || "unproven"),
            requestClass: String(providerCacheState?.requestClass || "auxiliary"),
            ...
            promptSegments: providerCacheState?.promptSegments || null,
            cacheLifecycle: providerCacheState?.cacheLifecycle || null,
            ...
            protocol: String(providerCacheAdapter?.protocol || "custom"),
            capabilityMatrix: providerCacheAdapter?.capabilityMatrix || null,
            ...
            evidenceUpdatedAt: String(providerCacheCapability?.evidence?.checkedAt || providerCacheCapability?.latestAttempt?.checkedAt || ""),
            contentStored: false,
        };
        res.setHeader("Cache-Control", "private, no-store");
        (0, utils_1.sendJson)(res, { success: true, status: { schema: "ccm-conversation-runtime-status-v1", identity: { scope, scopeId, exactSessionId, contentStored: false }, model, usage, providerCache, task, projects, contentStored: false } });
        return true;
    }
    if (pathname === "/api/tasks/active-runs" && req.method === "GET") {
        ...
        const project = (task) => {
            ...
            if (task?.acceptance_state === "recovery_required" && task?.recovery_preflight?.recoveryMode === "manual_reconciliation" && canStop) {
                availableActions.push({ id: "adopt_current_changes", kind: "adopt_current_changes", label: "采用当前改动并继续", enabled: true }, { id: "recheck", kind: "recheck", label: "查看并重新核验", enabled: true }, { id: "cancel", kind: "cancel", label: "停止任务", enabled: true });
            }
        ...
        req.on("end", async () => {
            try {
                ...
                if (pathname.endsWith("/resume-interrupted")) {
                    ...
                    const catchUp = (0, session_task_timeline_1.catchUpTaskContext)(taskId);
                    if (!catchUp.success)
                ...
                if (pathname.endsWith("/resume-interrupted")) {
                    ...
                    if (!context || !span || suppliedContextRevision === undefined || !suppliedContextChecksum || !suppliedSpanChecksum) {
                        return (0, utils_1.sendJson)(res, { error: "任务恢复标记不完整，请刷新任务回放后重试", code: "TASK_TIMELINE_GUARD_REQUIRED" }, 409);
                    }
                ...
                if (resumedAttemptFailed) {
                    const checkpoint = (0, task_interruption_1.interruptTaskExecution)({
                        task: resumableTask,
                    ...
                    resumableTask = (0, collaboration_1.updateTask)(taskId, {
                    ...
                    task = resumableTask;
                    recoveryWorkspace = (0, task_recovery_orchestrator_1.captureTaskRecoveryWorkspace)(resumableTask);
                }
                ...
                if (reconciliationAction === "adopt_current_changes") {
                    const reconciledReceipt = (0, task_interruption_1.reconcileTaskInterruptionReceipt)(resumableTask, { action: "adopt_current_changes", workspaceChecksum: recoveryWorkspace.checksum, actor: String(payload.actor || "local-user") });
                    resumableTask = (0, collaboration_1.updateTask)(taskId, { interruption_receipt: reconciledReceipt, recovery_preflight: null, status_detail: "已采用当前工作区改动，正在重新执行恢复检查" }) || resumableTask;
                    recoveryWorkspace = (0, task_recovery_orchestrator_1.captureTaskRecoveryWorkspace)(resumableTask);
                }
                const recovery = await (0, task_recovery_orchestrator_1.runTaskRecoveryOrchestrator)(resumableTask, {
        ...
        if (!stillRunning && task?.cancellation_requested_at && String(task?.status || "") !== "cancelled") {
            for (const row of rows) {
                if (taskStopTerminal(row) && String(row?.status || "") !== "in_progress")
                    continue;
                const stoppedWorkspace = (0, task_recovery_orchestrator_1.captureTaskRecoveryWorkspace)(row);
                const stoppedReceipt = (0, task_interruption_1.interruptTaskExecution)({
                    task: row,
