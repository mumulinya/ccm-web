...
exports.validatePresentedPlanForConfirmation = validatePresentedPlanForConfirmation;
...
exports.assignedPresentedPlanSteps = assignedPresentedPlanSteps;
...
function normalizePresentedGroupPlan(input) {
    ...
    const steps = presentedPlanSteps(source).map((step, index) => {
        ...
        return {
            ...
            title,
            description: compactText(step?.objective || step?.description || step?.detail || step?.task, 1800),
            changeSummary: compactText(step?.changeSummary || step?.change_summary || step?.behaviorChange || step?.behavior_change, 1800),
            affectedSymbols: asList(step?.affectedSymbols || step?.affected_symbols).map((location) => ({
                path: compactText(location?.path || location?.file || location?.relativePath || location?.relative_path, 500).replace(/\\/g, "/").replace(/^\.\//, ""),
                ...(compactText(location?.symbol || location?.name, 240) ? { symbol: compactText(location?.symbol || location?.name, 240) } : {}),
                ...(asList(location?.ranges).length ? { ranges: asList(location.ranges).slice(0, 8) } : {}),
                evidenceIds: asList(location?.evidenceIds || location?.evidence_ids || location?.sourceEvidenceIds || location?.source_evidence_ids).map((item) => compactText(item, 160)).filter(Boolean),
            ...
            projects: asList(step?.projects).length ? asList(step.projects) : [step?.project].filter(Boolean),
            files: asList(step?.files || step?.filePaths || step?.file_paths),
            acceptance: asList(step?.acceptance || step?.acceptanceCriteria || step?.acceptance_criteria).length
                ? asList(step?.acceptance || step?.acceptanceCriteria || step?.acceptance_criteria)
                : [step?.outcome || step?.expectedResult || step?.expected_result].filter(Boolean),
            acceptanceCriterionIds: asList(step?.acceptanceCriterionIds || step?.acceptance_criterion_ids),
            dependsOn: asList(step?.dependsOn || step?.depends_on),
            status: "pending",
        };
    }).filter(Boolean);
    ...
    const inferredTargetProjects = [...new Set(steps.flatMap((step) => asList(step?.projects).map((item) => compactText(item, 180))).filter(Boolean))];
    const canonical = (0, implementation_plan_1.normalizeImplementationPlanV2)({
        ...source,
        planId,
        goal,
        overview,
        planningReason: source.planningReason || source.planning_reason || source.triggerReason || source.trigger_reason
            || input.parsed?.workflowDecision?.reason || input.parsed?.workflow_decision?.reason || "",
        steps,
        createdAt,
        updatedAt,
    }, { planId, sourceMessageIds: input.sourceMessageIds, targetProjects: input.targetProjects?.length ? input.targetProjects : inferredTargetProjects });
    if (!canonical)
        return null;
    ...
    result.quality = { v2: (0, implementation_plan_1.validateImplementationPlanV2)(result) };
    return result;
}
function validatePresentedPlanForConfirmation(plan, allowedProjects = []) {
    if (!plan || plan.schema !== "ccm-implementation-plan-v2") {
        return { ok: false, issues: ["确认的计划不是权威 ccm-implementation-plan-v2"] };
    }
    return (0, implementation_plan_1.validateImplementationPlanV2)(plan, { allowedProjects });
}
...
function presentedPlanAcceptanceLines(plan) {
    if (!plan || typeof plan !== "object")
        return [];
    const requirementCriteria = asList(plan?.businessRequirement?.acceptanceCriteria)
        .map((item) => compactText(item?.description, 800))
        .filter(Boolean);
    if (requirementCriteria.length)
...
function assignedPresentedPlanSteps(plan, project) {
    const target = compactText(project, 180);
    const rows = presentedPlanSteps(plan);
    if (!target)
        return rows;
    return rows.filter((step) => {
        const projects = asList(step?.projects).length ? asList(step.projects) : [step?.project].filter(Boolean);
        return projects.map((item) => compactText(item, 180)).includes(target);
    });
}
function appendConfirmedPlanSliceContract(taskText, plan, project) {
    const task = String(taskText || "");
    const assignedSteps = assignedPresentedPlanSteps(plan, project);
    const goal = compactText(plan?.businessRequirement?.businessGoal || plan?.goal || plan?.overview, 1200);
    ...
    const requirementBinding = plan?.requirementBinding
        ? `Requirement binding: revision=${Math.max(1, Number(plan.requirementBinding.revision || 1))}, checksum=${compactText(plan.requirementBinding.checksum, 160) || "missing"}`
        : "";
    const slices = assignedSteps.flatMap((step) => [
        `- [${compactText(step?.id, 100)}] ${compactText(step?.title || step?.label, 240)}：${compactText(step?.objective || step?.description || step?.task, 1000)}`,
        ...asList(step?.acceptance || step?.acceptanceCriteria || step?.acceptance_criteria).map((item) => `  - Acceptance: ${compactText(item, 500)}`),
    ]);
    return [task, "", `${CONFIRMED_SLICE_CONTRACT_MARK}; preserve the business goal and execute only the slices assigned to ${compactText(project, 180) || "this project"}:`, goal ? `Business goal: ${goal}` : "", binding, requirementBinding, ...slices]
        .filter((item) => item !== "")
        .join("\n");
}
function attachConfirmedPlanSlicesToDispatchTargets(targets, plan) {
    ...
    return targets.map((target) => ({
        ...target,
        task: appendConfirmedPlanSliceContract(target?.task, plan, target?.member?.project || target?.project),
        presentedPlan: plan,
    }));
}
function mergePresentedPlanAcceptanceCriteria(existing, plan, limit = 80) {
...
function publishGroupPresentedRequirementPlan(input) {
    const published = input.plan && typeof input.plan === "object" && Array.isArray(input.plan.steps) && input.plan.steps.length
        ...
        : presentedPlanFromParsed({
            ...
            goalFallback: input.goalFallback,
            sourceMessageIds: [input.anchorMessageId || input.turnId || ""].filter(Boolean),
        });
    if (!published)
        return null;
    if (input.skip)
        return null;
    const qualityAttached = (0, presented_plan_quality_1.attachPresentedPlanQuality)(published);
    const publishedWithQuality = qualityAttached.plan;
    ...
    (0, user_visible_agent_events_1.appendUserVisibleRequirementPlan)({
        eventId: `${scope}-turn:${publishedWithQuality.planId}:requirement-plan:${publishedWithQuality.revision}:presented`,
        scope,
        scopeId,
        exactSessionId,
        ...
        plan: publishedWithQuality,
    });
    return publishedWithQuality;
}
