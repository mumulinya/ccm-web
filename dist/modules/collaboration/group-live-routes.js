...
const GROUP_SEND_SSE_STARTED = Symbol("ccm.group-send-sse-started");
function ensureGroupSendSse(res) {
    if (!res.headersSent) {
        res.writeHead(200, {
    ...
    if (!res[GROUP_SEND_SSE_STARTED]) {
        res[GROUP_SEND_SSE_STARTED] = true;
        (0, sse_heartbeat_1.startSseHeartbeat)(res);
    }
}
function finishGroupSendError(res, writeSse, payload, status = 400) {
    if (!res.headersSent) {
        (0, utils_1.sendJson)(res, payload, status);
        return;
    }
    writeSse(res, { type: "error", text: payload?.error || "群聊请求处理失败", code: payload?.code || "" });
    writeSse(res, { type: "done", failed: true });
    try {
        res.end();
    }
    catch { }
...
function updateGroupModelRecoveryMessages(groupId, sessionId, anchorMessageId, state, patch = {}, failureMessageId = "") {
    ...
    if (changed)
        (0, storage_1.saveGroupMessages)(groupId, next, sessionId);
}
...
function buildGroupClarificationSummary(input) {
    ...
    const structuredQuestions = input.dispatchPolicy?.structuredClarificationQuestions
    ...
    const anchorMessageId = String(input.dispatchPolicy?.responseMessageId || input.dispatchPolicy?.response_message_id || crypto.randomUUID());
...
function buildGroupClarificationContinuationMessage(context, answerForAgent) {
    ...
    return [
        ...
        answer,
        "[接续要求]",
        "这是对同一请求的计划前业务澄清。请合并原始请求与补充信息生成详细计划和执行清单；计划必须等待用户确认后才能派发执行成员。不要把这条简短回答当成独立新任务。",
    ].filter(Boolean).join("\n\n");
}
function resolveStoredGroupClarification(groupId, pending, answerMessageId, sessionId = "") {
    ...
    messages[index] = {
        ...current,
        clarification_summary: summary ? { ...summary, status: "resolved", status_label: "已确认", resolved_at: resolvedAt, answer_message_id: answerMessageId, pre_plan_clarification: summary.pre_plan_clarification ? { ...summary.pre_plan_clarification, status: "resolved", revision: Number(summary.pre_plan_clarification.revision || 1) + 1 } : undefined } : summary,
        clarificationSummary: summary ? { ...summary, status: "resolved", status_label: "已确认", resolved_at: resolvedAt, answer_message_id: answerMessageId, prePlanClarification: summary.prePlanClarification ? { ...summary.prePlanClarification, status: "resolved", revision: Number(summary.prePlanClarification.revision || 1) + 1 } : undefined } : summary,
...
async function handleGroupLiveRoutesSendPreface(payload, uploadedFiles, ctx, deps, res, readOnly = false, principal = {}) {
    ...
    if (principal?.kind === "browser" && !(0, access_policy_1.hasResourceAccess)(String(principal.userId || ""), String(principal.role || ""), "group", String(group_id || ""), "use")) {
        (0, utils_1.sendJson)(res, { error: "无权访问该群聊" }, 403);
        return { done: true };
    }
    ...
    const authorizedProjectIds = (0, group_orchestrator_1.getRoutableMembers)(group)
        ...
        .filter(Boolean)
        .filter((project) => (0, access_policy_1.hasResourceAccess)(String(principal.userId || ""), String(principal.role || ""), "project", project, "use"));
    ...
    let explicitContinuationTask = explicitContinuationResolution.task;
    if (explicitContinuationTaskId && !explicitContinuationTask) {
        (0, utils_1.sendJson)(res, { error: explicitContinuationResolution.error, code: "GUIDE_TARGET_STALE" }, explicitContinuationResolution.status || 404);
        return { done: true };
    }
    if (explicitContinuationTask && String(explicitContinuationTask.group_session_id || explicitContinuationTask.groupSessionId || "default") !== groupSessionId) {
        (0, utils_1.sendJson)(res, { error: "这个任务属于另一个群聊会话，请切换到任务所属会话后继续", code: "GUIDE_TARGET_SCOPE_MISMATCH" }, 409);
        return { done: true };
    }
    let explicitContinuationKind = String(payload.continuation_kind || payload.continuationKind || "supplement").trim().toLowerCase();
    ...
    const requestCoordinator = (0, group_orchestrator_1.getCoordinatorMember)(group);
    let prefaceDeltaEmitted = false;
    ensureGroupSendSse(res);
    writeSse(res, {
        type: "response_started",
        scope: "group",
        scope_id: String(group_id),
        exact_session_id: groupSessionId,
        agent: requestCoordinator.project,
        turn_id: requestTurnId,
        executionAnchorMessageId: requestExecutionAnchorMessageId,
        execution_anchor_message_id: requestExecutionAnchorMessageId,
        executionTurnId: requestTurnId,
        execution_turn_id: requestTurnId,
        executionAttempt: requestRecoveryAttempt,
        execution_attempt: requestRecoveryAttempt,
        generation: 0,
    });
    ...
    const semanticRoutingContext = recoverableCandidates.length
        ? `${exactSessionContext}\n\n[当前精确会话的可恢复任务摘要]\n${JSON.stringify(recoverableCandidates.slice(0, 3).map(conversation_message_routing_1.buildRecoverableTaskSummary))}\n请结合当前消息判断 continuationKind；不要仅因为存在旧任务就续接。`
        : exactSessionContext;
    let taskIntent;
    try {
        taskIntent = await classifyGroupProjectTaskIntentWithAgent({
            group,
            ...
            onModelActivity: (activity, event) => writeSse(res, {
                type: "model_activity",
                activity,
                event,
                agent: requestCoordinator.project,
        ...
        let routeTurn = conversationTurnId
            ? conversation_turn_control_1.conversationTurnControl.listInternal({ scope: "group", conversation_id: conversationId, limit: 500 }).turns.find(item => item.id === conversationTurnId)
            : null;
        if (!routeTurn) {
            const created = conversation_turn_control_1.conversationTurnControl.enqueue({
                ...
                mode: "queue",
                message: userMessage,
                request_id: `route:${client_message_id || crypto.randomUUID()}`,
                metadata: { group_id, group_session_id: groupSessionId, original_message_id: client_message_id },
            });
            routeTurn = conversation_turn_control_1.conversationTurnControl.claim({ scope: "group", conversation_id: conversationId, id: created.turn.id, revision: created.turn.revision });
        }
        ...
        writeSse(res, { type: "done", route_required: true, messageId: client_message_id });
        res.end();
        return { done: true };
    }
    if (readOnly && taskIntent?.workflowDecision?.actionRequired === true) {
        finishGroupSendError(res, writeSse, { success: false, error: "当前 Viewer 账户仅允许群聊只读问答；这条需求需要创建或执行任务，请联系 Operator 或 Admin", code: "VIEWER_EXECUTION_FORBIDDEN" }, 403);
        return { done: true };
    }
    if (explicitContinuationTask && taskIntent?.workflowDecision) {
        taskIntent = {
            ...
            workflowDecision: (0, workflow_decision_1.explicitWorkflowDecision)("用户显式继续现有任务", {
    ...
    const resolvedCandidateTaskId = String(payload.resolved_candidate_task_id || payload.resolvedCandidateTaskId || "").trim();
    const explicitRouteCandidate = explicitRouteChoice === "continue_original"
        ? recoverableCandidates.find((item) => String(item.id || "") === resolvedCandidateTaskId)
        : null;
    if (explicitRouteChoice === "continue_original") {
        if (!explicitRouteCandidate) {
            finishGroupSendError(res, writeSse, { error: "原任务已不可恢复，请重新选择处理方式", code: "CONVERSATION_ROUTE_CANDIDATE_STALE" }, 409);
            return { done: true };
        }
        explicitContinuationTask = explicitRouteCandidate;
        explicitContinuationKind = String(taskIntent?.workflowDecision?.continuationKind || "supplement") === "revise_goal" ? "revise_goal" : "supplement";
    }
    else if (explicitRouteChoice === "start_new_task" && taskIntent?.workflowDecision) {
        taskIntent = { ...taskIntent, workflowDecision: { ...taskIntent.workflowDecision, continuationKind: "new_task" } };
    }
    else if (explicitRouteChoice === "answer_only" && taskIntent?.workflowDecision) {
        taskIntent = {
            ...taskIntent,
            kind: "conversation",
            workflowDecision: (0, workflow_decision_1.explicitWorkflowDecision)("用户明确选择仅回答当前问题", {
                ...taskIntent.workflowDecision,
        ...
        if (["resume_task", "revise_task"].includes(routeDecision.decision) && routeDecision.candidate) {
            ...
            if (!routeTurn || routeTurn.status !== "sending") {
                finishGroupSendError(res, writeSse, { error: "消息处理方式已经变化，请刷新后重试", code: "QUEUE_REVISION_CONFLICT" }, 409);
                return { done: true };
            }
            const routed = conversation_turn_control_1.conversationTurnControl.requireRoute({
                id: routeTurn.id,
    ...
    const hasRoutableProjects = (0, group_orchestrator_1.getRoutableMembers)(group).length > 0;
    ...
    if (continuationTask) {
        ...
        if (!continuation.success) {
            finishGroupSendError(res, writeSse, { error: continuation.error, new_task_suggested: continuation.new_task_suggested }, continuation.status || 400);
            return { done: true };
        }
    ...
    return {
        ...
        authorizedProjectIds, prefaceDeltaEmitted,
    };
}
...
function handleGroupLiveRoutes(req, res, parsed, ctx, deps) {
    ...
    if (pathname === "/api/groups/send" && req.method === "POST") {
        ...
        const handleGroupSend = async (payload, uploadedFiles = []) => {
            ...
            let requestTurnId = "";
            let prefaceDeltaEmitted = false;
            let authorizedProjectIds = [];
            try {
                const preface = await handleGroupLiveRoutesSendPreface(payload, uploadedFiles, ctx, deps, res, (0, api_access_control_1.requestIsReadOnly)(req), req.ccmAuth || {});
                if (preface.done)
                    return;
                ({
                    payload,
                ...
                if (principal?.kind === "browser" && principal.role !== "admin" && !(0, access_policy_1.hasResourceAccess)(principal.userId, principal.role, "group", String(group_id || ""), persistentTaskRequest ? "use" : "use")) {
                ...
                if (persistentTaskRequest) {
                    ...
                    const sessionPlanMode = (0, conversation_plan_mode_gate_1.isConversationPlanModeEnabled)("group", String(group_id), groupSessionId);
                    // Group preflight always builds an internal execution contract for
                    // routing and safety.  Only expose it as a plan card when the user
                    // selected session Plan Mode or the task genuinely needs a user
                    // confirmation; ordinary Agent mode remains visually plan-free.
                    planModePreflight.session_plan_mode = sessionPlanMode;
                    planModePreflight.display_policy = {
                        ...(planModePreflight.display_policy || {}),
                        user_visible: sessionPlanMode || planModePreflight.requires_confirmation === true,
                        show_todo: sessionPlanMode || planModePreflight.requires_confirmation === true,
                        technical_details_default_collapsed: true,
                        hide_internal_protocols: true,
                    };
                    if (clarificationContext) {
                        ...
                        planModePreflight.clarification_resolved = true;
                    }
                    ...
                    // Clarification and Epic expansion above can raise the confirmation
                    // floor after the initial preflight. Recompute visibility from the
                    // final authoritative flags so a required confirmation is never
                    // hidden by the ordinary Agent-mode policy.
                    planModePreflight.display_policy = {
                        ...(planModePreflight.display_policy || {}),
                        user_visible: sessionPlanMode || planModePreflight.requires_confirmation === true,
                        show_todo: sessionPlanMode || planModePreflight.requires_confirmation === true,
                    ...
                    const intakeTimelineEvent = appendTaskTimelineEvent(task.id, {
                        ...
                        title: planModePreflight.requires_confirmation
                            ? "执行前计划已生成"
                            : "执行前检查已完成",
                    ...
                    const intakeProgressCheckpoint = {
                        ...
                        label: planModePreflight.requires_confirmation
                            ? "执行前计划已生成"
                            : "执行前检查已完成",
                    ...
                    if (intakePlanSteps.length && (sessionPlanMode || planModePreflight.requires_confirmation === true)) {
                    ...
                    updateGroupMemory(group_id, {
                        ...
                        reason: task.title,
                        nextAction: planModePreflight.requires_confirmation
                            ? "等待用户确认执行前计划"
                            : sessionPlanMode
                                ? "我会读取需求和项目上下文，生成执行计划并安排执行成员"
                                : "我会安排执行成员并持续回收结果和验收证据",
                    });
                ...
                if (isBroadcast && isOrchestrated) {
                    ...
                    ctx.setAgentActivity(coordinator.project, "working", readOnlyContextRequest ? "群聊主 Agent正在核对相关项目源码" : conversationalOnly ? "正在回复" : "正在协调群聊", { tab: "groups", groupId: group_id });
                    ctx.broadcastPetSpeech(coordinator.project, { role: "status", text: readOnlyContextRequest ? "群聊主 Agent正在核对相关项目源码..." : conversationalOnly ? "正在回复..." : "正在协调群聊...", source: "group" });
                    updateGroupMemory(group_id, {
                        groupSessionId,
                    ...
                    const groupHasRoutableProjects = (0, group_orchestrator_1.getRoutableMembers)(group).length > 0;
                    ...
                    // Source is selected and read inside the group main Agent tool loop;
                    // never pre-read every member project or inject source bodies here.
                    const readOnlyProjectContext = "";
                    ...
                    let coordinatorDeltaEmitted = prefaceDeltaEmitted;
                    const unsubscribeCoordinatorEvents = (0, user_visible_agent_events_1.subscribeUserVisibleAgentEvents)((event) => {
                        if (res.destroyed || res.writableEnded)
                            return;
                        if (event?.scope !== "group" || String(event?.scopeId || "") !== String(group_id) || event?.exactSessionId !== groupSessionId)
                            return;
                        if (event?.eventType !== "assistant_progress" && !String(event?.eventType || "").startsWith("tool_"))
                            return;
                        const belongsToCurrentTurn = String(event?.anchorMessageId || "") === executionAnchorMessageId
                            || String(event?.turnId || "") === visibleTurnId;
                        if (!belongsToCurrentTurn)
                            return;
                        writeSse(res, { type: "agent_execution", event, agent: coordinator.project });
                    });
                    let coordinatorResult;
                    try {
                        coordinatorResult = await (0, group_orchestrator_1.runGroupOrchestrator)({
                            group,
                            message: readOnlyContextRequest
                                ? `${messageForAgent}\n\n请使用当前群聊授权的只读工作区工具直接核对实际相关项目源码，再依据真实证据回答；不要派发开发任务、创建任务或承诺修改。多项目调用必须传入精确 project_id。`
                                : messageForAgent,
                            ...
                            onModelActivity: (activity, event) => writeSse(res, { type: "model_activity", activity, event, agent: coordinator.project }),
                            onAgentExecutionEvent: (event) => writeSse(res, { type: "agent_execution", event, agent: coordinator.project }),
                        });
                    }
                    finally {
                    ...
                    try {
                        ...
                        if (!coordinatorDeltaEmitted && String(outputText || "").trim()) {
                            writeSse(res, { type: "response_delta", agent: coordinator.project, text: String(outputText) });
                        }
                        const coordinatorRuntime = String(coordinatorResult.runtime || "");
                        const providerFailure = coordinatorResult.providerFailure || null;
                        const providerFailureTechnical = coordinatorResult.providerFailureTechnical || null;
                        ...
                        const clarificationSummary = dispatchPolicy?.action === "ask_user"
                            ...
                            ? buildGroupClarificationSummary({
                                ...
                                dispatchPolicy: { ...dispatchPolicy, responseMessageId },
                                analysis: coordinatorResult.analysis || null,
                                coordinator: coordinator.project,
                                round: clarificationContext ? Math.min(2, Number(clarificationContext?.pre_plan_clarification?.round || 1) + 1) : 1,
                            })
                            : null;
                        const clarificationContextRecord = clarificationSummary ? {
                            ...
                            revision: clarificationSummary.pre_plan_clarification?.revision || 1,
                            generation: clarificationSummary.pre_plan_clarification?.generation || 0,
                            original_request_checksum: clarificationSummary.pre_plan_clarification?.originalRequestChecksum || "",
                        ...
                        const failure = (0, model_retry_presentation_1.modelProviderFailurePresentation)(err);
                        writeSse(res, {
                            type: "error",
                            text: failure.presentable ? failure.text : err.message,
                            provider_retry_count: failure.retryCount,
                            max_retries: failure.maxRetries,
                            provider_request_evidence: failure,
                            messageId: responseMessageId,
                            responseMessageId,
                            response_message_id: responseMessageId,
                            executionAnchorMessageId,
                ...
                if (res.headersSent) {
                    const failure = (0, model_retry_presentation_1.modelProviderFailurePresentation)(e);
                    writeSse(res, { type: "error", text: failure.presentable ? failure.text : e.message || "群聊请求处理失败", provider_retry_count: failure.retryCount, max_retries: failure.maxRetries, provider_request_evidence: failure });
                    try {
                        res.end();
                    }
                    catch { }
