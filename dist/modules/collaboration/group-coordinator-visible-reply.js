...
exports.shouldSynthesizeToolBackedCoordinatorReply = shouldSynthesizeToolBackedCoordinatorReply;
...
/** Kept as a compatibility export; paid post-loop synthesis is disabled. */
function shouldSynthesizeToolBackedCoordinatorReply(parsed, observationCount = 0) {
    void parsed;
...
function runGroupCoordinatorVisibleReplySelfTest() {
    ...
    const checks = {
        emptyReplyDoesNotTriggerPaidSynthesis: shouldSynthesizeCoordinatorVisibleReply(emptyParsed) === false,
        toolBackedReplyUsesNativeFinalText: shouldSynthesizeToolBackedCoordinatorReply({ responseType: "reply", reply: "工具结论草稿" }, 2) === false,
        toolFreeReplyDoesNotNeedExtraSynthesis: shouldSynthesizeToolBackedCoordinatorReply({ responseType: "reply", reply: "直接回答" }, 0) === false,
        toolBackedPlanDoesNotNeedReplySynthesis: shouldSynthesizeToolBackedCoordinatorReply({ responseType: "plan", reply: "计划如下", plan: { steps: [{}] } }, 2) === false,
