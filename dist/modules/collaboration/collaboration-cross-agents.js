...
async function executeMentionJob(mention, env) {
    ...
    const testAgentHandoffPayload = getTestAgentHandoffPayload(testAgentHandoff);
    ...
    const buildCurrentWorkerHandoff = (memoryContext) => buildChildAgentWorkerHandoff(targetName, childTaskText, {
        ...
        task: sourceTask,
        plan: typeof mention === "string" ? null : mention.presentedPlan || null,
        group,
    ...
    const testAgentHandoffPacket = testAgentHandoffPayload ? [
        "[TestAgent 原生独立复核交接单]",
        "- 这是主 Agent 为独立验证 Agent 生成的当前 TestAgent 交接单。",
