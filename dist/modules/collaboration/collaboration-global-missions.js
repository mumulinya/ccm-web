...
function getGlobalDirectDispatchContinuationKey(task) {
    ...
    const kind = (0, collaboration_1.normalizeContinuationKind)(last?.kind || last?.rework_kind || "");
    ...
    if (!replanRequired && !last?.at && !interruption.requested_at)
        return "";
    const reason = (0, memory_1.compactMemoryText)(last?.reason || interruption.reason || task?.status_detail || "", 220);
    return [
        task?.id || "",
        kind || "continuation",
...
function shouldNotifyGlobalDirectDispatchContinuation(task, previousStatus = "") {
    ...
    if (!meta)
        return false;
    if (!String(meta.session_id || "").trim())
        return false;
    if (["done", "cancelled", "failed"].includes(String(task?.status || "").toLowerCase()))
        return false;
    if (["done", "cancelled"].includes(String(previousStatus || "").toLowerCase()))
        return false;
    const key = getGlobalDirectDispatchContinuationKey(task);
    if (!key)
        return false;
    if (String(meta.continuation_notified_key || "") === key)
        return false;
    return true;
}
function buildGlobalDirectDispatchContinuationMessage(task) {
    ...
    const interruption = state.goal_revision_interruption || {};
    const kind = (0, collaboration_1.normalizeContinuationKind)(last?.kind || last?.rework_kind || "");
    const reason = (0, memory_1.compactMemoryText)(last?.reason || interruption.reason || task?.status_detail || "用户补充了新的要求，我会重新核对计划。", 360);
...
function shouldNotifyGlobalDirectDispatchCompletion(task, previousStatus = "") {
    ...
    if (!meta)
        return false;
    if (meta.completion_notified_at)
        return false;
    if (!String(meta.session_id || "").trim())
        return false;
    ...
    if (String(task?.status || "") !== "done")
        return false;
    if (!(0, collaboration_1.hasStrongTaskAcceptanceEvidence)(task, [], task?.delivery_summary || {}))
        return false;
    return true;
}
function buildGlobalDirectDispatchCompletionMessage(task) {
    const report = (0, task_delivery_report_1.buildTaskDeliveryReport)(task, task?.delivery_summary || {}, "done", task?.status_detail || "任务已完成");
    return [
...
function shouldNotifyGlobalDirectDispatchRollback(task, previousStatus = "") {
    ...
    if (!meta)
        return false;
    if (meta.rollback_notified_at)
        return false;
    if (!String(meta.session_id || "").trim())
        return false;
    if (String(task?.status || "") !== "cancelled")
        return false;
    if (!task?.rolled_back_at && task?.delivery_summary?.reverted !== true)
        return false;
    if (previousStatus === "cancelled" && task?.delivery_summary?.reverted !== true)
        return false;
    return true;
}
function buildGlobalDirectDispatchRollbackMessage(task) {
    ...
    const reason = (0, memory_1.compactMemoryText)(task?.rollback_reason || summary.rollback_reason || "用户发起安全撤销", 280);
    return [
...
function superviseGlobalDevelopmentMissionCycle(id, ctx, options = {}) {
    ...
    const staleMs = Math.max(30_000, Number(options.stale_ms || options.staleMs || collaboration_1.TASK_WATCHDOG_STALE_MS));
    const autoMerge = options.auto_merge !== false && options.autoMerge !== false;
    const actions = [];
    const waitingUser = [];
    const childByRef = (ref) => initial.children.find((item) => (0, collaboration_1.missionChildMatchesRef)(item, ref));
    const dependencyGraph = new Map(initial.children.map((child) => [child.id, (0, collaboration_1.getMissionDependencyRefs)(child).map(ref => childByRef(ref)?.id).filter(Boolean)]));
    ...
    for (const child of initial.children) {
        ...
        const dependencyRefs = (0, collaboration_1.getMissionDependencyRefs)(child);
        ...
        const blockingDependencies = dependencies.filter(item => item.task && !(0, collaboration_1.globalMissionChildGatePassed)(item.task));
        ...
        if (child.status === "done") {
            const evidence = (0, collaboration_1.getGlobalMissionChildDeliveryEvidence)(child);
                ...
                for (const executionId of evidence.merge_pending_execution_ids) {
                    try {
                        ...
                        if (attempts >= maxAttempts) {
                            ...
                            const result = (0, collaboration_1.continueTaskWithMessage)(child.id, message, ctx, {
            ...
            else if (!(0, collaboration_1.globalMissionChildGatePassed)(child)) {
                if (attempts >= maxAttempts) {
                    waitingUser.push({ task_id: child.id, reason: "交付验收未通过且已达到自动返工上限" });
                }
                else {
                    const result = (0, collaboration_1.continueTaskWithMessage)(child.id, (0, collaboration_1.buildTaskGapContinuationDraft)(child), ctx, {
                        source: "mission_supervisor_gap_rework",
                        auto_execute: true,
        ...
        if (child.status === "failed") {
            if (attempts >= maxAttempts) {
                ...
                const runtimeFailure = (0, collaboration_1.isRecoverableRuntimeFailure)(child);
                const result = runtimeFailure
                    ? (0, collaboration_1.retryTask)(child.id, ctx, `全局任务 ${id} 持续跟进检测到执行器异常，恢复原生会话或切换执行器后重试`, true)
                    : (0, collaboration_1.continueTaskWithMessage)(child.id, `全局 Agent 检测到本轮失败：${child.status_detail || child.result || "未知失败"}。请先调查根因，只针对失败缺口返工，保留已完成成果，重新运行相关验证并提交结构化回执。`, ctx, {
        ...
        if (child.status === "in_progress" && !collaboration_1.runningTaskIds.has(child.id) && (0, collaboration_1.getTaskAgeMs)(child) >= staleMs) {
            if (attempts >= maxAttempts) {
                ...
                const result = (0, collaboration_1.retryTask)(child.id, ctx, `全局任务 ${id} 持续跟进检测到执行中断或超时`, true);
    ...
    if (actions.length > 0 || terminal || waitingUser.length > 0) {
        ...
        (0, collaboration_1.appendGlobalMissionSupervisorTimeline)(current.mission, actions, waitingUser, terminal);
    }
    return {
        success: true,
...
async function controlGlobalDevelopmentMission(id, operation, ctx, payload = {}) {
    ...
    if (op === "update_goal") {
        const previousGoal = (0, collaboration_1.compactFormText)(current.mission.business_goal || current.mission.description || current.mission.title, "");
        const requestedGoal = (0, collaboration_1.compactFormText)(payload.business_goal || payload.businessGoal || payload.goal, "");
        const acceptance = (0, collaboration_1.compactFormText)(payload.acceptance || payload.acceptance_criteria || payload.acceptanceCriteria, current.mission.acceptance_criteria || "");
        const continuation = payload.continuation && typeof payload.continuation === "object" ? payload.continuation : null;
        const source = (0, collaboration_1.compactFormText)(continuation?.source || payload.source || "mission_supervisor_goal_update", "mission_supervisor_goal_update");
        const continuationMessage = (0, collaboration_1.compactFormText)(payload.message
            || payload.followup
            || continuation?.message
            || continuation?.reason
            || continuation?.detail
            || payload.reason, "");
        const reworkKind = (0, collaboration_1.compactFormText)(continuation?.rework_kind || continuation?.reworkKind || "", "");
        ...
        const inferredKind = requestedKind && requestedKind !== "auto"
            ? (0, collaboration_1.normalizeContinuationKind)(requestedKind)
            : isNextWorkItem
                ? "supplement"
                : (0, collaboration_1.classifyTaskContinuation)(continuationMessage || (requestedGoal ? `目标调整为：${requestedGoal}` : "补充当前全局任务"));
        if (inferredKind === "new_task") {
            return { success: false, status: 409, error: "这条要求看起来是独立新任务，请新建全局任务，不会混入当前持续跟进。" };
        }
        const continuationKind = (0, collaboration_1.normalizeContinuationKind)(inferredKind);
        ...
        for (const child of current.children) {
            if (["cancelled"].includes(child.status))
                continue;
            const result = (0, collaboration_1.continueTaskWithMessage)(child.id, followup, ctx, {
                source,
                continuationKind,
            ...
            if (result.success) {
                (0, collaboration_1.updateTask)(child.id, {
                    business_goal: goal,
        ...
        const continuationState = {
            ...
            last_continuation: {
                ...
                rework_kind: reworkKind,
                target: (0, collaboration_1.compactFormText)(continuation?.target || continuation?.agent || continuation?.project || "", ""),
                reason: continuationMessage,
                title: (0, collaboration_1.compactFormText)(continuation?.title || continuation?.label || "", ""),
                work_item_id: (0, collaboration_1.compactFormText)(continuation?.work_item_id || continuation?.workItemId || "", ""),
        ...
        (0, logs_1.appendTaskTimelineEvent)(id, {
            ...
            phase: "rework",
            agent: (0, collaboration_1.compactFormText)(continuation?.target || "", ""),
            data: {
                source,
                kind: continuationKind,
                work_item_id: continuation?.work_item_id || continuation?.workItemId || "",
        ...
        for (const child of current.children) {
            if (["done", "cancelled"].includes(child.status))
                continue;
            (0, collaboration_1.removeTaskFromQueues)(child.id);
        ...
        for (const child of current.children) {
            if (["done", "cancelled"].includes(child.status))
                continue;
            (0, collaboration_1.updateTask)(child.id, { is_paused: paused, paused, status_detail: paused ? (op === "takeover" ? "已转为人工接管" : "我已暂停后续调度") : "我已恢复后续调度" });
            if (paused)
                (0, collaboration_1.removeTaskFromQueues)(child.id);
        }
        (0, collaboration_1.updateTask)(id, {
...
function buildGlobalMissionTargetHandoff(input) {
    ...
    const handoff = (0, worker_handoff_1.buildSelfContainedWorkerHandoff)({
        ...
        analysis: {
            ...
            documentFindings: (0, collaboration_1.splitUserAcceptanceText)(input.sourceDocuments || ""),
            constraints: [
                `这是全局任务 ${input.parent?.id || ""} 的子任务；必须向全局 Agent 提供可审计交付证据。`,
                "最终完成不能只靠口头说明，必须有实际产出、验证和主 Agent 复盘证据。",
            ],
        },
        dependencies: dependencyRefs.map((ref) => ({ project: ref, reason: "全局任务前置依赖" })),
        verificationHints: [],
        acceptance: (0, collaboration_1.splitUserAcceptanceText)(input.acceptance),
...
function createGlobalDevelopmentMission(payload, ctx) {
    ...
    if (requestedTargets.length === 0 && fallbackTarget.length === 0) {
        ...
        const groupNames = groups.slice(0, 8).map((item) => item.name || item.id).join("、");
    ...
    for (const target of targets) {
        if (target.type === "group") {
            ...
            let readiness;
            try {
                readiness = (0, collaboration_1.validateDailyDevGroupReady)(group);
            }
            catch (error) {
            ...
            resolved.push({
                ...
                ownership_chain: (0, collaboration_1.buildGlobalGroupTestAgentOwnership)(),
            });
            continue;
        }
    ...
    const title = (0, collaboration_1.compactFormText)(payload.title, (0, collaboration_1.compactFormText)(payload.business_goal || payload.businessGoal, "全局开发任务").slice(0, 80));
    const businessGoal = (0, collaboration_1.compactFormText)(payload.business_goal || payload.businessGoal || payload.goal || title, title);
    const acceptance = (0, collaboration_1.compactFormText)(payload.acceptance || payload.acceptance_criteria || payload.acceptanceCriteria, "所有子任务必须完成实际代码变更和已执行验证；群聊主 Agent与项目 Agent交付验收全部通过后，全局 Agent才能报告完成。");
    const sourceDocuments = (0, collaboration_1.compactFormText)(payload.documents || payload.source_documents || payload.sourceDocuments, "");
    ...
    const parent = (0, collaboration_1.createTask)({
        ...
        requires_independent_review: resolved.some((item) => item.requires_independent_review === true),
        mission_plan: {
    ...
    const children = [];
    const targetHandoffs = [];
    for (const target of resolved) {
        const childGoal = (0, collaboration_1.compactFormText)(target.task, businessGoal);
        const childTitle = (0, memory_1.compactMemoryText)(`${title} - ${target.name}`, 100);
        const targetGroup = target.type === "group" ? groups.find((item) => item.id === target.group_id) : null;
        ...
        const targetConversation = {
            scope: target.type,
            scopeId: target.type === "group" ? String(child.group_id || "") : String(child.target_project || ""),
            exactSessionId: targetSessionId,
            sessionKind: "automation",
            resolution: child.automation_session_binding_snapshot?.resolution === "explicit_binding" ? "explicit_binding" : "auto_created",
            taskId: child.id,
            sourceSessionId,
            contentStored: false,
        };
        ...
        const persistedChild = (0, collaboration_1.updateTask)(child.id, {
        ...
        if (persistedChild)
            Object.assign(child, persistedChild);
        if (target.type === "project" && child.project_session_id) {
            (0, sessions_1.appendProjectSessionTaskMessage)(target.project, child.project_session_id, {
            ...
            (0, sessions_1.appendProjectSessionTaskMessage)(target.project, child.project_session_id, {
                ...
                content: [`来源：全局 Agent`, ``, `业务目标：${childGoal}`, `本项目负责：${target.reason || "核对当前项目源码并完成本项目工作项"}`, `验收标准：${acceptance}`, ``, `已接收任务“${child.title}”。项目主 Agent 将先形成用户可读计划，再按当前会话队列执行，并在 TestAgent 验收后提交结果。`].join("\n"),
                timestamp: child.created_at,
                task_id: child.id,
                ...
                taskExperience: {
                    ...
                    source_conversation_ref: sourceConversationRef,
                    conversation_links: (0, task_conversation_links_1.buildTaskConversationLinks)(child)?.links || [],
        ...
        if (target.type === "group" && child.group_id && child.group_session_id) {
            (0, storage_1.appendGroupMessage)(child.group_id, {
                id: `global-task-intake-${child.id}`,
                group_session_id: child.group_session_id,
                role: "user",
                content: childGoal,
            ...
            (0, storage_1.appendGroupMessage)(child.group_id, {
                ...
                content: [`来源：全局 Agent`, ``, `业务目标：${childGoal}`, `群聊负责：${target.reason || "协调授权成员项目完成相关工作项"}`, `验收标准：${acceptance}`, ``, `已接收任务“${child.title}”。群聊主 Agent 将先制定用户可读计划，再安排项目 Agent 执行和 TestAgent 验收。`].join("\n"),
                timestamp: child.created_at,
                task_id: child.id,
                type: "global_task_dispatch_queued",
                source_conversation_ref: sourceConversationRef,
                task_card: {
                    ...(0, collaboration_task_card_1.buildTaskCardView)(child, [], []),
                    visible: true,
                    presentation: "plan",
                    phase: "queued",
                    phase_label: "准备制定计划",
        ...
        (0, runtime_kernel_1.recordAgentRuntimeLifecycle)({
            ...
            risk: "agent",
            target: (0, collaboration_1.targetProjectForMissionTarget)(target),
            status: "planned",
    ...
    const updatedParent = (0, collaboration_1.updateTask)(parent.id, {
        status: "in_progress",
...
//# sourceMappingURL=collaboration-global-missions.js.map