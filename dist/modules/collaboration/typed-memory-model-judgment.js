...
exports.MEMORY_MODEL_JUDGMENT_CACHE_MAX_ENTRIES = exports.MEMORY_MODEL_JUDGMENT_MAX_ITEMS = exports.MEMORY_MODEL_JUDGMENT_MAX_OUTPUT_TOKENS = exports.MEMORY_MODEL_JUDGMENT_CACHE_DIR = exports.MEMORY_MODEL_JUDGMENT_VERSION = void 0;
exports.isMemoryModelJudgmentEnabled = isMemoryModelJudgmentEnabled;
exports.readJudgmentCache = readJudgmentCache;
exports.validateAdmissionJudgment = validateAdmissionJudgment;
exports.validateRerankJudgment = validateRerankJudgment;
exports.judgeMemoryAdmissionWithModel = judgeMemoryAdmissionWithModel;
exports.rerankMemoryRecallWithModel = rerankMemoryRecallWithModel;
exports.applyModelRecallRerankToBundle = applyModelRecallRerankToBundle;
exports.modelRerankScoreAdjustment = modelRerankScoreAdjustment;
const path = __importStar(require("path"));
const utils_1 = require("../../core/utils");
const atomic_json_file_1 = require("../../core/atomic-json-file");
exports.MEMORY_MODEL_JUDGMENT_VERSION = 1;
exports.MEMORY_MODEL_JUDGMENT_CACHE_DIR = path.join(utils_1.CCM_DIR, "typed-memory-model-judgments");
exports.MEMORY_MODEL_JUDGMENT_MAX_OUTPUT_TOKENS = 4_000;
/** 每次最多送几条给模型判定，控制 token 成本。 */
exports.MEMORY_MODEL_JUDGMENT_MAX_ITEMS = Math.max(1, Number(process.env.CCM_MEMORY_MODEL_JUDGMENT_MAX_ITEMS || 12));
exports.MEMORY_MODEL_JUDGMENT_CACHE_MAX_ENTRIES = 500;
...
function isMemoryModelJudgmentEnabled(config = {}) {
    const explicit = config?.memoryModelJudgmentEnabled ?? config?.memory_model_judgment_enabled;
    if (explicit === true)
        return true;
    if (explicit === false)
        return false;
    if (typeof (config?.modelCall || config?.compactionModelCall) === "function")
        return true;
    return !!(config?.enabled && config?.apiUrl && config?.apiKey && config?.model);
}
// ---------------------------------------------------------------- 幂等缓存
function cacheFile(kind, scopeId) {
    return path.join(exports.MEMORY_MODEL_JUDGMENT_CACHE_DIR, safeSegment(scopeId), `${safeSegment(kind)}.json`);
}
function readJudgmentCache(kind, scopeId) {
...
function writeJudgmentCache(kind, scopeId, entries) {
    const file = cacheFile(kind, scopeId);
    const keys = Object.keys(entries);
    // 超量时丢最旧的，缓存只是省钱，不是事实源。
    const bounded = keys.length <= exports.MEMORY_MODEL_JUDGMENT_CACHE_MAX_ENTRIES
        ? entries
        : Object.fromEntries(keys
            .map(key => [key, entries[key]])
            .sort((left, right) => String(right[1]?.at || "").localeCompare(String(left[1]?.at || "")))
            .slice(0, exports.MEMORY_MODEL_JUDGMENT_CACHE_MAX_ENTRIES));
    (0, atomic_json_file_1.withFileLock)(file, () => (0, atomic_json_file_1.writeJsonAtomic)(file, {
        schema: "ccm-typed-memory-model-judgment-cache-v1",
        version: exports.MEMORY_MODEL_JUDGMENT_VERSION,
        entries: bounded,
        updatedAt: now(),
    }));
}
...
function validateAdmissionJudgment(value) {
    const issues = [];
    if (!value || typeof value !== "object")
        return { valid: false, issues: ["not_an_object"], value: null };
    if (typeof value.id !== "string" || !value.id.trim())
        issues.push("missing_id");
    if (typeof value.admit !== "boolean")
        issues.push("admit_not_boolean");
    const confidence = Number(value.confidence);
    if (!Number.isFinite(confidence) || confidence < 0 || confidence > 1)
        issues.push("confidence_out_of_range");
    if (value.admit === true && !String(value.why || "").trim())
        issues.push("admitted_without_why");
    if (value.admit === true && !String(value.howToApply || value.how_to_apply || "").trim())
        issues.push("admitted_without_how_to_apply");
    if (issues.length)
...
/** 召回重排判定的本地 schema 校验。 */
function validateRerankJudgment(value) {
    const issues = [];
    if (!value || typeof value !== "object")
        return { valid: false, issues: ["not_an_object"], value: null };
    if (typeof value.id !== "string" || !value.id.trim())
    ...
    if (!Number.isFinite(relevance) || relevance < 0 || relevance > 1)
        issues.push("relevance_out_of_range");
    if (issues.length)
        return { valid: false, issues, value: null };
    return {
        valid: true,
 ...
 * 调用方据此回退到本地启发式。
 */
async function judgeMemoryAdmissionWithModel(scopeId, candidates = [], config = {}) {
    const scope = String(scopeId || "").trim() || "global";
    const items = (Array.isArray(candidates) ? candidates : []).slice(0, exports.MEMORY_MODEL_JUDGMENT_MAX_ITEMS);
    const base = {
    ...
    if (!judgments)
        return { ...base, degraded: true, degradedReason: "model_returned_no_parsable_judgments" };
    const at = now();
    ...
    // 模型一条都没判对，等同于不可用。
    if (!accepted)
        return { ...base, degraded: true, degradedReason: "no_valid_judgment_in_model_output" };
    try {
        writeJudgmentCache("admission", scope, entries);
    }
    catch { }
    return base;
}
 ...
 */
async function rerankMemoryRecallWithModel(scopeId, query, docs = [], config = {}) {
    const scope = String(scopeId || "").trim() || "global";
    const items = (Array.isArray(docs) ? docs : []).slice(0, exports.MEMORY_MODEL_JUDGMENT_MAX_ITEMS);
    const base = {
    ...
    if (!queryText)
        return { ...base, degraded: true, degradedReason: "empty_query" };
    ...
    if (!accepted)
        return { ...base, degraded: true, degradedReason: "no_valid_judgment_in_model_output" };
    try {
        writeJudgmentCache("rerank", scope, entries);
    }
    catch { }
    return base;
}
 ...
 */
async function applyModelRecallRerankToBundle(scopeId, query, bundle, config = {}) {
    ...
    const state = {
        ...
        notApplicableCount: rows.filter((row) => row.modelRerank?.applicable === false).length,
 ...
 */
function modelRerankScoreAdjustment(judgment, options = {}) {
    if (!judgment)
        return { adjustment: 0, applied: false, reason: "no_model_judgment" };
...
//# sourceMappingURL=typed-memory-model-judgment.js.map