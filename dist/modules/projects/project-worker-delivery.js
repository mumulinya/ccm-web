...
const workspace_path_safety_1 = require("../../agents/workspace-path-safety");
...
function mainWorktreeAlreadyContainsCommit(mainWorkDir, commit, changedPaths) {
...
function cleanupWorktree(mainWorkDir, prepared) {
    // A reserved Windows device path cannot be removed through the normal
    // filesystem APIs.  Force-removing the isolated worktree is safe here: it
    // is never the user's main project directory.
    try {
        runGit(mainWorkDir, ["worktree", "remove", "--force", prepared.worktreePath]);
    }
    catch {
...
function enqueueProjectWorkerDelivery(input) {
    ...
    const run = async () => {
        ...
        const reservedPaths = (0, workspace_path_safety_1.reservedPathsFromPorcelain)(status);
        if (reservedPaths.length) {
            // Never stage an invalid device path.  It is an agent-side artifact in
            // an isolated worktree; valid files remain deliverable and the forced
            // cleanup below removes the contaminated snapshot.
            console.warn(`[project-worker-delivery] ignored Windows device paths: ${reservedPaths.join(", ")}`);
        }
        ...
        if (status) {
            const addArgs = reservedPaths.length
                ...
                : ["add", "-A"];
            runGit(prepared.worktreePath, addArgs);
