...
const project_orphan_removal_1 = require("./project-orphan-removal");
...
function buildProjectFeishuAcpRuntimeConfig(content, projectName, port) {
    ...
    runtimeContent = (0, cc_connect_feishu_runtime_config_1.configureNativeFeishuReactionFeedback)(runtimeContent);
...
function applyCcConnectDataDir(content) {
    const line = `data_dir = "${escapeTomlString(utils_1.CCM_DIR)}"`;
    if (/^\s*data_dir\s*=/im.test(content))
        return content.replace(/^\s*data_dir\s*=.*$/im, line);
    return `${line}\n${content}`;
}
...
function writeControlBotConfig(port = 3080) {
    ...
    const dataDirToml = `data_dir = "${escapeTomlString(utils_1.CCM_DIR)}"`;
...
async function startProject(projectName, agentType, port, discoveredPid = 0) {
    ...
    content = applyCcConnectDataDir(content);
    const adapterPath = path.join(__dirname, "..", "..", "integrations", "control-bot-acp.js");
...
function handleProjectsApi(pathname, req, res, parsed, ctx) {
    ...
    if (pathname === "/api/projects" && req.method === "GET") {
        ...
        const projects = configs.map((config) => {
            ...
            const directoryState = (0, project_lifecycle_1.inspectProjectDirectoryState)(config.name);
            return {
                name: config.name,
...
`;
                ...
                const instructionGeneration = (0, scope_instructions_1.ensureProjectScopeInstruction)(safeName);
                if (cloneReceiptId)
                ...
                (0, utils_1.sendJson)(res, {
                    ...
                    instruction_generation: instructionGeneration,
                });
            }
            catch (e) {
                if (cloneReceiptId)
...
`;
                ...
                const testAuth = test_auth ? (0, project_test_auth_1.saveProjectTestAuthProfile)(safeName, test_auth) : (0, project_test_auth_1.getProjectTestAuthProfile)(safeName);
                const instructionGeneration = (0, scope_instructions_1.ensureProjectScopeInstruction)(safeName);
                (0, utils_1.sendJson)(res, { success: true, message: "项目配置已更新", display_name: savedDisplayName, repository: repositoryStatus, test_auth: testAuth, instruction_generation: instructionGeneration });
            }
            catch (e) {
                (0, utils_1.sendJson)(res, { success: false, error: e.message }, 400);
            }
    ...
    if (pathname === "/api/projects/delete" && req.method === "POST") {
        ...
        req.on("end", () => {
            try {
                ...
                const result = (0, project_lifecycle_1.archiveProject)(safeName);
                (0, scope_instructions_1.deactivateProjectScopeInstruction)(safeName);
        ...
        req.on("end", () => {
            try {
                ...
                if (!allowProject(payload.name, "manage"))
                    return;
                const result = (0, project_lifecycle_1.restoreProject)(payload.name);
                const instructionGeneration = (0, scope_instructions_1.restoreProjectScopeInstruction)(payload.name);
                (0, utils_1.sendJson)(res, { ...result, instruction_generation: instructionGeneration });
            }
            catch (e) {
                (0, utils_1.sendJson)(res, { success: false, error: e.message }, 400);
            }
    ...
    if (pathname === "/api/projects/purge" && req.method === "POST") {
        ...
        req.on("end", () => {
            try {
                ...
                if (!allowProject(payload.name, "manage"))
                    return;
                const result = (0, project_lifecycle_1.purgeArchivedProject)(payload.name, payload.preview_token);
                (0, scope_instructions_1.purgeProjectScopeInstruction)(payload.name);
                (0, utils_1.sendJson)(res, result);
            }
            catch (e) {
                (0, utils_1.sendJson)(res, { success: false, error: e.message }, 400);
    ...
    if (pathname === "/api/projects/remove-preview" && req.method === "POST") {
        let body = "";
        req.on("data", (chunk) => body += chunk);
        req.on("end", () => {
            try {
                ...
                if (!allowProject(payload.name, "manage"))
                    return;
                (0, utils_1.sendJson)(res, (0, project_orphan_removal_1.previewActiveOrphanProjectRemoval)(payload.name));
            }
            catch (e) {
                (0, utils_1.sendJson)(res, { success: false, error: e.message, code: "ORPHAN_PROJECT_REMOVE_NOT_ALLOWED" }, 409);
            }
        });
        return true;
    }
    if (pathname === "/api/projects/remove" && req.method === "POST") {
        ...
        req.on("end", () => {
            try {
                ...
                if (!allowProject(payload.name, "manage"))
                    return;
                const result = (0, project_orphan_removal_1.commitActiveOrphanProjectRemoval)(payload.name, payload.preview_token);
                (0, scope_instructions_1.purgeProjectScopeInstruction)(payload.name);
                (0, utils_1.sendJson)(res, result);
            }
            catch (e) {
                (0, utils_1.sendJson)(res, { success: false, error: e.message, code: "ORPHAN_PROJECT_REMOVE_FAILED" }, 409);
            }
        });
        return true;
    }
