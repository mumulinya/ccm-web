...
const evidence_policy_1 = require("../../agents/evidence-policy");
...
function recordProjectMainToolUse(project, projectSessionId, toolName, args, runId = "", parallelGroupId = "", preparedToolCallId = "", correlation = {}) {
    const toolCallId = preparedToolCallId || projectMainToolCallId(projectSessionId, toolName);
    const executionCorrelation = {
        ...correlation,
        ...(runId && !correlation.taskId ? { taskId: runId } : {}),
        projectId: correlation.projectId || project,
    };
    ...
    projectMainToolStartedAt.set(toolCallId, { startedAt: Date.now(), arguments: args, ...(parallelGroupId ? { parallelGroupId } : {}), correlation: executionCorrelation });
    (0, user_visible_agent_events_1.appendToolProjection)({
        scope: "project",
        scopeId: project,
        exactSessionId: projectSessionId,
        ...(executionCorrelation.anchorMessageId ? { anchorMessageId: executionCorrelation.anchorMessageId } : {}),
        ...(executionCorrelation.turnId ? { turnId: executionCorrelation.turnId } : {}),
        generation: Math.max(0, Number(executionCorrelation.generation || 0)),
        attempt: Math.max(1, Number(executionCorrelation.attempt || 1)),
        ...(executionCorrelation.agentRunId ? { agentRunId: executionCorrelation.agentRunId } : {}),
        eventType: "tool_started",
        toolName,
        toolCallId,
        taskId: executionCorrelation.taskId || runId || undefined,
        arguments: args,
...
function recordProjectMainToolResult(project, projectSessionId, toolName, toolCallId, observation, error = "", runId = "") {
    ...
    (0, user_visible_agent_events_1.appendToolProjection)({
        ...
        scopeId: project,
        exactSessionId: projectSessionId,
        ...(started?.correlation?.anchorMessageId ? { anchorMessageId: started.correlation.anchorMessageId } : {}),
        ...(started?.correlation?.turnId ? { turnId: started.correlation.turnId } : {}),
        generation: Math.max(0, Number(started?.correlation?.generation || 0)),
        attempt: Math.max(1, Number(started?.correlation?.attempt || 1)),
        ...(started?.correlation?.agentRunId ? { agentRunId: started.correlation.agentRunId } : {}),
        eventType: error ? "tool_failed" : "tool_completed",
        toolName,
        toolCallId,
        taskId: started?.correlation?.taskId || runId || undefined,
...
function semanticTextValue(value, seen = new Set()) {
    ...
    if (Array.isArray(value))
        return value.map(item => semanticTextValue(item, seen)).filter(Boolean).join("；");
    if (typeof value !== "object" || seen.has(value))
        return "";
    seen.add(value);
    const preferredKeys = [
        "text", "label", "title", "subject", "description", "objective", "task", "goal",
        "criterion", "observableOutcome", "observable_outcome", "outcome", "expected",
        "summary", "content", "value", "activeForm", "active_form",
    ];
    for (const key of preferredKeys) {
        const candidate = semanticTextValue(value[key], seen);
        if (candidate)
            return candidate;
    }
    return "";
...
function cleanText(value, max = 1200) {
    return semanticTextValue(value).trim().slice(0, max);
}
...
function directDispatchAcceptanceEvidence(input) {
    const workflowOnly = (value) => {
        const text = cleanText(typeof value === "string" ? value : value?.criterion || value?.description || value?.observableOutcome || value, 1200);
        if (!text)
            return false;
        const processSignals = [
            /明确授权|授权你|创建并执行正式开发任务/i,
            /项目子 Agent|项目主 Agent|群聊主 Agent|全局主 Agent|TestAgent/i,
            /Terminal Gate|独立复核|主 Agent.*抽查|通过.*门禁/i,
            /必须由.*(?:Agent|代理)|必须交给.*(?:Agent|复核)/i,
        ];
        const observableSignals = [
            /修改|新增|删除|导出|显示|返回|等于|包含|只.*显示|文件|接口|页面|字段|构建|测试|验证/i,
        ];
        return processSignals.some(pattern => pattern.test(text)) && !observableSignals.some(pattern => pattern.test(text.replace(/必须由[\s\S]*$/i, "")));
    };
    const criterionText = (value) => cleanText(typeof value === "string" ? value : value?.criterion || value?.description || value?.observableOutcome || value, 800)
        .replace(/(?:必须由项目子 Agent|必须交给 TestAgent|再由主 Agent|并通过 Terminal Gate)[\s\S]*$/i, "")
        .replace(/^(?:我明确授权你立即创建并执行正式开发任务[。！.!]?\s*)/i, "")
        .trim();
    const provided = Array.isArray(input.value) ? input.value : [];
    const providedObservable = provided
        .map(item => ({ raw: item, text: criterionText(item) }))
        .filter(item => item.text && !workflowOnly(item.raw));
    const workItemCriteria = cleanList(input.workItems.flatMap(item => item.acceptanceCriteria || [])
        .map(criterionText)
        .filter(Boolean)
        .filter(item => !workflowOnly(item)), 20, 800);
    const derivedWorkItemCriteria = cleanList(input.workItems.flatMap(item => {
        const objective = criterionText(item.objective || item.title);
        return objective && !workflowOnly(objective) ? [objective] : [];
    }), 20, 800);
    const criteria = providedObservable.length
        ? providedObservable.map(item => item.text)
        : workItemCriteria.length
            ? workItemCriteria
            : derivedWorkItemCriteria.length
                ? derivedWorkItemCriteria
                : ["已确认的目标文件变更存在，并通过项目声明的验证命令。"];
    // The process itself is covered by signed task/Agent/Terminal-Gate records;
    // TestAgent criteria must remain observable in the project state.
...
function projectRequirementPlanProjection(plan, input) {
    ...
    const canonical = (0, implementation_plan_1.normalizeImplementationPlanV2)({
        ...
        ...(plan.planningReason ? { planningReason: plan.planningReason } : {}),
...
function projectMainModelCallOptions(config, messages, telemetry) {
    ...
    (0, canonical_context_accounting_1.recordCanonicalContextPreflight)({
        scope: "project",
    ...
    return {
        ...
        nativeToolChoice: telemetry.nativeToolChoice,
        ...
        providerContextCache: {
            ...
            cacheAffinity: telemetry.stage && telemetry.stage !== "main_tool_loop"
                ? (0, agent_cache_affinity_1.createAgentCacheAffinity)({
                    ...
                    agentRole: telemetry.stage === "plan_review" ? "plan_reviewer" : "semantic_auxiliary",
                    stage: telemetry.stage,
                    runtimeOwnership: "ccm_provider",
                    stablePromptVersion: `ccm-project-${telemetry.stage}-stable-v1`,
                    cacheKeyProfile: `project:${telemetry.stage}`,
                    exactSessionId: telemetry.projectSessionId,
                })
                : (0, agent_cache_affinity_1.mainAgentCacheAffinity)({
                    scope: "project",
                    scopeId: telemetry.project,
                    exactSessionId: telemetry.projectSessionId,
                    generation: 0,
        ...
        onUsage: (usage) => {
            ...
            try {
                ...
                (0, canonical_context_accounting_1.completeCanonicalContextAccounting)({
                    scope: "project",
                    scopeId: telemetry.project,
                    exactSessionId: telemetry.projectSessionId,
...
function projectMainLoadedContextItems(toolContext, results, roleSkills, runtimeResults = []) {
    ...
    const evidence = toolContext ? (0, main_agent_tool_runtime_1.buildMainAgentLoadedContextItems)(toolContext, results, selectedSkills) : (0, session_compaction_core_1.normalizeLoadedContextItems)({
    ...
    return (0, session_compaction_core_1.normalizeLoadedContextItems)(evidence);
}
...
function projectMainPlanMode(plan, decision, options = {}) {
    ...
    const sessionPlanMode = options.sessionPlanMode === true;
    ...
    return {
        ...
        // A task can carry an internal execution contract even when the user did
        // not opt into the session Plan Mode.  The UI uses this flag to avoid
        // presenting that internal contract as a plan card in ordinary Agent mode.
        session_plan_mode: sessionPlanMode,
        display_policy: {
            user_visible: sessionPlanMode || requiresConfirmation,
            show_todo: sessionPlanMode || requiresConfirmation,
...
function projectMainExactSessionContext(project, projectSessionId, currentRequest) {
    if (!projectSessionId)
        return "";
    const projection = (0, project_session_compaction_1.buildProjectSessionModelContextProjection)(project, projectSessionId, { currentRequest, consumeSessionStartHookContext: true });
...
async function ensureProjectMainModelCapacity(input) {
    ...
    const toolPreflight = (0, pre_request_tool_context_1.stagePreRequestToolContext)({
        ...
        exactSessionId: input.projectSessionId,
        messages,
        providerPayloadChecksum: String(payload.payloadChecksum || ""),
        tokensBefore: Number(payload.totalTokens || 0),
        config,
    });
    if (toolPreflight.changed) {
        ...
        if (payload.totalTokens < threshold)
            return { messages, payload, capacity, compacted: true, toolContextCompacted: true };
    }
...
async function hydrateProjectMainSource(input) {
    ...
    if (!manifestRows.length) {
        const toolCallId = input.recordTool === false ? "" : recordProjectMainToolUse(input.project, input.projectSessionId, "read_project_source", {
        ...
        if (toolCallId)
            recordProjectMainToolResult(input.project, input.projectSessionId, "read_project_source", toolCallId, projectSourceReadProjection(evidence));
...
3. Select at most ${input.readDepth === "focused" ? 6 : 12} files. For code changes, normally include project configuration and at least one related implementation file; an empty greenfield project may return an empty array.
4. Decide whether the selected evidence is sufficient to bound the requested change. Return JSON only: {"paths":["relative/path"],"reason":"selection reason","sufficient":true,"missingEvidence":[]}`,
        },
        {
            role: "user",
            content: JSON.stringify({
                ...
                read_depth: input.readDepth || "focused",
    ...
    const selectedPaths = cleanList(selected?.paths, input.readDepth === "focused" ? 6 : 12, 500);
    const toolCallId = input.recordTool === false ? "" : recordProjectMainToolUse(input.project, input.projectSessionId, "read_project_source", {
        purpose: input.purpose,
        manifest_checksum: manifest.checksum,
    ...
    if (input.requiresCodeChanges === true && manifest.files.length > 0) {
        if (selected?.sufficient === false) {
            const missing = cleanList(selected?.missingEvidence || selected?.missing_evidence, 8, 300);
            throw new Error(`项目源码证据不足，不能派发开发任务：${missing.join("；") || cleanText(selected?.reason, 500) || "尚未定位最小相关实现范围"}`);
        }
        if (!evidence.files.length)
            throw new Error("项目源码证据不足，不能派发开发任务：尚未读取实现入口、直接相关实现或测试/配置");
    }
...
function projectSourceReadProjection(evidence) {
    const summary = projectSourceEvidenceSummary(evidence);
    return {
        schema: "ccm-project-source-read-projection-v1",
        project: evidence.project,
...
function validateProjectDevelopmentSource(plan, requiresCodeChanges) {
    return (0, development_source_grounding_1.validateDevelopmentSourceGrounding)({
        requiresCodeChanges,
        targetProjects: [plan.project],
        sourceManifestChecksum: plan.sourceEvidence?.manifestChecksum,
        projects: [{
                project: plan.project,
                manifestChecksum: plan.sourceEvidence?.manifestChecksum,
                manifestFiles: plan.sourceEvidence?.manifestFiles,
                selectedPaths: plan.sourceEvidence?.selectedPaths,
                files: (plan.sourceEvidence?.files || []).map(file => ({
                    path: file.path,
                    checksum: file.checksum,
                    evidenceId: file.evidenceId,
                })),
                status: plan.sourceEvidence?.manifestFiles === 0 ? "empty_project" : "ready",
            }],
...
5. Return JSON only: {"toolRequests":[{"name":"tool_name","arguments":{},"reason":"reason"}]}`,
            ...
            stage: "semantic_decision",
            contextComponents: {
                loadedContextItems: (0, session_compaction_core_1.normalizeLoadedContextItems)({
                    schema: "ccm-loaded-context-items-v2",
                    skills: [],
...
async function runProjectMainAgentFirstTurn(input) {
    ...
    const loopBudget = (0, agent_loop_budget_1.resolveAgentLoopBudget)({
        ...
        toolBatchSize: budgetConfig.agentToolBatchSize || readonly_tool_concurrency_1.CCM_READONLY_TOOL_CONCURRENCY_DEFAULT,
        readOnlyParallelism: budgetConfig.agentReadOnlyParallelism || readonly_tool_concurrency_1.CCM_READONLY_TOOL_CONCURRENCY_DEFAULT,
    ...
    const auxiliaryStageUsage = new Map();
    ...
    const captureAuxiliaryUsage = (stage, usage) => {
        captureUsage(usage);
        const row = auxiliaryStageUsage.get(stage) || { cacheReadInputTokens: 0, directInputTokens: 0 };
        row.cacheReadInputTokens += Number(usage?.cacheReadInputTokens || usage?.cache_read_input_tokens || 0);
        row.directInputTokens += Number(usage?.directInputTokens || usage?.direct_input_tokens || usage?.inputTokens || usage?.input_tokens || 0);
        auxiliaryStageUsage.set(stage, row);
    };
    ...
    const projectIdentityRules = (0, main_agent_identity_1.buildProjectMainIdentityRules)(projectIdentityInput);
    const projectDynamicContext = (0, main_agent_identity_1.buildProjectMainDynamicContext)(projectIdentityInput);
    const buildMessages = () => {
    ...
    const stableProjectNativeTools = () => [
        ...
        ...(0, native_query_loop_1.catalogToNativeTools)(toolContext),
    ];
    const cacheAlignedFinalMessages = (instruction, payload) => (0, final_synthesis_cache_alignment_1.buildCacheAlignedFinalMessages)({
        identityRules: projectIdentityRules,
        sessionGuidance: [(0, main_agent_identity_1.buildProjectMainSessionGuidance)({ planAuthoring }), projectDynamicContext].filter(Boolean).join("\n\n"),
        mcpPolicy: toolContext.policyPrompt,
        instruction,
    ...
    const firstTurnToolCorrelation = {
    ...
    const executeSelectedRequest = async (request, parallelGroupId = "", preparedToolCallId = "") => {
        ...
        try {
            ...
            if (request.name === "read_project_source") {
                ...
                sourceHydration = await hydrateProjectMainSource({ project, projectSessionId, userMessage: input.userMessage, conversationContext: exactContext, purpose: decision.requiresCodeChanges ? "planning" : "analysis", requiresCodeChanges: decision.requiresCodeChanges, readDepth: decision.sourceReadDepth === "broad" ? "broad" : "focused", recordTool: false });
            ...
            const recordedOutput = request.name === "read_project_source" && sourceHydration
                ? projectSourceReadProjection(sourceHydration.evidence)
    ...
    const nativeLoop = await (0, project_native_query_adapter_1.runProjectMainNativeQueryLoop)({
        ...
        visibleTurnId,
        attempt: 1,
        visibleAnchorMessageId: String(input.anchorMessageId || "").trim(),
        loopBudget,
        signal: input.signal,
        ...
        captureUsage,
        boundaryGeneration: Number((0, project_session_compaction_1.buildProjectSessionModelContextProjection)(project, projectSessionId, { currentRequest: input.userMessage })?.boundaryGeneration || 0),
        roleSkills,
    });
    ...
    const sourceQuestionDecision = (0, workflow_decision_1.normalizeWorkflowDecision)(parsed?.workflowDecision || parsed?.workflow_decision || {});
    const hasUsableMainReply = () => {
        const reply = parsed?.reply ?? parsed?.content ?? parsed?.summary;
        return typeof reply === "string" && reply.trim().length > 0;
    };
    const hasReadEvidence = !!sourceHydration || toolResults.some((row) => {
        const name = String(row?.name || row?.toolName || "").replace(/^mcp__ccm__ccm_workspace_readonly__/, "");
        return row?.ok !== false && ["read_file", "read_files", "read_project_source"].includes(name);
    });
    if (!hasUsableMainReply()
        && sourceQuestionDecision.readAction === "inspect_source"
        && sourceQuestionDecision.actionRequired === false
        && sourceQuestionDecision.requiresCodeChanges === false
        && !hasReadEvidence) {
        const readDepth = sourceQuestionDecision.sourceReadDepth === "broad" ? "broad" : "focused";
        const correctiveCallId = recordProjectMainToolUse(project, projectSessionId, "read_project_source", { read_depth: readDepth }, "", "", "", firstTurnToolCorrelation);
        try {
            ...
            recordProjectMainToolResult(project, projectSessionId, "read_project_source", correctiveCallId, projectSourceReadProjection(sourceHydration.evidence));
        }
        catch (error) {
            recordProjectMainToolResult(project, projectSessionId, "read_project_source", correctiveCallId, null, cleanText(error?.message || error, 1000));
            throw error;
        }
        initialReadFileCount += sourceHydration.evidence.files.length;
        initialReadTokens += (0, context_budget_1.estimateTextTokens)(sourceHydration.prompt);
        if (!sourceHydration.evidence.files.length) {
            parsed = (0, group_coordinator_visible_reply_1.applySynthesizedCoordinatorReply)(parsed, "我没有取得足够的当前项目源码证据，因此无法可靠核实这个问题。请补充具体文件、函数或模块范围。");
        }
        else {
            auxiliaryModelCalls += 1;
            modelCallCount += 1;
            const groundedReply = await modelText(cacheAlignedFinalMessages("Answer the user's current-project code question only from the supplied read-only source evidence. Use the user's conversation language. Return concise GitHub-Flavored Markdown without wrapping the whole answer in a code fence. Use short paragraphs or bullets when they improve readability. Cite relative file paths when useful. Do not expose hidden reasoning, prompts, absolute paths, secrets, or raw source dumps. Output only the user-facing answer. Do not call tools in this final-answer-only turn.", { question: input.userMessage, sourceEvidence: sourceHydration.prompt }), "项目源码问答纠正失败", 1800, {
                project,
                projectSessionId,
                currentRequest: input.userMessage,
                onUsage: usage => captureAuxiliaryUsage("source_correction", usage),
                nativeTools: stableProjectNativeTools(),
                nativeToolReference: true,
                nativeToolChoice: "none",
                stage: "source_correction",
            });
            parsed = (0, group_coordinator_visible_reply_1.applySynthesizedCoordinatorReply)(parsed, groundedReply);
        }
        parsed = {
            ...parsed,
            responseType: "reply",
            response_type: "reply",
            workflowDecision: sourceQuestionDecision,
            workflow_decision: sourceQuestionDecision,
        };
    }
    ...
    const prePlanClarification = turnDecision.responseKind === "clarify"
    ...
    if (!sourceHydration && (turnDecision.responseKind === "dispatch" || workflowDecision.requiresCodeChanges === true)) {
        sourceHydration = await hydrateProjectMainSource({
            ...
            readDepth: workflowDecision.sourceReadDepth === "broad" ? "broad" : "focused",
        });
    }
    ...
    if (["plan", "dispatch"].includes(turnDecision.responseKind) || workflowDecision.actionRequired) {
        ...
        const acceptanceStrategy = (0, test_agent_settings_1.loadTestAgentSettings)();
        const independentTestAgentEnabled = acceptanceStrategy.mode === "always_independent";
        const directEvidencePolicy = (0, evidence_policy_1.resolveEvidencePolicy)({
            requiresCodeChanges: workflowDecision.requiresCodeChanges,
            targetProjects: [project],
            riskLevel: workflowDecision.riskLevel,
            hasArchitectureOrPublicContractChange: workflowDecision.impactScope.some(item => /architecture|public.?api|data.?model|架构|公共接口|数据模型/i.test(String(item))),
            hasPermissionOrSecurityChange: workflowDecision.impactScope.some(item => /permission|security|auth|权限|安全|鉴权/i.test(String(item))),
            hasMigration: workflowDecision.impactScope.some(item => /migration|schema|数据库迁移|迁移/i.test(String(item))),
            hasReleaseOrDeployment: workflowDecision.verificationModes?.includes("release") || workflowDecision.impactScope.some(item => /release|deploy|发布|部署/i.test(String(item))),
            verificationModes: workflowDecision.verificationModes,
        });
        plan = {
            ...
            planningReason: workflowDecision.requiresUserConfirmation || (0, implementation_plan_1.shouldRequireImplementationPlan)({
                riskLevel: workflowDecision.riskLevel,
            ...
            }) ? cleanText(workflowDecision.reason, 1200) : "",
            project,
            projectSessionId,
            ...
            evidencePolicy: directEvidencePolicy,
    ...
    if (["reply", "clarify"].includes(turnDecision.responseKind) || visiblePlanResult) {
        if (toolCallCount > 0 && String(turnDecision.reply || "").trim())
    ...
    return {
        ...
        mainAgentToolUsage: {
            ...
            toolCallCount,
            auxiliaryStages: auxiliaryModelCalls ? [{
                    stage: "source_correction",
                    calls: auxiliaryModelCalls,
                    cacheReadInputTokens: auxiliaryStageUsage.get("source_correction")?.cacheReadInputTokens || 0,
                    directInputTokens: auxiliaryStageUsage.get("source_correction")?.directInputTokens || 0,
                }] : [],
...
async function planProjectMainTask(input) {
    ...
    const independentTestAgentEnabled = input.acceptanceMode
        ...
        : acceptanceStrategy.mode === "always_independent";
    const acceptancePlanningRule = acceptanceStrategy.mode === "auto"
        ? "The server freezes a deterministic lightweight, standard, or strict acceptance policy at task creation. Record all risk signals and required evidence; do not lower the level or decide TestAgent scheduling from model text."
        : independentTestAgentEnabled
            ? "All code/file changes require independent TestAgent acceptance."
            : "The project main Agent performs the configured self-verification route and must not claim independent acceptance.";
    const roleSkills = (0, role_skills_1.buildRoleSkillPrompt)("project-main-agent", input.userMessage, {
        forceWork: true,
    ...
    const evidencePolicy = (0, evidence_policy_1.resolveEvidencePolicy)({
        ...
        targetProjects: [project],
        riskLevel: decision.riskLevel,
        hasArchitectureOrPublicContractChange: decision.impactScope.some(item => /architecture|public.?api|data.?model|架构|公共接口|数据模型/i.test(String(item))),
        hasPermissionOrSecurityChange: decision.impactScope.some(item => /permission|security|auth|权限|安全|鉴权/i.test(String(item))),
        hasMigration: decision.impactScope.some(item => /migration|schema|数据库迁移|迁移/i.test(String(item))),
        hasReleaseOrDeployment: decision.verificationModes?.includes("release") || decision.impactScope.some(item => /release|deploy|发布|部署/i.test(String(item))),
        destructive: decision.riskLevel === "high" && decision.authorizationDirective === "revoke",
        verificationModes: decision.verificationModes,
    });
    ...
    planningSession = (0, planning_orchestrator_1.updatePlanningSession)(planningSession, { phase: "drafting", evidenceManifest, evidenceManifestChecksum: evidenceManifest.checksum, sourceManifestChecksum: sourceHydration.evidence.manifestChecksum, evidencePolicy });
    ...
    const contextComponents = {
        ...
        scopeInstructionLoadedContext: configuredToolContext.loadedContext || "",
...
${implementation_plan_1.IMPLEMENTATION_PLAN_LANGUAGE_CONTRACT}\n${implementation_plan_1.IMPLEMENTATION_PLAN_PROMPTS.planning_draft}`;
    const buildPlanningMessages = () => {
        const native = (0, project_native_messages_1.tryBuildProjectNativeMainMessages)({
            ...
            sessionGuidance: [(0, main_agent_identity_1.buildProjectMainSessionGuidance)({
                    planAuthoring: (0, conversation_plan_mode_gate_1.isConversationPlanModeEnabled)("project", project, projectSessionId),
                }), (0, main_agent_identity_1.buildProjectMainDynamicContext)({
                    project,
                    planAuthoring: true,
                    roleSkillsPrompt: roleSkills.prompt,
                })].filter(Boolean).join("\n\n"),
    ...
    if (candidates.length > 1) {
        ...
        ], "项目主 Agent 计划候选收敛失败", { project, projectSessionId, currentRequest: input.userMessage, contextComponents, stage: "plan_candidate" });
    }
    ...
    let plan = {
        ...
        planningReason: cleanText(decision.reason, 1200),
        project,
        projectSessionId,
        acceptanceMode: independentTestAgentEnabled ? "test_agent" : "main_agent_self_verification",
    ...
    const canonicalPlan = () => {
        ...
        return (0, planning_orchestrator_1.applyEvidencePolicyToPlan)({ ...projected, evidencePolicy, checksum: (0, implementation_plan_1.implementationPlanChecksum)({ ...projected, evidencePolicy }) }, evidenceManifest, evidencePolicy);
    };
    ...
    if (planningLimits.independentReview) {
        ...
        ], "项目计划独立复核失败", { project, projectSessionId, currentRequest: input.userMessage, contextComponents, stage: "plan_review" });
    }
    let reviewReceipt = (0, planning_orchestrator_1.buildPlanReviewReceipt)({ plan: canonicalPlan(), evidenceManifest, reviewer, evidencePolicy });
    if (!(0, evidence_policy_1.isPlanReviewPassed)(reviewReceipt.verdict)) {
        ...
        ], "项目计划自动修复失败", { project, projectSessionId, currentRequest: input.userMessage, contextComponents, stage: "plan_review" });
        ...
        if (planningLimits.independentReview) {
            ...
            ], "项目计划修复后复核失败", { project, projectSessionId, currentRequest: input.userMessage, contextComponents, stage: "plan_review" });
        }
        reviewReceipt = (0, planning_orchestrator_1.buildPlanReviewReceipt)({ plan: canonicalPlan(), evidenceManifest, reviewer: repairedReviewer, evidencePolicy });
    }
    if (!(0, evidence_policy_1.isPlanReviewPassed)(reviewReceipt.verdict)) {
...
function createProjectMainTask(input) {
    const sourceGrounding = validateProjectDevelopmentSource(input.plan, input.workflowDecision.requiresCodeChanges === true);
    if (!sourceGrounding.ready) {
        const error = new Error(`开发任务派发前源码核对未通过：${sourceGrounding.issues.join("；")}`);
        error.code = "CCM_SOURCE_GROUNDING_REQUIRED";
        error.sourceGrounding = sourceGrounding;
        throw error;
    }
    const independentTestAgentEnabled = input.plan.acceptanceMode === "test_agent";
    const sessionPlanMode = (0, conversation_plan_mode_gate_1.isConversationPlanModeEnabled)("project", input.project, input.projectSessionId);
    ...
    const canonicalPresentedPlan = projectRequirementPlanProjection(input.plan, {
        ...
        sourceMessageIds: [input.projectMainRunId],
    });
    const task = (0, collaboration_task_service_1.createTask)({
        ...
        // Preserve the workflow's risk signal. The V3 acceptance policy snapshot is
        // the sole authority that maps it and the configured strategy to an actual
        // route; the legacy plan preference must never lower a strict classification.
        requires_independent_review: input.workflowDecision.requiresIndependentReview === true,
    ...
    const initialPresentedPlan = projectRequirementPlanProjection(input.plan, {
        ...
        revision: 1,
        status: input.plan.requiresConfirmation ? "ready" : "executing",
        sourceMessageIds: [String(task.message_id || input.projectMainRunId || "")].filter(Boolean),
    });
    const updated = (0, collaboration_task_service_1.updateTask)(task.id, {
        anchor_message_id: `project-main-task:${task.id}`,
        status: input.plan.requiresConfirmation ? "paused" : "pending",
        status_detail: input.plan.requiresConfirmation
            ? "项目主 Agent 已生成计划，等待用户确认"
            : sessionPlanMode
                ? "项目主 Agent 计划已就绪，等待进入会话串行队列"
                : "项目主 Agent 已完成执行前检查，等待进入会话串行队列",
        acceptance_state: "pending",
    ...
    if (sessionPlanMode || input.plan.requiresConfirmation) {
    ...
    (0, logs_1.appendTaskTimelineEvent)(updated.id, {
        ...
        title: sessionPlanMode || input.plan.requiresConfirmation
            ? "项目主 Agent 已生成执行计划"
            : "项目主 Agent 已完成执行前检查",
...
async function finalSummary(input) {
    // The main Agent's last no-tool response is already the user-facing answer.
    // Delivery data is assembled deterministically here so task completion never
    // opens a second "final synthesis" model request.
    const changes = aggregateFileChanges(input.results);
    const independentReview = input.review?.mode !== "main_agent_self_verification";
    void input.onDelta;
    void input.signal;
    const accepted = input.status === "completed" && input.review?.canAccept === true;
    const lines = [
        `## ${accepted ? "交付完成" : "交付未完成"}`,
        accepted
            ? (independentReview ? "TestAgent 独立验收已通过，项目主 Agent 已完成交付核对。" : "项目主 Agent 自验已通过，已完成交付核对。")
            : (independentReview ? "TestAgent 验收未通过，当前任务保留为阻塞状态。" : "项目主 Agent 自验未通过，当前任务保留为阻塞状态。"),
    ];
    const changedPaths = changes.files.map((item) => String(item?.path || item?.file || "").trim()).filter(Boolean);
    if (changedPaths.length)
        lines.push(`**文件变化**：${changedPaths.map(path => `\`${path}\``).join("、")}`);
    const verification = cleanList(input.review?.report?.verification || input.review?.verdict?.evidence || [], 20, 600);
    if (verification.length)
        lines.push(`**验证**：\n${verification.map(item => `- ${item}`).join("\n")}`);
    const risks = accepted ? cleanList(input.review?.report?.risks, 12, 600) : (0, project_test_agent_gate_1.projectTestAgentProblems)(input.review);
    if (risks.length)
        lines.push(`**风险/待处理**：\n${risks.map(item => `- ${item}`).join("\n")}`);
    const reviewSummary = cleanText(input.review?.report?.summary || input.review?.summary || "", 1600);
    if (reviewSummary)
        lines.push(`**验收结论**：${reviewSummary}`);
...
async function reviseProjectMainTask(input) {
    ...
    try {
        ...
        const revisedPresentedPlan = projectRequirementPlanProjection(revisedPlan, {
            ...
            sourceMessageIds: [String(task.message_id || task.origin_message_id || task.project_main_run_id || "")].filter(Boolean),
        });
        const updated = (0, collaboration_task_service_1.updateTask)(task.id, {
            status: "paused",
...
async function executeProjectMainTask(input) {
    ...
    const sourceGrounding = validateProjectDevelopmentSource(input.plan, input.task?.requires_code_changes === true || input.task?.workflow_decision?.requiresCodeChanges === true);
    if (!sourceGrounding.ready) {
        const reason = `开发任务派发前源码核对未通过：${sourceGrounding.issues.join("；")}`;
        const blockedTask = (0, collaboration_task_service_1.updateTask)(taskId, { status: "blocked", acceptance_state: "source_hydration_blocked", status_detail: reason }) || input.task;
        (0, logs_1.appendTaskTimelineEvent)(taskId, { type: "project_source_gate_blocked", title: "源码证据门禁阻止派发", detail: reason, status: "error", phase: "planning", agent: "project-main-agent", data: { contentStored: false } });
        return { task: blockedTask, status: "blocked", summary: reason, fileChanges: { count: 0, files: [] }, verification: [], risks: sourceGrounding.issues, testAgent: null };
    }
    ...
    const dispatchProjection = projectRequirementPlanProjection(input.plan, {
        ...
        status: "executing",
        sourceMessageIds: [String(input.task?.message_id || input.task?.origin_message_id || input.task?.project_main_run_id || "")].filter(Boolean),
    });
    ...
    const dispatchContract = dispatchPlan ? (0, plan_dispatch_contract_1.buildPlanDispatchContract)({
        ...
        evidencePolicy: input.plan.evidencePolicy,
    }) : null;
    ...
    input.plan.acceptanceCriteria = dispatchPlan.businessRequirement.acceptanceCriteria.map(item => item.description);
    input.plan.permissionBoundaries = [...dispatchPlan.businessRequirement.constraints, ...dispatchPlan.businessRequirement.exclusions];
    input.plan.workItems = input.plan.workItems.map(item => {
        ...
        return {
            ...item,
            ...(workItem ? { title: workItem.title, objective: workItem.objective, acceptanceCriteria: workItem.acceptance } : {}),
            dispatchContract: workItem ? { ...workItem, planId: dispatchContract.planId, planRevision: dispatchContract.planRevision, planChecksum: dispatchContract.planChecksum, sourceManifestChecksum: dispatchContract.sourceManifestChecksum, dispatchContractId: dispatchContract.contractId, dispatchContractChecksum: dispatchContract.contractChecksum } : null,
        };
    });
    ...
    (0, user_visible_agent_events_1.appendAssistantProgress)({
        ...
        anchorMessageId: taskToolCorrelation.anchorMessageId,
        generation: executionGeneration,
        taskId,
        turnId: taskToolCorrelation.turnId,
        attempt: taskToolCorrelation.attempt,
    ...
    const acceptancePolicyResult = (0, task_acceptance_policy_1.resolveTaskAcceptancePolicy)(getProjectMainTask(taskId) || input.task);
    ...
    let independentTestAgentEnabled = (0, task_acceptance_policy_1.taskAcceptanceUsesIndependentReview)(persistedAtResume, acceptancePolicy);
    ...
    const emitVisiblePlanState = (status = "executing", activeWorkItemId = "") => {
        ...
        return (0, user_visible_agent_events_1.appendUserVisibleRequirementPlan)({
            ...
            plan: projectRequirementPlanProjection(input.plan, {
                ...
                status,
                stepStatuses,
                sourceMessageIds: [String(persistedAtResume?.message_id || persistedAtResume?.origin_message_id || persistedAtResume?.project_main_run_id || "")].filter(Boolean),
            }),
        });
    };
    ...
    try {
        ...
        const executeWorkItem = async (item) => {
            ...
            }, input.task.project_main_run_id || taskId, "", "", taskToolCorrelation);
            let result;
            try {
        ...
        const runtimeFileChanges = aggregateFileChanges(results);
        const requiresAcceptanceReview = runtimeFileChanges.count > 0
            || input.task.requires_code_changes === true
            || input.task.requires_independent_review === true
            || input.task.requires_verification === true;
        let runtimeEscalation = (0, task_acceptance_policy_1.evaluateTaskAcceptanceEscalation)({
            task: getProjectMainTask(taskId) || input.task,
            policy: acceptancePolicy,
            changedFiles: runtimeFileChanges.files,
        });
        if (requiresAcceptanceReview && !independentTestAgentEnabled && runtimeEscalation.escalate) {
            const escalation = (0, task_acceptance_policy_1.buildTaskAcceptanceEscalationReceipt)({
                task: getProjectMainTask(taskId) || input.task,
                policy: acceptancePolicy,
                reasons: runtimeEscalation.reasons,
                changedFileCount: runtimeEscalation.changedFileCount,
                topLevelModuleCount: runtimeEscalation.topLevelModuleCount,
            });
            independentTestAgentEnabled = true;
            (0, collaboration_task_service_1.updateTask)(taskId, {
                acceptance_escalation: escalation,
                acceptance_actual_level: "strict",
                acceptance_actual_route: "independent_test_agent",
                acceptance_escalation_reasons: escalation.reasons,
                status_detail: "实际变更范围触发严格独立验收",
            });
            (0, logs_1.appendTaskTimelineEvent)(taskId, {
                type: "acceptance_escalated",
                title: "验收已升级为严格独立验收",
                detail: escalation.reasons.join("；"),
                status: "warn",
                phase: "reviewing",
                agent: "project-main-agent",
                data: { escalation, contentStored: false },
            });
        }
        let requiresTestAgent = requiresAcceptanceReview && independentTestAgentEnabled;
        ...
        if (requiresAcceptanceReview && !independentTestAgentEnabled && !resumeSummaryOnly) {
            ...
            persistExecutionState();
            (0, collaboration_task_service_1.updateTask)(taskId, { status: "reviewing", acceptance_state: "main_agent_self_verifying", status_detail: acceptancePolicy.schema === "ccm-task-acceptance-policy-snapshot-v3" && acceptancePolicy.level === "lightweight" ? "项目主 Agent 正在执行轻量自验" : "项目主 Agent 正在执行标准验收" });
            (0, logs_1.appendTaskTimelineEvent)(taskId, { type: "project_main_self_verification_started", title: acceptancePolicy.schema === "ccm-task-acceptance-policy-snapshot-v3" && acceptancePolicy.level === "lightweight" ? "开始轻量自验" : "开始标准验收", detail: "主 Agent正在核对范围、文件变化、验收标准和真实验证证据", status: "active", phase: "reviewing", agent: "project-main-agent" });
            ...
            if (!latestReview.canAccept) {
                runtimeEscalation = (0, task_acceptance_policy_1.evaluateTaskAcceptanceEscalation)({
                    task: getProjectMainTask(taskId) || input.task,
                    policy: acceptancePolicy,
                    changedFiles: runtimeFileChanges.files,
                    selfVerification: latestReview,
                });
                if (runtimeEscalation.escalate && !runtimeEscalation.implementationFailure) {
                    ...
                    (0, collaboration_task_service_1.updateTask)(taskId, {
                        ...
                        status_detail: "标准验收证据未闭环，已升级TestAgent独立复核",
                    });
                    (0, logs_1.appendTaskTimelineEvent)(taskId, { type: "acceptance_escalated", title: "标准验收已升级", detail: escalation.reasons.join("；"), status: "warn", phase: "reviewing", agent: "project-main-agent", data: { escalation, contentStored: false } });
                }
            }
        }
        ...
        const maxAcceptanceReviewRounds = Math.min(rework_policy_1.AUTO_REWORK_MAX_ROUNDS, (0, task_acceptance_policy_1.taskAcceptanceReviewRounds)(getProjectMainTask(taskId) || input.task, acceptancePolicy));
        for (let round = 1; requiresTestAgent && !resumeSummaryOnly && round <= maxAcceptanceReviewRounds; round += 1) {
            assertNotCancelled();
            executionPhase = "test_agent_running";
            persistExecutionState();
            (0, collaboration_task_service_1.updateTask)(taskId, { status: "reviewing", acceptance_state: "test_agent_running", status_detail: `TestAgent 正在执行第 ${round}/${maxAcceptanceReviewRounds} 轮独立验收`, review_round: round, review_round_total: reviewRoundTotalBase + round, rework_exhausted: null });
            ...
            emit("testing", { status: "running", round, max_rounds: maxAcceptanceReviewRounds });
            ...
            if (latestReview.canAccept)
                break;
            if (round >= maxAcceptanceReviewRounds)
                break;
        ...
        const blockedDetail = finalReviewDecision.route === "environment"
            ...
            : finalReviewDecision.route === "needs_user"
                ...
                : finalReviewDecision.route === "test_agent_recheck"
                    ? "增量复验后证据仍未闭环"
                    : `${maxAcceptanceReviewRounds}轮验收后仍有实现缺口`;
        ...
        const finalAcceptance = {
            ...
            accepted,
            mode: independentTestAgentEnabled ? "test_agent" : "main_agent_self_verification",
        ...
        const finalTask = (0, collaboration_task_service_1.updateTask)(taskId, {
            ...
            delivery_summary: {
                ...
                acceptance_mode: independentTestAgentEnabled ? "test_agent" : "main_agent_self_verification",
                acceptance_level: independentTestAgentEnabled && acceptancePolicy.schema === "ccm-task-acceptance-policy-snapshot-v3" && acceptancePolicy.level === "standard" ? "strict" : acceptancePolicy.schema === "ccm-task-acceptance-policy-snapshot-v3" ? acceptancePolicy.level : independentTestAgentEnabled ? "strict" : "standard",
                acceptance_route: independentTestAgentEnabled ? "independent_test_agent" : acceptancePolicy.schema === "ccm-task-acceptance-policy-snapshot-v3" ? acceptancePolicy.route : "main_agent_self_verification",
                acceptance_reasons: (getProjectMainTask(taskId) || input.task)?.acceptance_escalation_reasons || (acceptancePolicy.schema === "ccm-task-acceptance-policy-snapshot-v3" ? acceptancePolicy.reasons : []),
                main_agent_final_acceptance: finalAcceptance,
            },
            project_main_execution: {
                schema: "ccm-project-main-execution-v1",
...
function projectMainTaskPublic(task) {
    ...
    return {
        ...
        acceptance_level: task.acceptance_level || task.acceptance_policy_snapshot?.level || null,
        acceptance_route: task.acceptance_route || task.acceptance_policy_snapshot?.route || null,
        acceptance_reasons: Array.isArray(task.acceptance_reasons) ? task.acceptance_reasons : [],
        acceptance_actual_level: task.acceptance_actual_level || null,
        acceptance_actual_route: task.acceptance_actual_route || null,
        acceptance_escalation_reasons: Array.isArray(task.acceptance_escalation_reasons) ? task.acceptance_escalation_reasons : [],
        acceptance_escalation: task.acceptance_escalation || null,
        acceptance_review_round: Math.max(0, Number(task.review_round_total || task.review_round || 0)),
...
function runProjectMainAgentContractSelfTest() {
    ...
    const structuredItems = normalizedWorkItems([{
            id: "structured",
            task: { subject: "修改 deliveryMarker", description: "只修改 src/feature.js" },
            acceptanceCriteria: [{ criterion: { text: "deliveryMarker 等于 ready" } }],
    ...
    const mixedProcessRequestEvidence = directDispatchAcceptanceEvidence({
        value: null,
        userMessage: "我明确授权你执行任务：把 src/feature.js 的 deliveryMarker 改成 ready；必须由项目子 Agent 修改，运行 npm test，交给 TestAgent 独立复核并通过 Terminal Gate。",
        project: "demo",
        workItems: normalizedWorkItems([{ task: "把 src/feature.js 的 deliveryMarker 改成 ready", files: ["src/feature.js"], acceptanceCriteria: ["src/feature.js 的 deliveryMarker 等于 ready"] }], "fallback"),
        decision,
    });
    ...
    return {
        ...
        checks: {
            ...
            processRequirementsRemainOutsideTestCriteria: mixedProcessRequestEvidence.length === 1
                && !/TestAgent|Terminal Gate|项目子 Agent|授权/.test(mixedProcessRequestEvidence[0].criterion)
                && /deliveryMarker.*ready/i.test(mixedProcessRequestEvidence[0].criterion),
        },
    };
}
