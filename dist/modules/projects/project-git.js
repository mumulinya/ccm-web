...
exports.inspectProjectGitQuick = inspectProjectGitQuick;
 ...
 */
function inspectProjectGitQuick(workDir, timeoutMs = 1_500) {
    const resolved = path.resolve(String(workDir || ""));
    const gitAvailable = commandExists("git");
    const base = {
        git_available: gitAvailable,
    ...
    if (!gitAvailable || !fs.existsSync(resolved) || !fs.statSync(resolved).isDirectory())
        return base;
    const boundedTimeout = Math.max(250, Math.min(3_000, Number(timeoutMs || 1_500)));
    if (!runGit(resolved, ["rev-parse", "--is-inside-work-tree"], boundedTimeout).ok)
        return base;
    base.is_repository = true;
    const branch = runGit(resolved, ["branch", "--show-current"], boundedTimeout);
    const status = runGit(resolved, ["status", "--porcelain=v1", "--untracked-files=normal"], boundedTimeout);
    base.branch = branch.ok ? branch.stdout : "";
    if (!status.ok)
        return base;
    const lines = status.stdout.split(/\r?\n/).filter(Boolean);
