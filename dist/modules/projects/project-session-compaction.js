...
function projectCompactionSourceIdentity(data) {
    return {
        messageIds: (Array.isArray(data?.history) ? data.history : []).map((item, index) => String(item?.id || item?.messageId || `message-${index}`)),
        executionIds: projectExecutionEvents(data).map((item, index) => String(item?.id || `execution-${index}`)),
        generation: Number(data?.unifiedSessionCompaction?.boundaryGeneration || 0),
    };
}
function projectCompactionPrefixStable(acquired, current) {
    const latest = projectCompactionSourceIdentity(current);
    return latest.generation === acquired.generation
        && acquired.messageIds.every((id, index) => latest.messageIds[index] === id)
        && acquired.executionIds.every((id, index) => latest.executionIds[index] === id);
}
async function runUnifiedProjectSessionCompaction(project, projectSessionId, options = {}) {
    ...
    const initialGeneration = Number(initialData?.unifiedSessionCompaction?.boundaryGeneration || 0);
    const canonical = (0, canonical_context_accounting_1.readCanonicalContextAccounting)("project", safeProject, safeSessionId);
    ...
    const adapter = createProjectSessionCompactionAdapter({
        ...
        acquire: () => {
            ...
            acquiredIdentity = projectCompactionSourceIdentity(data);
        ...
        load: () => {
            ...
            return {
                ...
                rollingSessionMemory: data.rollingSessionMemory || state.sessionMemoryState || null,
                previousState: state,
        ...
        validate: () => {
            ...
            if (!projectCompactionPrefixStable(acquiredIdentity, current))
        ...
        commit: (result, fence) => {
            const data = JSON.parse(fs.readFileSync(file, "utf8"));
            if (!projectCompactionPrefixStable(acquiredIdentity, data))
            ...
            const preservedIds = (result.partialCompaction?.preservedMessageIds || result.preservedRecentWindow.messages.map((item) => String(item?.id || ""))).map(String).filter(Boolean);
            const summarizedCount = result.partialCompaction?.direction === "from" ? 0 : Number(result.preservedRecentWindow.startIndex || 0);
            const summarizedThroughMessageId = summarizedCount > 0 ? String(data.history?.[summarizedCount - 1]?.id || "") : "";
            ...
            const state = (0, unified_session_compaction_1.buildUnifiedSessionCompactionStateV1)({ receipt: result.receipt, summaryQuality: result.summaryQuality, microCompact: result.microCompact, recoveryContext: result.recoveryContext, triggerReason: options.reason || "request_preflight", summarizedThroughMessageId, summarizedMessageCount: summarizedCount, preservedRecentMessageIds: preservedIds, compactionMode: result.compactionMode, partialCompaction: result.partialCompaction });
            try {
                data.unifiedSessionSummary = summary;
                data.unifiedSessionCompaction = state;
                data.rollingSessionMemory = null;
                data.unifiedRecoveryContext = result.recoveryContext;
                data.unifiedSessionBoundary = { summarizedMessageCount: summarizedCount, summarizedThroughMessageId, preservedRecentMessageIds: preservedIds, compactionMode: result.compactionMode || "full", partialCompaction: result.partialCompaction || null, checksum: result.receipt.checksum };
    ...
    const engine = (0, unified_session_compaction_1.createUnifiedSessionCompactionEngine)({
        ...
        buildPostCompactPayload: ({ summary, preservedTimeline, recoveryContext }) => (0, session_compaction_core_1.buildModelVisiblePayloadSnapshot)({ scope: "project", sessionId: `${safeProject}:${safeSessionId}`, system: options.fixedContext || { project: safeProject, provider: modelCapacity.provider, model: modelCapacity.model }, tools: options.tools || null, activeSummary: summary, recentMessages: preservedTimeline, currentRequest: options.currentRequest || null, recoveryContext, hookResults: [], contextComponents: options.contextComponents || options.context_components || {}, mainAgentContextEnvelope: options.modelVisiblePayload?.mainAgentContextEnvelope, mainAgentCapabilityDirectory: options.modelVisiblePayload?.mainAgentCapabilityDirectory }),
        measure: (payload) => Number(payload?.totalTokens || (0, context_budget_1.estimateTextTokens)(JSON.stringify(payload || {}))),
        // Project identity is enforced by the adapter fence and recovery context;
        // it is not a semantic fact the model must repeat in every summary. Using
        // safeProject as a hidden durable anchor rejected otherwise valid short
        // conversation summaries with durable_anchor_missing.
    ...
    return {
        ...
        sessionStartHookContext: result.sessionStartHookContext || null,
...
function getProjectSessionCompactionActivity(project, projectSessionId) {
    ...
    return scopeId ? (0, session_compaction_runs_1.getSessionCompactionRunActivity)("project", `${(0, project_validation_1.validateProjectName)(project)}:${(0, project_validation_1.validateSessionId)(projectSessionId)}`) : (0, session_compaction_runs_1.getSessionCompactionRunActivity)("project", "");
}
...
function scheduleProjectSessionMemoryExtraction(project, projectSessionId, options = {}) {
    ...
    const data = JSON.parse(fs.readFileSync(file, "utf8"));
    const history = Array.isArray(data.history) ? data.history.filter((message) => ["user", "assistant"].includes(String(message?.role || ""))) : [];
    const previous = data.rollingSessionMemory || data.unifiedSessionCompaction?.sessionMemoryState || null;
    const generation = Number(data.unifiedSessionCompaction?.boundaryGeneration || 0);
    const tokenBasisSelection = (0, session_memory_token_basis_1.selectCanonicalSessionMemoryTokenBasis)((0, canonical_context_accounting_1.readCanonicalContextAccounting)("project", safeProject, safeSessionId), { scope: "project", scopeId: safeProject, exactSessionId: safeSessionId, boundaryGeneration: generation });
    const cadence = (0, session_compaction_core_1.evaluateSessionMemoryCadence)(history, previous ? {
    ...
    const scheduled = (0, session_compaction_core_1.scheduleSessionMemoryExtraction)({
        ...
        commit: (rolling, expected) => {
            if (!rolling || !fs.existsSync(file))
                return null;
            const latest = JSON.parse(fs.readFileSync(file, "utf8"));
            if (Number(latest.unifiedSessionCompaction?.boundaryGeneration || 0) !== expected.generation || projectCompactionSourceChecksum(latest) !== expected.sourceChecksum)
                return null;
            latest.rollingSessionMemory = rolling;
            latest.unifiedSessionCompaction = {
                ...(latest.unifiedSessionCompaction || {}),
...
async function compactProjectSessionWithModel(project, projectSessionId, options = {}) {
    const safeProject = (0, project_validation_1.validateProjectName)(project);
    const safeSessionId = (0, project_validation_1.validateSessionId)(projectSessionId);
    const exactSessionId = `${safeProject}:${safeSessionId}`;
    const admission = (0, session_compaction_runs_1.startSessionCompactionRun)({ scope: "project", exactSessionId, runId: options.compactionRunId, reason: options.reason, signal: options.signal });
    try {
        return await runUnifiedProjectSessionCompaction(safeProject, safeSessionId, { ...options, signal: admission.run.controller.signal, compactionRunId: admission.run.runId });
    }
    finally {
        (0, session_compaction_runs_1.finishSessionCompactionRun)("project", exactSessionId, admission.run.runId);
    }
}
...
function buildProjectSessionPostCompactContext(project, projectSessionId, targetAgentType = "", options = {}) {
    ...
    if (binding.status === "open"
        ...
        return "";
    const projection = buildProjectSessionModelContextProjection(project, projectSessionId, { ...options, consumeSessionStartHookContext: true });
    if (!projection)
        return "";
