...
function normalizeWebSessionMessage(message) {
    ...
    for (const key of [
        ...
        "task_id",
        "run_id",
        // Keep the immutable execution identity on persisted task cards so the
        // replay projection can attach pre-card assistant narration/tool events
...
function upsertProjectSessionTaskMessage(projectName, sessionId, message) {
    ...
    // Keep the original user intake, but collapse duplicate Assistant task cards
    // produced by retries/startup compensation into the one canonical card.
    // Ordinary Assistant progress messages do not carry a task-card marker and
    // therefore remain untouched.
    let storedMessage = existingIndex >= 0 ? data.history[existingIndex] : data.history[data.history.length - 1];
    if (taskId && normalized.role === "assistant") {
        const canonicalId = String(storedMessage?.id || normalized.id || "");
        data.history = data.history.filter((item) => {
            if (String(item?.id || "") === canonicalId)
                return true;
            if (item?.role !== "assistant")
                return true;
            if (String(item?.task_id || item?.taskExperience?.task_id || "") !== taskId)
                return true;
            const isTaskCard = item?.messageMode === "task"
                || item?.taskExperience?.requires_card === true
                || ["project_main_task", "global_task_dispatch_queued", "task_recovery_link"].includes(String(item?.type || ""));
            return !isTaskCard;
        });
        storedMessage = data.history.find((item) => String(item?.id || "") === canonicalId) || storedMessage;
    }
    (0, session_task_timeline_1.recordSessionTimelineMessage)({ exactSessionId: safeSessionId, scope: "project", scopeId: safeProject, role: normalized.role, messageId: normalized.id, taskId: normalized.task_id || normalized.taskId || normalized.taskExperience?.task_id, timestamp: normalized.timestamp });
    data.updated_at = new Date().toISOString();
    fs.writeFileSync(filePath, JSON.stringify(data, null, 2));
    ...
    if (pathname === "/api/sessions/message/delete" && req.method === "POST") {
        ...
        req.on("end", () => {
            try {
                ...
                if (deleted > 0) {
                    clearProjectMainDynamicContext(project, sessionId);
                    (0, post_turn_tool_context_compaction_1.deletePostTurnToolContextState)("project", project, sessionId);
                    (0, pre_request_tool_context_1.deletePreRequestToolContextState)("project", project, sessionId);
                    delete data.compaction;
    ...
    if (pathname === "/api/sessions/clear" && req.method === "POST") {
        ...
        req.on("end", () => {
            try {
                ...
                data.history = [];
                data.execution_history = [];
                delete data.rollingSessionMemory;
                delete data.unifiedSessionSummary;
                delete data.unifiedSessionCompaction;
                delete data.unifiedRecoveryContext;
                delete data.unifiedSessionBoundary;
                delete data.unifiedSessionCompactionFailure;
                clearProjectMainDynamicContext(project, sessionId);
                (0, post_turn_tool_context_compaction_1.deletePostTurnToolContextState)("project", project, sessionId);
                (0, pre_request_tool_context_1.deletePreRequestToolContextState)("project", project, sessionId);
                delete data.compaction;
        ...
        req.on("end", async () => {
            try {
                ...
                (0, utils_1.sendJson)(res, {
                    ...
                    summary_mode: "model_required",
                    compacted: result?.compacted === true,
                    reason: String(result?.reason || "manual_slash_compact"),
                    mode: result?.compactionMode || "full",
                    partial_compaction: result?.partialCompaction || null,
                    before_tokens: Math.max(0, Number(result?.receipt?.beforeTokens || 0)),
                    after_tokens: Math.max(0, Number(result?.receipt?.afterTokens || 0)),
                    next_generation: Math.max(0, Number(result?.boundaryGeneration || result?.receipt?.boundaryGeneration || 0)),
                    ...
                    hook_results: (0, session_compaction_command_hooks_1.projectSessionCompactionHookResults)(result?.hookResults),
                    session_start_hook_context: result?.sessionStartHookContext ? {
                        ...result.sessionStartHookContext,
                        contentStored: false,
                    } : null,
                });
            }
    ...
    if (pathname === "/api/sessions/delete" && req.method === "POST") {
        ...
        req.on("end", () => {
            try {
                ...
                if (!fs.existsSync(filePath))
                    return (0, utils_1.sendJson)(res, { error: "会话不存在" }, 404);
                (0, session_compaction_runs_1.cancelSessionCompactionRun)({ scope: "project", exactSessionId: `${project}:${sessionId}`, reason: "项目会话已删除" });
                (0, session_start_hook_context_1.clearSessionStartHookContext)("project", `${project}:${sessionId}`);
