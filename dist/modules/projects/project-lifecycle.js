...
exports.inspectProjectDirectoryState = inspectProjectDirectoryState;
...
exports.previewOrphanProjectRemoval = previewOrphanProjectRemoval;
exports.removeOrphanProject = removeOrphanProject;
...
function buildPurgeImpact(name, mode = "archived", affectedGroupIds = []) {
    ...
    const fingerprint = crypto.createHash("sha256").update(JSON.stringify({ mode, items, groupIds })).digest("hex");
    return {
        project,
        mode,
        items,
...
function inspectProjectDirectoryState(name) {
    const project = (0, project_validation_1.validateProjectName)(name);
    const activeConfig = configFile(project);
    if (!fs.existsSync(activeConfig))
        return { status: "project_missing", work_dir_configured: false };
    const workDir = String((0, db_1.getConfigInfo)(activeConfig)[0]?.workDir || "").trim();
    if (!workDir)
        return { status: "unconfigured", work_dir_configured: false };
    try {
        const stat = fs.statSync(workDir);
        return stat.isDirectory()
            ? { status: "available", work_dir_configured: true }
            : { status: "missing", work_dir_configured: true };
    }
    catch (error) {
        const code = String(error?.code || "");
        if (code === "ENOENT" || code === "ENOTDIR")
            return { status: "missing", work_dir_configured: true };
        return { status: "unavailable", work_dir_configured: true };
    }
}
...
function previewProjectPurge(name) {
    ...
    purgePreviews.set(token, { project: impact.project, fingerprint: impact.fingerprint, expiresAt, mode: "archived" });
    return { success: true, ...impact, preview_token: token, expires_at: new Date(expiresAt).toISOString() };
}
function previewOrphanProjectRemoval(name, affectedGroupIds = []) {
    const directory = inspectProjectDirectoryState(name);
    if (directory.status !== "missing") {
        throw new Error(directory.status === "unavailable"
            ? "项目目录当前无法确认是否存在，请恢复目录访问或改用归档"
            : "仅项目目录已不存在的活动项目可以直接从 CCM 移除");
    }
    const impact = buildPurgeImpact(name, "orphan", affectedGroupIds);
    if (!fs.existsSync(configFile(impact.project)))
        throw new Error("活动项目不存在或已经归档");
    const token = crypto.randomBytes(24).toString("hex");
    const expiresAt = Date.now() + PREVIEW_TTL_MS;
    purgePreviews.set(token, { project: impact.project, fingerprint: impact.fingerprint, expiresAt, mode: "orphan" });
    return {
        success: true,
        project: impact.project,
        mode: impact.mode,
        items: impact.items.map(({ label, exists, bytes, modified_at, kind }) => ({ label, exists, bytes, modified_at, kind })),
...
function removeOrphanProject(name, previewToken, affectedGroupIds = []) {
    const directory = inspectProjectDirectoryState(name);
    if (directory.status !== "missing")
        throw new Error("项目目录状态已变化，请重新检查后再操作");
    const impact = buildPurgeImpact(name, "orphan", affectedGroupIds);
    const preview = purgePreviews.get(String(previewToken || ""));
    if (!preview || preview.mode !== "orphan" || preview.project !== impact.project || preview.expiresAt < Date.now()) {
        throw new Error("移除预览已失效，请重新预览");
    }
    if (preview.fingerprint !== impact.fingerprint)
        throw new Error("项目或群聊关联已变化，请重新预览后再移除");
    if (!fs.existsSync(configFile(impact.project)))
        throw new Error("活动项目不存在或已经归档");
    ...
    const audit = recordAudit("remove_orphan", impact.project, {
        deleted: impact.items.filter((item) => item.exists).map(({ label, bytes, kind }) => ({ label, bytes, kind })),
        detached_groups: impact.affected_group_ids,
        retained: impact.retained,
    });
    return {
        success: true,
        ...
        project: impact.project,
        audit_id: audit.id,
        detached_group_ids: impact.affected_group_ids,
        retained: impact.retained,
        message: "项目已从 CCM 移除",
    };
}
