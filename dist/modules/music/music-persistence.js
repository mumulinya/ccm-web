...
exports.ensureMusicPersistenceMigrated = ensureMusicPersistenceMigrated;
exports.listPersistedMusicCommands = listPersistedMusicCommands;
exports.replacePersistedMusicCommandsForTest = replacePersistedMusicCommandsForTest;
exports.enqueuePersistedMusicCommand = enqueuePersistedMusicCommand;
exports.peekPersistedMusicCommand = peekPersistedMusicCommand;
exports.claimPersistedMusicCommand = claimPersistedMusicCommand;
exports.heartbeatPersistedMusicCommand = heartbeatPersistedMusicCommand;
exports.completePersistedMusicCommand = completePersistedMusicCommand;
exports.requeuePersistedMusicCommand = requeuePersistedMusicCommand;
exports.readPersistedLibraryState = readPersistedLibraryState;
exports.mutatePersistedLibraryState = mutatePersistedLibraryState;
exports.upsertPersistedDownloadJob = upsertPersistedDownloadJob;
exports.listPersistedDownloadJobs = listPersistedDownloadJobs;
exports.deletePersistedDownloadJob = deletePersistedDownloadJob;
...
exports.listMusicCatalogTracks = listMusicCatalogTracks;
...
const COMMANDS_V2_FILE = path.join(utils_1.CCM_DIR, "music-remote-commands.json");
const COMMAND_V1_FILE = path.join(utils_1.CCM_DIR, "music-remote-command.json");
const LIBRARY_V3_FILE = path.join(utils_1.CCM_DIR, "music-library-state.json");
const DOWNLOADS_V1_FILE = path.join(utils_1.CCM_DIR, "music-download-jobs.json");
...
function insertCommand(db, command, ignore = false) {
  ...
  `).run(command.id, command.type || "play", command.keyword || "", command.request_text || command.requestText || command.keyword || "", command.mode || "auto", command.source || "", command.status || "pending", Number(command.generation || 1), Number(command.attempts || 0), JSON.stringify(command.decision || {}), JSON.stringify(command.origin || {}), command.lease_id || "", Number(command.fencing_token || 0), command.claimed_at || null, command.lease_expires_at || null, JSON.stringify(command.result || {}), command.last_error || "", command.created_at || now(), command.terminal_at || null, command.updated_at || now());
}
function legacyCommands() {
    ...
    try {
        if (fs.existsSync(COMMAND_V1_FILE)) {
            ...
            if (value?.id && value?.keyword && !value?.consumed)
                return [value];
        }
    }
    catch { }
...
function ensureMusicPersistenceMigrated() {
    if (!(0, observability_database_1.observabilityMeta)("music.persistence.v3.commands-imported", false)) {
        (0, observability_database_1.withImmediateObservabilityTransaction)(db => {
            ...
            if (!existing) {
                legacyCommands().forEach((item, index) => insertCommand(db, {
                    ...item,
                    id: String(item.id),
        ...
        (0, observability_database_1.setObservabilityMeta)("music.persistence.v3.commands-imported", { at: now() });
    }
    ensureLibraryRow();
    ensureDownloadJobsImported();
}
function expireCommandLeases(db, leaseMs = 15_000) {
...
function listPersistedMusicCommands(limit = 200) {
    ...
    `).all(Math.max(1, Math.min(1000, limit))).map(commandFromRow);
    });
}
function replacePersistedMusicCommandsForTest(commands) {
    ensureMusicPersistenceMigrated();
    (0, observability_database_1.withImmediateObservabilityTransaction)(db => {
...
function enqueuePersistedMusicCommand(input) {
    ensureMusicPersistenceMigrated();
    return (0, observability_database_1.withImmediateObservabilityTransaction)(db => {
        expireCommandLeases(db);
...
function peekPersistedMusicCommand() {
    ensureMusicPersistenceMigrated();
    return (0, observability_database_1.withImmediateObservabilityTransaction)(db => {
        expireCommandLeases(db);
...
function claimPersistedMusicCommand(input) {
    ensureMusicPersistenceMigrated();
    return (0, observability_database_1.withImmediateObservabilityTransaction)(db => {
        expireCommandLeases(db);
        const row = input.id
        ...
        if (!row || !["ready", "pending"].includes(String(row.status)))
            return null;
        if (input.generation && Number(input.generation) !== Number(row.generation))
            return null;
...
function heartbeatPersistedMusicCommand(input) {
    ensureMusicPersistenceMigrated();
    return (0, observability_database_1.withImmediateObservabilityTransaction)(db => {
        expireCommandLeases(db);
        ...
        if (input.lease_id && input.lease_id !== row.lease_id)
            return { success: false, error: "播放租约不匹配" };
        if (input.fencing_token && Number(input.fencing_token) !== Number(row.fencing_token))
            return { success: false, error: "播放fencing token已失效" };
        if (!["claimed", "playing", "needs_user_gesture"].includes(String(row.status)))
            return { success: false, error: `当前状态不能续租：${row.status}` };
...
function completePersistedMusicCommand(input) {
    ensureMusicPersistenceMigrated();
    return (0, observability_database_1.withImmediateObservabilityTransaction)(db => {
        const row = db.prepare("SELECT * FROM music_playback_commands_v3 WHERE command_id = ?").get(String(input.id || ""));
        if (!row)
        ...
        if (TERMINAL_COMMAND_STATES.includes(String(row.status))) {
            if (row.status === input.status)
                return { success: true, duplicate: true, command: commandFromRow(row) };
            return { success: false, error: "播放指令已经进入不可修改的终态" };
        }
        const allowed = ["completed", "failed", "needs_user_gesture", "superseded", "cancelled"];
        if (!allowed.includes(String(input.status)))
            return { success: false, error: "无效播放终态" };
...
function requeuePersistedMusicCommand(input) {
    ensureMusicPersistenceMigrated();
    return (0, observability_database_1.withImmediateObservabilityTransaction)(db => {
...
function ensureLibraryRow() {
    ...
    if (db.prepare("SELECT 1 FROM music_library_state_v4 WHERE workspace_id=?").get(WORKSPACE_ID))
        return;
...
function readPersistedLibraryState() {
    ensureLibraryRow();
    const row = (0, observability_database_1.getObservabilityDatabase)().prepare("SELECT * FROM music_library_state_v4 WHERE workspace_id=?").get(WORKSPACE_ID);
...
function mutatePersistedLibraryState(mutator, expectedRevision) {
    ensureLibraryRow();
    return (0, observability_database_1.withImmediateObservabilityTransaction)(db => {
        ...
        if (expectedRevision !== undefined && Number(expectedRevision) !== revision) {
            const error = new Error("音乐库状态已经变化，请刷新后重试");
...
function ensureDownloadJobsImported() {
    if ((0, observability_database_1.observabilityMeta)("music.persistence.v2.downloads-imported", false))
        return;
    (0, observability_database_1.withImmediateObservabilityTransaction)(db => {
        ...
        if (!count) {
            try {
                ...
                for (const job of Array.isArray(rows) ? rows : [])
                    upsertPersistedDownloadJob(job, db);
            }
            catch { }
        }
    });
    (0, observability_database_1.setObservabilityMeta)("music.persistence.v2.downloads-imported", { at: now() });
}
function upsertPersistedDownloadJob(job, suppliedDb) {
    const db = suppliedDb || (0, observability_database_1.getObservabilityDatabase)();
    db.prepare(`
  ...
  `).run(job.id, job.source, String(job.sourceId), job.status, job.requestedQuality || job.quality || "high", job.actualQuality || "", job.leaseId || "", Number(job.fencingToken || 0), job.leaseExpiresAt || null, job.commandId || "", job.consumerKind || "manual", job.checkpoint || job.phase || "", JSON.stringify(job), job.createdAt || now(), job.updatedAt || now());
    return job;
}
function listPersistedDownloadJobs() {
    ensureMusicPersistenceMigrated();
  ...
  `).all().map(row => parseJson(row.job_json, null)).filter(Boolean);
}
function deletePersistedDownloadJob(id) {
    ensureMusicPersistenceMigrated();
    return (0, observability_database_1.getObservabilityDatabase)().prepare("DELETE FROM music_download_jobs_v2 WHERE job_id=?").run(id).changes > 0;
}
function findMusicMediaAsset(source, sourceId) {
    return (0, observability_database_1.getObservabilityDatabase)().prepare(`
    SELECT * FROM music_media_assets_v2 WHERE source=? AND source_id=?
  `).get(source, String(sourceId));
}
function findMusicMediaAssetByChecksum(checksum) {
    return (0, observability_database_1.getObservabilityDatabase)().prepare(`
    SELECT * FROM music_media_assets_v2 WHERE file_checksum=? LIMIT 1
  `).get(checksum);
}
function upsertMusicMediaAsset(asset) {
    const timestamp = now();
...
function setActiveMusicCatalogGeneration(generation) {
    (0, observability_database_1.setObservabilityMeta)("music.catalog.active-generation", generation);
}
function getMusicCatalogStatus() {
    const db = (0, observability_database_1.getObservabilityDatabase)();
...
function listMusicCatalogTracks(input = {}) {
    const db = (0, observability_database_1.getObservabilityDatabase)();
    const generation = activeMusicCatalogGeneration();
    if (!generation)
        return { tracks: [], total: 0, nextCursor: null, generation: 0 };
  ...
  `).all(generation, ...args, limit, cursor);
    return {
        generation,
        total,
        tracks: rows,
...
//# sourceMappingURL=music-persistence.js.map