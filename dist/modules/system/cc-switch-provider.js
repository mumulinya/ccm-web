...
function loadCcSwitchClaudeProvider() {
    ...
    const fileProvider = fromEnvironment(settings?.env, "external-file", "external-claude", "外部 Claude 配置");
    if (fileProvider)
        return fileProvider;
    // Headless runners (release gates, isolated CCM instances and service
    // managers) often provide Claude Code credentials through the process
    // environment instead of materialising a settings file in the isolated
    // home. Keep this as a final, non-persistent fallback; callers still redact
    // credentials from public settings and reports.
    return fromEnvironment(process.env, "external-file", "external-claude-env", "环境变量 Claude 配置");
}
