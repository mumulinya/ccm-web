...
exports.resolveClaudeCodeCommand = resolveClaudeCodeCommand;
exports.resolveCursorAgentCommand = resolveCursorAgentCommand;
exports.resolveAntigravityCliCommand = resolveAntigravityCliCommand;
...
exports.buildAgentProviderLoginSpec = buildAgentProviderLoginSpec;
exports.buildAgentProviderInstallSpec = buildAgentProviderInstallSpec;
exports.startAgentProviderInstall = startAgentProviderInstall;
exports.getAgentProviderInstallJobs = getAgentProviderInstallJobs;
exports.parseAntigravityModels = parseAntigravityModels;
exports.buildAgentProviderTestSpec = buildAgentProviderTestSpec;
exports.summarizeAgentProviderTestFailure = summarizeAgentProviderTestFailure;
exports.parseAgentProviderTestOutput = parseAgentProviderTestOutput;
exports.testAgentProvider = testAgentProvider;
exports.parseGeminiCliDefaultModels = parseGeminiCliDefaultModels;
...
const development_agent_auth_evidence_1 = require("./development-agent-auth-evidence");
const SETTINGS_FILE = path.join(os.homedir(), ".ccm", "agent-provider-settings.json");
...
const INSTALL_STATE_FILE = path.join(os.homedir(), ".ccm", "agent-install-jobs.json");
...
function ensureInstallStatesLoaded() {
    ...
    try {
        ...
        for (const [provider, state] of Object.entries(parsed?.jobs || {})) {
            ...
            INSTALL_STATES.set(provider, job);
        }
    }
    catch { }
}
function setInstallState(provider, state) {
...
const LOGIN_PREPARATION_TIMERS = new Map();
const runtimeRequire = (0, module_1.createRequire)(__filename);
let cachedLoginPtyModule;
...
function normalizeAntigravityModelId(value) {
    const model = String(value || "").trim();
    if (model === "gemini-3.5-flash")
        return "gemini-3.5-flash-high";
    if (["gemini-3.1-pro", "gemini-3.1-pro-preview"].includes(model))
...
function saveAgentProviderSettings(updates) {
    ...
    if (updates?.gemini?.model !== undefined)
        next.gemini.model = normalizeAntigravityModelId(updates.gemini.model);
        ...
        if (claude.apiUrl !== undefined) {
            ...
            if (apiUrl) {
                ...
                try {
                    ...
                    throw new Error("Claude Code API 地址格式无效");
                }
    ...
    if (updates?.claudecode)
        (0, development_agent_auth_evidence_1.revokeDevelopmentAgentAuthEvidence)("claudecode", "Agent配置已修改，需要重新验证");
...
/**
 * Resolve the Claude Code launcher even when npm has materialized its Windows
 * shim under a temporary `.claude.cmd-*` name.  npm can do this while another
 * process owns the stable shim; treating the CLI as missing in that case
 * makes a configured Claude runtime fail its readiness probe unnecessarily.
 */
function resolveClaudeCodeCommand() {
    const explicit = String(process.env.CCM_CLAUDE_CODE_COMMAND || process.env.CCM_CLAUDE_COMMAND || "").trim();
    if (explicit && commandExists(explicit))
        return explicit;
    if (commandExists("claude"))
        return resolveWindowsCommandPath("claude");
    if (process.platform === "win32") {
        ...
        for (const directory of pathEntries) {
            try {
                const bundledBinary = path.join(directory, "node_modules", "@anthropic-ai", "claude-code", "bin", "claude.exe");
                if (commandExists(bundledBinary))
                    return bundledBinary;
                const names = fs.readdirSync(directory);
                const candidate = names
                    .filter(name => /^(?:\.claude\.cmd|claude\.cmd|claude\.exe)(?:[-_][A-Za-z0-9]+)?$/i.test(name))
                    .sort((a, b) => {
                    const aStable = /^claude\.(?:cmd|exe)$/i.test(a) ? 0 : 1;
                    const bStable = /^claude\.(?:cmd|exe)$/i.test(b) ? 0 : 1;
                    return aStable - bStable || a.localeCompare(b);
                })[0];
                if (candidate) {
                    const resolved = path.join(directory, candidate);
                    if (commandExists(resolved))
                        return resolved;
                }
            }
            catch { }
...
function resolveWindowsCommandPath(command) {
    ...
    if (/[\\/]/.test(command))
        return command;
    try {
        const probe = (0, child_process_1.spawnSync)("where.exe", [command], {
            windowsHide: true,
            encoding: "utf-8",
            timeout: 5_000,
        });
...
function resolveCursorAgentCommand() {
    if (commandExists("cursor-agent"))
        return resolveWindowsCommandPath("cursor-agent");
    if (commandExists("agent"))
        return resolveWindowsCommandPath("agent");
    if (process.platform === "win32") {
...
function resolveAntigravityCliCommand() {
    ...
    if (commandExists("agy"))
        return resolveWindowsCommandPath("agy");
    if (process.platform === "win32") {
        const localAppData = String(process.env.LOCALAPPDATA || path.join(os.homedir(), "AppData", "Local"));
...
function antigravityCredentialPresent() {
    const evidence = (0, development_agent_auth_evidence_1.getDevelopmentAgentAuthEvidence)("gemini");
    if (evidence?.valid)
        return true;
    const roots = [
...
function credentialFileFingerprint(file) {
    try {
...
function providerCredentialFingerprint(provider) {
...
function geminiCredentialSources() {
    const sources = [];
    if (fs.existsSync(path.join(os.homedir(), ".gemini", "antigravity")))
        sources.push("antigravity_secure_state");
    if (fs.existsSync(path.join(os.homedir(), ".gemini", "antigravity-cli")))
        sources.push("antigravity_cli_settings");
    return sources;
}
...
function collectAgentProviderProbes() {
    ...
    return {
        ...
        claudeInstalled: commandExists(claudeCommand),
        versions: {
            ...
            claude: commandVersion(claudeCommand),
        },
    };
}
async function collectAgentProviderProbesAsync() {
    ...
    const [cursorInstalled, codexInstalled, geminiInstalled, openCodeInstalled, claudeInstalled] = await Promise.all([
        ...
        commandExistsAsync(claudeCommand),
    ]);
    ...
    const [cursor, codexVersion, cursorVersion, geminiVersion, openCodeVersion, claudeVersion] = await Promise.all([
        ...
        commandVersionAsync(claudeCommand, claudeInstalled),
    ]);
    return {
        cursorCommand,
        cursorInstalled,
...
function assembleAgentProviderStatuses(probes) {
    ...
    const authState = (provider, detected, account, version, model = "") => {
        let evidence = (0, development_agent_auth_evidence_1.getDevelopmentAgentAuthEvidence)(provider, { account, cliVersion: version, model });
        if (detected && !evidence?.valid && provider !== "gemini") {
            evidence = (0, development_agent_auth_evidence_1.recordDevelopmentAgentAuthEvidence)({
                provider,
                status: "verified",
        ...
        return {
            ...
            authEvidence: (0, development_agent_auth_evidence_1.publicDevelopmentAgentAuthEvidence)(evidence),
        };
    };
    ...
    const currentCursorEvidence = (0, development_agent_auth_evidence_1.getDevelopmentAgentAuthEvidence)("cursor", {
    ...
    const cursorEvidence = probes.cursor.loggedIn
        ? currentCursorEvidence?.valid
            ...
            : (0, development_agent_auth_evidence_1.recordDevelopmentAgentAuthEvidence)({ provider: "cursor", status: "verified", source: "native_status", account: probes.cursor.account, cliVersion: probes.versions.cursor, model: config.cursor.model, detail: "Cursor CLI返回有效登录状态" })
        : (0, development_agent_auth_evidence_1.getDevelopmentAgentAuthEvidence)("cursor");
    const claudeEvidence = (0, development_agent_auth_evidence_1.getDevelopmentAgentAuthEvidence)("claudecode", { cliVersion: probes.versions.claude, model: effectiveClaude.model });
    return {
        codex: {
            provider: "codex",
            command: "codex",
        ...
        cursor: {
            ...
            authEvidence: (0, development_agent_auth_evidence_1.publicDevelopmentAgentAuthEvidence)(cursorEvidence),
            account: probes.cursor.account,
        ...
        claudecode: {
            provider: "claudecode",
            command: probes.claudeCommand || resolveClaudeCodeCommand(),
            ...
            authEvidence: (0, development_agent_auth_evidence_1.publicDevelopmentAgentAuthEvidence)(claudeEvidence),
...
function buildAgentProviderLoginSpec(provider, options = {}) {
    if (provider === "codex") {
    ...
    if (provider === "opencode") {
        ...
        if (providerId && !/^[A-Za-z0-9][A-Za-z0-9._-]{0,79}$/.test(providerId))
            throw new Error("OpenCode Provider ID格式无效");
        if (methodId && (methodId.length > 120 || /[\r\n\u0000]/.test(methodId)))
            throw new Error("OpenCode认证方式格式无效");
        return {
            command: "opencode",
...
function launchAntigravityInteractive(command) {
    if (process.env.CCM_ANTIGRAVITY_DISABLE_INTERACTIVE_LAUNCH === "1")
        return false;
    if (process.platform !== "win32")
        return false;
...
function buildAgentProviderInstallSpec(provider, installed) {
    if (provider === "codex") {
    ...
    if (provider === "gemini") {
        if (installed)
            return { command: resolveAntigravityCliCommand(), args: ["update"] };
        return process.platform === "win32"
            ? {
...
function formatInstallProcessError(provider, operation, spec, error) {
    const providerLabel = {
        codex: "Codex CLI",
        cursor: "Cursor Agent",
...
function getAgentProviderInstallJobs() {
    ensureInstallStatesLoaded();
    return Object.fromEntries(["codex", "cursor", "gemini", "opencode", "claudecode"].map(provider => [provider, installState(provider)]));
}
...
function parseAntigravityModels(output) {
    ...
    for (const line of stripTerminalFormatting(String(output || "")).split(/\r?\n/)) {
        const id = line.trim().split(/\s+/)[0] || "";
        if (!id || id.startsWith("[") || !/^[A-Za-z0-9][A-Za-z0-9._:/+@\-]{1,179}$/.test(id) || seen.has(id))
            continue;
        seen.add(id);
        models.push({ id, label: id });
    }
...
function summarizeAgentProviderTestFailure(stderrValue, stdoutValue = "", fallback = "Agent 未返回预期测试响应") {
...
function structuredAgentReplies(provider, rawOutput) {
    ...
    for (const line of output.split(/\r?\n/).map(value => value.trim()).filter(Boolean)) {
        ...
        const add = (candidate) => { if (typeof candidate === "string" && candidate.trim())
            replies.push(candidate.trim()); };
        if (provider === "codex" && value?.type === "item.completed" && value?.item?.type === "agent_message")
            add(value.item.text);
        if (provider === "cursor" && ["result", "assistant"].includes(String(value?.type || "")))
            add(value.result || value.text || value.message?.content);
        if (provider === "gemini")
            add(value.response || (value.type === "result" ? value.result : ""));
        if (provider === "opencode" && !["user", "input"].includes(String(value?.role || value?.type || "")))
            add(value.text || value.part?.text || value.message?.content);
        if (provider === "claudecode" && value?.type === "result")
            add(value.result);
    }
    return replies;
}
function parseAgentProviderTestOutput(rawOutput, selectedModel = "", providerValue = "codex", challenge = "") {
    const provider = String(providerValue || "codex").trim().toLowerCase();
    const output = stripTerminalFormatting(String(rawOutput || ""));
...
async function runAgentProviderTest(provider, model) {
    ...
    const runDir = path.join(os.homedir(), ".ccm", "run", "agent-provider-tests");
    fs.mkdirSync(runDir, { recursive: true });
    const promptFile = path.join(runDir, `${provider}-${(0, crypto_1.randomUUID)()}.txt`);
    const challenge = (0, crypto_1.randomUUID)().replace(/-/g, "");
    ...
    try {
        ...
        const detail = usable
            ...
            : execution.timedOut
                ? "Agent 测试超过 90 秒，已终止"
                : summarizeAgentProviderTestFailure(execution.stderr, execution.stdout, `Agent 退出码 ${execution.code ?? "unknown"}`);
        const result = {
            provider,
            usable,
            latencyMs,
        ...
        if (usable || provider === "claudecode") {
            (0, development_agent_auth_evidence_1.recordDevelopmentAgentAuthEvidence)({
                provider,
...
function parseGeminiCliDefaultModels(source) {
    const models = [];
    const seen = new Set();
    const add = (idValue, label) => {
        const id = safeIdentity(idValue);
        if (!id || id === "none" || /embedding/i.test(id) || seen.has(id))
            return;
        seen.add(id);
...
function geminiCliPackageRoot() {
    ...
    try {
        const npmRoot = (0, child_process_1.spawnSync)("npm", ["root", "--global"], {
            shell: process.platform === "win32",
            windowsHide: true,
            encoding: "utf-8",
            timeout: 8_000,
    ...
    try {
        ...
        candidates.push(path.join(path.dirname(realCommand), "node_modules", "@google", "gemini-cli"), path.resolve(path.dirname(realCommand), "..", "lib", "node_modules", "@google", "gemini-cli"));
        const marker = `${path.sep}@google${path.sep}gemini-cli${path.sep}`;
        const markerIndex = realCommand.indexOf(marker);
        if (markerIndex >= 0)
            candidates.push(realCommand.slice(0, markerIndex + marker.length - 1));
    }
    catch { }
...
function geminiCliAccountModels() {
    ...
    if (geminiCliModelCache?.key === cacheKey)
        return geminiCliModelCache.models;
    const sources = [];
    try {
        ...
        if (mainRelative) {
            ...
            for (const match of mainSource.matchAll(importPattern)) {
                if (!GEMINI_CLI_MODEL_EXPORTS.some(item => String(match[1] || "").includes(item.name)))
                    continue;
    ...
    for (const source of sources) {
        ...
        if (parsed.length > models.length)
            models = parsed;
        if (models.length === GEMINI_CLI_MODEL_EXPORTS.length)
            break;
    }
...
async function computeAgentProviderModels(providerValue) {
    const provider = String(providerValue || "").trim().toLowerCase();
...
const MODEL_CATALOG_CACHE = new Map();
const MODEL_CATALOG_IN_FLIGHT = new Map();
let modelCatalogGeneration = 0;
function invalidateAgentModelCatalog(provider) {
    modelCatalogGeneration += 1;
    if (!provider)
        MODEL_CATALOG_CACHE.clear();
    else
        for (const key of MODEL_CATALOG_CACHE.keys())
            if (key.startsWith(`${provider}\u0000`))
                MODEL_CATALOG_CACHE.delete(key);
}
function getAgentProviderModels(providerValue) {
    const provider = String(providerValue || "").trim().toLowerCase();
    const status = getAgentProviderStatuses()?.[provider];
    ...
    if (cached && cached.expiresAt > Date.now())
        return Promise.resolve({ ...cached.value, cached: true, generation: modelCatalogGeneration });
...
function loadLoginPtyModule() {
    if (process.env.CCM_DISABLE_NODE_PTY === "1")
        return null;
    if (cachedLoginPtyModule !== undefined)
...
function quarantineGeminiCredential(sessionId) {
...
function snapshotGeminiSuccessfulCredential(session) {
...
function settleGeminiSuccessfulCredential(session) {
...
function clearLoginPreparationTimer(sessionId) {
...
function markLoginVerified(provider) {
...
function completeLoginFromFreshCredential(session) {
    ...
    if (!session.providerReportedSuccess && !(session.provider === "gemini" && session.codeSubmitted))
        return false;
    if (session.provider === "gemini" && !snapshotGeminiSuccessfulCredential(session))
        return false;
...
function finalizeLoginProcess(session, code) {
    clearLoginPreparationTimer(session.id);
    ...
    for (const [id, session] of LOGIN_SESSIONS) {
        ...
        if (Number.isFinite(expiresAt) && expiresAt <= now && !["succeeded", "failed"].includes(session.status)) {
            ...
            if (session.credentialSuccessSnapshotPath) {
                try {
                    fs.unlinkSync(session.credentialSuccessSnapshotPath);
                }
                catch { }
                session.credentialSuccessSnapshotPath = undefined;
            }
...
function startAgentProviderLogin(providerValue, options = {}) {
    const provider = String(providerValue || "").trim().toLowerCase();
...
function logoutAgentProvider(providerValue) {
    ...
    if (provider === "gemini") {
        const command = resolveAntigravityCliCommand();
        if (!commandExists(command))
            throw new Error("Antigravity CLI尚未安装");
        const launched = launchAntigravityInteractive(command);
        (0, development_agent_auth_evidence_1.revokeDevelopmentAgentAuthEvidence)(provider, "等待在Antigravity CLI中切换或退出账号");
        invalidateAgentModelCatalog(provider);
        markAgentProviderStatusesStale();
        return {
            provider,
    ...
    markAgentProviderStatusesStale();
    (0, development_agent_auth_evidence_1.revokeDevelopmentAgentAuthEvidence)(provider, "Agent退出登录");
    invalidateAgentModelCatalog(provider);
    if (provider === "codex") {
...
function purgeManagedCodexAuthCopies() {
    const root = path.join(os.homedir(), ".ccm", "agent-runtime", "codex");
...
function isDevelopmentAgentReady(agentType) {
    ...
    if (!isDevelopmentAgentEnabled(runtime) || !status?.installed)
        return false;
    // Claude Code can be configured by an external CC-Switch/settings
    // environment. In that case the provider probe intentionally reports
    // `credential_detected` until a CCM-owned auth receipt is recorded, but the
    // CLI is already runnable with the supplied Anthropic-compatible endpoint.
    // Do not silently switch the explicitly selected Claude runtime to another
    // agent merely because CCM has not persisted a duplicate auth receipt.
    return runtime === "claudecode"
        ? ["configured", "credential_detected"].includes(status.authState)
        : status.authState === "logged_in";
}
