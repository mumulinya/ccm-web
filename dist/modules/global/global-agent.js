...
const globalAgentStatusRuntime = (0, global_agent_status_1.createGlobalAgentStatusRuntime)({
    ...
    normalizeText,
    refreshGlobalDevelopmentMissions: collaboration_runtime_runtime_tools_1.refreshGlobalDevelopmentMissions,
...
const globalAgentAgenticRuntime = (0, global_agent_agentic_runtime_1.createGlobalAgentAgenticRuntime)({
    ...
    controlGlobalDevelopmentMission: collaboration_runtime_runtime_tools_1.controlGlobalDevelopmentMission,
    controlGlobalMissionSupervisor: mission_supervisor_1.controlGlobalMissionSupervisor,
    createGlobalDevelopmentMission: collaboration_runtime_runtime_tools_1.createGlobalDevelopmentMission,
    createRequirementEpicWithChildren: collaboration_runtime_runtime_tools_1.createRequirementEpicWithChildren,
    executeFeishuAction,
    executePlayMusic,
    executeStopMusic,
    ...
    getGlobalAgentRun: loop_1.getGlobalAgentRun,
    getGlobalDevelopmentMission: collaboration_runtime_runtime_tools_1.getGlobalDevelopmentMission,
    ...
    superviseGlobalDevelopmentMissionCycle: collaboration_runtime_runtime_tools_1.superviseGlobalDevelopmentMissionCycle,
...
function publicGlobalAgentRunSummary(run) {
    ...
    return {
        ...
        provider_retry_count: Math.max(0, Number(run.provider_retry_count || 0)),
...
function publicGlobalAgentRun(run, includeObservations = false) {
    ...
    return {
        ...
        pendingPlan: run.pendingPlan || run.pending_plan || null,
        presented_plan: run.presented_plan || run.presentedPlan || null,
        presentedPlan: run.presentedPlan || run.presented_plan || null,
        ...
        tool_calls: run.tool_calls,
        provider_retry_count: Math.max(0, Number(run.provider_retry_count || 0)),
        providerRetryCount: Math.max(0, Number(run.provider_retry_count || 0)),
        usage: run.usage || null,
        input_tokens: Number(run.input_tokens || run.usage?.inputTokens || 0) || 0,
        output_tokens: Number(run.output_tokens || run.usage?.outputTokens || 0) || 0,
