...
function createGlobalAgentStatusRuntime(deps) {
    ...
    function isGlobalProgressStatusRequest(message, modelDecision = null) {
        ...
        if (!text)
            return false;
        if (/^(?:\/status|status|progress)$/i.test(text))
            return true;
    ...
    function globalIndependentReviewStatusKind(summary = {}) {
        ...
        if (["needs_rework", "rework", "changes_requested", "failed", "fail", "rejected", "not_verified"].includes(status))
            return "needs_rework";
        if (["needs_user", "waiting_user", "unknown", "manual", "blocked"].includes(status))
            return "needs_user";
        if (["passed", "pass", "accepted", "approved", "success"].includes(status))
            return "passed";
        return "recorded";
    }
    ...
    function globalStatusTestAgentPlanStatusKind(summary = {}, plan = null) {
        ...
        if (plan?.valid === false || ["blocked", "invalid", "error", "failed", "fail"].includes(status))
            return "blocked";
        if (plan?.valid === true || ["ready", "valid", "planned"].includes(status))
            return "ready";
        return "recorded";
    }
