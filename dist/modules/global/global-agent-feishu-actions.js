...
exports.createGlobalAgentFeishuActions = createGlobalAgentFeishuActions;
// Feishu-triggered management, media, and development dispatch actions.
function createGlobalAgentFeishuActions(deps) {
    ...
    async function executePlayMusic(baseUrl, input = {}) {
    ...
    function fillCronParams(params, _originalText, _groups = [], _projects = []) {
    ...
    async function executeFeishuManagementAction(baseUrl, action, originalText = "") {
        ...
        if (action.type === "manage_cron") {
            ...
            else if (["start", "stop", "restart", "build"].includes(operation))
                result = await postLocalApi(baseUrl, "/api/projects/runtime/action", { project, profile_id: params.profile_id || params.profileId, action: operation });
            else if (operation === "connect_agent")
                result = await postLocalApi(baseUrl, "/api/projects/agent-connection", { project, action: "connect", agent: params.agent });
            else if (operation === "disconnect_agent")
                result = await postLocalApi(baseUrl, "/api/projects/agent-connection", { project, action: "disconnect" });
            else if (operation === "delete")
    ...
    async function executeFeishuAction(baseUrl, action, originalText = "", traceId = "", options = {}) {
        ...
        if (action.type === "send_group_cmd") {
            ...
            const group = loadGroups().find((item) => item.id === groupId) || null;
