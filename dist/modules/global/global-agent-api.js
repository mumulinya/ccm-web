...
function createGlobalAgentApi(deps) {
    ...
    function handleGlobalAgentApi(pathname, req, res, parsed, ctx) {
        ...
        if (pathname === "/api/global-agent/sessions/delete" && req.method === "POST") {
            void readJsonRequest(req).then((payload) => {
                const sessionId = String(payload.session_id || payload.sessionId || "").trim();
                const requestedSource = String(payload.source || payload.expected_source || "").trim().toLowerCase();
                const expectedSource = requestedSource === "feishu" ? "feishu" : requestedSource === "web" ? "web" : "";
                const result = deleteGlobalAgentConversationSession(sessionId, expectedSource);
                if (String(result?.session?.source || expectedSource) === "feishu") {
                ...
                const store = loadGlobalAgentHistoryStore();
                require("../../agents/global/memory").pruneDeletedGlobalWebSessionMemory((store.sessions || [])
                    .filter((session) => String(session.source || "web") === "web")
                    .map((session) => String(session.id || "")));
                (0, runtime_events_1.publishRuntimeEvent)("global", "global.session_deleted", {
                    sessionId,
                    source: String(result?.session?.source || expectedSource || "web"),
                ...
                sendJson(res, {
                    ...
                    deleted_session_ids: store.deleted_session_ids || [],
                    ...globalFeishuSessionSnapshot(),
                });
            }).catch((error) => sendJson(res, { success: false, error: error?.message || "删除全局会话失败" }, 400));
            return true;
        }
            ...
            req.on("end", async () => {
                try {
                    ...
                    sendJson(res, {
                        ...
                        compaction_run_id: result.receipt?.compactionRunId || payload.compactionRunId || payload.compaction_run_id || "",
                        execution_path: result.receipt?.executionPath || "none",
                        session_start_hook_context: result.sessionStartHookContext ? { ...result.sessionStartHookContext, contentStored: false } : null,
