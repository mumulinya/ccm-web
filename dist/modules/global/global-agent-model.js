...
async function callLlm(config, messages, options = {}) {
    ...
    if (options.nativeTools?.length) {
        ...
        if (turn.toolCalls.length) {
            if (options.nativeFinalAnswerOnly)
                throw new Error("最终回答整理阶段不允许再次调用工具");
...
async function callGlobalModelWithRetry(config, messages, options = {}) {
    if (!options.call)
        return callLlm(config, messages, {
            ...
            nativeToolReference: options.nativeToolReference,
            nativeToolChoice: options.nativeToolChoice,
            nativeFinalAnswerOnly: options.nativeFinalAnswerOnly,
        });
    ...
    return (0, model_call_retry_1.runModelCallWithRetry)(() => call(config, messages), {
        ...
        signal: options.signal,
        scope: "全局 Agent 模型调用",
        onRetry: options.onRetry || (notice => console.warn(`[全局 Agent] 统一大模型调用暂时失败，正在重试（${notice.attempt}/${Math.max(1, notice.maxAttempts - 1)}）：${(0, global_agent_test_agent_display_1.compactPetText)(notice.error?.message || notice.error, 240)}`)),
    });
}
