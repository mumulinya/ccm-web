...
function createGlobalAgentHistoryRuntime(deps) {
    ...
    const isSessionTitleAutoReplaceable = typeof deps.isSessionTitleAutoReplaceable === "function"
        ...
        : (title, origin = "") => {
            ...
            if (["manual", "model", "fallback"].includes(normalizedOrigin))
                return false;
            return normalizedOrigin === "provisional" || isSessionTitlePlaceholder(title, origin);
        };
    const globalSessionTitleJobs = new Map();
    function applyProvisionalTitleToSession(session) {
        if (!session || !isSessionTitleAutoReplaceable(session.name, session.titleOrigin))
            return false;
        ...
        if (!generated?.title)
            return false;
        if (session.name === generated.title && String(session.titleOrigin || "") === "provisional")
            return false;
    ...
    function runGlobalAgentHistorySyncSelfTest() {
        ...
        const deletedSession = {
            ...
            source: "web",
            name: "待删除会话",
            messages: [{ role: "user", content: "旧消息", timestamp }],
        };
        const keptSession = {
            id: "session-kept",
            source: "web",
            name: "保留会话",
            messages: [{ role: "user", content: "保留消息", timestamp }],
        };
        const afterDelete = reconcileGlobalAgentWebHistory({ sessions: [deletedSession, keptSession], current_session_id: deletedSession.id }, { sessions: [keptSession], deletedSessionIds: [deletedSession.id] });
        const afterStaleSync = reconcileGlobalAgentWebHistory(afterDelete, {
            sessions: [deletedSession, keptSession],
        ...
        const checks = {
            ...
            explicitDeletePersistsTombstone: Array.isArray(afterDelete.deleted_session_ids)
                && afterDelete.deleted_session_ids.includes(deletedSession.id),
            explicitDeleteSelectsRemainingSession: afterDelete.current_session_id === keptSession.id,
            staleSyncCannotReviveDeletedSession: !(afterStaleSync.sessions || []).some((session) => session.id === deletedSession.id),
        };
    ...
    function deletedGlobalSessionIds(store) {
        return new Set((Array.isArray(store?.deleted_session_ids) ? store.deleted_session_ids : [])
            .map((value) => String(value || "").trim())
            .filter(Boolean));
    }
    function rememberDeletedGlobalSessionIds(store, sessionIds) {
        const deleted = deletedGlobalSessionIds(store);
        for (const sessionId of sessionIds) {
            const id = String(sessionId || "").trim();
            if (id)
                deleted.add(id);
        }
        // Session ids are unique. A bounded tombstone list prevents stale browser
        // history and late Agent receipts from recreating explicitly deleted chats.
        store.deleted_session_ids = [...deleted].slice(-500);
        return deleted;
    }
    ...
    function scheduleGlobalSessionAutoTitle(sessionId) {
        ...
        const job = (async () => {
            ...
            if (!current || !isSessionTitleAutoReplaceable(current.name, current.titleOrigin))
    ...
    function reconcileGlobalAgentWebHistory(store, payload) {
        ...
        const deletedSessionIds = new Set((Array.isArray(payload.deletedSessionIds) ? payload.deletedSessionIds : payload.deleted_session_ids || [])
            .map((value) => String(value || "").trim())
            .filter(Boolean));
        const replaceWebSessions = payload.replaceWebSessions === true || payload.replace_web_sessions === true;
        const persistedDeletedSessionIds = deletedGlobalSessionIds(store);
        for (const id of deletedSessionIds)
            persistedDeletedSessionIds.add(id);
        const incomingSessionIds = new Set(incomingSessions
            .map((session) => String(session?.id || "").trim())
            .filter((id) => !!id && !persistedDeletedSessionIds.has(id)));
        ...
        for (const session of incomingSessions) {
            ...
            if (!id || persistedDeletedSessionIds.has(id))
                continue;
        ...
        if (!replaceWebSessions) {
            for (const session of existingWebSessions.values()) {
                const id = String(session?.id || "").trim();
                if (!id || incomingSessionIds.has(id) || deletedSessionIds.has(id))
                    continue;
            ...
            for (const session of existingWebSessions.values()) {
                const id = String(session?.id || "").trim();
                if (id && !incomingSessionIds.has(id))
                    persistedDeletedSessionIds.add(id);
            }
        }
        ...
        return {
            ...
            deleted_session_ids: [...persistedDeletedSessionIds].slice(-500),
        };
    }
    function syncGlobalAgentWebHistory(payload) {
        const store = loadGlobalAgentHistoryStore();
        const tombstones = deletedGlobalSessionIds(store);
        const sessions = (Array.isArray(payload.sessions) ? payload.sessions : [])
            .filter((session) => String(session?.source || "web").toLowerCase() === "web")
            .filter((session) => !tombstones.has(String(session?.id || "").trim()));
        ...
        for (const session of reconciled.sessions || []) {
            if (String(session.source || "") !== "web")
                continue;
            const previous = (store.sessions || []).find((item) => String(item.id || "") === String(session.id || ""));
    ...
    function deleteGlobalAgentConversationSession(sessionId, expectedSource = "") {
        ...
        (0, session_compaction_runs_1.cancelSessionCompactionRun)({ scope: "global", exactSessionId: id, reason: "全局会话已删除" });
        (0, session_start_hook_context_1.clearSessionStartHookContext)("global", id);
    ...
    function getGlobalAgentConversationMessages(sessionId) {
        const store = loadGlobalAgentHistoryStore();
        if (deletedGlobalSessionIds(store).has(String(sessionId || "").trim()))
            return [];
    ...
    function appendGlobalAgentConversationMessage(sessionId, role, content, source = "feishu", options = {}) {
        const store = loadGlobalAgentHistoryStore();
        if (deletedGlobalSessionIds(store).has(String(sessionId || "").trim())) {
            return { appended: false, reason: "session_deleted" };
        }
        const sessions = Array.isArray(store.sessions) ? store.sessions : [];
    ...
    function upsertGlobalAgentConversationTaskMessage(sessionId, messageInput) {
        const id = String(sessionId || "").trim();
        ...
        if (!id || !taskId)
            return { updated: false, reason: "identity_missing" };
        const store = loadGlobalAgentHistoryStore();
        const session = (store.sessions || []).find((item) => String(item.id || "") === id);
        if (!session)
            return { updated: false, reason: "session_unavailable" };
        const messages = normalizeGlobalAgentMessages(session.messages || []);
        const existingIndex = messages.findIndex((item) => String(item?.task_id || item?.taskId || item?.taskExperience?.task_id || item?.task?.id || "") === taskId);
        ...
        if (!normalized)
            return { updated: false, reason: "message_invalid" };
        ...
        session.updatedAt = new Date().toISOString();
        saveGlobalAgentHistoryStore(store);
        (0, conversation_search_dirty_1.markConversationSearchIndexDirty)(`global:${id}`);
    ...
    function runFeishuGlobalAgentSessionRoutingSelfTest() {
        ...
        const merged = reconcileGlobalAgentWebHistory(staleStore, { sessions: [currentWeb, recentWeb], currentSessionId: recentWeb.id });
        const reconciled = reconcileGlobalAgentWebHistory(staleStore, {
            sessions: [currentWeb, recentWeb],
            currentSessionId: recentWeb.id,
            deletedSessionIds: [oldWeb.id],
        });
        const checks = {
            preservesConcurrentWebSession: merged.sessions.some((session) => session.id === oldWeb.id),
