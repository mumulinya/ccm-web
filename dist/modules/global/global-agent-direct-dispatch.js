...
function createGlobalAgentDirectDispatchRuntime(deps) {
    ...
    function inferGlobalDirectDispatchRequiresCodeChanges(_message, modelValue) {
        if (typeof modelValue !== "boolean")
            throw new Error("缺少统一大模型的 requiresCodeChanges 语义决策");
        return modelValue;
    }
    ...
    function buildGlobalSingleProjectMissionPayload(input) {
        ...
        return {
            ...
            targets: [{
                    ...
                    source_grounding_required: requiresCodeChanges,
                }],
            requires_code_changes: requiresCodeChanges,
            requires_verification: true,
            requires_independent_review: true,
    ...
    function renderGlobalDirectGroupWorkOrder(input) {
        ...
        return [
            ...
            "",
            "请按这个链路接管：",
            "1. 先理解目标和影响范围；开发任务必须由目标项目主 Agent 核对最小相关源码并返回签名证据，全局主 Agent 不直接读取项目文件。",
            "2. 形成用户能看懂的计划；如果风险高，先等用户确认。",
            "3. 只有项目源码证据、权限和工作单门禁全部通过后，才能派发给合适的子 Agent，并持续跟踪执行和回执。",
