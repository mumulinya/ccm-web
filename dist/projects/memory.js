...
function loadProjectMemory(project, options = {}) {
    ...
    const currentStored = stored?.schema === "ccm-project-memory-v4"
        && Number(stored?.version || 0) === PROJECT_MEMORY_VERSION
        ? stored
        : null;
    const memory = {
        ...createEmptyProjectMemory(project, options.workDir || currentStored?.workDir || ""),
        ...(currentStored || {}),
        project,
        workDir: options.workDir || currentStored?.workDir || "",
    };
    if (currentStored?.storageRecovery?.recoveredFromBackup) {
...
function compactConclusions(memory) {
    ...
    memory.compressedConclusions = renderArchiveIndex(memory.conclusionArchives, "", 14_000);
