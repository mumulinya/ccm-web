...
exports.createAgentRunnerRuntime = createAgentRunnerRuntime;
...
function createAgentRunnerRuntime(deps) {
    ...
    const usageWithProvenance = (usage, runnerKind, continuationEvidence, runtimeVersionSnapshot = null) => {
        ...
        return {
            ...
            provenance: {
                ...
                providerRuntimeIdentityChecksum: String(continuationEvidence?.providerRuntimeIdentityChecksum
    ...
    const isMissingNativeSessionFailure = (error) => {
    ...
    async function callAgent(projectName, message, workDir, agentType, timeoutMs, workspaceTarget = null) {
        ...
        // Claude Code keeps MCP servers alive across --resume sessions.  A CCM
        // memory challenge is single-use and bound to the exact MCP context, so a
        // resumed Claude session could otherwise acknowledge a stale challenge
        // from a previous turn.  Start a fresh native session whenever a
        // challenge-bound MCP config is active; the complete task context is in
        // the current prompt and the receipt remains strictly bound to this run.
        const challengeBoundAgentSession = workspaceTarget?.memoryContextConsumptionReceiptRequired === true
            && workspaceTarget?.mcpConfigPath
            && normalizeAgentRuntimeId(agentType) === "claudecode";
        const effectiveAgentSession = challengeBoundAgentSession
            // Claude Code's exit-success continuation proof is tied to a CCM-
            // assigned session id.  A completely fresh session (empty id) cannot
            // be resumed for the one-shot memory receipt recovery, so assign a new
            // UUID while explicitly disabling resume.  The recovery turn can then
            // safely resume this exact session without reusing a stale challenge.
            ? { ...(workspaceTarget?.agentSession || {}), sessionId: node_crypto_1.default.randomUUID(), persistSession: true, resumeSession: false }
        ...
        const durableDirectDispatch = workspaceTarget?.durableDispatch === true
            ...
            : null;
        // Memory receipt verification is bound to the exact runner request. The
        // non-durable path still needs a stable identity; keep one identity for
        // the complete run so parent evidence and recovery agree.
        const runnerRequestId = durableDirectDispatch?.id || `direct-${taskId}`;
        ...
        try {
            ...
            if (workspaceTarget?.memoryContextConsumptionReceiptRequired === true) {
                ...
                if (!memoryReceipt.valid) {
                    const recovery = await recoverMemoryContextConsumptionReceipt({
                        ...
                        trustedMemoryEnvelopeChecksum: workspaceTarget?.trustedMemoryEnvelopeChecksum || "",
                        trustedMemoryEnvelopeSourceChecksum: workspaceTarget?.trustedMemoryEnvelopeSourceChecksum || "",
                        allowFreshProviderSession: normalizeAgentRuntimeId(agentType) === "claudecode",
                        ...
                        const recoveryChallenge = workspaceTarget?.memoryContextConsumptionChallenge || {};
                        const recoveryServerName = `ccm__knowledge_recovery_${String(recoveryChallenge.challenge_id || "").slice(-8)}`;
                        const recoveryToolName = `mcp__${recoveryServerName}__acknowledge_memory_context`;
                        const recoveryPrompt = normalizeAgentRuntimeId(agentType) === "claudecode"
                            ? `CCM signed memory acknowledgement recovery.\nDo not repeat, redo, or modify the development task and do not change files.\nThis invocation intentionally uses a fresh isolated Claude session. The signed challenge below is the authoritative trusted context for this recovery; do not search for or resume a preceding transcript.\nCall the exact tool ${recoveryToolName} once with challenge_id=${String(recoveryChallenge.challenge_id || "")}. The MCP server verifies the signed task binding.\nAfter the tool succeeds, return only CCM_MEMORY_ACK_RECOVERED. If the tool rejects the signed challenge, return only CCM_MEMORY_ACK_CONTEXT_UNAVAILABLE.\nTask binding: task_id=${taskId}; execution_id=${executionId}.`
                            : recoveryRequest.prompt;
                        fs.writeFileSync(memoryReceiptRecoveryPromptFile, recoveryPrompt, "utf-8");
                        if (durableDirectDispatch)
                            appendDirectAgentDispatchTranscript(durableDirectDispatch.id, "memory_receipt_recovery_started", { recoveryId: recoveryRequest.recoveryId, nativeSessionId: recoveryRequest.nativeSessionId });
                        const recoveryNativeSessionId = normalizeAgentRuntimeId(agentType) === "claudecode"
                            ? node_crypto_1.default.randomUUID()
                            : recoveryRequest.nativeSessionId;
                        let recoveryMcpConfigPath = workspaceTarget?.mcpConfigPath;
                        if (normalizeAgentRuntimeId(agentType) === "claudecode" && recoveryMcpConfigPath) {
                            const recoveryServers = (0, agent_internal_mcp_1.buildTaskBoundInternalMcpServers)({
                                taskId,
                                groupId: workspaceTarget?.groupId || workspaceTarget?.group_id || "",
                                groupSessionId: workspaceTarget?.groupSessionId || workspaceTarget?.group_session_id || "",
                                projectSessionId: workspaceTarget?.projectSessionId || workspaceTarget?.project_session_id || "",
                                project: projectName,
                                role: "project-child-agent",
                                agentType,
                                taskAgentSessionId: workspaceTarget?.taskAgentSessionId || workspaceTarget?.task_agent_session_id || "",
                                nativeSessionId: recoveryNativeSessionId,
                                workDir: safeCwd,
                                baseWorkDir: workDir,
                                memoryReceiptChallenge: recoveryChallenge,
                                memoryReceiptFile: (0, memory_context_consumption_receipt_1.memoryContextConsumptionReceiptFile)(recoveryChallenge.challenge_id),
                                memorySnapshotId: workspaceTarget?.memorySnapshotId || "",
                                memorySnapshotChecksum: workspaceTarget?.memorySnapshotChecksum || "",
                            });
                            const knowledgeServer = recoveryServers.ccm__knowledge_context;
                            if (!knowledgeServer)
                                throw new Error("Claude memory recovery MCP server is unavailable");
                            recoveryMcpConfigPath = path.join(path.dirname(recoveryMcpConfigPath), `mcp-recovery-${String(recoveryChallenge.challenge_id || "unknown")}.json`);
                            fs.writeFileSync(recoveryMcpConfigPath, JSON.stringify({
                        ...
                        const recoveryCommand = buildAgentCommand(agentType, memoryReceiptRecoveryPromptFile, {
                            ...
                            mcpConfigPath: recoveryMcpConfigPath,
                            persistSession: true,
                            // Claude's --resume restores the old transcript and its MCP
                            // binding, which can belong to a previous task.  Start a clean
                            // context with the same CCM-assigned session id instead; the
                            // recovery verifier still receives the explicit continuation
                            // proof while MCP sees the current challenge binding.
                            resumeSession: normalizeAgentRuntimeId(agentType) === "claudecode" ? false : true,
                            sessionId: recoveryNativeSessionId,
                        });
                        ...
                        if (recoveryMcpConfigPath && recoveryMcpConfigPath !== workspaceTarget?.mcpConfigPath) {
                            try {
                                fs.unlinkSync(recoveryMcpConfigPath);
                            }
                            catch { }
                        }
                        return {
                    ...
                    if (durableDirectDispatch)
                        appendDirectAgentDispatchTranscript(durableDirectDispatch.id, recovery.recovered ? "memory_receipt_recovery_completed" : "memory_receipt_recovery_blocked", { recoveryId: recovery.record?.recovery_id || "", status: recovery.record?.status || "blocked" });
            ...
            if (durableDirectDispatch)
                appendDirectAgentDispatchTranscript(durableDirectDispatch.id, "tool_loop_started", { nativeSessionId: normalized.sessionId || "" });
            const toolLoop = await continueAgentToolCalls({
                ...
                mcpConfigPath: workspaceTarget?.mcpConfigPath,
                agentSession: effectiveAgentSession,
                taskId,
                executionId,
            ...
            if (durableDirectDispatch)
                appendDirectAgentDispatchTranscript(durableDirectDispatch.id, "tool_loop_completed", { nativeSessionId: toolLoop.nativeSessionId || normalized.sessionId || "" });
            ...
            if (!workspaceTarget?.skipIndependentVerification && !background && !/CCM_AGENT_PROBE_OK|执行通道健康探针/i.test(message)) {
                if (durableDirectDispatch)
                    appendDirectAgentDispatchTranscript(durableDirectDispatch.id, "verification_started", { projectName });
                output += await runIndependentProjectVerification(projectName, workDir, timeoutMs, taskId, executionId, agentType);
                if (durableDirectDispatch)
                    appendDirectAgentDispatchTranscript(durableDirectDispatch.id, "verification_completed", { projectName });
            }
            ...
            if (workspaceTarget?.agentSession?.resumeSession === true
    ...
    function callAgentForGroupStream(projectName, message, workDir, agentType, options = {}) {
        ...
        setAgentActivity(projectName, options.petState || "building", options.detail || `${getAgentRuntime(agentType).label} 正在执行协作任务`, groupId ? { tab: "groups", groupId } : { tab: "groups" }, getAgentRunActivityDuration(options.timeoutMs), { runtime: agentType, actorKind: options.actorKind || "third-party", displayName: options.petDisplayName || `${projectName} · ${getAgentRuntime(agentType).label}`, source: "group" });
        const startedAt = Date.now();
        const changeSnapshot = workDir ? createFileChangeSnapshot(workDir) : null;
        const safeCwd = (workDir || process.cwd()).replace(/\\/g, "/");
        const tmpMsg = path.join(UPLOAD_DIR, `_msg_${Date.now()}_${Math.random().toString(36).slice(2, 6)}.txt`);
        ...
        if (durableGroupDispatch) {
            if (executionId)
                registerExternalRunnerRequest(executionId, durableGroupDispatch.id);
        ...
        return new Promise((resolve) => {
            ...
            try {
                ...
                child.once("spawn", () => {
                    ...
                    if (durableGroupDispatch)
                        markDirectAgentDispatchStarted(durableGroupDispatch.id, { runnerPid: child.pid, startedAt: new Date().toISOString() });
                });
            ...
            const timeoutId = setTimeout(() => {
                try {
                    if (child?.exitCode === null && child?.signalCode === null)
                        terminateManagedChildProcess(child);
                }
                catch { }
            ...
            const finish = async (text, isError = false) => {
                ...
                if (!isError) {
                    if (durableGroupDispatch)
                        appendDirectAgentDispatchTranscript(durableGroupDispatch.id, "tool_loop_started", { nativeSessionId: normalized.sessionId || "" });
                    const toolLoop = await continueAgentToolCalls({
                        output: finalText,
                        nativeSessionId: nativeContinuationEvidence.nativeSessionReusable
                    ...
                    if (durableGroupDispatch)
                        appendDirectAgentDispatchTranscript(durableGroupDispatch.id, "tool_loop_completed", { nativeSessionId: normalized.sessionId || "" });
                    if (!/CCM_AGENT_PROBE_OK|执行通道健康探针/i.test(message)) {
                        if (durableGroupDispatch)
                            appendDirectAgentDispatchTranscript(durableGroupDispatch.id, "verification_started", { projectName });
                        ...
                        if (durableGroupDispatch)
                            appendDirectAgentDispatchTranscript(durableGroupDispatch.id, "verification_completed", { projectName });
                    }
                }
                const fileChanges = getFileChanges(projectName, changeSnapshot);
                const durableNativeSessionId = nativeContinuationEvidence.nativeSessionReusable
    ...
    function callAgentStream(projectName, message, workDir, agentType, res, options = {}) {
        ...
        const changeSnapshot = workDir ? createFileChangeSnapshot(workDir) : null;
        const projectRun = createProjectChatRun(projectName, options.userMessage || message, workDir, String(options.parentRunId || options.parent_run_id || ""), String(options.projectSessionId || options.project_session_id || ""));
        ...
        child.on("close", (code) => {
            ...
            (async () => {
                ...
                let projectMemoryConsumptionReceipt = null;
                ...
                if (showTaskExperience && options.memorySnapshotId) {
                    ...
                    const candidates = reports.flatMap((report) => report.acceptedCandidates || []);
                    if (candidates.length) {
                        const projectMemory = { constraints: [], decisions: [], facts: [], lessons: [], risks: [], openItems: [], contracts: [] };
                        const targetKey = {
                        ...
                        try {
                            const memory = (0, memory_1.updateProjectMemoryFromReceipt)({
                                ...
                                receipt: {
                                    ...
                                    memoryUsed: reports.flatMap((report) => report.usedIds || []),
                                    memoryIgnored: reports.flatMap((report) => report.ignoredIds || []),
                                    projectMemory,
                                },
                            });
                            projectRun.memory_admission = memory.lastMemoryAdmission || null;
                            projectRun.memory_usage_report_ids = reports.map((report) => report.id);
                        }
                        catch (error) {
