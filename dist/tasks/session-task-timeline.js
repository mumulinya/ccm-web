...
exports.emptySessionTaskIndex = emptySessionTaskIndex;
exports.readSessionTaskIndex = readSessionTaskIndex;
exports.readVerifiedSessionTaskIndex = readVerifiedSessionTaskIndex;
exports.drainTaskContextOutbox = drainTaskContextOutbox;
exports.enqueueTaskConversationProjectionSync = enqueueTaskConversationProjectionSync;
exports.appendSessionTimelineEvent = appendSessionTimelineEvent;
exports.persistTaskMutationWithTimelineAtomically = persistTaskMutationWithTimelineAtomically;
exports.createTaskStartedTimeline = createTaskStartedTimeline;
exports.persistTaskStartedAtomically = persistTaskStartedAtomically;
exports.createTaskAttemptStartedTimeline = createTaskAttemptStartedTimeline;
exports.createTaskTerminalTimeline = createTaskTerminalTimeline;
exports.verifySessionTimelineChain = verifySessionTimelineChain;
...
const parse = (value, fallback) => { try {
    return JSON.parse(String(value || ""));
}
catch {
    return fallback;
} };
const digest = (value) => crypto.createHash("sha256").update(typeof value === "string" ? value : stringify(value)).digest("hex");
...
const contextDigest = (value) => crypto.createHash("sha256").update(canonicalJson(value)).digest("hex");
const spanIdFor = (taskId, exactSessionId) => `tspan_${digest([taskId, exactSessionId]).slice(0, 24)}`;
function withoutChecksum(value) { const copy = { ...value }; delete copy.checksum; return copy; }
function normalizeStatus(status) {
    ...
    if (["cancelled", "canceled"].includes(status))
        return "cancelled";
    if (["interrupted", "recovery_required"].includes(status))
        return "interrupted";
...
function spanFromRow(db, row) {
    const attempts = db.prepare("SELECT attempt, start_sequence, end_sequence, status FROM task_attempt_spans WHERE span_id = ? ORDER BY attempt ASC").all(row.span_id).map((item) => ({
...
function readSpans(db, clause, values) {
    return db.prepare(`SELECT * FROM task_timeline_spans WHERE ${clause} ORDER BY created_at ASC, start_sequence ASC`).all(...values).map((row) => spanFromRow(db, row));
}
function upsertSpan(db, scope, scopeId, span) {
  ...
  `);
    for (const attempt of span.attemptSpans)
        statement.run(span.spanId, attempt.attempt, attempt.startSequence, attempt.endSequence ?? null, attempt.status, digest({ spanId: span.spanId, ...attempt }), now, now);
}
function readIndexInDb(db, input) {
    const sessionId = text(input.exactSessionId, 300);
    const scopeId = text(input.scopeId, 200);
    const head = db.prepare("SELECT * FROM session_timeline_heads WHERE scope=? AND scope_id=? AND exact_session_id=?").get(input.scope, scopeId, sessionId);
    ...
    const base = { schema: "ccm-session-task-index-v1", exactSessionId: sessionId, scope: input.scope, scopeId, events, taskSpans, ...(head?.active_task_id ? { activeTaskId: String(head.active_task_id) } : {}), latestSequence: Number(head?.latest_sequence || 0), contentStored: false };
    return { ...base, checksum: digest(base) };
}
function emptySessionTaskIndex(input) {
    const base = { schema: "ccm-session-task-index-v1", exactSessionId: text(input.exactSessionId, 300), scope: input.scope, scopeId: text(input.scopeId, 200), events: [], taskSpans: [], latestSequence: 0, contentStored: false };
    return { ...base, checksum: digest(base) };
}
function readSessionTaskIndex(input) { return (0, task_store_1.withSqliteTaskStore)(db => readIndexInDb(db, input)); }
function readVerifiedSessionTaskIndex(input) {
    const index = readSessionTaskIndex(input);
    if (!index.activeTaskId)
        return index;
    const caughtUp = catchUpTaskContext(index.activeTaskId);
    if (!caughtUp.success)
        throw new Error("当前任务上下文事件链发生漂移，已阻止模型继续执行");
    return readSessionTaskIndex(input);
}
function cursorFor(span, event) { return { spanId: span.spanId, exactSessionId: span.exactSessionId, sequence: event.sequence, eventChecksum: event.checksum }; }
function advanceTaskContext(db, taskId, span, event, reason, forceSnapshot) {
    if (!span)
        return;
    const head = db.prepare("SELECT * FROM task_context_heads WHERE task_id=?").get(taskId);
    if (!head)
        return;
    const context = parse(head.projection_json, null);
    if (!context)
        return;
    const cursors = parse(head.applied_cursors_json, []).filter(item => item.spanId !== span.spanId);
    cursors.push(cursorFor(span, event));
    const keyEvent = !["assistant_message", "user_message", "tool_use", "tool_result"].includes(event.type);
    if (!keyEvent) {
        const latestSnapshot = db.prepare("SELECT created_at FROM task_context_snapshots WHERE task_id=? ORDER BY revision DESC LIMIT 1").get(taskId);
        ...
        if (snapshotAgeMs >= 5 * 60_000) {
            ...
            (0, task_store_1.persistTaskContextProjection)(db, taskId, next, "dirty_checkpoint", event.eventId, true);
            return;
        }
    ...
    const timelineSpans = readSpans(db, "task_id=?", [taskId]);
    const next = { ...context, timelineSpans, activeSpanId: span.status === "open" ? span.spanId : "", appliedCursors: cursors, latestCheckpointSequence: event.sequence, revision: Number(head.revision || 0) + 1, updatedAt: new Date().toISOString(), contentStored: false };
    (0, task_store_1.persistTaskContextProjection)(db, taskId, next, reason, event.eventId, forceSnapshot);
}
function appendInDb(db, input) {
    const exactSessionId = text(input.exactSessionId, 300);
    const scopeId = text(input.scopeId, 200);
    if (!exactSessionId)
        throw new Error("会话时间线缺少 exactSessionId");
    const eventId = text(input.eventId || `tle_${crypto.randomUUID()}`, 180);
    const idempotencyKey = text(input.idempotencyKey || eventId, 240);
    const existing = db.prepare("SELECT event_id FROM session_timeline_events WHERE event_id=? OR idempotency_key=? LIMIT 1").get(eventId, idempotencyKey);
    if (existing)
        return readIndexInDb(db, input);
    const head = db.prepare("SELECT * FROM session_timeline_heads WHERE scope=? AND scope_id=? AND exact_session_id=?").get(input.scope, scopeId, exactSessionId);
    const identityIssues = [];
    if (input.taskId && input.type !== "task_started") {
        const taskRow = db.prepare("SELECT payload_json FROM tasks WHERE id=?").get(String(input.taskId));
        const task = parse(taskRow?.payload_json, null);
        if (task) {
            ...
            if (input.generation !== undefined && taskGeneration && Number(input.generation) !== taskGeneration)
                identityIssues.push("generation_mismatch");
            if (input.attempt !== undefined && taskAttempt && Number(input.attempt) !== taskAttempt)
                identityIssues.push("attempt_mismatch");
            ...
            if (keyWriteEvent && taskLease && String(input.leaseId || "") !== taskLease)
                identityIssues.push(input.leaseId ? "lease_mismatch" : "lease_missing");
        }
    }
    const sequence = Number(head?.latest_sequence || 0) + 1;
    const previousChecksum = String(head?.head_checksum || "");
    const timestamp = text(input.timestamp || new Date().toISOString(), 80);
    const eventBase = { schema: "ccm-session-timeline-event-v2", sequence, eventId, exactSessionId, scope: input.scope, type: input.type,
        ...
        ...(input.leaseId ? { leaseId: text(input.leaseId, 160) } : {}), timestamp, ...(input.payloadRef ? { payloadRef: text(input.payloadRef, 300) } : {}), idempotencyKey, previousChecksum, contentStored: false };
    const event = { ...eventBase, checksum: digest(eventBase) };
    ...
    if (!identityIssues.length && span) {
        ...
        upsertSpan(db, input.scope, scopeId, span);
    }
    else if (!identityIssues.length && input.taskId) {
        const row = db.prepare("SELECT * FROM task_timeline_spans WHERE task_id=? AND exact_session_id=?").get(input.taskId, exactSessionId);
        if (row) {
            span = spanFromRow(db, row);
            span = { ...span, latestSequence: sequence, checksum: "" };
            span = { ...span, checksum: digest(withoutChecksum(span)) };
            upsertSpan(db, input.scope, scopeId, span);
        }
    }
    if (!identityIssues.length && input.projectContext !== false && input.taskId && span)
        advanceTaskContext(db, input.taskId, span, event, input.contextReason || input.type, input.forceSnapshot === true);
...
function drainTaskContextOutbox(limit = 100) {
    const rows = (0, task_store_1.withSqliteTaskStore)(db => db.prepare("SELECT * FROM task_context_outbox WHERE status='pending' ORDER BY created_at ASC LIMIT ?").all(Math.max(1, limit)));
    let published = 0;
    for (const row of rows) {
        const claimed = (0, task_store_1.withSqliteTaskStore)(db => db.prepare("UPDATE task_context_outbox SET status='processing' WHERE outbox_id=? AND status='pending'").run(row.outbox_id));
        if (!claimed?.changes)
            continue;
        try {
            ...
            if (String(row.event_type || "") === "task.conversation_projection_sync") {
                ...
                if (!task)
                    throw new Error("任务不存在，无法补偿会话投影");
                ...
                if (receipt?.status === "failed")
                    throw new Error((receipt?.issues || []).join("；") || "会话投影补偿失败");
                (0, runtime_events_1.publishRuntimeEvent)("task", "task.conversation_projection_synced", { ...payload, receipt, contentStored: false });
            }
            else {
                (0, runtime_events_1.publishRuntimeEvent)("task", String(row.event_type || "task_context.updated"), payload);
            }
            (0, task_store_1.withSqliteTaskStore)(db => db.prepare("UPDATE task_context_outbox SET status='published', attempts=attempts+1, published_at=? WHERE outbox_id=? AND status='processing'").run(new Date().toISOString(), row.outbox_id));
            published += 1;
        }
        catch {
            (0, task_store_1.withSqliteTaskStore)(db => db.prepare("UPDATE task_context_outbox SET status='pending', attempts=attempts+1 WHERE outbox_id=? AND status='processing'").run(row.outbox_id));
        }
    }
    return { published, pending: Math.max(0, rows.length - published) };
}
function enqueueTaskConversationProjectionSync(input) {
    const taskId = text(input.taskId, 160);
    if (!taskId)
        return { queued: false, reason: "task_id_missing" };
    const revision = Math.max(0, Number(input.taskRevision || 0));
    const dedupeKey = `task-conversation-projection:${taskId}:${revision}`;
    const result = (0, task_store_1.withSqliteTaskStore)(db => db.prepare(`INSERT INTO task_context_outbox(outbox_id, dedupe_key, task_id, event_type, payload_json, status, attempts, created_at, content_stored)
...
function appendSessionTimelineEvent(input) { const result = (0, task_store_1.withImmediateTaskStoreTransaction)(db => appendInDb(db, input)); drainTaskContextOutbox(); return result; }
function persistTaskMutationWithTimelineAtomically(input) {
    const result = (0, task_store_1.withImmediateTaskStoreTransaction)(db => {
        const stored = db.prepare("SELECT position, payload_json FROM tasks WHERE id=?").get(String(input.task?.id || ""));
        if (!stored)
            throw new Error("任务不存在，无法提交时间线事务");
        const storedTask = parse(stored.payload_json, null);
        ...
        const duplicate = db.prepare("SELECT * FROM session_timeline_events WHERE event_id=? OR idempotency_key=? LIMIT 1").get(requestedEventId, requestedIdempotencyKey);
        if (duplicate) {
            const duplicateIndex = readIndexInDb(db, input);
            const duplicateHead = db.prepare("SELECT revision, checksum, active_span_id, projection_json FROM task_context_heads WHERE task_id=?").get(String(input.task.id));
            const duplicateContext = parse(duplicateHead?.projection_json, null);
            return {
        ...
        const headBefore = db.prepare("SELECT projection_json FROM task_context_heads WHERE task_id=?").get(String(input.task.id));
        const previousContext = parse(headBefore?.projection_json, null);
        ...
        if (input.validateStoredTask && !input.validateStoredTask(storedTask, previousContext))
            throw new Error("任务状态已经变化，时间线事务未能锁定");
        (0, task_store_1.upsertTaskInSqliteTransaction)(db, input.task, Number.isFinite(Number(input.position)) ? Number(input.position) : Number(stored.position || 0), false);
        ...
        if (input.type === "task_attempt_started") {
            ...
            if (!span)
                span = startSpan({ taskId: String(input.task.id), exactSessionId: input.exactSessionId, attempt, eventId, sequence: current.latestSequence + 1 });
            else {
        ...
        const priorCursors = Array.isArray(previousContext?.appliedCursors) ? previousContext.appliedCursors.filter((item) => item.spanId !== persistedSpan?.spanId) : [];
        ...
        const persistedContext = (0, task_store_1.persistTaskContextProjection)(db, String(input.task.id), {
            ...contextBase,
            timelineSpans: spans,
...
function startSpan(input) {
    const base = { schema: "ccm-task-timeline-span-v1", spanId: spanIdFor(input.taskId, input.exactSessionId), taskId: text(input.taskId, 120), exactSessionId: text(input.exactSessionId, 300), startMarkerId: input.eventId, startSequence: input.sequence,
...
function createTaskStartedTimeline(input) {
    const result = (0, task_store_1.withImmediateTaskStoreTransaction)(db => {
        const current = readIndexInDb(db, input);
        const existing = current.taskSpans.find(span => span.taskId === input.taskId);
        const eventId = `task_started:${text(input.taskId, 120)}`;
        if (existing)
            return { index: current, span: existing, event: current.events.find(event => event.eventId === eventId) || null };
...
function persistTaskStartedAtomically(input) {
    const result = (0, task_store_1.withImmediateTaskStoreTransaction)(db => {
        if (db.prepare("SELECT id FROM tasks WHERE id=?").get(String(input.task.id)))
            throw new Error("任务已存在，不能重复创建开始区间");
        (0, task_store_1.upsertTaskInSqliteTransaction)(db, input.task, input.position);
        const current = readIndexInDb(db, input);
        const eventId = `task_started:${text(input.task.id, 120)}`;
        ...
        const event = index.events.find(item => item.eventId === eventId);
        const taskForContext = { ...input.task, active_timeline_span_id: span.spanId, latest_checkpoint_sequence: event.sequence };
        const context = input.buildContext(taskForContext);
        const cursors = [cursorFor(span, event)];
        const persisted = (0, task_store_1.persistTaskContextProjection)(db, input.task.id, { ...context, timelineSpans: [span], activeSpanId: span.spanId, appliedCursors: cursors, latestCheckpointSequence: event.sequence, revision: 1 }, "task_started", eventId, true);
...
function createTaskAttemptStartedTimeline(input) {
    const result = (0, task_store_1.withImmediateTaskStoreTransaction)(db => {
        const current = readIndexInDb(db, input);
        ...
        if (!span)
            span = startSpan({ ...input, eventId, sequence: current.latestSequence + 1 });
        else {
            span = { ...span, endMarkerId: undefined, endSequence: undefined, status: "open", attemptSpans: [...span.attemptSpans.filter(item => item.attempt !== Math.max(1, Number(input.attempt))), { attempt: Math.max(1, Number(input.attempt)), startSequence: current.latestSequence + 1, status: "running" }], latestSequence: current.latestSequence + 1, checksum: "" };
            span = { ...span, checksum: digest(withoutChecksum(span)) };
        }
...
function createTaskTerminalTimeline(input) {
    const result = (0, task_store_1.withImmediateTaskStoreTransaction)(db => {
        const current = readIndexInDb(db, input);
        const span = current.taskSpans.find(item => item.taskId === input.taskId);
        if (!span)
            return { index: current, span: null, event: null };
        const terminalStatus = normalizeStatus(input.status);
        const type = terminalStatus === "success" ? "task_finished" : terminalStatus === "failed" || terminalStatus === "blocked" ? "task_failed" : terminalStatus === "cancelled" ? "task_cancelled" : "task_interrupted";
        const attempt = Math.max(1, Number(input.attempt || span.attemptSpans.at(-1)?.attempt || 1));
        const eventId = `task_terminal:${text(input.taskId, 120)}:${terminalStatus}:${attempt}`;
        const existing = current.events.find(event => event.eventId === eventId);
        if (existing)
            return { index: current, span, event: existing };
        const sequence = current.latestSequence + 1;
        const attemptSpans = span.attemptSpans.map(item => item.attempt === attempt ? { ...item, endSequence: sequence, status: terminalStatus } : item);
        const spanStatus = terminalStatus === "success" ? "completed" : terminalStatus === "running" ? "interrupted" : terminalStatus;
        let nextSpan = { ...span, endMarkerId: eventId, endSequence: sequence, attemptSpans, status: spanStatus, latestSequence: sequence, summary: { title: text(span.summary?.title, 300), goal: text(span.summary?.goal, 1000), result: text(input.result, 1000), evidenceIds: [...new Set((input.evidenceIds || []).map(item => text(item, 160)).filter(Boolean))].slice(0, 100), contentStored: false }, checksum: "" };
...
function verifySessionTimelineChain(input) {
    const index = readSessionTaskIndex(input);
    let previous = "";
    const issues = [];
    for (let i = 0; i < index.events.length; i += 1) {
        ...
        if (event.sequence !== i + 1)
            issues.push(`sequence_gap:${i + 1}:${event.sequence}`);
        if (event.previousChecksum !== previous)
            issues.push(`previous_checksum_mismatch:${event.eventId}`);
        if (event.checksum !== digest(withoutChecksum(event)))
            issues.push(`event_checksum_mismatch:${event.eventId}`);
...
function rebuildTaskContextInDb(db, taskId) {
    const head = db.prepare("SELECT * FROM task_context_heads WHERE task_id=?").get(taskId);
    if (!head)
        return { success: false, status: "missing", issues: ["context_head_missing"], context: null };
    const snapshot = db.prepare("SELECT * FROM task_context_snapshots WHERE task_id=? AND revision<=? ORDER BY revision DESC LIMIT 1").get(taskId, Number(head.revision || 0));
    if (!snapshot)
        return { success: false, status: "drifted", issues: ["context_snapshot_missing"], context: null };
    let context = parse(snapshot.snapshot_json, null);
    if (!context || String(context.checksum || "") !== String(snapshot.checksum || ""))
        return { success: false, status: "drifted", issues: ["context_snapshot_checksum_mismatch"], context: null };
    let previousChecksum = String(snapshot.checksum || "");
    const revisions = db.prepare("SELECT * FROM task_context_revisions WHERE task_id=? AND revision>? ORDER BY revision ASC").all(taskId, Number(snapshot.revision || 0));
    const issues = [];
    for (const revision of revisions) {
        if (String(revision.previous_checksum || "") !== previousChecksum) {
            issues.push(`revision_previous_checksum_mismatch:${revision.revision}`);
            break;
        }
        const delta = parse(revision.delta_json, null);
        if (!delta || delta.schema !== "ccm-task-context-delta-v1" || !delta.changes || typeof delta.changes !== "object") {
            issues.push(`revision_delta_invalid:${revision.revision}`);
            break;
        }
        const next = { ...context, ...delta.changes };
        for (const key of Array.isArray(delta.removedKeys) ? delta.removedKeys : [])
            delete next[key];
        delete next.checksum;
        const rebuiltChecksum = contextDigest(next);
        if (rebuiltChecksum !== String(revision.checksum || "")) {
            issues.push(`revision_checksum_mismatch:${revision.revision}`);
            break;
        }
        context = { ...next, checksum: rebuiltChecksum };
        previousChecksum = rebuiltChecksum;
    }
    if (!issues.length && previousChecksum !== String(head.checksum || ""))
        issues.push("context_head_checksum_mismatch");
    return { success: issues.length === 0, status: issues.length ? "drifted" : "ready", issues, context };
}
function rebuildTaskContextFromSnapshot(taskId) {
    return (0, task_store_1.withImmediateTaskStoreTransaction)(db => {
        const result = rebuildTaskContextInDb(db, String(taskId || ""));
        if (!result.success && result.status === "drifted")
            db.prepare("UPDATE task_context_heads SET status='drifted', updated_at=? WHERE task_id=?").run(new Date().toISOString(), String(taskId || ""));
        return { ...result, taskId, contentStored: false };
    });
}
function catchUpTaskContext(taskId) {
    return (0, task_store_1.withImmediateTaskStoreTransaction)(db => {
        const rebuilt = rebuildTaskContextInDb(db, taskId);
        if (!rebuilt.success) {
            if (rebuilt.status === "drifted")
                db.prepare("UPDATE task_context_heads SET status='drifted', updated_at=? WHERE task_id=?").run(new Date().toISOString(), taskId);
            return { success: false, status: rebuilt.status, taskId, issues: rebuilt.issues, contentStored: false };
        }
        const head = db.prepare("SELECT * FROM task_context_heads WHERE task_id=?").get(taskId);
        if (!head)
            return { success: false, status: "missing", taskId, contentStored: false };
        const context = parse(head.projection_json, null);
        const spans = readSpans(db, "task_id=?", [taskId]);
        const cursors = [];
        const issues = [];
        for (const span of spans) {
            const meta = db.prepare("SELECT scope, scope_id FROM task_timeline_spans WHERE span_id=?").get(span.spanId);
            ...
            for (let index = 0; index < events.length; index += 1) {
                ...
                if (event.sequence !== index + 1 || event.previousChecksum !== previous || event.checksum !== digest(withoutChecksum(event)))
                    issues.push(`timeline_integrity:${span.spanId}:${event.sequence}`);
                previous = event.checksum;
            }
            const latest = [...events].reverse().find((event) => event.taskId === taskId && event.sequence <= span.latestSequence);
            if (latest)
                cursors.push(cursorFor(span, latest));
        }
        if (issues.length) {
            (0, task_store_1.persistTaskContextProjection)(db, taskId, { ...context, status: "drifted", revision: Number(head.revision || 0) + 1, updatedAt: new Date().toISOString(), contentStored: false }, "timeline_integrity_failed", "", true);
            return { success: false, status: "drifted", taskId, issues, contentStored: false };
        }
        const changed = stringify(cursors) !== String(head.applied_cursors_json || "[]") || stringify(spans) !== stringify(context.timelineSpans || []);
        if (changed)
            (0, task_store_1.persistTaskContextProjection)(db, taskId, { ...context, timelineSpans: spans, appliedCursors: cursors, activeSpanId: [...spans].reverse().find(span => span.status === "open")?.spanId || "", revision: Number(head.revision || 0) + 1, updatedAt: new Date().toISOString(), contentStored: false }, "task_context_catch_up", "", true);
        return { success: true, status: changed ? "caught_up" : "current", taskId, cursors, contentStored: false };
    });
}
function recoverTaskContextProjectors() {
...
function snapshotTaskContextForBoundary(taskId, reason = "compaction_boundary") {
    const result = (0, task_store_1.withImmediateTaskStoreTransaction)(db => {
        const head = db.prepare("SELECT * FROM task_context_heads WHERE task_id=?").get(String(taskId || ""));
        if (!head)
            return { success: false, status: "missing", taskId, contentStored: false };
        const context = parse(head.projection_json, null);
        if (!context)
            return { success: false, status: "invalid", taskId, contentStored: false };
        const persisted = (0, task_store_1.persistTaskContextProjection)(db, String(taskId), { ...context, revision: Number(head.revision || 0) + 1, updatedAt: new Date().toISOString(), contentStored: false }, reason, "", true);
...
function runSessionTaskTimelineSelfTest() {
    ...
    appendSessionTimelineEvent({ ...base, type: "assistant_message", taskId: "task-a", eventId: "assistant:a:1" });
    const finished = createTaskTerminalTimeline({ ...base, taskId: "task-a", status: "done", attempt: 1 });
    const other = createTaskStartedTimeline({ ...base, taskId: "task-b", attempt: 1 });
    const chain = verifySessionTimelineChain(base);
    const checks = { start: started.span.startSequence === 1, finish: finished.span?.status === "completed", independent: other.span.taskId === "task-b" && other.span.startSequence > (finished.span?.endSequence || 0), priorProjection: projectPriorTaskSummaries(other.index, "task-b").some(item => item.taskId === "task-a"), hashChain: chain.valid };
    return { pass: Object.values(checks).every(Boolean), checks };
}
//# sourceMappingURL=session-task-timeline.js.map