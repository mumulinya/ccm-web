...
function sealScopedToolCapability(input) {
    if (input.scope === "global")
        throw new Error("全局主 Agent禁止持有项目工作区能力");
...
const v3OnlyDefinitions = [
    ...
    },
    {
        ...
        inputSchema: { type: "object", additionalProperties: false, properties: { project_id: { type: "string" }, path: { type: "string" }, paths: { type: "array", maxItems: 50, items: { type: "string" } }, symbol: { type: "string" }, symbols: { type: "array", maxItems: 50, items: { type: "string" } }, max_files: { type: "integer", minimum: 100, maximum: 4000 } }, anyOf: [{ required: ["path"] }, { required: ["paths"] }, { required: ["symbol"] }, { required: ["symbols"] }] },
    },
    {
        name: "find_related_tests", loadPolicy: "search",
        description: "Locate tests and verification configuration related to files or symbols in an authorized project. Return evidence-backed candidates and commands to delegate; never execute tests.",
        inputSchema: { type: "object", additionalProperties: false, properties: { project_id: { type: "string" }, path: { type: "string" }, paths: { type: "array", maxItems: 50, items: { type: "string" } }, symbol: { type: "string" }, symbols: { type: "array", maxItems: 50, items: { type: "string" } }, max_files: { type: "integer", minimum: 100, maximum: 4000 }, limit: { type: "integer", minimum: 1, maximum: 200 } } },
    },
    {
        name: "inspect_dependency_graph", loadPolicy: "search",
        description: "Inspect bounded source and manifest dependencies, entry points, unresolved imports, and dependency cycles in an authorized project.",
        inputSchema: { type: "object", additionalProperties: false, properties: { project_id: { type: "string" }, max_files: { type: "integer", minimum: 100, maximum: 4000 } } },
    },
    {
        name: "inspect_public_contracts", loadPolicy: "search",
        description: "Inspect public routes, exports, events, schemas, migrations, and configuration boundaries in an authorized project, with file and line evidence.",
        inputSchema: { type: "object", additionalProperties: false, properties: { project_id: { type: "string" }, kinds: { type: "array", maxItems: 6, items: { enum: ["http_route", "export", "event", "schema", "migration", "config"] } }, max_files: { type: "integer", minimum: 100, maximum: 4000 } } },
    },
    {
        name: "compare_project_contracts", loadPolicy: "search",
        description: "Compare evidence-backed contracts between two explicitly authorized group member projects. This tool is available only to a group main Agent.",
        inputSchema: { type: "object", required: ["left_project_id", "right_project_id"], additionalProperties: false, properties: { left_project_id: { type: "string" }, right_project_id: { type: "string" }, contract_type: { enum: ["http_route", "export", "event", "schema", "migration", "config"] }, kinds: { type: "array", maxItems: 6, items: { enum: ["http_route", "export", "event", "schema", "migration", "config"] } } } },
    },
    {
        name: "read_git_blame", loadPolicy: "search",
        description: "Read commit attribution for a bounded line range in an authorized project file. Author email, remote addresses, and absolute paths are never returned.",
        inputSchema: { type: "object", required: ["path"], additionalProperties: false, properties: { project_id: { type: "string" }, path: { type: "string" }, start_line: { type: "integer", minimum: 1 }, end_line: { type: "integer", minimum: 1 } } },
    },
    {
        name: "discover_verification_commands", loadPolicy: "search",
        description: "Discover test, type-check, lint, and build commands from real project configuration. Return sources and side-effect warnings without executing commands.",
        inputSchema: { type: "object", additionalProperties: false, properties: { project_id: { type: "string" } } },
    },
    {
        name: "run_inspection_command", loadPolicy: "search",
        description: "Run a strictly risk-scanned read-only diagnostic command in the authorized project root without a shell. Unknown, mutating, networked, build, test, install, script, and interpreter commands are rejected.",
        inputSchema: { type: "object", required: ["command"], additionalProperties: false, properties: { project_id: { type: "string" }, command: { type: "string", minLength: 1, maxLength: 4000 }, timeout_ms: { type: "integer", minimum: 250, maximum: 10000 } } },
    },
];
...
function allowedProjectsFor(capability) {
    if (capability.scope === "global")
        throw new Error("全局主 Agent禁止读取项目源码");
...
async function executeWorkspaceReadonlyToolWithCapabilityRaw(toolName, args, capability, contractVersion = 2, options = {}) {
    ...
    if (name === "compare_project_contracts") {
        ...
        const leftProject = String(args?.left_project_id || "").trim();
        const rightProject = String(args?.right_project_id || "").trim();
        const allowed = allowedProjectsFor(capability);
        if (!leftProject || !rightProject || !allowed.includes(leftProject) || !allowed.includes(rightProject))
            throw new Error("跨项目契约比较需要两个明确且已授权的群聊成员项目");
        const result = await (0, workspace_readonly_analysis_1.compareWorkspaceProjectContracts)({
            identity: { scope: "group", scopeId: capability.scopeId, exactSessionId: capability.exactSessionId, generation: capability.generation },
            left: { projectId: leftProject, root: projectWorkDir(leftProject) },
            right: { projectId: rightProject, root: projectWorkDir(rightProject) },
            args,
        });
        const evidence = (0, unified_evidence_registry_1.recordEvidence)({
            evidenceType: "source", taskId: "", workItemId: String(args?.work_item_id || ""), scope: capability.scope,
            scopeId: capability.scopeId, exactSessionId: capability.exactSessionId, generation: capability.generation,
            attempt: Math.max(1, Number(args?.attempt || 1)), producerAgentId: "ccm-readonly-analysis", status: "valid",
            subject: name, references: result.safeReceipt?.evidenceRefs || [], summary: `Compared ${leftProject} and ${rightProject} public contracts`,
            sourceChecksum: result.safeReceipt?.resultChecksum || checksum(result),
        });
        return enforceResultBudget({ ...result, evidenceId: evidence.evidenceId });
    }
    let project = "";
    let root = "";
    try {
    ...
    if (contractVersion === 3 && workspace_readonly_analysis_1.WORKSPACE_ANALYSIS_TOOL_NAMES.has(name)) {
        const result = await (0, workspace_readonly_analysis_1.executeWorkspaceReadonlyAnalysisTool)({
            name, args,
            identity: { scope: capability.scope, scopeId: capability.scopeId, exactSessionId: capability.exactSessionId, generation: capability.generation, projectId: project },
            root, signal: options.signal,
        });
        const references = Array.isArray(result.safeReceipt?.evidenceRefs) ? result.safeReceipt.evidenceRefs : [];
        const evidence = (0, unified_evidence_registry_1.recordEvidence)({
            evidenceType: name === "run_inspection_command" ? "command" : "source",
            taskId: "", workItemId: "",
            scope: capability.scope, scopeId: capability.scopeId, exactSessionId: capability.exactSessionId,
            generation: capability.generation, attempt: Math.max(1, Number(args?.attempt || 1)),
            repoStateIdentity: (0, unified_evidence_registry_1.captureRepoStateIdentity)(root, references), producerAgentId: "ccm-readonly-analysis", status: "valid",
            subject: name, references, summary: `${name} inspected ${references.length} evidence references`,
            sourceChecksum: result.safeReceipt?.resultChecksum || checksum(result),
        });
        return enforceResultBudget({ ...result, evidenceId: evidence.evidenceId });
    }
