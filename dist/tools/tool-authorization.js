...
const TOOL_AUTHORIZATION_AUDIT_FILE = path.join(os.homedir(), ".ccm", "agent-runner", "tool-authorization-changes.jsonl");
...
function buildToolAuthorizationInventory(input = {}) {
    ...
    const uniqueRuntimeSnapshots = scopes
        .flatMap((scope) => Array.isArray(scope?.runtime?.snapshots) ? scope.runtime.snapshots : [])
        .sort((left, right) => String(right.snapshotGeneratedAt || right.checkedAt || "").localeCompare(String(left.snapshotGeneratedAt || left.checkedAt || "")))
        .filter(snapshot => {
...
function buildToolAuthorizationOptions(input = {}) {
    ...
    const skill = asArray(input.skills || input.skill)
        ...
        .map(skill => {
        const name = cleanGrant(skill.name);
        const systemManaged = skill.systemManaged === true
            || skill.roleSkill === true
            || String(skill.origin || "").trim().toLowerCase() === "internal"
            || String(skill.sourceType || "").trim().toLowerCase() === "builtin"
            || (0, internal_skill_catalog_1.isCcmInternalSkillName)(name);
        return {
            name,
            ...
            toolName: `skill:${name}`,
            systemManaged,
            roleSkill: systemManaged && (skill.roleSkill === true || (0, internal_skill_catalog_1.isCcmInternalSkillName)(name)),
            usageMode: systemManaged ? "automatic_and_optional_direct" : "direct_authorization_required",
            directInvocationConfigurable: true,
        };
    })
...
function runToolAuthorizationSelfTest() {
    ...
    const options = buildToolAuthorizationOptions({
        ...
        skills: [
            { name: "release-notes", description: "notes", enabled: true, prompt: "hidden" },
            { name: "ccm-acceptance-evidence", description: "builtin", enabled: true, systemManaged: true, roleSkill: true, prompt: "hidden" },
        ],
        status: {
    ...
    const internalOption = options.skill.find(item => item.name === "ccm-acceptance-evidence");
    const externalOption = options.skill.find(item => item.name === "release-notes");
    ...
    const checks = {
        ...
        marksSystemManagedSkillsForPresentation: internalOption?.systemManaged === true
            && internalOption?.roleSkill === true
            && internalOption?.usageMode === "automatic_and_optional_direct",
        keepsExternalSkillsExplicitlyAuthorized: externalOption?.systemManaged === false
            && externalOption?.usageMode === "direct_authorization_required",
