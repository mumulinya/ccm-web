"use strict";
// runtime-tool-sync.ts — merged from 2 part files (behavior-freeze merge).
...
// ===== merged from runtime-tool-sync-part-01.ts =====
...
function probeRuntimeToolReadiness(audit, options = {}) {
    ...
    if (pluginDir) {
        ...
        const shouldCheckPluginMcp = runtime === "cursor"
            ? (nativeSyncedMcpServers.length > 0 || !!pluginManifest?.mcpServers)
            : runtime === "claudecode" && !!pluginManifest?.mcpServers;
...
function writeSessionPlugin(pluginRoot, runtime, snapshotId, mcpServers, skills, audit, skillPackagesDir = db_1.SKILL_PACKAGES_DIR) {
    ...
    if (runtime === "claudecode") {
        writeJsonAtomic(path.join(pluginRoot, ".claude-plugin", "plugin.json"), {
            name: pluginName,
            version: "1.0.0",
            description: "CCM invocation-scoped Skill authorization snapshot",
            skills: "./skills/",
        });
        // Claude receives MCP servers through --mcp-config --strict-mcp-config.
        // Duplicating protected servers in the Skill plugin can route a call to
        // a stale task challenge, so this plugin remains Skill-only.
        try {
            fs.rmSync(path.join(pluginRoot, ".mcp.json"), { force: true });
        }
        catch { }
    }
    else {
...
function loadCodexGatewayConfig() {
    ...
    if (config?.enabled === false || !["openai-compatible", "openai-responses", "auto"].includes(format) || !apiUrl || !apiKey || !model)
        return null;
...
// ===== merged from runtime-tool-sync-part-02.ts =====
...
function buildCodexConfigToml(mcpServers, gateway, skillPaths = []) {
    ...
    if (process.platform === "win32") {
        // Codex selects the Windows sandbox backend from the isolated CODEX_HOME.
        // Without this table current Codex builds silently reduce an explicitly
        // requested workspace-write task to read-only, even when both CLI sandbox
        // flags request workspace-write. "elevated" is the isolated Windows
        // sandbox backend; the per-invocation --sandbox flag remains the authority
        // for whether a turn is read-only or writable.
        lines.push("[windows]", `sandbox = ${tomlString("elevated")}`, "");
    }
...
function runRuntimeToolSyncIntegrationSelfTest() {
    ...
    try {
        ...
        const checks = {
            ...
            claudePluginManifestDeclaresSkillsAndMcp: claudeManifest?.skills === "./skills/" && !claudeManifest?.mcpServers,
            claudePluginMcpContainsOnlyNativeSafeServers: !claudePluginMcpPath || !fs.existsSync(claudePluginMcpPath),
            ...
            claudeReadinessChecksPluginMcpInheritance: !claudeReadiness.checks.some(check => check.id === "plugin_mcp_config"),
...
function runRuntimeToolSyncSelfTest() {
    ...
    const checks = {
        ...
        codexWindowsWorkspaceSandboxEnabled: process.platform !== "win32"
            || config.includes('[windows]\nsandbox = "elevated"'),
...
function syncRuntimeToolsWithCatalog(workDir, agentType, allowedTools, catalog = {}, options = {}) {
    ...
    try {
        ...
        for (const [name, server] of Object.entries(options.internalMcpServers || {})) {
            try {
                if (!name.startsWith(exports.CCM_MCP_PREFIX))
        ...
        for (const tool of selectedMcp) {
            const serverName = `${exports.CCM_MCP_PREFIX}${safeSlug(tool.name)}`;
        ...
        for (const missing of audit.missing.mcp) {
            ...
            audit.mcp_statuses?.push({
                ...
                serverName: grant.server ? `${exports.CCM_MCP_PREFIX}${safeSlug(grant.server)}` : "",
                state: "missing",
                grants: [missing],
        ...
        if (runtime === "claudecode") {
            ...
            audit.configFormat = "claude-strict-mcp-plus-skill-plugin";
            ...
            settings.mcp = {
                ...Object.fromEntries(Object.entries(existingServers).filter(([name]) => !name.startsWith(exports.CCM_MCP_PREFIX))),
