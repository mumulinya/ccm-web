...
function buildMainAgentToolRuntimeContext(input) {
    ...
    const authorizedProjects = Array.from(new Set((input.scopeIdentity?.allowedProjects || [])
        .map(project => String(project || "").trim())
        .filter(Boolean)));
    // Project and group main Agents may inspect only their signed workspace
    // scope. The global Agent never receives a workspace capability.
    const workspaceAvailable = !!input.scopeIdentity && ((input.scopeIdentity.scope === "project" && authorizedProjects.length === 1 && authorizedProjects[0] === input.scopeIdentity.scopeId)
        || (input.scopeIdentity.scope === "group" && authorizedProjects.length > 0));
    const capabilityToken = workspaceAvailable && input.scopeIdentity ? (0, workspace_readonly_tools_1.sealScopedToolCapability)({
        ...
        allowedProjects: authorizedProjects,
    }) : "";
    ...
    const scopeInstructionIdentity = input.scopeIdentity ? {
        ...
        generation: Number(input.scopeIdentity.generation || 0),
        allowedProjects: authorizedProjects,
        allowedGroups: (input.scopeIdentity.allowedGroups || []).map(value => String(value || "")).filter(Boolean),
    } : undefined;
    const scopeInstructionPrompt = scopeInstructionIdentity ? (0, scope_instructions_1.renderScopeInstructionCatalog)(scopeInstructionIdentity) : "";
    const scopeInstructionCatalog = scopeInstructionIdentity ? (0, scope_instructions_1.listScopeInstructionCatalog)(scopeInstructionIdentity) : [];
    const restoredScopeInstructions = scopeInstructionIdentity ? (0, scope_instructions_1.restoreScopeInstructionContext)({ ...scopeInstructionIdentity, generation: Number(continuityIdentity?.generation || input.scopeIdentity?.generation || 0) }) : { context: "" };
...
function buildMainAgentLoadedContextItems(toolContext, results = [], additionalSkills = []) {
    const skills = [
        ...
        ...additionalSkills.map(skill => ({
            ...
            loadSource: "same_run",
            tokens: Math.max(0, Number(skill?.tokens || 0)),
        })),
    ...
    const loadedMcpNames = new Set(mcp.flatMap(item => [item.name, ...(item.aliases || [])]).map(value => String(value || "").toLowerCase()));
    mcp.push(...(toolContext.catalog.discoverableMcp || []).map((tool) => ({
        kind: "mcp",
        name: String(tool?.canonicalName || tool?.name || ""),
        aliases: [
            String(tool?.canonicalName || ""),
        ...
        loadLevel: "available",
        checksum: contextItemChecksum({ canonicalName: tool?.canonicalName || tool?.name, server: tool?.server }),
        loadSource: "catalog",
        tokens: 0,
    })).filter(item => item.name && !loadedMcpNames.has(item.name.toLowerCase())));
    mcp.push(...(Array.isArray(results) ? results : []).filter((row) => row?.toolKind === "scope_instruction" && row?.ok === true).map((row) => ({
        kind: "mcp",
        name: String(row?.itemName || row?.name || "read_scope_instruction"),
        aliases: ["read_scope_instruction", String(row?.documentId || "")].filter(Boolean),
        loadLevel: "result",
        checksum: String(row?.resultChecksum || contextItemChecksum(row?.output)),
        loadSource: "same_run",
        tokens: Math.max(0, Number(row?.outputTokens || 0)),
    })));
    ...
    return (0, session_compaction_core_1.normalizeLoadedContextItems)({ schema: "ccm-loaded-context-items-v2", skills, mcp, invocations });
}
async function executeMainAgentToolRequests(input) {
    ...
    const executeOne = async (requestInput) => {
        const wrappedMcpName = requestInput.name === "invoke_mcp"
            ? String(requestInput.arguments?.name || requestInput.arguments?.tool_name || "").trim()
            : "";
        const request = applyAdaptiveWorkspaceReadBudget(wrappedMcpName ? {
            ...requestInput,
            name: wrappedMcpName,
            arguments: requestInput.arguments?.input && typeof requestInput.arguments.input === "object"
                ? requestInput.arguments.input
                : requestInput.arguments?.arguments || {},
        } : requestInput);
        if (request.name === "read_scope_instruction") {
            const callId = String(input.onUse?.(request) || "");
            const startedAt = Date.now();
            try {
                if (!input.toolContext.scopeInstructionIdentity)
                    throw new Error("SCOPE_INSTRUCTION_EXACT_IDENTITY_REQUIRED");
                const rawOutput = (0, scope_instructions_1.readScopeInstructionForAgent)({
                    identity: input.toolContext.scopeInstructionIdentity,
                    documentId: String(request.arguments?.documentId || request.arguments?.document_id || ""),
                    expectedChecksum: String(request.arguments?.expectedChecksum || request.arguments?.expected_checksum || "") || undefined,
                });
                ...
                if (outputTokens > resultTokenLimit)
                    throw new Error(cc_tool_result_limits_1.MAIN_AGENT_TOOL_RESULT_LIMIT_ERROR);
                input.onResult?.(request, callId, rawOutput);
                return { name: request.name, itemName: request.name, documentId: rawOutput.documentId, toolKind: "scope_instruction", source: "ccm_scope_instruction", loaded: true, scope: "scoped_session", durationMs: Date.now() - startedAt, aliases: [request.name, rawOutput.documentId], ok: true, output, outputTokens, resultChecksum: contextItemChecksum({ documentId: rawOutput.documentId, checksum: rawOutput.checksum }), reason: request.reason };
            }
            catch (error) {
                const detail = String(error?.message || error || "认知文档读取失败").slice(0, 1000);
                input.onResult?.(request, callId, null, detail);
                return { name: request.name, itemName: request.name, toolKind: "scope_instruction", source: "ccm_scope_instruction", loaded: true, scope: "scoped_session", durationMs: Date.now() - startedAt, aliases: [request.name], ok: false, error: detail, resultChecksum: contextItemChecksum(detail), reason: request.reason };
            }
        }
        ...
        if (!(skillName ? allowedSkills.has(skillName) : workspaceTool ? workspaceLoaded : mcpLoadedNow)) {
    ...
    const isSafeReadOnly = (request) => {
        if (request.name === "read_scope_instruction")
            return true;
        if (request.name === "tool_search" || request.name === "invoke_skill" || request.name === "invoke_mcp")
            return false;
    ...
    for (let index = 0; index < requests.length;) {
        ...
        results.push(...await (0, readonly_tool_concurrency_1.runReadonlyToolsAdaptive)({
            items: readBatch,
            worker: executeOne,
