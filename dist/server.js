...
function recordMetric(agent, data) {
    ...
    if (!metrics.agents[agent])
        metrics.agents[agent] = { calls: 0, successes: 0, failures: 0, totalMs: 0, avgMs: 0, totalFileChanges: 0, lastFileChangeCount: 0, lastCall: null };
    ...
    if (!metrics.daily[today][agent])
        metrics.daily[today][agent] = { calls: 0, successes: 0, failures: 0, totalMs: 0, totalFileChanges: 0 };
...
返回 JSON 格式：
{
  ...
  "overall": "总体评价"
    ...
    if (pathname === "/api/git/log" && req.method === "GET") {
        ...
        const workDir = info[0]?.workDir;
        try {
            const log = execFileSync("git", ["log", "--pretty=format:%H|%h|%an|%ae|%at|%s", "-n", String(limit)], { encoding: "utf-8", cwd: workDir, stdio: ["pipe", "pipe", "pipe"] });
...
function isPetRunning() {
    if (!fs.existsSync(PET_PID_FILE_GLOBAL))
        return false;
    ...
    try {
        ...
        try {
            fs.unlinkSync(PET_PID_FILE_GLOBAL);
        }
        catch { }
        return false;
    }
...
function startServer(port) {
    PORT = port;
    toolManager.loadTools().catch((e) => console.error("[ToolManager]", e.message));
