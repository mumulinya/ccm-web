...
const delegated_inquiry_projections_api_1 = require("./system/delegated-inquiry-projections-api");
const scope_instruction_routes_1 = require("./system/scope-instruction-routes");
const scope_instructions_1 = require("./system/scope-instructions");
const storage_2 = require("./modules/collaboration/storage");
...
const session_compaction_hook_routes_1 = require("./system/session-compaction-hook-routes");
...
function handleRequest(req, res) {
        ...
        req.on("end", async () => {
            try {
                ...
                if (action === "confirm_plan") {
                    const storedPlan = guardedProjectTask.workflow_meta?.presentedPlan
                        || guardedProjectTask.workflow_meta?.presented_plan
                        || guardedProjectTask.intake_draft?.presentedPlan
                        || guardedProjectTask.intake_draft?.presented_plan
                        || guardedProjectTask.workflow_meta?.project_main_plan
                    ...
                    if (storedPlan?.schema === "ccm-implementation-plan-v2") {
                        ...
                        const expectedRequirementRevision = payload.requirement_revision ?? payload.requirementRevision;
                        const expectedRequirementChecksum = String(payload.requirement_checksum || payload.requirementChecksum || "").trim();
                        if (expectedRevision === undefined || !expectedChecksum || expectedRequirementRevision === undefined || !expectedRequirementChecksum) {
                            return (0, utils_1.sendJson)(res, { success: false, error: "确认计划必须绑定当前需求与计划版本", code: "PLAN_BINDING_REQUIRED" }, 409);
                        }
                        if (Number(expectedRevision) !== Number(storedPlan.revision)) {
                        ...
                        if (expectedChecksum !== String(storedPlan.checksum || "")) {
                        ...
                        if (Number(expectedRequirementRevision) !== Number(storedPlan.requirementBinding?.revision || 0)
                            || expectedRequirementChecksum !== String(storedPlan.requirementBinding?.checksum || "")) {
                            return (0, utils_1.sendJson)(res, { success: false, error: "业务需求版本已变化，请重新加载最新计划", code: "REQUIREMENT_CHECKSUM_CONFLICT", requirement_binding: storedPlan.requirementBinding || null }, 409);
                        }
    ...
    if ((0, automation_session_bindings_api_1.handleAutomationSessionBindingsApi)(pathname, req, res, parsed))
        return;
    if ((0, delegated_inquiry_projections_api_1.handleDelegatedInquiryProjectionsApi)(pathname, req, res, parsed))
        return;
    if ((0, scope_instruction_routes_1.handleScopeInstructionRoutes)(pathname, req, res, parsed))
        return;
...
function bootstrapServerRuntime(startupCollabCtx, port) {
    ...
    (0, scope_instructions_1.initializeScopeInstructions)({ projects: (0, db_1.getConfigs)().map(item => String(item.name || "")).filter(Boolean), groups: (0, storage_2.loadGroups)() });
...
function startServer(port, host = process.env.CCM_HOST || "127.0.0.1") {
    ...
    server.on("close", () => {
        ...
        (0, session_compaction_runs_1.cancelAllSessionCompactionRuns)("服务已关闭，取消活动压缩");
...
if (require.main === module) {
    ...
    const shutdown = async (signal, exitCode = 0) => {
        ...
        (0, projects_1.stopFeishuChannelSupervisorForServer)();
        (0, projects_1.stopControlBotConnection)();
        (0, session_compaction_runs_1.cancelAllSessionCompactionRuns)(`服务收到 ${signal}，取消活动压缩`);
