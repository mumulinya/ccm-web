...
function normalizeDurableMemoryType(value: any, fallback = "fact") {
...
function normalizeDurableMemoryCandidate(value: any, fallbackType: string, meta: any) {
  const object = value && typeof value === "object" ? value : {};
  const content = compact(
    typeof value === "string"
      ? value
      : object.content || object.value || object.text || object.decision || object.title || object.summary || "",
    1000,
  );
  if (!content || isLowValueDurableMemory(content) || !isCcDurableMemoryCandidate({
    content,
...
function extractDurableMemoryCandidates(receipt: any, meta: any) {
  if (String(receipt?.status || "").toLowerCase() !== "done" || meta.accepted !== true) return [];
  const block = receipt.projectMemory || receipt.project_memory || receipt.durableMemory || receipt.durable_memory || {};
  const candidates: any[] = [];
  const addMany = (values: any, type: string) => {
...
function mergeDurableMemories(existing: any[], candidates: any[]) {
...
function buildTaskHistoryRecord(receipt: any, meta: any) {
  ...
  if (!summary && !filesModified.length && !actions.length && !verification.length && !blockers.length && !needs.length) return null;
...
function upsertTaskHistory(existing: any[], record: any) {
  if (!record) return Array.isArray(existing) ? existing : [];
  const result = Array.isArray(existing) ? [...existing] : [];
  ...
  if (index >= 0) result[index] = { ...result[index], ...record, time: result[index]?.time || record.time };
...
function buildRelevantArchiveEvidence(memory: any, query: string, maxRecords = 6) {
  ...
  for (const record of memory.taskHistory || []) {
    ...
    if (score >= 2) candidates.push({ score, time: record.updatedAt || record.time || "", type: "task_history", text: compactPreserveEdges(text, 1200), archiveId: record.id || "" });
  }
  for (const record of memory.conclusions || []) {
    const text = `${record.summary || ""} ${(record.filesModified || []).join(" ")} ${(record.verification || []).join(" ")}`;
    const score = scoreRecord(text);
    if (score >= 2) candidates.push({ score, time: record.time || "", type: "legacy_conclusion", text: compactPreserveEdges(text, 1200), archiveId: record.taskId || "" });
  }
  for (const archive of [...(memory.conclusionArchives || []), ...(memory.decisionArchives || [])]) {
    for (const record of archive?.records || []) {
      ...
      const score = scoreRecord(text);
      if (score >= 2) candidates.push({ score, time: record.time || "", type: archive.kind, text: compactPreserveEdges(text, 1200), archiveId: archive.id || "" });
    }
  }
...
function scoreDurableMemory(item: any, queryTokens: string[]) {
  const text = `${item?.content || ""} ${item?.reason || ""} ${(item?.relatedFiles || []).join(" ")}`.toLowerCase();
  let score = 0;
  for (const token of queryTokens) if (text.includes(token)) score += token.length >= 4 ? 3 : 1;
  if (item?.type === "constraint") score += 30;
  if (["risk", "open_item"].includes(item?.type)) score += 20;
  if (["decision", "contract"].includes(item?.type)) score += 10;
  if (item?.confidence === "accepted") score += 4;
  return score;
}

function buildDurableMemoryContext(memory: any, query: string, maxRecords = 24) {
  const queryTokens = searchTokens(query);
  const active = (Array.isArray(memory.durableMemories) ? memory.durableMemories : [])
    .filter((item: any) => item?.status !== "resolved" && item?.status !== "superseded" && item?.content)
    .map((item: any) => ({ item, score: scoreDurableMemory(item, queryTokens) }))
    .filter((row: any) => row.score > 0 || row.item.type === "constraint")
    .sort((a: any, b: any) => b.score - a.score || String(b.item.updatedAt || "").localeCompare(String(a.item.updatedAt || "")))
    .slice(0, maxRecords)
    .map((row: any) => row.item);
  if (!active.length) return "- 核心长期记忆：当前没有通过验收门禁的相关记录。";
  ...
  return [
    ...
    ...active.map((item: any) => `  - [${labels[item.type] || item.type}] ${item.content}${item.reason ? `（${item.reason}）` : ""}`),
  ].join("\n");
}
...
export function updateProjectMemoryFromReceipt(input: {
  ...
  if (meta.accepted && projectMemory.architectureVerified === true && receipt.architecture) memory.architecture = compact(receipt.architecture, 3000);
...
export function recordAcceptedProjectDeliveryMemory(input: { task: any; deliverySummary: any; resources?: any }) {
  ...
  for (const receipt of receipts) {
    ...
    if (!project || role === "independent_verifier" || /(?:^|[-_\s])test[-_\s]?agent(?:$|[-_\s])|coordinator/i.test(project)) continue;
    if (String(receipt?.status || "").toLowerCase() !== "done") continue;
    const profile = profiles.find((item: any) => String(item?.project || item?.name || item?.agent || "") === project) || {};
    try {
...
export function buildProjectConversationBrief(project: string, message: string, options: { analysis?: boolean } = {}) {
...
export function runProjectMemorySelfTest() {
  ...
  try {
    ...
    taskHistoryIsIdempotent = stored.taskHistory.filter((item: any) => item.taskId === "task-history-only").length === 1;
    lowValueHistoryIsNotInjected = !defaultPacket.includes("使用 Skill 生成发布说明");
    durableMemoryAdmissionWorks = stored.durableMemories.filter((item: any) => item.content === "支付接口必须保持幂等").length === 1
      && stored.durableMemories.find((item: any) => item.content === "支付接口必须保持幂等")?.occurrences === 2
      && !stored.durableMemories.some((item: any) => item.content === "任务已完成")
      && !stored.durableMemories.some((item: any) => item.content.includes("当前导出 createPayment"))
      && stored.durableMemories.every((item: any) => ["user", "feedback", "project", "reference"].includes(item.ccMemoryType))
    ...
    failedReceiptCannotCommitDurableMemory = afterFailed.durableMemories.length === beforeFailed + 0
      && !afterFailed.durableMemories.some((item: any) => item.content.includes("失败任务不应"));
    ...
    finalAcceptanceControlsCommit = rejectedDelivery.committed === false
      ...
      && afterDelivery.durableMemories.some((item: any) => item.content === "正式验收后提交")
      && !afterDelivery.durableMemories.some((item: any) => item.content === "不应提交");
  } finally {
