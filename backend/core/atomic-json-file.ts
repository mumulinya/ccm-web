...
function readLock(file: string): any {
...
export function acquireFileLock(targetFile: string, options: FileLockOptions = {}): FileLockHandle {
...
export function releaseFileLock(handle: FileLockHandle) {
  const owner = readLock(handle.file);
  if (!owner || owner.token !== handle.token || Number(owner.pid || 0) !== handle.pid) return false;
  for (let attempt = 0; attempt < 12; attempt += 1) {
    try {
      fs.unlinkSync(handle.file);
      return true;
    } catch (error: any) {
      if (!RETRYABLE_FILE_CODES.has(String(error?.code || ""))) break;
      sleep(10 * (attempt + 1));
    }
  }
  try {
...
export function withFileLock<T>(targetFile: string, operation: () => T, options: FileLockOptions = {}): T {
...
function renameWithRetry(source: string, target: string, attempts = 12) {
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    try {
      ...
      lastError = error;
      if (!RETRYABLE_FILE_CODES.has(String(error?.code || ""))) throw error;
      sleep(20 * (attempt + 1));
    }
  }
  ...
  try {
    ...
    try {
      ...
      if (displacedExisting && !fs.existsSync(file) && fs.existsSync(displaced)) {
        try { renameWithRetry(displaced, file); } catch {}
      }
      throw fallbackError || replaceError;
    } finally {
      try { if (fs.existsSync(temp)) fs.unlinkSync(temp); } catch {}
      try { if (fs.existsSync(displaced) && fs.existsSync(file)) fs.unlinkSync(displaced); } catch {}
    }
  }
}

export function readJsonWithBackup<T>(file: string, fallback: T): T {
  try { return JSON.parse(fs.readFileSync(file, "utf-8")); } catch {}
  try { return JSON.parse(fs.readFileSync(`${file}.bak`, "utf-8")); } catch {}
  return fallback;
}
