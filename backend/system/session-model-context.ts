...
import { projectPriorTaskSummaries, type CcmSessionTaskIndexV1 } from "../tasks/session-task-timeline";
import { buildTaskAwareSessionProjection } from "./task-aware-session-projection";
...
export function sessionModelMicroCompactReceiptChecksum(receipt: any) {
...
export function verifySessionModelMicroCompactReceipt(receipt: any, expected: { scope?: string; sessionId?: string; scopeId?: string } = {}) {
  const issues = [
...
function requestContent(value: any) {
  if (value == null || value === "") return "";
  return sessionModelMessageContent(value);
}

function excludePendingRequest(messages: any[], currentRequest: any) {
  const content = requestContent(currentRequest);
  if (!content) return { messages, deduplicated: false };
...
function completedCompactableToolCalls(events: SessionExecutionEvent[]) {
  const uses = new Map<string, SessionExecutionEvent>();
  const results = new Map<string, SessionExecutionEvent>();
  for (const event of events) {
    if (!event.toolCallId || !CC_COMPACTABLE_TOOL.test(String(event.toolName || ""))) continue;
    if (event.type === "tool_use") uses.set(event.toolCallId, event);
    else results.set(event.toolCallId, event);
  }
...
function selectTimeBasedMicroCompact(events: SessionExecutionEvent[], messages: any[], policy: SessionModelContextMicroCompactPolicy = {}) {
  const receipt = {
    ...
    clearedToolCallIds: [] as string[],
    clearedResultTokens: 0,
    rawLedgerPreserved: true,
  };
  if (policy.enabled !== true) return receipt;
  ...
  if (!clear.length) return { ...receipt, reason: "no_old_completed_tool_results", gapMinutes };
  return {
    ...receipt,
    applied: true,
    ...
    clearedResultTokens: clear.reduce((sum, event) => sum + estimateTextTokens(JSON.stringify(event.payload ?? null)), 0),
  };
}

function contentReplacementReceiptChecksum(receipt: any) {
  const payload = { ...(receipt || {}) };
  delete payload.receiptChecksum;
  return crypto.createHash("sha256").update(JSON.stringify(payload)).digest("hex");
}
...
function selectRecoverableToolResultReplacements(events: SessionExecutionEvent[], policy: SessionModelContextContentReplacementPolicy = {}) {
  ...
  if (policy.enabled === false) return { receipt: { ...base, receiptChecksum: contentReplacementReceiptChecksum(base) }, replacements: new Map<string, string>() };
  const completed = completedCompactableToolCalls(events);
...
function normalizeProjectedMessage(message: any, index: number) {
  return {
...
function renderProjection(input: UnifiedSessionModelContextInput, projection: any) {
...
export function buildUnifiedSessionModelContextProjection(input: UnifiedSessionModelContextInput) {
  const sessionId = String(input.sessionId || "").trim();
  if (!sessionId) throw new Error("exact_session_required_for_model_context");
  const taskProjectionInput = buildTaskAwareSessionProjection({ messages: Array.isArray(input.messages) ? input.messages : [], sessionTaskIndex: input.sessionTaskIndex, currentTaskId: input.currentTaskId || input.taskContext?.taskId });
  ...
  const recentWindow = canonicalSummary ? calculateSessionMemoryKeepWindow(history, {
    floorIndex,
    lastSummarizedMessageId: String(input.lastSummarizedMessageId || ""),
  }) : {
    startIndex: 0,
    preservedTokenCount: history.reduce((sum, message) => sum + estimateTextTokens(sessionModelMessageContent(message)), 0),
  ...
  const replacementSelection = selectRecoverableToolResultReplacements(
    visibleEvents.filter(event => !clearedToolCallIds.has(event.toolCallId)),
    input.contentReplacement,
  );
  const contentReplacementCore: any = {
  ...
  const visibleMessages = mergeConversationWithExecution(visibleConversation, visibleEvents, {
    clearedToolCallIds,
    replacedToolResults: replacementSelection.replacements,
  })
    .map(normalizeProjectedMessage);
  const archiveMessages = mergeConversationWithExecution(archivedConversation, archivedEvents)
    .map(normalizeProjectedMessage);
  ...
  const projection: any = {
    ...
    visibleMessages,
    archiveMessages,
    visibleMessageTokens: visibleMessages.reduce((sum, message) => sum + estimateTextTokens(message.content), 0),
    archiveMessageTokens: archiveMessages.reduce((sum, message) => sum + estimateTextTokens(message.content), 0),
    recentWindow,
    microCompact,
    contentReplacement,
    ...
    taskProjection: {
      ...
      currentTaskSpanChecksum: String(input.taskContext?.timelineSpans?.find((span: any) => span.taskId === String(input.currentTaskId || input.taskContext?.taskId || ""))?.checksum || ""),
      priorTaskSummaries: input.sessionTaskIndex ? projectPriorTaskSummaries(input.sessionTaskIndex, String(input.currentTaskId || input.taskContext?.taskId || "")) : [],
...
export function runUnifiedSessionModelContextSelfTest() {
  const messages = [
  ...
  const executionEvents = Array.from({ length: 6 }, (_, index) => {
    ...
    return [
      { ...base, id: `use-${index + 1}`, type: "tool_use" as const, status: "running" as const, timestamp: `2026-01-01T00:00:${10 + index * 2}.000Z`, payload: { path: `src/${index}.ts` } },
      { ...base, id: `result-${index + 1}`, type: "tool_result" as const, status: "ok" as const, timestamp: `2026-01-01T00:00:${11 + index * 2}.000Z`, payload: { content: `result-${index + 1}` } },
    ];
  }).flat();
