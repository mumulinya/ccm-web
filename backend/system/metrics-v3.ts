...
import { getObservabilityDatabase, observabilityMeta, setObservabilityMeta, withImmediateObservabilityTransaction } from "./observability-database";
import { sanitizeTraceValue } from "./trace-sanitizer";
...
export function metricsTimezone() {
  const configured = String(process.env.CCM_METRICS_TIMEZONE || observabilityMeta("metrics_timezone", "Asia/Shanghai") || "Asia/Shanghai").trim();
  try { new Intl.DateTimeFormat("en-CA", { timeZone: configured }).format(new Date()); return configured; } catch { return "Asia/Shanghai"; }
}

export function metricDateKey(value: Date | string | number = new Date(), timezone = metricsTimezone()) {
...
function normalizedStatus(data: any) {
  const explicit = String(data.status || "").trim().toLowerCase();
  if (VALID_STATUS.has(explicit)) return explicit;
  if (data.success === true) return "completed";
  if (data.success === false) return "failed";
  return "unknown";
}

function normalizedEvent(agentInput: string, data: any, now: Date | string | number = new Date()) {
...
function updateAggregate(db: any, event: any, bucketDate: string) {
  const existing = db.prepare(`SELECT * FROM metric_aggregates_v3 WHERE bucket_date=? AND scope_type=? AND scope_id=? AND role=? AND agent=?`).get(bucketDate, event.scopeType, event.scopeId, event.role, event.agent) as any;
  const samples = existing ? (() => { try { return JSON.parse(existing.duration_samples_json || "[]"); } catch { return []; } })() : [];
  if (event.durationMs > 0) samples.push(event.durationMs);
...
function insertEvent(db: any, event: any) {
  const result = db.prepare(`INSERT OR IGNORE INTO metric_events_v3(
    ...
    event.firstVisibleFeedbackMs, event.firstTokenMs, event.maxSilentGapMs, event.providerRetryCount,
    event.fallbackStreamCount, event.initialReadFileCount, event.initialReadTokens, event.partialOutputBeforeCancel ? 1 : 0,
...
export function recordMetricV3(agent: string, data: any, now: Date | string | number = new Date()) {
  const event = normalizedEvent(agent, data, now);
  const inserted = withImmediateObservabilityTransaction(db => insertEvent(db, event));
  return { inserted, event };
}

export function ensureLegacyMetricsMigrated(store: any) {
  if (observabilityMeta("metrics_v3_legacy_migration", null)?.completed) return { migrated: false, count: 0 };
  const events = Array.isArray(store?.events) ? store.events : [];
  const count = withImmediateObservabilityTransaction(db => {
    let inserted = 0;
  ...
  setObservabilityMeta("metrics_v3_legacy_migration", { completed: true, migrated_at: new Date().toISOString(), source_events: events.length, inserted: count });
  return { migrated: true, count };
}

function aggregateView(row: any) {
  let samples: number[] = [];
  try { samples = JSON.parse(row.duration_samples_json || "[]"); } catch {}
  return {
...
export function loadMetricsDashboardV3() {
  const rows = getObservabilityDatabase().prepare("SELECT * FROM metric_aggregates_v3 ORDER BY bucket_date, scope_type, scope_id, role, agent").all() as any[];
  const metrics: any = { version: 3, schema: "ccm-metrics-dashboard-v3", agents: {}, daily: {}, scopes: {}, events: [], updatedAt: null, timezone: metricsTimezone() };
...
export function queryMetricEventsV3(filters: any = {}) {
  ...
  if (!fromDate && days > 0) fromDate = metricDateKey(Date.now() - (days - 1) * 86400000);
  const clauses: string[] = [];
  const params: any[] = [];
  if (scopeType) { clauses.push("scope_type = ?"); params.push(scopeType); }
  if (scopeId) { clauses.push("scope_id = ?"); params.push(scopeId); }
  if (fromDate) { clauses.push("date_key >= ?"); params.push(fromDate); }
  if (toDate) { clauses.push("date_key <= ?"); params.push(toDate); }
  const baseWhere = clauses.length ? `WHERE ${clauses.join(" AND ")}` : "";
  const db = getObservabilityDatabase();
  const countRows = db.prepare(`SELECT status, COUNT(*) count FROM metric_events_v3 ${baseWhere} GROUP BY status`).all(...params) as any[];
  const statusCounts: any = { all: 0, completed: 0, failed: 0, cancelled: 0, blocked: 0, unknown: 0 };
  for (const row of countRows) { statusCounts[row.status] = Number(row.count || 0); statusCounts.all += Number(row.count || 0); }
  const filteredClauses = [...clauses];
  const filteredParams = [...params];
  if (status !== "all") { filteredClauses.push("status = ?"); filteredParams.push(status); }
  const where = filteredClauses.length ? `WHERE ${filteredClauses.join(" AND ")}` : "";
  const total = Number((db.prepare(`SELECT COUNT(*) count FROM metric_events_v3 ${where}`).get(...filteredParams) as any)?.count || 0);
  const totalPages = Math.max(1, Math.ceil(total / pageSize));
  const page = Math.min(requestedPage, totalPages);
  const offset = (page - 1) * pageSize;
  const rows = db.prepare(`SELECT * FROM metric_events_v3 ${where} ORDER BY at DESC, event_id DESC LIMIT ? OFFSET ?`).all(...filteredParams, pageSize, offset) as any[];
...
export function pruneMetricEventsV3(force = false) {
  const previous = observabilityMeta("metrics_v3_last_prune", null);
  if (!force && previous?.at && Date.now() - Date.parse(previous.at) < 24 * 60 * 60_000) return { skipped: true, ...previous };
  ...
  const result = withImmediateObservabilityTransaction(db => ({
  ...
  setObservabilityMeta("metrics_v3_last_prune", receipt);
  return { skipped: false, ...receipt };
}

export function resetMetricsV3() {
