...
export function normalizeOpenAiResponsesUrl(value: string) {
  ...
  if (!base) return "";
  if (/\/v1\/responses$/i.test(base) || /\/responses$/i.test(base)) return base;
  if (/\/v1$/i.test(base)) return `${base}/responses`;
  return `${base}/v1/responses`;
}

function textContent(value: any) {
...
function responsesMessageContent(content: any, role: string) {
  ...
  const parts = content.flatMap((item: any) => {
    ...
    if (item.type === "input_image" && item.image_url) return [item];
    return [];
  });
  return parts.length ? parts : textContent(content);
}

export function encodeOpenAiResponsesInput(messages: LlmChatMessage[]) {
  const input: any[] = [];
  for (const message of Array.isArray(messages) ? messages : []) {
    ...
    if (role === "tool") {
      input.push({
        ...
        call_id: String((message as any).tool_call_id || (message as any).toolCallId || ""),
    ...
    if (hasContent || toolCalls.length === 0) input.push({ role, content });
...
export function buildOpenAiResponsesTools(tools: ProviderToolDefinition[] = []) {
  return tools
...
export function buildOpenAiResponsesBody(input: {
  model: string;
...
export function safeProviderHttpDetail(value: any, limit = 300) {
  const raw = String(value || "").trim();
  if (!raw) return "";
  if (/<!doctype\s+html|<html\b|<body\b/i.test(raw)) return "上游网关返回 HTML 错误页";
  try {
...
export async function consumeOpenAiResponsesSse(response: any, onEvent: (event: any) => void) {
