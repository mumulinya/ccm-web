...
function safeString(value: any, max = MAX_STRING) {
...
export function sanitizeTraceValue(value: any, depth = 0, seen = new WeakSet<object>()): any {
  ...
  for (const [key, nested] of entries) {
    ...
    else if (BINARY_KEY.test(key)) output[key] = "[binary-content]";
    else output[key] = sanitizeTraceValue(nested, depth + 1, seen);
  }
  if (Object.keys(value).length > MAX_KEYS) output._truncated_keys = Object.keys(value).length - MAX_KEYS;
  return output;
}

export function sanitizeTraceEvent(input: any) {
...
export function sanitizeLegacyTrace(trace: any) {
  ...
  return {
    ...
    legacy: true,
    events: (Array.isArray(trace.events) ? trace.events : []).slice(-1200).map((item: any) => sanitizeTraceEvent(item).event),
  };
}
