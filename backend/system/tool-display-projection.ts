...
function unwrapResult(value: any) {
  if (value?.rawOutput != null) return value.rawOutput;
  if (value && typeof value === "object" && ["items", "lines", "locations", "diagnostics", "commits", "configs", "snapshot"].some(key => value[key] != null)) return value;
  if (value?.output != null) {
    if (typeof value.output !== "string") return value.output;
    try { return JSON.parse(value.output); } catch { return value.output; }
  }
  return value;
}
