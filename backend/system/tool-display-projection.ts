  ...
  result: {
    ...
    presentation?: {
      ...
      groups?: Array<{
        ...
        label: string;
        count: number;
        items: Array<{ label: string; secondary?: string; path?: string; line?: number; status?: string }>;
      }>;
    };
  };
  contentStored: false;
...
type ToolResultPresentation = NonNullable<ToolDisplayDetailV1["result"]["presentation"]>;

function resultPresentation(operation: string, rows: any[], raw: any): ToolResultPresentation | undefined {
  const safeRows = Array.isArray(rows) ? rows : [];
  const item = (label: any, options: any = {}) => ({
  ...
  if (operation === "grep_text") {
    ...
    const byPath = new Map<string, ReturnType<typeof item>[]>();
  ...
  if (/git/.test(operation)) return { layout: "git", groups: safeRows.length ? [{ id: "git", label: "Git 结果", count: safeRows.length, items: safeRows.map(row => item(row.subject || row.path || row.label || row.name || "记录", { secondary: row.hash || row.status, path: row.path })) }] : [] };
  if (/test|build|lint|typecheck|verify|verification|maven|gradle/.test(operation)) return { layout: "verification" };
  if (safeRows.length) return { layout: "generic", groups: [{ id: "results", label: "结果", count: safeRows.length, items: safeRows.map(row => item(row.label || row.name || row.path || row.location || "记录", { secondary: row.value || row.status, path: row.path })) }] };
  return undefined;
}
...
function resultProjection(operation: string, rawInput: any, error: any, transientBody: boolean): ToolDisplayDetailV1["result"] {
  ...
  if (operation === "query_knowledge") {
    ...
    const names = [...new Set(sources.map((source: any) => cleanText(
      source?.documentName || source?.document_name || source?.filename || source?.sourceId || source?.source_id,
      500,
    )).filter(Boolean))].slice(0, 20);
    const citationNames = names.length ? names : [...new Set((Array.isArray(raw?.citations) ? raw.citations : [])
      .map((citation: any) => cleanText(citation, 500).replace(/#\d+$/, ""))
      .filter(Boolean))].slice(0, 20);
    return {
  ...
  if (operation === "read_files") {
    ...
    const failed = files.filter((file: any) => file?.status === "failed" || file?.type === "text_error").length;
    const rows = files.slice(0, 40).map((file: any) => {
  ...
  if (operation === "read_code_diagnostics") {
    ...
    const rows = safeValue(sourceRows) as any[];
    return {
      kind: "diagnostics",
  ...
  if (operation === "read_git_status") {
    const lines = (Array.isArray(raw?.lines) ? raw.lines : []).map((line: any) => cleanText(line, 900)).filter(Boolean);
    const branch = lines.find((line: string) => line.startsWith("## "))?.slice(3) || "";
    const changes = lines.filter((line: string) => !line.startsWith("## ")).map((line: string) => {
  ...
  if (operation === "read_git_history" && Array.isArray(raw?.commits)) {
    const rows = safeValue(raw.commits) as any[];
