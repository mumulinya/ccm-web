...
import { loadFeishuConfig, loadTasks, saveTasks } from "../core/db";
import { isRuntimeCommandAvailable } from "../modules/collaboration/collaboration-resilience";
import { getReliabilityDrillStatus } from "./reliability-drills";
import { getReliabilityLedgerStats, listTaskLeases, reconcileReliabilityLedgerDebt } from "./reliability-ledger";
import { getProcessBootId, getProcessLifecycleSnapshot, touchProcessLifecycle } from "./process-lifecycle";

const ROOT = path.join(CCM_DIR, "reliability", "soak");
const STATE_FILE = path.join(ROOT, "state.json");
const REPORT_DIR = path.join(ROOT, "reports");
const BOOT_ID = getProcessBootId();
let sampleTimer: NodeJS.Timeout | null = null;
let sampleInFlight = false;
const eventLoopHistogram = monitorEventLoopDelay({ resolution: 20 });
eventLoopHistogram.enable();
let eventLoopWindowStartedUptime = process.uptime();
let startupBoundaryTimer: NodeJS.Timeout | null = null;
let pendingStartupWindow: any = null;
let freezeHashWorker: Worker | null = null;
let freezeHashRequestId = 0;
const freezeHashPending = new Map<number, { resolve: (entries: any[]) => void; reject: (error: Error) => void; timeout: NodeJS.Timeout }>();
...
function currentActiveLeaseTaskIds() {
  const now = Date.now();
  return new Set(listTaskLeases().filter((item: any) => item.status === "active" && processAlive(Number(item.owner_pid || 0)) && Date.parse(item.expires_at || 0) > now).map((item: any) => item.task_id));
}

function orphanedTaskItems(tasks = loadTasks()) {
  const activeLeaseTasks = currentActiveLeaseTaskIds();
  return tasks.filter((task: any) => task.status === "in_progress" && Date.now() - Date.parse(task.updated_at || task.started_at || task.created_at || 0) > 30 * 60 * 1000 && !activeLeaseTasks.has(task.id))
    .map((task: any) => ({ task_id: task.id, trace_id: task.trace_id || "", title: task.title || "", updated_at: task.updated_at || "", group_id: task.group_id || "" }));
}

export function inspectReliabilityDebt() {
  const ledger = getReliabilityLedgerStats();
  const configured = configuredFeishuConnections();
  const staleLocks = configured.filter((item: any) => item.lock_pid && !processAlive(item.lock_pid)).map((item: any) => ({ config: item.config, project: item.project, lock: item.lock, pid: item.lock_pid }));
...
export function reconcileStabilityDebt(reason = "生产级稳定性验收前清理历史债务") {
  const before = inspectReliabilityDebt();
  const ledger = reconcileReliabilityLedgerDebt(reason);
  const orphanIds = new Set(before.orphaned_tasks.map((item: any) => item.task_id));
  const tasks = loadTasks();
  const recoveredTasks: any[] = [];
  if (orphanIds.size) {
    for (const task of tasks) {
      if (!orphanIds.has(task.id)) continue;
      task.status = "pending";
  ...
  const removedLocks: any[] = [];
  for (const lock of before.stale_feishu_locks) {
    const file = path.join(CCM_DIR, "configs", lock.lock);
    try {
      const resolved = path.resolve(file);
      const root = path.resolve(path.join(CCM_DIR, "configs"));
  ...
  return { reconciled_at: new Date().toISOString(), reason, before, ledger, recovered_tasks: recoveredTasks, removed_feishu_locks: removedLocks, after, pass: Object.values(after.counts).every((value: any) => Number(value || 0) === 0) };
}
...
function configuredFeishuConnections() {
  ...
  return fs.readdirSync(configDir).filter(name => /^config-.*\.toml$/i.test(name)).map(name => {
    ...
    if (!/\[\[projects\.platforms\]\][\s\S]*?type\s*=\s*["']feishu["']/i.test(content)) return null;
    ...
    const managedPid = readPid(path.join(CCM_DIR, "pids", `${project}.pid`));
    const tempDir = path.join(CCM_DIR, "temp");
...
function feishuSnapshot(expectedConfigs: string[] = []) {
  const config = loadFeishuConfig();
  const globalPid = readPid(path.join(CCM_DIR, "pids", "ccm-control-bot.pid"));
  const configured = configuredFeishuConnections();
  const wanted = expectedConfigs.length ? new Set(expectedConfigs) : new Set(configured.map((item: any) => item.config));
  const projectConnections = configured.filter((item: any) => wanted.has(item.config));
  ...
  const connections = [
    ...
    ...projectConnections.map((item: any) => ({ id: `project:${item.project}`, kind: "project", name: item.project, config: item.config, project: item.project, pid: item.pid, alive: item.alive, source: item.source })),
  ];
  ...
  return {
    ...
    missing_expected_connections: missingExpected.map((item: any) => ({ id: item.id, kind: item.kind, name: item.name, config: item.config, project: item.project, pid: item.pid })),
    stale_project_locks: configured.filter((item: any) => item.lock_pid && !processAlive(item.lock_pid)).length,
    healthy: missingExpected.length === 0 && projectConnections.length === wanted.size,
  };
}

async function walkFiles(root: string, filter: (file: string) => boolean, output: string[] = []) {
  let items: fs.Dirent[] = [];
  try { items = await fs.promises.readdir(root, { withFileTypes: true }); } catch { return output; }
  for (const item of items) {
    const file = path.join(root, item.name);
    if (item.isDirectory()) await walkFiles(file, filter, output);
    else if (item.isFile() && filter(file)) output.push(file);
  }
  return output;
}

function rejectFreezeHashPending(error: Error) {
...
function getFreezeHashWorker() {
  if (freezeHashWorker) return freezeHashWorker;
  ...
  worker.on("message", message => {
    ...
    else pending.resolve(Array.isArray(message?.entries) ? message.entries : []);
  });
  worker.on("error", error => {
    if (freezeHashWorker === worker) freezeHashWorker = null;
    rejectFreezeHashPending(error instanceof Error ? error : new Error(String(error)));
  });
  worker.on("exit", code => {
    if (freezeHashWorker === worker) freezeHashWorker = null;
    if (code !== 0 && freezeHashPending.size) rejectFreezeHashPending(new Error(`冻结清单 Worker 退出，code=${code}`));
  });
  return worker;
}
...
function hashFreezeFilesInWorker(files: Array<[string, string]>) {
...
function resolveProjectRoot() {
  const candidates = [
    path.join(CCM_DIR, "ccm"),
    process.cwd(),
    path.resolve(__dirname, ".."),
    path.resolve(__dirname, "../.."),
  ];
  return candidates.find(candidate => fs.existsSync(path.join(candidate, "package.json")) && fs.existsSync(path.join(candidate, "ccm-package"))) || path.join(CCM_DIR, "ccm");
}
...
async function freezeManifest() {
  ...
  const [backendFiles, frontendFiles, scriptFiles, distFiles, publicFiles, tomlFiles] = await Promise.all([
    ...
    walkFiles(path.join(CCM_DIR, "configs"), file => file.endsWith(".toml")),
  ]);
  const codeFiles = [
  ...
  const configFiles = [
    path.join(CCM_DIR, "project-configs.json"),
    path.join(CCM_DIR, "groups.json"),
    path.join(CCM_DIR, "config.toml"),
    path.join(CCM_DIR, "feishu-config.json"),
    path.join(CCM_DIR, "group-orchestrator-config.json"),
    path.join(CCM_DIR, "smithery-config.json"),
    ...tomlFiles,
  ].filter(file => fs.existsSync(file));
  const scoped = new Map<string, string>();
  for (const file of codeFiles) scoped.set(path.resolve(file), "code");
  for (const file of configFiles) scoped.set(path.resolve(file), "config");
  ...
  const entries = workerEntries.map(entry => ({
    path: path.relative(CCM_DIR, entry.file).replace(/\\/g, "/"),
    scope: entry.scope,
    hash: entry.hash,
    size: entry.size,
  ...
  return {
    ...
    required_runtime: path.relative(CCM_DIR, requiredRuntime).replace(/\\/g, "/"),
    entries,
  };
}
...
function nanosecondsToMilliseconds(value: any) {
  const number = Number(value || 0);
  return Number.isFinite(number) && number >= 0 && number < 1e15 ? Number((number / 1_000_000).toFixed(3)) : 0;
}

function readEventLoopHistogramWindow(phase: "startup" | "transition" | "runtime") {
  const endedUptime = process.uptime();
  const count = Number((eventLoopHistogram as any).count || 0);
  const value = {
    phase,
...
function armEventLoopStartupBoundary(graceSeconds: number) {
  if (startupBoundaryTimer) clearTimeout(startupBoundaryTimer);
...
function takeContinuousEventLoopWindow(graceSeconds: number) {
...
function makeAlert(code: string, severity: "info" | "warning" | "critical", message: string, data: any = {}) {
  return { code, severity, message, data };
}

function evaluateSample(sample: any, baseline: any, options: any = {}) {
  const alerts: any[] = [];
  ...
  if (sample.ledger.leases.stale > Number(baseline?.ledger?.leases?.stale || 0)) alerts.push(makeAlert("new_stale_task_lease", "warning", `新增失效任务租约：${sample.ledger.leases.stale - Number(baseline?.ledger?.leases?.stale || 0)}`, { entity_ids: (sample.ledger.leases.stale_items || []).map((item: any) => item.task_id), items: sample.ledger.leases.stale_items || [] }));
  if (sample.ledger.operations.stale_in_progress > Number(baseline?.ledger?.operations?.stale_in_progress || 0)) alerts.push(makeAlert("new_stale_idempotency", "warning", `新增失效幂等操作：${sample.ledger.operations.stale_in_progress - Number(baseline?.ledger?.operations?.stale_in_progress || 0)}`, { entity_ids: (sample.ledger.operations.stale_items || []).map((item: any) => item.operation_id), items: sample.ledger.operations.stale_items || [] }));
  ...
  if (sample.tasks.stuck_without_lease > Number(baseline?.tasks?.stuck_without_lease || 0)) alerts.push(makeAlert("new_stuck_tasks", "critical", `新增无有效租约的长时间执行任务：${sample.tasks.stuck_without_lease - Number(baseline?.tasks?.stuck_without_lease || 0)}`, { entity_ids: (sample.tasks.stuck_items || []).map((item: any) => item.task_id), items: sample.tasks.stuck_items || [] }));
  ...
  if (startupLag > 500) alerts.push(makeAlert("event_loop_lag_startup", "info", `启动期事件循环延迟峰值 ${startupLag}ms`, { boot_id: sample.boot_id, phase: "startup", window: sample.event_loop_startup_window_ms || sample.event_loop_delay_window_ms }));
  if (runtimeLag > 2000) alerts.push(makeAlert("event_loop_lag_critical", "critical", `非启动期事件循环严重延迟峰值 ${runtimeLag}ms`, { boot_id: sample.boot_id, phase: "runtime", window: sample.event_loop_delay_window_ms }));
  else if (runtimeLag > 500) alerts.push(makeAlert("event_loop_lag", "warning", `非启动期事件循环延迟峰值 ${runtimeLag}ms`, { boot_id: sample.boot_id, phase: "runtime", window: sample.event_loop_delay_window_ms }));
  ...
  if (options.clean_mode && sample.freeze?.hash !== baseline?.freeze?.hash) alerts.push(makeAlert("freeze_violation", "critical", "干净验收期间代码或配置发生变化", { baseline_hash: baseline?.freeze?.hash || "", current_hash: sample.freeze?.hash || "" }));
  const baselineStarts = Number(baseline?.lifecycle?.starts || 0);
  if (options.clean_mode && Number(sample.lifecycle?.starts || 0) > baselineStarts) alerts.push(makeAlert("process_restart_during_clean_soak", "critical", "干净验收期间检测到 CCM 进程重启", { starts: Number(sample.lifecycle?.starts || 0) - baselineStarts, boot_id: sample.boot_id }));
...
function alertFingerprint(alert: any) {
...
function updateAlertIncidents(state: any, alerts: any[], sample: any) {
  ...
  for (const alert of alerts) {
    ...
    let incident = [...incidents].reverse().find((item: any) => item.fingerprint === fingerprint && !item.resolved_at);
    if (!incident) {
      incident = {
  ...
  for (const incident of incidents) {
    if (!incident.resolved_at && !active.has(incident.fingerprint)) incident.resolved_at = sample.at;
  }
  ...
  for (const incident of incidents) {
    ...
    if (Date.parse(incident.first_at) < Date.parse(item.first_at)) item.first_at = incident.first_at;
    if (Date.parse(incident.last_at) > Date.parse(item.last_at)) item.last_at = incident.last_at;
    if (!incident.resolved_at) item.open_incidents++;
...
export async function collectSoakSample(options: any = {}) {
  ...
  const timed = (name: string, operation: () => any) => {
    const started = Date.now();
    const result = operation();
    stages[name] = Date.now() - started;
    return result;
  };
  const tasks = timed("tasks", loadTasks);
  await yieldToEventLoop();
  const leases = timed("leases", listTaskLeases);
  ...
  const stuckItems = tasks.filter((task: any) => task.status === "in_progress" && Date.now() - Date.parse(task.updated_at || task.started_at || task.created_at || 0) > 30 * 60 * 1000 && !activeLeaseTasks.has(task.id)).map((task: any) => ({ task_id: task.id, trace_id: task.trace_id || "", updated_at: task.updated_at || "" }));
  await yieldToEventLoop();
  ...
  const ledger = timed("ledger", getReliabilityLedgerStats);
  await yieldToEventLoop();
  ...
  await yieldToEventLoop();
  const lifecycle = timed("lifecycle", () => getProcessLifecycleSnapshot({ since: options.started_at || options.startedAt || 0, event_limit: 20 }).counts);
  ...
  const drill = timed("drill", getReliabilityDrillStatus);
  timed("lifecycle_touch", touchProcessLifecycle);
  await yieldToEventLoop();
...
function percentile(numbers: number[], quantile: number) {
...
function regressionPerHour(samples: any[], selector: (sample: any) => number) {
...
function aggregateSamples(state: any, samples: any[]) {
  ...
  const perConnection = expectedConnections.map((connection: any) => {
    const aliveSamples = samples.filter(sample => (sample.feishu?.connections || []).some((item: any) => item.id === connection.id && item.alive)).length;
    const pids = Array.from(new Set(samples.flatMap(sample => (sample.feishu?.connections || []).filter((item: any) => item.id === connection.id && item.pid).map((item: any) => Number(item.pid)))));
  ...
  const everyExpectedFeishuConnectionHealthy = perConnection.length === expectedConnections.length && perConnection.every((item: any) => item.availability_percent === 100);
  ...
  const lifecycle = getProcessLifecycleSnapshot({ since: state.started_at, event_limit: 1000 });
  const bootGroups = new Map<string, any[]>();
  ...
  const cleanRequirements = !state.clean_mode || (
    restarts === 0
  ...
  return {
    ...
    alerts: { critical: criticalAlerts.reduce((sum: number, item: any) => sum + Number(item.count || 1), 0), warning: warningAlerts.reduce((sum: number, item: any) => sum + Number(item.count || 1), 0), info: (state.alerts || []).filter((alert: any) => alert.severity === "info").reduce((sum: number, item: any) => sum + Number(item.count || 1), 0), items: state.alerts || [], incidents: state.incidents || [] },
  };
}
...
function reportMarkdown(state: any, summary: any) {
  ...
  return [
    ...
    ...((summary.availability.per_connection || []).map((item: any) => `- ${item.name || item.id}（${item.kind}）：${item.availability_percent}%｜PID ${item.pids?.join(", ") || "无"}`)),
    "",
    "## 资源",
    "",
    ...
    "## 告警",
    "",
    ...(summary.alerts.items.length ? summary.alerts.items.map((item: any) => `- [${item.severity}] ${item.code}：${item.message}（独立事故 ${item.count} 次，采样命中 ${item.observations || item.count} 次，首次 ${item.first_at}）`) : ["- 无"]),
    "",
  ].join("\n");
}
...
export async function startSoakTest(options: any = {}) {
  ...
  const expectedFeishuConfigs = Array.isArray(options.expected_feishu_configs || options.expectedFeishuConfigs)
    ...
    : configuredFeishuConnections().map((item: any) => item.config);
  ...
  (preflight as any).freeze_complete = initialSample.freeze?.complete === true;
  (preflight as any).ready = Object.values(debt.counts).every((value: any) => Number(value || 0) === 0) && preflight.runner_healthy && preflight.feishu_healthy && (preflight as any).freeze_complete;
  if (cleanMode && !(preflight as any).ready) return { started: false, blocked: true, message: "干净 24 小时验收前置条件未满足", preflight, reconciliation };
  const state: any = {
    version: 1,
...
export function runSoakTestSelfTest() {
  ...
  const incidentState: any = { incidents: [], alerts: [] };
  ...
  const checks = {
    ...
    startupLagIsInformational: startupAlerts.some((item: any) => item.code === "event_loop_lag_startup" && item.severity === "info") && !startupAlerts.some((item: any) => item.code === "event_loop_lag_critical"),
    alertsCarryProcessEvidence: startupAlerts.every((item: any) => Number(item.data?.pid || 0) > 0 && item.data?.boot_id === "boot" && !!item.data?.reason),
