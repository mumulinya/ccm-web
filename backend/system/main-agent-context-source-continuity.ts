...
import { getKnowledgeDocumentChunks, getKnowledgeIndexStatus } from "../modules/knowledge/knowledge-index";
import { isKnowledgeDocumentAllowed, type AgentKnowledgeAccessContext } from "../modules/knowledge/knowledge-access";
import { loadKnowledgeMetadata } from "../modules/knowledge/knowledge-files";
import { buildSharedFilesContextV2, deleteSharedFileV2, listSharedFilesV2, readSharedFileChunkV2, readSharedFileV2, upsertSharedTextV2, type SharedFileScope } from "../modules/tools/shared-files-v2";
...
export type ContextSourceRestoreReceiptV1 = {
  ...
  restored: Array<{ sourceKind: ContextSourceKind; sourceId: string; documentName: string; tokens: number; truncated: boolean; drift: "none" | "revision" | "checksum" | "index_generation" }>;
  dropped: Array<{ sourceKind: ContextSourceKind; sourceId: string; documentName: string; reason: string }>;
  restoredAt: string;
  contentStored: false;
  checksum: string;
};

type ContextSourceStoreV2 = {
...
const ROOT = path.join(CCM_DIR, "main-agent-context-source-continuity");
const EMPTY_BUDGET: ContextSourceBudgetReceipt = Object.freeze({
...
function normalizeIdentity(input: any): MainAgentContinuityIdentityV1 {
  const agentKind = String(input?.agentKind || input?.scope || "");
  if (!["global", "project", "group"].includes(agentKind)) throw new Error("context_source_identity_scope_invalid");
  ...
  if (!scopeId || !exactSessionId) throw new Error("context_source_identity_exact_session_required");
  return { agentKind: agentKind as any, scope: agentKind as any, scopeId, exactSessionId, generation: Math.max(0, Math.floor(Number(input?.generation || 0))) };
}

function storeFile(identityInput: MainAgentContinuityIdentityV1) {
  const identity = normalizeIdentity(identityInput);
  return path.join(ROOT, `${identity.agentKind}-${hash([identity.scopeId, identity.exactSessionId]).slice(0, 40)}.json`);
}

function emptyStore(identity: MainAgentContinuityIdentityV1): ContextSourceStoreV2 {
...
function sanitizeReceipt(input: any, identity: MainAgentContinuityIdentityV1): ContextSourceReceiptV1 {
  const sourceKind: ContextSourceKind = input?.sourceKind === "shared_file" ? "shared_file" : "knowledge";
  const core: Omit<ContextSourceReceiptV1, "receiptChecksum"> = {
    schema: "ccm-context-source-read-receipt-v2",
    version: 2,
    ...
    chunkIds: Array.from(new Set<string>((Array.isArray(input?.chunkIds) ? input.chunkIds : []).map((value: any) => String(value)).filter(Boolean))).slice(0, 100),
    headings: Array.from(new Set<string>((Array.isArray(input?.headings) ? input.headings : []).map((value: any) => String(value)).filter(Boolean))).slice(0, 100),
    ...
    state: (["discovered", "read", "injected", "used", "ignored", "promoted", "restored"].includes(String(input?.state)) ? input.state : input?.injected ? "injected" : "read") as ContextSourceState,
    ...
    promotionEvidence: (Array.isArray(input?.promotionEvidence) ? input.promotionEvidence : []).map((item: any) => ({
    ...
    })).filter((item: ContextSourcePromotionEvidence) => item.memoryId && item.sourceRefChecksum).slice(-100),
...
function normalizeStore(value: any, expected: MainAgentContinuityIdentityV1): ContextSourceStoreV2 {
  if (!value || !["ccm-context-source-continuity-store-v1", "ccm-context-source-continuity-store-v2"].includes(value.schema)) return emptyStore(expected);
  let identity: MainAgentContinuityIdentityV1;
  try { identity = normalizeIdentity(value.identity); } catch { return emptyStore(expected); }
  if (identity.agentKind !== expected.agentKind || identity.scopeId !== expected.scopeId || identity.exactSessionId !== expected.exactSessionId) return emptyStore(expected);
  const effectiveIdentity = { ...identity, generation: expected.generation };
  return {
    ...emptyStore(effectiveIdentity),
    receipts: (Array.isArray(value.receipts) ? value.receipts : []).map((row: any) => sanitizeReceipt(row, effectiveIdentity)).slice(-500),
...
function readStore(identityInput: MainAgentContinuityIdentityV1) {
  ...
  for (const candidate of [file, `${file}.bak`]) {
    try {
      ...
      if (!checksum || checksum !== hash({ ...core, checksum: undefined })) continue;
...
function writeStore(store: ContextSourceStoreV2) {
  const core = { ...store, updatedAt: new Date().toISOString(), contentStored: false as const, checksum: "" };
  const next = { ...core, checksum: hash({ ...core, checksum: undefined }) };
  writeJsonAtomic(storeFile(store.identity), next);
  return next;
}

function mutateStore(identityInput: MainAgentContinuityIdentityV1, operation: (store: ContextSourceStoreV2) => void) {
  const identity = normalizeIdentity(identityInput);
  return withFileLock(storeFile(identity), () => {
...
export function calculateContextSourceBudget(input: {
...
export function recordContextSourceReceipts(identityInput: MainAgentContinuityIdentityV1, inputs: any[], budget?: Partial<ContextSourceBudgetReceipt>) {
  ...
  return mutateStore(identityInput, store => {
    for (const raw of Array.isArray(inputs) ? inputs : []) {
      ...
      if (!receipt.sourceId || !receipt.documentName) continue;
      ...
      if (index < 0) store.receipts.push(receipt);
      else store.receipts[index] = sanitizeReceipt({
...
export function recordSharedFileProjection(identity: MainAgentContinuityIdentityV1, projection: any, budget?: Partial<ContextSourceBudgetReceipt>) {
  const files = new Map((projection?.files || []).map((file: any) => [String(file.id), file]));
  return recordContextSourceReceipts(identity, (Array.isArray(projection?.selected_chunks) ? projection.selected_chunks : []).map((chunk: any) => {
...
export function markContextSourcesFromOutput(identityInput: MainAgentContinuityIdentityV1, output: string, _legacyPromoted = false) {
...
function normalizeSourceReference(input: any): ContextSourceReference | null {
  const sourceKind: ContextSourceKind = input?.sourceKind === "shared_file" || input?.source_kind === "shared_file" ? "shared_file" : "knowledge";
  const sourceId = String(input?.sourceId || input?.source_id || input?.documentName || input?.document_name || input?.file_id || "").trim();
  if (!sourceId) return null;
  const chunkIds = Array.from(new Set<string>([
...
export function extractStructuredContextSourceRefs(...values: any[]): ContextSourceReference[] {
  ...
  const visit = (value: any) => {
    ...
    if (typeof value === "object") {
      ...
      if (direct && (value.sourceKind || value.source_kind || value.sourceId || value.source_id || value.file_id)) refs.push(direct);
    ...
    for (const match of text.matchAll(/\[(?:source|shared-file):([^\]#/]+)(?:[\/#]([^\]]+))?\]/g)) {
      ...
      if (sourceId) refs.push({ sourceKind: shared ? "shared_file" : "knowledge", sourceId, chunkIds: chunkId ? [chunkId] : [] });
    }
  };
  values.forEach(visit);
  const uniqueRefs = new Map<string, ContextSourceReference>();
  for (const ref of refs) uniqueRefs.set(hash([ref.receiptId || "", ref.sourceKind, ref.sourceId, ref.chunkIds || [], ref.revision || "", ref.checksum || ""]), ref);
  return [...uniqueRefs.values()];
}

export function promoteContextSourceReceipts(input: {
  identity: MainAgentContinuityIdentityV1;
  sourceRefs: any[];
  memoryKind: ContextSourceMemoryKind;
  memoryId: string;
  ...
  if (!memoryId || !admissionChecksum || !refs.length) throw new Error("context_source_promotion_evidence_required");
  const promotedAt = new Date().toISOString();
  let matched = 0;
  let alreadyPromoted = 0;
  const unmatched: ContextSourceReference[] = [];
  const store = mutateStore(input.identity, next => {
    for (const ref of refs) {
      ...
      if (!refMatched) unmatched.push(ref);
    }
  });
  return { matched, alreadyPromoted, unmatched, storeChecksum: store.checksum, contentStored: false as const };
}

export function finalizeContextSourceRun(identityInput: MainAgentContinuityIdentityV1) {
  return mutateStore(identityInput, store => {
...
export function readContextSourceContinuity(identityInput: MainAgentContinuityIdentityV1) {
...
export function buildContextSourceManifestReference(identityInput: MainAgentContinuityIdentityV1) {
  const store = readStore(identityInput);
  const core = {
    schema: "ccm-context-source-restore-manifest-reference-v1" as const,
...
export function buildContextSourceCatalog(input: { sources: any[]; maxTokens: number; explicitText?: string; recentReceipts?: ContextSourceReceiptV1[] }) {
  ...
  let usedTokens = estimateTextTokens(lines[0]);
  let included = 0;
  const includedSources: any[] = [];
  ...
  if (included < sources.length) lines.push(`- 其余 ${sources.length - included} 个来源已延迟；使用知识检索或共享文件读取工具按 ID 获取。`);
  return { context: lines.join("\n"), usedTokens: estimateTextTokens(lines.join("\n")), included, deferred: sources.length - included, total: sources.length, includedSources, checksum: hash(sources.map(row => [row.sourceKind, row.sourceId, row.revision, row.checksum])) };
}

export function recordContextSourceCatalog(identity: MainAgentContinuityIdentityV1, catalog: any, budget?: Partial<ContextSourceBudgetReceipt>) {
  return recordContextSourceReceipts(identity, (catalog?.includedSources || []).map((source: any) => ({ ...source, state: "discovered", discoveredAt: new Date().toISOString(), injected: false, tokenCount: 0 })), { catalogUsedTokens: Number(catalog?.usedTokens || 0), ...(budget || {}) });
}

function truncatePreservingEdges(body: string, maxTokens: number) {
  if (estimateTextTokens(body) <= maxTokens) return { body, tokens: estimateTextTokens(body), truncated: false };
  ...
  while (low <= high) {
    ...
    if (estimateTextTokens(candidate) <= maxTokens) { selected = candidate; low = keep + 1; } else high = keep - 1;
  }
  return { body: selected, tokens: estimateTextTokens(selected), truncated: true };
}

export function restoreContextSources(input: {
  identity: MainAgentContinuityIdentityV1;
  ...
  const rawCandidates = store.identity.generation > 0
    ...
    : [];
  const groupedCandidates = new Map<string, ContextSourceReceiptV1>();
  ...
  const restored: ContextSourceRestoreReceiptV1["restored"] = [];
  const dropped: ContextSourceRestoreReceiptV1["dropped"] = [];
  const sections: string[] = [];
  let used = 0;
  const metadata = loadKnowledgeMetadata();
  const indexGeneration = String(getKnowledgeIndexStatus()?.activeGeneration || "");
  for (const candidate of candidates) {
    if (used >= maxTotal) { dropped.push({ sourceKind: candidate.sourceKind, sourceId: candidate.sourceId, documentName: candidate.documentName, reason: "total_budget_exhausted" }); continue; }
    let body = "";
    let currentRevision = "";
    let currentChecksum = "";
    let drift: ContextSourceRestoreReceiptV1["restored"][number]["drift"] = "none";
    try {
      if (candidate.sourceKind === "knowledge") {
        const current = metadata[candidate.documentName];
        if (!current || !input.knowledgeContext || !isKnowledgeDocumentAllowed(current, input.knowledgeContext)) throw new Error("authorization_or_source_missing");
        currentRevision = String(current.version || "");
        currentChecksum = String(current.content_hash || "");
        const chunks = getKnowledgeDocumentChunks(candidate.documentName);
        let selected = chunks.filter(chunk => candidate.chunkIds.includes(String(chunk.id)));
        if (!selected.length && candidate.headings.length) selected = chunks.filter(chunk => candidate.headings.includes(String(chunk.heading || "")));
        if (!selected.length) throw new Error("knowledge_chunk_missing");
        body = selected.map(chunk => `[source:${chunk.id}]\n文件：${candidate.documentName}${chunk.heading ? `；章节：${chunk.heading}` : ""}\n${chunk.text}`).join("\n\n");
        if (candidate.indexGeneration && indexGeneration && candidate.indexGeneration !== indexGeneration) drift = "index_generation";
      } else {
        const scope = candidate.identity.scope as SharedFileScope;
        const file = readSharedFileV2(scope, candidate.identity.scopeId, candidate.sourceId);
        if (!file) throw new Error("authorization_or_source_missing");
        currentRevision = String(file.revision || "");
        currentChecksum = String(file.checksum || "");
        const selected = candidate.chunkIds.map(chunkId => readSharedFileChunkV2(scope, candidate.identity.scopeId, candidate.sourceId, chunkId)).filter(Boolean) as any[];
        body = selected.length ? selected.map(item => `[shared-file:${item.file_id}/${item.chunk.id}]\n文件：${item.file_name}\n${item.content}`).join("\n\n") : String(file.content || "");
        if (!body) throw new Error("shared_file_content_missing");
      }
      if (currentRevision !== candidate.revision) drift = "revision";
      else if (currentChecksum !== candidate.checksum) drift = "checksum";
      const clipped = truncatePreservingEdges(body, Math.min(maxPerItem, maxTotal - used));
      if (!clipped.tokens) throw new Error("item_budget_exhausted");
  ...
  const status: ContextSourceRestoreReceiptV1["status"] = !candidates.length ? "not_required" : restored.length === candidates.length ? "restored" : restored.length ? "partial" : "rejected";
  const receiptCore = { schema: "ccm-post-compact-source-restore-receipt-v1" as const, version: 1 as const, identity: normalizeIdentity(input.identity), status, budget: { maxPerItemTokens: maxPerItem, maxTotalTokens: maxTotal, hydrationTargetTokens: Math.max(0, Number(input.hydrationTargetTokens || 0)), restoredTokens: used, remainingSafeTokens: Math.max(0, Number(input.remainingSafeTokens || 0) - used) }, restored, dropped, restoredAt: new Date().toISOString(), contentStored: false as const };
  const receipt: ContextSourceRestoreReceiptV1 = { ...receiptCore, checksum: hash(receiptCore) };
...
export function listContextSourceCatalogEntries(input: { sharedScope?: SharedFileScope; sharedScopeId?: string; knowledgeContext?: AgentKnowledgeAccessContext }) {
  const shared = input.sharedScope && input.sharedScopeId
    ? listSharedFilesV2(input.sharedScope, input.sharedScopeId).map((file: any) => ({ sourceKind: "shared_file", sourceId: file.id, documentName: file.name, revision: String(file.revision || ""), checksum: file.checksum || "", readable: file.readable === true }))
    : [];
  const metadata = loadKnowledgeMetadata();
  const knowledge = input.knowledgeContext ? Object.entries(metadata).filter(([, value]) => isKnowledgeDocumentAllowed(value, input.knowledgeContext!)).map(([filename, value]) => ({ sourceKind: "knowledge", sourceId: filename, documentName: filename, revision: String(value.version || ""), checksum: value.content_hash || "", readable: value.parse_status !== "failed" })) : [];
  return [...shared, ...knowledge];
}

export function clearContextSourceContinuity(identityInput: MainAgentContinuityIdentityV1) {
  const file = storeFile(identityInput);
  return withFileLock(file, () => {
    let removed = false;
    for (const candidate of [file, `${file}.bak`]) {
      try { if (fs.existsSync(candidate)) { fs.unlinkSync(candidate); removed = true; } } catch {}
    }
    return removed;
  }, { timeoutMs: 30_000 });
}

export function runContextSourceContinuitySelfTest() {
  const suffix = crypto.randomBytes(5).toString("hex");
  const identity: MainAgentContinuityIdentityV1 = { agentKind: "project", scope: "project", scopeId: `source-selftest-${suffix}`, exactSessionId: `session-${suffix}`, generation: 1 };
  ...
  try {
    ...
    const shared = upsertSharedTextV2("project", identity.scopeId, sharedName, "OLD_AUTHORITATIVE_SOURCE");
    sharedId = shared.id;
    const projection = buildSharedFilesContextV2("project", identity.scopeId, { maxTokens: 5_000 });
    ...
    upsertSharedTextV2("project", identity.scopeId, sharedName, "NEW_AUTHORITATIVE_SOURCE");
    ...
    if (sharedId) deleteSharedFileV2("project", identity.scopeId, sharedId);
