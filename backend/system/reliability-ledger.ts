...
const heldTaskLeaseTokens = new Map<string, { leaseId: string; fencingToken: number }>();
...
function ownerStillAlive(row: any) {
  if (String(row?.owner_host || "") !== HOSTNAME) return true;
...
function traceEventRow(row: any) {
  return {
...
export function getTrace(traceId: string) {
  ...
  const trace = db.prepare("SELECT * FROM reliability_traces_v2 WHERE trace_id = ?").get(id) as any;
  if (!trace) return sanitizeLegacyTrace(readJson(traceFile(id), null));
  const rows = db.prepare("SELECT * FROM reliability_trace_events_v2 WHERE trace_id = ? ORDER BY sequence DESC LIMIT 1200").all(id) as any[];
  return {
    version: 2,
...
export function getTracePage(traceId: string, options: { offset?: any; limit?: any } = {}) {
  const id = ensureTraceId(traceId);
  ...
  const trace = db.prepare("SELECT * FROM reliability_traces_v2 WHERE trace_id = ?").get(id) as any;
  if (!trace) {
    const legacy = sanitizeLegacyTrace(readJson(traceFile(id), null));
  ...
  const rows = db.prepare("SELECT * FROM reliability_trace_events_v2 WHERE trace_id = ? ORDER BY sequence LIMIT ? OFFSET ?").all(id, limit, offset) as any[];
...
export function appendTraceEvent(traceId: string, input: any) {
  const id = ensureTraceId(traceId);
  const sanitized = sanitizeTraceEvent(input);
  const eventId = sanitized.event.id || `evt_${Date.now().toString(36)}_${crypto.randomBytes(4).toString("hex")}`;
  const next = withImmediateObservabilityTransaction(db => {
    ...
    const duplicate = db.prepare("SELECT * FROM reliability_trace_events_v2 WHERE trace_id = ? AND event_id = ?").get(id, eventId) as any;
    if (duplicate) return traceEventRow(duplicate);
    const trace = db.prepare("SELECT last_sequence FROM reliability_traces_v2 WHERE trace_id = ?").get(id) as any;
    ...
    db.prepare(`INSERT INTO reliability_trace_events_v2(
      ...
      id, eventId, sequence, at, sanitized.event.type, sanitized.event.status,
      sanitized.event.task_id, sanitized.event.group_id, sanitized.event.agent, sanitized.event.runtime,
      sanitized.event.message, JSON.stringify(sanitized.event.data), sanitized.dataChecksum,
    );
...
export function listTraces(limit = 50) {
  ...
  const db = getObservabilityDatabase();
  const rows = db.prepare("SELECT * FROM reliability_traces_v2 ORDER BY updated_at DESC LIMIT ?").all(size) as any[];
  ...
  const legacy = fs.readdirSync(TRACE_DIR)
    ...
    .map(name => sanitizeLegacyTrace(readJson(path.join(TRACE_DIR, name), null)))
    .filter((item: any) => item && !seen.has(item.trace_id))
...
export function getIdempotencyRecord(scopeInput: string, key: string) {
  const scope = safeName(scopeInput);
  const row = getObservabilityDatabase().prepare("SELECT * FROM reliability_idempotency_v2 WHERE scope = ? AND key_checksum = ?").get(scope, digest(key)) as any;
...
export function acquireIdempotency(input: { scope: string; key: string; traceId?: string; leaseMs?: number; metadata?: any; retryFailed?: boolean }) {
  ...
  const outcome = withImmediateObservabilityTransaction(db => {
    const existing = db.prepare("SELECT * FROM reliability_idempotency_v2 WHERE scope = ? AND key_checksum = ?").get(scope, keyHash) as any;
    ...
    if (existing?.status === "completed" || active || (existing?.status === "failed" && input.retryFailed === false)) {
      ...
      const duplicate = db.prepare("SELECT * FROM reliability_idempotency_v2 WHERE scope = ? AND key_checksum = ?").get(scope, keyHash) as any;
    ...
    const metadata = sanitizeTraceValue({ ...parseJson(existing?.metadata_json, {}), ...(input.metadata || {}) });
    ...
    ON CONFLICT(scope, key_checksum) DO UPDATE SET
      ...
      metadata_json=excluded.metadata_json, result_json='{}'`).run(
      scope, keyHash, operationId, traceId, INSTANCE_ID, process.pid, HOSTNAME, attempt, fencingToken,
      Number(existing?.duplicate_count || 0), existing?.created_at || nowIso, nowIso, new Date(now + leaseMs).toISOString(), existing?.last_duplicate_at || null, JSON.stringify(metadata),
    );
    const record = db.prepare("SELECT * FROM reliability_idempotency_v2 WHERE scope = ? AND key_checksum = ?").get(scope, keyHash) as any;
...
function finishIdempotency(scopeInput: string, key: string, status: "completed" | "failed", result: any) {
  const scope = safeName(scopeInput);
  const keyHash = digest(key);
  const current = withImmediateObservabilityTransaction(db => {
    const row = db.prepare("SELECT * FROM reliability_idempotency_v2 WHERE scope = ? AND key_checksum = ?").get(scope, keyHash) as any;
    if (!row) throw new Error(`幂等操作不存在：${scope}`);
    if (row.status === "completed") return operationRow(row);
    if (row.status === "in_progress" && row.owner_id !== INSTANCE_ID) throw new Error(`幂等操作所有权已变化：${scope}`);
    const at = new Date().toISOString();
    db.prepare(`UPDATE reliability_idempotency_v2 SET status = ?, updated_at = ?, completed_at = ?, failed_at = ?,
      lease_expires_at = NULL, result_json = ? WHERE scope = ? AND key_checksum = ? AND fencing_token = ?`).run(
      status, at, status === "completed" ? at : null, status === "failed" ? at : null,
      JSON.stringify(sanitizeTraceValue(result)), scope, keyHash, row.fencing_token,
    );
...
export function settleIdempotencyByTrace(traceId: string, status: "completed" | "failed", result: any = {}, scopes: string[] = []) {
  ...
  const settled = withImmediateObservabilityTransaction(db => {
    const rows = db.prepare("SELECT * FROM reliability_idempotency_v2 WHERE trace_id = ? AND status = 'in_progress'").all(wanted) as any[];
    const selected = rows.filter(row => wantedScopes.size === 0 || wantedScopes.has(row.scope));
    const at = new Date().toISOString();
    for (const row of selected) db.prepare(`UPDATE reliability_idempotency_v2 SET status=?, updated_at=?, completed_at=?, failed_at=?, lease_expires_at=NULL, result_json=? WHERE scope=? AND key_checksum=? AND fencing_token=?`).run(status, at, status === "completed" ? at : null, status === "failed" ? at : null, JSON.stringify(sanitizeTraceValue(result)), row.scope, row.key_checksum, row.fencing_token);
  ...
  for (const row of settled) appendTraceEvent(row.trace_id, { id: `${row.operation_id}:${status}:recovered:${row.attempt}`, type: `idempotency.${status}_after_recovery`, status: status === "completed" ? "ok" : "error", message: `${row.scope} 在持久运行恢复后完成账本结算`, data: { scope: row.scope, attempt: row.attempt, fencing_token: row.fencing_token } });
  return settled;
}

export function getTaskLease(taskId: string) {
  const row = getObservabilityDatabase().prepare("SELECT * FROM reliability_task_leases_v2 WHERE task_id = ?").get(String(taskId || "")) as any;
  return leaseRow(row) || readJson(leaseFile(taskId), null);
}

export function listTaskLeases() {
  return (getObservabilityDatabase().prepare("SELECT * FROM reliability_task_leases_v2 ORDER BY updated_at DESC").all() as any[]).map(leaseRow);
}

export function acquireTaskLease(taskIdInput: string, traceId: string, ttlMs = 45_000) {
  const taskId = String(taskIdInput || "").trim();
  if (!taskId) throw new Error("任务租约缺少taskId");
  const now = Date.now();
  const at = new Date(now).toISOString();
  const ttl = Math.max(10_000, Math.min(10 * 60_000, Number(ttlMs || 45_000)));
  const outcome = withImmediateObservabilityTransaction(db => {
    const existing = db.prepare("SELECT * FROM reliability_task_leases_v2 WHERE task_id = ?").get(taskId) as any;
    const active = existing?.status === "active" && Date.parse(existing.expires_at || 0) > now;
    if (active && existing.owner_id !== INSTANCE_ID) return { acquired: false, lease: leaseRow(existing), recovered: false };
    if (active && existing.owner_id === INSTANCE_ID) return { acquired: true, lease: leaseRow(existing), recovered: false };
    const fencingToken = Number(existing?.fencing_token || 0) + 1;
    ...
    db.prepare(`INSERT INTO reliability_task_leases_v2(task_id, lease_id, fencing_token, trace_id, status, owner_id, owner_pid, owner_host, acquired_at, renewed_at, expires_at, released_at, final_status, recovery_count, updated_at)
      ...
      expires_at=excluded.expires_at, released_at=NULL, final_status='', recovery_count=excluded.recovery_count, updated_at=excluded.updated_at`).run(
      taskId, leaseId, fencingToken, ensureTraceId(existing?.trace_id || traceId, "task"), INSTANCE_ID, process.pid, HOSTNAME,
      at, at, new Date(now + ttl).toISOString(), recoveryCount, at,
    );
...
export function renewTaskLease(taskIdInput: string, ttlMs = 45_000) {
  const taskId = String(taskIdInput || "").trim();
  const held = heldTaskLeaseTokens.get(taskId);
  if (!held) return false;
  const now = Date.now();
  const result = getObservabilityDatabase().prepare(`UPDATE reliability_task_leases_v2 SET renewed_at=?, expires_at=?, updated_at=?
    WHERE task_id=? AND status='active' AND owner_id=? AND lease_id=? AND fencing_token=?`).run(
    new Date(now).toISOString(), new Date(now + Math.max(10_000, Number(ttlMs || 45_000))).toISOString(), new Date(now).toISOString(),
    taskId, INSTANCE_ID, held.leaseId, held.fencingToken,
  );
  return result.changes === 1;
}

export function releaseTaskLease(taskIdInput: string, finalStatus = "released") {
  const taskId = String(taskIdInput || "").trim();
  const held = heldTaskLeaseTokens.get(taskId);
  if (!held) return false;
  const at = new Date().toISOString();
  const db = getObservabilityDatabase();
  const before = db.prepare("SELECT * FROM reliability_task_leases_v2 WHERE task_id=?").get(taskId) as any;
...
export function getReliabilityLedgerStats() {
  const db = getObservabilityDatabase();
  const operations = db.prepare("SELECT * FROM reliability_idempotency_v2").all() as any[];
  const leases = db.prepare("SELECT * FROM reliability_task_leases_v2").all() as any[];
  const traces = db.prepare("SELECT COUNT(*) total, COALESCE(SUM(event_count), 0) events FROM reliability_traces_v2").get() as any;
  const now = Date.now();
...
export function reconcileReliabilityLedgerDebt(reason = "稳定性验收前清理失效账本") {
  const at = new Date().toISOString();
  const db = getObservabilityDatabase();
  const operationRows = (db.prepare("SELECT * FROM reliability_idempotency_v2 WHERE status='in_progress'").all() as any[]).filter(row => Date.parse(row.lease_expires_at || 0) <= Date.now() || !ownerStillAlive(row));
  const leaseRows = (db.prepare("SELECT * FROM reliability_task_leases_v2 WHERE status='active'").all() as any[]).filter(row => Date.parse(row.expires_at || 0) <= Date.now() || !ownerStillAlive(row));
  withImmediateObservabilityTransaction(tx => {
    for (const row of operationRows) tx.prepare("UPDATE reliability_idempotency_v2 SET status='failed', updated_at=?, failed_at=?, lease_expires_at=NULL, result_json=? WHERE scope=? AND key_checksum=? AND fencing_token=?").run(at, at, JSON.stringify({ error: reason, reconciled: true, previous_owner_pid: row.owner_pid }), row.scope, row.key_checksum, row.fencing_token);
    for (const row of leaseRows) tx.prepare("UPDATE reliability_task_leases_v2 SET status='released', final_status='orphaned_reconciled', released_at=?, updated_at=?, expires_at=NULL WHERE task_id=? AND fencing_token=?").run(at, at, row.task_id, row.fencing_token);
  });
  for (const row of operationRows) appendTraceEvent(row.trace_id, { type: "idempotency.stale_reconciled", status: "warning", message: reason, data: { operation_id: row.operation_id, scope: row.scope, previous_owner_pid: row.owner_pid, fencing_token: row.fencing_token } });
  for (const row of leaseRows) appendTraceEvent(row.trace_id, { type: "task.lease_stale_reconciled", status: "warning", task_id: row.task_id, message: reason, data: { previous_owner_pid: row.owner_pid, fencing_token: row.fencing_token } });
