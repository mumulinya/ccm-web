...
export const OBSERVABILITY_DATABASE_FILE = path.join(STORE_DIR, "ccm.db");
...
function createSchema(db: Database.Database) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS metric_events_v3 (
      ...
      first_visible_feedback_ms INTEGER NOT NULL DEFAULT 0,
      first_token_ms INTEGER NOT NULL DEFAULT 0,
      max_silent_gap_ms INTEGER NOT NULL DEFAULT 0,
      provider_retry_count INTEGER NOT NULL DEFAULT 0,
      fallback_stream_count INTEGER NOT NULL DEFAULT 0,
      initial_read_file_count INTEGER NOT NULL DEFAULT 0,
      initial_read_tokens INTEGER NOT NULL DEFAULT 0,
      partial_output_before_cancel INTEGER NOT NULL DEFAULT 0,
  ...
  const ensureColumns = (table: string, columns: Record<string, string>) => {
    const existing = new Set((db.prepare(`PRAGMA table_info(${table})`).all() as any[]).map(row => String(row.name || "")));
...
export function getObservabilityDatabase(): any {
  if (database) return database;
  fs.mkdirSync(path.dirname(OBSERVABILITY_DATABASE_FILE), { recursive: true });
  database = new Database(OBSERVABILITY_DATABASE_FILE);
...
export function withImmediateObservabilityTransaction<T>(operation: (db: any) => T): T {
...
export function observabilityMeta(key: string, fallback: any = null) {
  const row = getObservabilityDatabase().prepare("SELECT value_json FROM observability_meta_v2 WHERE key = ?").get(key) as any;
  if (!row) return fallback;
  try { return JSON.parse(String(row.value_json || "null")); } catch { return fallback; }
}

export function setObservabilityMeta(key: string, value: any) {
...
export function closeObservabilityDatabaseForTests() {
  if (!database) return;
  database.close();
  database = null;
}
