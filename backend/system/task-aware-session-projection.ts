...
export function buildTaskAwareSessionProjection(input: { messages: any[]; sessionTaskIndex?: CcmSessionTaskIndexV1 | null; currentTaskId?: string }) {
  const messages = Array.isArray(input.messages) ? input.messages : [];
  const index = input.sessionTaskIndex;
  const currentTaskId = String(input.currentTaskId || "").trim();
  if (!index || !currentTaskId) return { messages, currentTaskMessageIds: [], priorTaskMessageIds: [], priorTaskSummaries: [] };
