...
export function turnForLegacyJsonLoop(turn: ProviderAgentTurn) {
  ...
  if (turn.toolCalls.length) return JSON.stringify({ responseType: "tool_calls", toolRequests: turn.toolCalls.map(item => ({ name: item.name, arguments: item.arguments, reason: "Provider原生工具调用" })), providerToolCallIds: turn.toolCalls.map(item => item.id) });
  return "";
}
