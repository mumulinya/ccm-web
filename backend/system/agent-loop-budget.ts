export const AGENT_LOOP_BUDGET_SCHEMA = "ccm-agent-loop-budget-v1" as const;
...
function configuredNumber(input: any, camelKey: string, snakeKey: string, fallback: number) {
  const raw = input?.[camelKey] ?? input?.[snakeKey];
  const value = raw === undefined || raw === null || raw === "" ? fallback : Number(raw);
  return Number.isFinite(value) ? value : fallback;
}

export function resolveAgentLoopBudget(input: any = {}): AgentLoopBudget {
  ...
  return {
    schema: AGENT_LOOP_BUDGET_SCHEMA,
    ...
    maxToolRounds,
    toolBatchSize: Math.max(1, Math.min(8, Math.floor(configuredNumber(input, "toolBatchSize", "tool_batch_size", 2)))),
    readOnlyParallelism: Math.max(1, Math.min(8, Math.floor(configuredNumber(input, "readOnlyParallelism", "read_only_parallelism", 2)))),
...
export function shouldContinueAgentLoop(input: {
  ...
  if (input.cancelled) return { continue: false, reason: "cancelled", resetSegment: false };
  if (input.authorizationBlocked) return { continue: false, reason: "authorization_blocked", resetSegment: false };
  if (input.contextSafe === false) return { continue: false, reason: "context_safety_gate", resetSegment: false };
  if (input.repeatedFailure) return { continue: false, reason: "repeated_failure", resetSegment: false };
  if (Number(input.noProgressCount || 0) >= input.budget.noProgressThreshold) return { continue: false, reason: "no_progress", resetSegment: false };
  if (input.waitingDependency || input.dependenciesReady === false) return { continue: false, reason: "waiting_dependency", resetSegment: false };
  if (input.unresolvedCriteria === 0) return { continue: false, reason: "acceptance_satisfied", resetSegment: false };
  ...
  if (input.round >= input.budget.maxToolRounds) return { continue: false, reason: "tool_round_budget", resetSegment: false };
  if (input.modelTurns >= input.budget.maxModelTurns) return { continue: false, reason: "model_turn_budget", resetSegment: false };
  if (input.toolCalls >= input.budget.toolCallBudget) return { continue: false, reason: "tool_call_budget", resetSegment: false };
  if (Number(input.elapsedMs || 0) >= input.budget.timeBudgetMs) return { continue: false, reason: "time_budget", resetSegment: false };
  return { continue: true, reason: "budget_available", resetSegment: false };
}

export function runAgentLoopBudgetSelfTest() {
