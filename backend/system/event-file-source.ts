...
import { getWorkDirForProject } from "../core/utils";
...
function normalizedPath(value: any) {
  ...
  if (SENSITIVE_FILE_PATTERN.test(result)) throw Object.assign(new Error("敏感文件不支持在线查看"), { statusCode: 403 });
  return result;
}

function git(cwd: string, args: string[]) {
  return spawnSync("git", args, { cwd, encoding: "utf-8", windowsHide: true, maxBuffer: 4 * 1024 * 1024 });
}

function safeFileFromRoot(rootValue: string, filePath: string) {
  const root = path.resolve(rootValue);
  const target = path.resolve(root, filePath);
  if (target === root || !target.startsWith(`${root}${path.sep}`) || !fs.existsSync(target)) return null;
  ...
  if (content.includes(0)) throw Object.assign(new Error("二进制文件不支持文本预览"), { statusCode: 409 });
  return content.toString("utf-8");
}

function attemptFor(event: UserVisibleAgentEvent) {
  return Math.max(0, Number(event.attempt || event.detail?.agentDisplay?.attempt || event.detail?.executionStage?.attempt || 0));
}

function fromActiveWorktree(event: UserVisibleAgentEvent, project: string, filePath: string) {
  if (!event.taskId) return null;
  const attempt = attemptFor(event);
  const executions = listExecutions({ taskId: event.taskId })
    .filter((record: any) => (!project || String(record?.project || "") === project)
  ...
  for (const execution of executions) {
    ...
    if (content != null) return { content, freshness: "active_worktree" as const };
  }
  return null;
}

function fromAcceptedDelivery(event: UserVisibleAgentEvent, project: string, filePath: string) {
  if (!event.taskId) return null;
  const attempt = attemptFor(event);
  const manifests = listExecutionRecoveryManifests({ taskIds: [event.taskId] })
  ...
  for (const manifest of manifests) {
    ...
    if (!repoRoot || !delivery || !fs.existsSync(repoRoot)) continue;
    const result = git(repoRoot, ["show", `${delivery}:${filePath}`]);
    if (result.status !== 0) continue;
    const content = String(result.stdout || "");
    if (Buffer.byteLength(content, "utf-8") > MAX_BYTES || content.includes("\0")) continue;
    return { content, freshness: "accepted_delivery" as const };
  }
  return null;
}

function fromCurrentAuthority(project: string, filePath: string) {
  const root = String(getWorkDirForProject(project) || "");
  if (!root || !fs.existsSync(root)) return null;
  const content = safeFileFromRoot(root, filePath);
  return content == null ? null : { content, freshness: "current_authority" as const };
}

export function projectEventFileSource(event: UserVisibleAgentEvent, projectHint = "") {
  const evidence = event.detail?.fileReadEvidence;
  if (!evidence?.path) throw Object.assign(new Error("该事件没有可验证的文件读取证据"), { statusCode: 409 });
  ...
  if (!project) throw Object.assign(new Error("文件读取事件缺少明确的项目归属"), { statusCode: 409 });
  ...
  if (!source) throw Object.assign(new Error("当前内容不可恢复；对应 worktree、交付引用和项目文件均不可用"), { statusCode: 409 });
  ...
  return {
    ...
    checksum,
    freshness: observedChecksum && observedChecksum !== checksum ? "drifted" as const : "current" as const,
