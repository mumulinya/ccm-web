...
import { providerCacheAdapterPublicSummary } from "./provider-context-cache-adapters";
import { providerCacheCapabilityIdentity, pruneProviderCacheCapabilityRegistry, readProviderCacheCapabilityState } from "./provider-cache-capability-registry";
import {
  estimateModelMessagesTokens,
  estimateModelTextTokens,
  recordModelTokenCalibrationForIdentity,
} from "./model-token-preflight";
...
export type ProviderContextCacheExecutionMode =
  | "native_api_context_management"
  | "provider_prompt_cache"
  | "provider_implicit_cache"
  | "provider_explicit_cache"
  | "stable_prefix_cache"
  | "ccm_controlled_projection"
  | "disabled";
...
type ContextMaterialization = {
  ...
  hits: number;
  source: "computed" | "memory_hot_cache" | "shared_state";
};

const hotMaterializations = new Map<string, ContextMaterialization>();
const materializationFlights = new Map<string, Promise<any>>();
...
function contentText(value: any) {
...
function stateFile(binding: ProviderContextCacheBinding) {
...
function bindingKey(binding: ProviderContextCacheBinding) {
...
function normalizeBinding(options: ProviderContextCacheOptions): ProviderContextCacheBinding {
  return {
...
function atomicWriteJson(file: string, value: any) {
  writeJsonAtomic(file, value);
  try { fs.chmodSync(file, 0o600); } catch {}
}

function appendReceipt(value: any) {
  withFileLock(LEDGER_FILE, () => {
    ...
    try { fs.chmodSync(LEDGER_FILE, 0o600); } catch {}
  }, { timeoutMs: 10_000, staleMs: 60_000 });
}

function appendMaintenanceReceipt(value: any) {
  withFileLock(MAINTENANCE_LEDGER_FILE, () => {
...
function blockKind(message: any, index: number) {
  const role = String(message?.role || "").toLowerCase();
  const type = String(message?.type || "").toLowerCase();
  const explicit = String(message?.contextBlockType || message?.context_block_type || "").trim().toLowerCase();
  if (["system", "rules", "skill", "mcp", "long_term_memory", "conversation", "tool_use", "tool_result", "summary", "recovery"].includes(explicit)) return explicit;
  const content = contentText(message?.content);
  if (role === "system") return "system";
  if (/tool[_-]?result|web_search_tool_result/.test(type)) return "tool_result";
  if (/tool[_-]?(use|call)|function_call/.test(type)) return "tool_use";
  if (/正式模型摘要|压缩摘要|canonical_summary/i.test(content.slice(0, 400))) return "summary";
  if (/恢复上下文|recovery_context|post.?compact/i.test(content.slice(0, 400))) return "recovery";
  if (/\bskills?\b|技能/i.test(content.slice(0, 2000))) return "skill";
  if (/\bmcp\b|mcp__|工具目录|工具定义/i.test(content.slice(0, 2000))) return "mcp";
  if (/长期记忆|long[_ -]?term[_ -]?memory|typed memory/i.test(content.slice(0, 2000))) return "long_term_memory";
  return index === 0 && role === "system" ? "system" : "conversation";
}

function logicalBlockId(message: any, index: number, kind: string) {
...
const STABLE_KIND_PRIORITY: Record<string, number> = {
  system: 100,
  rules: 92,
  skill: 84,
  mcp: 80,
...
function blockStability(previous: any, contentChecksum: string, kind: string, role: string) {
...
function adaptiveStablePrefixProjection(messages: any[], previousBlocks: any[], enabled = true) {
  const previous = new Map((previousBlocks || []).map((block: any) => [String(block.id), block]));
  let leadingSystemCount = 0;
  while (leadingSystemCount < messages.length && String(messages[leadingSystemCount]?.role || "").toLowerCase() === "system") leadingSystemCount += 1;
  const prefixRows = messages.slice(0, leadingSystemCount).map((message, originalIndex) => {
    ...
    const prior: any = previous.get(id);
  ...
  const projectedPrefix = enabled
    ? [...prefixRows].sort((left, right) => {
      if (left.originalIndex === 0 && left.kind === "system") return -1;
      if (right.originalIndex === 0 && right.kind === "system") return 1;
      if (left.stability.prefixEligible !== right.stability.prefixEligible) return left.stability.prefixEligible ? -1 : 1;
      if (left.stability.stabilityScore !== right.stability.stabilityScore) return right.stability.stabilityScore - left.stability.stabilityScore;
...
function blockFacets(message: any, kind: string, content: string) {
  const facets = new Set<string>([kind]);
  const sample = content.slice(0, 24_000);
  if (/\b(?:rules?|policy|instruction|constraint|permission|authorization)\b|规则|约束|权限|边界/i.test(sample)) facets.add("rules");
  if (/\bskills?\b|技能/i.test(sample)) facets.add("skills");
  if (/\bmcp\b|mcp__|工具目录|工具定义/i.test(sample)) facets.add("mcp_tools");
  if (/subagent|子\s*agent|成员项目|agent catalog/i.test(sample)) facets.add("subagent_definitions");
  return [...facets];
}

function immutableBlocks(messages: any[], protectedRecentMessages: number, tokenConfig: any, originalPositions: number[] = [], previousBlocks: any[] = []) {
  const previous = new Map((previousBlocks || []).map((block: any) => [String(block.id), block]));
  ...
  return messages.map((message, index) => {
    ...
    return {
      ...
      position: index,
      originalPosition,
      tokens: estimateModelTextTokens(content, tokenConfig).safetyAdjustedTokens,
      contentChecksum,
...
function materializationInputChecksum(messages: any[], binding: ProviderContextCacheBinding, options: ProviderContextCacheOptions, tokenConfig: any) {
  return checksum({
...
function evictHotMaterializations(now = Date.now()) {
  for (const [key, item] of hotMaterializations) {
    if (now - item.lastAccessAtMs <= HOT_CACHE_TTL_MS) continue;
  ...
  while (ordered.length && (hotMaterializations.size > HOT_CACHE_MAX_ENTRIES || bytes > HOT_CACHE_MAX_BYTES)) {
    ...
    if (!hotMaterializations.delete(key)) continue;
    bytes -= Number(item.approximateBytes || 0);
    hotCacheMetrics.evictions += 1;
  }
}

function clearSupersededHotEpochs(binding: ProviderContextCacheBinding) {
  const current = bindingKey(binding);
  const identityPrefix = `${binding.scope}\0${binding.scopeId || ""}\0${binding.sessionId}\0`;
  let cleared = 0;
  for (const [key, item] of hotMaterializations) {
    if (!item.bindingKey.startsWith(identityPrefix) || item.bindingKey === current) continue;
    hotMaterializations.delete(key);
    cleared += 1;
  }
  if (cleared) hotCacheMetrics.evictions += cleared;
  return cleared;
}

function copyMaterializedMessages(messages: any[]) {
  return messages.map(message => message && typeof message === "object" ? { ...message } : message);
}

function materializeContext(messages: any[], binding: ProviderContextCacheBinding, options: ProviderContextCacheOptions, tokenConfig: any, previous: any) {
  ...
  if (sharedStateHit) {
    const positions = previous.adaptiveStablePrefix.projectedOriginalPositions.map((value: any) => Number(value));
    const projectedMessages = positions.map((position: number) => messages[position]).filter((message: any) => message !== undefined);
    if (projectedMessages.length === messages.length) {
      const materialized: ContextMaterialization = {
        key,
        bindingKey: bindingKey(binding),
        messages: projectedMessages,
        blocks: previous.blocks.map((block: any) => ({ ...block })),
  ...
  const tokenPreflight = estimateModelMessagesTokens(projection.messages, tokenConfig);
...
function requestedMode(value: any): ProviderContextCacheMode {
  const mode = String(value || "auto").trim().toLowerCase();
  return ["auto", "native", "controlled", "off"].includes(mode) ? mode as ProviderContextCacheMode : "auto";
}

function resolveExecutionMode(options: ProviderContextCacheOptions) {
  ...
  if (requested === "off") return { requested, mode: "disabled" as ProviderContextCacheExecutionMode, nativeReady, downgradeReason: "disabled_by_configuration" };
  if ((requested === "auto" || requested === "native") && nativeReady) {
    return { requested, mode: "native_api_context_management" as ProviderContextCacheExecutionMode, nativeReady, downgradeReason: "" };
  }
  const adapter = String(options.adapterCapability?.adapter || "");
  if (requested !== "controlled" && adapter === "openai_prompt_cache") {
    return { requested, mode: "provider_prompt_cache" as ProviderContextCacheExecutionMode, nativeReady, downgradeReason: "" };
  }
  if (requested !== "controlled" && adapter === "gemini_implicit_cache") {
    return { requested, mode: "provider_implicit_cache" as ProviderContextCacheExecutionMode, nativeReady, downgradeReason: "" };
  }
  if (requested !== "controlled" && adapter === "gemini_explicit_cache") {
    return { requested, mode: "provider_explicit_cache" as ProviderContextCacheExecutionMode, nativeReady, downgradeReason: "" };
  }
  if (requested !== "controlled" && adapter === "stable_prefix") {
    return { requested, mode: "stable_prefix_cache" as ProviderContextCacheExecutionMode, nativeReady, downgradeReason: "provider_native_cache_unproven_stable_prefix_only" };
  }
  return {
    requested,
    mode: "ccm_controlled_projection" as ProviderContextCacheExecutionMode,
    nativeReady,
    downgradeReason: requested === "native" ? "provider_native_context_management_unavailable" : "provider_uses_ccm_materialized_context",
  };
}

function buildEdits(previousBlocks: any[], blocks: any[]) {
  ...
  for (const block of blocks) {
    const prior: any = previous.get(block.id);
    if (!prior) edits.push({ action: "insert", blockId: block.id, address: block.immutableAddress, tokens: block.tokens, protected: block.protected });
    else if (prior.contentChecksum === block.contentChecksum && Number(prior.position) === Number(block.position)) edits.push({ action: "keep", blockId: block.id, address: block.immutableAddress, tokens: block.tokens, protected: block.protected });
    else if (prior.contentChecksum === block.contentChecksum) edits.push({ action: "move", blockId: block.id, fromPosition: Number(prior.position), position: Number(block.position), address: block.immutableAddress, tokens: block.tokens, protected: block.protected });
    else edits.push({ action: "replace", blockId: block.id, fromAddress: prior.immutableAddress, address: block.immutableAddress, tokens: block.tokens, protected: block.protected });
  }
  for (const block of previousBlocks) {
    if (!current.has(String(block.id))) edits.push({ action: "delete", blockId: block.id, fromAddress: block.immutableAddress, tokens: block.tokens, protected: block.protected === true });
  }
  return edits;
}

function planChecksum(plan: any) {
  const payload = { ...(plan || {}) };
...
export function verifyProviderNeutralContextCachePlan(plan: any, expected: Partial<ProviderContextCacheBinding> = {}) {
...
function prepareProviderNeutralContextCacheRequestLocked(messagesInput: any[], options: ProviderContextCacheOptions, binding: ProviderContextCacheBinding, previous: any, file: string) {
  ...
  if (previous && !previousSameEpoch) clearSupersededHotEpochs(binding);
  const tokenConfig = {
  ...
  if (availableInputTokens > 0 && totalTokens > availableInputTokens) {
    const error: any = new Error(`Context Engine V2 Token 门禁拒绝请求：${totalTokens} > ${availableInputTokens}；必须先完成正式模型压缩`);
    error.code = "CONTEXT_PLAN_TOKEN_GATE_REQUIRES_FORMAL_COMPACTION";
    throw error;
  }
  const cacheIdentity = providerCacheCapabilityIdentity({
    apiUrl: options.apiUrl,
    format: options.format,
    model: options.model,
    providerNativeCacheFamily: options.providerNativeCacheFamily,
  ...
  const plan: ContextPlanV2 = {
    ...
    stablePrefixBlockCount: (() => {
      let count = 0;
      for (const block of blocks) {
        if (block.role !== "system" || block.prefixEligible !== true) break;
        count += 1;
      }
      return count;
    })(),
...
export function prepareProviderNeutralContextCacheRequest(messagesInput: any[], options: ProviderContextCacheOptions) {
  const messages = Array.isArray(messagesInput) ? messagesInput : [];
  if (!cleanIdentity(options.sessionId)) return { messages, plan: null };
  maybeRunProviderNeutralContextCacheMaintenance();
  const binding = normalizeBinding(options);
  const file = stateFile(binding);
  return withFileLock(file, () => prepareProviderNeutralContextCacheRequestLocked(messages, options, binding, readJson(file), file), {
    timeoutMs: 30_000,
    retryMs: 20,
    staleMs: 2 * 60_000,
  });
...
export async function prepareProviderNeutralContextCacheRequestSingleflight(messagesInput: any[], options: ProviderContextCacheOptions) {
  const messages = Array.isArray(messagesInput) ? messagesInput : [];
  if (!cleanIdentity(options.sessionId)) return { messages, plan: null };
  const binding = normalizeBinding(options);
  const tokenConfig = {
  ...
  try {
    ...
    if (materializationFlights.get(flightKey) === owner) materializationFlights.delete(flightKey);
  }
}

function estimatedInputCostUsd(plan: any, usage: any) {
...
function nextRollingMetrics(current: any, completion: any, plan: any) {
  const previous = current?.rollingMetrics || {};
  const samples = Math.max(0, Number(previous.samples || 0));
  const nextSamples = samples + 1;
  const average = (previousValue: any, nextValue: any) => Math.round((((Number(previousValue || 0) * samples) + Number(nextValue || 0)) / nextSamples) * 1000) / 1000;
  return {
...
function contextCacheRecommendation(plan: any, completion: any, rolling: any) {
...
export function completeProviderNeutralContextCacheRequest(plan: any, input: { ok: boolean; usage?: any; error?: any; providerRequestId?: string; adapterEvidence?: any } = { ok: true }) {
  if (!plan?.requestId || !["ccm-provider-neutral-context-cache-plan-v1", "ccm-context-plan-v2"].includes(String(plan?.schema || ""))) return null;
  ...
  const completion: any = {
    ...
    reusedBlockCount: plan.edits.filter((edit: any) => edit.action === "keep").length,
    changedBlockCount: plan.edits.filter((edit: any) => edit.action !== "keep").length,
  ...
  if (input.ok && observedInputTokens > 0) {
    try {
      calibration = recordModelTokenCalibrationForIdentity(String(plan.providerCapabilityIdentityChecksum || ""), {
  ...
  const binding: ProviderContextCacheBinding = {
    scope: plan.scope,
    scopeId: plan.scopeId,
    sessionId: plan.sessionId,
    generation: plan.generation,
  ...
  if (!completion.receiptChecksum) completion.receiptChecksum = checksum(completion);
...
export function readProviderNeutralContextCacheRuntimeStatus() {
  evictHotMaterializations();
  return {
...
export function clearProviderNeutralContextHotCache(binding: Partial<ProviderContextCacheBinding> = {}) {
  let cleared = 0;
  for (const [key, item] of hotMaterializations) {
    const parts = item.bindingKey.split("\0");
    if (binding.scope && parts[0] !== binding.scope) continue;
    if (binding.scopeId && parts[1] !== binding.scopeId) continue;
    if (binding.sessionId && parts[2] !== binding.sessionId) continue;
    hotMaterializations.delete(key);
    cleared += 1;
  }
...
export function invalidateProviderNeutralContextCacheState(binding: ProviderContextCacheBinding, reason = "session_deleted") {
  const file = stateFile(binding);
...
function runProviderNeutralContextCacheMaintenanceLocked(options: { now?: number; stateRetentionDays?: number; archiveRetentionDays?: number; dryRun?: boolean } = {}) {
  ...
  for (const name of stateFiles) {
    ...
    if (stale) staleStates += 1;
    if (corrupt) corruptStates += 1;
    if (dryRun) continue;
    try {
      if (corrupt) {
      ...
      try { if (fs.existsSync(`${file}.bak`)) fs.unlinkSync(`${file}.bak`); } catch {}
      deletedStates += 1;
    } catch {}
  }
  ...
  for (const name of archives) {
    ...
    let modifiedAt = now;
    try { modifiedAt = fs.statSync(file).mtimeMs; } catch {}
    if (now - modifiedAt <= archiveRetentionMs) continue;
    expiredArchives += 1;
    if (!dryRun) try { fs.unlinkSync(file); } catch {}
  }
  evictHotMaterializations(now);
  const capability = dryRun ? { removedEntries: 0, removedAttempts: 0, dryRun: true } : pruneProviderCacheCapabilityRegistry({ now, expiredRetentionDays: 30 });
  const result = {
    dryRun,
  ...
  if (!dryRun) appendMaintenanceReceipt({ action: "retention_cleanup", ...result });
  lastAutomaticMaintenanceAt = now;
  return result;
}

export function runProviderNeutralContextCacheMaintenance(options: { now?: number; stateRetentionDays?: number; archiveRetentionDays?: number; dryRun?: boolean } = {}) {
  const maintenanceLease = path.join(CACHE_DIR, "maintenance-lease");
  return withFileLock(
    maintenanceLease,
    () => runProviderNeutralContextCacheMaintenanceLocked(options),
    { timeoutMs: 60_000, retryMs: 50, staleMs: 10 * 60_000 },
  );
}

function maybeRunProviderNeutralContextCacheMaintenance() {
  if (Date.now() - lastAutomaticMaintenanceAt < 6 * 60 * 60_000) return;
  try { runProviderNeutralContextCacheMaintenance(); } catch {}
}

export function readLatestProviderNeutralContextCacheState(binding: ProviderContextCacheBinding) {
  return readJson(stateFile(binding));
}

export function readContextEngineV2Status(binding: ProviderContextCacheBinding, config: any = {}) {
  ...
  return {
    ...
    plan: state ? {
      ...
      blocks: blocks.map((block: any) => ({
        ...
        facets: Array.isArray(block.facets) ? block.facets.map((item: any) => cleanIdentity(item)).filter(Boolean).slice(0, 12) : [],
    ...
    adapter: providerCacheAdapterPublicSummary(config).active,
...
export function providerNeutralContextCacheCapability(config: any = {}) {
  ...
  return {
    ...
    adapterV2: providerCacheAdapterPublicSummary(config),
    rawTranscriptPreserved: true,
    contentStored: false,
  };
}

export function runProviderNeutralContextCacheSelfTest() {
  ...
  const checks = {
    ...
    secondRequestReusesAllBlocks: second.plan.edits.every((edit: any) => edit.action === "keep"),
    ...
    rawTranscriptNeverStoredInBlocks: first.plan.blocks.every((block: any) => block.contentStored === false && !("content" in block)),
