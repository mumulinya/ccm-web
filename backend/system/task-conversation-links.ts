...
import { listGroupChatSessions, loadGroups } from "../modules/collaboration/storage";
import { getSessionDetail } from "../modules/projects/sessions";
...
export type TaskMutationConflictCode =
...
export type TaskMutationGuardResult =
  | { valid: true; revision: number; generation: number; bindingChecksum: string }
  | { valid: false; status: 409; code: TaskMutationConflictCode; error: string; details: Record<string, any> };
...
function globalSessionAvailable(exactSessionId: string) {
  ...
  return (Array.isArray(store?.sessions) ? store.sessions : []).some((session: any) => String(session?.id || "") === exactSessionId);
}

function targetAvailability(scope: TaskConversationScope, scopeId: string, exactSessionId: string) {
  if (!scopeId || !exactSessionId) return { available: false, reason: "任务没有可定位的自动化会话" };
  if (scope === "global") return globalSessionAvailable(exactSessionId)
    ? { available: true, reason: "" }
    : { available: false, reason: "原全局会话不存在或已归档" };
  if (scope === "group") {
    const group = loadGroups().find((item: any) => String(item?.id || "") === scopeId);
    if (!group) return { available: false, reason: "目标群聊不存在或无权访问" };
    const session = listGroupChatSessions(scopeId).sessions.find((item: any) => String(item?.id || "") === exactSessionId);
    if (!session || session.archived === true) return { available: false, reason: "目标群聊自动化会话不存在或已归档" };
    return { available: true, reason: "" };
  }
  if (!getConfigs().some((item: any) => String(item?.name || "") === scopeId)) return { available: false, reason: "目标项目不存在或已归档" };
  const session: any = getSessionDetail(scopeId, exactSessionId);
  if (!session || session.archived === true) return { available: false, reason: "目标项目自动化会话不存在或已归档" };
  return { available: true, reason: "" };
}

function normalizeRef(value: any) {
  if (!value || typeof value !== "object") return null;
  const scope = clean(value.scope, 16) as TaskConversationScope;
  ...
  if (!["global", "project", "group"].includes(scope) || !scopeId || !exactSessionId) return null;
  return {
    scope,
    scopeId,
    exactSessionId,
...
function sourceRefForTask(task: any, mission: any) {
  ...
  const sessionId = clean(
    task?.workflow_meta?.global_mission?.source_session_id
      || mission?.workflow_meta?.intake?.session_id
      || mission?.origin_session_id,
    240,
  );
  return sessionId ? { scope: "global" as const, scopeId: "global-agent", exactSessionId: sessionId, messageId: "", title: "全局 Agent 任务" } : null;
}

function targetRefForTask(task: any) {
  const explicit = normalizeRef(task?.target_conversation_ref || task?.targetConversationRef);
  if (explicit) return explicit;
  if (task?.group_id && task?.group_session_id) return {
...
function buildLink(task: any, relation: "source" | "target", ref: any, missionId: string): TaskConversationLinkV1 {
  ...
  const base = {
    schema: "ccm-task-conversation-link-v1" as const,
    relation,
    taskId: clean(task?.id, 160),
    ...(missionId ? { missionId } : {}),
    scope: ref.scope as TaskConversationScope,
    scopeId: ref.scopeId,
...
export function buildTaskConversationLinks(taskOrId: any, tasksInput?: any[]) {
  const tasks = Array.isArray(tasksInput) ? tasksInput : loadTasks();
  const task = typeof taskOrId === "string" ? tasks.find((item: any) => String(item?.id || "") === taskOrId) : taskOrId;
  if (!task) return null;
  const missionId = clean(task.global_mission_id || task.globalMissionId || (task.workflow_type === "global_mission" ? task.id : ""), 160);
  const mission = missionId ? tasks.find((item: any) => String(item?.id || "") === missionId) : null;
  const links: TaskConversationLinkV1[] = [];
  const source = sourceRefForTask(task, mission);
  const target = targetRefForTask(task);
  if (source) links.push(buildLink(task, "source", source, missionId));
  if (target) links.push(buildLink(task, "target", target, missionId));
  return {
 ...
 */
export function validateTaskMutationGuard(task: any, payload: any = {}, options: { requireTarget?: boolean } = {}): TaskMutationGuardResult {
  ...
  const bindingChecksum = clean(
    task?.automation_session_binding_snapshot?.bindingChecksum
      || task?.automation_session_binding_snapshot?.binding_checksum
      || target?.bindingChecksum,
    128,
  );
...
function normalizeFile(value: any, fallbackProject = "") {
...
export function buildGlobalMissionSafeProjection(mission: any, children: any[] = [], supervisor: any = null) {
  ...
  for (const child of children) {
    ...
    for (const raw of [...(delivery.actual_file_changes || []), ...(delivery.file_changes || []), ...(child?.file_changes?.files || [])]) {
      ...
      if (file) files.set(`${file.project}:${file.path}`, file);
    }
    for (const value of delivery.verification_executed || delivery.verification || []) {
      ...
      if (text && !verification.includes(text)) verification.push(text);
    }
    for (const value of delivery.risks || []) { const text = clean(value, 320); if (text && !risks.includes(text)) risks.push(text); }
    for (const value of delivery.remaining_items || delivery.remainingItems || []) { const text = clean(value, 320); if (text && !remainingItems.includes(text)) remainingItems.push(text); }
  }
  ...
  for (const value of report.risks || []) { const text = clean(value, 320); if (text && !risks.includes(text)) risks.push(text); }
  for (const value of report.remaining_items || report.remainingItems || []) { const text = clean(value, 320); if (text && !remainingItems.includes(text)) remainingItems.push(text); }
