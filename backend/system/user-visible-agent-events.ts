...
function toolPresentation(toolNameInput: any, args: any = {}) {
  ...
  const target = compactText(
    args?.path || args?.file_path || args?.filePath || args?.symbol || args?.query || args?.pattern
      || args?.skill || args?.skill_name || args?.tool_name || args?.toolName || args?.work_item_id || args?.workItemId || "",
    300,
  );
  return { title, target };
}
