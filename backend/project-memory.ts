...
export function buildProjectMemoryPacket(project: string, options: { workDir?: string; resources?: any; query?: string } = {}) {
  ...
  if (memory.conclusions?.length) {
    ...
    for (const item of memory.conclusions.slice(-3)) lines.push(`  - [${item.status || "unknown"}] ${item.summary || "无摘要"}${item.filesModified?.length ? `；文件：${item.filesModified.slice(0, 8).join("、")}` : ""}`);
  }
