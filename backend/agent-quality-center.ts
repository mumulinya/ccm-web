...
const QUALITY_DIR = process.env.CCM_AGENT_QUALITY_DIR || path.join(CCM_DIR, "agent-quality");
...
function clamp(value: any, fallback = 0) { return Math.max(0, Math.min(1, Number.isFinite(Number(value)) ? Number(value) : fallback)); }
...
export function getAgentQualityPolicy(): AgentQualityPolicy {
  const fallback: AgentQualityPolicy = {
    version: 1,
    shadowMode: false,
  ...
  try {
    if (!fs.existsSync(POLICY_FILE)) return fallback;
...
export function setAgentQualityPolicy(input: Partial<AgentQualityPolicy> & { actor?: string; reason: string }) {
  if (!String(input.reason || "").trim()) throw new Error("修改 Agent 质量策略必须填写原因");
  const current = getAgentQualityPolicy();
  const next: AgentQualityPolicy = {
    ...current,
...
export function normalizeAgentDecisionIntent(value: any, message: string): AgentDecisionIntent {
  const fallback = fallbackIntent(message);
  const category = String(value?.category || fallback.category).toLowerCase() as AgentIntentCategory;
  return {
...
function targetValues(toolName: string, args: any) {
  ...
  const nested = Array.isArray(args?.targets)
    ? args.targets.flatMap((item: any) => [item?.project, item?.group_id, item?.groupId, item?.name]).map((value: any) => String(value || "").trim()).filter(Boolean)
    : [];
...
function isGrounded(value: string, message: string, priorSteps: any[]) {
...
export function evaluateAgentDecision(input: {
  message: string;
  decision: any;
  toolName?: string;
  args?: any;
  ...
  const policy = { ...getAgentQualityPolicy(), ...(input.policyOverride || {}) } as AgentQualityPolicy;
...
export function recordAgentDecision(input: any) {
  ...
  if (records.length > MAX_DECISION_RECORDS) atomicWrite(DECISIONS_FILE, records.slice(-MAX_DECISION_RECORDS).map(row => JSON.stringify(row)).join("\n") + "\n");
  return record;
}

function rate(numerator: number, denominator: number) { return denominator > 0 ? Math.round(numerator / denominator * 10_000) / 100 : 0; }

export function buildAgentQualitySnapshot(input: { tasks?: any[]; sessions?: any[] } = {}) {
  ...
  const conflictEvent = (task: any) => (task.workflow_timeline || task.delivery_summary?.timeline || []).find((event: any) => event?.type === "conflict_plan" && event?.data?.protected);
