...
function updatedAt(task: any) {
  return Date.parse(task?.updated_at || task?.updatedAt || task?.created_at || task?.createdAt || "") || 0;
}

function exactTaskSession(task: any, scope: ConversationRouteScope) {
  if (scope === "project") return text(task?.project_session_id || task?.projectSessionId || task?.exact_session_id || task?.exactSessionId || task?.origin_session_id || task?.originSessionId);
  if (scope === "group") return text(task?.group_session_id || task?.groupSessionId || task?.exact_session_id || task?.exactSessionId || task?.origin_session_id || task?.originSessionId);
...
function taskScopeId(task: any, scope: ConversationRouteScope) {
  if (scope === "project") return text(task?.target_project || task?.targetProject || task?.project || task?.project_id || task?.projectId);
  if (scope === "group") return text(task?.group_id || task?.groupId || task?.scope_id || task?.scopeId);
  return "global";
}

function isExplicitlyAbandoned(task: any) {
...
function isRecoverable(task: any) {
  if (!task || isExplicitlyAbandoned(task)) return false;
  const status = lower(task.status);
  ...
  if (receipt?.recoverable === true) return true;
  if (acceptance === "recovery_required") return true;
...
function targetExpanded(workflowDecision: any, candidate: any) {
  ...
  const requestedProjects = (Array.isArray(workflowDecision?.targetRefs) ? workflowDecision.targetRefs : [])
    .filter((item: any) => lower(item?.scope) === "project")
    .map((item: any) => text(item?.scopeId || item?.scope_id || item?.id))
    .filter(Boolean);
  return requestedProjects.some((project: string) => !candidateProjects.has(project));
}
...
export function conversationRouteAuditChecksum(value: any) {
  return crypto.createHash("sha256").update(JSON.stringify(value || {})).digest("hex");
}

export function runConversationMessageRoutingSelfTest() {
  const candidate = {
    id: "task-recoverable",
  ...
  const decision = (confidence: number, extra: any = {}) => ({
    mode: "execute",
    actionRequired: true,
