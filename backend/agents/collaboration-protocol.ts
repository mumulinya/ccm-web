...
export const AGENT_COLLABORATION_LIMITS = {
...
export function collaborationQuestionFingerprint(input: any) {
...
export function selectCollaborationTarget(input: {
  request: any;
  group: any;
  ...
  const members = (input.group?.members || [])
    ...
    .filter((name: string) => name && name !== input.sourceProject);
...
export function buildCollaborationQuestionContract(input: any) {
  const now = new Date();
  const deadlineMs = Math.max(30_000, Math.min(30 * 60 * 1000, Number(input?.deadline_ms || input?.deadlineMs || AGENT_COLLABORATION_LIMITS.defaultDeadlineMs)));
  const parentPath = Array.isArray(input?.hop_path) ? input.hop_path.map((item: any) => text(item, 120)).filter(Boolean) : [];
  const contract: any = {
    ...
    limits: { ...AGENT_COLLABORATION_LIMITS },
  };
  contract.fingerprint = collaborationQuestionFingerprint(contract);
  return contract;
}

export function evaluateCollaborationQuestionAdmission(contract: any, existingItems: any[] = []) {
  if (!contract?.question || contract.question.length < 4) return { allowed: false, code: "invalid_question", reason: "问题过短或为空" };
  if (!contract?.from_agent || !contract?.to_agent || contract.from_agent === contract.to_agent) return { allowed: false, code: "invalid_route", reason: "询问双方无效或形成自问" };
  if (Number(contract.depth || 0) > AGENT_COLLABORATION_LIMITS.maxDepth) return { allowed: false, code: "depth_limit", reason: "Agent 询问深度已达到上限" };
  if ((contract.hop_path || []).filter((item: string) => item === contract.to_agent).length > 0) return { allowed: false, code: "loop_detected", reason: "询问路径将形成 Agent 环路" };

  const taskItems = existingItems.filter(item => String(item?.task_id || "") === contract.task_id);
  if (taskItems.length >= AGENT_COLLABORATION_LIMITS.maxQuestionsPerTask) return { allowed: false, code: "task_budget", reason: "本任务 Agent 问答预算已耗尽" };
  const pairItems = taskItems.filter(item => item?.from_agent === contract.from_agent && item?.to_agent === contract.to_agent);
  if (pairItems.length >= AGENT_COLLABORATION_LIMITS.maxPairRepeats) return { allowed: false, code: "pair_budget", reason: "同一 Agent 对之间的询问次数已达到上限" };
  ...
  if (duplicate) return { allowed: false, code: "duplicate", reason: "相同问题已经存在，复用已有问答", existing_id: duplicate.id || duplicate.question_id };
  return { allowed: true, code: "accepted", reason: "问答协议门禁通过" };
}

export function evaluateCollaborationAnswer(answer: any, contract: any, siblingAnswers: any[] = []) {
  const answerText = text(answer?.answer ?? answer, AGENT_COLLABORATION_LIMITS.maxAnswerChars);
...
export function evaluateAdvisoryPermissionBoundary(fileChanges: any[] = [], beforeTools: any = {}, afterTools: any = {}) {
...
export function evaluateCollaborationTimeout(contract: any, now: Date | string | number = new Date()) {
...
export function runAgentCollaborationProtocolSelfTest() {
