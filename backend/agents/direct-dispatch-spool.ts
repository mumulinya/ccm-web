...
export const DIRECT_AGENT_DISPATCH_REQUEST_SCHEMA = "ccm-direct-agent-dispatch-request-v1";
export const DIRECT_AGENT_DISPATCH_RESULT_SCHEMA = "ccm-direct-agent-dispatch-result-v1";
export const DIRECT_AGENT_DISPATCH_TRANSCRIPT_SCHEMA = "ccm-direct-agent-dispatch-transcript-event-v1";

const transcriptState = new Map<string, { sequence: number; checksum: string; bytes: number; streamBytes: number }>();
...
function safeTerminalSummary(value: any) {
  return String(value || "")
    .replace(/\x1B\[[0-?]*[ -/]*[@-~]/g, "")
...
function recordChecksum(value: any) {
  const payload = { ...(value || {}) };
...
function validDispatchId(id: string) {
...
function writeRecord(file: string, record: any) {
  const next = { ...record };
...
function compactTranscriptValue(value: any, depth = 0): any {
  if (depth > 4) return "[depth-limited]";
  if (typeof value === "string") return value.slice(0, TRANSCRIPT_EVENT_TEXT_MAX);
  if (typeof value === "number" || typeof value === "boolean" || value === null) return value;
  if (Array.isArray(value)) return value.slice(0, 24).map(item => compactTranscriptValue(item, depth + 1));
  if (!value || typeof value !== "object") return String(value ?? "").slice(0, TRANSCRIPT_EVENT_TEXT_MAX);
  return Object.keys(value).slice(0, 32).reduce((result: any, key) => {
    result[key] = compactTranscriptValue(value[key], depth + 1);
    return result;
  }, {});
}

function transcriptEventChecksum(event: any) {
  const payload = { ...(event || {}) };
  delete payload.event_checksum;
  return checksum(payload);
}

export function readDirectAgentDispatchTranscript(id: string, options: any = {}) {
  if (!validDispatchId(id)) return { id, valid: false, issues: ["dispatch_id_invalid"], events: [], bytes: 0, file: "" };
  const file = transcriptFile(id);
  if (!fs.existsSync(file)) return { id, valid: true, issues: [], events: [], bytes: 0, file, last_event: null, stream_bytes: 0 };
  const issues: string[] = [];
  const events: any[] = [];
  ...
  try {
    ...
    for (const line of content.split(/\r?\n/).filter(Boolean)) {
      let event: any = null;
      try { event = JSON.parse(line); } catch { issues.push("transcript_json_invalid"); break; }
      if (event.schema !== DIRECT_AGENT_DISPATCH_TRANSCRIPT_SCHEMA) issues.push("transcript_schema_invalid");
      if (String(event.id || "") !== id) issues.push("transcript_dispatch_id_mismatch");
      if (Number(event.sequence || 0) !== expectedSequence) issues.push("transcript_sequence_invalid");
      if (String(event.previous_checksum || "") !== previousChecksum) issues.push("transcript_chain_invalid");
      if (String(event.event_checksum || "") !== transcriptEventChecksum(event)) issues.push("transcript_checksum_invalid");
      if (["stdout", "stderr"].includes(String(event.type || ""))) streamBytes += Number(event.payload_bytes || 0);
      events.push(event);
...
export function appendDirectAgentDispatchTranscript(id: string, type: string, payload: any = {}) {
  if (!validDispatchId(id) || !String(type || "").trim()) return null;
  try {
    let state = transcriptState.get(id);
    if (!state) {
      const current = readDirectAgentDispatchTranscript(id, { limit: 1 });
      if (current.valid !== true) return null;
      state = {
    ...
    if (state.bytes + Buffer.byteLength(line) > TRANSCRIPT_MAX_BYTES) return null;
...
export function createDirectAgentDispatchRequest(input: any = {}) {
  ...
  const request = writeRecord(requestFile(id), {
    schema: DIRECT_AGENT_DISPATCH_REQUEST_SCHEMA,
    version: 1,
    id,
...
export function markDirectAgentDispatchStarted(id: string, input: any = {}) {
  const file = requestFile(id);
  const current = readRecord(file);
  if (!current?.checksum_valid || current.schema !== DIRECT_AGENT_DISPATCH_REQUEST_SCHEMA) return null;
  if (current.status === "running" || current.status === "done" || current.status === "failed") return current;
...
export function completeDirectAgentDispatch(id: string, input: any = {}) {
  ...
  const result = writeRecord(resultFile(id), {
    schema: DIRECT_AGENT_DISPATCH_RESULT_SCHEMA,
    version: 1,
    id,
    transport: "server_direct_cli",
    success,
...
export function readDirectAgentDispatchRequest(id: string) {
  return readRecord(requestFile(id));
}

export function readDirectAgentDispatchResult(id: string) {
  return readRecord(resultFile(id));
}

export function validateDirectAgentDispatchPair(request: any, result: any) {
  const issues: string[] = [];
  if (request?.schema !== DIRECT_AGENT_DISPATCH_REQUEST_SCHEMA) issues.push("request_schema_invalid");
  if (result?.schema !== DIRECT_AGENT_DISPATCH_RESULT_SCHEMA) issues.push("result_schema_invalid");
  if (String(request?.record_checksum || "") !== recordChecksum(request)) issues.push("request_checksum_invalid");
  if (String(result?.record_checksum || "") !== recordChecksum(result)) issues.push("result_checksum_invalid");
  if (!request?.started_at || !result?.started_at) issues.push("dispatch_start_missing");
  if (String(request?.id || "") !== String(result?.id || "")) issues.push("request_result_id_mismatch");
...
export function listDirectAgentDispatchSpool(options: any = {}) {
...
export function cancelPreparedDirectAgentDispatch(id: string, input: any = {}) {
  const current = readDirectAgentDispatchRequest(id);
  if (!current?.checksum_valid || current.schema !== DIRECT_AGENT_DISPATCH_REQUEST_SCHEMA) throw new Error("direct dispatch request invalid");
  if (String(current.status || "") !== "prepared" || current.started_at) throw new Error("only prepared unstarted direct dispatch can be cancelled");
  if (readDirectAgentDispatchResult(id)) throw new Error("prepared direct dispatch already has a result");
...
export function pruneDirectAgentDispatchTerminalPair(id: string) {
  const request = readDirectAgentDispatchRequest(id);
  const result = readDirectAgentDispatchResult(id);
...
export function pruneDirectAgentDispatchSpool(options: any = {}) {
  if (!fs.existsSync(REQUESTS_DIR)) return { deleted_count: 0, deleted: [] };
  ...
  for (const entry of fs.readdirSync(REQUESTS_DIR, { withFileTypes: true })) {
    if (!entry.isFile() || !entry.name.startsWith("adr_") || !entry.name.endsWith(".json")) continue;
    const request = readRecord(path.join(REQUESTS_DIR, entry.name));
    if (!request?.checksum_valid || request.schema !== DIRECT_AGENT_DISPATCH_REQUEST_SCHEMA || !["done", "failed", "cancelled"].includes(String(request.status || ""))) continue;
    const completedAt = Date.parse(String(request.completed_at || ""));
    if (!Number.isFinite(completedAt) || nowMs - completedAt < retentionMs) continue;
    const result = readRecord(resultFile(String(request.id || "")));
    const pair = result ? validateDirectAgentDispatchPair(request, result) : { valid: false, issues: [] as string[] };
    ...
    if (pair.valid !== true && !cancellableTerminal && !preSpawnTerminal) continue;
    for (const file of [request.file, result?.file, transcriptFile(String(request.id || ""))]) {
      if (!file || !fs.existsSync(file)) continue;
      try { fs.unlinkSync(file); deleted.push(file); } catch {}
    }
