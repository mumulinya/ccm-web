...
import {
  ...
  createExecutionRecoveryManifest,
  finalizeExecutionRecoveryManifests,
  previewExecutionRecovery,
  purgeExecutionRecoveryManifests,
  readExecutionRecoveryManifest,
...
const KERNEL_DIR = path.join(CCM_DIR, "execution-kernel");
...
function executionFile(executionId: string) {
  ensureDirs();
  return path.join(EXECUTIONS_DIR, `${safePart(executionId)}.json`);
}

function checkpointFile(checkpointId: string) {
  ensureDirs();
  return path.join(CHECKPOINTS_DIR, `${safePart(checkpointId)}.json`);
}

function cancellationFile(taskId: string) {
  ensureDirs();
...
function runGit(workDir: string, args: string[], env: Record<string, string> = {}) {
...
function normalizeList(value: any, limit = 100) {
...
function defaultEnvAllowlist() {
  return [
    ...
    "ANTHROPIC_API_KEY", "ANTHROPIC_AUTH_TOKEN", "ANTHROPIC_BASE_URL", "ANTHROPIC_MODEL", "OPENAI_API_KEY", "OPENAI_BASE_URL", "CODEX_HOME",
...
export function sanitizeExecutionEnv(extra: Record<string, any> = {}, allowlist: string[] = []) {
  const allowed = new Set([...defaultEnvAllowlist(), ...allowlist.map(String)]);
  const env: Record<string, string> = {};
  for (const [key, value] of Object.entries(process.env)) {
    if (allowed.has(key) && value !== undefined) env[key] = String(value);
  }
  for (const [key, value] of Object.entries(extra || {})) {
    if (allowed.has(key) && value !== undefined && value !== null) env[key] = String(value);
  }
  return env;
}

export function isSafeVerificationCommand(command: string) {
...
export function buildDevelopmentTaskPacket(task: any, options: any = {}): DevelopmentTaskPacket {
  ...
  return {
    ...
    verification: {
      ...
      requiredGreenLevel: String(task?.required_green_level || task?.requiredGreenLevel || (requiredVerification ? "project" : "none")) as GreenLevel,
    },
    commitPolicy: String(task?.commit_policy || task?.commitPolicy || (isolationMode === "worktree" ? "verified_commit" : "manual")) as any,
    permissions: {
    ...
    escalationPolicy: String(task?.escalation_policy || task?.escalationPolicy || "ask_user") as any,
    acceptanceCriteria: String(task?.acceptance_criteria || task?.acceptanceCriteria || "").trim(),
    createdAt: now(),
  };
}

export function validateDevelopmentTaskPacket(packet: DevelopmentTaskPacket) {
  const errors: string[] = [];
  if (!packet.taskId) errors.push("taskId 不能为空");
  if (!packet.objective) errors.push("objective 不能为空");
  if (!packet.project) errors.push("project 不能为空");
  if (!packet.workDir || !fs.existsSync(packet.workDir)) errors.push("workDir 不存在");
  if (packet.scope.allowedPaths.some(item => item.includes(".."))) errors.push("allowedPaths 不能越过工作目录");
  if (packet.verification.commands.some(command => !isSafeVerificationCommand(command))) errors.push("验证命令包含危险操作");
  if (!["none", "targeted", "project", "workspace", "merge_ready"].includes(packet.verification.requiredGreenLevel)) errors.push("requiredGreenLevel 无效");
  return { pass: errors.length === 0, errors };
}

function createEvent(record: ExecutionRecord, name: string, message: string, extra: any = {}): ExecutionEvent {
  return {
...
export function ensureExecution(input: { task: any; project: string; agent?: string; workDir: string; executionId?: string; packet?: DevelopmentTaskPacket }) {
  ...
  if (existing) return existing as ExecutionRecord;
  ...
  if (!validation.pass) throw new Error(`任务包校验失败：${validation.errors.join("；")}`);
  const createdAt = now();
...
export function loadExecution(executionId: string) {
  return readJson(executionFile(executionId), null) as ExecutionRecord | null;
}

export function listExecutions(filters: any = {}) {
  ensureDirs();
  const records = fs.readdirSync(EXECUTIONS_DIR).filter(name => name.endsWith(".json"))
    .map(name => readJson(path.join(EXECUTIONS_DIR, name), null)).filter(Boolean) as ExecutionRecord[];
  return records.filter(record => !filters.taskId || record.taskId === filters.taskId)
    .sort((a, b) => String(b.updatedAt).localeCompare(String(a.updatedAt)));
}

export function purgeTaskExecutionArtifacts(taskId: string) {
  ...
  for (const record of records) {
    for (const checkpointId of record.checkpointIds || []) {
      try { fs.unlinkSync(checkpointFile(checkpointId)); checkpoints++; } catch {}
    }
    try { fs.unlinkSync(executionFile(record.id)); } catch {}
  }
  const outputPrefix = `${safePart(id)}-`;
  try {
    for (const name of fs.readdirSync(OUTPUTS_DIR)) {
      if (!name.startsWith(outputPrefix)) continue;
      try { fs.unlinkSync(path.join(OUTPUTS_DIR, name)); outputs++; } catch {}
    }
  } catch {}
  clearTaskCancellation(id);
  activeProcesses.delete(id);
  const recoveryManifests = purgeExecutionRecoveryManifests(id);
  return { executions: records.length, checkpoints, outputs, recoveryManifests };
}

export function transitionExecution(executionId: string, state: ExecutionState, message = "", extra: any = {}) {
  const record = loadExecution(executionId);
  if (!record) return null;
  const at = now();
  record.state = state;
  record.updatedAt = at;
  if (!record.startedAt && ["spawning", "ready", "prompt_accepted", "running"].includes(state)) record.startedAt = at;
  if (["succeeded", "failed", "cancelled"].includes(state)) record.finishedAt = at;
  if (extra.failure) record.failure = extra.failure;
  if (extra.green) record.green = extra.green;
  if (Object.prototype.hasOwnProperty.call(extra, "receipt")) record.receipt = extra.receipt;
  if (Object.prototype.hasOwnProperty.call(extra, "fileChanges")) record.fileChanges = extra.fileChanges;
  if (Object.prototype.hasOwnProperty.call(extra, "runnerVerification")) record.runnerVerification = extra.runnerVerification;
  if (Object.prototype.hasOwnProperty.call(extra, "outputPreview")) record.outputPreview = String(extra.outputPreview || "").slice(0, 12_000);
  if (extra.cancellation) record.cancellation = extra.cancellation;
  if (state === "succeeded" && (record.checkpointIds || []).length) {
    try {
      ...
      finalizeExecutionRecoveryManifests(record, { workDir });
    } catch {}
  }
...
export function attachExecutionWorkspace(executionId: string, workspace: any) {
  const record = loadExecution(executionId);
  if (!record) return null;
...
export function registerExternalRunnerRequest(executionId: string, requestId: string) {
  const record = loadExecution(executionId);
  if (!record) return;
...
function registerProcess(taskId: string, child: ChildProcess) {
  const group = activeProcesses.get(taskId) || new Set<ChildProcess>();
  group.add(child);
  activeProcesses.set(taskId, group);
}

function unregisterProcess(taskId: string, child: ChildProcess) {
...
export function cancelActiveAgentRun(input: any = {}) {
  ...
  if (!runId && !taskId && !executionId && !project) throw new Error("缺少 runId、taskId、executionId 或 project");
  const matched = Array.from(activeAgentRuns.values()).filter(run => {
    ...
    if (executionId && run.executionId !== executionId) return false;
    if (project && run.project !== project) return false;
    return true;
  });
  let killed = 0;
...
export function trackManagedChildProcess(taskId: string, executionId: string, child: ChildProcess, meta: any = {}) {
  ...
  const cancellationPoll = setInterval(() => {
    if (!isTaskCancellationRequested(safeTaskId)) return;
...
export function terminateManagedChildProcess(child: ChildProcess) {
  return killProcessTree(child);
}

export function clearTaskCancellation(taskId: string) {
  try { fs.unlinkSync(cancellationFile(taskId)); } catch {}
}

export function isTaskCancellationRequested(taskId: string) {
  return !!taskId && fs.existsSync(cancellationFile(taskId));
}

export function requestTaskCancellation(taskId: string, reason = "用户取消任务", actor = "local-user") {
  if (!taskId) throw new Error("缺少任务 ID");
  const requestedAt = now();
  writeJsonAtomic(cancellationFile(taskId), { taskId, reason, actor, requestedAt });
  let killed = 0;
  for (const child of activeProcesses.get(taskId) || []) if (killProcessTree(child)) killed++;
  ...
  try {
    for (const name of fs.readdirSync(AGENT_RUNNER_REQUESTS_DIR).filter(name => name.endsWith(".json"))) {
      ...
      if (!request || String(request.taskId || request.executionId || "") !== taskId || ["done", "failed", "cancelled"].includes(request.status)) continue;
  ...
  for (const execution of executions) {
    if (["succeeded", "failed", "cancelled"].includes(execution.state)) continue;
    transitionExecution(
      execution.id,
      hasLiveCancellationTarget ? "cancel_requested" : "cancelled",
      reason,
      { cancellation: { reason, actor, requestedAt }, status: "warning" }
    );
  }
...
export async function runManagedCommand(input: {
  ...
  if (isTaskCancellationRequested(taskId)) throw Object.assign(new Error("任务已取消"), { code: "CCM_CANCELLED" });
  if (executionId) transitionExecution(executionId, "spawning", "正在启动项目 Agent");
  ...
  const persistChunk = (stream: "stdout" | "stderr", chunk: Buffer) => {
    if (!outputFile) {
      ensureDirs();
      outputFile = path.join(OUTPUTS_DIR, `${safePart(taskId)}-${Date.now()}.log`);
      outputStream = fs.createWriteStream(outputFile, { flags: "a" });
      if (stdout) outputStream.write(`[stdout]\n${stdout}\n`);
      if (stderr) outputStream.write(`[stderr]\n${stderr}\n`);
    }
  ...
  child.stdout?.on("data", (chunk: Buffer) => {
    ...
    if (stdoutBytes + stderrBytes > maxOutputBytes) persistChunk("stdout", chunk);
    if (Buffer.byteLength(stdout) < hardMemoryLimit) stdout += text;
    else stdout = (stdout + text).slice(-512 * 1024);
    input.onStdout?.(text);
  });
  child.stderr?.on("data", (chunk: Buffer) => {
    const text = chunk.toString("utf-8");
    stderrBytes += chunk.length;
    if (stdoutBytes + stderrBytes > maxOutputBytes) persistChunk("stderr", chunk);
    if (Buffer.byteLength(stderr) < hardMemoryLimit) stderr += text;
  ...
  return await new Promise<any>((resolve, reject) => {
    let settled = false;
    let cancelled = false;
    const finish = (error: any, code: number | null, signal: string | null) => {
      if (settled) return;
      settled = true;
    ...
    const cancelPoll = setInterval(() => {
      if (!isTaskCancellationRequested(taskId)) return;
      cancelled = true;
      killProcessTree(child);
    ...
    child.on("close", (code, signal) => {
      ...
      if (code !== 0) return finish(Object.assign(new Error(stderr.trim() || stdout.trim() || `Agent 进程退出，exitCode=${code}`), { code: "CCM_PROCESS_FAILED" }), code, signal);
...
export function classifyExecutionFailure(value: any): { failureClass: FailureClass; recoverable: boolean; recovery: string[]; message: string } {
  const message = String(value?.message || value?.error || value || "未知错误");
  const rules: Array<[RegExp, FailureClass, boolean, string[]]> = [
  ...
  for (const [pattern, failureClass, recoverable, recovery] of rules) if (pattern.test(message)) return { failureClass, recoverable, recovery, message: message.slice(0, 1200) };
  return { failureClass: "unknown", recoverable: false, recovery: ["保留现场并请求人工检查"], message: message.slice(0, 1200) };
}

function greenRank(level: GreenLevel) {
  return { none: 0, targeted: 1, project: 2, workspace: 3, merge_ready: 4 }[level];
}

export function evaluateGreenContract(input: any) {
  const checks: any[] = [];
  ...
  const runnerPassed = runner.status === "passed" || (Array.isArray(runner.results) && runner.results.length > 0 && runner.results.every((item: any) => item.status === "passed"));
  ...
  const workspacePassed = !!input.workspacePassed || (runnerPassed && (runner.results || []).some((item: any) => /workspace|npm run build|npm test|mvn test|cargo test|go test|pytest/i.test(String(item.command || ""))));
  ...
  let level: GreenLevel = "none";
  if (receiptDone && (input.requiresVerification === false || hasExecutedVerification)) level = "targeted";
  if (level === "targeted" && (input.requiresVerification === false || !runnerFailed)) level = "project";
  if (level === "project" && workspacePassed) level = "workspace";
  if (level === "workspace" && branchFresh && reviewPassed && (input.requiresChanges === false || hasChanges)) level = "merge_ready";
  const requiredLevel = String(input.requiredLevel || "project") as GreenLevel;
  return { level, requiredLevel, pass: greenRank(level) >= greenRank(requiredLevel), checks, evaluatedAt: now() };
}

export function createExecutionCheckpoint(input: { executionId: string; taskId: string; workDir: string; mode?: string; label?: string }) {
  ...
  const commitResult = spawnSync("git", ["commit-tree", workingTree, "-p", originalHead, "-m", `CCM checkpoint ${input.taskId}`], {
    cwd: repoRoot, encoding: "utf-8", windowsHide: true, env: { ...process.env, ...env },
  });
  try { fs.unlinkSync(tempIndex); } catch {}
  if (commitResult.status !== 0) throw new Error(String(commitResult.stderr || "无法创建 Git 检查点").trim());
  ...
  try { createExecutionRecoveryManifest(checkpoint, record); } catch (error: any) {
...
export function rollbackExecutionCheckpoint(checkpointId: string, reason: string, options: any = {}): any {
  if (!String(reason || "").trim()) throw new Error("回滚必须填写原因");
  const checkpoint = readJson(checkpointFile(checkpointId), null);
  if (!checkpoint) throw new Error("检查点不存在");
  const durable = readExecutionRecoveryManifest(checkpointId);
  if (durable?.deliveryCommit && (durable?.mergeCommit || !fs.existsSync(String(checkpoint.repoRoot || "")) || options.authoritative === true)) {
    if (options.previewOnly === true) return previewExecutionRecovery(checkpointId, { paths: options.paths });
    return applyExecutionRecovery(checkpointId, {
      paths: options.paths,
      previewToken: String(options.previewToken || options.preview_token || ""),
      reason,
    });
  }
  if (checkpoint.mode !== "worktree" && options.allowShared !== true) throw new Error("共享工作目录默认禁止自动回滚；请显式确认 allowShared=true");
  const repoRoot = checkpoint.repoRoot;
  const selectedPaths: string[] = Array.from(new Set<string>((Array.isArray(options.paths) ? options.paths : [])
    .map((value: any) => String(value || "").trim().replace(/\\/g, "/").replace(/^\.\//, ""))
    .filter(Boolean)));
  if (selectedPaths.length) {
    if (selectedPaths.length > 100) throw new Error("单次最多选择 100 个文件撤销");
    for (const relative of selectedPaths) {
      ...
      const existsInCheckpoint = spawnSync("git", ["cat-file", "-e", `${checkpoint.checkpointCommit}:${relative}`], {
      ...
      if (existsInCheckpoint) {
        ...
        if (!stat.isFile() && !stat.isSymbolicLink()) throw new Error(`为避免误删目录，只能选择具体文件撤销：${relative}`);
  ...
  if (currentHead !== checkpoint.originalHead) runGit(repoRoot, ["reset", "--soft", checkpoint.originalHead]);
  ...
  for (const relative of untracked.split("\0").filter(Boolean)) {
    const existsInCheckpoint = spawnSync("git", ["cat-file", "-e", `${checkpoint.checkpointCommit}:${relative.replace(/\\/g, "/")}`], { cwd: repoRoot, windowsHide: true, stdio: "ignore" }).status === 0;
    if (!existsInCheckpoint) {
      const target = path.resolve(repoRoot, relative);
      if (target.startsWith(path.resolve(repoRoot) + path.sep) && fs.existsSync(target) && fs.statSync(target).isFile()) fs.unlinkSync(target);
    }
  }
...
export function inspectBranchFreshness(workDir: string, baseRef = "") {
  const repoRoot = runGit(workDir, ["rev-parse", "--show-toplevel"]);
...
export function mergeExecutionWorktree(executionId: string, options: any = {}) {
  const record = loadExecution(executionId);
  if (!record) throw new Error("执行记录不存在");
  ...
  if (record.workspace?.mode !== "worktree" || !record.workspace?.worktreeBranch || !record.workspace?.originalWorkDir) throw new Error("该执行没有可合并的 worktree");
  ...
  if (record.green?.level !== "merge_ready" && options.force !== true) throw new Error("只有 merge_ready 的执行才能合并");
  ...
  if (!freshness.fresh && options.force !== true) throw new Error(`worktree 分支落后基线 ${freshness.behind} 个提交，请先同步后再合并`);
  const status = runGit(record.workspace.worktreePath, ["status", "--porcelain"]);
  if (status.trim()) {
    if (record.packet.commitPolicy !== "verified_commit" && options.commit !== true) throw new Error("worktree 仍有未提交变更");
  ...
  if (record.workspace.baseBranch && currentBranch !== record.workspace.baseBranch && options.force !== true) throw new Error(`主工作目录当前分支 ${currentBranch} 与任务基线 ${record.workspace.baseBranch} 不一致`);
  try {
    runGit(original, ["merge", "--no-ff", record.workspace.worktreeBranch, "-m", options.mergeMessage || `merge: ${record.taskId}`]);
  } catch (error) {
    try { runGit(original, ["merge", "--abort"]); } catch {}
    throw error;
  }
  record.workspace.mergedAt = now();
  record.workspace.mergeCommit = runGit(original, ["rev-parse", "HEAD"]);
  const recoveryManifests = finalizeExecutionRecoveryManifests(record, {
    workDir: record.workspace.worktreePath,
    deliveryCommit,
    mergeCommit: record.workspace.mergeCommit,
  });
  record.workspace.recoveryManifests = recoveryManifests.map((item: any) => ({
  ...
  if (record.workspace?.conflictGroup) {
    for (const sibling of listExecutions({ taskId: record.taskId })) {
      if (sibling.id === record.id || sibling.workspace?.conflictGroup !== record.workspace.conflictGroup) continue;
...
export function previewExecutionCheckpointRecovery(checkpointId: string, options: any = {}) {
  return previewExecutionRecovery(checkpointId, { paths: options.paths });
}

export function applyExecutionCheckpointRecovery(checkpointId: string, options: any = {}) {
  return applyExecutionRecovery(checkpointId, options);
}

export function cleanupExecutionWorktree(executionId: string, force = false) {
  const record = loadExecution(executionId);
  if (!record) throw new Error("执行记录不存在");
  if (record.workspace?.mode !== "worktree" || !record.workspace?.worktreePath || !record.workspace?.originalWorkDir) throw new Error("该执行没有 worktree");
  if (!record.workspace.mergedAt && !force) throw new Error("未合并的 worktree 默认不能清理");
  ...
  if (record.workspace.worktreeBranch) {
    try { runGit(record.workspace.originalWorkDir, ["branch", "-d", record.workspace.worktreeBranch]); } catch {
      if (force) runGit(record.workspace.originalWorkDir, ["branch", "-D", record.workspace.worktreeBranch]);
    }
  }
...
export function runExecutionKernelSelfTest() {
  ...
  try {
    ...
    if (selfTestExecutionId) try { fs.unlinkSync(executionFile(selfTestExecutionId)); } catch {}
    if (selfTestCheckpointId) try { fs.unlinkSync(checkpointFile(selfTestCheckpointId)); } catch {}
  }
}

export async function runExecutionKernelCancellationSelfTest() {
