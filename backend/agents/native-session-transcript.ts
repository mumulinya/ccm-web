import {
  appendNativeTurnTranscript,
  nativeQueryFamily,
  type NativeQueryFamily,
  type NativeToolResult,
} from "./native-query-messages";
import { shouldUseNativeQueryLoop } from "./native-query-loop";
...
function messageRole(item: any) {
  return String(item?.role || "").toLowerCase();
}

function messageContent(item: any) {
...
function presentedPlanFrom(value: any) {
...
function pairExecutionEvents(events: SessionExecutionEvent[]) {
  ...
  const pairs: Array<{ use: SessionExecutionEvent; result: SessionExecutionEvent }> = [];
  for (const [id, use] of uses) {
    const result = results.get(id);
    if (!result) continue;
    if (SKIP_REPLAY_TOOLS.has(use.toolName) || SKIP_REPLAY_TOOLS.has(result.toolName)) continue;
...
export function shouldMaterializeNativeSessionTranscript(config: any, sessionId: string) {
  const id = String(sessionId || "").trim();
  if (!id) return false;
  return shouldUseNativeQueryLoop(config);
}

export function splitNativeSystemSegments(input: {
  ...
  if (mcp) messages.push({ role: "system", contextBlockType: "mcp", content: mcp } as any);
  return messages;
}

export function buildNativeMetaUserMessage(blocks: NativeSessionMetaBlock[], extra: string[] = []) {
  const parts = [
    NATIVE_SESSION_RESUME_HINT,
...
export function materializeNativeSessionTranscript(input: NativeSessionTranscriptInput): LlmChatMessage[] {
  ...
  const collectPlan = (plan: any, summary = "") => {
    ...
    if (seenPlan.has(body)) return;
    seenPlan.add(body);
    planMetas.push({ title: "已有计划稿", body });
  };
  if (input.presentedPlan) collectPlan(input.presentedPlan);
  ...
  for (const item of conversation) {
    ...
    if (role === "user") {
      ...
      const attached: typeof remaining = [];
      for (let index = remaining.length - 1; index >= 0; index -= 1) {
        const pair = remaining[index];
        if (id && pair.use.anchorMessageId && pair.use.anchorMessageId !== id) continue;
      ...
      if (!id) {
        while (remaining.length) attached.push(remaining.shift()!);
      }
  ...
  if (meta && (messages.length || planMetas.length || extras.length || (input.metaBlocks || []).length)) {
    messages.push(meta as any);
  }
  if (current) messages.push({ role: "user", content: current });
  return messages;
}

export function lastNativeUserText(messages: LlmChatMessage[]) {
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    if (messages[index]?.role === "user" && !(messages[index] as any).isMeta) return String(messages[index].content || "").trim();
  }
  return "";
}

export function inspectNativeResumePayload(messages: LlmChatMessage[], currentUserText: string) {
  ...
  const latestUserMessage = [...messages].reverse().find(item => item?.role === "user" && !(item as any).isMeta);
  ...
  return {
    ...
    hasNativeToolCall: messages.some((item: any) => item?.role === "assistant" && (item?.tool_calls?.[0]?.id || (Array.isArray(item?.content) && item.content.some((part: any) => part?.type === "tool_use")))),
    hasNativeToolResult: messages.some((item: any) => item?.role === "tool" || (Array.isArray(item?.content) && item.content.some((part: any) => part?.type === "tool_result"))),
    noPendingPresentPlan: serialized.includes("ccm_present_plan") === false,
    serialized,
  };
}

function sampleReadFilesEvents(anchorMessageId: string, body: string): SessionExecutionEvent[] {
  return [
    {
      id: "use-files",
      type: "tool_use",
...
function sampleReadFileEvents(anchorMessageId: string): SessionExecutionEvent[] {
  return [
    {
      id: "use-1",
      type: "tool_use",
...
export function runNativeSessionTranscriptSelfTest() {
  const conversation = [
  ...
  const checks = {
    ...
    hasNativeToolCall: messages.some((item: any) => item?.role === "assistant" && item?.tool_calls?.[0]?.id === "call_read"),
    hasNativeToolResult: messages.some((item: any) => item?.role === "tool" && item?.tool_call_id === "call_read"),
    ...
    compactWindowKeepsToolPair: compactWindowMessages.some((item: any) => item?.role === "assistant" && item?.tool_calls?.[0]?.id === "call_read")
      && compactWindowMessages.some((item: any) => item?.role === "tool" && item?.tool_call_id === "call_read")
    ...
    nativeFamilyHelper: nativeQueryFamily({ format: "openai-compatible" }) === "openai",
    ...
    readFilesReplayUsesReceipt: bulkyMessages.some((item: any) => item?.role === "tool" && item?.tool_call_id === "call_read_files")
