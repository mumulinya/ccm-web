...
import { verifyTrustedMemoryPromptEnvelope } from "./trusted-memory-prompt-envelope";
import { verifyNativeSessionContinuationEvidence } from "./native-continuation";

export const PROVIDER_MEMORY_CHANNEL_EVIDENCE_SCHEMA = "ccm-provider-memory-channel-evidence-v1";
export const CODEX_DEVELOPER_INSTRUCTIONS_MIN_VERSION = "0.115.0";

function normalizeProvider(value: any) {
  const provider = String(value || "").trim().toLowerCase();
  if (["claude", "claude-code", "claude_code", "cc", "claudecode"].includes(provider)) return "claudecode";
  if (["cursor", "cursor-agent", "agent"].includes(provider)) return "cursor";
...
function acknowledgementPolicy(providerInput: any) {
  const provider = normalizeProvider(providerInput);
  if (provider === "codex") return "structured_thread_started";
  if (provider === "cursor") return "structured_session_event";
  if (provider === "claudecode") return "process_exit_success";
  return "process_exit_success";
}

function semanticVersionAtLeast(actual: any, minimum: string) {
  const parse = (value: any) => String(value || "").match(/\b(\d+)\.(\d+)\.(\d+)\b/)?.slice(1).map(Number) || [];
...
export function getProviderMemoryChannelCapability(providerInput: any, options: { runtimeVersionSnapshot?: any } = {}) {
  const provider = normalizeProvider(providerInput);
  ...
  const nativeDeveloperInstructionsSupported = provider === "codex"
    && semanticVersionAtLeast(runtimeVersion, CODEX_DEVELOPER_INSTRUCTIONS_MIN_VERSION);
  return {
    provider,
    ...
    runtimeVersion,
    minimumRuntimeVersion: provider === "codex" ? CODEX_DEVELOPER_INSTRUCTIONS_MIN_VERSION : "",
  };
}

export function prepareProviderMemoryChannel(providerInput: any, renderedPrompt: string, expected: {
  required?: boolean;
  ...
  const envelope = verifyTrustedMemoryPromptEnvelope(originalPrompt);
  const required = expected.required === true;
  const issues = [...envelope.issues];
  if (!required && !envelope.present) issues.length = 0;
  if (required && !envelope.valid) issues.push("provider_memory_channel_trusted_envelope_invalid");
...
export function bindProviderMemoryChannelLaunch(prepared: any, input: {
  command?: string;
  ...
  if (prepared?.channel === "native_system_prompt_file") {
    ...
    if (!systemPromptFileBound) issues.push("provider_memory_channel_system_prompt_file_mismatch");
    if (!commandChannelBound) issues.push("provider_memory_channel_command_unbound");
  }
  ...
  if (prepared?.channel === "native_developer_instructions_config") {
    ...
    if (!developerInstructionsFileBound) issues.push("provider_memory_channel_developer_instructions_file_mismatch");
    if (!commandChannelBound) issues.push("provider_memory_channel_command_unbound");
  }
  ...
  const payload = {
    schema: PROVIDER_MEMORY_CHANNEL_EVIDENCE_SCHEMA,
    version: 2,
...
export function acknowledgeProviderMemoryChannelLaunch(evidence: any, input: {
  ...
  const issues = (Array.isArray(evidence?.issues) ? evidence.issues : [])
    .filter((issue: any) => !String(issue || "").startsWith("provider_memory_channel_acknowledgement_"));
  if (!executionSucceeded) issues.push("provider_memory_channel_acknowledgement_execution_failed");
  if (!runnerStarted) issues.push("provider_memory_channel_acknowledgement_runner_not_started");
  if (typeof exitCode === "number" && exitCode !== 0) issues.push("provider_memory_channel_acknowledgement_exit_nonzero");
  if (policy === "structured_thread_started" || policy === "structured_session_event") {
    ...
    if (outputContract?.status !== "recognized") issues.push("provider_memory_channel_acknowledgement_output_contract_unrecognized");
    if (String(outputContract?.provider || "") !== provider) issues.push("provider_memory_channel_acknowledgement_output_provider_mismatch");
    if (!String(outputContract?.trustedSessionId || "")) issues.push("provider_memory_channel_acknowledgement_session_missing");
    if (expectedEvent && String(outputContract?.matchedEventType || "") !== expectedEvent) issues.push("provider_memory_channel_acknowledgement_event_mismatch");
    ...
    const continuation = verifyNativeSessionContinuationEvidence(input.nativeContinuationEvidence, {
      provider,
...
export function verifyProviderMemoryChannelEvidence(evidence: any, expected: {
  provider?: string;
  ...
  const issues: string[] = [];
  if (evidence?.schema !== PROVIDER_MEMORY_CHANNEL_EVIDENCE_SCHEMA || ![1, 2].includes(Number(evidence?.version || 0))) issues.push("provider_memory_channel_schema_invalid");
  if (!String(evidence?.evidence_checksum || "") || String(evidence.evidence_checksum) !== evidenceChecksum(evidence)) issues.push("provider_memory_channel_checksum_invalid");
  const provider = normalizeProvider(expected.provider === undefined ? evidence?.provider : expected.provider);
  if (String(evidence?.provider || "") !== provider) issues.push("provider_memory_channel_provider_mismatch");
  ...
  if (originalPromptChecksum && String(evidence?.original_prompt_checksum || "") !== originalPromptChecksum) issues.push("provider_memory_channel_prompt_mismatch");
  if (expected.envelopeChecksum !== undefined && String(evidence?.trusted_envelope_checksum || "") !== String(expected.envelopeChecksum || "")) issues.push("provider_memory_channel_envelope_mismatch");
  if (expected.sourceChecksum !== undefined && String(evidence?.trusted_envelope_source_checksum || "") !== String(expected.sourceChecksum || "")) issues.push("provider_memory_channel_source_mismatch");
  if (expected.runnerRequestId !== undefined && String(evidence?.runner_request_id || "") !== String(expected.runnerRequestId || "")) issues.push("provider_memory_channel_runner_request_mismatch");
  ...
  if (expected.originalPrompt !== undefined) {
    ...
    if (String(evidence?.channel || "") !== String(replay.channel || "")) issues.push("provider_memory_channel_replay_channel_mismatch");
    if (String(evidence?.authority_role || "") !== String(replay.authorityRole || "")) issues.push("provider_memory_channel_replay_role_mismatch");
    if (String(evidence?.provider_user_prompt_checksum || "") !== checksum(replay.userPrompt)) issues.push("provider_memory_channel_replay_user_prompt_mismatch");
    const replaySystemChecksum = replay.systemPrompt ? checksum(replay.systemPrompt) : "";
    if (String(evidence?.provider_system_prompt_checksum || "") !== replaySystemChecksum) issues.push("provider_memory_channel_replay_system_prompt_mismatch");
    const replayDeveloperChecksum = replay.developerPrompt ? checksum(replay.developerPrompt) : "";
    if (String(evidence?.provider_developer_prompt_checksum || "") !== replayDeveloperChecksum) issues.push("provider_memory_channel_replay_developer_prompt_mismatch");
  }
  if (required && String(evidence?.status || "") !== "ready") issues.push("provider_memory_channel_not_ready");
  if (required && evidence?.trusted_envelope_unique !== true) issues.push("provider_memory_channel_envelope_not_unique");
  if (provider === "claudecode" && required) {
    if (String(evidence?.channel || "") !== "native_system_prompt_file" || String(evidence?.authority_role || "") !== "system") issues.push("provider_memory_channel_native_system_missing");
    if (evidence?.system_prompt_file_bound !== true || evidence?.command_channel_bound !== true || !String(evidence?.provider_system_prompt_checksum || "")) issues.push("provider_memory_channel_native_launch_unbound");
    ...
    if (capability.nativeDeveloperInstructionsSupported) {
      if (String(evidence?.channel || "") !== "native_developer_instructions_config" || String(evidence?.authority_role || "") !== "developer") issues.push("provider_memory_channel_native_developer_missing");
      if (evidence?.developer_instructions_file_bound !== true || evidence?.command_channel_bound !== true || !String(evidence?.provider_developer_prompt_checksum || "")) issues.push("provider_memory_channel_native_developer_launch_unbound");
    } else {
      if (String(evidence?.channel || "") !== "trusted_user_prompt_envelope" || String(evidence?.authority_role || "") !== "user") issues.push("provider_memory_channel_version_fallback_invalid");
      if (String(evidence?.provider_user_prompt_checksum || "") !== String(evidence?.original_prompt_checksum || "")) issues.push("provider_memory_channel_version_fallback_unbound");
    }
  } else if (required) {
    if (String(evidence?.channel || "") !== "trusted_user_prompt_envelope" || String(evidence?.authority_role || "") !== "user") issues.push("provider_memory_channel_user_fallback_invalid");
    if (String(evidence?.provider_user_prompt_checksum || "") !== String(evidence?.original_prompt_checksum || "")) issues.push("provider_memory_channel_user_fallback_unbound");
  }
  const acknowledgementRequired = expected.requireAcknowledgement === true || evidence?.acknowledgement_required === true;
  if (acknowledgementRequired) {
    const policy = acknowledgementPolicy(provider);
    if (Number(evidence?.version || 0) < 2) issues.push("provider_memory_channel_acknowledgement_schema_missing");
    if (String(evidence?.acknowledgement_policy || "") !== policy) issues.push("provider_memory_channel_acknowledgement_policy_mismatch");
    if (String(evidence?.acknowledgement_status || "") !== "acknowledged") issues.push("provider_memory_channel_acknowledgement_missing");
    if (evidence?.execution_succeeded !== true || evidence?.runner_started !== true) issues.push("provider_memory_channel_acknowledgement_execution_unverified");
    if (typeof evidence?.exit_code === "number" && evidence.exit_code !== 0) issues.push("provider_memory_channel_acknowledgement_exit_nonzero");
    if (expected.executionSucceeded === false) issues.push("provider_memory_channel_acknowledgement_delivery_failed");
    const acknowledgementCore = {
    ...
    if (policy === "structured_thread_started" || policy === "structured_session_event") {
      ...
      const continuation = verifyNativeSessionContinuationEvidence(expected.nativeContinuationEvidence, {
        provider,
        runnerRequestId: evidence?.runner_request_id,
      });
