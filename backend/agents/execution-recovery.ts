...
function safePart(value: any, fallback = "recovery") {
...
function manifestFile(checkpointId: string) {
  ensureDirs();
...
function git(cwd: string, args: string[], options: { allowFailure?: boolean; env?: any } = {}) {
  ...
  if (result.status !== 0 && !options.allowFailure) throw new Error(String(result.stderr || result.stdout || `git ${args[0]} 失败`).trim());
...
function blobAt(repoRoot: string, commit: string, relativePath: string) {
  const result = git(repoRoot, ["rev-parse", `${commit}:${relativePath}`], { allowFailure: true });
  return result.ok ? result.stdout : "";
}
function isProtectedRecoveryPath(relativePath: string) {
  const normalized = String(relativePath || "").replace(/\\/g, "/");
  return /(^|\/)(?:\.env(?:\.|$)|\.npmrc$|\.pypirc$|id_(?:rsa|ed25519)(?:\.|$)|credentials?(?:\.|$)|secrets?(?:\.|$)|.*\.(?:pem|key|p12|pfx|keystore))$/i.test(normalized);
}
function protectRef(repoRoot: string, kind: "checkpoints" | "deliveries" | "recoveries", id: string, commit: string) {
  const ref = `refs/ccm/${kind}/${safePart(id)}`;
  git(repoRoot, ["update-ref", ref, commit]);
  return ref;
}
function snapshotWorkingTree(repoRoot: string, parent: string, label: string) {
  ensureDirs();
  ...
  try {
    ...
    const result = spawnSync("git", ["commit-tree", tree, "-p", parent, "-m", `CCM delivery ${label}`], {
      cwd: repoRoot,
      encoding: "utf-8",
      windowsHide: true,
      env: {
    ...
    if (result.status !== 0) throw new Error(String(result.stderr || "无法固定交付版本").trim());
    return String(result.stdout || "").trim();
  } finally {
    try { fs.unlinkSync(tempIndex); } catch {}
  }
}
function parseChangedPaths(repoRoot: string, baselineCommit: string, deliveryCommit: string) {
  ...
  for (let index = 0; index < tokens.length;) {
    ...
    if (firstPath) paths.push(firstPath.replace(/\\/g, "/"));
    if (/^[RC]/.test(status)) {
      const secondPath = tokens[index++] || "";
      if (secondPath) paths.push(secondPath.replace(/\\/g, "/"));
    }
  }
...
export function createExecutionRecoveryManifest(checkpoint: any, execution: any = null) {
  ...
  if (!objectExists(authoritativeRepoRoot, baselineCommit)) throw new Error("检查点Git对象不可用");
...
export function finalizeExecutionRecoveryManifest(checkpointId: string, input: { workDir?: string; deliveryCommit?: string; mergeCommit?: string } = {}) {
  ...
  if (!deliveryCommit) deliveryCommit = snapshotWorkingTree(sourceRoot, parent, checkpointId);
  if (!objectExists(manifest.authoritativeRepoRoot, deliveryCommit)) throw new Error("交付Git对象不可用");
...
export function finalizeExecutionRecoveryManifests(execution: any, input: { workDir?: string; deliveryCommit?: string; mergeCommit?: string } = {}) {
  const results: any[] = [];
...
export function readExecutionRecoveryManifest(checkpointId: string) {
  return readJson(manifestFile(checkpointId), null);
}

export function listExecutionRecoveryManifests(filters: { taskIds?: string[]; executionIds?: string[] } = {}) {
  ensureDirs();
  ...
  return fs.readdirSync(MANIFEST_DIR).filter(name => name.endsWith(".json"))
    ...
    .filter((item: any) => (!taskIds.size && !executionIds.size) || taskIds.has(String(item.taskId)) || executionIds.has(String(item.executionId)))
    .sort((a: any, b: any) => String(b.createdAt || "").localeCompare(String(a.createdAt || "")));
}

function normalizedSelectedPaths(manifest: any, selectedPaths?: string[]) {
  const allowedRows = (manifest.changedFiles || []).filter((item: any) => item.recoverable !== false);
  const allowed = new Set(allowedRows.map((item: any) => String(item.path)));
  ...
  for (const relativePath of normalized) if (!allowed.has(relativePath)) throw new Error(`文件不属于本轮交付：${relativePath}`);
  return normalized;
}

export function previewExecutionRecovery(checkpointId: string, options: { paths?: string[] } = {}) {
  const manifest = readExecutionRecoveryManifest(checkpointId);
  if (!manifest) return { checkpointId, available: false, reason: "历史检查点不可用", conflicts: [], files: [] };
  ...
  const protectedFileCount = (manifest.changedFiles || []).filter((item: any) => item.recoverable === false).length;
  const currentHead = git(repoRoot, ["rev-parse", "HEAD"]).stdout;
  const conflicts: any[] = [];
  const files = paths.map(relativePath => {
    const row = (manifest.changedFiles || []).find((item: any) => item.path === relativePath) || {};
    ...
    if (conflict) conflicts.push({ path: relativePath, reason: dirty ? "当前工作区仍有未提交修改" : "文件已被后续提交修改" });
...
export function applyExecutionRecovery(checkpointId: string, options: { paths?: string[]; previewToken?: string; reason?: string } = {}) {
  const preview = previewExecutionRecovery(checkpointId, { paths: options.paths });
  if (!preview.available) throw new Error(preview.reason || "历史检查点不可用");
  if (!preview.canExecute) throw new Error("文件已发生变化，请处理冲突后重新预览");
  if (!options.previewToken || options.previewToken !== preview.previewToken) throw new Error("恢复预览已漂移，请重新预览");
  const manifest = readExecutionRecoveryManifest(checkpointId);
  const repoRoot = String(manifest.authoritativeRepoRoot);
  const expectedHead = String(preview.currentHead);
  if (git(repoRoot, ["rev-parse", "HEAD"]).stdout !== expectedHead) throw new Error("仓库HEAD已漂移，请重新预览");
  ...
  try {
    ...
    for (const relativePath of (preview.files || []).map((item: any) => item.path)) {
    ...
    if (staged.ok) return { success: true, duplicate: true, checkpointId, restoredFiles: preview.files.length, contentStored: false };
    const env = {
      GIT_AUTHOR_NAME: process.env.GIT_AUTHOR_NAME || "CCM Recovery",
      GIT_AUTHOR_EMAIL: process.env.GIT_AUTHOR_EMAIL || "ccm-recovery@localhost",
      GIT_COMMITTER_NAME: process.env.GIT_COMMITTER_NAME || "CCM Recovery",
    ...
    if (git(repoRoot, ["rev-parse", "HEAD"]).stdout !== expectedHead) throw new Error("仓库在恢复发布前发生变化，未应用恢复提交");
    ...
    manifest.recoveries = [...(manifest.recoveries || []), {
      ...
      paths: (preview.files || []).map((item: any) => item.path),
      restoredAt: now(),
    }].slice(-50);
    ...
    try { git(repoRoot, ["worktree", "remove", "--force", recoveryWorktree], { allowFailure: true }); } catch {}
    try { fs.rmSync(recoveryWorktree, { recursive: true, force: true }); } catch {}
  }
}

export function compensateExecutionRecovery(checkpointId: string, recoveryCommit: string, reason = "多项目恢复未全部成功，自动补偿") {
  const manifest = readExecutionRecoveryManifest(checkpointId);
  if (!manifest?.authoritativeRepoRoot || !recoveryCommit) throw new Error("缺少可补偿的恢复提交");
  const repoRoot = String(manifest.authoritativeRepoRoot);
  const currentHead = git(repoRoot, ["rev-parse", "HEAD"]).stdout;
  if (currentHead !== recoveryCommit) throw new Error("仓库在补偿前已发生变化，需要人工处理");
  const env = {
    GIT_AUTHOR_NAME: process.env.GIT_AUTHOR_NAME || "CCM Recovery",
    GIT_AUTHOR_EMAIL: process.env.GIT_AUTHOR_EMAIL || "ccm-recovery@localhost",
    GIT_COMMITTER_NAME: process.env.GIT_COMMITTER_NAME || "CCM Recovery",
  ...
  manifest.recoveries = (manifest.recoveries || []).map((item: any) => item.commit === recoveryCommit
...
export function purgeExecutionRecoveryManifests(taskId: string) {
  let removed = 0;
  for (const manifest of listExecutionRecoveryManifests({ taskIds: [taskId] })) {
    if (manifest.authoritativeRepoRoot && fs.existsSync(manifest.authoritativeRepoRoot)) {
      for (const ref of [manifest.baselineRef, manifest.deliveryRef, ...(manifest.recoveries || []).map((item: any) => `refs/ccm/recoveries/${safePart(item.id)}`)].filter(Boolean)) {
        try { git(manifest.authoritativeRepoRoot, ["update-ref", "-d", String(ref)], { allowFailure: true }); } catch {}
      }
    }
    try { fs.unlinkSync(manifestFile(manifest.checkpointId)); removed++; } catch {}
  }
  return removed;
}

export function runExecutionRecoverySelfTest() {
  ...
  try {
    ...
    try { fs.unlinkSync(manifestFile(checkpointId)); } catch {}
