...
export function classifyGlobalControlIntent(_message: string, resources: any = {}) {
  ...
  const route: IntentRoute = !decision
    ? "ambiguous"
    : needsClarification
      ? "ambiguous"
  ...
  return {
    ...
    matched_projects: targets.filter((item: any) => item.type === "project").map((item: any) => item.id || item.name),
    matched_groups: targets.filter((item: any) => item.type === "group").map((item: any) => ({ id: item.id, name: item.name || item.id })),
    dry_run: {
...
export function buildGlobalDispatchStrategy(_message: string, resources: any = {}) {
  const decision = resources.workflowDecision || resources.workflow_decision || null;
  const targets = Array.isArray(decision?.targetRefs) ? decision.targetRefs : [];
