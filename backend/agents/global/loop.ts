...
import { recordAgentRuntimeLifecycle } from "../runtime-kernel";
import { evaluateAgentDecision, normalizeAgentDecisionIntent, recordAgentDecision, type AgentDecisionIntent } from "../quality-center";
import {
...
const activeRunObjects = new Map<string, GlobalAgentRun>();
...
function hasGlobalUserSummaryTechnicalDetails(value: any) {
...
function uniqueGlobalStrings(values: any[]) {
...
function sanitizeGlobalVisibleReplyTerminology(value: string) {
...
function globalVisibleReplyFallback(status?: GlobalAgentRunStatus) {
...
function attachGlobalReplyTechnicalContent(target: any, content: any) {
...
function buildGlobalPlanExecutionFollowup(planMode: any = {}, at = nowIso(), feedback = "") {
...
function updateGlobalPlanModeStatus(planMode: any, status: "confirmed" | "completed" | "cancelled" | "failed", at: string, feedback = "") {
...
function normalizeGlobalAgentUserSteer(value: any): GlobalAgentUserSteer | null {
...
function normalizeGlobalAgentUserSteers(value: any, status?: GlobalAgentUserSteerStatus, limit = 30) {
...
export function completeGlobalAgentSupervision(id: string, report: any, outcome: "completed" | "failed" | "cancelled" = "completed") {
  ...
  const visibleReply = buildGlobalVisibleReplyContent({
    value: formatMainAgentDeliveryReply(deliveryReport),
...
function globalSupervisionStateVisibleSummary(state: any) {
  const value = String(state || "monitoring").trim().toLowerCase();
  if (value === "waiting_user" || value === "needs_user" || value === "blocked") {
    return {
      status: "supervising" as GlobalAgentRunStatus,
      phase: "needs_confirmation" as GlobalAgentDecisionState,
  ...
  if (/rework|reworking|repair|retry|返工|修复/.test(value)) {
    return {
      status: "supervising" as GlobalAgentRunStatus,
      phase: "execute" as GlobalAgentDecisionState,
  ...
  if (value === "paused") {
    return {
      status: "paused" as GlobalAgentRunStatus,
      phase: "execute" as GlobalAgentDecisionState,
...
function isReadOnlyGlobalConsultation(run: GlobalAgentRun, status: GlobalAgentRunStatus) {
  if (status !== "completed" || run.mission_id || run.decision_summary?.intent?.action_required === true) return false;
  const intentCategory = String(run.decision_summary?.intent?.category || "");
  if (["execution", "high_risk"].includes(intentCategory)) return false;
  if (run.tool_calls === 0) return true;
...
function projectRoutingValue(value: any): any {
...
function projectProjectRows(rows: any) {
...
function projectGroupRows(rows: any) {
  return (Array.isArray(rows) ? rows : []).slice(0, 100).map((row: any) => ({
...
function projectGlobalTaskRows(observation: any) {
...
export async function buildGlobalAgentModelMessages(run: GlobalAgentRun, runtime: GlobalAgentLoopRuntime) {
  ...
  const roleSkills = buildRoleSkillPrompt(
    ...
    { source: (run as any).source || "", phase: "planning" },
  );
...
function buildGlobalAgentEffectiveGoal(run: GlobalAgentRun) {
...
function applyPendingGlobalAgentUserSteers(run: GlobalAgentRun, runtime: GlobalAgentLoopRuntime) {
...
function applyGlobalResumeFeedback(run: GlobalAgentRun, runtime: GlobalAgentLoopRuntime, value: any, options: { source?: string } = {}) {
...
function buildGlobalRunWorkchain(run: GlobalAgentRun, status: GlobalAgentRunStatus, reply = "", report: any = null, options: { mode?: string } = {}) {
