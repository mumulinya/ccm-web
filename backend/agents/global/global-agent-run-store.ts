...
export function normalizeRun(run: any): GlobalAgentRun {
  ...
  return {
    ...
    status: (["running", "supervising", "paused", "waiting_confirmation", "waiting_clarification", "blocked", "completed", "failed", "cancelled"].includes(run?.status) ? run.status : "failed") as GlobalAgentRunStatus,
