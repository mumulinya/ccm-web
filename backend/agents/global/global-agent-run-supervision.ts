import { attachGlobalReplyTechnicalContent, buildGlobalVisibleReplyContent } from "./global-agent-run-replies";
import { buildMainAgentDeliveryReport, formatMainAgentDeliveryReply } from "../delivery-report";
import { recordGlobalAgentRunMetric } from "./global-agent-metrics";
import type { GlobalAgentDecisionState, GlobalAgentRun, GlobalAgentRunStatus } from "./loop";

export function createGlobalAgentRunSupervision(deps: { [key: string]: any }) {
...
function attachGlobalAgentRunSupervision(run: GlobalAgentRun, link: { mission_id: string; supervisor_id: string; state?: string }) {
...
function completeGlobalAgentSupervision(id: string, report: any, outcome: "completed" | "failed" | "cancelled" = "completed") {
  ...
  if (recordGlobalAgentRunMetric(run, outcome, { source: run.source || "global-agent-supervision", runtime: "global-agent-supervision" }) && run.metrics_recorded === true) {
    saveRun(run, !volatileRuns.has(id));
  }
  return run;
}
...
function updateGlobalAgentSupervisionState(id: string, state: string) {
  const stored = getGlobalAgentRun(id);
  if (!stored) return null;
  const run = normalizeRun(stored);
