...
import { loadMetrics, recordMetric } from "../../core/db";

const GLOBAL_AGENT_NAME = "global-agent";

function finiteToken(value: any): number {
  const number = Number(value);
  return Number.isFinite(number) && number > 0 ? Math.floor(number) : 0;
}

/** Accumulate provider usage onto a Global Agent run across multi-step model calls. */
export function accumulateGlobalAgentRunUsage(run: any, delta: any): any {
  if (!run || typeof run !== "object" || !delta || typeof delta !== "object") return run?.usage || null;
  ...
  if (!(inputTokens > 0 || outputTokens > 0 || totalCostUsd > 0)) return run.usage || null;
  run.usage = {
    inputTokens,
    outputTokens,
  ...
  if (run.usage.totalCostUsd > 0) run.total_cost_usd = run.usage.totalCostUsd;
  return run.usage;
}

function finiteDurationMs(run: any): number {
...
function alreadyRecorded(executionId: string): boolean {
  if (!executionId) return false;
  try {
    const metrics = loadMetrics();
    return (Array.isArray(metrics?.events) ? metrics.events : []).some(
      (event: any) => String(event?.executionId || "") === executionId
...
function collectUsage(run: any) {
  ...
  if (!(inputTokens > 0 || outputTokens > 0 || totalCostUsd > 0)) return null;
  return {
 ...
 */
export function recordGlobalAgentRunMetric(
  run: any,
  status: string,
  options: { source?: string; runtime?: string } = {},
): boolean {
  const normalizedStatus = String(status || "").trim().toLowerCase();
  if (!["completed", "failed", "cancelled"].includes(normalizedStatus)) return false;
  if (run?.metrics_recorded === true) return false;

  const executionId = String(run?.id || "").trim();
  if (!executionId || alreadyRecorded(executionId)) {
    if (run && typeof run === "object") run.metrics_recorded = true;
    return false;
  }
