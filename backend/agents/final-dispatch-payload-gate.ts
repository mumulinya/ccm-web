...
export const FINAL_DISPATCH_PROVIDER_USAGE_BASELINE_SCHEMA = "ccm-final-dispatch-provider-usage-baseline-v1";
...
function baselineChecksum(baseline: any) {
...
export function buildFinalDispatchProviderUsageBaseline(input: any = {}) {
  ...
  const provider = normalizeAgentRuntimeId(input.provider || usage.provider || "");
  ...
  const core = {
    schema: FINAL_DISPATCH_PROVIDER_USAGE_BASELINE_SCHEMA,
    version: 1,
...
export function verifyFinalDispatchProviderUsageBaseline(baseline: any, expected: any = {}) {
  const issues: string[] = [];
  if (baseline?.schema !== FINAL_DISPATCH_PROVIDER_USAGE_BASELINE_SCHEMA || Number(baseline?.version || 0) !== 1) issues.push("provider_usage_baseline_schema_invalid");
  if (String(baseline?.baseline_checksum || "") !== baselineChecksum(baseline)) issues.push("provider_usage_baseline_checksum_invalid");
  if (baseline?.status !== "ready") issues.push("provider_usage_baseline_not_ready");
  if (baseline?.body_free !== true) issues.push("provider_usage_baseline_body_free_missing");
  if (baseline?.measurement_scope !== "exact_task_agent_session_last_provider_call") issues.push("provider_usage_baseline_scope_invalid");
  if (!String(baseline?.group_session_id || "").startsWith("gcs_")) issues.push("provider_usage_baseline_group_session_missing");
  if (!String(baseline?.task_agent_session_id || "").startsWith("tas_")) issues.push("provider_usage_baseline_task_session_missing");
  if (!String(baseline?.provider || "")) issues.push("provider_usage_baseline_provider_missing");
  if (!String(baseline?.model || "").trim()) issues.push("provider_usage_baseline_model_missing");
  const sourceCompactEpoch = String(baseline?.source_compact_epoch || "").trim();
  if (sourceCompactEpoch !== "precompact" && !sourceCompactEpoch.startsWith("cmp_")) issues.push("provider_usage_baseline_compact_epoch_invalid");
  if (!String(baseline?.source_snapshot_checksum || "").trim()) issues.push("provider_usage_baseline_snapshot_checksum_missing");
  if (sourceCompactEpoch.startsWith("cmp_") && (!String(baseline?.source_compact_head_id || "").trim() || !String(baseline?.source_compact_head_checksum || "").trim() || finiteToken(baseline?.source_compact_head_generation) <= 0)) issues.push("provider_usage_baseline_compact_head_missing");
  if (!/^[a-f0-9]{64}$/.test(String(baseline?.source_usage_checksum || ""))) issues.push("provider_usage_baseline_source_checksum_invalid");
  ...
  if (observedContextTokens <= 0 || Number(baseline?.observed_context_tokens || 0) !== observedContextTokens) issues.push("provider_usage_baseline_observed_tokens_invalid");
  if (estimatedContextTokens <= 0) issues.push("provider_usage_baseline_estimated_tokens_invalid");
  if (Number(baseline?.positive_drift_tokens || 0) !== Math.max(0, observedContextTokens - estimatedContextTokens)) issues.push("provider_usage_baseline_drift_invalid");
  if (!Number.isFinite(Date.parse(String(baseline?.observed_at || "")))) issues.push("provider_usage_baseline_time_invalid");
  const bindings: Array<[string, any, any]> = [
    ...
    ["provider", expected.provider === undefined ? undefined : normalizeAgentRuntimeId(expected.provider), baseline?.provider],
  ...
  for (const [field, wanted, actual] of bindings) {
    if (wanted !== undefined && String(wanted || "") !== String(actual || "")) issues.push(`provider_usage_baseline_${field}_mismatch`);
  }
...
export function verifyFinalWorkerDispatchPayloadGate(gate: any, expected: any = {}) {
  ...
  if (baseline) {
    ...
    if (!baselineVerification.valid) issues.push(...baselineVerification.issues);
    if (gate?.provider_usage_baseline_status !== (baselineVerification.valid ? "provider_observed" : "invalid")) issues.push("final_dispatch_provider_usage_baseline_status_invalid");
    const expectedBias = baselineVerification.valid ? Math.max(0, Number(baseline?.positive_drift_tokens || 0)) : 0;
    if (Number(gate?.provider_usage_baseline_bias_tokens || 0) !== expectedBias) issues.push("final_dispatch_provider_usage_baseline_bias_invalid");
    if (baselineVerification.valid && modelVisibleInputTokens !== Number(gate?.estimated_total_input_tokens || 0) + expectedBias) issues.push("final_dispatch_model_visible_input_tokens_invalid");
  } else {
    if (gate?.provider_usage_baseline_status !== "estimated") issues.push("final_dispatch_provider_usage_baseline_status_invalid");
    if (Number(gate?.provider_usage_baseline_bias_tokens || 0) !== 0) issues.push("final_dispatch_provider_usage_baseline_bias_invalid");
    if (modelVisibleInputTokens !== Number(gate?.estimated_total_input_tokens || 0)) issues.push("final_dispatch_model_visible_input_tokens_invalid");
  }
