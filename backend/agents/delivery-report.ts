...
export function sanitizeMainAgentDeliveryText(value: any, fallback = "本轮处理已完成。", max = 260) {
  ...
  if (INTERNAL_DELIVERY_TEXT_PATTERN.test(text)) {
    ...
    else if (/done|完成|receipt|回执/i.test(text)) text = "子 Agent 已提交结果说明，主 Agent 已完成汇总。";
    else text = fallback;
  }
...
function collectDeliveryVerificationEvidence(input: MainAgentDeliveryReportInput, status: MainAgentDeliveryStatus) {
  ...
  const failed = uniqueDeliveryStrings(
  ...
  else if (sourceGatePassed === false) items.push("外部 Runner 证据缺失：当前不能只凭子 Agent 自述判定完成。");
  if (!items.length) {
...
function deliveryIndependentReviewLabel(value: any) {
  const text = String(value || "").trim().toLowerCase();
  if (!text) return "";
  if (["passed", "pass", "approved", "approve", "success", "ok", "done", "complete", "completed"].includes(text) || /通过|批准|无阻塞|无高风险/.test(text)) return "已通过";
  if (["failed", "fail", "rejected", "reject", "blocked", "block"].includes(text) || /未通过|拒绝|阻塞|风险/.test(text)) return "未通过";
...
function collectDeliveryPlanReview(input: MainAgentDeliveryReportInput, status: MainAgentDeliveryStatus) {
  ...
  const items = uniqueDeliveryStrings(
...
function collectDeliveryAcceptance(input: MainAgentDeliveryReportInput, status: MainAgentDeliveryStatus, verification: string[], risks: string[]) {
  ...
  const items: string[] = [];
  if (passed === true) items.push("主 Agent 验收：已通过");
  else if (passed === false) items.push("主 Agent 验收：未通过，仍需处理缺口");
  else if (status === "done") items.push(verification.length || !risks.length ? "主 Agent 验收：已完成交付复核" : "主 Agent 验收：仍有风险需要复核");
  else if (status === "failed") items.push("主 Agent 验收：未通过，原因已整理在未完成原因里");
  else if (status === "cancelled") items.push("主 Agent 验收：任务已停止，未继续验收");
  else items.push("主 Agent 验收：仍在等待最终复核");
...
function collectDeliveryCompleted(input: MainAgentDeliveryReportInput, files: string[], verification: string[], status: MainAgentDeliveryStatus) {
  const { report, summary, completion, workchainSummary } = getNestedReport(input);
  const task = input.task || {};
  ...
  if (status === "failed") return ["任务没有完成，主 Agent 已整理未完成原因和下一步。"];
  if (status === "cancelled") return ["任务已停止，主 Agent 已整理当前状态。"];
  if (status === "waiting") return ["任务仍在处理中，主 Agent 会继续跟进并在完成后总结。"];
  return ["本轮处理已经完成，结果已整理给你。"];
}

function collectDeliveryNextAction(input: MainAgentDeliveryReportInput, status: MainAgentDeliveryStatus, risks: string[]) {
  const { report, summary, completion, workchainSummary } = getNestedReport(input);
  ...
  if (status === "failed") return ["可以根据上面的风险重新执行，或补充范围后让主 Agent 继续处理。"];
  if (status === "cancelled") return ["任务已经停止；如果需要，可以重新发起新的需求。"];
  if (risks.length) return ["先处理风险或缺口，主 Agent 会继续跟进验收。"];
...
function buildDeliveryFinalSummaryQuality(status: MainAgentDeliveryStatus, sections: any[], nextAction: string[]) {
...
function buildDeliveryUserHandoff(
  ...
  if (failed) addAction("retry_or_continue", "重新执行或继续修复", risks[0] || nextAction[0] || "主 Agent 会复用已有证据继续处理。", "retry", "primary");
  ...
  if (files.length) addAction("view_changes", "查看改动", `已整理 ${files.length} 个文件变更。`, "view_changes", done && !risks.length ? "primary" : "outline");
  ...
  if (status === "waiting") addAction("wait_for_summary", "等待最终总结", nextAction[0] || "主 Agent 会继续协调执行并在完成后总结。", "continue", "outline");
  if (!actions.length) addAction("next_action", "继续跟进", nextAction[0] || "主 Agent 会继续处理并更新结果。", "continue", "primary");
