...
import { getConfiguredDevelopmentAgentModel, isDevelopmentAgentReady, resolveAntigravityCliCommand, resolveCursorAgentCommand } from "../modules/system/agent-provider-settings";
import { DEVELOPMENT_AGENT_CATALOG, findDevelopmentAgent } from "./catalog";
...
function getRuntimeVersionCommand(agentType: string) {
  ...
  if (provider === "gemini") return resolveAntigravityCliCommand();
...
function formatRuntimeEnvPrefix(values: Record<string, string>) {
  if (process.platform === "win32") return formatWindowsEnvPrefix(values);
  const assignments = Object.entries(values)
    .filter(([, value]) => String(value || "").trim())
...
function buildAntigravityAgentCommand(msgFile: string, options: AgentCommandOptions = {}) {
  ...
  if (selectedModel) args.push("--model", selectedModel);
  const sessionId = String(options.sessionId || "").trim();
  if (options.persistSession && options.resumeSession && sessionId) args.push("--conversation", sessionId);
  args.push("--print");
  const helper = path.join(__dirname, "cli-prompt-runner.js");
  return `node ${quoteCmdArg(helper)} ${quoteCmdArg(msgFile)} ${quoteCmdArg(resolveAntigravityCliCommand())} ${encodeCliArgs(args)}`;
}

function buildOpenCodeAgentCommand(msgFile: string, options: AgentCommandOptions = {}) {
  const metadata = readRuntimeLaunchMetadata(options);
  const configPath = String(options.mcpConfigPath || "").trim();
  ...
  const selectedModel = getConfiguredDevelopmentAgentModel("opencode");
  if (selectedModel) args.push("--model", selectedModel);
  const helper = path.join(__dirname, "cli-prompt-runner.js");
...
export function normalizeAgentRuntimeId(agentType = ""): RegisteredAgentRuntimeId {
  return findDevelopmentAgent(agentType)?.id || "claudecode";
}
...
export function getPublicAgentRuntimes() {
  return DEVELOPMENT_AGENT_CATALOG.map(definition => {
    const runtime = AGENT_RUNTIMES.find(item => item.id === definition.id);
...
const COMMAND_EXISTS_CACHE = new Map<string, { value: boolean; expiresAt: number }>();
...
export function isAgentRuntimeAvailable(agentType: string) {
  ...
  if (["claudecode", "cursor", "codex", "gemini", "opencode"].includes(runtime)) return isDevelopmentAgentReady(runtime);
...
export function getAgentRuntimeFallbackChain(preferred = "claudecode") {
  ...
  const priority: AgentRuntimeId[] = ["claudecode", "codex", "cursor", "gemini", "opencode"];
...
export function normalizeAgentCommandOutput(agentType: string, rawOutput: string, options: any = {}) {
  ...
  if (!raw) return { output: raw, sessionId: "", rawSessionId: "", usage, providerOutputContractEvidence };
  ...
  if (["gemini", "opencode"].includes(runtime)) {
    ...
    const append = (value: any) => {
      ...
      if (text && messages[messages.length - 1] !== text) messages.push(text);
    };
    const inspect = (event: any) => {
      if (!event || typeof event !== "object") return;
      if (runtime === "gemini") rawSessionId = String(event.conversation_id || event.conversationId || rawSessionId || "");
      if (typeof event.response === "string") append(event.response);
      if (typeof event.result === "string") append(event.result);
      if (event.type === "text") append(event.text || event.part?.text);
      ...
      if (event.part?.type === "text") append(event.part.text);
    };
    for (const line of raw.split(/\r?\n/)) {
      const text = line.trim();
      if (!text.startsWith("{")) continue;
    ...
    if (!messages.length && raw.startsWith("{") && raw.endsWith("}")) {
      try { inspect(JSON.parse(raw)); } catch {}
    }
    return {
