...
function narrativeList(value: any, limit = 12, fallback = "相关技术细节已放入技术详情。") {
...
function getIndependentReviewGateState(input: MainAgentWorkchainInput) {
...
function collectAcceptanceEvidence(input: MainAgentWorkchainInput) {
  const summary = input.summary || {};
  const completion = input.completion || {};
  const deliveryReport = summary.delivery_report || summary.deliveryReport || completion.delivery_report || completion.deliveryReport || {};
...
function collectIndependentReviewEvidence(input: MainAgentWorkchainInput) {
  const summary = input.summary || {};
  const completion = input.completion || {};
  const deliveryReport = summary.delivery_report || summary.deliveryReport || completion.delivery_report || completion.deliveryReport || {};
  const gate = summary.independent_review_gate || summary.independentReviewGate || deliveryReport.independent_review_gate || deliveryReport.independentReviewGate || {};
...
function normalizeWorkchainTodoSteps(input: MainAgentWorkchainInput, stages: any[], terminal: boolean) {
...
function buildWorkchainTodoVerificationReminder(input: MainAgentWorkchainInput, steps: any[], evidence: ReturnType<typeof collectCompletionEvidence>, terminal: boolean) {
...
function applyQualityFollowupTodoStep(steps: any[], qualityFollowup: any) {
...
function buildWorkchainTodoPlan(input: MainAgentWorkchainInput, stages: any[], evidence: ReturnType<typeof collectCompletionEvidence>, terminal: boolean, options: { qualityFollowup?: any } = {}) {
...
function eventStatusForCheckpoint(item: any) {
...
function checkpointLabelFromAction(action: any) {
  const id = String(action || "");
  if (/inspect|read|query|search|analyze|context/i.test(id)) return "已检查相关上下文";
  if (/plan|reason/i.test(id)) return "已形成执行计划";
  if (/orchestrate|dispatch|send_group|send_project|child|mission/i.test(id)) return "已安排执行目标";
  if (/verify|review|acceptance|receipt/i.test(id)) return "已进入验收检查";
  if (/complete|final|reply|summary|report/i.test(id)) return "已整理阶段结果";
  return "";
}

function checkpointLabelFromEvent(item: any, surface: MainAgentWorkchainSurface) {
  ...
  if (type === "reasoning_plan" || type === "reasoning_preflight") return "我已复核目标与验收";
  if (type === "sandbox_rehearsal") return "已完成任务前预演";
  if (type === "conflict_plan") return "已启用修改冲突保护";
  if (type === "global_mission_handoff_ready") return "全局 Agent 已补齐子任务交接";
  if (type === "worker_handoff_ready") return `${agent}工作单已补齐`;
  if (type === "global_mission_plan") return "全局 Agent 已制定跨项目计划";
  if (type === "dispatch") return "已安排执行成员";
  if (type === "direct_task") return "已安排项目执行成员";
  ...
  if (type === "next_work_item_dispatch") return "下一步工作已接上";
  if (type === "targeted_rework") return "我已发起定向补充";
  if (type === "auto_gap_rework") return "我已按缺口自动返工";
  if (type === "task_continuation") return "补充要求已接收";
  ...
  if (type === "runtime_fallback" || type === "runtime_switch") return agent ? `${agent}切换执行通道` : "Agent 已切换执行通道";
  ...
  if (type === "global_supervisor_cycle") return "全局监工已检查子任务";
  if (type === "global_supervisor_rework") return "全局监工已安排返工";
  if (type === "global_supervisor_waiting_user") return "全局监工等待你处理阻塞";
  ...
  if (title) return sanitizeWorkchainUserText(title, surface === "global" ? "我更新了处理进展。" : "协作群更新了处理进展。", 96);
  return "";
}

function buildMainAgentProgressCheckpoints(input: MainAgentWorkchainInput, stages: any[], evidence: ReturnType<typeof collectCompletionEvidence>, options: { qualityFollowup?: any } = {}) {
  const items: any[] = [];
  ...
  for (const event of (input.rawEvents || []).filter(Boolean)) {
    ...
    if (INTERNAL_TEXT_PATTERN.test(raw)) continue;
    push({
  ...
  if (!items.length) {
    for (const action of input.actionIds || []) {
      ...
      if (label) push({ id: `action-${action}`, label, status: "done", phase: input.phase || input.mode || "", source: "action" });
    }
  }

  if (!items.length) {
    for (const stage of stages) {
      if (!["completed", "in_progress", "needs_confirmation", "failed", "cancelled"].includes(String(stage.status || ""))) continue;
...
function stageStatus(input: MainAgentWorkchainInput, stage: string) {
  ...
  if (stage === "plan") {
    if (["waiting_confirmation", "waiting_clarification", "needs_user"].includes(status) || phase === "needs_user") return "needs_confirmation";
    return terminal || (input.steps || []).length || (input.actionIds || []).length ? "completed" : "in_progress";
  }
  if (stage === "execute") {
    if (["cancelled", "canceled"].includes(status) || phase === "cancelled") return "cancelled";
    if (status === "failed" || phase === "failed") return "failed";
    if (terminal) return "completed";
    if ((input.workers || []).length || (input.executions || []).length || (input.actionIds || []).some(id => /dispatch|create|execute|manage|send|orchestrate/i.test(String(id)))) return "in_progress";
    return "pending";
  }
  if (stage === "verify") {
    if (status === "failed" || phase === "failed") return "failed";
    if (["waiting_confirmation", "waiting_clarification"].includes(status) || phase === "needs_user") return "needs_confirmation";
    if (terminal) return "completed";
    const summary = input.summary || {};
    if (summary.acceptance_gate_passed === true || stringList(summary.verification_executed).length) return "in_progress";
    return "pending";
  }
  if (stage === "summarize") {
    if (terminal || status === "supervising") return "completed";
    if (["waiting_confirmation", "waiting_clarification"].includes(status) || phase === "needs_user") return "needs_confirmation";
    return "pending";
  }
  return "pending";
...
function collectCompletionEvidence(input: MainAgentWorkchainInput) {
  const summary = input.summary || {};
  const completion = input.completion || {};
...
function hasExecutableWorkEvidence(input: MainAgentWorkchainInput, evidence: ReturnType<typeof collectCompletionEvidence>) {
...
function isBareWorkchainAcceptanceLine(item: any) {
  return /^最终验收(已通过|未通过)/.test(String(item || "").trim());
}

function workchainVerificationFailureText(item: any) {
...
function workchainAcceptanceFailureText(item: any) {
  const text = String(item || "").trim();
  if (!text) return false;
  return /未通过|失败|缺口|待处理|需要返工|需返工/i.test(text);
}

function hasStrongWorkchainVerificationEvidence(evidence: ReturnType<typeof collectCompletionEvidence>) {
...
function collectWorkchainVisibleQualityText(value: any, depth = 0): string[] {
  if (depth > 8 || value === undefined || value === null) return [];
  if (typeof value === "string" || typeof value === "number" || typeof value === "boolean") return [String(value)];
  if (Array.isArray(value)) return value.flatMap(item => collectWorkchainVisibleQualityText(item, depth + 1));
...
function buildFinalSummaryQuality(input: MainAgentWorkchainInput, evidence: ReturnType<typeof collectCompletionEvidence>, terminal: boolean, headline: string, nextAction: string, options: { todoPlan?: any; progressCheckpoints?: any } = {}) {
  const summary = input.summary || {};
  const completion = input.completion || {};
  const deliveryReport = summary.delivery_report || summary.deliveryReport || completion.delivery_report || completion.deliveryReport || {};
...
function buildUserVisibleText(input: MainAgentWorkchainInput, evidence: ReturnType<typeof collectCompletionEvidence>) {
  ...
  if (evidence.independentReviewGate?.failed) return "这轮还不能算完成：独立复核未通过，需要原实现成员返工后重新复核。";
  ...
  if (["completed", "done", "succeeded"].includes(status) || phase === "completed") {
    ...
    if (evidence.independentReview.length && !evidence.independentReviewGate?.failed) parts.push(`完成独立复核`);
