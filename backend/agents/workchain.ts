...
function checkpointLabelFromEvent(item: any, surface: MainAgentWorkchainSurface) {
  ...
  if (type === "runtime_fallback" || type === "runtime_switch") return agent ? `${agent}切换执行通道` : "我已切换执行通道";
  ...
  if (type === "global_supervisor_cycle") return "我已检查子任务进展";
  if (type === "global_supervisor_rework") return "我已安排子任务返工";
  if (type === "global_supervisor_waiting_user") return "我在等待你处理阻塞";
