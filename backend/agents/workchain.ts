...
function checkpointLabelFromEvent(item: any, surface: MainAgentWorkchainSurface) {
  ...
  if (type === "reasoning_plan" || type === "reasoning_preflight") return "主 Agent 已复核目标与验收";
  ...
  if (type === "direct_task") return "已派发给项目 Agent";
  ...
  if (type === "targeted_rework") return "主 Agent 已发起定向补充";
  if (type === "auto_gap_rework") return "主 Agent 已按缺口自动返工";
  if (type === "task_continuation") return "补充要求已接收";
  if (type === "reasoning_recovery_check" || type === "startup_manual_recovery") return "主 Agent 已接上恢复任务";
  ...
  if (type === "global_agent.run_completed") return "全局主 Agent 已完成总结";
  if (title) return sanitizeWorkchainUserText(title, surface === "global" ? "全局主 Agent 更新了处理进展。" : "群聊主 Agent 更新了处理进展。", 96);
  return "";
}

function buildMainAgentProgressCheckpoints(input: MainAgentWorkchainInput, stages: any[], evidence: ReturnType<typeof collectCompletionEvidence>) {
  const items: any[] = [];
...
export function sanitizeWorkchainUserText(value: any, fallback = "Agent 正在处理当前请求。", max = 260) {
  let text = compactText(value, max);
  if (!text) text = fallback;
  if (INTERNAL_TEXT_PATTERN.test(text)) {
    if (/error|失败|denied|invalid|权限|门禁/i.test(text)) text = "Agent 遇到需要处理的执行保护或权限问题，排障信息已放入技术详情。";
    else if (/done|完成|receipt|回执/i.test(text)) text = "Agent 已提交结果说明，主 Agent 正在汇总验收。";
    else text = fallback;
  }
...
function buildFinalSummaryQuality(input: MainAgentWorkchainInput, evidence: ReturnType<typeof collectCompletionEvidence>, terminal: boolean, headline: string, nextAction: string) {
  const summary = input.summary || {};
...
export function formatMainAgentCompletionReply(options: { reply?: any; workchain: any; includeDetails?: boolean }) {
  ...
  if (shouldShape && summary.evidence?.length) lines.push(`处理总结：\n- ${summary.evidence.slice(0, 6).join("\n- ")}`);
  else if (shouldShape && quality.required) lines.push("处理总结：主 Agent 已完成本轮处理，但没有更多可展示的业务证据。");
  if (shouldShape && summary.verification?.length) lines.push(`验证与验收：\n- ${summary.verification.slice(0, 6).join("\n- ")}`);
  else if (shouldShape && summary.verification_status) lines.push(`验证与验收：${summary.verification_status}`);
  if (shouldShape && summary.risks?.length) lines.push(`需要留意：\n- ${summary.risks.slice(0, 5).join("\n- ")}`);
  else if (shouldShape && summary.risk_status) lines.push(`需要留意：${summary.risk_status}`);
  if (shouldShape && summary.next_action) lines.push(`下一步：${summary.next_action}`);
