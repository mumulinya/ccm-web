...
import type {
  MainAgentWorkchainInput,
  MainAgentWorkchainSurface,
} from "./workchain-part-01-part-01";
import {
  INTERNAL_TEXT_PATTERN,
  WORKCHAIN_USER_VISIBLE_PROTOCOL_PATTERN,
  applyQualityFollowupTodoStep,
  buildWorkchainTodoVerificationReminder,
  checkpointStatus,
  ...
  compactText,
  getIndependentReviewGateState,
  getPostReviewSpotCheckState,
  hasExecutableWorkEvidence,
  hasStrongWorkchainVerificationEvidence,
  narrativeList,
  normalizeWorkchainTodoSteps,
  sanitizeWorkchainTerminology,
  sanitizeWorkchainUserText,
  stringList,
  workchainAcceptanceFailureText,
  workchainVerificationFailureText,
} from "./workchain-part-01-part-01";

export function buildWorkchainTodoPlan(input: MainAgentWorkchainInput, stages: any[], evidence: ReturnType<typeof collectCompletionEvidence>, terminal: boolean, options: { qualityFollowup?: any } = {}) {
...
export function buildMainAgentProgressCheckpoints(input: MainAgentWorkchainInput, stages: any[], evidence: ReturnType<typeof collectCompletionEvidence>, options: { qualityFollowup?: any } = {}) {
  const items: any[] = [];
...
export function stageStatus(input: MainAgentWorkchainInput, stage: string) {
...
export function terminalWorkchain(input: MainAgentWorkchainInput) {
...
export function collectWorkchainVisibleQualityText(value: any, depth = 0): string[] {
...
export function buildFinalSummaryQuality(input: MainAgentWorkchainInput, evidence: ReturnType<typeof collectCompletionEvidence>, terminal: boolean, headline: string, nextAction: string, options: { todoPlan?: any; progressCheckpoints?: any } = {}) {
  const summary = input.summary || {};
