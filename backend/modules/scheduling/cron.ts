...
function readConflictResolutionMaintenanceSchedulerState(file = CONFLICT_RESOLUTION_MAINTENANCE_SCHEDULER_STATE_FILE) {
  try { return JSON.parse(fs.readFileSync(file, "utf-8")); } catch { return { schema: "ccm-conflict-resolution-maintenance-scheduler-state-v1", version: 1, groups: {}, updated_at: "" }; }
}
...
export function runConflictResolutionMemoryMaintenanceSchedulerTick(options: any = {}) {
  ...
  for (const groupId of groupIds) {
    ...
    try {
      ...
      for (const telemetryResult of [telemetryRecovery, telemetryOrphans, telemetryCleanupJournals, telemetryQuarantineRetention]) {
      ...
      rows.push({ groupId, status: Number(result?.dueCount || 0) > 0 ? "completed" : "not_due", skipped: Number(result?.dueCount || 0) === 0, operationKey, result, telemetryRecovery, telemetryOrphans, telemetryCleanupJournals, telemetryQuarantineRetention, telemetryRetention, destructiveActionAuthorized: false, deletedCount: 0 });
    } catch (error: any) {
