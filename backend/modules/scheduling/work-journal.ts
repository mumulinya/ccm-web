...
const PROJECT_SESSIONS_DIR = path.join(CCM_DIR, "web-sessions");
const TEST_AGENT_RUNS_DIR = path.join(CCM_DIR, "test-agent-runs");
...
function validIso(value: any) {
  const time = Date.parse(String(value || ""));
  return Number.isFinite(time) ? new Date(time).toISOString() : "";
}

function compact(value: any, fallback = "", max = 220) {
...
function uniqueStrings(values: any[], limit = 120) {
...
function event(input: Partial<WorkJournalEvent> & { id: string; at: string; type: string; title: string }): WorkJournalEvent | null {
  const at = validIso(input.at);
  if (!at || !input.id) return null;
  return {
...
function isWorkRequest(message: any) {
...
function sourceFromMessage(message: any, fallback: string) {
...
function verificationLines(task: any) {
  const summary = task?.delivery_summary || {};
...
function actorForTimeline(item: any): { type: WorkActorType; label: string } {
  const agent = String(item?.agent || "").trim();
  if (/test.?agent/i.test(agent) || /independent.?review/i.test(String(item?.type || ""))) return { type: "test_agent", label: "TestAgent" };
  if (agent) return { type: "agent", label: agent };
  if (/coordinator|child_agent|dispatch|review|mission_plan/i.test(String(item?.type || ""))) return { type: "agent", label: /coordinator/i.test(String(item?.type || "")) ? "群聊主 Agent" : "项目子 Agent" };
  return { type: "system", label: "系统" };
}

function timelineState(item: any) {
  const type = String(item?.type || "");
  const status = String(item?.status || "").toLowerCase();
  if (type === "acceptance_gate" && ["ok", "completed", "passed"].includes(status)) return "verified";
  if (/failed/.test(type) || ["fail", "failed", "error"].includes(status)) return "failed";
  if (/rework|waiting|rollback/.test(type) || ["warn", "warning", "blocked", "waiting"].includes(status)) return "blocked";
  if (/receipt|accepted/.test(type) && ["ok", "completed", "passed"].includes(status)) return "progress";
  return "progress";
}

export function projectTaskWorkEvents(tasks: any[] = loadTasks()) {
  const projected: WorkJournalEvent[] = [];
  for (const task of tasks) {
    ...
    if (created) projected.push(created);
    ...
    if (task.status === "done" || task.delivery_summary?.acceptance_gate_passed === true) {
      ...
      if (completed) projected.push(completed);
      ...
      if (blocked) projected.push(blocked);
    }
    ...
    for (const file of taskFiles(task)) {
      ...
      if (changed) projected.push(changed);
    }
    ...
    for (const verification of verificationLines(task)) {
      ...
      if (verified) projected.push(verified);
    }
    ...
    if (task.review && /test.?agent/i.test(String(task.review.agent || task.review.reviewer || ""))) {
      ...
      if (review) projected.push(review);
    }
  }
  return projected;
}

function projectConversation(messages: any[], context: any) {
  const projected: WorkJournalEvent[] = [];
  for (let index = 0; index < messages.length; index += 1) {
    const message = messages[index];
    if (!isWorkRequest(message)) continue;
    ...
    for (let cursor = index + 1; cursor < messages.length; cursor += 1) {
      ...
      if (response) projected.push(response);
      break;
    }
  }
  return projected;
...
export function projectConversationWorkEvents() {
  const projected: WorkJournalEvent[] = [];
  ...
  for (const session of globalSessions as any[]) {
  ...
  for (const group of loadGroups()) {
    const sessions = listGroupChatSessions(group.id).sessions || [];
    for (const session of sessions) {
      projected.push(...projectConversation(getGroupMessages(group.id, session.id), {
        source: "group_chat",
...
export function projectTestAgentWorkEvents(tasks: any[] = loadTasks()) {
  if (!fs.existsSync(TEST_AGENT_RUNS_DIR)) return [];
  const tasksById = new Map(tasks.map((task: any) => [String(task.id || ""), task]));
  const projected: WorkJournalEvent[] = [];
...
export function collectProjectedWorkEvents() {
...
export function readWorkJournalEvents() {
  if (!fs.existsSync(WORK_JOURNAL_FILE)) return [] as WorkJournalEvent[];
  return fs.readFileSync(WORK_JOURNAL_FILE, "utf-8").split(/\r?\n/).filter(Boolean).flatMap(line => {
    try {
      const parsed = JSON.parse(line);
      return parsed?.schema === WORK_EVENT_SCHEMA && parsed?.id && validIso(parsed.at) ? [parsed as WorkJournalEvent] : [];
    } catch {
      return [];
    }
  });
...
export function mergeWorkJournalEvents(existing: WorkJournalEvent[], candidates: WorkJournalEvent[]) {
  const seen = new Set(existing.map(item => item.id));
  const appended: WorkJournalEvent[] = [];
...
export function syncWorkJournal() {
...
function isReportableEvent(item: WorkJournalEvent) {
  return item.type !== "technical_progress";
}

function stateAtEnd(events: WorkJournalEvent[], end: Date) {
  ...
  for (const item of relevant) {
    if (["task_created"].includes(item.type)) states.set(item.task_id, "pending");
    else if (["task_queued"].includes(item.type)) states.set(item.task_id, "queued");
    else if (["task_started", "dispatch", "child_agent_start", "child_agent_receipt"].includes(item.type)) states.set(item.task_id, "in_progress");
    else if (item.type === "task_completed") states.set(item.task_id, "done");
    else if (item.type === "task_failed") states.set(item.task_id, "failed");
    else if (item.type === "task_blocked") states.set(item.task_id, "blocked");
  }
  return states;
}
...
function workTitle(workEvents: WorkJournalEvent[], taskMap: Map<string, any>) {
...
function groupActivities(dayEvents: WorkJournalEvent[], allEvents: WorkJournalEvent[], day: any) {
  const taskMap = new Map(loadTasks().map((task: any) => [String(task.id || ""), task]));
  const states = stateAtEnd(allEvents, day.end);
  const grouped = new Map<string, WorkJournalEvent[]>();
  ...
  return Array.from(grouped.entries()).map(([id, events]) => {
    ...
    return {
      ...
      status: state,
      status_label: ({ done: "已完成", in_progress: "执行中", queued: "已排队", failed: "失败", blocked: "等待处理", pending: "待开始" } as any)[state] || "已记录",
...
function ownershipSummary(events: WorkJournalEvent[]) {
  const count = (actor: WorkActorType) => events.filter(item => item.actor_type === actor && isReportableEvent(item)).length;
  return {
...
function evidenceSummary(events: WorkJournalEvent[]) {
  const reportable = events.filter(isReportableEvent);
  const sourceCounts: Record<string, number> = {};
  for (const item of reportable) sourceCounts[item.source] = Number(sourceCounts[item.source] || 0) + 1;
  return {
...
function formatActivity(item: any) {
  ...
  if (item.files_changed?.length) lines.push(`  - 代码变更：${item.files_changed.join("、")}`);
  if (item.verifications?.length) lines.push(`  - 已执行验证：${item.verifications.join("；")}`);
  if (item.test_status) lines.push(`  - TestAgent：${item.test_status === "passed" ? "验收通过" : item.test_status === "failed" ? "验收未通过" : "已复核"}`);
  return lines;
}

function buildDailyMarkdown(report: any) {
  const userActivities = report.activities.filter((item: any) => item.actor_types?.includes("user"));
  const agentActivities = report.activities.filter((item: any) => item.actor_types?.includes("agent"));
  const testActivities = report.activities.filter((item: any) => item.actor_types?.includes("test_agent"));
  const systemActivities = report.activities.filter((item: any) => item.actor_types?.includes("system"));
  return [
    `# 工作日报 ${report.date}`,
    "",
    "## 今天完成了什么",
    ...
    "",
    "## 仍在进行",
    ...(report.running.length ? report.running.map((item: any) => `- ${item.title}（${item.status_label}）`) : ["- 暂无执行中的工作"]),
    "",
    "## 风险与待处理",
    ...(report.blockers.length ? report.blockers.map((item: any) => `- ${item.title}：${item.reason}`) : ["- 暂无明确阻塞"]),
    "",
    "## 明天继续",
    ...(report.next_actions.length ? report.next_actions.map((item: string) => `- ${item}`) : ["- 等待新的业务需求"]),
    "",
    "## 数据依据",
...
export function generateEvidenceDailyReport(dateKey = localWorkDateKey(), inputEvents?: WorkJournalEvent[]) {
  ...
  const sourceTypeCount = (source: string) => dayEvents.filter(item => item.type === "work_request" && item.source === source).length;
  const nextActions = [
  ...
  const report: any = {
    ...
    summary: {
      ...
      enabled_daily_dev_jobs: loadCronJobs().filter((job: any) => job.workflow_type === "daily_dev" && job.enabled !== false).length,
...
function buildWeeklyMarkdown(report: any) {
  return [
    ...
    "",
    "## 代码与交付",
    ...(report.changed_files.length ? report.changed_files.map((file: string) => `- ${file}`) : ["- 本周暂无可核验的文件变更"]),
    "",
    "## 验收与质量",
    ...
    "",
    "## 下周继续",
    ...(report.next_week.length ? report.next_week.map((item: string) => `- ${item}`) : ["- 等待新的业务需求"]),
    "",
    "## 本周工作依据",
...
export function generateEvidenceWeeklyReport(dateKey = localWorkDateKey()) {
  ...
  const highlights = activities.filter((item: any) => item.evidence_count > 1 || item.files_changed?.length || item.verifications?.length).slice(0, 50);
  const nextWeek = [
    ...blockers.slice(0, 4).map((item: any) => `继续处理：${item.title}`),
    ...(lastDay.running || []).slice(0, 4).map((item: any) => `继续推进：${item.title}`),
...
export function listWorkJournalEvents(options: any = {}) {
  const sync = syncWorkJournal();
...
export function getWorkJournalAudit(options: { sync?: boolean } = {}) {
...
export function runWorkJournalSelfTest() {
