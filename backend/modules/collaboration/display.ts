import { buildMainAgentWorkchain } from "../../agents/workchain";
import {
...
function compactDisplayText(value: any, max = 220) {
...
function sanitizeUserVisibleTerminology(value: string) {
  return sanitizeUserFacingTerminology(value);
}

function sanitizeUserVisibleProtocolTerms(value: string) {
  return sanitizeUserFacingProtocolTerms(value);
}

export function sanitizeMainAgentUserText(value: any, fallback = "我正在处理当前请求。", max = 260) {
  ...
  text = sanitizeMainAgentRoleLanguage(sanitizeUserVisibleProtocolTerms(sanitizeUserVisibleTerminology(text)))
...
export function buildStreamlinedToolUseSummary(input: {
  actionIds?: string[];
  steps?: any[];
  workers?: any[];
  executions?: any[];
  ...
  if (counts.receipts) parts.push(`结果说明 ${counts.receipts} 条`);
...
export function buildTechnicalDetailSections(input: {
  traceId?: string;
  actionIds?: string[];
  permissions?: any[];
  observations?: any;
  ...
  if (technical.failed_gates?.length) troubleshooting.push({ label: "未通过验收", value: sanitizeMainAgentUserText(technical.failed_gates.map((item: any) => item.label || item.id || item).join("、"), "仍有验收检查未通过。", 240) });
  ...
  if (input.observations) records.push({ label: "观察", value: compactDisplayText(JSON.stringify(input.observations), 360) });
...
export function buildMainAgentDisplayStream(input: {
  ...
  const technicalDetails = [
    ...
    ...workchain.technical_details.filter((section: any) => !baseTechnicalDetails.some(existing => existing.id === section.id)),
  ];
  return {
