...
export const REVIEW_FRESHNESS_SCHEMA = "ccm-review-freshness-v1";

export type ReviewFreshnessState = "fresh" | "stale" | "unknown";

function normalizeChangePath(value: any): string {
  if (!value) return "";
...
export function buildReviewChangeFingerprint(...sources: any[]): string {
  ...
  const visit = (value: any) => {
    ...
    if (typeof value === "object" && Array.isArray(value.files)) {
      for (const item of value.files) visit(item);
      return;
    }
...
export function buildReviewFreshnessStamp(reviewedChanges: any[], at = new Date().toISOString()) {
  const fingerprint = buildReviewChangeFingerprint(reviewedChanges);
  return {
    schema: REVIEW_FRESHNESS_SCHEMA,
    reviewedAt: at,
    reviewed_at: at,
...
export function readReviewFingerprint(entry: any): string {
  return String(
    entry?.reviewedChangeFingerprint
 ...
 */
export function classifyReviewFreshness(entry: any, currentFingerprint: string): ReviewFreshnessState {
  const reviewed = readReviewFingerprint(entry);
  const current = String(currentFingerprint || "").trim();
  if (!reviewed || !current) return "unknown";
 ...
 */
export function applyReviewFreshnessToEvidence(evidence: any[], currentFingerprint: string) {
  const current = String(currentFingerprint || "").trim();
  let staleCount = 0;
  const rows = (Array.isArray(evidence) ? evidence : []).map((item: any) => {
...
export function runReviewFreshnessSelfTest() {
