...
import { readProviderNativeMicrocompactCapability } from "../../system/provider-native-microcompact-capability";
import { readProviderNativeToolCapability } from "../../system/provider-native-tool-capability";
import { readMainAgentContextPolicy } from "../../tools/main-agent-context-policy";
...
export function loadOrchestratorConfig() {
  try {
    ...
    for (const [field, credentialField] of [
      ...
      if (stored[field] && !isCredentialReference(stored[field])) {
        stored[field] = protectCredential("web-search", credentialField, stored[field]);
        try { writeStoredOrchestratorConfig(stored); } catch {}
      }
    }
    return {
      ...
      braveSearchApiKey: stored.braveSearchApiKey ? resolveCredential(stored.braveSearchApiKey) : "",
      bingSearchApiKey: stored.bingSearchApiKey ? resolveCredential(stored.bingSearchApiKey) : "",
      googleCseApiKey: stored.googleCseApiKey ? resolveCredential(stored.googleCseApiKey) : "",
      googleCseId: stored.googleCseId ? resolveCredential(stored.googleCseId) : "",
    };
  } catch {
...
function persistOrchestratorConfig(config: any) {
  const stored = {
    ...
    searchMcpToken: config.searchMcpToken ? protectCredential("web-search", "mcpToken", config.searchMcpToken) : "",
    braveSearchApiKey: config.braveSearchApiKey ? protectCredential("web-search", "braveApiKey", config.braveSearchApiKey) : "",
    bingSearchApiKey: config.bingSearchApiKey ? protectCredential("web-search", "bingApiKey", config.bingSearchApiKey) : "",
    googleCseApiKey: config.googleCseApiKey ? protectCredential("web-search", "googleApiKey", config.googleCseApiKey) : "",
    googleCseId: config.googleCseId ? protectCredential("web-search", "googleEngineId", config.googleCseId) : "",
  };
...
export function saveOrchestratorConfig(updates: any) {
  ...
  if (updates.format !== undefined) {
    ...
    if (!["auto", "openai-compatible", "openai-responses", "anthropic-compatible", "gemini-compatible"].includes(format)) throw new Error("不支持的大模型接口协议");
    next.format = format;
  }
  ...
  if (reasoningEffort !== undefined) {
    ...
    if (!["off", "low", "medium", "high"].includes(effort)) throw new Error("推理强度必须是 off、low、medium 或 high");
  ...
  if (memoryCompactionUseModel !== undefined && memoryCompactionUseModel !== true) throw new Error("群聊记忆压缩必须使用模型摘要");
  if (memoryCompactionMode !== undefined) {
    const mode = String(memoryCompactionMode || "model-required").trim().toLowerCase();
    if (mode !== "model-required") throw new Error("群聊记忆压缩模式只支持 model-required");
  }
  ...
  if (codeIndexStartPolicy !== undefined) {
    ...
    if (!["on_demand", "manual", "startup"].includes(value)) throw new Error("代码索引启动策略必须是 on_demand、manual 或 startup");
  ...
  if (codeIndexMaxConcurrentProjects !== undefined) {
    ...
    if (!Number.isFinite(value) || value < 1 || value > 8) throw new Error("并行代码索引项目数必须介于1和8");
  ...
  if (providerNativeToolsMode !== undefined) {
    ...
    if (!["auto", "native", "json"].includes(value)) throw new Error("Provider原生工具模式必须是 auto、native 或 json");
  ...
  if (webSearchProviderOrder !== undefined) {
    if (!Array.isArray(webSearchProviderOrder)) throw new Error("Web Search Provider顺序必须是数组");
    const allowed = new Set(["mcp", "brave", "bing", "google"]);
    const value = [...new Set(webSearchProviderOrder.map((item: any) => String(item || "").trim().toLowerCase().replace("google_cse", "google")).filter((item: string) => allowed.has(item)))];
    if (!value.length) throw new Error("至少保留一个Web Search Provider");
    next.webSearchProviderOrder = value;
  }
  const searchMcpUrl = updates.searchMcpUrl ?? updates.search_mcp_url;
  if (searchMcpUrl !== undefined) {
    if (searchMcpUrl === null) next.searchMcpUrl = "";
    else {
      const value = String(searchMcpUrl || "").trim();
      if (value && !/^https:\/\//i.test(value)) throw new Error("Search MCP地址必须使用HTTPS");
      if (value) next.searchMcpUrl = value;
    }
  }
  for (const [camelKey, snakeKey] of [
    ...
    const raw = updates[camelKey] ?? updates[snakeKey];
    if (raw !== undefined && String(raw || "").trim()) next[camelKey] = String(raw).trim();
  }
  const contextPolicy = readMainAgentContextPolicy({
  ...
  if (agentCommunicationV2Enabled !== undefined) next.agentCommunicationV2Enabled = agentCommunicationV2Enabled === true;
  const dynamicAgentBudgetEnabled = updates.dynamicAgentBudgetEnabled ?? updates.dynamic_agent_budget_enabled;
  if (dynamicAgentBudgetEnabled !== undefined) next.dynamicAgentBudgetEnabled = dynamicAgentBudgetEnabled === true;
  const adaptiveAgentLoopEnabled = updates.adaptiveAgentLoopEnabled ?? updates.adaptive_agent_loop_enabled;
  if (adaptiveAgentLoopEnabled !== undefined) next.adaptiveAgentLoopEnabled = adaptiveAgentLoopEnabled === true;
  for (const [camelKey, snakeKey] of [
  ...
  for (const [camelKey, snakeKey, min, max] of [
    ...
    if (!Number.isFinite(value) || value < min || value > max) throw new Error(`${camelKey} 必须介于 ${min} 和 ${max}`);
  ...
  if (testAgentPlannerFallbackMode !== undefined) {
    ...
    if (!["risk_based", "always", "never"].includes(value)) throw new Error("TestAgent规划降级策略必须是 risk_based、always 或 never");
  ...
  if (testAgentIsolationMode !== undefined) {
    ...
    if (!["sandbox_preferred", "sandbox_required", "strict_allowlist"].includes(value)) throw new Error("TestAgent隔离策略无效");
  ...
  if (testAgentSurfaceAuditMode !== undefined) {
    ...
    if (!["strict", "warn"].includes(value)) throw new Error("TestAgent变更面审计策略必须是 strict 或 warn");
  ...
  if (testAgentPostReviewSpotCheckMode !== undefined) {
    ...
    if (!["policy", "required", "off"].includes(value)) throw new Error("TestAgent完成前抽查策略必须是 policy、required 或 off");
  ...
  if (updates.summaryReviewerFormat !== undefined) {
    ...
    if (!["openai-compatible", "openai-responses", "anthropic-compatible", "gemini-compatible"].includes(value)) throw new Error("摘要复核模型接口协议无效");
...
export function publicOrchestratorConfig(config = loadOrchestratorConfig()) {
  const {
    apiKey, summaryReviewerApiKey, searchMcpToken, braveSearchApiKey,
    bingSearchApiKey, googleCseApiKey, googleCseId, searchMcpUrl, ...safe
  } = config;
  return {
    ...safe,
    ...
    providerNativeMicrocompactCapability: readProviderNativeMicrocompactCapability(config),
    providerNativeToolsCapability: readProviderNativeToolCapability(config),
    hasKey: !!apiKey,
...
export async function testUnifiedModelConnection() {
  ...
  const provider = shouldUseAnthropic(config) ? "anthropic-compatible" : shouldUseGemini(config) ? "gemini-compatible" : shouldUseOpenAiResponses(config) ? "openai-responses" : "openai-compatible";
  const consumers = [
