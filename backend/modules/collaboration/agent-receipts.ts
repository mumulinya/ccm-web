...
function normalizeVerificationResults(value: any) {
  return (Array.isArray(value) ? value : [])
    .filter((item: any) => item && typeof item === "object")
    .map((item: any) => {
      const status = String(item.status || item.verdict || "").trim().toLowerCase();
      if (!["passed", "failed", "blocked", "skipped", "not_run"].includes(status)) return null;
      return {
        name: compactMemoryText(item.name || item.command || item.check || "verification", 240),
        command: compactMemoryText(item.command || "", 500),
        status,
        exitCode: item.exitCode ?? item.exit_code ?? null,
        source: compactMemoryText(item.source || item.executor || "agent_receipt", 80),
...
function normalizeProviderSwitchExecution(value: any) {
  ...
  const normalized = {
    ...
    reason: compactMemoryText(item.reason || item.note || item.evidence || "", 600),
  };
...
export function checkTaskCompletion(response: string) {
  ...
  if (completionMarkers.some(marker => response.includes(marker))) return true;
...
function mergeRunnerVerificationIntoReceipt(receipt: any, raw: string) {
  ...
  return {
    ...
    verificationResults: [
      ...
      ...passed.map((name: string) => ({ name, command: "", status: "passed", exitCode: 0, source: "ccm_runner_verification", evidence: [] })),
      ...failed.map((name: string) => ({ name, command: "", status: "failed", exitCode: 1, source: "ccm_runner_verification", evidence: [] })),
    ],
...
function normalizeAgentReceipt(raw: any, agent: string) {
  ...
  const providerSwitchExecution = normalizeProviderSwitchExecution(
    raw.providerSwitchExecution
...
export function getReceiptAssignmentStatus(response: string, receipt: any) {
  void response;
  if (!receipt) return { status: "missing_receipt", text: "缺少结构化结果说明" };
...
export function formatAgentReceiptForReview(receipt: any) {
  ...
  return [
    ...
    `- 记忆溯源：${memoryProvenanceUsage.length ? memoryProvenanceUsage.map((item: any) => [
...
export function runAgentReceiptProviderDispatchOverrideFollowupSelfTest() {
  const response = [
    "CCM_AGENT_RECEIPT",
    "```json",
    JSON.stringify({
...
export function runAgentReceiptProviderSwitchExecutionSelfTest() {
  const response = [
    "CCM_AGENT_RECEIPT",
    "```json",
    JSON.stringify({
