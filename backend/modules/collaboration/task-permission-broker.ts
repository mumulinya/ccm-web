...
import {
  ...
  resolveUserNotificationsByDedupeKey,
...
async function askMainAgent(context: InternalMcpTaskContext, request: any) {
  ...
  if (!config?.apiKey || !config?.apiUrl || !config?.model) return { decision: "user", reason: `${reviewerLabel}模型未配置，无法可靠审批`, actor };
...
export async function requestTaskPermission(context: InternalMcpTaskContext, input: any) {
  ...
  if (existing) {
    if (existing.state === "rejected") {
      const repeated = withFileLock(STORE_FILE, () => {
        const requests = readStore();
  ...
  let decision: any;
  if (input?.forceUserDecision === true) decision = { decision: "user", reason: clean(input?.decisionReason || "当前会话要求在首次代码修改前征得用户同意", 600) };
  else if (classified.risk === "high") decision = { decision: "user", reason: classified.reasons.join("；") };
  else if (input?.forceMainAgentReview === true) decision = await askMainAgent(context, { ...classified, reason });
  ...
  else decision = await askMainAgent(context, { ...classified, reason });
...
export function decideTaskPermission(requestId: string, input: any) {
  ...
  resolveUserNotificationsByDedupeKey(`permission:${updated.id}`);
...
export async function deliverPendingTaskPermissionNotifications(ctx: any, dependencies: any = {}) {
  ...
  try {
    ...
    for (const request of due) {
      ...
      const notifications = createUserNotification({
      ...
      if (feishuEligible && !feishuSent) try {
        ...
        if (bound?.success || bound?.queued) {
          feishuSent = true;
          notificationV2Ids.forEach(id => setNotificationDeliveryState(id, "feishu", bound?.success ? "delivered" : "sending"));
        }
      } catch (error) {
        notificationV2Ids.forEach(id => setNotificationDeliveryState(id, "feishu", "failed", { error }));
      }
