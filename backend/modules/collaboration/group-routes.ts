...
import {
  acknowledgeInvalidPendingModelCapabilityRefreshOutcome,
  buildModelCapabilityRefreshPlan,
  readInvalidPendingModelCapabilityRefreshOutcomes,
  readModelCapabilityCache,
  readModelCapabilityDowngradeAlerts,
  readModelCapabilityRefreshOutcomeLedger,
  readModelCapabilityRefreshStatus,
  recordModelCapabilityEvidence,
  revokeModelCapabilityEvidence,
  runModelCapabilityCacheMaintenance,
  summarizeModelCapabilityCache,
} from "./model-capability-cache";
import {
  readGroupSessionRetentionMaintenanceStatus,
  runGroupSessionRetentionMaintenance,
} from "./group-session-maintenance";
import {
  runGroupMemoryAutoCompactionNow,
} from "./group-memory-context";
import {
  deleteGroupTestTarget,
  listGroupTestTargets,
  publicGroupWithoutTestTargetSecrets,
  saveGroupTestTarget,
} from "./group-test-targets";
import {
  deleteSharedFileV2,
  listSharedFilesV2,
...
// ===== merged from group-routes-part-01.ts =====
...
export function cleanGroupStatusFollowupText(value: any, fallback: string, max = 180) {
  const text = compactGroupStatusText(value, max);
  const fallbackText = sanitizeMainAgentDeliveryText(fallback, fallback, max);
  if (!text) return fallbackText;
...
export function handleBasicGroupRoutes(
  ...
  if (pathname === "/api/groups/shared" && req.method === "GET") {
    ...
    sendJson(res, { files: listSharedFilesV2("group", groupId) });
    return true;
  }
  ...
  if (pathname === "/api/groups/shared/add" && req.method === "POST") {
    ...
    req.on("end", () => {
      try {
        ...
        const name = validateSharedFileV2Name(rawName);
        if (!name || !content) return sendJson(res, { error: "文件名和内容不能为空" }, 400);
        const groups = loadGroups();
        const group = groups.find(g => g.id === group_id);
        if (!group) return sendJson(res, { error: "群聊不存在" }, 404);
        const file = upsertSharedTextV2("group", group_id, name, content);
        sendJson(res, { success: true, file, files: listSharedFilesV2("group", group_id) });
      } catch (e: any) {
        sendJson(res, { error: e.message }, 400);
      }
    });
  ...
  if (pathname === "/api/groups/shared/delete" && req.method === "POST") {
    ...
    req.on("end", () => {
      try {
        ...
        const group = groups.find(g => g.id === group_id);
        if (!group) return sendJson(res, { error: "群聊不存在" }, 404);
        const file = listSharedFilesV2("group", group_id).find((item: any) => item.name === name);
        if (file) deleteSharedFileV2("group", group_id, file.id);
        sendJson(res, { success: true, files: listSharedFilesV2("group", group_id) });
      } catch (e: any) {
        sendJson(res, { error: e.message }, 400);
      }
    });
      ...
      try {
        ...
        for (const name of file_names) {
          ...
          if (filePath && fs.existsSync(filePath)) {
            ...
            if (!record) continue;
            migrateLegacySharedFilesV2("group", group_id, [record], "global-shared-v1");
            imported++;
          }
        }
        sendJson(res, { success: true, imported, files: listSharedFilesV2("group", group_id) });
      } catch (e: any) {
        sendJson(res, { error: e.message }, 400);
      }
    });
