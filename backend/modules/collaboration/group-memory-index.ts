...
const SEMANTIC_RECALL_CONCEPTS: Array<[string, RegExp]> = [
...
function canonicalSemanticRecallText(value: any) {
...
function roundSemanticRecallScore(value: any, precision = 3) {
...
function postCompactCompletionMemoryPreservationClosureReceiptSourceReliability(entry: any = {}, options: any = {}) {
...
function postCompactCompletionMemoryPreservationClosureEvidenceSessionKey(entry: any = {}) {
...
function clusterPostCompactCompletionMemoryPreservationClosureEvidence(entries: any[] = []) {
  const clusters: any[] = [];
...
function normalizeGroupLogMemoryAdmission(message: any = {}) {
...
function normalizeGroupLogMemoryConfirmation(message: any = {}) {
...
function normalizeGroupLogMemoryRevocation(message: any = {}) {
...
function verifyGroupLogLifecycleCurrentSourceEvidence(evidence: any, projectRoot: string) {
...
function buildGroupLogPositiveConfirmationCandidate(groupId: string, messages: any[], index: number) {
...
function extractGroupLogPositiveFeedbackLifecycleRequests(groupId: string, messages: any[] = []) {
  const requests: any[] = [];
...
function applyGroupPositiveFeedbackLifecycle(groupId: string, facts: any, requests: any[] = [], previous: any = {}, options: any = {}) {
...
function addDistilledCandidate(candidates: any[], category: GroupTypedMemoryType, type: string, message: any, index: number, text: any, overrides: any = {}) {
...
function extractGroupLogDistillationCandidates(groupId: string, messages: any[] = []) {
...
function applyGroupLogDistillationAdmission(candidates: any[] = []) {
  const admitted: any[] = [];
  const rejected: any[] = [];
...
function filterExistingDistilledFactsByAdmission(facts: any = {}) {
...
function postCompactCompletionMemoryPreservationRepairClosureInputRows(input: any = {}) {
  if (Array.isArray(input)) return input;
  const direct = [
    ...(Array.isArray(input.rows) ? input.rows : []),
    ...(Array.isArray(input.items) ? input.items : []),
...
function postCompactCompletionMemoryPreservationRepairClosureRowId(row: any = {}) {
...
function conflictResolutionColdArchiveShardFile(groupId: string, relPath: string) {
...
function writeConflictResolutionMaintenanceNotificationReceiptLedger(groupId: string, entries: any[], at: string) {
  const file = getPostCompactCompletionMemoryPreservationClosureConflictResolutionMaintenanceNotificationReceiptFile(groupId);
  const value = {
    schema: "ccm-post-compact-completion-memory-preservation-closure-conflict-resolution-maintenance-notification-receipt-ledger-v1",
    version: 1,
...
function conflictResolutionMaintenanceNotificationDeliveryLedgerChecksum(value: any = {}) {
  return checksum({
    schema: value.schema || "",
...
function cleanupGroupLedgerLockChecksum(lock: any = {}) {
  return checksum({
...
function cleanupMetadataBackoffWait(ms: number) {
  if (!Number.isFinite(ms) || ms <= 0) return;
  try { Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, Math.max(1, Math.floor(ms))); } catch {}
}

function pruneCleanupMetadataArchives(directory: string, prefix: string, maxEntries: number) {
  let files: any[] = [];
  try {
...
function acquireCleanupGroupLedgerLock(groupId: string, at: string, options: any = {}) {
  const atMs = Date.parse(at);
...
function cleanupGroupLedgerLockHeld(groupId: string, handle: any) {
  if (!handle || handle.released === true || Number(handle.fd) < 0) return false;
  const current = readJson(handle.file, null);
  return !!current
...
function releaseCleanupGroupLedgerLock(groupId: string, handle: any) {
...
function retainConflictResolutionMaintenanceNotificationDeliveryQuarantine(groupId: string, entries: any[], compactedEntries: any[], at: string, options: any = {}) {
...
function conflictResolutionMaintenanceNotificationDeliveryCleanupReceiptLedgerChecksum(value: any = {}) {
  return checksum({
    group_id: value.group_id || "",
    revision: Number(value.revision || 0),
    previous_ledger_checksum: value.previous_ledger_checksum || "",
...
function writeConflictResolutionMaintenanceNotificationDeliveryCleanupReceiptLedger(groupId: string, entries: any[], at: string, options: any = {}) {
  return withCleanupGroupLedgerLock(groupId, at, options, groupLedgerLockHandle => {
    const file = getPostCompactCompletionMemoryPreservationClosureConflictResolutionMaintenanceNotificationDeliveryCleanupReceiptFile(groupId);
...
function conflictResolutionMaintenanceNotificationDeliveryCleanupLeaseChecksum(lease: any = {}) {
  return checksum({
...
function cleanupLeaseProcessAlive(pid: number) {
  if (!Number.isFinite(pid) || pid <= 0) return false;
  try { process.kill(pid, 0); return true; } catch { return false; }
}
...
function writeConflictResolutionMaintenanceNotificationDeliveryCleanupLeaseHandle(handle: any, leaseInput: any) {
...
function attachConflictResolutionMaintenanceNotificationDeliveryCleanupLease(journal: any, lease: any, leaseStatus = "active", at = "") {
  return {
    ...journal,
...
function conflictResolutionMaintenanceNotificationDeliveryCleanupJournalLedgerChecksum(value: any = {}) {
  return checksum({
    group_id: value.group_id || "",
    revision: Number(value.revision || 0),
    previous_ledger_checksum: value.previous_ledger_checksum || "",
...
function writeConflictResolutionMaintenanceNotificationDeliveryCleanupJournalLedger(groupId: string, entries: any[], at: string, options: any = {}) {
  return withCleanupGroupLedgerLock(groupId, at, options, groupLedgerLockHandle => {
    const file = getPostCompactCompletionMemoryPreservationClosureConflictResolutionMaintenanceNotificationDeliveryCleanupJournalFile(groupId);
...
function conflictResolutionMaintenanceNotificationDeliveryCleanupCommitCompactChecksum(value: any = {}) {
  return checksum({
    group_id: value.group_id || "",
...
function conflictResolutionMaintenanceNotificationDeliveryCleanupCommitLedgerChecksum(value: any = {}) {
  const payload: any = {
    group_id: value.group_id || "",
    revision: Number(value.revision || 0),
    previous_ledger_checksum: value.previous_ledger_checksum || "",
...
function upsertConflictResolutionMaintenanceNotificationDeliveryCleanupCommit(groupId: string, transactionInput: any, at: string, options: any = {}) {
  return withCleanupGroupLedgerLock(groupId, at, options, groupLedgerLockHandle => {
...
function maybeInterruptCleanupCommit(options: any, phase: string) {
...
function cleanupCommitRepairTargetSnapshots(entries: any[], idField: string, checksumField: string) {
  return (entries || []).map((entry: any) => ({ id: String(entry[idField] || ""), checksum: String(entry[checksumField] || "") }))
    .filter((entry: any) => entry.id)
    .sort((a: any, b: any) => a.id.localeCompare(b.id));
}

function cleanupCommitRepairTargetSnapshotsMatch(entries: any[], idField: string, checksumField: string, expected: any[]) {
...
function validateCleanupCommitRepairResolutionEvidence(groupId: string, evidenceChecksum: string) {
...
function cleanupCommitRepairResolutionTransactionArtifactChecksum(entry: any = {}, checksumField: string) {
  const value = { ...entry };
...
export function runPostCompactCompletionMemoryPreservationClosureConflictResolutionMaintenanceNotificationDeliveryCleanupCommitRepairLifecycleContextSelfTest() {
  return require("./group-memory-maintenance").runPostCompactCompletionMemoryPreservationClosureConflictResolutionMaintenanceNotificationDeliveryCleanupCommitRepairLifecycleContextSelfTest();
}
...
export function runPostCompactCompletionMemoryPreservationClosureConflictResolutionMaintenanceNotificationDeliveryCleanupCommitRepairResolutionTransactionSelfTest() {
  return require("./group-memory-maintenance").runPostCompactCompletionMemoryPreservationClosureConflictResolutionMaintenanceNotificationDeliveryCleanupCommitRepairResolutionTransactionSelfTest();
}
...
export function runPostCompactCompletionMemoryPreservationClosureConflictResolutionMaintenanceNotificationDeliveryCleanupCommitRepairResolutionTransactionStartupDiscoverySelfTest() {
  return require("./group-memory-maintenance").runPostCompactCompletionMemoryPreservationClosureConflictResolutionMaintenanceNotificationDeliveryCleanupCommitRepairResolutionTransactionStartupDiscoverySelfTest();
}
...
function pathWithinDirectory(target: string, directory: string) {
...
function retainConflictResolutionMaintenanceNotificationDeliveries(groupId: string, entries: any[], compactedEntries: any[], at: string, options: any = {}) {
  const atMs = Date.parse(at);
...
export function normalizeProviderDispatchOverrideFollowupRows(input: any = {}, options: any = {}) {
  const fallbackGroupId = String(options.groupId || options.group_id || input.groupId || input.group_id || "").trim();
...
export function mergeProviderDispatchOverrideFollowupRows(existing: any[] = [], incoming: any[] = [], options: any = {}) {
  const updatedAt = String(options.updatedAt || now());
  const merged = new Map<string, any>();
  for (const row of existing || []) {
...
export function pressureProvenanceProviderDispatchOverrideFollowupArchive(rows: any[] = [], options: any = {}) {
  const updatedAt = String(options.updatedAt || now());
...
export function renderPressureProvenanceProviderDispatchOverrideFollowupBody(archive: any = {}, options: any = {}) {
  const rows = Array.isArray(archive.rows) ? archive.rows : [];
...
export function normalizeProviderSwitchExecutionRows(input: any = {}, options: any = {}) {
  const fallbackGroupId = String(options.groupId || options.group_id || input.groupId || input.group_id || "").trim();
...
export function mergeProviderSwitchExecutionRows(existing: any[] = [], incoming: any[] = [], options: any = {}) {
  const updatedAt = String(options.updatedAt || now());
  const merged = new Map<string, any>();
  for (const row of existing || []) {
...
export function providerSwitchExecutionArchive(rows: any[] = [], options: any = {}) {
  const updatedAt = String(options.updatedAt || now());
  const attributionMap = new Map<string, any>();
  for (const row of rows || []) {
...
export function renderProviderSwitchExecutionBody(archive: any = {}, options: any = {}) {
  const rows = Array.isArray(archive.rows) ? archive.rows : [];
  const attributions = Array.isArray(archive.attributions) ? archive.attributions : [];
  const lines = [
...
export function normalizeProviderDispatchOverrideFollowupReceiptValidationRows(input: any = {}, options: any = {}) {
  const fallbackGroupId = String(options.groupId || options.group_id || input.groupId || input.group_id || "").trim();
  const fallbackGroupSessionId = String(options.groupSessionId || options.group_session_id || input.groupSessionId || input.group_session_id || "").trim();
...
export function mergeProviderDispatchOverrideFollowupReceiptValidationRows(existing: any[] = [], incoming: any[] = [], options: any = {}) {
  const updatedAt = String(options.updatedAt || now());
  const merged = new Map<string, any>();
  for (const row of existing || []) {
...
export function pressureProvenanceProviderDispatchOverrideFollowupReceiptValidationArchive(rows: any[] = [], options: any = {}) {
  const updatedAt = String(options.updatedAt || now());
...
export function renderPressureProvenanceProviderDispatchOverrideFollowupReceiptValidationBody(archive: any = {}, options: any = {}) {
  const rows = Array.isArray(archive.rows) ? archive.rows : [];
  const attributions = Array.isArray(archive.attributions) ? archive.attributions : [];
  const lines = [
...
export function normalizeIgnoreMemoryReceiptRepairRows(input: any = {}, options: any = {}) {
  const fallbackGroupId = String(options.groupId || options.group_id || input.groupId || input.group_id || "").trim();
...
export function mergeIgnoreMemoryReceiptRepairRows(existing: any[] = [], incoming: any[] = [], options: any = {}) {
  const updatedAt = String(options.updatedAt || now());
  const merged = new Map<string, any>();
  for (const row of existing || []) {
...
export function ignoreMemoryReceiptRepairArchive(rows: any[] = [], options: any = {}) {
  const updatedAt = String(options.updatedAt || now());
  return {
...
export function renderIgnoreMemoryReceiptRepairBody(rows: any[] = [], options: any = {}) {
  const lines = [
...
export function normalizePressureMemoryProvenanceReceiptRepairRows(input: any = {}, options: any = {}) {
  const fallbackGroupId = String(options.groupId || options.group_id || input.groupId || input.group_id || "").trim();
...
export function mergePressureMemoryProvenanceReceiptRepairRows(existing: any[] = [], incoming: any[] = [], options: any = {}) {
  const updatedAt = String(options.updatedAt || now());
  const merged = new Map<string, any>();
  for (const row of existing || []) {
...
export function pressureMemoryProvenanceReceiptRepairArchive(rows: any[] = [], options: any = {}) {
  const updatedAt = String(options.updatedAt || now());
  const relPaths = uniqueStrings(rows.flatMap((row: any) => Array.isArray(row.rel_paths) ? row.rel_paths : []), 80);
...
export function renderPressureMemoryProvenanceReceiptRepairBody(rows: any[] = [], options: any = {}) {
  const lines = [
...
function listProviderDispatchReliabilityDistillationLedgers(options: any = {}) {
  ...
  const explicitKeys = new Set(explicitGroupIds.flatMap((item: string) => [item.toLowerCase(), safeSegment(item).toLowerCase()]));
  const sortedCandidates = candidates
    .filter((item: any) => {
      if (!explicitKeys.size) return true;
    ...
    .sort((a: any, b: any) => Number(b.mtimeMs || 0) - Number(a.mtimeMs || 0));
  const selected: any[] = [];
  const selectedRootGroups = new Set<string>();
  const selectedLedgersPerGroup = new Map<string, number>();
  for (const item of sortedCandidates) {
    const identity = typedMemorySessionScopeIdentity(item.groupId, { groupId: item.groupId });
    const rootKey = identity.rootGroupId.toLowerCase();
    const isNewRoot = !selectedRootGroups.has(rootKey);
    if (isNewRoot && selectedRootGroups.size >= maxGroups) continue;
    if (Number(selectedLedgersPerGroup.get(rootKey) || 0) >= maxLedgersPerGroup) continue;
...
function typedMemoryDeliveryLeaseChecksum(lease: any = {}) {
  return checksum([
...
function typedMemoryConsumptionEntryChecksum(entry: any) {
  const fields: any[] = [
    entry.schema,
    entry.entry_id,
    entry.group_id,
...
function typedMemoryConsumptionLedgerChecksum(entries: any[], updatedAt: string) {
  return checksum([1, entries.map(entry => entry.checksum || ""), updatedAt], 64);
}

function resolveGroupTypedMemoryConsumptionScopeId(groupId: string) {
...
function typedMemoryConsumptionObservationId(groupId: string, row: any = {}, input: any = {}) {
...
function typedMemoryConsumptionEvidenceConfidence(row: any, usageState: string, proofValid: boolean) {
...
function typedMemoryStaleCandidateChecksum(candidate: any) {
  return checksum([
    candidate.schema,
    candidate.version,
...
function typedMemoryStaleResolutionChecksum(event: any) {
  return checksum([
    event.schema,
    event.version,
    event.event_id,
...
function typedMemoryStaleRejectionChecksum(rejection: any) {
  return checksum([
    rejection.schema,
    rejection.version,
...
function typedMemoryStaleCandidateLedgerChecksum(candidates: any[], events: any[], rejections: any[], updatedAt: string) {
  return checksum([
    1,
...
function writeGroupTypedMemoryStaleCandidateLedger(scopeId: string, input: any) {
  const file = getGroupTypedMemoryStaleCandidateLedgerFile(scopeId);
...
function typedMemoryStaleCandidateRejection(scopeId: string, row: any, codes: string[], at: string) {
  const payload = {
...
function verifyTypedMemoryStaleCandidateCurrentSource(candidate: any) {
...
function typedMemoryConsumptionQueryRelevance(entry: any, queryFeatures: any) {
...
export function getGroupTypedMemoryWriteShapeDir(scopeId: string) {
  return path.join(getGroupTypedMemoryDir(scopeId), GROUP_TYPED_MEMORY_WRITE_SHAPE_DIR);
}

export function getGroupTypedMemoryShapeTrendFile(scopeId: string) {
  return path.join(getGroupTypedMemoryDir(scopeId), GROUP_TYPED_MEMORY_SHAPE_TREND_LEDGER);
}

export function getGroupTypedMemoryShapeTrendIncidentFile(scopeId: string) {
  return path.join(getGroupTypedMemoryDir(scopeId), GROUP_TYPED_MEMORY_SHAPE_TREND_INCIDENT_LEDGER);
}

function groupTypedMemoryWriteShapeChecksum(value: any) {
  const payload = { ...(value || {}) };
  delete payload.checksum;
...
function groupTypedMemoryTextLineCount(value: string) {
  const text = String(value || "");
  return text ? text.split(/\r?\n/).length : 0;
}

export function verifyGroupTypedMemoryWriteShape(shape: any, expectedScopeId = "") {
  ...
  const valid = !!shape
    ...
    && Number(shape.version || 0) === GROUP_TYPED_MEMORY_WRITE_SHAPE_VERSION
    && scopeValid
    && !!String(shape.eventId || "")
    && path.basename(relPath) === relPath
    && relPath.toLowerCase().endsWith(".md")
    && relPath.toLowerCase() !== GROUP_TYPED_MEMORY_ENTRYPOINT.toLowerCase()
    && VALID_TYPES.has(String(shape.memoryType || "") as GroupTypedMemoryType)
    && operationValid
    && beforeBytes >= 0
    && afterBytes > 0
...
export function recordGroupTypedMemoryWriteShape(scopeId: string, input: any = {}) {
  ...
  const core = {
    ...
    version: GROUP_TYPED_MEMORY_WRITE_SHAPE_VERSION,
  ...
  const shape: any = { ...core, checksum: groupTypedMemoryWriteShapeChecksum(core) };
  if (!verifyGroupTypedMemoryWriteShape(shape, scopeId).valid) throw new Error("typed_memory_write_shape_invalid");
  ...
  if (path.dirname(file).toLowerCase() !== dir.toLowerCase()) throw new Error("typed_memory_write_shape_path_invalid");
  ...
  try {
    ...
    for (const item of files.slice(GROUP_TYPED_MEMORY_WRITE_SHAPE_MAX_EVENTS)) {
      try { fs.unlinkSync(item.file); } catch {}
    }
  } catch {}
  let trendContribution: any = null;
...
export function summarizeGroupTypedMemoryWriteShapes(scopeId: string, options: any = {}) {
  const rows: any[] = [];
...
function groupTypedMemoryShapeTrendBucketChecksum(value: any) {
  const payload = { ...(value || {}) };
  delete payload.checksum;
  return checksum(JSON.stringify(payload), 64);
}

function groupTypedMemoryShapeTrendLedgerChecksum(value: any) {
  const payload = { ...(value || {}) };
  delete payload.checksum;
  delete payload.file;
...
function groupTypedMemoryShapeTrendSummaryChecksum(value: any) {
  const payload = { ...(value || {}) };
  delete payload.checksum;
  return checksum(JSON.stringify(payload), 64);
}

function groupTypedMemoryShapeTrendDate(value: any) {
  const ms = typeof value === "number" ? value : Date.parse(String(value || ""));
  return Number.isFinite(ms) ? new Date(ms).toISOString().slice(0, 10) : "";
}

function emptyGroupTypedMemoryShapeTrendBucket(date: string) {
  return {
    date,
    sealed: false,
    sealedAt: "",
...
function emptyGroupTypedMemoryShapeTrendLedger(scopeId: string) {
  const core = {
    schema: "ccm-group-typed-memory-shape-trend-ledger-v1",
    version: GROUP_TYPED_MEMORY_SHAPE_TREND_VERSION,
    scopeId,
    bodyFree: true,
    retentionDays: GROUP_TYPED_MEMORY_SHAPE_TREND_RETENTION_DAYS,
    mutableDays: GROUP_TYPED_MEMORY_SHAPE_TREND_MUTABLE_DAYS,
    maxKeysPerBucket: GROUP_TYPED_MEMORY_SHAPE_TREND_MAX_KEYS_PER_BUCKET,
    generation: 0,
    headBucketChecksum: "",
    buckets: [] as any[],
    updatedAt: "",
...
function groupTypedMemoryShapeTrendMetricsValid(bucket: any) {
...
export function verifyGroupTypedMemoryShapeTrendLedger(ledger: any, expectedScopeId = "") {
  ...
  for (const bucket of buckets) {
    ...
    const bucketValid = /^\d{4}-\d{2}-\d{2}$/.test(date)
      ...
      && keys.length <= GROUP_TYPED_MEMORY_SHAPE_TREND_MAX_KEYS_PER_BUCKET
      && new Set(keys).size === keys.length
      && keys.every((key: any) => typeof key === "string" && /^[a-f0-9]{32}$/.test(key))
      && (bucket.sealed !== true || keys.length === 0);
    if (!bucketValid) chainValid = false;
  ...
  const valid = !!ledger
    ...
    && Number(ledger.version || 0) === GROUP_TYPED_MEMORY_SHAPE_TREND_VERSION
    && scopeValid
    && ledger.bodyFree === true
    && Number(ledger.retentionDays || 0) === GROUP_TYPED_MEMORY_SHAPE_TREND_RETENTION_DAYS
    && Number(ledger.mutableDays || 0) === GROUP_TYPED_MEMORY_SHAPE_TREND_MUTABLE_DAYS
    && Number(ledger.maxKeysPerBucket || 0) === GROUP_TYPED_MEMORY_SHAPE_TREND_MAX_KEYS_PER_BUCKET
    && buckets.length <= GROUP_TYPED_MEMORY_SHAPE_TREND_RETENTION_DAYS
...
export function readGroupTypedMemoryShapeTrendLedger(scopeId: string) {
  const file = getGroupTypedMemoryShapeTrendFile(scopeId);
  let primary: any = null;
  let backup: any = null;
  try { primary = JSON.parse(fs.readFileSync(file, "utf-8")); } catch {}
  const primaryVerification = verifyGroupTypedMemoryShapeTrendLedger(primary, scopeId);
  if (primaryVerification.valid) return { ...primary, file, present: true, valid: true, primaryValid: true, recoveredFromBackup: false };
  try { backup = JSON.parse(fs.readFileSync(`${file}.bak`, "utf-8")); } catch {}
  const backupVerification = verifyGroupTypedMemoryShapeTrendLedger(backup, scopeId);
  if (backupVerification.valid) return { ...backup, file, present: true, valid: true, primaryValid: false, recoveredFromBackup: true };
  const present = fs.existsSync(file) || fs.existsSync(`${file}.bak`);
  if (present) return { ...(primary || backup || {}), file, present: true, valid: false, primaryValid: false, recoveredFromBackup: false };
...
function normalizeGroupTypedMemoryShapeTrendContribution(kind: string, input: any = {}) {
  ...
  if (kind === "selector") {
    ...
    return {
      ...
      candidateCapacityRunCount: candidateCount >= GROUP_TYPED_MEMORY_MANIFEST_MAX_FILES ? 1 : 0,
      selectionCapacityRunCount: selectedCount >= GROUP_TYPED_MEMORY_MANIFEST_MAX_SELECTION ? 1 : 0,
    };
  }
  if (kind === "write") {
  ...
  return {
    ...
    usedDocumentCount: documents.filter((row: any) => row.usageState === "used").length,
    verifiedDocumentCount: documents.filter((row: any) => row.usageState === "verified").length,
    ignoredDocumentCount: documents.filter((row: any) => row.usageState === "ignored").length,
    unreportedDocumentCount: documents.filter((row: any) => row.usageState === "unreported").length,
    strongReceiptOutcomeCount: metrics.receiptBindingValid === true ? 1 : 0,
    unexpectedClaimCount: Number(metrics.unexpectedClaimCount || 0),
  };
}

function applyGroupTypedMemoryShapeTrendContribution(bucket: any, kind: string, metrics: any) {
  if (kind === "selector") {
...
function rechainGroupTypedMemoryShapeTrendBuckets(buckets: any[]) {
...
export function recordGroupTypedMemoryShapeTrendContribution(scopeId: string, input: any = {}, options: any = {}) {
  if (!isExactGroupTypedMemorySessionScope(scopeId)) return { recorded: false, reason: "exact_group_gcs_scope_required" };
  const kind = String(input.kind || "");
  if (!["selector", "write", "consumption"].includes(kind)) return { recorded: false, reason: "trend_contribution_kind_invalid" };
  const eventKey = String(input.eventKey || input.event_key || "");
  if (!eventKey) return { recorded: false, reason: "trend_contribution_key_required" };
  const nowMs = Number(options.nowMs || options.now_ms || Date.now());
  ...
  if (!Number.isFinite(recordedMs) || recordedMs > nowMs + 86_400_000) return { recorded: false, reason: "trend_contribution_time_invalid" };
  const bucketDate = groupTypedMemoryShapeTrendDate(recordedMs);
  const mutableCutoffDate = groupTypedMemoryShapeTrendDate(nowMs - (GROUP_TYPED_MEMORY_SHAPE_TREND_MUTABLE_DAYS - 1) * 86_400_000);
  const retentionCutoffDate = groupTypedMemoryShapeTrendDate(nowMs - (GROUP_TYPED_MEMORY_SHAPE_TREND_RETENTION_DAYS - 1) * 86_400_000);
  if (!bucketDate || bucketDate < retentionCutoffDate) return { recorded: false, reason: "trend_contribution_outside_retention", bucketDate };
  if (bucketDate < mutableCutoffDate) return { recorded: false, reason: "trend_bucket_sealed", bucketDate };
  const metrics = normalizeGroupTypedMemoryShapeTrendContribution(kind, input);
  const contributionKey = checksum([kind, eventKey], 32);
  const file = getGroupTypedMemoryShapeTrendFile(scopeId);
  const contribution: any = withFileLock(file, () => {
    const current = readJsonWithBackup<any>(file, null);
    if (!current && (fs.existsSync(file) || fs.existsSync(`${file}.bak`))) throw new Error("typed_memory_shape_trend_ledger_unrecoverable");
    if (current && verifyGroupTypedMemoryShapeTrendLedger(current, scopeId).valid !== true) throw new Error("typed_memory_shape_trend_ledger_invalid");
    const ledger: any = current || emptyGroupTypedMemoryShapeTrendLedger(scopeId);
    let bucket = (ledger.buckets || []).find((row: any) => row.date === bucketDate);
    if (bucket?.sealed === true) return { recorded: false, reason: "trend_bucket_sealed", bucketDate, ledgerFile: file };
    if (!bucket) {
      bucket = emptyGroupTypedMemoryShapeTrendBucket(bucketDate);
      ledger.buckets.push(bucket);
    }
    if ((bucket.contributionKeys || []).includes(contributionKey)) return { recorded: false, idempotent: true, reason: "already_recorded", bucketDate, ledgerFile: file };
    if ((bucket.contributionKeys || []).length >= GROUP_TYPED_MEMORY_SHAPE_TREND_MAX_KEYS_PER_BUCKET) {
    ...
    if (!groupTypedMemoryShapeTrendMetricsValid(bucket)) throw new Error("typed_memory_shape_trend_contribution_invalid");
    bucket.contributionKeys.push(contributionKey);
    bucket.updatedAt = new Date(nowMs).toISOString();
    ledger.buckets = (ledger.buckets || [])
      .filter((row: any) => String(row.date || "") >= retentionCutoffDate)
    ...
    if (!verifyGroupTypedMemoryShapeTrendLedger(ledger, scopeId).valid) throw new Error("typed_memory_shape_trend_ledger_write_invalid");
    writeJsonAtomicWithBackup(file, ledger);
...
function summarizeGroupTypedMemoryShapeTrendBuckets(buckets: any[]) {
  const selector = buckets.reduce((acc, bucket) => {
    for (const key of Object.keys(acc)) acc[key] += Number(bucket.selector?.[key] || 0);
    return acc;
  ...
  const write = buckets.reduce((acc, bucket) => {
    for (const key of ["eventCount", "changedCount", "createCount", "updateCount", "noopCount", "growthBytes", "afterBytesTotal", "nearBodyLimitCount", "truncatedBodyCount"]) acc[key] += Number(bucket.write?.[key] || 0);
    acc.maxAfterBytes = Math.max(acc.maxAfterBytes, Number(bucket.write?.maxAfterBytes || 0));
    return acc;
  }, { eventCount: 0, changedCount: 0, createCount: 0, updateCount: 0, noopCount: 0, growthBytes: 0, afterBytesTotal: 0, maxAfterBytes: 0, nearBodyLimitCount: 0, truncatedBodyCount: 0 } as any);
  const consumption = buckets.reduce((acc, bucket) => {
    for (const key of Object.keys(acc)) acc[key] += Number(bucket.consumption?.[key] || 0);
    return acc;
...
export function verifyGroupTypedMemoryShapeTrendSummary(summary: any, expectedScopeId = "") {
...
export function summarizeGroupTypedMemoryShapeTrend(scopeId: string, options: any = {}) {
  ...
  const buckets = loaded.valid === true && Array.isArray(loaded.buckets)
    ? loaded.buckets.filter((row: any) => String(row?.date || "") <= currentDate)
    : [];
  const recentBuckets = buckets.filter((row: any) => row.date >= recentStartDate);
  const baselineBuckets = buckets.filter((row: any) => row.date >= baselineStartDate && row.date < recentStartDate);
  ...
  const signals: any[] = [];
  const signal = (code: string, severity: string, delta: number | null) => signals.push({ code, severity, delta });
  if (selectorSufficient) {
    if (deltas.selectionRate !== null && Math.abs(deltas.selectionRate) >= 0.2) signal("selection_rate_shift", "info", deltas.selectionRate);
    if (deltas.emptySelectionRate !== null && deltas.emptySelectionRate >= 0.25) signal("empty_selection_rise", "warning", deltas.emptySelectionRate);
    if (deltas.averageSelectedAgeDays !== null && deltas.averageSelectedAgeDays >= 7) signal("selected_age_rise", "warning", deltas.averageSelectedAgeDays);
    if (deltas.selectedStaleRate !== null && deltas.selectedStaleRate >= 0.25) signal("stale_selection_rise", "warning", deltas.selectedStaleRate);
  }
  if (consumptionSufficient) {
    if (deltas.receiptCoverageRate !== null && deltas.receiptCoverageRate <= -0.2) signal("receipt_coverage_drop", "warning", deltas.receiptCoverageRate);
    if (deltas.consumedUtilityRate !== null && deltas.consumedUtilityRate <= -0.2) signal("consumed_utility_drop", "warning", deltas.consumedUtilityRate);
  }
  if (writeSufficient) {
    if (deltas.averageWriteBytes !== null && deltas.averageWriteBytes >= 4_000) signal("write_size_growth", "warning", deltas.averageWriteBytes);
    if (recent.nearBodyLimitWriteCount / Math.max(1, recent.writeEventCount) >= 0.5) signal("write_capacity_pressure", "warning", null);
  }
  if (recent.truncatedBodyWriteCount > 0) signal("write_body_truncated", "warning", null);
  ...
  const core = {
    ...
    mutableBucketCount: buckets.filter((row: any) => row.sealed !== true).length,
    sealedBucketCount: buckets.filter((row: any) => row.sealed === true).length,
    oldestDate,
...
function groupTypedMemoryShapeTrendIncidentEventChecksum(value: any) {
  const payload = { ...(value || {}) };
  delete payload.checksum;
  return checksum(JSON.stringify(payload), 64);
}

function groupTypedMemoryShapeTrendIncidentLedgerChecksum(value: any) {
  const payload = { ...(value || {}) };
  delete payload.checksum;
  delete payload.file;
  delete payload.present;
...
function groupTypedMemoryShapeTrendIncidentSummaryChecksum(value: any) {
  const payload = { ...(value || {}) };
  delete payload.checksum;
  return checksum(JSON.stringify(payload), 64);
}

function groupTypedMemoryShapeTrendSignalState(trend: any) {
  const signals = (Array.isArray(trend?.signals) ? trend.signals : [])
    .map((row: any) => ({ code: String(row?.code || ""), severity: String(row?.severity || "") }))
    .filter((row: any) => row.code && ["info", "warning"].includes(row.severity))
    .sort((a: any, b: any) => a.code.localeCompare(b.code) || a.severity.localeCompare(b.severity));
  ...
  return {
    ...
    signalCodes: signals.map((row: any) => row.code),
    warningSignalCount: signals.filter((row: any) => row.severity === "warning").length,
  };
}

function emptyGroupTypedMemoryShapeTrendIncidentLedger(scopeId: string) {
  const core = {
    schema: "ccm-group-typed-memory-shape-trend-incident-ledger-v1",
    version: GROUP_TYPED_MEMORY_SHAPE_TREND_INCIDENT_VERSION,
    scopeId,
    bodyFree: true,
    advisoryOnly: true,
    visibilityOnly: true,
    memoryMutationAuthorized: false,
    maxEvents: GROUP_TYPED_MEMORY_SHAPE_TREND_INCIDENT_MAX_EVENTS,
    retentionDays: GROUP_TYPED_MEMORY_SHAPE_TREND_INCIDENT_RETENTION_DAYS,
    generation: 0,
    headEventChecksum: "",
    events: [] as any[],
    updatedAt: "",
...
function replayGroupTypedMemoryShapeTrendIncidentEvents(events: any[], expectedScopeId = "") {
  ...
  for (const event of events) {
    ...
    const eventValid = !!event
      ...
      && Number(event.version || 0) === GROUP_TYPED_MEMORY_SHAPE_TREND_INCIDENT_VERSION
    ...
    if (!eventValid) chainValid = false;
    ...
    if (type === "opened") {
      const openValid = !activeIncident
        ...
        && event.signalCodes.every((code: any) => typeof code === "string" && !!code)
      ...
      if (!openValid) semanticsValid = false;
    ...
    if (type === "acknowledged") {
      if (!targetValid || incident.acknowledgement || String(event.actor || "").length > 100 || !/^[a-f0-9]{32}$/.test(String(event.noteChecksum || ""))) semanticsValid = false;
      else incident.acknowledgement = event;
    } else {
      if (!targetValid || activeIncident !== incident) semanticsValid = false;
      else {
...
export function verifyGroupTypedMemoryShapeTrendIncidentLedger(ledger: any, expectedScopeId = "") {
  ...
  const valid = !!ledger
    ...
    && Number(ledger.version || 0) === GROUP_TYPED_MEMORY_SHAPE_TREND_INCIDENT_VERSION
    && scopeValid
    && ledger.bodyFree === true
    && ledger.advisoryOnly === true
    && ledger.visibilityOnly === true
    && ledger.memoryMutationAuthorized === false
    && Number(ledger.maxEvents || 0) === GROUP_TYPED_MEMORY_SHAPE_TREND_INCIDENT_MAX_EVENTS
    && Number(ledger.retentionDays || 0) === GROUP_TYPED_MEMORY_SHAPE_TREND_INCIDENT_RETENTION_DAYS
    && events.length <= GROUP_TYPED_MEMORY_SHAPE_TREND_INCIDENT_MAX_EVENTS
...
export function readGroupTypedMemoryShapeTrendIncidentLedger(scopeId: string) {
  const file = getGroupTypedMemoryShapeTrendIncidentFile(scopeId);
  let primary: any = null;
  let backup: any = null;
  try { primary = JSON.parse(fs.readFileSync(file, "utf-8")); } catch {}
  if (verifyGroupTypedMemoryShapeTrendIncidentLedger(primary, scopeId).valid) return { ...primary, file, present: true, valid: true, primaryValid: true, recoveredFromBackup: false };
  try { backup = JSON.parse(fs.readFileSync(`${file}.bak`, "utf-8")); } catch {}
  if (verifyGroupTypedMemoryShapeTrendIncidentLedger(backup, scopeId).valid) return { ...backup, file, present: true, valid: true, primaryValid: false, recoveredFromBackup: true };
  const present = fs.existsSync(file) || fs.existsSync(`${file}.bak`);
  if (present) return { ...(primary || backup || {}), file, present: true, valid: false, primaryValid: false, recoveredFromBackup: false };
...
function appendGroupTypedMemoryShapeTrendIncidentEvent(ledger: any, scopeId: string, type: string, input: any = {}) {
...
function compactGroupTypedMemoryShapeTrendIncidentEvents(events: any[], nowMs: number) {
  const replay = replayGroupTypedMemoryShapeTrendIncidentEvents(events);
  if (!replay.valid) return events;
  const cutoffMs = nowMs - GROUP_TYPED_MEMORY_SHAPE_TREND_INCIDENT_RETENTION_DAYS * 86_400_000;
  const removable = new Set<string>();
  for (const incident of replay.incidents) {
    const resolvedMs = Date.parse(String(incident.resolved?.at || ""));
    if (incident.resolved && Number.isFinite(resolvedMs) && resolvedMs < cutoffMs) removable.add(String(incident.opened.incidentId || ""));
  }
  let kept = events.filter(event => !removable.has(String(event.incidentId || "")));
  while (kept.length > GROUP_TYPED_MEMORY_SHAPE_TREND_INCIDENT_MAX_EVENTS) {
    const nextReplay = replayGroupTypedMemoryShapeTrendIncidentEvents(kept);
    const oldestResolved = nextReplay.incidents.find((incident: any) => !!incident.resolved);
  ...
  let previousEventChecksum = "";
  const openedChecksums = new Map<string, string>();
  for (const event of kept) {
    if (event.type !== "opened") event.targetIncidentChecksum = String(openedChecksums.get(String(event.incidentId || "")) || event.targetIncidentChecksum || "");
    event.previousEventChecksum = previousEventChecksum;
    event.checksum = groupTypedMemoryShapeTrendIncidentEventChecksum(event);
    if (event.type === "opened") openedChecksums.set(String(event.incidentId || ""), event.checksum);
    previousEventChecksum = event.checksum;
  }
  return kept;
}

function commitGroupTypedMemoryShapeTrendIncidentLedger(file: string, ledger: any, scopeId: string, nowMs: number) {
  ledger.events = compactGroupTypedMemoryShapeTrendIncidentEvents(ledger.events || [], nowMs);
  if (ledger.events.length > GROUP_TYPED_MEMORY_SHAPE_TREND_INCIDENT_MAX_EVENTS) throw new Error("typed_memory_shape_trend_incident_capacity_exceeded");
  ledger.generation = Number(ledger.generation || 0) + 1;
  ledger.updatedAt = new Date(nowMs).toISOString();
  ...
  if (!verifyGroupTypedMemoryShapeTrendIncidentLedger(ledger, scopeId).valid) throw new Error("typed_memory_shape_trend_incident_ledger_write_invalid");
  writeJsonAtomicWithBackup(file, ledger);
  return ledger;
}

export function syncGroupTypedMemoryShapeTrendIncident(scopeId: string, trendInput: any = null, options: any = {}) {
  if (!isExactGroupTypedMemorySessionScope(scopeId)) return { changed: false, reason: "exact_group_gcs_scope_required" };
  const nowMs = Number(options.nowMs || options.now_ms || Date.now());
  ...
  if (trend.valid !== true || !verifyGroupTypedMemoryShapeTrendSummary(trend, scopeId).valid) return { changed: false, reason: "shape_trend_invalid" };
  const signalState = groupTypedMemoryShapeTrendSignalState(trend);
  const file = getGroupTypedMemoryShapeTrendIncidentFile(scopeId);
  return withFileLock(file, () => {
    const current = readJsonWithBackup<any>(file, null);
    if (!current && (fs.existsSync(file) || fs.existsSync(`${file}.bak`))) throw new Error("typed_memory_shape_trend_incident_ledger_unrecoverable");
    if (current && !verifyGroupTypedMemoryShapeTrendIncidentLedger(current, scopeId).valid) throw new Error("typed_memory_shape_trend_incident_ledger_invalid");
    const ledger: any = current || emptyGroupTypedMemoryShapeTrendIncidentLedger(scopeId);
...
export function verifyGroupTypedMemoryShapeTrendIncidentSummary(summary: any, expectedScopeId = "") {
  ...
  const valid = !!summary
    ...
    && Number(summary.version || 0) === GROUP_TYPED_MEMORY_SHAPE_TREND_INCIDENT_VERSION
    && scopeValid
    && summary.bodyFree === true
    && summary.advisoryOnly === true
...
export function summarizeGroupTypedMemoryShapeTrendIncidents(scopeId: string, options: any = {}) {
  ...
  const core = {
    ...
    acknowledgedCount: incidents.filter((incident: any) => !!incident.acknowledgement).length,
    resolvedCount: incidents.filter((incident: any) => !!incident.resolved).length,
...
export function acknowledgeGroupTypedMemoryShapeTrendIncident(scopeId: string, input: any = {}) {
  if (!isExactGroupTypedMemorySessionScope(scopeId)) return { acknowledged: false, reason: "exact_group_gcs_scope_required" };
  if (input.explicitConfirmation !== true && input.explicit_confirmation !== true) return { acknowledged: false, reason: "explicit_confirmation_required" };
  ...
  return withFileLock(trendFile, () => {
    const trend = summarizeGroupTypedMemoryShapeTrend(scopeId, { nowMs });
    if (trend.valid !== true || !verifyGroupTypedMemoryShapeTrendSummary(trend, scopeId).valid) return { acknowledged: false, reason: "shape_trend_invalid" };
    syncGroupTypedMemoryShapeTrendIncident(scopeId, trend, { nowMs });
    const file = getGroupTypedMemoryShapeTrendIncidentFile(scopeId);
    return withFileLock(file, () => {
      const current = readJsonWithBackup<any>(file, null);
      if (!current && !fs.existsSync(file) && !fs.existsSync(`${file}.bak`)) return { acknowledged: false, reason: "no_active_shape_trend_incident" };
      if (!current) throw new Error("typed_memory_shape_trend_incident_ledger_unrecoverable");
      if (!verifyGroupTypedMemoryShapeTrendIncidentLedger(current, scopeId).valid) throw new Error("typed_memory_shape_trend_incident_ledger_invalid");
      const replay = replayGroupTypedMemoryShapeTrendIncidentEvents(current.events || [], scopeId);
      const active = replay.activeIncident;
      if (!active) return { acknowledged: false, reason: "no_active_shape_trend_incident" };
      if (expectedIncidentId && expectedIncidentId !== String(active.opened.incidentId || "")) return { acknowledged: false, reason: "shape_trend_incident_changed" };
      if (expectedIncidentChecksum && expectedIncidentChecksum !== String(active.opened.checksum || "")) return { acknowledged: false, reason: "shape_trend_incident_changed" };
      if (active.acknowledgement) return { acknowledged: true, idempotent: true, event: active.acknowledgement, summary: summarizeGroupTypedMemoryShapeTrendIncidents(scopeId) };
      const signalState = groupTypedMemoryShapeTrendSignalState(trend);
      if (!signalState.actionable || signalState.signalFingerprint !== String(active.opened.signalFingerprint || "")) return { acknowledged: false, reason: "shape_trend_incident_changed" };
...
export function groupTypedMemoryManifestSelectorShapeChecksum(value: any) {
  const payload = { ...(value || {}) };
  delete payload.checksum;
  delete payload.shapeFile;
  delete payload.valid;
...
function groupTypedMemoryShapeDriftChecksum(value: any) {
  const payload = { ...(value || {}) };
  delete payload.checksum;
  delete payload.valid;
  return checksum(JSON.stringify(payload), 64);
...
function summarizeGroupTypedMemoryShapeWindow(shapeRows: any[], writeRows: any[]) {
  ...
  return {
    ...
    candidateCapacityRunCount: shapeRows.filter(row => Number(row.candidateCount || 0) >= GROUP_TYPED_MEMORY_MANIFEST_MAX_FILES).length,
    selectionCapacityRunCount: shapeRows.filter(row => Number(row.selectedCount || 0) >= GROUP_TYPED_MEMORY_MANIFEST_MAX_SELECTION).length,
...
function groupTypedMemoryShapeMetricDelta(recent: any, baseline: any, key: string) {
...
export function verifyGroupTypedMemoryShapeDrift(report: any, expectedScopeId = "") {
...
export function buildGroupTypedMemoryShapeDrift(scopeId: string, shapeRows: any[], writeRows: any[], options: any = {}) {
  const nowMs = Number(options.nowMs || options.now_ms || Date.now());
  const recentWindowDays = Math.max(1, Math.min(30, Number(options.recentWindowDays || options.recent_window_days || 7)));
  const baselineWindowDays = Math.max(recentWindowDays, Math.min(90, Number(options.baselineWindowDays || options.baseline_window_days || 21)));
  ...
  const inWindow = (row: any, startMs: number, endMs: number) => {
  ...
  const signals: any[] = [];
  const signal = (code: string, severity: string, metric: string, delta: number | null, reason: string) => signals.push({ code, severity, metric, delta, reason });
  if (selectorConfidenceSufficient) {
    if (deltas.selectionRate !== null && Math.abs(deltas.selectionRate) >= 0.2) signal("selection_rate_shift", "info", "selectionRate", deltas.selectionRate, "selection rate changed by at least 20 percentage points");
    if (deltas.emptySelectionRate !== null && deltas.emptySelectionRate >= 0.25) signal("empty_selection_rise", "warning", "emptySelectionRate", deltas.emptySelectionRate, "empty selector runs rose by at least 25 percentage points");
    if (deltas.averageSelectedAgeDays !== null && deltas.averageSelectedAgeDays >= 7) signal("selected_age_rise", "warning", "averageSelectedAgeDays", deltas.averageSelectedAgeDays, "selected memory age rose by at least seven days");
    if (deltas.selectedStaleRate !== null && deltas.selectedStaleRate >= 0.25) signal("stale_selection_rise", "warning", "selectedStaleRate", deltas.selectedStaleRate, "stale selected-memory share rose by at least 25 percentage points");
    if (recent.candidateCapacityRunCount / Math.max(1, recent.selectorRunCount) >= 0.5) signal("candidate_capacity_pressure", "warning", "candidateCapacityRunCount", null, "at least half of recent selector runs reached the 200-header cap");
    if (recent.selectionCapacityRunCount / Math.max(1, recent.selectorRunCount) >= 0.5) signal("selection_capacity_pressure", "info", "selectionCapacityRunCount", null, "at least half of recent selector runs filled all five selection slots");
  }
  if (consumptionConfidenceSufficient) {
    if (deltas.receiptCoverageRate !== null && deltas.receiptCoverageRate <= -0.2) signal("receipt_coverage_drop", "warning", "receiptCoverageRate", deltas.receiptCoverageRate, "structured receipt coverage fell by at least 20 percentage points");
    if (deltas.consumedUtilityRate !== null && deltas.consumedUtilityRate <= -0.2) signal("consumed_utility_drop", "warning", "consumedUtilityRate", deltas.consumedUtilityRate, "observed consumed utility fell by at least 20 percentage points");
  }
  if (writeConfidenceSufficient) {
    if (deltas.averageWriteBytes !== null && deltas.averageWriteBytes >= 4_000) signal("write_size_growth", "warning", "averageWriteBytes", deltas.averageWriteBytes, "average written memory size grew by at least 4000 bytes");
    if (recent.nearBodyLimitWriteCount / Math.max(1, recent.writeEventCount) >= 0.5) signal("write_capacity_pressure", "warning", "nearBodyLimitWriteCount", null, "at least half of recent writes approached the configured body limit");
  }
  if (recent.truncatedBodyWriteCount > 0) signal("write_body_truncated", "warning", "truncatedBodyWriteCount", null, "one or more recent typed-memory writes exceeded the configured body limit");
  ...
  const core = {
    ...
    maxShapeRows: 200,
    maxWriteRows: GROUP_TYPED_MEMORY_WRITE_SHAPE_MAX_EVENTS,
    confidence: {
...
export function runGroupTypedMemoryDistillationMutationCoordinatorSelfTest() {
  return require("./group-memory-distillation").runGroupTypedMemoryDistillationMutationCoordinatorSelfTest();
}

export function runGroupTypedMemoryIndexSelfTest() {
  return require("./group-memory-recall-self-tests").runGroupTypedMemoryIndexSelfTest();
}

export function runGroupTypedMemoryPostCompactUsageScoringSelfTest() {
  return require("./group-memory-recall-self-tests").runGroupTypedMemoryPostCompactUsageScoringSelfTest();
}

export function runGroupTypedMemoryWorkerContextPressureRecallSelfTest() {
  return require("./group-memory-recall-self-tests").runGroupTypedMemoryWorkerContextPressureRecallSelfTest();
}

export function runGroupTypedMemoryCrossGroupPressureRecallUsageSelfTest() {
  return require("./group-memory-recall-self-tests").runGroupTypedMemoryCrossGroupPressureRecallUsageSelfTest();
}

export function runGroupTypedMemoryPressureRecallUsageRepairProvenanceSelfTest() {
  return require("./group-memory-recall-self-tests").runGroupTypedMemoryPressureRecallUsageRepairProvenanceSelfTest();
}

export function runGroupTypedMemoryLoadPlanSelfTest() {
  return require("./group-memory-recall-self-tests").runGroupTypedMemoryLoadPlanSelfTest();
}

export function runGroupTypedMemoryPathConditionSelfTest() {
  return require("./group-memory-recall-self-tests").runGroupTypedMemoryPathConditionSelfTest();
}

export function runGroupProjectMemoryImportSelfTest() {
  return require("./group-memory-recall-self-tests").runGroupProjectMemoryImportSelfTest();
}

export function runGroupGlobalClaudeMemoryImportSelfTest() {
  return require("./group-memory-recall-self-tests").runGroupGlobalClaudeMemoryImportSelfTest();
}

export function runGroupClaudeMemoryExternalIncludeApprovalSelfTest() {
  return require("./group-memory-distillation-self-tests").runGroupClaudeMemoryExternalIncludeApprovalSelfTest();
}

export function runGroupClaudeMemorySettingSourcePolicySelfTest() {
  return require("./group-memory-distillation-self-tests").runGroupClaudeMemorySettingSourcePolicySelfTest();
}

export function runGroupInstructionsLoadedHookPipelineSelfTest() {
  return require("./group-memory-distillation-self-tests").runGroupInstructionsLoadedHookPipelineSelfTest();
}

export function runGroupTypedMemoryLogDistillationSelfTest() {
  return require("./group-memory-distillation").runGroupTypedMemoryLogDistillationSelfTest();
}

export function runGroupTypedMemoryPostCompactUsageDistillationSelfTest() {
  return require("./group-memory-distillation").runGroupTypedMemoryPostCompactUsageDistillationSelfTest();
}

export function runGroupTypedMemoryProviderReproofReceiptConsumptionDistillationSelfTest() {
  return require("./group-memory-distillation").runGroupTypedMemoryProviderReproofReceiptConsumptionDistillationSelfTest();
}

export function runGroupTypedMemoryProviderRankingProvenanceCompactRepairReceiptConsumptionDistillationSelfTest() {
  return require("./group-memory-distillation").runGroupTypedMemoryProviderRankingProvenanceCompactRepairReceiptConsumptionDistillationSelfTest();
}

export function runGroupTypedMemoryPostCompactReinjectionRepairReceiptConsumptionDistillationSelfTest() {
  return require("./group-memory-distillation").runGroupTypedMemoryPostCompactReinjectionRepairReceiptConsumptionDistillationSelfTest();
}

export function runGroupTypedMemoryPostCompactCompletionMemoryPreservationRepairClosureDistillationSelfTest() {
  return require("./group-memory-distillation").runGroupTypedMemoryPostCompactCompletionMemoryPreservationRepairClosureDistillationSelfTest();
}

export function runGroupTypedMemoryDistillationQualitySelfTest() {
  return require("./group-memory-distillation").runGroupTypedMemoryDistillationQualitySelfTest();
}
