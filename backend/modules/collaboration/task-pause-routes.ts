...
import { hasTaskResourceAccess } from "../system/access-policy";
import {
  TASK_PAUSE_STUCK_MS,
...
function taskIdFrom(value: any) {
  return String(value?.task_id || value?.taskId || value?.id || "").trim();
}

function readJsonRequest(req: any) {
...
function descendantsOf(task: any, tasks: any[]) {
  ...
  while (queue.length) {
    ...
    if (Array.isArray(current?.child_task_ids)) {
      queue.push(...current.child_task_ids.map((childId: any) => byId.get(String(childId))).filter(Boolean));
    }
  }
  return rows;
}

function terminal(task: any) {
  return ["done", "failed", "cancelled", "canceled", "reverted", "archived"]
    .includes(String(task?.status || "").toLowerCase());
}

function writerCount(taskId: string, deps: any) {
  return Math.max(0, Number(deps.listActiveAgentRuns({ taskId })?.length || 0));
}

function mutationConflict(task: any, payload: any, requireTarget = false) {
  const guard = validateTaskMutationGuard(task, payload, { requireTarget });
  return "error" in guard ? guard : null;
}

function appendPauseVisibleEvent(task: any, kind: "requested" | "paused" | "resumed", deps: any) {
...
function markRequested(task: any, deps: any) {
  const activeWriters = writerCount(String(task.id || ""), deps);
  const sessions = deps.listTaskAgentSessions({ taskId: String(task.id || "") });
  const control = createTaskPauseRequest(task, {
    pendingWriterCount: activeWriters,
    suspendedSessionCount: sessions.filter((item: any) => item.status === "suspended").length,
  });
  try { deps.requestActiveAgentRunPause?.({ taskId: String(task.id || ""), reason: "任务请求在最近安全检查点暂停" }); } catch {}
  ...
  const timer = setTimeout(() => {
    const current = deps.loadTasks().find((item: any) => String(item?.id || "") === String(task.id || ""));
    if (!current || !isTaskPauseRequested(current)) return;
...
export function finalizeTaskPauseAtSafeBoundary(task: any, deps: any, options: { allowRuntimeHolder?: boolean } = {}) {
  if (isTaskSafelyPaused(task) || !isTaskPauseRequested(task)) return task;
  const activeWriters = writerCount(String(task.id || ""), deps);
  ...
  if (activeWriters > 0 || (runtimeHeld && options.allowRuntimeHolder !== true)) {
    const control = updateTaskPauseProgress(task, { state: "quiescing", pendingWriterCount: activeWriters + (runtimeHeld ? 1 : 0) });
  ...
  const control = updateTaskPauseProgress(task, {
    state: "paused",
    pendingWriterCount: 0,
    suspendedSessionCount: suspended.length,
    workspaceChecksum: taskPauseWorkspaceChecksum(task),
  });
  const updated = deps.updateTask(task.id, {
    status: "paused",
    auto_execute: false,
  ...
  for (const execution of deps.listExecutions({ taskId: String(task.id || "") })) {
    if (["succeeded", "failed", "cancelled", "paused"].includes(String(execution?.state || ""))) continue;
...
function reconcileTree(task: any, deps: any) {
  const tasks = deps.loadTasks();
  const rows = descendantsOf(task, tasks).filter((item: any) => !terminal(item));
  const children = rows.slice(1).map((item: any) => finalizeTaskPauseAtSafeBoundary(item, deps));
  const freshRoot = deps.loadTasks().find((item: any) => String(item.id) === String(task.id)) || task;
  const childrenSafe = children.every((item: any) => isTaskSafelyPaused(item) || terminal(item));
...
export const reconcileTaskPauseTree = reconcileTree;

export function requestTaskPauseTree(task: any, deps: any) {
  const tree = descendantsOf(task, deps.loadTasks()).filter((item: any) => !terminal(item));
  for (const row of tree.slice().reverse()) markRequested(row, deps);
  const freshRoot = deps.loadTasks().find((item: any) => String(item?.id || "") === String(task?.id || "")) || task;
  return reconcileTree(freshRoot, deps);
}

export async function resumeTaskPauseTree(task: any, ctx: any, deps: any) {
  const tree = descendantsOf(task, deps.loadTasks()).filter((item: any) => !terminal(item));
  const resumable = tree.slice().reverse();
  const validations = resumable.map((row: any) => {
    ...
    const validationRow = String(row?.pause_control?.state || "") === "blocked"
      ? { ...row, pause_control: updateTaskPauseProgress(row, { state: "paused", pendingWriterCount: 0 }) }
      : row;
    const sessions = deps.listTaskAgentSessions({ taskId: String(row.id || "") });
    const runtimeValid = sessions.every((session: any) => session.resumeMode !== "native" || !!session.nativeSessionId);
    ...
    return {
      row: validationRow,
      validation: validateTaskPauseResume(validationRow, {
        currentWorkspaceChecksum: taskPauseWorkspaceChecksum(validationRow),
        authorizationValid,
        runtimeValid,
  ...
  if (rejected) {
    const blocked = updateTaskPauseProgress(rejected.row, { state: "blocked", blockedReason: rejected.validation.reason });
    deps.updateTask(rejected.row.id, { pause_control: blocked, status_detail: `无法继续：${rejected.validation.reason}` });
    const error: any = new Error(rejected.validation.reason);
    error.code = "TASK_PAUSE_RESUME_BLOCKED";
    error.status = 409;
    error.checks = rejected.validation.checks;
    throw error;
  }
  const resumedRows: any[] = [];
  for (const { row } of validations) {
    const resumedAt = new Date().toISOString();
    const control = createTaskResumeControl(row, resumedAt);
    ...
    for (const execution of deps.listExecutions({ taskId: String(row.id || "") })) {
      if (String(execution?.state || "") !== "paused") continue;
  ...
  for (const row of resumedRows) {
    if (row.auto_execute === false || String(row.workflow_type || "") === "global_mission") continue;
    deps.enqueueTask(String(row.id || ""), ctx);
  }
  const root = deps.loadTasks().find((item: any) => String(item?.id || "") === String(task?.id || "")) || resumedRows[resumedRows.length - 1] || task;
  await ctx.onTaskStatusChange?.(root, "resumed", root.status_detail);
  return { root, resumedRows };
}

export function handleTaskPauseRoutes(req: any, res: any, parsed: any, ctx: any, deps: any) {
  ...
  if (pathname === "/api/tasks/pause/status" && req.method === "GET") {
    ...
    const task = deps.loadTasks().find((item: any) => String(item?.id || "") === taskId);
    if (!task) { deps.sendJson(res, { success: false, error: "任务不存在" }, 404); return true; }
    if (!hasTaskResourceAccess(task, req.ccmAuth, "use")) { deps.sendJson(res, { success: false, error: "当前账户没有该任务的访问权限", code: "RESOURCE_ACCESS_DENIED" }, 403); return true; }
    const reconciled = reconcileTree(task, deps);
    const descendants = reconciled.rows.slice(1);
    res.setHeader("Cache-Control", "private, no-store");
    deps.sendJson(res, {
      success: true,
      status: taskPauseStatusProjection(reconciled.root, {
        activeWriterCount: writerCount(taskId, deps),
        descendantCount: descendants.length,
        childPausedCount: descendants.filter((item: any) => isTaskSafelyPaused(item) || terminal(item)).length,
      }),
    });
    return true;
  }
  if (!pausePaths.includes(pathname) || req.method !== "POST") return false;
  void readJsonRequest(req).then(async payload => {
    const taskId = taskIdFrom(payload);
    let task = deps.loadTasks().find((item: any) => String(item?.id || "") === taskId);
    if (!task) return deps.sendJson(res, { success: false, error: "任务不存在" }, 404);
    if (!hasTaskResourceAccess(task, req.ccmAuth, "manage")) return deps.sendJson(res, { success: false, error: "当前账户没有该任务的管理权限", code: "RESOURCE_ACCESS_DENIED" }, 403);
    const conflict = mutationConflict(task, payload, pathname === "/api/tasks/resume-paused");
    if (conflict) return deps.sendJson(res, { success: false, error: conflict.error, code: conflict.code, ...conflict.details }, conflict.status);
    ...
    if (pathname === "/api/tasks/pause/preview") {
      ...
      return deps.sendJson(res, {
        ...
        preview: {
          ...
          activeWriterCount: tree.reduce((sum: number, item: any) => sum + writerCount(String(item.id || ""), deps), 0),
    ...
    if (pathname === "/api/tasks/pause") {
      ...
      return deps.sendJson(res, {
        ...
        status: taskPauseStatusProjection(reconciled.root, {
          ...
          childPausedCount: reconciled.rows.slice(1).filter((item: any) => isTaskSafelyPaused(item) || terminal(item)).length,
        }),
      });
    }
  ...
  }).catch((error: any) => deps.sendJson(res, { success: false, error: error?.message || String(error), checks: error?.checks || undefined }, Number(error?.status || (/冲突|版本|generation/i.test(String(error?.message || "")) ? 409 : 400))));
  return true;
}
