...
function compactPlanText(value: any, max = 220) {
...
export const PLAN_STEP_LIBRARY: Record<string, PlanStepText> = {
  // 通用理解与上下文
...
export function planStepText(id: string, overrides: Partial<PlanStepText> = {}): PlanStepText {
  const base = PLAN_STEP_LIBRARY[id] || { content: id, activeForm: `正在处理 ${id}` };
  return {
...
export function readTaskPlanMode(task: any) {
...
export function extractPlanModeWorkSteps(planMode: any) {
...
export function planWorkStepLiveStatus(phase: string, task: any) {
  if (["cancelled", "reverted"].includes(phase)) return "cancelled";
  if (task?.status === "failed" || phase === "blocked") return "needs_confirmation";
  if (phase === "completed") return "completed";
  if (phase === "reviewing") return "reviewing";
  if (phase === "reworking") return "reworking";
  if (["dispatching", "executing"].includes(phase)) return "in_progress";
  return "pending";
}
...
export interface PlanFollowupInput {
  message: string;
  kind?: string;      // supplement | revise_goal | intake_revise
  source?: string;
  at?: string;
  executing?: boolean; // true = 任务已在执行，不回退确认门槛；false = 仍在等待确认，需要重新确认
}

export function mergeFollowupIntoPlanMode(planMode: any, input: PlanFollowupInput): any {
  ...
  const followupIndex = existingSteps.filter((step: any) => /^followup_requirement_\d+$/.test(String(step?.id || ""))).length + 1;
  const followupStep = {
  ...
  const gateIndex = existingSteps.findIndex((step: any) => PLAN_GATE_STEP_IDS.includes(String(step?.id || "")));
...
export function buildPlanRevisionTaskUpdates(task: any, revisedPlan: any): any {
...
// 给用户看的修订说明（群消息 / 时间线用）。
export function summarizePlanRevisionForUser(revisedPlan: any, input: { executing?: boolean } = {}) {
...
export function runMainAgentPlanCoreSelfTest() {
  const basePlan = {
    title: "执行前计划",
    revision_count: 0,
    requires_confirmation: true,
  ...
  const followupStepAwaiting = awaiting.steps.find((step: any) => step.id === "followup_requirement_1");
  const followupStepInFlight = inFlight.steps.find((step: any) => step.id === "followup_requirement_2");
  const gatePosition = awaiting.steps.findIndex((step: any) => step.id === "confirm_boundary");
  const followupPosition = awaiting.steps.findIndex((step: any) => step.id === "followup_requirement_1");
