...
function collectProjectAnalysisFiles(root: string, maxEntries = 1200, maxDepth = 5) {
  const files: string[] = [];
  let visited = 0;
...
function truncateProjectSourceToTokens(value: string, maxTokens: number) {
...
export function buildProjectCodeReadOnlyEvidence(
  project: string,
  workDir: string,
  message: string,
  ...
  try {
    ...
    const files: GroupMainProjectSourceFileEvidence[] = [];
    let totalChars = 0;
    let truncated = false;
    ...
    if (ranked.length > files.length || totalChars >= maxChars) truncated = true;
    return {
      project,
      workDir: realRoot,
...
export function buildGroupMainPlanningSourceContext(
  group: any,
  message: string,
  configs: any[],
  options: { targetProjects?: string[]; maxProjects?: number } = {},
  ...
  const allowed = new Map<string, any>(members.map((member: any) => [String(member.project || ""), member] as [string, any]));
  const requested = Array.from(new Set((options.targetProjects || []).map(value => String(value || "").trim()).filter(value => allowed.has(value))));
  const selectedNames: string[] = (requested.length ? requested : Array.from(allowed.keys()))
    .slice(0, Math.max(1, Math.min(8, Number(options.maxProjects || 8))));
  const projects = selectedNames.map(project => {
    const runtime = resolveMemberRuntime(project, normalized, configs);
...
export type ModelDrivenSourcePlanningReceiptV1 = {
  ...
  requestedFiles: Array<{ project: string; path: string; reason: string }>;
  searchQueries: Array<{ project: string; query: string }>;
  tokenBudget: number;
  projectedTokens: number;
  checksum: string;
};

function sourcePlannerList(value: any, max: number) {
  return Array.isArray(value) ? value.slice(0, max) : [];
}

function safeSourcePlannerRelativePath(value: any) {
  return String(value || "").replace(/\\/g, "/").replace(/^\.\//, "").trim();
}

function readSourcePlannerFile(root: string, relativePath: string, maxTokens: number) {
  const safeRelative = safeSourcePlannerRelativePath(relativePath);
  const absolute = path.resolve(root, safeRelative);
  const relative = path.relative(root, absolute);
  if (!safeRelative || relative.startsWith("..") || path.isAbsolute(relative) || !isProjectAnalysisCandidate(absolute, root)) return null;
  try {
    const realAbsolute = fs.realpathSync(absolute);
    const realRelative = path.relative(root, realAbsolute);
    if (realRelative.startsWith("..") || path.isAbsolute(realRelative) || !isProjectAnalysisCandidate(realAbsolute, root)) return null;
    ...
    return {
      ...
      chars: content.length,
      content,
    } as GroupMainProjectSourceFileEvidence;
  } catch {
    return null;
  }
}

async function callSourcePlanningModel(config: any, payload: any) {
  const messages = [
    {
      role: "system",
  ...
  return shouldUseAnthropic(config)
    ? callAnthropicCompatibleJson(config, { messages, maxTokens: 1400, defaultTimeoutMs: config.timeoutMs || 120_000, httpErrorPrefix: "源码规划模型调用失败" })
    : callOpenAiCompatibleJson(config, { messages, defaultTimeoutMs: config.timeoutMs || 120_000, httpErrorPrefix: "源码规划模型调用失败" });
}

export async function buildModelDrivenGroupPlanningSourceContext(
  group: any,
  message: string,
  configs: any[],
  options: { targetProjects?: string[]; maxRounds?: number } = {},
): Promise<GroupMainPlanningSourceContext & { modelPlanning: ModelDrivenSourcePlanningReceiptV1 }> {
  ...
  const normalized = normalizeGroupOrchestrator(group);
  const allowedMembers = getRoutableMembers(normalized);
  const allowedNames = new Set<string>(allowedMembers.map((member: any) => String(member.project || "")).filter(Boolean));
  const requested = Array.from(new Set<string>((options.targetProjects || []).map(value => String(value || "").trim()).filter(value => allowedNames.has(value))));
  const projectNames: string[] = (requested.length ? requested : Array.from(allowedNames)).slice(0, 8);
  const projectState = projectNames.map(project => {
    ...
    try {
      ...
      return {
        ...
        files: new Map<string, GroupMainProjectSourceFileEvidence>(),
        issue: "",
      };
    } catch (error: any) {
      return { project, root: rawRoot, candidates: [] as string[], paths: [] as string[], files: new Map<string, GroupMainProjectSourceFileEvidence>(), issue: `项目工作目录不可读：${String(error?.message || error)}` };
    }
  });
  ...
  const requestedFiles: Array<{ project: string; path: string; reason: string }> = [];
  const searchQueries: Array<{ project: string; query: string }> = [];
  ...
  for (let round = 1; round <= maxRounds; round += 1) {
    ...
    const payload = {
      ...
      remaining_token_budget: Math.max(0, tokenBudget - estimateTextTokens(JSON.stringify(evidenceRows))),
    };
    if (estimateTextTokens(JSON.stringify(payload)) > tokenBudget) {
    ...
    for (const selection of selections) {
      ...
      if (!state || !state.paths.includes(relativePath) || state.files.has(relativePath)) continue;
      const remaining = Math.max(400, tokenBudget - projectState.reduce((sum, item) => sum + Array.from(item.files.values()).reduce((fileSum, file) => fileSum + estimateTextTokens(file.content), 0), 0));
    ...
    for (const queryRow of queries) {
      ...
      if (!state || query.length < 2) continue;
      searchQueries.push({ project: state.project, query: query.slice(0, 160) });
      const lowerQuery = query.toLowerCase();
      for (const candidate of state.candidates) {
        if (state.files.size >= 16) break;
        let matched = false;
        try {
          const stat = fs.statSync(candidate);
          if (stat.size <= 512_000) matched = fs.readFileSync(candidate, "utf-8").toLowerCase().includes(lowerQuery);
        } catch {}
        if (!matched) continue;
        const relativePath = path.relative(state.root, candidate).replace(/\\/g, "/");
        if (state.files.has(relativePath)) continue;
        const consumed = projectState.reduce((sum, item) => sum + Array.from(item.files.values()).reduce((fileSum, file) => fileSum + estimateTextTokens(file.content), 0), 0);
        ...
        if (file) state.files.set(relativePath, file);
      }
    }
    projectedTokens = projectState.reduce((sum, item) => sum + Array.from(item.files.values()).reduce((fileSum, file) => fileSum + estimateTextTokens(file.content), 0), 0);
    if (finalDecision?.sufficient === true && projectedTokens > 0) break;
  }
  const sufficient = finalDecision?.sufficient === true && projectedTokens > 0;
  const projects: GroupMainProjectSourceEvidence[] = projectState.map(state => {
  ...
  const receiptBase = {
    schema: "ccm-model-driven-source-planning-receipt-v1" as const,
    rounds: roundsUsed,
    sufficient,
  ...
  const modelPlanning: ModelDrivenSourcePlanningReceiptV1 = { ...receiptBase, checksum: crypto.createHash("sha256").update(JSON.stringify(receiptBase)).digest("hex") };
  const issues = [
...
export function buildProjectCodeReadOnlySnapshot(
  ...
  if (evidence.status === "unavailable") return `- 代码快照：读取失败，${deps.compactMemoryText(evidence.issue, 180)}。`;
  if (!evidence.files.length) return "- 代码快照：未找到适合只读分析的源码/配置文件。";
  return [
    "### 只读代码快照",
