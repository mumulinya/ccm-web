...
import {
  buildPlanModeWorkStepSpecs,
  extractPlanModeWorkSteps,
  planStepText,
  readTaskPlanMode,
...
export function hasStrongTaskAcceptanceEvidence(task: any, executions: any[] = [], explicitSummary: any = null) {
  ...
  const verificationFailures = uniqueStrings(summary.verification_failed, summary.verificationFailed);
  if (verificationRows.length > 0 && verificationFailures.length === 0) return true;
  ...
  if (acceptanceRows.some((row: any) => isPositiveAcceptanceEvidenceText(row) && !isBareAcceptanceMarker(row))) return true;
...
export function taskCardPhase(task: any, executions: any[]) {
  ...
  if (pauseState === "resuming") return "resuming";
  if (pauseState === "blocked") return "needs_user";
...
export function buildUserTaskActions(task: any, phase: string, executions: any[]) {
  ...
  const pauseStatus = taskPauseStatusProjection(task);
  if (["requested", "quiescing"].includes(pauseStatus.state)) {
    if (pauseStatus.stuck) actions.push({ id: "force_interrupt", label: "强制中断", kind: "force_interrupt", tone: "danger" });
    return actions;
  }
  ...
  if (!terminal) actions.push({ id: "pause", label: "暂停", kind: "pause", tone: "outline" });
  if (!completed && !stopped) actions.push({ id: "cancel", label: "停止任务", kind: "cancel", tone: "outline" });
  return actions;
}
...
export function buildTaskCardView(task: any, executions: any[], sessions: any[]) {
  ...
  const progressByPhase: any = { planning: 10, queued: 20, dispatching: 30, executing: 55, reworking: 65, reviewing: 85, needs_user: 70, change_review: 95, blocked: 60, pausing: 55, paused: 55, resuming: 55, completed: 100, cancelled: 0, reverted: 100 };
  ...
  const responsibleProjects = uniqueStrings([
    ...workItems.map((item: any) => item.target || item.owner),
    ...workers.map((item: any) => item.agent),
  ].filter((item: any) => item && !/^(coordinator|test-agent)$/i.test(String(item)))).slice(0, 6);
  ...
  else if (phase === "resuming") nextAction = "正在从原任务和原检查点继续";
  ...
  return {
    ...
    pause_status: taskPauseStatusProjection(task),
    pauseStatus: taskPauseStatusProjection(task),
    ...
    plan_mode: planMode ? {
      ...
      architecture_plan: planMode.architecture_plan ? {
        goal: compactMemoryText(planMode.architecture_plan.goal || "", 700),
        boundaries: Array.isArray(planMode.architecture_plan.boundaries) ? planMode.architecture_plan.boundaries.slice(0, 12).map((item: any) => compactMemoryText(item, 300)) : [],
        data_relationships: Array.isArray(planMode.architecture_plan.dataRelationships || planMode.architecture_plan.data_relationships)
          ? (planMode.architecture_plan.dataRelationships || planMode.architecture_plan.data_relationships).slice(0, 16).map((item: any) => compactMemoryText(item, 420))
          : [],
        dependency_steps: Array.isArray(planMode.architecture_plan.dependencySteps || planMode.architecture_plan.dependency_steps)
          ? (planMode.architecture_plan.dependencySteps || planMode.architecture_plan.dependency_steps).slice(0, 12).map((item: any) => ({
              id: item.id || "",
              title: compactMemoryText(item.title || "", 220),
              project: compactMemoryText(item.project || "", 120),
              depends_on: Array.isArray(item.dependsOn || item.depends_on) ? (item.dependsOn || item.depends_on).slice(0, 12) : [],
              acceptance: Array.isArray(item.acceptance) ? item.acceptance.slice(0, 8).map((value: any) => compactMemoryText(value, 300)) : [],
            }))
          : [],
      ...
      read_only_exploration: planMode.read_only_exploration ? {
        summary: compactMemoryText(planMode.read_only_exploration.summary || "", 520),
        ...
        source_evidence: Array.isArray(planMode.read_only_exploration.source_evidence)
          ? planMode.read_only_exploration.source_evidence.slice(0, 8).map((item: any) => ({
              project: item.project || "",
              status: item.status || "",
              selected_paths: Array.isArray(item.selected_paths) ? item.selected_paths.slice(0, 12) : [],
              issue: compactMemoryText(item.issue || "", 240),
            }))
          : [],
      } : null,
...
export function planAlignmentEvidenceLabels(summary: any = {}, task: any = {}) {
  const files = [
    ...(Array.isArray(summary.actual_file_changes) ? summary.actual_file_changes.map((item: any) => item?.path || item) : []),
    ...(Array.isArray(summary.file_changes) ? summary.file_changes.map((item: any) => item?.path || item) : []),
    ...(Array.isArray(task?.file_changes?.files) ? task.file_changes.files.map((item: any) => item?.path || item) : []),
  ].filter(Boolean);
  const verification = Array.isArray(summary.verification_executed) ? summary.verification_executed : [];
  ...
  const assignments = Array.isArray(summary.assignment_evidence) ? summary.assignment_evidence : [];
  return {
    files: uniqueStrings(files).slice(0, 8),
    verification: uniqueStrings(verification).slice(0, 8),
    receipts,
    assignments,
  };
}

export function planCriterionStatus(criterion: string, summary: any = {}, task: any = {}, acceptanceReview: any = null) {
  ...
  if (/文件|改动|diff|代码|修改|变更/i.test(text)) {
    const ok = !taskRequiresCodeChanges(task) || evidence.files.length > 0;
  ...
  if (/验证|测试|检查|test|check|lint|build/i.test(text)) {
    const ok = !taskRequiresVerification(task) || evidence.verification.length > 0;
  ...
  if (/回执|agent|子\s*Agent|工作单|派发|协作/i.test(text)) {
    const doneReceipts = evidence.receipts.filter((item: any) => String(item?.status || "") === "done");
    const ok = doneReceipts.length > 0 || evidence.assignments.length > 0;
    return { ok, evidence: doneReceipts.slice(0, 2).map((item: any) => item.summary || item.agent || item.project).filter(Boolean), detail: ok ? `已收集 ${doneReceipts.length || evidence.assignments.length} 条协作证据` : "还没有可核对的执行成员证据" };
  }
  return {
    ok: acceptancePassed,
...
export function buildUserPlanAlignmentReview(task: any, summary: any = {}, phase = "planning", planMode: any = null, workOrderPreview: any = null, acceptanceReview: any = null) {
  const plan = planMode || getTaskPlanMode(task);
  ...
  const workOrderAcceptance = Array.isArray(workOrderPreview?.orders)
    ? workOrderPreview.orders.flatMap((order: any) => Array.isArray(order.acceptance) ? order.acceptance : [])
    : [];
  ...
  const checks: any[] = [
    ...
    },
    {
      ...
      evidence: Array.isArray(workOrderPreview?.orders) ? workOrderPreview.orders.map((item: any) => item.project).filter(Boolean).slice(0, 4) : [],
    },
    ...criteria.map((criterion: string, index: number) => {
      const status = planCriterionStatus(criterion, summary, task, acceptanceReview);
      return {
        id: `criterion_${index + 1}`,
        label: compactMemoryText(criterion, 90),
        ok: status.ok,
  ...
  if (!criteria.length) {
    checks.push(
      {
        id: "code_changes",
        label: taskRequiresCodeChanges(task) ? "计划要求代码改动" : "计划允许无代码改动",
        ok: !taskRequiresCodeChanges(task) || evidence.files.length > 0,
        detail: taskRequiresCodeChanges(task) ? `捕获 ${evidence.files.length} 个文件改动` : "无需强制代码改动",
        evidence: evidence.files.slice(0, 3),
      },
      {
        id: "verification",
        label: taskRequiresVerification(task) ? "计划要求验证" : "计划允许说明性验证",
        ok: !taskRequiresVerification(task) || evidence.verification.length > 0,
        detail: taskRequiresVerification(task) ? `已执行 ${evidence.verification.length} 项验证` : "无需强制验证命令",
...
export function buildUserHandoffSummary(
  task: any,
  summary: any = {},
  phase = "planning",
  nextAction = "",
  ...
  const handoffText = (item: any) => {
    if (!item || typeof item !== "object") return String(item || "").trim();
    return compactMemoryText(item.label || item.reason || item.summary || item.detail || item.message || item.title || item.path || item.id || "", 220);
  };
  const riskItems = uniqueStrings([
    ...
    ...planProblems.map((item: any) => handoffText(item.reason || item.label)).filter(Boolean),
  ]).slice(0, 8);
  ...
  const addAction = (id: string, label: string, detail = "", kind = "", tone = "outline") => {
    ...
    actions.push({ id, label, detail: compactMemoryText(detail || label, 180), kind: kind || id, tone });
  };
  if (needsUser) addAction("provide_input", task?.intake_state === "awaiting_confirmation" ? "确认执行计划" : "补充所需信息", riskItems[0] || nextAction || "我正在等待你的确认。", task?.intake_state === "awaiting_confirmation" ? "confirm_plan" : "continue", "primary");
  if (failed || blocked || riskItems.length) addAction("continue_rework", failed ? "重新执行或继续修复" : "继续处理缺口", riskItems[0] || nextAction || "我会复用已有证据继续处理。", failed ? "retry" : "gap_continue", failed ? "primary" : "warning");
  if (fileCount > 0) addAction("view_changes", "查看改动", changeSummary?.headline || `已捕获 ${fileCount} 个文件改动。`, "view_changes", terminal && !riskItems.length ? "primary" : "outline");
  if (deliveryReport?.schema || summary.delivery_report) addAction("review_delivery", "核对交付总结", "查看完成内容、验证结果和风险提示。", "review_delivery", fileCount ? "outline" : "primary");
  if (terminal && !riskItems.length) addAction("continue_request", "继续提出新要求", "如果结果符合预期，可以直接继续补充下一步需求。", "continue", actions.length ? "outline" : "primary");
  ...
  const summaryCards = [
    {
      ...
      value: compactMemoryText(deliveryReport?.headline || summary.headline || (fileCount ? `已整理 ${fileCount} 个文件改动` : status === "ready" ? "任务结果已整理，等待你核对。" : "当前状态已整理。"), 180),
    ...
    },
    {
      id: "next",
      label: "下一步",
      value: compactMemoryText(actions[0]?.detail || actions[0]?.label || nextAction || "可以继续提出新要求。", 180),
...
export function buildLiveMainAgentTodoPlan(task: any, phase: string, workers: any[], executions: any[], summary: any = {}) {
  ...
  const add = (id: string, content: string, status: string, activeForm = "", detail = "") => {
  ...
  if (recoveryVisible) add(
    ...
    task?.recovery_pending === true ? "恢复上次任务上下文，等待确认继续" : "",
  ...
  add(
    ...
    hasDispatchEvidence ? `派发给 ${Math.max(assignmentCount, workers.length, executions.length, 1)} 个执行成员或执行通道` : "",
    dispatchStatus
  );

  // 执行前计划中的模型步骤和用户追加步骤随任务阶段贯穿到执行期，用户批准的计划就是执行中展示的计划。
  for (const spec of buildPlanModeWorkStepSpecs(task, phase)) {
  ...
  add(
    ...
    activeWorkers.length ? `${activeWorkers.join("、")} 正在执行` : workers.length ? "" : "等待执行成员开始执行",
    workerStatus,
    "",
  ...
  add(
    ...
    receiptCount > 0 ? `读取 ${receiptCount} 条执行成员结果说明` : "",
    receiptStatus
  );
  ...
  add(
    "coordinator_review",
    verificationCount > 0 ? `最终验收并检查 ${verificationCount} 项验证` : "",
    reviewStatus,
    "",
...
export function buildTodoStepEvidence(input: { task: any; summary: any; workers: any[]; executions: any[]; stepId: string; phase: string }) {
  ...
  if (/^model_plan_\d+$/.test(stepId) || /^followup_requirement_\d+$/.test(stepId)) {
    ...
    const planStep = extractPlanModeWorkSteps(planMode).find((item: any) => String(item?.id || "") === stepId) || {};
    if (/^followup_requirement_\d+$/.test(stepId)) {
      const revision = (Array.isArray(planMode?.plan_revisions) ? planMode.plan_revisions : []).find((item: any) => item?.step_id === stepId);
...
export function buildTodoStepActions(input: { task: any; stepId: string; status: string; phase: string; summary: any }) {
  ...
  if (["in_progress", "reviewing", "reworking"].includes(status) && !["completed", "cancelled"].includes(phase)) add("cancel", "停止任务", "cancel", "danger");
