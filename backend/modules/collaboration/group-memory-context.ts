...
import {
  isCanonicalGroupSessionMemory,
  modelVisibleGroupRuntimeState,
} from "./group-runtime-memory-admission";
import {
...
import {
  buildChildTypedMemoryDeliveryCapsule,
  buildChildTypedMemoryRecallLedgerScope,
  verifyGroupSessionMemoryModelExtractionDeliveryEvidenceForContext,
...
import {
  ...
  buildGroupMemoryDispatchFreshnessGate,
  ...
  getGroupApiMicrocompactNativeApplyRequestTelemetryLedgerFile,
  getGroupPostCompactCandidateUsageLedgerFile,
  latestGroupCompactFileReferenceReadPlanRows,
  recordGroupCompactFileReferenceSurfacing,
  recordGroupMemoryReloadAudit,
  recordGroupPostCompactFirstDispatchMarker,
...
import {
  buildChildGlobalAgentMemoryContext,
  distillGroupGlobalMemoryArbitrationToTypedMemory,
  getGroupGlobalMemoryArbitrationLedgerFile,
  recordGroupGlobalMemoryArbitrationLedger,
} from "./group-global-memory-arbitration";
import {
  GROUP_GLOBAL_MEMORY_ARBITRATION_DIR,
  GROUP_MEMORY_AUTO_COMPACT_DEBOUNCE_MS,
  GROUP_MEMORY_POST_COMPACT_REINJECTION_GATE_VERSION,
  POST_COMPACT_COMPLETION_MEMORY_PRESERVATION_CLOSURE_CONFLICT_RESOLUTION_REL_PATH,
  POST_COMPACT_COMPLETION_MEMORY_PRESERVATION_REPAIR_CLOSURE_REL_PATH,
  POST_COMPACT_RECEIPT_MEMORY_USAGE_REPAIR_COMPLETION_REL_PATH,
  POST_COMPACT_REINJECTION_REPAIR_RECEIPT_CAUTION_REL_PATH,
  POST_COMPACT_REINJECTION_REPAIR_RECEIPT_MEMORY_REL_PATH,
  ...
  buildCompressedGroupMessageDigest,
  buildGroupCompactBoundaryHistorySummary,
  buildGroupMemoryResumeEffectiveTokenBaseline,
  clearUntrustedGroupCompactionState,
  compactMemoryText,
  compactPreserveLines,
  createEmptyAgentMemory,
  formatAgentMemoryReceipt,
  getCompactBoundaryIndex,
  getGroupMessagesFileHint,
  groupMemoryAutoCompactPending,
  groupMemoryAutoCompactRunning,
  groupMemoryAutoCompactTimers,
  hasToolGrantSet,
  hashSessionMemoryText,
  importGroupProjectMemoriesForMembers,
  isGroupModelCompactionEnabled,
  isPostCompactCompletionMemoryPreservationRepairClosureRecallQuery,
  isPostCompactReceiptMemoryUsageRepairCompletionRecallQuery,
  isProviderRankingProvenanceCompactRepairReceiptRecallQuery,
  latestGroupMessageTimestamp,
  loadGroupMemoryCompactionConfig,
  normalizeAgentMemoryProject,
  normalizeGlobalGroupMemoryMembers,
  resolveGroupProjectMemoryRoot,
  scoreGlobalGroupMemoryCandidate,
  uniqueProviderRankingCompactRepairRecallStrings,
  validateGroupMemoryResumeEffectiveTokenBaseline,
...
import {
  ...
  getGroupReplayRepairLedgerFile,
  getGroupReplayRepairWorkItemsFile,
  getGroupSessionMemoryMarkdownFile,
  loadGroupMemory,
  persistGroupMemoryResumeEffectiveTokenBaseline,
  readGroupReplayRepairDispatchCandidatesSummary,
  readGroupReplayRepairLedgerSummary,
  readGroupReplayRepairWorkItemsSummary,
  saveGroupMemory,
...
export function buildAgentMemoryContextBundle(groupId: string, targetProject: string, task = "", options: any = {}) {
  ...
  const taskAgentInvocationLineage = options.invocationEdgeId || options.invocation_edge_id ? {
    ...
    capsule_checksum: String(postTurnSummaryDeliveryCapsule?.capsule_checksum || "") } : null;
  ...
  if (timeBasedToolResultProjection.applied) {
    memory = saveGroupMemory(groupId, {
      ...
      messageCompression: {
        ...
        timeBasedToolResultProjection: timeBasedToolResultProjection.receipt } }, groupSessionId);
  }
  if (timeBasedThinkingProjection.shouldPersist) {
    memory = saveGroupMemory(groupId, {
      ...
      messageCompression: {
        ...(memory.messageCompression || {}),
        timeBasedThinkingProjection: timeBasedThinkingProjection.receipt } }, groupSessionId);
  }
  ...
  const formalSessionContinuity = formalSummaryAvailable ? {
    ...
    session_memory: isCanonicalGroupSessionMemory(memory.sessionMemory) ? memory.sessionMemory : null,
  ...
  const projectedModelReceipt = persistedModelReceipt ? {
    ...
    requestAudit: modelReceiptCore.requestAudit ? {
      ...
      sourceMessageIds: Array.isArray(modelReceiptCore.requestAudit.sourceMessageIds) ? modelReceiptCore.requestAudit.sourceMessageIds.slice(0, 240) : [] } : null,
    mergeQuality: receiptMergeQuality ? {
      ...
      unjustifiedLostFactCount: Number(receiptMergeCore.unjustifiedLostFactCount || 0) } : null } : null;
  const sessionMemorySnapshot = isCanonicalGroupSessionMemory(persistedSessionMemorySnapshot) ? {
    ...sessionMemoryCore,
    ...
    modelExtractionReplayEvidence: persistedReplayEvidence?.schema ? {
      ...
      checksumValid: replayEvidenceValid } : null,
    ...
    factSupersession: persistedSupersessionGraph?.schema ? {
      ...
      activeFacts: (persistedSupersessionGraphValid && Array.isArray(persistedSupersessionGraph.activeFacts) ? persistedSupersessionGraph.activeFacts : []).map((fact: any) => ({
        ...
        sourceMessageId: String(fact.sourceMessageId || "") })).slice(0, 120) } : null } : persistedSessionMemorySnapshot;
    ...
    group_state: {
      ...
      postTurnSummaries: {
        ...
        deliveryCapsule: postTurnSummaryDeliveryCapsule ? {
          ...
          selectedCount: postTurnSummaryDeliveryCapsule.selected_count } : null,
