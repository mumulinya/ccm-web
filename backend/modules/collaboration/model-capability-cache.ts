...
import { markTaskAgentSessionsForCapacityDowngrade } from "../../tasks/agent-sessions";

export const MODEL_CAPABILITY_CACHE_SCHEMA = "ccm-model-capability-cache-v1";
export const MODEL_CONTEXT_CAPACITY_SCHEMA = "ccm-model-context-capacity-v2";
export const CONSERVATIVE_CONTEXT_WINDOW_TOKENS = 200_000;
export const CONSERVATIVE_MAX_OUTPUT_TOKENS = 20_000;
export const CONTEXT_AUTOCOMPACT_BUFFER_TOKENS = 13_000;

const CACHE_FILE = path.join(CCM_DIR, "memory-control", "model-capability-cache.json");
const REFRESH_QUEUE_FILE = path.join(CCM_DIR, "memory-control", "model-capability-refresh-queue.json");
const REFRESH_LEASE_FILE = path.join(CCM_DIR, "memory-control", "model-capability-refresh-queue.lease.json");
const REFRESH_JOURNAL_FILE = path.join(CCM_DIR, "memory-control", "model-capability-refresh-queue.jsonl");
const REFRESH_STATUS_FILE = path.join(CCM_DIR, "memory-control", "model-capability-refresh-status.json");
const REFRESH_OUTCOME_LEDGER_FILE = path.join(CCM_DIR, "memory-control", "model-capability-refresh-outcomes.json");
const REFRESH_OUTCOME_PENDING_DIR = path.join(CCM_DIR, "memory-control", "model-capability-refresh-outcome-pending");
const REFRESH_OUTCOME_QUARANTINE_DIR = path.join(CCM_DIR, "memory-control", "model-capability-refresh-outcome-invalid");
const REFRESH_OUTCOME_ACK_LEDGER_FILE = path.join(CCM_DIR, "memory-control", "model-capability-refresh-outcome-invalid-acknowledgements.json");
const REFRESH_ARCHIVE_DIR = path.join(CCM_DIR, "memory-control", "model-capability-refresh-archive");
const DOWNGRADE_JOURNAL_FILE = path.join(CCM_DIR, "memory-control", "model-capability-downgrades.jsonl");
...
const SOURCE_TTL_MS: Record<string, number> = {
...
function emptyCache() {
  return { schema: MODEL_CAPABILITY_CACHE_SCHEMA, version: 1, entries: [] as any[], updatedAt: "", checksum: "" };
}

function cacheChecksum(cache: any) {
...
function readRawCache() {
  try {
    ...
    const entries = Array.isArray(parsed.entries)
      ? parsed.entries.filter((entry: any) => entry && entry.checksum === entryChecksum(entry))
      : [];
    return { schema: MODEL_CAPABILITY_CACHE_SCHEMA, version: 1, entries, updatedAt: String(parsed.updatedAt || ""), checksum: String(parsed.checksum || "") };
  } catch {
    return emptyCache();
  }
}
...
function writeCache(cache: any) {
  ...
  const next = { ...cache, schema: MODEL_CAPABILITY_CACHE_SCHEMA, version: 1, updatedAt: new Date().toISOString() };
...
function normalizeEvidence(input: ModelCapabilityEvidence, now = new Date()) {
  const source = String(input?.source || "") as CapabilitySource;
  if (!SOURCE_PRIORITY[source]) throw new Error("不可信的模型能力来源");
  const nativeValidation = source === "native_executor_receipt" ? verifyNativeModelCapabilityReceipt(input) : null;
  if (nativeValidation && !nativeValidation.valid) throw new Error(`原生执行器能力回执验证失败：${nativeValidation.gaps.join(",")}`);
  const provider = clean(input.provider);
  const model = clean(input.model);
  if (!provider) throw new Error("模型能力必须指定 provider");
  ...
  const maxOutputTokens = Math.floor(Number(input.maxOutputTokens || input.max_output_tokens || CONSERVATIVE_MAX_OUTPUT_TOKENS));
  if (!Number.isFinite(contextWindow) || contextWindow < 32_000 || contextWindow > 4_000_000) throw new Error("上下文窗口必须介于 32,000 和 4,000,000 token");
  if (!Number.isFinite(maxOutputTokens) || maxOutputTokens < 0 || maxOutputTokens > contextWindow - 16_000) throw new Error("最大输出 token 与上下文窗口不兼容");
  const checkedAt = new Date(input.checkedAt || input.checked_at || (input as any).capturedAt || now.toISOString());
  if (!Number.isFinite(checkedAt.getTime()) || checkedAt.getTime() > now.getTime() + 5 * 60 * 1000) throw new Error("模型能力校验时间无效");
  ...
  if (!Number.isFinite(expiresAt.getTime()) || expiresAt <= checkedAt) throw new Error("模型能力过期时间无效");
  ...
  const core: any = {
    ...
    ...(source === "native_executor_receipt" ? {
      nativeReceiptChecksum: String((input as any).checksum || ""),
      runner: String((input as any).runner || ""),
      runnerRequestId: String((input as any).runnerRequestId || ""),
      groupId: String((input as any).groupId || ""),
      taskId: String((input as any).taskId || ""),
      executionId: String((input as any).executionId || ""),
      taskAgentSessionId: String((input as any).taskAgentSessionId || ""),
      nativeSessionId: String((input as any).nativeSessionId || ""),
      nativeEventChecksum: String((input as any).eventChecksum || ""),
    } : {}),
  };
  return { ...core, checksum: entryChecksum(core) };
}

export function recordModelCapabilityEvidence(input: ModelCapabilityEvidence) {
  ...
  const refreshRequest = buildModelCapabilityRefreshPlan().requests.find((request: any) =>
    request.provider === entry.provider
    && request.model === entry.model
    && request.source === entry.source
  ) || null;
  const cache = readRawCache();
  const entries = cache.entries.filter((item: any) => !(item.key === entry.key && item.source === entry.source));
  entries.push(entry);
  ...
  if (Number(before.contextWindow || 0) > Number(after.contextWindow || 0)) {
    const affected = markTaskAgentSessionsForCapacityDowngrade({
...
export function recordVerifiedNativeModelCapabilityReceipt(receipt: any, expected: any = {}) {
  const validation = verifyNativeModelCapabilityReceipt(receipt, expected);
  if (!validation.valid) return { recorded: false, validation };
...
export function revokeModelCapabilityEvidence(input: any = {}) {
  const provider = clean(input.provider);
  const model = clean(input.model);
  const evidenceId = String(input.evidenceId || input.evidence_id || "").trim();
  if (!provider && !evidenceId) throw new Error("撤销能力证据必须指定 provider 或 evidenceId");
  const cache = readRawCache();
  const revokedAt = new Date().toISOString();
  let revoked = 0;
  const entries = cache.entries.map((entry: any) => {
    ...
    if (!matches || entry.revoked === true) return entry;
    revoked++;
    const next = {
      ...entry,
      revoked: true,
...
export function runModelCapabilityCacheMaintenance(input: any = {}) {
  ...
  const evidenceIds = new Set((Array.isArray(input.evidenceIds || input.evidence_ids) ? (input.evidenceIds || input.evidence_ids) : []).map((value: any) => String(value || "").trim()).filter(Boolean));
  const candidates = cache.entries.filter((entry: any) => {
    if (providerFilter && entry.provider !== providerFilter) return false;
    if (evidenceIds.size && !evidenceIds.has(entry.evidenceId)) return false;
  ...
  if (input.dryRun !== false || !candidates.length) {
    return { schema: "ccm-model-capability-cache-maintenance-v1", dryRun: input.dryRun !== false, candidateCount: candidates.length, deletedCount: 0, candidateEvidenceIds: candidates.map((entry: any) => entry.evidenceId), ranAt: now.toISOString() };
  }
  const candidateChecksums = new Set(candidates.map((entry: any) => entry.checksum));
  const raw = readRawCache();
  const entries = raw.entries.filter((entry: any) => !candidateChecksums.has(entry.checksum));
  writeCache({ ...raw, entries });
  return { schema: "ccm-model-capability-cache-maintenance-v1", dryRun: false, candidateCount: candidates.length, deletedCount: candidates.length, candidateEvidenceIds: candidates.map((entry: any) => entry.evidenceId), ranAt: now.toISOString() };
}

export function readModelCapabilityCache(now = new Date()) {
  const cache = readRawCache();
  const entries = cache.entries.map((entry: any) => {
...
export function summarizeModelCapabilityCache(cache = readModelCapabilityCache()) {
  const entries = Array.isArray(cache.entries) ? cache.entries : [];
  return {
    schema: MODEL_CAPABILITY_CACHE_SCHEMA,
    file: CACHE_FILE,
    total: entries.length,
    active: entries.filter((entry: any) => entry.expired !== true && entry.revoked !== true).length,
    expired: entries.filter((entry: any) => entry.expired === true).length,
    revoked: entries.filter((entry: any) => entry.revoked === true).length,
    refreshDue: entries.filter((entry: any) => entry.refreshDue === true && entry.revoked !== true).length,
    providers: [...new Set(entries.map((entry: any) => entry.provider).filter(Boolean))].length,
    models: [...new Set(entries.map((entry: any) => entry.model).filter(Boolean))].length,
    updatedAt: cache.updatedAt || "",
    checksumValid: cache.checksumValid !== false,
  };
}

function capacityFromEntry(entry: any, cacheStatus: string) {
  const reservedOutputTokens = Math.min(20_000, Math.max(0, Number(entry.maxOutputTokens || CONSERVATIVE_MAX_OUTPUT_TOKENS)));
  const effectiveContextWindow = Math.max(18_000, Number(entry.contextWindow) - reservedOutputTokens);
  return {
    schema: MODEL_CONTEXT_CAPACITY_SCHEMA,
    provider: entry.provider || "",
    model: entry.model || "",
    ...
    autoCompactBufferTokens: CONTEXT_AUTOCOMPACT_BUFFER_TOKENS,
    autoCompactThreshold: Math.max(18_000, effectiveContextWindow - CONTEXT_AUTOCOMPACT_BUFFER_TOKENS),
    source: entry.source,
...
export function resolveTrustedModelContextCapacity(input: any = {}) {
  ...
  for (const raw of evidenceInputs) {
    try { transient.push(normalizeEvidence({ provider, model, ...raw }, now)); } catch {}
  }
  ...
  if (explicitWindow > 0) {
    try {
      transient.push(normalizeEvidence({
        ...
        maxOutputTokens: Number(input.modelMaxOutputTokens || input.model_max_output_tokens || input.maxOutputTokens || CONSERVATIVE_MAX_OUTPUT_TOKENS),
        checkedAt: input.capacityCheckedAt || input.capacity_checked_at,
      }, now));
    } catch {}
  }
  const cacheSnapshot = readModelCapabilityCache(now);
  const matchingCached = (cacheSnapshot.checksumValid === false ? [] : cacheSnapshot.entries).filter((entry: any) => {
    const providerMatches = entry.provider === provider;
    const modelMatches = model ? entry.model === model || !entry.model : !entry.model;
    return providerMatches && modelMatches;
  });
  const cached = matchingCached.filter((entry: any) => entry.expired !== true && entry.revoked !== true && entry.checksumValid !== false);
  const candidates = [...transient, ...cached]
    .filter((entry: any) => new Date(entry.expiresAt).getTime() > now.getTime())
    .sort((a: any, b: any) => Number(b.priority || 0) - Number(a.priority || 0) || String(b.checkedAt).localeCompare(String(a.checkedAt)));
  if (candidates.length) return capacityFromEntry(candidates[0], transient.includes(candidates[0]) ? "transient_trusted" : candidates[0].refreshDue === true ? "cache_hit_refresh_due" : "cache_hit");
  const staleSafeEntry = matchingCached
    .filter((entry: any) => entry.expired === true && entry.revoked !== true && entry.checksumValid !== false)
    .sort((a: any, b: any) => Number(a.contextWindow || 0) - Number(b.contextWindow || 0))[0];
  if (staleSafeEntry && Number(staleSafeEntry.contextWindow || 0) < CONSERVATIVE_CONTEXT_WINDOW_TOKENS) {
    return {
  ...
  const contextWindow = CONSERVATIVE_CONTEXT_WINDOW_TOKENS;
  const reservedOutputTokens = CONSERVATIVE_MAX_OUTPUT_TOKENS;
  ...
  return {
    ...
    effectiveContextWindow,
    autoCompactBufferTokens: CONTEXT_AUTOCOMPACT_BUFFER_TOKENS,
    autoCompactThreshold: effectiveContextWindow - CONTEXT_AUTOCOMPACT_BUFFER_TOKENS,
    ...
    fallbackReason: cacheSnapshot.checksumValid === false
      ...
      : matchingCached.some((entry: any) => entry.revoked === true)
        ? "trusted_capability_revoked"
        : matchingCached.some((entry: any) => entry.expired === true)
          ? "trusted_capability_expired"
          : "no_trusted_capability",
  };
}

export function buildModelCapabilityRefreshPlan(input: any = {}) {
  const now = input.now instanceof Date ? input.now : new Date(input.now || Date.now());
  ...
  for (const row of readRefreshJournal(2_000, String(input.journalFile || REFRESH_JOURNAL_FILE))) {
    if (row.type === "refresh_outcome" && row.evidenceId) latestOutcomes.set(String(row.evidenceId), row);
  }
  const requests = cache.entries
    .filter((entry: any) => entry.refreshDue === true && entry.revoked !== true && entry.checksumValid !== false)
    .map((entry: any) => {
...
export function recordModelCapabilityRefreshOutcome(input: any = {}) {
  const provider = clean(input.provider || input.agentType || input.agent_type);
  const model = clean(input.model || input.modelId || input.model_id);
  const outcome = String(input.outcome || "").trim();
  if (!provider || !["refreshed", "metadata_absent", "unsupported"].includes(outcome)) return { recorded: false, reason: "invalid_outcome" };
  ...
  const matches = suppliedRequest
    ? [suppliedRequest].filter((request: any) => request.provider === provider && (!request.model || (!!model && request.model === model)))
    : plan.requests.filter((request: any) => request.status === "pending_native_telemetry" && request.provider === provider && (!request.model || (!!model && request.model === model)));
  if (!matches.length) return { recorded: false, reason: "no_refresh_due", outcome };
  const rows = matches.map((request: any) => {
  ...
  const acquired: any = acquireModelCapabilityRefreshLease({ file: leaseFile, journalFile, ttlMs: 30_000 });
  if (acquired.acquired) {
    for (const row of rows) appendRefreshJournal({ ...row, fencingToken: Number(acquired.lease?.fencingToken || 0), leaseId: acquired.lease?.leaseId || "" }, journalFile);
    if (acquired.handle) releaseModelCapabilityRefreshLease(acquired.handle, "refresh-outcome-recorded");
...
export function writeModelCapabilityRefreshPlan(input: any = {}) {
...
function refreshLeaseChecksum(value: any) {
...
function appendRefreshJournal(entry: any, file = REFRESH_JOURNAL_FILE) {
  fs.mkdirSync(path.dirname(file), { recursive: true });
  fs.appendFileSync(file, `${JSON.stringify(entry)}\n`, "utf-8");
}

function invalidOutcomeEnvelopeChecksum(value: any) {
  const { checksum: _checksum, ...core } = value || {};
  return hash(core);
}

function quarantineInvalidPendingRefreshOutcome(input: any = {}) {
  ...
  try {
    ...
    if (fs.existsSync(incomingFile) && !fs.existsSync(sourceFile)) {
      try { fs.renameSync(incomingFile, sourceFile); } catch {}
    }
    throw error;
  }
...
function drainPendingRefreshOutcomes(input: any = {}) {
  const pendingDir = String(input.pendingDir || REFRESH_OUTCOME_PENDING_DIR);
  const journalFile = String(input.journalFile || REFRESH_JOURNAL_FILE);
  if (!fs.existsSync(pendingDir)) return { drained: 0, quarantined: 0, failed: 0 };
  let drained = 0;
  let quarantined = 0;
  let failed = 0;
  for (const name of fs.readdirSync(pendingDir).filter(name => name.endsWith(".json")).slice(0, 2_000)) {
    ...
    try {
      ...
      if (row.schema !== "ccm-model-capability-refresh-outcome-v1") throw new Error("invalid pending outcome schema");
...
export function buildModelCapabilityRefreshOutcomeLedger(input: any = {}) {
  const journalFile = String(input.journalFile || REFRESH_JOURNAL_FILE);
  ...
  const outcomes = [...archivedRows, ...activeRows]
    .filter((row: any) => row.type === "refresh_outcome" && Date.parse(String(row.at || "")) >= now.getTime() - retentionMs)
  ...
  const providers = [...providerRows.entries()].map(([provider, rows]) => {
    ...
    for (let index = rows.length - 1; index >= 0 && rows[index].outcome === "metadata_absent"; index--) consecutiveMetadataAbsent++;
  ...
  const core: any = {
    ...
    providers,
    totals: {
      refreshed: outcomes.filter((row: any) => row.outcome === "refreshed").length,
      metadataAbsent: outcomes.filter((row: any) => row.outcome === "metadata_absent").length,
      unsupported: outcomes.filter((row: any) => row.outcome === "unsupported").length,
...
function writeModelCapabilityRefreshOutcomeLedger(input: any = {}) {
...
function retainRefreshJournal(input: any = {}) {
  const file = String(input.journalFile || REFRESH_JOURNAL_FILE);
  const maxRows = Math.max(10, Number(input.maxRows || 5_000));
  if (!fs.existsSync(file)) return { before: 0, retained: 0, archived: 0, archiveFile: "" };
  const lines = fs.readFileSync(file, "utf-8").split(/\r?\n/).filter(Boolean);
  if (lines.length <= maxRows) return { before: lines.length, retained: lines.length, archived: 0, archiveFile: "" };
...
function retainRefreshArchives(input: any = {}) {
  const archiveDir = String(input.archiveDir || REFRESH_ARCHIVE_DIR);
  ...
  if (!fs.existsSync(archiveDir)) return { scanned: 0, deleted: 0, expired: 0, overflow: 0, remaining: 0, maxArchives, retentionDays };
  ...
  for (const row of afterExpiry.slice(0, Math.max(0, afterExpiry.length - maxArchives))) deleteFiles.add(row.file);
  let deleted = 0;
  for (const file of deleteFiles) {
    try { fs.unlinkSync(file); deleted++; } catch {}
  }
  return {
...
export function readModelCapabilityRefreshOutcomeLedger(input: any = {}) {
  const file = String(input.file || REFRESH_OUTCOME_LEDGER_FILE);
...
function invalidOutcomeAcknowledgementChecksum(value: any) {
  const { checksum: _checksum, ...core } = value || {};
  return hash(core);
}
...
function readInvalidOutcomeAcknowledgementLedger(file = REFRESH_OUTCOME_ACK_LEDGER_FILE) {
  try {
    ...
    if (ledger.schema !== "ccm-model-capability-invalid-refresh-outcome-ack-ledger-v1" || checksum !== invalidOutcomeAcknowledgementChecksum(core)) throw new Error("invalid acknowledgement ledger");
    return ledger;
  } catch {
    const core = { schema: "ccm-model-capability-invalid-refresh-outcome-ack-ledger-v1", version: 1, updatedAt: "", acknowledgements: [] as any[] };
    return { ...core, checksum: invalidOutcomeAcknowledgementChecksum(core) };
  }
}

function writeInvalidOutcomeAcknowledgementLedger(ledger: any, file = REFRESH_OUTCOME_ACK_LEDGER_FILE) {
  const core = {
...
export function readInvalidPendingModelCapabilityRefreshOutcomes(input: any = {}) {
  const quarantineDir = String(input.quarantineDir || REFRESH_OUTCOME_QUARANTINE_DIR);
  ...
  const acknowledgements = new Map((ledger.acknowledgements || []).map((row: any) => [String(row.invalidOutcomeId || ""), row]));
  const outcomes: any[] = [];
  if (fs.existsSync(quarantineDir)) {
    for (const name of fs.readdirSync(quarantineDir).filter(name => name.endsWith(".json")).slice(-2_000)) {
      ...
      try {
        ...
        const acknowledgement: any = acknowledgements.get(String(envelope.invalidOutcomeId || "")) || null;
...
export function acknowledgeInvalidPendingModelCapabilityRefreshOutcome(input: any = {}) {
  const invalidOutcomeId = String(input.invalidOutcomeId || input.invalid_outcome_id || "").trim();
  if (!/^mcroi_[a-f0-9]{24}$/.test(invalidOutcomeId)) throw new Error("无效的隔离回执 ID");
  const quarantineDir = String(input.quarantineDir || REFRESH_OUTCOME_QUARANTINE_DIR);
  const file = path.join(quarantineDir, `${invalidOutcomeId}.json`);
  if (!fs.existsSync(file)) throw new Error("隔离回执不存在");
  const journalFile = String(input.journalFile || REFRESH_JOURNAL_FILE);
  const leaseFile = String(input.leaseFile || (input.journalFile ? `${journalFile}.lease.json` : REFRESH_LEASE_FILE));
  const acquired: any = acquireModelCapabilityRefreshLease({ file: leaseFile, journalFile, ttlMs: 30_000 });
  if (!acquired.acquired) return { acknowledged: false, reason: acquired.reason || "lease_busy" };
  try {
    const acknowledgementLedgerFile = String(input.acknowledgementLedgerFile || REFRESH_OUTCOME_ACK_LEDGER_FILE);
    const ledger = readInvalidOutcomeAcknowledgementLedger(acknowledgementLedgerFile);
    const existing = (ledger.acknowledgements || []).find((row: any) => row.invalidOutcomeId === invalidOutcomeId);
    if (existing) return { acknowledged: true, alreadyAcknowledged: true, acknowledgement: existing };
    ...
    if (acquired.handle) releaseModelCapabilityRefreshLease(acquired.handle, "invalid-outcome-acknowledged");
  }
}

function writeRefreshStatus(status: any, file = REFRESH_STATUS_FILE) {
  fs.mkdirSync(path.dirname(file), { recursive: true });
  const temp = `${file}.${process.pid}.${Date.now()}.tmp`;
...
export function inspectModelCapabilityRefreshLease(input: any = {}) {
  const file = String(input.file || REFRESH_LEASE_FILE);
  let lease: any = null;
  try { lease = JSON.parse(fs.readFileSync(file, "utf-8")); } catch {}
  if (!lease) return { file, present: false, valid: true, active: false, lease: null };
...
function writeRefreshLease(handle: any, value: any) {
  const core = { ...value };
...
export function acquireModelCapabilityRefreshLease(input: any = {}) {
  const file = String(input.file || REFRESH_LEASE_FILE);
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    try {
      ...
      const journalToken = readRefreshJournal(500, String(input.journalFile || REFRESH_JOURNAL_FILE)).reduce((max: number, row: any) => Math.max(max, Number(row.fencingToken || row.lease?.fencingToken || 0)), 0);
...
export function releaseModelCapabilityRefreshLease(handle: any, finalStatus = "completed") {
  ...
  try {
    ...
    if (current.lease?.leaseId === handle.lease.leaseId) try { fs.unlinkSync(handle.file); } catch {}
    return true;
  } catch {
...
export function runModelCapabilityRefreshMaintenance(input: any = {}) {
  const startedAt = new Date().toISOString();
  const acquired: any = input.skipLease === true
  ...
  let error = "";
  let plan: any = null;
  let pending: any = { drained: 0, quarantined: 0, failed: 0 };
  let retention: any = { before: 0, retained: 0, archived: 0, archiveFile: "" };
  let archiveRetention: any = { scanned: 0, deleted: 0, expired: 0, overflow: 0, remaining: 0 };
  let outcomeLedger: any = null;
  let ledgerRecovery: any = { recoveryRequired: false, recoveryReason: "", recovered: false, recoveredAt: "", previousChecksumValid: true };
  try {
    if (Number(input.holdMs || 0) > 0) Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, Number(input.holdMs));
    const journalFile = String(input.journalFile || REFRESH_JOURNAL_FILE);
  ...
  if (acquired.handle) releaseModelCapabilityRefreshLease(acquired.handle, error ? "failed" : "completed");
  return status;
}

export function readModelCapabilityRefreshStatus() {
  try { return JSON.parse(fs.readFileSync(REFRESH_STATUS_FILE, "utf-8")); }
  catch { return { schema: "ccm-model-capability-refresh-status-v1", success: true, skipped: true, reason: "not_run", requestCount: 0, lease: null }; }
}

export function readModelCapabilityDowngradeAlerts(limit = 50) {
  try {
    if (!fs.existsSync(DOWNGRADE_JOURNAL_FILE)) return [];
...
export function pruneModelCapabilityDowngradeAlerts(input: any = {}) {
  const provider = clean(input.provider);
  const providerPrefix = clean(input.providerPrefix || input.provider_prefix);
  if (!provider && !providerPrefix) throw new Error("清理容量降级告警必须指定 provider 或 providerPrefix");
  const before = readModelCapabilityDowngradeAlerts(10_000);
  const keep = before.filter((alert: any) => {
  ...
  if (!removed) return { removed: 0, remaining: before.length };
  fs.mkdirSync(path.dirname(DOWNGRADE_JOURNAL_FILE), { recursive: true });
  const temp = `${DOWNGRADE_JOURNAL_FILE}.${process.pid}.${Date.now()}.tmp`;
  fs.writeFileSync(temp, keep.length ? `${keep.map((alert: any) => JSON.stringify(alert)).join("\n")}\n` : "", "utf-8");
  fs.renameSync(temp, DOWNGRADE_JOURNAL_FILE);
  return { removed, remaining: keep.length };
}

let refreshScheduler: NodeJS.Timeout | null = null;
export function startModelCapabilityRefreshScheduler(intervalMs = 60 * 60 * 1000) {
  if (refreshScheduler) return { started: false, reason: "already_running" };
  runModelCapabilityRefreshMaintenance({ trigger: "startup" });
  refreshScheduler = setInterval(() => {
    try { runModelCapabilityRefreshMaintenance({ trigger: "interval" }); } catch {}
...
export function stopModelCapabilityRefreshScheduler() {
...
export function runModelCapabilityCacheSelfTest() {
  ...
  return {
    pass: explicit.contextWindow === 516_000
      ...
      && unverifiedNative.contextWindow === CONSERVATIVE_CONTEXT_WINDOW_TOKENS
      && unverifiedNative.conservativeFallback === true
      && expired.contextWindow === CONSERVATIVE_CONTEXT_WINDOW_TOKENS,
    checks: {
