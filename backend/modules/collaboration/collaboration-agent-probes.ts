...
export function buildDailyDevAgentDiagnostics() {
  ...
  const runSelfTestSafely = (runner: () => any, label: string) => {
    try { return runner(); }
    catch (error: any) { return { pass: false, error: String(error?.message || error || `${label} 自检执行时抛出异常`) }; }
  };
  const collaborationUx = runSelfTestSafely(() => runCollaborationUxSelfTest(), "群聊协作体验");
  checks.push(createDiagnosticCheck(
  ...
  const planCore = runSelfTestSafely(() => runMainAgentPlanCoreSelfTest(), "主 Agent 计划核心");
  checks.push(createDiagnosticCheck(
    "main-agent-plan-core",
    "主 Agent 计划核心",
    planCore.pass ? "ok" : "fail",
    planCore.pass
      ? "计划书修订与执行期贯穿自检通过：追加要求会并入计划步骤并记录修订历史，等待确认的计划要求重新确认，模型计划步骤随任务阶段推进"
      : "主 Agent 计划核心自检失败，需要检查追加要求并入、修订历史或模型步骤贯穿逻辑",
    planCore
  ));
  ...
  const mainAgentToolLoop = runSelfTestSafely(() => runGroupMainAgentToolLoopSelfTest(), "群聊主 Agent 多步工作循环");
  checks.push(createDiagnosticCheck(
