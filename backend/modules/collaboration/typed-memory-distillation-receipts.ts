...
import {
  pressureMemoryProvenanceRowsFromRawRecovery,
  pressureMemoryProvenanceStringList,
  roundPressureRecallUsageWeight,
} from "./typed-memory-recall";
import {
  GLOBAL_PROVIDER_DISPATCH_RELIABILITY_DIR,
  GLOBAL_PROVIDER_DISPATCH_RELIABILITY_SNAPSHOT_TTL_MS,
  GLOBAL_PROVIDER_DISPATCH_RELIABILITY_SNAPSHOT_VERSION,
  ...
  GROUP_LOG_ACTIVITY_NOISE_PATTERN,
  GROUP_LOG_DURABLE_PATTERN,
  GROUP_LOG_EPHEMERAL_PATTERN,
  GROUP_LOG_EXTERNAL_RESOURCE_PATTERN,
  GROUP_LOG_NON_OBVIOUS_PATTERN,
  GROUP_LOG_POSITIVE_REVOCATION_PATTERN,
  GROUP_LOG_RATIONALE_PATTERN,
  GROUP_LOG_RESOURCE_PURPOSE_PATTERN,
  GROUP_LOG_USER_CORRECTION_PATTERN,
  ...
  GROUP_PROVIDER_DISPATCH_RELIABILITY_MAX_SOURCE_GROUPS,
  ...
  GROUP_SESSION_MODEL_EXTRACTION_MAX_TOPICS_PER_CATEGORY,
  GROUP_SESSION_MODEL_EXTRACTION_TOPIC_ASSIGNMENT_MIN_CONFIDENCE,
  GROUP_SESSION_MODEL_EXTRACTION_TOPIC_MERGE_MIN_SIMILARITY,
  GROUP_SESSION_MODEL_EXTRACTION_TOPIC_REUSE_MIN_SIMILARITY,
  GROUP_SESSION_MODEL_EXTRACTION_TOPIC_VERSION,
  GROUP_TYPED_MEMORY_DIR,
  GROUP_TYPED_MEMORY_DISTILLATION_FACT_LIMIT,
  GROUP_TYPED_MEMORY_DISTILLATION_LEDGER,
  GROUP_TYPED_MEMORY_DISTILLATION_MAX_MESSAGES,
  GROUP_TYPED_MEMORY_DISTILLATION_TRANSACTION_VERSION,
  GROUP_TYPED_MEMORY_DISTILLATION_VERSION,
  GROUP_TYPED_MEMORY_WRITE_ADMISSION_VERSION,
  GroupTypedMemoryType,
  MODEL_EXTRACTION_TOPIC_CANONICAL_CONCEPTS,
  MODEL_EXTRACTION_TOPIC_GENERIC_CONCEPTS,
  activeGroupTypedMemoryDistillationMutations,
  buildGroupLogPositiveConfirmationCandidate,
  buildPostCompactCandidateUsageArchive,
  checksum,
  compactText,
  extractMessageFiles,
  extractMessageSkills,
  extractMessageVerification,
  getGroupTypedMemoryDir,
  isExactGroupTypedMemorySessionScope,
  messageActor,
  messageContent,
  messageIdentity,
  normalizeGroupLogMemoryAdmission,
  normalizeGroupLogMemoryRevocation,
  normalizeMemoryType,
  now,
  readJson,
  safeSegment,
  ...
  uniqueStrings,
  verifyGroupLogLifecycleCurrentSourceEvidence,
  writeJsonAtomic,
...
export function providerDispatchOverrideFollowupCompletion(entry: any = {}, raw: any = {}) {
...
export function providerDispatchOverrideFollowupRepair(entry: any = {}, completion: any = {}, raw: any = {}) {
...
export function providerDispatchOverrideFollowupUsageRows(receipt: any = {}) {
  return [
...
export function providerDispatchOverrideFollowupRowId(row: any = {}) {
...
export function providerSwitchExecutionInputRows(input: any = {}) {
  if (Array.isArray(input)) return input;
  const rows = [
    ...(Array.isArray(input.rows) ? input.rows : []),
    ...(Array.isArray(input.items) ? input.items : []),
...
export function providerSwitchExecutionReceiptFromInput(entry: any = {}, raw: any = {}) {
...
export function providerSwitchDecisionReceiptFromInput(entry: any = {}, raw: any = {}, executionReceipt: any = {}) {
...
export function providerSwitchExecutionSessionBindingFromInput(entry: any = {}, raw: any = {}) {
...
export function providerSwitchExecutionRowId(row: any = {}) {
...
export function providerDispatchOverrideFollowupReceiptValidationInputRows(input: any = {}) {
  if (Array.isArray(input)) return input;
  const rows = [
    ...(Array.isArray(input.rows) ? input.rows : []),
    ...(Array.isArray(input.items) ? input.items : []),
...
export function providerDispatchOverrideFollowupReceiptValidationRowId(row: any = {}) {
...
export function ignoreMemoryReceiptRepairInputRows(input: any = {}) {
  if (Array.isArray(input)) return input;
  const rows = [
    ...(Array.isArray(input.rows) ? input.rows : []),
    ...(Array.isArray(input.items) ? input.items : []),
...
export function ignoreMemoryReceiptRepairRowId(row: any = {}) {
...
export function pressureMemoryProvenanceReceiptRepairInputRows(input: any = {}) {
  if (Array.isArray(input)) return input;
  const rows = [
    ...(Array.isArray(input.rows) ? input.rows : []),
    ...(Array.isArray(input.items) ? input.items : []),
...
export function pressureMemoryProvenanceReceiptRepairRowId(row: any = {}) {
...
export function summarizeProviderDispatchOverrideFollowupPolicyAttributions(attributions: any[] = []) {
...
export function summarizeProviderDispatchOverrideFollowupReceiptValidationPolicyAttributions(attributions: any[] = []) {
...
export function summarizeProviderSwitchExecutionPolicyAttributions(attributions: any[] = []) {
  const ordered = [...(attributions || [])]
...
export function scoreProviderSwitchExecutionRows(rows: any[] = [], options: any = {}) {
...
export function providerDispatchReliabilityNowMs(options: any = {}) {
...
export function providerDispatchReliabilityRound(value: any, digits = 4) {
  const number = Number(value || 0);
  const scale = 10 ** digits;
  return Math.round(number * scale) / scale;
}

export function providerDispatchReliabilityDecayWeight(at: any, options: any = {}) {
...
export function scoreProviderDispatchReliabilityRows(rows: any[] = [], options: any = {}) {
...
export function listProviderDispatchReliabilityDistillationLedgers(options: any = {}) {
