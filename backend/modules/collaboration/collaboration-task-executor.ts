...
function summarizeAckPreflightOutput(value: any) {
  let serialized = "";
...
function parseStructuredAckFallback(value: any, expectedMessageId: string) {
  const serialized = typeof value === "string" ? value : (() => { try { return JSON.stringify(value || ""); } catch { return ""; } })();
  ...
  for (let start = serialized.indexOf("{"); start >= 0; start = serialized.indexOf("{", start + 1)) {
    ...
    if (end > start) candidates.push(serialized.slice(start, end + 1));
  }
  const list = (input: any) => Array.from(new Set((Array.isArray(input) ? input : input == null ? [] : [input])
    .map(item => String(item || "").trim()).filter(Boolean))).slice(0, 100);
  for (const candidate of candidates) {
    try {
      const parsed = JSON.parse(candidate);
      if (String(parsed?.schema || "") !== "ccm-agent-ack-fallback-v1") continue;
      if (String(parsed?.message_id || "") !== expectedMessageId) continue;
      const understoodGoal = String(parsed?.understood_goal || parsed?.objective || "").trim();
      if (!understoodGoal) continue;
      return {
        status: "acknowledged",
...
export async function executeTask(task: any, ctx: CollabCtx, deps: any) {
  ...
  if (task.assign_type === "group" && task.group_id) {
    ...
    const communicationAttempt = Math.max(
      1,
      Number(task.execution_attempt || task.executionAttempt || 0),
      directMemoryDeliveryAttemptSequence || 0,
    ...
    const targetAnchorMessageId = taskConversationAnchorMessageId(task, `task-message:${task.id}`);
...
\`\`\``;
    ...
    if (directTaskSession?.turnCount > 0) {
      ...
      const startCommunicationHeartbeat = () => {
        if (communicationHeartbeat || !communicationEnvelope?.messageId || !communicationIdentity) return;
        const activePolicy = readAgentCommunicationPolicy();
      ...
      if (communicationEnvelope?.messageId && communicationPolicy.strictPreExecutionAckEnabled === true) {
        ...
        let acknowledged = getAgentCommunication(communicationEnvelope.messageId, { includeEvents: false, includeReceipts: false });
        if (!acknowledged || !["acknowledged", "executing"].includes(String(acknowledged.state))) {
          ...
          if (fallbackAck && communicationIdentity) {
            const recorded = recordAgentCommunicationReceipt(
              communicationEnvelope.messageId,
              "dispatch_ack",
            ...
            if (recorded?.accepted === true) {
              ...
              acknowledged = getAgentCommunication(communicationEnvelope.messageId, { includeEvents: false, includeReceipts: false });
            }
          }
        }
        if (!acknowledged || !["acknowledged", "executing"].includes(String(acknowledged.state))) {
    ...
    if (receipt && workDir) {
      ...
      const workspaceByPath = new Map(workspaceFiles.map((item: any) => [String(item?.path || item?.file || "").replace(/\\/g, "/"), item]));
      ...
      const recoveredFiles = receiptFiles
        .map((item: any) => String(item?.path || item?.file || item || "").replace(/\\/g, "/").trim())
        .filter(Boolean)
        .map((filePath: string) => workspaceByPath.get(filePath))
        .filter(Boolean);
      ...
      const recoveryTask = loadTasks().find((item: any) => item.id === task.id) || task;
      ...
      const merged = [...currentFiles, ...recoveredFiles, ...(adoptedRecoveryWorkspace ? workspaceFiles : [])].filter((item: any, index: number, items: any[]) => {
        const filePath = String(item?.path || item?.file || "").replace(/\\/g, "/");
        return filePath && items.findIndex((candidate: any) => String(candidate?.path || candidate?.file || "").replace(/\\/g, "/") === filePath) === index;
      });
      ...
      ? {
          ...
          verification: Array.from(new Set([
            ...
            ...(Array.isArray(projectReview?.verification_results)
              ? projectReview.verification_results.filter((item: any) => item?.status === "passed").map((item: any) => item.command || item.id)
              : []),
          ])),
          verificationResults: [
            ...(Array.isArray(receipt?.verificationResults) ? receipt.verificationResults : []),
            ...(Array.isArray(projectReview?.verification_results)
              ? projectReview.verification_results.filter((item: any) => item?.status === "passed").map((item: any) => ({
    ...
    if (ccmRevalidated) {
      const currentWorkItems = Array.isArray((loadTasks().find((item: any) => item.id === task.id) || task)?.work_items)
        ? (loadTasks().find((item: any) => item.id === task.id) || task).work_items
        : [];
      for (const item of currentWorkItems) {
        if (task.assign_type === "group" && item?.target && String(item.target) !== String(task.target_project)) continue;
