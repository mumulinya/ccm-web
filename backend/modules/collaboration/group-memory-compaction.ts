...
export async function compactGroupConversationMemory(input: {
  ...
  if (keepIndex <= summarizedThroughIndex + 1 && !partialSidecarSegment) return { compacted: false, memory, keepIndex, partialCompact };
  ...
  if (!shouldCompactPrimary || !messagesToCompact.length) return { compacted: false, memory: warningOnlyMemory, keepIndex, partialCompact, contextPressureWarning: preCompactWarning };
