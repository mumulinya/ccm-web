...
} from "./collaboration-coordination-ux-part-01";

export function buildUserReceiptReworkSummary(task: any, summary: any = {}, agentCoordination: any = null) {
  ...
  const memoryDispatchGates = Array.isArray(summary.memory_dispatch_gates || summary.memoryDispatchGates)
    ...
    : collectTaskMemoryDispatchFreshnessGates(task, { assignmentEvidence: assignments, execution: summary.execution || null });
  const globalMemoryReceiptGates = Array.isArray(summary.global_memory_receipt_gates || summary.globalMemoryReceiptGates)
    ...
    : collectTaskGlobalMemoryReceiptGates(task, { assignmentEvidence: assignments, execution: summary.execution || null });
  const globalMemoryHealthGates = Array.isArray(summary.global_memory_health_gates || summary.globalMemoryHealthGates)
    ...
    : collectTaskGlobalMemoryHealthGates(task, { assignmentEvidence: assignments, execution: summary.execution || null });
  const readPlanRevalidationGates = Array.isArray(summary.read_plan_revalidation_gates || summary.readPlanRevalidationGates)
    ...
    : collectTaskReadPlanRevalidationGates(task, { assignmentEvidence: assignments, execution: summary.execution || null });
  const postCompactReinjectionGates = Array.isArray(summary.post_compact_reinjection_gates || summary.postCompactReinjectionGates)
    ...
    : collectTaskPostCompactReinjectionGates(task, { assignmentEvidence: assignments, execution: summary.execution || null });
  const apiMicrocompactEditPlans = Array.isArray(summary.api_microcompact_edit_plans || summary.apiMicrocompactEditPlans)
    ...
    : collectTaskApiMicrocompactEditPlans(task, { assignmentEvidence: assignments, execution: summary.execution || null });
  const receiptTarget = (receipt: any) => String(receipt?.agent || receipt?.project || receipt?.target_project || receipt?.target || "").trim().toLowerCase();
  ...
  ].filter((item: any) => item && /receipt|ack|verification|missing_receipt|weak_receipt|memory_gate|read_plan|revalidation|post_compact|reinject|重注入|重读|当前源|记忆/i.test(`${item.rework_kind || item.reworkKind || item.kind || ""} ${item.source || ""} ${item.title || ""} ${item.reason || ""}`));
  const receiptRows = Array.isArray(agentCoordination?.receipt_quality) ? agentCoordination.receipt_quality : receipts.map((receipt: any) => ({
  ...
  const memoryGateSummary = agentCoordination?.memory_gate_summary || buildMemoryGateVisibleSummary({
    ...summary,
  ...
  const globalMemoryHealthGateSummary = agentCoordination?.global_memory_health_gate_summary || buildGlobalMemoryHealthGateVisibleSummary({
    ...summary,
  ...
  const readPlanRevalidationGateSummary = agentCoordination?.read_plan_revalidation_gate_summary || buildReadPlanRevalidationGateVisibleSummary({
    ...summary,
  ...
  const reinjectionGateSummary = agentCoordination?.post_compact_reinjection_gate_summary || buildPostCompactReinjectionGateVisibleSummary({
    ...summary,
  ...
  const apiMicrocompactSummary = agentCoordination?.api_microcompact_receipt_summary || buildApiMicrocompactReceiptVisibleSummary({
    ...summary,
  ...
  const rows: any[] = [];
  const add = (input: any = {}) => {
    const target = compactMemoryText(input.target || input.agent || input.project || "", 100);
    const reason = compactMemoryText(input.reason || input.detail || "", 220);
  ...
  for (const receipt of receipts) {
    ...
    if (strongReceiptTargets.has(receiptTarget(receipt))) continue;
  ...
  for (const row of Array.isArray(memoryGateSummary?.rows) ? memoryGateSummary.rows : []) {
    if (row.status !== "missing_receipt_reference") continue;
    add({
  ...
  for (const row of Array.isArray(globalMemoryHealthGateSummary?.rows) ? globalMemoryHealthGateSummary.rows : []) {
    ...
    add({
      ...
      missing: uniqueStrings(["全局记忆健康门禁声明"], row.missing_gate_ids || [], row.missing_ignore_gate_ids || [], row.blocked_global_memory_used_gate_ids || []),
    });
  }

  for (const row of Array.isArray(readPlanRevalidationGateSummary?.rows) ? readPlanRevalidationGateSummary.rows : []) {
    if (!["missing_receipt_reference", "missing_read_plan_reference", "missing_current_source_verification", "session_mismatch"].includes(row.status)) continue;
  ...
  for (const row of Array.isArray(reinjectionGateSummary?.rows) ? reinjectionGateSummary.rows : []) {
    if (!["missing_receipt_reference", "missing_candidate_reference", "missing_candidate_usage"].includes(row.status)) continue;
  ...
  for (const row of Array.isArray(apiMicrocompactSummary?.rows) ? apiMicrocompactSummary.rows : []) {
    if (!["missing_usage_declaration", "unsafe_native_applied", "session_mismatch"].includes(row.status)) continue;
  ...
  for (const assignment of assignments) {
    ...
    if (!agent) continue;
    const hasReceipt = receipts.some((receipt: any) => String(receipt.agent || receipt.project || receipt.target_project || "").toLowerCase() === String(agent).toLowerCase());
    if (!hasReceipt) {
      add({
        id: "missing_receipt",
        rework_kind: "missing_receipt",
  ...
  for (const notification of notifications) {
    ...
    if (!["failed", "blocked", "partial", "missing_receipt", "needs_info"].includes(status) && (!receiptStatus || receiptStatus === "done")) continue;
    const target = notification.task_id || notification.agent || notification.project || "";
    if (strongReceiptTargets.has(String(target).trim().toLowerCase())) continue;
    add({
  ...
  for (const item of Array.isArray(agentCoordination?.targeted_rework) ? agentCoordination.targeted_rework : []) {
    if (!/receipt|ack|verification|memory|记忆|reinject|重注入|压缩/i.test(`${item.id || ""} ${item.title || ""}`)) continue;
    if (strongReceiptTargets.has(String(item.target || "").trim().toLowerCase())) continue;
    add({
  ...
  const isReceiptGap = (item: any) => /receipt|回执|ACK|验证|verification|ack|memory|记忆|read[_ -]?plan|revalidation|重读|当前源|reinject|重注入|压缩/i.test(`${item.id || ""} ${item.title || ""} ${item.reason || ""}`);
  const gaps = rows
    .filter((item: any) => {
      const target = String(item.target || "").trim().toLowerCase();
      if (target && strongReceiptTargets.has(target)) return false;
      if (!target && strongReceiptTargets.size > 0 && isReceiptGap(item)) return false;
      return true;
    })
    .slice(0, 8)
    .map(({ key, ...item }) => item);
  const activeRework = continuationEvents.slice(-5).map((item: any) => ({
    target: compactMemoryText(item.target || item.agent || item.project || "", 100),
    title: item.title || (/missing_receipt/i.test(`${item.rework_kind || ""}`) ? "已发起补结果说明" : "已发起结果说明补充"),
    reason: sanitizeUserAgentProgressText(item.reason || item.detail || "等待执行成员补齐证据后重新验收", "等待执行成员补齐证据后重新验收", 220),
    at: item.at || "",
    status: item.status || "accepted",
  })).filter((item: any) => item.target || item.reason);
  const gapKeys = new Set(gaps.map(item => String(item.target || "").toLowerCase()).filter(Boolean));
  const resolved = activeRework
    .filter((item: any) => item.target && !gapKeys.has(String(item.target || "").toLowerCase()))
    .map((item: any) => {
      const receipt = receipts.find((row: any) => String(row.agent || row.project || row.target_project || "").toLowerCase() === String(item.target || "").toLowerCase());
  ...
  const targets = uniqueStrings([
    ...
    ...activeRework.map((item: any) => item.target),
    ...resolved.map((item: any) => item.target),
  ].filter(Boolean)).slice(0, 4);
  const status = gaps.length ? (activeRework.length ? "reworking" : "needs_rework") : resolved.some((item: any) => item.status === "passed") ? "passed" : "rechecking";
  return {
    ...
    gaps,
    active_rework: activeRework.filter((item: any) => item.target && gapKeys.has(String(item.target || "").toLowerCase())).slice(0, 5),
    resolved,
...
export function sanitizeDispatchLaunchText(value: any, fallback = "", max = 220) {
  let text = compactMemoryText(value || "", max);
  ...
  text = text
    ...
    .replace(/\s+/g, " ")
    .trim();
  return sanitizeUserAgentProgressText(text, fallback, max) || fallback;
}
...
export function normalizeGroupDispatchLaunchRowStatus(rawValue: any = "dispatched") {
  ...
  if (["blocked", "failed", "error"].includes(raw)) return { status: "failed", label: "待排查" };
  if (["queued", "pending"].includes(raw)) return { status: raw, label: "已入队" };
...
export function buildWorkerContinuationHandoff(task: any, targetProject = "", options: any = {}) {
  ...
  const latestFollowup = [...pending].reverse().find((item: any) => item?.message || item?.kind) || {};
  const kind = normalizeContinuationKind(last?.kind || latestFollowup.kind || "");
  ...
  if (!last?.at && !latestFollowup?.message && !replanRequired && interruption.requested !== true) return options.fallback || null;
  const latestUserChange = compactMemoryText(latestFollowup.message || last?.reason || last?.title || interruption.reason || "", 900);
  const currentGoal = compactMemoryText(task?.business_goal || task?.businessGoal || task?.title || "", 1000);
  const previousGoal = compactMemoryText(options.previous_goal || options.previousGoal || task?.title || "", 700);
  ...
  const preserved = uniqueStrings([
    task?.delivery_summary?.headline ? `上一轮交付摘要：${compactMemoryText(task.delivery_summary.headline, 220)}` : "",
    ...(Array.isArray(task?.delivery_summary?.actual_file_changes) ? task.delivery_summary.actual_file_changes.map((item: any) => item.path || item.file || item.name).filter(Boolean).slice(0, 5).map((path: string) => `已有文件证据：${path}`) : []),
    ...(Array.isArray(task?.delivery_summary?.verification_executed) ? task.delivery_summary.verification_executed.slice(0, 4).map((item: any) => `已有验证证据：${compactMemoryText(item, 180)}`) : []),
  ]);
...
export function extractMemoryDispatchFreshnessGate(memory: any): any {
  if (!memory || typeof memory !== "object") return null;
  if (memory.dispatch_freshness_gate?.schema === "ccm-child-agent-memory-dispatch-freshness-gate-v1") return memory.dispatch_freshness_gate;
  if (memory.group_memory) return extractMemoryDispatchFreshnessGate(memory.group_memory);
  if (memory.memory) return extractMemoryDispatchFreshnessGate(memory.memory);
  return null;
}

export function renderMemoryDispatchFreshnessGateForContract(memory: any, handoff: any = null) {
...
export function buildChildAgentDevelopmentContract(targetProject: string, taskText = "", options: any = {}) {
  ...
  const handoff = options.handoff || buildChildAgentWorkerHandoff(targetProject, taskText, options);
...
export function buildProjectAgentProfileContractLines(profile: any) {
