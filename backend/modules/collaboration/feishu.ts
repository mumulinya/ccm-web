...
export function buildFeishuReportCard(
  ...
  if (safeMarkdown.length > 12_000) throw new Error(`飞书卡片正文超过安全容量：${safeMarkdown.length}/12000`);
  const card: any = {
...
export function generateFeishuWebhookSignature(timestamp: string | number, secret: string) {
  const stringToSign = `${timestamp}\n${secret}`;
  return crypto.createHmac("sha256", stringToSign).update("").digest("base64");
}

export async function sendFeishuWebhookReportMessage(config: any, options: { title: string; markdown: string; timeoutMs?: number }): Promise<any> {
