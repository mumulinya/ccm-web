...
export function recordGroupPromptCacheUsage(input: any = {}) {
  ...
  return withFileLock(file, () => {
    ...
    const eventCore: any = {
      ...
      endpoint,
      model,
      provider_identity_checksum: String(input.providerIdentityChecksum || input.provider_identity_checksum || buildCcmProviderIdentityChecksum({ provider, protocol, endpoint, model })),
