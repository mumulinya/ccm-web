...
export function getLlmConfigIssue(config: any) {
  ...
  if (!["openai-compatible", "openai-responses", "anthropic-compatible", "auto"].includes(config.format)) return `暂不支持的主 Agent API 格式: ${config.format}`;
  return "";
}
...
export async function runGroupOrchestratorCore(input: GroupOrchestratorInput) {
  ...
  try {
    ...
    if (group.id && groupSessionId) appendUserVisibleAgentEvent({
