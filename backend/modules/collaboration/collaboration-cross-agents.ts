...
import {
  ...
  mergeThirdPartyMemoryUsageIntoReceipt,
} from "../../integrations/third-party-memory-snapshot";
import {
  buildTaskAgentContinuityBinding,
  extractGroupSessionMemoryBinding,
} from "../../tasks/agent-sessions-shared";
import {
  runNativeTestAgentDispatchBranch,
} from "./collaboration-cross-agents-part-02-part-02-native-test";
import { projectTestAgentHandoffForPersistence } from "../../test-agent/evidence-projection";
import {
...
// ===== merged from collaboration-cross-agents-part-01.ts =====
   ...
   if (preparedWorkDir.mode === "worktree") {
    ...
    if (targetName === coordinatorProject) {
      ...
      appendGroupMessage(groupId, {
        ...
        providerFailure: (result as any).providerFailure || null,
        providerFailureTechnical: (result as any).providerFailureTechnical || null,
        dispatchPolicy,
    ...
    if (communicationEnvelope?.messageId && typeof mention !== "string") mention.agent_communication_message_id = communicationEnvelope.messageId;
...
请直接回复本次请求：给出结论、必要的执行/修改说明、风险、汇总意见，或需要继续 @ 的成员。`;
    ...
    if (thirdPartyMemoryMcpEnabled && thirdPartyMemorySnapshot && parentSessionCapacityGate.provider_call_allowed === true) {
      ...
      thirdPartyMemoryMcpEnabled = (runtimeToolContext.audit?.internal_mcp || [])
        .some((item: any) => item.name === "ccm__knowledge_context" && item.state === "synced");
      if (!thirdPartyMemoryMcpEnabled) {
...
export async function executeMentionJobTryA(mention: any, env: CrossAgentEnv): Promise<string[]> {
  const { deps, groupId, group, sourceProject, output, configs, ctx, streamRes, depth, seenMentions, executionOrder, planMessageId, taskId, sourceTask, completedOutputsByAgent, processCrossAgents } = env;
  const {
    addGroupLog, addTaskLog, admitChildTypedMemoryDelivery, appendAgentQaTrace,
    appendGroupMessage, appendTaskTimelineEvent, attachExecutionWorkspace, attachInvokedSkillsToReceipt,
  ...
  let targetReceipt: any = L.targetReceipt || null;
  let tOutput = L.tOutput || "";
  let activeRuntime = L.activeRuntime || tAgentType;
  let targetFileChanges = L.targetFileChanges || null;
  let targetWorkEvents: any[] = L.targetWorkEvents || [];
  let targetNativeSessionId = L.targetNativeSessionId || "";
  let targetInvokedSkills: any[] = L.targetInvokedSkills || [];
  let testAgentNativeReport: TestAgentReport | null = L.testAgentNativeReport || null;
  let testAgentExecutionPlan: any = L.testAgentExecutionPlan || null;
  let testAgentPlanDispatch: any = L.testAgentPlanDispatch || null;
  let testAgentCliDispatch: any = L.testAgentCliDispatch || null;
  ...
  try {
      ...
      let targetProviderToolAccessEvidence: any = null;
      let targetProviderMemoryChannelEvidence: any = null;
      let targetMemoryContextConsumptionReceipt: any = null;
      let targetMemoryContextConsumptionRecovery: any = null;
      let targetProviderUsage: any = null;
      let lastTypedMemoryDispatchAdmission: any = { required: false };
      ...
      let targetInvokedSkills: any[] = [];
      let testAgentNativeReport: TestAgentReport | null = null;
      let testAgentExecutionPlan: any = null;
      ...
      if (nativeTestAgentDispatch) {
        ...
        await runNativeTestAgentDispatchBranch({
          ...
          if (!sameRuntimeResume) {
            activeTaskSession = taskId ? openTaskAgentSession({
              ...
              continuity: activeGroupSessionId.startsWith("gcs_")
                ? buildTaskAgentContinuityBinding({ scope: "group", scopeId: groupId, exactSessionId: activeGroupSessionId, project: targetName, agentType: activeRuntime })
                : null,
            }) : null;
          }
        ...
        const activeMemoryBinding: any = activeMemoryContextSnapshot?.context?.group_session_memory_binding || {};
        ...
        let finalDispatchReactiveCompactReceipt: any = null;
        let finalDispatchReactiveCompactCircuitBreaker: any = activeTaskSession && activeGroupSessionId.startsWith("gcs_")
          ? inspectTaskAgentFinalDispatchReactiveCompactCircuitBreaker(activeTaskSession.id, {
            groupId,
            groupSessionId: activeGroupSessionId,
            taskId,
        ...
        if (finalDispatchRecoveryRequested && finalDispatchReactiveCompactCircuitBreaker?.blocked !== true) {
          ...
          if (finalDispatchRecovery.recovered === true) {
            ...
            if (taskId) appendTaskTimelineEvent(taskId, {
        ...
        if (activeTaskSession && activeMemoryContextSnapshot?.snapshot_id) {
          ...
          if (finalGateBinding?.updated !== true) {
            const error: any = new Error(`FINAL_WORKER_DISPATCH_SNAPSHOT_BINDING_FAILED:${finalGateBinding?.reason || "unknown"}`);
          ...
          if (typeof mention !== "string") mention.task_agent_memory_context_snapshot = activeMemoryContextSnapshot;
        }
        if (!finalDispatchPayloadGateVerification.valid || finalDispatchPayloadGate.provider_call_allowed !== true) {
          ...
          const error: any = new Error(`FINAL_WORKER_DISPATCH_PAYLOAD_BLOCKED:${reason}`);
        ...
        const attemptOutput = await ctx.callAgentForGroupStream(targetName, attemptPrompt, tWorkDir, activeRuntime, {
          ...
          onRunnerRequestCreated: (requestId: string) => {
            ...
            if (communicationEnvelope?.messageId && communicationIdentity) {
              markAgentCommunicationRunnerStarted(communicationEnvelope.messageId, { runnerRequestId: targetRunnerRequestId, runtime: activeRuntime, worktreeRef: preparedWorkDir.mode === "worktree" ? preparedWorkDir.worktreePath || preparedWorkDir.workDir : "" });
              communicationHeartbeat = setInterval(() => {
                try { heartbeatAgentCommunication(communicationEnvelope.messageId, communicationIdentity, { phase: "executing" }); }
                catch (error: any) { if (taskId) addTaskLog(taskId, "warning", `Agent Communication心跳写入失败：${String(error?.message || error).slice(0, 240)}`); }
              }, readAgentCommunicationPolicy().agentHeartbeatIntervalMs);
          ...
          onDone: (opts: any) => {
            ...
            if (opts.providerMemoryChannelEvidence?.required === true) targetProviderMemoryChannelEvidence = opts.providerMemoryChannelEvidence;
            if (opts.memoryContextConsumptionReceipt) targetMemoryContextConsumptionReceipt = opts.memoryContextConsumptionReceipt;
            if (opts.memoryContextConsumptionRecovery) targetMemoryContextConsumptionRecovery = opts.memoryContextConsumptionRecovery;
        ...
        if (providerPromptTooLong && !providerPromptTooLongReactiveRetryAttempted) {
          ...
          if (retryCircuit?.blocked === true) {
            if (taskId) addTaskLog(taskId, "error", `${targetName} 最终上下文自动恢复断路器已打开，本会话不再重复 Provider PTL 重试`);
          } else {
...
export function handleExecuteMentionJobCatch(error: any, ctx: any) {
...
export async function executeMentionJobTryB(mention: any, env: CrossAgentEnv): Promise<string[]> {
  const { deps, groupId, group, sourceProject, output, configs, ctx, streamRes, depth, seenMentions, executionOrder, planMessageId, taskId, sourceTask, completedOutputsByAgent, processCrossAgents } = env;
  const {
    addGroupLog, addTaskLog, admitChildTypedMemoryDelivery, appendAgentQaTrace,
    appendGroupMessage, appendTaskTimelineEvent, attachExecutionWorkspace, attachInvokedSkillsToReceipt,
  ...
  try {
      ...
      let providerSwitchExecutionReceipt: any = null;
      ...
      if (targetReceipt) {
        if (taskId) updateTaskWorkItemFromReceipt(taskId, targetName, targetReceipt, targetFileChanges, tOutput, { ctx });
        if (laneExecutionId) {
