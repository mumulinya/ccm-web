// Behavior-freeze split from group-compaction-engine.ts (part 2/3).
...
import { readGroupMainContextUsageBaseline, recordGroupPromptCacheState, recordGroupPromptCacheUsage } from "./group-prompt-cache-break-detection";
...
import {
  extractHookAnchors,
  summarizeWithModel,
} from "./group-compaction-engine-part-01";
import { validateGroupSessionLifecycleRuntimeFence } from "./group-session-lifecycle-head";
...
export async function compactGroupConversationMemory(input: {
  ...
  const finalModelVisiblePayload = buildModelVisiblePayloadSnapshot({
    scope: "group",
    sessionId: `${groupId}:${groupSessionId}`,
    system: input.config?.modelVisibleSystemContext || input.config?.model_visible_system_context || input.config?.systemPrompt || input.config?.system_prompt || null,
    tools: input.config?.modelVisibleTools || input.config?.model_visible_tools || input.config?.toolSchemas || input.config?.tool_schemas || null,
  ...
  if (sessionMemoryCompactSelection?.selected === true && postCompactPayloadBudget.will_retrigger_next_turn === true) {
    const error: any = new Error(`群聊 Session Memory 压缩后仍超过阈值：${Number(postCompactPayloadBudget.true_post_compact_token_count || 0)}/${triggerTokens}`);
