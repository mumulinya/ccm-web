...
function planRequiresConfirmation(task: any) {
  const plans = [
    task?.intake_draft,
  ...
  return task?.intake_state === "awaiting_confirmation"
    || plans.some((plan: any) => plan?.requires_confirmation === true || plan?.control_state === "awaiting_confirmation");
}

function isStartupManualHold(task: any) {
  return task?.recovery_pending === true
    && String(task?.recovery?.mode || "") === "manual_startup_recovery";
}

function hasExplicitUserPause(task: any) {
  if (isStartupManualHold(task)) return false;
...
function authorizationReadinessRows(task: any) {
  return [
...
function hasAuthorizationBlock(task: any) {
  return authorizationReadinessRows(task).some((item: any) => {
...
function hasRealUserInputBlock(task: any) {
  if (isStartupManualHold(task)) return false;
...
function authorizationEvidence(task: any, forceAuto: boolean) {
  const evidence: string[] = [];
  if (task?.intake_state === "confirmed") evidence.push("intake_confirmed");
  if (task?.confirmed_at) evidence.push("plan_confirmed_at");
  if (task?.queued_at) evidence.push("queued_at");
  if (task?.started_at) evidence.push("started_at");
  if (task?.execution_lease?.acquired_at) evidence.push("execution_lease_acquired");
  if (forceAuto) evidence.push("operator_resume_override");
...
export function buildStartupTaskRecoveryDecision(task: any, forceAuto = false): StartupTaskRecoveryDecision {
  const status = taskStatus(task);
  ...
  if (planRequiresConfirmation(task)) {
    return decision(
      ...
      "执行前计划仍待用户确认。",
      "这轮任务还在等你确认执行计划，服务重启后没有自动开始。",
      "确认计划后，我会沿用同一任务上下文继续执行。",
      { requiresUser: true },
    );
  }
  ...
  if (task?.runtime_debt_cleanup) {
    return decision(
      task,
      "manual",
      "runtime_debt_requires_review",
      "任务已由运行治理中心暂停，需要人工确认恢复。",
      "这轮任务之前因运行状态异常被暂停，服务重启后仍保持等待确认。",
      "请先检查当前工作区和执行环境，再决定是否继续。",
      { requiresUser: true },
    );
  }
  if (isStartupManualHold(task) && !forceAuto) {
    return decision(
      ...
      "startup_manual_hold",
      "任务已在上一轮启动恢复中转为人工确认。",
      task?.recovery?.user_headline || "这轮任务仍在等待你确认是否继续，服务重启后没有自动恢复。",
      task?.recovery?.user_next_action || "确认继续后，我会复用原任务、计划和已有执行上下文。",
      { requiresUser: true },
    );
  }
  if (task?.recovery_pending === true && !isStartupManualHold(task)) {
    return decision(
      ...
      "任务带有人工恢复标记，不能由启动流程清除。",
      "这轮任务仍在等待你确认如何继续，服务重启后没有自动恢复。",
      "确认继续后，我会复用原任务、计划和已有执行上下文。",
      { requiresUser: true },
    );
  }
  if (hasExplicitUserPause(task)) {
    ...
    return decision(
      task,
      "manual",
      takeover ? "manual_takeover" : "user_paused",
      takeover ? "任务已由用户人工接管。" : "任务已由用户主动暂停。",
      takeover
        ? "这轮任务目前由你人工接管，服务重启后不会恢复自动调度。"
        : "这轮任务之前由你暂停，服务重启后仍保持暂停。",
      takeover ? "切回自动跟进后，我会继续协调执行与验收。" : "恢复任务后，我会从原计划和当前进度继续。",
      { requiresUser: true },
    );
  }
  if (hasRealUserInputBlock(task)) {
    return decision(
      ...
      "user_input_required",
      "任务仍缺少用户输入或确认。",
      "这轮任务还需要你的确认或补充，服务重启后没有自动继续。",
      "补充卡片中列出的信息后，我会接着原任务推进。",
      { requiresUser: true },
    );
  }
  ...
  if (!evidence.length) {
    return decision(
      ...
      "没有找到计划确认、入队或开始执行的持久化证据。",
      "我找到了未完成任务，但还不能确认它在重启前已经获准执行，所以没有自动开始。",
      "确认继续后，我会保留原计划和验收条件重新入队。",
      { requiresUser: true },
    );
  }
  ...
  return decision(
    task,
    "auto",
    isStartupManualHold(task) ? "explicit_manual_resume" : "authorized_incomplete_task",
    "任务已明确获准执行，且持久化记录证明它已确认、入队或开始运行。",
    "服务重启后，我已自动接上这轮任务，并重新核对目标、当前状态和验收条件。",
    "我会沿用原计划和执行上下文继续推进，完成后再给你最终总结。",
    {
...
export function buildStartupTaskRecoveryPlan(tasks: any[] = [], forceAuto = false) {
  const entries = (Array.isArray(tasks) ? tasks : []).map((task: any) => ({
    task,
...
export function runStartupTaskRecoveryDecisionSelfTest() {
