...
function buildTaskNotificationStatusSummary(agent: string, status: string, receiptStatus = "") {
  const name = sanitizeMainAgentRoleLanguage(String(agent || "执行成员").trim() || "执行成员");
...
export function sanitizeTaskNotificationUserText(value: any, fallback = "", max = 600) {
  const fallbackText = compactMemoryText(sanitizeMainAgentRoleLanguage(fallback || "执行成员已返回结果，我正在汇总验收。"), max);
  ...
  const text = sanitizeMainAgentRoleLanguage(compactMemoryText(source, max)
