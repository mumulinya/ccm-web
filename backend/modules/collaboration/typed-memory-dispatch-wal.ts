...
export const TYPED_MEMORY_DISPATCH_WAL_DIR = path.join(CCM_DIR, "group-typed-memory-dispatch-wal");
...
function recordChecksum(record: any) {
...
function acquireLock(file: string) {
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    try {
      ...
      if (!stale) throw new Error("typed memory dispatch WAL is locked by another process");
...
export function getTypedMemoryDispatchWalFile(groupId: string, groupSessionId: string, ticketId: string) {
  return path.join(getTypedMemoryDispatchWalScopeDir(groupId, groupSessionId), `${cleanPart(ticketId)}.json`);
}

export function getTypedMemoryDispatchWalScopeDir(groupId: string, groupSessionId: string) {
  const scopeId = `${cleanPart(groupId)}--${cleanPart(groupSessionId)}`;
  return path.join(TYPED_MEMORY_DISPATCH_WAL_DIR, scopeId);
}

export function readTypedMemoryDispatchWal(file: string) {
  try {
...
function mutateWal(file: string, expectedRevision: number | null, mutate: (record: any) => any) {
  const lock = acquireLock(file);
  try {
    const current = fs.existsSync(file) ? readTypedMemoryDispatchWal(file) : null;
    if (current && current.checksum_valid !== true) throw new Error("typed memory dispatch WAL checksum mismatch");
    if (expectedRevision !== null && Number(current?.revision || 0) !== expectedRevision) throw new Error("typed memory dispatch WAL CAS revision mismatch");
...
export function createTypedMemoryDispatchWal(input: any = {}) {
  ...
  if (!ticket?.ticket_id) return { required: false, created: false, reason: "dispatch_ticket_not_required" };
  ...
  if (!groupId || !groupSessionId.startsWith("gcs_")) throw new Error("typed memory dispatch WAL requires groupId--gcs_* identity");
...
export function transitionTypedMemoryDispatchWal(recordOrFile: any, nextState: string, evidence: any = {}) {
  ...
  return mutateWal(file, expectedRevision, current => {
    if (!current) throw new Error("typed memory dispatch WAL record missing");
    const state = String(current.state || "");
    if (state === nextState) return { ...current, ...evidence };
    if (TERMINAL_STATES.has(state)) throw new Error(`typed memory dispatch WAL is terminal: ${state}`);
    ...
    if (!(allowed[state] || []).includes(nextState)) throw new Error(`invalid typed memory dispatch WAL transition: ${state} -> ${nextState}`);
    const now = new Date().toISOString();
    return {
      ...current,
      ...evidence,
...
export function listTypedMemoryDispatchWal() {
  if (!fs.existsSync(TYPED_MEMORY_DISPATCH_WAL_DIR)) return [];
  const files: string[] = [];
  for (const scope of fs.readdirSync(TYPED_MEMORY_DISPATCH_WAL_DIR, { withFileTypes: true })) {
    if (!scope.isDirectory()) continue;
    const dir = path.join(TYPED_MEMORY_DISPATCH_WAL_DIR, scope.name);
    let entries: fs.Dirent[] = [];
    try { entries = fs.readdirSync(dir, { withFileTypes: true }); } catch { continue; }
    for (const entry of entries) {
      if (entry.isFile() && entry.name.endsWith(".json") && !entry.name.endsWith(".lock")) files.push(path.join(dir, entry.name));
    }
  }
  return files.map(readTypedMemoryDispatchWal).filter(Boolean);
}

export function pruneTypedMemoryDispatchWal(options: any = {}) {
  ...
  for (const record of listTypedMemoryDispatchWal()) {
    if (!TERMINAL_STATES.has(String(record.state || ""))) continue;
    const at = Date.parse(String(record.committed_at || record.updated_at || ""));
    if (Number.isFinite(at) && now - at >= retentionMs) {
      try { fs.unlinkSync(record.file); deleted.push(record.file); } catch {}
    }
  }
  return { deleted_count: deleted.length, deleted };
}

export function verifyTypedMemoryDispatchWal(record: any) {
  const issues: string[] = [];
  if (record?.schema !== WAL_SCHEMA) issues.push("unsupported_schema");
  if (String(record?.record_checksum || "") !== recordChecksum(record)) issues.push("record_checksum_mismatch");
  if (!String(record?.group_session_id || "").startsWith("gcs_")) issues.push("group_session_not_gcs");
  if (!String(record?.task_agent_session_id || "").startsWith("tas_")) issues.push("task_agent_session_not_tas");
  if (String(record?.state || "") !== "committed" && (!record?.recovery_payload?.dispatch_ticket || !record?.recovery_payload?.worker_context_packet)) issues.push("recovery_payload_missing");
