...
const GROUP_MEMORY_RELOAD_DIR = path.join(CCM_DIR, "group-memory-reload");
const GROUP_MEMORY_POST_COMPACT_DISPATCH_DIR = path.join(CCM_DIR, "group-memory-post-compact-dispatch");
const GROUP_MEMORY_POST_COMPACT_CANDIDATE_USAGE_DIR = path.join(CCM_DIR, "group-memory-post-compact-candidate-usage");
const GROUP_API_MICROCOMPACT_NATIVE_APPLY_PROOF_DIR = path.join(CCM_DIR, "group-api-microcompact-native-apply-proof");
const GROUP_API_MICROCOMPACT_NATIVE_APPLY_REQUEST_TELEMETRY_DIR = path.join(CCM_DIR, "group-api-microcompact-native-apply-request-telemetry");
...
function cleanGroupMemoryScopePart(value: string) {
  return String(value || "").replace(/[^a-zA-Z0-9._-]+/g, "-").slice(0, 120) || "unknown";
}

function getGroupSessionSidecarFile(root: string, groupId: string, sessionId = "") {
...
function readGroupReplayRepairLedgerSummary(groupId: string, sessionId = "") {
...
function readGroupReplayRepairWorkItemsSummary(groupId: string, sessionId = "") {
...
function replayRepairWorkItemStatusForMemory(item: any) {
  const status = String(item?.status || "").toLowerCase();
  if (["in_progress", "running", "claimed", "dispatching"].includes(status)) return "in_progress";
  if (["blocked", "needs_info", "needs_user", "waiting"].includes(status)) return "blocked";
  if (["completed", "done", "resolved", "ok"].includes(status)) return "completed";
...
function replayRepairCandidatePriorityRank(item: any) {
...
function buildGroupCompactBoundaryHistorySummary(memory: any = {}) {
...
function buildChildAgentTypeSummary(memory: any = {}) {
...
function getGroupMemoryBackupFile(groupId: string, sessionId = "") {
...
function buildChildGlobalAgentMemoryContext(query: string, options: any = {}) {
...
function hashSessionMemoryText(value: any, length = 16) {
  return crypto.createHash("sha256").update(String(value || "")).digest("hex").slice(0, length);
}

function verifyGroupSessionMemoryFactSupersessionGraphForContext(graph: any) {
...
function verifyGroupSessionMemoryModelExtractionDeliveryEvidenceForContext(evidence: any) {
...
function globalMemorySuppressionKey(value: any = {}) {
...
function buildCrossGroupGlobalMemorySuppressionIndex(currentGroupId: string, options: any = {}) {
...
function buildCrossGroupGlobalMemorySuppressionForItem(item: any, index: any = {}, options: any = {}) {
...
function applyCrossGroupGlobalMemorySuppression(arbitration: any = {}, suppression: any = {}) {
...
function summarizeCrossGroupGlobalMemorySuppression(items: any[] = [], index: any = {}) {
...
function writeTextAtomic(file: string, text: string) {
  fs.mkdirSync(path.dirname(file), { recursive: true });
  const temp = `${file}.${process.pid}.${Date.now()}.tmp`;
...
function globalMemoryArbitrationSignature(groupId: string, targetProject: string, item: any = {}, arbitration: any = {}) {
...
function summarizeGroupGlobalMemoryArbitrationLedger(groupId: string, ledger: any, recordedRows: any[] = []) {
  const entries = Array.isArray(ledger.entries) ? ledger.entries : [];
...
function renderGlobalMemoryArbitrationTypedMemoryBody(entries: any[] = [], options: any = {}) {
  const lines = [
...
function memoryArbitrationTokens(value: any) {
...
function memoryArbitrationTimestamp(value: any, messagesById: Map<string, any> = new Map()) {
...
function memoryArbitrationTextForItem(type: string, item: any) {
  if (!item) return "";
...
function collectChildGlobalMemoryLocalEvidence(options: any = {}) {
...
function memoryTextsMayConflict(globalText: string, localText: string) {
  return scoreMemorySemanticContradiction(globalText, localText).conflict === true;
}

function uniqueMemoryArbitrationValues(values: any[] = [], limit = 24) {
  return [...new Set(values.map(value => String(value || "").trim()).filter(Boolean))].slice(0, limit);
}

function memoryArbitrationEntities(value: any) {
  const text = String(value || "");
...
function memoryArbitrationSignals(value: any) {
  const text = String(value || "");
  return {
...
function intersectionValues(a: string[] = [], b: string[] = []) {
  const right = new Set(b);
  return a.filter(value => right.has(value));
}

function scoreMemorySemanticContradiction(globalText: string, localText: string, options: any = {}) {
...
function arbitrateChildGlobalAgentMemoryItem(item: any, localEvidence: any[] = []) {
...
function groupSessionMemorySectionTokenLimit(header: string) {
...
function splitGroupSessionMemorySections(markdown: string) {
...
function truncateGroupSessionMemorySection(section: { header: string; lines: string[] }, maxTokens: number) {
...
function groupSessionMemoryToolCallCount(message: any = {}) {
...
function groupSessionMemoryToolCallsSince(messages: any[], sinceMessageId = "") {
  ...
  const start = sinceMessageId
    ...
    : -1;
  return rows.slice(Math.max(0, start + 1)).reduce((sum: number, message: any) => sum + groupSessionMemoryToolCallCount(message), 0);
}

function groupSessionMemoryLastAssistantTurnHasToolCalls(messages: any[]) {
  const rows = Array.isArray(messages) ? messages : [];
...
function renderGroupSessionMemoryMarkdown(groupId: string, memory: any = {}) {
...
function summarizeGroupSessionMemorySnapshot(snapshot: any = {}) {
...
function mergeToolGrantSets(...sets: any[]) {
...
function countToolGrantSet(set: any = {}) {
  return (Array.isArray(set.mcp) ? set.mcp.length : 0) + (Array.isArray(set.skill) ? set.skill.length : 0);
}

function hasToolGrantSet(set: any = {}) {
  return countToolGrantSet(set) > 0;
}

function normalizeToolContinuitySkill(value: any, source = "", at = "") {
...
function compactToolContinuityStatus(row: any = {}, kind = "mcp", source = "") {
...
function compactToolContinuityReadiness(readiness: any = {}, source = "") {
...
function extractToolGrantSet(value: any = {}) {
  return mergeToolGrantSets(value?.allowedTools || value?.allowed_tools || value?.tools || value);
}

function buildGroupToolContinuityConfigSources(groupId: string, memory: any = {}) {
  const sources: any[] = [];
...
function renderGroupToolContinuityMarkdown(snapshot: any = {}) {
  const lines = [
...
function buildGroupToolContinuitySnapshot(groupId: string, memory: any = {}, options: any = {}) {
  const generatedAt = String(options.generatedAt || options.generated_at || new Date().toISOString());
...
function summarizeGroupToolContinuitySnapshot(snapshot: any = {}) {
  if (!snapshot?.schema) return null;
  const { markdown, ...rest } = snapshot;
  return {
    ...rest,
...
function compactFileReferenceId(groupId: string, type: string, filePath: string) {
...
function compactFileReferenceKind(filePath: string) {
  try {
...
function compactFileReferenceEntry(groupId: string, type: string, filePath: any, reason: string, extra: any = {}) {
...
function uniqueCompactFileReferences(refs: any[] = [], limit = 40) {
  return uniqueByKey(
    refs.filter(Boolean),
...
function compactFileReferenceReadPlanPriority(reference: any = {}) {
...
function writeGroupCompactFileReferenceLedger(groupId: string, ledger: any) {
  const file = getGroupCompactFileReferenceLedgerFile(groupId);
  fs.mkdirSync(path.dirname(file), { recursive: true });
  const temp = `${file}.${process.pid}.${Date.now()}.tmp`;
...
function compactReferenceFingerprint(references: any[] = []) {
...
function compactFileReferenceTextForDetection(reference: any = {}) {
  return [
...
function compactFileReferenceMentioned(text: string, reference: any = {}) {
...
function compactFileReferenceReadPlanMentionTokens(entry: any = {}) {
  return [
    entry.read_plan_id,
    entry.reference_id,
    entry.path,
...
function compactFileReferenceReadPlanMentioned(text: string, entry: any = {}) {
  const lower = String(text || "").replace(/\\/g, "/").toLowerCase();
...
function compactReadPlanRevalidationGateRow(row: any = {}, action: string) {
  return {
    read_plan_id: row.read_plan_id || "",
    reference_id: row.reference_id || "",
    type: row.type || "",
...
function buildChildAgentSessionBinding(groupId: string, targetProject: string, task = "", options: any = {}) {
...
export function buildGroupMemoryContext(memory: any) {
  if (!memory || (!memory.goal && !memory.summary && !memory.messageDigest && !memory.conversationSummary && !memory.sessionMemory?.schema && !memory.toolContinuity?.schema && !memory.compaction?.postCompactReinject?.invokedSkillAttachments?.length && !memory.compaction?.postCompactReinject?.planAttachment && !memory.compaction?.postCompactReinject?.dynamicContextDeltaAttachment && !memory.decisions?.length && !memory.completed?.length && !memory.blocked?.length && !memory.workerLedger?.length && !Object.keys(memory.agentMemories || {}).length && !memory.openQuestions?.length && !memory.nextActions?.length)) {
    return "";
  }
  const lines = [
...
function resolveGroupProjectMemoryRoot(project: string, options: any = {}) {
...
function removeSessionDirectoryWithin(root: string, target: string) {
...
function mergeFactAnchorList(existing: any[] = [], incoming: any[] = [], limit = 300) {
...
function extractGroupFactAnchors(messages: any[]) {
  const anchors: any[] = [];
...
function extractPersistentRequirementsFromAnchors(anchors: any[]) {
...
function getCompactBoundaryIndex(memory: any, messages: any[]) {
...
function clearUntrustedGroupCompactionState(memory: any, reason: string) {
...
function persistGroupMemoryResumeEffectiveTokenBaseline(
  groupId: string,
  groupSessionId: string,
  allMessages: any[],
  memory: any,
...
function refreshGroupConversationMemorySnapshot(groupId: string, allMessages: any[], memory: any, options: any = {}) {
...
function getGroupMessagesFileHint(groupId: string, sessionId = "") {
  return getGroupChatSessionMessagesFile(groupId, sessionId);
}

function hashGroupMemoryFileWindow(file: string, stat: fs.Stats, maxBytes = 256_000) {
...
function buildGroupMemorySourceEntry(id: string, sourcePath: string, purpose: string, extra: any = {}) {
...
function readGroupMemoryReloadLedger(groupId: string, sessionId = "") {
...
function writeGroupMemoryReloadLedger(groupId: string, ledger: any, sessionId = "") {
  const file = getGroupMemoryReloadLedgerFile(groupId, sessionId);
  fs.mkdirSync(path.dirname(file), { recursive: true });
  const temp = `${file}.${process.pid}.${Date.now()}.tmp`;
  fs.writeFileSync(temp, JSON.stringify({
...
function writeGroupPostCompactDispatchLedger(groupId: string, ledger: any, sessionId = "") {
  const file = getGroupPostCompactDispatchLedgerFile(groupId, sessionId);
  fs.mkdirSync(path.dirname(file), { recursive: true });
  const temp = `${file}.${process.pid}.${Date.now()}.tmp`;
  fs.writeFileSync(temp, JSON.stringify({
...
function writeGroupPostCompactCandidateUsageLedger(groupId: string, ledger: any, sessionId = "") {
  const file = getGroupPostCompactCandidateUsageLedgerFile(groupId, sessionId);
  fs.mkdirSync(path.dirname(file), { recursive: true });
...
function writeGroupApiMicrocompactNativeApplyProofLedger(groupId: string, ledger: any, sessionId = "") {
  const file = getGroupApiMicrocompactNativeApplyProofLedgerFile(groupId, sessionId);
  fs.mkdirSync(path.dirname(file), { recursive: true });
...
function writeGroupApiMicrocompactNativeApplyRequestTelemetryLedger(groupId: string, ledger: any, sessionId = "") {
  const file = getGroupApiMicrocompactNativeApplyRequestTelemetryLedgerFile(groupId, sessionId);
  fs.mkdirSync(path.dirname(file), { recursive: true });
  const entries = (Array.isArray(ledger.entries) ? ledger.entries : []).slice(-320);
  const temp = `${file}.${process.pid}.${Date.now()}.tmp`;
...
function normalizePostCompactUsageState(value: any) {
...
function postCompactCandidateStatsKey(row: any = {}, targetProject = "") {
...
function buildPostCompactCandidateEntry(groupId: string, input: any = {}, row: any = {}) {
...
function usageRecommendationForStats(stats: any = {}) {
...
function normalizeApiMicrocompactNativeApplyProofStatus(row: any = {}) {
...
function buildApiMicrocompactNativeApplyProofEntry(groupId: string, input: any = {}, receiptRow: any = {}, planRow: any = {}) {
...
function apiMicrocompactNativeApplyProofStatsKey(entry: any = {}) {
  return [
...
function apiMicrocompactNativeApplyProofTotals(entries: any[] = []) {
...
function normalizeApiMicrocompactNativeApplyTelemetryStatus(row: any = {}) {
...
function stableApiMicrocompactJson(value: any): string {
...
function stableApiMicrocompactChecksum(value: any, length = 24) {
  return crypto.createHash("sha256").update(stableApiMicrocompactJson(value)).digest("hex").slice(0, length);
}

function apiMicrocompactHeaderValue(headers: any, name: string) {
  if (!headers) return "";
...
function apiMicrocompactBetaHeadersFromHeaders(headers: any) {
  const raw = [
...
function uniqueApiMicrocompactStrings(...values: any[]) {
  const seen = new Set<string>();
  const result: string[] = [];
...
function buildApiMicrocompactNativeApplyTelemetryEntry(groupId: string, input: any = {}, receiptRow: any = {}, row: any = {}) {
  if (!row || typeof row !== "object") return null;
...
function apiMicrocompactNativeApplyTelemetryTotals(entries: any[] = []) {
  return entries.reduce((acc: any, entry: any) => {
...
function apiMicrocompactNativeApplyTelemetrySourceCounts(entries: any[] = []) {
  return entries.reduce((acc: any, entry: any) => {
...
function apiMicrocompactNativeApplyTelemetryStatsKey(entry: any = {}) {
  return [
    String(entry.target_project || "").trim().toLowerCase(),
    String(entry.plan_checksum || "").trim(),
...
function apiMicrocompactNativeApplyTelemetryMatchesProof(telemetry: any = {}, proof: any = {}) {
...
function enrichApiMicrocompactNativeApplyProofWithTelemetry(entry: any = {}, telemetryEntries: any[] = [], options: any = {}) {
...
function buildStableSourceFingerprint(sourceManifest: any = {}) {
...
function buildSourceManifestSnapshot(sourceManifest: any = {}) {
  const entries = Array.isArray(sourceManifest.entries) ? sourceManifest.entries : [];
...
function diffSourceManifestSnapshots(previousEntries: any[] = [], currentEntries: any[] = []) {
...
function buildGroupMemoryDispatchFreshnessGate(input: any = {}) {
  const sourceManifest = input.sourceManifest || input.source_manifest || {};
...
function normalizePostCompactReinjectionRows(plan: any = {}) {
...
function buildGroupMemoryPostCompactReinjectionGate(input: any = {}) {
...
function resolvePostCompactBoundaryMarkerParts(groupId: string, input: any = {}) {
...
function recordGroupPostCompactFirstDispatchMarker(groupId: string, input: any = {}) {
  const generatedAt = String(input.generatedAt || input.generated_at || new Date().toISOString());
  const groupSessionId = String(input.groupSessionId || input.group_session_id || "default");
...
const GROUP_MEMORY_AUTO_COMPACT_DEBOUNCE_MS = Math.max(
  250,
  Number(process.env.CCM_GROUP_MEMORY_AUTO_COMPACT_DEBOUNCE_MS || 2500)
);
const groupMemoryAutoCompactTimers = new Map<string, ReturnType<typeof setTimeout>>();
const groupMemoryAutoCompactRunning = new Set<string>();
const groupMemoryAutoCompactPending = new Set<string>();
let groupMemoryAutoCompactHookRegistered = false;

function loadGroupMemoryCompactionConfig(overrides: any = {}) {
  let config: any = {};
  try {
...
function normalizeDynamicContextToolScope(value: any = {}) {
...
function isGroupModelCompactionEnabled(config: any) {
  return config?.memoryCompactionUseModel === true
    || String(config?.memoryCompactionMode || "").toLowerCase() === "hybrid";
}

function buildBackgroundCompactionState(input: any = {}) {
  return {
...
function pressureMemoryProvenanceDisciplineStatus(value: any) {
  return String(value || "").trim().toLowerCase();
}

function pressureMemoryProvenanceDisciplineUnderRepair(value: any = {}) {
...
function buildPressureMemoryProvenanceReceiptDiscipline(input: any = {}, options: any = {}) {
...
function boundedTypedMemoryDeliveryInteger(value: any, fallback: number, min: number, max: number) {
  const parsed = Number(value);
  return Math.min(max, Math.max(min, Math.floor(Number.isFinite(parsed) ? parsed : fallback)));
}

function typedMemoryDeliveryLineCount(value: any) {
  const text = String(value || "");
  return text ? text.split("\n").length : 0;
}

function truncateTypedMemoryDeliveryContent(source: string, limits: { maxLines: number; maxBytes: number; maxTokens: number }) {
...
export function buildAgentMemoryContextBundle(groupId: string, targetProject: string, task = "", options: any = {}) {
  ...
  let compactHead = groupSessionId.startsWith("gcs_") ? readGroupCompactHead(groupId, groupSessionId) : null;
  if (ignoreMemory) {
    const bundle: any = {
      ...
      group_state: {
        ...
        typedMemory: {
          ...
          recall: {
            ...
            reason: "user_requested_ignore_memory",
            indexFile: "",
          memoryDir: getGroupTypedMemoryDir(typedMemoryScopeId),
            recalled: [],
            surfaced: [],
          },
        },
      ...
      raw_sources: {
        ...
        group_typed_memory_dir: getGroupTypedMemoryDir(typedMemoryScopeId),
      },
    };
  ...
  const postTurnSummaryBackfill = backfillGroupPostTurnSummaries(groupId, groupSessionId, allMessages, { maxMessages: 500 });
  const postTurnSummaryLedger = postTurnSummaryBackfill.ledger || readGroupPostTurnSummaries(groupId, groupSessionId, { limit: 20 });
  ...
  compactHead = resumePreparation.compactHeadRecovery?.head || (groupSessionId.startsWith("gcs_") ? readGroupCompactHead(groupId, groupSessionId) : null);
  const typedMemoryRecallLedgerScope = buildChildTypedMemoryRecallLedgerScope(project, sessionBinding, memory, options);
  const postTurnSummaryDeliveryCapsule = buildGroupPostTurnSummaryDeliveryCapsule({
    groupId,
    groupSessionId,
  ...
  const selectedPostTurnSummaryIds = new Set((postTurnSummaryDeliveryCapsule?.selected_summaries || []).map((row: any) => String(row.summary_id || "")));
  const deliveredPostTurnSummaries = (Array.isArray(postTurnSummaryLedger?.latest) ? postTurnSummaryLedger.latest : [])
    .filter((row: any) => !postTurnSummaryDeliveryCapsule || selectedPostTurnSummaryIds.has(String(row.summary_id || "")))
  ...
  const timeBasedToolResultProjection = buildGroupTimeBasedToolResultProjection(rawProjectedMessages, {
    groupId,
    groupSessionId,
  ...
  const childCompactEpoch = buildGroupCompactEpoch(String(
    memory.compactBoundary?.boundaryId
      || memory.compactBoundary?.boundary_id
  ...
  const typedLogDistillation = distillGroupMessagesToTypedMemoryUntilCaughtUp(typedMemoryScopeId, allMessages, memory, {
  ...
  const globalClaudeMemoryImport = options.includeGlobalClaudeMemory === false || options.include_global_claude_memory === false
    ? null
    : importGlobalClaudeMemoryToGroupTypedMemory(typedMemoryScopeId, {
  ...
  const projectMemoryImport = projectMemoryRoot
    ? importProjectMemoryFilesToGroupTypedMemory(typedMemoryScopeId, projectMemoryRoot, {
      project,
      settingSources: options.settingSources ?? options.setting_sources,
  ...
  const typedMemoryLoadPlan = buildGroupTypedMemoryLoadPlan(typedMemoryScopeId, {
  ...
  const recentTools = [
    ...
    ...(agentMemory.recentReceipts || []).flatMap((item: any) => [
  ...
  const ledgerAlreadySurfaced = getAlreadySurfacedGroupTypedMemory(typedMemoryScopeId, typedMemoryRecallLedgerScope.scope);
  ...
  const globalAgentMemoryRecall = buildChildGlobalAgentMemoryContext(
    [task, memory.goal, memory.currentPhase, project].filter(Boolean).join("\n"),
    {
      ...options,
      groupId,
      targetProject: project,
  ...
  const globalMemoryArbitrationDistillation: any = distillGroupGlobalMemoryArbitrationToTypedMemory(typedMemoryScopeId, {
    generatedAt,
  ...
  const effectiveTypedMemoryLoadPlan = globalMemoryArbitrationDistillation?.index?.schema
    ? buildGroupTypedMemoryLoadPlan(typedMemoryScopeId, {
      maxEntries: options.maxTypedMemoryLoadEntries || options.max_typed_memory_load_entries,
      query: typedMemoryRecallQuery,
      targetPaths: typedMemoryTargetPaths,
    })
  ...
  const typedMemoryRecallScopeStatsBefore = getGroupTypedMemoryRecallScopeStats(typedMemoryScopeId, typedMemoryRecallLedgerScope.scope);
  ...
  const typedMemoryDeliveryLease = buildWorkerTypedMemoryDeliveryLease(typedMemoryDeliveryCapsule, {
    query: typedMemoryRecallQuery,
  ...
  const deliveredTypedMemoryRelPathSet = new Set(deliveredTypedMemoryRelPaths.map((item: any) => String(item || "").toLowerCase()));
  const deliveredEffectiveTypedMemoryRecall = {
    ...effectiveTypedMemoryRecall,
    recalled: (effectiveTypedMemoryRecall.recalled || []).filter((doc: any) => deliveredTypedMemoryRelPathSet.has(String(doc.relPath || doc.rel_path || "").toLowerCase())),
  ...
  const effectiveProviderRankingCompactRepairReceiptRecall = {
    ...
    recalledThisTurn: providerRankingCompactRepairReceiptRecall.active === true
      && (deliveredEffectiveTypedMemoryRecall.surfaced || []).some((item: any) => String(item || "").toLowerCase() === PROVIDER_RANKING_PROVENANCE_COMPACT_REPAIR_RECEIPT_MEMORY_REL_PATH),
    surfacedRelPaths: (deliveredEffectiveTypedMemoryRecall.surfaced || []).filter((item: any) =>
      [
        PROVIDER_RANKING_PROVENANCE_COMPACT_REPAIR_RECEIPT_MEMORY_REL_PATH,
        PROVIDER_RANKING_MEMORY_USAGE_RECEIPT_DISCIPLINE_REL_PATH,
    ...
    memoryUsageReceiptDisciplineRecalledThisTurn: providerRankingCompactRepairReceiptRecall.active === true
      && (deliveredEffectiveTypedMemoryRecall.surfaced || []).some((item: any) => String(item || "").toLowerCase() === PROVIDER_RANKING_MEMORY_USAGE_RECEIPT_DISCIPLINE_REL_PATH),
  };
  const effectivePostCompactReinjectionRepairReceiptRecall = {
    ...
    recalledThisTurn: postCompactReinjectionRepairReceiptRecall.active === true
      && (postCompactReinjectionRepairReceiptRecall.docRelPaths || []).some((relPath: any) =>
        (deliveredEffectiveTypedMemoryRecall.surfaced || []).some((item: any) => String(item || "").toLowerCase() === String(relPath || "").toLowerCase())
      ),
    surfacedRelPaths: (deliveredEffectiveTypedMemoryRecall.surfaced || []).filter((item: any) =>
      (postCompactReinjectionRepairReceiptRecall.docRelPaths || []).some((relPath: any) =>
  ...
  const pressureProvenanceDispatchFeedbackPolicy = buildPressureProvenancePreDispatchComplianceDispatchPolicy(groupId, {
    targetProject: project,
    agentType: options.agentType || options.agent_type || "unknown",
    generatedAt,
  ...
  const typedMemoryLedger = readGroupTypedMemoryRecallLedger(typedMemoryScopeId);
  ...
  const relevantHistoricalEvidence = buildRelevantHistoricalGroupContext(
    projectedMessages,
    boundaryIndex,
  ...
  const {
    factSupersessionGraph: _auditGraph,
    modelExtractionReceipt: persistedModelReceipt,
    modelMergeQuality: persistedMergeQuality,
    modelExtractionReplayEvidence: persistedReplayEvidence,
    ...sessionMemoryCore
  ...
  const sessionMemorySnapshot = persistedSessionMemorySnapshot?.schema ? {
    ...
    factSupersession: persistedSupersessionGraph?.schema ? {
      ...
      activeFacts: (persistedSupersessionGraphValid && Array.isArray(persistedSupersessionGraph.activeFacts) ? persistedSupersessionGraph.activeFacts : []).map((fact: any) => ({
  ...
  const providerNativeCompactSessionGenerationFence = providerNativeCompactSessionCapacityReady
    ...
    : null;
  const apiMicrocompactNativeApplyPlan = buildGroupApiMicrocompactNativeApplyPlan(storedApiMicroCompactEditPlan || {}, {
    groupId,
    groupSessionId,
    targetProject: project,
    agentType: options.agentType || options.agent_type || "unknown",
  ...
  const bundle: any = {
    ...
    compaction: {
      ...
      promptCacheBreakDetection: groupSessionId.startsWith("gcs_")
        ? readGroupPromptCacheBreakDetection(groupId, groupSessionId)
        : null,
    ...
    group_state: {
      ...
      postTurnSummaries: {
        ...
        latest: deliveredPostTurnSummaries.map((row: any) => ({
    ...
    resume_context: {
      ...
      verified: resumeProjection.verified === true,
      projectedMessageCount: Number(resumeProjection.projectedMessageCount || projectedMessages.length),
      text: compactPreserveLines(
        buildBoundedRecentGroupContext(projectedMessages, Math.min(8, Math.max(3, Number(options.fullCount || options.full_count || 6)))),
        Number(options.maxResumeContextChars || options.max_resume_context_chars || 4200)
      ),
      timeBasedToolResultProjection: timeBasedToolResultProjection.receipt,
      timeBasedThinkingProjection: timeBasedThinkingProjection.receipt,
    },
    ...
    raw_sources: {
      ...
      global_agent_memory_file: globalAgentMemoryRecall?.file || GLOBAL_AGENT_MEMORY_FILE,
      ...
      provider_native_compact_execution_receipt_ledger_file: apiMicrocompactNativeApplyProofLedger.platform_execution_receipts?.ledger_file || getProviderNativeCompactExecutionReceiptLedgerFile(groupId, groupSessionId),
      ...
      group_compact_boundary_journal_file: getGroupMemoryBoundaryJournalFile(groupId, groupSessionId),
      group_resume_projection_proof_file: getGroupMemoryResumeProofFile(groupId, groupSessionId),
      group_post_turn_summary_ledger_file: getGroupPostTurnSummaryLedgerFile(groupId, groupSessionId),
  ...
  bundle.context_budget = buildContextBudget({ context: renderedWithReferences, maxChars: 36_000, maxTokens: 90_000 });
  ...
  bundle.rendered_text = compactPreserveLines(renderedWithReferences, Math.min(requestedRenderedChars, payloadSafeRenderedChars));
  return bundle;
}
...
function findMemoryArtifactBySchema(value: any, schema: string, seen = new Set<any>()): any {
...
function runnerRequestHasDurableReturnEvidence(record: any) {
...
function recoverDeliveryReceiptFromRunnerWal(record: any) {
  const runnerRequestId = String(record.runner_request_id || "");
...
function tokenizeGlobalGroupMemoryQuery(value: any) {
  const text = String(value || "").toLowerCase();
  const tokens = new Set<string>();
...
function globalGroupMemoryCorpus(group: any, memory: any) {
...
function scoreGlobalGroupMemoryCandidate(group: any, memory: any, messages: any[], query = "") {
...
function latestGroupMessageTimestamp(messages: any[] = []) {
...
function normalizeGlobalGroupMemoryMembers(group: any) {
...
function importGroupProjectMemoriesForMembers(groupId: string, group: any, options: any = {}) {
