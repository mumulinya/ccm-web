...
export const MISSION_EVIDENCE_CONTRACT_VERSION = 1;

/** 与 global-mission 门禁共用的"实质性检查项"集合：命中其一才算有真实交付证据。 */
export const SUBSTANTIVE_GATE_CHECK_IDS = new Set([
  "actual_changes",
  "actual_diff",
  "verification",
...
export function buildMissionEvidenceContract(summary: any): MissionEvidenceContract {
  ...
  const failedCount = toCount(gate?.failed_count)
    ...
    || gateChecks.filter((item: any) => item?.ok === false || item?.pass === false).length;
  ...
  return {
    version: MISSION_EVIDENCE_CONTRACT_VERSION,
...
export function readMissionEvidenceContract(task: any): MissionEvidenceContract | null {
  ...
  if (toCount(raw.version) !== MISSION_EVIDENCE_CONTRACT_VERSION) return null;
  if (typeof raw.acceptance_gate_passed !== "boolean") return null;
  return {
    version: MISSION_EVIDENCE_CONTRACT_VERSION,
...
export function missionEvidenceContractStrong(contract: MissionEvidenceContract): boolean {
  if (!contract.acceptance_gate_passed) return false;
  if (contract.substantive_gate_check_passed) return true;
  if (contract.verification_executed_count > 0 && contract.verification_failed_count === 0) return true;
  if (contract.verification_source_gate_passed && contract.external_runner_verification_count > 0) return true;
  if (contract.independent_review_gate_passed && contract.independent_review_evidence_count > 0) return true;
  return false;
}

/** 契约版子任务交付门禁：对应旧路径 globalMissionChildGatePassedFromEvidence 的结构化部分。 */
export function missionEvidenceContractGatePassed(contract: MissionEvidenceContract, requirements: {
  requiresCodeChanges: boolean;
  requiresVerification: boolean;
}): boolean {
  if (!missionEvidenceContractStrong(contract)) return false;
  if (requirements.requiresCodeChanges && contract.actual_file_change_count <= 0) return false;
  if (requirements.requiresVerification && contract.verification_executed_count <= 0) return false;
  if (contract.verification_failed_count > 0) return false;
  if (!contract.verification_required_gate_passed) return false;
  if (requirements.requiresVerification && !contract.verification_source_gate_passed) return false;
  if (contract.independent_review_required && !contract.independent_review_gate_passed) return false;
  return true;
}

export function runMissionEvidenceContractSelfTest() {
  const strongSummary = {
