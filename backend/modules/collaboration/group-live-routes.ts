...
} from "../../agents/user-facing-text";
import {
  ingestRequirementSources,
} from "../requirements/source-ingestion";
import {
  buildGroupDirectMemoryAction,
  commitGroupDirectMemoryAction,
...
import { visibleGroupPresentedPlanFields } from "./group-presented-plan";
import { resolveGroupLiveDispatchPolicy } from "./group-clarification-attach";
...
import {
  resolveGroupModelRecovery,
} from "./group-model-recovery";

export {
  isGroupModelRecoveryContinuePhrase,
  resolveGroupModelRecovery,
  runGroupModelRecoverySelfTest,
} from "./group-model-recovery";
...
function updateGroupModelRecoveryMessages(
  groupId: string,
  sessionId: string,
  ...
  const next = stored.map((item: any) => {
    ...
    if (item?.role !== "assistant"
      ...
      || (failureMessageId && itemId !== failureMessageId)) return item;
    changed = true;
...
export function runGroupClarificationSummarySelfTest() {
  ...
  const checks = {
    ...
    structuredCards: structured.prePlanClarification?.questions?.[0]?.label === "核销方式"
      && structured.prePlanClarification?.questions?.[0]?.options?.some((item: any) => item.label === "到店核销"),
  };
...
export async function handleGroupLiveRoutesSendPreface(
  ...
  let reliabilityOperationKey = "";
        ...
        if (routing.rejectedDirectTarget) {
          sendJson(res, { error: "群聊不再支持用户直接调用项目成员，请把消息交给群聊主 Agent 统一分派", code: "GROUP_DIRECT_MEMBER_REMOVED" }, 410);
          return { done: true as const };
        }
        ...
        try {
          ...
          if (String(payload.resolved_route || payload.resolvedRoute || "")) throw error;
          if (recoverableCandidates.length === 0) throw error;
          ...
          let routeTurn = conversationTurnId
            ? conversationTurnControl.listInternal({ scope: "group", conversation_id: conversationId, limit: 500 }).turns.find(item => item.id === conversationTurnId)
            : null;
          if (!routeTurn) {
            ...
            routeTurn = conversationTurnControl.claim({ scope: "group", conversation_id: conversationId, id: created.turn.id, revision: created.turn.revision });
          }
          if (!routeTurn || routeTurn.status !== "sending") throw error;
          const routed = conversationTurnControl.requireRoute({
            ...
            routing: {
              ...
              candidateTaskIds: recoverableCandidates.map((item: any) => String(item?.id || "")).filter(Boolean),
              candidateSummaries: recoverableCandidates.slice(0, 6).map(buildRecoverableTaskSummary).filter(Boolean),
              exactSessionId: groupSessionId,
              scope: "group",
              confidence: 0,
        ...
        if (explicitRouteChoice === "continue_original") {
          if (!explicitRouteCandidate) {
            sendJson(res, { error: "原任务已不可恢复，请重新选择处理方式", code: "CONVERSATION_ROUTE_CANDIDATE_STALE" }, 409);
            return { done: true as const };
          }
          ...
          const routeDecision = decideConversationMessageRoute({ workflowDecision: taskIntent?.workflowDecision, candidates: recoverableCandidates, exactSessionId: groupSessionId, scope: "group" });
          if (["resume_task", "revise_task"].includes(routeDecision.decision) && routeDecision.candidate) {
            ...
            if (!routeTurn || routeTurn.status !== "sending") {
              sendJson(res, { error: "消息处理方式已经变化，请刷新后重试", code: "QUEUE_REVISION_CONFLICT" }, 409);
              return { done: true as const };
            }
            const routed = conversationTurnControl.requireRoute({
              id: routeTurn.id,
        ...
        if (modelRecovery) updateGroupModelRecoveryMessages(
          ...
          modelRecovery.failureMessageId,
        );
      ...
      try {
        ...
        if (isBroadcast && isOrchestrated) {
          ...
          const coordinatorResult = await runGroupOrchestrator({
            ...
            onModelActivity: (activity: any) => writeSse(res, { type: "model_activity", activity, agent: coordinator.project }),
          });
          writeSse(res, {
            type: "turn_decision",
            decision: (coordinatorResult as any).mainAgentTurnDecision || null,
            receipt: (coordinatorResult as any).mainAgentTurnReceipt || null,
          });
          for (const item of ((coordinatorResult as any).mainAgentToolUsage?.results || [])) {
          ...
          try {
            ...
            const clarificationSummary = dispatchPolicy?.action === "ask_user"
              || (coordinatorResult as any).mainAgentTurnDecision?.responseKind === "clarify"
