...
export async function runNativeTestAgentDispatchBranch(input: {
  mention: any;
  deps: any;
  ctx: any;
  targetName: string;
  ...
  const {
    ...
    testAgentProjectWorkDir, tWorkDir, state,
  } = input;
  const {
    addTaskLog, appendTaskTimelineEvent, writeSse, runTestAgentCliJob,
    compactMemoryText, summarizeNativeTestAgentExecutionPlan,
    buildNativeTestAgentPlanBlockedReceipt, formatNativeTestAgentPlanBlockedOutput,
    transitionExecution, buildNativeTestAgentReceipt, uniqueStrings,
    runMainAgentPostReviewSpotCheck, buildPostReviewSpotCheckSummary,
  ...
  let {
    ...
    testAgentCliDispatch, testAgentNativeReport, testAgentReviewSummary, targetWorkEvents,
      ...
      ctx.setAgentActivity(
        ...
        targetReceipt.status === "done"
          ? `${reviewSubjectLabel} 独立复核已通过，主 Agent 正在完成最终验收`
          : `${reviewSubjectLabel} 独立复核发现问题，主 Agent 正在安排返工`,
        { tab: "groups", groupId },
        90000,
        { actorKind: "test-agent", displayName: "TestAgent", detail: `复核对象：${reviewSubjectLabel}`, source: "test-agent" },
      );
