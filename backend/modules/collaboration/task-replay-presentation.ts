...
function buildActionCenter(input: TaskReplayPresentationInput) {
  ...
  const taskIdsWithContinuation = new Set(
    rows
  ...
  for (const task of input.tasks) {
    ...
    if (!taskId || taskIdsWithContinuation.has(taskId) || !["blocked", "failed", "cancelled", "canceled"].includes(status)) continue;
    ...
    rows.push({
      ...
      bindingChecksum: safeText(task?.binding_checksum || task?.bindingChecksum, 160),
    });
  }
  const seen = new Set<string>();
