...
const MODEL_EXECUTOR_UNAVAILABLE_RETRY_MS = Math.max(
  5_000,
...
function writeBufferAtomic(file: string, value: Buffer) {
  fs.mkdirSync(path.dirname(file), { recursive: true });
  const temp = `${file}.${process.pid}.${Date.now()}.${crypto.randomBytes(3).toString("hex")}.tmp`;
...
function withArtifactRetentionLock<T>(scopeId: string, fn: () => T): T {
...
function appendGroupSessionMemoryModelExtractionHistory(scopeId: string, event: any, options: any = {}) {
  const file = getGroupSessionMemoryModelExtractionHistoryFile(scopeId);
...
function messageIdentity(message: any, index: number) {
...
function transcriptRows(messages: any[]) {
...
function safeExtractionExecutionId(executionId: string) {
...
function extractionArtifactRetentionManifestChecksum(manifest: any) {
  const payload = { ...(manifest || {}) };
  delete payload.checksum;
  return hashText(JSON.stringify(payload), 64);
}

function extractionArtifactArchiveFile(scopeId: string, executionId: string, kind: "request" | "result") {
...
function readExtractionArtifactFile(file: string, scopeId: string, executionId: string, kind: "request" | "result") {
  try {
...
function writeGroupSessionMemoryModelExtractionArtifact(
  scopeId: string,
  executionId: string,
  kind: "request" | "result",
  payload: any
...
function resolveExtractionArtifactRetentionPolicy(options: any = {}) {
  let config: any = {};
  try {
...
function listHotExtractionArtifacts(scopeId: string) {
...
function readCurrentExtractionReceiptExecutionIds(scopeId: string) {
...
function runAutomaticExtractionArtifactRetention(scopeId: string) {
  try {
