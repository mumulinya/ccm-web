...
export const GROUP_REACTIVE_COMPACT_RETRY_OWNERSHIP_SCHEMA = "ccm-group-reactive-compact-retry-ownership-v1";
export const GROUP_REACTIVE_COMPACT_RETRY_OWNERSHIP_DIR = path.join(CCM_DIR, "group-reactive-compact-retry-ownership");
export const GROUP_REACTIVE_COMPACT_RETRY_LEASE_MS = 2 * 60 * 1000;
...
function normalizeEpoch(value: any) {
  return String(value || "").trim().replace(/[^a-zA-Z0-9._:-]+/g, "-").slice(0, 220);
}

function entryKey(channel: string, retryEpoch: string) {
  return `${channel}::${retryEpoch}`;
}

export function getGroupReactiveCompactRetryOwnershipFile(groupId: string, groupSessionId: string) {
  return path.join(GROUP_REACTIVE_COMPACT_RETRY_OWNERSHIP_DIR, clean(groupId), `${clean(groupSessionId)}.json`);
}

function emptyLedger(groupId: string, groupSessionId: string, file: string) {
  return {
    schema: GROUP_REACTIVE_COMPACT_RETRY_OWNERSHIP_SCHEMA,
    version: 1,
    group_id: groupId,
...
export function verifyGroupReactiveCompactRetryOwnership(ledger: any, expected: any = {}) {
  const issues: string[] = [];
  if (ledger?.schema !== GROUP_REACTIVE_COMPACT_RETRY_OWNERSHIP_SCHEMA || Number(ledger?.version || 0) !== 1) issues.push("reactive_compact_retry_ownership_schema_invalid");
  if (!String(ledger?.group_id || "")) issues.push("reactive_compact_retry_ownership_group_missing");
  if (!String(ledger?.group_session_id || "").startsWith("gcs_")) issues.push("reactive_compact_retry_ownership_exact_session_missing");
  if (String(ledger?.scope_id || "") !== `${String(ledger?.group_id || "")}::${String(ledger?.group_session_id || "")}`) issues.push("reactive_compact_retry_ownership_scope_invalid");
  if (!Array.isArray(ledger?.entries)) issues.push("reactive_compact_retry_ownership_entries_invalid");
  if (!Number.isInteger(Number(ledger?.revision || 0)) || Number(ledger?.revision || 0) < 0) issues.push("reactive_compact_retry_ownership_revision_invalid");
  if (!Number.isInteger(Number(ledger?.next_fencing_token || 0)) || Number(ledger?.next_fencing_token || 0) < 1) issues.push("reactive_compact_retry_ownership_fence_invalid");
  ...
  for (const entry of Array.isArray(ledger?.entries) ? ledger.entries : []) {
    ...
    if (!retryEpoch) issues.push("reactive_compact_retry_epoch_missing");
    if (keys.has(key)) issues.push("reactive_compact_retry_epoch_duplicate");
    keys.add(key);
    if (!String(entry?.entry_id || "")) issues.push("reactive_compact_retry_entry_id_missing");
    if (!["claimed", "recovered", "failed"].includes(String(entry?.state || ""))) issues.push("reactive_compact_retry_state_invalid");
    if (!Number.isInteger(Number(entry?.fencing_token || 0)) || Number(entry?.fencing_token || 0) < 1) issues.push("reactive_compact_retry_entry_fence_invalid");
    if (!String(entry?.claim_id || "")) issues.push("reactive_compact_retry_claim_missing");
  }
  if (expected.groupId && String(ledger?.group_id || "") !== String(expected.groupId)) issues.push("reactive_compact_retry_group_mismatch");
  if (expected.groupSessionId && String(ledger?.group_session_id || "") !== String(expected.groupSessionId)) issues.push("reactive_compact_retry_group_session_mismatch");
  if (String(ledger?.ledger_checksum || "") !== ledgerChecksum(ledger)) issues.push("reactive_compact_retry_ownership_checksum_invalid");
...
function summarizeLedger(ledger: any) {
  ...
  const totals = entries.reduce((result: any, entry: any) => {
...
export function readGroupReactiveCompactRetryOwnership(groupId: string, groupSessionId: string) {
...
function leaseActive(entry: any, nowMs: number) {
  const ownerHost = String(entry?.owner_hostname || "");
  const ownerPid = Number(entry?.owner_pid || 0);
  if (ownerHost === os.hostname() && ownerPid > 0) return processAlive(ownerPid);
  const leaseMs = Date.parse(String(entry?.lease_expires_at || ""));
  return Number.isFinite(leaseMs) && leaseMs > nowMs;
}

function saveLedger(file: string, current: any, entries: any[], now: string, nextFencingToken: number) {
  const payload: any = {
    schema: GROUP_REACTIVE_COMPACT_RETRY_OWNERSHIP_SCHEMA,
    version: 1,
...
export function claimGroupReactiveCompactRetry(input: any = {}) {
  ...
  if (!groupId || !groupSessionId.startsWith("gcs_")) throw new Error("reactive compact retry ownership requires groupId + gcs_* identity");
  if (!retryEpoch) throw new Error("reactive compact retry ownership requires retry epoch");
  ...
  return withFileLock(file, () => {
    ...
    if (current.blocked || current.state === "fail_closed") return { status: "fail_closed", acquired: false, ledger: current, issues: current.issues || [] };
    const nowMs = Number(input.nowMs || input.now_ms || Date.now());
    const now = String(input.at || new Date(nowMs).toISOString());
    const leaseMs = Math.max(5_000, Number(input.leaseMs || input.lease_ms || GROUP_REACTIVE_COMPACT_RETRY_LEASE_MS));
    const key = entryKey(channel, retryEpoch);
    const entries = [...(current.entries || [])];
    const index = entries.findIndex((entry: any) => entryKey(normalizeChannel(entry.channel), normalizeEpoch(entry.retry_epoch)) === key);
...
export function completeGroupReactiveCompactRetry(input: any = {}) {
  const groupId = String(input.groupId || input.group_id || "").trim();
  const groupSessionId = String(input.groupSessionId || input.group_session_id || "").trim();
  const channel = normalizeChannel(input.channel);
  const retryEpoch = normalizeEpoch(input.retryEpoch || input.retry_epoch || input.contextId || input.context_id);
  ...
  if (!groupId || !groupSessionId.startsWith("gcs_") || !retryEpoch || !claimId || !Number.isInteger(fencingToken) || fencingToken < 1) throw new Error("reactive compact retry completion requires exact scope and fencing identity");
  if (!['recovered', 'failed'].includes(outcome)) throw new Error("reactive compact retry completion outcome must be recovered or failed");
  const file = getGroupReactiveCompactRetryOwnershipFile(groupId, groupSessionId);
  return withFileLock(file, () => {
    const current = readGroupReactiveCompactRetryOwnership(groupId, groupSessionId);
    if (current.blocked || current.state === "fail_closed") return { status: "fail_closed", accepted: false, ledger: current, issues: current.issues || [] };
    const key = entryKey(channel, retryEpoch);
    const entries = [...(current.entries || [])];
    const index = entries.findIndex((entry: any) => entryKey(normalizeChannel(entry.channel), normalizeEpoch(entry.retry_epoch)) === key);
    const existing = index >= 0 ? entries[index] : null;
    if (!existing) return { status: "missing_claim", accepted: false, ledger: current };
...
export function deleteGroupReactiveCompactRetryOwnership(groupId: string, groupSessionId: string) {
  const file = getGroupReactiveCompactRetryOwnershipFile(groupId, groupSessionId);
  return withFileLock(file, () => {
    let deleted = 0;
