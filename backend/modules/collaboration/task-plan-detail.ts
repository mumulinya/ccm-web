...
const text = (value: any, max = 320) => {
  const normalized = String(value || "").replace(/\s+/g, " ").trim();
  return normalized.length > max ? `${normalized.slice(0, max)}...` : normalized;
};

const list = (value: any, max = 80) => (Array.isArray(value) ? value : value ? [value] : [])
  .map(item => text(item, 400))
  .filter(Boolean)
  .slice(0, max);

const firstArray = (...values: any[]) => values.find(value => Array.isArray(value) && value.length) || [];

function planModeOf(task: any) {
  return task?.workflow_meta?.plan_mode || task?.workflow_meta?.intake?.plan_mode || task?.intake_draft || {};
}

function taskScope(task: any): TaskPlanScope {
...
function normalizedStatus(value: any) {
  ...
  if (["done", "success", "succeeded", "accepted", "completed"].includes(status)) return "completed";
  if (["running", "in_progress", "executing", "active"].includes(status)) return "running";
  if (["reviewing", "awaiting_review", "verification"].includes(status)) return "reviewing";
  ...
  if (["waiting_permission", "permission_required", "needs_confirmation"].includes(status)) return "waiting_permission";
  if (["failed", "blocked", "rejected"].includes(status)) return "blocked";
  if (["cancelled", "canceled", "skipped"].includes(status)) return "skipped";
  return "pending";
}

function planStatus(task: any) {
  const runtime = String(task?.phase || task?.runtime_status?.phase || task?.status || "").toLowerCase();
  if (task?.intake_state === "awaiting_confirmation" || task?.status === "paused") return "awaiting_confirmation";
  if (["done", "completed", "succeeded"].includes(runtime) && task?.acceptance_state === "accepted") return "completed";
  if (["reviewing", "accepting", "verifying"].includes(runtime)) return "reviewing";
  if (["failed", "blocked", "recovery_required", "needs_user"].includes(runtime)) return "blocked";
  if (["pending", "ready", "created"].includes(runtime)) return "ready";
  return "executing";
}

function rawWorkItems(task: any) {
  const planMode = planModeOf(task);
  const architecture = planMode?.architecture_plan || task?.architecture_plan || {};
  return firstArray(
    task?.work_items,
    task?.workflow_meta?.project_main_plan?.workItems,
    architecture?.dependencySteps,
    architecture?.dependency_steps,
    planMode?.steps,
  );
}

function workItems(task: any) {
  const scope = taskScope(task);
  const defaultProject = text(task?.target_project || task?.project || "", 120);
  return rawWorkItems(task).slice(0, 80).map((item: any, index: number) => {
...
function assignments(task: any, items: any[]) {
  ...
  if (rows.length) return rows.slice(0, 40).map((item: any, index: number) => {
  ...
  }).filter((item: any) => item.project || item.label);
  ...
  for (const item of items) {
    ...
    if (["running", "reviewing", "rework", "blocked"].includes(item.status)) current.status = item.status;
    else if (item.status === "completed" && current.status === "pending") current.status = "completed";
    if (!current.waitingReason && item.waitingReason) current.waitingReason = item.waitingReason;
    grouped.set(project, current);
  }
  return [...grouped.values()];
}

function sourceReferences(task: any) {
  ...
  const rows: any[] = [];
  if (projectEvidence && (projectEvidence.selectedPaths || projectEvidence.selected_paths)) rows.push({
  ...
  for (const item of projects.slice(0, 20)) rows.push({
...
function revisionHistory(task: any, planMode: any) {
  return firstArray(task?.plan_revisions, planMode?.plan_revisions, planMode?.revisions).slice(-30).map((item: any, index: number) => ({
...
export function buildTaskPlanDetail(task: any) {
  const planMode = planModeOf(task);
  ...
  const detail: any = {
    ...
    acceptanceCriteria: list(task?.acceptance_evidence_plan?.map?.((item: any) => item?.criterion || item?.observableOutcome) || task?.acceptance_criteria || planMode?.acceptance, 30),
  ...
  detail.bindingChecksum = crypto.createHash("sha256").update(JSON.stringify({
    ...
    workItems: detail.workItems.map((item: any) => ({ id: item.id, status: item.status, project: item.project, dependsOn: item.dependsOn })),
  })).digest("hex");
  return detail;
}

function assertAcyclic(items: any[]) {
  const map = new Map(items.map(item => [item.id, item]));
  const visiting = new Set<string>();
  const visited = new Set<string>();
  const visit = (id: string) => {
    if (visiting.has(id)) throw new Error("执行清单存在循环依赖，请调整依赖关系");
    if (visited.has(id)) return;
    visiting.add(id);
    for (const dependency of map.get(id)?.dependsOn || []) if (map.has(dependency)) visit(dependency);
    visiting.delete(id);
    visited.add(id);
  };
...
export function buildTaskPlanPatch(task: any, payload: any) {
  ...
  if (Number(payload?.revision) !== current.revision
    ...
    const error: any = new Error("计划已经更新，请刷新后重新调整");
    error.code = "TASK_PLAN_REVISION_CONFLICT";
    error.status = 409;
    error.current = current;
    throw error;
  }
  if (current.status === "completed") throw new Error("已完成计划不能再修改");
  const incoming = Array.isArray(payload?.workItems) ? payload.workItems : [];
  if (!incoming.length) throw new Error("执行清单至少需要保留一项");
  const existing = new Map(current.workItems.map((item: any) => [item.id, item]));
  const ids = new Set<string>();
  const nextItems = incoming.slice(0, 80).map((item: any, index: number) => {
    const id = text(item?.id || `work_${index + 1}`, 100);
    if (!id || ids.has(id)) throw new Error("执行清单包含重复或无效的工作项 ID");
    ids.add(id);
    const previous: any = existing.get(id);
    if (previous && ["completed", "running", "reviewing"].includes(previous.status)) {
      ...
      if (!unchanged) throw new Error(`“${previous.title}”已经执行，不能直接修改`);
    }
    return {
      ...(task?.work_items || []).find((raw: any) => String(raw?.id || "") === id),
      id,
  ...
  for (const item of nextItems) {
    const missing = item.dependsOn.filter((id: string) => !ids.has(id));
    if (missing.length) throw new Error(`“${item.title}”引用了不存在的前置工作项`);
  }
...
export function runTaskPlanDetailSelfTest() {
  const task: any = {
  ...
  const patch = buildTaskPlanPatch(task, {
    ...
    workItems: detail.workItems.map((item: any) => item.id === "ui" ? { ...item, title: "接入商家端审核页面" } : item),
  });
  let conflictBlocked = false;
  try { buildTaskPlanPatch(task, { revision: 1, generation: 2, bindingChecksum: "stale", workItems: detail.workItems }); }
  catch (error: any) { conflictBlocked = error?.code === "TASK_PLAN_REVISION_CONFLICT"; }
  ...
  try {
    buildTaskPlanPatch(task, {
      ...
      workItems: detail.workItems.map((item: any) => item.id === "ui" ? { ...item, dependsOn: ["pad"] } : item.id === "pad" ? { ...item, dependsOn: ["ui"] } : item),
    });
  } catch (error: any) { cycleBlocked = /循环依赖/.test(String(error?.message || "")); }
  return {
