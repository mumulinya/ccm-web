...
import {
  acknowledgePetDelivery,
  listPendingPetDeliveries,
  projectPetNotification,
} from "../../system/user-notifications";
import { getPetAssetPackStatus, prepareOfficialPetAssets } from "./pet-asset-pack";
...
const PET_RUNTIME_FILE = path.join(path.dirname(PET_PID_FILE_GLOBAL), "pet-runtime.json");

function parsePetPid(value: any) {
  const pid = Number.parseInt(String(value || "").trim(), 10);
  return Number.isSafeInteger(pid) && pid > 0 ? pid : 0;
}

function readPetRuntimeIdentity() {
  const runtime = readJsonWithBackup<any>(PET_RUNTIME_FILE, null);
  const pid = parsePetPid(runtime?.pid) || (() => {
    try { return parsePetPid(fs.readFileSync(PET_PID_FILE_GLOBAL, "utf-8")); } catch { return 0; }
  })();
  return {
...
function emptyPetConfig(): PetConfigV2 {
  return {
    schema: "ccm-pet-config-v2",
    revision: 0,
    configs: {},
...
function normalizePetConfig(value: any): PetConfigV2 {
...
export function readPetConfig() {
  return normalizePetConfig(readJsonWithBackup<any>(PETS_FILE, emptyPetConfig()));
}

function mergePetConfigPatch(current: PetConfigV2, patch: any): PetConfigV2 {
  ...
  if (patch?.configs && typeof patch.configs === "object" && !Array.isArray(patch.configs)) {
    for (const [agent, value] of Object.entries(patch.configs)) {
      if (!/^[a-zA-Z0-9._:-]{1,160}$/.test(agent)) throw new Error("宠物Agent标识无效");
      if (value === null) delete next.configs[agent];
      else next.configs[agent] = { ...(next.configs[agent] || {}), ...(value as any) };
    }
  }
  if (patch?.positions && typeof patch.positions === "object" && !Array.isArray(patch.positions)) {
    for (const [agent, value] of Object.entries(patch.positions)) {
      if (!/^[a-zA-Z0-9._:-]{1,160}$/.test(agent)) throw new Error("宠物Agent标识无效");
      if (value === null) delete next.positions[agent];
      else {
        const x = Number((value as any)?.x);
        const y = Number((value as any)?.y);
        if (!Number.isFinite(x) || !Number.isFinite(y)) throw new Error("宠物位置无效");
...
function savePetConfigPatch(expectedRevision: number, patch: any) {
  return withFileLock(PETS_FILE, () => {
    const current = readPetConfig();
    if (Number(expectedRevision) !== current.revision) {
      const error: any = new Error("宠物配置已经变化，请刷新后重试");
      error.code = "state_drift";
      error.current = current;
      throw error;
    }
    const next = mergePetConfigPatch(current, patch);
    writeJsonAtomic(PETS_FILE, next);
    return next;
  });
}
...
function isPetRunning() {
  ...
  try {
    ...
    try { fs.unlinkSync(PET_RUNTIME_FILE); } catch {}
    return false;
  }
}
...
function launchPet(port: number) {
  try {
    ...
    writeJsonAtomic(PET_RUNTIME_FILE, {
      schema: "ccm-pet-runtime-v2",
      pid: child.pid,
...
function stopPet() {
  ...
  if (!pid) return { success: false, error: "桌面宠物未在运行" };
  try {
    if (process.platform === "win32") {
      const result = spawnSync("taskkill.exe", ["/PID", String(pid), "/T", "/F"], { stdio: "ignore", windowsHide: true });
      if (result.status !== 0 && isPetRunning()) throw new Error(`taskkill退出码 ${result.status}`);
    } else {
...
function validatePngAsset(content: Buffer) {
...
function validateSvgAsset(content: string) {
  if (!/^\s*(?:<\?xml[^>]*>\s*)?<svg[\s>]/i.test(content)) throw new Error("请上传有效的 SVG 文件");
  ...
  if (references.some(value => value && !value.startsWith("#"))) throw new Error("SVG 不能引用外部资源");
  const cssUrls = [...content.matchAll(/url\(\s*["']?([^)"']+)/gi)].map(match => match[1].trim());
  if (cssUrls.some(value => value && !value.startsWith("#"))) throw new Error("SVG 不能引用外部 CSS 资源");
}

function collectPetUploadBuffer(req: any) {
...
function writePetAsset(assetPath: string, sourcePath: string) {
  ...
  if (!stat.isFile() || stat.isSymbolicLink()) throw new Error("宠物资源必须是普通文件");
  ...
  for (const target of targets) {
    ...
    try { fs.renameSync(temp, target.file); }
    catch {
    ...
    if (!fs.existsSync(immutableFile)) fs.writeFileSync(immutableFile, contentBuffer, { flag: "wx" });
  }

  return { assetPath: safePath, checksum };
}

export function runPetAssetSecuritySelfTest() {
  ...
  try { validatePngAsset(png); checks.validPng = true; } catch { checks.validPng = false; }
  try { validatePngAsset(Buffer.from("not-png")); checks.invalidPngRejected = false; } catch { checks.invalidPngRejected = true; }
  try { validateSvgAsset('<svg xmlns="http://www.w3.org/2000/svg"><path d="M0 0h1v1z"/></svg>'); checks.validSvg = true; } catch { checks.validSvg = false; }
  try { validateSvgAsset('<svg xmlns="http://www.w3.org/2000/svg"><script>alert(1)</script></svg>'); checks.scriptRejected = false; } catch { checks.scriptRejected = true; }
  try { validateSvgAsset('<svg xmlns="http://www.w3.org/2000/svg"><image href="https://example.com/a.png"/></svg>'); checks.externalReferenceRejected = false; } catch { checks.externalReferenceRejected = true; }
...
// === 暴露供外部查询进程状态 ===
export function maybeAutoStartPet(port: number) {
  ...
  if (!desktopAvailable) return { success: true, skipped: true, reason: "desktop_unavailable" };
  if (isPetRunning()) return { success: true, skipped: true, reason: "already_running" };
...
export function handlePetsApi(
  ...
  if (pathname === "/api/pets/assets/status" && req.method === "GET") {
    sendJson(res, { success: true, ...getPetAssetPackStatus() });
    return true;
  }
  ...
  if (pathname === "/api/pets/assets/prepare" && req.method === "POST") {
    ...
    req.on("data", (chunk: any) => {
      body += chunk;
      if (Buffer.byteLength(body, "utf-8") > 32 * 1024) req.destroy();
    });
    req.on("end", () => {
      let payload: any = {};
      try { payload = JSON.parse(body || "{}"); }
      catch { return sendJson(res, { success: false, error: "请求格式无效" }, 400); }
      prepareOfficialPetAssets(payload.skin)
        .then(result => sendJson(res, result))
        .catch((error: any) => sendJson(res, {
          success: false,
          error: error?.message || "宠物资源准备失败",
          code: "PET_ASSET_PREPARE_FAILED",
          status: getPetAssetPackStatus(),
        }, 503));
    });
    return true;
  }
  ...
  if (pathname === "/api/pets/runtime/bootstrap" && req.method === "GET") {
    ...
    let config: any = { schema: "ccm-pet-config-v2", revision: 0, configs: {}, positions: {}, customTypes: [] };
    try {
      if (fs.existsSync(PETS_FILE)) config = { ...config, ...JSON.parse(fs.readFileSync(PETS_FILE, "utf-8")) };
    } catch {}
    const pending = listPendingPetDeliveries({ channel, recipient_user_ids: recipientUserIds, limit: 30 })
      .map(item => projectPetNotification(item.notification, item.delivery));
    sendJson(res, {
      success: true,
      runtime: {
        version: 2,
  ...
  if (deliveryAckMatch && req.method === "POST") {
    ...
    req.on("end", () => {
      try {
        ...
        const acknowledged = acknowledgePetDelivery(
          decodeURIComponent(deliveryAckMatch[1]),
          String(payload.client_id || "pet-client"),
        );
        sendJson(res, { success: acknowledged }, acknowledged ? 200 : 404);
      } catch (error: any) {
        sendJson(res, { success: false, error: error?.message || "确认通知失败" }, 400);
      }
    });
    return true;
  }
  ...
  if (pathname === "/api/pets/config" && req.method === "POST") {
    ...
    req.on("end", () => {
      try {
        ...
        const saved = withFileLock(PETS_FILE, () => {
          const current = readPetConfig();
          const next = normalizePetConfig({
            ...payload,
        ...
        sendJson(res, { success: true, config: saved });
      } catch (e: any) {
  ...
  if (pathname === "/api/pets/config" && req.method === "PATCH") {
    ...
    req.on("data", (chunk: any) => {
      body += chunk;
      if (Buffer.byteLength(body, "utf-8") > 256 * 1024) req.destroy();
    });
    req.on("end", () => {
      try {
        const payload = JSON.parse(body || "{}");
  ...
  if (pathname === "/api/pets/status" && req.method === "GET") {
    sendJson(res, { running: isPetRunning(), runtime: readPetRuntimeIdentity() });
    return true;
  }
