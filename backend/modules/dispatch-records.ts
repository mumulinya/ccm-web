...
export function normalizeDispatchTask(text: string) {
...
function safeDispatchKeyPart(value: any, max = 80) {
  return String(value || "")
    .replace(/[|\r\n\t]+/g, " ")
    .replace(/\s+/g, " ")
    .trim()
...
export function buildDispatchKey(input: any = {}) {
...
export function normalizeReceiptStatus(status: any): ReceiptStatus {
  const value = String(status || "").trim();
  if (["done", "partial", "blocked", "failed", "needs_info"].includes(value)) return value as ReceiptStatus;
  return "missing";
}

export function receiptStatusToDispatchStatus(status: any): DispatchStatus {
  const normalized = normalizeReceiptStatus(status);
  if (normalized === "done") return "completed";
  if (normalized === "missing") return "missing_receipt";
  return normalized;
}

export function dispatchStatusToAssignmentStatus(status: any) {
  const value = String(status || "").trim();
  if (value === "completed" || value === "skipped") return "done";
  if (["running", "done", "partial", "blocked", "failed", "needs_info"].includes(value)) return value;
  if (value === "missing_receipt") return "blocked";
  return "pending";
}

export function normalizeDispatchAssignment(raw: any = {}, context: DispatchContext = {}): DispatchAssignment {
...
export function normalizeDispatchBatch(items: any[] = [], context: DispatchContext = {}) {
  ...
  for (const item of items || []) {
    ...
    if (!assignment.project || !assignment.task || seen.has(key)) continue;
    seen.add(key);
...
export function attachDispatchIdentityToMention(mention: any, context: DispatchContext = {}) {
...
export function createDispatchRecord(input: Partial<DispatchRecord> & { assignment: DispatchAssignment }): DispatchRecord {
  return {
...
export function recordToAssignmentStatus(record: DispatchRecord) {
  return {
...
export function recordToWorkerLedger(record: DispatchRecord) {
...
export function recordToTaskNotification(record: DispatchRecord) {
...
export function recordToCollectedOutput(record: DispatchRecord, formatter: (agent: string, text: string, receipt: any) => string) {
...
export function recordToTimelineEvent(record: DispatchRecord, type = "child_agent_dispatch") {
  return {
    type,
