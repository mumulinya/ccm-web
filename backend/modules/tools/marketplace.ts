...
function installationKey(type: string, name: string) {
...
function appendMarketplaceOperationAudit(entry: any, store: MarketplaceInstallStore = {}) {
  const safeEntry = {
...
function readMarketplaceOperationAudit(input: any = {}, store: MarketplaceInstallStore = {}) {
...
function buildMarketplaceAuthorizationImpact(input: { action: string; type: string; name: string }, store: MarketplaceInstallStore = {}) {
...
function previewMarketplaceAuthorizationImpact(payload: any, store: MarketplaceInstallStore = {}) {
...
function buildMarketplaceRuntimeImpact(input: { action: string; type: string; name: string }, store: MarketplaceInstallStore = {}) {
  const type = input.type === "skill" ? "skill" : "mcp";
  const name = cleanImpactText(input.name, 240);
  const snapshots: any[] = [];
...
function buildMarketplaceRuntimeCatalog(store: MarketplaceInstallStore = {}) {
  return {
...
function maybeAutoResyncMarketplaceRuntime(impact: any, options: any = {}, store: MarketplaceInstallStore = {}) {
...
function saveInstallations(items: InstallationRecord[]) {
  writeJsonAtomic(INSTALLATIONS_FILE, { version: 1, items });
}

function marketplaceSourceId(url: string) {
  return `external-${sha256(url).slice(0, 12)}`;
}

function normalizeSavedSource(value: any): MarketplaceSavedSource | null {
...
function loadMarketplaceSources(): MarketplaceSavedSource[] {
  try {
...
function saveMarketplaceSources(items: MarketplaceSavedSource[]) {
...
function removeManagedPackage(packagePath: string, skillPackagesDir = SKILL_PACKAGES_DIR) {
...
function buildMarketplaceSourceProof(item: any, input: any = {}) {
...
function sanitizeMarketplacePreviewItem(item: any) {
  return {
...
function sanitizeMarketplaceSourceProof(value: any) {
...
function baseMarketplaceSourceId(value: any) {
...
function normalizeMarketplaceItem(item: any, fallbackSource: MarketplaceSource) {
...
function normalizeMarketplaceInstallRequest(item: any, fallbackSource: MarketplaceSource) {
  const type = String(item?.type || "").toLowerCase();
  if (!["mcp", "skill"].includes(type)) throw new Error("商城条目 type 必须为 mcp 或 skill");
  const name = String(item?.name || item?.displayName || item?.slug || "").trim();
  if (!name || name.length > 120 || /[\\/\0\r\n]/.test(name)) throw new Error("商城条目名称无效");
...
function githubRepoFromUrlOrShorthand(value: any) {
...
function catalogItemsFromParsedJson(parsed: any, source: MarketplaceSource, catalogUrl = "") {
...
function decorateInstallState(items: any[]) {
...
async function fetchRemote(value: string, maxBytes: number, headers: Record<string, string> = {}, redirects = 0): Promise<{ body: Buffer; contentType: string; finalUrl: string }> {
...
function parseSkillMarkdown(content: string, fallbackName = "", fallbackDescription = "") {
...
function parseGithubSkillSource(value: string) {
  try {
    const parsed = new URL(value);
...
function validateSkillDirectory(root: string) {
...
async function cloneGithubSkill(item: any, staging: string) {
