...
import { saveMcpTool, saveSkill } from "../../core/db";
...
export function handleMarketplaceApi(pathname: string, req: any, res: any, parsed: any): boolean {
  ...
  if (pathname === "/api/smithery/config" && req.method === "GET") {
    ...
    try {
      if (fs.existsSync(smitheryConfigPath)) {
        ...
        sendJson(res, { success: true, key: config.key || "" });
        return true;
      }
  ...
  if (pathname === "/api/marketplace/list" && req.method === "GET") {
    ...
    if (source === "local") {
      sendJson(res, { success: true, items: localItems });
      return true;
    }
    ...
    if (source === "smithery") {
      ...
      if (!key) {
        sendJson(res, { success: true, needKey: true, items: [] });
        return true;
      }
    ...
    if (source === "github") {
      ...
      sendJson(res, { success: false, error: "未指定有效的自定义 URL", items: localItems });
      return true;
    }
