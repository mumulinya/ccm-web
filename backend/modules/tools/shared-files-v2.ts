...
type SharedFileStoreV2 = {
  schema: "ccm-shared-files-v2";
  version: 2;
  revision: number;
  files: SharedFileManifestV2[];
  updated_at: string;
};

const ROOT = path.join(CCM_DIR, "shared-files-v2");
...
function emptyStore(): SharedFileStoreV2 {
  return {
    schema: "ccm-shared-files-v2",
    version: 2,
    revision: 0,
...
function readStoreUnlocked(): SharedFileStoreV2 {
  const value = readJsonWithBackup<any>(STORE_FILE, emptyStore());
  return {
    ...emptyStore(),
    ...value,
...
function saveStoreUnlocked(store: SharedFileStoreV2) {
...
function validateScope(scopeType: unknown, scopeId: unknown) {
  const type = String(scopeType || "") as SharedFileScope;
  if (!["global", "group", "project"].includes(type)) throw new Error("共享文件作用域无效");
  const id = type === "global" ? "global" : String(scopeId || "").trim();
  if (!id || id.length > 200 || /[\0\r\n]/.test(id)) throw new Error("共享文件作用域标识无效");
  return { type, id };
}

export function validateSharedFileV2Name(value: unknown) {
  const raw = String(value || "").replace(/\0/g, "").trim();
  if (!raw || raw.length > 240) throw new Error("共享文件名无效");
  ...
  if (segments.some(segment => blocked.has(segment))) throw new Error("共享文件不允许使用可执行文件扩展名");
  return raw;
}
...
function safeDataPath(relative: string) {
  ...
  if (!rel || rel.startsWith("..") || path.isAbsolute(rel)) throw new Error("共享文件存储路径越界");
  return target;
}

function scopeKey(scopeType: SharedFileScope, scopeId: string) {
  return checksum(`${scopeType}\0${scopeId}`).slice(0, 32);
}

function classify(name: string) {
  if (isTextFileName(name)) return "text" as const;
  if (isImageFileName(name)) return "image" as const;
  if (isOoxmlFileName(name) || path.extname(name).toLowerCase() === ".pdf") return "document" as const;
  return "file" as const;
}

function splitWithinTokenLimit(value: string, tokenLimit: number) {
  const parts: string[] = [];
  let remaining = value;
  while (remaining && estimateTextTokens(remaining) > tokenLimit) {
    let low = 1;
    let high = remaining.length;
    while (low < high) {
      const middle = Math.ceil((low + high) / 2);
      if (estimateTextTokens(remaining.slice(0, middle)) <= tokenLimit) low = middle;
      else high = middle - 1;
    }
    let end = Math.max(1, low);
    const preferredStart = Math.floor(end * 0.7);
    const preferred = remaining.slice(preferredStart, end).search(/[\s,.;:!?，。；：！？][^]*$/);
    if (preferred >= 0) end = preferredStart + preferred + 1;
...
function splitCompleteText(text: string): Array<{ content: string; token_count: number }> {
  ...
  const flush = () => {
    if (!current) return;
    chunks.push({ content: current, token_count: estimateTextTokens(current) });
    current = "";
  };
  for (const paragraph of paragraphs) {
    ...
    if (paragraphTokens > CHUNK_TOKEN_HARD_LIMIT) {
      ...
      for (const line of lines) {
        if (estimateTextTokens(line) > CHUNK_TOKEN_HARD_LIMIT) {
          if (lineChunk) {
            chunks.push({ content: lineChunk, token_count: estimateTextTokens(lineChunk) });
            lineChunk = "";
          }
          for (const part of splitWithinTokenLimit(line, CHUNK_TOKEN_HARD_LIMIT)) {
            chunks.push({ content: part, token_count: estimateTextTokens(part) });
          }
          continue;
        }
        if (lineChunk && estimateTextTokens(lineChunk + line) > CHUNK_TOKEN_HARD_LIMIT) {
          chunks.push({ content: lineChunk, token_count: estimateTextTokens(lineChunk) });
          lineChunk = "";
        }
        lineChunk += line;
      }
      if (lineChunk) chunks.push({ content: lineChunk, token_count: estimateTextTokens(lineChunk) });
      continue;
    }
    if (current && estimateTextTokens(current + paragraph) > CHUNK_TOKEN_TARGET) flush();
    current += paragraph;
  }
  flush();
  return chunks;
...
function writeContentFiles(scopeType: SharedFileScope, scopeId: string, id: string, raw: Buffer, parsedText: string) {
  ...
  if (!fs.existsSync(sourceFile)) fs.writeFileSync(sourceFile, raw, { flag: "wx", mode: 0o600 });
  let contentRelative = "";
  if (parsedText) {
    ...
    if (!fs.existsSync(contentFile)) fs.writeFileSync(contentFile, parsedText, { encoding: "utf8", flag: "wx", mode: 0o600 });
  }
  return { sourceRelative, contentRelative };
}

function manifestChunks(id: string, parsedText: string) {
...
function publicManifest(item: SharedFileManifestV2) {
  const { content_file: _contentFile, source_file: _sourceFile, ...safe } = item;
  return safe;
}

function parseUploadedText(filePath: string, name: string) {
  if (isTextFileName(name)) return fs.readFileSync(filePath, "utf8");
  if (isOoxmlFileName(name)) return extractOoxmlText(filePath, name);
  return "";
}
...
function upsertBuffer(
  ...
  nameInput: unknown,
  raw: Buffer,
  options: { mimeType?: string; parsedText?: string; legacySource?: string } = {},
) {
  const { type: scopeType, id: scopeId } = validateScope(scopeTypeInput, scopeIdInput);
  const name = validateSharedFileV2Name(nameInput);
  if (!Buffer.isBuffer(raw) || raw.length <= 0) throw new Error("共享文件内容不能为空");
  if (raw.length > MAX_TEXT_BYTES) throw new Error("单个共享文件不能超过 25 MB");
  return withFileLock(STORE_FILE, () => {
    ...
    if (!existing && matching.length >= MAX_SCOPE_FILES) throw new Error("当前作用域的共享文件数量已达上限");
    const id = existing?.id || `sf_${crypto.randomUUID()}`;
    const parsedText = String(options.parsedText ?? (isTextFileName(name) ? raw.toString("utf8") : ""));
    const files = writeContentFiles(scopeType, scopeId, id, raw, parsedText);
    const timestamp = now();
    const record: SharedFileManifestV2 = {
      schema: "ccm-shared-file-manifest-v2",
      id,
      ...
      name,
      type: classify(name),
      mime_type: String(options.mimeType || (isTextFileName(name) ? "text/plain" : "application/octet-stream")),
      size: raw.length,
    ...
    for (const relative of oldFiles) {
      if (relative === record.content_file || relative === record.source_file) continue;
      try { fs.unlinkSync(safeDataPath(String(relative))); } catch {}
    }
    return publicManifest(record);
  }, { timeoutMs: 30_000 });
}

export function upsertSharedTextV2(scopeType: unknown, scopeId: unknown, name: unknown, content: unknown, legacySource = "") {
  const text = String(content ?? "");
  const raw = Buffer.from(text, "utf8");
  if (raw.length > 1024 * 1024) throw new Error("单个共享文本文件不能超过 1 MB");
  return upsertBuffer(scopeType, scopeId, name, raw, { mimeType: "text/plain", parsedText: text, legacySource });
}

export function adoptSharedUploadV2(scopeType: unknown, scopeId: unknown, uploaded: any) {
  const source = path.resolve(String(uploaded?.savedPath || ""));
  const stat = fs.lstatSync(source);
  if (!stat.isFile() || stat.isSymbolicLink()) throw new Error("共享上传必须是普通文件");
...
export function listSharedFilesV2(scopeTypeInput: unknown, scopeIdInput: unknown) {
...
export function readSharedFileV2(scopeTypeInput: unknown, scopeIdInput: unknown, fileIdInput: unknown) {
  const { type, id } = validateScope(scopeTypeInput, scopeIdInput);
...
export function resolveSharedFileSourceV2(scopeTypeInput: unknown, scopeIdInput: unknown, fileIdInput: unknown) {
  const { type, id } = validateScope(scopeTypeInput, scopeIdInput);
  ...
  if (!stat.isFile() || stat.isSymbolicLink()) throw new Error("共享文件源文件无效");
  return { file, manifest: publicManifest(item) };
}

export function deleteSharedFileV2(scopeTypeInput: unknown, scopeIdInput: unknown, fileIdInput: unknown) {
  const { type, id } = validateScope(scopeTypeInput, scopeIdInput);
  const fileId = String(fileIdInput || "");
  return withFileLock(STORE_FILE, () => {
    const store = readStoreUnlocked();
...
export function migrateLegacySharedFilesV2(scopeType: SharedFileScope, scopeId: string, files: any[], legacySource: string) {
  const existing = new Set(listSharedFilesV2(scopeType, scopeId).map((item: any) => item.name.toLowerCase()));
  const imported: any[] = [];
...
export function migrateLegacyGlobalSharedDirectoryV2() {
  if (!fs.existsSync(SHARED_DIR)) return [];
  const records = fs.readdirSync(SHARED_DIR, { withFileTypes: true })
    .filter(entry => entry.isFile() && !entry.isSymbolicLink() && !entry.name.startsWith("."))
    .map(entry => ({ name: entry.name, path: path.join(SHARED_DIR, entry.name) }));
...
export function readSharedFileChunkV2(scopeType: unknown, scopeId: unknown, fileId: unknown, chunkId: unknown) {
  const file = readSharedFileV2(scopeType, scopeId, fileId);
  if (!file || !file.content) return null;
  const pieces = splitCompleteText(file.content);
  const index = file.chunks.findIndex((chunk: any) => chunk.id === String(chunkId || ""));
  if (index < 0 || !pieces[index]) return null;
  return {
    file_id: file.id,
    file_name: file.name,
...
export function sharedFileV2Status() {
  const store = readStoreUnlocked();
  return {
    schema: store.schema,
...
export const SHARED_FILE_V2_TEXT_EXTENSIONS = TEXT_FILE_EXTENSIONS;
