...
import {
  ...
  resyncMissingRuntimeToolSnapshots,
  resyncRecentRuntimeToolSnapshots,
...
import {
  completeToolCatalogMutationLifecycle,
  previewToolCatalogMutationImpact,
} from "./marketplace";
import {
  mergeMcpToolUpdate,
  mergeSkillUpdate,
  normalizeToolCatalogName,
  redactMcpToolForDisplay,
} from "../../tools/tool-catalog-management";
import {
...
export function buildLivePerformanceSnapshot() {
...
export async function reloadToolManagerAfterCatalogMutation(entry: any) {
  await toolManager.loadTools();
  const lifecycle = completeToolCatalogMutationLifecycle({
    action: entry.action,
    type: entry.type,
    name: entry.name,
...
export async function rollbackCatalogMutation(type: "mcp" | "skill", name: string, previous: any) {
  if (previous) {
    if (type === "mcp") saveMcpTool(previous);
    else saveSkill(previous);
  } else if (type === "mcp") deleteMcpTool(name);
  else deleteSkill(name);
  try { await toolManager.loadTools(); } catch {}
}

function cleanAuditText(value: any, max = 240) {
  return String(value || "").replace(/[\0\r\n\t]+/g, " ").trim().slice(0, max);
}

function safeAuditNumber(value: any) {
  const number = Number(value || 0);
  return Number.isFinite(number) && number > 0 ? number : 0;
}

function readRecentJsonl(file: string, limit = 80) {
  try {
...
function readJsonFileSafe(file: string) {
  try {
...
export function selectLatestRuntimeToolAudits(audits: any[]) {
...
export function loadLatestRuntimeToolReadiness(limit = 240, options: { businessOnly?: boolean } = {}) {
  const readiness = selectLatestRuntimeToolAudits(listRecentRuntimeToolAudits(limit))
    .map(audit => probeRuntimeToolReadiness(audit, { record: false }));
...
function auditContextFromEntry(entry: any = {}) {
...
function sanitizeToolInvocationAuditEntry(entry: any, source: "tool_loop" | "skill_invocation" | "permission_violation") {
...
function toolInvocationAuditMatchesFilter(item: any, input: any = {}) {
  ...
  if (filters.runtime && String(item.runtime || "").toLowerCase() !== filters.runtime) return false;
  if (projectFilters.length && !projectFilters.includes(String(item.project || "").toLowerCase())) return false;
  if (filters.groupId && String(item.groupId || "").toLowerCase() !== filters.groupId) return false;
  if (filters.taskId && String(item.taskId || "").toLowerCase() !== filters.taskId) return false;
  if (filters.category && String(item.category || "").toLowerCase() !== filters.category) return false;
  if (filters.source
    && String(item.source || "").toLowerCase() !== filters.source
    && String(item.invocationSource || "").toLowerCase() !== filters.source) return false;
  return true;
}

export function buildToolInvocationAudit(input: any = {}) {
  ...
  return {
    ...
    summary: {
      ...
      toolCalls: items.filter((item: any) => item.category === "tool").length,
      successfulToolCalls: items.filter((item: any) => item.category === "tool" && item.ok === true).length,
      failedToolCalls: items.filter((item: any) => item.category === "tool" && item.ok === false).length,
      skillInvocations: items.filter((item: any) => item.category === "skill").length,
      unauthorized: items.filter((item: any) => item.category === "unauthorized").length,
      loopsFinished: items.filter((item: any) => item.type === "tool_loop_finished").length,
    },
    items,
  };
}

function chainVerificationStatus(row: any, invocationSummary: any) {
  const configured = Number(row?.counts?.mcp || 0) + Number(row?.counts?.skill || 0) > 0;
  const runtimeSummary = row?.runtime?.summary || {};
  if (!configured) return "not_configured";
  if (row?.authorization_readiness?.dispatchReady === false) return "authorization_blocked";
  if (Number(runtimeSummary.total || 0) === 0) return "runtime_missing";
...
function chainVerificationStatusLabel(status: string) {
...
function auditItemMatchesInventoryScope(item: any, row: any) {
  const id = cleanAuditText(row?.id || "", 240).toLowerCase();
  const name = cleanAuditText(row?.name || "", 240).toLowerCase();
  if (!id && !name) return false;
  if (row?.scope === "group") return cleanAuditText(item?.groupId || "", 240).toLowerCase() === id;
  const project = cleanAuditText(item?.project || "", 240).toLowerCase();
  return project === id || (!!name && project === name);
}

function buildScopeInvocationEvidence(row: any, auditItems: any[]) {
  const items = auditItems.filter(item => auditItemMatchesInventoryScope(item, row));
  const grantedSkills = new Set((Array.isArray(row?.tools?.skill) ? row.tools.skill : []).map((item: any) => cleanAuditText(item).toLowerCase()).filter(Boolean));
...
function buildChainVerificationAuditFilter(row: any, category = "") {
  const filter: any = {};
  if (row?.scope === "group") filter.groupId = cleanAuditText(row.id || "", 240);
  if (row?.scope === "project") {
    filter.project = cleanAuditText(row.id || "", 240);
    const name = cleanAuditText(row.name || "", 240);
    if (name && name !== filter.project) filter.projectAliases = [name];
  }
  if (category) filter.category = category;
  return filter;
}

function buildChainVerificationResyncPayload(row: any) {
  const payload: any = { staleOnly: false, limit: 20 };
  const staleSnapshots = (Array.isArray(row?.runtime?.snapshots) ? row.runtime.snapshots : [])
    .filter((snapshot: any) => snapshot?.catalogStale || snapshot?.dispatchReady === false || snapshot?.deliveryReady === false)
    .map((snapshot: any) => cleanAuditText(snapshot?.snapshotId || "", 120))
    .filter(Boolean);
  if (staleSnapshots.length) {
    payload.snapshotIds = staleSnapshots.slice(0, 20);
    return payload;
  }
  if (row?.scope === "group") payload.groupId = cleanAuditText(row.id || "", 240);
  if (row?.scope === "project") payload.projectName = cleanAuditText(row.id || "", 240);
  return payload;
}

function buildChainVerificationNextActions(row: any, status: string) {
  const base = {
    scope: row?.scope || "",
...
function chainVerificationRowIsConfigured(row: any) {
  return Number(row?.counts?.mcp || 0) + Number(row?.counts?.skill || 0) > 0;
}

function compactChainVerificationScope(row: any) {
  return {
    ...
    nextActionKinds: (Array.isArray(row?.nextActions) ? row.nextActions : [])
      .map((action: any) => cleanAuditText(action?.kind || "", 80))
      .filter(Boolean)
      .slice(0, 8),
  };
}
...
function buildChainVerificationGate(rows: any[]) {
  ...
  const nextActions = [...blockingRows, ...pendingObservationRows]
    .flatMap(row => (Array.isArray(row.nextActions) ? row.nextActions : []).map((action: any) => ({
      ...action,
      status: row.status,
  ...
  return {
    ...
    counts: {
      ...
      unconfiguredScopes: rows.filter((row: any) => !chainVerificationRowIsConfigured(row)).length,
    },
...
export function buildToolChainVerification(input: any = {}) {
  ...
  const inventory = input.inventory || buildToolAuthorizationInventory({
    projects: input.projects || loadProjectConfigs(),
    groups: input.groups || loadGroups(),
    runtimeReadiness,
  });
  ...
  const rows = (inventory.scopes || [])
    ...
    .filter((row: any) => !scopeFilter || row.scope === scopeFilter)
    .filter((row: any) => !scopeIdFilter || cleanAuditText(row.id, 240).toLowerCase() === scopeIdFilter)
    .filter((row: any) => !groupIdFilter || (row.scope === "group" && cleanAuditText(row.id, 240).toLowerCase() === groupIdFilter))
    .filter((row: any) => !projectFilter || (row.scope === "project" && (cleanAuditText(row.id, 240).toLowerCase() === projectFilter || cleanAuditText(row.name, 240).toLowerCase() === projectFilter)))
    .filter((row: any) => !statusFilter || row.status === statusFilter);
  ...
  return {
    ...
    summary: {
      ...
      unauthorizedAttempts: rows.reduce((sum: number, row: any) => sum + Number(row.invocation?.summary?.unauthorized || 0), 0),
      observedInvocations: rows.reduce((sum: number, row: any) => sum + Number(row.invocation?.summary?.totalObserved || 0), 0),
      statusCounts,
    },
    gate,
    rows,
...
function goalRequirement(id: string, label: string, status: "proven" | "partial" | "missing", evidence: any = {}, blockers: any[] = [], nextActions: any[] = []) {
  return {
    id,
    label,
    status,
...
function buildMarketplaceGoalEvidence(input: any = {}) {
  ...
  const installOrUpdate = entries.filter((entry: any) => ["install", "update"].includes(String(entry?.action || "")));
  const uninstall = entries.filter((entry: any) => String(entry?.action || "") === "uninstall");
  const sourceProof = entries.filter((entry: any) => entry?.sourceProof?.schema === "ccm-marketplace-source-proof-v1");
  const runtimeBridge = entries.filter((entry: any) =>
    entry?.runtimeImpact?.schema === "ccm-marketplace-runtime-impact-v1"
...
function buildRuntimeGoalEvidence(chain: any = {}) {
...
function hasOwnKey(value: any, key: string) {
  return !!value && Object.prototype.hasOwnProperty.call(value, key);
}

export function normalizeTruthFlag(value: any) {
  return value === true || value === 1 || String(value || "").toLowerCase() === "true" || String(value || "") === "1";
}

function normalizeRealCliRuntime(value: any) {
  const compact = String(value || "").toLowerCase().replace(/[^a-z0-9]+/g, "");
  if (["claudecode", "claudecodecli", "claude"].includes(compact)) return "claudecode";
  if (compact === "cursor") return "cursor";
  if (compact === "codex") return "codex";
  return "";
}

function normalizeRealCliProbeTarget(target: any = {}) {
  return {
...
function normalizeRealCliProbeFilter(input: any = {}) {
...
function realCliProbeMatchesFilter(probe: any, filter: any) {
...
function listRealCliProbeStatusInputs(input: any = {}) {
  const provided = input.realCliProbeStatuses || input.real_cli_probe_statuses;
  if (Array.isArray(provided)) return provided.map((probe: any) => ({ probe, sourceFile: "", source: "input" }));
  ...
  try {
    if (fs.existsSync(AGENT_PROBE_TARGET_STATUS_DIR)) {
      for (const file of fs.readdirSync(AGENT_PROBE_TARGET_STATUS_DIR)) {
        ...
        if (probe) entries.push({ probe, sourceFile: file, source: "persisted_agent_probe" });
      }
    }
  } catch {}
  return entries;
...
function normalizeRealCliProbeStatus(entry: any) {
  ...
  if (!["claudecode", "cursor", "codex"].includes(runtime)) return null;
...
function buildPersistedRealCliProbeEvidence(input: any = {}) {
  const filter = normalizeRealCliProbeFilter(input);
  const seen = new Set<string>();
  const probes = listRealCliProbeStatusInputs(input)
    .filter((entry: any) => realCliProbeMatchesFilter(entry?.probe, filter))
    .map((entry: any) => normalizeRealCliProbeStatus(entry))
    .filter(Boolean)
    .filter((probe: any) => {
      const key = [
        probe.runtime,
  ...
  for (const probe of probes) {
    ...
    if (!runtime) continue;
    if (!runtime.latest) runtime.latest = probe;
    if (probe.success) runtime.successes += 1;
    else runtime.failures += 1;
    if (probe.fresh) runtime.freshSuccesses += 1;
  }
  return {
    source: probes.some((probe: any) => probe.source === "input") ? "input_probe_status" : "persisted_agent_probe",
...
function hasExplicitRealCliEvidence(input: any = {}) {
  return hasOwnKey(input, "realCliE2E") || hasOwnKey(input, "real_cli_e2e");
}

function normalizeRealCliEvidence(value: any = {}, input: any = {}) {
...
export function buildMcpSkillGoalCompletionAudit(input: any = {}) {
  ...
  const requirements = [
    goalRequirement(
      "central_authorization_catalog",
      "CCM central MCP/Skill catalog is connected to project/group authorization",
      runtime.configuredScopes > 0 ? "proven" : "missing",
      { configuredScopes: runtime.configuredScopes, gateStatus: runtime.gateStatus },
      runtime.configuredScopes > 0 ? [] : [{ id: "no_configured_scope", detail: "No configured project/group MCP/Skill authorization scope was found" }],
      runtime.configuredScopes > 0 ? [] : [{ kind: "configure_scope_tools", label: "Configure MCP/Skill for a project or group" }],
    ),
    goalRequirement(
      "dispatch_gate_ready",
      "Runtime dispatch gate blocks unsafe scopes and allows ready scopes",
      runtime.dispatchReady ? (runtime.verified ? "proven" : "partial") : "missing",
      { gateStatus: runtime.gateStatus, dispatchReady: runtime.dispatchReady, verified: runtime.verified, needsAttention: runtime.needsAttention },
      runtime.dispatchReady ? [] : [{ id: "dispatch_gate_blocked", detail: `Gate status: ${runtime.gateStatus}` }],
      runtime.dispatchReady ? [] : [{ kind: "open_chain_verification", label: "Open chain verification blockers" }],
    ),
    goalRequirement(
      "runtime_artifact_delivery",
      "Authorized MCP/Skill runtime artifacts are ready for configured child-agent scopes",
      runtime.runtimeMissing === 0 && runtime.runtimeNeedsResync === 0 && runtime.configuredScopes > 0 ? "proven" : "missing",
      { runtimeMissing: runtime.runtimeMissing, runtimeNeedsResync: runtime.runtimeNeedsResync, configuredScopes: runtime.configuredScopes },
      [
    ...
    goalRequirement(
      "observed_child_agent_invocation",
      "Authorized MCP/Skill use has been observed through child-agent execution audit",
    ...
    goalRequirement(
      ...
      "Unauthorized MCP/Skill attempts are blocked and visible",
      runtime.unauthorizedAttempts === 0 ? "proven" : "missing",
      { unauthorizedAttempts: runtime.unauthorizedAttempts },
      runtime.unauthorizedAttempts ? [{ id: "unauthorized_attempts", detail: `${runtime.unauthorizedAttempts} unauthorized attempts found in audit` }] : [],
      runtime.unauthorizedAttempts ? [{ kind: "open_invocation_audit", label: "Inspect unauthorized invocation audit" }] : [],
    ),
    goalRequirement(
      "marketplace_lifecycle_bridge",
      "Marketplace external source lifecycle reaches authorization and runtime resync",
      marketplace.hasLifecycleEvidence && marketplace.hasRuntimeBridgeEvidence ? "proven" : (marketplace.operations > 0 ? "partial" : "missing"),
      marketplace,
      marketplace.hasLifecycleEvidence && marketplace.hasRuntimeBridgeEvidence ? [] : [{ id: "marketplace_evidence_gap", detail: "Need source proof plus install/update/uninstall runtime bridge evidence" }],
      [{ kind: "marketplace_install_e2e", label: "Install or update a marketplace MCP/Skill with auto runtime resync" }],
    ),
    goalRequirement(
      "real_cli_e2e",
      "Real Claude Code, Cursor, and Codex CLI E2E has proven native discovery and invocation",
      realCliReady ? "proven" : "missing",
      realCli,
      realCliReady ? [] : [{ id: "real_cli_e2e_missing", detail: "Artifact/self-test evidence exists, but real CLI invocation proof is not complete for all target runtimes" }],
      realCliReady ? [] : [{ kind: "run_real_cli_matrix", label: "Run real Claude Code/Cursor/Codex E2E matrix" }],
    ),
  ];
  const counts = requirements.reduce((acc: any, item: any) => {
    acc[item.status] = Number(acc[item.status] || 0) + 1;
    return acc;
  }, { proven: 0, partial: 0, missing: 0 });
  const complete = requirements.every((item: any) => item.status === "proven");
  return {
...
export function buildToolChainVerificationSelfTestRow(input: any = {}) {
