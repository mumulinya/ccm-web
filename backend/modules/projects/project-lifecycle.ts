...
const LIFECYCLE_DIR = path.join(CCM_DIR, "project-lifecycle");
const STATE_FILE = path.join(LIFECYCLE_DIR, "state.json");
const AUDIT_FILE = path.join(LIFECYCLE_DIR, "audit.jsonl");
const PREVIEW_TTL_MS = 10 * 60 * 1000;
const purgePreviews = new Map<string, { project: string; fingerprint: string; expiresAt: number }>();

type LifecycleState = Record<string, { archived_at: string; config_file: string }>;
...
function readState(): LifecycleState {
  ensureDirectories();
  try { return JSON.parse(fs.readFileSync(STATE_FILE, "utf-8")); } catch { return {}; }
}

function writeState(state: LifecycleState) {
  ensureDirectories();
...
function recordAudit(action: string, project: string, detail: Record<string, unknown> = {}) {
  ensureDirectories();
...
function configFile(name: string, archived = false) {
  return resolveContainedPath(archived ? ARCHIVE_DIR : CONFIGS_DIR, `config-${validateProjectName(name)}.toml`);
}
...
function fileDescriptor(label: string, target: string) {
  if (!fs.existsSync(target)) return { label, path: target, exists: false, bytes: 0 };
  const stat = fs.statSync(target);
  let bytes = stat.size;
  if (stat.isDirectory()) {
    bytes = fs.readdirSync(target, { withFileTypes: true }).reduce((total, entry) => {
      if (!entry.isFile()) return total;
      try { return total + fs.statSync(path.join(target, entry.name)).size; } catch { return total; }
    }, 0);
  }
  return { label, path: target, exists: true, bytes, modified_at: stat.mtime.toISOString(), kind: stat.isDirectory() ? "directory" : "file" };
}

function buildPurgeImpact(name: string) {
  const project = validateProjectName(name);
  const webSessions = resolveContainedPath(WEB_SESSIONS_DIR, project);
  const ccSession = findCcSessionFile(project);
  const archivedConfig = configFile(project, true);
  const logFile = resolveContainedPath(LOG_DIR, `${project}.log`);
  const items = [
...
export function restoreProject(name: string) {
  const project = validateProjectName(name);
  const source = configFile(project, true);
  const destination = configFile(project);
  if (!fs.existsSync(source)) throw new Error("归档项目不存在");
  if (fs.existsSync(destination)) throw new Error("活动项目中已存在同名项目");
  fs.renameSync(source, destination);
  const state = readState();
...
export function previewProjectPurge(name: string) {
  const impact = buildPurgeImpact(name);
  if (!fs.existsSync(configFile(impact.project, true))) throw new Error("仅已归档项目可以永久删除");
...
export function purgeArchivedProject(name: string, previewToken: string) {
  const impact = buildPurgeImpact(name);
...
export function getProjectLifecycleAudit(limit = 100) {
  ensureDirectories();
