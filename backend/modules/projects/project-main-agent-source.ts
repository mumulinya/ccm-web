...
export type ProjectSourceEvidence = {
  ...
  files: Array<{ path: string; checksum: string; chars: number; content: string }>;
  totalChars: number;
  truncated: boolean;
};
...
function safeResolvedFile(root: string, relativePath: string) {
...
function redactSensitiveValues(content: string) {
  return content.replace(
    /^(\s*(?:password|passwd|secret|token|api[-_.]?key|private[-_.]?key|access[-_.]?key)\s*[:=]\s*)(.+)$/gim,
    "$1[REDACTED]",
  );
}

function sourcePriority(relativePath: string) {
  const lower = relativePath.toLowerCase();
  const base = path.posix.basename(lower);
  if (["package.json", "pom.xml", "build.gradle", "build.gradle.kts", "settings.gradle", "settings.gradle.kts", "cargo.toml", "go.mod"].includes(base)) return 0;
  if (base.startsWith("readme") || base === "dockerfile") return 1;
  if (/(?:^|\/)(?:src|app|server|backend|frontend|api|lib)\//.test(lower)) return 2;
  if (/\.(?:ts|tsx|js|jsx|java|kt|go|rs|py|vue|svelte|cs)$/.test(lower)) return 3;
  return 4;
}

export function buildProjectSourceManifest(project: string, workDir: string) {
  const root = path.resolve(workDir);
  if (!fs.existsSync(root) || !fs.statSync(root).isDirectory()) throw new Error("项目源码目录不存在");
  const files: ProjectSourceManifestEntry[] = [];
  ...
  while (stack.length && scanned < MAX_SCAN_FILES) {
    ...
    for (const entry of entries) {
      ...
      if (!entry.isFile() || isSensitivePath(relative) || !isTextSourceFile(entry.name)) continue;
      try {
  ...
  return {
    schema: "ccm-project-main-source-manifest-v1" as const,
    project,
    workDir: root,
    files: limited,
...
export function readProjectSourceEvidence(input: {
  project: string;
  workDir: string;
  ...
  const files: ProjectSourceEvidence["files"] = [];
  const rejectedPaths: ProjectSourceEvidence["rejectedPaths"] = [];
  let totalChars = 0;
  let truncated = false;
  ...
  return {
    schema: "ccm-project-main-source-evidence-v1" as const,
    ...
    totalChars,
    truncated,
  } satisfies ProjectSourceEvidence;
}

export function projectSourceEvidencePrompt(evidence: ProjectSourceEvidence) {
  const header = [
    "[当前项目只读源码证据]",
...
export function runProjectMainSourceContractSelfTest() {
  return {
    success: true,
    limits: {
