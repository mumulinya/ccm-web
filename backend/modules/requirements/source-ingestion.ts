...
function fileKind(name: string) {
  ...
  if ([".docx", ".pptx", ".xlsx"].includes(ext)) return ext.slice(1);
  if ([".txt", ".md", ".json", ".csv", ".yaml", ".yml", ".toml", ".xml", ".html", ".css", ".js", ".jsx", ".ts", ".tsx", ".vue", ".log", ".py", ".java", ".go", ".rs", ".sql"].includes(ext)) return "text";
...
async function analyzeImageWithConfiguredModel(filePath: string, name: string, configOverride?: any) {
  ...
  if (stat.size > MAX_VISION_IMAGE_BYTES) throw new Error("图片超过 12 MB，无法发送给视觉模型");
...
async function parseUploadedFile(file: UploadedFile, options: { visionConfig?: any } = {}): Promise<RequirementSourceRecord> {
  ...
  const stat = fs.statSync(filePath);
  base.size = stat.size;
  if (stat.size > MAX_REQUIREMENT_FILE_BYTES) {
...
function isPrivateIpv4(ip: string) {
  ...
  if (parts.length !== 4 || parts.some(value => !Number.isFinite(value))) return true;
  return parts[0] === 10
    || parts[0] === 127
    || parts[0] === 0
...
function looksLikeAuthorizationPage(text: string, url: string) {
...
export function extractOnlineDocumentUrls(text: string) {
  const matches = String(text || "").match(/https?:\/\/[^\s<>"'，。；！？）\]]+/gi) || [];
  return unique(matches.map(value => value.replace(/[),.;]+$/, "")), 3);
}

function renderSourcesForAgent(sources: RequirementSourceRecord[]) {
  if (!sources.length) return "";
  const rows = sources.map(source => {
    const meta = [`状态=${source.status}`, `解析器=${source.parser}`];
    if (source.path) meta.push(`本地路径=${source.path}`);
    if (source.url) meta.push(`来源链接=${source.url}`);
    if (source.readable && source.content) return `--- ${source.name}（${meta.join("；")}） ---\n${source.content}`;
...
export function shouldDecomposeRequirementIntent(input: {
  modelDecision?: { actionRequired?: boolean; intentKind?: string; decomposeRequirement?: boolean } | null;
} = {}): boolean {
  const decision = input.modelDecision;
  if (!decision) throw new Error("需求拆解意图缺少模型语义决定");
  if (typeof decision.decomposeRequirement === "boolean") return decision.decomposeRequirement;
...
export function requirementToIntakeDraft(requirement: BusinessRequirementExtraction | null, fallback: any = {}) {
  return {
