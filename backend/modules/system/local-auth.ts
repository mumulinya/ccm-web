...
import { getEffectiveAccess, removeUserAccess } from "./access-policy";
...
function normalizeRole(value: any): AuthRole { return value === "admin" ? "admin" : "user"; }
...
function validateRole(value: any): AuthRole { if (!['user', 'admin'].includes(String(value || ''))) throw new Error("账户角色无效"); return value as AuthRole; }
...
export function localAuthPublicState(req: IncomingMessage) { const users = peekUsers(); const auth = resolveLocalAuthSession(req); const access = auth ? getEffectiveAccess(auth.user.id, auth.user.role) : { policyRevision: 0, features: [], resources: [] }; return { authenticated: !!auth, registration_enabled: users.registrationEnabled, first_install: users.users.length === 0, login_theme: users.loginTheme, user: publicUser(auth?.user), capabilities: auth?.capabilities || [], access, csrf: auth?.session.csrfToken || null, session_error: (req as any).ccmSessionError || null, session: auth ? { id: auth.session.id, created_at: auth.session.createdAt, last_seen_at: auth.session.lastSeenAt, expires_at: auth.session.expiresAt } : null }; }
...
export function handleLocalAuthApi(pathname: string, req: IncomingMessage, res: ServerResponse) {
  ...
  if (pathname === "/api/auth/users" && req.method === "GET") { const auth = requireUser(req, res, true); if (!auth) return true; sendJson(res, { success: true, users: loadUsers().users.map(user => ({ ...publicUser(user), access: getEffectiveAccess(user.id, user.role) })) }); return true; }
  if (pathname === "/api/auth/users" && req.method === "POST") { const auth = requireUser(req, res, true); if (!auth) return true; void readJsonBody(req).then(payload => { const username = validateUsername(payload.username); const password = validatePassword(payload.password); const created = withFileLock(USERS_FILE, () => { const store = readUsersUnlocked(); if (store.users.some(item => item.normalizedUsername === normalizedUsername(username))) throw new Error("用户名已存在"); const createdAt = now(); const user: StoredUser = { id: `usr_${crypto.randomUUID()}`, username, normalizedUsername: normalizedUsername(username), role: "user", password: hashPassword(password), createdAt, updatedAt: createdAt, securityAudit: [{ at: createdAt, action: "admin_created", actorId: auth.user.id }] }; store.users.push(user); saveUsers(store); return user; }); sendJson(res, { success: true, user: publicUser(created) }, 201); }).catch((error: any) => sendJson(res, { success: false, error: error?.message || "创建用户失败" }, 400)); return true; }
  const resetPasswordMatch = pathname.match(/^\/api\/auth\/users\/([^/]+)\/password-reset$/);
  if (resetPasswordMatch && req.method === "POST") { const auth = requireUser(req, res, true); if (!auth) return true; void readJsonBody(req).then(payload => { const userId = decodeURIComponent(resetPasswordMatch[1]); const next = validatePassword(payload.password); withFileLock(USERS_FILE, () => { const store = readUsersUnlocked(); const target = store.users.find(item => item.id === userId); if (!target) throw new Error("用户不存在"); target.password = hashPassword(next); auditUser(target, "password_reset", auth.user.id); saveUsers(store); }); revokeUserSessions(userId, "admin_password_reset"); sendJson(res, { success: true }); }).catch((error: any) => sendJson(res, { success: false, error: error?.message || "重置密码失败" }, 400)); return true; }
  ...
  if (deleteUserMatch && req.method === "DELETE") { const auth = requireUser(req, res, true); if (!auth) return true; try { const userId = decodeURIComponent(deleteUserMatch[1]); if (userId === auth.user.id) throw new Error("管理员不能删除自己"); withFileLock(USERS_FILE, () => { const store = readUsersUnlocked(); const target = store.users.find(item => item.id === userId); if (!target) throw new Error("用户不存在"); if (target.role === "admin" && !target.disabledAt && activeAdmins(store).length <= 1) throw new Error("不能删除最后一个有效管理员"); store.users = store.users.filter(item => item.id !== userId); saveUsers(store); }); removeUserAccess(userId, auth.user.id); revokeUserSessions(userId, "account_deleted"); sendJson(res, { success: true }); } catch (error: any) { sendJson(res, { success: false, error: error?.message || "删除失败" }, 400); } return true; }
