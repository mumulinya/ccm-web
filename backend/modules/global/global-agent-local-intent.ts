...
export function resolveImplicitCurrentProject(message: string, projects: string[]) {
 ...
 */
export function hasExplicitDevelopmentExecutionIntent(message: string) {
...
export function normalizeCronHour(raw: string, text: string) {
...
export function inferLocalConversationFallback(message: string): LocalIntentResult | null {
...
export function hasExplicitGlobalWriteAuthorization(message: string) {
    const text = normalizeText(message);
    if (!text) return false;
...
export function inferLocalGlobalAction(message: string, projects: string[], groups: any[], resources: any = {}): LocalIntentResult | null {
  const text = normalizeText(message);
  if (!text) return null;
  ...
  if (
    /(?:关掉|关闭|停止|停下|暂停|别放|不要放).{0,8}(?:音乐|歌曲|歌|播放)|(?:音乐|歌曲|播放).{0,8}(?:关掉|关闭|停止|停下|暂停)|停歌|别放歌|不要放歌/.test(text)
  ...
  if (/(播放|放一首|放|来一首|来点|听|我想听|我要听|搜首歌|搜索.*歌)/.test(text) && !/页面|列表|打开音乐/.test(text)) {
    ...
    return {
      ...
      action: { type: "play_music", params: { keyword: RANDOM_MUSIC_KEYWORD, request_text: text, random: true } }
    };
  }
