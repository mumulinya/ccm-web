...
import { SESSIONS_DIR } from "../../core/utils";
import { loadMcpTools, loadSkills } from "../../core/db";
import { toolManager } from "../../tools/tool-manager";
import { buildToolAuthorizationOptions } from "../../tools/tool-authorization";
import {
  beginFeishuReactionFeedback,
  finishFeishuReactionFeedback,
} from "../../integrations/feishu-reaction-feedback";
import {
  getGlobalAgentToolAuthorizationPayload,
...
import { resolveFeishuInboundAttachments } from "../../integrations/feishu-inbound-attachments";
...
function normalizeGlobalRequestedTargets(value: any, message = "") {
  ...
  const available = listGlobalDispatchTargets();
  const byKey = new Map(available.map((item: any) => [`${item.scope}:${item.scopeId}`, item]));
  const requestedRows = Array.isArray(rows) ? rows : [];
  const explicit = requestedRows.map((item: any) => {
  ...
  if (requestedRows.length) {
    if (explicit.some((item: any) => !item || item.ready !== true)) throw new Error("选择的项目或群聊已经不可投放，请刷新目标列表后重试");
    return [...new Map(explicit.map((item: any) => [`${item.scope}:${item.scopeId}`, item])).values()];
  }
  const text = String(message || "");
  const exactNameMentioned = (nameValue: any) => {
  ...
  const matches = available.filter((item: any) => item.ready === true && [item.canonicalName, item.displayName]
    .some(exactNameMentioned));
  const ambiguousNames = new Set<string>();
  for (const match of matches) {
    for (const name of [match.canonicalName, match.displayName].filter(Boolean)) {
      if (matches.filter((item: any) => item.canonicalName === name || item.displayName === name).length > 1) ambiguousNames.add(String(name));
    }
  }
  return matches.filter((item: any) => !ambiguousNames.has(String(item.canonicalName)) && !ambiguousNames.has(String(item.displayName)));
}

function resolveControlBotAcpPlatformContext(acpSessionIdValue: string) {
  const acpSessionId = String(acpSessionIdValue || "").trim();
  if (!acpSessionId || acpSessionId.length > 240 || !fs.existsSync(SESSIONS_DIR)) throw new Error("全局 ACP 会话 ID 无效");
  const files = fs.readdirSync(SESSIONS_DIR)
    .filter((file) => /^ccm-control-bot(?:_[^/\\]+)?\.json$/i.test(file))
    .map((file) => ({ file, mtime: fs.statSync(path.join(SESSIONS_DIR, file)).mtimeMs }))
    .sort((a, b) => b.mtime - a.mtime || a.file.localeCompare(b.file));
  for (const candidate of files) {
    try {
      const store = JSON.parse(fs.readFileSync(path.join(SESSIONS_DIR, candidate.file), "utf-8"));
...
export function createGlobalAgentApi(deps: any) {
  ...
  const globalFeishuSessionSnapshot = () => {
    ...
    const sessions = (store.sessions || [])
      .filter((session: any) => String(session.source || "web") === "feishu")
      .map((session: any) => ({
        ...session,
        feishuBindings: bindings.filter((binding: any) => String(binding.active_session_id || "") === String(session.id || "")),
      }));
    ...
    try {
      while (true) {
        ...
        const resolvedCandidate = resolvedRoute === "continue_original"
          ...
          : null;
        if (resolvedCandidate) routeHistory.push({ role: "system", content: `The user explicitly chose to continue the original task. Recoverable task summary: ${JSON.stringify(buildRecoverableTaskSummary(resolvedCandidate))}.` });
        try {
  ...
  function handleGlobalAgentApi(
    ...
    if (pathname === "/api/global-agent/feishu-sessions/create" && req.method === "POST") {
      ...
      }).catch((error: any) => sendJson(res, { success: false, error: error?.message || "创建飞书会话失败" }, 400));
      return true;
    }
    ...
    if (pathname === "/api/global-agent/feishu-sessions/bind" && req.method === "POST") {
      void readJsonRequest(req).then((payload: any) => {
        ...
        if (action === "bind") {
          const session = (loadGlobalAgentHistoryStore().sessions || []).find((item: any) => String(item.id || "") === sessionId);
          if (!session || String(session.source || "web") !== "feishu") throw new Error("只能将飞书目标绑定到飞书会话");
        }
      ...
      }).catch((error: any) => sendJson(res, { success: false, error: error?.message || "更新飞书会话绑定失败" }, 400));
      return true;
    }
    ...
    if (pathname === "/api/global-agent/feishu-sessions/delete" && req.method === "POST") {
      ...
      }).catch((error: any) => sendJson(res, { success: false, error: error?.message || "删除飞书会话失败" }, 400));
      return true;
    }
    ...
    if (pathname === "/api/feishu/control-bot/message" && req.method === "POST") {
      ...
      req.on("end", async () => {
        ...
        const finishReaction = (status: "completed" | "failed") => {
          if (!reactionInput) return;
          try {
            finishFeishuReactionFeedback({ ...reactionInput, status });
        ...
        try {
          ...
          if (isAcp && Array.isArray(payload.cc_connect_attachment_refs) && payload.cc_connect_attachment_refs.length) {
            const resolvedAttachments = await resolveFeishuInboundAttachments({
              messageId: getFeishuMessageId(payload),
          ...
          if (!isAcp && /^om_[a-z0-9_-]{8,200}$/i.test(messageId)) {
            ...
            try {
              beginFeishuReactionFeedback(reactionInput);
            } catch (reactionError: any) {
    ...
    if (pathname === "/api/global-agent/tools" && req.method === "POST") {
      void readJsonRequest(req)
        .then(payload => saveGlobalAgentToolAuthorization(payload))
        .then(result => sendJson(res, { success: true, ...result }))
        .catch((error: any) => sendJson(res, { success: false, error: error?.message || "保存全局 Agent 工具配置失败" }, 400));
      return true;
    }

    if (pathname === "/api/global-agent/tools/self-test" && req.method === "GET") {
      const result = runGlobalAgentToolAuthorizationSelfTest();
    ...
    if (pathname === "/api/global-agent/run" && req.method === "POST") {
      ...
      const handleRun = async (payload: any, files: any[] = []) => {
        ...
        const emit = (event: any) => {
          ...
          if (["text", "response_delta"].includes(String(event?.type || "")) && String(event?.text || "").trim()) visibleTextEmitted = true;
        ...
        const emitCompleteGlobalReply = (content: string) => {
          const text = String(content || "");
          if (text && !res.destroyed && !res.writableEnded) emit({ type: "response_delta", text, final: true });
        };
        try {
          ...
          if (explicitCandidate) {
            history.push({ role: "system", content: `The user explicitly chose to continue the original task. Recoverable task summary: ${JSON.stringify(buildRecoverableTaskSummary(explicitCandidate))}. Preserve this task identity and do not create a duplicate task.` });
          } else if (recoverableCandidates.length) {
            history.push({ role: "system", content: `Recoverable task summaries for the exact session: ${JSON.stringify(recoverableCandidates.slice(0, 3).map(buildRecoverableTaskSummary))}. Decide continuationKind from the current message; do not resume merely because an old task exists.` });
          }
          ...
          const suppliedTurn = suppliedTurnId
            ? conversationTurnControl.listInternal({ scope: "global", conversation_id: sessionId, limit: 500 }).turns.find((item: any) => item.id === suppliedTurnId)
            : null;
          ...
          if (isStream) emit({ type: "response_completed", final: true });
          conversationTurnControl.settle({
            id: activeTurn.id,
            status: run.status === "failed" ? "failed" : "completed",
            run_id: run.id,
          ...
          if (activeTurn?.id && error?.code === "CONVERSATION_ROUTE_REQUIRED") {
            try {
              ...
              if (isStream) {
                ...
                res.end();
              } else sendJson(res, { success: true, route_required: true, turn: { id: routed.id, revision: routed.revision, status: routed.status, routing: routed.routing } }, 202);
              return;
