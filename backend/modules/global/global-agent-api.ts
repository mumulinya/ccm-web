    ...
    try {
      while (true) {
        ...
        const resolvedCandidate = resolvedRoute === "continue_original"
          ...
          : null;
        if (resolvedCandidate) routeHistory.push({ role: "system", content: `用户明确选择继续原任务。可恢复任务摘要：${JSON.stringify(buildRecoverableTaskSummary(resolvedCandidate))}。` });
        try {
      ...
      const handleRun = async (payload: any, files: any[] = []) => {
        ...
        try {
          ...
          if (explicitCandidate) {
            history.push({ role: "system", content: `用户明确选择继续原任务。可恢复任务摘要：${JSON.stringify(buildRecoverableTaskSummary(explicitCandidate))}。必须沿用该任务目标，不得创建重复任务。` });
          } else if (recoverableCandidates.length) {
            history.push({ role: "system", content: `当前精确会话的可恢复任务摘要：${JSON.stringify(recoverableCandidates.slice(0, 3).map(buildRecoverableTaskSummary))}。请结合当前消息判断 continuationKind；不要仅因为存在旧任务就续接。` });
          }
