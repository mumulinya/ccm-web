...
import {
  ...
  resolveGlobalAgentExecutionSkills,
...
export function createGlobalAgentAgenticRuntime(deps: any) {
  ...
  function verifyGlobalAgentContextBoundary(context: any = {}) {
    ...
    for (const project of Array.isArray(context?.projects) ? context.projects : []) {
      for (const key of Object.keys(project || {})) if (!new Set(["name", "display_name", "work_dir", "agent", "platform"]).has(key)) issues.push(`global_context_project_directory_field_not_allowed:${key}`);
    }
  ...
  function buildAgenticContext(query = "", sessionId = "", options: any = {}) {
    ...
    const authorizedTools = buildGlobalAgentToolRuntimeContext({
      ...
      executionSkills: resolveGlobalAgentExecutionSkills({
        message: query,
        source: String(options.source || "global-agent-context"),
    ...
    const context: any = {
      ...
      requested_dispatch_targets: {
        ...
        targets: (Array.isArray(options.requestedTargetRefs || options.requested_target_refs) ? (options.requestedTargetRefs || options.requested_target_refs) : [])
          .map((target: any) => ({ scope: target.scope, scope_id: target.scopeId || target.scope_id, name: target.displayName || target.canonicalName || target.name }))
          .filter((target: any) => target.scope && target.scope_id),
      ...
      global_shared_files: {
        ...
        files: globalSharedFiles.files.map((file: any) => ({
          id: file.id,
          name: file.name,
  ...
  function buildGlobalProviderPayloadSnapshot(messages: Array<{ role: string; content: string }>, sessionId: string, run?: GlobalAgentRun) {
    ...
    if (selectedSkillStart >= 0) skillSections.push(systemText.slice(selectedSkillStart));
    const authorizedTools = buildGlobalAgentToolRuntimeContext(
      { sessionId, taskId: String(run?.id || ""), source: "global-agent-provider-payload" },
      (run?.loaded_tool_names || run?.loadedToolNames || []) as string[],
    ...
    const loadedSkills: any[] = authorizedTools.catalog.skills
      .map((skill: any) => ({
        ...
        loadLevel: restoredSkillNames.has(String(skill?.name || "")) ? "body" as const : "catalog" as const,
        checksum: String(skill?.contentHash || ""),
        loadSource: restoredSkillNames.has(String(skill?.name || "")) ? "post_compact_restored" as const : "catalog" as const,
        tokens: Number((authorizedTools.restored_skill_attachments || []).find((item: any) => item?.name === skill?.name)?.tokenCount || 0),
      }));
    const allSkillCatalog = new Map((Array.isArray(loadSkills()) ? loadSkills() : [])
      .map((skill: any) => [String(skill?.name || ""), skill]));
    for (const match of systemText.matchAll(/^## Skill:([^\r\n]+)/gm)) {
      const name = String(match[1] || "").trim();
      if (!name || loadedSkills.some((skill: any) => skill.name === name)) continue;
      const skill: any = allSkillCatalog.get(name) || null;
      loadedSkills.push({
        kind: "skill",
        name,
    ...
    const modelVisibleToolSpecs = GLOBAL_AGENT_TOOL_SPECS.filter(spec => !isGlobalDeferredTool(spec.name, loadedToolNames));
    const invocations = (Array.isArray(run?.steps) ? run!.steps : []).flatMap((step: any) => {
      const wrapper = String(step?.tool?.name || "");
      const kind: "skill" | "mcp" | "" = wrapper === "invoke_skill" ? "skill" : wrapper === "invoke_mcp" ? "mcp" : "";
      ...
      if (!kind || !name || (step?.observation === undefined && !step?.error)) return [];
      return [{
        kind,
        name,
    ...
    return buildModelVisiblePayloadSnapshot({
      ...
      contextComponents: {
        ...
        subagentDefinitions: {
          projects: getConfigs().map((project: any) => String(project?.name || "")).filter(Boolean),
          groups: loadGroups().map((group: any) => ({ id: String(group?.id || ""), name: String(group?.name || "") })),
        },
        loadedContextItems: {
  ...
  function compactGlobalProviderMessageContent(value: any, maxChars: number) {
   ...
   */
  function buildEmergencyGlobalProviderMessages(messages: Array<{ role: string; content: string }>, run: GlobalAgentRun) {
  ...
  async function prepareGlobalProviderMessages(
    ...
    const sessionPreferences: any = readSlashCommandSessionState("global", "global", sessionId).preferences;
    ...
    if (compaction?.reason === "circuit_breaker") return buildEmergencyGlobalProviderMessages(messages, run);
    if (compaction?.compacted !== true) return buildEmergencyGlobalProviderMessages(messages, run);
    ...
    if (postCompactGate.providerCallAllowed !== true || rebuiltPayload.totalTokens >= modelCapacity.contextWindow) return buildEmergencyGlobalProviderMessages(rebuiltMessages, run);
  ...
  async function executeAgenticTool(baseUrl: string, ctx: CollabCtx, name: string, args: any, run: GlobalAgentRun, onEvent?: (event: any) => void, signal?: AbortSignal) {
    ...
    if (isGlobalDeferredTool(name, loadedNames)) {
      throw new Error(`MAIN_AGENT_TOOL_SCHEMA_NOT_LOADED:${name}`);
    }
    if (name === "invoke_mcp") {
      const preflight = buildGlobalAgentToolRuntimeContext(
        { taskId: run.id, sessionId: run.session_id, source: run.source || "global-agent-preflight" },
        run.loaded_tool_names || run.loadedToolNames || [],
        { executionSkills: resolveGlobalAgentExecutionSkillsFromRun(run) },
      );
      const requested = String(args?.tool_name || args?.toolName || args?.name || "");
      const deferred = preflight.discoverable_tools.find((tool: any) => requested === tool.canonicalName || requested === tool.name || requested === `${tool.server}/${tool.name}`);
    ...
    if (!operation.acquired) {
      ...
      if (settled?.status === "completed") {
        ...
        if (name === "tool_search") {
          ...
          const names = rows.map((tool: any) => String(tool?.canonicalName || tool?.name || "")).filter(Boolean);
    ...
    const executionSkills = resolveGlobalAgentExecutionSkillsFromRun(run);
    const sourceRuntime = buildGlobalAgentToolRuntimeContext(
      { taskId: run.id, executionId: operationKey, sessionId: run.session_id, source: run.source || "global-agent-source" },
      run.loaded_tool_names || run.loadedToolNames || [],
      { executionSkills },
    );
    ...
    try {
      ...
      if (name === "invoke_skill") {
        observation = await executeGlobalAgentAuthorizedTool("skill", args, {
          taskId: run.id,
        ...
        observation = await executeGlobalAgentAuthorizedTool("mcp", args, {
          taskId: run.id,
          executionId: operationKey,
          sessionId: run.session_id,
          source: run.source || "global-agent",
        ...
        if (name === "tool_search") {
          run.loaded_tool_names = Array.from(new Set((toolContext.loadedToolNames || []).map((value: any) => String(value || "")).filter(Boolean)));
          run.loadedToolNames = run.loaded_tool_names.slice();
          saveGlobalAgentRun(run, true);
        }
        ...
        const workflowDecision = (run.workflow_decision || run.workflowDecision) as any;
        if (!isDevelopmentTaskWorkflowDecision(workflowDecision)) {
          throw Object.assign(new Error("当前请求没有明确要求修改代码或项目配置，因此不会创建开发任务；我会先以只读分析方式回答。"), {
            code: "CCM_DEVELOPMENT_TASK_WRITE_INTENT_REQUIRED",
          });
        }
        const continuationTaskId = String(
          workflowDecision?.continuationTaskId
        ...
        if (continuationTaskId) {
          ...
          const boundTask = (loadTasks() || []).find((item: any) => String(item?.id || "") === continuationTaskId);
          if (!boundTask) {
          ...
          const continuationKind = String(workflowDecision?.continuationKind || "supplement") === "revise_goal"
            ? "revise_goal"
            : "supplement";
          const continuation = await Promise.resolve(continueBoundTaskWithMessage(
            continuationTaskId,
            String(run.original_user_message || run.user_message || ""),
            ctx,
            {
              continuationKind,
          ...
          const globalMissionId = String(
            boundTask.global_mission_id
          ...
          (run as any).conversation_route_kind = String(workflowDecision?.conversationRouteKind || "resume_existing_task");
          (run as any).continuation_task_id = continuationTaskId;
          return {
            success: true,
            ...
            route_kind: (run as any).conversation_route_kind,
        ...
        const groundedTargets = Array.isArray((run as any).requested_target_refs) ? (run as any).requested_target_refs : [];
        if (groundedTargets.length) {
          const allowed = new Set(groundedTargets.map((target: any) => `${String(target.scope || target.type)}:${String(target.scopeId || target.scope_id || target.id)}`));
          const missionTargets = Array.isArray((missionArgs as any).targets) ? (missionArgs as any).targets : [];
          for (const target of missionTargets) {
            ...
            if (!allowed.has(`${scope}:${scopeId}`)) throw new Error(`任务目标 ${scope}:${scopeId} 不在用户本轮明确选择的投放范围内`);
        ...
        const files = Array.isArray(args.files) ? args.files.map((item: any) => String(item || "").trim()).filter(Boolean) : [];
        if (!files.length) throw new Error("全局 Agent提交代码必须提供精确文件清单，禁止隐式提交整个工作区");
  ...
  function createAgenticRuntime(baseUrl: string, ctx: CollabCtx, input: {
    ...
    routeGuard?: (workflowDecision: any) => void;
    ...
    const runtime: GlobalAgentLoopRuntime = {
      callModel: async (messages, run, signal) => {
        ...
        const sessionState = readSlashCommandSessionState("global", "global", String(run.session_id || input.sessionId || ""));
        const config = { ...baseConfig, model: (sessionState.preferences as any)?.model || baseConfig.model, reasoningEffort: (sessionState.preferences as any)?.effort || baseConfig.reasoningEffort };
        if (!config.apiKey || !config.apiUrl || !config.model) throw new Error("统一大模型尚未配置");
        const directive = renderSlashCommandSessionDirective("global", "global", String(run.session_id || input.sessionId || ""));
        ...
        const invoke = (providerMessages: Array<{ role: string; content: string }>) => {
          ...
          (run as any).main_agent_prompt_bindings = buildInternalPromptBindings({
            scope: "global",
          ...
          const streamMetric: any = (run as any).streaming_metric || ((run as any).streaming_metric = {
          ...
          const markVisibleFeedback = (at = Date.now()) => {
            if (!streamMetric.firstVisibleFeedbackMs && Number.isFinite(runStartedAt)) streamMetric.firstVisibleFeedbackMs = Math.max(0, at - runStartedAt);
          ...
          const markProviderToken = (at = Date.now()) => {
            if (!streamMetric.firstTokenMs && Number.isFinite(runStartedAt)) streamMetric.firstTokenMs = Math.max(0, at - runStartedAt);
          };
          const modelCallIndex = Math.max(0, Number((run as any).main_model_call_count || 0));
          (run as any).main_model_call_count = modelCallIndex + 1;
          (run as any).latest_model_visible_payload = buildGlobalProviderPayloadSnapshot(providerMessages, String(run.session_id || ""), run);
          ...
          const activity = createModelActivityController({
            ...
            generation: Number((run as any).generation || run.resume_count || 0),
          ...
          return runGlobalNativeQueryCall({
            ...
            try {
              const synthesized = await callGlobalModelWithRetry(config, [
                ...
                { role: "user", content: JSON.stringify({ request: String(run.original_user_message || run.user_message || "").slice(0, 4000), draft: String((result as any)?.message || "").slice(0, 8000) }) },
              ], {
                signal,
              ...
              return { ...(result as any), message: String(synthesized || (result as any)?.message || "") };
            } catch (error) {
      ...
      onWorkflowDecision: (workflowDecision, run, modelCallIndex, modelDecision) => {
        ...
        const targetRequired = isDevelopmentTaskWorkflowDecision(workflowDecision)
          && ["orchestrate_development", "send_project_cmd", "send_group_cmd", "create_task"].includes(requestedToolName);
        if (groundedTargets.length) {
          const targetRefs = groundedTargets.map((target: any) => ({
            type: target.scope,
            id: target.scopeId,
        ...
        const decision = normalizeMainAgentTurnDecision({
          ...
          turnId: String((run as any).turn_id || input.turnId || run.id),
        ...
        const receipt = createMainAgentTurnReceipt({
          decision,
          modelCallIndex,
          toolRound: Math.max(0, modelCallIndex - 1),
          usage: (run as any).latest_context_usage || null,
          inputIdentity: { sessionId: run.session_id, turnId: (run as any).turn_id || input.turnId || "", message: input.authorizationMessage || run.original_user_message || run.user_message },
          promptBindings: (run as any).main_agent_prompt_bindings,
        });
        (run as any).main_agent_turn_decision = decision;
        (run as any).main_agent_turn_receipt = receipt;
        input.onEvent?.({ type: "turn_decision", decision: publicMainAgentTurnDecision(decision), receipt });
        if (modelCallIndex !== 1) return;
  ...
  async function runAgenticGlobalRequest(baseUrl: string, ctx: CollabCtx, input: {
    ...
    let clarificationCandidate: any = null;
    ...
    if (input.onEvent) {
      ...
      if (canonicalReply.trim()) input.onEvent({ type: "text", text: canonicalReply, run_id: run.id, trace_id: run.trace_id, canonical: true });
    ...
    try {
      recordGlobalAgentSessionProviderUsage(sessionId, {
        ...
        protocol: String((run as any).latest_context_usage?.protocol || loadOrchestratorConfig()?.format || ""),
        endpoint: String((run as any).latest_context_usage?.endpoint || loadOrchestratorConfig()?.apiUrl || ""),
        anchorMessageId: assistantMessageId,
        currentRequest: { role: "user", content: input.message },
        fixedContext: { main_agent_loop: true },
        tools: (run as any).direct_reply_fast_path === true ? [] : GLOBAL_AGENT_TOOL_SPECS.filter(spec => !isGlobalDeferredTool(spec.name, run.loaded_tool_names || run.loadedToolNames || [])),
        modelVisiblePayload: (run as any).latest_model_visible_payload || null,
        contextComponents: (() => {
          if ((run as any).direct_reply_fast_path === true) return { skills: [], mcpTools: [] };
          const authorizedTools = buildGlobalAgentToolRuntimeContext(
            { taskId: run.id, sessionId: run.session_id, source: run.source || "global-agent-usage" },
            run.loaded_tool_names || run.loadedToolNames || [],
            { executionSkills: resolveGlobalAgentExecutionSkillsFromRun(run) },
          );
          return {
