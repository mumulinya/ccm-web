...
import { resolveFeishuUserAccess, signFeishuCardAction, verifyFeishuCardAction } from "../collaboration/feishu-access";
...
import { runConversationAside } from "../tools/slash-command-conversations";
import {
  extractFeishuEventResourceHints,
...
export function createGlobalAgentFeishuChannel(deps: any) {
  ...
  function formatFeishuTaskJourney(run: GlobalAgentRun, mission: any, sourceIngestion: any, fallback: string) {
    ...
    if (status === "waiting_confirmation") {
      ...
      lines.push(base);
      const clarification = (run as any).clarification_summary?.pre_plan_clarification
        || (run as any).clarificationSummary?.prePlanClarification;
      if (Array.isArray(clarification?.questions) && clarification.questions.length) {
        lines.push(clarification.questions.map((question: any, index: number) => {
          const options = Array.isArray(question.options) && question.options.length
            ? `\n${question.options.map((option: any, optionIndex: number) => `   ${optionIndex + 1}. ${option.label}${option.recommended ? "（推荐）" : ""}`).join("\n")}`
            : "";
  ...
  async function processFeishuCardAction(baseUrl: string, payload: any, ctx?: CollabCtx) {
    const value = cardActionValue(payload);
    const action = String(value?.ccm_action || "");
    if (!verifyFeishuCardAction(value)) throw new Error("飞书交互卡片签名无效");
    if (!value.expires_at || Date.parse(String(value.expires_at)) <= Date.now()) throw new Error("飞书交互卡片已经过期");
    ...
    if (action === "global_target_selection") {
      if (!access.canOperate) throw new Error("当前飞书用户没有任务投放权限");
      if (!ctx) throw new Error("全局 Agent 运行上下文不可用");
      const conversationId = String(value.conversation_id || "").trim();
      if (!binding || !conversationId || !binding.session_ids?.includes(conversationId)) throw new Error("目标选择卡片与原飞书会话不匹配");
      const target = listGlobalDispatchTargets().find((item: any) => item.ready !== false && item.scope === String(value.scope || "") && item.scopeId === String(value.scope_id || ""));
      if (!target) throw new Error("所选项目或群聊已经不可投放，请重新选择");
      const waiting = getGlobalAgentRun(String(value.decision || ""));
      if (!waiting || waiting.status !== "waiting_clarification" || waiting.session_id !== conversationId) throw new Error("原任务已不再等待目标选择");
    ...
    if (action !== "permission_decision") throw new Error("不支持的飞书卡片操作");
  ...
  function feishuRequirementTargets() {
    return listGlobalDispatchTargets().filter((target: any) => target.ready !== false).map((target: any) => ({
      type: target.scope,
      id: target.scopeId,
  ...
  function normalizeFeishuRequestedTargets(textValue: any) {
    ...
    }).map((target: any) => ({ scope: target.type, scopeId: target.id, canonicalName: target.canonicalName, displayName: target.name }));
  }

  function feishuTargetSelectionMarkdown() {
    const targets = feishuRequirementTargets();
    if (!targets.length) return "当前没有可投放的项目或群聊，请先在 CCM 页面完成目标配置。";
    return [
      "请选择本次任务要投放的目标（可直接回复序号或完整名称）：",
      ...targets.map((target: any, index: number) => `${index + 1}. ${target.name}（${target.type === "group" ? "群聊" : "项目"}）`),
    ].join("\n");
  }

  function feishuTargetSelectionActions(run: any, conversationId: string) {
    try {
      return feishuRequirementTargets().slice(0, 5).map((target: any) => {
        const value: any = {
        ...
        return { text: String(target.name || target.id).slice(0, 24), type: "default" as const, value };
      });
    } catch {
      return [];
    }
  ...
  async function ingestFeishuRequirementAttachment(payload: any, userText: string): Promise<RequirementIngestionResult | null> {
    ...
    if (controlledAttachments.length) {
      const files = materializeFeishuInboundAttachments(controlledAttachments);
      ...
      (result as any).feishu_attachments = controlledAttachments;
      (result as any).public_attachments = serializeGlobalRequestAttachments(files).map((item: any) => {
        const parsed = (result.sources || []).find((sourceItem: any) => String(sourceItem?.name || "") === item.name);
    ...
    if (["file", "media", "image"].includes(messageType)) {
      ...
      if (!messageId) throw new Error("飞书附件事件缺少消息标识");
      const resolved = await resolveFeishuInboundAttachments({
        messageId,
        resourceHints: extractFeishuEventResourceHints(payload),
        source: "event_callback",
      });
      if (!resolved.attachments.length) throw new Error(resolved.failures[0]?.reason || "飞书附件下载失败");
      payload.feishu_attachments = resolved.attachments;
      payload.feishu_attachment_warnings = resolved.warnings;
      const files = materializeFeishuInboundAttachments(resolved.attachments);
      const result = await ingestRequirementSources({
        files,
        userText,
        extractRequirement: true,
      ...
      (result as any).feishu_attachments = resolved.attachments;
      (result as any).public_attachments = serializeGlobalRequestAttachments(files).map((item: any) => {
        const parsed = (result.sources || []).find((sourceItem: any) => String(sourceItem?.name || "") === item.name);
        return parsed ? { ...item, status: String(parsed.status || (parsed.readable ? "parsed" : "failed")), readable: parsed.readable === true, checksum: String(parsed.checksum || parsed.manifest?.source_checksum || "") } : item;
      });
  ...
  async function processFeishuGlobalAgentMessage(baseUrl: string, ctx: CollabCtx, text: string, payload: any, options: { sendReport?: boolean; traceId?: string; inboundRecorded?: boolean; destination?: any; conversationId?: string; onDelta?: (delta: string) => void; originReceipt?: any; principal?: any; turnId?: string; turn_id?: string; resolvedRoute?: string; resolvedCandidateTaskId?: string } = {}) {
    ...
    if (explicitCandidate) {
      historyBeforeUser.push({ role: "system", content: `The user explicitly chose to continue the original task. Recoverable task summary: ${JSON.stringify(buildRecoverableTaskSummary(explicitCandidate))}. Preserve this task identity and do not create a duplicate task.` });
    } else if (recoverableCandidates.length) {
      historyBeforeUser.push({ role: "system", content: `Recoverable task summaries for the exact session: ${JSON.stringify(recoverableCandidates.slice(0, 3).map(buildRecoverableTaskSummary))}. Decide continuationKind from the current message; do not resume merely because an old task exists.` });
    }
    ...
    if (controlledAttachments.length) {
      ...
      const unreadableNames = [
        ...
        ...(Array.isArray(payload?.feishu_attachment_failures) ? payload.feishu_attachment_failures.map((item: any) => String(item?.name || "附件")) : []),
      ...
      await sendFeishuConversationReply({
        ...
        dedupeSuffix: `attachments:${getFeishuMessageId(payload) || controlledAttachments.map((item: any) => item.id).join(":")}`,
      });
    }
    ...
    appendGlobalAgentConversationMessage(conversationId, "user", text, "feishu", {
      files: Array.isArray((sourceIngestion as any)?.public_attachments)
        ? (sourceIngestion as any).public_attachments
        : publicFeishuInboundAttachments(payload?.feishu_attachments),
    });
    const auditBase = {
      source: "feishu-control-bot",
    ...
    try {
      ...
      if (confirmationMatch) {
        ...
        const clarificationRun = requestedTargetRefs.length
          ? listGlobalAgentRuns({ sessionId: conversationId }).find((item: any) => item.status === "waiting_clarification")
          : null;
        ...
        run = await runAgenticGlobalRequest(baseUrl, ctx, {
          ...
          requestedTargetRefs,
          routeGuard: (workflowDecision: any) => {
      ...
      if (run.status === "waiting_clarification" && !(run as any).requested_target_refs?.length) {
      ...
      if (sendReport) await sendFeishuConversationReply({
        ...
        stage: run.status,
        actions: run.status === "waiting_clarification" && !(run as any).requested_target_refs?.length
  ...
  function parseFeishuConversationTurnCommand(value: any): FeishuTurnCommand {
    ...
    if (aside) return { kind: "aside", message: String(aside[1] || "").trim() };
    ...
    try {
      while (true) {
        ...
        try {
          ...
          if (error?.routeHandled === true) break;
  ...
  async function processFeishuControlledMessage(baseUrl: string, ctx: CollabCtx, text: string, payload: any, options: any = {}) {
    ...
    if (command.kind === "aside") {
      if (!earlyAccess.allowed) {
        ...
        if (options.sendReport !== false) await sendFeishuConversationReply({ conversationId, title: "临时提问", markdown: reply, traceId: options.traceId, dedupeSuffix: `aside-denied:${messageId}` });
        return { reply, denied: true, ephemeral: true };
      }
      const exactBinding: any = getFeishuBindingByMessageId(messageId || payload?.event?.message?.root_id || payload?.root_message_id || "");
      ...
      if (!asideSessionId || !asideScopeId || !command.message) {
        ...
        if (options.sendReport !== false) await sendFeishuConversationReply({ conversationId, title: "临时提问", markdown: reply, traceId: options.traceId, dedupeSuffix: `aside-invalid:${messageId}` });
        return { reply, ephemeral: true };
      }
      const result = await runConversationAside({ scope: asideScope, scopeId: asideScopeId, exactSessionId: asideSessionId, question: command.message });
      const reply = `**临时提问 · 基于提问时上下文**\n\n${result.answer}`;
      if (options.sendReport !== false) await sendFeishuConversationReply({ conversationId: asideSessionId, title: "临时提问", markdown: reply, traceId: options.traceId, dedupeSuffix: `aside:${messageId || crypto.randomUUID()}` });
    ...
    const buildQueuedContext = () => {
      const context: any = buildFeishuQueuedTurnContextV2(envelope, payload, destination);
