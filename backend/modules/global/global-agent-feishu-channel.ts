...
export function createGlobalAgentFeishuChannel(deps: any) {
  ...
  async function processFeishuGlobalAgentMessage(baseUrl: string, ctx: CollabCtx, text: string, payload: any, options: { sendReport?: boolean; traceId?: string; inboundRecorded?: boolean; destination?: any; conversationId?: string; onDelta?: (delta: string) => void; originReceipt?: any; principal?: any; turnId?: string; turn_id?: string; resolvedRoute?: string; resolvedCandidateTaskId?: string } = {}) {
    ...
    if (explicitCandidate) {
      historyBeforeUser.push({ role: "system", content: `用户明确选择继续原任务。可恢复任务摘要：${JSON.stringify(buildRecoverableTaskSummary(explicitCandidate))}。必须沿用该任务目标，不得创建重复任务。` });
    } else if (recoverableCandidates.length) {
      historyBeforeUser.push({ role: "system", content: `当前精确会话的可恢复任务摘要：${JSON.stringify(recoverableCandidates.slice(0, 3).map(buildRecoverableTaskSummary))}。请结合当前消息判断 continuationKind；不要仅因为存在旧任务就续接。` });
    }
