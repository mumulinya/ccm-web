...
import { buildGlobalVisibleReplyContent } from "../../agents/global/loop";
import type { CollabCtx } from "../collaboration/collaboration";

export const GLOBAL_PET_AGENT_NAME = "global-agent";
export const GLOBAL_AGENT_VISIBLE_RESULT_FALLBACK = "我已整理这次处理结果，是否已经交付以任务卡验收和最终总结为准。";
export const GLOBAL_AGENT_VISIBLE_COMPLETED_EVENT_FALLBACK = "这轮处理结果已整理，最终是否交付以总结和验收结果为准。";

export function compactPetText(value: any, max = 260) {
  const text = String(value || "").replace(/\s+/g, " ").trim();
  return text.length > max ? `${text.slice(0, max)}...` : text;
}

export function globalVisibleText(value: any, fallback = "我正在处理当前请求。", max = 260) {
  return sanitizeMainAgentUserText(value, fallback, max);
}

export function globalSafeArray(value: any) {
  return Array.isArray(value) ? value : [];
}

export function globalUniqueStrings(...lists: any[]) {
...
export function globalTestAgentCoverageItemKey(item: any = {}) {
...
export function globalUniqueCoverageItems(...lists: any[]) {
  const seen = new Set<string>();
  const result: any[] = [];
  for (const list of lists) {
...
export function globalTestAgentCoverageLabel(type: any) {
...
export function globalTestAgentCoverageByStatus(report: any, key: "requiredCheckCoverage" | "acceptanceCoverage", status: "not_verified" | "unknown") {
  return globalSafeArray(report?.[key]).filter((item: any) => item?.status === status);
}

export function globalTestAgentSummaryObjects(...values: any[]) {
  return values.filter(value => value && typeof value === "object" && !Array.isArray(value));
}

export function globalTestAgentSummaryItems(summary: any, key: string, fallbackStatus: string) {
  return globalSafeArray(summary?.[key])
    .map((item: any) => ({ ...item, status: item?.status || fallbackStatus }));
}

export function globalTestAgentSummaryByStatus(report: any, verdict: any, kind: "required" | "acceptance", status: "not_verified" | "unknown") {
...
export function globalTestAgentAcceptanceWeakReason(item: any) {
...
export function globalTestAgentWeakAcceptanceItems(report: any, verdict: any) {
...
export function summarizeGlobalTestAgentCoverageGap(item: any, kind: "required" | "acceptance", state: "failed" | "unknown") {
  if (kind === "required") {
...
export function collectGlobalTestAgentCoverageGaps(report: any = {}, verdict: any = null) {
...
export function globalTestAgentFailureTypeLabel(type: any) {
  const value = String(type || "").trim().toLowerCase();
  const labels: Record<string, string> = {
    issue: "工作单问题",
    server: "服务启动",
...
export function scrubGlobalTestAgentEvidencePathText(value: any) {
  return String(value || "")
...
export function collectGlobalTestAgentFailureItemsFromSource(source: any, depth = 0, seenObjects = new Set<any>()): any[] {
...
export function collectGlobalTestAgentFailureItems(report: any = {}, verdict: any = null) {
  const seen = new Set<string>();
  const result: any[] = [];
  for (const item of [
...
export function summarizeGlobalTestAgentFailureItem(item: any) {
...
export function summarizeGlobalTestAgentDiagnosticItem(item: any) {
...
export function collectGlobalTestAgentFailureSummaries(report: any = {}, verdict: any = null) {
...
export function globalRunVisibleReply(run: any, fallback = GLOBAL_AGENT_VISIBLE_RESULT_FALLBACK, max = 8000) {
...
export function getGlobalPetToolState(toolName: string) {
...
export function getGlobalToolDisplayName(toolName: string) {
  const labels: Record<string, string> = {
...
export function buildGlobalAgentEventUi(event: any = {}) {
...
export function relayGlobalPetEvent(ctx: CollabCtx, event: any = {}, options: { message?: string; finalRun?: any; error?: string } = {}) {
  const type = String(event.type || "");
