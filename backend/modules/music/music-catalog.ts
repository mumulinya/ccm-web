...
import { refreshEnvPath } from "../../core/utils";
import {
  activeMusicCatalogGeneration,
  getMusicCatalogStatus,
  listMusicCatalogTracks,
  setActiveMusicCatalogGeneration,
...
let runningBuild: Promise<any> | null = null;
...
function parseFilename(filename: string) {
  ...
  if (parts.length >= 2) return { artist: parts[0].trim() || "未知艺术家", title: parts.slice(1).join(" - ").trim() || cleaned, bvid };
...
export function resolveSafeMusicFile(filenameValue: any, options: { mustExist?: boolean } = {}) {
  const filename = cleanFilename(filenameValue);
  const root = fs.realpathSync(MUSIC_DIR);
  const filePath = path.join(root, filename);
  if (options.mustExist !== false && !fs.existsSync(filePath)) throw new Error("音乐文件不存在");
  if (!fs.existsSync(filePath)) return { filename, filePath, root };
  const lstat = fs.lstatSync(filePath);
  if (!lstat.isFile() || lstat.isSymbolicLink()) throw new Error("音乐文件必须是曲库内的普通文件");
  const real = fs.realpathSync(filePath);
  if (path.dirname(real) !== root) throw new Error("音乐文件超出曲库范围");
...
function rowToTrack(row: any) {
  const metadata = (() => { try { return JSON.parse(String(row.metadata_json || "{}")); } catch { return {}; } })();
  return {
    id: row.track_id,
...
function beginGeneration() {
  return withImmediateObservabilityTransaction(db => {
    ...
    `).get() as any;
    if (building && Date.now() - Date.parse(building.started_at) < INDEX_BUILD_STALE_MS) return { generation: Number(building.generation), owned: false };
    if (building) {
...
async function buildGeneration(reason: string) {
  const lease = beginGeneration();
  if (!lease.owned) return getMusicCatalogStatus();
  ...
  try {
    ...
    const previousGeneration = activeMusicCatalogGeneration();
    const previousRows = previousGeneration
      ? (getObservabilityDatabase().prepare("SELECT * FROM music_catalog_tracks_v4 WHERE generation=?").all(previousGeneration) as any[])
      : [];
    ...
    const rows = await mapLimit(filenames, 2, async filename => {
      try {
        ...
        const parsed = parseFilename(filename);
        let stat: any = null;
        try { stat = fs.statSync(path.join(MUSIC_DIR, filename)); } catch {}
        return {
          generation,
    ...
    for (const row of rows) digest.update(`${row.filename}:${row.file_checksum}:${row.state}\n`);
    ...
    setActiveMusicCatalogGeneration(generation);
    pruneCatalogGenerations();
    return getMusicCatalogStatus();
  } catch (error: any) {
...
export function ensureMusicCatalogPrepared() {
  const status = getMusicCatalogStatus();
  if (!status.activeGeneration && status.indexStatus !== "indexing") scheduleMusicCatalogRescan("first-use", 0);
  return getMusicCatalogStatus();
}

export function queryMusicCatalog(input: { cursor?: number; limit?: number; query?: string } = {}) {
  const status = ensureMusicCatalogPrepared();
  const result = listMusicCatalogTracks(input);
  return {
    ...result,
...
export function findMusicCatalogTrackByFilename(filenameValue: any) {
  const filename = cleanFilename(filenameValue);
  ...
  const row = getObservabilityDatabase().prepare(`
    ...
    LIMIT 1
  `).get(generation, filename) as any;
  return row ? { ...rowToTrack(row), indexGeneration: generation } : null;
}

export async function ensureMusicCatalogTrackReady(filenameValue: any, reason = "file_change") {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    if (lastTrack?.state === "ready") return lastTrack;
  }
...
export async function ensureMusicCatalogTrackRemoved(filenameValue: any, reason = "file_delete") {
  const filename = cleanFilename(filenameValue);
  for (let attempt = 0; attempt < 2; attempt += 1) {
    await startMusicCatalogRescan(`${reason}_${attempt + 1}`);
...
export function musicCatalogFileDiagnostics(trackId: string) {
  const generation = activeMusicCatalogGeneration();
  const row = getObservabilityDatabase().prepare(`
...
export function assertSafeId3Allocation(filePath: string) {
  const stat = fs.statSync(filePath);
  if (stat.size < 10) throw new Error("音频文件过小");
  const fd = fs.openSync(filePath, "r");
  try {
    const header = Buffer.alloc(10);
    fs.readSync(fd, header, 0, 10, 0);
    if (header.toString("ascii", 0, 3) !== "ID3") return { tagSize: 0, maxCoverBytes: MAX_COVER_BYTES };
    const tagSize = (header[6] << 21) | (header[7] << 14) | (header[8] << 7) | header[9];
    if (tagSize < 0 || tagSize > MAX_ID3_TAG_BYTES || tagSize + 10 > stat.size) throw new Error("ID3标签大小无效");
...
export const MUSIC_MEDIA_LIMITS = {
