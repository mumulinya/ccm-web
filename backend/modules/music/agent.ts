...
function normalizeMusicSourceMode(value: any): MusicIntentDecisionV2["sourceMode"] {
...
function validateMusicIntentDecision(value: any, requestText: string, mode: string) {
  ...
  if (!actions.has(action)) throw new Error("模型返回了无效音乐动作");
  if (!strategies.has(strategy)) throw new Error("模型返回了无效音乐播放策略");
  if (action === "play" && strategy === "none") throw new Error("播放动作缺少播放策略");
  if (action !== "play" && strategy !== "none") throw new Error("非播放动作不能携带播放策略");
  ...
  if (["play", "search", "convert"].includes(action) && !searchQuery) searchQuery = artist || genre;
  if (["play", "search", "convert"].includes(action) && !searchQuery) throw new Error("模型没有返回可执行的音乐搜索条件");
  return {
    schema: "ccm-music-intent-decision-v2" as const,
    action: action as MusicIntentDecisionV2["action"],
    strategy: strategy as MusicIntentDecisionV2["strategy"],
...
export async function resolveMusicIntentDecisionV2(input: {
  config?: any;
  message: string;
  mode?: string;
  history?: any[];
  ...
  if (!message) throw new Error("音乐请求不能为空");
...
export async function callClaudeAgent(cfg: any, system: string, messages: any[], res: any, _chatMode: string, _options: { allowTools?: boolean } = {}) {
  ...
  try {
    ...
    const text = isAnthropicCompat
      ? await callAnthropicCompatibleChat(cfg, {
          system,
          messages: normalized,
          maxTokens: 1_024,
          temperature: 0.2,
      ...
      : await callOpenAiCompatibleChat(cfg, {
