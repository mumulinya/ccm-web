import { getConfigInfo, getConfigs, loadCronJobs, loadMcpTools, loadSkills, loadTasks } from "./db";
import { loadGroups, refreshGlobalDevelopmentMissions } from "./modules/collaboration";
import { listGlobalMissionSupervisors } from "./global-mission-supervisor";
import { GLOBAL_AGENT_TOOL_SPECS, type GlobalAgentToolRisk } from "./global-agent-loop";
import { buildGlobalAgentToolDefinitions, loadGlobalAgentHooks, loadGlobalAgentPermissionRules } from "./global-agent-runtime";
...
function riskOf(spec: any, args: any = {}): GlobalAgentToolRisk | "dynamic" {
  try {
...
function projectRows(projects = getConfigs()) {
  return projects.map((config: any) => {
...
function groupRows(groups = loadGroups()) {
...
export function classifyGlobalControlIntent(message: string, resources: any = {}) {
  ...
  let route: IntentRoute = "ordinary_question";
  let confidence = 0.72;
  ...
  return {
    ...
    reason,
    recommended_tool,
    matched_projects: matchedProjects.map((item: any) => item.name),
    matched_groups: matchedGroups.map((item: any) => ({ id: item.id, name: item.name })),
    dry_run: {
...
export function buildGlobalDispatchStrategy(message: string, resources: any = {}) {
  const input = text(message);
  const projects = resources.projects || projectRows();
  const groups = resources.groups || groupRows();
  const matchedProjects = matchProjects(input, projects);
  ...
  const targets = [
    ...matchedGroups.map((group: any) => ({ type: "group", id: group.id, name: group.name, reason: "命中群聊名称或 ID，由群聊主 Agent 协调成员项目" })),
    ...matchedProjects.map((project: any) => ({ type: "project", id: project.name, name: project.name, reason: "命中项目名称，由项目 Agent 直接执行或作为 mission 子目标" })),
  ];
  if (!targets.length && workspaceWide) {
    targets.push(...projects.slice(0, 12).map((project: any) => ({ type: "project", id: project.name, name: project.name, reason: "用户要求全局/跨项目覆盖" })));
  }
...
export function buildGlobalSystemHealth(resources: any = {}) {
  const projects = resources.projects || projectRows();
  const groups = resources.groups || groupRows();
  const tasks = resources.tasks || loadTasks();
  const cronJobs = resources.cronJobs || loadCronJobs();
  const mcpTools = resources.mcpTools || loadMcpTools();
  const skills = resources.skills || loadSkills();
  const supervisors = resources.supervisors || listGlobalMissionSupervisors({ limit: 100 });
  const missions = resources.missions || refreshGlobalDevelopmentMissions();
  const activeTasks = tasks.filter((task: any) => ["pending", "queued", "in_progress", "running"].includes(String(task.status)));
  const failedTasks = tasks.filter((task: any) => ["failed", "error"].includes(String(task.status)));
  const waitingSupervisors = supervisors.filter((item: any) => item.status === "waiting_user");
  const failedSupervisors = supervisors.filter((item: any) => ["failed", "cancelled"].includes(String(item.status)));
  const rows = [
    {
      id: "projects",
      label: "项目配置",
      severity: projects.some((item: any) => !item.configured) ? "warn" : "ok",
      summary: `${projects.length} 个项目，${projects.filter((item: any) => !item.configured).length} 个缺少工作目录`,
    },
    {
      id: "groups",
      label: "群聊主 Agent",
      severity: groups.some((item: any) => !item.member_count) ? "warn" : "ok",
      summary: `${groups.length} 个群聊，${groups.filter((item: any) => !item.member_count).length} 个无成员`,
    },
    {
      id: "tasks",
      label: "任务队列",
    ...
    },
    {
      id: "cron",
      label: "定时调度",
      severity: cronJobs.some((job: any) => job.enabled === false) ? "warn" : "ok",
      summary: `${cronJobs.length} 个定时任务，${cronJobs.filter((job: any) => job.enabled === false).length} 个停用`,
    },
    {
      id: "tools",
      label: "MCP / Skill",
  ...
  const severity: Severity = rows.some(row => row.severity === "error") ? "error" : rows.some(row => row.severity === "warn") ? "warn" : "ok";
  return {
    severity,
...
export function buildGlobalGovernanceSnapshot() {
  const tools = buildGlobalAgentToolDefinitions(GLOBAL_AGENT_TOOL_SPECS);
  const permissions = loadGlobalAgentPermissionRules();
  const hooks = loadGlobalAgentHooks();
  const highRiskTools = GLOBAL_AGENT_TOOL_SPECS
...
export function buildGlobalSupervisionDashboard(resources: any = {}) {
  const supervisors = resources.supervisors || listGlobalMissionSupervisors({ limit: 50 });
  return {
    total: supervisors.length,
    rows: supervisors.slice(0, 20).map((item: any) => ({
      id: item.id,
...
export function buildGlobalControlCenterSnapshot(message = "") {
...
export function runGlobalControlCenterSelfTest() {
  const resources = {
  ...
  const checks = {
    ...
    dispatchFindsGroupAndProject: dispatch.targets.some((item: any) => item.type === "group") && dispatch.targets.some((item: any) => item.type === "project"),
