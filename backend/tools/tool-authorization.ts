...
function cleanGrant(value: any) {
  const text = String(value || "").replace(/[\0\r\n\t]+/g, " ").trim();
  if (!text || text.length > 180) return "";
  return text;
}

function mcpGrantFromObject(value: any) {
...
function skillGrantFromObject(value: any) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return "";
  return cleanGrant(value.name || value.skill || value.id || value.raw);
}

export function parseMcpGrant(value: string) {
...
function normalizeList(values: any[], mapper: (value: any) => string) {
  const seen = new Set<string>();
  const result: string[] = [];
  for (const value of values) {
    const grant = mapper(value);
    if (!grant || seen.has(grant)) continue;
    seen.add(grant);
    result.push(grant);
  }
  return result;
...
function normalizeMcpServerKey(value: any) {
  return cleanGrant(value).replace(/^ccm__/, "").toLowerCase();
}

function compactMcpGrants(grants: string[]) {
...
function diffGrantList(before: string[] = [], after: string[] = []) {
...
function auditSummary(toolAudit: any) {
  return {
...
function cleanInventoryText(value: any, max = 260) {
  return String(value || "").replace(/[\0\r\n\t]+/g, " ").trim().slice(0, max);
}

function safeInventoryObject(value: any) {
...
function buildInventoryScopeRow(input: {
  ...
  buildPayload: (tools: any) => { tools: ToolGrantSet; tool_audit?: any; authorization_readiness?: any };
}) {
...
function sanitizeInventoryRuntimeSnapshot(snapshot: any) {
...
export function buildToolAuthorizationInventory(input: any = {}) {
  ...
  const summary = scopes.reduce((acc: any, row: any) => {
...
export function buildAuthorizationReadiness(toolAudit: any, tools: ToolGrantSet) {
  const mcpRows = Array.isArray(toolAudit?.mcp) ? toolAudit.mcp : [];
  const skillRows = Array.isArray(toolAudit?.skills) ? toolAudit.skills : [];
  const unavailableMcp = mcpRows.filter((row: any) => row?.state !== "available");
  const unavailableSkills = skillRows.filter((row: any) => row?.state !== "available");
  ...
  const invalidMcp = unavailableMcp.filter((row: any) => row?.state === "invalid_grant").length;
  const requested = {
  ...
  return {
    ...
    requested,
    available: {
      mcp: mcpRows.filter((row: any) => row?.state === "available").length,
      skill: skillRows.filter((row: any) => row?.state === "available").length,
    },
    missing: missingSummary,
    invalid_mcp_grants: invalidMcp,
    unavailable: {
      mcp: unavailableMcp.map((row: any) => ({
        raw: cleanGrant(row?.raw || row?.server || ""),
        server: cleanGrant(row?.server || ""),
        tool: cleanGrant(row?.tool || ""),
        state: cleanGrant(row?.state || "unknown"),
      })).slice(0, 50),
      skill: unavailableSkills.map((row: any) => ({
...
export function normalizeToolAuthorization(input: any = {}): ToolGrantSet {
  ...
  const mcp = compactMcpGrants(normalizeList(asArray(source.mcp), value => (
    typeof value === "object" ? mcpGrantFromObject(value) : cleanGrant(value)
  )));
  const skill = normalizeList(asArray(source.skill), value => (
    typeof value === "object" ? skillGrantFromObject(value) : cleanGrant(value)
  ));
  return { mcp, skill };
}
...
export function recordToolAuthorizationChange(input: Parameters<typeof buildToolAuthorizationChangeRecord>[0]) {
...
export function buildToolAuthorizationOptions(input: any = {}): { success: true; mcp: any[]; skill: any[]; summary: any } {
  ...
  const serverStatus = new Map(serverRows.map((row: any) => [String(row?.name || ""), row]));
  const mcp = asArray(input.mcpTools || input.mcp)
    ...
    .map(tool => {
      ...
      const statusRow: any = serverStatus.get(name) || {};
...
export function runToolAuthorizationSelfTest() {
