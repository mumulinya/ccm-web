...
export async function executeMainAgentToolRequests(input: {
  ...
  const applyAdaptiveWorkspaceReadBudget = (request: MainAgentToolRequest) => {
    ...
    if (name === "read_files") args.token_budget = Math.max(1024, Math.min(8_000, Number(args.token_budget || 8_000)));
    if (name === "read_file") args.token_budget = Math.max(256, Math.min(8_000, Number(args.token_budget || 4_000)));
    if (name === "glob_files") args.limit = Math.max(1, Math.min(100, Number(args.limit || 100)));
    if (name === "grep_text") {
