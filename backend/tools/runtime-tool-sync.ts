...
function cleanRuntimeResyncText(value: any, max = 240) {
  return String(value || "").replace(/[\0\r\n\t]+/g, " ").trim().slice(0, max);
}

function getAuthorizationReadiness(auditOrReadiness: any = {}) {
...
function formatAuthorizationReadinessNotice(audit: RuntimeToolSyncAudit) {
...
function stableHash(value: any, length = 16) {
...
function mcpGrantsForServer(grants: string[], serverName: string) {
  return uniqueNames(grants).filter(grant => mcpGrantMatchesServer(grant, serverName));
}

function mcpGrantToolsForServer(grants: string[], serverName: string) {
...
function shouldExposeMcpServerNatively(grants: string[], serverName: string) {
...
function nativeMcpNamesFromAudit(audit: any) {
...
function proxyOnlyMcpNamesFromAudit(audit: any) {
  const statuses = Array.isArray(audit?.mcp_statuses) ? audit.mcp_statuses : [];
...
function resolveSkillPackage(skill: any, skillPackagesDir = SKILL_PACKAGES_DIR) {
...
function syncManagedSkills(skillRoot: string, skills: any[], audit: RuntimeToolSyncAudit, skillPackagesDir = SKILL_PACKAGES_DIR) {
...
function loadCodexLocalAccessConfig(): CodexGatewayConfig | null {
...
function loadCodexProviderConfig(): CodexGatewayConfig | null {
