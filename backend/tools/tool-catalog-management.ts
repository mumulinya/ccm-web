...
function owns(value: any, key: string) {
  return !!value && Object.prototype.hasOwnProperty.call(value, key);
}

export function normalizeToolCatalogName(value: any) {
...
export function normalizeMcpEnvironment(value: any, options: { strict?: boolean } = {}) {
  const strict = options.strict === true;
  const rows: Record<string, string> = {};
  const add = (rawKey: any, rawValue: any) => {
    const key = String(rawKey || "").trim();
    if (!key) return;
    if (!ENV_NAME_PATTERN.test(key)) {
      if (strict) throw new Error(`环境变量名称无效：${key}`);
      return;
    }
  ...
  for (const line of String(value || "").split(/\r?\n/)) {
    ...
    if (separator < 1) {
      if (strict) throw new Error(`环境变量必须使用 KEY=value 格式：${line.trim().slice(0, 40)}`);
      continue;
    }
...
export function redactMcpCommand(value: any) {
  return redactSensitiveText(value)
    .replace(/(--?(?:api[-_]?key|access[-_]?token|token|password|secret)(?:=|\s+))([^\s"']+)/gi, "$1***")
    .trim();
}

function redactMcpArgs(value: any) {
...
export function redactMcpToolForDisplay(tool: any = {}) {
...
export function mergeMcpToolUpdate(existing: any, input: any = {}, options: { create?: boolean } = {}) {
  ...
  if (!create && JSON.stringify(redactMcpArgs(current.args)) === JSON.stringify(redactMcpArgs(args))
    && JSON.stringify(args) !== JSON.stringify(current.args)) args = current.args;
  args = Array.isArray(args) ? args.map(item => String(item)).filter(Boolean) : redactMcpArgs(args);
  let env = current.env;
  if (input.clearEnv === true) env = {};
...
export function mergeSkillUpdate(existing: any, input: any = {}, options: { create?: boolean } = {}) {
  const current = existing && typeof existing === "object" ? existing : {};
  const name = normalizeToolCatalogName(input.name || current.name);
  const prompt = owns(input, "prompt") ? String(input.prompt || "") : String(current.prompt || "");
  if (options.create === true && !prompt.trim()) throw new Error("Prompt 模板不能为空");
  return {
    ...current,
    name,
    description: owns(input, "description") ? String(input.description || "").trim() : String(current.description || ""),
...
export function runToolCatalogManagementSelfTest() {
  const existing = {
    name: "secure-mcp",
  ...
  try { normalizeToolCatalogName("../unsafe"); } catch { checks.invalidCatalogNameRejected = true; }
  try { normalizeMcpEnvironment("BAD-NAME=value", { strict: true }); } catch { checks.invalidEnvironmentRejected = true; }
