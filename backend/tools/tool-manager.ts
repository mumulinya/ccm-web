...
export interface PostCompactDynamicToolCatalog {
  tools: Array<{ name: string; description: string; server: string; line: string }>;
  skills: Array<{ name: string; description: string; contentHash: string; line: string }>;
  mcpInstructions: Array<{ name: string; instructions: string; block: string }>;
}
...
function mcpToolCandidates(tools: ToolDef[], toolName: string) {
...
function cleanAuditContextText(value: any, max = 180) {
  return String(value || "").replace(/[\0\r\n\t]+/g, " ").trim().slice(0, max);
}

function auditContextFromScope(scope?: ToolScope): ToolInvocationAuditContext {
...
function auditMetaFromScope(scope?: ToolScope) {
...
- 成功调用 Skill 后，应在 CCM_AGENT_RECEIPT.memoryUsed 中写入 Skill:<name>`);
  ...
  getPostCompactDynamicToolCatalog(scope?: ToolScope): PostCompactDynamicToolCatalog {
    ...
    for (const raw of normalizeScopeList(scope?.mcp)) {
      const grant = parseMcpGrant(raw);
      if (grant.server) allowedServers.add(grant.server);
    }
  ...
  private parseEnv(value: any): Record<string, string> {
...
export async function runToolManagerRuntimeSelfTest() {
  ...
  manager.clients = new Map([["payments", {
    isConnected: () => true,
    callTool: async (name: string, args: any) => ({ isError: false, content: [{ text: `called:${name}:${args.id}` }] }),
  ...
  const skillAuditRows = fs.existsSync(SKILL_INVOCATION_AUDIT_FILE)
    ...
    : [];
  const contextAudit = skillAuditRows.find((row: any) => row.taskId === auditContextTaskId);
  ...
  const reloadManager = new ToolManager() as any;
  ...
  const checks = {
    ...
    reloadClearsStaleMcpStatus: !reloadList.servers.some((server: any) => server.name === "stale-server"),
    reloadRefreshesSkillCatalog: reloadList.skillTools.some((skill: any) => skill.name === "fresh-skill"),
  };
