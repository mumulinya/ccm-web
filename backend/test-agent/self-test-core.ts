// self-test-core.ts — merged from 3 part files (behavior-freeze merge).
...
import {
  verifyTestAgentArtifactManifestFile,
} from "./artifact-verifier";
import {
  ACCEPTANCE_CLIPBOARD_FLOW_PROBE_TYPE,
  buildAcceptanceClipboardFlowBrowserChecks,
} from "./browser/acceptance-clipboard-flows";
import {
  ACCEPTANCE_CLICK_FLOW_PROBE_TYPE,
  buildAcceptanceClickFlowBrowserChecks,
} from "./browser/acceptance-click-flows";
import {
  buildAcceptanceDerivedBrowserAssertions,
} from "./browser/acceptance-derived-checks";
import {
  ACCEPTANCE_DIALOG_FLOW_PROBE_TYPE,
  buildAcceptanceDialogFlowBrowserChecks,
} from "./browser/acceptance-dialog-flows";
import {
  ACCEPTANCE_DRAG_FLOW_PROBE_TYPE,
  buildAcceptanceDragFlowBrowserChecks,
} from "./browser/acceptance-drag-flows";
import {
  ACCEPTANCE_DOWNLOAD_FLOW_PROBE_TYPE,
  buildAcceptanceDownloadFlowBrowserChecks,
} from "./browser/acceptance-download-flows";
import {
  ACCEPTANCE_FORM_FLOW_PROBE_TYPE,
  buildAcceptanceFormFlowBrowserChecks,
} from "./browser/acceptance-form-flows";
import {
  ACCEPTANCE_HISTORY_FLOW_PROBE_TYPE,
  buildAcceptanceHistoryFlowBrowserChecks,
} from "./browser/acceptance-history-flows";
import {
  ACCEPTANCE_HOVER_FLOW_PROBE_TYPE,
  buildAcceptanceHoverFlowBrowserChecks,
} from "./browser/acceptance-hover-flows";
import {
  ACCEPTANCE_KEYBOARD_FLOW_PROBE_TYPE,
  buildAcceptanceKeyboardFlowBrowserChecks,
} from "./browser/acceptance-keyboard-flows";
import {
  ACCEPTANCE_NETWORK_STATE_FLOW_PROBE_TYPE,
  buildAcceptanceNetworkStateFlowBrowserChecks,
} from "./browser/acceptance-network-state-flows";
import {
  MULTI_SESSION_BROWSER_PROBE_TYPE,
} from "./browser/multi-session";
import {
  ACCEPTANCE_POPUP_FLOW_PROBE_TYPE,
  buildAcceptancePopupFlowBrowserChecks,
} from "./browser/acceptance-popup-flows";
import {
  runBrowserSessionComparison,
} from "./browser/session-comparison";
import {
  ACCEPTANCE_REPEATED_CLICK_PROBE_TYPE,
  buildAcceptanceRepeatedClickBrowserChecks,
} from "./browser/acceptance-repeated-click-checks";
import {
  ACCEPTANCE_RESPONSIVE_PROBE_TYPE,
  buildAcceptanceResponsiveBrowserChecks,
} from "./browser/acceptance-responsive-checks";
import {
  ACCEPTANCE_SCROLL_FLOW_PROBE_TYPE,
  buildAcceptanceScrollFlowBrowserChecks,
} from "./browser/acceptance-scroll-flows";
import {
  buildBrowserStabilitySummary,
} from "./browser/stability-summary";
import {
  ACCEPTANCE_UPLOAD_FLOW_PROBE_TYPE,
  buildAcceptanceUploadFlowBrowserChecks,
} from "./browser/acceptance-upload-flows";
import {
  AUTO_BROWSER_SMOKE_PROBE_TYPE,
  buildAcceptancePathBrowserSmokeChecks,
  buildAutoBrowserSmokeCheck,
  buildBrowserChecksForProject,
} from "./browser/auto-checks";
import {
  checkPlaywrightAvailability,
} from "./browser/playwright-provider";
import {
  buildSemanticLocatorPlan,
} from "./browser/semantic-locator";
import {
  createStaticBrowserToolExecutor,
} from "./browser/tool-executor";
import {
  formatTestAgentCliArtifactVerificationSummary,
  formatTestAgentCliExecutionPlanSummary,
  formatTestAgentCliReportSummary,
  formatTestAgentCliValidationSummary,
  runTestAgentCli,
} from "./cli";
import {
  cliOverrides,
  parseTestAgentCliArgs,
} from "./cli-options";
import {
...
import {
  buildTestAgentMarkdownReport,
} from "./artifacts";
import {
  buildTestAgentReport,
} from "./result-builder";
import {
...
} from "./self-test-matrix";
import {
  buildTestAgentVerdict,
} from "./verdict";
import {
  buildTestAgentWorkOrderFromHandoff,
} from "./work-order-builder";
import {
  buildEmptyZip,
  buildStoredZip,
  getFreePort,
...
// ===== merged from self-test-core/part-01.ts =====
...
export function runTestAgentHandoffBuilderSelfTest() {
  ...
  const minimalExample = buildTestAgentWorkOrderFromHandoff(TEST_AGENT_MINIMAL_HANDOFF_EXAMPLE);
  const webExample = buildTestAgentWorkOrderFromHandoff(TEST_AGENT_WEB_APP_HANDOFF_EXAMPLE);
  const minimalValidation = validateTestAgentWorkOrderContract(minimalExample.workOrder);
  const webValidation = validateTestAgentWorkOrderContract(webExample.workOrder);
  const webRequired = new Set(webExample.workOrder.requiredChecks || []);
  const webProject = webExample.workOrder.projects?.[0] as any;
...
export function runTestAgentHandoffContractSelfTest() {
  const minimal = validateTestAgentHandoffContract(TEST_AGENT_MINIMAL_HANDOFF_EXAMPLE, { browserProvider: "none" });
  const web = validateTestAgentHandoffContract(TEST_AGENT_WEB_APP_HANDOFF_EXAMPLE);
  const singleProject = validateTestAgentHandoffContract({
  ...
  const warning = validateTestAgentHandoffContract({
  ...
  const missingProjects = validateTestAgentHandoffContract({
    originalUserGoal: "This handoff is missing project targets.",
    completedTasks: ["No project target supplied."],
  });
  const invalidProjectsType = validateTestAgentHandoffContract({
    originalUserGoal: "This handoff has the wrong projects type.",
    projects: "not-an-array",
  } as any);
  const invalidNestedHttp = validateTestAgentHandoffContract({
  ...
  return {
    pass: minimal.valid
      ...
      && warning.normalized?.metadata?.handoffWarnings?.some((item: string) => item.includes("missing workDir"))
...
// ===== merged from self-test-core/part-02.ts =====
...
// ===== merged from self-test-core/part-03.ts =====
