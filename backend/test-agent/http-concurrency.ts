...
export const MIN_HTTP_CONCURRENT_REQUESTS = 2;
export const MAX_HTTP_CONCURRENT_REQUESTS = 50;
...
export function httpConcurrencyValueAtPath(input: any, path: string) {
  let current = input;
...
function valueEvidence(path: string, value: any) {
  if (value === undefined) return { path, present: false };
...
export function concurrencyAggregatePaths(assertions: HttpConcurrencyAssertionSpec[] = []) {
...
export function buildHttpConcurrencyValueEvidence(
  json: any,
  assertions: HttpConcurrencyAssertionSpec[] = [],
) {
  return concurrencyAggregatePaths(assertions).map(path =>
    valueEvidence(path, httpConcurrencyValueAtPath(json, path))
  );
}

export function interpolateHttpConcurrencyValue(value: any, requestIndex: number): any {
  if (typeof value === "string") {
    return value
      .replace(/\{\{\s*requestIndex\s*\}\}/g, String(requestIndex))
      .replace(/\{\{\s*requestNumber\s*\}\}/g, String(requestIndex + 1));
  }
  if (Array.isArray(value)) return value.map(item => interpolateHttpConcurrencyValue(item, requestIndex));
  if (!value || typeof value !== "object") return value;
  return Object.fromEntries(Object.entries(value)
    .map(([key, item]) => [key, interpolateHttpConcurrencyValue(item, requestIndex)]));
}

export function httpConcurrencySpecFor(check: {
  ...
  if (!check.concurrency || typeof check.concurrency !== "object") return undefined;
  return {
...
function assertionResult(name: string, status: HttpAssertionResult["status"], detail = "", error = ""): HttpAssertionResult {
  return {
    name,
    status,
    ...(detail ? { detail } : {}),
...
function expectedCount(assertion: HttpConcurrencyAssertionSpec) {
...
function aliasNumberErrors(
  ...
  if (distinct.size > 1) errors.push(`${label} contains inconsistent numeric aliases.`);
  return errors;
}

function assertionSpecErrors(
  assertion: HttpConcurrencyAssertionSpec,
  label: string,
) {
  const errors: string[] = [];
  if (!assertion || typeof assertion !== "object") return [`${label} must be an object.`];
  const type = String((assertion as any).type || "");
...
export function evaluateHttpConcurrencyAssertions(
  assertions: HttpConcurrencyAssertionSpec[],
  ...
  if (blocked) {
    return assertions.map(assertion => assertionResult(
      `http:concurrency:${assertion.type}`,
      "skipped",
  ...
  return assertions.map(assertion => {
    if (assertion.type === "responseCount") {
      return countResult(
        "http:concurrency:responseCount",
    ...
    const values = requests.map(request =>
      request.aggregateValues.find(value => value.path === path)
    );
    ...
    if (assertion.type === "jsonPathUniqueCount") {
      if (missing) {
        return assertionResult(
          "http:concurrency:jsonPathUniqueCount",
          "failed",
          `path=${path}; missing=${missing}; unique=${unique}`,
          `JSON path ${JSON.stringify(path)} was missing from ${missing} concurrent response(s).`,
        );
      }
    ...
    if (assertion.type === "jsonPathAllEqual") {
      ...
      return assertionResult(
        "http:concurrency:jsonPathAllEqual",
        passed ? "passed" : "failed",
        `path=${path}; missing=${missing}; unique=${unique}`,
        `Expected JSON path ${JSON.stringify(path)} to be present and equal across all concurrent responses.`,
      );
    }
    return assertionResult(
      `http:concurrency:${String((assertion as any).type || "unknown")}`,
      "failed",
      "",
      `Unsupported HTTP concurrency assertion ${String((assertion as any).type || "(missing)")}.`,
    );
  });
}

function maxInFlight(requests: HttpConcurrentRequestResult[]) {
...
export function buildHttpConcurrencyEvidence(input: {
  requested: number;
  requests: HttpConcurrentRequestResult[];
  ...
  const overlapObserved = maximum >= Math.min(MIN_HTTP_CONCURRENT_REQUESTS, input.requested);
  const overlapAssertion = assertionResult(
    "http:concurrency:overlap",
    overlapObserved ? "passed" : "failed",
    `requested=${input.requested}; maxInFlight=${maximum}`,
    "Concurrent HTTP requests did not overlap in flight.",
  );
  return {
...
export function httpConcurrencyResultStatus(
  evidence: HttpConcurrencyEvidence | undefined,
): "passed" | "failed" | "blocked" {
  if (!evidence || !Array.isArray(evidence.requests) || !Array.isArray(evidence.aggregateAssertions)) {
    return "failed";
  }
  if (evidence.requests.some(request => request?.status === "failed")) return "failed";
  if (evidence.requests.some(request => request?.status === "blocked")) return "blocked";
  if (
    evidence.requests.length !== evidence.requested
    || evidence.requests.some(request => request?.status !== "passed")
  ) return "failed";
  if (
    !evidence.aggregateAssertions.length
    || evidence.aggregateAssertions.some(assertion => assertion?.status !== "passed")
  ) return "failed";
  return "passed";
}

export function buildHttpConcurrencySummary(httpResults: HttpCheckResult[] = []): HttpConcurrencySummary {
...
export function formatHttpConcurrencySummaryLine(summary: HttpConcurrencySummary | undefined) {
  return [
...
export function httpConcurrencyEvidenceErrors(
  evidence: HttpConcurrencyEvidence | undefined,
  label = "HTTP concurrency evidence",
) {
  if (!evidence) return [`${label} is missing.`];
  const errors: string[] = [];
  ...
  if (!Array.isArray(evidence.requests)) errors.push(`${label}.requests must be an array.`);
  if (!Array.isArray(evidence.assertionSpecs)) errors.push(`${label}.assertionSpecs must be an array.`);
  if (!Array.isArray(evidence.aggregateAssertions)) errors.push(`${label}.aggregateAssertions must be an array.`);
  if (
    !Number.isInteger(evidence.requested)
    || evidence.requested < MIN_HTTP_CONCURRENT_REQUESTS
    || evidence.requested > MAX_HTTP_CONCURRENT_REQUESTS
  ) {
  ...
  for (const [index, request] of requests.entries()) {
    ...
    if (!String(request.url || "").trim()) errors.push(`${requestLabel}.url is required.`);
    if (!String(request.method || "").trim()) errors.push(`${requestLabel}.method is required.`);
    ...
    if (!Array.isArray(request.assertions)) errors.push(`${requestLabel}.assertions must be an array.`);
    ...
    if (!Array.isArray(request.aggregateValues)) errors.push(`${requestLabel}.aggregateValues must be an array.`);
  ...
  if (new Set(indexes).size !== indexes.length) errors.push(`${label}.requests contains duplicate requestIndex values.`);
  if (Number.isInteger(evidence.requested) && evidence.requested >= 0) {
    ...
    const actualIndexes = [...indexes].sort((left: any, right: any) => Number(left) - Number(right));
  ...
  try {
    ...
    if (!sameJson(evidence, expected)) errors.push(`${label} does not match its request evidence.`);
  } catch (error: any) {
...
export function httpConcurrencySummaryErrors(
  summary: HttpConcurrencySummary | undefined,
  httpResults: HttpCheckResult[],
  label = "HTTP concurrency summary",
) {
  if (!summary) return [`${label} is missing.`];
