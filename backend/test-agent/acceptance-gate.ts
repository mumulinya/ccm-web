...
function statusFor(input: {
  ...
  if (input.total === 0) return "not_applicable";
  if (input.notVerified > 0) return "failed";
  if (input.unknown > 0) return "incomplete";
  if (input.fallbackEvidence > 0) return "weak";
  return "verified";
}

export function buildAcceptanceEvidenceGateSummary(
  ...
  const fallbackItems = coverage.filter(item =>
    item.evidenceSource === "single_criterion_report_status"
...
export function formatAcceptanceEvidenceGateSummaryLine(
  summary: AcceptanceEvidenceGateSummary | undefined,
) {
  if (!summary) {
...
export function acceptanceEvidenceGateSummaryErrors(
  summary: AcceptanceEvidenceGateSummary | Record<string, any> | undefined,
  coverage?: AcceptanceCoverageItem[],
  label = "acceptance evidence gate summary",
) {
  ...
  const numeric = (key: keyof AcceptanceEvidenceGateSummary) => Number(summary[key]);
