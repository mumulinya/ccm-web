...
export const TEST_AGENT_EVIDENCE_PROJECTION_SCHEMA = "ccm-test-agent-evidence-projection-v2" as const;
export const TEST_AGENT_HANDOFF_PROJECTION_SCHEMA = "ccm-test-agent-handoff-persistence-projection-v2" as const;
...
export function testAgentEvidenceChecksum(value: any) {
...
function shouldStripKey(key: string, fieldPath: string) {
  const normalized = normalizedKey(key);
  if (FORBIDDEN_DURABLE_KEYS.has(normalized)) return true;
  if (["observedoutputpreview", "reviewoutputpreview", "responsebody", "requestbody", "tooloutput", "toolresponse"].includes(normalized)) return true;
...
function uniqueStrings(value: any, max = 200) {
...
export function findForbiddenTestAgentEvidencePaths(value: any) {
  const paths: string[] = [];
  const visit = (node: any, path: string) => {
    ...
    for (const [key, child] of Object.entries(node)) {
      ...
      if (shouldStripKey(key, childPath)) paths.push(childPath);
 ...
 */
export function projectTestAgentValueForPersistence<T = any>(value: T): {
  value: any;
  redactions: TestAgentEvidenceRedactionReference[];
} {
  const redactions: TestAgentEvidenceRedactionReference[] = [];
  const visit = (node: any, fieldPath: string): any => {
    if (Array.isArray(node)) return node.map((item, index) => visit(item, `${fieldPath}[${index}]`));
...
function statusOf(value: any) {
  if (value?.success === true || value?.passed === true || value?.status === "passed" || value?.status === "completed") return "passed";
  if (value?.success === false || value?.passed === false || ["failed", "blocked", "error", "runtime_error"].includes(String(value?.status || ""))) return "failed";
  return "unknown";
}

function fileRefsFromResult(result: any) {
  const candidates = [
  ...
  return uniqueStrings(candidates.map((item: any) => item?.path || item?.file || item), 300);
}

function verificationRefsFromResult(result: any) {
  const values = [
...
function sourceRefsFromResult(result: any) {
  const refs = result?.sourceRefs || result?.source_refs || result?.receipt?.sourceRefs || [];
  return (Array.isArray(refs) ? refs : []).slice(0, 100).map((item: any, index: number) => ({
...
export function buildTestAgentEvidenceProjection(input: {
  taskId?: string;
  scope?: "global" | "project" | "group";
  scopeId?: string;
  workerResults?: any[];
  ...
  const core = {
    schema: TEST_AGENT_EVIDENCE_PROJECTION_SCHEMA,
...
export function summarizeTestAgentEvidenceProjection(value: TestAgentEvidenceProjectionV2) {
  return `结构化交付证据：${value.totals.workers} 个执行回执，${value.totals.files} 个文件引用，${value.totals.checks} 个验证引用；正文未持久化。`;
}

export function readTestAgentEvidenceProjection(value: any): TestAgentEvidenceProjectionV2 {
  if (value?.schema === TEST_AGENT_EVIDENCE_PROJECTION_SCHEMA) {
    const forbidden = findForbiddenTestAgentEvidencePaths(value);
    if (!forbidden.length) return value;
    recordTestAgentHardeningMetric("test_agent_projection_rejected_total", forbidden.length);
  }
...
export function projectTestAgentHandoffForPersistence(handoff: any) {
  const projected = projectTestAgentValueForPersistence(handoff || {});
  const value = {
    ...projected.value,
    schema: "ccm-test-agent-handoff-v2",
    persistenceProjection: {
      schema: TEST_AGENT_HANDOFF_PROJECTION_SCHEMA,
...
export function projectTestAgentExecutionResultForPersistence(result: any) {
...
export function runTestAgentEvidenceProjectionSelfTest() {
