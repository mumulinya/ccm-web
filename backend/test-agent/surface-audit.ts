...
export const TEST_AGENT_SURFACE_AUDIT_SCHEMA = "ccm-test-agent-surface-audit-v1" as const;
...
export function surfaceAuditChecksum(value: any) {
...
function isIgnoredSurface(file: string, extra: string[] = []) {
...
function gitOutput(workDir: string, args: string[]) {
  try {
...
function parsePorcelainZ(value: string) {
  const entries: Array<{ path: string; state: string }> = [];
...
export function captureTestAgentSurfaceSnapshot(workDir: string, options: { ignoredPrefixes?: string[]; baselineRef?: string } = {}): TestAgentSurfaceSnapshot {
  ...
  if (options.baselineRef && headResult.ok) {
    ...
    if (diff.ok) entries.push(...parsePorcelainZ(diff.stdout));
  }
  const byPath = new Map<string, { path: string; state: string }>();
  for (const item of entries) {
    const normalized = normalizePath(item.path);
    if (!normalized || isIgnoredSurface(normalized, options.ignoredPrefixes || [])) continue;
  ...
  const core = {
    status: statusResult.ok ? "available" as const : "unavailable" as const,
...
function stringList(value: any, max = 200) {
  return [...new Set((Array.isArray(value) ? value : []).map(normalizePath).filter(Boolean))].slice(0, max);
}

function criterionEntry(value: any, index: number, globalChecks: any[]) {
...
export type TestAgentSurfaceAuditReceipt = {
  ...
  actualFiles: Array<{ path: string; state: string }>;
  ...
  exemptions: Array<{ criterionId: string; reasonChecksum: string }>;
...
export function auditTestAgentSurface(input: {
  workDir: string;
  declaredFiles?: string[];
  ...
  const checkDefinitions = stringList((input.checkDefinitions || []).map((item: any) => item?.id || item?.checkId || item?.name || item), 200);
  ...
  const criteria = (Array.isArray(criteriaInput) ? criteriaInput : []).slice(0, 200).map((item, index) => {
    ...
    const exemption = (input.exemptions || []).find((candidate: any) => String(candidate?.criterionId || candidate?.criterion_id || "") === entry.criterionId);
    const waived = !!entry.waiver || !!exemption;
    const covered = waived || (hasChecks && refs.length > 0);
    return {
      ...entry,
      fileRefs: refs,
      status: waived ? "waived" as const : covered ? "covered" as const : "uncovered" as const,
  ...
  const status: TestAgentSurfaceAuditReceipt["status"] = unavailable
    ? "unavailable"
    : undeclaredChanges.length || (strict && criterionUncoveredCount)
      ...
      : undeclaredChanges.length || criterionUncoveredCount
        ? "warn"
        : "passed";
  if (undeclaredChanges.length > 0) recordTestAgentHardeningMetric("test_agent_undeclared_change_total", undeclaredChanges.length);
  const core = {
    schema: TEST_AGENT_SURFACE_AUDIT_SCHEMA,
    status,
...
export function readTestAgentSurfaceAudit(value: any): TestAgentSurfaceAuditReceipt {
  if (value?.schema === TEST_AGENT_SURFACE_AUDIT_SCHEMA && !findSurfaceAuditBodyKeys(value)) return value;
...
function findSurfaceAuditBodyKeys(value: any): boolean {
  if (!value || typeof value !== "object") return false;
  if (Array.isArray(value)) return value.some(findSurfaceAuditBodyKeys);
  return Object.entries(value).some(([key, child]) => {
    if (["content", "text", "body", "rawOutput", "stdout", "stderr", "prompt", "toolResult", "context"].includes(key)) return true;
    return findSurfaceAuditBodyKeys(child);
  });
}

export function runTestAgentSurfaceAuditSelfTest() {
  ...
  try {
    ...
    return {
      pass: result.schema === TEST_AGENT_SURFACE_AUDIT_SCHEMA
