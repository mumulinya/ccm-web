...
async function listen(server: http.Server) {
  ...
  if (!address || typeof address === "string") throw new Error("Unable to allocate adversarial gate self-test port.");
...
export async function runTestAgentAdversarialEvidenceGateSelfTest() {
  ...
  try {
    const missingReport = await runTestAgent({
    ...
    const passedHttpReport = await runTestAgent({
    ...
    const unlinkedHttpReport = await runTestAgent({
    ...
    const goalLinkedHttpReport = await runTestAgent({
    ...
    const unknownExplicitCriterionReport = await runTestAgent({
    ...
    const unlinkedPlan = buildTestAgentExecutionPlan({
    ...
    const explicitPlan = buildTestAgentExecutionPlan({
    ...
    const failedHttpReport = await runTestAgent({
    ...
    const blockedHttpReport = await runTestAgent({
    ...
    const browserExecutor = createStaticBrowserToolExecutor({
      tools: [
      ...
      onCall: toolName => {
        if (toolName.endsWith("browser_snapshot")) return "Invalid submission handled";
    ...
    const passedBrowserReport = await runTestAgent({
    ...
    const waivedReport = await runTestAgent({
    ...
    const invalidWaiverValidation = validateTestAgentWorkOrderContract({
    ...
    const snakeCaseWaiverValidation = validateTestAgentWorkOrderContract({
    ...
    const cliSummary = formatTestAgentCliReportSummary(passedHttpReport);
    const markdown = buildTestAgentMarkdownReport(passedHttpReport);
    ...
    const summaryTamperedVerification = verifyTestAgentArtifactManifestFile(httpPaths.manifestPath);
    ...
    const relevanceTamperedVerification = verifyTestAgentArtifactManifestFile(browserPaths.manifestPath);
    ...
    const waiverTamperedVerification = verifyTestAgentArtifactManifestFile(waiverPaths.manifestPath);
    ...
    const pass = missingReport.status === "partial"
      ...
      && validateTestAgentReportContract(passedHttpReport).valid
      && validateTestAgentVerdictContract(passedHttpVerdict).valid
      ...
      && unlinkedHttpReport.requiredCheckCoverage.some(item =>
        item.check === "adversarial"
        && item.status === "unknown"
        && String(item.missingReason || "").includes("none were linked")
      )
      && unlinkedHttpVerdict.canAccept === false
      && unlinkedHttpVerdict.nextActions.some((item: string) => item.includes("explicitly linked"))
      && validateTestAgentReportContract(unlinkedHttpReport).valid
      && validateTestAgentVerdictContract(unlinkedHttpVerdict).valid
      ...
      && validateTestAgentReportContract(goalLinkedHttpReport).valid
      && validateTestAgentVerdictContract(goalLinkedHttpVerdict).valid
      ...
      && validateTestAgentReportContract(unknownExplicitCriterionReport).valid
      && validateTestAgentVerdictContract(unknownExplicitCriterionVerdict).valid
      ...
      && validateTestAgentReportContract(passedBrowserReport).valid
      && validateTestAgentVerdictContract(browserVerdict).valid
      ...
      && validateTestAgentReportContract(waivedReport).valid
      && validateTestAgentVerdictContract(waiverVerdict).valid
