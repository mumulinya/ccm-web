...
export const MIN_BROWSER_TOOL_CALL_TIMEOUT_MS = 1_000;

export function isBrowserToolCallTimeout(value: any) {
...
export function browserToolCallTimeoutEvidenceErrors(input: {
  ...
  const errors: string[] = [];
  if (!input.summary) errors.push("browserToolCallTimeoutSummary is missing.");
  ...
  const resultByCallId = new Map<string, BrowserCheckResult[]>();
  ...
  for (const [index, record] of (input.browserToolCalls || []).entries()) {
    ...
    if (!Number.isFinite(timeoutMs) || timeoutMs < MIN_BROWSER_TOOL_CALL_TIMEOUT_MS) {
      errors.push(`${label}.timeoutMs must be at least ${MIN_BROWSER_TOOL_CALL_TIMEOUT_MS}.`);
    }
    if (record.timedOut === true) {
      if (record.status !== "failed") errors.push(`${label} timed out but status is not failed.`);
      if (record.abortRequested !== true) errors.push(`${label} timed out without abortRequested=true.`);
      if (record.outputPreview) errors.push(`${label} timed out but retained outputPreview.`);
      if (Number(record.durationMs || 0) < timeoutMs) errors.push(`${label}.durationMs is shorter than its timeoutMs.`);
      const allowedErrors = new Set([
        `Browser tool call timed out after ${timeoutMs}ms.`,
        "Browser tool call failed; raw provider error suppressed.",
      ]);
      if (!allowedErrors.has(String(record.error || ""))) errors.push(`${label} has invalid timeout error evidence.`);
...
export function formatBrowserToolCallTimeoutSummaryLine(summary?: BrowserToolCallTimeoutSummary) {
  if (!summary) return "calls=0; passed=0; failed=0; timedOut=0; abortRequested=0";
  return [
...
export function formatBrowserToolCallTimeoutAttentionLines(summary?: BrowserToolCallTimeoutSummary, limit = 5) {
  if (!summary?.items.length) return ["Browser tool timeout attention: none"];
  return [
    "Browser tool timeout attention:",
    ...summary.items.slice(0, Math.max(0, limit)).map(item =>
      `- ${item.toolName} [${item.id}]: timeout=${item.timeoutMs}ms; duration=${item.durationMs}ms; abortRequested=${item.abortRequested}${item.checkId ? `; execution=${item.checkId} run ${item.run}` : ""}`
    ),
  ];
}
