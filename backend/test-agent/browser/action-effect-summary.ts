import {
  ...
  BrowserActionEffectSummaryItem,
  BrowserCheckResult,
} from "../types";
import { BROWSER_ACTION_EFFECT_SIGNALS } from "./action-effects";

function emptySignalCounts(): Record<BrowserActionEffectSignal, number> {
  return {
    url: 0,
    title: 0,
    page_text: 0,
...
function summarizeResult(result: BrowserCheckResult): BrowserActionEffectSummaryItem {
  ...
  for (const effect of effects) {
    ...
    for (const signal of effect.changedSignals) changedSignals[signal] += 1;
  }
  ...
  return {
    ...
    crossSession: effects.filter(effect =>
      Boolean(effect.session && effect.effectSession && effect.session !== effect.effectSession)
    ).length,
    actionTypes,
    changedSignals,
  };
...
export function buildBrowserActionEffectSummary(results: BrowserCheckResult[]): BrowserActionEffectSummary {
  ...
  for (const item of items) {
    ...
    for (const signal of BROWSER_ACTION_EFFECT_SIGNALS) {
...
export function formatBrowserActionEffectSummaryLine(summary: BrowserActionEffectSummary | undefined) {
  if (!summary) return "checks=0; actions=0; changed=0; failed=0; unchanged=0; unavailable=0; crossSession=0; detailSuppressed=0";
  return [
...
export function browserActionEffectSummaryErrors(
  summary: BrowserActionEffectSummary | Record<string, any> | undefined,
  results?: BrowserCheckResult[],
  label = "browser action effect summary",
) {
  ...
  for (const key of Object.keys(summary)) {
    if (!allowedSummaryKeys.has(key)) errors.push(`${label}.${key} is not permitted.`);
  }
  const numericKeys = ["actions", "changed", "unchanged", "unavailable", "failed", "detailSuppressed", "crossSession"] as const;
  if (summary.checks !== items.length) errors.push(`${label}.checks must match items.`);
  ...
  for (const key of numericKeys) {
    ...
    if (Number(summary[key]) !== expected) errors.push(`${label}.${key} must match items.`);
  }
  const expectedActionTypes: Record<string, number> = {};
  ...
  for (const item of items) {
    ...
    for (const signal of Object.keys(item?.changedSignals || {})) {
      if (!BROWSER_ACTION_EFFECT_SIGNALS.includes(signal as BrowserActionEffectSignal)) {
