// playwright-provider.ts — merged from 4 part files (behavior-freeze merge).
...
import {
  writePlaywrightAccessibilitySnapshotArtifact,
} from "./accessibility-snapshot-artifacts";
import {
  writePlaywrightEvidenceScreenshot,
  writePlaywrightFailureScreenshot,
} from "./failure-screenshots";
import {
...
import {
  runBrowserSessionComparison,
} from "./session-comparison";
import {
  browserCheckStabilityRuns,
  browserStabilityGroupId,
  withBrowserStabilityMetadata,
} from "./stability-summary";
import {
  checksForProject,
} from "./shared";
import {
...
import {
  withBrowserCheckExecutionIdentity,
...
// ===== merged from playwright-provider-part-01.ts =====
...
export interface PlaywrightMultiSessionRuntime extends BrowserRuntimeSignals {
  ...
  artifactBase: string;
  screenshots: string[];
  screenshotRefs?: Array<{ stepName: string; path: string; kind: "failure" | "capture" }>;
...
export function redactBrowserStepResult(step: BrowserStepResult, secretBindings: BrowserSecretBinding[]) {
...
export async function capturePlaywrightActionEffectObservation(
  page: any,
...
export function expectedValue(assertion: BrowserAssertionSpec) {
  return assertion.value !== undefined ? assertion.value : assertion.text;
}

export function valuesEqual(actual: any, expected: any) {
...
export function cookieName(assertion: BrowserAssertionSpec) {
  return String(assertion.key || assertion.name || assertion.text || assertion.value || "").trim();
}

export function cookieAssertionDetail(assertion: BrowserAssertionSpec) {
...
export function inputValueAssertionDetail(assertion: BrowserAssertionSpec) {
...
export function attributeName(assertion: BrowserAssertionSpec) {
  return String(assertion.attribute || assertion.attributeName || assertion.attribute_name || assertion.key || "").trim();
}

export function attributeAssertionDetail(assertion: BrowserAssertionSpec) {
  const expected = String(assertion.value ?? assertion.text ?? "");
...
export function computedStyleAssertionDetail(assertion: BrowserAssertionSpec) {
...
export function clipboardExpectedText(assertion: BrowserAssertionSpec | BrowserActionSpec) {
  return String(assertion.value ?? assertion.text ?? "");
}

export function clipboardAssertionDetail(assertion: BrowserAssertionSpec) {
...
export function optionalTableIndex(...values: any[]) {
  for (const value of values) {
...
export function tableExpectedTexts(assertion: BrowserAssertionSpec) {
...
export function textOrderExpectedTexts(assertion: BrowserAssertionSpec) {
  return firstNonEmptyStringList(
    assertion.expectedTexts,
    assertion.expected_texts,
    assertion.texts,
...
export function tableRowText(assertion: BrowserAssertionSpec) {
  return String(assertion.rowText || assertion.row_text || "").trim();
}

export function tableColumnName(assertion: BrowserAssertionSpec) {
  return String(assertion.columnName || assertion.column_name || assertion.columnHeader || assertion.column_header || "").trim();
}

export function tableAssertionDetail(assertion: BrowserAssertionSpec) {
...
export function optionalVisualNumber(value: any) {
  if (value === undefined || value === null || value === "") return undefined;
  const number = Number(value);
  return Number.isFinite(number) ? number : undefined;
}

export function visualAssertionDetail(assertion: BrowserAssertionSpec) {
...
export function textOrderAssertionDetail(assertion: BrowserAssertionSpec) {
...
export function urlAssertionExpected(assertion: BrowserAssertionSpec) {
  return String(assertion.url || assertion.urlIncludes || assertion.url_includes || assertion.value || assertion.text || "").trim();
}

export function comparableUrl(actualUrl: string, expected: string) {
...
export function titleAssertionExpected(assertion: BrowserAssertionSpec) {
...
export function expectedOnlineState(assertion: BrowserAssertionSpec) {
...
export async function waitForTitleMatch(page: any, predicate: (title: string) => boolean, timeout: number) {
...
export async function waitForOnlineState(page: any, expected: boolean, timeout: number) {
  const deadline = Date.now() + Math.max(1, timeout);
  let actual = true;
  while (Date.now() <= deadline) {
...
export function stateAssertionDetail(assertion: BrowserAssertionSpec) {
...
export async function capturePageFinalState(page: any, secretBindings: BrowserSecretBinding[] = []) {
  if (!page) return {};
  let finalUrl = "";
  let title = "";
  let pageText = "";
...
export async function evaluatePageNotBlank(page: any) {
...
export async function evaluateNoHorizontalOverflow(page: any, expectedViewportWidth = 0) {
...
export async function evaluateElementInViewport(locator: any, expectedViewportWidth = 0, expectedViewportHeight = 0) {
...
export async function readBrowserCookie(page: any, assertion: BrowserAssertionSpec) {
  const name = cookieName(assertion);
...
export async function waitForComputedStyle(page: any, assertion: BrowserAssertionSpec, timeout: number) {
...
export async function waitForFocusedState(locator: any, expectedFocused: boolean, timeout: number) {
  const deadline = Date.now() + Math.max(1, timeout);
...
export async function waitForElementCount(locator: any, assertion: BrowserAssertionSpec, timeout: number) {
  const expected = elementCountExpected(assertion);
...
export async function waitForBrowserDialog(signals: BrowserRuntimeSignals, assertion: BrowserAssertionSpec, timeout: number) {
  const deadline = Date.now() + Math.max(1, timeout);
...
export function browserPopupLogLine(record: CapturedBrowserPopup) {
  const parts = [
...
export async function captureBrowserPopup(popup: any, secretBindings: BrowserSecretBinding[] = []): Promise<CapturedBrowserPopup> {
...
export async function waitForBrowserPopup(signals: BrowserRuntimeSignals, assertion: BrowserAssertionSpec, timeout: number) {
  const deadline = Date.now() + Math.max(1, timeout);
  const index = popupIndex(assertion);
  while (Date.now() <= deadline) {
...
export async function writeClipboardText(page: any, value: string) {
...
export async function waitForClipboardText(page: any, assertion: BrowserAssertionSpec, timeout: number) {
  const expected = clipboardExpectedText(assertion);
...
export function pngChannelCount(colorType: number) {
...
export function pngPaeth(left: number, up: number, upperLeft: number) {
...
// ===== merged from playwright-provider-part-02.ts =====
...
// ===== merged from playwright-provider-part-03.ts =====
...
export async function runBrowserCheck(browser: any, context: BrowserProviderContext, project: NormalizedTestAgentProjectTarget, check: BrowserCheckSpec, index: number): Promise<BrowserCheckResult> {
  ...
  const normalScreenshotRequested = check.screenshot !== false || requiredCheckEnabled(workOrder.requiredChecks, "screenshots");
  ...
  try {
    ...
    for (let actionIndex = 0; actionIndex < actions.length; actionIndex += 1) {
      ...
      if (normalScreenshotRequested && verifyEffect) {
        const beforeRefs = await writePlaywrightEvidenceScreenshot({
          page,
      ...
      if (normalScreenshotRequested && verifyEffect) {
        const afterRefs = await writePlaywrightEvidenceScreenshot({
          page,
          artifactDir: workOrder.options.artifactDir,
          projectName: project.name,
          checkName: name,
      ...
      if (step.status === "failed" || effectFailed) break;
    }
    ...
    if (normalScreenshotRequested) {
      const finalRefs = await writePlaywrightEvidenceScreenshot({
        page,
        artifactDir: workOrder.options.artifactDir,
        projectName: project.name,
        checkName: name,
      ...
      if (!finalRefs.length) {
        ...
        screenshotRefs.push(...(assertionSteps.length
          ? assertionSteps.map(item => ({ stepName: item.name, path: finalPath, kind: item.status === "failed" ? "failure" as const : "capture" as const }))
          : finalRefs));
      }
    }
...
// ===== merged from playwright-provider-part-04.ts =====
...
async function runMultiSessionBrowserCheck(browser: any, context: BrowserProviderContext, project: NormalizedTestAgentProjectTarget, check: BrowserCheckSpec, index: number): Promise<BrowserCheckResult> {
  ...
  const normalScreenshotRequested = check.screenshot !== false || requiredCheckEnabled(context.workOrder.requiredChecks, "screenshots");
  const steps: BrowserStepResult[] = [];
...
export const PlaywrightBrowserProvider: BrowserProvider = {
  ...
  async run(context) {
    ...
    try {
      ...
      return [blockedBrowserResult("playwright", "Load Playwright", `Playwright is unavailable: ${error.message || String(error)}. Install the package and then run: npx playwright install chromium`)];
    }
    ...
    try {
      ...
      return [blockedBrowserResult("playwright", "Launch browser", `${error.message || String(error)}. ${PLAYWRIGHT_BROWSER_INSTALL_HINT}`)];
    }
    try {
