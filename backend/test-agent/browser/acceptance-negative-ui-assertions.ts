...
function negativeAfterQuotedTarget(after: string) {
...
function associatedNegativeContext(criterion: string, quote: QuotedTextSpan) {
  ...
  if (!negativeBeforeQuotedTarget(before) && !negativeAfterQuotedTarget(after)) return "";
  return `${before}${quote.text}${after}`;
}

function requiresDomAbsence(context: string) {
...
function assertionForQuote(criterion: string, quote: QuotedTextSpan): BrowserAssertionSpec | null {
...
export function buildAcceptanceNegativeUiBrowserAssertions(criterion: string): BrowserAssertionSpec[] {
