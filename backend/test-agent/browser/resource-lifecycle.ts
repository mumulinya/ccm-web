...
function cloneEvents(events: BrowserResourceLifecycleEvent[]) {
  return events.map(event => ({ ...event }));
}

export function createBrowserResourceLifecycleRecorder(): BrowserResourceLifecycleRecorder {
  const events: BrowserResourceLifecycleEvent[] = [];
  const byId = new Map<string, BrowserResourceLifecycleEvent>();

  const add = (
    input: Parameters<BrowserResourceLifecycleRecorder["acquire"]>[0],
    ownership: BrowserResourceLifecycleEvent["ownership"],
    status: BrowserResourceLifecycleEvent["status"],
  ) => {
    const event: BrowserResourceLifecycleEvent = {
      id: makeRunId("browser-resource"),
      planId: input.planId,
  ...
  return {
    ...
    cleanupFailed(id, error) {
      const event = byId.get(id);
      if (!event || event.status !== "open") return;
      event.releaseAttemptedAt = nowIso();
...
function emptyResourceTypeCounts(): Record<BrowserResourceType, number> {
  return { browser: 0, browser_context: 0, external_browser_session: 0 };
}

export function buildBrowserResourceLifecycleSummary(input: {
  ...
  for (const event of events) {
    if (event.resourceType in resourceTypeCounts) resourceTypeCounts[event.resourceType] += 1;
    if (!input.plan?.planId || event.planId !== input.plan.planId) planMismatchCount += 1;
    const validOwnership = event.ownership === "external"
      ? event.resourceType === "external_browser_session" && event.status === "retained"
      : event.resourceType !== "external_browser_session" && event.status !== "retained";
    if (!validOwnership) invalidOwnershipCount += 1;
    ...
    const timestamps = [acquired, attempted, released].filter(value => value !== undefined) as number[];
    ...
    for (const value of [attempted, released]) {
      if (value !== undefined && Number.isFinite(value) && (
        value < acquired || value < reportStarted || value > reportFinished
      )) outsideReportWindowCount += 1;
    }
    if (event.status === "released" && (!event.releaseAttemptedAt || !event.releasedAt)) invalidTimestampCount += 1;
    if (event.status === "cleanup_failed" && (!event.releaseAttemptedAt || !event.error)) invalidTimestampCount += 1;
    if ((event.status === "open" || event.status === "retained") && (event.releaseAttemptedAt || event.releasedAt)) invalidTimestampCount += 1;
  }
  ...
  const status: BrowserResourceLifecycleSummary["status"] = invalid
    ? "invalid"
...
export function browserResourceLifecycleErrors(report: Pick<
  TestAgentReport,
  "startedAt" | "finishedAt" | "status" | "browserResourceLifecycleEvents" | "browserResourceLifecycleSummary" | "metadata"
>) {
  ...
  const errors: string[] = [];
  if (!report.browserResourceLifecycleSummary) errors.push("browserResourceLifecycleSummary is missing.");
  ...
  if (expected.openResourceCount) errors.push(`${expected.openResourceCount} owned browser resource(s) remain open.`);
  if (expected.cleanupFailureCount) errors.push(`${expected.cleanupFailureCount} browser resource cleanup operation(s) failed.`);
  if (expected.planMismatchCount) errors.push(`${expected.planMismatchCount} browser resource event(s) belong to another execution plan.`);
  if (expected.duplicateResourceCount) errors.push(`${expected.duplicateResourceCount} duplicate browser resource event ID(s) were recorded.`);
  if (expected.invalidOwnershipCount) errors.push(`${expected.invalidOwnershipCount} browser resource ownership policy violation(s) were recorded.`);
...
export function formatBrowserResourceLifecycleLine(summary?: BrowserResourceLifecycleSummary) {
  if (!summary) return "status=incomplete; owned=0; released=0; external=0; open=0; cleanupFailed=0; invalid=0";
  return [
...
export function formatBrowserResourceLifecycleAttentionLines(summary?: BrowserResourceLifecycleSummary, limit = 5) {
  const attention = (summary?.events || []).filter(event => event.status === "open" || event.status === "cleanup_failed");
  if (!attention.length) return ["Browser resource lifecycle attention: none"];
  return [
    "Browser resource lifecycle attention:",
    ...attention.slice(0, Math.max(0, limit)).map(event =>
      `- ${event.provider} ${event.resourceType} ${event.scope}: ${event.status}${event.error ? `; ${event.error}` : ""}`
    ),
  ];
}
