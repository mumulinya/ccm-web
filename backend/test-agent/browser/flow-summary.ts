import {
  BrowserCheckResult,
  BrowserFlowSummary,
  BrowserFlowSummaryFailureItem,
  BrowserFlowSummaryItem,
} from "../types";

function emptyStatusCounts(): BrowserFlowSummary["statusCounts"] {
  return {
    passed: 0,
    failed: 0,
    blocked: 0,
...
function acceptanceCriteria(result: BrowserCheckResult) {
  const raw = result.context?.acceptanceCriteria;
  if (!Array.isArray(raw)) return [];
  return raw.map(item => String(item || "").trim()).filter(Boolean);
}

function flowType(result: BrowserCheckResult) {
  ...
  if (source && source !== "acceptance_criteria") return "";
  return generatedBy || String(result.probeType || "").trim();
}

function failedStepLines(result: BrowserCheckResult) {
  return result.steps
    .filter(step => step.status === "failed")
    .map(step => `${step.name}${step.error ? `: ${step.error}` : step.detail ? `: ${step.detail}` : ""}`);
}

function failureItem(result: BrowserCheckResult): BrowserFlowSummaryFailureItem | null {
  const failedSteps = failedStepLines(result);
  if (result.status !== "failed" && result.status !== "blocked" && !failedSteps.length) return null;
  return {
...
function summarizeGroup(group: MutableFlowGroup): BrowserFlowSummaryItem {
  const statusCounts = emptyStatusCounts();
  let actionCount = 0;
  let assertionCount = 0;
  let failedStepCount = 0;
  const failures: BrowserFlowSummaryFailureItem[] = [];
...
export function buildBrowserFlowSummary(browserResults: BrowserCheckResult[]): BrowserFlowSummary {
  const groups = new Map<string, MutableFlowGroup>();
  const included: BrowserCheckResult[] = [];
  ...
  for (const result of browserResults) {
    ...
    for (const criterion of criteria) allCriteria.add(criterion);
    ...
    for (const criterion of criteria) group.criteria.add(criterion);
    if (result.project) group.projects.add(result.project);
    if (result.provider) group.providers.add(result.provider);
  }
...
export function formatBrowserFlowSummaryLine(summary?: BrowserFlowSummary) {
  if (!summary) return "passed:0, failed:0, blocked:0, skipped:0, total:0, types:0, criteria:0";
...
export function formatBrowserFlowAttentionLines(summary?: BrowserFlowSummary, limit = 5) {
  if (!summary) return ["Browser flow attention: none"];
  const attention = summary.items.filter(item => item.statusCounts.failed > 0 || item.statusCounts.blocked > 0 || item.failedStepCount > 0);
  if (!attention.length) return ["Browser flow attention: none"];
  return [
