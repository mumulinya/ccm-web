import {
  ...
  BrowserToolEvidenceLineageItem,
  BrowserToolEvidenceLineageStatus,
  BrowserToolEvidenceLineageSummary,
} from "../types";

export function browserExecutionLineageKey(execution: BrowserCheckExecutionIdentity) {
  return `${execution.planId}::${execution.checkId}::${execution.run}`;
}

function sameExecution(left?: BrowserCheckExecutionIdentity, right?: BrowserCheckExecutionIdentity) {
...
function emptyStatusCounts(): BrowserToolEvidenceLineageSummary["statusCounts"] {
  return { complete: 0, incomplete: 0, invalid: 0 };
}

export function buildBrowserToolEvidenceLineage(
  browserResults: BrowserCheckResult[],
  browserToolCalls: BrowserToolCallRecord[],
): BrowserToolEvidenceLineageSummary {
  const recordsById = new Map<string, BrowserToolCallRecord>();
  const duplicateRecordIds = new Set<string>();
  for (const record of browserToolCalls) {
    if (recordsById.has(record.id)) duplicateRecordIds.add(record.id);
    else recordsById.set(record.id, record);
  }

  const mcpResults = browserResults.filter(result =>
    result.provider === "mcp"
    && result.execution?.evidence === "provider"
  );
  const resultByExecution = new Map<string, BrowserCheckResult>();
  for (const result of mcpResults) resultByExecution.set(browserExecutionLineageKey(result.execution!), result);

  const references = new Map<string, Set<string>>();
  const items: BrowserToolEvidenceLineageItem[] = mcpResults.map(result => {
    ...
    const foreignToolCallIds: string[] = [];
    ...
    for (const id of toolCallIds) {
      ...
      if (record.status === "failed") failedToolCallCount += 1;
    }
    const evidenceRequired = result.status !== "blocked" || toolCallIds.length > 0;
    const status: BrowserToolEvidenceLineageStatus = duplicateToolCallIds.length || missingToolCallIds.length || foreignToolCallIds.length
      ? "invalid"
  ...
  const crossLinkedIds = new Set(
    [...references.entries()].filter(([, owners]) => owners.size > 1).map(([id]) => id),
  );
  ...
  const status: BrowserToolEvidenceLineageStatus = invalid
    ? "invalid"
...
export function browserToolEvidenceLineageErrors(input: {
  browserResults: BrowserCheckResult[];
  browserToolCalls: BrowserToolCallRecord[];
  ...
  const errors: string[] = [];
  if (!input.summary) errors.push("browserToolEvidenceLineage is missing.");
...
export function formatBrowserToolEvidenceLineageLine(summary?: BrowserToolEvidenceLineageSummary) {
  if (!summary) return "status=incomplete; results=0/0; calls=0/0; orphan=0; unscoped=0; invalid=0";
  return [
...
export function formatBrowserToolEvidenceLineageAttentionLines(summary?: BrowserToolEvidenceLineageSummary, limit = 5) {
  const attention = (summary?.items || []).filter(item => item.status !== "complete");
  if (!attention.length) return ["Browser tool evidence lineage attention: none"];
  return [
    "Browser tool evidence lineage attention:",
    ...attention.slice(0, Math.max(0, limit)).map(item =>
      `- ${item.project} / ${item.name} [${item.checkId} run ${item.run}]: ${item.status}; linked=${item.linkedToolCallCount}; missing=${item.missingToolCallIds.join(",") || "none"}; foreign=${item.foreignToolCallIds.join(",") || "none"}; duplicate=${item.duplicateToolCallIds.join(",") || "none"}`
    ),
  ];
}
