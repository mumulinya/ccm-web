import {
  ...
  BrowserRecoverySummaryItem,
} from "../types";

export function buildBrowserRecoverySummary(results: BrowserCheckResult[]): BrowserRecoverySummary {
  const items: BrowserRecoverySummaryItem[] = results
...
export function formatBrowserRecoverySummaryLine(summary: BrowserRecoverySummary | undefined) {
  if (!summary) return "checks=0; attempted=0; recovered=0; failed=0; unsafeRetriesPrevented=0";
  return [
