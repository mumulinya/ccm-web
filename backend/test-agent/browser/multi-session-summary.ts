import {
  ...
  BrowserMultiSessionSummaryItem,
} from "../types";

function emptyStatusCounts(): BrowserMultiSessionSummary["statusCounts"] {
  return {
    passed: 0,
    failed: 0,
    blocked: 0,
...
function sessionNameFromStep(name: string) {
  return /^session:([^:]+):/.exec(String(name || ""))?.[1] || "";
}

function summarizeResult(result: BrowserCheckResult): BrowserMultiSessionSummaryItem {
  ...
  for (const session of sessions) {
    if (session.consoleErrors.length || session.pageErrors.length || session.networkErrors.length) failedSessionNames.add(session.name);
  }
  return {
...
export function buildBrowserMultiSessionSummary(browserResults: BrowserCheckResult[]): BrowserMultiSessionSummary {
  ...
  for (const item of items) {
    ...
    for (const sessionName of item.sessionNames) sessionNames.add(sessionName);
  }
  return {
    total: items.length,
    statusCounts,
...
export function formatBrowserMultiSessionSummaryLine(summary?: BrowserMultiSessionSummary) {
...
export function formatBrowserMultiSessionAttentionLines(summary?: BrowserMultiSessionSummary, limit = 5) {
  if (!summary) return [];
