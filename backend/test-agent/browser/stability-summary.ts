import {
  BrowserCheckResult,
  BrowserCheckSpec,
  BrowserStabilityStatus,
  BrowserStabilitySummary,
  BrowserStabilitySummaryItem,
} from "../types";

export const MAX_BROWSER_STABILITY_RUNS = 10;

export function browserCheckStabilityRuns(check: BrowserCheckSpec) {
  const parsed = Number(check.stabilityRuns ?? check.stability_runs ?? 1);
  if (!Number.isFinite(parsed)) return 1;
  return Math.max(1, Math.min(MAX_BROWSER_STABILITY_RUNS, Math.floor(parsed)));
}

export function browserStabilityGroupId(project: string, check: BrowserCheckSpec, index: number) {
  return [String(project || ""), String(index), String(check.name || `Browser check ${index + 1}`)].join("::");
}

export function withBrowserStabilityMetadata(input: {
  result: BrowserCheckResult;
  groupId: string;
  run: number;
  runs: number;
}) {
  if (input.runs <= 1) return input.result;
  return {
    ...input.result,
    context: {
...
export function browserStabilityMetadata(result: BrowserCheckResult): StabilityMetadata | null {
  ...
  if (result.context?.browserStability !== true && runs <= 1) return null;
  if (!groupId || !Number.isInteger(run) || run <= 0 || !Number.isInteger(runs) || runs <= 1) return null;
  return { groupId, run, runs };
}

function emptyResultCounts(): BrowserStabilitySummaryItem["statusCounts"] {
  return { passed: 0, failed: 0, blocked: 0, skipped: 0 };
}

function emptyStabilityCounts(): BrowserStabilitySummary["statusCounts"] {
  return { stable_pass: 0, stable_fail: 0, flaky: 0, blocked: 0 };
}

function groupStatus(
  expectedRuns: number,
  entries: Array<{ result: BrowserCheckResult; metadata: StabilityMetadata }>,
): BrowserStabilityStatus {
  ...
  if (!metadataIsConsistent || results.length !== expectedRuns || runSet.size !== expectedRuns || !hasEveryRun) return "blocked";
  if (results.some(result => result.status === "blocked" || result.status === "skipped")) return "blocked";
  if (results.every(result => result.status === "passed")) return "stable_pass";
  if (results.every(result => result.status === "failed")) return "stable_fail";
  return "flaky";
}

function failureLine(result: BrowserCheckResult, run: number) {
...
export function buildBrowserStabilitySummary(browserResults: BrowserCheckResult[]): BrowserStabilitySummary {
  const groups = new Map<string, Array<{ result: BrowserCheckResult; metadata: StabilityMetadata }>>();
  ...
  const items: BrowserStabilitySummaryItem[] = [];
...
export function formatBrowserStabilitySummaryLine(summary?: BrowserStabilitySummary) {
  if (!summary) return "groups=0; stable=0; flaky=0; failed=0; blocked=0; runs=0/0";
  return [
...
export function formatBrowserStabilityAttentionLines(summary?: BrowserStabilitySummary, limit = 5) {
  const attention = (summary?.items || []).filter(item => item.status !== "stable_pass");
  if (!attention.length) return ["Browser stability attention: none"];
  return [
    "Browser stability attention:",
    ...attention.slice(0, Math.max(0, limit)).map(item =>
      `- ${item.project} / ${item.name}: ${item.status}; runs=${item.runCount}/${item.expectedRuns}; failedRuns=${item.failedRuns.join(",") || "none"}; blockedRuns=${item.blockedRuns.join(",") || "none"}${item.firstFailure ? `; first=${item.firstFailure}` : ""}`
    ),
  ];
}
