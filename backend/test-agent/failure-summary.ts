...
function firstFailedBrowserStep(result: BrowserCheckResult) {
  return result.steps.find(step => step.status === "failed");
}

function browserReason(result: BrowserCheckResult) {
  const failedStep = firstFailedBrowserStep(result);
  if (failedStep?.error) return `${failedStep.name}: ${failedStep.error}`;
  if (result.error) return result.error;
  if (result.consoleErrors.length) return `console error: ${result.consoleErrors[0]}`;
  if (result.pageErrors.length) return `page error: ${result.pageErrors[0]}`;
  if ((result.networkErrors || []).length) return `network error: ${(result.networkErrors || [])[0]}`;
  return "Browser check failed without a specific failed step.";
}

function browserEvidence(result: BrowserCheckResult) {
  return [
...
function uniqueDiagnostics(items: string[]) {
...
function commandDiagnostics(result: CommandRunResult) {
...
function serverDiagnostics(result: DevServerResult) {
  return uniqueDiagnostics([
...
function httpDiagnostics(result: HttpCheckResult) {
  return uniqueDiagnostics([
...
function browserDiagnostics(result: BrowserCheckResult) {
  const failedStep = firstFailedBrowserStep(result);
...
function requiredCheckDiagnostics(item: RequiredCheckCoverageItem) {
  return uniqueDiagnostics([
...
function acceptanceDiagnostics(item: AcceptanceCoverageItem) {
...
function issueDiagnostics(issue: WorkOrderIssue) {
  return uniqueDiagnostics([
...
function nextActionForType(type: TestAgentFailureSummaryItem["type"]) {
  if (type === "command") return "Fix the failing command or its inputs, then rerun TestAgent.";
  if (type === "http") return "Fix the HTTP/API behavior or configured endpoint, then rerun TestAgent.";
  if (type === "browser") return "Reproduce the browser step, fix the UI behavior, then rerun the browser check.";
  if (type === "required_check") return "Add or repair evidence for the required check before accepting the delivery.";
  if (type === "acceptance") return "Map this acceptance criterion to passing command, HTTP, or browser evidence.";
  if (type === "server") return "Fix dev server startup/readiness before rerunning verification.";
  return "Resolve the work-order issue before rerunning TestAgent.";
}

function acceptanceNextAction(item: AcceptanceCoverageItem) {
  const strength = item.matchStrength || "none";
...
function acceptanceReason(item: AcceptanceCoverageItem) {
  const strength = item.matchStrength || "none";
  const source = item.evidenceSource || "none";
...
function coverageFailureStatus(status: RequiredCheckCoverageItem["status"] | AcceptanceCoverageItem["status"]): TestAgentFailureSummaryItem["status"] {
  return status === "not_verified" ? "not_verified" : "unknown";
}

export function buildTestAgentFailureSummary(input: {
  ...
  const items: TestAgentFailureSummaryItem[] = [];
