...
import { getConfigs, loadFeishuConfig } from "../core/db";
import { resolveCredential } from "../core/credential-store";
import { sendJson } from "../core/utils";

type ReactionScope = "global" | "project";
type ReactionStatus = "completed" | "failed" | "cancelled";
type ReactionCredentials = { appId: string; appSecret: string };
type ReactionJob = {
  key: string;
  scope: ReactionScope;
  project: string;
...
const activeReactionJobs = new Map<string, ReactionJob>();
const tenantTokenCache = new Map<string, { token: string; expiresAt: number }>();

function normalizeScope(value: any): ReactionScope {
...
function normalizeMessageId(value: any) {
  const messageId = String(value || "").trim();
  if (!/^om_[a-z0-9_-]{8,200}$/i.test(messageId)) throw new Error("飞书消息 ID 无效");
  return messageId;
}

function parseProjectFeishuCredentials(project: string): ReactionCredentials {
  const config = getConfigs().find(item => String(item.name || "") === project);
  if (!config?.path || !fs.existsSync(config.path)) throw new Error("项目飞书配置不存在");
  ...
  if (!feishuBlock) throw new Error("项目没有配置飞书机器人");
  ...
  const appSecret = appSecretValue ? resolveCredential(appSecretValue) : "";
  if (!appId || !appSecret) throw new Error("项目飞书机器人凭据不完整");
  return { appId, appSecret };
}

function resolveReactionCredentials(scope: ReactionScope, project: string): ReactionCredentials {
  if (scope === "project") return parseProjectFeishuCredentials(project);
  ...
  if (!appId || !appSecret) throw new Error("全局飞书机器人凭据不完整");
  return { appId, appSecret };
}

async function readFeishuJson(response: Response, action: string) {
...
async function getTenantToken(credentials: ReactionCredentials, deps: ReactionDependencies) {
  ...
  if (!token) throw new Error("飞书机器人 Token 为空");
...
async function callReactionApi(
  credentials: ReactionCredentials,
  messageId: string,
  input: { action: "add"; emoji: string } | { action: "delete"; reactionId: string },
...
function reactionJobKey(scope: ReactionScope, project: string, messageId: string) {
  return `${scope}:${project}:${messageId}`;
}

function logReactionFailure(job: Pick<ReactionJob, "scope" | "project">, phase: string, error: any) {
  console.warn(
    `[飞书状态表情] ${phase}失败 scope=${job.scope} project=${job.project || "-"} reason=${String(error?.message || error).slice(0, 180)}`,
  );
}

export function beginFeishuReactionFeedback(input: {
  scope?: ReactionScope;
  project?: string;
  messageId?: string;
}, deps: ReactionDependencies = {}) {
  ...
  if (scope === "project" && !project) throw new Error("项目状态表情缺少项目 ID");
  const messageId = normalizeMessageId(input.messageId);
  const key = reactionJobKey(scope, project, messageId);
  const existing = activeReactionJobs.get(key);
  if (existing) return { accepted: true, duplicate: true, key };
...
export function finishFeishuReactionFeedback(input: {
  scope?: ReactionScope;
  project?: string;
  messageId?: string;
  ...
  const status = ["failed", "cancelled"].includes(String(input.status || ""))
    ? String(input.status) as ReactionStatus
    : "completed";
  const key = reactionJobKey(scope, project, messageId);
  const job = activeReactionJobs.get(key);
  if (!job || job.finalizing) return { accepted: false, missing: !job, key };
  job.finalizing = true;
  clearTimeout(job.timer);
  const resolveCredentials = deps.resolveCredentials || resolveReactionCredentials;
  let credentials: ReactionCredentials;
  try {
...
export function getFeishuReactionFeedbackState() {
...
function readBoundedJson(req: any, maxBytes = 64 * 1024) {
...
export function handleFeishuReactionFeedbackApi(pathname: string, req: any, res: any) {
  ...
  if (req.ccmAuth?.kind !== "internal" || req.ccmAuth?.caller !== "feishu-acp") {
    return sendJson(res, { success: false, error: "仅允许签名飞书 ACP 调用" }, 403);
  }
  void readBoundedJson(req).then(payload => {
    const rawScope = String(payload.scope || "").trim();
    if (rawScope !== "global" && rawScope !== "project") throw new Error("无效的飞书状态作用域");
    const common = {
      scope: rawScope as ReactionScope,
    ...
    if (!res.headersSent) sendJson(res, { success: false, error: error?.message || String(error) }, 400);
  });
  return true;
}

export async function runFeishuReactionFeedbackSelfTest() {
  ...
  const deps: ReactionDependencies = {
    fetchImpl,
