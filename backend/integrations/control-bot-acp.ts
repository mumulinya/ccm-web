...
const requestTimeoutMs = Math.max(1_000, Math.min(15 * 60 * 1000, Number(
  projectMode
...
const adapterBuildId = (() => {
  try { return crypto.createHash("sha256").update(fs.readFileSync(__filename)).digest("hex").slice(0, 12); }
...
function enqueueOutput(payload: string) {
  const pending = stdoutWriteQueue.then(() => new Promise<void>((resolve, reject) => {
    process.stdout.write(payload, (error) => {
      if (error) reject(error);
      else resolve();
    });
  }));
  stdoutWriteQueue = pending.catch((error: any) => {
...
function writeAsync(message: any) {
  return enqueueOutput(JSON.stringify({ jsonrpc: "2.0", ...message }) + "\n");
}

function writeBatchAsync(messages: any[]) {
...
function extractPlatformContext(params: any = {}) {
  ...
  const read = (...keys: string[]) => {
    for (const source of sources) for (const key of keys) if (source?.[key]) return String(source[key]);
    return "";
  };
  return {
...
type FeishuReactionStatus = "completed" | "failed" | "cancelled";

async function postFeishuReactionFeedback(
  action: "start" | "finish",
  platformContext: any,
  status?: FeishuReactionStatus,
) {
  const messageId = String(platformContext?.platform_message_id || "").trim();
  if (!/^om_[a-z0-9_-]{8,200}$/i.test(messageId)) return false;
  ...
  try {
    const response = await fetch(`${baseUrl}/api/internal/feishu-reaction/${action}`, {
      method: "POST",
      headers: { "Content-Type": "application/json", ...buildInternalApiHeaders("feishu-acp", "POST", `/api/internal/feishu-reaction/${action}`) },
      body: JSON.stringify({
    ...
    process.stderr.write(
      `[CCM control bot ACP] reaction feedback failed action=${action} mode=${projectMode ? "project" : "global"} reason=${String(error?.message || error).slice(0, 160)}\n`,
    );
    return false;
  } finally {
...
function beginFeishuReactionFeedback(platformContext: any) {
  const messageId = String(platformContext?.platform_message_id || "").trim();
  if (!/^om_[a-z0-9_-]{8,200}$/i.test(messageId)) return (_status: FeishuReactionStatus) => {};
  const started = postFeishuReactionFeedback("start", platformContext);
  let finished = false;
  return (status: FeishuReactionStatus) => {
    if (finished) return;
    finished = true;
...
async function callGlobalAgent(
  text: string,
  sessionId = "default",
  messageId = "",
  platformContext: any = {},
  ...
  try {
    const run = (async () => {
      const response = await fetch(`${baseUrl}/api/feishu/control-bot/message`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "Accept": "text/event-stream", ...buildInternalApiHeaders("feishu-acp", "POST", "/api/feishu/control-bot/message") },
        body: JSON.stringify({
          text,
          sessionId,
...
function projectTargetMatches(target: any, context: any) {
  ...
  if (!chatId || String(target?.chat_id || "") !== chatId) return false;
  if (rootId) return String(target?.thread_id || "") === rootId;
  if (openId) return String(target?.open_id || "") === openId;
  return true;
}

async function jsonRequest(pathname: string, init: any = {}) {
  const method = String(init.method || "GET").toUpperCase();
  const response = await fetch(`${baseUrl}${pathname}`, { ...init, headers: { ...(init.headers || {}), ...buildInternalApiHeaders("feishu-acp", method, pathname) } });
  const text = await response.text();
  let data: any = {};
  try { data = text ? JSON.parse(text) : {}; } catch {}
  if (!response.ok || data?.success === false) throw new Error(data?.error || `CCM 请求失败 (${response.status})`);
  return data;
}

async function notifyProjectSessionChanged(projectSessionId: string, status: "inbound" | "reply") {
  if (!projectMode || !projectSessionId) return;
  try {
...
function projectReplyFromSse(payload: string) {
  const chunks: string[] = [];
  let error = "";
  let fallback = "";
  ...
  if (!reply && error) throw new Error(error);
  if (!reply) throw new Error("项目主 Agent 未返回可展示内容");
  return reply;
}

async function readProjectSseResponse(response: any, onDelta?: (delta: string) => Promise<void> | void) {
  ...
  if (!reader) {
    ...
    if (data) {
      if (data?.success === false) throw new Error(data?.error || "Agent 请求失败");
      reply = String(data?.reply || data?.run?.final_reply || data?.message || "").trim();
    }
    if (!reply) reply = projectReplyFromSse(body);
    if (onDelta) await onDelta(reply);
    return reply;
  }
...
async function callProjectAgent(
  text: string,
  sessionId = "default",
  messageId = "",
  platformContext: any = {},
  ...
  try {
    const run = (async () => {
      ...
      const metadataMatches = (targets.targets || []).filter((target: any) => projectTargetMatches(target, platformContext));
      const resolvedTarget = targets.resolved_target || (metadataMatches.length === 1 ? metadataMatches[0] : null);
      if (!resolvedTarget) throw new Error(metadataMatches.length > 1 ? "当前飞书目标匹配到多个项目会话" : "当前 ACP 会话尚未绑定项目会话");
      ...
      if (!projectSessionId) throw new Error("当前飞书目标尚未绑定活动项目会话");
      ...
      const inboundClaim = acquireFeishuInboundReceipt(inboundEnvelope, requestTimeoutMs + 30_000);
      ...
      const originReceipt = buildFeishuOriginReceiptV2({ envelope: inboundEnvelope, sessionId: projectSessionId });
      ...
      try {
        const response = await fetch(`${baseUrl}/api/send-stream`, {
          method: "POST",
          headers: { "Content-Type": "application/json", ...buildInternalApiHeaders("feishu-acp", "POST", "/api/send-stream") },
          body: JSON.stringify({
            project,
...
function textUpdateMessage(sessionId: string, text: string) {
...
function thinkingUpdateMessage(sessionId: string) {
  return { method: "session/update", params: {
    sessionId,
    update: {
...
function startTurnHeartbeat(sessionId: string) {
  let active = true;
  let timer: any = null;
  ...
  const schedule = (delay: number) => {
    timer = setTimeout(() => {
      if (!active) return;
      pending = writeAsync(thinkingUpdateMessage(sessionId)).catch((error: any) => {
...
async function completeTurnWithText(id: any, sessionId: string, text: string) {
  const normalized = String(text || "").trim();
  if (!normalized) throw new Error("ACP turn cannot complete without visible text");
...
async function completeStreamedTurn(id: any, sessionId: string) {
...
async function handleRequest(message: any) {
  ...
  try {
    ...
    if (method === "session/prompt") {
      ...
      let finishReaction = (_status: FeishuReactionStatus) => {};
      let reactionStatus: FeishuReactionStatus = "failed";
      ...
      try {
        ...
        if (streamedReply) await completeStreamedTurn(id, sessionId);
        else await completeTurnWithText(id, sessionId, reply);
