...
import { verifyLiveProviderMemorySoakReport } from "./live-provider-memory-soak-report-store";

export const LIVE_PROVIDER_MEMORY_VERSION_TRANSITION_LEDGER_SCHEMA = "ccm-live-provider-memory-version-transition-ledger-v1";
const LEDGER_FILE = path.join(CCM_DIR, "reliability", "live-provider-memory-version-transition-ledger.json");
...
function entryCore(entry: any) {
  const { entryChecksum: _entryChecksum, ...core } = entry || {};
  return core;
}

function ledgerCore(ledger: any) {
...
function emptyLedger() {
  return {
    schema: LIVE_PROVIDER_MEMORY_VERSION_TRANSITION_LEDGER_SCHEMA,
    version: 1,
    generation: 0,
    updatedAt: "",
    chainAnchorChecksum: "",
...
function sealLedger(ledger: any) {
  const core = ledgerCore(ledger);
  return { ...core, ledgerChecksum: digest(core) };
}

export function verifyLiveProviderMemoryVersionTransitionLedger(ledger: any) {
  const issues: string[] = [];
  if (!ledger || ledger.schema !== LIVE_PROVIDER_MEMORY_VERSION_TRANSITION_LEDGER_SCHEMA || Number(ledger.version || 0) !== 1) issues.push("transition_ledger_schema_invalid");
  const entries = Array.isArray(ledger?.entries) ? ledger.entries : [];
  let previous = String(ledger?.chainAnchorChecksum || "");
  for (const entry of entries) {
    if (String(entry?.previousEntryChecksum || "") !== previous) issues.push("transition_entry_chain_invalid");
    const expected = digest(entryCore(entry));
    if (String(entry?.entryChecksum || "") !== expected) issues.push("transition_entry_checksum_invalid");
    previous = String(entry?.entryChecksum || expected);
  }
  const expectedLedgerChecksum = digest(ledgerCore(ledger));
  if (String(ledger?.ledgerChecksum || "") !== expectedLedgerChecksum) issues.push("transition_ledger_checksum_invalid");
  return {
...
function loadLedger() {
  const ledger = readJsonWithBackup<any>(LEDGER_FILE, null);
  if (!ledger) return { ledger: sealLedger(emptyLedger()), present: false, verification: { valid: true, issues: [], headChecksum: "", entryCount: 0 } };
  return { ledger, present: true, verification: verifyLiveProviderMemoryVersionTransitionLedger(ledger) };
}

function compactComparison(comparison: any) {
  return {
...
export function recordLiveProviderMemoryVersionTransitionEvidence(report: any, options: any = {}) {
  const verification = verifyLiveProviderMemorySoakReport(report, "endurance");
  if (!verification.valid) throw new Error("version transition evidence requires a valid endurance report");
  const comparisons = Array.isArray(report?.providerVersionTrend?.comparisons) ? report.providerVersionTrend.comparisons : [];
  return withFileLock(LEDGER_FILE, () => {
    const loaded = loadLedger();
    if (loaded.present && !loaded.verification.valid) throw new Error(`version transition ledger invalid:${loaded.verification.issues.join(",")}`);
    ...
    for (const raw of comparisons) {
      ...
      if (!/^[a-f0-9]{64}$/.test(comparison.transitionId) || !/^[a-f0-9]{64}$/.test(comparison.fromVersionKey) || !/^[a-f0-9]{64}$/.test(comparison.toVersionKey)) continue;
      const evidenceChecksum = digest(comparison);
      if (entries.some(entry => entry.transitionId === comparison.transitionId && entry.evidenceChecksum === evidenceChecksum)) continue;
      const core = {
    ...
    if (appendedCount > 0 || !loaded.present) writeJsonAtomic(LEDGER_FILE, next);
    return { ...readLiveProviderMemoryVersionTransitionLedger(), appendedCount };
  });
}

export function readLiveProviderMemoryVersionTransitionLedger() {
  const loaded = loadLedger();
  const ledger = loaded.ledger;
  const verification = loaded.present ? loaded.verification : verifyLiveProviderMemoryVersionTransitionLedger(ledger);
  const entries = Array.isArray(ledger.entries) ? ledger.entries : [];
  const latestByTransition = new Map<string, any>();
  for (const entry of entries) latestByTransition.set(String(entry.transitionId || ""), entry);
