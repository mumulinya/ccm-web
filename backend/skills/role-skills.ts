...
export function selectRoleSkills(role: CcmAgentRole, taskText = "", options: RoleSkillSelectionOptions = {}): SelectedRoleSkill[] {
  ...
  const roleRoots = new Set<RoleSkillName>([
    CCM_ROLE_SKILL_NAMES.global,
    CCM_ROLE_SKILL_NAMES.group,
    CCM_ROLE_SKILL_NAMES.project,
    CCM_ROLE_SKILL_NAMES.test,
  ]);
...
export function runRoleSkillSelectionSelfTest() {
  ...
  const projectWork = selectRoleSkills("project-child-agent", "model-selected", { phase: "execution", selectedSkillNames: [CCM_ROLE_SKILL_NAMES.documentDrivenDelivery, CCM_ROLE_SKILL_NAMES.frontendVisualQa] });
  const incidentWork = selectRoleSkills("project-child-agent", "model-selected", { phase: "execution", selectedSkillNames: [CCM_ROLE_SKILL_NAMES.incidentDiagnosis] });
  const releaseWork = selectRoleSkills("project-child-agent", "model-selected", { phase: "release", selectedSkillNames: [CCM_ROLE_SKILL_NAMES.releaseReadiness] });
  const testWork = selectRoleSkills("test-agent", "model-selected", { phase: "verification", selectedSkillNames: [CCM_ROLE_SKILL_NAMES.frontendVisualQa] });
  const businessPlanning = selectRoleSkills("group-main-agent", "model-selected", { source: "task", phase: "planning", selectedSkillNames: [CCM_ROLE_SKILL_NAMES.businessRuleModeling] });
  const contractWork = selectRoleSkills("project-child-agent", "model-selected", { phase: "execution", selectedSkillNames: [CCM_ROLE_SKILL_NAMES.interfaceDataContract, CCM_ROLE_SKILL_NAMES.businessScenarioAcceptance] });
  const businessReview = selectRoleSkills("group-main-agent", "model-selected", { forceWork: true, phase: "review", selectedSkillNames: [CCM_ROLE_SKILL_NAMES.businessScenarioAcceptance] });
  const businessTest = selectRoleSkills("test-agent", "model-selected", { phase: "verification", selectedSkillNames: [CCM_ROLE_SKILL_NAMES.businessScenarioAcceptance] });
  const visualOnly = selectRoleSkills("project-child-agent", "model-selected", { phase: "execution", selectedSkillNames: [CCM_ROLE_SKILL_NAMES.frontendVisualQa] });
  const directProjectBusinessWork = selectRoleSkills("project-child-agent", "model-selected", { source: "project-chat", phase: "execution", selectedSkillNames: [CCM_ROLE_SKILL_NAMES.businessRuleModeling, CCM_ROLE_SKILL_NAMES.interfaceDataContract, CCM_ROLE_SKILL_NAMES.businessScenarioAcceptance] });
  const checks = {
    ...
    planAuthoringOnlyLoadsPlanSkill: planAuthoringOnly.length === 1
      && planAuthoringOnly[0]?.name === CCM_ROLE_SKILL_NAMES.implementationPlanAuthoring
      && planAuthoringProjectOnly.length === 1
      && planAuthoringProjectOnly[0]?.name === CCM_ROLE_SKILL_NAMES.implementationPlanAuthoring,
    ...
    businessTestGetsScenarioAcceptance: businessTest.some(item => item.name === CCM_ROLE_SKILL_NAMES.businessScenarioAcceptance),
    visualOnlyAvoidsBusinessSkills: visualOnly.some(item => item.name === CCM_ROLE_SKILL_NAMES.frontendVisualQa)
      && !visualOnly.some(item => item.name === CCM_ROLE_SKILL_NAMES.businessRuleModeling)
      && !visualOnly.some(item => item.name === CCM_ROLE_SKILL_NAMES.interfaceDataContract)
      && !visualOnly.some(item => item.name === CCM_ROLE_SKILL_NAMES.businessScenarioAcceptance),
