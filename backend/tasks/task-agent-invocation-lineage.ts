...
import {
  verifyNativeModelCapabilityReceipt,
} from "../agents/runtime";
import {
...
export function sha256(value: any, length = 64) {
...
export function rawJsonSha256(value: any, length = 64) {
...
export function assertIdentity(groupId: string, groupSessionId: string, taskAgentSessionId: string) {
...
export function assertGroupSessionIdentity(groupId: string, groupSessionId: string) {
  if (!groupId || !groupSessionId.startsWith("gcs_")) {
    throw new Error("task-agent invocation recovery requires groupId--gcs_* identity");
  }
}

export function recordInvocationSoakPhase(edge: any, phase: string, status: string, evidence: any = {}, eventKey = "") {
...
export function eventChecksum(event: any) {
  const payload = { ...(event || {}) };
...
export function edgeChecksum(edge: any) {
  const payload = { ...(edge || {}) };
...
export function readEventsFromFile(file: string) {
...
export function appendEvent(file: string, edge: any, transition: string) {
...
export function latestCommittedEdge(filter: any = {}) {
...
export function transitionEdge(edgeOrId: any, status: string, evidence: any = {}) {
...
export function dispatchTaskAgentInvocationEdge(edgeOrId: any, evidence: any = {}) {
  ...
  const finalDispatchPayloadGateValidation = finalDispatchPayloadGateRequired
    ? verifyFinalWorkerDispatchPayloadGate(edge.final_dispatch_payload_gate, {
        groupId: edge.group_id,
        groupSessionId: edge.group_session_id,
        taskId: edge.task_id,
        taskAgentSessionId: edge.task_agent_session_id,
  ...
  if (finalDispatchPayloadGateRequired && (!finalDispatchPayloadGateValidation.valid || edge.final_dispatch_payload_gate?.provider_call_allowed !== true || edge.final_dispatch_payload_gate?.status !== "ready")) {
    const error: any = new Error(`task-agent final dispatch payload blocked: ${finalDispatchPayloadGateValidation.issues.join(",") || edge.final_dispatch_payload_gate?.status || "unknown"}`);
...
export function nativeContinuationReceiptChecksum(receipt: any) {
...
export function buildTaskAgentNativeContinuationReceipt(edge: any, evidence: any, success: boolean) {
...
export function contextRebudgetProofChecksum(proof: any) {
  const payload = { ...(proof || {}) };
...
export function buildTaskAgentContextRebudgetProof(edge: any, evidence: any) {
...
export function adoptionReceiptChecksum(receipt: any) {
  const payload = { ...(receipt || {}) };
  delete payload.receipt_checksum;
  delete payload.checksum_valid;
  return sha256(payload);
...
export function buildTaskAgentInvocationAdoptionReceipt(edge: any, evidence: any, success: boolean, nativeContinuationReceipt: any) {
...
export function verifyMemoryDeliveryReceiptChecksum(receipt: any) {
...
export function reinjectionProofChecksum(proof: any) {
  const payload = { ...(proof || {}) };
  delete payload.proof_checksum;
  delete payload.checksum_valid;
  return sha256(payload);
...
export function recoveryEventChecksum(event: any) {
  const payload = { ...(event || {}) };
  delete payload.event_checksum;
  delete payload.checksum_valid;
  return sha256(payload);
...
export function recoveryStatusChecksum(status: any) {
...
export function getInvocationRecoveryHistoryFile(groupId: string, groupSessionId: string) {
  return path.join(TASK_AGENT_INVOCATION_RECOVERY_DIR, `${clean(groupId)}--${clean(groupSessionId)}.jsonl`);
}

export function getInvocationRecoveryStatusFile(groupId: string, groupSessionId: string) {
