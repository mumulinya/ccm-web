...
export const TASK_PAUSE_STUCK_MS = 30_000;

function hash(value: any) {
  return crypto.createHash("sha256").update(JSON.stringify(value)).digest("hex");
}

function unsigned<T extends Record<string, any>>(value: T) {
  const clone = { ...value };
  delete clone.checksum;
  return clone;
}

function generationOf(task: any) {
  return Math.max(1, Number(task?.generation || task?.workflow_generation || 1));
}

function attemptOf(task: any) {
  return Math.max(0, Number(task?.execution_attempt || task?.project_main_execution?.attempt || task?.attempt || 0));
}

export function taskPausePlanChecksum(task: any) {
  ...
  return hash({
    ...
    workItems: (Array.isArray(task?.work_items) ? task.work_items : []).map((item: any) => ({
      id: item?.id || "",
      title: item?.title || "",
...
export function taskPauseWorkspaceChecksum(task: any) {
  return String(
    task?.workspace_snapshot_checksum
...
export function taskPauseCompletedWorkItemIds(task: any) {
  const checkpoint = task?.resume_checkpoint || task?.interruption_receipt?.resume_checkpoint || {};
  if (Array.isArray(checkpoint?.completedWorkItemIds)) return Array.from(new Set(checkpoint.completedWorkItemIds.map(String).filter(Boolean)));
  return (Array.isArray(task?.work_items) ? task.work_items : [])
    .filter((item: any) => ["completed", "accepted", "awaiting_review"].includes(String(item?.status || "")))
    .map((item: any) => String(item?.id || ""))
    .filter(Boolean);
}

export function isTaskPauseRequested(task: any) {
  return ["requested", "quiescing"].includes(String(task?.pause_control?.state || ""));
}

export function isTaskSafelyPaused(task: any) {
  return String(task?.pause_control?.state || "") === "paused";
}

export function isTaskPauseHeld(task: any) {
  return ["requested", "quiescing", "paused", "resuming", "blocked"].includes(String(task?.pause_control?.state || ""));
}

export function validateTaskPauseControl(control: any) {
  return !!control
...
export function createTaskPauseRequest(task: any, input: {
  pendingWriterCount?: number;
  phase?: string;
  workItemId?: string;
  suspendedSessionCount?: number;
  ...
  if (previous && ["requested", "quiescing", "paused"].includes(previous.state)) return previous;
  const raw: Omit<TaskPauseControlV1, "checksum"> = {
    schema: "ccm-task-pause-control-v1",
...
export function updateTaskPauseProgress(task: any, input: {
  state?: TaskPauseState;
  pendingWriterCount?: number;
  suspendedSessionCount?: number;
  workspaceChecksum?: string;
  ...
  const current = validateTaskPauseControl(task?.pause_control)
    ? task.pause_control as TaskPauseControlV1
...
export function createTaskResumeControl(task: any, resumedAt = new Date().toISOString()): TaskPauseControlV1 {
  if (!validateTaskPauseControl(task?.pause_control)) throw Object.assign(new Error("暂停记录无效，不能安全继续"), { code: "TASK_PAUSE_CONTROL_INVALID" });
  const current = task.pause_control as TaskPauseControlV1;
  const raw: Omit<TaskPauseControlV1, "checksum"> = {
    ...unsigned(current),
    state: "resuming",
...
export function validateTaskPauseResume(task: any, input: {
...
export function taskPauseStatusProjection(task: any, input: {
  activeWriterCount?: number;
  ...
  const stuck = ["requested", "quiescing"].includes(state) && elapsedMs >= TASK_PAUSE_STUCK_MS;
  return {
...
export function taskPauseBoundaryError(task: any, phase = "executing", workItemId = "") {
  const error: any = new Error("任务已到达安全暂停点");
...
export function assertTaskPauseBoundary(task: any, phase = "executing", workItemId = "") {
  if (isTaskPauseRequested(task)) throw taskPauseBoundaryError(task, phase, workItemId);
}

export function runTaskPauseControlSelfTest() {
