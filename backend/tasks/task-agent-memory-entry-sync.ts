...
export const TASK_AGENT_MEMORY_ENTRY_SYNC_SCHEMA = "ccm-task-agent-memory-entry-sync-v1";
export const TASK_AGENT_MEMORY_ENTRY_MANIFEST_SCHEMA = "ccm-task-agent-memory-entry-manifest-v1";
...
function memoryRoot(memory: any) {
  if (memory?.schema === "ccm-group-memory-context-v1") return memory;
  if (memory?.group_memory?.schema === "ccm-group-memory-context-v1") return memory.group_memory;
  if (memory?.groupMemory?.schema === "ccm-group-memory-context-v1") return memory.groupMemory;
  return memory && typeof memory === "object" ? memory : null;
}

function relPath(row: any) {
  return String(row?.rel_path || row?.relPath || row?.path || "").trim().replace(/\\/g, "/");
}

function typedRows(root: any) {
  const candidates = [
...
function omitDerivedMemoryFields(rootInput: any) {
  ...
  if (root.group_state && typeof root.group_state === "object") {
    ...
    if (!Object.keys(root.group_state).length) delete root.group_state;
  }
  return root;
}

function manifestChecksumPayload(manifest: any) {
  return {
...
export function stripTaskAgentMemoryEntrySync(memory: any) {
...
export function taskAgentMemorySemanticChecksum(memory: any) {
  return digest(stripTaskAgentMemoryEntrySync(memory));
}

export function buildTaskAgentMemoryEntryManifest(memoryInput: any) {
  ...
  const entryContents: Record<string, any> = {};
  if (root) {
    const core = omitDerivedMemoryFields(root);
    if (Object.keys(core).length) entryContents["group/core"] = core;
    for (const [path, row] of typedRows(root)) entryContents[`typed/${path}`] = row;
  }
  const globalMission = memory?.global_mission_memory || memory?.globalMissionMemory || null;
  if (globalMission) entryContents["global/mission"] = clone(globalMission);
  if (!Object.keys(entryContents).length) entryContents["memory/value"] = memory;
  ...
  const payload = {
    schema: TASK_AGENT_MEMORY_ENTRY_MANIFEST_SCHEMA,
    version: 1,
    group_id: groupId,
...
export function verifyTaskAgentMemoryEntryManifest(manifest: any, expected: any = {}) {
  const issues: string[] = [];
  if (manifest?.schema !== TASK_AGENT_MEMORY_ENTRY_MANIFEST_SCHEMA) issues.push("manifest_schema_invalid");
  if (Number(manifest?.version || 0) !== 1) issues.push("manifest_version_invalid");
  if (!manifest?.entries || typeof manifest.entries !== "object" || Array.isArray(manifest.entries)) issues.push("manifest_entries_invalid");
  if (String(manifest?.manifest_checksum || "") !== digest(manifestChecksumPayload(manifest))) issues.push("manifest_checksum_invalid");
  if (expected.groupId !== undefined && String(manifest?.group_id || "") !== String(expected.groupId || "")) issues.push("manifest_group_id_mismatch");
  if (expected.groupSessionId !== undefined && String(manifest?.group_session_id || "") !== String(expected.groupSessionId || "")) issues.push("manifest_group_session_id_mismatch");
  if (expected.sourceMemoryContextChecksum !== undefined && String(manifest?.source_memory_context_checksum || "") !== String(expected.sourceMemoryContextChecksum || "")) issues.push("manifest_source_checksum_mismatch");
  for (const [key, row] of Object.entries(manifest?.entries || {})) {
    if (!key || !/^sha256:[a-f0-9]{64}$/.test(String((row as any)?.checksum || ""))) issues.push("manifest_entry_checksum_invalid");
    if (Number((row as any)?.chars || 0) < 0 || Number((row as any)?.bytes || 0) < 0) issues.push("manifest_entry_size_invalid");
  }
  return { valid: issues.length === 0, issues: [...new Set(issues)] };
}

function renderEntry(key: string, content: any) {
...
export function buildTaskAgentMemoryEntrySyncPlan(input: {
  memory: any;
  groupId: string;
  ...
  const payload: any = {
    schema: TASK_AGENT_MEMORY_ENTRY_SYNC_SCHEMA,
    version: 1,
    group_id: input.groupId,
    group_session_id: input.groupSessionId,
...
export function verifyTaskAgentMemoryEntrySyncPlan(plan: any, expected: any = {}) {
  const issues: string[] = [];
  if (plan?.schema !== TASK_AGENT_MEMORY_ENTRY_SYNC_SCHEMA) issues.push("entry_sync_schema_invalid");
  if (Number(plan?.version || 0) !== 1) issues.push("entry_sync_version_invalid");
  if (!new Set(["full", "delta", "continuation"]).has(String(plan?.transport_mode || ""))) issues.push("entry_sync_transport_mode_invalid");
  const payload = { ...(plan || {}) };
  delete payload.plan_checksum;
  if (String(plan?.plan_checksum || "") !== digest(payload)) issues.push("entry_sync_plan_checksum_invalid");
  ...
  for (const [field, wanted, actual] of bindings) if (wanted !== undefined && String(wanted || "") !== String(actual || "")) issues.push(`entry_sync_${field}_mismatch`);
  ...
  if (mode === "delta" && (!transportText || String(plan?.transport_text_checksum || "") !== digest(transportText))) issues.push("entry_sync_delta_text_invalid");
  if (mode !== "delta" && transportText) issues.push("entry_sync_non_delta_text_present");
  if (mode === "continuation" && (Number(plan?.changed_entry_count || 0) !== 0 || Number(plan?.removed_entry_count || 0) !== 0)) issues.push("entry_sync_continuation_has_changes");
  if (mode !== "full" && plan?.previous_baseline_trusted !== true) issues.push("entry_sync_delta_without_trusted_baseline");
  if (!/^tamerl_[a-f0-9]{24}$/.test(String(plan?.render_lease_id || ""))) issues.push("entry_sync_render_lease_id_invalid");
  if (!Number.isInteger(Number(plan?.render_fencing_token || 0)) || Number(plan?.render_fencing_token || 0) <= 0) issues.push("entry_sync_render_fencing_token_invalid");
  if (!Number.isInteger(Number(plan?.render_lease_owner_pid || 0)) || Number(plan?.render_lease_owner_pid || 0) <= 0) issues.push("entry_sync_render_lease_owner_invalid");
  ...
  if (!Number.isFinite(acquiredMs) || !Number.isFinite(expiresMs) || expiresMs <= acquiredMs) issues.push("entry_sync_render_lease_window_invalid");
  return { valid: issues.length === 0, issues: [...new Set(issues)], mode };
}

export function attachTaskAgentMemoryEntrySyncPlan(memoryInput: any, plan: any) {
...
export function taskAgentMemoryEntrySyncPlan(memory: any) {
  const root = memoryRoot(memory);
...
export function taskAgentMemoryTransport(memory: any) {
  ...
  }) : { valid: false, issues: [] as string[], mode: "" };
  return {
    present: !!plan,
