...
export function createPetActivityRuntime(deps: any) {
  const {
    CCM_DIR,
  ...
  const petSpeechBuffers = new Map<string, string>();
  ...
  function broadcastPetSpeech(agent: string, payload: any = {}) {
    ...
    if (!isMusic && agentWorkflowSource && payload.petMilestone !== true) return;
