...
function buildDailyDevAgentDiagnostics() {
  ...
  if (totalReadyBacklogs > 0 && dailyDevExecutionReady) autopilotNextActions.push("可以批量派发待认领需求，主 Agent 会拆分给子 Agent 执行");
...
export function markDailyDevBacklogStatus(groupId: string, fileName: string, status: string, meta: any = {}) {
  ...
  if (status === "done") file.completed_at = now;
  if (status === "blocked" || status === "failed") file.blocked_at = now;
...
function listDailyDevBacklogs(groupId = "") {
  ...
  return groups
    ...
    .sort((a: any, b: any) => {
      const statusOrder: Record<string, number> = { ready: 0, blocked: 1, queued: 2, in_progress: 3, done: 4 };
