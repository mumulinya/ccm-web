...
const mutationGuard = read('frontend/src/utils/taskMutationGuard.js')
...
expect('stale group generation cannot overwrite the current card', messaging.includes('compareGroupMessageVersion(incoming, current)')
  && messaging.includes('if (version < 0) return true')
  && messaging.includes('shouldIgnoreStaleGroupMessage(msg, latestSameAnchor)'))
...
expect('group task operations submit revision generation and binding', groupActions.includes('resolveTaskMutationGuard') && mutationGuard.includes('expected_revision') && mutationGuard.includes('binding_checksum'))
expect('project task operations submit revision generation and binding', project.includes('resolveTaskMutationGuard') && mutationGuard.includes('expected_revision') && mutationGuard.includes('binding_checksum'))
