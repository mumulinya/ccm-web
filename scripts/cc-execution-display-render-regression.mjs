...
try {
  ...
  assert.equal(await desktop.locator('.running-task-message').getByText('项目构建通过', { exact:true }).isVisible(), true, '独立的已完成工具必须保留安全结果摘要')
  assert.equal(await desktop.locator('.running-task-message').getByText('正在检查运行日志', { exact:true }).isVisible(), true, '运行中工具必须原位显示')
