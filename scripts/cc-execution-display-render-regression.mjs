...
try {
  ...
  assert.ok((await desktop.locator('.task-message .cc-execution').evaluate(node => parseFloat(getComputedStyle(node).borderRadius))) <= 1, '完成态执行记录不得继续使用重卡片圆角')
  ...
  assert.equal(await desktop.getByText('Find definition', { exact:true }).count(), 0, '阶段未展开时不应直接铺开工具详情')
