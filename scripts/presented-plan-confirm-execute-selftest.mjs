...
assert.match(message, /The user confirmed the following plan card/)
assert.match(message, /authorized execution under this plan/)
assert.match(message, /ccm_dispatch/)
assert.match(message, /Do not rewrite the plan as frontend\/backend\/test workstreams/)
assert.match(message, /Never put TestAgent in targets/)
...
assert.match(composable, /conversationPlanModeSupported\(id\.scope\)/)
