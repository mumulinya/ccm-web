...
try {
  ...
  await visible(page.getByText('回执通过：已完成登录恢复修复，项目测试与浏览器验证均已完成。'),'work item receipt')
  const planTag = page.locator('.task-replay-plan-board .task-tag').first()
  await visible(planTag,'plan board task tag')
  if ((await planTag.textContent()).trim() !== '父任务') throw new Error('root plan tag should show 父任务, not duplicate the page title')
