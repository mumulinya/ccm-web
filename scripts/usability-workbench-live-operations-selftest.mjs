...
const checks = [
  ...
  ['项目Agent与源码运行分离', backend, 'agent_connection: { connected: agentConnected }'],
  ['源码运行摘要只读', backend, 'getProjectRuntimeSummaryReadOnly'],
  ['工作台V3首屏', backend, 'ccm-usability-workbench-snapshot-v3'],
  ['工作台游标分页', backend, '/api/usability/workbench/items'],
  ['结构化任务动作', backend, 'taskActionMatch'],
  ['任务revision门禁', backend, 'assertTaskRevision(task, payload.expected_revision)'],
  ...
  ['按用户隔离本地快照', live, 'ccm:usability-workbench:snapshot:v3:'],
  ['旧响应隔离', live, 'requestGeneration'],
  ['可取消请求', live, 'AbortController'],
  ...
  ['Agent连接独立入口', component, "api('/api/projects/agent-connection'"],
  ['源码运行独立入口', component, "api('/api/projects/runtime/action'"],
  ['任务动作统一入口', component, "/api/usability/tasks/${encodeURIComponent(task.id)}/action"],
  ['分页加载', component, "loadMore('active')"],
