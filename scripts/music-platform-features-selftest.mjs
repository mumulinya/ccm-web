...
const playbackDecision = read('backend/modules/music/playback-decision.ts')
...
assert.ok(selectionGate >= 0 && selectionRequest > selectionGate, 'AI auto-selection must return candidates before its model request')
assert.match(player, /aiRecommendationEnabled:\s*aiRecommendationEnabled\.value/, 'player must pass recommendation preference to the server decision')
assert.match(player, /aiAutoSelectEnabled:\s*aiAutoSelectEnabled\.value/, 'player must pass auto-selection preference to the server decision')
