...
const run = async () => {
  ...
  try {
    ...
    assert.equal(viewerRegistration.response.status, 201); assert.equal(viewerRegistration.data.user.role, 'user')
    const viewer = asSession(viewerRegistration)
    assert.equal((await request('/api/projects', {}, viewer)).response.status, 403)
    ...
    const promote = await request(`/api/auth/users/${viewerUser.id}/role`, json({ role: 'user' }), admin)
    ...
    assert.equal(operatorLogin.data.user.role, 'user')
    assert.equal((await request('/api/projects/create', json({ name: 'still-forbidden' }), operator)).response.status, 403)
    assert.equal((await request('/api/tasks/queue', json({ task_id: 'missing' }), operator)).response.status, 403)
    checks.push({ name: 'ordinary users require explicit feature and resource grants', pass: true })

    const featurePolicy = await request('/api/admin/feature-access', {}, admin)
    assert.equal(featurePolicy.response.status, 200)
    const grantWorkbench = await request(`/api/admin/feature-access/${viewerUser.id}`, json({ modules: ['workbench', 'resource_workspace'], revision: featurePolicy.data.revision }, 'PUT'), admin)
    assert.equal(grantWorkbench.response.status, 200)
    assert.equal((await request('/api/projects', {}, operator)).response.status, 200)
    const staleResourceGrant = await request(`/api/admin/resource-access/${viewerUser.id}`, json({ resourceType: 'project', resourceId: 'missing-project', level: 'use', revision: featurePolicy.data.revision }, 'PUT'), admin)
    assert.equal(staleResourceGrant.response.status, 409)
    checks.push({ name: 'feature grants take effect immediately and stale policy writes are rejected', pass: true })

    const lastAdmin = await request(`/api/auth/users/${first.data.user.id}/role`, json({ role: 'user' }), admin)
