...
try {
  ...
  await page.getByRole('button', { name: /管理账户/ }).click()
  await page.getByText('Viewer，不能执行任务', { exact: false }).waitFor()
  await page.locator('.security-modal').screenshot({ path: path.join(outputDir, 'account-dialog.png') })
  await page.locator('.security-modal').getByRole('button', { name: '关闭' }).first().click()
  await page.getByRole('button', { name: /修改密码/ }).click()
  await page.getByLabel('显示当前密码').waitFor()
  await page.locator('.security-modal').screenshot({ path: path.join(outputDir, 'password-dialog.png') })
  await page.locator('.security-modal').getByRole('button', { name: '关闭' }).first().click()
  await page.setViewportSize({ width: 390, height: 844 })
  await page.screenshot({ path: path.join(outputDir, 'mobile-security.png'), fullPage: true })
  assert.equal(await page.locator('.security-setting-row').count() >= 5, true)
  console.log(JSON.stringify({ pass: true, checks: { firstInstallForm: true, setupCodeRegistration: true, csrfAuthenticatedNavigation: true, compactSecuritySettings: true, accountDialog: true, passwordDialog: true, desktop: true, mobile: true }, screenshots: ['desktop-security.png', 'mobile-security.png', 'account-dialog.png', 'password-dialog.png'], paidProviderCalls: 0 }, null, 2))
} finally {
