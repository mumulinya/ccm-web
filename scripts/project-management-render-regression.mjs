...
try {
  ...
  assert.match(await desktop.locator('.message.assistant .agent-final-answer').innerText(), /正常。\n关键服务/)
  assert.match(await desktop.locator('.project-selector-trigger').innerText(), /CCM 演示项目/)
  assert.equal(await desktop.getByText('项目运行', { exact: true }).count(), 1)
  assert.equal(await desktop.getByText('连接飞书通道', { exact: true }).count(), 0)
  assert.equal(await desktop.getByText('断开飞书通道', { exact: true }).count(), 1)
  ...
  await desktop.getByRole('button', { name: '更多项目操作' }).click()
  await desktop.getByRole('button', { name: /工具与能力/ }).click()
  ...
  await desktop.locator('.runtime-bar').getByRole('button', { name: '更多运行操作' }).click()
  await desktop.locator('.runtime-bar').getByRole('button', { name: '运行配置', exact: true }).click()
  ...
  await desktop.locator('.runtime-bar').getByRole('button', { name: '更多运行操作' }).click()
  await desktop.locator('.runtime-bar').getByRole('button', { name: '运行日志', exact: true }).click()
  ...
  await desktop.locator('.project-manager .send-button').click()
  await desktop.getByText('已完成登录页修改，并通过测试。', { exact: true }).waitFor()
  assert.equal(await desktop.locator('.task-summary').count(), 0)
  assert.equal(await desktop.locator('.file-changes').count(), 0)
  assert.equal((await desktop.locator('.messages').innerText()).includes('修改登录页并运行测试'), true)
  report.checks.push({ name: 'explicit implementation request renders the final delivery without a duplicate task card', pass: true })
  await capture(desktop, 'desktop-explicit-task')

  await desktop.getByRole('button', { name: '更多项目操作' }).click()
  await desktop.getByRole('button', { name: '归档项目管理', exact: true }).click()
  ...
  const webSessionGroup = mobile.locator('.conversation-session-toggle').filter({ hasText: '普通会话' })
  ...
  report.checks.push({ name: 'mobile session drawer opens ordinary conversations by default and supports compact folding', pass: true })
