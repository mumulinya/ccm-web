...
assert.doesNotMatch(nativeLoop, /PRESENTED_PLAN_SHAPE_GUIDANCE/)
assert.doesNotMatch(nativeLoop, /PRESENTED_PLAN_DISPATCH_HANDOFF_GUIDANCE/)
assert.match(nativeLoop, /One demonstrable slice/)
assert.match(nativeLoop, /Never put TestAgent in targets/)
assert.match(groupLoop, /applyConversationPlanModeToRound/)
assert.match(groupLoop, /runGroupMainNativeQueryLoop/)
assert.match(groupLoop, /buildGroupMainIdentityRules/)
assert.match(groupLoop, /buildGroupMainSessionGuidance/)
assert.match(groupLoop, /renderSlashCommandSessionDirective\("group"/)
assert.match(identity, /During the first plan/)
assert.match(identity, /Do not reread files just to restate a plan/)
assert.match(identity, /confirmed plan slice/)
...
assert.doesNotMatch(prompts, /During the first plan/)
assert.match(projectMessages, /buildProjectMainSessionGuidance/)
assert.doesNotMatch(projectMessages, /PRESENTED_PLAN_SHAPE_GUIDANCE/)
assert.doesNotMatch(projectMessages, /PRESENTED_PLAN_DISPATCH_HANDOFF_GUIDANCE/)
assert.doesNotMatch(projectMessages, /During the first plan/)
assert.match(identity, /Expanding or restating a plan is not dispatch authorization/)
assert.match(projectLoop, /buildProjectMainIdentityRules/)
assert.match(projectLoop, /buildProjectMainSessionGuidance/)
assert.doesNotMatch(projectLoop, /PRESENTED_PLAN_SHAPE_GUIDANCE/)
assert.doesNotMatch(projectLoop, /PRESENTED_PLAN_DISPATCH_HANDOFF_GUIDANCE/)
...
assert.match(identity, /Planning is read-only/)
assert.match(identity, /complex, high-risk/)
assert.doesNotMatch(identity, /用户要计划、方案或步骤时调用/)
assert.match(slashState, /Explore read-only first, then call ccm_present_plan/)
assert.match(slashState, /Do not dispatch writes, edit code, or perform side effects/)
assert.match(slashState, /conversationPlanModeSupported\(scope\) && state\.planMode\?\.enabled === true/)
assert.match(slashState, /GLOBAL_CONVERSATION_PLAN_MODE_UNSUPPORTED/)
...
assert.match(confirmComposable, /canConfirmPlanScope\(identity\(\)\.scope\)/)
