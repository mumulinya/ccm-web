...
try {
  ...
  assert.equal(await desktop.locator('.nav-sidebar .nav-group-items').filter({ hasText: '代码变更' }).count() > 0, true)
  ...
  await desktop.reload({ waitUntil: 'commit', timeout: 30_000 })
  ...
  assert.equal(bottomLabels.includes('代码变更'), true)
